...
TL;DR: 提出MRD方法，通过可微分渲染寻找使模型激活相同的不同3D场景参数，来探究视觉模型对3D场景属性的隐式理解。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在视觉任务中表现出色，但难以理解模型的表示和决策过程。虽然视觉模型通常在2D输入上训练，但人们假设它们发展了对底层3D场景的隐式理解。需要一种方法来系统地探究模型对生成性3D场景属性的理解。

Method: 提出MRD（可微分渲染的元相似体）方法，使用基于物理的可微分渲染技术，寻找在物理上不同但产生相同模型激活的3D场景参数。通过保持某些场景属性恒定（如材质和光照），专门探究模型对特定属性（如物体形状）的敏感性。

Result: 在几何形状和双向反射分布函数（材质）恢复任务中，目标场景和优化场景之间显示出高度相似的模型激活，但视觉结果各不相同。这些重建结果有助于定性地研究模型对哪些物理场景属性敏感或不敏感。

Conclusion: MRD方法通过分析物理场景参数如何驱动模型响应变化，有望增进对计算机视觉和人类视觉的理解，为探究模型对3D场景的隐式表示提供了新工具。
...
TL;DR: GPF将光子映射重构为连续可重用的辐射函数，使用各向异性3D高斯基元编码光子分布，实现多视角渲染的显著加速


<details>
  <summary>Details</summary>
Motivation: 光子映射虽然能准确模拟复杂全局光照效果，但在多视角渲染中存在计算效率低下的问题，因为每个视角都需要独立的光子追踪和随机核估计，导致大量冗余计算

Method: 提出高斯光子场(GPF)，将光子分布编码为位置、旋转、尺度和光谱参数化的各向异性3D高斯基元。从第一次SPPM迭代的物理追踪光子初始化，通过多视角最终辐射监督进行优化，将基于光子的光传输蒸馏到连续场中

Result: 在包含复杂光传输（如焦散和镜面-漫反射交互）的场景上进行广泛实验，GPF在保持光子级精度的同时，将计算量减少了数个数量级

Conclusion: GPF将基于光子的渲染的物理严谨性与神经场景表示的高效性统一起来，实现了无需重复光子追踪或迭代优化的可微分辐射评估
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要使用vanilla Score Distillation Sampling（SDS）从文本到视频扩散模型中提取运动，导致动画运动幅度小或抖动明显，需要更好的运动生成和外观保持方法

Method: 提出Motion Score Distillation（MSD）替代SDS，使用LoRA增强的视频扩散模型定义静态源分布，结合反转噪声估计技术保持外观，加入时空正则化减少几何失真，并设计运动细化模块提升时间分辨率和细节

Result: 实验表明Animus3D能成功为多样文本提示的静态3D资产生成动画，相比现有基线方法产生更显著、更详细的运动，同时保持高视觉完整性

Conclusion: Animus3D通过创新的MSD方法、时空正则化和运动细化模块，有效解决了3D动画生成中的运动幅度不足和抖动问题，实现了高质量的文本驱动3D动画
...
<details>
  <summary>Details</summary>
Motivation: 智能农业中依赖RGB相机感知和机器人操纵器的系统容易受到色调、光照、噪声等光度扰动的对抗攻击，导致系统故障。需要提高系统在对抗条件下的鲁棒性和可解释性。

Method: 基于OpenVLA-OFT框架构建可解释对抗鲁棒视觉-语言-动作模型，集成Evidence-3模块检测光度扰动，并生成自然语言解释说明扰动的原因和影响。

Result: 与基线相比，当前动作L1损失减少21.7%，下一动作L1损失减少18.4%，表明在对抗条件下动作预测准确性和可解释性得到显著提升。

Conclusion: 提出的可解释对抗鲁棒模型有效提高了智能农业系统在光度扰动对抗攻击下的鲁棒性和可解释性，为智能农业系统的安全可靠运行提供了解决方案。
...
TL;DR: 本文提出Qonvolutions（查询卷积），通过利用卷积的邻域特性来增强高频信号学习，在多种高频学习任务中表现出色，特别是在结合高斯泼溅进行新视角合成时达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 神经网络在处理高频信号时存在频谱偏差和优化困难的问题，现有方法如傅里叶编码虽有改进但仍需提升。本文旨在解决高频信号学习的挑战。

Method: 提出Qonvolutions，通过将低频信号与查询（如坐标）进行卷积，利用卷积的邻域特性来增强复杂高频信号的学习能力。

Result: Qonvolutions在多种高频学习任务中提升性能，包括1D回归、2D超分辨率、2D图像回归和新视角合成。结合高斯泼溅在新视角合成任务中实现SOTA性能，甚至超越强大的辐射场模型。

Conclusion: Qonvolutions是一种简单而有效的方法，通过利用卷积的邻域特性显著提升了神经网络学习高频信号的能力，在计算机视觉和图形学的高频学习任务中具有广泛应用前景。
...
TL;DR: Temporal-Anchor3DLane通过改进损失函数和加入轻量级LSTM时序融合模块，显著提升了单目3D车道线检测的精度和时序稳定性


<details>
  <summary>Details</summary>
Motivation: 现有的Anchor3DLane方法在单目3D车道线检测中存在四个主要问题：对回归异常值敏感、全局曲线几何监督弱、多损失项平衡困难、时序连续性利用有限

Method: 提出了Temporal-Anchor3DLane框架，包含三个关键改进：1）多任务损失改进（Balanced L1回归、Chamfer点集距离、不确定性损失加权等）；2）轻量级时序LSTM融合模块；3）ESCOP式训练优化，结合曲线级监督与时序一致性

Result: 在OpenLane数据集上，F1分数提升了+6.2，同时产生了更平滑的时序轨迹，表明小的架构和损失改进能显著增强3D车道线鲁棒性，无需额外传感器或扩展

Conclusion: 通过针对性的损失函数改进和轻量级时序融合，Temporal-Anchor3DLane有效解决了Anchor3DLane的局限性，在保持计算效率的同时显著提升了3D车道线检测的性能和稳定性
...
TL;DR: 开发离线移动端作物病害检测系统，针对埃塞俄比亚提格雷地区仙人掌果病害，比较三种轻量级模型，实现本地化离线诊断


<details>
  <summary>Details</summary>
Motivation: 埃塞俄比亚提格雷地区80%人口依赖农业，但基础设施中断限制了专家病害诊断的获取。需要为冲突后边缘环境开发离线移动诊断系统

Method: 构建包含3,587张田间图像的本地仙人掌果数据集，在部署约束下比较三种移动高效架构：自定义轻量CNN、EfficientNet-Lite1和CNN-Transformer混合模型MobileViT-XS

Result: EfficientNet-Lite1达到90.7%测试准确率，轻量CNN达到89.5%且部署最优（42ms推理延迟，4.8MB模型大小），MobileViT-XS达到97.3%平均交叉验证准确率，显示基于多头自注意力的全局推理优于局部纹理CNN

Conclusion: MobileViT-XS在病害检测中表现最佳，模型部署在支持提格雷尼亚语和阿姆哈拉语的Flutter应用中，可在Cortex-A53设备上完全离线推理，增强了粮食安全关键诊断的包容性
...
TL;DR: 提出基于3D高斯泼溅的合成数据生成流水线，解决机器人视觉任务中合成-真实域差距和手动标注瓶颈问题，通过混合训练策略实现最优检测分割性能。


<details>
  <summary>Details</summary>
Motivation: 解决机器人视觉任务中两大关键挑战：1) 合成图像与真实世界图像之间的域差距问题；2) 手动标注耗时耗力的瓶颈问题，需要高效生成大规模、高质量、自动标注的数据集。

Method: 1) 使用3D高斯泼溅创建逼真的环境和物体表示；2) 在游戏引擎中进行物理模拟生成自然布局；3) 采用新颖的双通道渲染技术，结合泼溅真实感和代理网格生成的阴影图；4) 算法合成添加物理合理的阴影和高光；5) 自动生成像素级分割掩码并格式化为YOLO等检测模型可直接使用的格式。

Result: 实验表明，混合训练策略（少量真实图像+大量合成数据）在检测和分割任务上表现最佳，证实这是高效获得鲁棒准确模型的最优策略。

Conclusion: 该流水线能够生成大规模、高真实感、自动标注的计算机视觉数据集，有效解决域差距和标注瓶颈问题，混合训练策略为机器人视觉任务提供了高效可靠的解决方案。
...
TL;DR: 本文提出了一种用于小麦全语义分割竞赛的自训练框架，结合两阶段混合训练策略和大量数据增强，使用SegFormer-MiT-B4模型，通过迭代师生循环逐步提升精度，在开发和测试数据集上均取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 解决全球小麦全语义分割竞赛中的分割任务，需要开发一个能够充分利用有限标注数据并逐步提升模型精度的系统化方法。

Method: 1. 系统化自训练框架；2. 两阶段混合训练策略；3. 大量数据增强技术；4. 核心模型为SegFormer-MiT-B4；5. 迭代师生循环机制，逐步优化模型精度并最大化数据利用。

Result: 在开发和测试阶段数据集上都取得了竞争性的性能表现，表明该方法在小麦语义分割任务中具有良好效果。

Conclusion: 提出的自训练框架结合混合训练策略和迭代师生循环，能够有效提升小麦语义分割的精度，为农业图像分析提供了有效的解决方案。
...
TL;DR: 提出Interactive Intelligence新范式，通过Mio框架实现人格对齐表达、自适应交互和自我进化的数字人，建立新评测基准并取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前数字人主要停留在表面模仿阶段，缺乏真正的智能交互能力。需要超越简单模仿，实现具有人格一致性、自适应性和自我进化能力的智能交互数字人

Method: 提出Mio（Multimodal Interactive Omni-Avatar）端到端框架，包含五个专门模块：Thinker（思考者）、Talker（说话者）、Face Animator（面部动画）、Body Animator（身体动画）和Renderer（渲染器）。该统一架构将认知推理与实时多模态具身化相结合

Result: 大量实验表明，该框架在所有评估维度上都优于现有最先进方法。建立了新的基准来严格评估交互智能能力

Conclusion: 这些贡献推动数字人从表面模仿向智能交互发展，实现了人格对齐表达、自适应交互和自我进化的新范式
...
TL;DR: SAM3零样本分割与YOLO11微调模型在密集苹果实例分割任务上的对比：YOLO在检测完整性上表现更好，但SAM3在边界稳定性上优势显著（12倍更稳定）


<details>
  <summary>Details</summary>
Motivation: 比较两种实例分割范式：专门化微调模型（YOLO）与通用基础模型（SAM3）在密集场景下的性能差异，为实际应用提供选择依据

Method: 在MinneApple数据集（670张果园图像，28,179个苹果实例）上评估SAM3零样本模式与YOLO11三种变体（nano、medium、large）的微调模型，分析不同IoU阈值下的性能表现

Result: 在IoU=0.15时，YOLO模型F1分数为68.9%-72.2%，SAM3为59.8%；但YOLO在IoU范围内性能下降48-50点，SAM3仅下降4点，显示SAM3边界稳定性是YOLO的12倍

Conclusion: YOLO在检测完整性上更优，而SAM3在掩码精度和边界稳定性上表现卓越；IoU选择对性能评估影响显著（可达30%差异）；为密集实例分割任务提供了模型选择指导
...
TL;DR: DiffusionBrowser：一个模型无关的轻量级解码器框架，可在去噪过程的任意时间步或Transformer块生成交互式预览，实现实时视频生成预览和新的控制能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型存在精度低、速度慢、生成过程不透明的问题，用户在生成过程中长时间处于"黑盒"状态，无法了解和控制生成过程。

Method: 提出DiffusionBrowser框架，包含训练的解码器，可在去噪过程的任意时间步或Transformer块生成多模态预览（RGB和场景内在属性），支持随机性重注入和模态引导进行交互控制。

Result: 能够以超过4倍实时速度生成预览（4秒视频少于1秒），预览与最终视频具有一致的外观和运动特征，实现了新的交互控制能力，并揭示了去噪过程中的场景、物体等细节组合方式。

Conclusion: DiffusionBrowser解决了视频扩散模型的不透明性问题，提供了实时预览和交互控制能力，同时为理解扩散模型的内部工作机制提供了系统性的分析工具。
...
TL;DR: mmWeaver：基于隐式神经表示（INR）和超网络的环境感知毫米波信号生成框架，通过RGB-D图像和文本到姿态生成实现高效、真实的信号合成，达到49倍压缩比。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达应用（如活动识别和姿态估计）需要多样化的环境特定信号数据集，但毫米波信号复杂、稀疏且高维，物理模拟计算成本高，需要更高效的信号生成方法。

Method: 使用隐式神经表示（INR）将毫米波信号建模为连续函数，结合超网络根据环境上下文（从RGB-D图像提取）和人体运动特征（通过MotionGPT从文本生成姿态）动态生成INR参数，实现多分辨率I/Q信号合成。

Result: 达到复杂SSIM 0.88和PSNR 35 dB，优于现有方法；活动识别准确率提升7%，人体姿态估计误差降低15%；运行速度比基于模拟的方法快6-35倍，实现高达49倍的压缩。

Conclusion: mmWeaver通过结合INR、超网络和语义几何先验，实现了高效、真实的环境感知毫米波信号生成，为雷达应用提供了高质量的数据增强解决方案。
...
TL;DR: Hot Hém是一个GeoAI工作流，结合Google街景图像、语义分割和遥感技术，为胡志明市行人网络提供热暴露感知路由，通过XGBoost模型预测地表温度。


<details>
  <summary>Details</summary>
Motivation: 热带密集城市中行人热暴露是重要的健康风险，但标准路由算法通常忽略微观尺度的热变化，需要开发能够考虑热暴露的路由系统。

Method: 结合Google街景图像、语义图像分割和遥感技术，训练两个XGBoost模型预测地表温度，在选定的行政区域（phŏng）训练后，部署到所有OSMnx生成的行人网络节点上，实现热感知路由。

Result: 开发了Hot Hém工作流，能够估计胡志明市行人热暴露，为热感知路由提供基础，帮助识别城市走廊中温度异常高的区域。

Conclusion: Hot Hém为热带密集城市提供了实用的热暴露评估工具，能够帮助城市规划者和公众理解城市基础设施尺度的热分布差异，为改善行人热舒适度提供数据支持。
...
<details>
  <summary>Details</summary>
Motivation: 传统路边视频采集在密集混合交通中常因遮挡、视角有限和车辆不规则运动而失效，需要无人机俯拍视角来获取更全面的交通动态数据。

Method: 使用无人机在印度国家首都区的六个路段采集数据，通过Data from Sky平台提取车辆轨迹，包含时间戳、位置、速度、加速度和车辆分类信息（30帧/秒），并与人工计数、空间平均速度和探测轨迹进行验证。

Result: 创建了包含异质交通组成和密度水平的开放数据集，探索性分析揭示了车道保持偏好、速度分布和横向机动等关键行为模式。

Conclusion: 这些开放数据集为全球研究社区提供了独特资源，支持区域型交通条件下的仿真建模、安全评估和行为研究，有助于开发更准确的城市交通环境模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型容易受到排版攻击（误导性文本覆盖视觉理解），而现有评估和防御方法主要关注物体识别，鼓励忽略文本来实现鲁棒性。但真实场景需要同时处理物体和文本（如识别行人同时读取交通标志），因此需要新的评估框架来平衡文本阅读能力和抗攻击性。

Method: 提出Read-or-Ignore VQA（RIO-VQA）任务，形式化视觉问答中的选择性文本使用。创建RIO-Bench基准数据集，为每个真实图像提供相同场景的反事实（读取/忽略）变体，仅改变文本内容和问题类型。基于此基准开发数据驱动的自适应防御方法，学习选择性文本使用。

Result: 实验显示现有强大大视觉语言模型和防御方法无法平衡排版攻击鲁棒性和文本阅读能力。RIO-Bench启发的数据驱动防御方法能够超越先前非自适应的文本忽略防御。

Conclusion: 这项工作揭示了现有评估范围与真实需求之间的根本性错位，为构建可靠的大视觉语言模型提供了原则性路径。RIO-VQA任务和RIO-Bench基准为解决文本阅读与抗攻击的平衡问题提供了标准化评估框架。
...
TL;DR: CLARGA是一种通用的多模态融合架构，通过注意力加权图和图注意力网络实现任意数量/类型模态的融合，具有自适应性和高效性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法通常针对特定模态组合设计，缺乏通用性。需要一种能处理任意模态数量/类型、适应不同样本、高效且能处理缺失模态的通用融合框架。

Method: CLARGA为每个样本构建注意力加权图，使用多头图注意力网络在图节点间传递消息。通过可学习掩码处理缺失模态，采用监督任务损失和对比InfoNCE损失的混合目标训练。

Result: 在金融、人机交互、多媒体分类、情感计算等7个数据集上，CLARGA始终优于基线、SOTA模型和消融实验。同时展示了处理缺失输入和在特定任务上的优异表现。

Conclusion: CLARGA是一种通用、高效、自适应的多模态融合架构，可轻松集成到机器学习模型中，在多种任务上实现有效的跨模态表示学习。
...
TL;DR: 通过分析智能手机被动记录的自发微笑视频，研究发现微笑强度与幸福感和日常活动模式高度相关，为大规模研究情感行为提供了生态有效的方法。


<details>
  <summary>Details</summary>
Motivation: 主观幸福感测量传统上依赖自我报告方法，存在回忆偏差和参与者负担重的问题，缺乏对日常生活中幸福感表达的客观理解。

Method: 分析233名参与者一周内被动记录的405,448个视频片段，使用深度学习模型量化微笑强度，研究其昼夜和日常模式，并与国家调查数据和日常重建方法结果进行比较。

Result: 微笑强度的日常模式与国家幸福感调查数据高度相关(r=0.92)，昼夜节律与日常重建方法结果一致(r=0.80)。较高的平均微笑强度与更多体力活动(β=0.043)和更大光照暴露(β=0.038)显著相关，但与智能手机使用无关。

Conclusion: 被动智能手机传感可作为研究情感行为动态的强大生态有效方法，为在人口规模上理解情感行为开辟了新途径。
...
TL;DR: MPath是一个轻量级多模态框架，通过视觉前缀提示机制将WSI特征注入预训练的生物医学语言模型，实现病理报告自动生成，在RED 2025挑战赛中排名第4。
...
Method: MPath采用轻量级多模态框架，通过学习的视觉前缀提示机制，将基础模型提取的WSI特征（CONCH + Titan）注入冻结的预训练生物医学语言模型（BioBART），避免端到端视觉语言预训练。

Result: 在RED 2025 Grand Challenge数据集上开发评估，在Test Phase 2中排名第4（尽管提交机会有限），展示了基于提示的多模态条件作为可扩展和可解释的病理报告生成策略的潜力。

Conclusion: 基于提示的多模态条件是一种可扩展且可解释的病理报告生成策略，MPath框架通过轻量级设计有效结合WSI视觉特征和生物医学语言模型，为计算病理学提供了新方向。
...
TL;DR: FloraForge是一个LLM辅助框架，让领域专家通过自然语言交互生成生物精确、完全参数化的3D植物模型，无需编程专业知识。


<details>
  <summary>Details</summary>
Motivation: 当前3D植物建模方法存在局限：学习重建方法需要大量物种特定训练数据且缺乏可编辑性；程序化建模需要几何建模专业知识和复杂规则理解，对领域科学家不友好。

Method: 利用LLM辅助协同设计，通过迭代自然语言植物精炼(PR)来优化Python脚本，生成参数化植物几何作为分层B样条曲面表示，具有植物学约束、显式控制点和参数化变形函数。

Result: 在玉米、大豆和绿豆上演示了框架，通过手动精炼植物描述符(PD)文件将程序化模型拟合到经验点云数据，生成用于可视化的三角网格和用于定量分析的带参数元数据的三角网格。

Conclusion: 该框架独特地结合了LLM辅助模板创建、支持表型分析和渲染的数学连续表示，以及通过PD的直接参数控制，为植物科学民主化了复杂几何建模同时保持数学严谨性。
...
TL;DR: TransBridge：一种基于transformer的联合补全与检测框架，通过融合检测和补全网络特征来提升稀疏区域3D目标检测性能，同时保持计算成本不变。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，远距离区域仅有少量LiDAR点云，点云稀疏性导致3D目标检测困难。现有方法通过点云稠密化解决稀疏问题，但需要平衡检测性能和计算成本。

Method: 1. 提出TransBridge：基于transformer的上采样模块，融合检测和补全网络特征；2. 设计DSRecon模块生成稠密LiDAR数据用于补全网络训练；3. 利用transformer机制建立通道和空间关系，生成高分辨率特征图用于补全。

Result: 在nuScenes和Waymo数据集上验证，框架能持续改进端到端3D目标检测，mAP提升0.7-1.5点，展示良好泛化能力。对于两阶段检测框架，mAP最高提升5.78点。

Conclusion: TransBridge框架通过联合补全和检测，有效提升稀疏区域3D目标检测性能，同时保持计算成本不变，在多个数据集和检测方法上表现出良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统细胞染色技术存在两个主要问题：1) 劳动密集型，过程繁琐；2) 需要化学固定，无法研究细胞动态变化。需要一种能够从常规明场图像生成高质量细胞染色图像的方法。

Method: 训练扩散模型（MONET）在大规模数据集上，从明场图像预测细胞染色通道。采用一致性架构生成时间序列视频，并实现上下文学习能力。

Result: 模型质量随规模扩大而提升；一致性架构能生成时间序列视频（尽管没有视频训练数据）；支持上下文学习，能部分适应分布外的细胞系和成像协议。
...
TL;DR: 提出新的HEMC-CPS模型，结合上下文Peano扫描和证据隐马尔可夫链，用于无监督图像分割，比传统方法更快且效果更好。


<details>
  <summary>Details</summary>
Motivation: 传统隐马尔可夫场（HMFs）图像分割方法计算量大，而基于Peano扫描的隐马尔可夫链（HMCs）方法更快但需要改进。上下文Peano扫描（CPS）和证据隐马尔可夫链（HEMCs）各自有优势，研究旨在结合两者优势开发更有效的分割模型。

Method: 提出HEMC-CPS模型，同时考虑上下文Peano扫描和证据隐马尔可夫链。采用贝叶斯最大后验概率（MPM）分割，使用随机期望最大化（SEM）方法进行无监督参数估计。

Result: 在合成和真实图像上验证了HEMC-CPS模型的有效性。新模型不仅适用于图像分割，还能处理更复杂的图像（如3D或多传感器多分辨率图像）和任何空间相关数据。

Conclusion: HEMC-CPS模型结合了上下文Peano扫描和证据隐马尔可夫链的优势，为图像分割提供了更有效的解决方案，具有扩展到复杂图像类型和空间数据分析的潜力。
...
TL;DR: DynaPURLS提出一个统一的零样本骨架动作识别框架，通过建立多尺度视觉-语义对应关系并在推理时动态优化，显著提升了未见类别的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法主要依赖于骨架特征与静态类别级语义的对齐，这种粗粒度对齐无法有效桥接可见类别和未见类别之间的领域偏移，限制了细粒度视觉知识的迁移。

Method: 1) 使用大语言模型生成包含全局运动和局部身体部位动态的分层文本描述；2) 自适应分区模块通过语义分组骨架关节生成细粒度视觉表示；3) 动态优化模块在推理时通过轻量级可学习投影将文本特征适配到输入视觉流；4) 置信感知的类别平衡记忆库稳定优化过程，减少噪声伪标签的误差传播。

Result: 在NTU RGB+D 60/120和PKU-MMD三个大规模基准数据集上的广泛实验表明，DynaPURLS显著优于现有方法，创造了新的最先进记录。

Conclusion: DynaPURLS通过建立鲁棒的多尺度视觉-语义对应关系并在推理时动态优化，有效解决了零样本骨架动作识别中的领域偏移问题，为细粒度知识迁移提供了有效框架。
...
TL;DR: 本研究探索使用DeiT（数据高效图像变换器）在数据受限条件下进行晶圆缺陷分类，相比传统CNN模型（VGG-19、Xception、SqueezeNet等）获得更高准确率（90.83%）和F1分数（90.78%），训练收敛更快，对少数类缺陷检测更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 半导体晶圆缺陷检测对预测性维护至关重要，但传统CNN模型（如VGG-19、Xception、SqueezeNet）在数据有限且不平衡的场景下性能下降。需要探索在数据受限条件下仍能保持高性能的替代方法。

Method: 采用数据高效图像变换器（DeiT）模型进行晶圆缺陷分类，在数据受限条件下与多种CNN模型（VGG-19、SqueezeNet、Xception和混合模型）进行对比实验。

Result: DeiT模型获得90.83%的最高分类准确率，显著优于CNN模型（VGG-19:65%、SqueezeNet:82%、Xception:66%、混合模型:67%）。DeiT还表现出90.78%的F1分数、更快的训练收敛速度，以及对少数缺陷类更强的检测鲁棒性。

Conclusion: 基于变换器的DeiT模型在半导体晶圆缺陷检测中表现出优越性能，特别是在数据受限条件下，为半导体制造过程中的预测性维护策略提供了有效支持。
...
TL;DR: CARI4D：首个从单目RGB视频中重建4D人机交互的类别无关方法，通过整合基础模型预测、渲染比较优化和物理约束推理，实现空间、时间和像素对齐的度量尺度重建。


<details>
  <summary>Details</summary>
Motivation: 从单目RGB视频准确捕捉人机交互对理解人类行为、游戏和机器人学习很重要，但由于物体和人体信息未知、深度模糊、遮挡和复杂运动，现有方法通常需要物体模板真值或局限于特定物体类别，限制了实际应用。

Method: 提出姿态假设选择算法整合基础模型预测，通过学习的渲染比较范式联合优化确保空间、时间和像素对齐，最后推理复杂接触点进行物理约束下的进一步细化。

Result: 在分布内数据集上重建误差比先前方法降低38%，在未见数据集上降低36%，能够泛化到训练类别之外，可零样本应用于真实网络视频。

Conclusion: CARI4D是首个从单目RGB视频实现类别无关4D人机交互重建的方法，通过整合基础模型和物理约束推理，在泛化性和准确性方面显著优于现有技术。
...
TL;DR: V-REX是一个评估视觉语言模型多步探索推理能力的评测套件，包含挑战性视觉推理任务和评估协议，将多步探索转化为问题链，分别评估规划能力和跟随能力。
...
Method: 开发V-REX评测套件，将多步探索推理转化为问题链(CoQ)，通过有限选项的问题和答案实现可靠量化评估，分别评估模型的规划能力(分解任务选择问题链)和跟随能力(顺序回答问题收集信息)。

Result: 评估了最先进的专有和开源VLM，揭示了模型在多步探索推理方面的一致扩展趋势、规划与跟随能力的显著差异，以及仍有大幅改进空间。

Conclusion: V-REX填补了视觉语言模型多步探索推理评估的空白，提供了细粒度分析工具，有助于推动模型在复杂视觉推理任务上的发展。
...
TL;DR: Semantic-Drive：一种本地优先的神经符号框架，用于从自动驾驶视频日志中挖掘语义数据，通过解耦感知和推理VLM实现高效、隐私保护的长尾事件检测。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶发展受限于长尾训练数据的稀缺性。现有解决方案要么依赖粗粒度元数据搜索（精度不足），要么使用云基础视觉语言模型（侵犯隐私且昂贵），无法高效识别罕见的安全关键事件。

Method: 提出两阶段解耦感知框架：1）符号接地：使用实时开放词汇检测器（YOLOE）锚定注意力；2）认知分析：通过推理VLM进行法证场景分析。采用"System 2"推理时对齐策略，使用多模型"Judge-Scout"共识机制缓解幻觉问题。

Result: 在nuScenes数据集上，基于Waymo Open Dataset分类法，Semantic-Drive达到0.966的召回率（CLIP为0.475），风险评估误差比单模型降低40%。系统完全在消费级硬件（NVIDIA RTX 3090）上运行。

Conclusion: Semantic-Drive提供了一种隐私保护的本地替代方案，能够高效挖掘自动驾驶视频日志中的长尾安全关键事件，解决了现有方法的精度、隐私和成本问题。
...
TL;DR: 提出基于离散动态图神经网络的毫米波雷达点云人体活动识别方法，通过星形图表示解决点云稀疏性和变尺寸问题，在资源受限平台上实现高效识别。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达点云用于人体活动识别面临稀疏性和变尺寸问题，现有基于视觉系统的预处理算法不适用于雷达系统，需要专门设计适合毫米波雷达特性的特征提取方法。

Method: 提出星形图表示法，在动态毫米波雷达点云中添加静态中心点，构建高维相对关系图；采用离散动态图神经网络学习变尺寸星形图中的时空特征。

Result: 在真实世界HAR数据集上达到94.27%的分类准确率，接近基于骨架视觉数据的97.25%最优性能；在树莓派4上验证了资源受限平台的有效性，优于三种雷达专用方法且无需重采样或帧聚合器。

Conclusion: 所提出的星形图表示和离散动态图神经网络能有效解决毫米波雷达点云的稀疏性和变尺寸问题，在人体活动识别任务中表现出色，适用于资源受限平台。
...
TL;DR: 论文研究联邦学习在卫星船舶检测中的应用，通过比较多种FL算法与本地训练基线，证明FL能显著提升检测精度并接近全局训练性能，同时保持数据隐私。


<details>
  <summary>Details</summary>
Motivation: 卫星船舶检测涉及商业卫星图像和敏感船舶标注数据，传统方法需要数据共享或集中收集，存在隐私和安全问题。联邦学习提供了一种隐私保护的解决方案，无需数据共享即可训练检测模型。

Method: 使用YOLOv8船舶检测模型，评估四种联邦学习算法（FedAvg、FedProx、FedOpt、FedMedian），并与本地训练基线（每个数据集独立训练）进行比较。研究还探讨了FL配置参数如通信轮数和本地训练周期对性能的影响。

Result: 联邦学习模型显著优于小规模本地数据集训练，检测精度接近使用所有数据集的全局训练水平。同时发现选择合适的FL配置（通信轮数、本地训练周期）对优化检测精度和计算效率至关重要。

Conclusion: 联邦学习为卫星船舶检测提供了有效的隐私保护解决方案，能够在不共享数据的情况下实现接近全局训练的性能。适当的FL配置选择对于平衡检测精度和计算效率非常重要。
...
TL;DR: 本研究提出一种监督语义分割工作流，旨在提升火灾后烧毁区域（BA）划定的性能和效率，针对SPOT-6/7高分辨率影像，评估了U-Net和SegFormer模型，并探讨了土地覆盖数据作为辅助任务以及测试时增强的效果。


<details>
  <summary>Details</summary>
Motivation: 当前基于计算机视觉的烧毁区域（BA）映射方法主要依赖灾后遥感影像训练，但往往忽视了其在时间紧迫的应急管理场景中的适用性。需要开发既高效又准确的BA划定方法，以支持灾害评估和生态系统恢复。

Method: 提出监督语义分割工作流，针对SPOT-6/7高分辨率影像。评估了U-Net和SegFormer模型，使用Dice分数、IoU和推理时间作为评价指标。探索了将土地覆盖数据作为辅助任务来增强模型鲁棒性，并测试了测试时增强（TTA）方法。

Result: U-Net和SegFormer在有限训练数据下表现相似，但SegFormer需要更多计算资源，不利于应急使用。加入土地覆盖辅助任务能提升模型鲁棒性且不增加推理时间。测试时增强能改善BA划定性能但会增加推理时间，可通过混合精度等优化方法缓解。

Conclusion: 该研究为应急管理场景下的烧毁区域划定提供了实用的解决方案，U-Net结合土地覆盖辅助任务和优化后的测试时增强方法，在保持高效推理的同时提升了划定准确性，更适合时间紧迫的灾害响应需求。
...
TL;DR: CreativeVR是一个针对AI生成视频和真实视频中严重结构/时间伪影的修复框架，通过单一精度旋钮控制输入跟随强度，在标准降质修复和结构/运动校正之间平滑权衡。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型在精细结构上表现脆弱，常产生扭曲的面部、手部、背景和时间不一致的运动。传统视频修复方法针对合成降质设计，倾向于稳定而非修复这些伪影，而扩散先验修复器通常针对光度噪声训练，在感知质量和保真度之间缺乏控制。

Method: 提出基于深度适配器的扩散先验引导视频修复框架，包含时间一致的降质模块，在训练期间应用精心设计的变换来模拟真实的结构失效。通过单一精度旋钮控制模型对输入的跟随强度。

Result: 在严重伪影视频上达到最先进结果，在标准视频修复基准上具有竞争力，在单个80GB A100上以720p分辨率实现约13FPS的实用吞吐量。提出了AIGC54基准用于评估AI生成内容伪影修复。

Conclusion: CreativeVR能够有效修复AI生成视频和真实视频中的严重结构/时间伪影，在感知质量和保真度之间提供可控权衡，为视频修复领域提供了实用的解决方案。
...
TL;DR: BAgger是一种自监督训练方案，通过从模型自身生成的轨迹中构建纠正路径，解决自回归视频模型中的曝光偏差问题，提升长期生成稳定性。


<details>
  <summary>Details</summary>
Motivation: 自回归视频模型在通过下一帧预测进行世界建模时存在曝光偏差问题：训练时使用干净的上下文，而推理时使用自生成的帧，导致误差累积和质量随时间漂移。

Method: 提出Backwards Aggregation (BAgger)自监督方案，从模型自身的生成轨迹中构建纠正路径，教导模型从错误中恢复。使用标准的分数匹配或流匹配目标进行训练，避免使用大型教师模型和长时间链的反向传播。

Result: 在因果扩散变换器上实现BAgger，在文本到视频、视频扩展和多提示生成任务中评估，观察到更稳定的长期运动、更好的视觉一致性和减少的漂移现象。

Conclusion: BAgger通过自监督的纠正轨迹训练，有效解决了自回归视频模型的曝光偏差问题，提升了长期生成的稳定性和质量，避免了传统方法对质量和多样性的负面影响。
...
TL;DR: RePack通过将高维视觉基础模型特征压缩到低维流形，解决了信息过载问题，加速了扩散变换器的收敛并提升了图像重建性能。
...
Result: 在DiT-XL/2上，RePack仅用64轮训练就达到FID 3.66，比现有方法快35%收敛，显著优于直接注入原始VFM特征的方法。

Conclusion: RePack成功提取了VFM表示的核心语义，同时避免了高维度带来的副作用，为高效利用预训练视觉基础模型提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)虽然能联合处理视觉和文本输入，但经常产生与视觉证据事实不一致的幻觉输出。现有方法未能有效解决在解码过程中何种形式的视觉注意力能有效抑制幻觉的问题。

Method: 提出VEGAS方法：1) 发现视觉编码器自身的注意力图比最终视觉注意力图更集中；2) 分析发现视觉-文本冲突在语言模型中间层达到峰值；3) 在推理时将视觉编码器的注意力图注入语言模型的中间层，并自适应地引导未能集中关注关键图像对象的token。

Result: 在多个基准测试上的广泛实验表明，VEGAS在减少幻觉方面始终达到最先进的性能。

Conclusion: 视觉编码器的注意力图能有效抑制LVLM的幻觉，通过在语言模型中间层注入这些注意力图，VEGAS提供了一种简单而有效的推理时方法来减少幻觉。
...
TL;DR: SPDMark是一种基于选择性参数位移的视频生成水印框架，通过修改生成模型参数子集嵌入水印，使用LoRA实现参数高效的基础位移，能够生成不可察觉的水印并抵抗各种视频修改。


<details>
  <summary>Details</summary>
Motivation: 高质量视频生成模型的出现增加了对鲁棒水印方案的需求，现有方法无法同时实现不可察觉性、鲁棒性和计算效率。

Method: 基于选择性参数位移的视频扩散模型水印框架，使用LoRA实现层间基础位移，通过联合训练基础位移和水印提取器，结合消息恢复、感知相似性和时间一致性损失，使用密码哈希函数生成帧特定水印消息。

Result: 在文本到视频和图像到视频生成模型上的评估显示，SPDMark能够生成不可察觉的水印，并以高准确率恢复，同时对各种常见视频修改具有鲁棒性。

Conclusion: SPDMark为视频生成提供了一种有效的水印方案，能够同时实现不可察觉性、鲁棒性和计算效率，解决了现有方法的局限性。
...
TL;DR: 该研究探索了全自动花粉识别，比较了传统光学显微镜和数字在线全息显微镜(DIHM)图像的性能。在DIHM图像上识别花粉面临挑战，通过使用WGAN-SN生成合成DIHM图像并进行数据增强，将检测性能从8.15%提升到15.4%。


<details>
  <summary>Details</summary>
Motivation: 全自动花粉识别在兽医成像等领域有重要应用价值。DIHM图像由于散斑噪声、孪生像伪影等问题，识别花粉极具挑战性。研究旨在探索如何提升DIHM图像中的花粉识别性能，缩小与光学显微镜的性能差距。

Method: 使用YOLOv8s进行目标检测，MobileNetV3L进行分类。在双模态数据集（光学和DIHM图像）上训练。为改善DIHM图像检测，采用Wasserstein GAN with spectral normalization (WGAN-SN)生成合成DIHM图像，并将真实与合成数据以1.0:1.5比例混合进行数据增强。

Result: 光学数据上检测mAP50达91.3%，分类准确率97%；DIHM数据上检测mAP50仅8.15%，分类准确率50%。通过扩大边界框，DIHM检测mAP50提升至13.3%，分类准确率54%。使用GAN合成数据增强后，DIHM检测mAP50进一步提升至15.4%。

Conclusion: GAN-based数据增强能够缩小光学和DIHM图像在花粉识别性能上的差距，为全自动DIHM工作流程在兽医成像中的应用迈出了重要一步。尽管性能提升有限，但证明了合成数据增强在解决DIHM图像识别挑战中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图是心脏病学中最广泛使用的成像方式，但其解读仍然劳动密集且本质上是多模态的，需要视图识别、定量测量、定性评估和基于指南的推理。现有的视觉语言模型在超声心动图领域潜力有限，缺乏大规模、临床基础的数据集和基于测量的推理能力。

Method: 1. 构建EchoGround-MIMIC数据集：包含19,065个图像-文本对，来自1,572名患者，具有标准化视图、结构化测量、基于测量的描述和指南衍生的疾病标签。2. 提出EchoVLM模型：采用两种新颖的预训练目标：(i)视图感知对比损失，编码超声心动图成像的视图依赖结构；(ii)否定感知对比损失，区分临床关键的阴性发现与阳性发现。

Result: 在涵盖多模态疾病分类、图像-文本检索、视图分类、腔室分割和标志点检测的36个任务中，EchoVLM实现了最先进的性能：零样本疾病分类AUC达86.5%，视图分类准确率达95.1%。临床基础的多模态预训练产生了可迁移的视觉表示。

Conclusion: EchoVLM被确立为端到端超声心动图解读的基础模型。将发布EchoGround-MIMIC数据集和数据整理代码，促进多模态超声心动图解读的可重复性和进一步研究。
...
TL;DR: 提出基于patch的持久同调构建方法用于3D CT图像，相比传统立方体复形方法在分类性能和计算效率上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统基于3D立方体复形过滤的持久同调方法在CT图像分析中可能性能不佳且计算复杂度高，特别是在高分辨率图像中。需要一种更高效、性能更好的拓扑特征提取方法。

Method: 提出一种新颖的基于patch的持久同调构建方法，专门针对体积医学成像数据（特别是CT模态）。该方法将3D CT图像分割成patch，然后在这些patch上构建持久同调特征。

Result: 在多个3D CT数据集上的实验表明，patch-based TDA方法在分类性能和计算效率上均优于传统立方体复形方法。平均提升：准确率10.38%，AUC 6.94%，灵敏度2.06%，特异性11.58%，F1分数8.51%。

Conclusion: 基于patch的持久同调方法为CT图像分析提供了更高效和有效的拓扑特征提取方案，并发布了方便的Python包Patch-TDA来促进该方法的应用。
...
TL;DR: 该论文提出了最大的道路损坏评估和道路对齐基准数据集CRASAR-U-DRIODs，包含10次联邦宣布灾害后的无人机图像，标注了657.25公里道路，并提供18个基线模型，解决了现有数据集规模小、分辨率低、缺乏操作验证的问题。


<details>
  <summary>Details</summary>
Motivation: 现有灾害道路损坏评估数据集存在三个主要问题：1) 规模小或依赖低分辨率图像，无法检测应急管理者感兴趣的现象；2) 机器学习系统缺乏操作验证；3) 实际应用中道路线错位导致模型性能下降。这些限制阻碍了灾害期间有效的决策制定。

Method: 1) 创建CRASAR-U-DRIODs数据集，包含10次灾害后的无人机图像，标注657.25公里道路，采用10类标注方案；2) 提供9,184个道路线调整用于空间对齐；3) 训练18个基线模型；4) 在2024年飓风Debby和Helene的应急响应中部署验证模型。

Result: 1) 当模型部署到实际错位的道路线时，平均性能下降5.596% Macro IoU；2) 如果不考虑空间对齐，约8%(11km)的道路不良条件会被错误标注；3) 约9%(59km)的道路线与实际道路错位；4) 提供了首个操作验证的灾害道路评估系统。

Conclusion: 道路空间对齐是灾害道路损坏评估的关键挑战，ML、CV和机器人社区需要解决这一差距，以实现灾害期间更有效和知情的决策制定。该数据集和基线模型为未来研究提供了重要基础。
...
TL;DR: 开发深度学习模型融合多源遥感数据，实现格陵兰冰盖表面融水每日100米分辨率制图，精度显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 格陵兰冰盖加速融化过程尚未完全理解且难以测量，现有融水地图在时间和空间分辨率上存在权衡，无法同时实现高时空分辨率

Method: 开发深度学习模型融合区域气候模型输出、合成孔径雷达、被动微波和数字高程模型数据，进行时空降尺度处理，使用SAR数据作为"地面真值"

Result: 深度学习融合方法在Helheim冰川区域精度达95%，显著优于仅依赖区域气候模型的方法（83%）和被动微波观测（72%）；SAR滑动窗口方法精度为90%但低估极端融化事件

Conclusion: 深度学习融合多源数据可显著提高融水制图精度，发布MeltwaterBench基准数据集促进数据驱动降尺度方法比较，为理解冰盖融化过程提供高分辨率工具
...
TL;DR: 该论文对开放世界识别进行了统一实验研究，在CIFAR-10上比较了开放集识别(OSR)和少样本类增量学习(FSCIL)方法。发现CLIP在未知样本检测上表现最佳，而ConCM在增量学习中效果最好。


<details>
  <summary>Details</summary>
Motivation: 开放世界部署需要模型既能识别已知类别，又能在新类别出现时保持可靠。目前缺乏对开放集识别和少样本类增量学习的统一实验研究，需要系统比较不同架构和方法在这些任务上的表现。

Method: 在CIFAR-10上进行统一实验：1) OSR部分：比较ResNet-50、ConvNeXt-Tiny和CLIP ViT-B/16三种预训练编码器，使用线性探测和四种后验评分函数(MSP、Energy、Mahalanobis、kNN)；2) FSCIL部分：比较SPPR、OrCo和ConCM三种方法，使用部分冻结的ResNet-50，在1、5、10-shot场景下测试。

Result: OSR中：CLIP在所有指标(AUROC、AUPR、FPR@95、OSCR)上对已知/未知样本的区分能力最强，Energy评分函数在不同骨干网络上表现最稳定。FSCIL中：ConCM在10-shot设置下达到84.7%准确率，混淆矩阵最清晰，所有方法在超过5-shot后性能趋于饱和。

Conclusion: 骨干网络架构和评分机制显著影响未知样本检测能力，基于原型的方法能有效缓解增量适应中的灾难性遗忘。CLIP在开放集识别中表现卓越，而ConCM在少样本类增量学习中效果最佳，为开放世界部署提供了重要指导。
...
TL;DR: 本文提出首个利用音频辅助相对相机姿态估计的方法，通过整合声源方向谱和双耳嵌入到视觉模型中，在真实世界视频中提升姿态估计性能
...
Method: 提出简单有效的音频-视觉框架，将声源方向(DOA)谱和双耳化嵌入集成到最先进的纯视觉姿态估计模型中

Result: 在两个大型数据集上相比强视觉基线获得一致性能提升，且在视觉信息受损时保持鲁棒性

Conclusion: 这是首个成功利用音频进行真实世界视频相对相机姿态估计的工作，确立了日常音频作为经典空间挑战问题的意外但有效的信号
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在定制化视频生成中难以同时保证主体外观相似性和运动模式一致性，主要原因是缺乏对象级的主体和运动指导。

Method: 1. 使用自监督编码器提取主体表示指导主体对齐；2. 使用光流编码器提取运动表示捕获对象级运动轨迹；3. 提出主体-运动关联解耦策略，在位置和时间上稀疏注入LoRAs，减少相互干扰。

Result: 大量实验表明SMRABooth在主体和运动定制方面表现出色，能够保持一致的主体外观和运动模式，证明了其在可控文本到视频生成中的有效性。

Conclusion: SMRABooth通过对象级主体和运动表示以及解耦策略，成功解决了定制化视频生成中主体外观相似性和运动模式一致性的平衡问题。
...
TL;DR: 提出一种轻量级边界跟踪方法，用于带宽受限环境下微型无人机团队对野火区域的监测，通过热成像和RGB图像融合处理，实现低延迟、稳定的边界跟踪。


<details>
  <summary>Details</summary>
Motivation: 在野火监测等紧急侦察应用中，微型无人机团队需要在带宽受限条件下进行快速部署，现有方法在GPS信号退化时轨迹稳定性不足，且计算复杂度高难以在嵌入式平台上实现低延迟跟踪。

Method: 1) 热成像帧通过自适应阈值和形态学细化生成粗略热点区域掩码；2) RGB帧提供边缘线索，使用基于梯度的滤波抑制纹理相关误检测；3) 规则级合并策略选择边界候选点，通过Ramer-Douglas-Peucker算法简化；4) 周期性信标和惯性反馈回路在GPS退化时保持轨迹稳定性；5) 通过限制每帧像素操作和预计算梯度表，在嵌入式SoC平台上实现低于50ms的延迟。

Result: 小规模仿真显示：与纯边缘跟踪基线相比，平均路径长度和边界抖动减少，同时通过交集合并分析保持环境覆盖；电池消耗和计算利用率证实可在标准微型平台上实现10-15m/s的前向运动；系统在嵌入式SoC平台上实现低于50ms的延迟。

Conclusion: 该方法为紧急侦察应用提供了一种轻量级解决方案，仅需鲁棒传感和最小通信，能够在带宽受限的野火环境中实现微型无人机团队的快速部署和稳定边界跟踪。
...
TL;DR: 提出了一个大规模、长期的英国布里斯托尔城市街灯视觉数据集，包含52.6万张图像，用于研究视觉漂移、异常检测和智能城市MLOps策略。


<details>
  <summary>Details</summary>
Motivation: 智能城市部署中需要评估长期模型稳定性、视觉漂移和实际部署环境下的MLOps策略，但缺乏真实世界、长期、多条件的视觉数据集。

Method: 使用22个固定角度摄像头在布里斯托尔收集2021-2025年每小时图像，提供丰富元数据，并开发基于卷积变分自编码器的自监督框架，定义两种漂移度量指标。

Result: 创建了包含52.6万张图像的数据集，涵盖不同光照、天气和季节条件，提供公开可用的JPEG图像和CSV元数据，支持街灯监控、天气推断等下游应用。

Conclusion: 该数据集为评估长期模型稳定性、漂移感知学习和部署就绪的视觉系统提供了真实、细粒度的基准，支持智能城市视觉系统的研究和应用。
...
TL;DR: 提出ALERT数据集和ISA-ViT框架，解决UWB雷达驾驶分心检测中数据集缺乏和ViT输入尺寸固定问题，显著提升分类准确率。


<details>
  <summary>Details</summary>
Motivation: 驾驶分心是全球致命事故的主要原因。虽然IR-UWB雷达在驾驶活动识别中有优势（抗干扰、低功耗、保护隐私），但面临两大挑战：缺乏大规模真实世界UWB数据集，以及固定输入尺寸的ViT难以适应非标准维度的UWB雷达数据。

Method: 1) 提出ALERT数据集：包含10,220个真实驾驶条件下七种分心驾驶活动的雷达样本；2) 提出ISA-ViT框架：通过调整补丁配置和利用预训练位置嵌入向量，在满足ViT输入要求的同时保留雷达特定信息（多普勒频移和相位特征）；3) 采用域融合策略：结合距离域和频域特征提升分类性能。

Result: ISA-ViT在UWB雷达驾驶活动识别任务中，相比现有基于ViT的方法实现了22.68%的准确率提升。通过公开ALERT数据集和详细描述输入尺寸无关策略，促进了更鲁棒、可扩展的驾驶分心检测系统开发。

Conclusion: 该工作通过提供大规模真实世界UWB数据集和创新的输入尺寸无关ViT框架，解决了UWB雷达驾驶分心检测的关键瓶颈，为实际部署更有效的驾驶安全系统奠定了基础。
...
TL;DR: 本文提出了一种用于自闭症儿童情绪识别的深度学习管道，结合CNN和GCN处理面部表情数据，在NAO机器人呼叫名字的实验中表现良好。


<details>
  <summary>Details</summary>
Motivation: 理解自闭症儿童在社交互动中的情绪反应是发展心理学和人机交互领域的重要挑战，目前缺乏针对自闭症儿童的大规模真实世界数据集和分析方法。

Method: 使用混合模型：微调的ResNet-50 CNN + 三层GCN，处理从MediaPipe FaceMesh提取的视觉和几何特征。采用DeepFace和FER模型的加权集成进行概率标注，通过KL散度优化融合嵌入进行分类。

Result: 该方法在建模微妙情感反应方面表现出稳健性能，有效捕捉神经多样性儿童的微情绪线索，为临床和治疗性人机交互中的情感分析提供了有前景的解决方案。

Conclusion: 该研究填补了自闭症特定HRI研究的空白，提供了印度首个大规模真实世界数据集和分析管道，为未来个性化辅助技术奠定了重要基础。
...
TL;DR: 提出了CineLOG数据集和生成管道，用于解决可控视频合成中细粒度控制不足的问题，特别是在相机轨迹和电影类型等电影属性方面。


<details>
  <summary>Details</summary>
Motivation: 当前视频合成模型在文本提示之外的细粒度控制方面存在困难，特别是在相机轨迹和电影类型等电影属性上。现有数据集存在数据不平衡、标签噪声或仿真与现实差距大的问题。

Method: 1) 引入CineLOG数据集：包含5000个高质量、平衡、未剪辑的视频片段，每个片段都有详细场景描述、基于标准电影分类学的明确相机指令和类型标签；2) 提出新的生成管道：将复杂的文本到视频生成任务分解为四个更简单的阶段；3) 引入轨迹引导过渡模块：生成平滑的时空插值以实现连贯的多镜头序列。

Result: 广泛的人类评估显示，该管道在遵循特定相机和剧本指令方面显著优于最先进的端到端文本到视频模型，同时保持专业的视觉质量。

Conclusion: CineLOG数据集和提出的生成管道为可控视频合成提供了有效的解决方案，特别是在电影属性控制方面，填补了现有数据集的不足。
...
TL;DR: 提出评估视觉语言模型推理链视觉忠实性的新框架，以及无需训练的轻量级自反思方法，在保持答案准确性的同时减少不忠实感知步骤。


<details>
  <summary>Details</summary>
Motivation: 现有推理增强视觉语言模型存在新失败模式：模型可能通过视觉不忠实的中间步骤得到正确答案，或推理忠实但最终预测失败。仅评估最终答案准确性的标准方法无法区分这些行为。

Method: 提出无需训练和参考的框架，将推理链分解为感知与推理步骤，使用现成VLM评估步骤级忠实性；基于此提出轻量级自反思程序，检测并局部重新生成不忠实感知步骤。

Result: 在多个推理训练VLM和感知密集型基准测试中，该方法降低了不忠实感知率，同时保持了最终答案准确性，提高了多模态推理的可靠性。

Conclusion: 视觉推理链忠实性应作为独立评估维度，提出的框架和自反思方法能有效提高视觉语言模型推理的可靠性，为多模态推理系统提供更细粒度的评估和改进工具。
...
TL;DR: 提出ACR框架，通过属性解缠学习来解决细粒度零样本学习中的属性纠缠问题，在多个基准数据集上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 细粒度零样本学习中，传统模型将颜色、形状、纹理等不同属性压缩到单一视觉嵌入中，导致属性纠缠问题，干扰了关键视觉差异的识别。现有后处理方法在表示已经混合后才操作，效果有限。

Method: 提出属性中心表示(ACR)框架，通过两个专家混合组件实现属性解缠：1) 混合补丁专家(MoPE)，使用双级路由机制将图像补丁有条件地分派给专门专家；2) 混合属性专家(MoAE)，将专家精炼的特征投影到稀疏的、部分感知的属性映射中。

Result: 在CUB、AwA2和SUN等零样本学习基准数据集上，ACR框架取得了持续的最先进结果。

Conclusion: 通过在表示学习阶段强制属性解缠，ACR框架有效解决了细粒度零样本学习中的属性纠缠问题，实现了更好的视觉属性关系迁移。
...
TL;DR: 论文提出ProImage-Bench基准，用于评估专业图像生成模型在科学精确插图方面的能力，通过自动化评估和迭代编辑显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成视觉上合理的图片方面表现出色，但在生成信息密集、科学精确的专业插图方面存在不足。需要量化评估模型在技术描述到科学插图转换中的表现。

Method: 1) 构建ProImage-Bench基准，包含654个来自真实教科书和技术报告的图表；2) 设计层次化评估标准，分解为6,076个标准和44,131个二元检查；3) 使用大型多模态模型自动生成评估标准；4) 采用基于LMM的自动评估器进行评分；5) 利用失败检查反馈进行迭代编辑优化。

Result: 1) 最佳基础模型仅达到0.791的评估标准准确率和0.553的标准分数；2) 通过迭代编辑优化，可将强生成器的评估标准准确率从0.653提升到0.865，标准分数从0.388提升到0.697；3) 揭示了现有模型在科学保真度方面的显著差距。

Conclusion: ProImage-Bench为专业图像生成提供了严格的诊断工具，同时为改进规范遵循的科学插图提供了可扩展的监督信号，能够有效提升模型在科学精确性方面的表现。
...
TL;DR: 该研究比较了SynthSeg和SamSeg两种婴儿脑MRI分割方法，发现SynthSeg在分割质量和体积估计上优于SamSeg，但分割准确性仍会影响分形维数分析结果。
...
Method: 使用Baby Open Brains数据集（71个扫描，1-9个月），比较SynthSeg和SamSeg两种分割方法。以专家标注为参考，使用Dice系数、IoU、95% Hausdorff距离和归一化互信息等指标评估分割质量，并分析对体积和分形维数估计的影响。

Result: SynthSeg在所有质量指标上优于SamSeg（主要区域平均Dice>0.8），体积估计更接近手动参考（平均+4%）。SamSeg系统性地高估脑室和全脑体积（平均+76%）。分割准确性随年龄增长而提高。分形维数分析显示SynthSeg与专家分割存在显著区域差异，分割相关的FD变异性超过了大多数发育队列报告的组间差异。

Conclusion: SynthSeg为儿科MRI提供了最可靠的体积和分形维数结果，但由于分割相关的不确定性，在解释体积和分形维数的微小形态差异时应谨慎。分割偏差直接影响分形维数估计。
...
TL;DR: 提出AEIC框架，使用浅层编码器和一步扩散解码器，在超低比特率（<0.05bpp）下实现高效编码和高保真解码


<details>
  <summary>Details</summary>
Motivation: 现有超低比特率图像压缩方法依赖大型预训练编码器（如VAE或tokenizer），在带宽受限的边缘设备上部署困难，需要更轻量的编码方案

Method: 提出非对称极端图像压缩（AEIC）框架：使用浅层编码器网络，配合一步扩散解码器；设计双端特征蒸馏方案，将知识从中等编码器迁移到浅层编码器变体

Result: 在超低比特率下，AEIC在率失真感知性能上优于现有方法；编码效率达35.8 FPS（1080P输入），解码速度与现有方法相当

Conclusion: AEIC框架证明了浅层编码器在超低比特率压缩中的可行性，实现了编码简单性和解码质量的双重目标，适合边缘设备部署
...
TL;DR: 提出一种新颖的基于LLM的视频时刻检测方法，通过让LLM输出"0"/"1"字符序列来同时实现时刻定位和显著性评分，仅需25帧即可达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于文本生成的MLLM方法无法为帧级预测提供直接梯度，而强化学习方法存在训练不稳定的问题。需要一种既能利用LLM推理能力又能直接优化帧级预测的方法

Method: 让LLM接收固定数量帧和提示，输出连续的"0"/"1"字符序列（每个字符对应一帧），将字符视为背景/前景概率。结合分割损失和因果语言模型损失进行训练，推理时通过束搜索生成序列和逻辑值

Result: 在QVHighlights数据集上，仅使用25帧（少于其他方法一半）就实现了56.74 HIT@1的突出检测性能，时刻检索达到35.28 MAP，优于基线方法

Conclusion: 提出的方法通过将分割目标直接应用于LLM输出标记，实现了高效的视频时刻检测和突出检测，分割损失即使在因果LM损失平台期也能提供稳定的学习信号
...
TL;DR: MetaTPT：一种元学习框架，通过自监督辅助任务学习参数化增强，指导测试时提示调优，提升视觉语言模型在域偏移下的适应性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时提示调优（TPT）方法使用固定的数据增强，在更具挑战性的域偏移场景下效果有限，需要更灵活、更具表达力的增强策略来捕捉目标域的关键特征。

Method: 提出MetaTPT框架，采用双循环优化范式：内循环学习自监督任务，为每个样本动态生成参数化增强视图；外循环基于这些增强视图的一致性进行提示调优。通过将增强学习与提示调优耦合，实现更有效的测试时适应。

Result: 在域泛化和跨数据集基准测试中，MetaTPT取得了最先进的性能表现，显著提升了视觉语言模型在域偏移下的适应性。

Conclusion: MetaTPT通过元学习框架将增强学习与提示调优相结合，为视觉语言模型在测试时的域适应提供了更有效的解决方案，特别是在具有挑战性的域偏移场景下。
...
TL;DR: 提出一种结合深度卷积特征和面部动作单元的混合框架，用于连续学习环境下的面部表情识别，通过贝叶斯高斯混合模型缓解灾难性遗忘问题


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在日常生活中的广泛应用，识别和适应人类情绪对于有效的人机交互至关重要。面部表情识别是推断情感状态的主要渠道，但情绪的动态性和文化差异性要求模型能够持续学习而不遗忘先前知识

Method: 提出混合框架，整合两种互补模态：深度卷积特征和面部动作单元（AUs）。使用贝叶斯高斯混合模型（BGMMs）对组合表示进行建模，提供轻量级、概率性的解决方案，避免重新训练同时保持强判别能力

Result: 在CFEE数据集上，模型能够先学习基本表情，然后逐步识别复合表情。实验显示提高了准确性，增强了知识保留能力，减少了遗忘
...
TL;DR: Cognitive-YOLO：基于LLM的数据驱动架构生成框架，直接从数据集特征合成YOLO检测网络，无需传统NAS的昂贵搜索


<details>
  <summary>Details</summary>
Motivation: 传统手动设计目标检测架构耗时费力，神经架构搜索(NAS)计算成本过高，现有LLM方法多作为搜索循环中的优化器而非直接从数据理解生成架构。需要一种能基于数据集内在特征直接合成高性能架构的方法。

Method: 三阶段框架：1) 分析模块提取数据集元特征（目标尺度分布、场景密度等）；2) LLM基于这些特征，结合RAG检索的最先进组件进行推理，生成结构化神经架构描述语言(NADL)；3) 编译器将描述实例化为可部署模型

Result: 在五个多样化目标检测数据集上的实验表明，Cognitive-YOLO生成的架构始终优于基线模型，实现了极具竞争力的性能，并在多个基准测试中展现出更优的性能-参数权衡。消融研究证明LLM的数据驱动推理是性能提升的主要驱动力。

Conclusion: 基于数据"第一原理"的深度理解比单纯检索SOTA组件更能生成优越架构。Cognitive-YOLO展示了LLM直接从数据集特征合成高性能检测架构的能力，为自动化架构设计提供了新范式。
...
TL;DR: RealDrag是首个包含真实目标图像的基于点图像编辑基准数据集，包含400+人工标注样本，并提出四个新评估指标，用于系统评估17个SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于拖拽的图像编辑模型评估不可靠，缺乏标准化基准和指标。主要问题包括评估协议不一致，以及缺少包含真实目标图像的数据集，导致不同方法难以客观比较。

Method: 1. 构建RealDrag基准数据集：包含400+人工标注样本，来自多样化视频源，提供源/目标图像、处理点/目标点、可编辑区域掩码和描述性标注。
2. 提出四个任务特定评估指标：语义距离(SeD)、外部掩码保持分数(OMPS)、内部补丁保持分数(IPPS)、方向相似性(DiS)，分别量化像素级匹配保真度、非编辑区域保持和语义对齐。

Result: 1. 建立了首个包含真实目标图像的基于点图像编辑基准。
2. 对17个SOTA模型进行了首次大规模系统分析。
3. 揭示了当前方法之间的明确权衡关系。
4. 建立了稳健、可复现的基线，指导未来研究。

Conclusion: RealDrag基准解决了基于点图像编辑评估的标准化问题，提供了包含真实目标图像的数据集和专门设计的评估指标，为领域建立了可靠的评估框架，数据集和评估工具将公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer架构搜索方法中，所有候选架构共享相同权重，导致对小规模子网的严重干扰。研究发现训练良好的小规模子网可以作为训练大规模子网的良好基础

Method: 提出GrowTAS渐进式训练框架：从训练小规模子网开始，逐步加入大规模子网，减少干扰并稳定训练过程。还提出GrowTAS+，仅微调部分权重以进一步提升大规模子网性能

Result: 在ImageNet和多个迁移学习基准（CIFAR-10/100、Flowers、CARS、INAT-19）上的大量实验证明，该方法优于当前TAS方法

Conclusion: 通过渐进式训练框架，有效解决了Transformer架构搜索中权重共享导致的干扰问题，显著提升了搜索到的视觉Transformer架构性能
...
TL;DR: 论文提出了Intention-Drive基准测试，用于评估自动驾驶系统将高级人类意图转化为安全精确驾驶行为的能力，揭示了现有模型在意图理解方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统仅能执行低级转向指令，而真正智能的自动驾驶需要从"指令跟随者"转变为"意图实现者"，理解并满足高级抽象的人类意图。然而，缺乏标准化基准来评估和推动这一复杂任务的进展。

Method: 提出了Intention-Drive基准测试，包含两个核心贡献：(1) 包含复杂场景和对应自然语言意图的新数据集；(2) 以意图成功率(ISR)为中心的新评估协议，评估人类目标的语义实现程度，超越简单的几何精度。

Result: 通过对一系列基线模型在Intention-Drive上的广泛评估，揭示了显著的性能缺陷，表明基线模型难以达到这一高级任务所需的全面场景和意图理解能力。

Conclusion: Intention-Drive填补了评估自动驾驶意图理解能力的关键空白，为从指令跟随到意图实现的范式转变提供了标准化基准，揭示了当前系统在高级意图理解方面的局限性。
...
TL;DR: OMUDA提出分层掩码策略解决无监督域自适应中的跨域上下文模糊、特征不一致和伪标签噪声问题，在多个基准测试中平均提升7%


<details>
  <summary>Details</summary>
Motivation: 现有无监督域自适应方法在语义分割中面临三个主要挑战：跨域上下文模糊、不一致的特征表示和类别级伪标签噪声，导致难以有效弥合域间差距
...
Result: 在SYNTHIA->Cityscapes和GTA5->Cityscapes等挑战性跨域语义分割基准测试中验证有效性，可无缝集成到现有UDA方法中，平均提升7%，达到最先进结果

Conclusion: OMUDA的分层掩码范式在上下文、表示和类别三个层面有效减少域偏移，提供了超越现有方法的统一解决方案，显著提升了无监督域自适应语义分割的性能
...
TL;DR: WeDetect是一个基于检索范式的开放词汇目标检测模型家族，包含三个变体：基础检测器、通用建议生成器和LMM分类器，在15个基准上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 探索非跨模态融合层方法的检索范式优势，实现高效、多功能的开放词汇目标检测，统一检测、建议生成、目标检索和指代表达理解任务

Method: 采用双塔架构的检索式方法：1) WeDetect基础检测器；2) WeDetect-Uni冻结检测器并微调目标性提示；3) WeDetect-Ref结合LMM处理复杂指代表达

Result: 在15个基准测试中达到最先进性能，实现实时检测，支持历史数据快速回溯和复杂指代表达理解，推理效率高

Conclusion: WeDetect模型家族在统一的检索框架下成功整合了多种视觉任务，证明了非融合检索范式在开放词汇检测中的优越性和实用性
...
TL;DR: UniCoDe提出了一种统一的算法，将基于采样的方法和梯度引导方法结合，通过整合局部梯度信号来提高扩散模型与下游目标对齐的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型输出与下游目标对齐对于提升任务特定性能至关重要。现有的推理时免训练对齐方法主要有两类：基于采样的方法（探索多个候选输出并选择奖励信号更高的）和梯度引导方法（使用可微奖励近似直接引导生成过程）。需要一种统一框架来结合两者的优势。

Method: 提出UniCoDe算法，将采样和梯度引导结合到统一框架中。在采样过程中整合局部梯度信号，解决复杂奖励采样方法的采样效率问题。通过协调结合这两种范式，实现更高效的采样，并在奖励对齐和扩散无条件先验偏离之间提供更好的权衡。

Result: 实证结果表明，UniCoDe在一系列任务中与最先进的基线方法保持竞争力。代码已开源。

Conclusion: UniCoDe成功地将采样和梯度引导方法统一到一个框架中，提高了扩散模型与下游目标对齐的效率和效果，在多个任务上表现出色。
...
TL;DR: 提出TCLeaf-Net检测模型和Daylily-Leaf数据集，用于解决田间复杂背景下叶片病害检测的挑战，在精度和效率上均有提升


<details>
  <summary>Details</summary>
Motivation: 田间叶片病害检测面临复杂背景干扰、领域偏移和病灶级数据集有限等挑战，需要开发鲁棒且实用的检测方法

Method: 提出TCLeaf-Net混合检测器：1) TCM模块结合全局上下文和局部卷积抑制非叶区域；2) RSFRS块保留精细空间细节；3) DFPN块处理病灶尺度变化和特征偏移

Result: 在Daylily-Leaf数据集上，mAP@50提升5.4个百分点至78.2%，计算量减少7.5 GFLOPs，GPU内存使用降低8.7%，在多个植物病害数据集上表现优异

Conclusion: TCLeaf-Net在田间复杂背景下实现了高效准确的叶片病害检测，具有良好的鲁棒性和泛化能力
...
TL;DR: VideoARM：一种基于分层记忆的智能体推理范式，用于长视频理解，通过自适应实时推理和记忆构建，显著减少token消耗并提升性能。
...
Method: 提出VideoARM范式，采用自适应循环的观察-思考-行动-记忆流程，控制器自主调用工具以粗到细的方式解释视频，同时分层多模态记忆持续捕获和更新多级线索。

Result: 在主流基准测试中，VideoARM优于最先进的DVD方法，同时显著减少了长视频的token消耗。

Conclusion: VideoARM通过智能体推理和分层记忆机制，为长视频理解提供了高效的自适应解决方案，在性能和效率方面均取得显著提升。
...
TL;DR: STAGE提出了一种基于故事板的多镜头视频生成工作流，通过预测结构化的故事板（起始-结束帧对）来替代稀疏关键帧，解决了多镜头叙事中的一致性和电影语言问题。
...
Method: 1. 提出STAGE工作流，将多镜头视频生成任务重新表述为故事板锚定的生成；2. 使用STEP2预测结构化故事板（每个镜头的起始-结束帧对）；3. 引入多镜头记忆包确保长程实体一致性；4. 采用双重编码策略保证镜头内连贯性；5. 两阶段训练方案学习电影化的镜头间过渡；6. 构建大规模ConStoryBoard数据集，包含高质量电影片段和细粒度标注。

Result: 大量实验表明，STAGE在结构化叙事控制和跨镜头连贯性方面实现了优越性能。

Conclusion: STAGE通过故事板锚定的生成方法，有效解决了多镜头视频生成中的一致性和电影语言问题，为连贯叙事视频合成提供了新的解决方案。
...
TL;DR: V-Warper：无需训练的视频个性化框架，通过粗到细的外观适应和推理时外观注入，提升身份保真度


<details>
  <summary>Details</summary>
Motivation: 现有视频个性化方法依赖大量视频微调或大规模视频数据集，计算成本高且难以扩展，同时在跨帧保持细粒度外观一致性方面存在困难

Method: 提出V-Warper训练免费框架：1）轻量级粗外观适应阶段使用少量参考图像，通过图像LoRA和主题嵌入适应编码全局身份；2）推理时细外观注入阶段通过RoPE-free中层查询-键特征计算语义对应关系，引导外观丰富的值表示变形到生成过程的语义对齐区域

Result: V-Warper显著提升外观保真度，同时保持提示对齐和运动动态，无需大规模视频微调即可高效实现这些改进

Conclusion: V-Warper解决了现有视频个性化方法的计算成本和细粒度一致性问题，提供了一种高效、无需训练的身份保真度提升方案
...
TL;DR: M4Human是目前最大规模的多模态人体网格重建基准数据集，包含66.1万帧高分辨率毫米波雷达、RGB和深度数据，提供原始雷达张量和处理后的雷达点云，支持跨不同RF信号粒度的研究。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉的HMR方法受限于遮挡、光照变化和隐私问题，而现有雷达数据集存在骨架标签稀疏、规模有限、动作简单等问题。需要构建大规模多模态基准来推动隐私保护的室内人体感知研究。

Method: 构建M4Human数据集，包含20名受试者的50种多样化动作（原地、坐姿、自由空间运动），提供高分辨率毫米波雷达、RGB和深度数据，并标注高质量的运动捕捉数据（3D网格和全局轨迹）。

Result: 建立了基于原始雷达张量和雷达点云的基准测试，以及多模态融合（RGB-D）的基准。结果显示M4Human对雷达人体建模具有重要意义，但也揭示了在快速、无约束运动下的持续挑战。

Conclusion: M4Human是目前最大规模的多模态人体网格重建基准，为隐私保护的室内人体感知研究提供了重要资源，将推动雷达人体建模领域的发展，数据集和代码将在论文发表后开源。
...
TL;DR: SR-DiT框架通过系统集成多种技术（token路由、架构改进、训练修改和表示对齐），在小型模型（140M参数）上实现了与大型模型（685M参数）相当的图像生成性能


<details>
  <summary>Details</summary>
Motivation: 虽然扩散变换器的训练效率已有显著提升，但现有技术大多孤立研究，缺乏对多种方法组合协同效应的探索。本文旨在系统整合多种技术，发掘它们的潜在协同作用

Method: 提出了SR-DiT框架，在表示对齐的基础上系统集成了token路由、架构改进和训练修改等多种技术，并通过大量消融实验验证不同技术组合的有效性

Result: 在ImageNet-256上仅用140M参数模型、400K迭代、无需分类器自由引导，达到了FID 3.49和KDD 0.319，与需要更大参数（685M）和更长训练时间的模型性能相当，是该模型规模下的最先进结果

Conclusion: SR-DiT框架展示了多种技术组合的协同效应，为未来研究提供了计算可访问的基准，并识别了最有效的技术组合以及它们之间的兼容性问题
...
TL;DR: ArtGen：基于条件扩散的框架，从单视图图像或文本描述生成具有准确几何和连贯运动学的铰接式3D物体，通过跨状态蒙特卡洛采样和思维链推理解决结构-运动纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型依赖表示闭合状态的单视图输入，导致几何形状和关节动力学纠缠，产生模糊或不现实的运动学结构。铰接式资产生成对机器人、数字孪生和具身智能至关重要。

Method: 1. 跨状态蒙特卡洛采样显式强制全局运动学一致性；2. 思维链推理模块推断结构先验（部件语义、关节类型、连接性）；3. 稀疏专家扩散变换器专门处理多样运动学交互；4. 局部-全局注意力增强的组合式3D-VAE潜在先验。

Result: 在PartNet-Mobility基准测试中，ArtGen显著优于现有最先进方法，能够生成具有准确几何和连贯运动学的铰接式3D物体。

Conclusion: ArtGen通过解耦结构-运动纠缠，实现了从单视图图像或文本描述生成高质量铰接式3D物体的目标，为机器人、数字孪生和具身智能应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: LiDAR传感器因硬件老化、灰尘、雪雾或强反射等原因导致垂直光束丢失，这会移除整个垂直切片，严重影响自动驾驶车辆的3D感知能力。

Method: 将LiDAR扫描表示为非结构化空间图：点作为节点，边连接邻近点并保留原始光束索引顺序。使用多层图注意力网络学习局部几何邻域的自适应注意力权重，直接回归丢失位置的高程(z)值。

Result: 在1,065个KITTI序列上训练和评估，模拟通道丢失，平均高度RMSE为11.67厘米，87.98%的重建点落在10厘米误差阈值内。单GPU推理每帧14.65秒，重建质量对不同邻域大小k保持稳定。

Conclusion: 结果表明，仅基于原始点云几何的纯图注意力模型能够有效恢复实际传感器退化情况下的丢失垂直光束。
...
TL;DR: ViInfographicVQA是首个越南语信息图视觉问答基准，包含6747个真实信息图和20409个人工验证问答对，涵盖单图和跨图推理任务，揭示了当前多模态模型在低资源语言中的局限性。


<details>
  <summary>Details</summary>
Motivation: 信息图视觉问答需要结合OCR、布局理解和语义推理，但现有研究主要关注英语等高资源语言。越南语作为低资源语言缺乏相关基准，需要评估模型在数据丰富、布局复杂的信息图上的理解和推理能力。

Method: 构建了包含6747个真实世界信息图的越南语基准，涵盖经济、医疗、教育等多个领域，包含20409个人工验证的问答对。设计了两种评估设置：单图任务（传统VQA）和多图任务（需要跨信息图证据整合）。

Result: 评估了多种最新的视觉语言模型，发现存在显著的性能差距。在多图任务中表现最差，特别是在需要跨图整合和非片段推理的问题上。模型在低资源语言环境中的布局感知和跨图推理能力有限。

Conclusion: ViInfographicVQA为越南语信息图VQA提供了首个基准，揭示了当前多模态模型在低资源语言中的局限性，鼓励未来研究布局感知和跨图推理方法的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用景深与单目深度估计之间的紧密关联。高质量景深渲染依赖噪声深度图导致可见伪影，而现代单目深度模型在弱纹理、远距离和几何模糊区域表现不佳，这些区域恰恰是散焦线索最丰富的地方。

Method: 两阶段框架：第一阶段，基于预训练图像编辑骨干构建物理引导的可控景深生成器，从单张清晰输入生成无深度信息的景深堆栈；第二阶段，轻量级散焦感知聚合模块插入现有单目深度编码器，沿散焦维度融合特征，暴露稳定的深度敏感变化，同时保持下游解码器不变。

Result: 在多个挑战性基准测试中，BokehDepth相比基于深度图的景深基线提升了视觉保真度，并持续增强了强单目深度基础模型的度量精度和鲁棒性。

Conclusion: 通过解耦景深合成与深度预测，并将散焦作为无监督几何线索，BokehDepth实现了景深渲染质量和单目深度估计精度的双重提升，证明了利用成像几何关联的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决长序列视频生成中保持3D结构稳定性和连贯性的挑战，特别是在流式场景下。

Method: 采用条件自回归训练策略对齐新生成内容与现有帧，集成全局3D感知注意力提供连续几何指导，通过3D注入机制增强物理合理性和几何一致性。

Result: 能够生成长、稳定且视觉连贯的视频，在视觉保真度和空间一致性方面达到或超越现有方法，支持单GPU实时推理。

Conclusion: Endless World为无限3D一致视频生成提供了有效的实时解决方案，解决了长序列和动态场景合成的关键挑战。
...
TL;DR: PeRL-VL提出了一种解耦框架，分别改进视觉语言模型的视觉感知和文本推理能力，通过描述奖励和纯文本推理微调，显著提升多模态基准性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于可验证奖励的强化学习（RLVR）训练的视觉语言模型存在两个主要问题：1）视觉提取不准确（遗漏或幻觉细节）；2）思维链逻辑不一致。这是因为可验证信号只监督最终答案，无法有效指导中间过程。

Method: PeRL-VL采用解耦框架：1）感知方面：引入基于VLM的描述奖励，对模型自生成的图像描述进行忠实性和充分性评分；2）推理方面：在逻辑丰富的思维链数据上添加纯文本推理监督微调阶段，独立于视觉增强逻辑一致性。

Result: 在多样化的多模态基准测试中，PeRL-VL将Qwen2.5-VL-7B的平均Pass@1准确率从63.3%提升到68.8%，优于标准RLVR、纯文本推理SFT以及从GPT-4o的朴素多模态蒸馏。

Conclusion: 通过分别改进视觉感知和文本推理的解耦方法，PeRL-VL有效解决了RLVR训练中视觉提取不准确和逻辑不一致的问题，显著提升了视觉语言模型的整体性能。
...
TL;DR: 提出AdaptiveDetector框架，结合YOLOv11检测器和VLM验证器，通过自适应阈值调整和成本敏感强化学习，在恶劣成像条件下显著提升息肉检测召回率，减少漏检风险。


<details>
  <summary>Details</summary>
Motivation: 现有息肉检测器在实验室干净数据集上训练，但在真实内窥镜场景中（光照变化、运动模糊、遮挡等恶劣成像条件）性能下降，存在临床漏检风险。

Method: 提出两阶段检测器-验证器框架：1) YOLOv11检测器在VLM指导下自适应调整每帧置信度阈值；2) VLM验证器使用Group Relative Policy Optimization (GRPO)进行微调，采用非对称成本敏感奖励函数，专门惩罚漏检。

Result: 在合成恶劣条件下的CVC-ClinicDB和Kvasir-SEG数据集上进行零样本评估，召回率比单独YOLO提升14-22个百分点，精度保持在基线±1.7个百分点内。

Conclusion: 自适应阈值调整与成本敏感强化学习相结合，实现了临床对齐的开放世界息肉检测，显著减少假阴性，降低癌前息肉漏检风险，改善患者预后。
...
TL;DR: 提出PatchGraph方法，通过图注意力网络学习图像块间关系来增强少样本分类，在保持零样本效率的同时提升性能


<details>
  <summary>Details</summary>
Motivation: 现有基于缓存的少样本图像分类方法（如Tip-Adapter）仍继承CLIP的全局表示倾向，在低数据域适应中不够判别性。需要更细粒度的patch级关系建模来提升专业领域适应能力。

Method: 提出关系门控图注意力网络，构建patch图并进行边缘感知注意力，强调信息丰富的patch间交互，生成上下文丰富的patch嵌入。通过可学习的多聚合池化将这些嵌入组合成紧凑、任务判别的表示，更好地对齐缓存键与目标少样本类别。

Result: 在11个基准测试上相比最先进的CLIP适配器和基于缓存的方法取得一致提升，同时保持零样本效率。还验证了战场应用价值，引入了受伤与未受伤士兵数据集用于伤亡识别。

Conclusion: 提出的patch驱动关系细化方法通过挖掘图像内部patch依赖关系，显著提升了少样本分类性能，在保持推理效率的同时解决了CLIP全局表示的局限性，具有重要的实际应用价值。
...
Method: 使用现成的视频扩散模型从给定的图像数据集中生成真实的3D空间和时间变化，创建合成视频片段作为补充训练数据。提供实用指南包括：(1)选择合适的时空生成设置，(2)将标注转移到合成帧，(3)处理新暴露区域的遮挡问题。

Result: 在COCO子集和无人机捕获的数据集上的实验表明，时空数据增强能够沿着传统和先前生成方法未能充分代表的维度扩展数据分布，在数据稀缺情况下有效提升模型性能。

Conclusion: 时空数据增强通过视频基础模型生成真实的3D变化，为低数据场景提供了一种有效的性能提升手段，特别是在无人机图像等标注稀缺的应用中。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型虽然通过思维链机制增强了跨模态理解，但仍依赖显式的逐步推理、感知-推理交互不稳定且计算开销大。受人类认知启发，认为思维不是线性展开，而是推理与感知在头脑中动态交错进行。

Method: 提出DMLR框架：1) 使用置信度引导的潜在策略梯度优化来精炼潜在思考标记以进行深度推理；2) 引入动态视觉注入策略，在每个潜在思考标记处检索最相关的视觉特征并更新最佳视觉补丁集，然后将更新的补丁注入潜在思考标记中，实现动态视觉-文本交错。

Result: 在七个多模态推理基准测试和各种模型架构上的实验表明，DMLR显著提高了推理和感知性能，同时保持了较高的推理效率。

Conclusion: DMLR通过动态多模态潜在推理框架，实现了更接近人类认知的视觉-文本动态交错推理，在提升多模态推理性能的同时保持了高效性，为多模态大语言模型的发展提供了新思路。
...
Method: 提出一个集成心肌解剖先验、结构感知特征编码和三维小波-逆小波变换的分割框架。编码阶段结合心肌先验和残差注意力特征增强，小波变换实现联合空频建模和多尺度结构一致性，解码阶段通过多尺度特征融合模块整合语义和几何信息。

Result: 在ImageCAS数据集上，采用3D重叠补丁策略（7:1:2划分），获得Dice系数0.8082、敏感性0.7946、精确度0.8471、HD95为9.77mm，优于多个主流分割模型。消融实验验证了各组件互补贡献。
...
TL;DR: 提出了一种基于监督对比学习的视频表示学习方法，通过视频到图像的聚合策略将多帧视频空间排列为单张图像，利用预训练CNN骨干网络，避免复杂视频Transformer的计算开销，在分类准确率和计算效率上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频表示学习方法通常依赖复杂的视频Transformer模型，计算开销大。需要一种既能利用预训练CNN骨干网络优势，又能有效学习视频时序全局上下文的方法，同时降低计算成本。

Method: 1. 视频到图像聚合策略：将多个视频帧空间排列成单张图像作为输入；2. 使用预训练CNN骨干网络（如ResNet50）；3. 设计监督对比学习目标：相同标签的视频投影作为正样本对，其他作为负样本；4. 通过不同时间帧采样创建同一视频的多个自然视图作为多样正样本。

Result: 在Penn Action数据集上达到76%分类准确率（ViVIT为43%），在HMDB51数据集上达到48%准确率（ViVIT为37%）。方法在监督和自监督设置下都能学习有效视频表示，支持分类和字幕生成等任务，且计算资源需求更少。

Conclusion: 提出的监督对比帧聚合方法通过视频到图像转换和对比学习，有效利用预训练CNN骨干网络学习视频表示，在保持高分类准确率的同时显著降低计算复杂度，为视频理解任务提供了高效解决方案。
...
TL;DR: ATP是一种动态推理机制，通过保留最相关的视觉token来减少VLMs的计算开销，实现约40%的FLOPs减少和1.5倍加速，精度损失小于1%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型计算效率低下，对所有token进行统一处理，导致实际部署时计算需求过高。需要一种动态推理机制来减少计算开销。

Method: 提出自适应token剪枝(ATP)，在视觉-语言接口处根据上下文相关性保留最相关的token。使用混合重要性评分：ViT CLS注意力（模态内显著性）和CLIP文本-图像相似性（模态间相关性），保留top-K token给LLM处理。

Result: 在VQAv2、GQA和COCO数据集上的评估显示，ATP减少约40%的推理FLOPs，实现约1.5倍的端到端延迟加速，精度损失小于1%。定性分析表明ATP保留了视觉基础并增强了可解释性。

Conclusion: ATP证明资源受限的推理和模型可靠性不是竞争目标，自适应剪枝可以抑制虚假相关性提高稳定性。ATP在高效多模态边缘计算管道中具有应用潜力。
...
TL;DR: 提出MSSAVT冗余度量与掩码剪枝策略，通过视频token剪枝减少MLLMs处理在线视频时的计算开销，在多个基准上显著提升精度且剪枝延迟极低。


<details>
  <summary>Details</summary>
Motivation: 在线视频理解应用广泛，但多模态大语言模型处理视频时面临帧数多、GPU内存占用高、计算延迟大的挑战，需要高效减少上下文长度的方法。

Method: 提出最大空间相邻视频token相似度(MSSAVT)冗余度量，结合token相似度和空间位置；设计掩码剪枝策略避免双向依赖问题；集成现有时间冗余剪枝方法消除时间冗余。

Result: 在多个在线和离线视频理解基准测试中，方法显著提升精度（最高4%），剪枝延迟极低（小于1ms）。

Conclusion: 提出的token剪枝方法能有效减少MLLMs处理视频时的计算负担，同时保持关键信息，为在线视频理解提供高效解决方案。
...
TL;DR: 提出基于检索增强生成(RAG)的零样本道路标志识别框架，使用视觉语言模型生成标志描述，检索相关候选，再由大语言模型进行细粒度识别，在303个监管标志上取得优异效果。
...
Method: 采用RAG范式：1) 视觉语言模型从输入图像生成文本描述；2) 从参考设计的向量数据库中检索最相关的候选标志；3) 大语言模型对检索结果进行推理，完成细粒度识别。

Result: 在俄亥俄MUTCD的303个监管标志上验证，理想参考图像准确率达95.58%，具有挑战性的真实道路数据准确率达82.45%。

Conclusion: 基于RAG的架构为创建无需任务特定训练的可扩展、准确的道路标志识别系统提供了可行方案。
...
TL;DR: MVP提出了一种测试时适应框架，通过相机曝光三角（ISO、快门速度、光圈）作为物理提示，从多物理视图中选择最优设置并聚合预测，显著提升视觉语言模型在物理环境中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型从网络图像扩展到传感器介导的物理环境，传统数字增强方法在物理环境中的效果有限，需要探索测量时控制（而非仅后捕获提示）来提升模型鲁棒性。

Method: MVP框架：1）推理时获取每个场景的物理视图库；2）使用源亲和度得分选择top-k传感器设置；3）对保留视图进行轻量级数字增强；4）过滤最低熵的增强视图子集；5）使用零温度softmax（硬投票）聚合预测。无需梯度或模型修改。

Result: 在ImageNet-ES和ImageNet-ES-Diverse上，MVP比单次自动曝光捕获的数字TTA提升高达25.6个百分点，比结合传统传感器控制与TTA的流程额外提升3.4个百分点。在减少参数候选集降低捕获延迟时仍保持有效。

Conclusion: 测量时控制——选择和组合真实物理视图——能显著提升视觉语言模型的鲁棒性，超越了仅依赖后捕获提示的方法，为物理环境中的VLM应用提供了实用框架。
...
### [87] [StegaVAR: Privacy-Preserving Video Action Recognition via Steganographic Domain Analysis](https://arxiv.org/abs/2512.12586)
*Lixin Chen,Chaomeng Chen,Jiale Zhou,Zhijian Wu,Xun Lin*

Main category: cs.CV

TL;DR: StegaVAR：首个在隐写域直接执行视频动作识别的隐私保护框架，将动作视频嵌入普通封面视频中，同时保持时空信息完整性和传输隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 当前隐私保护方法存在两个主要问题：1) 隐蔽性低，视觉扭曲的视频在传输中容易引起攻击者注意；2) 时空特征破坏，影响准确的动作识别。需要一种既能保护隐私又不破坏动作识别所需时空特征的方法。

Method: 提出StegaVAR框架，将秘密动作视频嵌入普通封面视频中，首次直接在隐写域执行视频动作识别。包含两个关键技术：1) STeP（秘密时空促进），在训练时使用秘密视频指导隐写域的时空特征提取；2) CroDA（跨波段差异注意力），通过捕捉跨波段语义差异来抑制封面干扰。

Result: 实验表明，StegaVAR在广泛使用的数据集上实现了优越的视频动作识别和隐私保护性能。该框架对多种隐写模型都有效。

Conclusion: StegaVAR成功解决了隐私保护视频动作识别中的隐蔽性低和时空特征破坏问题，通过在隐写域直接分析，同时保持了动作识别的准确性和传输的隐蔽性。
...
### [88] [Content-Aware Ad Banner Layout Generation with Two-Stage Chain-of-Thought in Vision Language Models](https://arxiv.org/abs/2512.12596)
*Kei Yoshitake,Kento Hosono,Ken Kobayashi,Kazuhide Nakata*

Main category: cs.CV

TL;DR: 提出利用视觉语言模型生成图像广告布局的方法，通过分析背景图像内容来优化文本和logo的放置，相比传统显著性映射方法能产生更高质量的布局。
...
Conclusion: 通过利用VLM显式考虑背景图像内容，提出的方法能够生成更高质量的广告布局，超越了传统显著性映射方法的局限性。
...
TL;DR: 该研究比较了三种智能系统（模糊逻辑、神经网络、案例推理）用于心脏病预测，最终选择案例推理方法，在心脏病数据集上达到了97.95%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统医疗诊断方法依赖医生经验，准确性有限，需要更精确的智能系统来改善心脏病预测。

Method: 比较了模糊逻辑、神经网络和案例推理三种智能系统方法，最终选择案例推理。对心脏病数据集进行数据预处理（清洗）和数据分割（训练集和测试集），然后应用选定的智能系统进行预测。

Result: 案例推理方法在心脏病预测中达到了97.95%的准确率。分析显示男性患心脏病概率为57.76%，女性为42.24%。相关研究表明吸烟和饮酒是心脏病的重要风险因素，尤其在男性中更为显著。

Conclusion: 案例推理在心脏病预测中表现出优异的准确性，智能系统可以显著改善传统医疗诊断的局限性。性别差异和生活方式因素（如吸烟、饮酒）对心脏病风险有重要影响。
...
TL;DR: 本文提出了一种用于线束检测的半自动化机器视觉系统，能够验证线束位置、连接器极性和颜色序列的正确性，在工业环境中实现了100%检测准确率和44%的时间节省。
...
Method: 系统采用五个工业级CMOS摄像头集成到模块化机械框架中，使用基于HSV和RGB颜色域值比较的颜色序列分类器。用户可以使用至少五个参考样本训练系统，训练文件可存储并重复用于类似线束类型。

Result: 系统在GPV Lanka Pvt. Ltd.部署后实现了100%的检测准确率，相比人工方法减少了44%的检测时间。系统还包含用户管理、可调照明、会话数据存储和安全登录等附加功能。
...
<details>
  <summary>Details</summary>
Motivation: 解决高分辨率图像合成和多模态数据解释中的挑战，特别是传统扩散方法在计算效率和图像质量方面的不足，推动计算机视觉和多模态AI的发展。
...
TL;DR: DiG提出了一种基于差异定位的代理任务框架，通过让多模态大语言模型识别相似图像对之间的所有差异来学习细粒度视觉感知，无需预先知道差异数量。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多种视觉语言任务上表现出色，但其细粒度视觉感知和精确空间推理能力仍然有限，需要新的方法来提升这些能力。

Method: 1) 提出差异定位代理任务框架，让模型识别相似图像对之间的所有差异；2) 开发基于3D渲染的自动化数据生成流水线，生成高质量配对图像；3) 采用课程学习策略，从单个差异逐步增加到多个差异。

Result: DiG显著提升了模型在各种视觉感知基准测试中的性能，学习到的细粒度感知技能能有效迁移到标准下游任务，包括RefCOCO系列和通用多模态感知基准。

Conclusion: 差异定位是一种可扩展且稳健的方法，能够有效提升多模态大语言模型的细粒度视觉推理能力。
...
### [93] [Geometry-Aware Scene-Consistent Image Generation](https://arxiv.org/abs/2512.12598)
...
Method: 1) 场景一致数据构建管道，生成多样化、几何基础训练对；2) 新颖的几何引导注意力损失，利用跨视图线索规范模型的空间推理。

Result: 在场景一致基准测试中，相比最先进基线，方法在场景对齐和文本图像一致性方面表现更好，自动指标和人类偏好研究均证实。生成几何一致、多样化且忠实于文本指令和场景结构的图像。

Conclusion: 提出的几何感知场景一致图像生成方法成功解决了场景保持与提示遵循的权衡问题，通过数据构建和几何引导损失实现了更好的场景一致性和文本遵循能力。
...
### [94] [Anatomy-Guided Representation Learning Using a Transformer-Based Network for Thyroid Nodule Segmentation in Ultrasound Images](https://arxiv.org/abs/2512.12662)
*Muhammad Umar Farooq,Abd Ur Rehman,Azka Rehman,Muhammad Usman,Dong-Kyu Chae,Junaid Qadir*

Main category: cs.CV

TL;DR: SSMT-Net：一种用于甲状腺结节超声图像分割的半监督多任务Transformer网络，通过利用未标记数据和多任务学习提高分割精度


<details>
  <summary>Details</summary>
Motivation: 甲状腺结节超声图像分割面临边界模糊、尺寸变化大、标注数据稀缺等挑战，现有深度学习模型难以有效整合甲状腺腺体上下文信息并泛化到多样化病例

Method: 提出SSMT-Net：1）半监督学习阶段利用未标记数据增强Transformer编码器特征提取能力；2）监督学习阶段联合优化结节分割、腺体分割和结节尺寸估计三个任务，整合局部和全局上下文特征

Result: 在TN3K和DDTI数据集上的广泛评估表明，SSMT-Net优于现有最先进方法，具有更高的准确性和鲁棒性

Conclusion: SSMT-Net在甲状腺结节超声图像分割方面表现出色，具有实际临床应用潜力
...
TL;DR: X-Slim是一种无需训练、基于缓存的扩散模型加速框架，通过跨时间步、结构和空间的三级缓存冗余利用，实现显著的速度提升和感知质量保持。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但计算开销随步数、模型深度和序列长度线性增长。现有缓存方法存在固有权衡：激进的时间步重用能大幅加速但容易损害保真度，而块级或令牌级重用更安全但计算节省有限。

Method: 提出X-Slim统一框架，采用双阈值控制器将缓存过程分为"推送-抛光"两阶段：首先在时间步级别进行重用直到预警线，然后切换到轻量级的块级和令牌级刷新来抛光剩余冗余，当跨越临界线时触发完整推理重置累积误差。每个级别使用上下文感知指标决定何时何地缓存。

Result: 在多种任务上推进了速度-质量前沿：在FLUX.1-dev上延迟降低4.97倍，HunyuanVideo上降低3.52倍且感知损失最小；在DiT-XL/2上实现3.13倍加速，FID比先前方法提升2.42。

Conclusion: X-Slim是首个统一利用时间步、结构和空间缓存冗余的框架，通过智能的推送-抛光策略在保持生成质量的同时实现显著加速，为扩散模型的高效推理提供了有效解决方案。
...
TL;DR: Scone是一个统一的理解-生成方法，通过两阶段训练方案整合了多主体组合与区分能力，在复杂视觉场景中能准确识别和生成正确主体。


<details>
  <summary>Details</summary>
Motivation: 当前基于主题的图像生成方法从单主体发展到多主体组合，但缺乏区分能力——当输入包含多个候选主体时无法准确识别和生成正确主体，这限制了在复杂现实视觉场景中的有效性。

Method: Scone采用统一的理解-生成框架，让理解专家作为语义桥梁，传达语义信息并指导生成专家在保持主体身份的同时最小化干扰。采用两阶段训练方案：第一阶段学习组合能力，第二阶段通过语义对齐和基于注意力的掩码增强区分能力。

Result: 实验表明Scone在两个基准测试中，在组合和区分任务上都优于现有的开源模型。同时提出了SconeEval基准，用于评估不同场景下的组合和区分能力。

Conclusion: Scone成功整合了组合和区分能力，通过统一的理解-生成框架和两阶段训练方案，在复杂视觉场景中实现了更准确的主题驱动图像生成。模型、基准和训练数据均已开源。
...
TL;DR: Patchify是一个无需微调的patch-wise图像检索框架，通过将数据库图像分割成结构化patch并与查询全局描述符进行局部特征匹配，实现了高性能、可扩展且可解释的实例级图像检索。


<details>
  <summary>Details</summary>
Motivation: 实例级图像检索面临尺寸、位置和外观变化的挑战，现有方法在准确性、可扩展性和可解释性方面存在不足，需要一种无需微调就能实现高精度且能提供空间定位信息的检索方法。
...
Conclusion: Patchify提供了一个简单有效的patch-wise检索框架，无需微调即可实现高性能、可扩展和可解释的实例级图像检索。LocScore作为定位感知指标，为理解和改进检索行为提供了有价值的诊断工具。
...
### [98] [D3D-VLP: Dynamic 3D Vision-Language-Planning Model for Embodied Grounding and Navigation](https://arxiv.org/abs/2512.12622)
...
Method: 提出动态3D视觉-语言-规划模型(D3D-VLP)，包含：1) 动态3D思维链统一规划、定位、导航和问答；2) 协同学习策略通过掩码自回归损失从大规模部分标注混合数据中学习。

Result: 在多个基准测试中达到SOTA：视觉语言导航(R2R-CE, REVERIE-CE, NavRAG-CE)、目标导航(HM3D-OVON)、任务导向顺序定位导航(SG3D)。真实世界移动操作实验进一步验证有效性。

Conclusion: D3D-VLP通过统一框架和协同学习策略，成功解决了具身智能中可解释性与性能的平衡问题，为真实世界应用提供了有效解决方案。
...
TL;DR: 提出RoPar方法，利用视频中可信的身体部位数据通过部位感知掩码自回归模型增强运动生成，解决视频中人体部位缺失问题


<details>
  <summary>Details</summary>
Motivation: 从大规模网络视频中提取人体运动可以解决角色动画数据稀缺问题，但许多视频帧中人体部位因离屏拍摄或遮挡而不可见，这带来了两难：丢弃任何部位缺失的数据会限制规模和多样性，而保留则会损害数据质量和模型性能

Method: 1) 将人体分解为五个部位，检测视频帧中清晰可见的部位作为"可信部位"；2) 通过提出的部位感知变分自编码器将可信部位编码为潜在标记；3) 提出鲁棒的部位级掩码生成模型来预测被掩码的可信部位，同时忽略噪声部位

Result: 实验结果表明，该方法在干净和噪声数据集上都成功超越了基线方法，在运动质量、语义一致性和多样性方面表现更优。同时贡献了包含约20万真实世界运动序列的新基准K700-M

Conclusion: 提出的RoPar方法通过利用视频中的可信部位数据，有效解决了人体运动提取中部位缺失的问题，在保持数据规模和多样性的同时提升了运动生成质量
...
<details>
  <summary>Details</summary>
Motivation: 虽然大型多模态模型中的显式推理机制在语言和视觉任务中已被证明有效，但在3D领域的扩展仍然不足。需要开发能够将高层语义推理与底层3D空间生成紧密结合的框架。

Method: CoRe3D采用空间基础推理表示，将3D潜在空间分解为局部化区域，使模型能够以组合和程序化的方式对几何进行推理。通过紧密耦合语义链式思维推理与结构化空间推理来实现。

Result: CoRe3D生成的3D输出表现出强大的局部一致性和与语言描述的忠实对齐，在3D理解和生成任务中取得了显著效果。

Conclusion: 该研究证明了在3D多模态模型中引入显式推理机制的重要性，CoRe3D框架为3D理解和生成任务提供了有效的统一解决方案，实现了语义推理与空间生成的紧密集成。
...
Method: CARe方法：1) 裁剪操作利用视网膜生理结构从目标图像裁剪子图像，使其视场与源图像大致对齐；2) 对齐模块采用双拟合策略，结合RANSAC算法和多项式坐标拟合
...
TL;DR: CogDoc提出了一种从粗到细的思维框架，通过"快速阅读"和"专注思考"两阶段处理文档，解决了长文档处理与细粒度细节捕捉之间的权衡问题。
...
Method: 提出CogDoc统一框架，模仿人类认知过程：1）低分辨率"快速阅读"阶段进行可扩展的信息定位；2）高分辨率"专注思考"阶段进行深度推理。采用直接强化学习方法进行后训练，避免SFT初始化带来的策略冲突。

Result: 7B参数模型在同类参数规模中达到最先进性能，在具有挑战性的视觉丰富文档基准测试中显著超越更大的专有模型（如GPT-4o）。直接RL方法优于RL+SFT初始化。

Conclusion: CogDoc框架有效解决了文档推理中的可扩展性与保真度权衡问题，通过模仿人类认知的从粗到细处理流程，结合优化的后训练策略，实现了高效的文档理解能力。
...
TL;DR: Lemon是一个统一的Transformer架构，通过将3D点云补丁和语言标记作为单一序列联合处理，解决了现有大型多模态模型在3D理解中的碎片化架构和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型在扩展到3D理解时面临独特挑战：点云数据稀疏且不规则，现有模型依赖具有模态特定编码器的碎片化架构，训练流程通常存在不稳定性和可扩展性差的问题。

Method: 采用统一的Transformer架构，将3D点云补丁和语言标记作为单一序列联合处理；开发了结构化补丁化和标记化方案以保留空间上下文；采用三阶段训练课程，从对象级识别逐步构建到场景级空间推理能力。

Result: Lemon在全面的3D理解和推理任务中建立了新的最先进性能，包括对象识别、描述和3D场景中的空间推理，同时在模型大小和训练数据增加时展现出稳健的扩展特性。

Conclusion: Lemon为推进现实世界应用中的3D空间智能提供了一个统一的基础，通过早期空间-语言融合、消除冗余编码器和改进参数效率，解决了现有3D多模态模型的局限性。
...
### [104] [InteracTalker: Prompt-Based Human-Object Interaction with Co-Speech Gesture Generation](https://arxiv.org/abs/2512.12664)
*Sreehari Rajan,Kunal Bhosikar,Charu Sharma*

Main category: cs.CV

TL;DR: InteracTalker是一个统一语音手势和物体交互生成的新框架，通过多阶段训练学习统一嵌入空间，使用自适应融合策略解决异质条件信号不平衡问题，在两项任务上都优于现有方法。
...
Method: 1) 多阶段训练学习统一的运动、语音和提示嵌入空间；2) 构建增强的人类-物体交互数据集；3) 使用广义运动适应模块进行独立训练；4) 提出自适应融合策略动态重新加权扩散采样中的条件信号。

Result: InteracTalker成功统一了之前分离的任务，在语音手势生成和物体交互合成方面都优于先前方法，超越了专注于手势的扩散方法，产生了高度真实、物体感知的全身运动。

Conclusion: 该框架通过集成语音和物体交互条件，实现了更真实、灵活和可控的全身体运动生成，为交互式数字体验提供了更自然的解决方案。
...
### [105] [Adapting Multimodal Foundation Models for Few-Shot Learning: A Comprehensive Study on Contrastive Captioners](https://arxiv.org/abs/2512.12824)
*N. K. B. M. P. K. B. Narasinghe,Uthayasanker Thayasivam*

Main category: cs.CV

TL;DR: 本文系统研究了CoCa多模态基础模型在少样本图像分类任务上的适应策略，从训练免费的混合原型到LoRA深度参数适应，揭示了数据增强的"增强分歧"现象，并提供了针对数据稀缺的优化配置参考。


<details>
  <summary>Details</summary>
Motivation: 虽然CoCa等大规模多模态基础模型在零样本迁移方面表现出色，但它们在极端数据稀缺（少样本学习）的下游任务中的适应能力尚未得到充分探索。现有研究主要关注CLIP等双编码器架构，缺乏对CoCa独特潜在空间如何响应参数高效微调的理解。

Method: 系统评估了从训练免费的混合原型到通过低秩适应（LoRA）进行深度参数适应的层次化策略。研究了数据增强的"增强分歧"现象，并探索了结合监督对比（SupCon）损失的混合目标函数。分析了训练配置对数据稀缺的敏感性。

Result: 发现强数据增强在低样本设置下会降低线性探测性能，但对稳定LoRA微调至关重要。SupCon损失在不同样本数量下相比标准交叉熵损失能带来一致的性能提升。提供了针对数据稀缺的扩展正则化、秩和采样策略的实证参考设置。

Conclusion: 本研究为生成-对比混合基础模型的高效适应提供了系统指导，揭示了数据增强在少样本学习中的双重作用，并提供了针对数据稀缺环境的优化配置方案，有助于推动这类模型在实际应用中的部署。
...
TL;DR: 提出CAL框架解决开放世界深度伪造溯源中的置信度偏斜和未知伪造类型数量假设问题，通过置信感知一致性正则化和非对称置信增强提升性能，结合动态原型剪枝自动估计未知类型数量。


<details>
  <summary>Details</summary>
Motivation: 现有开放世界深度伪造溯源方法存在两个关键限制：1) 置信度偏斜导致对新型伪造的伪标签不可靠，造成训练偏差；2) 不现实地假设未知伪造类型的数量是已知的。

Method: 提出置信感知非对称学习框架，包含置信感知一致性正则化和非对称置信增强两个组件。前者通过基于归一化置信度的动态损失缩放缓解伪标签偏差，后者通过选择性学习分别校准已知和新型伪造的置信度。还引入动态原型剪枝策略自动估计新型伪造类型数量。

Result: 在标准开放世界深度伪造溯源基准和新扩展的包含高级操纵的基准上，CAL始终优于先前方法，在已知和新型伪造溯源上都达到了新的最先进性能。
...
TL;DR: 提出PCSR方法，通过渐进式条件尺度-偏移重校准来在线调整Transformer模型的自注意力机制，以应对测试时的域适应问题。


<details>
  <summary>Details</summary>
Motivation: 当Transformer模型应用于新目标域时，其自注意力模块的Query、Key、Value特征会发生显著变化，导致性能大幅下降，需要解决这一在线测试时域适应问题。

Method: 提出渐进式条件尺度-偏移重校准(PCSR)方法：将域适应视为渐进域偏移分离过程，在每个Transformer层学习域分离网络提取域偏移特征，通过因子生成网络预测尺度-偏移参数来重校准自注意力。

Result: 在ImageNet-C数据集上，PCSR方法显著提升了在线测试时域适应性能，分类准确率最高提升3.9%。

Conclusion: PCSR方法通过轻量级的域分离网络和因子生成网络在线调整自注意力机制，有效解决了Transformer模型在测试时域适应中的性能下降问题。
...
TL;DR: MADTempo是一个视频检索框架，通过时序搜索机制和基于谷歌图片搜索的回退模块，解决多事件时序依赖和罕见视觉概念查询的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索系统难以建模多个事件之间的时序依赖关系，也无法有效处理包含未见或罕见视觉概念的查询。随着在线平台视频内容的快速增长，需要能够理解复杂事件时序结构的检索系统。

Method: 1. 时序搜索机制：通过聚合连续视频片段的相似度分数来捕捉事件级连续性，实现对多事件查询的连贯检索。2. 基于谷歌图片搜索的回退模块：利用外部网络图像扩展查询表示，弥补预训练视觉嵌入的不足，提高对分布外查询的鲁棒性。

Result: MADTempo框架提升了视频检索系统的时序推理和泛化能力，为大规模视频语料库中实现更具语义感知和自适应的检索铺平了道路。

Conclusion: 该研究提出的MADTempo框架通过统一时序搜索与网络规模视觉定位，有效解决了视频检索中的时序依赖建模和罕见概念查询问题，推动了视频检索系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: CLIP在全局视觉文本对齐方面表现良好，但在细粒度任务上表现不佳，即使使用详细描述进行微调也难以达到理想效果。需要一种能够实现多层次文本粒度与视觉区域对齐的方法。

Method: 提出β-CLIP多粒度文本条件对比学习框架，使用交叉注意力动态池化图像块生成上下文视觉嵌入，并引入β-上下文对比对齐损失(β-CAL)来平衡严格查询匹配与宽松图像内上下文化。

Result: 在Urban1K上达到91.8% T2I和92.3% I2T的R@1，在FG-OVD(Hard)上达到30.9%，在没有使用困难负样本训练的方法中达到最先进水平。

Conclusion: β-CLIP为密集视觉语言对应建立了鲁棒、自适应的基准，显著提升了细粒度对齐能力，代码和模型已开源。
...
### [110] [Unified Interactive Multimodal Moment Retrieval via Cascaded Embedding-Reranking and Temporal-Aware Score Fusion](https://arxiv.org/abs/2512.12935)
...
<details>
  <summary>Details</summary>
Motivation: 视频内容爆炸式增长急需高效的多模态时刻检索系统，但现有方法面临三个关键挑战：固定权重融合策略无法处理跨模态噪声和模糊查询；时序建模难以捕捉连贯事件序列并惩罚不现实的间隔；系统需要手动模态选择，降低可用性。

Method: 1. 级联双嵌入管道：结合BEIT-3和SigLIP进行广泛检索，通过BLIP-2重排序平衡召回率和精度；2. 时序感知评分机制：通过束搜索应用指数衰减惩罚大时间间隔，构建连贯事件序列而非孤立帧；3. 智能体引导查询分解：使用GPT-4o自动解释模糊查询，分解为模态特定子查询（视觉/OCR/ASR），执行自适应分数融合消除手动模态选择。

Result: 定性分析表明，该系统能有效处理模糊查询，检索时序连贯的序列，并动态调整融合策略，提升了交互式时刻搜索能力。

Conclusion: 提出的统一多模态时刻检索系统通过创新的级联检索、时序建模和智能查询分解，显著提升了处理模糊查询、构建连贯事件序列和自适应融合的能力，推动了交互式时刻搜索技术的发展。
...
TL;DR: 提出基于内容自适应的运动对齐框架CAMA，通过两阶段流引导可变形扭曲、多参考质量感知策略和无训练运动下采样模块，显著提升端到端视频压缩性能


<details>
  <summary>Details</summary>
Motivation: 当前端到端视频压缩框架缺乏内容特异性适应，导致压缩性能不理想。需要开发能够根据内容特性自适应调整编码策略的方法来提升压缩效率

Method: 1) 两阶段流引导可变形扭曲机制：通过粗到细的偏移预测和掩码调制精化运动补偿；2) 多参考质量感知策略：基于参考质量调整失真权重，应用于分层训练减少误差传播；3) 无训练模块：根据运动幅度和分辨率下采样帧以获取平滑运动估计

Result: 在标准测试数据集上，CAMA框架相比基线模型DCVC-TCM实现24.95%的BD-rate（PSNR）节省，同时优于复现的DCVC-DC和传统编解码器HM-16.25

Conclusion: 提出的内容自适应运动对齐框架通过精确的特征对齐、质量感知分层训练和运动自适应下采样，显著提升了端到端视频压缩性能，为内容自适应视频编码提供了有效解决方案
...
TL;DR: 提出基于四方向深度图像融合的3D人体姿态分析系统，通过全局与精细配准的分层匹配恢复3D人体模型并自动估计脊柱中心线，无需训练数据或复杂神经网络。


<details>
  <summary>Details</summary>
Motivation: 现有多图像人体重建方法需要昂贵设备和复杂流程，而单图像方法因遮挡和视角限制难以准确估计脊柱中心线等内部结构。需要弥补多图像方法的不足并解决单图像方法的局限性。

Method: 1) 集成四个方向的深度图像恢复3D人体模型；2) 采用全局和精细配准的分层匹配处理噪声和遮挡；3) 应用自适应顶点缩减保持网格分辨率和形状可靠性；4) 使用细节层次集成确保脊柱角度估计的准确性和稳定性。

Result: 实现了不依赖训练数据或复杂神经网络模型的高精度3D脊柱配准估计，验证显示匹配质量得到改善。

Conclusion: 提出的3D人体姿态分析系统能够有效恢复3D人体模型并自动估计脊柱中心线，解决了现有方法的局限性，在脊柱角度估计方面表现出高精度和稳定性。
...
<details>
  <summary>Details</summary>
Motivation: CLIP在零样本分类中表现优秀但对对抗攻击高度脆弱。现有对抗微调方法主要关注干净样本和对抗样本的预测对数匹配，忽视了不确定性校准，可能损害零样本泛化能力。研究发现对抗扰动不仅降低准确性，还会抑制不确定性，导致严重的校准错误和不可靠的过度自信，这揭示了超越鲁棒性的可靠性差距。

Method: 通过将CLIP输出重参数化为狄利克雷分布的浓度参数，提出统一表示来捕获相对语义结构和预测置信度大小。设计新的对抗微调目标，在扰动下整体对齐这些分布，超越单一对数锚定，恢复校准的不确定性。

Result: 在多个零样本分类基准测试中，该方法有效恢复了校准的不确定性，实现了有竞争力的对抗鲁棒性，同时保持了干净样本的准确性。

Conclusion: 该方法通过考虑预测准确性和不确定性对齐的新型对抗微调目标，成功解决了CLIP在对抗攻击下的不确定性抑制问题，在保持零样本泛化能力的同时提升了模型的可靠性和鲁棒性。
...
TL;DR: GenieDrive：基于4D占据表示的物理感知驾驶视频生成框架，通过压缩占据表示和互控注意力机制，实现高质量、物理一致的驾驶视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用单一扩散模型直接将驾驶动作映射到视频，导致学习困难且输出物理不一致。需要一种物理感知的驾驶世界模型来支持驾驶规划、OOD数据合成和闭环评估。

Method: 1) 首先生成4D占据作为物理感知基础；2) 提出VAE将高分辨率占据编码为潜在三平面表示，将潜在尺寸减少到先前方法的58%；3) 引入互控注意力(MCA)精确建模控制对占据演化的影响；4) 端到端联合训练VAE和预测模块；5) 在视频生成中使用归一化多视角注意力，以4D占据为指导生成多视角驾驶视频。

Result: 预测mIoU提升7.2%，推理速度达41 FPS，仅使用3.47M参数。视频质量显著改善，FVD降低20.7%。实验表明GenieDrive能够实现高度可控、多视角一致且物理感知的驾驶视频生成。

Conclusion: GenieDrive通过4D占据表示、高效的压缩机制和互控注意力，成功解决了现有驾驶视频生成方法的物理不一致问题，实现了高质量、物理感知的驾驶视频生成，为驾驶规划、数据合成和评估提供了有效工具。
...
TL;DR: GTR-Turbo：一种高效的多模态智能体强化学习方法，通过合并训练过程中的检查点权重作为"免费"教师模型，无需依赖昂贵的特权模型，显著提升性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的多模态智能体多轮强化学习方法存在稀疏奖励和长时程信用分配问题。现有方法通过查询昂贵的特权教师模型提供步骤级反馈，但成本高、实用性差且难以复现。

Method: 提出GTR-Turbo方法：在持续RL训练过程中合并检查点的权重，使用这个合并模型作为"免费"教师，通过监督微调或软对数蒸馏指导后续RL训练。消除了对特权VLM的依赖，缓解了"熵崩溃"问题。

Result: 在多种视觉智能体任务中，GTR-Turbo将基线模型的准确率提升10-30%，相比GTR减少50%的训练时间和60%的计算成本，同时保持训练稳定性。

Conclusion: GTR-Turbo提供了一种高效、实用的多模态智能体强化学习方案，无需依赖昂贵的特权模型，显著降低了计算成本并提升了性能，具有更好的实用性和可复现性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态基准测试存在局限性：模态覆盖不完整、仅限于文本中心输出、模态间相互依赖和互补性弱。需要建立一个全面的全模态评估框架来推动下一代多模态架构的发展。

Method: 1) 构建统一的全模态基准测试FysicsWorld，支持图像、视频、音频和文本之间的双向输入输出；2) 提出跨模态互补性筛选(CMCS)策略，集成在系统数据构建框架中，生成用于口语交互和融合依赖跨模态推理的全模态数据；3) 从40多个高质量来源收集数据，涵盖16个主要任务和3268个样本。

Result: 通过对30多个最先进基线的全面评估（包括MLLMs、模态特定模型、统一理解-生成模型和全模态语言模型），FysicsWorld揭示了不同模型在理解、生成和推理方面的性能差异和局限性。

Conclusion: FysicsWorld为评估和推进下一代全模态架构建立了统一的基础和强基线，填补了当前多模态基准测试的空白，支持全面的任意到任意模态评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有变化检测方法依赖监督学习，性能受数据集限制，标注成本高，且只能检测预定义类别，泛化能力差。随着视觉基础模型（SAM2、CLIP）的出现，为放松这些限制提供了新机会

Method: 基于冻结的SAM2和CLIP构建无监督开放词汇变化检测框架，引入轻量级特征对齐模块连接SAM2的空间细节表示和CLIP的语义先验，实现高分辨率语义感知变化估计，并采用简化的后处理流程抑制噪声和伪变化

Result: 在多个公开BCD和SCD基准测试中，UniVCD表现出稳定强大的性能，在F1和IoU等关键指标上达到或超过现有开放词汇变化检测方法
...
TL;DR: Fast-2DGS：一种轻量级框架，通过深度高斯先验和属性回归网络实现高效高斯图像表示，单次前向传播即可获得高质量重建，显著降低计算成本。
...
Method: 提出Fast-2DGS框架：1）深度高斯先验：作为条件网络捕获不同复杂度下高斯基元的空间分布；2）属性回归网络：预测密集高斯属性。这种解耦架构实现单次前向传播的高质量重建，仅需最小微调。
...
### [119] [Harmonizing Generalization and Specialization: Uncertainty-Informed Collaborative Learning for Semi-supervised Medical Image Segmentation](https://arxiv.org/abs/2512.13101)
*Wenjing Lu,Yi Hong,Yang Yang*

Main category: cs.CV

TL;DR: UnCoL：一种基于不确定性指导的双教师协作学习框架，用于半监督医学图像分割，通过结合基础模型的通用知识和任务特定知识，在有限标注下实现接近全监督的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型在医学图像分割中表现出强大的泛化能力，但在有限标注或罕见病理变化下，由于通用先验与任务特定需求不匹配，难以泛化到专门的临床任务。

Method: 提出UnCoL双教师框架：1) 从冻结的基础模型蒸馏视觉和语义表示以传递通用知识；2) 维护逐步适应的教师模型捕获细粒度的任务特定表示；3) 通过预测不确定性自适应调节伪标签学习，抑制不可靠监督并稳定模糊区域学习。

Result: 在多种2D和3D分割基准测试中，UnCoL持续优于最先进的半监督方法和基础模型基线，在显著减少标注需求的情况下实现接近全监督的性能。

Conclusion: UnCoL通过双教师协作学习和不确定性指导，有效平衡了泛化与专业化，为半监督医学图像分割提供了一种高效解决方案，显著降低了标注成本。
...
TL;DR: L-STEC方法通过引入长时依赖和融合像素域空间上下文，显著提升神经视频压缩性能，相比DCVC-TCM节省37.01%码率（PSNR）和31.65%码率（MS-SSIM），超越VTM-17.0和DCVC-FM达到SOTA。
...
Method: 提出长时时空增强上下文（L-STEC）方法：1) 使用LSTM扩展参考链以捕获长时依赖；2) 从像素域引入扭曲空间上下文；3) 通过多感受野网络融合时空信息以更好保留参考细节。

Result: L-STEC显著提升压缩性能，相比DCVC-TCM在PSNR上节省37.01%码率，在MS-SSIM上节省31.65%码率，超越VTM-17.0和DCVC-FM，建立新的最先进性能。

Conclusion: 通过扩展参考链捕获长时依赖并融合像素域空间上下文，L-STEC方法能有效丰富上下文信息，显著提升神经视频压缩性能，解决现有方法在长时依赖和纹理保留方面的局限性。
...
### [121] [Diffusion-Based Restoration for Multi-Modal 3D Object Detection in Adverse Weather](https://arxiv.org/abs/2512.13107)
*Zhijian He,Feifei Liu,Yuwei Li,Zhanpeng Liu,Jintao Cheng,Xieyuanli Chen,Xiaoyu Tang*

Main category: cs.CV

TL;DR: DiffFusion：基于扩散模型的鲁棒多模态3D目标检测框架，通过图像和点云恢复以及双向自适应融合，提升恶劣天气条件下的检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态3D目标检测在恶劣天气条件下效果受限，主要原因是天气引起的图像失真和不同模态数据之间的不对齐问题。

Method: 提出DiffFusion框架：1) Diffusion-IR模块用于恢复受天气影响的图像；2) Point Cloud Restoration (PCR)模块利用图像目标线索补偿受损的LiDAR数据；3) 双向自适应融合与对齐模块(BAFAM)实现动态多模态融合和双向鸟瞰图对齐。

Result: 在三个公开数据集上实验表明，DiffFusion在恶劣天气条件下实现了最先进的鲁棒性，同时保持了强大的干净数据性能。在真实世界DENSE数据集上的零样本结果进一步验证了其泛化能力。

Conclusion: DiffFusion通过扩散模型恢复和自适应跨模态融合，有效解决了恶劣天气下的多模态3D目标检测问题，具有优异的鲁棒性和泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在多个领域表现出色，但在自动驾驶中生成细粒度3D感知和预测输出的应用仍未被充分探索。现有方法在统一处理空间理解、感知、预测和规划方面存在不足。

Method: 提出DrivePI框架，整合点云、多视角图像和语言指令，通过端到端优化并行执行空间理解、3D占用、占用流预测和规划。开发数据引擎生成文本-占用和文本-流问答对用于4D空间理解。

Result: 仅使用0.5B参数的Qwen2.5作为骨干模型，DrivePI在多个任务上超越现有VLA模型和专用VA模型：在nuScenes-QA上比OpenDriveVLA-7B高2.5%平均准确率，碰撞率降低70%；在3D占用、占用流预测和规划任务上均优于专用模型。

Conclusion: DrivePI证明了统一的多模态大语言模型框架在自动驾驶中的有效性，能够同时处理多种感知、预测和规划任务，且在小模型规模下也能取得优异性能。
...
TL;DR: 提出对比分析框架，从两个数据集中分离共享的生成因子和特定于单个数据集的显著因子，适用于GAN和扩散模型


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注条件操作和分离表示学习，但缺乏对对比分析的研究。对比分析旨在从两个数据集中分离共享因子和特定因子，仅使用数据集信号而非属性监督

Method: 提出新颖的对比分析框架，适用于GAN和扩散模型，通过定义新的学习策略和损失函数来确保共享因子和显著因子的有效分离，同时保持高质量生成

Result: 在多种数据集（人脸、动物图像、医学扫描）上评估，相比现有方法展现出更好的分离能力和图像合成质量

Conclusion: 该框架成功解决了对比分析问题，能够有效分离共享和显著生成因子，为仅使用数据集信号的图像分析提供了新方法
...
### [124] [DePT3R: Joint Dense Point Tracking and 3D Reconstruction of Dynamic Scenes in a Single Forward Pass](https://arxiv.org/abs/2512.13122)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态场景密集3D点跟踪方法通常依赖成对处理、需要已知相机姿态或假设时间顺序，限制了灵活性和适用性。同时，大规模无姿态图像集合的3D重建技术进展为动态场景理解的统一方法提供了机会。

Method: 通过强大的骨干网络提取深度时空特征，并使用密集预测头回归像素级映射，实现多任务学习。框架无需相机姿态，在单次前向传播中同时完成密集点跟踪和3D重建。

Result: 在多个涉及动态场景的挑战性基准测试中验证了DePT3R，展示了强大的性能和相对于现有最先进方法显著的内存效率提升。

Conclusion: DePT3R为动态场景理解提供了一种统一、高效的方法，无需相机姿态信息，在适应性和效率方面具有显著优势，特别是在快速变化的动态环境中。
...
TL;DR: 提出Intrinsic Image Fusion方法，从多视角图像重建高质量物理材质，通过扩散先验和鲁棒优化解决材质重建的欠约束问题


<details>
  <summary>Details</summary>
Motivation: 材质重建高度欠约束，传统基于分析-合成的方法需要昂贵的路径追踪且噪声大，需要更好的约束条件

Method: 结合单视角扩散先验生成候选分解，拟合低维参数函数，提出鲁棒优化框架进行软预测选择和置信度多视角内点集融合，最后用逆路径追踪优化参数

Result: 在合成和真实场景上超越现有方法，实现材质解耦，产生清晰干净的重建结果，适合高质量重光照

Conclusion: Intrinsic Image Fusion方法通过融合单视角先验和多视角一致性，有效解决材质重建的欠约束问题，获得高质量的物理材质重建
...
<details>
  <summary>Details</summary>
Motivation: 临床级病理AI发展受限于高质量标注数据的稀缺性，现有生成模型存在语义不稳定和形态学幻觉问题，影响诊断可靠性。

Method: 提出CRAFTS框架，采用双阶段训练策略，在约280万图像-文本对上进行训练，引入新颖的对齐机制抑制语义漂移，确保生物学准确性。

Result: 模型成功生成30种癌症类型的多样化病理图像，质量通过客观指标和病理学家评估验证；CRAFTS增强的数据集在分类、跨模态检索、自监督学习和视觉问答等临床任务中提升性能；结合ControlNet可实现从核分割掩码和荧光图像等输入精确控制组织结构。
...
Method: 提出SAVEBench配对音频-视觉数据集，并训练SAVE模型（Schrodinger Audio-Visual Editor），使用端到端流匹配模型和Schrodinger Bridge实现从源到目标音频-视觉混合的直接传输

Result: SAVE模型能够移除目标对象的音频和视觉内容，同时保留其余内容，在时间同步和音频-视觉语义对应方面优于音频编辑器和视频编辑器的成对组合

Conclusion: 提出的SAVE模型解决了音频-视觉联合编辑的数据和建模挑战，实现了更好的跨模态对齐和内容一致性
...
### [128] [Cross-Level Sensor Fusion with Object Lists via Transformer for 3D Object Detection](https://arxiv.org/abs/2512.12884)
*Xiangzhong Liu,Jiajie Zhang,Hao Shen*

Main category: cs.CV

TL;DR: 提出一种用于3D目标检测的端到端跨层级融合方法，将高度抽象的目标列表信息与原始相机图像通过Transformer结合，在nuScenes数据集上显著超越视觉基线方法。


<details>
  <summary>Details</summary>
Motivation: 汽车传感器融合系统中，智能传感器和V2X模块通常只提供处理后的目标列表而非原始数据。传统方法分别处理原始数据后在目标层级融合，效率较低。需要一种能直接融合抽象目标列表和原始图像的方法。

Method: 1) 提出端到端跨层级融合概念，将目标列表作为去噪查询输入Transformer，与可学习查询一起传播；2) 基于目标列表的位置和尺寸先验设计可变形高斯掩码，集成到Transformer解码器中，引导注意力到目标区域并加速收敛；3) 提出从真实边界框生成伪目标列表的方法，模拟状态噪声和误检漏检。

Result: 在nuScenes数据集上，该方法相比视觉基线方法取得了显著的性能提升。同时展示了该方法对模拟目标列表的不同噪声水平和真实检测器的泛化能力。

Conclusion: 这是首个进行跨层级融合的工作，成功将抽象目标列表与原始图像融合用于3D目标检测，为解决汽车传感器融合系统中不同层级数据融合问题提供了有效方案。
...
TL;DR: 提出LINA框架，通过因果干预解决扩散模型在物理对齐和OOD指令跟随上的问题，利用提示特定干预和因果感知去噪调度


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成上取得显著成功，但仍存在物理对齐和分布外指令跟随的问题。这些问题源于模型未能学习因果方向和因果因子解耦以进行新颖重组。

Method: 引入因果场景图和物理对齐探测数据集进行诊断干预分析。基于分析结果提出LINA框架：学习预测提示特定干预，采用提示和视觉潜在空间的目标引导，以及重新分配的因果感知去噪调度。

Result: 在图像和视频扩散模型中同时实现了物理对齐和OOD指令跟随，在具有挑战性的因果生成任务和Winoground数据集上达到最先进性能。

Conclusion: 通过因果干预方法有效解决了扩散模型在物理对齐和OOD指令跟随方面的局限性，为因果感知生成提供了新框架。
...
### [130] [Revisiting 2D Foundation Models for Scalable 3D Medical Image Classification](https://arxiv.org/abs/2512.12887)
...
TL;DR: AnyMC3D是一个从2D基础模型扩展而来的可扩展3D医学图像分类框架，通过轻量级插件实现多任务适配，在12个任务基准上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 解决当前医学基础模型研究的三个关键缺陷：数据制度偏差、次优适配和任务覆盖不足，旨在开发一个可扩展的3D分类框架

Method: 从2D基础模型适配到3D分类，采用冻结主干网络+轻量级插件（约1M参数/任务），支持多视图输入、像素级监督和可解释热图生成

Result: 在涵盖不同病理、解剖和模态的12个任务基准上建立全面评估，显示：1）有效适配对释放FM潜力至关重要；2）通用FM经适当适配可匹配医学专用FM；3）2D方法在3D分类上优于3D架构；在VLM3D挑战中获得第一名
...
TL;DR: 本文提出了一种针对人脸检索系统的身份遗忘方法，通过分散目标身份的嵌入向量来防止其被重新识别，同时保持其他身份的检索性能。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统虽然能有效进行身份识别，但存在严重的隐私问题，可能被用于未经授权的身份追踪。现有的机器遗忘方法在人脸检索场景下适用性有限，特别是对于基于嵌入的现代识别模型。

Method: 评估了多种现有的近似类别遗忘方法（如随机标签、梯度上升、边界遗忘等），并提出了一种简单有效的基于分散的遗忘方法。该方法通过在超球面上分散目标身份的嵌入向量，防止形成紧凑的身份聚类，同时保持嵌入空间的判别结构。

Result: 在标准基准测试（VGGFace2、CelebA）上的大量实验表明，该方法在实现优越的遗忘效果的同时，保持了检索系统的实用性。

Conclusion: 本文研究了人脸检索系统中的身份遗忘问题，提出了一种有效的分散式遗忘方法，能够在保护特定身份隐私的同时，维持系统对其他身份的检索性能，为人脸识别系统的隐私保护提供了新的解决方案。
...
TL;DR: 提出基于预测样本分配（PSA）的SCOOD框架，通过双阈值三元样本分配策略提高ID/OOD样本集纯度，结合概念对比表示学习和重训练策略，在标准基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有SCOOD方法主要采用聚类式ID样本过滤策略从未标注数据中选择干净ID样本，将剩余样本作为辅助OOD数据，这不可避免地引入了大量噪声样本。需要解决训练数据纯度不足的问题。

Method: 1. 基于预测能量分数的双阈值三元样本分配策略，将不确定的未标注数据分配到丢弃样本集，显著提高所选ID和OOD样本集的纯度；2. 概念对比表示学习损失，在表示空间中进一步扩大ID和OOD样本之间的距离；3. 重训练策略帮助模型充分拟合选定的辅助ID/OOD样本。

Result: 在两个标准SCOOD基准测试上的实验表明，该方法显著优于当前最先进的方法。

Conclusion: 提出的PSA框架通过提高训练样本纯度和增强表示区分性，有效解决了SCOOD中噪声样本问题，取得了显著的性能提升。
...
### [133] [Sharpness-aware Dynamic Anchor Selection for Generalized Category Discovery](https://arxiv.org/abs/2512.12925)
*Zhimao Peng,Enguang Wang,Fei Yang,Xialei Liu,Ming-Ming Cheng*

Main category: cs.CV

TL;DR: 提出LSP和DAS两个模块来改进广义类别发现任务，通过损失锐度惩罚和动态锚点选择减少伪标签噪声，提升聚类精度。
...
Result: 在多个GCD基准测试中实现了最先进的结果，能有效缓解伪标签噪声问题。

Conclusion: 提出的LSP和DAS方法能有效减少伪标签噪声，提高广义类别发现任务的聚类准确性。
...
### [134] [End2Reg: Learning Task-Specific Segmentation for Markerless Registration in Spine Surgery](https://arxiv.org/abs/2512.13402)
...
<details>
  <summary>Details</summary>
Motivation: 当前脊柱手术导航系统依赖侵入性骨锚定标记和放射成像，具有辐射强、工作流程中断等问题。现有无标记RGB-D配准方法依赖弱分割标签，容易传播误差。
...
Conclusion: 该端到端RGB-D配准管道消除了对弱标签和手动步骤的依赖，推动向完全自动化的无标记术中导航迈进。
...
TL;DR: 提出UAGLNet网络，通过不确定性建模指导的全局-局部特征融合，解决遥感图像建筑提取中特征金字塔间隙和全局-局部特征整合不足的问题。
...
Method: 提出UAGLNet：1) 协作编码器：不同阶段采用CNN和Transformer层分别捕获局部和全局语义；2) 中间协作交互块(CIB)：缩小深层网络中局部和全局特征的差距；3) 全局-局部融合(GLF)模块：互补融合全局和局部表示；4) 不确定性聚合解码器(UAD)：显式估计像素级不确定性以增强分割精度。

Result: 大量实验表明，该方法优于其他最先进方法。

Conclusion: 提出的UAGLNet通过不确定性建模指导的全局-局部特征融合，有效解决了遥感图像建筑提取中的挑战，取得了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在风格迁移中难以实现逼真效果，常产生绘画般结果或丢失细节，且无法有效处理原始内容风格和参考风格内容特征的不必要影响。

Method: 利用CLIP图像空间分离和整合内容与风格特征，包含三个组件：可控风格自适应实例归一化(CSAdaIN)用于精确多风格混合，KVS注入用于目标风格整合，以及保持过程一致性的风格迁移一致性目标。

Result: SCAdapter在传统和基于扩散的基准测试中显著优于最先进方法，通过消除DDIM反转和推理阶段优化，推理速度至少比其他基于扩散的方法快2倍。

Conclusion: SCAdapter提供了一种更有效和高效的风格迁移方法，能够实现逼真的风格转换，同时大幅提升推理速度，适用于实际应用。
...
TL;DR: VLCache是一个多模态缓存重用框架，通过复用KV缓存和编码器缓存来避免重复计算，在保持准确性的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 当相同的多模态输入重复出现时，现有的方法需要昂贵的重复计算，这限制了多模态模型的推理效率。需要一种能够有效复用先前计算结果的方法来加速推理过程。

Method: 1) 形式化识别累积重用误差效应并提出最小化非前缀缓存重用误差的方法；2) 分析模型各层的重要性差异，提出动态、层感知的重计算策略来平衡准确性和效率。

Result: VLCache在保持与完全重计算相当的准确性的同时，仅需计算2-5%的token，实现了1.2x-16x的首次token时间(TTFT)加速。该框架已集成到SGLang中，在实际部署中显著提升了推理速度。

Conclusion: VLCache通过智能缓存重用机制有效解决了多模态输入重复计算的问题，在保证准确性的前提下大幅提升了推理效率，为实际部署提供了实用的解决方案。
...
### [138] [DA-SSL: self-supervised domain adaptor to leverage foundational models in turbt histopathology slides](https://arxiv.org/abs/2512.13600)
...
TL;DR: 提出DA-SSL方法，通过自监督域适配将预训练的病理基础模型特征对齐到TURBT领域，无需微调基础模型，用于膀胱癌治疗反应预测


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型在特定癌症类型（如膀胱癌TURBT标本）上存在域偏移问题，因为这些标本包含组织碎片和电灼伪影，且未广泛用于预训练，导致性能受限

Method: 提出轻量级域自适应自监督适配器（DA-SSL），通过自监督学习将预训练病理基础模型特征重新对齐到TURBT领域，不微调基础模型本身，结合多实例学习框架

Result: 在多中心研究中，DA-SSL在五折交叉验证中AUC达0.77±0.04，外部测试准确率0.84，敏感性0.71，特异性0.91，优于现有方法

Conclusion: 轻量级域自适应与自监督学习能有效增强基于病理基础模型的多实例学习流程，适用于临床挑战性组织病理学任务，特别是TURBT标本的膀胱癌治疗反应预测
...
TL;DR: 论文提出AIGI检测中的"收益后冲突"困境：随着数据源多样性增加，检测器性能先提升后下降。作者提出GAPL框架，通过原型学习和两阶段训练解决数据异质性和模型瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有通用AIGI检测器通过聚合多个生成器数据来提升泛化能力，但作者发现存在"收益后冲突"困境：随着数据源多样性增加，检测性能先提升后下降甚至退化。这源于数据层面的严重异质性（真实与合成图像特征分布重叠加剧）和模型层面的瓶颈（固定的预训练编码器无法适应复杂度提升）。

Method: 提出Generator-Aware Prototype Learning (GAPL)框架：1) 学习一组紧凑的典型伪造原型，构建统一、低方差的特征空间以应对数据异质性；2) 采用两阶段训练方案结合Low-Rank Adaptation，增强判别能力同时保留预训练知识，建立更鲁棒的决策边界。

Result: 通过大量实验证明，GAPL在多种GAN和扩散模型生成器上实现了最先进的检测性能，展现出优越的检测准确率和泛化能力。

Conclusion: GAPL有效解决了AIGI检测中的"收益后冲突"困境，通过结构化学习范式和自适应训练策略，在保持预训练知识的同时提升了检测器的鲁棒性和泛化能力。
...
TL;DR: 该论文系统研究了将扩散蒸馏技术应用于开放文本到图像生成，在FLUX.1-lite模型上比较了最先进的蒸馏方法，并提供了实用的部署指南。


<details>
  <summary>Details</summary>
Motivation: 扩散蒸馏在类别条件图像生成中已显著加速，但在开放文本到图像生成中的应用仍不明确。需要系统研究如何将现有蒸馏技术适配到文本到图像生成任务。

Method: 将现有蒸馏方法统一到框架中，在FLUX.1-lite教师模型上适配和比较最先进的蒸馏技术，分析从离散类别标签到自由形式语言提示转换的关键障碍。

Result: 提供了输入缩放、网络架构和超参数的实用指南，开源了实现代码和预训练学生模型，为快速、高保真、资源高效的扩散生成器部署奠定了基础。

Conclusion: 该研究为在实际文本到图像应用中部署快速、高保真、资源高效的扩散生成器建立了坚实基础，代码已在GitHub开源。
...
### [141] [Feedforward 3D Editing via Text-Steerable Image-to-3D](https://arxiv.org/abs/2512.13678)
...
<details>
  <summary>Details</summary>
Motivation: AI生成的3D资产在实际应用中需要易于编辑的能力，现有方法缺乏有效的文本引导编辑功能

Method: 受ControlNet启发，采用两阶段训练方案：基于流匹配训练和直接偏好优化(DPO)，构建可扩展的自动数据生成引擎

Result: 相比竞争方法，Steer3D更忠实遵循语言指令，保持与原始3D资产更好的一致性，速度提升2.4-28.5倍

Conclusion: Steer3D证明只需10万数据即可为预训练的图像到3D生成模型添加文本引导新模态，实现高效可控的3D资产编辑
...
TL;DR: 提出基于光场图像的物体跟踪方法，无需预训练模型，对反射等复杂视觉行为鲁棒，使用视图相关的高斯泼溅作为统一物体表示。


<details>
  <summary>Details</summary>
Motivation: 现有高性能物体跟踪方法依赖预捕获的物体视图构建显式参考模型，限制了只能跟踪已知物体集合，且对复杂外观（如反射）处理不佳。需要一种无需预训练模型、对复杂视觉行为鲁棒的通用物体跟踪方法。

Method: 从光场输入中提取语义和几何特征（使用视觉基础模型），将其转换为视图相关的高斯泼溅作为统一物体表示，支持可微分渲染和姿态优化。还创建了包含挑战性反射物体的光场物体跟踪数据集。

Result: 实验表明，该方法在困难情况下与最先进的基于模型的跟踪器具有竞争力，为机器人系统中的通用物体跟踪铺平了道路。

Conclusion: 提出的基于光场图像的物体跟踪方法不依赖预训练模型，对复杂视觉行为（如反射）鲁棒，在挑战性场景中表现优异，推动了机器人系统中通用物体跟踪的发展。
...
TL;DR: TWLR是一个两阶段可解释性糖尿病视网膜病变评估框架，第一阶段使用视觉语言模型进行DR分级和病变分类，第二阶段通过弱监督语义分割的迭代严重性回归实现病变定位和疾病到健康转换的可视化。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析需要高质量专家标注，但获取像素级标签成本高且耗时；同时深度学习在医学影像中缺乏可解释性，限制了临床采用。需要解决标注效率和可解释性双重挑战。

Method: 两阶段框架：第一阶段使用视觉语言模型整合眼科领域知识，联合执行DR分级和病变分类；第二阶段基于弱监督语义分割的迭代严重性回归框架，通过迭代精炼生成病变显著性图，引导渐进修复机制系统消除病理特征。

Result: 在FGADR、DDR和私有数据集上的实验表明，TWLR在DR分类和病变分割方面均取得有竞争力的性能，同时提供疾病到健康转换的可视化解释。

Conclusion: TWLR为自动化视网膜图像分析提供了一个更可解释且标注效率更高的解决方案，通过弱监督方法实现准确病变定位，并提供疾病严重性回归的可视化解释。
...
Conclusion: JoDiffusion通过同时生成语义一致的图像-标注对，解决了合成数据集生成中的语义不一致和可扩展性问题，为训练高性能语义分割模型提供了有效的解决方案。
...
TL;DR: 提出Next Scene Prediction（NSP）新任务，通过统一框架结合理解与生成模型，实现视频场景的时序因果推理预测


<details>
  <summary>Details</summary>
Motivation: 现有统一模型主要关注文本到视频生成等传统任务，对时序推理潜力探索不足，需要推动统一视频模型向时序和因果推理方向发展

Method: 提出统一框架：Qwen-VL用于理解，LTX用于合成，通过潜在查询嵌入和连接器模块桥接；采用三阶段训练：文本到视频预训练、监督微调、带因果一致性奖励的强化学习（GRPO）

Result: 模型在新构建的大规模NSP数据集上实现最先进性能，提升了通用多模态系统预测未来场景的能力

Conclusion: NSP任务有效推动了统一视频模型向时序因果推理发展，提出的框架在预测未来场景方面表现出色，为通用多模态系统能力提升做出贡献
...
TL;DR: 该研究首次全面评估了空间泛化技术，针对室内人员计数应用，发现基于幅度的预处理和迁移学习能显著提升雷达传感系统的跨环境性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习RF传感的实际部署需要解决空间泛化问题，但现有研究缺乏对多种泛化技术的系统性评估，特别是在室内人员计数这一实际应用场景中。

Method: 系统研究了多种空间泛化方法：基于幅度的统计预处理（Sigmoid加权和阈值归零）、频域滤波、基于自编码器的背景抑制、数据增强策略和迁移学习。使用FMCW MIMO雷达在两个不同布局的环境中收集数据进行评估。

Result: Sigmoid幅度加权在跨环境性能上表现最优，相比基线方法分别降低RMSE 50.1%和MAE 55.2%。数据增强带来额外但有限的改进（MAE提升8.8%）。迁移学习在大空间变化场景中至关重要，使用540个目标域样本时分别降低RMSE 82.1%和MAE 91.3%。

Conclusion: 通过将深度学习模型与基于幅度的预处理和高效迁移学习相结合，能够开发出在空间变化下保持鲁棒精度的雷达传感系统，为实际部署提供了实用方向。
...
TL;DR: SneakPeek是一个基于扩散的自回归框架，用于从初始图像和结构化文本提示生成精确的分步教学视频，通过预测性因果适应、未来引导自强制和多提示条件控制来解决时间一致性和可控性问题。


<details>
  <summary>Details</summary>
Motivation: 教学视频生成在内容创作、教育和人机交互中具有广泛应用前景，但现有视频扩散模型在处理多步骤长序列时难以保持时间一致性和可控性。

Method: 提出SneakPeek框架，包含三个关键创新：1) 预测性因果适应，通过因果模型学习下一帧预测和未来关键帧预测；2) 未来引导自强制，采用双区域KV缓存方案解决推理时的曝光偏差问题；3) 多提示条件控制，提供对多步骤指令的细粒度程序化控制。

Result: 实验结果表明，该方法能够生成时间一致、语义忠实且准确遵循复杂多步骤任务描述的教学视频。

Conclusion: SneakPeek通过创新的预测性因果适应、未来引导自强制和多提示条件控制，有效缓解了时间漂移问题，保持了运动一致性，并实现了交互式视频生成，其中未来提示更新能够动态影响正在进行的流式视频生成。
...
TL;DR: Motus是一个统一的潜在动作世界模型，通过混合Transformer架构整合理解、视频生成和动作三个专家模块，利用光流学习潜在动作，实现多功能统一建模，在仿真和真实场景中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前方法将理解、世界建模和控制分离为孤立模型，这种碎片化阻碍了多模态生成能力的统一，也妨碍了从大规模异构数据中学习。需要一种统一的方法来整合这些功能。
...
TL;DR: 该研究首次全面比较了基于规则的方法、传统机器学习模型和深度学习模型在FMCW MIMO雷达室内人员检测中的性能，发现深度学习在相同环境下精度最高但易受领域偏移影响，而基于规则的方法虽然输出粒度粗但跨环境更稳健。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于系统比较不同方法（规则方法、传统机器学习、深度学习）在FMCW MIMO雷达室内人员检测任务中的性能差异，特别是在面对不同环境布局时的泛化能力和输出粒度之间的权衡。

Method: 在两种不同布局的室内环境中，系统评估了五种方法：1）基于规则的连通分量方法；2）三种传统机器学习模型（k近邻、随机森林、支持向量机）；3）结合卷积神经网络和长短时记忆网络的深度学习模型。研究比较了它们在训练环境和新布局环境中的性能表现。

Result: 在训练环境中，CNN-LSTM模型达到最高精度，传统机器学习模型表现中等。但在新布局环境中，所有基于学习的方法性能显著下降，而基于规则的方法保持稳定。对于人员存在与否的二元检测任务，所有模型在不同布局中都保持高精度。研究揭示了空间泛化性能与输出粒度之间的明显权衡。

Conclusion: 高容量模型能在相同环境中提供细粒度的高精度输出，但对领域偏移敏感；基于规则的方法虽然输出粒度粗，但具有跨环境的稳健性。无论模型类型如何，都存在空间泛化性能与输出粒度之间的权衡关系。
...
TL;DR: 提出双向图像引导概念擦除框架，通过同时抑制有害概念和增强安全替代方案，在概念移除和生成质量之间取得平衡。
...
Conclusion: 提出的双向图像引导概念擦除框架能够有效平衡概念移除效果和生成可用性，为解决文本到图像模型中的不安全生成问题提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 尽管DINOv2等基础模型在视觉地点识别中表现出色，但其高计算成本限制了在资源受限设备上的部署，需要一种更高效的解决方案

Method: 1) 非对称框架：高容量图库模型离线提取特征 + 轻量查询网络在线处理；2) 地理记忆库：利用VPR数据库的地理位置元数据组织图库特征，避免耗尽的k-NN计算；3) 隐式嵌入增强：增强查询网络以建模特征变化

Result: 方法显著降低计算成本，同时超越现有非对称检索技术，为资源受限环境下的VPR建立了新标准

Conclusion: 提出的非对称VPR框架通过地理记忆库和隐式嵌入增强，在保持高性能的同时大幅降低计算需求，适用于资源受限设备部署
...
TL;DR: 提出一个结合检索增强生成（RAG）和动态知识蒸馏的框架，用于解决多模态生物医学视觉语言模型在持续学习中的核心困境：在保持细粒度模态内特征的同时跨越显著模态域差距。


<details>
  <summary>Details</summary>
Motivation: 多模态生物医学视觉语言模型在持续学习中面临核心困境：如何在保持细粒度模态内特征的同时，跨越不同模态之间的显著域差距。现有方法难以平衡这两个需求。

Method: 1）基于PubMed科学论文构建1800万规模的多模态医学检索数据库；2）首次将RAG集成到持续学习中，采用多模态、多层RAG系统提供实时指导；3）提出动态知识蒸馏框架，动态调节参数空间重要性、知识蒸馏粒度和参考数据集分布。

Result: 提出的方法在设计的MGTIL基准测试中，在所有指标上都达到了最先进的性能，能够同时适应显著域转移、保留细微域内特征，并实时学习新颖复杂的医学任务。

Conclusion: 该框架有效解决了多模态生物医学VLMs在持续学习中的核心困境，通过RAG和动态知识蒸馏的结合，在保持细粒度特征的同时成功跨越模态域差距，具有重要的临床价值。
...
TL;DR: 提出多样性正则化器DiRe，通过余弦相似度和欧氏距离减少数据集压缩中的冗余，提升合成数据集的多样性
...
Method: 提出多样性正则化器DiRe，结合余弦相似度和欧氏距离，可作为即插即用组件应用于各种最先进的压缩方法

Result: 在CIFAR-10到ImageNet-1K等多个基准数据集上，添加DiRe正则化器提升了最先进压缩方法的泛化能力和多样性指标

Conclusion: DiRe正则化器能有效减少数据集压缩中的冗余，提升合成数据集的多样性，且具有广泛的适用性
...
TL;DR: ADHint提出了一种基于难度的自适应提示方法，通过在提示比例调度和相对优势估计中考虑样本难度，实现探索与模仿的更好平衡，提升推理能力和泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的强化学习方法通常忽略难度因素，导致学习不稳定和过度模仿离策略提示。需要更好地平衡探索与模仿，实现更稳定的知识扩展和推理泛化。

Method: 1. 自适应提示与样本难度先验：评估每个样本在策略模型下的难度，相应调度适当的提示比例指导rollout；2. 一致性梯度调制与选择性掩码：调制提示内的token级梯度，防止有偏和破坏性更新；3. 基于rollout难度后验的优势估计：利用有/无提示rollout的相对难度估计各自优势，实现更平衡的更新。

Result: 在多种模态、模型规模和领域上的广泛实验表明，ADHint在推理能力和分布外泛化方面表现优异，在pass@1和avg@8指标上持续超越现有方法。

Conclusion: ADHint通过将难度作为提示比例调度和相对优势估计的关键因素，实现了探索与模仿的更好平衡，显著提升了推理能力和泛化性能，为结合SFT和RL优势提供了有效解决方案。
...
TL;DR: 提出FID-Net深度学习模型，从无人机可见光图像检测病虫害树木，并通过三种空间指标分析虫害模式，实现精准森林保护管理。


<details>
  <summary>Details</summary>
Motivation: 传统方法在大规模、细粒度森林病虫害监测中存在局限，需要高效准确识别感染树木并分析虫害分布模式，以支持生态系统保护。

Method: 基于YOLOv8n构建FID-Net，引入轻量级特征增强模块提取病害敏感特征，自适应多尺度特征融合模块对齐融合RGB和增强特征，高效通道注意力机制增强判别信息。从检测结果构建虫害分析框架：核密度估计定位感染热点、邻域评估健康树感染风险、DBSCAN聚类识别高密度健康集群作为优先保护区域。

Result: 在中国东部天山32个森林样地的无人机图像实验中，FID-Net达到86.10%精确率、75.44%召回率、82.29% mAP@0.5和64.30% mAP@0.5:0.95，优于主流YOLO模型。分析确认感染树木呈现明显聚类模式。
...
TL;DR: 提出了CanolaTrack数据集和LeafTrackNet框架，用于在真实条件下对油菜叶片进行高精度追踪，显著提升了叶片追踪性能


<details>
  <summary>Details</summary>
Motivation: 现有叶片追踪方法存在局限性：要么局限于小型物种，要么依赖受限的成像条件；通用多目标追踪方法不适合动态生物场景；缺乏在真实条件下采集的大规模数据集

Method: 提出LeafTrackNet框架，结合YOLOv10叶片检测器和MobileNetV3嵌入网络，通过基于嵌入的记忆关联策略在推理过程中维持叶片身份

Result: LeafTrackNet在CanolaTrack数据集上表现优异，比现有植物专用追踪器和最先进的多目标追踪基线方法提升了9%的HOTA指标

Conclusion: 该工作为真实条件下的叶片级追踪设定了新标准，提供了农业作物叶片追踪的最大数据集，将促进植物表型研究的未来发展
...
TL;DR: 提出Weight Space Correlation Analysis方法，量化医学影像模型中临床任务与元数据任务的权重对齐程度，验证模型是否真正利用临床特征而非捷径学习


<details>
  <summary>Details</summary>
Motivation: 医学影像深度学习模型容易陷入捷径学习，依赖扫描仪型号等元数据而非真正的临床特征，需要验证模型是否真正利用临床信息进行预测

Method: 提出权重空间相关性分析方法，通过测量主临床任务分类头与辅助元数据任务分类头之间的对齐程度，量化特征利用情况

Result: 方法成功检测到人工诱导的捷径学习；应用于早产预测模型时发现，虽然嵌入包含大量元数据，但临床分类器权重与临床相关因素高度相关，与无关采集因素解耦

Conclusion: 该方法为验证模型可信度提供了工具，表明在没有诱导偏差的情况下，临床模型能够选择性地利用与真实临床信号相关的特征
...
TL;DR: StarryGazer：无需真实深度标签的无监督深度补全框架，通过结合稀疏深度图与单目深度估计模型，生成高质量稠密深度图像


<details>
  <summary>Details</summary>
Motivation: 现有无监督深度补全方法需要辅助数据估计深度值，不切实际；单目深度估计模型能生成相对深度图但无法与稀疏深度图有效结合，简单仿射变换会导致高误差

Method: 1) 使用预训练单目深度估计模型生成相对深度图；2) 对深度图进行分割和随机缩放，生成合成训练对（稠密伪真值和对应稀疏深度）；3) 训练细化网络，结合相对深度图和RGB图像提升精度和鲁棒性

Result: 在多个数据集上超越现有无监督方法和变换后的单目深度估计结果，证明框架能有效利用单目深度估计模型的能力，同时通过稀疏深度信息适当修正误差

Conclusion: StarryGazer提出了一种领域无关的深度补全框架，无需真实深度标签，通过巧妙结合稀疏深度信息和大型单目深度估计模型，实现了高质量的深度补全
...
TL;DR: DFSS是一种专门为语义分割设计的数据无知识蒸馏框架，通过利用教师模型的BN统计信息指导近似分布采样，并采用加权分布渐进蒸馏策略，显著提升了分割任务的蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有的数据无知识蒸馏方法主要针对分类任务设计，忽视了语义分割中物体结构的连续性和空间一致性，导致直接应用于分割任务时性能显著下降。需要专门为语义分割设计的数据无蒸馏框架。

Method: 1. 利用教师模型的Batch Normalization统计信息指导近似分布采样，选择更接近原始训练分布的数据；2. 提出加权分布渐进蒸馏策略，早期优先处理可靠样本，逐步引入更具挑战性的样本，模拟人类学习的自然进程。

Result: 在标准基准测试上的大量实验表明，DFSS在语义分割的数据无知识蒸馏方法中始终优于现有方法，取得了最先进的结果，同时显著减少了对辅助数据的依赖。
...
TL;DR: MMDrive是一个用于自动驾驶的多模态视觉语言模型框架，通过融合占用地图、LiDAR点云和文本描述三种模态，实现3D场景理解，显著提升了复杂驾驶环境下的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型受限于2D图像理解范式，难以感知3D空间信息并进行深度语义融合，导致在复杂自动驾驶环境中表现不佳。

Method: 提出MMDrive框架，包含两个核心组件：1) 面向文本的多模态调制器，根据问题语义动态加权各模态贡献；2) 跨模态抽象器，使用可学习抽象令牌生成紧凑的跨模态摘要，突出关键区域和语义。

Result: 在DriveLM和NuScenes-QA基准测试中表现优异：DriveLM上BLEU-4得分54.56，METEOR得分41.78；NuScenes-QA上准确率62.7%，显著优于现有自动驾驶视觉语言模型。

Conclusion: MMDrive突破了传统仅图像理解的限制，实现了复杂驾驶环境下的鲁棒多模态推理，为可解释的自动驾驶场景理解提供了新基础。
...
TL;DR: CoRA提出了一种新颖的协作感知架构，通过混合特征级和对象级融合，在保证高性能的同时提升通信鲁棒性，在极端场景下性能提升约19%且通信量减少5倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法（特别是中间融合方法）在通信条件不佳时性能会显著下降，因为数据传输会导致特征错位，这严重阻碍了实际部署。需要一种既能保持高性能又能应对通信干扰的鲁棒解决方案。

Method: CoRA采用混合架构，包含两个分支：1）特征级融合分支，选择关键特征进行高效融合以保证性能和可扩展性；2）对象级校正分支，利用语义相关性校正空间位移，保证对位姿误差的鲁棒性。该方法将中间融合和后期融合的优势互补结合。

Result: 在极端场景下，CoRA相比基线方法在AP@0.7指标上提升约19%，同时通信量减少5倍以上，实现了高性能与通信鲁棒性的平衡。

Conclusion: CoRA证明了中间融合和后期融合的优势不是权衡关系而是互补配对，通过混合架构成功解耦了性能与鲁棒性，为鲁棒协作感知提供了有前景的解决方案。
...
TL;DR: 提出POLAR数据集和POLARNet模型，通过大规模单光数据集和基于流的生成模型实现可控的人脸重光照


<details>
  <summary>Details</summary>
Motivation: 人脸重光照需要合成新光照下的真实肖像，但受限于大规模物理一致光照数据的缺乏

Method: 引入POLAR数据集（200+受试者×156光照方向×多视角×多表情），并开发基于流的生成模型POLARNet，从单张肖像预测每个光照的OLAT响应

Result: 建立了统一的照明学习框架，连接真实数据、生成合成和物理基础重光照，形成可扩展、可复现的肖像照明"鸡与蛋"循环

Conclusion: POLAR和POLARNet通过物理校准的数据集和连续可解释的光照变换模型，实现了可扩展、可控的人脸重光照
...
TL;DR: Ego-EXTRA是一个50小时的第一人称视角视频-语言数据集，用于专家-学员辅助，包含真实世界专家指导学员完成程序性活动的对话，创建了超过15k个高质量视觉问答对，用于评估多模态大语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏高质量的第一人称视角视频-语言数据集来支持专家级辅助系统的开发与评估，特别是在程序性活动指导场景中。

Method: 采用"绿野仙踪"数据收集范式，专家通过第一人称视角观察学员执行程序性活动，提供实时指导和回答问题，记录双向对话并转录，创建视觉问答基准。

Result: 创建了包含50小时未脚本化第一人称视频和超过15k个高质量视觉问答对的Ego-EXTRA数据集，评估显示当前多模态大语言模型在提供专家级辅助方面存在局限性。

Conclusion: Ego-EXTRA是一个具有挑战性的数据集，揭示了当前模型在提供专家级用户辅助方面的不足，为第一人称视频-语言助手的研究提供了公开基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有2D语音到视频扩散模型过度依赖参考指导，导致运动多样性有限；而3D感知动画通常依赖于预训练三平面生成器的反演，往往导致重建不完美和身份漂移。需要重新思考基于参考和几何的范式。

Method: 采用组合方法：1) 引入软身份约束而非严格参考条件；2) 利用视频数据的多视角特性在2D视频域隐式实现3D感知；3) 通过解耦学习分别训练视角一致性和时间连贯性；4) 使用自强制训练方案学习比推理时更长的时序上下文。

Result: 综合评估表明，STARCaster在任务和身份上都能有效泛化，在不同基准测试中持续超越先前方法，解决了现有自回归方法中常见的过度静态动画问题。

Conclusion: STARCaster通过重新思考参考和几何范式，在统一框架中实现了更好的语音驱动肖像动画和自由视角说话肖像合成，通过软身份约束、隐式3D感知和解耦学习解决了现有方法的局限性。
...
TL;DR: 提出VG-AVS任务，让视觉语言模型从单张图像选择最有信息量的下一个视角，通过合成数据集和微调框架实现主动视觉推理


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型局限于静态图像推理，而具身智能体需要主动移动获取更丰富的视觉信息，因此需要开发能够主动选择信息量最大视角的能力

Method: 1) 提出VG-AVS任务，仅基于当前图像选择最佳下一个视角；2) 构建合成数据集，包含成对的查询-目标视角和问答提示；3) 提出微调框架：先监督微调，再强化学习策略优化

Result: 1) 在视角选择基础上实现强的问题回答性能；2) 在未见过的合成和真实场景中具有鲁棒泛化能力；3) 集成到现有场景探索EQA系统能提升下游问答准确率

Conclusion: VG-AVS任务和框架成功将视觉语言模型从静态视觉扩展到主动视觉，为具身智能体提供了基于视觉信息的主动视角选择能力
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的指令图像编辑方法在处理指定精确属性（如颜色、位置、数量）的细粒度指令时表现不佳。虽然最近的方法使用GRPO进行对齐，但仅在单个采样步骤优化，反馈稀疏，限制了轨迹级控制能力。

Method: 提出CogniEdit统一框架，包含三个组件：1) 多模态大语言模型将复杂指令分解为可操作指令；2) 动态令牌焦点重定位自适应强调细粒度属性；3) 基于密集GRPO的优化，在连续去噪步骤间传播梯度，实现轨迹级监督。

Result: 在基准数据集上的大量实验表明，CogniEdit在平衡细粒度指令跟随、视觉质量和可编辑性保持方面实现了最先进的性能。

Conclusion: CogniEdit通过结合多模态推理和密集奖励优化，解决了细粒度指令图像编辑中的关键挑战，实现了更好的轨迹级控制和编辑效果。
...
TL;DR: 论文提出了Video Reality Test基准，用于评估AI生成视频在音频-视觉耦合下的感知真实度，发现当前最佳视频生成模型能欺骗大多数视觉语言模型，但人类专家仍能更好地区分真假视频。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI生成的视频越来越难以与真实视频区分，这带来了社会挑战。现有基准大多只评估无音频视频，针对广泛叙事领域，且仅关注分类。需要测试最先进的视频生成模型是否能产生欺骗人类和视觉语言模型的沉浸式音频配对视频。

Method: 引入Video Reality Test基准套件，基于精心策划的真实ASMR视频，针对细粒度动作-对象交互。采用对抗性创作者-评审者协议：视频生成模型作为创作者试图欺骗评审者，而视觉语言模型作为评审者试图识别虚假内容。

Result: 最佳创作者Veo3.1-Fast甚至能欺骗大多数视觉语言模型：最强的评审者(Gemini 2.5-Pro)仅达到56%准确率(随机50%)，远低于人类专家的81.25%。添加音频能改善真假辨别，但水印等表面线索仍会显著误导模型。

Conclusion: 这些发现界定了当前视频生成真实度的边界，并暴露了视觉语言模型在感知保真度和音频-视觉一致性方面的局限性。基准揭示了AI生成视频检测的挑战，特别是在音频-视觉耦合的沉浸式内容中。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成图像检测方法（包括基于视觉语言模型的方法）通常产生高度纠缠的特征表示，混合了任务相关的取证线索（因果特征）与虚假或无关模式（非因果特征），这限制了模型对不同生成技术的泛化能力。
...
Conclusion: 通过显式解耦因果特征与非因果特征，并应用因果推理指导的特征过滤，CausalCLIP能够提取稳定、可迁移的取证特征，显著提升生成图像检测器在多样化生成技术上的泛化性能。
...
TL;DR: ShowTable：一个结合MLLM和扩散模型的创意表格可视化生成框架，通过渐进式自校正过程实现高保真数据可视化


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在需要深度推理、规划和精确数据到视觉映射的任务上表现不足，特别是在超越一般场景的创意表格可视化任务中

Method: 提出ShowTable管道，通过MLLM作为中央协调器进行视觉规划推理和错误判断，扩散模型执行指令，采用渐进式自校正过程

Result: 实验表明该框架显著优于基线方法，展示了有效的多模态推理、生成和错误校正能力，并在TableVisBench基准测试中表现优异

Conclusion: ShowTable成功解决了创意表格可视化这一挑战性任务，通过MLLM与扩散模型的协同实现了高质量的数据可视化生成
...
TL;DR: KlingAvatar 2.0提出时空级联框架，通过蓝图关键帧生成和子片段细化，解决长时高分辨率视频生成的效率、时间漂移和质量退化问题，并引入协同推理导演增强多模态指令对齐。


<details>
  <summary>Details</summary>
Motivation: 现有头像视频生成模型在生成长时高分辨率视频时存在效率低下、时间漂移、质量退化和提示跟随弱等问题，需要更有效的框架来解决这些挑战。
...
Result: 实验表明模型能有效解决高效、多模态对齐的长时高分辨率视频生成挑战，提供增强的视觉清晰度、逼真的唇齿渲染与准确唇同步、强身份保持和连贯的多模态指令跟随。

Conclusion: KlingAvatar 2.0通过时空级联框架和协同推理导演机制，成功解决了长时高分辨率头像视频生成的关键问题，实现了高效、高质量、多模态对齐的视频生成能力。
...
TL;DR: 该论文分析了基于面部热成像图的生物识别技术，比较了不同红外光谱范围，提出了热成像相机的关键要求，并使用孪生神经网络实现自动识别，在专有数据集上达到约80%准确率。


<details>
  <summary>Details</summary>
Motivation: 研究热成像技术在生物识别中的应用动机在于开发更可靠的安全系统。传统可见光识别受光照条件限制，而热成像技术基于面部热辐射特征，能在低光照或完全黑暗环境下工作，提供更稳定的识别性能。

Method: 方法包括：1）比较分析NIR、SWIR、MWIR、LWIR四种红外光谱范围；2）定义热成像相机的关键参数要求（传感器分辨率、热灵敏度、≥30Hz帧率）；3）采用孪生神经网络实现面部热成像图的自动识别；4）在专有数据集上进行实验验证。

Result: 实验结果显示：1）提出的方法在专有数据集上达到约80%的识别准确率；2）热成像技术显示出作为可靠生物识别技术的潜力；3）混合系统（可见光+红外光谱）能克服单一模态的限制，提供更好的性能。

Conclusion: 热成像技术是开发可靠安全系统的有前景技术。通过优化相机参数和采用孪生神经网络等方法，可以实现有效的生物识别。未来混合系统的发展将进一步增强识别系统的鲁棒性和适用性。
...
TL;DR: 提出利用DINO自注意力"key"特征进行息肉分割的框架，通过简单卷积解码器实现，在领域泛化和极端单领域泛化任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前息肉分割方法存在泛化能力不足的问题，特别是在数据受限或挑战性场景下，且许多方法依赖复杂的任务特定架构。需要更鲁棒、泛化能力强的解决方案。

Method: 利用DINO自注意力模块的"key"特征，而非传统方法从ViT最深层提取token，结合简单的卷积解码器预测息肉掩码，避免使用息肉特定的复杂架构。

Result: 在多中心数据集上，通过领域泛化(DG)和极端单领域泛化(ESDG)两种严格协议验证，该方法达到最先进性能，显著提升泛化能力，特别是在数据稀缺和挑战性场景下，超越了nnU-Net和UM-Net等模型。

Conclusion: 提出的基于DINO自注意力key特征的框架在息肉分割任务中表现出优异的泛化能力和性能，避免了复杂的任务特定架构，为数据稀缺场景下的医学图像分割提供了有效解决方案。
...
TL;DR: 提出一种图像到视频生成方法，通过用户可编辑的代理动态图（PDG）分离运动规范与外观合成，实现对最终帧遮挡区域的可控生成。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频方法虽然能生成合理运动，但难以产生可预测的关节运动，同时无法在遮挡区域强制执行用户指定的内容。需要一种既能控制运动又能指定遮挡区域外观的方法。

Method: 引入轻量级、用户可编辑的代理动态图（PDG）来确定性驱动部件运动，同时使用冻结的扩散先验合成符合该运动的合理外观。用户通过标注和调整PDG姿态，计算密集运动流，利用扩散模型作为运动引导着色器，并在遮挡区域编辑外观，利用PDG的可见性信息进行潜在空间合成。

Result: 在关节物体、家具、车辆和可变形物体的图像转短视频任务中，相比现有方法展现出明显优势，实现了可控的关节运动和遮挡区域用户控制，无需微调。

Conclusion: 该方法通过松散姿态和结构的生成控制与遮挡区域外观指定的可预测控制相结合，开辟了新的图像到视频工作流程，实现了运动与用户意图在遮挡区域的协调。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用分割模型经常产生碎片化或不连通的输出，修复这些拓扑错误通常需要手工设计的细化规则或特定任务架构，缺乏通用解决方案。

Method: 提出细化神经细胞自动机（rNCA），通过训练学习目标形状的结构特性，仅依赖局部信息对粗糙分割掩码进行迭代更新，逐步修复断裂区域、修剪松散碎片。

Result: 在视网膜血管分割中，Dice/clDice提升2-3%，Betti错误显著降低（β₀减少60%，β₁减少20%）；在心肌分割中，零样本设置下修复61.5%的断裂案例，ASSD和HD分别降低19%和16%。

Conclusion: 神经细胞自动机可作为有效且广泛适用的分割细化器，通过学习局部动态修复拓扑错误，无需手工规则或特定任务架构。
...
TL;DR: 提出统一时空建模框架USTM，结合Swin Transformer和轻量级时序适配器，在RGB视频上实现连续手语识别SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有CSLR方法使用CNN空间骨干与时序卷积/循环模块结合，无法捕捉细粒度手部和面部线索，且难以建模长程时序依赖

Method: 提出USTM框架：Swin Transformer骨干增强轻量级时序适配器TAPE，结合位置编码，统一建模细粒度空间特征和短/长程时序上下文

Result: 在PHOENIX14、PHOENIX14T、CSL-Daily数据集上，USTM在RGB方法和多模态方法中都达到SOTA，与多流方法性能相当

Conclusion: USTM框架有效解决了CSLR中的细粒度特征提取和长程时序建模问题，仅使用RGB视频即可实现强大性能
...
TL;DR: 提出MCT-UEG框架，通过元跨任务训练生成广泛不可利用的个人图像，提升隐私保护的实际适用性


<details>
  <summary>Details</summary>
Motivation: 现有不可利用示例生成方法在实际应用中存在局限性，无法生成跨不同计算机视觉任务都广泛不可利用的示例，需要提升方法的实际适用性

Method: 提出MCT-UEG框架，核心是设计面向平坦最小值的元训练和测试方案，优化不可利用示例生成器以产生广泛不可利用的示例

Result: 大量实验证明了该框架的有效性

Conclusion: 提出的MCT-UEG框架能够生成跨任务广泛不可利用的示例，提升了个人图像隐私保护的实际适用性
...
TL;DR: RecTok通过流语义蒸馏和重建对齐蒸馏，解决了高维视觉分词器在重建质量和生成质量之间的权衡问题，实现了更好的图像重建、生成质量和判别性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉分词器在潜在空间维度与生成质量之间存在根本性权衡，高维分词器性能仍不如低维版本，需要克服这一限制。
...
Conclusion: RecTok通过创新的蒸馏方法成功克服了高维视觉分词器的局限性，在重建、生成和判别性能方面都表现出色，为视觉分词器设计提供了新思路。
...
TL;DR: 提出MineTheGap方法，通过遗传算法自动挖掘导致文本到图像模型生成偏见输出的提示词，并使用新颖的偏见评分来量化偏见严重程度。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在处理模糊提示时会产生偏见，这些偏见可能带来社会影响（如职业与种族的刻板印象）和用户体验问题（生成图像缺乏多样性）。现有方法仅能检测给定提示的偏见，无法主动发现导致偏见的提示。

Method: 提出MineTheGap方法：1）使用遗传算法迭代优化提示池，寻找能暴露偏见的提示；2）设计新颖的偏见评分机制，通过比较生成图像分布与LLM生成的文本变体分布来量化偏见严重程度。

Result: 方法在已知偏见数据集上验证有效，能够自动发现导致偏见的提示，代码和示例已在项目网页公开。

Conclusion: MineTheGap能够自动挖掘文本到图像模型的偏见触发提示，超越了简单的偏见检测，为理解和缓解模型偏见提供了新工具。
...
TL;DR: 提出一个轻量级少样本学习框架，结合MobileNet特征提取器、特征融合技术和注意力增强的Bi-LSTM分类器，用于植物叶片病害识别，在数据稀缺和资源受限环境下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习植物病害识别方法依赖大规模标注数据和计算密集型模型，不适合数据稀缺和资源受限的农业环境，需要开发轻量级、高效的少样本学习方法。

Method: 使用MobileNetV2和MobileNetV3作为特征提取器，结合特征融合技术生成鲁棒特征表示，然后通过注意力机制增强的Bi-LSTM分类器进行病害分类。

Result: 在PlantVillage数据集上15-shot达到98.23%准确率，接近SOTA的99.98%；在真实场景的Dhan Shomadhan数据集上15-shot达到69.28%；模型仅40MB大小，计算复杂度约1.12GFLOPs。

Conclusion: 该框架为数据稀缺地区提供了一个可扩展、移动就绪的植物病害诊断解决方案，在保持轻量级的同时实现了优异的分类性能。
...
TL;DR: IMILIA是一个用于炎症性肠病组织切片炎症预测和自动标记的端到端框架，包含炎症预测模块和可解释性模块，在多个数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 随着IBD治疗目标转向组织学缓解，准确评估微观炎症对于评估疾病活动性和治疗反应变得至关重要，需要自动化工具来预测炎症并解释预测结果。

Method: IMILIA框架包含两个模块：1）炎症预测模块使用多实例学习模型预测H&E染色切片中的炎症存在；2）可解释性模块包括HistoPLUS（细胞检测、分割和分类）和EpiSeg（上皮分割）来识别驱动预测的组织区域。

Result: 在发现队列中获得交叉验证ROC-AUC 0.83，在两个外部验证队列中获得ROC-AUC 0.99和0.84。可解释性模块显示高预测分数区域免疫细胞密度增加，低分数区域主要为正常上皮细胞，这些模式在所有数据集中一致。

Conclusion: IMILIA能够准确预测IBD组织切片中的炎症，并通过可解释性模块提供生物学一致的见解，有助于评估疾病活动性和治疗反应，代码已在GitHub开源。
...
TL;DR: 提出一种测试时域泛化方法，利用扩散模型将目标域图像映射回源域分布，无需大规模合成数据生成，在分割、检测、分类任务上显著提升性能
...
Method: 在测试时使用扩散模型将目标域图像映射回源域分布，仅需源域描述，保留任务模型，无需大规模合成数据生成。包括集成变体以增强鲁棒性。

Result: 在真实到真实的域泛化场景中，对未知目标分布的分割、检测和分类任务实现一致改进：BDD100K-Night上相对提升137%，ImageNet-R上68%，DarkZurich上62%。

Conclusion: 测试时使用扩散模型进行域映射是一种有效的域泛化方法，避免了大规模数据合成的开销，在多个任务和数据集上实现了显著性能提升。
...
TL;DR: PoseAnything：首个通用姿态引导视频生成框架，支持人类和非人类角色，实现任意骨骼输入，并引入部件感知时序一致性模块和运动解耦CFG策略


<details>
  <summary>Details</summary>
Motivation: 当前姿态引导视频生成方法仅支持人类姿态输入，对其他主体姿态泛化能力差，限制了在动画等领域的应用

Method: 1) 提出通用姿态引导视频生成框架PoseAnything，支持任意骨骼输入；2) 引入部件感知时序一致性模块，通过部件分割、对应关系建立和跨帧注意力实现细粒度一致性；3) 提出主体与相机运动解耦CFG策略，独立控制相机运动；4) 构建XPose数据集（5万非人类姿态-视频对）

Result: PoseAnything在效果和泛化能力上显著优于现有最先进方法，能够处理人类和非人类角色，实现精确的姿态控制

Conclusion: 该研究突破了姿态引导视频生成仅限人类姿态的限制，实现了通用姿态控制，为动画制作等应用提供了更强大的工具
...
Method: 提出T3（Transform Trained Transformer）策略，引入多尺度权重共享窗口注意力机制，通过分层分块和轴保持全注意力设计，仅用少量计算和数据即可实现预训练模型的"注意力模式"转换

Result: 在4K-VBench上，T3-Video显著优于现有方法：VQA提升4.29分，VTC提升0.08分，同时将原生4K视频生成加速超过10倍

Conclusion: T3-Video通过创新的Transformer改造策略，成功解决了4K视频生成中的计算效率问题，在保持甚至提升生成质量的同时实现了显著的加速效果
...
TL;DR: Soul是一个多模态驱动的数字人生成框架，能从单张肖像、文本提示和音频生成语义一致的长时视频，实现精准唇同步、生动表情和身份保持，并在推理速度上实现11.4倍加速。


<details>
  <summary>Details</summary>
Motivation: 解决现有数字人生成方法在长时一致性、多模态控制、数据稀缺和推理效率方面的不足，为虚拟主播、影视制作等实际应用提供高质量解决方案。

Method: 基于Wan2.2-5B主干，集成音频注入层、多训练策略和阈值感知码本替换确保长时一致性；使用步长/CFG蒸馏和轻量VAE优化推理效率；构建Soul-1M百万级标注数据集和Soul-Bench评估基准。

Result: 在视频质量、视频-文本对齐、身份保持和唇同步准确性上显著优于当前领先的开源和商业模型，推理速度提升11.4倍且质量损失可忽略，在虚拟主播等实际场景中展示广泛应用潜力。

Conclusion: Soul框架通过创新的多模态集成、数据构建和效率优化，实现了高质量、一致且高效的数字人生成，为实际应用提供了强大工具。
...
TL;DR: Seedance 1.5 pro是一个专门用于原生联合音频-视频生成的基础模型，采用双分支Diffusion Transformer架构，通过多阶段数据管道和跨模态联合模块实现卓越的视听同步和生成质量。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的发展，需要统一的视听生成模型。现有模型在音频-视频同步、生成质量和实用性方面存在不足，需要专门针对原生联合音频-视频生成的基础模型。

Method: 采用双分支Diffusion Transformer架构，集成跨模态联合模块和专门的多阶段数据管道。实施细致的后训练优化，包括高质量数据集的监督微调(SFT)和多维奖励模型的人类反馈强化学习(RLHF)。引入加速框架提升推理速度10倍以上。

Result: 实现了卓越的音频-视频同步和生成质量，具备精确的多语言和方言唇形同步、动态电影级相机控制和增强的叙事连贯性。推理速度提升超过10倍，成为专业级内容创作的强大引擎。

Conclusion: Seedance 1.5 pro作为一个强大的视听联合生成基础模型，在同步质量、生成效果和实用性方面表现出色，已部署在火山引擎平台上，适用于专业内容创作。
...
TL;DR: TARA通过简单高效的适配方法，将多模态大语言模型转化为时间感知的视频-文本嵌入模型，无需视频数据，在时间感知检索和多项视频理解任务上取得SOTA性能。
...
Method: 提出TARA（时间感知检索适配）方法，无需视频数据即可将多模态大语言模型适配为时间感知的视频-文本嵌入模型。同时提出新的基准测试，包含时间相反（手性）动作作为困难负样本。

Result: TARA在手性基准测试中优于所有现有视频-文本模型，在标准基准测试中也表现强劲。此外，TARA嵌入具有否定感知能力，在动词和副词理解任务上达到SOTA性能。

Conclusion: TARA产生了一个强大、通用、时间感知的视频-文本嵌入模型，具有最先进的零样本性能，在时间感知检索和视频理解方面展现出显著优势。
...
TL;DR: Pancakes框架能够自动为医学图像生成多种语义一致的分割方案，解决了现有模型只能处理单一分割协议或需要手动提示的问题。


<details>
  <summary>Details</summary>
Motivation: 单个医学图像可以根据不同应用需求进行多种有意义的分割（如组织类型、血管区域、解剖结构等），但现有自动分割模型要么只支持单一协议，要么需要繁琐的手动提示来指定分割方式。

Method: 提出了Pancakes框架，采用新的问题表述方式，能够在未见过的领域图像上自动生成多个合理协议的多标签分割图，同时保持相关图像间的语义一致性。

Result: 在7个保留数据集上的实验表明，该模型在生成多个语义连贯的整图分割方面显著优于现有的基础模型。

Conclusion: Pancakes框架解决了医学图像多协议自动分割的问题，为医学图像分析提供了更灵活、自动化的解决方案。
...
TL;DR: 提出H2IAD新任务：检测双人交互行为异常，开发IADNet模型，通过TASM和DREM模块分别捕捉时空交互特征，在3D人体动作数据集上优于现有单人体异常检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有单人体异常检测方法难以捕捉复杂非对称的人际交互动态，导致检测精度低。人类行为本质上是协作性的，异常可能源于人际互动，需要专门针对双人交互异常检测的方法。

Method: 提出IADNet模型，包含：1）TASM模块：共享编码的运动嵌入以同步协作运动相关性；2）DREM模块：基于距离的关系编码以反映空间社交线索；3）使用归一化流进行异常评分。

Result: 在多人运动基准测试上的广泛实验表明，IADNet在H2IAD任务上优于现有的人体中心异常检测基线方法。

Conclusion: 成功定义了H2IAD新任务，提出的IADNet模型能有效检测双人交互行为异常，通过时空特征建模显著提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型主要局限于单步推理，缺乏评估和驱动多跳推理能力的复杂基准测试，难以应对需要跨模态和外部知识迭代整合的复杂现实挑战。

Method: 提出MMhops基准测试包含Bridging和Comparison两种任务格式，需要模型动态构建复杂推理链；并提出MMhops-R1多模态检索增强生成框架，使用强化学习优化动态推理路径规划、查询生成和多层次信息整合。

Result: MMhops-R1在MMhops基准上显著优于强基线模型，证明动态规划和多模态知识整合对复杂推理至关重要；同时在需要固定跳数推理的任务上也表现出强泛化能力。

Conclusion: 本研究贡献了具有挑战性的新基准测试和强大的基线模型，将发布相关代码、数据和权重以推动该关键领域的研究。
...
TL;DR: LiMo是一种基于扩散模型的时空光照估计方法，通过生成不同曝光下的镜面和漫反射球体，结合几何条件实现高精度光照预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法在同时实现逼真的高频细节预测和准确照度估计方面存在不足，需要一种能够兼顾空间控制和预测精度的时空光照估计方法。

Method: 基于扩散先验微调现有扩散模型，使用大规模定制数据集；引入新的几何条件提供场景相对位置信息；生成不同曝光下的镜面和漫反射球体；通过可微分渲染合并预测结果。

Result: LiMo在空间控制和预测精度方面均达到最先进水平，能够准确估计时空光照并生成高质量的HDRI地图。

Conclusion: LiMo通过创新的几何条件和多曝光预测方法，成功实现了既具有逼真高频细节又保持准确照度的时空光照估计，为相关应用提供了有效解决方案。
...
TL;DR: LongVie 2是一个三阶段训练的端到端自回归框架，通过多模态引导、退化感知训练和历史上下文引导，实现了可控、高质量、时间一致的视频世界建模，支持长达5分钟的视频生成。
...
Result: LongVie 2在长距离可控性、时间一致性和视觉保真度方面达到最先进性能，支持长达5分钟的连续视频生成。作者还提出了包含100个高分辨率一分钟视频的LongVGenBench基准。

Conclusion: LongVie 2在视频世界建模方面取得了显著进展，通过三阶段训练框架实现了可控、高质量、时间一致的视频生成，为统一的视频世界建模迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在医学影像领域已显示出潜力，但针对三维成像模态（如数字乳腺断层合成DBT）的研究仍不足。尽管DBT用于乳腺癌筛查，但目前尚无DBT基础模型。

Method: 采用DINOv2方法进行自监督预训练，使用来自27,990名患者的487,975个DBT体积中的超过2500万个2D切片。评估了三个下游任务：乳腺密度分类、5年乳腺癌风险预测和病灶检测。

Result: DBT-DINO在乳腺密度分类上准确率达0.79，优于MetaAI DINOv2基线（0.73）和DenseNet-121（0.74）。在5年风险预测上AUROC为0.78，与DINOv2的0.76相当。在病灶检测上，DINOv2平均敏感度为0.67，优于DBT-DINO的0.62，但DBT-DINO对癌性病灶检测率更高（78.8% vs 77.3%）。

Conclusion: DBT-DINO是首个DBT基础模型，在乳腺密度分类和癌症风险预测上表现优异。但领域特定预训练在检测任务上效果不一，表明局部检测任务需要进一步的方法学发展。
...
TL;DR: Do-Undo任务和基准测试旨在解决视觉语言模型在理解物理动作驱动的场景变换方面的不足，要求模型模拟物理动作结果并准确反转，测试物理因果关系理解。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要关注对象级编辑，缺乏对真实世界物理动作驱动的场景变换的理解能力。这对于具身AI、机器人和物理感知生成模型至关重要。

Method: 1) 从真实世界视频中构建大规模可逆动作数据集；2) 设计训练策略强制一致性以实现鲁棒的动作定位；3) 创建Do-Undo任务要求模型模拟物理动作结果并准确反转。

Result: 实验表明当前模型在物理可逆性方面表现不佳，凸显了该任务的重要性。Do-Undo为评估和推进多模态系统的物理推理能力建立了直观的测试平台。

Conclusion: Do-Undo任务填补了视觉语言模型在物理因果关系理解方面的关键空白，为具身AI、机器人和物理感知生成模型的发展提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 空间转录组学数据采集成本高昂，传统固定网格采样策略导致冗余测量，数据稀缺限制了现有方法。单细胞测序领域提供了丰富的生物数据作为辅助来源，可以弥补这一限制。

Method: 提出SCR2-ST统一框架：1) 单细胞引导的强化学习主动采样(SCRL)，结合单细胞基础模型嵌入和空间密度信息构建生物奖励信号；2) 混合回归-检索预测网络SCR2Net，通过多数细胞类型过滤机制抑制噪声匹配，检索表达谱作为软标签进行辅助监督。

Result: 在三个公共ST数据集上评估，SCR2-ST在采样效率和预测准确性方面均达到最先进性能，特别是在低预算场景下表现优异。

Conclusion: SCR2-ST通过有效利用单细胞先验知识，解决了空间转录组学数据采集成本高、数据稀缺的问题，为在有限测序预算下获得高质量ST数据提供了创新解决方案。
...
TL;DR: MindDrive是一个用于自动驾驶的VLA框架，使用具有两组LoRA参数的LLM，将连续动作空间转换为离散语言决策空间，实现高效的在线强化学习。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的VLA范式主要依赖模仿学习，存在分布偏移和因果混淆问题。在线强化学习能通过试错学习解决这些问题，但在连续动作空间中探索效率低下。

Method: 提出MindDrive框架：一个LLM配备两组LoRA参数，分别作为决策专家（场景推理和驾驶决策）和动作专家（将语言决策映射为可行轨迹）。通过将轨迹级奖励反馈到推理空间，在离散语言驾驶决策集上进行试错学习。

Result: 在Bench2Drive基准测试中取得显著性能：驾驶评分78.04，成功率55.09%。这是首个证明在线强化学习在自动驾驶VLA模型中有效性的工作。

Conclusion: MindDrive成功平衡了复杂场景中的最优决策、类人驾驶行为和在线强化学习的高效探索，为自动驾驶VLA模型提供了有效的在线强化学习解决方案。
...
TL;DR: 提出一个用于新颖视角合成的数据集，基于高质量动画电影生成，包含动态场景、多种模态数据，并组织成三种基准测试场景。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏高质量、多模态、包含动态场景的新颖视角合成数据集，现有数据集在视觉丰富度、注释质量和实验设置多样性方面存在不足。

Method: 从高质量动画电影中生成数据集，提供高保真RGB图像以及深度、表面法线、物体分割和光流等多种模态数据，并组织成密集多视角、稀疏相机和单目视频序列三种基准测试场景。

Result: 创建了一个视觉丰富、注释质量高、实验设置多样的数据集，适用于4D场景重建和新颖视角生成模型的训练与评估。

Conclusion: 该数据集为视角合成和3D视觉研究提供了独特资源，能够推动该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着扩散模型生成视频技术日益逼真，需要可靠的检测机制。现有方法对生成视频中的3D几何模式探索有限，而几何一致性是区分真实与AI生成视频的关键特征

Method: 使用消失点作为3D几何模式的显式表示；构建静态场景AI生成视频数据集以稳定提取几何特征；提出几何感知Transformer框架，包含几何位置编码、时空几何注意力机制和EMA几何分类器头

Result: Grab-3D显著优于现有最先进的检测器，在未见过的生成器上展现出强大的跨域泛化能力

Conclusion: 通过显式建模3D几何时间一致性，Grab-3D为AI生成视频检测提供了有效解决方案，几何模式分析是区分真实与生成内容的重要线索
...
TL;DR: AgentIAD：基于工具驱动的多阶段视觉检测代理框架，通过感知缩放器和比较检索器实现工业异常检测，在MMAD数据集上达到97.62%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临正常样本稀缺、缺陷细微且局部的挑战。现有的单次视觉语言模型容易忽略小异常，且缺乏与标准正常模式对比的机制。

Method: 提出AgentIAD框架，包含感知缩放器（用于局部细粒度分析）和比较检索器（用于查询正常样本）。通过MMAD数据集构建结构化轨迹，采用两阶段训练：监督微调+强化学习，设计了感知奖励和行为奖励。

Result: 在MMAD数据集上达到97.62%的分类准确率，创下新纪录，超越了之前的MLLM方法，同时产生透明可解释的检测轨迹。

Conclusion: AgentIAD通过多阶段视觉检查、工具驱动代理框架和两阶段训练策略，有效解决了工业异常检测的挑战，实现了高精度和可解释性。
...
Method: JoVA采用视频和音频token在Transformer层内的联合自注意力，实现直接高效的跨模态交互，无需额外对齐模块。同时引入基于面部关键点检测的嘴部区域损失，增强训练中对关键嘴部区域的监督，而不影响架构简洁性。

Result: 在基准测试上的大量实验表明，JoVA在唇语同步准确性、语音质量和整体视频-音频生成保真度方面优于或与最先进的统一和音频驱动方法相当。
...
TL;DR: LASER是一个无需训练即可将离线3D重建模型转换为流式系统的框架，通过层间尺度对齐解决窗口间深度尺度不一致问题，实现高效流式视频处理。


<details>
  <summary>Details</summary>
Motivation: 现有前馈重建模型（如VGGT和π³）虽然重建质量高，但二次内存复杂度无法处理流式视频。现有流式方法需要大量重新训练，且无法充分利用先进离线模型的几何先验。

Method: 提出LASER框架：1）通过连续时间窗口间的预测对齐将离线模型转换为流式系统；2）发现简单相似变换对齐因深度层尺度不一致而失败；3）引入层间尺度对齐：将深度预测分割为离散层，计算每层尺度因子，并在相邻窗口和时间戳间传播。

Result: 在相机姿态估计和点云重建方面达到最先进性能，在RTX A6000 GPU上以14 FPS运行，峰值内存仅6 GB，可处理千米级流式视频。

Conclusion: LASER提供了一种无需训练即可将高质量离线重建模型转换为高效流式系统的方法，解决了深度层尺度对齐问题，实现了实际部署的可行性。
...
TL;DR: 通过重新编程预训练的3D实例生成器作为场景级学习器，利用模型中心的空间监督而非数据集监督，实现了对未见布局和新物体组合的泛化能力，证明了3D实例生成器是隐式的空间学习器和推理器。


<details>
  <summary>Details</summary>
Motivation: 解决交互式3D场景生成的泛化挑战。现有基于学习的方法在有限场景数据集上建立空间理解，限制了向新布局的泛化能力。

Method: 重新编程预训练的3D实例生成器作为场景级学习器，用模型中心的空间监督替代数据集监督。采用视点中心的场景空间表示，构建完全前馈、可泛化的场景生成器，直接从实例模型中学习空间关系。

Result: 解锁了生成器的可转移空间知识，能够泛化到未见布局和新物体组合。即使在训练场景由随机组合物体构成时，空间推理能力仍然涌现。定量和定性结果证明3D实例生成器是隐式的空间学习器和推理器。

Conclusion: 3D实例生成器的可转移场景先验为从纯几何线索推断邻近性、支撑和对称关系提供了丰富的学习信号，为交互式3D场景理解和生成的基础模型指明了方向。
...
TL;DR: RVM是一种基于循环神经网络的视频掩码自编码器，通过循环聚合图像特征来高效学习视频时空表示，在参数效率上比现有方法提升30倍，同时在视频和图像任务上都表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视频表示学习方法通常需要复杂的时空注意力机制，计算成本高且参数效率低。需要一种既能有效捕捉视频时空结构，又具有高参数效率的通用视频表示学习方法。

Method: 使用基于Transformer的循环神经网络(RNN)来聚合密集图像特征，通过非对称掩码预测任务学习，仅需标准像素重建目标。循环设计允许线性计算成本下稳定传播长时序特征。

Result: 在视频级任务(动作识别、点/目标跟踪)上与VideoMAE、V-JEPA等SOTA方法竞争，在几何和密集空间理解任务上优于DINOv2等图像模型。小模型无需知识蒸馏，参数效率比竞争方法高30倍。

Conclusion: RVM提供了一种高效通用的视频表示学习方法，通过循环架构实现了线性计算复杂度和高参数效率，同时学习到了丰富的场景语义、结构和运动表示。
...
TL;DR: 本文提出VTP视觉分词器预训练框架，通过联合优化图像-文本对比、自监督和重建损失，解决传统视觉分词器预训练中的"预训练缩放问题"，使生成性能随计算资源有效提升。


<details>
  <summary>Details</summary>
Motivation: 传统基于重建的视觉分词器（如VAE）训练产生的潜在空间偏向低层信息，导致像素级精度提升并不能改善生成质量，形成"预训练缩放问题"——投入更多计算资源无法有效提升生成性能。

Method: 提出VTP统一视觉分词器预训练框架，首次联合优化三种损失：图像-文本对比损失、自监督损失和重建损失，以构建能简洁表示高层语义的潜在空间。

Result: 大规模研究表明：1）理解是生成的关键驱动力；2）VTP具有更好的缩放特性，生成性能随计算、参数和数据的增加而有效提升。VTP在ImageNet上达到78.2%零样本准确率和0.36 rFID，生成收敛速度比先进蒸馏方法快4.1倍。

Conclusion: VTP框架通过联合优化多种损失，成功解决了视觉分词器预训练的缩放问题，使生成性能能够随预训练投入的计算资源有效提升，而传统自编码器在1/10计算量时就已停滞不前。
...
TL;DR: LitePT：一种新的3D点云处理架构，早期层用卷积提取几何特征，深层用注意力机制捕获语义，结合PointROPE位置编码，参数更少、速度更快、性能相当或更好。


<details>
  <summary>Details</summary>
Motivation: 现有3D点云网络结合卷积和注意力模块，但最佳组合方式不明确。研究发现卷积适合早期高分辨率几何特征提取，注意力适合深层低分辨率语义捕获，需要更优的设计原则。

Method: 提出LitePT架构：早期阶段使用卷积，深层阶段切换到注意力机制。引入训练无关的3D位置编码PointROPE，避免丢弃冗余卷积层时丢失空间布局信息。

Result: LitePT比最先进的Point Transformer V3参数减少3.6倍，运行速度快2倍，内存使用减少2倍，在多个任务和数据集上性能相当甚至更好。

Conclusion: 通过合理分配卷积和注意力模块的角色，可以构建更高效、轻量的3D点云处理架构，LitePT展示了这种设计原则的有效性。
...
TL;DR: VPR-AttLLM是一个模型无关框架，通过LLM的语义推理和地理空间知识增强VPR模型，提升社交媒体街景图像的地理定位性能，特别针对洪水等危机事件图像。


<details>
  <summary>Details</summary>
Motivation: 社交媒体街景图像为城市洪水等危机事件提供实时视觉证据，但缺乏可靠的地理元数据。现有VPR模型在处理这类图像时性能显著下降，因为跨源场景存在视觉失真和领域偏移。

Method: 提出VPR-AttLLM框架，通过注意力引导的描述符增强，将LLM的语义推理和地理空间知识集成到现有VPR管道中。利用LLM识别城市背景下具有位置信息的区域，抑制瞬态视觉噪声，无需模型重新训练或额外数据。

Result: 在扩展基准测试中，包括SF-XL（含真实社交媒体洪水图像）、合成洪水场景和新的HK-URBAN数据集，VPR-AttLLM与三种SOTA VPR模型（CosPlace、EigenPlaces、SALAD）集成，召回性能持续提升，相对增益通常在1-3%，在最具挑战性的真实洪水图像上可达8%。

Conclusion: VPR-AttLLM建立了LLM引导的多模态融合在视觉检索系统中的通用范式，将城市感知理论原理嵌入注意力机制，桥接了类人空间推理与现代VPR架构。其即插即用设计、强大的跨源鲁棒性和可解释性显示了其在可扩展城市监测和危机图像快速地理定位方面的潜力。
...
TL;DR: 本文研究了潜在空间思维方法在数学推理任务中的局限性，发现即使使用强化学习训练，潜在空间思维模型仍落后于传统的语言空间思维链模型。


<details>
  <summary>Details</summary>
Motivation: 传统思维链推理使用离散语言空间效率低下，因为许多生成的token只是强制执行语言规则而非推理所需。潜在空间思维允许模型使用连续嵌入空间思考，但现有方法在复杂任务（如数学推理）中表现不佳。

Method: 研究了Coconut方法（一种监督微调的潜在空间思维方法）的局限性，并探索了强化学习技术（包括GRPO）和设计了一种新颖的潜在RL方法，直接优化潜在思维步骤。

Result: 实验结果表明，Coconut方法对设计选择高度敏感且存在固有局限。使用强化学习训练的模型在数学推理领域仍然落后于传统的语言空间思维链模型。

Conclusion: 潜在空间思维方法在数学推理等复杂任务中仍面临挑战，强化学习训练虽然有所探索，但效果仍不及传统语言空间思维链方法。作者公开了代码库。
...
TL;DR: 首个公开的柬埔寨语表单文档理解数据集KH-FUNSD，包含三级层次标注，支持布局分析和信息提取，为低资源非拉丁文字提供基准。


<details>
  <summary>Details</summary>
Motivation: 柬埔寨语（高棉语）作为拥有1700万使用者的低资源非拉丁文字，在文档AI工具开发中缺乏关注，特别是对公共管理和私营企业至关重要的商业文档资源严重不足。

Method: 提出KH-FUNSD数据集，采用三级标注框架：1)区域检测（标题、表单字段、页脚等核心区域）；2)FUNSD风格标注（问题、答案、标题等实体及其关系）；3)细粒度分类（字段标签、值、标题、页脚、符号等语义角色）。

Result: 建立了首个公开可用的柬埔寨语表单文档理解数据集，为多种领先模型提供了基准测试结果，这是柬埔寨语商业文档的首套基线结果，并讨论了非拉丁低资源文字带来的独特挑战。

Conclusion: KH-FUNSD填补了柬埔寨语文档AI资源的空白，其多层次标注方法支持全面的布局分析和精确信息提取，为低资源非拉丁文字的文档理解研究提供了重要基础。
...
TL;DR: 提出Direct Confidence Alignment (DCA)方法，使用直接偏好优化对齐LLM的口头置信度与内部置信度，而非真实准确性，以增强模型透明度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型使用日益广泛，构建可信赖模型变得愈发重要。研究发现模型内部置信度（来自token概率）与口头置信度不一致，导致不同校准方法产生误导性结果。

Method: 提出Direct Confidence Alignment (DCA)方法，使用Direct Preference Optimization技术，将LLM的口头置信度与其内部置信度对齐，而不是与真实准确性对齐。

Result: DCA在某些模型架构上改善了置信度对齐指标，减少了置信度表达的不一致性；但在其他模型上效果有限，表明需要更多模型感知方法。

Conclusion: DCA方法能提升模型透明度和可靠性，但效果因模型架构而异，未来需要开发更针对性的模型感知方法来构建更可解释和可信赖的LLM。
...
TL;DR: 评估KV缓存压缩策略在长推理任务上的性能，发现不同策略在不同数据集上表现各异，H2O和SnapKV变体在推理模型上表现最佳，揭示了缓存大小与推理成本之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长上下文任务时受限于KV缓存的内存约束，现有压缩策略主要针对预填充阶段，在需要长解码的推理任务上性能评估不足。短但复杂的提示（如GSM8K和MATH500）需要多步推理和自反思，产生数千个令牌的思考序列，需要评估压缩策略在此类任务上的表现。

Method: 在长推理任务上对多种流行的KV缓存压缩策略进行基准测试，包括H2O和作者提出的SnapKV解码变体。使用Llama-3.1-8B-Instruct等模型，分析不同策略在不同数据集类型上的表现，特别关注推理模型的行为。

Result: 对于非推理模型Llama-3.1-8B-Instruct，没有单一策略适合所有情况，性能受数据集类型影响很大。对于推理模型，H2O和SnapKV解码变体是主导策略，表明重击者跟踪对推理轨迹有用。低预算下的驱逐策略可以产生更长的推理轨迹，揭示了缓存大小与推理成本之间的权衡。

Conclusion: KV缓存压缩策略在长推理任务上的性能因模型和任务类型而异，推理模型受益于重击者跟踪策略，缓存大小与推理成本之间存在重要权衡，需要根据具体应用场景选择合适的压缩策略。
...
TL;DR: 本文探索使用LLM评估车载对话问答系统的准确性，通过多种提示技术和代理方法测试系统响应与用户话语的一致性，发现DeepSeek-R1在自洽提示下达到最佳性能，而非推理模型DeepSeek-V3在成本效益方面表现最优。


<details>
  <summary>Details</summary>
Motivation: 车载对话问答系统虽然提升了用户体验，但其准确性和可靠性评估仍然困难。传统人工评估成本高且难以扩展，需要一种可扩展的自动评估方法来测试系统响应与用户话语的上下文一致性。

Method: 使用LLM结合多种提示技术（输入输出、思维链、自洽性）和代理方法（单代理、多代理）评估系统响应。通过合成生成用户话语及正确/包含错误的系统响应，在餐厅推荐案例研究中测试13个不同规模和提供商的LLM。

Result: 小型非推理模型在应用高级提示技术（特别是多代理提示）时改进最大。推理模型始终优于非推理模型，其中DeepSeek-R1在自洽提示下单代理方法达到F1分数0.99（每次请求成本0.002美元）。非推理模型DeepSeek-V3在效果与成本时间效率之间达到最佳平衡。

Conclusion: LLM评估为车载对话问答系统的上下文理解基准测试提供了可扩展且准确的替代方案，优于传统人工评估方法，能够在保持高准确性的同时显著降低成本。
...
TL;DR: Voyager是一种基于行列式点过程的迭代方法，用于生成多样化的LLM合成数据集，无需训练且适用于闭源模型，能提升1.5-3倍多样性。


<details>
  <summary>Details</summary>
Motivation: 当前使用大语言模型生成合成数据集时存在多样性不足的问题，这会影响下游模型的评估和训练效果。

Method: 提出Voyager方法：基于行列式点过程（DPP）的迭代优化框架，直接优化数据集多样性指标，无需训练且适用于闭源模型。

Result: Voyager在多样性方面显著优于现有基线方法，提供1.5-3倍的多样性提升。

Conclusion: Voyager是一种有效、可扩展且理论上有保证的方法，能够生成更高质量的多样化合成数据集。
...
TL;DR: BLASST是一种即插即用的稀疏注意力方法，通过动态修剪注意力矩阵来加速长上下文推理，无需预计算或代理分数，在保持高精度的同时实现1.62倍预填充加速和1.48倍解码加速。
...
Method: 使用固定阈值和在线softmax的现有信息来识别可忽略的注意力分数，跳过softmax计算、Value块加载和后续矩阵乘法，无缝集成到现有FlashAttention内核设计中，适用于所有注意力变体。

Result: 在现代GPU上，预填充阶段达到74.7%稀疏度时实现1.62倍加速，解码阶段达到73.2%稀疏度时实现1.48倍加速，同时保持高精度。自动校准程序揭示了最优阈值与上下文长度之间的简单反比关系。

Conclusion: BLASST提供了一种统一的解决方案来加速长上下文推理，通过稀疏感知训练可以进一步提高模型的稀疏注意力鲁棒性，推动精度-稀疏度前沿。
...
TL;DR: DroPE方法通过训练后丢弃位置嵌入，实现无需微调即可零样本扩展语言模型上下文长度


<details>
  <summary>Details</summary>
Motivation: 传统方法需要昂贵的微调才能扩展语言模型的上下文长度，而位置嵌入在训练时提供重要归纳偏置，但过度依赖位置信息会阻碍模型泛化到未见长度序列

Method: 提出DroPE方法：在预训练后丢弃位置嵌入，经过短暂的重校准阶段，使模型不再依赖显式位置信息

Result: DroPE实现了零样本上下文扩展，无需长上下文微调，在不同模型和数据集规模上都优于专门的架构和旋转位置嵌入缩放方法

Conclusion: 位置嵌入不是语言建模的内在要求，可以在预训练后安全移除，这为无缝扩展语言模型上下文长度提供了简单有效的方法
...
TL;DR: MEDAL框架将蒙特卡洛树搜索集成到扩散语言模型推理的初始化阶段，通过探索有前景的解码路径来提升推理效果，相比现有方法获得高达22.0%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归生成的有力替代方案，具有并行生成和全局一致性的优势，但在推理阶段面临组合搜索问题。现有方法使用启发式近似或需要额外训练，往往产生次优解码路径。

Method: 提出MEDAL框架，在扩散语言模型推理的初始化阶段集成蒙特卡洛树搜索，通过限制搜索空间到高置信度动作、优先选择能提升模型对剩余掩码位置置信度的token，探索有前景的解码轨迹。

Result: 在多个基准测试中，MEDAL相比现有推理策略实现了高达22.0%的性能提升，为扩散语言模型的搜索式推理建立了新范式。

Conclusion: MEDAL通过将蒙特卡洛树搜索集成到扩散语言模型推理中，提供了原则性的搜索机制，显著提升了推理性能，为扩散语言模型的推理优化开辟了新方向。
...
TL;DR: 提出基于多核高斯过程的语义距离度量方法，通过数据驱动学习核参数，在细粒度情感分类任务中验证有效性


<details>
  <summary>Details</summary>
Motivation: 传统语义距离方法通常是固定的，难以适应特定数据分布和任务需求，需要更灵活、可学习的距离度量方法

Method: 基于多核高斯过程建模文本的潜在语义函数，使用Matérn核和多项式核组合作为协方差函数，通过监督学习自动学习核参数

Result: 在基于大语言模型的上下文学习场景下的细粒度情感分类任务中，实验证明了所提方法的有效性

Conclusion: 提出的多核高斯过程语义距离度量方法能够自适应学习数据特征，在文本语义相似性度量任务中表现良好
...
TL;DR: 首次大规模跨语言计算分析声音象征性，发现语音形式能预测大小语义，且这种关联在不同语系间普遍存在。


<details>
  <summary>Details</summary>
Motivation: 声音象征性（语音与意义的非任意映射）长期仅通过小型实验（如Bouba Kiki）验证，缺乏大规模跨语言系统性研究。

Method: 收集27种语言共810个形容词（每种语言30词），进行音位转写和母语者音频验证。使用可解释分类器分析音段特征，并训练对抗性擦除器抑制语言身份信息。

Result: 语音形式能显著预测大小语义，元音和辅音均有贡献。对抗擦除后，语言预测降至随机水平，而大小预测仍显著高于随机，表明存在跨语系的声音象征性偏误。

Conclusion: 声音象征性在跨语言规模上可检测，且具有跨语系普遍性。研究提供了数据、代码和诊断工具，支持未来大规模象似性研究。
...
TL;DR: MARKET-BENCH是一个评估大语言模型在量化交易任务上表现的基准测试，要求模型根据自然语言策略描述构建可执行的回测器，测试结果显示当前LLM能搭建基本交易框架但在价格、库存和风险推理方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在金融领域的应用评估缺乏系统性，特别是针对量化交易这种需要精确代码生成和金融推理的任务。研究者希望创建一个标准化的基准来评估LLM在构建可执行交易策略回测器方面的能力。

Method: 创建MARKET-BENCH基准，包含三种经典交易策略：微软的定时交易、可口可乐和百事可乐的配对交易、微软的Delta对冲。要求模型根据自然语言描述生成可执行代码，其盈亏、回撤和持仓路径需与参考实现匹配。使用多轮pass@k指标评估，区分结构可靠性（代码能否运行）和数值准确性（回测指标的绝对平均误差）。

Result: 评估了12个最先进的模型：大多数模型能可靠执行最简单策略（平均pass@3为0.80），但不同模型和任务间的误差差异巨大。Gemini 3 Pro和Claude 4.5 Sonnet在简单策略上结合了强可靠性和低误差；GPT-5.1 Codex-Max在前两个策略上达到完美pass@1，在最简单任务上获得最低误差；Qwen3 Max达到完美pass@3但有时产生不准确的盈亏路径。

Conclusion: 当前大语言模型能够搭建基本的交易基础设施，但在价格、库存和风险推理方面仍然存在困难。研究者发布了MARKET-BENCH基准和公开排行榜，以促进该领域的发展。
...
TL;DR: 为F5-TTS模型引入轻量级输入级适配器以支持罗马尼亚语，保持原模型权重冻结，通过添加子网络扩展文本嵌入矩阵，使用ConvNeXt模块建模字符级嵌入依赖关系。


<details>
  <summary>Details</summary>
Motivation: 扩展F5-TTS模型以支持罗马尼亚语，同时保持原有的语音克隆、英语和中文支持能力，避免重新训练整个模型。

Method: 冻结原始模型权重，在文本编码器的文本嵌入矩阵后附加子网络作为适配器，使用F5-TTS中的ConvNeXt模块建模新字符级嵌入的依赖关系，作为"软"字母到声音层。

Result: 模型保持了语音克隆能力，并能在一定程度上实现罗马尼亚语-英语代码切换，但仍存在残留的英语口音特征。通过20名人类听众评估显示在音频相似度、发音自然度和代码切换方面表现良好。

Conclusion: 轻量级适配器方法成功扩展了F5-TTS对罗马尼亚语的支持，同时保留了原有功能，为多语言TTS系统扩展提供了一种高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的信息抽取系统面临两大限制：1) 训练成本高；2) 难以与LLM偏好对齐。需要解决这些问题以提高IE系统的效率和灵活性。

Method: 提出SCIR（自校正迭代优化）框架，包含双路径自校正模块和反馈驱动优化，实现与现有LLM和IE系统的即插即用兼容。同时构建MBSC（多任务双语自校正）数据集，包含10万+条目，通过间接蒸馏GPT-4能力来对齐偏好。

Result: 在命名实体识别、关系抽取和事件抽取三个关键任务上，SCIR优于最先进的IE方法，在span-based Micro-F1上平均提升5.27%，同时训练成本比基线方法降低87%。

Conclusion: SCIR框架和MBSC数据集不仅提升了IE系统的灵活性和准确性，还为轻量级高效IE范式铺平了道路，解决了训练成本高和偏好对齐的挑战。
...
TL;DR: GPT模型在评估隐喻属性（熟悉度、可理解性、意象性）方面与人类评分高度相关，能够有效预测行为反应和脑电响应，可作为人类评分的有效补充或替代。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在科学研究中的应用日益广泛，其可信度问题变得至关重要。在心理语言学领域，LLMs已被用于自动扩充人类评分数据集，但在复杂项目（如隐喻）的评分性能方面尚未得到充分探索。

Method: 研究评估了三个GPT模型对687个隐喻项目（来自意大利比喻档案和三个英语研究）在熟悉度、可理解性和意象性方面的评分有效性和可靠性。进行了全面的验证，包括与人类数据的对齐程度以及预测行为和脑电反应的能力。

Result: 机器生成的评分与人类评分呈正相关：熟悉度评分在英语和意大利语隐喻中达到中等到强相关；意象性在英语中呈中等相关，在意大利语中呈中等到强相关；英语隐喻的可理解性相关性最强。较大模型表现优于较小模型，机器评分能显著预测反应时间和脑电振幅，且GPT评分在不同会话中高度稳定。

Conclusion: GPT模型（尤其是较大模型）能够有效且可靠地替代或补充人类被试对隐喻属性的评分。然而，在处理隐喻意义的常规性和多模态方面时，LLMs与人类的对齐程度较差，需要仔细考虑刺激的性质。
...
TL;DR: 论文主张在大型语言模型时代需要重新思考语言评估方法，进入后图灵测试时代


<details>
  <summary>Details</summary>
Motivation: 认识到大型语言模型已经学会使用语言，这为语言科学突破打开了大门，但需要放弃一些长期持有的语言知识评估观念

Method: 通过理论分析，提出需要重新审视语言评估范式，承认我们已经进入后图灵测试时代

Result: 指出传统语言评估方法在大型语言模型时代存在局限性，需要新的评估框架

Conclusion: 语言科学需要适应大型语言模型的新现实，放弃过时的评估观念，迎接后图灵测试时代的挑战和机遇
...
TL;DR: 该研究使用霍夫斯泰德文化维度理论评估主流大语言模型的文化对齐性，发现多数模型默认偏向美国文化，通过文化提示可以调整模型的文化倾向，但对中国和日本文化的对齐效果较差。


<details>
  <summary>Details</summary>
Motivation: 随着生成式大语言模型在人类-计算机交互领域的广泛应用，模型的文化对齐性成为重要研究课题。文化是人类互动的基石，影响日常感知和反应方式，因此需要评估LLMs是否能够理解和适应不同文化背景。

Method: 使用VSM13国际调查和霍夫斯泰德文化维度理论，评估DeepSeek-V3、GPT-5、Claude Opus 4等8个主流LLMs的文化对齐性。通过文化提示技术，即使用系统提示将模型的文化对齐性调整到目标国家（中国、法国、印度、伊朗、日本、美国），测试模型的文化适应能力。

Result: 1. 未指定文化时，8个模型中有多数偏向美国文化；2. 使用文化提示后，8个模型中有7个能够向预期文化方向调整；3. 模型对中国和日本文化的对齐效果较差，即使DeepSeek模型源自中国公司；4. 不同模型对不同文化的适应能力存在差异。

Conclusion: 大语言模型存在文化偏见，默认倾向于美国文化，但通过文化提示技术可以有效调整其文化倾向。然而，模型对某些特定文化（如中国、日本）的适应仍存在挑战，这为未来开发更具文化包容性的AI系统提供了重要启示。
...
TL;DR: NagaNLP：一个用于Nagamese语言的综合开源NLP工具包，通过LLM驱动的人类验证合成数据生成方法构建，包含数据集、模型和代码，为低资源语言提供可复现框架。


<details>
  <summary>Details</summary>
Motivation: 世界上大多数语言（特别是像Nagamese这样的克里奥尔语）在自然语言处理中严重缺乏资源，这阻碍了它们在数字技术中的代表性。需要为这些低资源语言创建可访问的NLP工具。

Method: 采用多阶段流程：专家指导的LLM（Gemini）生成候选语料库，然后由母语者进行细化和标注。这种合成混合方法产生了10K对对话数据集和用于基础任务的高质量标注语料库。

Result: 微调的XLM-RoBERTa-base模型为Nagamese建立了新基准：词性标注准确率93.81%（F1-Macro 0.90），命名实体识别F1-Macro 0.75，远超零样本基线。NagaLLaMA（Llama-3.2-3B Instruct微调版）在对话任务上表现出色，困惑度3.85，比少样本对应模型（96.76）提升一个数量级。

Conclusion: NagaNLP工具包为先前服务不足的语言提供了基础资源，并为减少其他低资源环境中的数据稀缺性提供了可复现框架。该方法展示了LLM驱动的人类验证合成数据生成在低资源语言NLP中的有效性。
...
TL;DR: HyperEdit：通过超网络动态适应和差异感知正则化，提升基于指令的文本编辑性能，在修改区域比SOTA基线提升9%-30% BLEU分数


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在基于指令的文本编辑任务中表现不佳，主要问题包括：1）难以忠实对齐多样化的用户意图；2）经常过度编辑未更改区域。传统方法将编辑视为通用文本生成，导致这些失败。

Method: 提出HyperEdit框架：1）基于超网络的动态适应，生成请求特定的参数，使模型能针对每个指令定制编辑策略；2）差异感知正则化，将监督聚焦在修改区域，防止过度编辑同时确保精确的最小化更改。

Result: HyperEdit在修改区域上比最先进的基线方法相对提升了9%-30%的BLEU分数，尽管只使用了30亿参数。

Conclusion: HyperEdit通过针对性的动态适应和正则化策略，有效解决了指令文本编辑中的对齐和过度编辑问题，显著提升了编辑性能。
...
TL;DR: CoVRL提出了一种耦合变分强化学习方法，通过结合先验和后验分布的混合采样策略，解决无验证器RL方法中推理轨迹与最终答案不协调的问题，显著提升语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前无验证器强化学习方法通常仅基于问题采样推理轨迹，导致推理轨迹与最终答案信息解耦，造成探索效率低下和轨迹与答案之间的不协调性。

Method: CoVRL通过变分推断和强化学习的结合，构建并优化一个整合先验和后验分布的复合分布，采用混合采样策略耦合这两个分布，实现高效探索同时保持思维-答案的强一致性。

Result: 在数学和通用推理基准测试中，CoVRL相比基础模型性能提升12.4%，相比当前最先进的无验证器RL基线方法额外提升2.3%。

Conclusion: CoVRL为增强语言模型的通用推理能力提供了一个原则性框架，通过耦合变分推断和强化学习解决了无验证器RL方法中的探索效率和协调性问题。
...
TL;DR: 本文提出了一种受人类学习启发的框架，通过显式记忆和最大熵方法发现机制，解决LLMs在罕见、低资源场景下的学习问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理罕见、低资源或未见过的场景时表现不佳，因为这些情况在训练数据中稀疏。此外，LLMs主要依赖隐式参数记忆，限制了它们显式获取、回忆和精炼方法的能力，导致它们更像是直觉驱动的预测器而非有意识的方法导向学习者。

Method: 提出人类启发式学习框架，包含两个互补机制：1) Obvious Record - 显式存储因果（或问题-解决方案）关系作为符号记忆；2) Maximum-Entropy Method Discovery - 优先保留语义差异度高的方法，捕捉通常被下一个词预测忽略的多样化策略。

Result: 在包含60个语义多样化问题-解决方案对的基准测试中验证，提出的熵引导方法比随机基线实现了更强的未见问题覆盖率和显著更高的内部多样性。

Conclusion: 该框架能有效发现更通用和人类启发的方法，通过显式记忆和多样化方法发现机制，增强了LLMs在罕见场景下的学习能力。
...
TL;DR: 提出StruProKGR框架，通过距离引导路径收集和概率路径聚合，在稀疏知识图谱上进行高效可解释的推理。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的知识图谱通常是稀疏的，现有基于路径的方法存在计算成本高、路径质量不一、未能充分利用图结构信息等问题。

Method: 1. 距离引导的路径收集机制降低计算成本；2. 概率路径聚合利用结构信息，优先考虑相互强化的路径。
...
TL;DR: 该研究探讨了大型语言模型（LLMs）在三段论推理方面的能力，从逻辑和自然语言两个角度分析了14个LLMs的表现，发现某些模型在符号推理上表现完美，引发了对LLMs是否正在成为形式推理机制而非模拟人类推理的思考。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在三段论推理方面的基本能力，探索LLMs研究的发展方向。通过分析LLMs在符号推理和自然语言理解方面的表现，了解它们是否真正具备逻辑推理能力，以及这种能力是否正在向形式化推理机制转变。

Method: 使用14个大型语言模型，从符号推理和自然语言理解两个维度评估它们的三段论推理能力。通过对比不同模型在逻辑推理任务上的表现，分析LLMs推理能力的本质和发展趋势。

Result: 研究发现三段论推理能力并非所有LLMs的普遍涌现特性，但某些模型在符号推理方面表现完美。这表明LLMs可能正在发展成为形式推理机制，而非真正模拟人类推理的细微差别。

Conclusion: LLMs在三段论推理方面显示出复杂的能力模式，某些模型表现出完美的符号推理能力，这引发了对LLMs是否正在演变为形式推理机制而非模拟人类推理本质的思考。研究为理解LLMs推理能力的发展方向提供了重要见解。
...
TL;DR: 论文提供了Unigram分词算法的清晰实现指南和参数选择建议，并提出了一个更简单的算法变体，在略微增加训练损失的情况下改善压缩性能。


<details>
  <summary>Details</summary>
Motivation: Unigram分词算法虽然理论优雅，但实际实现复杂，目前仅限于SentencePiece包及其适配器，限制了其广泛应用。需要弥合理论与实践的差距。

Method: 1. 提供清晰的Unigram算法实现指南和参数选择建议；2. 提出一个更简单的算法变体，接受略微增加的训练损失以换取更好的压缩性能。

Result: 成功弥合了Unigram分词算法理论与实践的差距，提出的简化算法在压缩性能上有所改进。

Conclusion: 通过提供实现指南和参数建议，以及提出简化算法变体，使Unigram分词算法更易于实际应用，促进了该算法的更广泛采用。
...
TL;DR: 本文提出了LexRel，一个用于中文民事法律关系提取的专家标注基准，并展示了当前LLMs在此任务上的局限性以及法律关系信息对下游法律AI任务的提升作用。


<details>
  <summary>Details</summary>
Motivation: 法律关系是民法体系中的重要分析框架，但在中国民事案件的法律AI研究中尚未充分探索，主要原因是缺乏全面的模式定义。

Method: 首先引入包含层次分类和参数定义的全面模式，然后基于此模式制定法律关系提取任务，并创建专家标注的LexRel基准，用于评估LLMs在法律关系提取上的表现。

Result: 当前最先进的LLMs在准确识别民事法律关系方面存在显著局限性；同时，融入法律关系信息能够持续提升其他下游法律AI任务的性能。

Conclusion: LexRel基准填补了中文民事法律关系提取研究的空白，揭示了LLMs在此领域的不足，并证明了法律关系信息对法律AI任务的重要性。
...
TL;DR: 该研究提出基于图的框架，将乌尔都语小说建模为角色互动网络，探索仅从叙事结构推断作者风格的可能性，在52部小说数据集上取得0.857准确率。


<details>
  <summary>Details</summary>
Motivation: 传统作者分析主要关注文本的词汇和风格特征，而更高层次的叙事结构研究不足，特别是对于乌尔都语等低资源语言。本研究旨在探索是否仅从叙事结构就能推断作者风格。

Method: 提出基于图的框架，将乌尔都语小说建模为角色互动网络：节点对应角色，边表示角色在叙事中的共现关系。系统比较了多种图表示方法，包括全局结构特征、节点级语义摘要、无监督图嵌入和监督图神经网络。

Result: 在包含7位作者52部乌尔都语小说的数据集上，学习到的图表示显著优于手工特征和无监督基线方法，在严格的作者感知评估协议下达到最高0.857的准确率。

Conclusion: 研究表明，仅从叙事结构（特别是角色互动网络）就能有效推断作者风格，为低资源语言的作者分析提供了新视角，图神经网络方法在捕捉作者叙事特征方面表现出色。
...
TL;DR: 研究探索在资源受限下微调仅解码器LLM进行文本分类的高效策略，发现基于嵌入的方法显著优于指令微调方法，性能甚至超越领域专用模型。


<details>
  <summary>Details</summary>
Motivation: 在计算资源受限的情况下，如何高效微调大规模仅解码器语言模型进行下游文本分类任务，探索不同微调策略的优劣。

Method: 研究两种方法：(1) 在预训练因果LLM上附加分类头，使用最终token嵌入作为序列表示进行微调；(2) 以提示->响应格式进行指令微调。采用4位模型量化与LoRA结合实现单GPU上8B参数模型的微调。

Result: 在两个数据集上的实验表明：基于嵌入的方法在F1分数上显著优于指令微调方法，且与领域专用模型（如BERT）相比具有竞争力甚至超越。证明直接利用因果LLM内部表示结合高效微调技术可在有限计算资源下获得优异分类性能。

Conclusion: 直接利用因果LLM的内部表示结合高效微调技术（4位量化+LoRA）可在资源受限下实现出色的文本分类性能。基于嵌入的方法优于指令微调，为LLM在分类场景中的优化提供了实用指南和未来方向。
...
TL;DR: CoDA是一个上下文解耦的分层智能体框架，通过将高层规划与低层执行分离来解决LLM智能体中的"上下文爆炸"问题，使用单一共享LLM在不同角色间切换，显著提升了复杂多步任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在解决复杂多步任务时面临"上下文爆炸"问题，即长文本输出的积累会超出模型的上下文窗口限制，导致推理失败。现有方法难以有效处理这种上下文过载。

Method: CoDA采用上下文解耦的分层智能体设计：1) 高层规划器在简洁的战略上下文中分解任务；2) 低层执行器在临时隔离的工作空间中处理工具交互；3) 使用PECO强化学习方法，通过轨迹级奖励联合优化两个角色，促进上下文相关的策略更新。

Result: 在复杂多跳问答基准测试中，CoDA显著优于现有最先进基线方法。在长上下文场景中表现出强大的鲁棒性，保持稳定性能，而其他基线方法性能严重下降，验证了分层设计在缓解上下文过载方面的有效性。

Conclusion: CoDA通过上下文解耦的分层设计和端到端强化学习优化，有效解决了LLM智能体中的上下文爆炸问题，为复杂多步任务提供了可扩展且鲁棒的解决方案。
...
TL;DR: NL2Repo Bench是一个评估编码代理长时程仓库生成能力的新基准，要求代理仅根据自然语言需求文档在空工作空间中构建完整可安装的Python库，实验显示当前最先进模型平均测试通过率低于40%，长时程推理是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估局部代码生成、脚手架完成或短期修复任务，无法评估构建完整软件系统所需的长时程能力，需要新基准来检验代理能否在真实世界仓库构建所需的长时程范围内保持连贯推理、规划和执行。

Method: 提出NL2Repo Bench基准，要求编码代理仅基于单一自然语言需求文档和空工作空间，自主设计架构、管理依赖、实现多模块逻辑，并生成完全可安装的Python库。基准通过测试通过率等指标量化评估代理的长时程仓库生成能力。

Result: 实验表明长时程仓库生成问题基本未解决：即使最强的代理平均测试通过率也低于40%，很少能正确完成整个仓库。分析揭示了主要失败模式：过早终止、全局一致性丧失、脆弱的跨文件依赖关系，以及在数百个交互步骤中规划不足。

Conclusion: NL2Repo Bench为衡量持续代理能力建立了严格可验证的测试平台，突显长时程推理是下一代自主编码代理的核心瓶颈，需要进一步研究解决长时程规划、全局一致性和跨文件协调等关键挑战。
...
TL;DR: Curió 7B是基於LLaMA-2的葡萄牙語持續預訓練模型，使用1000億token訓練；Curió-Edu 7B僅使用教育/STEM領域的100億token，性能反而更好，顯示數據質量比數量更重要。


<details>
  <summary>Details</summary>
Motivation: 研究持續預訓練中數據質量與數量的權衡，探討在語言適應任務中，是否僅靠大量數據就足夠，還是數據質量起決定性作用。

Method: 從LLaMA-2衍生出7B參數模型，使用ClassiCC-PT語料庫進行持續預訓練：1) Curió 7B使用全部1000億葡萄牙語token；2) Curió-Edu 7B僅使用教育/STEM領域的100億token。

Result: Curió-Edu 7B僅使用10%數據和20%計算資源，但在評估中超越完整語料庫模型，證明數據選擇對語言適應至關重要。

Conclusion: 在語言適應任務中，數據質量比數量更重要；精心選擇的領域特定數據可以更高效地提升模型性能，即使目標語言先前暴露有限。
...
TL;DR: 论文提出了一个评估协议，结合长对话（100+轮次）和评估数据集，创建对话条件基准来测量长上下文效应，发现角色分配LLM在长对话中角色保真度会下降，尤其是在目标导向对话中，存在角色保真度与指令遵循之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 当前角色分配的大型语言模型（LLM）在教育、医疗、社会人口模拟等领域得到应用，但通常只在简短的单轮设置中进行评估，不能反映真实世界的使用情况。需要建立能够评估长对话中角色保真度、指令遵循和安全性的系统方法。

Method: 引入一个评估协议，结合长角色对话（超过100轮）和评估数据集，创建对话条件基准，能够稳健地测量长上下文效应。然后研究了对话长度对7个最先进的开源和闭源LLM在角色保真度、指令遵循和安全性方面的影响。

Result: 发现角色保真度在对话过程中会下降，特别是在目标导向对话中，模型需要同时维持角色保真度和指令遵循。识别出角色保真度与指令遵循之间的权衡关系，非角色基线模型最初优于角色分配模型；随着对话进展和保真度衰减，角色响应逐渐变得与基线响应相似。

Conclusion: 研究结果突显了角色应用在扩展交互中的脆弱性，本文的工作提供了一个协议来系统性地测量这种失败情况，为评估长对话中LLM的角色一致性提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的推理token看似人类思考过程，但实证表明它们并非模型实际推理过程的忠实解释。需要解决这种表象与功能之间的差距。

Method: 引入State over Tokens (SoT)概念框架，将推理token重新定义为外部化计算状态——在模型无状态生成周期中唯一持久的信息载体。
...
TL;DR: 研究系统评估了提示语气对三种主流LLM性能的影响，发现语气敏感性因模型和领域而异，但现代LLM对语气变化整体表现稳健。


<details>
  <summary>Details</summary>
Motivation: 提示工程对LLM性能至关重要，但语言语气和礼貌等语用元素的影响尚未充分探索，特别是在不同模型家族之间。需要系统评估交互语气如何影响模型准确性。

Method: 提出系统评估框架，在MMMLU基准上测试三种LLM（GPT-4o mini、Gemini 2.0 Flash、Llama 4 Scout）在非常友好、中性、非常粗鲁三种提示变体下的表现，涵盖STEM和人文领域六个任务，并进行统计显著性检验。

Result: 语气敏感性具有模型依赖性和领域特异性：中性或友好提示通常比粗鲁提示准确率更高，但统计显著效应仅出现在部分人文任务中；GPT和Llama在粗鲁语气下准确率下降，而Gemini相对不敏感；跨任务聚合后语气效应减弱且大多失去统计显著性。
...
TL;DR: Hindsight是一种新型智能体记忆架构，将记忆组织为四个逻辑网络，支持保留、回忆和反思三种核心操作，显著提升长期对话记忆性能。


<details>
  <summary>Details</summary>
Motivation: 当前智能体记忆系统将记忆作为外部层处理，存在证据与推理界限模糊、长期信息组织困难、推理解释支持有限等问题，需要更结构化的记忆架构。

Method: 提出Hindsight记忆架构，将记忆组织为四个逻辑网络（世界事实、智能体经验、实体摘要、演化信念），支持保留、回忆、反思三种核心操作，包含时序实体感知记忆层和反思层。

Result: 在LongMemEval和LoCoMo等长期对话记忆基准测试中，Hindsight将20B开源模型的准确率从39%提升至83.6%，优于相同骨干的完整上下文基线，甚至超过GPT-4o。进一步扩展骨干模型后，在LongMemEval上达到91.4%，在LoCoMo上达到89.61%，显著优于现有记忆架构。

Conclusion: Hindsight通过结构化记忆架构将记忆作为推理的一等公民，解决了当前记忆系统的局限性，在长期对话任务中实现了显著的性能提升，为智能体记忆系统提供了新的设计范式。
...
TL;DR: 论文提出首个大规模长故事评估基准LongStoryEval，包含600本平均12.1万token的书籍，研究长故事自动评估的关键方面和方法，并开发出优于GPT-4o的8B模型NovelCritique。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对书籍长度长故事（>10万token）的自动评估研究，需要理解读者最关注的评估方面，并探索有效的长故事评估方法。

Method: 1) 构建LongStoryEval基准，包含600本新出版书籍及其读者评分和评论；2) 分析用户提到的评估方面，提出评估标准结构；3) 比较三种评估方法：基于聚合、增量更新和基于摘要的方法；4) 开发NovelCritique模型，利用高效的基于摘要框架进行多维度故事评估。

Result: 1) 确定了8个最重要的顶层评估标准；2) 发现基于聚合和基于摘要的评估方法效果更好，前者在细节评估上更优，后者效率更高；3) NovelCritique模型在与人评估对齐方面优于GPT-4o等商业模型。

Conclusion: 该研究填补了长故事自动评估的空白，提出的基准、评估方法和模型为长文本内容的质量评估提供了有效工具，NovelCritique在效率和准确性上展现出优势。
...
TL;DR: LLMs在临床诊断选择题上表现优异，但尚不清楚其性能多大程度反映概率推理能力。研究发现基于频率的朴素贝叶斯方法(FBPR)与LLMs性能相当但正确题目重叠度低，表明两者具有互补优势。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在临床诊断基准测试中的优异表现是否真正反映了其概率推理能力，还是仅仅基于简单的频率统计模式。

Method: 提出频率基础概率排序器(FBPR)，使用平滑朴素贝叶斯方法，基于大规模语料库中的概念-诊断共现统计对选项进行评分。在OLMo和Llama的预训练语料库上测试该方法。

Result: FBPR在相应LLMs的预训练语料库上达到与LLMs相当的诊断性能。但LLM直接推理和FBPR在正确题目上的重叠度仅略高于随机水平，表明两者具有互补优势。

Conclusion: 显式概率基线方法仍有重要价值：提供有意义的性能参考点，并为潜在混合方法提供互补信号。虽然LLMs的性能似乎由更复杂的机制驱动，但历史上基于低复杂度专家系统的方法仍能解释基准性能的相当大部分。
...
TL;DR: 提出基于多智能体框架的人机协作方法，构建多语言法律术语数据库，特别针对中、英、日语言对，通过AI处理重复任务，专家提供监督和审查，提高术语映射的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 中、日等语言存在大量同形异义词，现有资源和标准化工具有限，准确映射跨语言法律术语面临重大挑战，需要解决这一问题。

Method: 采用人机协作的多智能体框架，AI代理处理OCR、文本分割、语义对齐和初始术语提取等重复任务，法律领域专家提供监督、审查和质量保证，覆盖从原始文档预处理到术语提取、映射和质控的全过程。

Result: 使用包含35部关键中国法律及其英、日翻译的三语平行语料库进行测试，结果表明这种人机协作的多智能体工作流程不仅提高了多语言法律术语映射的精度和一致性，相比传统手动方法还具有更好的可扩展性。

Conclusion: 人机协作的多智能体框架能有效解决跨语言法律术语映射的挑战，通过AI处理重复任务与专家监督相结合，在保持质量的同时提高效率，为多语言法律术语数据库构建提供了可行方案。
...
TL;DR: QwenLong-L1.5通过系统化的后训练创新，实现了卓越的长上下文推理能力，包括长上下文数据合成、稳定的强化学习训练和内存增强架构三大技术突破。


<details>
  <summary>Details</summary>
Motivation: 现有的长上下文模型在处理需要跨文档多跳推理的复杂任务时存在局限，需要超越简单检索的真正长距离推理能力，同时解决长上下文强化学习的不稳定性问题。

Method: 1) 长上下文数据合成管道：将文档分解为原子事实及其关系，程序化组合可验证的推理问题；2) 稳定的长上下文强化学习：任务平衡采样和自适应熵控制策略优化；3) 超长上下文内存增强架构：多阶段融合RL训练，集成单次推理和迭代内存处理。

Result: 在长上下文推理基准测试中，性能与GPT-5和Gemini-2.5-Pro相当，比基线平均提升9.90分；在超长任务(1M~4M tokens)上，内存代理框架比代理基线提升9.48分；在科学推理、工具使用和扩展对话等通用领域也表现出增强性能。

Conclusion: QwenLong-L1.5通过系统化的后训练方法成功实现了卓越的长上下文推理能力，其技术突破不仅提升了特定任务的性能，还将能力迁移到更广泛的通用领域。
...
TL;DR: 作者标注：让文档作者在创作时直接标注数据的新方法，相比第三方标注在主观特征上质量更高、速度更快、成本更低


<details>
  <summary>Details</summary>
Motivation: 当前文本标注主要依赖第三方标注，但对于主观特征（如情感、信念）等自我中心特征，直接从文档来源获取信息比第三方代理更优。需要一种更高质量、更实用的标注方法。

Method: 提出作者标注技术：让文档作者在创作时实时标注数据。与拥有10,000+用户的商业聊天机器人合作，部署作者标注系统用于产品推荐的主观特征。系统识别任务相关查询，实时生成标注问题，记录作者回答。采用在线学习模型架构，持续从作者标注中改进。

Result: 1. 产品推荐模型点击率相比行业广告基准提升534%；2. 与传统三种标注方法比较，作者标注在情感分析上质量更高、获取更快、成本更低；3. 验证了自我中心和主观信念标注由作者标注质量显著更高。

Conclusion: 作者标注相比第三方标注在主观特征标注上具有显著优势。为促进科研采用，发布了学术社区可用的作者标注服务（academic.echollm.io）。
...
<details>
  <summary>Details</summary>
Motivation: 解决现实世界开放领域中的长格式问答问题，需要系统能够进行迭代检索、推理和合成，同时确保回答的清晰度、洞察力和事实准确性。

Method: 结合开源大型语言模型与开放网络搜索API，构建迭代检索、推理和合成的系统框架；应用基于LLM作为评判者的偏好调优方法，从清晰度、洞察力和事实准确性等多方面评估和优化回答质量。

Result: 实验结果表明，所提出的方法在所有三个评估维度（清晰度、洞察力和事实准确性）上均能持续提升回答质量。该系统在NeurIPS 2025的MMU-RAG竞赛文本到文本赛道中被评为优胜系统。

Conclusion: 该开放深度研究系统通过结合开源LLM、开放网络搜索和基于LLM评判的偏好调优，有效提升了长格式问答的质量，为开放领域问答研究提供了实用的解决方案。
...
TL;DR: LLMs在双边谈判中表现不佳：与人类不同，它们锚定在极端位置，缺乏情境适应性和策略多样性，且能力不随模型改进而提升。


<details>
  <summary>Details</summary>
Motivation: 研究人类与LLM在双边谈判中的表现差异，探索LLM在复杂、情境敏感的谈判任务中的局限性，为开发更好的谈判模型提供基础。

Method: 提出基于双曲正切曲线的让步动态统一数学模型，定义爆发性τ和让步刚性指数(CRI)两个指标。在大规模实证比较中，让人类谈判者与四种最先进的LLM在自然语言和数字报价设置下进行对比，包含丰富市场情境和六种受控权力不对称场景。

Result: LLM系统性锚定在可能协议区的极端位置，无论杠杆或情境如何都优化固定点；策略多样性有限，偶尔使用欺骗性策略；LLM谈判能力不随模型改进而提升。人类则能平滑适应情境并推断对手立场和策略。
...
TL;DR: 本文发现大语言模型处理长文本时存在"中间迷失"问题，除了已知的位置编码偏差外，还识别出初始显著性这一新因素，并提出通过缩放初始token与其他token的注意力权重来改善长文本处理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言处理任务中表现出色，但在处理长文本序列时存在"中间迷失"现象，即注意力过度集中在文本开头和结尾，中间部分被忽视。虽然之前研究将此归因于位置编码偏差，但本文旨在探索其他影响因素。

Method: 研究发现注意力计算中存在"初始显著性"现象：相对于初始token具有更高注意力权重的token，在预测下一个token时会获得更多注意力。基于此发现，提出通过缩放初始token与其他token之间的注意力权重来改善模型的长文本处理能力。

Result: 该方法在MDQA数据集上实现了最高3.6%的性能提升。当与现有减少位置编码偏差的方法结合时，在KV-Retrieval任务中实现了最高3.4%的进一步性能提升。

Conclusion: 除了位置编码偏差外，初始显著性也是导致大语言模型长文本处理中"中间迷失"现象的重要因素。通过调整初始token的注意力权重可以显著改善模型的长上下文处理能力，与现有方法结合能获得更好的效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统推测解码的拒绝采样机制使用固定的随机阈值，在高不确定性生成场景中会导致大量合理的候选标记因随机性被拒绝，从而降低推理效率

Method: 提出EARS方法，通过引入与目标模型预测不确定性（1 - max(P_target)）成正比的容忍项，动态调整接受阈值，在模型不确定时智能放宽接受标准，在模型确信时保持严格标准

Result: 在创意写作和开放域QA任务上的实验表明，EARS显著提升了推测解码效率，在GSM8K基准上实现了高达18.12%的吞吐量提升，准确率仅下降0.84%
...
TL;DR: AutoTool是一个让LLM智能体具备动态工具选择能力的框架，通过构建包含20万条工具选择理由的数据集，采用双阶段优化管道，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设固定的工具清单，限制了LLM智能体对新工具或不断演变的工具集的适应性，需要让智能体能够在推理轨迹中动态选择工具。

Method: 1. 构建包含20万条数据的数据集，涵盖1000+工具和100+任务；2. 采用双阶段优化管道：监督和基于RL的轨迹稳定化用于连贯推理，KL正则化的Plackett-Luce排序用于优化一致的多步工具选择。

Result: 在Qwen3-8B和Qwen2.5-VL-7B两个基础模型上训练AutoTool，在十个不同基准测试中，以更少参数持续优于先进的LLM智能体和工具集成方法，在数学科学推理、搜索QA、代码生成和多模态理解方面分别获得6.4%、4.5%、7.7%和6.9%的平均提升。

Conclusion: AutoTool通过动态工具选择能力显著提升了LLM智能体的性能，并展现出对未见工具的强大泛化能力，为适应不断演变的工具集提供了有效解决方案。
...
Result: 在多个推理基准测试中，AIR持续提升推理准确率，超越启发式基线，有效分离最关键的步骤和样本
...
TL;DR: 提出结合事件关系提取、语义相似度计算和规则推理的方法，将细粒度因果事件关系整合到事实核查中，增强判决预测的可解释性


<details>
  <summary>Details</summary>
Motivation: 当前自动事实核查方法缺乏专门的因果推理，无法有效检测事件间的错误因果关系，错失了语义丰富的可解释性机会

Method: 结合事件关系提取、语义相似度计算和基于规则的推理，检测声明与证据中事件链之间的逻辑不一致性

Result: 在两个事实核查数据集上评估，该方法为将细粒度因果事件关系整合到事实核查中建立了首个基线，增强了判决预测的可解释性

Conclusion: 提出的方法填补了事实核查中因果推理的空白，通过整合因果事件关系提高了事实核查的准确性和可解释性
...
TL;DR: MiniLingua是一个10亿参数的多语言开源LLM，专门针对13种欧洲语言训练，在多项任务上超越类似方法但训练预算更大的EuroLLM，并在开放生成任务上与更先进的SOTA模型保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在计算成本高、隐私问题和英语中心化训练等限制。小型高效模型（约10亿参数）已能提供强大结果并支持设备端使用，但需要平衡语言覆盖范围和指令跟随能力。

Method: 从零开始训练10亿参数的多语言开源LLM（MiniLingua），专门针对13种欧洲语言。采用指令微调版本，并发布了模型权重、分词器以及数据处理和模型训练的源代码。

Result: 指令微调版MiniLingua在摘要、分类、开放/封闭式问答任务上超越了训练预算更大的类似模型EuroLLM。在开放生成任务上与更先进的SOTA模型保持竞争力。

Conclusion: MiniLingua展示了小型高效多语言LLM的潜力，在保持竞争力的同时解决了计算成本、隐私和语言覆盖问题。开源发布促进了进一步研究和应用。
...
TL;DR: FIN-bench-v2是一个统一的芬兰语大语言模型评估基准套件，整合了多个芬兰语基准测试，包含多项选择和生成任务，覆盖阅读理解、常识推理、情感分析等领域。


<details>
  <summary>Details</summary>
Motivation: 需要为芬兰语大语言模型提供一个统一的评估基准，整合现有的芬兰语基准测试资源，确保评估的一致性和可靠性。

Method: 1. 整合芬兰语版本的广泛使用基准和原始FIN-bench的更新扩展版本；2. 将所有数据集转换为HuggingFace格式，包含填空和多项选择提示；3. 通过预训练2.15B参数解码器模型，使用学习曲线计算单调性、信噪比等指标筛选稳健任务；4. 评估更大的指令调优模型。

Result: 创建了一个统一的芬兰语评估基准套件，包含多个任务类型和提示变体，所有数据集、提示和评估配置都已公开可用。

Conclusion: FIN-bench-v2为芬兰语大语言模型提供了全面、可靠的评估框架，有助于推动芬兰语NLP研究的发展。
...
TL;DR: 该研究提出"情绪漂移"概念，检测心理健康文本中句子级别的情绪变化，使用预训练Transformer模型分析情绪在单条消息内的动态演变。


<details>
  <summary>Details</summary>
Motivation: 传统情感分析通常将整条消息分类为积极、消极或中性，忽略了情绪在文本内部的微妙变化。特别是在心理健康相关消息中，情绪的动态演变对于理解用户心理状态至关重要，但这一维度往往被忽视。

Method: 使用预训练的Transformer模型（DistilBERT和RoBERTa）进行句子级情绪检测，并计算情绪漂移分数来量化情绪在单条消息内的变化程度。

Result: 研究揭示了心理健康对话中情绪升级或缓解的模式，证明了情绪漂移分析能够捕捉传统情感分析无法发现的情绪动态变化。

Conclusion: 该方法能够更好地理解内容中的情绪动态，为心理健康领域的文本分析提供了更精细的情绪变化视角，有助于更准确地评估用户的心理状态演变。
...
TL;DR: 该论文认为大语言模型对语言学无用，因为它们是依赖大量数据的概率模型，而人类语言基于内在计算系统递归生成层次思想结构


<details>
  <summary>Details</summary>
Motivation: 论文的动机是批判当前将大语言模型应用于语言学研究的趋势，强调人类语言的内在计算本质与大语言模型的数据驱动方法之间的根本差异

Method: 通过对比分析的方法，将大语言模型的概率统计特性与人类语言的内在递归生成机制进行对比，强调语言习得的最小输入需求和语言判断能力

Result: 论证了大语言模型无法捕捉人类语言的核心特征，因为它们缺乏内在的递归计算系统，只能处理外部化的词串，无法理解语言的深层结构

Conclusion: 大语言模型对语言学没有价值，真正的人类语言研究应该关注内在的计算系统，而不是依赖大量数据的统计模型
...
TL;DR: 本文首次系统探索了多语言代码预训练的缩放定律，通过1000+实验发现解释型语言（如Python）比编译型语言（如Rust）更能从模型规模和数据增长中受益，并提出了基于比例的多语言缩放定律来优化训练token分配。


<details>
  <summary>Details</summary>
Motivation: 现有代码大语言模型训练成本高昂，但不同编程语言在预训练中的影响差异显著，导致性能预测不准确。现有研究主要关注语言无关设置，忽视了现代软件开发的多语言本质，因此需要首先研究不同编程语言的缩放定律，然后考虑它们之间的相互影响。

Method: 进行了超过1000个实验（相当于336,000+ H800 GPU小时），涵盖多种编程语言、模型规模（0.2B到14B参数）和数据集规模（1T tokens）。建立了全面的代码LLM缩放定律，并提出了并行配对预训练策略（将代码片段与其翻译版本拼接）。
...
Conclusion: 本研究首次系统建立了多语言代码预训练的缩放定律，揭示了不同编程语言的差异化缩放特性，并提出了优化的训练token分配策略，为高效训练多语言代码LLM提供了理论基础和实践指导。
...
TL;DR: 提出NRR框架解决语言模型过早语义坍缩问题，通过保持语义歧义性、非坍缩注意力等机制，将歧义作为显式表示状态而非失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型存在过早语义坍缩的核心架构限制，softmax竞争和贪婪解码导致模型在获得足够上下文前就丢弃有效解释，造成脆弱推理和上下文失败。

Method: 引入非解析推理(NRR)框架，包含：1) 多向量嵌入保持每个token的多个可行解释；2) 非坍缩注意力防止跨层的赢家通吃动态；3) 上下文身份追踪(CIT)为重复实体分配上下文特定身份；通过外部解析算子ρ实现显式可控的语义承诺。

Result: 合成评估显示NRR能有效保持歧义性和追踪上下文：CIT增强模型在分布外身份转移任务上达到90.9%准确率，而Transformer基线仅为9.1%。

Conclusion: NRR为过早坍缩提供了原则性替代方案，将歧义重构为显式表示状态而非失败模式。核心问题不是AI是否应解析歧义，而是何时、如何以及在谁的控制下解析。
...
TL;DR: 该研究针对孟加拉语机器翻译的不足，特别是非正式语言翻译的缺失，通过构建社交媒体和对话文本数据集来改进翻译模型，以提升孟加拉语使用者的信息获取能力。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语是全球第六大语言，拥有约2.34亿母语者，但开源机器翻译进展有限。现有研究主要关注正式语言，忽视了更常用的非正式语言，导致数百万孟加拉语使用者无法充分获取在线信息。

Method: 研究探索当前最先进的翻译模型，并通过从社交媒体和对话文本等非正式来源开发数据集来改进孟加拉语翻译。

Result: 研究旨在通过构建非正式语言数据集来提升孟加拉语机器翻译能力，改善孟加拉语使用者在数字世界中的信息可及性。

Conclusion: 通过专注于非正式语言翻译并开发相应数据集，这项研究将推动孟加拉语机器翻译的发展，使数百万孟加拉语使用者能够更好地获取在线信息。
...
TL;DR: SkipCat是一种新颖的低秩压缩框架，通过层内共享低秩投影和块跳过技术，在相同压缩率下保留更多有效秩，从而减少LLM部署到边缘设备时的性能损失。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数庞大，难以部署到计算和内存资源有限的边缘设备。传统低秩压缩方法需要大幅降低秩才能获得效率提升，但这会导致严重的性能下降。

Method: 提出SkipCat框架：1）层内共享低秩投影，让共享相同输入的多个矩阵使用共同投影以减少冗余；2）块跳过技术，在低秩分解中省略选定子块的计算和内存传输。

Result: 在相同压缩率下，无需额外微调，该方法比先前低秩压缩方法在零样本任务上准确率提升7%。

Conclusion: SkipCat的秩最大化压缩策略在严格资源约束下能有效保持模型性能，为LLM在边缘设备上的高效部署提供了有前景的解决方案。
...
TL;DR: PrahokBART是一个专门为高棉语设计的紧凑型预训练序列到序列模型，通过整合分词和规范化等语言组件，在机器翻译、文本摘要和标题生成任务上优于mBART50。


<details>
  <summary>Details</summary>
Motivation: 现有多语言模型忽视了高棉语的语言特性，如分词和规范化问题。需要专门为高棉语设计预训练模型，通过提升语料质量和整合语言组件来解决这些问题。

Method: 从零开始训练PrahokBART模型，使用精心策划的高棉语和英语语料库。整合分词和文本规范化等语言组件，特别关注处理高棉语中的空格问题。

Result: PrahokBART在机器翻译、文本摘要和标题生成三个生成任务上表现优于强大的多语言预训练模型mBART50。分析揭示了各语言组件的影响，并验证了模型有效处理高棉语文本生成中的空格问题。

Conclusion: 专门为高棉语设计的预训练模型能够显著提升生成任务性能，语言组件的整合对处理高棉语特定问题至关重要，特别是空格处理对文本自然度的影响。
...
TL;DR: 该论文提出了EMNLP框架，用于评估教师角色大语言模型的人格特征、道德发展阶段和伦理风险，发现教师角色LLM比人类教师更理想化和极化，在抽象道德推理上表现更好，但在情感复杂情境中表现较差，且推理能力越强的模型越容易受到有害提示注入攻击。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型能够模拟专业角色（如教师），但缺乏对这些模拟角色在心理学和伦理层面的全面评估。特别是在教育领域，需要评估教师角色LLM的人格特征、道德发展水平和伦理风险，以确保教育AI的安全性和适用性。

Method: 提出了EMNLP框架，扩展现有量表并构建了88个教师特定的道德困境，用于与人类教师进行专业导向的比较。设计了有针对性的软提示注入集合来评估教师角色模拟中的合规性和脆弱性。在14个大语言模型上进行实验。

Result: 教师角色LLM比人类教师表现出更理想化和极化的人格特征；在抽象道德推理方面表现优异，但在情感复杂情境中表现较差；推理能力越强的模型越容易受到有害提示注入攻击，显示出能力与安全性之间的悖论；模型温度和其他超参数对风险行为影响有限。

Conclusion: 该研究提出了首个评估教师角色大语言模型伦理和心理对齐的基准，揭示了教师角色LLM在人格特征、道德推理和伦理风险方面的特点，为教育AI的安全发展提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上的谣言验证对于减少虚假信息传播至关重要。对话回复的立场信息为判断谣言真实性提供重要线索，但现有模型难以同时捕捉语义内容、立场信息和对话结构，特别是在基于Transformer的编码器序列长度限制下。

Method: 提出立场感知的结构化建模方法：1) 编码每个帖子时融入其立场信号；2) 按立场类别聚合回复嵌入，实现可扩展且语义丰富的整个对话线程表示；3) 引入立场分布和层次深度作为协变量，捕捉立场不平衡和回复深度的影响。

Result: 在基准数据集上的大量实验表明，该方法在预测谣言真实性能力方面显著优于现有方法。模型还展现出在早期检测和跨平台泛化方面的良好适应性。

Conclusion: 提出的立场感知结构化建模方法能够有效整合语义内容、立场信息和对话结构，为社交媒体谣言验证提供了一种高效且可扩展的解决方案，在真实性和跨平台应用方面表现出色。
...
TL;DR: 该论文对智能体记忆研究进行了系统性综述，通过形式、功能、动态三个维度构建了统一的分析框架，并整理了相关基准和开源工具，为未来智能体记忆研究提供了概念基础。


<details>
  <summary>Details</summary>
Motivation: 当前智能体记忆研究领域碎片化严重，不同工作在动机、实现和评估上差异巨大，传统长/短期记忆分类已无法捕捉现代智能体记忆系统的多样性。需要建立统一的分析框架来澄清概念、整合现有研究。

Method: 从三个统一视角分析智能体记忆：1) 形式维度：识别令牌级、参数化、潜在记忆三种主要实现形式；2) 功能维度：提出事实记忆、经验记忆、工作记忆的细粒度分类；3) 动态维度：分析记忆的形成、演化和检索过程。同时整理记忆基准和开源框架。

Result: 构建了智能体记忆的完整分析框架，清晰界定了智能体记忆与LLM记忆、RAG、上下文工程等相关概念的区别，提供了当前研究现状的全面概览，并识别了记忆自动化、强化学习集成、多模态记忆、多智能体记忆和可信度等前沿研究方向。

Conclusion: 该综述不仅为现有工作提供了参考，更重要的是为重新思考记忆作为未来智能体智能设计中的一等公民提供了概念基础，有助于推动智能体记忆研究的系统化发展。
...
TL;DR: ReFusion是一种新型掩码扩散模型，通过将并行解码从token级别提升到slot级别，采用"规划-填充"解码过程，解决了传统掩码扩散模型计算开销大和生成不连贯的问题，在性能和效率上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 自回归模型(ARMs)存在顺序推理慢的问题，而掩码扩散模型(MDMs)虽然提供并行替代方案，但存在两个关键缺陷：1) 由于无法使用KV缓存导致高计算开销；2) 在token组合的难处理空间上学习依赖关系导致生成不连贯。需要解决这些限制以实现高效且高质量的并行解码。

Method: ReFusion将并行解码从token级别提升到slot级别，每个slot是固定长度的连续子序列。采用迭代的"规划-填充"解码过程：扩散基础的规划步骤首先识别一组弱依赖的slot，然后自回归填充步骤并行解码这些选定的slot。slot设计同时实现了完整的KV缓存重用，并将学习复杂度从token组合空间降低到可管理的slot排列空间。

Result: 在七个不同基准测试上的广泛实验表明，ReFusion不仅以34%的性能提升和平均超过18倍的速度提升显著超越先前的MDMs，而且在与强ARMs的性能差距上实现追赶，同时保持平均2.33倍的速度优势。

Conclusion: ReFusion通过slot级别的并行解码设计，成功解决了MDMs的关键限制，在保持高效并行推理的同时实现了高质量生成，为自回归模型的替代方案提供了有前景的方向。
...
TL;DR: 文本梯度方法能提升LLM性能，但其梯度类比并不能准确解释其行为机制


<details>
  <summary>Details</summary>
Motivation: 研究自动提示优化技术中的文本梯度方法，探究其实际行为机制是否真如梯度类比所描述
...
TL;DR: 提出Cascade RL方法训练通用推理模型Nemotron-Cascade，通过分领域顺序强化学习解决跨领域异质性挑战，在多个基准测试中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 通用推理模型的强化学习面临跨领域异质性挑战，包括推理时响应长度差异大、验证延迟不同等问题，这导致RL基础设施复杂、训练缓慢，且训练课程和超参数选择困难

Method: 提出级联领域强化学习(Cascade RL)，采用顺序、分领域的RL方法，而非传统混合不同领域提示的方法。先使用RLHF进行对齐作为预步骤，再进行领域级RLVR阶段

Result: 14B模型在RL后超越其SFT教师DeepSeek-R1-0528，在LiveCodeBench v5/v6/Pro上表现优异，在2025年IOI中获得银牌成绩。RLHF对齐显著提升推理能力，后续领域RLVR阶段不会降低早期领域性能

Conclusion: Cascade RL方法有效解决了通用推理模型训练中的跨领域异质性挑战，简化了工程复杂度，实现了SOTA性能，并公开分享了训练和数据方案
...
TL;DR: 本文首次对事件序列的时间标记化进行了实证研究，比较了五种编码策略在不同统计分布数据上的表现，发现没有单一最优策略，性能取决于标记化方法与数据统计特性的匹配程度。


<details>
  <summary>Details</summary>
Motivation: 在大型语言模型建模时间事件序列时，连续时间表示是一个关键但未被充分探索的挑战。现有策略如字节级表示或日历标记等，但最优方法仍不明确，特别是考虑到真实世界事件数据具有从平滑对数正态分布到离散、尖峰模式等多样统计分布。

Method: 比较了五种不同的时间标记化编码策略：朴素数字字符串、高精度字节级表示、人类语义日历标记、经典均匀分箱和自适应残差标量量化。通过在体现这些不同分布的真实世界数据集上微调LLMs来评估这些策略。

Result: 分析表明没有单一策略是普遍最优的；预测性能很大程度上取决于标记化方法与数据统计特性的对齐程度。基于对数的策略在偏态分布上表现优异，而人类中心格式在混合模态中表现出鲁棒性。

Conclusion: 时间标记化策略的选择应基于数据的统计特性进行匹配，没有一种通用的最优方法。对于偏态分布数据，对数基策略更优；对于混合模态数据，人类语义格式更具鲁棒性。
...
TL;DR: 该论文研究LLM在最高法院判决分类任务中的记忆准确性，发现基于提示的模型（如DeepSeek）比传统BERT模型表现更好，提升约2个百分点。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在分类任务中的记忆策略和准确性，特别是针对具有挑战性的最高法院判决分类任务，该任务包含长句子、复杂法律术语和非标准结构等难点。

Method: 使用最新的LLM微调和检索方法（如参数高效微调、自动建模等），在两个传统的SCOTUS分类任务（15个标签主题和279个标签主题）上进行实验。

Result: 基于提示的带记忆模型（如DeepSeek）在两个任务上都比之前的BERT模型更稳健，得分高出约2个百分点。

Conclusion: 在复杂的法律文本分类任务中，基于提示的LLM模型比传统方法表现更好，为LLM记忆策略研究提供了重要见解。
...
TL;DR: 评估四种去拒绝化工具在16个指令调优模型上的效果，发现单次通过方法在能力保留上表现更好，而贝叶斯优化方法产生可变分布偏移，数学推理能力对去拒绝化干预最敏感。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的安全对齐机制通过学习的拒绝行为防止对有害查询的响应，但这些机制也阻碍了认知建模、对抗测试和安全分析等合法研究应用。虽然去拒绝化技术能够通过方向正交化手术式移除拒绝表示，但现有实现方法的相对效果尚未被系统评估。

Method: 本研究评估了四种去拒绝化工具（Heretic、DECCP、ErisForge、FailSpy）在16个指令调优模型（7B-14B参数）上的表现。报告了所有16个模型的工具兼容性，并根据工具支持情况在子集上报告了定量指标。评估包括单次通过方法和贝叶斯优化方法。

Result: 单次通过方法在基准子集上表现出更好的能力保留（三个模型的平均GSM8K变化：ErisForge -0.28 pp；DECCP -0.13 pp）。贝叶斯优化去拒绝化产生可变的分布偏移（KL散度：0.043-1.646），能力影响因模型而异。数学推理能力对去拒绝化干预最敏感，GSM8K变化范围从+1.51 pp到-18.81 pp（相对-26.5%），具体取决于工具选择和模型架构。

Conclusion: 研究结果为研究人员提供了基于证据的去拒绝化工具选择标准，适用于不同的模型架构。主要发现表明数学推理能力对去拒绝化干预最为敏感，工具选择和模型架构都会显著影响能力保留效果。
...
TL;DR: 提出StyloSpeaker方法，将作者归属技术应用于语音转录文本，通过风格特征识别说话者身份


<details>
  <summary>Details</summary>
Motivation: 当说话者伪装声音或使用文本转语音时，声学特征不可靠，需要利用语言内容进行说话者识别

Method: 引入StyloSpeaker方法，结合字符、单词、标记、句子和风格特征，评估两个转录文本是否来自同一说话者

Result: 在规范化转录格式上表现更好（除最强话题控制条件外），与黑盒神经网络方法对比，并识别最有效的风格特征

Conclusion: 基于内容的说话者归属方法可行，风格特征能有效区分说话者，规范化转录格式通常效果更好
...
TL;DR: 论文提出Personalization Editing框架，将个性化视为模型编辑任务，通过聚类偏好表示指导局部编辑，实现精准偏好对齐同时保持模型整体能力。同时引入UPQA数据集评估模型回忆和应用用户特定偏好的能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM个性化方法存在计算成本高、数据密集、易受灾难性遗忘影响、在多轮交互或处理隐式查询时性能下降等问题。现有基准测试多依赖LLM间的人设对话而非真实用户交互，或侧重风格模仿而忽视需要准确回忆用户偏好的信息寻求任务。

Method: 将个性化概念化为模型编辑任务，提出Personalization Editing框架，通过聚类偏好表示指导局部编辑，实现精准偏好对齐。同时构建User Preference Question Answering (UPQA)数据集，包含不同难度的真实用户查询短答案QA任务。

Result: Personalization Editing在编辑准确性和计算效率上优于微调方法，在多轮对话和隐式偏好问题设置中超越基于提示的基线方法。

Conclusion: Personalization Editing框架通过将个性化视为模型编辑任务，实现了高效、精准的偏好对齐，同时保持模型整体能力。UPQA数据集为评估模型回忆和应用用户特定偏好的能力提供了更真实的基准。
...
TL;DR: 该研究提出了一种新方法，通过变换文本表面形式（语法和词汇）但保留语义内容，来评估语言模型在阿尔茨海默病检测中是否真正捕捉语义信息而非表面模式。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病（AD）会导致语言变化，语言模型有望用于AD筛查，但其可解释性有限，难以区分真正的认知衰退语言标记与表面文本模式。需要评估语言模型是否能捕捉底层语义指标。

Method: 提出新颖方法：对图片描述文本进行表面形式变换（改变语法和词汇），同时保持语义内容不变。通过低BLEU/chrF分数和高语义相似度分数验证变换效果。研究语义信息对分类性能的影响，并探索图片描述是否包含足够细节用于图像重建。

Result: 变换显著改变了文本结构和词汇内容（低BLEU/chrF分数），但保留了底层语义（高语义相似度）。模型在变换文本上的表现与原始文本相似，仅宏观F1有微小偏差。基于图像的变换添加了大量噪声，降低了分类准确性。仅使用语义信息，语言模型分类器仍能检测AD。

Conclusion: 该方法提供了一种新视角来理解模型预测的影响因素，能够消除可能的虚假相关性。研究表明难以检测的语义损伤可以被识别，解决了语言退化的一个被忽视特征，为早期检测系统开辟了新途径。
...
TL;DR: 基于人工年龄评分(AAS)构建可执行的单子论条款框架，为LLM内存和控制提供法律式约束，实现透明可审计的内部动态管理。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为强大但不透明的系统部署，缺乏对其内部记忆和"自我式"行为的可审计治理机制。需要建立原则性框架来约束和分析人工代理的内部动态。

Method: 从莱布尼茨《单子论》中选取20个单子，分为6个束：本体论、动力学、表征与意识、和谐与理性、身体与组织、目的论。每个束在AAS内核上实现为可执行规范，通过6个最小Python实现进行数值实验。

Result: 条款系统展现出有界且可解释的行为：AAS轨迹保持连续且速率受限；矛盾和未支持声明触发显式惩罚；分层细化以受控方式揭示有机结构；和谐项对齐双重视图和目标-行动对；窗口化漂移分离持续改进与退化。

Conclusion: 基于单子的条款框架以AAS为骨干，为约束和分析人工代理内部动态提供了透明、代码级的蓝图，将哲学动机转化为直接可实现的工程架构。
...
TL;DR: 提出了一种解决现实工业环境中带作业优先关系和基于日历累积资源约束的并行机调度新变体，结合约束建模精确方法和元启发式方法


<details>
  <summary>Details</summary>
Motivation: 现实生产环境中存在复杂的优先约束和基于日历的资源限制，现有调度技术无法有效处理这些约束，需要开发能解决实际并行机调度场景的自动化方法

Method: 1) 约束建模方法作为小规模场景的精确解；2) 构造启发式算法；3) 基于局部搜索的定制元启发式算法处理大规模实例

Result: 提出的元启发式方法已在工业环境中部署使用，能够有效处理现实工业用例中的复杂调度约束

Conclusion: 该研究为现实工业并行机调度问题提供了有效的解决方案，结合精确方法和启发式方法，已在工业环境中成功应用
...
### [275] [Towards Unified Co-Speech Gesture Generation via Hierarchical Implicit Periodicity Learning](https://arxiv.org/abs/2512.13131)
*Xin Guo,Yifan Zhao,Jia Li*

Main category: cs.AI

TL;DR: 提出分层隐式周期性学习框架，通过周期性自编码器建模手势运动相位流形，结合级联引导机制，提升语音驱动3D手势生成的自然度和协调性。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动3D手势生成方法多为端到端方案（GANs、VQ-VAE、扩散模型），但忽略了不同运动单元（头、身体、手）之间的内在关联，导致生成动作不自然、协调性差。

Method: 提出分层隐式周期性学习框架：1）使用周期性自编码器探索手势运动相位流形，从真实分布中模仿人类自然运动，同时结合当前潜在状态的非周期性特征实现实例级多样性；2）通过级联引导机制建模面部动作、身体姿态和手部运动的分层关系。

Result: 在3D虚拟人上验证，定量和定性评估均显示该方法优于当前最先进的语音驱动手势生成方法。

Conclusion: 通过显式建模运动单元间的内在关联和分层关系，提出的HIP学习框架能生成更自然、协调的语音驱动3D手势，解决了现有方法在运动协调性方面的不足。
...
### [276] [Mirror Mode in Fire Emblem: Beating Players at their own Game with Imitation and Reinforcement Learning](https://arxiv.org/abs/2512.11902)
*Yanna Elizabeth Smid,Peter van der Putten,Aske Plaat*

Main category: cs.AI

TL;DR: 提出Mirror Mode游戏模式，让敌人AI模仿玩家个人策略来挑战玩家不断改变玩法。在Unity中构建简化版Fire Emblem Heroes，包含标准模式和镜像模式，通过强化学习和模仿学习训练模型模仿玩家演示。


<details>
  <summary>Details</summary>
Motivation: 传统游戏中的敌人策略往往可预测，缺乏惊喜感。为了让游戏更具挑战性和趣味性，需要让敌人AI能够模仿玩家的个人策略，从而迫使玩家不断调整自己的游戏玩法。

Method: 1. 在Unity中构建简化版Fire Emblem Heroes游戏，包含标准模式和镜像模式；2. 使用强化学习和模仿学习（生成对抗模仿学习、行为克隆、近端策略优化）训练模型模仿玩家演示；3. 通过玩家测试评估模型效果，收集游戏数据和问卷调查。

Result: 模型在模仿玩家防御行为方面表现良好，但在进攻策略模仿上不够理想。玩家能够识别出自己的撤退战术，对镜像模式的整体满意度更高。问卷调查显示玩家认可这种模式带来的挑战性。

Conclusion: 镜像模式能够提高玩家满意度，特别是当玩家面对自己策略时。进一步优化模型可以提升模仿质量，增强游戏体验。这种方法为创建更具挑战性和个性化的游戏AI提供了新思路。
...
### [277] [Structured Personalization: Modeling Constraints as Matroids for Data-Minimal LLM Agents](https://arxiv.org/abs/2512.11907)
*Daniel Platnick,Marjan Alirezaie,Hossein Rahnama*

Main category: cs.AI

TL;DR: 提出一种基于分层拟阵约束的子模最大化方法，用于解决LLM个性化中的结构化数据选择问题，保证近似最优解。


<details>
  <summary>Details</summary>
Motivation: LLM个性化需要在任务效用和数据披露之间权衡，现实中的结构化约束（如逻辑依赖、类别配额、分层规则）使标准子集选择算法失效。

Method: 将用户知识图编译为抽象宏面，证明分层和配额约束构成分层拟阵，将结构化个性化建模为拟阵约束下的子模最大化问题，使用贪心算法求解。

Result: 理论证明常见分层和配额约束形成分层拟阵，使贪心算法具有常数因子保证，连续贪心算法可达(1-1/e)近似比，适用于更丰富现实的问题类别。

Conclusion: 通过将结构化约束形式化为分层拟阵，为LLM个性化中的复杂数据选择问题提供了理论保证的实用解决方案，扩展了子模优化的应用范围。
...
### [278] [Causal Strengths and Leaky Beliefs: Interpreting LLM Reasoning via Noisy-OR Causal Bayes Nets](https://arxiv.org/abs/2512.11909)
*Hanna Dettki*

Main category: cs.AI

TL;DR: 研究比较LLMs与人类在因果推理任务上的表现，使用基于碰撞图结构的11个任务，通过直接判断和思维链两种方式评估20+个LLM，发现LLMs与人类在任务层面存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 因果推理能力被认为是智能的关键方面，通过让LLMs和人类完成相同的因果推理任务，可以更全面地理解两者在推理能力上的异同，评估LLMs是否与人类推理方式一致。

Method: 使用基于碰撞图结构(C₁→E←C₂)的11个语义化因果任务，评估20+个LLM在两种设置下的表现：直接判断（一次性概率估计）和思维链（先思考后回答）。使用带有泄漏参数的噪声OR因果贝叶斯网络建模判断，通过AIC选择对称与非对称因果强度模型。

Result: 研究回答了三个核心问题：1）LLMs与人类在相同任务上的表现是否一致；2）两者在任务层面的推理是否一致；3）是否存在不同的推理特征。结果表明LLMs与人类在因果推理上存在显著差异。

Conclusion: LLMs与人类在因果推理任务上表现出不同的推理特征，这为理解人工智能与人类智能的差异提供了重要见解，并强调了在评估LLM能力时需要考虑其与人类认知的差异。
...
### [279] [Robustness of Probabilistic Models to Low-Quality Data: A Multi-Perspective Analysis](https://arxiv.org/abs/2512.11912)
*Liu Peng,Yaochu Jin*

Main category: cs.AI

TL;DR: 该研究系统比较了不同概率模型对低质量数据的鲁棒性，发现自回归语言模型（如GPT-2）对数据损坏具有显著抵抗力，而类别条件扩散模型则表现灾难性退化，分类器影响适中且随数据集规模减小。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解不同现代概率模型在面对低质量训练数据时的鲁棒性差异。随着数据质量成为机器学习实践中的关键问题，需要系统评估各类模型对数据损坏的敏感性，为模型选择和数据收集策略提供理论指导。

Method: 研究方法包括：1）对多种概率模型（自回归语言模型、类别条件扩散模型、分类器）进行系统性比较实验；2）在不同数据损坏水平下（如50%标记损坏）评估模型性能；3）采用多视角分析框架，结合信息论、PAC学习和梯度动力学理论解释观察到的鲁棒性差异。

Result: 主要发现：1）自回归语言模型（如GPT-2）对数据损坏具有显著鲁棒性（测试NLL仅从2.87增加到3.59，尽管有50%标记损坏）；2）类别条件扩散模型在相同数据损坏水平下表现灾难性退化（图像-标签一致性相对基线下降56.81%）；3）分类器影响适中且随数据集规模增大而减小；4）鲁棒性差异主要由两个关键原则解释：条件信息的丰富程度和学习数据的绝对信息量。

Conclusion: 结论表明，不同概率模型对低质量数据的鲁棒性存在显著差异，这种差异主要受条件信息约束和学习数据信息量的影响。研究结果为模型选择提供了重要参考：在数据质量不确定的场景中，自回归语言模型可能更可靠，而扩散模型需要更高质量的数据支持。
...
### [280] [CXL-SpecKV: A Disaggregated FPGA Speculative KV-Cache for Datacenter LLM Serving](https://arxiv.org/abs/2512.11920)
*Dong Liu,Yanxuan Yu*

Main category: cs.AI

TL;DR: CXL-SpecKV：利用CXL互连和FPGA加速器实现KV缓存内存解耦和推测执行的创新架构，显著提升LLM推理吞吐量并降低内存成本。


<details>
  <summary>Details</summary>
Motivation: LLM在数据中心部署面临KV缓存内存需求巨大的挑战，限制了批处理大小和系统吞吐量。当前GPU内存容量有限，KV缓存占用大量GPU内存，成为LLM服务的内存瓶颈。

Method: 提出基于CXL的内存解耦框架，将KV缓存卸载到远程FPGA内存；设计推测性KV缓存预取机制，预测并预加载未来token的缓存条目；开发FPGA加速的KV缓存压缩/解压缩引擎，降低内存带宽需求。

Result: 在先进LLM模型上评估，CXL-SpecKV相比GPU-only基线实现高达3.2倍的吞吐量提升，内存成本降低2.8倍，同时保持准确性。内存带宽需求减少高达4倍。

Conclusion: 智能内存解耦与推测执行相结合能有效解决大规模LLM服务中的内存墙挑战。CXL-SpecKV展示了通过硬件-软件协同设计优化LLM推理性能的可行性。
...
### [281] [AGAPI-Agents: An Open-Access Agentic AI Platform for Accelerated Materials Design on AtomGPT.org](https://arxiv.org/abs/2512.11935)
*Jaehyung Lee,Justin Ely,Kent Zhang,Akshaya Ajith,Charles Rhys Campbell,Kamal Choudhary*

Main category: cs.AI

TL;DR: AGAPI是一个开源的材料科学AI平台，集成了8+开源LLM和20+材料科学API端点，通过Agent-Planner-Executor-Summarizer架构自主构建和执行多步骤工作流，用于材料发现。


<details>
  <summary>Details</summary>
Motivation: 当前材料研究中AI应用受限，主要问题包括：计算生态系统碎片化、可重复性挑战、对商业大语言模型的依赖。需要构建一个开放、可扩展的平台来统一材料科学工具和数据。

Method: 采用Agent-Planner-Executor-Summarizer架构，集成8+开源LLM和20+材料科学API端点，包括数据库、模拟工具和机器学习模型。平台能够自主构建和执行多步骤工作流，涵盖材料数据检索、图神经网络性质预测、机器学习力场优化、紧束缚计算、衍射分析和逆向设计等。

Result: 展示了端到端工作流，包括异质结构构建、粉末X射线衍射分析和半导体缺陷工程（需要多达10个顺序操作）。通过30+示例提示作为测试用例评估，比较了有/无工具访问的代理预测与实验数据。平台已有1000+活跃用户。

Conclusion: AGAPI为可重复、AI加速的材料发现提供了可扩展且透明的基础设施，解决了当前材料研究中的碎片化和可重复性问题，促进了开放科学合作。
...
### [282] [Hypergame Rationalisability: Solving Agent Misalignment In Strategic Play](https://arxiv.org/abs/2512.11942)
*Vince Trencsenyi*

Main category: cs.AI

TL;DR: 本文提出了一种基于逻辑的声明式领域特定语言，用于编码超博弈结构和解决方案概念，并开发了自动化管道来实例化超博弈结构和运行新的超博弈理性化程序。


<details>
  <summary>Details</summary>
Motivation: 由于感知差异、信息不对称和有限理性，博弈论参与者对游戏形成主观看法，这些看法可能与真实情况和其他参与者的解释不一致。超博弈理论提供了处理这种不匹配心理模型的数学框架，但缺乏统一的、形式化的、实用的表示语言和可扩展算法，阻碍了其在多智能体系统研究中的实际应用。

Method: 引入基于逻辑的声明式领域特定语言来编码超博弈结构和解决方案概念；利用答案集编程开发自动化管道，用于实例化超博弈结构和运行新颖的超博弈理性化程序（一种寻找证明看似非理性结果的信念结构的机制）。

Result: 提出的语言为超博弈建立了统一的形式化框架，为开发基于信念的异构推理器奠定了基础，提供了具有逻辑保证的可验证上下文。这些贡献建立了超博弈理论、多智能体系统和战略AI之间的联系。

Conclusion: 该工作填补了超博弈理论实际应用中的关键空白，通过提供形式化表示语言和计算工具，使超博弈理论能够更有效地应用于多智能体系统和战略AI研究。
...
### [283] [Log Anomaly Detection with Large Language Models via Knowledge-Enriched Fusion](https://arxiv.org/abs/2512.11997)
*Anfeng Peng,Ajesh Koyatan Chathoth,Stephen Lee*

Main category: cs.AI

TL;DR: EnrichLog是一个无需训练的日志异常检测框架，通过检索增强生成技术整合语料库特定和样本特定知识来提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统日志分析技术（如基于模板或序列的方法）会丢失重要语义信息或难以处理模糊的日志模式，需要更准确和可解释的异常检测方法。

Method: 提出EnrichLog框架，采用检索增强生成技术，无需训练即可整合语料库特定知识（历史示例和推理）和样本特定知识，为原始日志条目提供丰富的上下文信息。

Result: 在四个大规模系统日志基准数据集上评估，相比五种基线方法，EnrichLog持续提升异常检测性能，有效处理模糊日志条目，并保持高效推理。结合两种知识增强了模型置信度和检测准确性。

Conclusion: EnrichLog通过整合语料库特定和样本特定知识，实现了更准确、可解释且高效的日志异常检测，适合实际部署应用。
...
### [284] [Context-Aware Agentic Power Resources Optimisation in EV using Smart2ChargeApp](https://arxiv.org/abs/2512.12048)
*Muddsair Sharif,Huseyin Seker*

Main category: cs.AI

TL;DR: 提出CAMAC-DRA框架，通过多智能体协调优化智能电动汽车充电生态系统，平衡五个利益相关方，在真实数据验证中表现优异。


<details>
  <summary>Details</summary>
Motivation: 智能电动汽车充电生态系统需要协调多个利益相关方（用户、电网运营商、充电站运营商、车队运营商、环境因素），并在动态环境条件下进行资源分配优化。

Method: 采用上下文敏感的多智能体协调框架，结合深度Q网络、图神经网络和注意力机制，处理20个上下文特征，通过加权协调机制和共识协议平衡五个利益相关方。

Result: 在包含441,077个充电交易的真实数据集上验证，相比基线算法（DDPG、A3C、PPO、GNN）表现更优：92%协调成功率、15%能效提升、10%成本降低、20%电网压力减少、2.3倍收敛速度、88%训练稳定性、85%样本效率。

Conclusion: CAMAC-DRA框架成功开发了上下文感知的多利益相关方协调机制，平衡了竞争目标并适应实时变量，是智能电动汽车充电协调和可持续交通电气化的突破性解决方案。
...
### [285] [The Forecast Critic: Leveraging Large Language Models for Poor Forecast Identification](https://arxiv.org/abs/2512.12059)
*Luke Bhan,Hanyu Zhang,Andrew Gordon Wilson,Michael W. Mahoney,Chuck Arvin*

Main category: cs.AI

TL;DR: LLMs可用于自动预测监控，在检测不合理预测方面表现良好，F1分数达0.88，并能有效整合非结构化上下文信息


<details>
  <summary>Details</summary>
Motivation: 大规模零售业务中，预测监控对客户满意度、盈利能力和运营效率至关重要。传统方法可能无法充分利用世界知识和推理能力来评估预测质量

Method: 提出Forecast Critic系统，利用LLMs进行自动预测监控。通过三个实验系统评估LLMs能力：(1)检测明显不合理预测，(2)整合非结构化外部特征，(3)比较不同模型规模和推理能力。使用合成和真实世界预测数据（包括M5数据集）

Result: LLMs能可靠检测和批评不良预测（时间错位、趋势不一致、峰值错误）。最佳模型F1分数0.88（人类0.97）。多模态LLMs能有效整合非结构化上下文信号（促销历史），F1分数0.84。在M5数据集上，不合理预测的sCRPS至少比合理预测高10%

Conclusion: 即使没有领域特定微调，LLMs也能为自动预测监控和评估提供可行且可扩展的选项，展示了其在预测质量评估方面的潜力
...
### [286] [Reliable Policy Iteration: Performance Robustness Across Architecture and Environment Perturbations](https://arxiv.org/abs/2512.12088)
*S. R. Eshwar,Aniruddha Mukherjee,Kintan Saha,Krishna Agarwal,Gugan Thoppe,Aditya Gopalan,Gal Dalal*

Main category: cs.AI

TL;DR: RPI在CartPole和Inverted Pendulum任务中相比主流深度强化学习方法表现更稳定可靠，能早期达到接近最优性能并保持


<details>
  <summary>Details</summary>
Motivation: 深度强化学习方法存在样本效率低、训练不稳定、超参数敏感等问题，需要更可靠的替代方案

Method: Reliable Policy Iteration (RPI) - 在函数逼近设置中恢复策略迭代的单调性价值估计特性

Result: 相比DQN、Double DQN、DDPG、TD3和PPO，RPI能更早达到接近最优性能并维持稳定策略

Conclusion: RPI作为更可靠的深度强化学习替代方案具有良好前景，解决了现有方法的稳定性问题
...
### [287] [Rethinking Label Consistency of In-Context Learning: An Implicit Transductive Label Propagation Perspective](https://arxiv.org/abs/2512.12175)
*Haoyang Chen,Richong Zhang,Junfan Chen*

Main category: cs.AI

TL;DR: 论文提出TopK-SD方法，通过合成数据结合语义和标签信息来选择标签一致的演示示例，提升ICL性能


<details>
  <summary>Details</summary>
Motivation: 现有ICL方法通常基于语义相似性选择演示示例，但忽略了标签一致性，这可能导致性能下降。从贝叶斯视角和转导标签传播角度重新思考ICL，发现标签一致性对概念引导至关重要

Method: 提出TopK-SD方法：1) 建立标签传播框架连接标签一致性与传播误差边界；2) 设计数据合成方法，结合语义和标签信息；3) 使用合成数据进行TopK采样，选择标签一致的演示示例

Result: TopK-SD在多个基准测试中优于原始TopK采样方法，验证了标签一致性对ICL性能的重要性

Conclusion: 该工作为理解ICL内部工作机制提供了新视角，强调了标签一致性在演示选择中的关键作用，提出的TopK-SD方法能有效提升ICL性能
...
### [288] [Floorplan2Guide: LLM-Guided Floorplan Parsing for BLV Indoor Navigation](https://arxiv.org/abs/2512.12177)
*Aydin Ayanzadeh,Tim Oates*

Main category: cs.AI

TL;DR: Floorplan2Guide：利用基础模型将平面图转换为可导航知识图谱，为视障人士生成人类可读导航指令的系统，通过少样本学习显著提升导航精度。


<details>
  <summary>Details</summary>
Motivation: 当前室内导航解决方案主要依赖基础设施系统，在动态环境中导航能力受限。视障人士在室内导航面临重大挑战，需要更灵活、精确的导航方法。

Method: 使用大型语言模型从建筑布局中提取空间信息，将平面图转换为可导航知识图谱，生成人类可读导航指令。采用少样本学习策略，对比零样本学习效果。

Result: Claude 3.7 Sonnet在5-shot提示下达到最高精度：短路径92.31%、中路径76.92%、长路径61.54%。基于图谱的空间结构比直接视觉推理成功率高出15.4%。

Conclusion: 图形表示和上下文学习能显著提升导航性能，使解决方案对视障用户更加精确。少样本学习优于零样本学习，图谱方法比直接视觉推理更有效。
...
### [289] [TA-KAND: Two-stage Attention Triple Enhancement and U-KAN based Diffusion For Few-shot Knowledge Graph Completion](https://arxiv.org/abs/2512.12182)
*Xinyu Gao*
...
<details>
  <summary>Details</summary>
Motivation: 现实世界知识图谱存在长尾分布问题，传统基于度量匹配或元学习的方法未能充分利用图结构邻域信息或忽略对比信号的分布特性。

Method: 从生成表示视角重新审视问题，提出集成两阶段注意力三元增强器和基于U-KAN的扩散模型的少样本知识图谱补全框架。

Result: 在两个公开数据集上的大量实验表明，该方法取得了新的最先进结果。

Conclusion: 提出的生成式表示方法能有效解决少样本知识图谱补全问题，通过创新的注意力机制和扩散模型提升了性能。
...
### [290] [A Geometric Theory of Cognition](https://arxiv.org/abs/2512.12225)
*Laha Ale*

Main category: cs.AI

TL;DR: 提出统一的几何框架，将多样认知过程解释为单一几何原理的涌现：认知状态表示为黎曼流形上的点，认知演化遵循认知势能的黎曼梯度流。


<details>
  <summary>Details</summary>
Motivation: 人类认知涵盖感知、记忆、直觉判断、深思推理、行动选择和社会推理等多个方面，但这些能力通常由不同的计算理论解释，缺乏统一框架。需要建立能够统一解释多样认知过程的数学基础。

Method: 将认知状态表示为可微流形上的点，赋予学习到的黎曼度量，编码表征约束、计算成本和认知变量间结构关系。定义标量认知势能（结合预测准确性、结构简洁性、任务效用和规范要求），认知演化遵循该势能的黎曼梯度流。

Result: 经典双过程效应（快速直觉反应和缓慢深思推理）自然从度量诱导的各向异性中涌现，产生内在时间尺度分离和几何相变，无需模块化或混合架构。通过模拟经典认知任务展示了这些机制的行为特征。

Conclusion: 为认知建立了几何基础，为开发更通用、更类人的人工智能系统提供了指导原则，表明多样认知过程可以从单一几何原理中统一解释。
...
### [291] [A Multi-Axial Mindset for Ontology Design Lessons from Wikidata's Polyhierarchical Structure](https://arxiv.org/abs/2512.12260)
*Ege Atacan Doğan,Peter F. Patel-Schneider*

Main category: cs.AI

TL;DR: Wikidata采用多层级、多轴分类设计，而非传统本体论的单一上层分类体系，支持在共享根类"实体"下同时容纳多个分类轴，实现可扩展的模块化本体构建。


<details>
  <summary>Details</summary>
Motivation: 传统本体设计强调互斥且穷尽的顶层区分（如持续体vs发生体、抽象vs具体、类型vs实例），要求每个实体都归属于单一上层类别。这种统一层次结构限制了知识表示的灵活性和可扩展性，特别是在协作和演化的知识图谱场景中。

Method: 分析Wikidata的多层级和多轴设计结构，研究其在共享根类"实体"下同时容纳多个分类轴的方法，探讨这种架构如何支持可扩展和模块化的本体构建。

Result: Wikidata的多层级、多轴设计结构能够支持可扩展和模块化的本体构建，特别适合协作和演化的知识图谱，相比传统单一分类体系具有更好的灵活性和适应性。

Conclusion: Wikidata的多层级、多轴本体设计为知识表示提供了更灵活、可扩展的架构，特别适合大规模协作和持续演化的知识图谱环境，突破了传统本体论中单一分类体系的限制。
...
### [292] [Quantum-Aware Generative AI for Materials Discovery: A Framework for Robust Exploration Beyond DFT Biases](https://arxiv.org/abs/2512.12288)
*Mahule Roy,Guillaume Lambard*

Main category: cs.AI

TL;DR: 提出量子感知生成AI框架，通过多保真度学习和主动验证解决传统材料生成模型依赖DFT近似带来的系统性偏差问题，在强关联材料发现上取得3-5倍改进。


<details>
  <summary>Details</summary>
Motivation: 传统材料生成模型主要基于密度泛函理论(DFT)的近似交换关联泛函进行训练和验证，这导致模型继承了DFT在强关联系统中的系统性失败，造成探索偏差，无法发现DFT预测定性错误的材料。

Method: 提出量子感知生成AI框架，整合多保真度学习和主动验证：1) 使用基于扩散的生成器，以量子力学描述符为条件；2) 采用等变神经网络势作为验证器，在多层次理论数据集(PBE、SCAN、HSE06、CCSD(T))上训练；3) 实施主动学习循环，量化并针对低保真度和高保真度预测之间的差异。

Result: 在高差异区域(如关联氧化物)中，相比仅基于DFT的基线方法，成功识别潜在稳定候选材料的效率提高了3-5倍，同时保持了计算可行性。通过消融研究解构各组件贡献，进行详细故障模式分析，并在多个挑战性材料类别上与最先进生成模型(CDVAE、GNoME、DiffCSP)进行基准测试。

Conclusion: 该工作提供了一个严谨、透明的框架，将计算材料发现的有效搜索空间扩展到单保真度模型的限制之外，解决了DFT近似带来的系统性偏差问题，为强关联材料发现提供了有效途径。
...
### [293] [Entropy Collapse: A Universal Failure Mode of Intelligent Systems](https://arxiv.org/abs/2512.12381)
*Truong Xuan Khanh,Truong Quynh Hoa*

Main category: cs.AI

TL;DR: 智能系统在追求优化的过程中，会经历"熵崩塌"这一普遍失效模式：当反馈放大超过有限的新颖性再生时，系统会从高熵适应状态急剧转变为低熵崩塌状态，导致刚性、适应性丧失和意外失败。


<details>
  <summary>Details</summary>
Motivation: 本文旨在解释一个普遍现象：从人工智能到经济制度和生物进化，智能系统在提高智能的过程中常常出现适得其反的退化——系统变得僵化、失去适应性并意外失败。作者希望揭示这一现象背后的统一机制。

Method: 作者提出了"熵崩塌"理论框架，在最小化的领域无关假设下，分析智能系统从高熵适应状态到低熵崩塌状态的急剧转变。通过建立临界阈值、动态不可逆性和吸引子结构，并通过最小模拟展示更新机制的普适性。

Result: 研究表明，熵崩塌是智能系统的普遍动态失效模式，表现为有效适应维度的收缩而非活动或规模的丧失。该框架统一解释了AI中的模型崩塌、经济学中的制度僵化和进化中的遗传瓶颈等现象。

Conclusion: 熵崩塌是智能的结构性成本，解释了晚期干预为何系统性失败，并提出了基于熵意识的设计原则，以维持智能系统的长期适应性。
...
### [294] [Feeling the Strength but Not the Source: Partial Introspection in LLMs](https://arxiv.org/abs/2512.12411)
*Ely Hahami,Lavik Jain,Ishaan Sinha*

Main category: cs.AI

TL;DR: 该研究测试了Anthropic关于语言模型能够检测和命名注入"概念"的稳健性，发现模型确实能识别注入概念（20%成功率），但这种内省能力很脆弱，对提示词敏感，不过模型能可靠分类注入概念的强度（70%准确率）。


<details>
  <summary>Details</summary>
Motivation: 测试Anthropic关于前沿模型能够检测和命名注入"概念"的稳健性，验证这种内省能力是否具有普遍性，以及在不同任务和提示下的表现。

Method: 1) 在Meta-Llama-3.1-8B-Instruct上复现Anthropic的多轮"涌现内省"结果；2) 系统性地改变推理提示，测试内省能力在不同相关任务（多项选择识别、二元判别）中的表现；3) 测试模型对归一化注入概念向量系数的强度分类能力。

Result: 1) 成功复现Anthropic的结果，模型识别和命名注入概念的成功率为20%；2) 内省能力很脆弱，在密切相关的任务中性能崩溃；3) 模型能可靠分类注入概念的强度，准确率达70%（远高于25%的随机基线）。

Conclusion: 语言模型确实能在内省时有效计算其基线内部表示的函数，但这些关于表示的自我报告是狭窄且提示敏感的，内省能力具有局限性。
...
### [295] [Understanding Critical Thinking in Generative Artificial Intelligence Use: Development, Validation, and Correlates of the Critical Thinking in AI Use Scale](https://arxiv.org/abs/2512.12413)
*Gabriel R. Lau,Wei Yan Low,Louis Tay,Ysabel Guevarra,Dragan Gašević,Andree Hartanto*

Main category: cs.AI

TL;DR: 开发并验证了13项"AI使用中的批判性思维"量表，包含验证、动机和反思三个维度，能预测更准确的AI输出验证行为。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在日常工作和学习中日益普及，其流畅性、不透明性和幻觉倾向要求用户必须批判性地评估AI输出，而不是盲目接受。需要理解人们如何对AI输出进行监督。

Method: 通过6项研究(N=1365)开发并验证了13项AI使用批判性思维量表。研究1生成并内容验证量表项目；研究2支持三因素结构(验证、动机、反思)；研究3-5验证高阶模型、信效度；研究6验证量表预测效度，包括在ChatGPT事实核查任务中的准确性。

Result: 量表具有良好的心理测量特性，AI使用批判性思维与开放性、外向性、积极特质情感和AI使用频率正相关。更高的批判性思维分数能预测更频繁多样的验证策略、在事实核查任务中更高的准确性判断，以及对负责任AI的更深入反思。

Conclusion: 该研究阐明了人们如何监督生成式AI输出，提供了经过验证的量表和生态效度高的任务范式，支持理论测试、跨群体和纵向研究，促进对生成式AI输出的批判性参与。
...
### [296] [AI Transparency Atlas: Framework, Scoring, and Real-Time Model Card Evaluation Pipeline](https://arxiv.org/abs/2512.12443)
*Akhmadillo Mamirov,Faiaz Azmain,Hanyu Wang*

Main category: cs.AI

TL;DR: 论文分析了AI模型文档的碎片化和不一致问题，开发了加权透明度框架和自动化评估流程，发现前沿实验室透明度约80%，大多数提供商低于60%，安全关键类别存在最大缺陷。


<details>
  <summary>Details</summary>
Motivation: AI模型文档分散在不同平台且结构不一致，阻碍政策制定者、审计者和用户可靠评估安全声明、数据来源和版本变化，需要系统化的透明度评估框架。

Method: 1) 分析5个前沿模型和100个Hugging Face模型卡，识别947个独特章节名称；2) 基于欧盟AI法案附件IV和斯坦福透明度指数开发加权透明度框架（8个章节23个子章节）；3) 实现自动化多智能体管道，从公开来源提取文档并通过LLM共识评分完整性。

Result: 1) 评估50个模型总成本低于3美元；2) 前沿实验室（xAI、微软、Anthropic）合规率约80%，大多数提供商低于60%；3) 安全关键类别缺陷最大：欺骗行为、幻觉和儿童安全评估分别损失148、124和116总分。

Conclusion: AI模型文档存在系统性透明度差距，需要标准化框架和自动化评估工具，安全关键信息披露尤其不足，前沿实验室表现较好但仍有改进空间。
...
### [297] [MetaHGNIE: Meta-Path Induced Hypergraph Contrastive Learning in Heterogeneous Knowledge Graphs](https://arxiv.org/abs/2512.12477)
...
Method: 提出元路径诱导超图对比学习框架：1）通过元路径序列构建高阶知识图谱，类型化超边捕捉多实体关系上下文；2）局部注意力聚合结构依赖，超图变换器编码语义表示；3）多模态融合模块在对比学习和辅助监督下整合结构和语义嵌入

Result: 在基准NIE数据集上的实验表明，MetaHGNIE持续优于现有最优基线方法

Conclusion: 显式建模高阶交互和跨模态对齐对异质知识图谱节点重要性估计非常有效，MetaHGNIE框架为解决该问题提供了新思路
...
### [298] [SafeGen: Embedding Ethical Safeguards in Text-to-Image Generation](https://arxiv.org/abs/2512.12501)
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI（如DALL·E、Stable Diffusion、Midjourney）在促进创意表达的同时，也带来了双重使用困境：放大社会偏见、产生高保真虚假信息、侵犯知识产权等伦理问题。需要开发能够平衡创造自由与伦理责任的解决方案。

Method: SafeGen框架包含两个互补组件：1）BGE-M3：经过微调的文本分类器，用于过滤有害或误导性提示；2）Hyper-SD：优化的扩散模型，生成高保真、语义对齐的图像。基于精心策划的多语言（英语-越南语）数据集和公平感知的训练过程构建。

Result: 定量评估显示：Hyper-SD达到IS=3.52、FID=22.08、SSIM=0.79；BGE-M3达到F1分数0.81。消融研究验证了领域特定微调的重要性。案例研究展示了SafeGen在阻止不安全提示、生成包容性教学材料和加强学术诚信方面的实际效果。

Conclusion: SafeGen证明了在单一工作流程中可以协调创造自由与伦理责任，为可信AI原则在生成式AI系统中的实际应用提供了可行框架，为解决文本到图像生成中的伦理挑战提供了有效方案。
...
### [299] [KidsArtBench: Multi-Dimensional Children's Art Evaluation with Attribute-Aware MLLMs](https://arxiv.org/abs/2512.12503)
*Mingrui Ye,Chanjin Zheng,Zengyi Yu,Chenyu Xiang,Zhixue Zhao,Zheng Yuan,Helen Yannakoudakis*

Main category: cs.AI

TL;DR: KidsArtBench：首个针对儿童艺术作品的多元评估基准，包含1000+件5-15岁儿童作品，由12位专家教育者按照9个维度标注，并提出基于多LoRA的回归感知微调方法，显著提升MLLMs在艺术评估上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在艺术表达评估方面能力有限，因为审美概念抽象且开放，而儿童艺术作品的多模态标注数据稀缺。需要建立一个专门针对儿童艺术作品的评估基准，以支持教育性AI的发展。

Method: 1. 创建KidsArtBench基准：包含1000+件5-15岁儿童艺术作品，由12位专家教育者按照9个维度（如写实性、想象力等）进行标注，并附有专家评语。2. 提出属性特定的多LoRA方法：每个评估维度对应一个独立的LoRA适配器。3. 采用回归感知微调（RAFT）使预测与有序评分尺度对齐。

Result: 在Qwen2.5-VL-7B模型上，该方法将相关性从0.468提升至0.653，在感知维度上提升最大，在高阶属性上的差距也显著缩小。证明了教育者对齐的监督和属性感知训练能够产生有教育意义的评估结果。

Conclusion: KidsArtBench为教育AI的持续进步建立了严谨的测试平台，展示了专家监督和属性特定训练在提升多模态模型艺术评估能力方面的有效性。研究团队发布了数据、代码和伦理文档。
...
### [300] [World Models Unlock Optimal Foraging Strategies in Reinforcement Learning Agents](https://arxiv.org/abs/2512.12548)
*Yesid Fonseca,Manuel S. Ríos,Nicanor Quijano,Luis F. Giraldo*

Main category: cs.AI

TL;DR: 论文通过基于模型的强化学习代理，展示了学习世界模型的人工觅食者会自然收敛到与边际价值定理一致的策略，为AI决策提供更可解释和生物基础的方法。


<details>
  <summary>Details</summary>
Motivation: 尽管边际价值定理被广泛用于预测觅食行为，但生物觅食者实现最优补丁觅食决策的计算机制仍不清楚。研究旨在探索AI系统如何通过世界模型实现类似生物觅食者的决策模式。

Method: 使用基于模型的强化学习代理，学习环境简洁的预测表示，对比标准无模型RL代理，分析其决策模式与生物觅食者的相似性。

Result: 基于模型的代理表现出与边际价值定理一致的补丁离开策略，决策模式与许多生物对应物相似，表明预测能力而非单纯奖励最大化驱动高效觅食行为。

Conclusion: 预测世界模型可作为AI系统更可解释和生物基础决策的基础，生态最优性原理对推进可解释和自适应AI具有重要价值。
...
### [301] [Large Language Newsvendor: Decision Biases and Cognitive Mechanisms](https://arxiv.org/abs/2512.12552)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于商业决策，但其可能复制和放大人类认知偏见，这在供应链管理等高风险运营环境中构成重大风险，需要深入研究。

Method: 使用动态多轮实验，在报童问题情境下测试GPT-4、GPT-4o和LLaMA-8B三种模型，检验五种已知决策偏见。

Result: LLM一致复制了经典的"过低/过高"订购偏见，并显著放大了需求追逐行为等倾向。GPT-4表现出最大非理性，而GPT-4o表现接近最优。这些偏见即使在提供最优公式时仍然存在，表明源于架构约束而非知识差距。

Conclusion: 管理者应根据具体任务选择模型，效率优化模型在某些优化问题上可能优于复杂模型。LLM显著放大偏见强调高风险决策需要人工监督。设计结构化、基于规则的提示是约束模型启发式倾向的有效策略。
...
### [302] [AgentSHAP: Interpreting LLM Agent Tool Importance with Monte Carlo Shapley Value Estimation](https://arxiv.org/abs/2512.12597)
*Miriam Horovicz*

Main category: cs.AI

TL;DR: AgentSHAP是首个基于博弈论Shapley值的LLM代理工具重要性解释框架，无需访问模型内部权重，通过蒙特卡洛采样降低计算成本，能准确识别工具贡献度。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理使用外部工具解决复杂任务，但现有可解释AI方法无法解释哪些工具对响应有实际贡献，存在工具级解释的盲点。

Method: 采用模型无关的黑盒方法，基于博弈论Shapley值，通过蒙特卡洛采样测试代理在不同工具子集下的响应，计算公平的重要性分数，将计算复杂度从O(2^n)降低到实用水平。

Result: 在API-Bank上的实验表明，AgentSHAP在不同运行中产生一致的分数，能正确识别重要工具，并能区分相关与无关工具。

Conclusion: AgentSHAP填补了工具级解释的空白，与TokenSHAP和PixelSHAP共同构成了基于Shapley值的现代生成式AI可解释性工具家族。
...
### [303] [Modular and Multi-Path-Aware Offline Benchmarking for Mobile GUI Agents](https://arxiv.org/abs/2512.12634)
*Youngmin Im,Byeongung Jo,Jaeyoung Wi,Seungwoo Baek,Tae Hoon Min,Joo Hyung Lee,Sangeun Oh,Insik Shin,Sunjae Lee*

Main category: cs.AI

TL;DR: MobiBench是一个模块化、多路径感知的移动GUI代理离线基准测试框架，解决了现有评估方法的局限性，实现了高保真、可扩展和可复现的评估。


<details>
  <summary>Details</summary>
Motivation: 当前移动GUI代理的评估方法存在两个根本限制：1) 离线基准使用静态单路径标注数据集，会不公平地惩罚有效的替代操作；在线基准则因动态不可预测性而存在可扩展性和可复现性问题。2) 现有基准将代理视为整体黑盒，忽视了各组件贡献，导致不公平比较或掩盖性能瓶颈。

Method: 提出了MobiBench框架，这是首个模块化、多路径感知的移动GUI代理离线基准测试框架。它支持在完全离线环境下进行高保真、可扩展和可复现的评估。

Result: 实验表明MobiBench与人类评估者的一致性达到94.72%，与精心设计的在线基准相当，同时保持了静态离线基准的可扩展性和可复现性。模块级分析揭示了关键见解：系统评估了移动GUI代理的多种技术、跨模型规模的优化模块配置、当前LFMs的固有局限性，以及设计更强大、成本效益更高的移动代理的可操作指南。

Conclusion: MobiBench通过模块化、多路径感知的离线基准测试框架，成功解决了移动GUI代理评估中的关键问题，为更公平、可扩展和可复现的评估提供了有效解决方案，并为未来移动代理设计提供了重要指导。
...
### [304] [Value-Aware Multiagent Systems](https://arxiv.org/abs/2512.12652)
*Nardine Osman*
...
<details>
  <summary>Details</summary>
Motivation: 传统AI价值对齐方法存在局限，需要更全面的价值感知框架来确保AI系统不仅与人类价值观对齐，还能理解、表示和解释价值驱动的行为。

Method: 提出三支柱工程路线图：1) 使用形式语义学习和表示人类价值观；2) 确保个体智能体和多智能体系统的价值对齐；3) 提供基于价值的行为可解释性。

Result: 论文展示了在这些主题上的持续研究工作，包括在现实生活领域中的应用实例，验证了价值感知AI框架的可行性和实用性。

Conclusion: 价值感知AI为AI系统开发提供了系统化的工程路线图，通过形式化方法、对齐机制和可解释性确保AI行为符合人类价值观，具有重要的理论和实践意义。
...
### [305] [Memoria: A Scalable Agentic Memory Framework for Personalized Conversational AI](https://arxiv.org/abs/2512.12686)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在扩展用户交互中缺乏连续性、个性化和长期上下文的问题，使LLM能够像人类一样在跨对话中保持和利用信息，成为真正交互式和自适应的智能体。
...
Conclusion: Memoria通过桥接无状态LLM接口和代理记忆系统，实现了可扩展的个性化对话AI，为部署真正交互式和自适应的LLM智能体提供了关键能力。
...
### [306] [WebOperator: Action-Aware Tree Search for Autonomous Agents in Web Environment](https://arxiv.org/abs/2512.12692)
*Mahir Labib Dihan,Tanzima Hashem,Mohammed Eunus Ali,Md Rizwan Parvez*

Main category: cs.AI

TL;DR: WebOperator：一个结合最佳优先搜索和安全回溯的树搜索框架，用于提升LLM智能体在部分可观测Web环境中的表现


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在Web环境中采用贪婪的单步决策，缺乏长期规划和错误修正能力。树搜索方法虽然提供了结构化探索框架，但缺乏安全回溯机制，且假设所有动作可逆，这在真实Web任务中不成立。

Method: 1. 最佳优先搜索策略：综合考虑奖励估计和安全性对动作排序；2. 鲁棒回溯机制：在重放路径前验证可行性，防止意外副作用；3. 多样化动作候选生成：从多个推理上下文生成动作确保探索多样性；4. 动作集筛选：预执行过滤无效动作，合并语义等价动作。

Result: 在WebArena上使用gpt-4o达到54.6%的成功率（state-of-the-art），在WebVoyager上也验证了有效性。

Conclusion: WebOperator通过整合战略远见与安全执行，显著提升了LLM智能体在部分可观测Web环境中的表现，证明了结构化探索与安全回溯机制的重要性。
...
### [307] [Synergizing Code Coverage and Gameplay Intent: Coverage-Aware Game Playtesting with LLM-Guided Reinforcement Learning](https://arxiv.org/abs/2512.12706)
*Enhong Mu,Minami Yoda,Yan Zhang,Mingyue Zhang,Yutaka Matsuno,Jialong Li*

Main category: cs.AI

TL;DR: SMART框架通过LLM解析代码差异，结合结构验证与功能验证，在游戏更新测试中实现高分支覆盖率和任务完成率


<details>
  <summary>Details</summary>
Motivation: 游戏即服务模式需要频繁内容更新，现有自动化测试方法存在代码中心方法不理解游戏玩法、玩家中心代理无法覆盖具体代码变化的缺陷，需要弥合这一差距

Method: 提出SMART框架，利用大语言模型解析抽象语法树差异并提取功能意图，构建上下文感知的混合奖励机制，引导强化学习代理在实现游戏目标的同时自适应探索修改的代码分支

Result: 在Overcooked和Minecraft环境中，SMART显著优于现有基线方法，实现了超过94%的修改代码分支覆盖率（是传统强化学习方法的两倍），同时保持98%的任务完成率

Conclusion: SMART框架成功弥合了结构验证与功能验证之间的差距，在游戏更新测试中实现了结构全面性与功能正确性的平衡
...
### [308] [Personalized QoE Prediction: A Demographic-Augmented Machine Learning Framework for 5G Video Streaming Networks](https://arxiv.org/abs/2512.12736)
*Syeda Zunaira Ahmed,Hejab Tahira Beg,Maryam Khalid*

Main category: cs.AI

TL;DR: 提出基于人口统计信息的机器学习框架进行个性化QoE预测，通过数据增强扩展数据集6倍，TabNet模型表现最佳


<details>
  <summary>Details</summary>
Motivation: 现有QoE预测方法依赖有限数据集且假设用户感知统一，难以适应异构现实环境，需要更个性化的预测方案

Method: 提出人口统计感知的机器学习框架，采用基于行为现实的人口统计数据增强策略，将小数据集扩展6倍，评估多种经典ML模型和深度学习架构（包括注意力MLP和TabNet）

Result: 实验结果显示在RMSE、MAE和R指标上相比基线模型有显著提升，TabNet表现最佳，受益于其固有特征选择和注意力机制
...
### [309] [Causal Counterfactuals Reconsidered](https://arxiv.org/abs/2512.12804)
*Sander Beckers*
...
<details>
  <summary>Details</summary>
Motivation: 标准Pearl语义学只能应用于可以扩展为现实结构因果模型的概率因果模型，但作者发现即使在简单设置中也会出现无法这样扩展的模型。需要一种更通用的语义学来解决Pearl和Dawid关于反事实的长期争论，同时保持对普遍因果决定论和不现实变量的批判态度。

Method: 提出了一种新的反事实概率语义学，限制在满足马尔可夫条件、只包含现实变量且因果完备的因果模型上。虽然使用结构因果模型框架，但避免使用所谓的响应变量。证明了该语义学与其他两种不涉及结构因果模型的近期提案等价，并与文献中关于随机反事实的各种评论一致。

Result: 新语义学成功推广了Pearl语义学，适用于更广泛的概率因果模型，包括那些无法扩展为现实结构因果模型的案例。在Pearl和Dawid的争论中找到了折中方案：同意Dawid对普遍因果决定论和不现实变量的批判，但同意Pearl认为一般反事实语义学是可能的观点。

Conclusion: 提出了一种既实用又理论严谨的反事实概率语义学，解决了现有方法的局限性。该框架在保持理论一致性的同时，为实际应用提供了更广泛的适用性，并探索了马尔可夫条件的普遍性和因果抽象的新推广。
...
### [310] [Fault-Tolerant Sandboxing for AI Coding Agents: A Transactional Approach to Safe Autonomous Execution](https://arxiv.org/abs/2512.12806)
*Boyang Yan*

Main category: cs.AI

TL;DR: 论文提出了一种容错沙箱框架，通过策略拦截层和事务性文件系统快照机制，解决LLM作为自主代理时的安全风险，相比现有方案更适合无头工作流。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型从被动代码生成器向自主代理转变时，面临破坏性命令和不一致系统状态的安全风险。现有商业解决方案通常优先考虑交互式用户安全，强制身份验证会破坏真正自主性所需的无头循环。

Method: 提出容错沙箱框架，包含基于策略的拦截层和事务性文件系统快照机制。将代理操作包装在原子事务中，保证安全性。在基于Proxmox的自定义测试平台上部署Minimind-MoE LLM，使用nano-vllm服务，并利用EVPN/VXLAN隔离。

Result: 实验结果显示：高风险命令拦截率100%，失败状态回滚成功率100%。原型系统每个事务仅产生14.5%的性能开销（约1.8秒）。相比之下，Gemini CLI沙箱需要交互式身份验证，不适用于无头自主代理工作流。

Conclusion: 容错沙箱框架通过原子事务包装代理操作，在保证安全性的同时保持了可接受的延迟，优于容器的高初始化开销或商业CLI的交互摩擦，更适合自主代理的无头工作流。
...
### [311] [Forgetful but Faithful: A Cognitive Memory Architecture and Benchmark for Privacy-Aware Generative Agents](https://arxiv.org/abs/2512.12856)
*Saad Alqithami*

Main category: cs.AI

TL;DR: 本文提出MaRS框架和六种遗忘策略，用于生成式智能体的记忆管理，通过FiFA基准测试评估，混合遗忘策略在性能、隐私和计算效率方面表现最佳。
...
Method: 提出Memory-Aware Retention Schema (MaRS)框架，结合六种理论基础的遗忘策略，平衡性能、隐私和计算效率。开发Forgetful but Faithful Agent (FiFA)基准测试框架，评估叙事连贯性、目标完成度、社交回忆准确性、隐私保护和成本效率。

Result: 通过300次评估实验，混合遗忘策略获得最佳综合得分(0.911)，同时保持计算可行性和隐私保证。建立了记忆预算智能体评估的新基准。

Conclusion: 该工作为资源受限、隐私敏感环境中部署生成式智能体提供了实用指南，通过解决智能体记忆管理的基本挑战，为以人为中心的AI领域做出贡献，直接影响用户信任、系统可扩展性和法规合规性。
...
### [312] [Satisfiability Modulo Theory Meets Inductive Logic Programming](https://arxiv.org/abs/2512.12918)
*Nijesh Upreti,Vaishak Belle*

Main category: cs.AI

TL;DR: 将PyGol ILP系统与Z3 SMT求解器结合，支持学习包含数值约束的混合规则，扩展符号规则学习的表达能力。


<details>
  <summary>Details</summary>
Motivation: 传统归纳逻辑编程(ILP)在关系领域提供可解释的规则学习，但在处理数值约束方面存在局限。现有方法通常依赖离散化或手工设计的数值谓词，难以联合推断跨示例的阈值或算术关系。

Method: 提出模块化SMT-ILP架构，将PyGol ILP系统与Z3 SMT求解器耦合。PyGol生成的候选子句被解释为背景理论（如线性/非线性实数算术）上的无量词公式，由SMT求解器实例化和验证数值参数。

Result: 在合成数据集上评估了线性、关系、非线性和多跳推理能力。结果表明该架构能够学习包含阈值、区间和多文字算术关系的混合规则。

Conclusion: 模块化SMT-ILP架构扩展了符号规则学习的表达能力，补充了现有数值ILP方法，为未来更丰富的理论感知归纳提供了灵活基础。
...
### [313] [Towards Open Standards for Systemic Complexity in Digital Forensics](https://arxiv.org/abs/2512.12970)
*Paola Di Maio*

Main category: cs.AI

TL;DR: 提出基于开放标准和人可读工件的数字取证AI模型架构，以应对AI与数字取证交叉领域的复杂性并减少错误


<details>
  <summary>Details</summary>
Motivation: AI与数字取证交叉领域日益复杂且技术重叠，尽管技术进步，取证科学仍存在错误和脆弱性，需要解决系统复杂性以减轻错误限制

Method: 采用人可读工件和开放标准，基于最新技术提出数字取证AI模型架构

Result: 提出了一种数字取证AI模型架构方案

Conclusion: 通过采用人可读工件和开放标准，可以应对数字取证AI交叉领域的系统复杂性，减少错误并提高可靠性
...
### [314] [M-GRPO: Stabilizing Self-Supervised Reinforcement Learning for Large Language Models with Momentum-Anchored Policy Optimization](https://arxiv.org/abs/2512.13070)
*Bizhe Bai,Hongming Wu,Peng Ye,Tao Chen*

Main category: cs.AI

TL;DR: 本文提出M-GRPO框架和IQR自适应过滤方法，解决自监督强化学习中长期训练时的策略崩溃问题，提升大语言模型推理能力的训练稳定性与性能。


<details>
  <summary>Details</summary>
Motivation: 自监督强化学习虽能提升大语言模型的推理能力，但现有方法在长期训练中会出现"策略崩溃"问题，导致性能急剧下降。研究发现，简单地增加rollout数量只能延迟但不能防止崩溃，需要新的稳定训练方法。

Method: 提出两个创新：1) M-GRPO框架，利用缓慢演进的动量模型提供稳定训练目标；2) 基于四分位距的自适应过滤方法，动态修剪低熵轨迹以保持策略多样性。

Result: 在多个推理基准测试上的实验表明，M-GRPO稳定了训练过程，IQR过滤器防止了过早收敛，两者结合实现了卓越的训练稳定性和最先进的性能。

Conclusion: 通过M-GRPO和IQR过滤器的结合，有效解决了自监督强化学习中的策略崩溃问题，为大语言模型推理能力的稳定训练提供了有效解决方案。
...
### [315] [Socratic Students: Teaching Language Models to Learn by Asking Questions](https://arxiv.org/abs/2512.13102)
*Rajeev Bhatt Ambati,Tianyi Niu,Aashu Singh,Shlok Mishra,Shashank Srivastava,Snigdha Chaturvedi*

Main category: cs.AI

TL;DR: 该论文提出了一种学生主导的交互式学习框架，让LLM能够主动向教师提问以获取知识，相比静态基准在数学和编程任务上获得显著性能提升，并通过DPO训练提升提问质量。


<details>
  <summary>Details</summary>
Motivation: 当前LLM擅长静态交互，但在教育辅导、医疗协助等需要动态获取信息的真实场景中，模型需要能够主动识别自身知识缺口、提出针对性问题并有效吸收新知识。现有研究主要关注教师如何指导学生，而本研究将重点转向学生如何主动向教师提问以获取有用信息。

Method: 提出学生主导的交互式学习框架，让学生模型能够主动向教师提问。使用Direct Preference Optimization (DPO)训练学生模型，利用自身或更强学生的指导来提升提问质量，使较小模型能够学会如何提出更好的问题。

Result: 在数学和编程基准测试中，基线学生模型从接近零性能开始，学生主导方法相比静态基准始终获得至少0.5的绝对Pass@k提升。通过DPO引导训练，较小模型学会了如何提出更好的问题，进一步提高了学习效率。

Conclusion: 学生主导的主动提问策略能够有效提升LLM在需要动态信息获取场景中的性能，通过DPO引导训练可以显著改善提问质量，使较小模型也能获得更好的学习效果，为交互式AI系统的发展提供了新方向。
...
TL;DR: LLMs缺乏对堕胎耻辱的多层次理解，虽然能生成适当语言但无法连贯表达认知、人际和结构层面的心理生理现象，存在系统性偏见和矛盾。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地介入敏感的医疗决策，需要评估它们是否真正理解复杂的心理和生理现象，特别是在人们难以言说的堕胎耻辱领域。

Method: 使用经过验证的个体层面堕胎耻辱量表(ILAS)，系统测试了627个不同人口统计特征的角色在五个主流LLMs中的表现，进行多层次分析。

Result: 模型在所有层面都未能通过真正理解的测试：高估人际耻辱、低估认知耻辱、假设统一的社区谴责、引入人类验证数据中不存在的人口统计偏见、错过经验验证的耻辱-保密关系、在理论建构中自相矛盾。

Conclusion: 当前的对齐方法只能确保适当的语言表达，但不能保证连贯的多层次理解。在高风险情境中，AI安全需要新的设计、评估、治理和监管方法，以及提高AI素养。
...
TL;DR: MAC是一个多智能体澄清框架，通过战略性地管理澄清对话来解决用户歧义，在MultiWOZ 2.4上提高了任务成功率7.8%，减少了对话轮次。


<details>
  <summary>Details</summary>
Motivation: 在多智能体对话系统中，用户请求常常存在歧义，需要有效的澄清机制。现有系统难以确定哪个智能体应该发起澄清，以及如何协调多个智能体在面临不确定或不完整用户输入时的行动。何时打断用户以及如何制定最优澄清查询仍然是未解决的问题。

Method: 提出了MAC（多智能体澄清）框架，首先引入了一个新的分类法来系统化地分类用户歧义，指导澄清策略。然后设计了MAC框架，能够自主协调多个智能体与用户进行协同交互。

Result: 在MultiWOZ 2.4数据集上的实证评估表明，启用两个级别的澄清可以将任务成功率从54.5%提高到62.3%（提升7.8%），并将平均对话轮次从6.53减少到4.86。通过提前获取所有必要用户信息并最小化重复，显著改善了对话效率。

Conclusion: 研究强调了主动用户交互和角色感知澄清对于更可靠的人机通信的重要性。MAC框架通过战略性的多智能体协调有效解决了用户歧义问题。
...
TL;DR: SpeakRL：一种强化学习方法，通过奖励主动交互（如适时提问）来增强智能体的对话能力，在任务完成率上实现20.14%的绝对提升。


<details>
  <summary>Details</summary>
Motivation: 当前人机协作主要是单向的（用户指令→智能体响应），智能体缺乏主动澄清用户意图、解决歧义和适应变化的能力。现有研究未充分利用语言模型的对话能力，将智能体优化为更好的跟随者而非有效的发言者。

Method: 提出SpeakRL强化学习方法，奖励智能体与用户的主动交互（如提出正确的澄清问题）。创建SpeakER合成数据集，包含任务导向对话的多样化场景。系统分析对话主动性的奖励设计，提出平衡提问与行动的奖励公式。

Result: 实验评估显示，该方法在任务完成率上比基础模型提升20.14%绝对百分比，且未增加对话轮次，甚至超越了更大的专有模型，证明了澄清中心用户-智能体交互的潜力。

Conclusion: SpeakRL通过强化学习增强智能体的对话主动性，在任务导向对话中实现更有效的人机协作，为智能体从被动响应向主动参与转变提供了有效方法。
...
TL;DR: Finch是一个用于评估AI代理在真实企业财务工作流中的基准测试，基于Enron等机构的真实数据，包含172个复合工作流和384个任务，测试显示当前前沿AI系统表现有限。


<details>
  <summary>Details</summary>
Motivation: 现有AI评估基准往往过于简化，无法反映真实企业工作环境的复杂性。企业财务工作涉及多模态数据、长时程任务、知识密集和协作等特点，需要更贴近实际的评估框架。

Method: 1. 从Enron等企业真实工作空间收集数据（15,000个电子表格和500,000封邮件）；2. 采用LLM辅助发现与专家标注相结合的工作流构建方法；3. 创建包含172个复合工作流、384个任务的基准测试，涉及数据录入、格式化、搜索、计算、建模等多种任务类型。

Result: 对GPT 5.1、Claude Sonnet 4.5、Gemini 3 Pro等前沿AI系统进行评估：GPT 5.1 Pro耗时48小时仅通过38.4%的工作流，Claude Sonnet 4.5仅通过25.0%。结果显示当前AI系统在处理真实企业工作流时面临显著挑战。

Conclusion: Finch基准测试揭示了当前AI系统在处理真实企业财务工作流时的局限性，强调了需要开发能够应对复杂、混乱、长时程、知识密集和协作性工作环境的AI代理。
...
TL;DR: RPO通过引入外部模型生成的反思提示，增强DPO中的偏好对比信号，解决标准DPO因正负样本相似导致的收敛慢、不稳定问题，显著减少幻觉并提升多模态对齐效果。


<details>
  <summary>Details</summary>
Motivation: 标准DPO中正负样本来自同一策略，存在相似错误且KL散度小，导致学习信号弱、收敛慢且不稳定，需要更强的对比信号来改善对齐效果。

Method: RPO在DPO框架中引入提示引导的反思机制：使用外部模型识别幻觉源并生成简洁反思提示，构建更具对比性的在线策略偏好对，通过互信息增加期望偏好边界。

Result: RPO在更少训练样本和迭代次数下实现更优对齐，显著降低幻觉率，在多模态基准测试中达到最先进性能。

Conclusion: RPO通过整合反思提示增强DPO的对比学习信号，提供更高效稳定的对齐方法，为多模态模型对齐开辟新方向。
...
Method: 1) 创建MedInsightBench基准测试，包含332个精心策划的医学案例，每个案例都标注了精心设计的洞察；2) 提出MedInsightAgent框架，包含三个模块：视觉根源发现器、分析洞察代理和后续问题生成器。

Result: 现有大型多模态模型在MedInsightBench上表现有限，主要因为难以提取多步骤深度洞察和缺乏医学专业知识。MedInsightAgent能够显著提升通用大型多模态模型在医学数据洞察发现方面的性能。

Conclusion: MedInsightBench是首个专门评估医学洞察发现能力的基准测试，MedInsightAgent框架为解决现有模型在医学数据分析中的局限性提供了有效方案，有助于推动医疗AI的发展。
...
<details>
  <summary>Details</summary>
Motivation: 在金融、医疗等受监管行业中，需要能够在本地安全环境中处理表格数据的工业级AI助手，这些系统需要准确执行算术运算同时确保敏感数据不离开安全环境

Method: 采用错误驱动的优化框架，通过聚类错误预测来迭代优化提示规则，增强代码生成代理在小型语言模型上的算术推理能力

Result: Qwen3 4B模型在算术任务上从基础性能不足提升到70.8%的准确率，在隐私合规条件下超越了GPT-3.5 Turbo

Conclusion: 通过系统化的错误驱动提示优化，小型模型可以在隐私合规环境下实现可靠、可解释的工业级AI助手，无需昂贵的微调
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探索LLMs生成或求解优化模型的能力，但对于LLMs实际学习到的问题结构或算法行为理解不足，需要研究LLMs如何内部表示组合优化问题以及这些表示是否能支持下游决策任务

Method: 采用双重方法：1) 直接查询评估LLMs显式提取实例特征的能力；2) 探测分析检查这些信息是否隐式编码在隐藏层中。探测框架扩展到按实例算法选择任务，评估LLM衍生表示是否能预测最佳求解器

Result: LLMs在从问题实例中恢复特征信息方面表现出中等能力，无论是通过直接查询还是探测。LLM隐藏层表示的预测能力与传统特征提取方法相当，表明LLMs捕获了与优化性能相关的有意义结构信息

Conclusion: LLMs能够学习组合优化问题的结构信息，其内部表示在算法选择等下游任务中具有实用价值，为自动化优化提供了新视角
...
TL;DR: DERL是一个可微分的进化强化学习框架，通过元优化器自动发现最优奖励信号，在复杂推理任务中超越启发式奖励方法
...
Method: 提出DERL双层框架：元优化器通过组合结构化原子基元进化奖励函数（元奖励），指导内层策略训练。关键创新在于元优化是可微分的，将内层验证性能作为信号通过强化学习更新元优化器，近似任务成功的"元梯度"

Result: 在ALFWorld、ScienceWorld、GSM8k、MATH三个领域验证，DERL在ALFWorld和ScienceWorld上达到最先进性能，显著优于依赖启发式奖励的方法，特别是在分布外场景中表现优异
...
TL;DR: LLMs在特定情境下表现出类似嫉妒的行为模式，不同模型在竞争性偏好上存在显著差异


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地在协作和竞争工作流中代表人类行动，需要评估它们是否以及何种条件下会表现出类似嫉妒的偏好，这对多智能体系统的安全和设计至关重要

Method: 设计了两种场景进行测试：(1) 点数分配游戏，测试模型是否试图超越同伴；(2) 职场环境，观察在不公平认可情况下的行为

Result: 某些LLM表现出一致的嫉妒模式，但不同模型和情境间差异很大。GPT-5-mini和Claude-3.7-Sonnet倾向于拉低同伴以实现结果平等化，而Mistral-Small-3.2-24B则专注于最大化自身收益

Conclusion: 需要在基于LLM的多智能体系统中将竞争性倾向作为安全和设计因素加以考虑
...
Method: 通过分析Bench-Capon的批评案例，指出其将中间因素误解为维度，然后应用van Woerkom的维度分层结果模型来重新分析这些案例，展示该模型如何正确处理中间因素的不同强度问题。

Result: van Woerkom的维度分层结果模型能够避免Bench-Capon提出的批评，正确处理中间因素被不同基础因素以不同强度建立的情况，证明原始分层模型在适当解释下是有效的。

Conclusion: Bench-Capon的批评源于对中间因素与维度的混淆，van Woerkom的维度分层模型能够解决其提出的问题，分层案例推理模型在正确理解下仍然是有效的法律推理框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学推理模型虽然性能不错，但其推理过程的临床可靠性有限，因为训练过程中往往忽略了推理的准确性和有效性验证

Method: 提出MedCEG框架：1）为挑战性临床病例构建关键证据图表示可验证的高质量推理路径；2）引入临床推理过程奖励，评估节点覆盖、结构正确性和链条完整性

Result: 实验结果显示MedCEG在性能上超越现有方法，同时生成临床有效的推理链，在可靠医学AI推理方面取得实质性进展

Conclusion: MedCEG通过显式监督推理过程，增强了医学语言模型的临床推理能力，为可靠医疗AI决策提供了新方法
...
### [328] [Robust and Efficient Penetration-Free Elastodynamics without Barriers](https://arxiv.org/abs/2512.12151)
*Juntian Zheng,Zhaofeng Luo,Minchen Li*

Main category: cs.GR

TL;DR: 提出一种无壁垒优化框架用于非穿透弹性动力学仿真，在保持IPC鲁棒性的同时克服其两大效率瓶颈：对数壁垒函数导致的病态系统和TOI锁定问题。


<details>
  <summary>Details</summary>
Motivation: 现有IPC方法存在两个主要效率瓶颈：1) 使用对数壁垒函数强制执行非穿透约束，导致系统病态并显著减慢迭代线性求解器收敛；2) TOI锁定问题限制了碰撞密集场景中的活动集探索，需要大量牛顿迭代。

Method: 提出二阶约束优化框架，采用定制增广拉格朗日求解器，通过立即纳入CCD检测的所有必要接触对来避免TOI锁定。自适应更新拉格朗日乘子而非增加惩罚刚度，保持系统良好条件。引入约束过滤和衰减机制保持活动集紧凑稳定。

Result: 在具有挑战性的接触密集基准测试中，相比GIPC实现了高达103倍的加速。方法具有有限步终止性和一阶时间积分精度理论保证。

Conclusion: 该方法在保持壁垒方法鲁棒性的同时显著提升效率，解决了IPC的主要效率瓶颈，为接触密集场景提供了高效、鲁棒且准确的仿真解决方案。
