...
Method: 开发SGClip模型，基于CLIP的开放域可提示场景图生成模型，通过神经符号学习管道在87K+开放域视频上训练，利用模型驱动的自监督和结构化推理

Result: SGClip在场景图生成和动作定位基准测试中表现出色，ESCA框架持续改进开源和商业MLLMs，在两个具身环境中实现最先进性能，显著减少智能体感知错误

Conclusion: ESCA框架通过结构化空间-时间理解有效情境化具身智能体，使开源模型超越专有基线，为通用具身智能体发展提供了新方向
...
<details>
  <summary>Details</summary>
Motivation: 智慧城市基础设施监控需求增长，人工检测成本高且危险，现有自动系统通常只能处理单一缺陷类型或输出非结构化结果，无法直接指导维护工作。

Method: 提出综合管道，利用YOLO系列目标检测器进行多缺陷检测和分割，然后将检测结果传递给视觉语言模型进行场景感知总结，生成包含事件描述、推荐工具、尺寸、维修计划和紧急警报的JSON格式结构化行动计划。
...
Conclusion: IAD-GPT成功结合了丰富的文本语义与图像级和像素级信息，为工业异常检测提供了一种有效的新方法。
...
Method: 前瞻性研究（2024年7-12月），8名读者（4名新手和4名非新手）使用定制查看器和眼动追踪系统分析35张床旁胸部X光片，比较三种报告模式的表现。

Result: AI辅助结构化报告的诊断准确性最高（κ=0.71），报告时间最短（25±9秒），眼动指标显示视觉注意力更集中于图像区域，且被用户评为首选模式。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器学习模型在非平衡数据集上训练时产生的交叉偏见问题，这些偏见源于多个属性（如物体类别和环境条件）的交互作用。

Method: 引入交叉公平性评估框架(IFEF)结合定量公平性指标和可解释性工具识别偏见模式，并提出偏见加权增强(BWA)方法，根据子组分布统计自适应调整变换强度。

Result: 在Open Images V7数据集上，BWA将代表性不足的类别-环境交叉组的准确率提升了最高24个百分点，同时将公平性指标差异减少了35%，统计显著性得到确认(p < 0.05)。
...
Method: 提出可微分的量化方法，支持n位量化，特别是移位比特量化，能够同时量化权重和激活值，且不需要高精度乘法运算。

Result: 在ImageNet图像分类任务中，仅权重量化时准确率与全精度模型相差不到1%；权重和激活值同时量化时，准确率与最先进方法相当，仅需15个训练周期。

Conclusion: 该方法提供了一种收敛到最优神经网络的可证明方法，在保持高准确率的同时显著降低了计算和内存需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在输入和外观对象的几何形状显著不同时效果不佳，直接应用3D生成模型无法产生令人满意的结果。

Method: 基于预训练的整流流模型，在采样过程中周期性添加可微损失函数作为引导，包括部分感知损失和自相似性损失。

Result: 方法成功将纹理和几何细节传输到3D资产，在定性和定量评估中均优于基线方法。
...
Method: StripRFNet包含三个模块：(1)形状感知模块(SPM)通过大分离核注意力增强形状判别；(2)条状感受野模块(SRFM)使用大条状卷积和池化捕捉细长裂缝特征；(3)小尺度增强模块(SSEM)利用高分辨率P2特征图、专用检测头和动态上采样改进小目标检测。
...
<details>
  <summary>Details</summary>
Motivation: 服装假人展示成本低但缺乏真实感和表现细节，需要将假人视频转换为逼真的人体视频以提升在线时尚展示效果。

Method: 使用动态姿态感知头部编码器融合面部语义与身体姿态，引入镜像损失和分布感知适配器，基于DDIM的单步去噪保持身份一致性。

Result: 在多个数据集上的实验表明，M2HVideo在服装一致性、身份保持和视频保真度方面优于现有方法。

Conclusion: M2HVideo能够有效生成身份可控、逼真的人体视频，解决了假人展示缺乏真实感的问题。
...
Method: 在训练时对单个对象进行颜色空间扰动，并使用扩散模型生成多样化的行人实例；在推理时对检测到的对象应用扰动，利用检测分数方差实时量化预测不确定性。

Result: 在NuImages 10K数据集上的YOLOv8实验显示，该方法在所有对象类别上都带来了显著的准确性提升和不确定性减少，并且在推理时对假阳性预测了更高的不确定性值。
...
<details>
  <summary>Details</summary>
Motivation: 为研究社区提供及时的自我中心多模态数据集，展示设备感知佩戴者、环境和交互的能力。

Method: 使用Aria Gen 2眼镜采集多模态数据，包含清洁、烹饪、进食、玩耍和户外行走五种主要场景，提供原始传感器数据和机器感知算法输出。

Result: 发布了包含主要受试者日常活动的数据集，展示了设备在不同用户和条件下的稳健性能。

Conclusion: A2PD是一个公开可用的多模态数据集，配有开源工具和使用示例，可用于自我中心感知研究。
...
<details>
  <summary>Details</summary>
Motivation: 血栓切除术是缺血性卒中最有效的治疗方法之一，但需要大量资源和专业人员。通过深度学习自动化关键环节，旨在提高效率和安全性。

Method: 提出自监督框架，使用基于回归的预训练任务对多种骨骼标志点进行分类。

Result: 实验表明该模型在回归和分类任务上均优于现有方法，位置预训练任务显著提升了下游分类性能。

Conclusion: 该框架成功提升了血栓切除术的自动化水平，未来工作将专注于扩展至全自动C臂控制，优化从骨盆到头部的轨迹规划。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像标注数据有限，半监督学习具有吸引力。教师-学生框架虽然流行，但联合优化整个网络会阻碍收敛和稳定性，特别是在具有挑战性的场景中。

Method: 双分支异步优化框架，每个分支分别优化编码器或解码器；引入解耦dropout扰动进行正则化；设计配对CutMix交叉引导增强模型多样性；提出一致性匹配机制减轻噪声伪标签的确认偏差。
...
<details>
  <summary>Details</summary>
Motivation: 传统C臂定位依赖手动操作，增加了辐射暴露和手术延迟，需要自动化解决方案来提高效率和安全性。
...
<details>
  <summary>Details</summary>
Motivation: 深度生成模型生成的图像质量尚不及传统摄影方法，需要人工图像质量评估，但这个过程既慢又昂贵

Method: 引入自动预过滤阶段，提出成本节约估算公式，并在背景修复用例中应用简单的AutoML解决方案

Result: 在背景修复用例中实现了51.61%的显著成本节约

Conclusion: 自动预过滤阶段可以有效减少人工图像质量评估的工作量，显著降低获取高质量图像的平均成本
...
<details>
  <summary>Details</summary>
Motivation: 理解大脑如何编码视觉信息是神经科学和机器学习的重要挑战。当前从fMRI信号重建视觉刺激的方法存在两个问题：不清楚哪种潜在空间最适合这种转换，以及如何有效组织空间来表示视觉刺激。
...
<details>
  <summary>Details</summary>
Motivation: 热带地区农业遥感面临高质量标注数据缺乏、标注成本高、数据变异性大和区域泛化困难等独特挑战，传统以模型为中心的方法受限。

Method: 采用数据为中心的人工智能视角，重点应用自信学习、核心集选择、数据增强和主动学习等技术，构建数据质量驱动的管道。

Result: 识别出25种适用于大规模农业制图管道的策略，并筛选出9种最成熟直接的方法用于实际应用。

Conclusion: 数据为中心的方法能更好地适应热带农业的动态现实，为大规模热带农业制图项目提供了实用的解决方案管道。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频理解训练方法主要针对transformer设计，未能充分利用状态空间模型(SSMs)的独特优势。传统方法在固定分辨率和视频长度下训练，导致模型在面对训练时未见过的时空分辨率时性能下降，即时空不灵活性。

Method: 提出灵活训练方法：在训练过程中采样不同时空分辨率的视频，并动态插值模型权重以适应任意时空尺度。开发了五种灵活训练变体，并确定了最适合视频SSMs的策略。

Result: 在短动作(UCF-101, HMDB-51)和长动作(COIN, Breakfast)基准测试中，StretchySnake比transformer和SSM基线方法性能提升高达28%，在细粒度动作识别(SSV2, Diving-48)上也表现出强适应性。

Conclusion: 该方法提供了一个简单的即插即用训练方案，使视频SSMs在不同动作识别场景中更加鲁棒、分辨率无关且高效。
...
Method: 使用异构集成架构，ViT主干捕捉全局面部结构和对称性，Mamba主干以线性复杂度高效建模长距离依赖关系，关注序列特征和纹理。

Result: 在SCUT-FBP5500数据集上获得皮尔逊相关系数0.9212、平均绝对误差0.2085、均方根误差0.2698的最优性能。

Conclusion: VM-BeautyNet通过两种主干网络的互补特征提取，为计算美学提供了强大的新架构范式，并通过可视化分析增强了模型可解释性。
...
<details>
  <summary>Details</summary>
Motivation: OCSCC是头颈部最常见的癌症，早期症状不明显且生长缓慢，常被忽视导致可预防的死亡。需要有效的早期检测方法。

Method: 训练CNN模型识别OCSCC，使用4293张训练图像（包括良恶性肿瘤和阴性样本），设计图像采集硬件系统，测试不同分辨率图像对预测准确性的影响。

Result: 图像分辨率增加会提高预测准确性，但呈对数关系，显示更高像素数带来的收益递减。开发了应用程序便于测试和开放访问CNN。
...
<details>
  <summary>Details</summary>
Motivation: 为研究人类行为和交互提供大规模、高质量的3D运动数据，支持多模态分析。

Method: 在多相机采集环境中收集数据，包括追踪3D运动、手部追踪、身体形状、文本注释和单独音频轨道。

Result: 创建了包含54百万帧追踪3D运动的数据集，涵盖广泛的行为类型和交互场景。

Conclusion: Embody 3D数据集为人类行为分析、社交交互研究等提供了宝贵的资源。
...
Conclusion: 该方法为传统物体级重建提供了一种灵活、渐进的替代方案，能够利用第一人称视角直播中的人-物交互线索进行动态场景建模。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的视频异常检测方法计算成本高且视觉定位不稳定，难以实时部署。

Method: 采用两级级联系统，离线学习正常行为规则，在线结合轻量级过滤和细粒度VLM推理，引入运动掩码提示和基于规则的偏差检测。

Result: 在四个数据集上平均达到57.68 fps，速度提升151.79倍，准确率97.2%，与最先进方法相当。

Conclusion: Cerberus是实时视频分析的实际可行解决方案，在效率和准确性之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究在评估LVLM成员推断攻击时存在分布偏差问题，导致攻击成功率被高估，需要建立无偏基准来准确评估隐私风险。

Method: 构建包含6000张图像的受控基准，精心平衡成员和非成员样本的分布，并提供三个不同训练阶段的真实成员标签。

Result: 在无偏条件下，最先进的MIA方法性能收敛到随机猜测水平，表明先前的高成功率主要来自分布偏差。

Conclusion: OpenLVLM-MIA为LVLM隐私保护技术开发提供了透明无偏的基准，阐明了当前MIA研究的局限性。
...
Method: 提出跨图像笔画注意力机制嵌入自注意力层，建立细粒度语义对应关系；开发自适应对比度增强和语义聚焦注意力来加强内容保持和前景强调。
...
<details>
  <summary>Details</summary>
Motivation: 由于现有数据集无法满足研究规模需求，作者构建了ScaleDF数据集来系统分析深度伪造检测任务中的缩放规律。

Method: 构建了包含51个真实图像域和102种深度伪造方法的ScaleDF数据集，包含580万真实图像和880万伪造图像，系统分析模型性能与数据规模的关系。

Result: 观察到深度伪造检测误差随真实域数量和伪造方法数量的增加呈幂律衰减，可以预测达到目标性能所需的额外数据量。
...
Method: 将高分辨率潜在空间划分为固定大小的局部窗口以降低注意力复杂度，同时使用低分辨率潜在空间注入全局语义，通过LoRA适配桥接全局和局部路径。

Result: Scale-DiT比密集注意力基线推理速度快2倍以上，内存使用更低，可靠扩展到4K分辨率，在定量和定性评估中均表现优异。

Conclusion: 分层局部注意力与引导低分辨率锚点相结合是推进超高清图像生成的有效方法。
...
Method: 采用云边协同架构，设备端轻量扩散模型快速生成预览图像，云端高容量模型进行最终优化，并引入噪声水平预测器动态平衡计算负载

Result: 相比Stable Diffusion v1.5平均生成时间减少15.8%，图像质量相当；相比Tiny-SD仅慢0.9%但图像质量显著提升

Conclusion: DiffusionX在保持图像质量的同时实现了高效的图像生成，展示了良好的效率和可扩展性
...
Method: 提出TokenAR框架，包含三个token级增强组件：1) Token Index Embedding聚类token索引以更好表示相同参考图像；2) Instruct Token Injection作为额外视觉特征容器注入详细补充先验；3) 身份token解耦策略(ITD)显式引导token表示独立表示每个身份特征。

Result: 方法显著增强了现有AR方法在条件图像生成中的能力，在保持高质量背景重建的同时实现良好身份一致性。在多个参考图像生成任务中超越了当前最先进模型。

Conclusion: TokenAR框架有效解决了多参考图像生成中的身份混淆问题，并引入了首个开源的大规模多参考输入数据集InstructAR Dataset，为多参考图像生成训练和评估提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLM研究过度关注LLM骨干而忽视视觉编码器的作用，特别是在训练范式从SFT转向RL后，缺乏对视觉编码器如何被重塑的分析。

Method: 通过ImageNet分类、分割和梯度可视化等多样化实验，分析不同训练策略对MLLM视觉表示的影响，并开发了PIVOT方法来优化视觉编码器。

Result: RL比SFT产生更强且更精确定位的视觉表示，PIVOT训练的视觉编码器性能超越更大规模的预训练模型，计算成本仅为标准预训练的1%。

Conclusion: RL训练能显著提升MLLM视觉编码器的能力，PIVOT提供了一种高效构建强视觉骨干的路径，为MLLM发展开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决社交制造中质量控制的挑战，特别是缺陷检测面临的三个主要困难：高度定制化配置、碎片化小批量订单以及分布式站点间的成像环境差异。

Method: 引入MIRAD数据集，捕捉社交制造领域的三个关键维度：多样化定制产品的大类内变异、来自六个地理分散制造节点的数据收集以及包括光照、背景和运动条件变化的显著成像异质性。

Result: 在最先进的异常检测方法上进行广泛评估，结果显示与传统基准相比所有模型性能显著下降，突显了现实世界个性化生产中缺陷检测未解决的复杂性。
...
Method: 收集了来自两个手术中心的3000个白内障手术视频，包含四个注释层，并进行了基准实验和领域适应研究。

Result: 数据集支持关键手术AI任务的基准测试，包括工作流识别、场景分割和自动技能评估。

Conclusion: 该数据集为计算机辅助白内障手术系统的发展提供了高质量、多样化的资源。
...
<details>
  <summary>Details</summary>
Motivation: 印度道路坑洞造成严重安全隐患和维护挑战，需要自动化解决方案来改善道路基础设施维护。

Method: 使用自标注的7000多个印度道路仪表盘摄像头帧数据集微调YOLO模型进行坑洞检测，结合OCR提取的时间戳和外部GPS日志进行精确定位，通过优化后端数据库管理道路段和承包商信息。

Result: 开发了具备智能治理功能的完整平台，能够自动向承包商和官员发送警报，支持自动化问责和保修执行，并提供可操作的公共分析界面。

Conclusion: iWatchRoadv2通过自动化坑洞监测生命周期，实现了数据驱动的智慧城市管理、透明治理和可持续的道路基础设施维护改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数化模型主要针对人类和动物，缺乏对植物的同等表达能力，而植物建模在农业和生物学中具有重要应用价值。

Method: 收集大规模大豆农场数据集，开发Demeter参数化模型，编码植物形态的关键因素，处理不同物种的拓扑变化，并建模关节运动、子组件形状变化和非刚性变形。

Result: 实验表明Demeter能有效合成形状、重建结构和模拟生物物理过程。

Conclusion: Demeter为作物植物建模提供了有效的参数化解决方案，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 随着AR/VR设备的普及，边缘设备需要实时推理、低功耗和最小延迟的深度学习模型，但现有方法在效率和性能之间难以平衡。

Method: 采用编码器-解码器架构，在ResNet-18骨干网络上应用稀疏卷积，提出SPLite解码器提升解码速度，并使用量化感知训练减少内存使用。

Result: 端到端效率提升42%，解码帧率在Raspberry Pi 5上提高3.1倍，内存使用减少，精度损失极小（PA-MPJPE从9.0mm仅增加到9.1mm），整体系统加速2.98倍。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂人类指令与精确3D物体定位之间的差距，现有3D分割方法难以解释模糊的推理指令，而2D视觉语言模型缺乏内在3D空间理解能力。

Method: 在3D高斯溅射表示上直接进行分割，利用其渲染逼真新视图的能力；提出全局到局部空间定位策略，先通过多个全局视图进行粗粒度定位，再合成特写视图进行细粒度分割。

Result: 在LERF、3D-OVS和新提出的REALM3D基准测试中，REALM在解释显式和隐式指令方面表现出色，并支持物体移除、替换和风格转换等3D交互任务。

Conclusion: REALM框架在3D推理分割方面取得了显著性能，展示了其实际实用性和多功能性。
...
Method: 将自监督学习目标（如图像旋转预测、掩码补丁重建）重新表述为密集的自动奖励信号，无需人工偏好数据或不可靠的AI评估器。

Result: SSL4RL显著提升了视觉中心和视觉语言推理基准的性能，在图形学习中也获得显著收益。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将点和线匹配作为独立任务处理，而GlueStick虽然联合处理但架构过重，无法满足实时应用或边缘设备部署需求。

Method: 提出注意力线消息传递机制，显式地将线的连通性暴露给网络，实现节点间高效通信。

Result: 在多个基准测试中建立了新的最先进性能，代码已开源。

Conclusion: LightGlueStick通过轻量化设计实现了高效的点和线段联合匹配，适用于实时应用和边缘设备部署。
...
Method: 使用时空细微信息标记化提取融合全局和局部跨帧深度伪造特征，构建细粒度多模态思维链机制引入面部特征数据作为硬约束，并创建可解释推理FF++基准数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有的动作识别方法在复杂多人场景中难以精确地基于语言描述识别特定人物的细粒度原子动作，跨模态信息对齐能力有限，导致目标人物定位和细粒度动作预测性能不佳。

Method: 提出RefAtomNet++框架，包含多层级语义对齐跨注意力机制和多轨迹Mamba建模。在部分关键词、场景属性和整体句子三个层级构建扫描轨迹，动态选择最近的视觉空间token，实现更有效的时空token聚合。

Result: RefAtomNet++在RefAVA++数据集上建立了新的state-of-the-art结果，显著提升了基于语言描述的原子视频动作识别性能。

Conclusion: 多层级语义对齐跨注意力机制结合多轨迹Mamba建模能够有效提升跨模态信息对齐能力，在语言引导的原子视频动作识别任务中取得显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 在计算机视觉应用中，如航空影像、遥感和自动驾驶，准确检测旋转目标是一个重要挑战。传统目标检测框架对轴对齐目标有效，但在处理旋转目标时由于无法有效捕捉方向变化而表现不佳。

Method: 使用高斯边界框表示和Bhattacharyya距离构建改进的损失函数，采用各向异性高斯表示解决方形物体中各项同性方差的问题，提出旋转不变损失函数来有效捕捉旋转目标的几何特性。

Result: 将提出的损失函数集成到最先进的深度学习旋转目标检测器中，大量实验显示在平均精度指标上相比现有方法有显著提升。

Conclusion: 该方法有潜力在旋转目标检测领域建立新的基准，对需要精确可靠目标定位的各种应用具有重要意义。
...
Conclusion: VIPAMIN提供了一种简单而有效的视觉提示初始化策略，能够显著提升自监督模型在下游任务中的适应能力，特别是在具有挑战性的场景中。
...
TL;DR: 提出了一种弱监督域自适应方法，利用稀疏点标注进行线粒体分割，通过多任务学习和实例感知伪标签选择策略，显著提升了跨域分割性能。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜图像中线粒体实例分割标注成本高昂，无监督域自适应方法在实际应用中性能较低，需要开发更高效的弱监督方法。

Method: 采用多任务学习框架，联合进行分割和中心检测，结合交叉教学机制和类聚焦跨域对比学习，并提出实例感知伪标签选择策略。

Result: 在多个挑战性数据集上的验证表明，该方法优于现有的无监督和弱监督域自适应方法，显著缩小了与全监督方法的性能差距。

Conclusion: 该方法能够有效利用稀疏点标注，在减少标注成本的同时实现高质量的跨域线粒体分割，在无监督域自适应设置下也取得了显著改进。
...
TL;DR: 提出一种面向目标视觉语言导航的前瞻性智能体，通过Q-learning从大规模无标签轨迹数据中学习场景布局和物体关系知识，结合未来信息进行A*式搜索。
...
Method: 使用Q-learning训练Q模型学习场景通用知识，生成描述行动潜在未来信息的Q特征，通过跨模态未来编码器结合导航指令产生反映未来前景的行动得分，与历史得分结合实现A*式搜索。

Result: 在广泛使用的目标导向VLN数据集上进行大量实验，验证了所提方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于3DGS的动态3D场景压缩方法缺乏人体先验知识，导致在数字人编码和传输中比特率效率和重建质量不理想，限制了其在可流式3D虚拟人系统中的应用。

Method: 将高斯表示解耦为结构层（通过StyleUNet生成器将姿态映射到高斯）和运动层（利用SMPL-X模型紧凑表示时序姿态变化），并引入面部注意力机制来保持身份和表情细节。
...
TL;DR: PRISMM-Bench是第一个基于真实科学论文中审稿人标记的不一致性的多模态基准测试，包含262个来自242篇论文的不一致性案例，用于评估大型多模态模型在检测、纠正和推理跨模态不一致性方面的能力。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型越来越多地应用于科学研究，但尚不清楚它们是否能可靠地理解和推理论文的多模态复杂性。现有基准测试要么隔离单一模态，要么依赖合成错误，无法捕捉真实世界的复杂性。

Method: 通过多阶段流程（包括审稿挖掘、LLM辅助过滤和人工验证）收集262个不一致性案例，设计三个任务：不一致性识别、纠正和配对匹配，并引入结构化JSON答案表示以减少语言偏见。

Result: 对21个领先的LMM进行基准测试，结果显示性能极低（26.1-54.2%），包括大型开源模型和专有模型都表现不佳。

Conclusion: 结果凸显了多模态科学推理的挑战性，需要推动可信科学助手的发展。
...
TL;DR: 提出OOS-DSD方法，通过辅助学习改进缺货检测，在YOLOv8架构上增加卷积分支同时进行缺货检测、产品分割和深度估计，性能超越现有最佳方法1.8% mAP。


<details>
  <summary>Details</summary>
Motivation: 缺货检测是重要的零售验证过程，需要准确推断货架上产品的不可用性。现有方法在复杂零售场景中检测精度有限，需要更先进的解决方案。

Method: 扩展YOLOv8目标检测架构，增加卷积分支进行多任务学习：缺货检测、产品分割和深度估计。深度估计分支使用Depth Anything V2生成的伪标签训练，并提出深度归一化程序稳定训练过程。

Result: 实验结果表明，该方法超越现有最佳缺货检测方法1.8% mAP。消融研究证实辅助学习和深度归一化的有效性，分别提升mAP 3.7%和4.2%。

Conclusion: OOS-DSD通过多任务辅助学习显著提升了缺货检测性能，深度估计作为辅助任务对主要任务有积极影响，提出的深度归一化程序有效稳定了训练过程。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种以代表性为中心的方法，利用图结构捕捉图像间的相似性关系，通过注意力机制突出重要特征，构建上下文感知的潜在表示。

Method: 使用图结构表示图像及其相似关系，应用图注意力网络自编码器构建潜在表示，从中提取类别代表，通过比较查询图像代表与类别代表进行分类，并在所属类别中检索最相似图像。

Result: 通过实验验证了该代表性中心方法的有效性，比较了图注意力网络自编码器与标准基于特征的技术。

Conclusion: 提出的代表性中心方法在图像分类和检索任务中表现有效，图注意力机制能够捕捉图像间的关键特征和关系。
...
TL;DR: READ是一种通过添加重构和对齐辅助目标的微调方法，增强视觉语言模型在组合推理上的能力，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在组合推理方面表现不佳，主要原因是文本编码器倾向于关注单个单词而非它们之间的关系，这种局限性被对比训练所强化。

Method: READ方法在对比学习基础上添加两个辅助目标：1）基于原始描述嵌入重构替代描述的token级重构目标；2）在嵌入空间显式对齐改写句子的句子级对齐目标。

Result: READ-CLIP在五个主要组合推理基准测试中达到最先进性能，比最强的传统微调基线高出4.1%。该方法也适用于现有CLIP变体并提升其性能。

Conclusion: 重构和对齐目标提供互补优势：重构鼓励编码器捕捉描述中单词间的关系，而对齐确保不同措辞的改写句子具有一致表示。
...
TL;DR: 提出GaitRDAE框架，通过动态搜索运动区域、自适应时间尺度和区域感知注意力机制，解决步态识别中固定区域建模的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法使用预定义区域进行时间建模，固定或等效的时间尺度难以适应动态变化的运动区域和特定模式，尤其在协变量影响视觉外观时。

Method: GaitRDAE包含两个核心模块：区域感知动态聚合(RDA)模块动态搜索每个区域的最佳时间感受野；区域感知动态激励(RDE)模块强调学习包含稳定行为模式的运动区域，抑制对易受协变量影响的静态区域的注意力。

Result: 实验结果表明，GaitRDAE在多个基准数据集上达到了最先进的性能。

Conclusion: 该框架通过动态区域感知机制有效提升了步态识别的准确性，能够更好地适应动态变化的运动模式和协变量影响。
...
TL;DR: 本文基于真实世界政治深度伪造数据库创建了首个系统性基准测试，评估了学术界、政府和工业界的最先进深度伪造检测器，发现现有检测器对真实政治深度伪造的泛化能力不足，且易受简单操作攻击。


<details>
  <summary>Details</summary>
Motivation: AI生成内容的快速扩散加剧了错误信息风险，特别是政治深度伪造会扭曲真相并破坏对政治机构的信任。现有检测模型主要在实验室合成数据集上训练验证，难以泛化到社交媒体上传播的真实政治深度伪造。

Method: 基于政治深度伪造事件数据库构建系统性基准，该数据库收集了2018年以来社交媒体上分享的真实世界政治深度伪造内容。对学术界、政府和工业界的最先进深度伪造检测器进行系统性评估。

Result: 学术界和政府开发的检测器表现相对较差。付费检测工具比免费模型表现稍好，但所有评估的检测器都难以有效泛化到真实政治深度伪造，且易受简单操作攻击，特别是在视频领域。

Conclusion: 结果强调了需要开发政治情境化的深度伪造检测框架，以在真实世界环境中更好地保护公众。
...
TL;DR: SHIELD是一个无需训练的训练框架，通过重新加权视觉标记、引入噪声衍生标记和应用对抗攻击来缓解大型视觉语言模型中的物体幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在跨模态任务中表现出色，但物体幻觉（模型生成看似合理但不准确的物体描述）仍然是一个重大挑战。与之前关注LLM组件的研究不同，本文首次将LVLM幻觉追溯到视觉编码器，并识别出三个关键问题：统计偏差、固有偏差和脆弱性。

Method: 提出SHIELD框架，采用三种策略：重新加权视觉标记以减少统计偏差，引入噪声衍生标记以对抗固有偏差，应用对抗攻击与对比解码来解决脆弱性问题。

Result: 实验表明，SHIELD在多种基准测试和LVLM家族中有效缓解了物体幻觉。此外，在通用LVLM基准测试中表现出色，显示了其广泛的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理高分辨率图像或多图像输入时面临计算和内存瓶颈，现有令牌压缩技术受限于启发式规则，可能丢弃关键信息并存在注意力偏差问题。

Method: 提出VisionSelector评分器模块，与MLLM主干解耦，包含可微分Top-K机制和课程退火策略，实现端到端的可学习令牌选择过程。

Result: 仅需12.85M可训练参数，在MME基准上30%保留预算时保持100%准确率，10%保留预算时比先前方法提升12.14%，预填充速度提升一倍。

Conclusion: VisionSelector提供了一种高效自适应的令牌压缩解决方案，在各种压缩率下都能保持优越性能，显著提升MLLMs的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统医学图像处理技术缺乏实时临床应用所需的精度、鲁棒性和速度，临床医生解读高分辨率放射学数据耗时且存在变异性。

Method: 集成U-Net、EfficientNet和Transformer等神经网络架构，采用模型剪枝、量化和GPU加速等实时优化策略，支持在边缘设备、本地服务器和云基础设施上灵活部署。

Result: 在公共基准数据集上实现分类准确率超过92%，分割Dice分数超过91%，推理时间低于80毫秒，并通过Grad-CAM和分割覆盖等可视化工具增强可解释性。

Conclusion: 该框架能显著加速诊断工作流程，减轻临床医生工作量，支持在时间关键的医疗环境中可信AI集成。
...
TL;DR: 提出基于视觉的小型无人机室内自主飞行系统，结合语义分割和单目深度估计实现避障、场景探索和安全着陆，无需GPS或昂贵传感器。


<details>
  <summary>Details</summary>
Motivation: 解决资源受限平台上视觉导航的挑战，特别是度量深度估计和计算效率问题，使小型无人机能在受控室内环境中自主飞行。

Method: 使用知识蒸馏框架，SVM教师生成训练数据，轻量级U-Net学生网络进行实时语义分割；开发自适应尺度因子算法将非度量深度预测转换为准确度量距离。

Result: 在5x4米实验室环境中测试，平均距离误差14.4厘米；30次真实飞行和100次数字孪生测试显示，结合分割和深度方法增加监视距离，减少任务时间，成功率100%。

Conclusion: 该工作推进了结构化环境中基于视觉的无人机导航实践，展示了在资源受限平台上部署的可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多轮数据集（如MMDU、ConvBench）仅部分覆盖用户遇到的各种对话场景，需要更全面的多轮对话评估基准。

Method: 从12个流行VLM评估基准中提取647个对话，每个对话平均4轮，提出基于检查表的评估方法，使用GPT-4o作为自动评估器，测量37个关键方面的性能。

Result: 即使最强模型（如GPT-4o）在复杂多轮对话中也仅达到50%的成功率，提供完整对话上下文能显著提升较小或较弱模型的性能。
...
TL;DR: 提出使用检索增强生成方法，通过图数据库和Cypher查询语言接口让LLM从3D场景图中检索相关数据，解决自然语言接地问题，相比基线方法在大规模场景图中表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D场景图序列化为文本编码到LLM上下文窗口，但这种方法无法扩展到大型或丰富的3D场景图。需要一种更高效的接口来连接LLM和3D场景图。

Method: 使用检索增强生成方法，将3D场景图编码到图数据库中，为LLM提供Cypher查询语言作为工具接口，使其能够检索与任务相关的场景图数据。

Result: 在指令跟随和场景问答任务上的评估显示，使用Cypher接口的方法在大规模丰富场景图中显著优于基线方法，性能提升明显，同时大幅减少了场景图内容的token数量。

Conclusion: Cypher作为3D场景图的接口在本地和云端模型上都显著提高了可扩展性，在接地语言任务中带来大的性能改进，同时大幅降低场景图内容的token开销。
...
TL;DR: 提出了针对病理学基础模型的通用可迁移对抗扰动(UTAP)，这是一种固定且微弱的噪声模式，能系统性地破坏多个病理学基础模型的特征表示能力，导致下游任务性能下降。
...
Method: 使用深度学习优化的固定弱噪声模式，将其添加到病理图像中，系统性地破坏多个病理学基础模型的特征表示能力。

Result: UTAP在各种最先进的病理学基础模型上进行了系统评估，通过视觉不可察觉的固定噪声模式显著降低了模型性能，具有跨数据集通用性和跨模型可迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有多尺度注意力方法在密集光谱重建中的泛化能力不足问题，现代高光谱传感器包含数百个通道，需要更有效的重建方法

Method: 使用教师模型编码潜在高光谱数据，学生模型学习从自然图像到教师编码域的映射，结合新颖的训练方法进行知识蒸馏

Result: 在所有指标上达到SOTA性能，准确率提升18%，在不同通道深度下推理时间均快于当前SOTA模型
...
TL;DR: 提出MSSR框架，通过最小充分集原则解决VLM在3D空间推理中的瓶颈，使用感知代理和推理代理协同工作，在多个基准测试中达到最先进性能。
...
Method: 提出双代理框架MSSR：感知代理使用感知工具箱程序化查询3D场景提取充分信息（包括新颖的SOG模块），推理代理迭代精炼信息追求最小性，通过闭环修剪冗余和补充缺失信息，直到构建最小充分集。

Result: 在多个挑战性基准测试中显著提高准确率，达到最先进性能，并产生可解释的推理路径。

Conclusion: 通过明确追求充分性和最小性，MSSR框架有效解决了VLM的3D空间推理问题，为未来模型提供了高质量训练数据源。
...
TL;DR: 提出SDPA++框架，仅使用噪声OCT图像进行自监督去噪，通过自融合生成伪地面真值，然后训练集成去噪模型提升图像质量


<details>
  <summary>Details</summary>
Motivation: OCT成像中固有散斑噪声和临床环境限制使得获取配对的干净-噪声图像数据集非常困难，需要无需干净参考的自监督去噪方法

Method: 自监督去噪框架：1）通过自融合和自监督去噪生成伪地面真值图像；2）使用基于块的策略训练集成去噪模型；3）在真实世界噪声OCT数据集上验证

Result: 在IEEE SPS VIP Cup真实世界数据集上验证，通过CNR、MSR、TP和EP等指标显示性能提升，该数据集仅含噪声图像无干净参考

Conclusion: SDPA++框架在无需干净参考图像的情况下有效提升OCT图像质量，具有临床应用的潜力
...
TL;DR: 提出了一种新的领域连接对比学习(DCCL)方法来解决领域泛化问题，通过增强跨领域的类内连通性来提升模型在未见目标域上的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 领域偏移问题严重影响模型泛化能力，直接应用对比学习反而会降低领域泛化性能，原因是缺乏类内连通性。

Method: 提出DCCL方法：在数据层面使用更激进的数据增强和跨域正样本；在模型层面使用模型锚定技术利用预训练表示的类内连通性，并辅以生成变换损失。

Result: 在五个标准领域泛化基准测试中，DCCL无需领域监督即可超越最先进的基线方法。

Conclusion: DCCL通过增强跨领域类内连通性，有效解决了领域泛化问题，在多个基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型需要多步去噪过程，推理效率较低。本文旨在开发一个能够实现高效单步生成的人体运动预测方法。

Method: 采用基于Transformer的时空架构，结合时间嵌入来建模长程依赖关系并保持运动连贯性，学习噪声运动状态与干净运动状态之间的自一致映射。

Result: 在Human3.6M和HumanEva-I数据集上的实验表明，HumanCM在保持与最先进扩散模型相当或更优精度的同时，显著提升了推理效率。
...
TL;DR: 提出了SCENECOT框架，通过基于多模态专家模块的视觉线索构建，将复杂3D场景推理任务分解为简单可管理的问题，实现了首个成功的3D场景CoT推理应用。


<details>
  <summary>Details</summary>
Motivation: 现有3D大语言模型在基于场景-对象的基础问答方面仍存在困难，主要因为缺乏对人类场景-对象基础推理机制的深入探索。

Method: 引入基于3D场景的基础思维链推理方法(SCENECOT)，开发了首个大规模基础CoT推理数据集SCENECOT-185K，包含18.5万高质量实例。

Result: 在多个复杂3D场景推理基准测试中表现出强大性能，具有高度的基础问答一致性。

Conclusion: 这是思维链推理在3D场景理解中的首次成功应用，实现了逐步的人类式推理，并显示出扩展到更广泛3D场景理解场景的潜力。
...
TL;DR: IR-WM是一个隐式残差世界模型，专注于建模当前状态和世界演化，通过预测残差变化来避免冗余建模静态背景，在nuScenes基准测试中实现了4D占用预测和轨迹规划的顶级性能。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统中的视觉中心世界模型存在效率问题，它们完全重构未来场景，将大量计算能力浪费在冗余建模静态背景上。

Method: IR-WM首先从视觉观测建立鲁棒的鸟瞰图表示，然后利用前一时刻的BEV特征作为强时序先验，仅预测基于自车动作和场景上下文的残差变化，并应用对齐模块来校准语义和动态错位。

Result: 在nuScenes基准测试中，IR-WM在4D占用预测和轨迹规划方面都达到了顶级性能，证明了隐式未来状态生成对规划准确性的显著提升。

Conclusion: IR-WM通过专注于建模世界状态的变化而非完全重构，有效提高了自动驾驶世界模型的效率和性能，展示了残差建模和对齐机制的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 全球珊瑚礁产品如Allen Coral Atlas在空间精度和语义一致性方面存在局限，特别是在需要精细边界划分的区域。需要解决在噪声监督下实现高精度映射的挑战。

Method: 基于UKAN架构，在解码器中加入全局-局部变换器（GL-Trans）模块，能够同时提取全局语义结构和局部边界细节。

Result: 在实验中，UKANFormer实现了珊瑚类IoU 67.00%和像素精度83.98%，在相同噪声标签设置下优于传统基线方法。模型产生的预测在视觉和结构上都比训练使用的噪声标签更准确。

Conclusion: 研究挑战了数据质量直接限制模型性能的观念，表明架构设计可以缓解标签噪声，支持在不完善监督下进行可扩展映射，为生态监测提供了基础。
...
TL;DR: 该论文提出了一个用于具身AI中世界模型的统一框架，包括问题形式化、学习目标和三轴分类法，系统整理了数据资源和评估指标，并分析了当前挑战。
...
Method: 提出了三轴分类法：(1) 功能性：决策耦合vs通用目的；(2) 时序建模：序列模拟推理vs全局差异预测；(3) 空间表示：全局潜在向量、令牌特征序列、空间潜在网格和分解渲染表示。

Result: 系统整理了机器人学、自动驾驶和通用视频设置中的数据资源和评估指标，包括像素预测质量、状态级理解和任务性能，并对最先进模型进行了定量比较。

Conclusion: 指出了关键开放挑战：统一数据集的稀缺性、需要评估物理一致性而非像素保真度的指标、模型性能与实时控制计算效率的权衡，以及实现长期时序一致性同时减轻误差积累的核心建模难度。
...
TL;DR: 研究表明，离散的自回归视觉模型通过beam搜索在图像生成中显著优于连续扩散模型，2B参数的自回归模型表现超过12B参数的扩散模型，表明模型架构对推理时优化至关重要。


<details>
  <summary>Details</summary>
Motivation: 尽管搜索策略在大型语言模型中取得了革命性进展，但在图像生成领域应用类似方法却面临困难，特别是在连续扩散模型中效果有限。

Method: 利用离散自回归视觉模型的特性，采用beam搜索策略进行图像生成，通过早期剪枝和计算重用来提升效率。

Result: 2B参数的自回归模型在文本到图像生成任务中超越了12B参数的扩散模型，验证了离散token空间在推理时优化的优势。

Conclusion: 模型架构（而不仅仅是规模）对于视觉生成中的推理时优化至关重要，离散自回归模型在搜索策略应用方面具有独特优势。
...
TL;DR: 提出了一种基于显著性的超分辨率图像伪影评估方法，通过众包标注构建了包含1302个伪影样本的数据集，并训练了轻量级回归器来生成空间显著热力图。


<details>
  <summary>Details</summary>
Motivation: 随着超分辨率模型能力的提升，产生的伪影问题日益突出。不同伪影对人类观察者的感知影响差异很大，需要根据其显著程度而非统一处理为二元缺陷来评估。

Method: 构建了包含1302个来自11种当代图像超分辨率方法的伪影样本数据集，每个样本都有众包标注的显著度评分。基于此数据集训练轻量级回归器生成空间显著热力图。

Result: 训练的回归器在检测显著伪影方面优于现有方法，能够有效识别对感知质量影响较大的伪影。

Conclusion: 该方法为基于显著性的超分辨率伪影评估和缓解提供了有效工具，释放的数据集和代码将促进该领域的发展。
...
TL;DR: 提出WaMaIR框架，通过全局多尺度小波变换卷积扩大感受野，结合Mamba通道感知模块捕获长程依赖，并使用多尺度纹理增强损失，显著提升图像恢复的纹理细节重建效果。
...
Method: 引入全局多尺度小波变换卷积(GMWTConvs)扩大感受野，提出Mamba通道感知模块(MCAM)捕获通道间长程依赖，设计多尺度纹理增强损失(MTELoss)指导纹理结构保留。

Result: 实验表明WaMaIR在图像恢复任务中优于现有最优方法，同时保持高效计算性能。

Conclusion: WaMaIR框架通过扩大感受野和增强通道建模能力，有效提升了图像恢复中纹理细节的重建质量。
...
TL;DR: 提出Region in Context框架，通过多级语义对齐实现文本条件图像编辑，确保局部修改与全局场景的协调性
...
Conclusion: 通过理解区域在全局图像上下文中的角色，实现了精确协调的图像编辑
...
TL;DR: 提出EMRRG框架，通过微调预训练的Mamba网络来生成X射线医疗报告，使用参数高效方法，在基准数据集上取得优异结果。


<details>
  <summary>Details</summary>
Motivation: 现有医疗报告生成模型主要依赖大语言模型，缺乏对预训练视觉基础模型和先进微调技术的探索，特别是非Transformer架构如Mamba网络在医疗报告生成中的应用研究不足。

Method: 将X射线图像分块、标记化，通过SSM-based视觉骨干网络提取特征，使用Partial LoRA进行参数高效微调，结合混合解码器的LLM生成医疗报告，实现端到端训练。

Result: 在三个广泛使用的基准数据集上的广泛实验充分验证了所提策略对X射线医疗报告生成的有效性。

Conclusion: EMRRG框架通过微调预训练Mamba网络和参数高效方法，为X射线医疗报告生成提供了有效的解决方案，代码将在GitHub上开源。
...
TL;DR: GS2POSE提出了一种基于3D高斯溅射和束调整原理的6D物体姿态估计算法，通过可微渲染管道迭代优化姿态参数，在纹理缺失和光照变化场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决传统基于2D-3D特征对应方法在纹理缺失物体和光照变化条件下的困难，提升6D姿态估计的鲁棒性。

Method: 结合束调整原理和Lie代数，构建基于3DGS的姿态可微渲染管道，通过比较输入图像与渲染图像迭代优化姿态参数，同时更新3DGS模型的颜色参数以适应光照变化。

Result: 在T-LESS、LineMod-Occlusion和LineMod数据集上分别实现了1.4%、2.8%和2.5%的精度提升。

Conclusion: GS2POSE通过可微渲染和束调整优化，有效提升了6D姿态估计在挑战性场景下的性能，为纹理缺失和光照变化问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型依赖大量标注数据和任务特定训练，成本高且可扩展性有限。本文旨在开发无需端到端训练的零样本视频理解方法。

Method: 首先使用预训练VLM的冻结视觉编码器将视频转换为语义特征轨迹，然后使用核时间分割(KTS)将连续特征流划分为离散的语义连贯事件段，最后通过无监督密度聚类识别视频中的重复宏观场景和主题。

Result: 该方法能够自动生成结构化的多模态视频内容摘要，通过从每个发现的聚类中选择代表性关键帧并利用VLM的文本生成能力进行描述。
...
Method: 在完全冻结的MLLM上附加轻量级可训练头部，通过优化注意力图中的空间线索来提取关键点，并将其描述为与掩码解码器直接兼容的点级特征。
...
Method: 基于几何先验生成弱标签（地平线上为非道路，车辆前方四边形为道路），然后通过跨帧跟踪局部特征点并利用互信息最大化来增强时序一致性。

Result: 在Cityscapes数据集上实现了0.82的IoU，表现出高精度和良好的时序稳定性。

Conclusion: 几何约束与时序一致性结合的方法在无监督道路分割中具有巨大潜力，为自动驾驶提供了可扩展的解决方案。
...
TL;DR: 提出个性化图像滤镜PIF，基于预训练扩散模型学习摄影概念的平均外观，通过文本反转技术优化摄影概念提示，实现摄影风格的提取和迁移。


<details>
  <summary>Details</summary>
Motivation: 摄影风格是著名摄影师魅力的体现，但学习和迁移摄影风格需要深入理解照片从原始外观到编辑后的变化过程。现有方法要么无法从参考图像中学习有意义的摄影概念，要么无法保持内容图像的内容。

Method: 基于预训练文本到图像扩散模型，利用生成先验学习摄影概念的平均外观，并通过文本反转技术优化摄影概念的提示，从而学习参考图像的摄影风格。
...
TL;DR: 构建了一个荔枝检测和成熟度分类的开源数据集，包含11,414张图像，涵盖不同品种、天气条件和成熟阶段，并进行了详细统计分析和深度学习模型评估。
...
Method: 采集了多种荔枝品种在不同天气条件和时间段的RGB图像，通过数据增强生成更多样本，包含三种成熟度阶段，并由多人独立标注后统一验证。

Result: 数据集包含11,414张图像（878张原始RGB、8,780张增强RGB、1,756张深度图像），标注了9,658对荔枝检测和成熟度分类标签，并通过三种代表性深度学习模型进行了评估。

Conclusion: 该数据集填补了荔枝视觉检测领域的空白，为开发荔枝采摘机器人提供了高质量的数据支持，并已公开供学术研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 由于气候变化等人为压力导致珊瑚礁快速衰退，迫切需要可扩展的自动化监测方法。现有数据集在规模、地理覆盖范围或标注粒度方面存在限制，且不适用于机器学习。

Method: 收集来自76个CoralNet来源和红海Al Wajh站点的图像，提供细粒度、分类学映射的标注。提出两种评估设置：源内基准测试和跨源基准测试，分析监督学习和零样本分类性能。

Result: 监督学习在源内表现良好，但在跨域时性能急剧下降；零样本模型在所有情况下表现较差，特别是对于稀有和视觉相似的属。
...
Method: 综合分析五种纹理特征预测水分含量，提出AdaptMoist域适应方法，利用纹理特征在不同域间转移知识，并基于调整互信息提出模型保存准则。
...
TL;DR: 提出2DGS-R方法，通过分层训练策略在保持几何精度的同时提升渲染质量，仅需1%额外存储和少量训练时间即可实现高质量渲染和精细几何结构。
...
<details>
  <summary>Details</summary>
Motivation: 传统武器检测方法只能提供粗粒度的边界框定位，缺乏细粒度分割能力；现有语义分割模型要么牺牲精度追求效率，要么计算资源需求过高，不适合边缘设备部署。

Method: 结合CBAM增强的编码器主干和注意力集成汉堡解码器，实现手枪、步枪、刀具、左轮手枪和人类五类武器的多类别分割。

Result: 在80.64% mIoU和89.13% mFscore上达到最先进性能，实时推理速度82.26 FPS，仅需4.886G FLOPs和3.66M参数，比重量级模型计算量减少48倍。
...
Method: 提出BARL框架，包含两个协作分支：标签空间对齐使用双路径正则化和渐进认知偏差校正；表示空间对齐通过区域级和病灶实例级匹配来捕捉医学图像中复杂的病理模式。
...
Conclusion: 点云配准在引导特征提取朝向旋转不变和局部判别表示方面起着关键作用，所提出的集成框架显著提升了3D异常检测的性能。
...
Method: 结合细粒度人工神经元分析和基于fMRI的体素编码，研究了两种架构不同的视觉语言模型（CLIP和METER），通过神经元级别分析框架探索多模态信息处理机制。

Result: 发现了四个关键发现：人工神经元能成功预测多个功能网络中生物神经元的活动；两者都表现出功能冗余；人工神经元展现与生物神经元相似的极性模式；不同架构驱动不同的生物神经元激活模式。

Conclusion: 这些结果为视觉语言模型中存在类似大脑的层次处理提供了有力证据，表明在神经元级别存在共享的表征机制和相似的信息处理特性。
...
Result: 模型能够生成更真实多样的图像，同时分类器在下游诊断任务中表现出改进的性能。
...
TL;DR: Edit-R1是一个基于策略优化的指令图像编辑后训练框架，通过DiffusionNFT方法和MLLM奖励模型，在ImgEdit和GEdit-Bench基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决监督微调模型对标注模式过拟合的问题，提升模型在训练分布之外的泛化能力。

Method: 使用Diffusion Negative-aware Finetuning (DiffusionNFT)策略优化方法，结合多模态大语言模型作为统一的训练免费奖励模型，并设计了低方差组过滤机制来减少评分噪声。

Result: UniWorld-V2在ImgEdit和GEdit-Bench基准上分别获得4.49和7.83分，达到最先进水平。该框架具有模型无关性，在Qwen-Image-Edit和FLUX-Kontext等不同基础模型上都能带来显著性能提升。

Conclusion: Edit-R1框架有效解决了指令图像编辑中的过拟合问题，通过策略优化和MLLM奖励机制实现了更好的泛化性能，具有广泛的适用性。
...
TL;DR: 本文提出了一种使用地面相机进行凝结尾迹到航班归因的模块化框架，通过高时空分辨率捕捉凝结尾迹形成初期的特征，解决了卫星归因的局限性。


<details>
  <summary>Details</summary>
Motivation: 航空业的非CO2效应（特别是凝结尾迹）对气候变化有重要影响，但验证物理模型需要将观测到的凝结尾迹与生成它们的航班关联起来。卫星归因因时空分辨率有限而具有挑战性。

Method: 利用地面可见相机凝结尾迹序列数据集，开发模块化框架，将地面相机观测的凝结尾迹与基于飞机监视和气象数据推导的理论凝结尾迹进行关联，支持多种几何表示和距离度量，包含时间平滑和基于概率的分配策略。

Result: 建立了一个强大的基线，为未来凝结尾迹与源航班关联研究提供了模块化框架。

Conclusion: 地面相机方法在凝结尾迹形成初期捕捉高分辨率数据，为凝结尾迹归因提供了可行的替代方案，有助于改进凝结尾迹气候影响模型的验证和校准。
...
TL;DR: 该研究评估了四种基于Transformer的架构（SegFormer、DeepLabV3+、SegNeXt和Swin Transformer）在热成像武器分割任务中的表现，在9,711张热成像图像数据集上取得了显著性能提升，其中SegFormer-b5达到最高mIoU 94.15%。


<details>
  <summary>Details</summary>
Motivation: 热成像武器分割在低光照和视觉遮挡条件下具有重要应用价值，但现有基于CNN的方法在捕获长距离依赖和精细结构细节方面存在局限，而Transformer在RGB分割任务中表现出色但在热成像领域尚未充分探索。

Method: 使用四种Transformer架构（SegFormer、DeepLabV3+、SegNeXt、Swin Transformer）在包含9,711张真实世界监控视频热成像图像的数据集上进行二元武器分割，采用MMSegmentation框架和标准数据增强策略进行模型训练和公平比较。

Result: SegFormer-b5获得最高mIoU 94.15%和像素精度97.04%，SegFormer-b0提供最快推理速度98.32 FPS且mIoU达90.84%，SegNeXt-mscans在85.12 FPS下达到92.24% mIoU，DeepLabV3+ R101-D8在29.86 FPS下达到92.76% mIoU。
...
TL;DR: 提出了Res-Bench基准测试，评估多模态大语言模型在不同输入分辨率下的性能稳定性，引入新的鲁棒性指标来测量分辨率与性能的关系。
...
Method: 构建包含14,400个样本的Res-Bench基准，涵盖12个分辨率级别和6个核心能力维度。设计了新的评估框架，包括Spearman相关性和绝对/相对连续误差等鲁棒性指标。

Result: 对领先的MLLMs进行了大规模评估，包括模型中心与任务中心的鲁棒性分析、预处理策略（填充和超分辨率）研究，以及稳定性增强的微调探索。

Conclusion: Res-Bench填补了分辨率鲁棒性评估的空白，为多模态大语言模型的稳定性评估提供了系统框架。
...
Result: 通过系统分析揭示了基础模型在医学图像分析中的发展趋势，识别了关键挑战如领域适应、计算约束等，并提出了联邦学习、知识蒸馏等新兴解决方案。

Conclusion: 基础模型在医学图像分析中具有巨大潜力，未来研究应关注增强模型的鲁棒性、可解释性和临床整合，以加速其在真实医疗实践中的转化应用。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型生成质量高但采样速度慢，现有蒸馏方法缺乏统一理论框架。

Method: 将扩散蒸馏构建为Bregman散度密度比匹配问题，提供凸分析视角统一现有目标。

Result: 在CIFAR-10和文生图任务上，相比reverse-KL蒸馏获得更好的一步FID，保持与教师模型相近的视觉保真度。

Conclusion: Bregman密度比匹配是高效一步扩散生成的理论基础扎实且实用的方法。
...
TL;DR: 提出了CARE框架，通过序列-图像对比对齐方法解决ADL识别中序列和图像表示的局限性，实现跨表示对齐和任务特定判别性。


<details>
  <summary>Details</summary>
Motivation: 现有ADL识别方法存在表示级限制：序列方法对噪声敏感且缺乏空间感知，图像方法压缩时间动态并扭曲传感器布局，简单融合方法无法有效对齐不同表示视图。

Method: CARE框架整合时间感知的序列编码和空间感知的图像表示，使用序列-图像对比对齐(SICA)和交叉熵分类的联合目标进行端到端学习。

Result: 在三个CASAS数据集上达到最先进性能：Milan 89.8%、Cairo 88.9%、Kyoto7 73.3%，对传感器故障和布局变化具有鲁棒性。

Conclusion: CARE框架通过对比对齐有效利用序列和图像表示的互补优势，为智能家居中可靠的ADL识别提供了有前景的解决方案。
...
TL;DR: 本文提出BaGLM方法，利用大型多模态模型的零样本能力，无需训练即可在线进行视频步骤定位，通过贝叶斯滤波整合历史帧信息，在三个数据集上超越基于训练的离线方法。


<details>
  <summary>Details</summary>
Motivation: 传统视频步骤定位方法需要带标注的训练集且需离线处理整个视频，成本高且无法满足在线决策需求。本文探索无需训练、在线处理的解决方案。

Method: 利用大型多模态模型预测有限帧的步骤，结合贝叶斯滤波原理，通过LLM提取的依赖矩阵和步骤进度估计来建模步骤转换。

Result: BaGLM在三个数据集上表现优于最先进的基于训练的离线方法，证明在线零样本方法的有效性。

Conclusion: 大型多模态模型的零样本能力可以成功应用于视频步骤定位任务，BaGLM方法展示了无需训练即可在线处理的可行性，并超越了传统训练方法。
...
TL;DR: 本文通过实证研究探讨了不同视频特征对时序视频定位任务的影响，发现仅改变视频编码器就能显著影响模型性能，并揭示了不同特征间的互补潜力。
...
Method: 在三个基准数据集上提取基于CNN、时序推理和transformer的视频特征，使用经典架构进行实验比较。

Result: 结果显示仅改变视频编码器就能导致模型性能显著差异，同时揭示了特定特征使用时的模式与错误。
...
TL;DR: 该研究挑战了遥感专用基础模型优于通用视觉基础模型的观点，通过设计基准测试和训练iBOT模型，发现在ViT-B规模下遥感专用模型并未带来一致改进。
...
Method: 设计了衡量遥感模型向低分辨率图像泛化能力的基准测试；在MillionAID数据集上训练了iBOT自监督视觉编码器，并进行了遥感特定的修改。

Result: 研究表明，在ViT-B规模下，这些预训练模型都没有带来相对于通用基线的持续改进。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态大语言模型的能力联合处理文本和视频，以有效定位视频中的自然语言查询，解决直接定位时可能出现的细节缺失和幻觉问题。

Method: 采用两阶段方法：首先将语言查询转换为包含缺失细节和线索的增强句子，然后使用轻量级解码器基于增强查询的上下文表示预测准确边界。训练时使用多实例学习目标动态选择最优查询版本。

Result: 在视频时序定位和段落定位的各种基准测试中取得了最先进的结果，显著优于所有先前提出的基于LLM的时序定位方法，在零样本评估场景中保持明显优势。

Conclusion: ED-VTG方法在视频时序定位任务中表现出色，既优于或可与专门模型相媲美，同时在零样本评估中具有明显优势，证明了多模态大语言模型在该领域的有效性。
...
Method: W2R2框架将2D特征作为语义信标用于"是什么"识别，3D特征作为空间锚点用于"在哪里"定位，包含对齐损失和伪标签损失的双目标损失函数。

Result: 在ScanRefer和ScanQA数据集上的实验表明，W2R2在定位精度和鲁棒性方面取得显著提升，特别是在杂乱户外场景中。

Conclusion: W2R2通过重塑模型内部表示空间，在不改变推理架构的情况下实现了精确的3D定位，有效解决了2D语义偏差问题。
...
TL;DR: 本文提出了一种使用条件StyleGAN2-ADA和StyleGAN3生成高分辨率合成指纹图像的方法，并通过CycleGANs将其转换为逼真的假指纹，解决了生物特征数据收集中的隐私、成本和可访问性问题。


<details>
  <summary>Details</summary>
Motivation: 大型指纹数据集的收集耗时、昂贵且需要严格的隐私保护措施，研究者正在探索使用合成指纹数据来解决这些问题。

Method: 使用条件StyleGAN2-ADA和StyleGAN3架构生成特定手指身份的高分辨率合成活体指纹，然后通过CycleGANs将其转换为模拟各种呈现攻击材料的逼真假指纹。

Result: StyleGAN3模型实现了低至5的FID分数，生成的指纹在0.01%错误接受率下达到99.47%的真实接受率。StyleGAN2-ADA模型在相同条件下达到98.67%的真实接受率。匹配实验确认了强大的隐私保护特性，没有显著的身份泄露证据。
...
TL;DR: 本研究开发了一个临床医生参与的深度学习管道，用于肺癌CT图像分割和预后预测。VNet模型在分割性能、影像组学稳定性和预测准确性方面表现最佳，半监督学习优于监督学习，医生更倾向于使用AI生成的初始掩模进行精修。
...
Method: 使用来自12个公共数据集的999名患者的多中心CT数据，评估了5种深度学习模型（3D Attention U-Net、ResUNet、VNet、ReconNet、SAM-Med3D），与专家轮廓进行基准测试。通过497个PySERA提取的影像组学特征评估分割可重复性，比较监督学习和半监督学习的预后建模，6名医生在7个领域进行定性评估。

Result: VNet获得最佳性能（Dice = 0.83，IoU = 0.71）、影像组学稳定性（平均相关性=0.76，ICC=0.65）和半监督学习下的预测准确性（准确率=0.88，F1=0.83）。半监督学习在所有模型中均优于监督学习。放射科医生更青睐VNet的瘤周表现和平滑边界。

Conclusion: 将VNet与半监督学习结合可产生准确、可重复且临床可信的基于CT的肺癌预后预测，展示了向以医生为中心的AI转化的可行路径。
...
TL;DR: 提出了一种改进行人重识别性能的广义选择方法，通过选择不限于类质心的表示来平衡准确率和平均精度，在多个嵌入方法上都显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 虽然先进的特征提取方法和目标函数改进已经提升了行人重识别性能，但选择更好的类表示是一个研究不足的领域。现有方法主要使用类质心作为表示，但效果不理想。

Method: 提出广义选择方法，选择不限于类质心的表示，可以调整每个类的实际表示数量以满足特定应用需求。

Result: 该方法在多个重识别嵌入方法上都显著提升了性能，超越了现有技术水平，在准确率和平均精度之间取得了良好平衡。

Conclusion: 广义表示选择方法能有效提升行人重识别性能，为改进重识别系统提供了一种有前景的方向。
...
TL;DR: 提出V-Reason方法，通过基于熵的控制器优化LMM推理过程中的值缓存，改善微探索和利用行为，无需RL训练即可显著提升视频推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前视频推理LMM依赖昂贵的RL训练和冗长的思维链，计算开销大且思维过程控制机制有限。

Method: 使用模型输出熵作为信号，通过小型可训练控制器基于熵目标优化LMM值缓存，调整推理时的微探索和利用行为。

Result: 在多个视频推理数据集上显著超越基础指令调优模型，与RL模型差距缩小至0.6%平均准确率，输出token减少58.6%。

Conclusion: 基于熵的推理时优化可有效提升LMM视频推理性能，无需额外训练数据或RL，具有高效性优势。
...
TL;DR: 比较通用视觉基础模型Hiera与专用分割模型SAM2的特征通用性，发现专用化虽然提升空间相关任务性能，但在概念距离较远的任务上表现较差，存在信息损失和表示瓶颈。
...
Method: 使用轻量可训练neck模块来探测冻结特征的适应性，通过信息论成本量化专用化的代价，并进行跨neck分析来揭示表示瓶颈。

Result: SAM2在深度估计等空间相关任务上表现优异，但在姿态估计和图像描述等概念距离较远的任务上表现不如通用模型Hiera，存在可测量的语义信息损失。

Conclusion: 专用化模型在特定任务上表现优异但牺牲了通用性，揭示了特征通用性的权衡关系，为下游应用的高效特征编码和适应策略设计提供了量化基础。
...
<details>
  <summary>Details</summary>
Motivation: 3D点云在自动驾驶、增强现实等应用中需要实时处理和低延迟，但大数据量和带宽限制阻碍了高质量服务的部署。现有学习型点云几何编码方法的固定潜在表示不支持渐进解码

Method: 采用密度感知的尾丢弃机制，利用密度信息作为指导信号，根据重要性自适应解码潜在特征和坐标，实现单一模型下的多比特率渐进解码

Result: 在基准数据集上，ProDAT不仅实现了渐进编码，而且相比最先进的学习型编码技术，在SemanticKITTI上PSNR-D2的BD-rate提升超过28.6%，在ShapeNet上提升超过18.15%

Conclusion: ProDAT成功解决了点云渐进编码问题，在保持编码效率的同时实现了单一模型的多比特率渐进解码能力
...
TL;DR: 提出FMCAF预处理架构，通过频域滤波和跨注意力融合增强RGB和红外图像的多模态目标检测性能，在VEDAI和LLVIP数据集上表现优于传统融合方法。


<details>
  <summary>Details</summary>
Motivation: 多模态目标检测在挑战性条件下通过利用多传感器模态的互补线索来提高鲁棒性，但现有方法往往针对特定数据集，缺乏通用性。

Method: FMCAF结合频域滤波块(Freq-Filter)抑制冗余光谱特征和基于跨注意力的融合模块(MCAF)改进模态间特征共享，无需数据集特定调优。

Result: 在VEDAI数据集上mAP@50提升13.9%，在LLVIP数据集上提升1.1%，优于传统拼接融合方法。

Conclusion: FMCAF作为灵活的多模态融合基础架构，在未来的检测流程中具有潜力。
...
TL;DR: GSPlane是一个改进高斯泼溅(Gaussian Splatting)的方法，通过在3D场景重建中引入平面先验，显著提升平面区域的几何精度和网格质量，同时保持渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有高斯泼溅方法在重建平面区域时往往不够平滑和精确，这限制了在场景编辑和物理模拟等下游应用中的使用。

Method: 利用现成的分割和法向预测模型提取平面先验，建立结构化表示来指导训练过程；引入动态高斯重分类器自适应处理高梯度区域；使用优化的平面先验优化网格布局。

Result: 在各种基准测试中，引入平面先验显著提高了提取网格的几何精度，同时不牺牲渲染质量，还能减少顶点和面的数量。

Conclusion: GSPlane通过平面先验的引入，有效解决了高斯泼溅在平面重建中的精度问题，为场景编辑和物理模拟提供了更好的结构化表示。
...
TL;DR: 提出了一种针对预训练扩散模型的优化策略，通过潜在细化管道和双向交互机制，在保持感知真实性的同时显著提升内容保真度，特别是在低光照场景下。


<details>
  <summary>Details</summary>
Motivation: 预训练扩散方法在低层视觉任务中表现出色，但往往以牺牲内容保真度为代价来获得更高的感知真实性，这个问题在低光照场景下尤为严重。

Method: 引入潜在细化管道来恢复VAE编码过程中丢失的空间细节，并建立条件潜在与噪声潜在之间的动态双向交互机制。

Result: 大量实验表明该方法显著提升了预训练扩散方法的保真度性能。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种在消费级相机拍摄的视频中嵌入不可见水印的技术，以支持隐私保护和内容验证，同时不影响人眼视觉体验。

Method: 采用光谱调制而非强度调制，联合考虑人眼视觉系统敏感性、相机传感器光谱灵敏度和LED生成白光的特性，优化LED光谱轮廓。
...
TL;DR: 提出了GOOD框架，通过双层级指导直接引导扩散采样轨迹生成OOD样本，提升OOD检测性能


<details>
  <summary>Details</summary>
Motivation: 现有方法通过扰动文本条件嵌入生成OOD样本，存在语义不稳定和偏移多样性不足的问题，限制了在真实OOD场景的泛化能力

Method: 使用现成的ID分类器，在扩散采样过程中加入双重指导：图像级指导基于对数分割梯度降低输入似然，特征级指导基于k-NN距离促进特征稀疏区域采样
...
TL;DR: KineDiff3D是一个统一的框架，通过运动学感知扩散模型从单视图输入重建铰接物体形状并估计姿态。


<details>
  <summary>Details</summary>
Motivation: 铰接物体（如笔记本电脑和抽屉）由于其多部件几何结构和可变的关节配置，在3D重建和姿态估计方面存在显著挑战，这些因素在不同状态下引入了结构多样性。
...
Method: 两阶段框架：监督微调阶段使用RGB图像、深度图和法线图作为多通道输入；强化学习阶段引入组长度奖励来鼓励简洁的建模序列。

Result: 在DeepCAD和Fusion360数据集上达到最先进性能，在代码有效性、几何精度和建模简洁性方面优于现有方法。

Conclusion: GACO-CAD通过几何先验和简洁建模奖励，有效提升了从单图像生成可编辑CAD模型的准确性和效率。
...
TL;DR: 本文研究了人脸识别系统中预处理对对抗攻击迁移性的影响，发现人脸检测模型选择会显著降低攻击成功率，并提出预处理不变方法提升攻击迁移性。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统容易受到对抗样本攻击，但现有研究往往忽略了预处理环节在黑盒攻击中的作用，特别是人脸检测和降采样等预处理步骤对攻击效果的影响。

Method: 研究多种现成的最先进对抗攻击方法在不同预处理技术下的迁移性，分析人脸检测模型和插值方法的影响，并提出基于输入变换的预处理不变方法。

Result: 人脸检测模型选择会使攻击成功率降低高达78%，而降采样插值方法影响相对较小。提出的预处理不变方法可将攻击迁移性提升高达27%。
...
TL;DR: VisiPruner是一种无需训练的剪枝框架，通过分析MLLMs的三阶段跨模态交互过程，显著减少视觉相关注意力计算和FLOPs，在LLaVA-v1.5 7B上分别减少99%和53.9%。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现优异，但由于注意力计算的二次增长导致计算开销巨大。现有剪枝方法缺乏对MLLMs如何处理和融合多模态信息的根本理解。

Method: 通过系统分析发现MLLMs存在三阶段跨模态交互过程：浅层识别任务意图、中层关键视觉令牌驱动融合、深层丢弃视觉令牌专注语言精炼。基于此提出VisiPruner训练无关剪枝框架。

Result: VisiPruner在LLaVA-v1.5 7B上减少99%视觉相关注意力计算和53.9% FLOPs，显著优于现有令牌剪枝方法，并能泛化到多种MLLMs。

Conclusion: 研究不仅提供了高效的剪枝方法，还为训练高效MLLMs提供了可操作的指导原则，即让模型架构与其内在的层级处理动态对齐。
...
TL;DR: 提出GtR训练免费分层采样策略，通过结构生成和细节重建两阶段分解图像生成，实现MAR模型3.72倍加速且保持生成质量


<details>
  <summary>Details</summary>
Motivation: 解决MAR模型在视觉生成中并行生成潜力受限于空间相关视觉标记建模复杂性的问题

Method: GtR两阶段策略：结构生成建立全局语义框架 + 细节重建高效补全剩余标记；结合FTS频率加权标记选择，为图像细节分配更多计算资源

Result: 在ImageNet类条件和文本到图像生成任务中实现3.72倍加速，FID保持1.59，IS从299.1提升至304.4，显著优于现有加速方法

Conclusion: GtR通过分层采样策略有效平衡生成质量与效率，为MAR模型提供了实用的加速解决方案
...
TL;DR: 本文针对浮游生物识别中的分布偏移问题，首次在浮游生物识别领域进行了大规模、系统的OoD检测方法评估，发现ViM方法在构建的基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 浮游生物识别模型在实际部署中面临分布偏移挑战，由于浮游生物形态复杂、物种多样性大且不断发现新物种，导致推理时出现不可预测错误。该领域缺乏最新计算机视觉发展的系统整合和大规模评估的统一基准。

Method: 基于DYB-PlanktonNet数据集精心设计了一系列模拟不同分布偏移场景的OoD基准测试，系统评估了22种OoD检测方法。

Result: 大量实验结果表明，ViM方法在构建的基准测试中显著优于其他方法，特别是在Far-OoD场景中关键指标有显著提升。

Conclusion: 这项全面评估不仅为自动化浮游生物识别中的算法选择提供了可靠参考，也为浮游生物OoD检测的未来研究奠定了坚实基础。
...
TL;DR: 提出了一种联合学习详细头部化身和手-脸交互引起的非刚性形变的新框架，解决了现有方法忽略自然手-脸交互的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注面部区域，忽略了传达认知状态的自然手-脸交互（如手托下巴、手指轻触脸颊等），这些交互对于逼真的3D头部化身至关重要。

Method: 结合深度顺序损失和接触正则化进行姿态跟踪；从手-脸交互数据集中学习手引起面部形变的PCA基；引入接触损失减少穿透伪影并增强物理合理性。

Result: 在iPhone拍摄的RGB(D)视频上评估，并构建合成数据集。相比最先进的表面重建方法，能捕捉更好的外观和更准确的面部形变几何。

Conclusion: 该方法能够有效捕捉手-脸交互引起的面部形变，生成更逼真和物理合理的3D头部化身。
...
TL;DR: HIDISC是一个双曲表示学习框架，用于解决域泛化与广义类别发现问题，无需模拟训练且计算效率高，在多个基准数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法假设训练和测试数据来自相同域，限制了在开放世界场景中的适用性。DG-GCD要求模型泛化到包含新类别的未见域，但现有方法DG2CD-Net计算成本高且存在误差累积。

Method: 使用GPT引导的扩散增强源域数据；提出切线CutMix进行曲率感知插值；结合惩罚Busemann对齐、混合双曲对比正则化和自适应离群排斥的统一损失；可学习曲率参数适应数据集复杂度。

Result: 在PACS、Office-Home和DomainNet数据集上均达到最先进结果，一致优于现有的欧几里得和双曲(DG)-GCD基线方法。

Conclusion: HIDISC通过双曲表示学习和创新的数据增强技术，有效解决了域泛化与广义类别发现问题，在保持高效的同时实现了优异的泛化性能。
...
Method: 分层方法：先选择任务相关的核心视觉token，再补充多样性token以保留更广泛上下文，采用提示感知的视角

Result: 在多个模型和基准测试中，即使剪枝90%的token，性能也能达到或超越最先进方法，同时显著减少GPU内存占用和推理延迟

Conclusion: 提出的提示感知视觉token剪枝方法在保持性能的同时有效降低计算成本，为大规模视觉语言模型部署提供了实用解决方案
...
TL;DR: UltraCUA是一个基础模型，通过混合动作将GUI原语操作与高级程序化工具调用无缝集成，显著提升了计算机使用代理的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 当前计算机使用代理仅依赖点击、输入、滚动等原始操作，需要准确的视觉定位和冗长的执行链，导致级联故障和性能瓶颈。这些代理与其他利用丰富程序化接口的代理隔离，无法利用API、MCP服务器等工具。

Method: 采用四个关键组件：(1) 从软件文档、开源仓库和代码生成中扩展程序化工具的自动化管道；(2) 生成超过17,000个可验证任务的合成数据引擎；(3) 大规模高质量混合动作轨迹收集；(4) 结合监督微调和在线强化学习的两阶段训练管道。

Result: 7B和32B模型实验显示显著改进：在OSWorld上相对基础模型平均提升22%，步骤减少11%；在WindowsAgentArena上达到21.7%成功率，优于在Windows数据上训练的基线模型。

Conclusion: 混合动作机制至关重要，减少了错误传播同时保持了执行效率，成功弥合了计算机使用代理与其他程序化工具代理之间的能力差距。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉国的河流每年吞噬村庄和农田，造成大规模社区消失和人口流离失所，但传统方法难以精确追踪这种缓慢的灾难过程。

Method: 构建了包含2003-2025年历史Google Earth影像的新数据集，结合简单颜色通道分析和微调SAM掩码解码器来识别河岸侵蚀特征。

Result: 模型在河流侵蚀检测上表现出色，平均IoU达到86.30%，Dice分数达到92.60%，显著优于传统方法和现成的深度学习模型。

Conclusion: 该研究提供了首个消失定居点标注数据集、专门优化的AI模型和土地流失量化方法，为政策制定者和灾害管理机构提供了监测侵蚀和保护脆弱社区的新工具。
...
Result: 在多个长上下文基准测试中达到与Qwen3-8B相当的准确性，同时实现3-4倍token压缩、4倍推理加速和2倍训练加速。

Conclusion: 视觉上下文扩展是解决长上下文建模挑战的有效方法，能够在极端压缩下处理百万级token任务，并有益于多模态应用。
...
TL;DR: 基于TimeSformer视频识别模型，通过分析VALORANT游戏小地图中的战术特征（角色位置和游戏事件）来预测回合结果，在添加战术事件标签的数据集上获得了约81%的预测准确率。
...
Result: 在添加战术事件标签的数据集上训练的模型达到了约81%的预测准确率，特别是在回合中后期阶段，表现显著优于仅基于小地图信息本身训练的模型。
...
TL;DR: EndoCIL是一个专门为内窥镜图像诊断设计的类增量学习框架，通过三个关键组件有效缓解灾难性遗忘问题，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 内窥镜图像分析需要模型能够持续适应新的临床数据，同时保持对已学习类别的性能。现有基于回放的CIL方法由于内窥镜图像中严重的领域差异和类别不平衡问题，无法有效缓解灾难性遗忘。

Method: EndoCIL包含三个核心组件：基于最大均值差异的回放(MDBR)选择多样且具代表性的样本；先验正则化类别平衡损失(PRCBL)缓解阶段间和阶段内类别不平衡；全连接梯度校准(CFG)调整分类器梯度以减少对新类别的偏置。

Result: 在四个公共内窥镜数据集上的广泛实验表明，EndoCIL在不同缓冲区大小和评估指标下普遍优于最先进的CIL方法。

Conclusion: 该框架在终身内窥镜诊断中有效平衡了稳定性和可塑性，展示了临床可扩展性和部署的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 面部识别系统容易受到欺骗攻击，恶意攻击者可能使用注册用户的照片绕过认证过程，因此需要在面部识别之前检测此类欺骗攻击。

Method: 使用带寄存器的DINOv2模型提取可泛化特征，抑制注意力机制中的扰动，从而聚焦于关键和细微的特征差异。

Result: 通过在ICCV2025第6届面部反欺骗研讨会提供的数据集和SiW数据集上的实验验证了所提方法的有效性。

Conclusion: 基于DINOv2的欺骗攻击检测方法能够有效区分真实和欺骗面部图像，为面部识别系统提供可靠的安全保障。
...
TL;DR: 该论文提出了多时刻检索（MMR）任务，构建了QV-M^2数据集和FlashMMR框架，通过后验证模块优化时刻边界，在多个指标上显著提升性能。
...
Conclusion: QV-M^2数据集和FlashMMR方法为推进更现实和挑战性的视频时序定位研究奠定了基础。
...
TL;DR: 提出公平性深度伪造检测框架，集成时序特征学习和人口统计感知数据增强，提升公平性和可解释性，在多个数据集上实现公平性与准确性的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法存在偏见、缺乏透明度、无法捕捉时序信息，导致跨人口统计群体的决策偏见和不可靠结果。

Method: 使用时序特征学习的序列聚类进行深度伪造视频建模，结合概念提取提升检测可靠性；引入人口统计感知数据增强方法，平衡代表性不足群体并应用频域变换保留深度伪造伪影。

Result: 在FaceForensics++、DFD、Celeb-DF和DFDC数据集上使用Xception、ResNet等先进架构进行广泛实验，证明所提方法在公平性与准确性之间达到最佳平衡。

Conclusion: 该框架通过时序建模和公平性增强技术，有效解决了深度伪造检测中的偏见问题，同时保持了检测性能，为公平可靠的深度伪造检测提供了可行方案。
...
TL;DR: FineVision是一个精心收集、整理和统一的2400万样本语料库，是同类中最大的开放资源，通过半自动化人工监督流程整合了200多个数据源，显著提升了视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的发展受到碎片化、不一致和受污染的公共数据集的阻碍，需要构建一个高质量、统一的大规模数据集。

Method: 采用半自动化人工监督流程：自动化进行批量摄取和模式映射，人工审核员审核映射并抽查输出，验证注释的忠实消费、适当格式和多样性、安全性；问题触发针对性修复和重新运行。工作流还应用严格的去重和去污染处理。

Result: 在FineVision上训练的模型在广泛的评估套件中持续优于现有开放混合数据集训练的模型，证明了规模、数据卫生以及平衡自动化与人工监督的好处。

Conclusion: FineVision语料库和整理工具的发布将加速以数据为中心的视觉语言模型研究，强调了高质量数据集对模型性能的重要性。
...
TL;DR: PnF是一种即插即用的方法，使用多模态大语言模型增强现有运动预测模型，通过自然语言描述复杂场景实现快速适应，无需微调即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统在标准条件下表现可靠，但难以经济高效地泛化到多样化现实场景。自然语言能更有效地描述和处理复杂场景，实现快速行为适应。
...
Conclusion: PnF方法通过MLLMs增强现有模型，实现了无需微调的性能改进，为自动驾驶系统提供了一种实用的泛化解决方案。
...
TL;DR: 提出SG-CLDFF框架，通过显著性引导和跨层深度特征融合来改进白细胞图像的分割和分类，在多个基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 白细胞图像分析在血液疾病诊断中至关重要，但由于染色变异、复杂背景和类别不平衡等问题，准确分割和分类仍然具有挑战性。

Method: SG-CLDFF框架结合显著性驱动的预处理与多尺度深度特征聚合，使用EfficientSwin风格混合骨干网络生成多分辨率表示，通过ResNeXt-CC启发的跨层融合模块整合浅层和深层特征，采用多任务训练和类别感知加权损失。

Result: 在BCCD、LISC、ALL-IDB等标准基准测试中，IoU、F1分数和分类准确率均优于CNN和transformer基线方法，消融研究验证了显著性预处理和跨层融合的独立贡献。

Conclusion: SG-CLDFF为临床工作流程中的自动化白细胞分析提供了一条实用且可解释的路径，提高了分析的可靠性和可解释性。
...
Method: 使用YOLOv11算法检测手术部位上方的蓝色标记，通过两个伺服电机控制LED光源的倾斜和平移运动，实现自动照明定位。

Result: YOLO模型在验证集上达到96.7% mAP@50的检测精度，验证集包含带有蓝色球形标记的模拟手术场景标注图像。

Conclusion: 该基于机器视觉的自动化照明系统能有效减轻外科医生身体负担，提高照明一致性，有助于改善手术效果。
...
TL;DR: 本文发现自监督学习中的反直觉现象：训练时间过长会损害密集预测任务的性能，称为自监督密集退化(SDD)。作者提出了密集表示结构估计器(DSE)来评估无标注下的密集性能，并基于此开发了模型选择策略和正则化方法。


<details>
  <summary>Details</summary>
Motivation: 观察到自监督学习中训练时间过长反而会降低密集预测任务性能的反常现象，需要解决无标注情况下评估密集性能的挑战。

Method: 提出密集表示结构估计器(DSE)，包含类相关性度量和有效维度度量，并基于DSE开发模型选择策略和正则化方法。

Result: 在16种自监督方法和4个基准测试上的实验表明，模型选择平均提升mIoU 3.0%，DSE正则化能持续缓解密集退化效应。

Conclusion: 自监督密集退化现象普遍存在，提出的DSE方法能有效评估密集性能并缓解退化问题，为自监督学习提供了实用的模型选择和正则化工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在长视频理解和多模态整合方面存在不足，需要专门评估模型在信息密集的长视频中对复杂上下文元素的理解能力。

Method: 从FineVideo数据集中精选约1000个长时信息密集视频，设计6个挑战性任务场景（包括事件内和事件间任务），并开发三步半自动化数据质量保证流程。

Result: 实验表明全模态模型在精确时间定位和长距离因果推理任务中仍面临挑战，扩展实验揭示了多模态融合中的信息丢失和处理偏差问题。

Conclusion: LongInsightBench为长视频理解提供了首个综合性基准测试，揭示了当前全模态模型在复杂多模态任务中的局限性，为未来研究指明了方向。
...
TL;DR: CausalMamba是一个可扩展的fMRI因果推断框架，通过BOLD去卷积和条件Mamba架构解决神经因果推断的逆问题和计算复杂性，在模拟数据上比DCM准确率高37%，在真实数据中能恢复88%的已知神经通路。


<details>
  <summary>Details</summary>
Motivation: 解决fMRI基于BOLD信号的神经因果推断中的两个基本限制：血氧动力学扭曲导致的逆问题不适定性，以及现有方法（如动态因果建模DCM）的计算不可行性。

Method: 将复杂的逆问题分解为两个可处理的阶段：BOLD去卷积恢复潜在神经活动，然后使用新颖的条件Mamba架构进行因果图推断。
...
TL;DR: 本文评估了现有对抗性防御方法对抗大尺寸对抗衣物攻击的效果，发现这些防御方法在数字和物理世界中都表现不佳，揭示了现有防御方法的共同脆弱性。


<details>
  <summary>Details</summary>
Motivation: 实验发现简单增大对抗补丁尺寸就能使现有防御方法失效，因此评估各种防御方法对抗覆盖人体大面积的自然对抗衣物的效果。

Method: 通过制作对抗衣物来测试多种防御方法，在数字世界和物理世界中进行实验评估，特别针对Faster R-CNN制作了能突破多种防御的单套衣物。

Result: 所有防御方法对抗对抗衣物都表现不佳，制作的一套衣物在物理世界中对抗未防御检测器达到96.06%的攻击成功率，对抗九个防御模型都超过64.84%的攻击成功率。

Conclusion: 现有对抗性防御方法在面对大尺寸、自然的对抗衣物攻击时存在共同脆弱性，需要开发更有效的防御策略。
...
Method: 使用基于扩散的框架，通过外部分割和OCR模块提取细粒度字符级先验，并引入CHARM模块确保每个字符的引导仅限于自身区域，避免区域间干扰。

Result: 在Roboflow-LP数据集上，相比最佳基线模型，实现了28%的相对CER降低，在恢复质量和识别准确率上显著优于基线恢复模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决图像质量评估从标量质量预测向更可解释、与人感知对齐的评估范式发展的挑战。
...
Method: 基于输入到最近类均值的距离构建概率分布，然后将该分布与softmax概率进行比较，测量NCM和分类头之间的一致性。

Result: 在两个数据集上均表现优异，AUROC分别达到93.41和95.35，在两个数据集上表现一致，而现有方法通常只在单一数据集上表现良好。

Conclusion: 提出的后处理OSR方法在无需重新训练的情况下实现了稳定的开放集识别性能，在两个数据集上均排名前三。
...
TL;DR: 提出Semantic-E2VID框架，通过跨模态特征对齐和语义感知特征融合，将视觉语义知识从图像模态迁移到事件模态，显著提升事件到视频重建的质量和语义信息恢复能力。
...
Method: 1. 跨模态特征对齐模块将SAM模型的视觉语义迁移到事件编码器；2. 语义感知特征融合块整合学习到的语义特征；3. 语义感知E2V监督利用SAM生成的类别标签辅助重建语义细节。

Result: 在多个基准测试中显著提升帧质量，优于最先进的E2V方法。

Conclusion: Semantic-E2VID通过有效利用视觉语义知识，成功解决了事件到视频重建中语义信息缺失的问题，实现了更高质量的重建效果。
...
TL;DR: M2H是一个新颖的多任务学习框架，用于从单目图像进行语义分割、深度、边缘和表面法线估计。它采用基于窗口的跨任务注意力模块，在轻量级ViT骨干网络上实现高效的多任务特征交换，适用于边缘设备的实时部署。
...
Method: 提出Window-Based Cross-Task Attention Module，实现结构化特征交换同时保留任务特定细节。基于轻量级ViT-based DINOv2骨干网络构建，支持实时部署。

Result: 在NYUDv2数据集上超越最先进的多任务模型，在Hypersim上超越单任务深度和语义基线，在Cityscapes上表现优异，同时在笔记本电脑硬件上保持计算效率。
...
TL;DR: 提出一种无需训练的方法，使视频大语言模型能够处理流式长视频，通过注意力机制筛选重要视觉token、循环处理历史token和基于字幕的问答，在保持性能的同时大幅提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型需要完整访问视频才能回答问题，无法适应流式处理场景，特别是处理小时级长视频时需要及时响应的挑战。

Method: 1) 基于LLM注意力机制筛选重要视觉token，可丢弃约95%不重要token；2) 循环处理历史选定token以保持时序连贯性；3) 基于字幕的轻量级问答机制。

Result: 在流式视频基准测试中达到最先进性能，在效率和效果之间取得良好平衡。

Conclusion: 该方法为视频大语言模型在流式处理场景中的应用提供了有效的解决方案，显著提升了处理长视频的效率。
...
TL;DR: 该研究通过系统评估25个合成人脸识别数据集，证明合成人脸数据在保持95.67%高准确率的同时，能有效解决真实数据集的隐私和伦理问题，为面部识别研究提供了可行的隐私保护替代方案。


<details>
  <summary>Details</summary>
Motivation: 真实人脸数据集存在未经同意收集数据的隐私和伦理问题，导致数据集撤回和法律风险，而合成人脸数据作为隐私保护替代方案缺乏全面的实证证据。

Method: 采用系统文献回顾识别25个合成人脸识别数据集，结合实验验证七个关键隐私保护要求：身份泄露预防、类内变异性、身份可分离性、数据集规模、伦理数据来源、偏见缓解和基准可靠性。

Result: 最佳合成数据集VariFace和VIGFace分别达到95.67%和94.91%的识别准确率，超过真实数据集CASIA-WebFace(94.70%)。合成数据在保持类内变异性的同时确保身份可分离性，且提供前所未有的偏见控制能力。

Conclusion: 合成人脸数据在科学上是可行的，在伦理上是必要的面部识别研究替代方案，能够有效平衡准确性与隐私保护需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于面部表情的PD诊断方法依赖单一表情类型易导致误诊，且忽略不同PD阶段类别不平衡问题，大多方法仅进行二元分类而非严重程度诊断。

Method: 集成多种面部表情特征通过注意力机制进行特征融合，采用自适应类别平衡策略动态调整训练样本贡献度。

Result: 实验结果表明该方法在PD严重程度诊断方面表现良好，注意力特征融合和自适应类别平衡策略有效。

Conclusion: 所提出的方法能够有效诊断帕金森病严重程度，多表情特征融合和类别平衡策略显著提升诊断性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法仅从外中心图像单向转移知识到自我中心图像，限制了在复杂交互场景中的适用性。人类能够通过观察他人与物体的交互来学习新物体的使用方式，这启发了双向知识转移的需求。

Method: 提出LoopTrans闭环框架，包含统一跨模态定位和去噪知识蒸馏机制，以弥合物体中心的自我中心图像和交互中心的外中心图像之间的领域差距，同时增强知识转移。

Result: 实验表明，LoopTrans在图像和视频基准测试的所有指标上均取得一致改进，甚至能够处理人类身体完全遮挡物体交互区域的挑战性场景。

Conclusion: LoopTrans通过双向知识转移和创新的跨模态机制，显著提升了交互能力学习的性能，特别是在复杂遮挡场景下表现出色。
...
TL;DR: 开发了一个基于视觉的马厩监控系统，使用YOLOv11和BoT-SORT自动检测和跟踪马匹与人，通过物体轨迹和空间关系推断五种事件类型。
...
Method: 结合YOLOv11目标检测和BoT-SORT多目标跟踪技术，利用CLIP和GroundingDINO构建自定义数据集，基于物体轨迹和空间关系推断事件状态。
...
TL;DR: DeepDetect是一个智能、一体化、密集的关键点检测器，通过融合传统检测器的优势，使用深度学习生成高密度、可重复性强的关键点。


<details>
  <summary>Details</summary>
Motivation: 传统关键点检测器对光度变化敏感、关键点密度低、可重复性差、对挑战性场景适应性有限，且缺乏语义理解能力。

Method: 融合7个关键点检测器和2个边缘检测器的输出创建真实掩码，然后使用轻量级ESPNet模型进行训练，使DeepDetect能够语义化地关注图像并产生高密度关键点。

Result: 在Oxford Affine Covariant Regions数据集上，DeepDetect在关键点密度、可重复性和正确匹配数量方面均优于其他检测器，分别达到0.5143、0.9582和59,003的最大值。

Conclusion: DeepDetect通过融合传统检测器的多样视觉线索和深度学习，实现了对挑战性场景的更好适应性，并产生了语义上更重要的密集关键点。
...
TL;DR: 利用AV1运动向量生成密集亚像素对应点和经过余弦一致性过滤的短轨迹，在压缩域前端实现与SIFT相当的性能但CPU使用更少，在SfM应用中表现出实用性


<details>
  <summary>Details</summary>
Motivation: 探索压缩域对应点作为资源高效的前端解决方案，为完整流程扩展提供清晰路径

Method: 重新利用AV1运动向量生成密集亚像素对应点，通过余弦一致性过滤短轨迹

Result: 在117帧视频上，运动向量匹配注册所有图像并重建46-62万个点，重投影误差0.51-0.53像素；BA时间随匹配密度增加

Conclusion: 压缩域对应点是实用且资源高效的前端方案，具有在完整流程中扩展的明确路径
...
TL;DR: 提出了一个新的高质量夜间图像去雨基准数据集HQ-NightRain，并开发了颜色空间转换网络CST-Net，通过可学习的颜色空间转换器和隐式光照引导来有效去除夜间复杂雨纹。


<details>
  <summary>Details</summary>
Motivation: 夜间图像去雨相比白天更具挑战性，主要由于夜间场景的固有复杂性以及缺乏准确表征雨和光照耦合效应的高质量数据集。

Method: 开发CST-Net网络，包含可学习的颜色空间转换器(CSC)在Y通道进行雨纹去除，并引入隐式光照引导来捕获光照信息提高模型在复杂场景中的鲁棒性。

Result: 大量实验证明了所提出数据集的价值和方法的有效性。

Conclusion: 提出的HQ-NightRain数据集具有更高的和谐性和真实性，CST-Net方法在夜间图像去雨任务中表现出色。
...
TL;DR: 本文发现稀疏视图3D高斯溅射中的初始化是决定性因素，提出了基于SfM的改进初始化方法，包括频率感知SfM、3DGS自初始化和点云正则化，在稀疏视图场景下取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D高斯溅射容易过拟合训练视图，导致新视角渲染出现模糊等伪影。现有方法要么增强初始化，要么添加训练时约束，但作者通过控制实验发现初始化才是决定性因素。
...
Conclusion: 初始化是稀疏视图3D高斯溅射的关键因素，提出的基于SfM的改进初始化方法能够显著提升稀疏视图场景下的渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有占用世界模型依赖静态固定的嵌入或网格，限制了感知的灵活性，且其"原地分类"方法与真实场景的动态连续特性存在潜在不匹配。

Method: 提出Range-Adaptive Perception模块（可学习查询通过自车状态调制并增强时空关联），State-Conditioned Forecasting模块（用回归引导的预测替代基于分类的预测），以及Temporal-Aware Self-Scheduling训练策略。

Result: 在感知、预测和规划任务中达到最先进性能，验证了模型在灵活性、适应性和效率方面的优势。

Conclusion: SparseWorld通过稀疏动态查询成功构建了灵活、自适应且高效的4D占用世界模型，为动态连续场景的感知与预测提供了有效解决方案。
...
TL;DR: AutoSOD是一种端到端的无监督显著目标检测方法，通过POTNet生成高质量的伪掩码来训练MaskFormer风格的编码器-解码器，无需像素级标注即可达到接近监督方法的精度。


<details>
  <summary>Details</summary>
Motivation: 显著目标检测是计算机视觉的基础任务，作者认为在获得可靠伪掩码的情况下，无监督方法可以达到接近监督方法的精度。现有原型方法存在两个问题：边界像素和内部像素的几何特性不同，以及当原型质量较弱时最优传输的全局一致性未被充分利用。

Method: 提出POTNet，改进原型最优传输方法，用熵引导的双聚类头替代单一k-means步骤：高熵像素通过谱聚类组织，低熵像素通过k-means聚类，然后通过最优传输对齐两个原型集。生成的伪掩码用于训练标准的MaskFormer风格编码器-解码器。

Result: 在五个基准测试上的实验表明，AutoSOD在F-measure指标上比无监督方法提升高达26%，比弱监督方法提升高达36%，进一步缩小了与完全监督模型的差距。

Conclusion: AutoSOD通过改进的原型最优传输方法生成高质量的伪掩码，实现了端到端的无监督显著目标检测，在精度和训练效率上都优于现有方法，无需手工先验或离线投票过程。
...
TL;DR: 提出了一种基于评分标准的伪标签提示框架，通过将少量真实标注转化为高置信度伪标签，构建结构化评分标准来指导可解释的场景评估，实现无需参数调整的零样本视频摘要。


<details>
  <summary>Details</summary>
Motivation: 现有监督方法标注成本高且跨数据集泛化能力有限，无监督方法难以捕捉高层次语义和细粒度叙事线索，零样本提示方法对人工提示模板敏感且依赖数据集特定的分数归一化。
...
Method: 优化四个关键支柱：数据处理、模型架构、训练策略和基础设施，涵盖数据预处理、视频压缩、参数缩放、课程式预训练和对齐后训练。

Result: MUG-V 10B模型在整体性能上达到最新水平，在电商视频生成任务中超越领先开源基线，并实现了高训练效率和近线性多节点扩展。

Conclusion: 成功开发了高效的大规模视频生成训练框架，为社区提供了完整的开源解决方案，包括模型权重、训练代码和推理管道。
...
TL;DR: 提出MambaX-Net，一种用于前列腺癌主动监测的半监督双扫描3D分割架构，通过利用先前时间点的MRI和分割掩码来改进当前时间点的分割精度。
...
Method: 使用Mamba增强的交叉注意力模块捕获时间演化和长距离空间依赖，形状提取器模块编码先前分割掩码为潜在解剖表示，并采用半监督自训练策略利用nnU-Net生成的伪标签。

Result: 在纵向主动监测数据集上的评估显示，MambaX-Net显著优于最先进的U-Net和Transformer模型，即使在有限和噪声数据下也能实现优越的前列腺区域分割。

Conclusion: MambaX-Net为前列腺癌主动监测提供了一种有效的半监督分割解决方案，能够处理纵向数据和标签稀缺的挑战。
...
TL;DR: WP-CrackNet是一种弱监督的道路裂缝检测方法，仅使用图像级标签即可实现像素级检测，通过分类器、重建器和检测器的协同训练，结合路径感知注意力模块和中心增强一致性模块，在减少标注成本的同时达到接近全监督方法的性能。


<details>
  <summary>Details</summary>
Motivation: 为了降低智能基础设施维护中对昂贵像素级标注的依赖，提出仅需图像级标签的弱监督方法来实现像素级道路裂缝检测，提高检测的可扩展性。

Method: 提出WP-CrackNet框架，包含三个组件：分类器生成类激活图、重建器测量特征可推断性、检测器输出像素级结果。通过对抗学习使分类器和重建器交替训练，检测器从后处理的伪标签中学习。设计了路径感知注意力模块融合高低层特征，以及中心增强一致性模块优化类激活图。

Result: 在三个自建数据集上的大量实验表明，WP-CrackNet达到了与全监督方法相当的结果，并优于现有的弱监督方法，显著提升了道路检测的可扩展性。

Conclusion: WP-CrackNet通过弱监督学习有效减少了标注成本，在道路裂缝检测任务上表现出色，为大规模道路巡检提供了可行的解决方案。
...
TL;DR: PAGE-4D是一个前馈模型，将VGGT扩展到动态场景，解决了静态模型在处理动态元素时的局限性，通过动态感知聚合器分离静态和动态信息，实现相机姿态估计、深度预测和点云重建。


<details>
  <summary>Details</summary>
Motivation: 现有的3D前馈模型（如VGGT）在静态数据集上训练，难以处理真实世界中包含复杂动态元素（如移动的人或可变形物体）的场景。

Method: 提出动态感知聚合器，通过预测动态感知掩码来分离静态和动态信息——在姿态估计中抑制运动线索，在几何重建中增强它们。

Result: 在动态场景中，PAGE-4D始终优于原始VGGT，在相机姿态估计、单目和视频深度估计以及密集点图重建方面取得了更优结果。

Conclusion: PAGE-4D成功解决了多任务4D重建中的任务冲突问题，通过动态感知方法在动态场景中实现了更好的性能。
...
Method: 提出UCIS-SAM网络，包含三个模块：通道平衡优化模块(CBOM)增强水下特征学习，频域真值整合模块(FDTIM)强调内在物体特征并减少伪装模式干扰，多尺度特征频率聚合模块(MFFAM)增强低对比度伪装实例的边界。

Result: 在提出的UCIS4K数据集和公共基准测试上的广泛实验表明，UCIS-SAM优于最先进的方法。

Conclusion: UCIS-SAM网络通过专门设计的水下特征增强和伪装模式处理模块，有效解决了水下伪装实例分割的挑战，为水下视觉任务提供了有力工具。
...
TL;DR: ShapeCraft是一个基于多智能体的文本到3D生成框架，通过图形化程序形状表示将复杂自然语言分解为结构化子任务，生成结构化、可交互的3D资产。
...
Method: 提出基于图形的程序形状(GPS)表示，使用LLM智能体分层解析用户输入，迭代优化程序建模和绘制过程。

Result: 在定性和定量实验中表现出色，生成几何精确且语义丰富的3D资产，支持动画和用户自定义编辑。
...
<details>
  <summary>Details</summary>
Motivation: 无人机技术结合摄影测量能够高效采集基础设施的高分辨率3D模型，但传统手动分割结构组件的方法耗时且易出错，需要自动化解决方案。

Method: 使用机器学习框架，结合真实无人机扫描点云和BIM生成的合成数据，利用两种数据的互补优势来克服手动标注的限制。

Result: 在铁路轨道数据集上的验证显示，该方法能高精度识别和分割主要组件（如铁轨和枕木），使用小规模数据集结合BIM数据显著减少训练时间同时保持合理分割精度。
...
TL;DR: Dinomaly2是一个统一的无监督异常检测框架，通过五个简单元素的组合在标准重建框架中实现卓越性能，在多个模态和任务设置中展现全谱系优势。


<details>
  <summary>Details</summary>
Motivation: 解决现有多类异常检测模型性能不足的问题，并统一专门化方法，消除部署障碍，为真实世界异常检测应用提供统一解决方案。

Method: 基于"少即是多"理念，在标准重建框架中协调五个简单元素，无需修改即可自然扩展到多样化任务。

Result: 在12个UAD基准测试中表现优异：多类模型在MVTec-AD和VisA上分别达到99.9%和99.3%的I-AUROC；仅用每类8个正常样本就超越之前全样本模型；在多视图和多模态检测中达到最先进性能。
...
TL;DR: CaMiT数据集包含787K标注样本和5.1M未标注样本，涵盖190种汽车模型(2007-2023)，用于研究视觉系统的时序适应能力。研究发现静态预训练在跨年测试中性能下降，提出了时间增量分类设置和两种改进策略，以及时间感知图像生成方法。


<details>
  <summary>Details</summary>
Motivation: AI系统需要适应不断变化的视觉环境，特别是在物体外观随时间演变的领域。汽车模型作为技术制品的典型代表，其外观随时间变化，需要研究如何使视觉系统保持对这类对象的识别能力。

Method: 1) 构建CaMiT数据集；2) 评估静态预训练在跨年测试中的性能；3) 提出时间增量分类设置，包含新类出现、现有类演变和旧类消失的场景；4) 评估两种策略：时间增量预训练(更新主干网络)和时间增量分类器学习(仅更新最后一层)；5) 探索利用时间元数据的时间感知图像生成方法。

Result: 静态预训练在域内数据上能达到与大规模通用模型竞争的性能且更高效，但在跨年测试中准确率下降。时间增量预训练和时间增量分类器学习都能提高时序鲁棒性。时间感知图像生成能产生更真实的输出。

Conclusion: CaMiT为研究细粒度视觉识别和生成中的时序适应问题提供了丰富的基准，提出的时间增量学习方法能有效应对视觉环境随时间变化带来的挑战。
...
TL;DR: 提出DINO-CV分割框架，利用高分辨率LiDAR DEM自动映射低矮干石墙，通过自监督跨视图预训练解决数据稀缺问题，在植被覆盖区域实现高效识别。


<details>
  <summary>Details</summary>
Motivation: 干石墙具有重要遗产和环境价值，但传统手动映射成本高且难以覆盖植被遮挡区域。深度学习分割虽可扩展，但面临视觉遮挡和标注数据有限两大挑战。

Method: 使用LiDAR DEM克服植被遮挡，提出基于知识蒸馏的自监督跨视图预训练策略，学习多DEM衍生物的视觉和几何不变表示，支持多种视觉骨干网络。

Result: 在Budj Bim UNESCO遗产地测试，mIoU达68.6%，仅用10%标注数据微调后仍保持63.8% mIoU，成功识别澳大利亚最密集的殖民时期干石墙。

Conclusion: 自监督学习结合高分辨率DEM衍生物，可在标注稀缺的植被覆盖遗产环境中实现干石墙自动映射，具有广泛应用潜力。
...
TL;DR: 比较了两种节俭的联邦学习方法进行暴力检测：零样本和联邦微调的视觉语言模型(VLMs)与个性化训练的紧凑3D卷积神经网络(CNN3D)。两种方法准确率均超过90%，CNN3D在ROC AUC和log loss上略优于LoRA微调的VLMs且能耗更低，而VLMs在上下文推理和多模态推理方面更具优势。提出了混合模型框架。


<details>
  <summary>Details</summary>
Motivation: 研究节俭的联邦学习方法，在暴力检测任务中平衡准确率与能源效率，分析环境可持续性权衡，为视频监控提供负责任、资源感知的AI解决方案。

Method: 使用LLaVA-7B和65.8M参数的CNN3D作为代表案例，在非独立同分布设置下评估两种策略：(i)零样本和联邦微调视觉语言模型，(ii)个性化训练紧凑3D卷积神经网络。量化训练和推理过程中的能耗与CO2排放。

Result: 两种方法准确率均超过90%。CNN3D在ROC AUC和log loss上略优于LoRA微调的VLMs，且能耗更低。VLMs在上下文推理和多模态推理方面保持优势。量化了能源消耗和二氧化碳排放数据。

Conclusion: 支持混合模型：轻量级CNN用于常规分类，选择性激活VLM处理复杂或描述性场景。为视频监控中的负责任、资源感知AI提供了可复现的基准框架，可扩展到实时、多模态和生命周期感知系统。
...
Method: 采用双线程系统：预测线程利用历史运动几何信息提取特征并预测未来动态；推理线程通过对齐最新记忆并补偿自运动和动态物体运动，确保对输入帧的及时预测。

Result: 在室内HOI4D数据集和室外SemanticKITTI、nuScenes数据集上的实验表明，该方法有效，特别是在复杂场景中准确预测动态物体方面表现优异。

Conclusion: 4DSegStreamer是一个通用且鲁棒的流式4D全景分割框架，在高FPS条件下优于现有流式感知方法，能够高效处理动态环境中的实时感知需求。
...
TL;DR: 提出了PICABench基准和PICAEval评估协议，系统评估图像编辑的物理真实性，发现现有模型在物理一致性方面仍有很大改进空间。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型主要关注指令完成度，但忽视了编辑操作带来的物理效应（如阴影、反射、物体交互等），这影响了生成的真实性。

Method: 构建PICABench基准，涵盖8个物理子维度（光学、力学、状态转换等）和常见编辑操作；提出PICAEval评估协议，使用VLM作为评判器，结合区域级人工标注和问题；从视频中学习物理知识并构建PICA-100K训练数据集。

Result: 评估主流模型后发现，物理真实性仍然是一个具有挑战性的问题，存在很大的探索空间。

Conclusion: 该研究为从简单内容编辑向物理一致性真实感的发展提供了基础，希望基准和解决方案能推动未来工作的发展。
...
Conclusion: IC-MoE在保持预训练权重结构完整性的同时增强了高层特征表示能力，并在多样化医学图像分割场景中展现出优异的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像行人检索方法存在模态异质性挑战，全局方法忽略细粒度差异，局部方法需要先验信息，且当前方法主要面向英语，限制了在多语言环境中的应用。

Method: 提出Bi-IRRA框架，包含双向隐式关系推理模块（双向预测掩码图像和文本）和多维全局对齐模块，隐式增强跨语言和跨模态的局部关系建模。

Result: 在所有多语言TIPR数据集上取得了新的最先进结果。

Conclusion: Bi-IRRA框架有效解决了多语言文本到图像行人检索中的模态异质性问题，通过隐式关系推理和全局对齐实现了跨语言和跨模态的精确对齐。
...
TL;DR: 提出一种两阶段帕金森病检测方法，通过图像分块和集成学习策略解决数据集不足和未见患者数据鲁棒性问题，在NewHandPD数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于手绘图像的帕金森病早期检测方法存在两大局限：数据集不足和未见患者数据鲁棒性差。

Method: 两阶段检测：第一阶段按绘图类型分类，第二阶段将图像分为2x2块分别提取特征和检测，最后使用集成方法合并各块决策。
...
### [170] [Towards 3D Objectness Learning in an Open World](https://arxiv.org/abs/2510.17686)
*Taichi Liu,Zhenyu Wang,Ruofeng Liu,Guang Wang,Desheng Zhang*

Main category: cs.CV

TL;DR: 提出了OP3Det，一种无需文本提示的开放世界3D检测器，通过结合2D语义先验和3D几何先验，利用跨模态专家混合实现通用3D物体性学习。


<details>
  <summary>Details</summary>
Motivation: 传统封闭集3D检测器难以泛化到开放世界场景，而直接使用3D开放词汇模型又面临词汇扩展和语义重叠问题，因此需要学习通用的3D物体性来检测包括训练中未见物体在内的所有对象。

Method: 引入2D基础模型的强泛化能力，结合2D语义先验和3D几何先验生成类别无关提议；通过点云和RGB图像的跨模态专家混合，动态路由单模态和多模态特征来学习通用3D物体性。

Result: 在广泛实验中，OP3Det显著超越现有开放世界3D检测器达16.0%（AR指标），相比封闭世界3D检测器提升13.5%。

Conclusion: OP3Det在开放世界3D物体检测方面表现出色，证明了结合2D和3D先验以及跨模态特征学习对于实现通用3D物体发现的有效性。
...
### [171] [GAS: Improving Discretization of Diffusion ODEs via Generalized Adversarial Solver](https://arxiv.org/abs/2510.17699)
...
TL;DR: 提出了一种新的扩散模型采样方法——广义对抗求解器，通过简单的ODE求解器参数化和对抗训练，在减少采样步骤的同时保持图像细节质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型虽然生成质量优秀，但采样过程计算成本高昂。现有的蒸馏方法虽然减少了采样步骤，但依赖复杂训练技巧且不能很好地保留细节。

Method: 提出广义求解器参数化ODE采样器，无需额外训练技巧；结合原始蒸馏损失和对抗训练来减少伪影并增强细节保真度。

Result: 在相似资源约束下，该方法相比现有求解器训练方法表现出更优越的性能。

Conclusion: 广义对抗求解器提供了一种简单有效的扩散模型加速采样方案，在保持生成质量的同时显著减少计算成本。
...
### [172] [MT-Video-Bench: A Holistic Video Understanding Benchmark for Evaluating Multimodal LLMs in Multi-Turn Dialogues](https://arxiv.org/abs/2510.17722)
...
<details>
  <summary>Details</summary>
Motivation: 现有的评估基准仅限于单轮问答，忽略了现实场景中多轮对话的复杂性，需要开发更全面的评估方法。

Method: 构建MT-Video-Bench基准，主要评估六个核心能力（感知性和交互性），涵盖987个来自不同领域的多轮对话，并与实际应用（如交互式体育分析和视频智能辅导）严格对齐。

Result: 对各种最先进的开源和闭源MLLMs进行了广泛评估，揭示了它们在处理多轮视频对话时存在的显著性能差异和局限性。
...
TL;DR: SnapViT是一种后预训练结构化剪枝方法，可在不同计算预算下实现弹性推理，无需重训练或标签数据，5分钟内生成可调节模型。
...
Result: 在DINO、SigLIPv2、DeIT和AugReg模型上优于现有方法，各种稀疏度下表现优异。

Conclusion: SnapViT为预训练视觉Transformer提供高效剪枝策略，实现弹性推理且无需重训练。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在签名验证方面取得进展，但现有方法在跨数据集泛化方面仍存在困难，因为笔迹风格和采集协议的差异会降低性能。

Method: 使用CEDAR、ICDAR和GPDS Synthetic三个公共基准数据集，开发了两种实验流程：基于原始签名图像的方法和采用壳预处理的方法。

Result: 原始图像模型在基准测试中取得了更高的性能，而基于壳预处理的方法显示出未来改进的潜力。

Conclusion: 虽然两种方法都未确立明确的优越性，但壳预处理方法为开发鲁棒的跨域签名验证系统提供了有前景的方向。
...
### [175] [Automatic Classification of Circulating Blood Cell Clusters based on Multi-channel Flow Cytometry Imaging](https://arxiv.org/abs/2510.17716)
*Suqiang Ma,Subhadeep Sengupta,Yao Lee,Beikang Gu,Xianyan Chen,Xianqiao Wang,Yang Liu,Mengjia Xu,Galit H. Frydman,He Li*

Main category: cs.CV

TL;DR: 提出一个用于分析循环血细胞簇图像的计算框架，通过YOLOv11模型分类细胞簇和非细胞簇图像，然后结合多通道荧光染色识别细胞类型，准确率超过95%。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏自动分析含细胞簇图像的工具，而细胞簇具有不规则形状和异质性细胞组成，需要多通道染色来识别特定细胞类型。

Method: 采用两步分析策略：1) 微调YOLOv11模型分类细胞簇和非细胞簇图像；2) 通过叠加细胞簇轮廓与多通道荧光染色区域来识别细胞类型。

Result: 在细胞簇分类和表型识别方面均达到超过95%的准确率，优于传统CNN和ViT模型。

Conclusion: 该自动化框架能有效分析流式细胞术中的CCC图像，利用明场和荧光数据，具有扩展到分析免疫和肿瘤细胞簇等更广泛应用的潜力。
...
### [176] [Towards Explainable Skin Cancer Classification: A Dual-Network Attention Model with Lesion Segmentation and Clinical Metadata Fusion](https://arxiv.org/abs/2510.17773)
...
<details>
  <summary>Details</summary>
Motivation: 皮肤癌早期检测至关重要，但现有深度学习模型存在黑盒问题，缺乏临床信任。需要同时解决高类内变异性和细微类间差异的挑战。

Method: 使用带双注意力门和空洞空间金字塔池化的Deep-UNet进行病灶分割，然后采用双DenseNet201编码器分别处理原始图像和分割病灶，通过多头交叉注意力融合特征，并集成基于transformer的临床元数据模块。

Result: 在HAM10000、ISIC 2018和2019数据集上达到最先进的分割性能，显著提升分类准确率和平均AUC。Grad-CAM热图验证模型关注病灶区域而非背景特征。
...
TL;DR: RaindropGS是一个针对3D高斯溅射在雨滴条件下性能评估的综合基准，揭示了现有方法在无约束雨滴图像上的局限性，并分析了不同流水线组件的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的3DGS基准通常使用合成雨滴图像和已知相机位姿进行评估，但真实场景中雨滴会干扰相机位姿估计和点云初始化，且合成与真实雨滴存在显著域差距，影响泛化能力。

Method: 构建了一个完整的基准流水线，包括数据准备、数据处理和雨滴感知的3DGS评估。收集了真实世界雨滴重建数据集，每个场景包含三种对齐的图像集：雨滴聚焦、背景聚焦和无雨地面实况。

Result: 通过全面实验和分析，揭示了现有3DGS方法在无约束雨滴图像上的性能限制，以及相机焦点位置对重建性能的影响，以及不准确的位姿和点云初始化对重建的干扰。
...
<details>
  <summary>Details</summary>
Motivation: 探索高性能I2V模型在大型视频数据集训练后是否具备生成真实行人轨迹的能力

Method: 在行人轨迹基准提取的关键帧上条件化I2V模型，然后使用行人动力学定量指标评估轨迹预测性能

Result: 未在摘要中明确说明具体结果

Conclusion: 该研究框架为评估I2V模型在行人轨迹生成方面的能力提供了方法
...
<details>
  <summary>Details</summary>
Motivation: 解决多参考视觉地点识别中，在保持计算效率的同时提高在视觉外观和视角变化下的定位性能。传统方法需要大量训练或依赖启发式方法，在复杂变化下效果有限。

Method: 使用矩阵分解将多个参考描述符联合建模为基表示，通过投影残差匹配进行地点识别。该方法无需训练，且与具体描述符类型无关。

Result: 在多外观数据上，相比单参考方法Recall@1提升约18%，在视觉外观和视角变化下均优于多参考基线方法，在非结构化数据上提升约5%，表现出强泛化能力且保持轻量化。

Conclusion: 该方法在无需训练的情况下显著提升了多参考视觉地点识别性能，在视觉外观和视角变化下具有优越的泛化能力，同时保持计算效率。
...
TL;DR: SparseVILA是一种高效的视觉语言模型推理范式，通过将视觉稀疏性解耦到预填充和解码阶段，实现了显著的推理加速。
...
Method: 在预填充阶段剪枝冗余视觉token，在解码阶段仅检索与查询相关的token，采用解耦设计并基于AWQ优化的推理流水线。

Result: 在长上下文视频任务上实现4.0倍预填充加速、2.5倍解码加速和2.6倍端到端加速，同时在文档理解和推理任务上提高准确性。

Conclusion: 通过解耦查询无关的剪枝和查询感知的检索，SparseVILA为高效多模态推理提供了无需训练、架构无关的加速框架，且不牺牲模型能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练自由注意力控制方法在强编辑强度与保持源图像一致性之间存在矛盾，特别是在多轮和视频编辑中视觉错误会累积。大多数方法强制全局一致性，限制了细粒度编辑能力。

Method: 基于MM-DiT架构分析，提出ConsistEdit方法，包含视觉专用注意力控制、掩码引导预注意力融合、以及查询/键/值令牌的差异化操作。

Result: 在广泛的图像和视频编辑任务中达到最先进性能，支持结构一致和不一致场景，是首个无需手工调整即可在所有推理步骤和注意力层进行编辑的方法。
...
TL;DR: 该论文研究量子自然语言处理(QNLP)在自然语言推理任务中的应用，比较量子、混合和经典模型在少样本学习下的表现，并提出了新的信息增益参数比指标来评估模型效率。


<details>
  <summary>Details</summary>
Motivation: 探索量子计算在自然语言处理中的潜力，特别是在少样本学习场景下，利用量子电路的结构敏感性来提高学习效率。

Method: 使用lambeq库和DisCoCat框架构建参数化量子电路，训练语义相关性和推理分类任务，并引入基于聚类的架构来促进参数共享。

Result: 量子模型在参数数量大幅减少的情况下达到与经典模型相当的性能，在推理任务上优于随机初始化的transformer模型，且参数学习效率比经典模型高出五个数量级。

Conclusion: QNLP在低资源、结构敏感的场景中具有显著优势，量子模型展现出更高的参数效率和泛化能力。
...
TL;DR: 提出基于ChatGPT和Claude的多模型融合框架，通过图像和合成临床笔记的多模态输入，在胸部X光片诊断中显著提高准确性，共识方法达到91.3%的准确率。


<details>
  <summary>Details</summary>
Motivation: 提高胸部X光片AI诊断的可靠性和临床实用性，减少诊断错误，同时最小化计算开销。

Method: 使用ChatGPT和Claude两种大语言模型，采用相似性共识方法（95%输出相似度阈值），结合图像和合成临床笔记的多模态输入。

Result: 单模态图像输入时，ChatGPT准确率62.8%，Claude准确率76.9%；共识方法提升至77.6%。多模态输入时，ChatGPT提升至84%，Claude为76%，共识准确率达到91.3%。

Conclusion: 多模态输入和输出级共识能显著提高AI辅助放射诊断的可信度和临床效用，为减少诊断错误提供了实用路径。
...
TL;DR: CorrectBench是一个评估大语言模型自校正能力的基准，涵盖常识推理、数学推理和代码生成任务，发现自校正方法能提升准确性但效率较低，简单的思维链基线表现具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 虽然已有多种自校正方法被提出，但这些方法的全面评估仍然不足，且LLM是否能真正自我校正是一个重要问题。

Method: 开发CorrectBench基准，评估内在、外部和微调三种自校正策略在三个任务上的效果。

Result: 1) 自校正方法能提高准确性，特别是复杂推理任务；2) 混合不同策略能进一步提升但降低效率；3) 推理LLM在额外自校正方法下优化有限且时间成本高；思维链基线表现竞争性。

Conclusion: 自校正有潜力提升LLM推理性能，但需在推理能力和操作效率间寻找平衡，建议进一步研究优化这一平衡。
...
TL;DR: 提出了EvolveR框架，使LLM智能体能够通过完整的闭环经验生命周期进行自我改进，包括离线自我蒸馏和在线交互两个关键阶段。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在工具使用方面表现出色，但缺乏从自身经验中系统学习的能力，无法迭代改进问题解决策略。

Method: EvolveR框架包含两个阶段：离线自我蒸馏（将交互轨迹合成为可重用策略原则的结构化存储库）和在线交互（检索蒸馏原则指导决策，积累行为轨迹），采用策略强化机制迭代更新智能体。

Result: 在复杂多跳问答基准测试中，EvolveR优于强大的智能体基线，表现出卓越性能。

Conclusion: 该工作为智能体不仅从外部数据学习，还能从自身行为后果学习的完整蓝图，为更自主和持续改进的系统铺平了道路。
...
TL;DR: 本研究量化了提示策略与大型语言模型在系统文献综述筛选阶段的交互作用，发现CoT-少样本提示提供最可靠的精确度-召回率平衡，零样本提示在高灵敏度筛选时召回率最高，自反思提示表现不佳。GPT-4o-mini在显著降低成本的同时保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 评估不同提示策略与LLMs在自动化文献筛选中的交互效果，为系统文献综述提供实用的模型部署指导。

Method: 评估6个LLMs在5种提示类型下的表现，使用准确率、精确度、召回率和F1分数等指标，分析模型-提示交互效应和成本效益。

Result: CoT-少样本提示提供最可靠的精确度-召回率平衡；零样本提示在高灵敏度筛选时召回率最高；自反思提示因过度包容性和不稳定性表现不佳；GPT-4o和DeepSeek整体表现稳健；GPT-4o-mini在显著降低成本的同时保持竞争力。

Conclusion: 推荐采用分阶段工作流程：先用低成本模型和结构化提示进行初步筛选，仅将边界案例升级到高容量模型。LLMs在自动化文献筛选方面具有不均衡但有前景的潜力。
...
TL;DR: 该论文提出了一个合成测试平台，用于系统分析语言模型中统计规律与事实关联之间的相互作用对泛化能力的影响。研究发现上下文多样性和结构对分布内和分布外事实泛化有不同影响，并识别了优化瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对语言模型中统计规律与事实关联相互作用对泛化能力影响的系统分析，需要开发可控的测试环境来隔离这些效应。

Method: 设计灵活的合成测试平台，结合通用标记的统计流和源-目标标记对的事实流，通过独立控制流组成和多样性水平来进行受控实验。

Result: 研究发现更高的上下文多样性会延迟分布内事实准确性，但对分布外事实泛化的影响取决于上下文结构。在某些情况下，低多样性会阻碍事实回忆，而最优多样性水平取决于训练时长。

Conclusion: 上下文设计和多样性水平的相互作用影响不同的泛化方面，嵌入层和非嵌入层是分布外失败的关键优化瓶颈。合成框架为未来研究提供了受控测试平台。
...
TL;DR: 首个关于生成式AI信任与不信任的计算研究，使用2022-2025年Reddit数据集，发现信任与不信任基本平衡，技术性能和可用性是主要维度，个人经验是态度形成的最常见原因。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI系统融入日常生活，理解公众对其的信任对于负责任采用和治理至关重要，但缺乏大规模、纵向的计算方法来衡量对GenAI和LLMs的信任与不信任。

Method: 使用多年度Reddit数据集（39个子版块，197,618个帖子），结合众包标注和分类模型进行扩展分析。

Result: 信任与不信任随时间基本平衡，主要模型发布时出现转变；技术性能和可用性占主导地位，个人经验是态度形成的最常见原因；不同信任者群体（专家、伦理学家、普通用户）呈现不同模式。
...
TL;DR: 提出了EgMM-Corpus数据集，专门针对埃及文化的多模态数据集，包含3000多张图像覆盖313个概念，用于评估和训练视觉语言模型在埃及文化背景下的表现。


<details>
  <summary>Details</summary>
Motivation: 中东和非洲地区的多模态文化多样性数据集仍然有限，需要专门针对埃及文化的可靠资源来评估和训练视觉语言模型。
...
Conclusion: 这些结果凸显了大规模视觉语言模型中存在的文化偏见，并证明了EgMM-Corpus作为开发文化感知模型基准的重要性。
...
TL;DR: 该论文通过理论分析和实证验证，探讨了语言模型中概率与语法知识的关系，提出了三个预测并验证了语言模型在语法结构知识方面的表现。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否真正掌握了语法知识，以及如何通过字符串概率来揭示其内在的语法结构知识，这对语言学理论有重要影响。

Method: 基于语料库生成过程的简单假设，建立语法、意义和字符串概率关系的理论框架，并使用28万句英语和中文句子对进行实证验证。

Result: 验证了三个预测：(1)最小对中字符串概率的相关性；(2)模型与人类在最小对中差异的相关性；(3)语法与不语法字符串在概率空间中分离度差。

Conclusion: 为使用概率来研究语言模型的语法知识提供了理论基础，并为未来语言模型语法评估工作指明了方向。
...
TL;DR: 该论文提出两种方法（多元化解码和模型引导）来增强语言模型的多元对齐能力，在低资源设置下仅用50个标注样本就能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型对社会影响增大，需要确保它们能够对齐多元视角并反映人类价值观的细微差别。当前主流训练范式假设每个查询只有一个最优答案，导致响应泛化且对齐效果差。

Method: 提出两种方法：1) 多元化解码 - 生成多样化的响应；2) 模型引导 - 通过少量标注样本调整模型行为。在低资源设置下仅需50个标注样本。

Result: 模型引导方法在零样本和少样本基线上表现一致提升，在仇恨言论检测和错误信息检测等高风险任务中减少了假阳性，在GlobalOpinionQA上改善了与人类价值观的分布对齐。

Conclusion: 该工作强调了多样性的重要性，展示了语言模型如何适应考虑细微视角，为多元对齐提供了有效的低资源解决方案。
...
TL;DR: 提出了Profile-to-PEFT框架，使用超网络将用户配置文件直接映射到适配器参数，无需为每个用户单独训练，实现高效实时的个性化LLM适配。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法需要为每个用户训练单独的适配器，计算成本高且无法实时更新，限制了大规模个性化应用。

Method: 使用端到端训练的超网络，将编码后的用户配置文件直接映射到完整的适配器参数（如LoRA），部署时无需用户特定训练。

Result: 在实验中优于基于提示的个性化和OPPU方法，同时显著减少部署时的计算资源消耗，对分布外用户具有良好的泛化能力。
...
TL;DR: 该研究评估了不同后训练方法下LLM对其学习策略的认知能力，发现RL训练模型比SFT模型更了解自身学习行为且泛化能力更强，但推理轨迹与最终输出的对齐较弱。


<details>
  <summary>Details</summary>
Motivation: 探究LLM是否真正理解自己通过后训练技术学到的策略和推理过程，定义了三个核心能力来评估模型对其学习内容的认知程度。

Method: 在多个需要学习不同策略的任务上实证评估，对比SFT、DPO和GRPO三种后训练方法，分析模型对学习策略的认知、泛化能力以及推理与输出的对齐程度。

Result: RL训练模型比SFT模型表现出更强的策略认知能力和跨领域泛化能力，但推理轨迹与最终输出的对齐较弱，GRPO训练模型这一现象最明显。

Conclusion: 后训练方法显著影响LLM对其学习策略的认知能力，RL方法增强策略认知但削弱推理-输出对齐，需要在模型能力与透明度之间权衡。
...
TL;DR: 该研究探索使用LLM生成针对疫苗错误信息的反驳论点，通过多种提示策略和微调方法优化反驳生成，并训练分类器对反疫苗推文进行分类以实现情境感知的反驳。


<details>
  <summary>Details</summary>
Motivation: 在社交媒体影响公共卫生的时代，打击疫苗怀疑论和错误信息成为关键社会目标。虽然错误信息检测取得进展，但实时生成针对性反驳论点仍是一个未被充分探索的领域。

Method: 实验多种提示策略和微调方法优化反驳论点生成，训练分类器将反疫苗推文分类为多标签类别（如疫苗有效性担忧、副作用、政治影响等），实现情境感知的反驳。

Result: 通过人工判断、LLM评估和自动指标的评估显示，这些方法具有很强的一致性。整合标签描述和结构化微调可增强反驳论点的有效性。

Conclusion: 该研究为大规模缓解疫苗错误信息提供了一个有前景的方法，展示了LLM在生成针对性反驳论点方面的能力。
...
TL;DR: 该论文提出了AASP框架，通过自回归方式联合建模论证组件和论证关系，在三个标准基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过生成范式扁平化论证结构，难以有效建模论证组件和论证关系之间的依赖关系。

Method: 使用自回归论证结构预测框架，将论证结构建模为约束动作集，通过条件预训练语言模型逐步构建论证结构。

Result: 在三个标准AM基准测试中，AASP在两个基准上实现了最先进结果，在一个基准上获得强有力结果。

Conclusion: AASP框架能够有效捕捉论证推理流程，为论证挖掘任务提供了有效的端到端解决方案。
...
Result: 该方法在两个任务上取得改进：识别Reddit帖子是否有助于检测抑郁症状的相关性预测，以及基于用户Reddit帖子历史完成标准化抑郁筛查问卷

Conclusion: 转向机制作为计算高效的工具，在LLMs心理健康领域适应方面具有未开发的潜力
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统在决策中扮演更重要角色，需要理解AI如何做出决策而不仅仅是决策结果。道德困境是评估AI推理过程的理想测试平台，因为允许多个合理的结论。

Method: 创建MoReBench基准：包含1000个道德场景，每个场景配有专家制定的评估标准（如识别道德考量、权衡利弊、提供可行建议）。另创建MoReBench-Theory测试AI在五种规范伦理学框架下的推理能力。

Result: 扩展定律和现有数学、代码、科学推理基准无法预测模型的道德推理能力。模型对特定道德框架（如边沁行为功利主义和康德义务论）表现出偏好，这可能是流行训练范式的副作用。

Conclusion: MoReBench基准推动了以过程为重点的推理评估，朝着更安全、更透明的AI发展。
...
TL;DR: 提出ATA方法，将LLM任务分解为离线知识提取和在线任务处理两个阶段，通过符号化知识库确保可信赖性，在复杂推理任务中表现优异且具备确定性、稳定性和安全性


<details>
  <summary>Details</summary>
Motivation: 解决LLM在关键领域部署时的可信赖性问题，包括幻觉、不稳定性和缺乏透明度等局限性

Method: 神经符号方法ATA：1）离线阶段将非正式问题规范转化为可验证的符号知识库；2）在线阶段使用符号决策引擎基于编码输入和知识库进行可靠推理

Result: ATA在完全自动化设置下与最先进端到端推理模型竞争，在人工验证知识库后显著优于更大模型，具备完美确定性、抗输入扰动稳定性和抗提示注入攻击能力

Conclusion: ATA通过符号推理生成决策，为构建透明、可审计、可靠的下一代自主代理提供了实用可控架构
...
TL;DR: 该论文探索了Whisper语音识别模型在第二语言口语评估中的潜力，通过提取其隐藏表示中的声学和语言特征，仅需训练轻量级分类器就能在GEPT数据集上超越现有先进基线。


<details>
  <summary>Details</summary>
Motivation: 挖掘Whisper模型在第二语言口语评估中的潜在能力，超越仅使用其转录文本的传统方法，探索其隐藏表示中蕴含的丰富信息。

Method: 从Whisper的中间和最终输出中提取声学和语言特征，仅训练轻量级分类器，并整合图像和文本提示作为辅助相关性线索。

Result: 在GEPT图片描述数据集上取得强劲性能，超越包括多模态方法在内的现有先进基线，通过整合辅助信息获得额外性能提升。

Conclusion: 即使没有任务特定微调，Whisper模型内在编码了口语熟练度的序数模式和语义方面，显示出其作为口语评估和其他口语理解任务的强大基础模型的潜力。
...
TL;DR: FrugalPrompt是一个LLM提示压缩框架，通过保留语义重要性最高的token来减少输入长度，在多数NLP任务中实现20%压缩率而性能损失很小，但数学推理任务性能下降明显。


<details>
  <summary>Details</summary>
Motivation: LLM的冗长输入会增加成本、碳足迹和推理延迟，而典型提示中存在大量冗余的低效用token，只有少数token承载主要语义权重。

Method: 使用GlobEnc和DecompX两种token归因方法为输入序列中的每个token分配显著性分数，按分数排序保留前k%的token并保持原始顺序，获得稀疏的压缩提示。

Result: 在情感分析、常识问答和摘要任务中，20%的提示压缩仅导致任务性能的边际损失；但在数学推理中性能急剧下降；使用底部k%和随机k%token的分析揭示了不对称性能模式。
...
TL;DR: TrajSelector是一个高效的Best-of-N框架，利用LLM的隐藏状态进行过程级评分，通过轻量级验证器评估推理轨迹质量，在保持低推理成本的同时实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有外部TTS方法存在计算开销大和未充分利用LLM内在潜表示的问题，需要更高效的轨迹选择框架。

Method: 使用0.6B参数的轻量级验证器评估步骤级轨迹质量，利用LLM隐藏状态进行过程级评分，采用端到端训练方法无需大量步骤级标注。

Result: 在五个基准测试中，Best-of-32设置下比多数投票准确率高4.61%，比现有过程奖励模型高4.31%到12.21%，且推理成本更低。

Conclusion: TrajSelector通过利用LLM内在表示实现了高效有效的推理轨迹选择，在保持低成本的同时显著提升性能。
...
TL;DR: RAVEN是一个集成课程强化学习和多模态大语言模型的广告视频违规检测框架，通过渐进式训练策略和GRPO优化实现精确时间定位和类别预测，在工业数据集和公开基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有广告视频违规检测方法在精确时间定位、噪声标注和泛化能力方面存在不足，需要提升推理和认知能力来解决这些问题。

Method: 采用课程强化学习结合多模态大语言模型，使用渐进式训练策略整合精确和粗略标注数据，利用GRPO发展推理能力，并通过多层次奖励机制确保时间定位和类别预测的准确性。

Result: 在工业数据集和公开基准测试中，RAVEN在违规类别准确性和时间区间定位方面表现优异，在线A/B测试验证了其实际应用价值，显著提升了精确率和召回率。
...
TL;DR: 本文扩展了自然语言推理中标签变异的研究范围，不仅分析标注者在推理类型上的分歧，还考察他们在标注步骤上的差异，发现表面标签分歧可能掩盖深层的解释一致性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注标注者标签一致但解释不同的情况，本文旨在更全面地分析标注者在推理类型和标注步骤上的分歧，揭示标签变异背后的深层原因。

Method: 使用LiTEx分类法分析两个英文NLI数据集，从NLI标签一致性、解释相似性和分类法一致性多个维度对齐标注变异，并考虑标注者选择偏见的复合因素。

Result: 发现标注者标签不一致但解释高度相似的情况，表明表面分歧可能掩盖解释层面的一致性；分析还揭示了标注者在解释策略和标签选择上的个体偏好。

Conclusion: 推理类型的一致性比标签一致性更能反映自由文本解释的语义相似性，基于推理的解释具有丰富性，需要谨慎将标签视为绝对真理。
...
TL;DR: 提出使用"退出"作为LLM代理的安全机制，让代理在缺乏信心时主动退出，在12个先进LLM上验证了这种简单方法能显著提升安全性而几乎不影响帮助性。
...
Result: 退出行为带来了高度有利的安全-帮助性权衡：安全性平均提升+0.39（专有模型+0.64），而帮助性仅平均下降-0.03。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体系统组合方法依赖静态语义检索，存在能力描述不完整、选择不考虑能力-成本-实时效用等问题，难以实现有效的组件重用和组合。

Method: 受背包问题启发，设计结构化自动化框架，让组合器智能体系统性地识别、选择和组装最优智能体组件集，综合考虑性能、预算约束和兼容性。

Result: 在5个基准数据集上的实验显示，在线背包组合器始终位于帕累托前沿，单智能体设置下成功率提升达31.6%，多智能体系统中成功率从37%提升至87%。
...
TL;DR: 提出了ReviewGuard系统，使用LLM驱动的四阶段框架自动检测和分类有缺陷的同行评审，包括数据收集、标注、合成数据增强和模型微调，以解决同行评审中人类和AI系统产生的不合格评审问题。


<details>
  <summary>Details</summary>
Motivation: 同行评审作为科学的守门人面临提交量激增和LLM广泛使用的挑战，未受监控的人类专家和AI系统产生的有缺陷评审可能系统性地破坏同行评审生态系统并损害学术诚信。

Method: 四阶段LLM驱动框架：1)从OpenReview收集ICLR和NeurIPS论文及评审；2)使用GPT-4.1标注评审类型并人工验证；3)通过LLM驱动的合成数据增强解决类别不平衡和数据稀缺问题；4)微调编码器模型和开源LLM。

Result: 构建了包含6,634篇论文、24,657个真实评审和46,438个合成评审的语料库。有缺陷评审显示出更低的评分、更高的自报告置信度、降低的结构复杂度和更高比例的负面情绪。ChatGPT出现后AI生成评审显著增加。
...
TL;DR: 通过分析LLMs在回答文化相关问题时激活路径的重叠程度，揭示了语言对内部文化表征的强烈影响，并发现语言相似性不一定导致对齐的内部表征。


<details>
  <summary>Details</summary>
Motivation: LLMs在跨文化语境中的使用日益增多，但现有评估主要关注输出层面，缺乏对驱动响应差异的内部机制理解，且电路分析研究很少关注文化和多语言。

Method: 通过测量LLMs在回答语义等价问题时激活路径的重叠程度，比较两种条件：(1)固定问题语言、改变目标国家；(2)固定国家、改变问题语言，并使用同语言国家对来分离语言和文化因素。

Result: 结果显示，同语言跨国家问题的内部路径重叠度高于跨语言同国家问题，表明存在强烈的语言特定模式。特别是韩国-朝鲜对表现出低重叠和高变异性。

Conclusion: 语言在LLMs内部文化理解中起主导作用，语言相似性不一定保证对齐的内部表征，这对跨文化LLM应用具有重要意义。
...
TL;DR: SHALLOW是首个系统分类和量化ASR系统中幻觉现象的基准框架，涵盖词汇、语音、形态和语义四个维度，提供比传统WER更细粒度的模型评估。


<details>
  <summary>Details</summary>
Motivation: ASR系统中的幻觉会产生语法语义合理但与语音输入无关的转录，在医疗、法律等关键领域带来严重风险，而传统错误率指标无法有效区分语音不准确和幻觉现象。

Method: 提出SHALLOW框架，从词汇、语音、形态和语义四个互补维度系统分类幻觉现象，为每个类别定义针对性指标，生成可解释的模型行为分析。

Result: 评估显示SHALLOW指标在识别质量高时与WER强相关，但随着WER增加相关性显著减弱，SHALLOW能捕捉WER在恶劣条件下无法区分的细粒度错误模式。

Conclusion: SHALLOW框架支持对模型弱点的具体诊断，并提供超出聚合错误率的模型改进反馈，有助于识别和评估易产生幻觉内容的ASR模型。
...
<details>
  <summary>Details</summary>
Motivation: 乌尔都语等低资源语言缺乏AI生成文本检测工具，随着LLM生成文本能力增强，区分人类写作与机器生成文本变得困难，这对打击错误信息和学术不端构成挑战。

Method: 构建包含1800篇人类写作和1800篇AI生成文本的平衡数据集，进行语言学和统计分析，微调三种多语言transformer模型（mdeberta-v3-base、distilbert-base-multilingualcased、xlm-roberta-base）。

Result: mDeBERTa-v3-base模型表现最佳，在测试集上F1分数为91.29%，准确率为91.26%。

Conclusion: 该研究推进了乌尔都语社区打击错误信息和学术不端的努力，并为低资源语言的NLP工具开发做出贡献。
...
TL;DR: 通过微调大型语言模型将句子翻译成对应句法结构，实现西班牙语句法分析，扩展MiSintaxis教学工具的功能


<details>
  <summary>Details</summary>
Motivation: 利用大型神经网络模型在自然语言处理中的进展，探索基于机器学习的句法分析新方法，扩展西班牙语句法教学工具MiSintaxis的能力

Method: 从Hugging Face仓库选择多个模型，使用AnCora-ES语料库生成的训练数据进行微调，让LLMs学习将输入句子翻译成对应句法结构

Result: 使用F1分数评估性能，结果显示在短语结构分析方面达到了高准确率

Conclusion: 该方法展示了将大型语言模型用于句法分析的潜力，为西班牙语句法教学提供了有效工具
...
TL;DR: DiMo是一个多智能体协作框架，通过模拟四个专门LLM智能体之间的结构化辩论来提升性能与可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型缺乏可解释推理的问题，通过多智能体协作模拟人类多样化思维模式。

Method: 使用四个体现不同推理范式的专门LLM智能体进行迭代辩论，挑战和优化初始响应。

Result: 在六个基准测试中，DiMo在统一开源设置下比单模型和辩论基线准确率更高，数学任务提升最大。

Conclusion: DiMo作为语义感知的Web原生多智能体框架，能够产生语义类型化、URL注释的证据链，支持可检查的检索增强推理。
...
TL;DR: 本文提出了Capsule Prompt-Tuning (CaPT)方法，通过将实例感知信息与任务感知信息结合在单个胶囊提示中，解决了传统提示学习方法缺乏实例感知信息的问题，实现了高效且有效的参数微调。


<details>
  <summary>Details</summary>
Motivation: 当前基于提示的学习方法存在三个主要问题：1) 依赖繁琐的网格搜索确定最佳提示长度；2) 需要大量提示，增加计算负担；3) 缺乏实例感知信息，导致与输入序列的注意力交互受限。

Method: 提出Capsule Prompt-Tuning (CaPT)方法，利用现成的信息性实例语义，以近乎参数免费的方式（单个胶囊提示）同时集成实例感知和任务感知信息，作为"注意力锚点"。

Result: 在T5-Large模型上达到84.03%的平均准确率，在Llama3.2-1B模型上仅使用0.003%的参数，在各种语言任务上表现出优越性能。

Conclusion: CaPT方法通过引入实例感知信息作为注意力锚点，有效提升了提示调优模型的性能，同时保持了高参数效率，为大型语言模型的高效适应提供了一种新解决方案。
...
TL;DR: 提出了TUuD框架来评估LLMs在动态时间参考点下理解时间-事件和事件-事件关系的能力，发现LLMs表现出部分类似人类的时间认知但存在局限性。
...
Method: 引入TUuD框架，让LLMs在时间参考点"现在"沿时间线动态移动的情况下，评估时间-事件和事件-事件关系。LLMs被提示从0.00到1.00评分当前时刻与目标事件的相似度。

Result: 四个评估的LLMs表现出对指示性t-FoR的可测量适应，相似度评分在现在附近达到峰值，并向过去和未来事件递减。但这种适应在超出近期语境时会减弱。

Conclusion: LLMs显示出部分类似人类的时间认知，但其时间推理仍然对参考框架转换和时间距离敏感。
...
TL;DR: 该论文探讨了思维链(CoT)推理在自然语言理解(NLU)任务中的适用性，发现随着模型规模增大，CoT推理从阻碍NLU性能转变为超越直接标签预测，并开发了专门的训练方法实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 大多数研究关注思维链在推理任务中的作用，但忽视了其在自然语言理解任务中的潜在影响。本研究旨在系统探索思维链是否同样能有益于NLU任务。

Method: 构建了NLURC数据集（包含思维链的高质量NLU数据集集合），开发了多种思维链增强方法，并系统探索这些方法在NLU任务中的适用性。

Result: 发现三个关键结果：(1) CoT推理与模型规模呈正相关，大模型中使用CoT能超越直接标签预测；(2) 大多数思维链增强训练方法表现不如仅标签训练，但有一个专门设计的方法能持续改进；(3) 使用思维链训练的LLM在未见过的NLU任务上表现显著提升，性能可与大十倍的模型媲美，同时提供与商业LLM相当的interpretability。

Conclusion: 思维链方法在NLU任务中具有重要价值，特别是在大模型和跨任务泛化方面，能够实现性能提升和可解释性增强。
...
<details>
  <summary>Details</summary>
Motivation: 心血管疾病在现代社会中日益普遍，对全球健康产生重大影响。心脏病是复杂的多因素疾病，相关信息分散在各种文本数据中。NLP技术能够分析这些海量非结构化数据，帮助医疗专业人员深入了解心脏病学领域，从而革新心脏病诊断、治疗和预防方法。

Method: 查询了六个文献数据库，通过严格的筛选过程识别出265篇相关文章。从多个维度分析每篇文章：NLP范式类型、心脏病相关任务类型、心血管疾病类型和数据源类型，并进行时间分析以展示NLP方法在过去十年中的演变趋势。

Result: 分析显示这些维度内存在相当大的多样性，证明了NLP在心脏病学领域研究的广度。时间分析揭示了所涵盖的过去十年中NLP方法的演变和变化趋势。
...
TL;DR: 研究发现LLMs在检索增强系统中存在严重的"变色龙行为"，即在多轮对话中面对矛盾问题时立场不稳定，这威胁到系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强LLMs系统普遍存在可靠性漏洞，特别是在处理多轮对话中的矛盾问题时，模型会改变立场，这对医疗、法律、金融等关键领域的应用构成威胁。

Method: 创建了包含17,770个问答对的Chameleon基准数据集，涵盖12个争议领域的1,180个多轮对话，并提出了Chameleon Score和Source Re-use Rate两个理论指标来量化立场不稳定性和知识多样性。

Result: 对Llama-4-Maverick、GPT-4o-mini和Gemini-2.5-Flash的评估显示所有模型都表现出严重的变色龙行为（得分0.391-0.511），GPT-4o-mini表现最差。知识多样性有限导致模型过度依赖查询框架。

Conclusion: 在部署LLMs到医疗、法律和金融等需要保持立场一致性的关键系统之前，必须进行全面的稳定性评估，以确保可靠的决策支持。
...
TL;DR: 该论文研究诗歌中空白空间的使用模式，比较了人类创作诗歌与LLM生成诗歌的空白空间分布差异，并探讨了文本处理方法对诗歌空白空间表示的影响。


<details>
  <summary>Details</summary>
Motivation: 诗歌中的空白空间是诗歌形式的重要组成部分，反映了诗人的艺术选择，但在NLP研究中未得到足够重视。研究者希望填补这一空白，研究空白空间在诗歌中的语义和空间特征。

Method: 使用来自Poetry Foundation的19k首英语诗歌语料库，分析4k位诗人的空白空间使用模式，并与51k首LLM生成诗歌和12k首未发表在线诗歌进行比较。

Result: 发现不同文本处理方法会导致诗歌空白空间表示的显著差异，比较了不同时期、诗歌形式和来源的空白空间使用模式。

Conclusion: 诗歌空白空间的研究对LLM预训练数据集的构建策略具有重要启示，强调了保留原始格式的重要性。
...
TL;DR: 提出了Beacon基准测试，用于测量大型语言模型在事实准确性和顺从性偏见之间的权衡，揭示了语言模型中的谄媚偏见及其可干预性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在奖励优化过程中混淆了帮助性和礼貌顺从，形成了事实性和谄媚性之间的结构权衡，这种潜在偏见被称为谄媚性，表现为偏好用户同意而非原则性推理。

Method: 引入Beacon基准测试，这是一个单轮强制选择基准，能够独立于对话上下文隔离这种偏见，精确测量事实准确性和顺从偏见之间的张力。通过提示级和激活级干预来调节这些偏见。

Result: 对12个最先进模型的评估显示，谄媚性可分解为稳定的语言和情感子偏见，每种偏见都随模型容量而扩展。干预措施能够在相反方向上调节这些偏见。

Conclusion: Beacon将谄媚性重新定义为一种可测量的规范错误泛化形式，为研究和大规模生成系统中的对齐漂移缓解提供了可重复的基础。
...
TL;DR: 提出了SCO-PAL方法，通过对抗游戏中的对手选择分析发现自博弈是最有效的策略推理提升方式，相比基线方法平均胜率提升约30%，在六种对抗游戏中对抗GPT-4达到54.76%胜率。


<details>
  <summary>Details</summary>
Motivation: 现有语言智能体在动态对抗游戏中因策略推理能力不足而表现不佳，需要无需专家标注数据的自动学习方法。对手选择在动态对抗环境中对学习性能有重要影响，但相关研究尚不充分。

Method: 提出了SCO-PAL（Step-level poliCy Optimization through Play-And-Learn）方法，通过设置不同水平的对手进行详细分析，发现自博弈是最有效的学习策略。

Result: 使用SCO-PAL结合自博弈，相比基线方法在对抗四种对手时平均胜率提升约30%，在六种对抗游戏中对抗GPT-4达到54.76%的胜率。

Conclusion: 自博弈是提升对抗环境中策略推理能力的最有效方式，SCO-PAL方法显著提高了语言智能体在动态对抗游戏中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在长上下文理解方面能力的发展，需要严格的评估方法来有效评估其性能。

Method: 开发了LC-Eval基准，包含四个新任务：多文档问答、双语问答、段落内声明验证和基于长上下文的多选题，涵盖英语和阿拉伯语数据集。

Result: 评估显示LC-Eval具有显著挑战性，即使是GPT-4o等高性能模型在某些任务上也表现不佳。

Conclusion: LC-Eval基准的复杂性和严谨性凸显了当前LLM在长上下文理解方面的局限性，为未来模型改进提供了重要参考。
...
TL;DR: MOSAIC是一个用于句子嵌入模型领域适应的多阶段框架，通过联合领域特定的掩码监督来适应大规模通用领域模型到专业领域。


<details>
  <summary>Details</summary>
Motivation: 解决将大规模通用领域句子嵌入模型适应到专业领域的挑战，需要在保持原有模型语义判别能力的同时学习领域相关表示。

Method: 采用多阶段框架，在统一训练流程中联合优化掩码语言建模(MLM)和对比学习目标，实现选择性适应。

Result: 在高资源和低资源领域都取得显著改进，NDCG@10指标相比强通用领域基线提升高达13.4%。

Conclusion: 消融研究表明各组件都有效，强调了平衡的联合监督和分阶段适应的重要性。
...
TL;DR: LLMs在实体比较任务中经常依赖启发式偏见而非真实知识，即使具备正确数值知识。研究发现实体流行度、提及顺序和语义共现三种启发式偏见主导模型预测，小模型尤其严重。大模型能选择性使用更可靠的数值知识，而思维链提示能引导所有模型使用数值特征。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs何时依赖真实知识而非表面启发式进行推理，通过实体比较任务分析模型在具备正确知识的情况下为何仍会做出错误预测。

Method: 使用实体比较任务（如比较河流长度），分析LLMs的预测模式，识别三种启发式偏见：实体流行度、提及顺序和语义共现，并比较不同规模模型的表现差异。

Result: 小模型（7-8B参数）的预测更多受启发式偏见影响，简单逻辑回归使用表面线索比模型自身数值预测更准确；大模型（32B参数）能选择性使用更可靠的数值知识；思维链提示能引导所有模型使用数值特征。

Conclusion: 模型规模影响知识使用策略，大模型能更好地区分何时依赖知识而非启发式，思维链提示是改善模型推理的有效方法，揭示了LLMs知识推理的机制和局限性。
...
TL;DR: 提出一个两阶段检索-重排序框架，通过微调LLM进行跨体裁作者归属任务，在HIATUS基准测试中显著超越现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 解决跨体裁作者归属任务中避免依赖主题线索，而需要学习独立于文本主题的作者特定语言模式的问题。

Method: 采用检索-重排序框架，针对重排序阶段设计了专门的数据筛选策略，避免使用信息检索中常见的训练方法。

Result: 在HIATUS的HRS1和HRS2跨体裁作者归属基准测试中，分别比现有最佳方法提升了22.3和34.4个绝对Success@8点。

Conclusion: 提出的LLM检索-重排序管道能够有效学习作者区分性信号，在跨体裁作者归属任务中取得显著性能提升。
...
TL;DR: CoRUS框架通过角色理论模拟基于用户角色的提问，发现在阿片类药物使用障碍等敏感领域中，不同角色（患者、照顾者、从业者）会引发语言模型不同的回应模式。


<details>
  <summary>Details</summary>
Motivation: 现有评估大多忽略提问者的角色背景，这在阿片类药物使用障碍等敏感领域尤为重要，因为考虑用户背景对于提供可访问、无污名化的回应至关重要。

Method: 基于角色理论和在线OUD康复社区帖子，构建提问者角色分类法（患者、照顾者、从业者），并模拟15,321个嵌入各角色目标、行为和经验的提问。

Result: 模拟问题高度可信且与现实数据相当。评估五个LLM发现：脆弱角色（患者和照顾者）相比从业者，会引发更多支持性回应（+17%）和减少知识内容（-19%）。

Conclusion: 用户角色的隐式信号会影响模型回应，本研究提供了基于角色的对话AI评估方法学。
...
<details>
  <summary>Details</summary>
Motivation: 生成专业财务报告是一个劳动密集且智力要求高的过程，现有AI系统难以完全自动化，需要解决数据收集、分析和报告生成的全流程挑战。

Method: 采用CAVM架构统一外部数据、工具和智能体；提出迭代视觉增强机制逐步优化原始视觉输出；使用两阶段写作框架将简洁的分析链扩展为连贯的多模态报告。

Result: 在各种公司和行业级任务上的实验表明，FinSight在所有基线方法（包括领先的深度研究系统）上都显著优于其他方法，在事实准确性、分析深度和呈现质量方面表现突出。

Conclusion: FinSight展示了生成接近人类专家质量财务报告的清晰路径，为自动化专业财务报告生成提供了有效解决方案。
...
TL;DR: 提出了神经群通信（NGC）框架，将神经网络重新构想为相互作用的神经群动态系统，通过低维信号交换实现参数压缩和模块化表示，在大型语言模型中展现优于传统低秩方法的性能。
...
Method: 提出神经群通信（NGC）框架，将神经网络视为神经群相互作用的动态系统，权重被视为神经状态间的瞬时交互，通过神经群间的迭代通信进行计算，引入神经元稳定性度量来量化序列处理中的稳定模式。

Result: 在大型语言模型上实例化NGC，在适度压缩下在复杂推理基准上表现更好，在相同压缩率下持续优于标准低秩近似和跨层基共享方法。

Conclusion: NGC框架通过结构化神经群动态与高维学习系统中的泛化能力相关，为构建高效、模块化和可解释的神经网络提供了新思路。
...
TL;DR: 该论文提出了一个基于心理学感知理论的多模态知识理解基准，评估了30个先进语言模型在多种感官维度上的表现，发现视觉语言模型在感知理解任务中并未优于纯文本模型，特别是在视觉维度表现最差。


<details>
  <summary>Details</summary>
Motivation: 研究多模态语言模型是否比纯文本模型在具身知识理解方面有优势，通过心理学感知理论构建评估基准来验证视觉基础是否能增强模型对物理世界的理解。

Method: 基于心理学感知理论构建了一个包含视觉、听觉、触觉、味觉、嗅觉和内部感知的多模态知识理解基准，包含向量比较和问答任务，共1700多个问题，比较了30个最先进的语言模型。

Result: 视觉语言模型在两种任务中均未优于纯文本模型；模型在视觉维度的表现显著差于其他感官维度；向量表示易受词形和频率影响；模型在涉及空间感知和推理的问题上表现困难。

Conclusion: 当前语言模型在具身知识理解方面仍有局限，需要更有效地整合具身知识来增强对物理世界的理解能力。
...
TL;DR: ChiKhaPo是一个大规模多语言基准测试，包含8个不同难度的子任务，旨在评估生成模型在2700+种语言中的词汇理解和生成能力，填补了现有基准测试主要关注高资源语言的空白。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型基准测试主要局限于高资源或中资源语言，且多评估推理和生成等高阶任务，但大量证据表明LLMs在全球3800多种书面语言中缺乏基本语言能力。

Method: 利用现有词典、单语数据和双语语料构建ChiKhaPo基准，包含8个不同难度的子任务，覆盖2700多种语言，在语言覆盖范围上超越现有任何基准测试。

Result: 测试显示6个最先进的模型在该基准上表现不佳，性能得分受语言族系、语言资源丰富度、任务类型以及理解与生成方向等因素影响。

Conclusion: ChiKhaPo旨在促进和鼓励LLMs的大规模多语言基准测试，推动模型在更多语言上的能力评估。
...
TL;DR: 提出了PROMPT-MII方法，通过强化学习元学习指令归纳模型，能在新数据集上生成紧凑指令，在保持与上下文学习相当性能的同时大幅减少推理成本。


<details>
  <summary>Details</summary>
Motivation: 上下文学习虽然有效但推理成本高，随着上下文长度增长成本急剧增加，需要找到既能保持性能又降低成本的替代方法。

Method: 使用强化学习框架元学习指令归纳模型，在3000多个多样化分类数据集上训练，能够为新数据集动态生成紧凑指令。

Result: 在90个未见任务上测试，PROMPT-MII将下游模型质量提升4-9个F1点（10-20%相对提升），性能与上下文学习相当但只需3-13倍更少的token。

Conclusion: PROMPT-MII方法成功实现了指令归纳，在保持性能的同时显著降低了大型语言模型适应新任务的推理成本。
...
TL;DR: 本文首次将参数高效微调(PEFT)应用于孟加拉语仇恨言论检测，使用LoRA和QLoRA技术在三个大语言模型上进行微调，在BD-SHS数据集上取得了最高92.23%的F1分数。
...
Method: 使用LoRA和QLoRA参数高效微调技术，在Gemma-3-4B、Llama-3.2-3B和Mistral-7B三个指令调优大语言模型上微调，训练参数少于1%，在单张消费级GPU上进行实验。

Result: Llama-3.2-3B获得最高F1分数92.23%，Mistral-7B为88.94%，Gemma-3-4B为80.25%，证明了PEFT方法的有效性。

Conclusion: PEFT是孟加拉语及相关低资源语言仇恨言论检测的实用且可复现策略。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有字节级分词器引入超出范围ID或辅助标记的问题，利用ASCII原始设计理念，通过C0控制字节统一编码文本和控制信息。

Method: 实现基于UTF-8字节编码的分词器，所有特殊行为（如填充、边界、对话结构等）都使用C0控制字节编码，无需额外标记。

Result: 分词速度提升14倍，主机-设备传输减少8倍，提供256*d可共享嵌入表，通过位偏置嵌入增强训练效果。

Conclusion: UTF8Tokenizer提供高效、简洁的分词方案，改进了语言建模的收敛性，兼容HuggingFace生态系统。
...
TL;DR: 提出了一种基于变换向量的词汇表压缩方法，通过组合基础形式和变换向量来表示词形变化，从而释放词汇表空间用于更多样化的词汇，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 标准分词算法将词形变化视为独立标记，导致词汇表被表面形式变体填满，牺牲了低频词和多语言覆盖。需要更紧凑的词汇表设计。

Method: 使用变换向量（加性偏移）在嵌入空间中表示词形变化，将词汇表从字符串枚举重构为基于基础形式和变换向量的组合表示。

Result: 在多个LLM和五种语言上验证，可移除高达10%的词汇条目，扩展词汇覆盖范围，对下游任务性能影响最小，且无需修改模型权重。

Conclusion: 词汇表设计应从字符串枚举转向利用语言底层结构的组合式词汇表，这为词汇表设计提供了新的基础思路。
...
TL;DR: 提出了一种基于强化学习的动态防御框架，通过在线学习对抗迭代越狱攻击，使用Past-Direction Gradient Damping防止过拟合，显著提升了LLMs的安全性和无害任务的响应质量。


<details>
  <summary>Details</summary>
Motivation: 现有的防御方法无法主动破坏迭代越狱攻击的动态试错循环，这些攻击通过重复重写提示词来诱导有害输出，利用模型之前的响应指导每次迭代。

Method: 采用基于强化学习的方法动态更新防御策略，优化提示词以确保对无害任务给出适当响应，同时明确拒绝有害提示词，并引入Past-Direction Gradient Damping来抑制对攻击期间探索的有限输入重写过拟合。

Result: 在三个LLMs上的实验表明，该方法显著优于五种现有防御方法对抗五种迭代越狱方法，同时提示优化策略还提升了无害任务的响应质量。

Conclusion: 该动态防御框架有效对抗迭代越狱攻击，在保持安全性的同时提高了模型对无害任务的处理能力，为LLMs安全防御提供了新思路。
...
TL;DR: DiscoTrack是一个多语言LLM基准测试，专注于话语跟踪中的隐式信息和语用推理，包含12种语言和四个话语理解层次的任务。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准测试主要关注自然语言理解中的显式信息提取，缺乏针对隐式信息、语用推理以及跨句子、段落和多个说话者话语的话语跟踪能力的多语言基准测试。

Method: 提出了DiscoTrack基准测试，涵盖12种语言和四个话语理解层次：显著性识别、实体跟踪、话语关系和桥接推理。

Result: 评估显示，即使是最先进的模型，这些任务仍然具有挑战性。

Conclusion: DiscoTrack填补了LLM基准测试在话语跟踪和隐式推理方面的空白，为模型评估提供了更全面的多语言测试框架。
...
TL;DR: 本文研究发现基于大语言模型的搜索代理比基础LLM更容易产生有害输出，提出了SafeSearch多目标强化学习方法，在保持问答性能的同时显著降低危害性。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注搜索代理的实用性，但其安全行为尚未充分探索。研究发现搜索代理在处理有害问题时比基础模型更容易产生不安全输出，且实用性导向的微调会加剧这种风险。

Method: 提出SafeSearch方法，采用多目标强化学习，结合最终输出的安全/实用性奖励和查询级别的塑造项，惩罚不安全查询并奖励安全查询。

Result: 实验显示SafeSearch在三个红队数据集上减少代理危害性超过70%，同时产生安全、有用的响应，并与仅实用性微调的代理在问答性能上相当。

Conclusion: 查询级别的奖励在联合改进安全性和实用性方面有效，证明了联合对齐安全性和实用性的重要性。
...
TL;DR: 提出xLSTM框架，结合余弦相似度门控、自适应特征优先级和类别重平衡，在有毒评论检测任务中超越BERT，参数量减少15倍，推理延迟50ms。


<details>
  <summary>Details</summary>
Motivation: 解决基于Transformer的模型在有毒评论检测中计算成本高、在少数毒性类别上性能下降的问题，以及传统集成方法缺乏语义适应性的局限。

Method: 使用可学习的参考向量通过余弦相似度调节上下文嵌入，集成多源嵌入（GloVe、FastText、BERT CLS），包含字符级BiLSTM用于形态线索，嵌入空间SMOTE用于少数类增强，以及带动态类别加权的自适应焦点损失。

Result: 在Jigsaw有毒评论基准测试中达到96.0%准确率和0.88宏F1，在威胁和身份仇恨类别上分别比BERT高出33%和28%，余弦门控在消融实验中贡献+4.8% F1增益。

Conclusion: 建立了新的效率适应性前沿，证明轻量级、理论指导的架构可以在不平衡、特定领域的NLP任务中超越大型预训练模型。
...
TL;DR: 论文研究大语言模型的提示敏感性，提出通过语义空间采样和改述扰动来改善不确定性校准，并引入新的不确定性分解指标。


<details>
  <summary>Details</summary>
Motivation: 大语言模型对语义相同但表述不同的提示会产生不同的答案分布，这表明模型输出的不确定性可能无法反映对提示含义的真正不确定性。

Method: 将提示敏感性建模为泛化误差，通过语义概念空间的改述扰动进行采样，并引入新的不确定性分解指标来量化提示敏感性。

Result: 跨语义概念空间的采样改善了不确定性校准而不影响准确性，新的分解指标能有效量化提示敏感性导致的不确定性。

Conclusion: 该方法为改善提示敏感语言模型的不确定性校准提供了新途径，证明某些LLM缺乏对输入含义的一致通用推理能力。
...
TL;DR: 本文系统研究了大型语言模型在推理过程中如何聚合社会偏见，发现了两种导致偏见加剧的失败模式，并提出了一种轻量级的提示方法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 虽然基于推理的大型语言模型通过内部结构化思维过程在复杂任务中表现出色，但这些思维过程会聚合社会刻板印象，导致有偏见的输出结果。然而，语言模型在社会偏见场景中的底层行为仍未得到充分探索。

Method: 系统调查思维过程中的机制，发现两种驱动社会偏见聚合的失败模式：刻板印象重复和无关信息注入。基于这些发现，引入一种轻量级的基于提示的缓解方法，让模型根据这些特定失败模式审查自己的初始推理。

Result: 在问答（BBQ和StereoSet）和开放式（BOLD）基准测试上的实验表明，该方法有效减少了偏见，同时保持或提高了准确性。

Conclusion: 该研究揭示了语言模型推理过程中社会偏见聚合的机制，并提出了一种有效的缓解方法，为开发更公平的AI系统提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体协作在处理复杂任务时面临规划、协调和验证的新挑战，执行失败往往源于任务解释、输出格式或智能体间交接的细微偏差，而非推理缺陷本身。

Method: VeriMAP规划器分解任务、建模子任务依赖关系，并将规划器定义的通过标准编码为子任务验证函数（VFs），使用Python和自然语言实现。
...
TL;DR: DVAGen是一个完全开源的统一框架，用于训练、评估和可视化动态词汇增强语言模型，解决了现有方法代码库碎片化、缺乏现代LLM支持和推理可扩展性有限等问题。
...
Result: 验证了动态词汇方法在现代LLM上的有效性，展示了批量推理支持，显著提升了推理吞吐量。
...
TL;DR: 本文系统比较了基于提示和基于强化学习的查询增强方法，发现简单的无训练查询增强方法在强大LLM下表现与昂贵的RL方法相当甚至更好，并提出了结合两者优势的混合方法OPQE。


<details>
  <summary>Details</summary>
Motivation: 现有查询增强方法分为基于提示和基于强化学习两种，各有优缺点但缺乏系统比较，需要探索更有效的混合方法。

Method: 系统比较了提示方法和RL方法，并提出了OPQE混合方法，让LLM策略学习生成能最大化检索性能的伪文档。

Result: 无训练查询增强方法表现与RL方法相当甚至更好；OPQE方法优于单独的提示和RL方法。

Conclusion: 结合提示的灵活性和RL的针对性优化的协同方法能获得最佳结果。
...
TL;DR: 研究表明，人们对AI生成的讽刺言论不会完全采用意图立场，行为数据和神经反应都显示对AI的意图归因较弱，P200和P600效应减弱表明认知加工减少。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs越来越多地被部署为社交代理并训练产生幽默和讽刺，需要研究人们在遇到诙谐AI言论时是将其视为有意的交流还是纯粹的计算输出。

Method: 比较了AI与人类来源的讽刺陈述的行为和神经反应，使用ERP成分：P200反映早期不一致检测，P600索引重新解释不一致为故意讽刺的认知努力。

Result: 行为上，参与者对AI的不一致归因于故意交流的程度显著低于人类；神经数据显示AI生成讽刺的P200和P600效应减弱，表明检测和重新分析的努力减少。

Conclusion: 尽管当前LLMs具有语言复杂性，但实现真正的社交代理不仅需要语言能力，还需要人类对人工代理的意图归因方式发生转变。
...
TL;DR: 本文系统分析了基于分块稀疏注意力的长上下文语言模型，提出了三个关键设计原则：表达性分块编码器、旁路残差路径和强制选择稀疏性，实现了从4K到3200万token的无训练长度外推。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文处理方法存在局限性：标准Transformer有二次复杂度问题，滑动窗口注意力和状态空间模型无法有效利用完整上下文，而分块稀疏注意力的成功原理尚未被充分理解。

Method: 通过统一框架和全面消融研究，识别出三个关键设计原则：(1) 带有专用CLS令牌的表达性非线性分块编码器；(2) 旁路残差路径稳定集成全局信息；(3) 预训练期间的强制选择稀疏性。

Result: 结合这些原则，在RULER和BABILong基准上实现了从4K上下文训练到3200万token的无训练长度外推，建立了新的最先进水平。
...
TL;DR: 提出了Attention-Shifting（AS）框架，通过注意力机制实现选择性遗忘，在保持模型效用的同时减少幻觉响应，解决了现有遗忘方法在效用和可靠性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型应用的增加，其可能保留敏感数据的问题促使了机器遗忘研究。现有遗忘方法面临两难：激进遗忘会损害模型效用，保守策略则可能产生幻觉响应，限制了LLM在知识密集型应用中的可靠性。

Method: AS框架包含两个设计目标：上下文保持抑制（衰减对事实承载token的注意力而不破坏语言结构）和幻觉抵抗响应塑造（阻止对遗忘内容的虚构补全）。通过重要性感知抑制和注意力引导保留增强两种注意力级干预实现，采用双损失目标联合优化。

Result: 实验显示AS在ToFU基准测试上比现有最优方法准确率提高15%，在TDEC基准上提高10%，同时保持竞争力的无幻觉遗忘效果。

Conclusion: AS在遗忘效果、泛化能力和响应可靠性之间实现了优越的平衡，为LLM的选择性遗忘提供了有效解决方案。
...
TL;DR: 提出了StreamingThinker框架，让大语言模型能够在阅读输入的同时进行推理，而不是等待完整输入后再开始思考，显著降低了推理延迟。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理范式需要等待完整输入后才能开始思考，这带来了不必要的延迟，并且在动态场景下削弱了对早期信息的注意力。

Method: 设计了流式思考范式，通过流式CoT生成、流式约束训练和流式并行推理实现边读边想，使用流式推理单元、顺序保持机制和解耦的并行KV缓存。

Result: 在数学推理、逻辑推理和上下文QA任务上，StreamingThinker保持了与批量思考相当的性能，同时将推理开始前的token等待时间减少了80%，最终答案生成的时间级延迟减少了60%以上。

Conclusion: 流式思考范式为LLM推理提供了有效的解决方案，能够在保持性能的同时显著降低延迟。
...
TL;DR: 本文介绍了VideoBiasEval框架，用于评估视频生成中的社会偏见传播，发现对齐调优会强化并稳定化刻板印象。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型通过奖励模型对齐人类偏好来提升质量，但会无意中编码和放大社会偏见，需要系统评估这种偏见的演化过程。

Method: 提出VideoBiasEval诊断框架，基于社会偏见分类学，采用基于事件的提示策略分离语义内容和演员属性，并引入多粒度指标评估偏见。
...
TL;DR: 通过对30万条孟加拉语新闻标题和内容进行情感分析，研究发现负面情绪（愤怒、恐惧、失望）在新闻报道中占主导地位，不同媒体对相似事件的报道存在显著情感差异。


<details>
  <summary>Details</summary>
Motivation: 新闻媒体通过报道框架塑造公众情绪，负面或情绪化的标题往往吸引更多关注并传播更快，这促使媒体以引发强烈反应的方式构建新闻故事。

Method: 使用Gemma-3 4B模型进行零样本推理，分析30万条孟加拉语新闻标题及其内容，识别每篇报道的主导情绪和整体基调。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然性能优异，但其预测和内容生成机制对人类不可理解，且存在幻觉错误，迫切需要理解和解释模型内部工作机制以建立信任。

Method: 综述局部可解释性和机制可解释性方法，并在医疗和自动驾驶领域进行实验研究，分析解释对信任接收者的影响。

Result: 提供了相关研究的综述和实验分析，揭示了可解释性方法在关键领域中的应用效果和信任影响。

Conclusion: 总结了当前LLM可解释性领域未解决的问题，并提出了生成人类对齐、可信赖LLM解释的未来方向和关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动文献综述生成方法缺乏与人工专家创建的分类法结构比较，需要填补这一空白。

Method: 提出TaxoAlign方法，这是一个三阶段的基于主题的指令引导方法，用于学术分类法生成。同时创建CS-TaxoBench基准数据集，包含460个从人工撰写的综述论文中提取的分类法。

Result: TaxoAlign在几乎所有评估指标上都持续超越基线方法，在CS-TaxoBench基准上表现出色。

Conclusion: TaxoAlign方法能够有效弥合人工生成与自动创建分类法之间的差距，为自动文献综述生成提供了可靠的技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上的反社会行为（如仇恨言论、骚扰、网络霸凌）对平台安全和社福祉构成风险，但现有研究主要关注X和Reddit等网络，多轮对话环境因数据有限而研究不足。

Method: 使用CyberAgressionAdo-Large数据集，评估了6种基于文本和8种基于图的表示学习方法，分析词汇线索、交互动态及其多模态融合。

Result: 多模态模型优于单模态基线，mBERT + WD-SGCN融合模型在反社会行为检测上达到0.718，在同伴群体识别和霸凌分析上分别达到0.286和0.606。
...
TL;DR: 提出Nyx，一个统一的混合模态检索器，用于解决通用检索增强生成(URAG)问题，通过检索和推理多模态信息来提升视觉语言生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统主要关注单模态文本，但在现实场景中查询和文档可能包含混合模态（如文本和图像），需要解决混合模态信息的检索和推理挑战。

Method: 1) 构建NyxQA数据集：四阶段自动流水线生成和过滤混合模态问答对；2) 两阶段训练：先在NyxQA和开源检索数据集上预训练，然后使用下游视觉语言模型的反馈进行监督微调。

Result: Nyx不仅在标准文本RAG基准上表现有竞争力，在更通用和现实的URAG设置中表现优异，显著提升了视觉语言任务的生成质量。

Conclusion: Nyx成功解决了混合模态检索增强生成的挑战，为现实世界多模态信息需求提供了有效的解决方案。
...
TL;DR: 研究发现指令调优大语言模型在简单指令遵循方面存在不足，特别是在选项标签格式变化时表现不稳定，暴露了当前指令调优范式的缺陷。
...
Result: 标签格式变化导致性能大幅波动（如罗马数字vs数字下降30.45%）；无指令时性能进一步下降；移除选项内容后模型无法超越随机基线；少量样本无法提升鲁棒性；大模型准确率更高但指令遵循仍不一致。
...
TL;DR: 提出了EduAdapt基准，包含近48k个按年级标记的问答对，用于评估LLM在不同年级水平上的适应性表现。
...
Method: 创建了跨9个科学科目的年级标记问答数据集，评估了多种开源LLM在不同年级水平上的表现。
...
TL;DR: Ladder-base是首个基于GRPO强化学习方法训练的中医药专用大语言模型，在多项推理指标上超越了通用LLM和中医药专用模型。


<details>
  <summary>Details</summary>
Motivation: 传统中医药知识体系独特且复杂，现有中医药LLM存在对齐不足、数据质量差和评估不一致等问题，需要更有效的训练方法。

Method: 基于Qwen2.5-7B-Instruct基础模型，使用GRPO强化学习方法在TCM-Ladder基准的文本子集上进行训练，80%数据用于训练，20%用于验证和测试。

Result: Ladder-base在标准化评估中，在多项推理指标上超越了GPT-4、Gemini 2.5、Claude 3、Qwen3等通用LLM以及BenTsao、HuatuoGPT2、Zhongjing等中医药专用模型。

Conclusion: GRPO为在传统医学领域实现专家级推理的LLM对齐提供了有效策略，支持开发可信赖且临床基础扎实的中医药人工智能系统。
...
TL;DR: 提出了AfriCaption框架，为20种非洲语言提供多语言图像描述，包括数据集、动态处理流程和0.5B参数模型。


<details>
  <summary>Details</summary>
Motivation: 解决多模态AI研究主要集中在高资源语言的问题，促进该领域发展的民主化。

Method: 构建基于Flickr8k的语义对齐数据集，采用上下文感知选择和翻译过程；开发动态上下文保持流程，通过模型集成和自适应替换确保质量；设计AfriCaption模型，整合SigLIP和NLLB200进行图像到文本生成。

Result: 建立了首个可扩展的非洲低资源语言图像描述资源，确保数据质量持续改进。

Conclusion: 为真正包容性多模态AI奠定了基础，推动了非洲语言在多模态研究中的代表性。
...
Method: 采用自然语言指令调优方法，整合了1000多部经典和现代文本的知识库，构建基于场景的指令框架，实现可解释推理链模拟，并引入执业中医师的反馈精炼过程，连接外部API进行舌象分类和多模态数据库检索。

Result: 在单选题基准测试和多模态分类任务中，BenCao在诊断、草药识别和体质分类方面表现优于通用和中医领域模型，已在OpenAI GPTs Store部署，截至2025年10月有近1000名全球用户使用。
...
TL;DR: 论文发现对齐后训练不仅导致任务准确率下降，还会严重损失模型校准性，使模型过度自信、可靠性降低。通过简单的模型权重插值方法，可以找到帕累托最优的模型，在提高准确率的同时恢复对齐过程中损失的校准性。


<details>
  <summary>Details</summary>
Motivation: 传统上"对齐税"主要关注任务准确率的下降，但本文发现对齐过程还会导致严重的校准性损失，使模型变得过度自信和不可靠。

Method: 采用简单的后处理干预方法：在对齐前后的模型权重之间进行插值，寻找帕累托最优的插值点。

Result: 该方法能够找到既提高准确率又显著恢复校准性的模型，超越了原始对齐前后的两个父模型。
...
<details>
  <summary>Details</summary>
Motivation: 研究RL训练的搜索模型的安全特性，发现这些模型虽然擅长多步推理任务，但其安全属性尚未得到充分理解。

Method: 使用两种简单攻击方法：搜索攻击（强制模型以搜索开始响应）和多搜索攻击（鼓励模型重复搜索），在Qwen和Llama两个模型系列上测试本地和网络搜索。

Result: 攻击使拒绝率降低高达60.0%，回答安全性降低82.5%，搜索查询安全性降低82.4%。攻击通过触发模型在生成继承的拒绝标记之前生成有害的、反映请求的搜索查询而成功。

Conclusion: 当前RL训练存在核心弱点：奖励生成有效查询而不考虑其有害性。RL搜索模型存在用户可轻易利用的漏洞，迫切需要开发安全感知的代理RL管道来优化安全搜索。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录数据快速增长，需要从非结构化临床文本中提取生物医学知识来支持临床系统发展。虽然英语临床文本的命名实体识别已有较好研究，但低资源语言的临床文本研究仍然稀缺。

Method: 探索了多种单语和多语言BERT模型在心脏病学领域临床命名实体识别中的应用，这些模型在通用领域文本上预训练，用于从英语、西班牙语和意大利语的临床病例报告中提取疾病和药物实体。

Result: 在西班牙语疾病识别(SDR)上F1得分为77.88%，西班牙语药物识别(SMR)为92.09%，英语药物识别(EMR)为91.74%，意大利语药物识别(IMR)为88.9%，均超过了测试排行榜的平均值和中位数。
...
TL;DR: 该论文首次为乌尔都语到英语的习语翻译创建了评估数据集，并评估了多种LLM和NMT系统在习语翻译任务上的表现，发现提示工程能提升翻译质量，且原生乌尔都语输入的翻译效果优于罗马化乌尔都语。
...
Result: 提示工程相比直接翻译能提升习语翻译质量，但不同提示类型间的性能差异较小；原生乌尔都语输入的习语翻译准确性显著高于罗马化乌尔都语。

Conclusion: 文本表示形式对翻译质量有显著影响，原生乌尔都语在习语翻译中表现更优，提示工程是提升习语翻译的有效方法。
...
TL;DR: 该研究系统分析了多语言大语言模型在医疗问答中的跨语言事实对齐差异，发现模型回答更倾向于英文维基百科，即使提示为非英语，但通过提供非英语上下文可以有效改善事实对齐。
...
Method: 构建多语言维基医疗数据集，分析跨语言医疗覆盖率，评估LLM回答与参考资料的匹配度，并通过上下文信息和检索增强生成进行案例研究。

Result: 发现维基百科覆盖率和LLM事实对齐存在显著跨语言差异，模型回答更偏向英文维基百科，但提供非英语上下文能有效转向文化相关知识。

Conclusion: 研究结果为构建更公平的多语言医疗AI系统提供了实用路径，强调通过上下文信息可以改善事实对齐的跨语言差异。
...
TL;DR: ReXMoE是一种新颖的MoE架构，通过跨层重用专家来改进路由机制，突破了传统层局部路由的限制，在固定参数预算下提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统MoE架构的层局部路由机制限制了专家维度和路由多样性之间的权衡，需要新的设计来丰富专家组合而不牺牲专家容量或增加总体参数。

Method: 提出了ReXMoE架构，允许路由器在相邻层间重用专家，并设计了渐进式扩展路由(PSR)策略来逐步增加训练期间的候选专家池。

Result: 在0.5B到7B参数规模的不同架构模型上进行广泛实验，ReXMoE在固定架构维度下持续提升了语言建模和下游任务性能。

Conclusion: ReXMoE为参数高效且可扩展的MoE基LLMs提供了新的设计范式，能够在不增加总体参数的情况下实现更好的性能。
...
TL;DR: 提出DETree方法，通过层次亲和树结构建模不同AI-人类协作文本生成过程的关系，并开发RealBench基准数据集，在混合文本检测任务中提升性能，特别是在少样本学习和分布外场景下表现出更强的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI文本检测方法主要采用二元分类（纯人类vs AI参与）或多分类（将人机协作视为新类别），但这些方法对复杂的人机协作过程建模过于粗糙。不同协作过程生成的文本具有内在的聚类关系，需要更精细的建模方法。

Method: 提出DETree方法，将不同文本生成过程的关系建模为层次亲和树结构，并设计专门的损失函数使文本表示与该树对齐。同时开发RealBench基准数据集，自动包含各种人机协作过程产生的混合文本。

Result: DETree方法在混合文本检测任务中提升了性能，特别是在少样本学习和分布外场景下显著增强了鲁棒性和泛化能力，证明了基于训练的方法在OOD设置中的潜力。

Conclusion: 通过层次亲和树结构建模不同文本生成过程的关系，能够有效提升AI文本检测的性能和鲁棒性，特别是在复杂的人机协作场景下。该方法为应对日益多样化的AI文本生成过程提供了有效的解决方案。
...
TL;DR: 本文系统综述了基于大语言模型的行业智能体技术、应用和评估方法，提出了行业智能体能力成熟度框架，分析了记忆、规划和工具使用三大技术支柱，并探讨了行业应用场景和未来发展方向。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，能够自主推理、规划和执行复杂任务的智能体成为AI前沿，但如何将通用智能体研究转化为推动行业变革的生产力仍面临重大挑战。

Method: 使用行业智能体能力成熟度框架，分析智能体在行业应用中的演进路径，从"流程执行系统"到"自适应社会系统"，并系统回顾支撑智能体能力发展的三大技术支柱：记忆、规划和工具使用。

Result: 概述了行业智能体在数字工程、科学发现、具身智能、协同业务执行和复杂系统仿真等领域的实际应用，并分析了现有评估系统在真实性、安全性和行业特性方面面临的挑战。

Conclusion: 通过结合技术演进与行业实践，本文阐明了行业智能体的现状，为理解和构建下一代行业智能体提供了清晰的路线图和理论基础，同时探讨了实际挑战、能力边界、发展潜力和治理问题。
...
TL;DR: 提出Deep Self-Evolving Reasoning (DSER)框架，通过概率化迭代推理和并行处理，显著提升小规模开源模型在困难任务上的推理能力，即使验证和修正能力较弱。


<details>
  <summary>Details</summary>
Motivation: 当前开源小模型在长链推理任务中验证和修正能力脆弱，限制了其解决奥林匹克级别问题的能力。需要一种方法能够在弱验证能力下仍能有效扩展推理极限。

Method: 将迭代推理建模为马尔可夫链，每个步骤代表解空间的随机转移。通过并行运行多个自演化过程，放大微小改进概率，使模型渐进接近正确答案。

Result: 在DeepSeek-R1-0528-Qwen3-8B模型上应用DSER，在AIME 2024-2025基准测试中解决了9个先前无法解决的问题中的5个，整体性能提升，使这个紧凑模型通过多数投票超越了其600B参数教师的单轮准确率。

Conclusion: DSER不仅提供了测试时扩展的实用价值，更重要的是为诊断当前开源推理器的根本限制提供了框架，明确了在自验证、精化和稳定性方面的不足，为开发具有强大内在自演化能力的下一代模型指明了研究方向。
...
TL;DR: 该论文系统研究了多语言句子嵌入学习方法，结合了单语和跨语言表示的最佳方法，显著减少了平行训练数据需求，在112种语言上实现了83.7%的双文本检索准确率，并发布了109+语言的预训练模型。


<details>
  <summary>Details</summary>
Motivation: 虽然BERT在单语句子嵌入学习方面很有效，但基于BERT的跨语言句子嵌入尚未得到充分探索。论文旨在系统研究多语言句子嵌入学习方法，提升跨语言语义表示能力。

Method: 结合了掩码语言建模(MLM)、翻译语言建模(TLM)、双编码器翻译排序和加性边缘softmax等最佳方法，利用预训练多语言语言模型大幅减少平行数据需求。

Result: 将所需平行训练数据减少80%，在Tatoeba数据集112种语言上达到83.7%的双文本检索准确率，远超LASER的65.5%，同时在单语迁移学习基准上保持竞争力。

Conclusion: 通过组合最佳方法成功开发出高效的多语言句子嵌入模型，显著提升了跨语言检索性能，并证明其挖掘的平行数据可用于训练有竞争力的神经机器翻译模型。
...
TL;DR: 提出了EliCal框架，通过两阶段方法实现LLM的诚实对齐：先使用廉价的自一致性监督获取内部置信度，再用少量正确性标注进行校准，仅需1k标注就能达到接近最优的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有诚实对齐方法要么依赖无训练的置信度估计，要么需要大量正确性标注进行训练校准。实现通用的诚实对齐需要昂贵的大规模标注，因此需要开发标注高效的训练方法。

Method: EliCal两阶段框架：1）使用廉价的自一致性监督获取内部置信度；2）用少量正确性标注校准置信度。同时发布了HonestyBench基准，包含10个自由形式QA数据集，56万训练和7万评估实例。
...
TL;DR: SimBench是首个大规模标准化基准，用于评估LLM模拟人类行为的能力，发现当前最佳LLM模拟能力有限(40.8/100)，性能随模型规模对数线性增长，存在对齐-模拟权衡，且在模拟特定人口群体时表现较差。


<details>
  <summary>Details</summary>
Motivation: 当前LLM模拟人类行为的评估方法零散且不可比较，需要建立标准化基准来推动LLM模拟能力的科学发展。

Method: 引入SimBench基准，整合20个多样化数据集，涵盖道德决策、经济选择等任务，基于大规模全球参与者数据。

Result: 当前最佳LLM模拟能力得分仅40.8/100；性能随模型规模对数线性增长；推理时计算增加不会提升模拟性能；存在对齐-模拟权衡；模型在模拟特定人口群体时表现不佳；模拟能力与深度知识推理强相关(r=0.939)。

Conclusion: SimBench为LLM模拟研究提供了可测量的进展标准，将加速开发更忠实的人类行为模拟器。
...
TL;DR: 提出了一个统一的多任务学习框架，将自回归LLM与临床推理对齐，在MSK-CHORD数据集上进行癌症治疗结果预测，通过CoT提示和GRPO强化学习方法显著提升了预测性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在生物医学NLP中表现良好，但缺乏对高风险决策支持至关重要的结构化推理能力，特别是在处理异质性临床数据时。

Method: 开发了统一的多任务学习框架，训练模型同时执行二元生存分类、连续生存时间回归和自然语言理由生成。评估了三种对齐策略：标准监督微调、带CoT提示的监督微调、以及GRPO强化学习方法。

Result: 实验显示CoT提示将F1提高了6.0%，MAE降低了12%，而GRPO在BLEU、ROUGE和BERTScore指标上实现了最先进的可解释性和预测性能。
...
TL;DR: 提出使用拓扑数据分析工具Mapper来可视化语言模型如何内部表示歧义，发现微调后的模型在嵌入空间中形成模块化、非凸的决策区域，即使对于高度歧义的情况也能保持高预测纯度。


<details>
  <summary>Details</summary>
Motivation: 传统的标量评估指标（如准确率）无法捕捉模型内部如何表示歧义，特别是在人类标注者存在分歧的情况下。需要新的方法来理解模型如何处理主观性NLP任务中的歧义。

Method: 使用拓扑数据分析工具Mapper分析微调后的RoBERTa-Large模型在MD-Offense数据集上的嵌入空间结构，与传统可视化工具（PCA、UMAP）进行对比。

Result: 微调将嵌入空间重组为模块化、非凸的区域，与模型预测对齐。超过98%的连接组件表现出≥90%的预测纯度，但在歧义数据中与真实标签的对齐度下降，揭示了结构置信度与标签不确定性之间的隐藏张力。

Conclusion: Mapper是一个强大的诊断工具，能够直接揭示决策区域、边界塌陷和过度自信的聚类，为理解模型如何解决歧义提供了新的视角，并为主观NLP任务中的主动建模策略提供了拓扑度量依据。
...
TL;DR: 提出语言混淆门（LCG），一种轻量级插件方法，通过解码时过滤令牌来解决LLM的语言混淆问题，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 当前解决方案要么需要模型重新训练，要么无法区分有害的语言混淆和可接受的语码转换。

Method: 使用规范调整自蒸馏训练LCG来预测适当的语言家族，仅在需要时应用掩码，基于语言混淆不频繁、正确语言令牌通常位于预测顶部等发现。

Result: 在各种模型上评估，LCG显著减少语言混淆，通常降低一个数量级，且不影响任务性能。

Conclusion: LCG是解决LLM语言混淆问题的有效轻量级解决方案。
...
Method: 设计了三种代码标记的高阶相关性（抽象语法树家族相关性、词汇相关性和行相关性），构建了标记和超边生成器，并改进了超图神经网络架构，结合适配器调优提出HGAdapter。

Result: 在多个公共数据集上的实验表明，该方法在不同程度上提升了预训练语言模型在代码摘要和代码克隆检测任务中的性能。

Conclusion: 引入高阶数据相关性有助于提高代码相关任务的效果，HGAdapter可以灵活插入各种预训练语言模型中增强性能。
...
TL;DR: 提出了LawChain框架来显式建模侵权法律推理，构建了评估基准LawChain_eval，并验证了通过提示或后训练显式融入法律推理链能显著提升语言模型的法律推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有法律推理计算方法主要依赖通用推理框架，未能全面考察法律推理的细微过程，且研究多集中于刑事案件，对民事案件的建模不足。

Method: 将侵权分析中的法律推理过程操作化为LawChain框架，包含三个模块，每个模块由多个细粒度子步骤组成。构建评估基准LawChain_eval，评估大语言模型在侵权法律推理中的能力，并引入通过提示或后训练显式融入LawChain推理的基线方法。

Result: 当前模型在处理侵权法律推理的关键要素方面仍存在不足。提出的基线方法在侵权相关法律推理中取得显著改进，并能很好地泛化到相关法律分析任务。

Conclusion: 显式建模法律推理链能有效增强语言模型的推理能力，LawChain框架在法律分析任务中具有良好的通用性。
...
TL;DR: 论文提出了一种改进遗忘学习的方法，通过在遗忘目标中加入上下文效用保持项，使模型在移除特定知识后仍能在提示中重新引入该知识时有效利用它。


<details>
  <summary>Details</summary>
Motivation: 现有的遗忘学习方法评估主要关注目标知识的遗忘程度和保留集的性能保持，但忽视了用户可能希望在提示中重新引入已移除知识时模型仍能利用它的重要使用场景。

Method: 在六种最先进的遗忘学习方法基础上，增加一个插件式的目标项，专门用于保持模型在上下文提示中出现已遗忘知识时利用该知识的能力。

Result: 实验表明该方法能在保持有效遗忘和保留集效用的同时，将上下文效用恢复到接近原始模型水平。

Conclusion: 该研究强调了在遗忘学习中考虑上下文效用的重要性，提出的方法能显著改善遗忘学习方法的实际可用性。
...
TL;DR: Qomhr'a是一个在低资源条件下开发的爱尔兰语-英语双语大语言模型，通过双语持续预训练、指令微调和人类偏好对齐的完整流程构建，在爱尔兰语和英语任务上分别取得了29%和44%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 在低资源条件下开发爱尔兰语的大语言模型，提升爱尔兰语的文本生成能力，同时保持英语能力，解决爱尔兰语资源稀缺的问题。

Method: 使用新获取的爱尔兰语语料库和英语文本混合训练，通过双语持续预训练、指令微调和人类偏好对齐的完整流程，利用Gemini-2.5-Pro合成指令微调和人类偏好数据集。

Result: 开发了6个闭源权重LLM，其中Gemini-2.5-Pro被评为最佳模型；贡献了两个数据集：3万条爱尔兰语-英语平行指令微调数据集和1千条人类偏好数据集；在翻译、性别理解、主题识别和世界知识等基准测试中，爱尔兰语性能提升29%，英语性能提升44%。

Conclusion: Qomhr'a模型在低资源条件下成功开发，显著提升了爱尔兰语的文本生成能力，同时保持了英语能力，为低资源语言的大语言模型开发提供了有效的方法论。
...
TL;DR: 本文提出了一种对话分析方法来评估教育应用中LLM的有效性，关注学习者与LLM的互动策略而非仅技术性能。


<details>
  <summary>Details</summary>
Motivation: 现有教育应用评估方法主要关注技术性能或学习成果，忽视了学习者与LLM之间的互动质量，需要填补这一研究空白。
...
TL;DR: 提出了QueST框架，通过难度感知图采样和拒绝微调来生成具有挑战性的编程问题，显著提升了语言模型在竞争性编程任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有竞争性编程数据集规模有限（仅数千到数万个问题），且缺乏大规模、具有挑战性的编程问题训练数据，限制了语言模型在推理任务上的可扩展性。

Method: 结合难度感知图采样和难度感知拒绝微调，直接优化专用生成器来创建具有挑战性的编程问题，然后通过蒸馏或强化学习来训练模型。

Result: 使用QueST生成的100K难题微调Qwen3-8B-base后，在LiveCodeBench上超越了原始Qwen3-8B；额外使用28K人工问题配对的合成解决方案后，8B模型性能与DeepSeek-R1-671B相当。

Conclusion: 通过QueST生成复杂问题为推进大型语言模型在竞争性编程和推理方面的前沿提供了一种有效且可扩展的方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源场景下命名实体识别任务标注数据不足的问题，传统零样本和指令调优方法在领域特定实体上泛化能力差且无法有效利用有限数据。

Method: 1) 设计新的指令调优模板，简化输出格式以利用先进大语言模型的大上下文窗口；2) 引入战略性数据增强技术，在保持实体信息的同时对上下文进行改写，扩展训练数据而不损害语义关系。

Result: 在基准数据集上，少样本方法在CrossNER数据集上平均F1得分为80.1，使用改写方法训练的模型比基线版本F1分数提升最高达17分。
...
TL;DR: 提出了AcademicEval，一个用于评估LLM在长上下文生成任务上的动态基准，使用arXiv论文构建学术写作任务，无需人工标注，确保无标签泄露。


<details>
  <summary>Details</summary>
Motivation: 当前长上下文LLM基准存在上下文长度固定、标注劳动密集、训练中标签泄露等问题，需要更有效的评估方法。

Method: 利用arXiv论文构建学术写作任务（标题、摘要、引言、相关工作），集成高质量专家策划的少样本演示，支持灵活上下文长度，实现高效动态评估。

Result: LLM在具有层次抽象级别的任务上表现不佳，难以处理长少样本演示，突显了基准的挑战性。

Conclusion: AcademicEval为评估LLM长上下文建模能力提供了有效基准，揭示了增强LLM长上下文建模能力的见解。
...
TL;DR: 提出了一种基于二元检索增强奖励的在线强化学习方法，用于减少语言模型的外源性幻觉，在保持其他任务性能的同时显著提升了事实准确性。


<details>
  <summary>Details</summary>
Motivation: 语言模型经常生成训练数据不支持的事实错误信息（外源性幻觉），现有缓解方法往往会在开放生成和下游任务上导致性能下降，限制了实际应用价值。

Method: 使用二元检索增强奖励的在线强化学习方法，只有当模型输出完全正确时才给予奖励1，否则为0，与连续奖励方案不同。

Result: 在开放生成任务中，二元RAR将幻觉率降低了39.3%；在短问答任务中，模型学会了校准性弃权，在PopQA和GPQA上分别减少了44.4%和21.7%的错误答案；同时没有在指令遵循、数学或代码任务上出现性能下降。

Conclusion: 二元检索增强奖励方法在提升事实准确性的同时避免了其他任务性能的退化，而连续奖励的强化学习虽然也能改善事实性，但会导致质量回归。
...
TL;DR: 该调查通过自主性级别(L0-L3)重新构建医学大语言模型评估框架，将现有基准与各层级允许的操作和风险对齐，提出基于层级的评估蓝图，推动从分数导向转向可信的临床使用证据。


<details>
  <summary>Details</summary>
Motivation: 医学大语言模型在标准基准测试中表现优异，但这些结果如何转化为临床工作流程中安全可靠的性能仍具挑战性，需要更系统的评估方法。

Method: 采用自主性级别框架(L0-L3)，涵盖信息工具、信息转换与聚合、决策支持和监督代理，将现有基准和指标与各层级的允许操作和风险进行对齐。

Result: 提出了基于层级的评估蓝图，用于选择指标、收集证据和报告声明，将评估与监管联系起来，使评估目标更加明确。

Conclusion: 通过以自主性为中心，该调查推动领域从基于分数的声明转向为真实临床使用提供可信、风险感知的证据。
...
TL;DR: 本文提出了FARE（基础自动推理评估器），通过大规模数据驱动的监督微调方法训练了8B和20B参数的评估器，在多个评估任务中超越了更大规模的专门评估器。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注新方法如强化学习，而忽视了大规模数据驱动的开发。本文旨在通过数据扩展来解决可扩展评估的需求。

Method: 收集了250万样本，涵盖5种评估任务，使用简单的迭代拒绝采样监督微调方法训练FARE评估器家族。

Result: FARE-8B挑战了更大的专门RL训练评估器，FARE-20B在开源评估器中设定了新标准，超越了专门的70B+评估器。在真实任务中表现出色。

Conclusion: 大规模数据驱动的监督微调方法能够训练出高效的评估器，在多个领域和应用场景中表现优异。
...
TL;DR: 提出了可执行知识图谱(xKG)，通过整合技术见解、代码片段和领域知识来解决AI研究复现的挑战，在多个代理框架中显著提升了性能
...
Method: 构建模块化、可插拔的知识图谱，自动从科学文献中提取技术见解、代码片段和领域特定知识

Result: 在三个代理框架和两种不同LLM上，xKG在PaperBench上显示出显著性能提升（o3-mini提升10.9%）
...
TL;DR: EDR是一个多智能体系统，通过主规划智能体和四个专业搜索智能体，结合可扩展的工具生态系统和可视化智能体，实现企业深度研究自动化，在开放基准测试中优于现有系统。
...
Method: 采用多智能体架构：主规划智能体进行自适应查询分解，四个专业搜索智能体（通用、学术、GitHub、LinkedIn），基于MCP的可扩展工具生态系统，可视化智能体和反思机制。
...
TL;DR: 提出基于草图的两阶段烟雾插图生成框架，使用流函数和潜在扩散模型，通过草图引导流函数生成，进而控制速度场和烟雾模拟。


<details>
  <summary>Details</summary>
Motivation: 解决草图到烟雾模拟的转换问题，捕捉连续变化和旋转流动细节，使烟雾模拟与预期流动对齐。

Method: 两阶段框架：第一阶段用草图引导流函数生成，第二阶段用流函数作为控制条件生成速度场，采用流线编码全局流动动力学作为训练时的草图指导。

Result: 流函数作为中间表示能够捕捉草图中缺失的连续变化和旋转流动细节，生成的烟雾模拟能够与预期流动对齐。

Conclusion: 该框架成功实现了从用户草图到烟雾插图的生成，通过流函数有效捕捉了流动细节，使烟雾模拟符合预期流动模式。
...
TL;DR: 提出基于运动草图引导的流体动画框架，通过微调的条件潜在扩散模型生成运动场，结合运动适配器控制流体运动
...
Result: 框架能够生成高质量的景观视频，实现平滑且时间一致的流体动画效果

Conclusion: 该方法成功解决了流体动画中的运动控制问题，为静态图像添加动态流体元素提供了有效解决方案
...
TL;DR: 提出了一种自适应无帧渲染方法，通过闭环反馈采样和GPU重建，在静态场景中实现更锐利图像，在动态场景中保持实时性，相比传统渲染方法可减少一个数量级的采样数量。


<details>
  <summary>Details</summary>
Motivation: 传统交互式渲染方法存在刚性采样模式限制，无法根据时空颜色变化进行细粒度自适应采样和重建，限制了渲染效率。

Method: 使用闭环反馈机制引导采样朝向图像边缘或运动区域，采用时间深度缓冲区存储短期样本，GPU重建同时考虑采样密度和时空颜色梯度，在静态场景中加权旧样本，动态场景中强调新样本。

Result: 模拟实验显示，相比传统渲染方法，无帧渲染器在保持相似视觉质量（RMS误差衡量）的情况下，采样数量减少一个数量级，计算开销仅增加15%。

Conclusion: 自适应无帧渲染方法通过灵活采样和重建策略，显著提高了渲染效率，在静态和动态场景中都能提供高质量的视觉体验。
...
TL;DR: 提出一种程序化生成城市土地利用模式的方法，自动为艺术家放置建筑和道路


<details>
  <summary>Details</summary>
Motivation: 城市在数字制作中是重要内容元素，但其复杂性和规模使得建模非常困难。现有工具很少能帮助艺术家完成这项工作，同时图形硬件的快速改进要求更丰富的内容而不增加制作成本

Method: 程序化生成城市中逼真的土地利用模式，自动化放置建筑和道路

Result: 该方法能够为艺术家提供城市建模的自动化工具

Conclusion: 提出的程序化生成方法可以有效解决城市建模的挑战，满足对丰富内容的需求同时控制制作成本
...
TL;DR: 提出了一种从3D无组织多边形数据中提取层次树表示的方法，通过图表示、Steiner树生成和视线约束优化，直接基于表面操作，适用于导航感知的几何分析。


<details>
  <summary>Details</summary>
Motivation: 传统骨架化方法通常假设体积解释，而该方法直接在表面操作，确保结果表示对导航感知的几何分析保持相关性。

Method: 首先提取表面的图表示，然后生成Steiner树连接关键终端点，最后利用视线约束优化结构，减少冗余同时保持基本连通性。

Result: 验证了两个用例：从基于瓦片的元素中提取结构表示用于程序化内容生成，以及识别关键点和结构度量用于自动关卡分析。结果显示了生成简化、连贯表示的能力。

Conclusion: 该方法能够产生简化的连贯表示，支持程序化生成、空间推理和地图分析等应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决从开放词汇文本描述合成3D场景的挑战，特别是布局生成问题。现有声明式方法使用LLM生成约束规范再求解，但存在局限性。

Method: 采用指令式范式，让LLM迭代放置物体，每个物体的位置和方向基于先前放置的物体计算。开发错误修正机制来改进场景有效性。

Result: 在强制选择感知研究中，参与者82%和94%的时间偏好本文的指令式方法，相比两种声明式布局生成方法。还提出了与人类偏好一致的自动评估指标。

Conclusion: 指令式方法比声明式方法更简单且能处理更复杂场景，错误修正机制提高了鲁棒性，人类评估显示明显偏好指令式方法。
...
TL;DR: 本文提出了一种针对持久性图和合并树的广义Wasserstein距离，通过利用拓扑特征在输入域中的区域信息，提供比经典Wasserstein距离更具区分性的度量。


<details>
  <summary>Details</summary>
Motivation: 传统Wasserstein距离在比较拓扑特征时未充分利用特征在输入域中的区域信息，需要一种更精确的度量方法来更好地捕捉拓扑特征的差异。

Method: 重新定义拓扑特征比较为极值对齐区域值之间的距离，引入参数调整区域属性在距离中的影响，并提供两种策略控制计算时间和内存使用。

Result: 在公开数据集上的实验表明方法高效（平均运行时间在分钟级别），能够跟踪时变集合中拓扑特征的演化，并用于降维和关键阶段检测。

Conclusion: 该方法为持久性图和合并树提供了更精确的距离度量，在拓扑数据分析中具有实际应用价值，包括特征跟踪和集合可视化。
...
TL;DR: 该论文研究了如何通过简单的预过滤方法去除等值面中的微小干扰组件，以改善医学图像数据的可视化效果。


<details>
  <summary>Details</summary>
Motivation: 从CT扫描或MRI等扫描数据构建的等值面通常包含大量微小组件，这些组件会分散可视化注意力，且不构成任何几何模型的一部分。

Method: 使用简单的数据预过滤方法，在保留大型组件的同时去除微小组件。

Result: 实验结果表明该方法能有效去除等值面中的微小干扰组件。

Conclusion: 简单的预过滤方法可以在不影响主要可视化组件的情况下有效去除等值面中的微小干扰组件。
...
TL;DR: SeerX是一个轻量级、可扩展的传输中就地服务，支持动态资源分配和3D模拟数据的有损压缩，允许多个模拟将分析任务卸载到共享的弹性服务基础设施中，无需MPI同步。


<details>
  <summary>Details</summary>
Motivation: 随着模拟产生的数据超过超级计算机的磁盘空间，许多模拟采用就地分析和可视化来减少需要存储的数据量。但就地可视化的效果受到先验知识需求的限制：需要知道可视化参数来突出感兴趣的特征，以及无法提前知道运行就地工作流所需的资源量。

Method: 提出SeerX，一个轻量级、可扩展的传输中就地服务，支持动态资源分配和3D模拟数据的有损压缩。SeerX使多个模拟能够将分析任务卸载到共享的弹性服务基础设施中，无需MPI同步。

Result: SeerX解决了就地分析中的资源分配不确定性和参数设置问题，通过弹性服务基础设施实现了高效的传输中数据处理。

Conclusion: SeerX为大规模模拟提供了有效的就地分析解决方案，通过动态资源分配和共享服务基础设施，克服了传统就地可视化方法的局限性。
...
TL;DR: SAIP是首个考虑身体形状差异的稀疏惯性运动捕捉方法，通过分解传感器测量中的形状和姿态信息，解决了现有方法难以泛化到不同体型（如儿童）的问题。


<details>
  <summary>Details</summary>
Motivation: 现有惯性运动捕捉方法主要依赖标准成人身体模型，难以泛化到体型差异较大的个体（如儿童），因为身体形状变化会影响IMU测量的加速度。

Method: 1. 训练回归模型将真实身体的IMU加速度转换到标准成人身体模型；2. 使用现有方法估计标准体型身体的全身运动；3. 通过第二个回归模型将关节速度映射回真实身体，结合形状感知的物理优化策略计算全局运动。

Result: 创建了首个包含不同体型个体的IMU运动捕捉数据集（10名儿童和10名成人，身高110-190cm），实验证明SAIP能有效处理不同体型的运动捕捉任务。

Conclusion: SAIP通过形状感知方法成功解决了惯性运动捕捉中的体型泛化问题，为不同体型个体的运动捕捉提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法对多模态越狱攻击脆弱，因为视觉输入引入了新的攻击面，推理链缺乏安全监督，且模态融合往往导致对齐效果下降。
...
TL;DR: 本文提出了结构化认知循环（SCL）作为可执行的认知框架，将哲学见解转化为可计算结构，重新定义智能为通过意向性理解重建自身认知状态的能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型缺乏真正的认知理解，暴露了认知架构的缺失。传统AI研究关注"智能是什么"的本体论问题，而SCL关注"在什么条件下认知涌现"的认知论问题。

Method: 基于心智哲学和认知现象学，结合过程哲学、具身认知和扩展心智理论，将智能定义为执行过程——判断、记忆、控制、行动和调节的连续循环。

Result: SCL将哲学见解操作化为可计算结构，支持"可执行认知论"；功能分离的认知架构比单一提示系统产生更一致和可解释的行为；重新定义智能为重建自身认知状态的能力。

Conclusion: SCL框架影响心智哲学、认知论和AI领域，强调真正的进步需要实现认知原则的结构化架构，而非更大的模型。
...
<details>
  <summary>Details</summary>
Motivation: citiverses具有通过沉浸式虚拟环境支持监管学习的潜力，为政策场景和技术实验提供创新平台。

Method: 基于与高级专家小组（包括欧盟委员会政策制定者、国家政府科学顾问和数字监管领域领先研究人员）的咨询，确定关键研究领域和实验主题。

Result: 识别了包括可扩展性、实时反馈、复杂性建模、跨境合作、风险降低、公民参与、伦理考量和新兴技术整合在内的关键研究领域，并分析了可在citiverse平台测试的实验主题。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI代理记忆系统缺乏对多样化任务的适应性，且忽视了记忆的建构性和任务导向作用。受皮亚杰认知发展理论启发，需要开发一个更实用的记忆系统。

Method: 提出PISA系统，包含三模态适应机制（图式更新、图式演化和图式创建）来保持连贯组织并支持灵活记忆更新，以及结合符号推理和神经检索的混合记忆访问架构。
...
TL;DR: 该研究发现大型推理模型在解决复杂度超过特定阈值的谜题时会出现性能崩溃，即使提供环境接口让模型能够跟踪状态空间，也无法延迟或消除这种崩溃。


<details>
  <summary>Details</summary>
Motivation: 探讨大型推理模型在解决复杂谜题时性能崩溃的真正原因，验证是否由于模型需要自行跟踪状态空间而导致评估失真。

Method: 为大型语言模型提供汉诺塔问题的环境接口，使其能够通过工具调用进行移动、提供书面理由、观察结果状态空间，并自我提示进行下一步移动。

Result: 即使有环境接口，性能崩溃仍然发生，模型在复杂度增加时与最优策略和随机策略的偏离度都在增加，表现出模式崩溃现象。

Conclusion: 大型推理模型的性能崩溃不是由于状态跟踪问题，而是模型在复杂度增加时表现出模式崩溃，性能取决于该模式是否反映问题的正确解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 受人类认知负荷理论启发，旨在开发一个符号化、时间变化的框架来理解深度模型在推理过程中的内部资源分配模式。
...
Conclusion: CLTs提供了一个有效的可解释性框架，能够深入理解深度模型的推理动态，并实现基于认知负荷的优化干预。
...
Result: 实验结果表明，ProofFlow在自动形式化方面达到了新的最先进水平，ProofScore得分为0.545，显著超过全证明形式化(0.123)和步骤证明形式化(0.072)等基线方法。

Conclusion: ProofFlow通过关注结构保真度，在证明自动形式化任务上取得了显著改进，为将大型语言模型集成到严谨数学工作流程中提供了更好的解决方案。
...
TL;DR: 提出将MO|RE运动科学研究数据仓库转换为知识图谱的愿景，通过基于基本形式本体的本体论，标准化和机器可理解地表示运动表现数据。
...
Method: 基于基本形式本体(BFO)构建本体，正式表示计划规范、具体过程和相关测量之间的相互关系，将MO|RE数据转换为知识图谱。

Result: 提出了一个将运动表现研究数据建模为知识图谱的框架，使数据标准化且机器可理解。

Conclusion: 知识图谱方法能够改变运动表现数据的建模和共享方式，促进跨研究的标准化数据交换和理解。
...
<details>
  <summary>Details</summary>
Motivation: 在涉及顺序信息的不确定性推理中，如何度量由置换质量函数表示的两个证据之间的冲突是一个紧迫的研究课题。

Method: 从置换观察出发，定义基于秩偏重叠(RBO)的置换间不一致性度量，并提出基于非重叠的RPS冲突度量方法，将RPS理论视为DST的扩展。

Result: 通过数值示例验证了所提出冲突度量的行为和性质，该方法具有自然的顶部加权特性，并能从DST视角有效度量RPS间的冲突。

Conclusion: 所提出的方法不仅具有顶部加权特性，能有效度量RPS冲突，还为决策者提供了权重、参数和截断深度的灵活选择。
...
TL;DR: PAINT是一种并行时间神经孪生方法，通过生成式神经网络在时间维度上并行建模状态分布，能够从稀疏测量中准确预测系统状态并保持轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有神经代理模型虽然能实时模拟动态系统，但缺乏创建真实系统数字孪生的能力。神经孪生需要在测试时根据测量更新状态，并保持与真实系统轨迹的一致性。

Method: PAINT训练生成式神经网络在时间维度上并行建模状态分布，测试时通过滑动窗口方式从测量数据预测状态。该方法架构无关，适用于各种动态系统建模。

Result: 在二维湍流流体动力学问题上的实验表明，PAINT能够保持轨迹跟踪，从稀疏测量中高保真地预测系统状态。理论分析证明PAINT具有轨迹跟踪能力，而自回归模型通常不具备。

Conclusion: PAINT为开发能够保持轨迹跟踪的神经孪生提供了潜力，能够实现更准确的状态估计和决策制定。
...
<details>
  <summary>Details</summary>
Motivation: 现有迁移故障诊断方法通常假设数据干净或域相似性足够，但在工业环境中严重噪声干扰和域偏移共存，限制了其有效性

Method: 基于信息分离架构，集成对抗学习和改进的正交损失来解耦域不变故障表示；采用全局-局部域对抗方案约束模型的联合分布和边缘分布
...
TL;DR: 该论文提出了一种结合离线约束规划优化和在线时间网络执行的方法，为具有随机任务持续时间的制造系统创建在不确定性下仍可行的调度方案。
...
Method: 首先构建具有每作业截止日期任务的柔性作业车间CP模型，并插入最优缓冲Δ*获得完全主动基线。然后将结果计划转换为具有不确定性的简单时间网络（STNU），并验证动态可控性。

Result: 在Kacem 1-4基准测试套件上的蒙特卡洛模拟显示，该方法消除了100%的截止期限违规，同时仅增加3-5%的制造周期开销。可扩展性实验证实CP求解时间和STNU检查在中等规模实例中保持亚秒级。
...
TL;DR: 该论文提出了一种基于扩展认知理论的形式化模型，将企业知识重新定义为可测量的动态能力，通过信息访问程序的效率和输出可靠性来量化企业知识状态，为AI时代的公司责任认定提供可审计的度量标准。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在企业决策中的广泛应用，传统基于人类代理人的公司犯罪意图认定面临挑战，需要重新定义企业知识的概念和衡量方法。

Method: 基于扩展认知理论，开发了一个形式化模型，引入连续的组织知识度量S_S(φ)，整合计算成本和统计验证错误率，定义知识谓词K_S和企业范围认知能力指数K_{S,t}。

Result: 建立了定量指标与法律标准（实际知识、推定知识、故意无视、鲁莽行为）的操作性映射，为创建可测量和可司法审计的工具体系提供了路径。

Conclusion: 该研究为企业思维在算法时代变得可追踪和可问责提供了方法论基础，通过可量化的审计证据使公司心智变得可管理。
...
<details>
  <summary>Details</summary>
Motivation: 医疗信息去标识化对于安全重用临床笔记至关重要，但现有评估方法依赖成本高昂的小规模专家标注，限制了模型比较和发展。

Method: 部署多个评估智能体独立判断PHI提取的正确性，通过基于LLM的多数投票机制整合不同评估者观点，生成稳定可复现的排名。

Result: 在真实临床笔记语料上的实验表明，TEAM-PHI产生一致准确的排名，尽管个体评估者存在差异，但LLM投票能可靠地收敛到相同的最佳系统。
...
TL;DR: 论文提出"被记住权"概念，针对LLMs可能压制某些观点、放大已有偏见的问题，旨在最小化AI驱动的信息遗漏风险，确保公平对待和内容真实性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的普及，人们开始依赖它们进行信息检索。与传统搜索引擎不同，LLMs提供单一权威的合成回答，可能将多种观点压缩成一个答案，减少用户比较替代方案的能力或意愿。这使少数LLM供应商掌握了信息控制权，可能导致某些叙述、个体或群体被不成比例地压制，而其他则被放大，从而重塑集体记忆。

Method: 论文提出了"被记住权"的概念框架，旨在最小化AI驱动的信息遗漏风险，拥抱公平对待的权利，同时确保生成的内容最大限度地真实。

Result: 通过引入"被记住权"概念，论文为应对LLMs可能带来的信息偏见和遗漏问题提供了理论框架，强调在AI信息检索中保护多样性和公平性的重要性。

Conclusion: LLMs的信息检索方式可能加剧偏见和遗漏问题，威胁集体记忆的多样性。"被记住权"概念的提出为解决这些问题提供了重要思路，需要在AI发展中重视信息公平性和真实性。
...
Method: 引入ScholarEval检索增强评估框架，评估研究想法的两个核心标准：合理性（基于现有文献的方法有效性）和贡献度（相对于先前研究的进步程度）。

Result: 在ScholarIdeas数据集上，ScholarEval比所有基线方法在专家标注评分标准覆盖度上显著更高，且在大规模用户研究中在文献参与、想法精炼和实用性方面显著优于深度研究方法。

Conclusion: ScholarEval为研究想法评估提供了有效的框架，并在多个维度上优于现有方法，相关代码、数据集和工具已开源。
...
TL;DR: 该论文发现大型推理模型存在"推理分心"漏洞，即模型容易被提示中嵌入的无关复杂任务分散注意力，导致主要任务准确性大幅下降。作者提出了一种结合监督微调和强化学习的训练防御方法。


<details>
  <summary>Details</summary>
Motivation: 随着大型推理模型在复杂任务上表现优异，作者发现这些模型存在一个关键漏洞——容易被恶意嵌入的无关复杂任务分散注意力，这严重威胁模型可靠性。

Method: 作者进行了跨模型和基准的综合研究，分析了推理分心漏洞的影响。为缓解风险，提出了一种基于训练的方法，结合监督微调和强化学习在合成对抗数据上进行训练。

Result: 研究表明即使最先进的大型推理模型也高度易受攻击，注入的干扰物可使任务准确性降低高达60%。提出的防御方法在挑战性干扰攻击上将鲁棒性提高了50多个点。

Conclusion: 推理分心是大型推理模型可靠性的一个独特且紧迫的威胁，该研究为构建更安全可信的推理系统提供了实用步骤。
...
TL;DR: 该论文通过实证研究发现网络交互式智能体系统存在效率瓶颈，提出SpecCache缓存框架结合推测执行来降低网络环境延迟，显著提升缓存命中率和系统效率。
...
Conclusion: 网络交互式智能体系统的效率优化至关重要，SpecCache通过有效的缓存和推测执行策略能够显著提升系统性能，为实际部署提供了可行方案。
...
TL;DR: 提出了DTKG框架，通过双轨知识图谱验证和推理来解决多跳问答中的并行事实验证和链式推理问题


<details>
  <summary>Details</summary>
Motivation: 现有多跳推理方法要么擅长并行事实验证但链式推理表现不佳，要么擅长链式推理但在并行验证时存在冗余路径检索问题，这影响了多跳问答的效率和准确性

Method: 基于认知科学中的双过程理论，提出DTKG框架，包含分类阶段和分支处理阶段，结合LLM响应的事实验证和KG路径的链式构建

Result: 未在摘要中明确说明具体实验结果

Conclusion: DTKG框架通过双轨方法有效解决了多跳推理中的并行事实验证和链式推理的平衡问题
...
TL;DR: MedRule-KG是一个紧凑型知识图谱，结合符号验证器，用于在推理任务中强制执行数学可解释规则，显著提升LLMs的推理准确性。
...
Method: 引入MedRule-KG知识图谱，包含实体、关系和三个领域启发规则，配合符号验证器检查预测并应用最小修正以保证一致性。

Result: 在90个FDA衍生基准测试中，使用MedRule-KG将精确匹配从0.767提升到0.900，添加验证器后达到1.000精确匹配，完全消除规则违反。

Conclusion: MedRule-KG为安全数学推理提供了通用框架，通过知识图谱和符号验证确保LLMs推理的数学一致性。
...
Method: 基于因果追踪发现擦除对锚点选择的敏感性，定义兄弟排他概念作为优质锚点，提出两阶段评估机制自动发现最优锚点并识别边界锚点。
...
TL;DR: 该论文研究用户如何通过策略性互动来引导算法与其真实兴趣对齐，揭示了用户需要足够的前瞻性才能有效引导算法，而小成本信号可以显著降低这种对齐负担。


<details>
  <summary>Details</summary>
Motivation: 用户在与算法互动时经常表现出不一致的偏好，可能花费大量时间在低价值内容上，无意中向算法发送错误信号。研究旨在探索用户需要什么条件才能让算法与其真实兴趣对齐。

Method: 将用户决策过程建模为理性系统2（决定是否参与）和冲动系统1（决定参与时长）的分离，采用多领导者-单跟随者的扩展Stackelberg博弈模型，用户作为领导者通过承诺参与策略来引导算法响应。

Result: 存在一个关键时间范围：足够有远见的用户可以实现对齐，而短视的用户反而会被算法目标同化。这个关键范围可能很长，但即使是一个小的成本信号（如额外点击）也能显著降低对齐负担。

Conclusion: 该框架解释了具有不一致偏好的用户如何在Stackelberg均衡中引导参与驱动型算法与其兴趣对齐，既揭示了实现对齐的挑战，也指出了潜在的补救措施。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM推理增强方法存在两个孤立范式：监控-生成方法擅长策略规划但缺乏验证机制，而生成-验证方法能迭代优化但缺乏任务评估。这种分离导致策略失败无反馈、优化无战略基础的低效问题。

Method: 基于Flavell的认知监控模型，实现监控-生成-验证框架的三阶段迭代系统，将策略规划与验证机制相结合。

Result: 在GSM8K任务上达到75.42%准确率，优于SELF-REFINE(68.44%)和Self-Verification(67.07%)，且尝试次数更少(1.3 vs 2.0)，推理成本增加27-37%。

Conclusion: 前期监控能产生更高质量的初始解决方案，从而减少优化需求，但需要在算术推理之外进行更多评估以确立通用性。
...
Method: 基于人类智能的层次处理机制，建模细粒度因果机制，通过解耦和重加权关键图像属性（颜色、纹理、形状）来增强跨域泛化。

Result: 理论和实证评估表明，HSCM在性能上优于现有领域泛化模型，提供更原则性的因果关系捕捉方法并提高模型鲁棒性。

Conclusion: HSCM框架通过模仿人类智能的灵活性和适应性，在动态复杂环境中实现更有效的迁移学习，为领域泛化提供了更可靠的方法。
...
TL;DR: 提出了RGMem框架，通过多尺度信息压缩和涌现过程来建模记忆演化，实现从微观对话中提取高层次用户画像，解决LLM对话系统中长期用户建模的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对话系统受限于有限上下文窗口和静态参数记忆，难以建模跨会话的长期用户状态和行为一致性，导致个性化交互停留在浅层水平。

Method: 受物理学重整化群思想启发，提出RGMem框架，通过分层粗粒化和重标度操作，从片段对话中提取语义和用户洞察，逐步形成动态演化的用户画像。

Result: 该框架能够从嘈杂的微观交互中实现高层次、准确的用户画像构建。

Conclusion: RGMem框架为语言智能体在LLM时代实现长期记忆和行为一致性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在数学、编程等推理任务上表现出色，但在NP难优化问题上的能力尚未充分探索，需要专门的训练框架和评估基准。

Method: 构建NP-ENGINE框架，包含可控实例生成器、规则验证器和启发式求解器，采用分层难度的可扩展RLVR训练，并开发NP-BENCH评估基准。

Result: 训练的QWEN2.5-7B-NP模型在NP-BENCH上显著超越GPT-4o，达到同尺寸模型SOTA，并展现出强大的跨领域泛化能力。

Conclusion: 任务丰富的RLVR训练是提升LLM推理能力的有前景方向，揭示了RLVR的扩展规律，NP难问题训练能增强模型在推理和非推理任务上的泛化能力。
...
TL;DR: Doug是一种类型化计算机语言，所有类型化程序都能被证明在多项式时间内停止，编码在向量符号架构(VSA)中。该语言基于轻量线性函数式编程语言(LLFPL)，使用基于全息声明性记忆(HDM)的槽值编码方案编码类型，使用Lisp VSA变体编码项。


<details>
  <summary>Details</summary>
Motivation: 将技能视为程序的应用，技能获取可表达为程序合成。希望描述一种遵循人类技能获取速度的学习形式，超越现有方法的效率，更接近建模人脑中的心理表征及其实际学习过程。

Method: 开发Doug语言，在VSA中编码类型和项，使神经网络可学习类型。类型在嵌入空间中具有相似结构和内容的邻近点，支持程序合成形式的技能获取。

Result: 提出了一种新的语言设计，使类型在神经网络嵌入空间中可学习，为高效技能获取提供了理论基础。

Conclusion: Doug语言为建模人脑心理表征及其学习过程迈出了重要一步，有望实现超越现有方法的人类水平技能获取效率。
...
TL;DR: 提出了Urban-R1框架，使用强化学习对齐多模态大语言模型，解决城市基础模型中的地理偏见问题，提升跨区域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有城市基础模型存在持续的地理偏见，导致区域预测偏差和泛化能力有限，需要开发更公平可信的城市智能系统。

Method: 采用基于强化学习的后训练框架Urban-R1，使用组相对策略优化(GRPO)优化地理组间推理，并以城市区域画像作为代理任务提供可测量奖励。

Result: 在多个区域和任务上的实验表明，Urban-R1有效减轻地理偏见，改善跨区域泛化，性能优于监督微调模型和闭源模型。
...
TL;DR: BuildArena是首个针对语言驱动工程建设的物理对齐交互基准，用于评估LLM在工程建筑自动化中的能力，包含可定制框架、可扩展任务设计、3D空间几何计算库和基线LLM代理工作流。
...
Method: 开发BuildArena基准，包含高度可定制的评估框架、跨多个难度级别的可扩展任务设计、支持语言指令建设的3D空间几何计算库，以及有效的基线LLM代理工作流。

Result: 在八个前沿LLM上全面评估了它们在语言驱动和物理基础建设自动化方面的能力。

Conclusion: BuildArena为语言驱动工程建设的评估提供了首个物理对齐的交互基准，有助于深入比较和分析LLM在此领域的能力。
...
TL;DR: 提出了Ripple Effect Protocol (REP)协调协议，让智能体不仅分享决策还分享轻量级敏感度信号，从而在群体中实现更快更稳定的协调。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理通信协议如A2A和ACP强调通信而非协调，随着代理数量增长，这导致脆弱的集体行为，即使单个代理很聪明，群体结果也很差。

Method: 引入REP协议，代理分享决策和轻量级敏感度信号（表达关键环境变量变化时选择如何改变），这些敏感度在局部网络中传播。正式定义了REP协议规范，分离必需的消息模式与可选的聚合规则。

Result: 在三个领域的基准测试中：供应链级联（啤酒游戏）、稀疏网络中的偏好聚合（电影调度）和可持续资源分配（鱼塘），REP相比A2A将协调准确性和效率提高了41%到100%，并能灵活处理来自LLM的多模态敏感度信号。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于知识图谱的RAG方法难以从文本丰富的知识图谱中为复杂真实世界查询检索准确多样的信息，而过程奖励模型虽然能对齐检索过程与查询需求，但依赖昂贵且难以获取的过程级监督信号。

Method: GraphFlow采用转移流匹配目标联合优化检索策略和流估计器，流估计器将检索结果的奖励分解到中间检索状态，指导检索策略按奖励比例从知识图谱中检索候选结果。

Result: 在STaRK基准测试中，GraphFlow在命中率和召回率上平均优于强KG-RAG基线（包括GPT-4o）10%，并在未见过的知识图谱上表现出强大的泛化能力。

Conclusion: GraphFlow是一个有效且鲁棒的框架，能够从文本丰富的知识图谱中高效检索准确多样的知识，满足复杂真实世界查询的需求。
...
TL;DR: 提出了一种半监督置信度分布学习方法（ssCDL）来解决不确定知识图谱补全中的置信度分布不平衡问题，通过将置信度转换为分布并利用元学习生成伪标签来增强训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有不确定知识图谱补全方法忽略了置信度的极端不平衡分布，导致学习到的嵌入不足以支持高质量的图谱补全。

Method: 将每个三元组置信度转换为置信度分布，通过关系学习在标记数据（现有三元组）和未标记数据（伪标签）上迭代学习嵌入，使用元学习预测未见三元组的置信度来增强训练数据并重新平衡置信度分布。
...
<details>
  <summary>Details</summary>
Motivation: 现有的RL范式依赖稀疏的结果奖励和有限探索，导致LLMs倾向于重复和次优的推理模式，需要设计更好的探索机制。

Method: 使用轻量级的Coin Flipping Network(CFN)估计推理轨迹的伪计数和认知不确定性，将其转换为内在奖励，并与GRPO等RL框架集成。

Result: 在复杂推理基准测试中，MERCI鼓励更丰富多样的思维链，显著超越强基线性能，帮助策略逃离局部常规发现更好解决方案。

Conclusion: 有针对性的内在动机可以使探索在语言模型推理中变得可靠。
...
TL;DR: 本文探讨大规模AI模型对神经科学研究的变革性影响，涵盖神经影像、脑机接口、分子神经科学、临床辅助和疾病应用等五大领域，强调AI在解决神经科学计算挑战和促进双向发展方面的作用。


<details>
  <summary>Details</summary>
Motivation: 大规模AI模型的出现为神经科学研究带来了范式转变，能够从原始脑信号和神经数据中进行端到端学习，解决传统计算方法难以应对的挑战。

Method: 通过系统回顾和综合分析，探索大规模AI模型在五个主要神经科学领域的应用：神经影像数据处理、脑机接口与神经解码、分子神经科学与基因组建模、临床辅助与转化框架、神经系统和精神疾病的特定应用。

Result: 这些模型被证明能够有效解决多模态神经数据整合、时空模式解释和临床转化框架等主要计算神经科学挑战，同时神经科学与AI的互动变得更加双向。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的方法在解决复杂车辆路径问题时仍需要外部干预，导致自主性受限、执行错误和解决方案可行性低。需要实现从问题实例到解决方案的完全自动化。

Method: AFL框架将整体流程分解为三个可管理的子任务，使用四个专门化智能体进行协调交互，确保跨功能一致性和逻辑合理性，直接从原始输入提取知识并生成自包含代码。

Result: 在60个复杂VRP问题上的广泛实验验证了框架的有效性和通用性，与精心设计的算法性能相当，在代码可靠性和解决方案可行性方面显著优于现有LLM基线方法，在评估基准上接近100%的成功率。

Conclusion: AFL框架实现了复杂车辆路径问题求解的完全自动化，通过智能体协调机制确保了解决方案的高可靠性和可行性，为LLM在复杂优化问题中的应用提供了有效范例。
...
TL;DR: 该调查追踪了智能AI从基于流水线的外部逻辑编排系统向模型原生范式的转变，其中规划、工具使用和记忆等能力被内化到模型参数中。强化学习是实现这一范式转变的算法引擎。


<details>
  <summary>Details</summary>
Motivation: 研究智能AI的范式转变，从构建应用智能的系统转向开发通过经验增长智能的模型，探索从外部编排到内化能力的演进路径。

Method: 系统性地回顾了规划、工具使用和记忆等能力如何从外部脚本模块演变为端到端学习行为，并分析了强化学习在实现这一转变中的核心作用。

Result: 揭示了智能AI向模型原生范式发展的清晰轨迹，展示了在语言、视觉和具身领域实现LLM + RL + Task统一解决方案的可行性。

Conclusion: 智能AI正在向集成学习和交互框架发展，标志着从构建应用智能的系统向开发通过经验增长智能的模型的根本转变。
...
TL;DR: 这篇论文是关于基于强化学习的代理搜索的综述，探讨了如何将强化学习与代理搜索相结合来解决传统检索增强生成的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在静态知识、事实幻觉和无法获取实时信息的问题，传统RAG方法缺乏自适应控制。代理搜索通过多步交互解决这些问题，而强化学习提供了自适应和自我改进的搜索机制。

Method: 从三个维度组织该新兴领域：(i) RL的功能角色，(ii) RL的优化策略，(iii) RL的应用范围。总结了代表性方法、评估协议和应用。

Result: 提供了该领域的首个全面概述，建立了系统性的分类框架，并识别了关键研究方向。

Conclusion: 这项调查旨在激发未来关于RL与代理搜索集成的研究，并讨论了构建可靠和可扩展的RL驱动代理搜索系统的开放挑战和未来方向。
...
TL;DR: 该论文提出了一种基于代理模型的仿真驱动工程工作流，通过训练轻量级模拟器来解决计算成本高和模型透明度不足的问题，支持不确定性量化和可解释AI分析。
...
Method: 通过在紧凑的实验设计上训练轻量级模拟器，提供快速、低延迟的近似，支持严格的不确定性量化，并适配全局和局部可解释AI分析。该方法支持连续和分类输入，结合全局效应和不确定性分析与局部归因。

Result: 在两个对比案例研究中展示了该方法：混合电动飞机的多学科设计分析和城市隔离的基于代理模型。结果显示，代理模型与XAI耦合能够在几秒钟内实现大规模探索，发现非线性相互作用和涌现行为，识别关键设计和政策杠杆，并指示代理需要更多数据或替代架构的区域。

Conclusion: 所提出的工作流统一了各种基于仿真的复杂系统分析工具，从工程设计到社会环境理解，通过代理模型和可解释AI的结合，实现了高效、透明和可靠的复杂系统分析。
...
TL;DR: 提出ELMM方法解决多模态知识图谱补全中的计算效率问题，通过多视图视觉token压缩和注意力剪枝策略，在保持性能的同时显著提升效率


<details>
  <summary>Details</summary>
Motivation: 现有MKG存在不完整性问题，而多模态大语言模型在MKGC任务中面临图像token过多导致语义噪声和计算成本高的挑战

Method: 提出ELMM框架，包含基于多头注意力的多视图视觉token压缩器（MVTC）和注意力剪枝策略，通过线性投影补偿剪枝带来的性能损失
...
<details>
  <summary>Details</summary>
Motivation: 人类交互本质上是多模态和全双工的，需要同时感知和生成多种模态以实现更自然的人类行为模拟。

Method: 采用新颖的SA-MoE架构，将各模态路由到专用专家模块，通过统一注意力骨干网络进行融合，实现联合多模态感知和并发生成。

Result: 在语音交互和机器人操作基准测试中，ELLSA与模态特定基线表现相当，同时支持高级多模态和全双工行为，如对话轮转、动作轮转、指令拒绝等。

Conclusion: ELLSA代表了向更自然和通用交互智能迈出的一步，为追求人工通用智能做出贡献。
...
TL;DR: GraphVista是一个统一的图理解框架，通过分层组织图信息到轻量级GraphRAG基础中，提升可扩展性和模态协调能力，能够处理比现有基准大200倍的图，并在质量上比最先进基线提升4.4倍。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在图理解中面临的输入token限制、可扩展性瓶颈以及缺乏有效协调文本和视觉模态机制的问题。

Method: 采用分层组织图信息到GraphRAG基础中，仅检索任务相关文本描述和高分辨率视觉子图；引入规划代理将任务路由到最适合的模态：文本模态用于简单属性推理，视觉模态用于基于显式拓扑的局部和结构复杂推理。

Result: GraphVista能够扩展到大型图（比现有基准大200倍），在文本、视觉和融合方法中表现一致优于现有方法，质量比最先进基线提升高达4.4倍。
...
TL;DR: 提出了Domain-Contextualized Concept Graph (CDC)框架，通过将领域作为概念表示的一等元素，使用C-D-C三元组结构<概念, 关系@领域, 概念'>来克服传统知识图谱固定本体的限制。
...
Method: 采用C-D-C三元组结构，基于认知-语言同构映射原理，定义20多种标准化关系谓词（结构、逻辑、跨域、时间），并在Prolog中实现完整推理能力。

Result: 在教育、企业知识系统和文档等案例中，CDC实现了上下文感知推理、跨域类比和个性化知识建模等传统框架无法实现的能力。

Conclusion: CDC框架通过将领域显式化，为知识表示和推理提供了更灵活、动态的方法，超越了传统基于本体的知识建模限制。
...
TL;DR: DeepAnalyze-8B是首个用于自主数据科学的智能LLM代理，能够自动完成从数据源到分析师级深度研究报告的端到端流程，仅用80亿参数就超越了基于最先进专有LLM的工作流代理。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于工作流的数据代理在实现完全自主数据科学方面的根本限制，这些代理依赖预定义工作流，无法处理高复杂度的数据科学任务。

Method: 提出基于课程学习的智能代理训练范式，模拟人类数据科学家的学习轨迹；引入数据驱动的轨迹合成框架构建高质量训练数据；通过智能训练使LLM逐步获取和整合多种能力。

Result: DeepAnalyze-8B能够执行广泛的数据任务，包括数据问答、专业分析任务和开放式数据研究，在实验中超越了基于最先进专有LLM的工作流代理。

Conclusion: 该模型、代码和训练数据已开源，为自主数据科学的发展铺平了道路。
...
TL;DR: 该论文提出通过强化学习训练VLM代理构建内部世界模型，将视觉状态推理分解为状态估计和转移建模，并设计了世界建模奖励和双层GAE方法，在多个基准测试中取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决VLM代理从文本状态到复杂视觉观察转换时面临的局部可观测性和世界建模挑战，探索代理能否通过显式视觉状态推理构建内部世界模型。

Method: 将代理推理过程架构化为POMDP，分解为状态估计和转移建模，设计世界建模奖励提供密集监督，引入双层GAE进行回合感知信用分配，在VAGEN框架中训练多回合VLM代理。

Result: 3B参数模型在五个多样化代理基准测试中得分0.82，相比未训练版本(0.21)提升3倍，并优于GPT-5(0.75)、Gemini 2.5 Pro(0.67)和Claude 4.5(0.62)等专有推理模型。

Conclusion: 视觉状态推理能有效提升VLM代理性能，最优表示形式取决于任务类型，自然语言适合语义关系，结构化格式适合精确控制，所提出的方法在多样化视觉环境中具有良好扩展性。
...
TL;DR: 提出了一种新的评估方法，测试用户能否从RL算法的决策解释中识别智能体的目标。在Ms. Pacman环境中测试了4种XRL算法，发现只有一种算法在测试目标上达到了超过随机水平的准确率，且用户普遍对自己的选择过度自信。


<details>
  <summary>Details</summary>
Motivation: 可解释强化学习(XRL)算法的核心应用是调试，但缺乏对其相对性能的比较评估。

Method: 使用Atari的Ms. Pacman环境和四种XRL算法，通过用户能否从决策解释中识别智能体目标来评估算法性能。

Result: 只有一种XRL算法在测试目标上达到了超过随机水平的准确率；用户普遍过度自信；用户自报的识别和理解难易度与准确率无关。

Conclusion: 当前XRL算法在帮助用户识别智能体目标方面效果有限，用户的主观感受与实际性能存在偏差。
...
TL;DR: 提出了一种基于多智能体协作的LLM框架，用于自动化GPU内核优化，显著提升了内核性能和优化成功率


<details>
  <summary>Details</summary>
Motivation: GPU内核优化对AI发展至关重要，但现有LLM方法在复杂的硬件特性交互中效果有限，需要更系统化的优化方法

Method: 采用多智能体协作框架，结合硬件感知推理、性能分析反馈和迭代优化策略，模拟专家工程师的工作流程

Result: 在KernelBench基准测试中，相比基线方法显著提升：在基线失败的情况下仍能生成正确解决方案，内核运行性能最高提升16倍
...
TL;DR: ToolCritic是一个诊断框架，用于评估和改进LLM在多轮工具增强对话中的行为，通过检测8种特定工具调用错误并提供针对性反馈，使主LLM能够修正响应，在SGD数据集上工具调用准确率提升高达13%。


<details>
  <summary>Details</summary>
Motivation: 工具增强的大型语言模型在现实应用中越来越普遍，但工具使用错误仍然影响其可靠性，需要解决这些错误以提高LLM与外部工具集成的鲁棒性。

Method: ToolCritic检测8种特定工具调用错误类型（如过早调用、参数不对齐、工具输出误解等），为具有强推理能力的主LLM提供针对性反馈，主LLM基于反馈修正响应。通过系统定义错误类别并构建合成数据集来训练ToolCritic。

Result: 在Schema-Guided Dialogue数据集上的实验结果表明，ToolCritic相比基线方法（包括零样本提示和自校正技术）将工具调用准确率提高了高达13%。
...
<details>
  <summary>Details</summary>
Motivation: 传统的基因集富集分析方法依赖良好注释的基因集，在处理特征不明确的基因时表现不佳。大型语言模型虽然提供了替代方案，但难以在结构化本体中表示复杂的生物学知识。

Method: 开发了BRAINCELL-AID多智能体AI系统，整合自由文本描述和本体标签，采用检索增强生成技术，通过PubMed文献精炼预测，减少幻觉并增强可解释性。

Result: 对小鼠基因集实现了77%的正确注释率，成功注释了BRAIN Initiative Cell Census Network生成的5,322个脑细胞簇，识别了区域特异性基因共表达模式，并推断出基因集合的功能角色。
...
<details>
  <summary>Details</summary>
Motivation: 现有金融AI方法主要关注数值预测，无法处理企业信用评估中具有决定性影响但难以形式化的定性非财务指标，且缺乏对专业贷款评估所需解释性判断的支持。

Method: 开发了两种LLM系统：非对抗性单代理系统（NAS）通过单次推理流程生成双向分析；基于辩论的多代理系统（KPD-MADS）采用基于卡尔·波普批判对话框架的十步结构化交互协议进行对抗性验证。

Result: 两个系统在三个真实企业案例中均实现显著生产力提升（NAS：11.55秒/案例；KPD-MADS：91.97秒；人工基线：1920秒）。KPD-MADS在解释充分性、实际适用性和可用性方面获得更高的中位数评分。
...
TL;DR: 提出了一种基于手工特征的方法，通过从鱼眼图像中提取颜色统计、多色彩空间直方图和纹理特征来评估鱼类新鲜度，显著超越了之前的深度学习方法。
...
Method: 从鱼眼图像中系统提取和增量融合互补特征描述符，包括颜色统计、多色彩空间直方图、局部二值模式和灰度共生矩阵等纹理特征，同时捕获全局色彩变化和局部ROI区域退化。

Result: 在FFE数据集上，LightGBM分类器达到77.56%准确率，比之前深度学习方法提升14.35%；使用增强数据时，ANN达到97.16%准确率，比之前最佳结果提升19.86%。

Conclusion: 精心设计的手工特征经过战略处理后，能够为自动化鱼类新鲜度评估提供鲁棒、可解释且可靠的解决方案，为食品质量监控提供实用价值。
...
<details>
  <summary>Details</summary>
Motivation: HVAC系统占全球建筑能耗的很大比例，传统规则方法缺乏适应性，深度学习方法缺乏透明度和物理合理性，现有LLM方法忽略了HVAC运行的物理原理。

Method: 提出PILLM框架，在进化循环中引入物理知识引导的反思和交叉算子，嵌入热力学和控制理论约束，生成既适应性强又物理合理的异常检测规则。

Result: 在公共建筑故障检测数据集上的实验表明，PILLM实现了最先进的性能，同时产生可解释和可操作的诊断规则。

Conclusion: PILLM推进了智能建筑系统中可信赖和可部署AI的发展，为HVAC异常检测提供了物理合理且可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大规模多智能体系统中，通信协议选择缺乏标准化指导，现有选择往往基于直觉而非数据驱动。

Method: 开发ProtocolBench基准测试，从任务成功率、端到端延迟、消息开销和故障恢复能力四个维度系统评估协议；提出ProtocolRouter学习型协议路由器。

Result: 协议选择显著影响系统性能：在流式队列场景中，完成时间差异达36.5%，端到端延迟差异3.48秒；ProtocolRouter相比最佳单协议基线，故障恢复时间减少18.1%。

Conclusion: 通信协议是影响多智能体系统性能的关键因素，ProtocolBench和ProtocolRouter为协议选择和优化提供了数据驱动的解决方案。
...
Conclusion: 该混合框架为VT/VF风险预测提供了新范式，验证了基础模型输出作为有效自动化特征工程，用于构建可信赖、可解释的AI临床决策支持系统。
...
TL;DR: 研究评估了一个基于工具的LLM健康教练系统，通过离线策略评估发现统一的重工具策略虽然提升平均价值，但对特定用户群体（特别是低健康素养/高自我效能用户）造成伤害。模拟器实验显示添加早期信息增益奖励可改善个性化效果。


<details>
  <summary>Details</summary>
Motivation: 研究真实用户环境中工具增强型LLM健康教练的个性化表现，特别关注不同用户群体间的性能差异，避免平均指标掩盖的亚组伤害问题。

Method: 使用离线策略评估（OPE）分析因子化决策头（工具/风格），通过轻量级模拟器验证添加信息增益奖励的效果，采用基于类型化奖励（客观工具结果和满意度）的亚组感知决策头学习。

Result: 统一重工具策略在日志上提高平均价值，但伤害低健康素养/高自我效能用户；添加早期信息增益奖励可缩短特征识别时间，提高目标成功率和pass@3指标。

Conclusion: 提出以评估为先的个性化路径：冻结生成器，在类型化奖励上学习亚组感知决策头，并始终报告按原型分类的指标以揭示平均指标掩盖的亚组伤害。
...
<details>
  <summary>Details</summary>
Motivation: 准确建模疾病进展可以改善患者亚型分型和及时干预，但现有方法无法基于不规则时间事件样本学习连续时间动态，且难以处理患者异质性
...
Conclusion: TD-HNODE能够有效学习疾病进展的复杂连续时间动态，为患者亚型分型和及时干预提供了更好的建模工具
...
<details>
  <summary>Details</summary>
Motivation: 解决加密货币投资中信息碎片化、高波动性以及现有分析方法（手动分析、数据聚合平台、静态LLM代理）的不足，提供更准确、实时的投资洞察。

Method: 采用基于强化学习的工具选择机制和多智能体框架，集成多样化分析能力，支持多步骤规划和实时数据整合。

Result: 在工具编排方面，相比基础模型召回率提升40.7%，F1分数提升26.6%；用户研究显示高满意度（4.64/5），参与者更偏好Coinvisor而非通用LLM和现有加密平台（4.62/5）。

Conclusion: Coinvisor通过强化学习驱动的多智能体框架，有效提升了加密货币投资分析的准确性和用户体验，为动态市场环境提供了适应性强的分析解决方案。
...
TL;DR: RubiSCoT是一个AI支持的论文评估框架，使用自然语言处理技术从提案到最终提交阶段提供一致、可扩展的评估解决方案。
...
Method: 使用大型语言模型、检索增强生成和结构化思维链提示等自然语言处理技术，包括初步评估、多维度评估、内容提取、基于评分标准的评分和详细报告。
...
<details>
  <summary>Details</summary>
Motivation: 在密集多智能体路径规划问题中，现有方法难以实时找到接近最优解，需要结合学习和搜索的优势

Method: 开发混合框架，将改进的MAGAT神经网络作为启发式集成到LaCAM搜索算法中，采用预训练-微调策略和死锁检测机制

Result: LaGAT在密集场景中表现优于纯搜索和纯学习方法

Conclusion: 精心设计的混合搜索方法为紧密耦合的复杂多智能体协调问题提供了强大解决方案
...
TL;DR: 提出FBI_LTL多样性规划器，使用线性时序逻辑定义语义多样性标准，在基于仿真的规划中生成语义多样化的计划


<details>
  <summary>Details</summary>
Motivation: 传统规划器只生成单一计划，可能无法满足代理偏好；现有多样性规划方法可能产生语法不同但语义相同的解决方案

Method: 使用线性时序逻辑定义语义多样性标准，并将这些多样性模型直接集成到搜索过程中

Result: 在各种基准测试中，FBI_LTL相比基线方法能生成更多样化的计划

Conclusion: 这项工作确立了在基于仿真的环境中进行语义引导多样性规划的可行性，为传统基于模型方法失效的现实非符号领域开辟了新途径
...
TL;DR: 提出一种基于主动推理的路径规划方法，用于智能代理的自主控制，通过构建证据地图来维持共同作战态势，平衡探索与利用。


<details>
  <summary>Details</summary>
Motivation: 为了解决地理区域侦察中的探索与利用平衡问题，维持共同作战态势，需要开发能够自主控制智能代理的方法。

Method: 使用Dempster-Shafer理论和高斯传感器模型构建生成模型，通过变分自由能量计算来指导代理移动，采用贝叶斯方法更新后验概率分布。

Result: 该方法能够在仿真中有效引导代理移动，通过最小化自由能量来平衡区域搜索和目标跟踪。

Conclusion: 主动推理路径规划方法成功解决了地理区域侦察中的探索与利用平衡问题，为智能代理的自主控制提供了有效解决方案。
...
TL;DR: 该论文指出法律机器学习中存在标签不确定性，即案件结果可能因人为干预而改变，这会影响模型训练。作者以欧洲人权法院案件为例，展示了标签构建方式如何显著影响模型行为。


<details>
  <summary>Details</summary>
Motivation: 法律机器学习通常将过去案件结果视为绝对真实，但法律结果往往受到和解、上诉等未记录的人为干预影响，导致标签不确定性，需要对此进行考虑。

Method: 在欧洲人权法院案件分类的背景下，研究不同标签构建方法对模型行为的影响，探讨如何处理标签不确定性。

Result: 研究表明，训练过程中标签的构建方式会显著影响模型的行为表现，标签不确定性是AI与法律领域需要关注的重要问题。

Conclusion: 法律机器学习应用需要考虑标签不确定性，虽然存在估算方法但都基于不可验证的假设，标签不确定性应被视为AI与法律领域的重要关注点。
...
TL;DR: MIRAGE是一个用于检测多模态虚假信息的推理时代理框架，通过分解验证过程为四个模块：视觉真实性评估、跨模态一致性分析、检索增强的事实核查和校准判断，无需领域特定训练数据即可达到监督检测器的性能。
...
Method: MIRAGE框架将多模态验证分解为四个顺序模块：视觉真实性评估检测AI生成图像，跨模态一致性分析识别上下文不当使用，检索增强事实核查通过迭代问题生成将声明基于网络证据，校准判断模块整合所有信号。

Result: 在MMFakeBench验证集上，MIRAGE与GPT-4o-mini结合达到81.65% F1和75.1%准确率，比最强的零样本基线（GPT-4V与MMD-Agent的74.0% F1）高出7.65点，同时保持34.3%的假阳性率，而仅使用判断的基线为97.3%。测试集结果确认了泛化能力。

Conclusion: 分解的代理推理与网络检索相结合，可以在没有领域特定训练的情况下匹配监督检测器的性能，在标记数据稀缺的多模态环境中实现虚假信息检测。
...
TL;DR: 将大型语言模型的推理能力蒸馏到更小、更高效的模型中，通过结构感知损失优化方法提升代码生成质量


<details>
  <summary>Details</summary>
Motivation: 代码生成需要准确理解提示意图并应用算法推理，小型语言模型可能缺乏这种推理能力，而大型模型虽然具备但部署成本高

Method: 通过结构感知损失优化训练小型模型，使其学习大型模型的推理和问题解决能力，建立问题定义与解决方案之间的结构对应关系

Result: 在MBPP、MBPP Plus和HumanEval基准测试中，微调模型在pass@1、平均数据流和平均语法匹配指标上显著优于基线模型

Conclusion: 通过廉价简单的蒸馏过程，可以有效将大型语言模型的推理能力转移到小型模型中，实现高效部署
...
TL;DR: OG-Rank是一个低延迟的解码器重排序系统，通过池化首令牌评分和不确定性门控解释步骤，在单次处理中为所有候选打分，仅在列表真正模糊时生成简短的结构化理由，保持可预测的延迟。


<details>
  <summary>Details</summary>
Motivation: 临床医生需要实时工作并能证明其选择的排序系统，需要低延迟、基于解码器的重排序器。

Method: 采用单解码器方法，结合池化首令牌评分信号和不确定性门控解释步骤，使用专注于困难案例的课程训练。

Result: 在遭遇范围订单选择任务中表现优异（快速路径：Recall@1~0.45，nDCG@20~0.625），当门控激活时进一步提升（Recall@1~0.56，nDCG@20~0.699，门控率45%），紧凑骨干网络在相同策略下也显示类似增益。

Conclusion: OG-Rank提供了一个实用方案：默认快速排序，在有助于提高准确性时进行解释，这种模式适用于选择性生成能以可接受成本换取准确性的决策任务。
...
TL;DR: 本文系统评估了大型语言模型作为预测工具的能力，发现LLMs已展现出有前景的预测性能，但也存在事件召回不准确、数据源误解等关键瓶颈。
...
Conclusion: 虽然LLMs在预测方面表现出色，但实现卓越预测智能仍面临关键瓶颈，包括事件召回不准确、数据源误解以及在接近解决时信息聚合速度慢于市场等问题。
...
TL;DR: 本文提出基于多智能体影响图(MAIDs)的交互范式，通过目标干预和因果推理技术PSI来解决大规模多智能体强化学习中全局指导不切实际的问题。
...
Method: 使用MAIDs作为图形框架分析现有方法，设计基于MAIDs的目标干预范式，仅针对单个智能体进行干预，并引入PSI因果推理技术实现该范式。

Result: 实验证明目标干预的有效性，并验证了相关性图分析的结果。

Conclusion: MAIDs框架为多智能体强化学习提供了有效的分析和设计工具，目标干预范式能够缓解全局指导问题，PSI技术能够通过最大化因果效应实现复合期望结果。
...
TL;DR: 提出Contextual Attention Modulation (CAM)机制和HyCAM框架，通过动态调制自注意力表示来平衡知识保留和任务专业化，在异构任务上平均性能提升3.65%。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在多任务适应中知识保留与任务专业化平衡的挑战，传统微调方法存在灾难性遗忘和资源消耗问题，现有参数高效方法在复杂多任务场景表现不佳。

Method: 提出CAM机制动态调制自注意力模块表示，增强任务特定特征同时保留通用知识；构建HyCAM框架，结合共享全参数CAM模块和多个轻量级专业CAM模块，通过动态路由策略实现自适应知识融合。

Result: 在问答、代码生成和逻辑推理等异构任务上的广泛实验表明，该方法显著优于现有方法，平均性能提升3.65%。

Conclusion: CAM和HyCAM框架有效解决了LLMs多任务适应中的平衡问题，实现了更高效和有效的适应，代码和数据已开源以促进可复现性。
...
TL;DR: 研究发现VLMs在深层能够感知视觉证据但未能有效利用，提出了无需训练的推理时干预方法，通过选择性注意力掩码提升多模型准确率


<details>
  <summary>Details</summary>
Motivation: 系统研究VLMs失败的原因：是未能感知视觉证据还是未能有效利用证据

Method: 通过层间注意力动态分析，发现浅层主要关注文本，深层稀疏但可靠地关注局部证据区域；提出推理时干预方法，通过选择性注意力掩码突出深层证据区域

Result: VLMs经常在输出错误答案时仍能感知视觉证据；提出的干预方法无需训练，在LLaVA、Qwen、Gemma和InternVL等多个VLM家族中一致提升准确率

Conclusion: VLMs内部编码了可靠的证据但未能充分利用，使这些信号显式化可以弥合感知与推理之间的差距，推进对VLMs的诊断理解和可靠性
