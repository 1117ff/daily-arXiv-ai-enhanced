...
Motivation: 随着月球探索任务增加，需要高质量3D月球地图，特别是长距离任务如NASA的Endurance任务。Kaguya TC图像虽然全球覆盖10米/像素分辨率，但存在立体匹配误差和JPEG压缩伪影导致的高程不准确问题。

Method: 分析Kaguya TC图像的压缩行为，识别系统性视差噪声模式（尤其在暗区），提出通过减少压缩图像视差图中的残余噪声来增强3D地图质量的方法。
...
<details>
  <summary>Details</summary>
Motivation: 探究CLIP在视觉语言模型中优于自监督模型DINO的原因，是由于语言监督还是更大训练数据的影响。

Method: 在相同架构、数据集和训练配置下预训练CLIP和DINO，分析嵌入特征，并在20个VQA基准上评估性能。

Result: CLIP擅长捕获高级语义（如对象类别、文本），在文本密集型任务中表现优异；DINO对低级特征（如颜色、风格）更敏感，在视觉中心任务中略胜一筹。

Conclusion: 语言监督对视觉编码器性能有重要影响，CLIP和DINO在VLM中各有优势，为视觉编码器设计提供了科学见解。
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习在通用视觉领域已成功应用，但在医学影像中由于数据有限和领域特定偏差而未被充分利用。本文旨在为乳腺X线摄影开发有效的自监督学习方法。

Method: 提出MammoDINO框架，包含乳腺组织感知数据增强采样器（图像级和补丁级监督）和跨切片对比学习目标，将3D DBT结构信息融入2D预训练。

Result: 在多个乳腺癌筛查任务中达到最先进性能，在五个基准数据集上表现出良好的泛化能力。

Conclusion: MammoDINO为乳腺X线摄影提供了可扩展、无需标注的多用途计算机辅助诊断工具基础，有助于减轻放射科医生工作量并提高乳腺癌筛查效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统的动态高斯泼溅模型对所有高斯基元进行统一优化，忽略了它们在视图中的可见性差异，导致遮挡下的运动漂移和极端视角下的合成质量下降。

Method: 提出不确定性感知的动态高斯泼溅框架，估计每个高斯基元的时间变化不确定性，构建时空图进行不确定性感知优化，将可靠的运动线索传播到整个场景。

Result: 在真实和合成数据集上的实验表明，显式建模不确定性持续改进动态高斯泼溅模型，在遮挡下产生更稳定的几何结构，在极端视角下实现高质量的合成。

Conclusion: 不确定性建模对于动态3D场景重建至关重要，USplat4D通过不确定性感知优化有效解决了遮挡和极端视角带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 交通安防分析需要细粒度行为模式识别和全面描述生成，现有方法难以同时兼顾描述生成和视觉问答任务。

Method: 采用双模型框架，分别训练VideoLLaMA用于描述生成，Qwen2.5-VL用于视觉问答，避免任务干扰，实现专业化优化。

Result: VideoLLaMA在时序推理上CIDEr得分1.1001，Qwen2.5-VL在视觉理解上VQA准确率60.80%，在AI City Challenge Track 2中S2得分45.7572，排名第10。

Conclusion: 分离训练策略有效减少任务干扰，比联合训练在VQA准确率上提升8.6%，同时保持描述生成质量，验证了双模型框架的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 传统创建逼真数字人虚拟形象的过程昂贵且耗时，需要大型相机捕捉设备和专业3D艺术家的大量手动工作。现有基于单张图像的方法虽然降低了门槛，但缺乏多视角信息或显式3D表示的约束，导致视角偏离参考图像时质量下降。

Method: 基于预训练的视频扩散模型，从单张参考图像和目标表情生成可动画的多视角视频，同时生成数百帧图像，视角变化可达360度，然后将模型输出提炼为可实时渲染的4D虚拟形象。

Result: 与先前方法相比，该方法显著提高了生成虚拟形象的逼真度、时间一致性和3D一致性。

Conclusion: MVP4D方法通过生成多视角视频并提炼为4D虚拟形象，解决了单图像方法在视角偏离时的质量下降问题，为数字人虚拟形象创建提供了更高效、更逼真的解决方案。
...
Conclusion: 该方法在房间布局估计中具有高精度，证明了TPS变换与全景图像的兼容性，能稳健处理立方体和非立方体布局。
...
Method: 将掩码维度（边界框坐标）直接嵌入输入提示，使模型能够推理对象几何和布局；在四个问题类别上进行微调；在训练集中向GPT响应添加规范化答案以提高一致性。

Result: 综合管道实现了73.0606的最终得分，在公共排行榜上排名第四。
...
<details>
  <summary>Details</summary>
Motivation: 解决水下图像退化（低对比度、颜色失真）和海洋生物自然伪装带来的检测挑战，传统图像增强方法难以恢复关键特征，陆地场景的伪装目标检测方法无法适应水下光学特性。

Method: 提出APGNet网络：1）使用MSRCR算法进行数据增强生成光照不变图像；2）设计扩展感受野模块和多尺度渐进解码器捕获多尺度上下文信息；3）提出自适应先验引导机制，在高层特征嵌入空间注意力进行粗定位，在低层特征使用可变形卷积细化轮廓。

Result: 在两个公开MAS数据集上的大量实验结果表明，APGNet在广泛使用的评估指标下优于15种最先进方法。

Conclusion: APGNet通过集成孪生网络架构和新型先验引导机制，有效提升了水下伪装目标检测的鲁棒性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视频编辑方法在结构可变编辑中存在的时序不一致、主体身份漂移和需要人工干预等问题。

Method: 利用姿态和位置先验从源视频学习通用运动表示，基于扩散模型生成保持原始运动的新视频。

Result: 定性和定量评估均显示该方法优于现有方法，能够有效保持时序一致性并避免身份漂移。

Conclusion: VidMP3在保持运动一致性的同时实现了结构和语义的灵活编辑，为创作者提供了更好的视频编辑工具。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉领域面临高质量标注数据稀缺的挑战，获取标注数据成本高昂且有时不可能，需要可靠方法来克服数据不足问题。

Method: 采用领域自适应方法，使用在特定数据集上训练的模型来预测来自不同领域但相同类型的数据，例如用飞机画作训练的模型预测真实飞机图像。

Result: 该方法能够在不依赖大量标注数据的情况下，实现与先前基准结果相当的性能。

Conclusion: 领域自适应是解决标注数据稀缺问题的有效方法，能够在不同领域间迁移学习，降低对标注数据的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有音频驱动视频生成方法在唇形同步准确性、长视频时序一致性和多角色动画方面仍面临挑战，需要更高效、可控的解决方案。

Method: 采用LoRA训练策略结合位置偏移推理实现长视频生成；通过部分参数更新和奖励反馈增强唇形同步和身体动作；提出无需训练的Mask-CFG方法支持多角色动画。

Result: 实验结果表明该方法在质量和时序一致性方面优于现有最先进方法，能够以简单高效的方式实现高质量多角色音频驱动视频生成。

Conclusion: 该方法提供了一种简单、高效且成本效益高的解决方案，在音频驱动视频生成领域取得了显著进展，特别是在长视频生成和多角色动画方面。
...
Method: 构建包含18种常见房间类型的大规模数据集，提供实例级自然语言标注和多模态数据导出能力。通过监督微调( SFT )在IL3D上训练LLMs。

Result: 在IL3D上进行SFT显著提高了LLMs的泛化能力，性能优于在其他数据集上的SFT。

Conclusion: IL3D作为一个多功能、强大的资源，显著推进了3D场景生成和具身智能的研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法很少显式利用QR码的高度结构化边缘模式这一强大先验，而QR码去模糊的关键目标是确保成功解码而非感知质量。

Method: 设计了Edge-Guided Attention Block将显式边缘先验嵌入Transformer架构，基于此构建EG-Restormer处理严重模糊QR码；针对轻度模糊设计了轻量级LENet；最后集成这两个网络为自适应双网络ADNet，根据输入模糊程度动态选择合适网络。

Result: 大量实验表明EG-Restormer和ADNet在保持竞争性速度的同时达到了最先进的性能，显著提升了严重模糊QR码的解码率。

Conclusion: 通过显式利用QR码的边缘先验，提出的方法在QR码去模糊任务中取得了优异性能，特别适合资源受限的移动设备应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练扩散模型的3D场景重建方法存在两个关键限制：缺乏可靠的几何监督导致重建质量差，以及缺乏有效机制缓解生成图像的多视角不一致性。
...
Conclusion: 该方法自然支持单视图输入和无位姿视频，在室内外场景中具有良好的泛化能力和实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决预训练模型在类增量学习中的三个核心挑战：模型复杂度随增量阶段增长、表示空间在增量过程中发生不连续变化、阶段优化与全局推理之间的不一致性。

Method: 1. 增量并行适配器(IPA)：基于PTM构建轻量级适配器，通过传输门实现并行连接，保证表示空间的平滑变化；2. 解耦锚点监督(DAS)：通过虚拟锚点分别约束正负样本，促进判别性表示学习并对齐不同阶段特征空间。

Result: 在六个基准测试上的广泛实验表明，DRL在整个CIL过程中始终优于其他最先进方法，同时在训练和推理阶段都保持高效率。

Conclusion: DRL框架通过增量并行适配器和解耦锚点监督有效解决了类增量学习中的关键挑战，实现了高性能和高效率的增量学习。
...
<details>
  <summary>Details</summary>
Motivation: 解决老照片人脸修复中存在的复合退化问题（如破损、褪色、严重模糊），现有方法依赖显式退化先验或全局统计指导，难以处理局部伪影和面部颜色问题。

Method: 利用预训练扩散模型在弱指导下生成伪参考人脸，通过面部解析图和划痕掩码实现选择性修复，采用分阶段监督：结构引导贯穿去噪过程，颜色细化在后期步骤进行。

Result: 在构建的VintageFace基准测试（300张真实老照片）上，SSDiff在感知质量、保真度和区域可控性方面优于现有的GAN和扩散方法。

Conclusion: SSDiff通过自监督选择性引导扩散机制，有效解决了老照片人脸修复中的复合退化问题，在保持身份一致性的同时实现了高质量的修复效果。
...
<details>
  <summary>Details</summary>
Motivation: 检索增强图像生成系统的广泛采用引发了对私有图像数据集未授权使用的担忧，传统数字水印方法在该系统中存在局限性。

Method: 利用视觉语言模型合成与原始数据集视觉一致的哨兵图像，通过随机生成的字符序列作为检索密钥进行保护验证。

Result: 实验结果表明ImageSentinel能有效检测未授权的数据集使用，同时为授权应用保持生成质量。

Conclusion: ImageSentinel为视觉数据集在RAIG系统中的保护提供了一种有效的解决方案。
...
Method: 使用梯度下降联合优化照明和编码矩阵，确保设计符合硬件约束条件，包括带宽和峰值功率限制。

Result: 通过大量仿真验证，该方法在带宽和峰值功率约束下始终优于传统编码设计，特别是在峰值功率受限系统中优势更加明显，并能适应任意参数化脉冲响应。

Conclusion: 提出的约束优化方法能够有效设计实用的编码函数，显著提升压缩单光子3D成像在真实硬件约束下的性能。
...
Method: 提出CapFlow多智能体协作工作流，利用开源模型协同工作；使用CapFlow作为数据合成器大规模生成高质量图像和视频描述；通过微调获得通用视觉描述器MetaCaptioner。

Result: CapFlow在多个视觉领域达到与GPT-4相当的描述质量，成本降低89.5%；MetaCaptioner不仅具备与商业模型相当的描述能力，还在开源社区达到顶级多模态性能。
...
<details>
  <summary>Details</summary>
Motivation: 远程生理测量需要收集用户隐私敏感信息，现有非接触式测量仍依赖带标签的客户端数据，这在更新现实世界部署模型时面临标签缺失的挑战。

Method: 提出FedHUG框架，包含：(1)最小偏置聚合模块动态调整聚合权重处理异构非IID特征；(2)全局分布感知学习控制器参数化标签分布并动态调整客户端训练策略。

Result: 在RGB视频和毫米波雷达的生理参数估计中，该方法优于现有最先进技术。

Conclusion: FedHUG框架有效解决了联邦学习环境下的隐私保护和标签缺失问题，在远程生理测量中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有CTTA方法主要通过恢复初始模型或重用历史模型来减少灾难性遗忘，但通常伴随新知识学习不足和有害历史知识干扰，导致性能严重下降。

Method: 设计了域知识分裂(KFI)模块自适应地从配对类感知域提示池中分离新域知识，减轻旧域负面知识影响；设计了域知识融合(KFU)模块以最小成本将分裂的新知识合并到现有知识池中，采用贪心知识动态合并策略提高新旧知识兼容性。

Result: 在ImageNet-C数据集上的大量实验验证了所提方法相对于其他方法的有效性。

Conclusion: KFF方法通过类感知域知识融合与分裂机制，能够动态积累判别性历史知识，有效解决连续测试时间适应中的知识遗忘和干扰问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于原型的单样本分割方法由于使用确定性平均特征，无法捕捉类内多样性，导致泛化能力不足

Method: 提出扩散原型学习框架：1）扩散原型增强模块通过前向-反向扩散过程生成多样化原型变体；2）空间感知条件机制利用原型特征统计的几何特性；3）保守融合策略平衡原型保真度和多样性

Result: 在腹部MRI和CT数据集上取得显著改进，建立了单样本医学图像分割的最新性能

Conclusion: DPL通过将原型建模为概率分布并利用扩散过程进行特征空间探索，有效解决了单样本医学图像分割中的原型表示挑战
...
<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法在多模态渲染和几何一致性方面存在不足，需要统一的框架同时处理RGB图像、深度图、表面法线和语义信息的高质量重建。

Method: 采用CUDA加速的光栅化流水线，通过可微分射线-椭球体相交计算深度，推导表面法线渲染的解析梯度公式，并引入可学习属性进行高斯剪枝优化。

Result: 定量和定性实验表明，该方法在所有模态上都达到了最先进的重建精度，验证了几何感知范式的有效性。

Conclusion: UniGS框架成功实现了高质量的多模态3D重建，在几何一致性和计算效率方面表现出色，为3D重建领域提供了有效的解决方案。
...
Conclusion: BEEP3D提供了一种高效且有效的端到端解决方案，能够在降低标注成本的同时实现高质量的3D实例分割。
...
Result: 在组合推理任务中显著提升性能，同时保持视觉问答任务的强性能，并在更先进骨干网络上验证了泛化性。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在计算机视觉领域的进展为物流等行业优化工作流程提供了新机会，但AI应用常受限于专业知识和资源，依赖通用模型。使用真实数据进行微调成本高且低效，因此合成数据成为成本效益高的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别因面部线索细微短暂且标注数据有限而具有挑战性。传统动态图像方法忽略了微表情不同时间阶段的特性，需要更精细的相位建模。

Method: DIANet双流框架：一个流编码起始到顶点阶段，另一个流编码顶点到结束阶段；每个流使用专用CNN处理，通过交叉注意力融合模块自适应整合特征。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在推理任务上表现出色，但构建高性能视觉语言推理训练数据集的原则仍不清楚。本文旨在探索有效的数据筛选策略，以提升模型的推理能力。

Method: 引入多种数据筛选方法，分析上下文来源的影响，实施针对性数据干预（如图像描述辅助信号、纯文本推理），并探索图像、问题和思维链解决方案的规模化扩展。

Result: 研究发现上下文来源策略显著影响模型性能，数据干预带来显著提升，多维度数据扩展持续改善推理能力。基于此构建的HoneyBee数据集（250万示例）使模型在多个基准测试中超越现有最先进模型。

Conclusion: 本文提出了改进的视觉语言推理数据集筛选策略，通过HoneyBee数据集和测试时扩展策略，在显著降低解码成本的同时保持准确性，为视觉语言推理研究提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 提升生成模型的推理效率是商业可行性和科学挑战的关键，但并行预测多个标记会导致结构不一致问题，传统方法缺乏回溯修正机制。

Method: 采用新颖的训练方案，在上下文中注入随机标记以提高鲁棒性，并在采样过程中实现标记修正，保持并行标记预测的效率优势。

Result: 在ImageNet-256、CIFAR-10图像生成和UCF-101、NuScenes视频生成任务中均显示出显著改进。

Conclusion: 该方法在保持高效推理的同时，显著提升了生成质量，为图像和视频生成提供了有效的自校正机制。
...
Method: 两阶段框架：1) 小波引导跨域合成生成目标域对齐的训练样本；2) 真实域噪声不变性学习构建动态噪声库，通过自监督损失学习噪声不变性。

Result: 实验结果表明该方法在多个定量指标上优于现有最先进方法，在跨域场景中表现出优异的鲁棒性。

Conclusion: Ivan-ISTD框架有效解决了红外小目标检测中的跨域和噪声挑战，创建了动态ISTD基准数据集验证方法有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频表示方法严重依赖不稳定且过于细粒度的运动和外观建模先验（像素级匹配和跟踪），跟踪误差、遮挡和大运动会导致视觉对象表示崩溃

Method: 使用分层代理节点稳定表达视觉对象的多尺度结构，通过动态表示更新机制利用视频时空先验，并采用形状和纹理表示的分离编码方式

Result: 实验表明该方法以更少的参数实现高视频重建精度，并支持复杂的视频处理任务，包括视频修复和基于关键帧的时间一致性视频编辑

Conclusion: 代理节点表示能够有效处理跟踪误差、长期运动、遮挡和视角变化，同时支持可控和细粒度的外观编辑能力
...
<details>
  <summary>Details</summary>
Motivation: 医学图像存在数据稀缺问题（隐私、伦理、标注成本高），传统交叉熵和Dice损失的加性组合对超参数敏感且在有限数据下表现不佳，需要更鲁棒高效的训练目标。

Method: 1. 乘法损失：将交叉熵和Dice损失相乘，基于预测置信度动态调节梯度；2. 置信度自适应乘法损失：应用置信度驱动的指数缩放，整合预测概率和Dice系数来强调困难样本。

Result: 在细胞和医学分割基准测试中，该框架一致优于调优的加性损失和现有损失函数。

Conclusion: 该框架为在挑战性数据限制下实现鲁棒分割提供了一种简单、有效且无需超参数调整的机制。
...
Method: 利用ID图像中的背景区域作为伪OOD特征，基于卷积的局部不变性提取背景特征，通过优化降低这些特征的L2范数来训练模型。

Result: 在多个标准OOD检测基准测试中表现出有效性，与现有后处理方法具有良好的组合兼容性，达到了新的最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督融合方法依赖人工设计的损失函数，存在参考图像细节不足、亮度不均，以及梯度损失仅关注梯度幅度的问题。
...
Conclusion: AngularFuse通过创新的角度感知框架，在可见光-红外图像融合中实现了纹理强度和边缘方向的同时保持，展现出优越的融合能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的医学基准主要评估一般医学任务，无法充分捕捉在脊柱等依赖视觉输入的细微领域的性能表现。
...
Method: 提出两个核心贡献：1）语义引导的剪枝和正则化策略，使用混合重要性度量简化非关键场景元素，同时保留导航关键对象的精细细节；2）优先级驱动的渲染管线，通过基于优先级的深度预通道来剔除被遮挡基元并加速最终着色计算。

Result: 在Waymo和KITTI数据集上的广泛实验表明，PAGS实现了卓越的重建质量（特别是在安全关键对象上），同时显著减少训练时间，并将渲染速度提升至超过350 FPS。

Conclusion: PAGS通过引入任务感知语义优先级，成功解决了动态3D城市场景重建中保真度与计算效率之间的权衡问题，为自动驾驶应用提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本-视频检索方法假设视频经过预剪辑且仅包含文本相关内容，但实际中视频通常未经剪辑且包含复杂背景内容，因此需要解决部分相关视频检索这一更具挑战性的任务。

Method: 采用双学习框架与动态知识蒸馏，包含大型教师模型和紧凑的双分支学生网络。学生网络包括继承分支（从教师模型吸收可迁移知识）和探索分支（从PRVR数据集学习任务特定信息）。引入动态软目标构建机制，用自适应软目标替代硬目标监督。

Result: 在TVR、ActivityNet和Charades-STA数据集上实现了最先进的PRVR性能。

Conclusion: 所提出的DL-DKD++框架通过动态知识蒸馏有效解决了部分相关视频检索问题，在多个基准数据集上取得了优异表现。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态推理方面取得了显著进展，但仍然容易产生幻觉，即输出结果没有基于视觉证据。本文特别关注logo幻觉这一被忽视的问题，即模型在纯符号logo上生成品牌名称或文本内容。

Method: 使用纯符号、混合和含文本logo的精选数据集，以及具有挑战性的Hard-60子集，系统测量领先VLMs的幻觉程度。通过九种结构化扰动测试鲁棒性，并对开源LLaVA进行嵌入级分析，识别与幻觉相关的投影器维度。

Result: 研究表明幻觉在强失真下仍然存在，遮挡暴露了最严重的弱点。嵌入分析显示幻觉与投影器的小部分维度相关，针对性消融能显著减少错误同时保持OCR准确性。模型特别容易对标志性圆形logo产生幻觉。

Conclusion: VLMs往往依赖符号先验而非真实字形感知，投影器子空间在这种失败模式中起决定性作用。研究为构建更可信的多模态系统提供了新的诊断视角和可行的缓解策略，特别是投影器解耦和OCR引导解码方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决街景新视角合成问题，从车辆中心视角的训练轨迹生成不同轨迹（如不同车道或行驶方向）的渲染视图。

Method: 采用两阶段方法：首先通过3D场景重建渲染新视角，然后使用专用单步扩散模型增强图像质量。重点优化了高斯基元初始化和增强模型的微调。

Result: 在公开测试排行榜上获得综合得分0.432，总体排名第二。通过PSNR、SSIM和LPIPS指标评估了新视角质量。

Conclusion: 提出的混合方法在街景新视角合成任务中表现优异，证明了高斯泼溅与扩散模型结合的有效性。
...
TL;DR: 提出了一种可学习的自适应子采样框架，通过注意力引导的动态采样机制来减少深度神经网络的计算复杂度，同时保持性能。
...
Method: 提出注意力引导的采样模块，该模块在推理过程中也能适应不同输入，解决了传统方法只能任务自适应而不能输入自适应的问题。

Result: 在MedMNIST3D医学影像数据集和两个超声视频数据集上验证了方法的有效性，其中一个是在真实临床条件下收集的具有挑战性的内部数据集。

Conclusion: 动态自适应采样机制能够在减少深度神经网络模型复杂度的同时实现性能提升，适用于实际应用场景。
...
TL;DR: 提出了STORM-PSR双流框架，通过结合空间和时间特征来改进过程步骤识别，在物体部分遮挡时仍能准确识别步骤完成。
...
Method: 采用双流框架：装配状态检测流处理无遮挡物体视图，时空流包含空间编码器和基于Transformer的时间编码器，捕获空间和时间特征以识别步骤完成。

Result: 在MECCANO和IndustReal数据集上，相比先前方法，平均延迟分别减少了11.2%和26.1%。

Conclusion: 时空流不依赖无遮挡物体视图即可推断步骤完成，显著减少了预测延迟，证明了时空特征在过程步骤识别中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当训练图像缺乏足够多视角约束时，传统的场景坐标回归模型会退化，需要引入先验知识来提升模型性能。

Method: 采用概率重解释方法，引入从简单深度分布先验到基于大规规模室内扫描数据训练的3D点云扩散模型等多种先验，在训练过程中将预测的3D场景点推向合理的几何结构。

Result: 在三个室内数据集上，该方法能够学习到更好的场景表示，产生更连贯的场景点云、更高的配准率和更好的相机位姿，对下游任务如新视角合成和相机重定位有积极影响。

Conclusion: 通过引入高级重建先验，可以有效改善场景坐标回归模型在训练数据约束不足时的性能退化问题，提升场景表示质量和下游任务效果。
...
TL;DR: 本文系统综述了视觉语言模型(VLMs)在城市基础设施监控中的应用，特别关注零样本学习能力，分析了32篇相关研究以回答四个核心研究问题。


<details>
  <summary>Details</summary>
Motivation: 传统城市基础设施监控方法依赖物联网传感器和人工检查，成本高、难以扩展，且与市民的视觉感知存在差异。研究探索VLMs是否能像市民一样"看见"并评估城市基础设施状况。

Method: 采用PRISMA系统综述方法，分析了2021-2025年间发表的32篇同行评审研究，重点关注零样本应用。

Result: 综述识别了VLMs在城市监控中的有效应用任务、常用架构框架、支持数据集资源以及评估方法和性能水平。

Conclusion: VLMs展示了在城市基础设施监控中的巨大潜力，特别是其零样本学习能力为解决传统监控方法的局限性提供了新的技术路径。
...
TL;DR: 提出基于条件流匹配(CFM)的图像质量转移框架，用于从低场MRI重建高场MRI图像，在减少参数量的同时实现最先进性能。
...
Result: CFM不仅实现了最先进的性能，还能稳健地泛化到分布内和分布外数据，同时使用的参数量显著少于竞争性深度学习方法。
...
TL;DR: 提出了VideoLucy框架，通过分层记忆结构和迭代回溯机制解决长视频理解中的时序上下文丢失和关键信息遗漏问题，在多个基准测试中表现优于现有方法。
...
Method: VideoLucy采用分层记忆结构，模拟人类从粗到细的回忆过程，通过基于代理的迭代回溯机制系统挖掘视频范围内的深层记忆，直到收集足够信息给出答案。

Result: 在多个长视频理解基准测试中，VideoLucy显著优于现有最先进方法，甚至超越了GPT-4o等专有模型的性能。

Conclusion: VideoLucy通过分层记忆和回溯机制有效解决了长视频理解中的时序理解和细节保留问题，为长视频理解提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统胸部X光报告生成方法依赖单张图像，无法提供临床所需的纵向对比分析。随着对纵向数据应用的关注增加，需要系统性的框架来指导模型设计。

Method: 综述了数据集构建策略、报告生成架构（特别是纵向定制设计）以及包含纵向特定指标和常用基准的评估协议。

Result: 总结了LRRG方法的性能表现，通过消融研究分析表明纵向信息和架构设计选择对提升模型性能具有关键作用。

Conclusion: 指出了当前研究的五大主要局限性，并为这一新兴领域的未来发展指明了有前景的方向。
...
TL;DR: MS-GAGA是一个两阶段的黑盒对抗攻击框架，针对深度伪造检测器生成可迁移且视觉不可察觉的对抗样本。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法在深度伪造检测器上的可迁移性和视觉不可察觉性不足，需要一种能同时优化这两个指标的攻击框架。

Method: 第一阶段使用双流攻击模块：MNTD-PGD针对小扰动预算优化梯度计算，SG-PGD聚焦于视觉显著区域；第二阶段通过度量感知选择模块基于攻击成功率和结构相似性评估候选样本。

Result: 在未见过的检测器上，MS-GAGA相比最先进攻击方法实现了高达27%的更高误分类率。

Conclusion: MS-GAGA通过两阶段设计和联合优化策略，有效提升了对抗样本的可迁移性和视觉不可察觉性。
...
Method: 设计早期融合框架，在数据增强前融合文本和视觉特征；使用轻量级生成器将文本嵌入投影到视觉空间，弥合语义差距。

Result: 在三个医学影像任务和四个分割框架上评估，实现了最先进的性能；生成伪图像显示准确的区域定位。

Conclusion: 提出的早期融合方法有效解决了多模态学习中数据增强破坏空间一致性的问题，在医学图像分割中取得了优异结果。
...
TL;DR: 提出了Bi-Stage 3D Gaussian Splatting (BSGS)框架，用于从运动模糊图像中准确重建3D场景，通过两阶段优化和子帧梯度聚合策略解决现有方法的局限性。
...
Method: BSGS包含两个阶段：相机姿态精炼阶段粗略优化相机姿态以减少运动引起的失真；全局刚性变换阶段进一步校正运动模糊失真。采用子帧梯度聚合策略优化两个阶段，并引入时空双阶段优化策略动态调整基元密集化阈值。

Result: 综合实验验证了所提出的去模糊方法的有效性，并显示其在性能上优于现有最先进方法。

Conclusion: BSGS框架能够有效解决从运动模糊图像重建高质量3D场景的挑战，通过两阶段优化和梯度聚合策略显著提升了重建质量。
...
TL;DR: 提出VAD方法，一种轻量级、无网络的方法，直接从无定向点云计算无符号距离场，通过Voronoi辅助扩散处理各种拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 现有神经方法学习无符号距离场存在数值不稳定、计算成本高和可控性有限的问题，需要更稳定高效的解决方案。

Method: 使用Voronoi辅助扩散：首先基于Voronoi几何准则为输入点分配双向法向量，然后扩散法向量形成近似UDF梯度场，最后积分得到最终UDF。

Result: 实验表明VAD能稳健处理水密和开放表面，以及复杂的非流形和非定向几何结构，同时保持计算效率和稳定性。

Conclusion: VAD提供了一种计算高效、数值稳定的无符号距离场计算方法，能够处理各种复杂拓扑结构。
...
TL;DR: 本文提出了一种基于分数扩散模型的无条件人体运动生成方法，仅通过特征空间归一化和分析推导的L2分数匹配损失权重，就能达到最先进水平，同时直接生成运动和形状，避免了缓慢的关节后处理形状恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的人体运动生成方法（如VAE、GAN、扩散模型）通常依赖过度参数化的输入特征和辅助损失来改善结果，但这些策略对于扩散模型匹配人体运动分布并非严格必要。

Method: 使用基于分数的扩散模型，仅采用特征空间归一化和分析推导的L2分数匹配损失权重，直接生成运动和形状。

Result: 在无条件人体运动生成任务中达到了与最先进方法相当的结果。

Conclusion: 通过逐步构建方法并针对每个组件提供理论动机和针对性消融实验，证明了所提出方法的有效性。
...
TL;DR: 提出了首个无人机多光谱多目标跟踪基准MMOT，包含125个视频序列和48.8万标注，并开发了光谱3D-Stem、方向感知卡尔曼滤波器和端到端方向自适应变换器，显著提升了小目标和密集场景的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有RGB跟踪算法在无人机视角下依赖空间外观线索（颜色、纹理）容易退化，而多光谱图像提供像素级光谱反射率，能在空间条件退化时增强目标区分能力，但缺乏专用多光谱无人机数据集阻碍了该领域发展。

Method: 1）构建大规模多光谱无人机跟踪基准MMOT；2）开发轻量级光谱3D-Stem整合光谱特征并保持RGB预训练兼容性；3）方向感知卡尔曼滤波器进行精确状态估计；4）端到端方向自适应变换器。

Result: 大量实验表明，多光谱输入相比RGB基线显著提升了跟踪性能，特别是对小目标和密集目标效果更明显。

Conclusion: 该工作将推动无人机多光谱多目标跟踪研究发展，MMOT数据集、代码和基准测试已公开。
...
<details>
  <summary>Details</summary>
Motivation: 基于时间相关输入信号学习人体运动是一个具有挑战性但影响广泛的任务。现有方法主要依赖交叉注意力机制融合条件与运动，但这种方法主要捕获全局交互，难以维持逐步的时间对齐。

Method: 引入Temporally Conditional Mamba模型，将条件信息整合到Mamba块的循环动态中，实现更好的时间对齐运动生成。

Result: 在多种人体运动任务上的广泛实验表明，该模型在时间对齐、运动真实性和条件一致性方面显著优于现有最先进方法。

Conclusion: Temporally Conditional Mamba模型通过改进的条件整合机制，有效解决了人体运动生成中的时间对齐问题，为基于条件输入的运动生成提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决农业图像分割中的标注瓶颈问题，避免收集和标注新数据集的成本，利用现有植物专用模型来适应复杂的田间条件。

Method: 使用Plantnet的植物专用表征识别植物区域生成粗分割掩码，然后通过SAM进行精细化分割，比较了Plantnet微调DinoV2与基础DinoV2模型的性能。

Result: 在四个不同复杂度的公开数据集上评估，使用Plantnet微调DinoV2相比基础DinoV2模型在IoU指标上获得了一致的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明扩散模型的生成质量与其学习到的表示相关，外部指导可以加速训练。但作者认为模型自身的中间表示质量差异可以相互指导，从而减少对外部监督的需求。

Method: 提出LayerSync自监督正则化方法，利用扩散模型中语义丰富的表示层来指导较弱的表示层，无需额外开销即可提升训练效率。

Result: 在图像生成任务上显著提升训练效率（flow-based transformer在ImageNet上训练加速8.75倍）和生成质量（提升23.6%），并成功应用于音频、视频和运动生成等其他领域。

Conclusion: LayerSync是一种高效、通用的扩散模型正则化方法，能够在不增加额外成本的情况下显著提升生成质量和训练效率，且适用于多种模态。
...
Method: 两阶段训练框架：第一阶段预训练编码器从干净图像中捕获语义，并与确定性采样轨迹对齐；第二阶段将编码器与随机初始化解码器集成，对完整模型进行端到端微调。

Result: 在ImageNet-256上扩散模型达到FID 2.04，ImageNet-512上达到FID 2.35（75 NFE）；一致性模型在ImageNet-256上单步采样达到FID 8.82，显著超越潜在空间对应模型。

Conclusion: 该框架在生成质量和效率方面大幅超越先前的像素空间方法，在可比较的训练成本下与领先的VAE模型相媲美，并且首次成功训练了不依赖预训练VAE或扩散模型的高分辨率一致性模型。
...
TL;DR: 提出IVT-LR方法，通过潜在空间中的视觉-文本交错推理，实现多模态推理的高效性和减少标注依赖
...
Method: IVT-LR在潜在空间中注入视觉和文本信息，将推理步骤表示为潜在文本和潜在视觉的组合，采用渐进式多阶段训练策略

Result: 在M3CoT和ScienceQA上平均准确率提升5.45%，推理速度比现有方法快5倍以上

Conclusion: IVT-LR方法有效解决了多模态推理中的标注依赖和推理效率问题
...
TL;DR: 提出WaterFlow框架，通过将水下物理成像信息作为显式先验引入网络训练，并引入时间维度建模，显著提升水下显著目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法往往忽略水下成像的物理原理，或将水下图像退化现象单纯视为干扰因素，未能充分利用其中包含的有价值信息。

Method: 基于整流流的框架，将水下物理成像信息作为显式先验直接融入网络训练过程，并引入时间维度建模。

Result: 在USOD10K数据集上，S_m指标提升0.072，证明了方法的有效性和优越性。

Conclusion: WaterFlow框架通过充分利用水下物理成像信息，显著提升了水下显著目标检测的性能。
...
TL;DR: 提出ZSCFC方法，一种基于跨频一致性的零样本去噪器，能够在单张噪声图像上进行训练和去噪，无需依赖噪声分布假设，在计算效率和去噪性能上优于现有零样本方法。
...
Method: 利用图像纹理在不同频带具有位置相似性和内容一致性的特性，而噪声不具备此特性，开发了跨频一致性损失和超轻量网络来实现图像去噪。

Result: 在多个真实世界图像数据集上的实验表明，ZSCFC在计算效率和去噪性能上优于其他最先进的零样本方法。

Conclusion: ZSCFC方法能够有效处理真实世界去噪问题，无需噪声分布假设，且具有优异的计算效率和去噪性能。
...
TL;DR: 开发简单高效的人体网格恢复和人体姿态估计模型，通过利用分层视觉基础模型的早期阶段作为编码器，在保持性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的人体网格恢复方法依赖大型非分层视觉变换器作为编码器，计算开销大。希望建立跨不同计算预算的基准，并探索分层视觉基础模型早期阶段的潜力。

Method: 构建三个轻量级HMR2.0变体，并利用Swin Transformer、GroupMixFormer和VMamba等分层视觉基础模型的前两到三个阶段作为编码器，替代传统的全阶段模型。

Result: 评估了27个基于分层视觉基础模型的HMR和HPE模型，发现仅使用前两到三个阶段就能达到与全阶段模型相当的性能，且在准确性和计算效率之间取得更好的平衡。

Conclusion: 分层视觉基础模型的早期阶段能够有效替代全阶段模型，为人体网格恢复和人体姿态估计任务提供更高效的计算解决方案。
...
TL;DR: TerraCodec (TEC) 是一个专门为地球观测数据设计的深度学习压缩算法家族，包括图像版本和时序Transformer模型，在Sentinel-2数据上实现了比传统编解码器3-10倍的压缩效果，并支持零样本云修复。


<details>
  <summary>Details</summary>
Motivation: 地球观测卫星产生大量多光谱图像时间序列数据，现有压缩方法存在存储传输挑战，学习型压缩方法缺乏公开预训练模型，且传统图像编解码器忽略时间冗余，视频编解码器无法捕捉静态场景的辐射演化特征。

Method: 提出TerraCodec系列，包括适应多光谱输入的高效图像版本和利用时间依赖性的时序Transformer模型(TEC-TT)，并引入Latent Repacking方法训练灵活码率的Transformer模型。

Result: 在Sentinel-2数据上训练，TerraCodec在同等图像质量下比传统编解码器实现3-10倍更强的压缩，TEC-TT在AllClear基准测试中的零样本云修复性能超过现有最优方法。

Conclusion: 专门定制的学习型压缩算法是地球观测数据处理的可行方向，代码和模型权重将在宽松许可下发布。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于鸟瞰图的无人机群协作感知方法存在两个主要局限：边界框表示无法捕捉完整的场景语义和几何信息，以及在遇到未定义或被遮挡物体时性能显著下降。

Method: 提出多无人机协作占用预测框架，包含空间感知特征编码器、跨智能体特征集成、高度感知特征缩减和双掩码感知引导机制，有效减少通信开销。

Result: 实验结果表明，该方法在三个扩展数据集（两个虚拟数据集和一个真实世界数据集）上实现了最先进的精度，显著优于现有协作方法，同时将通信开销降低到先前方法的仅一小部分。

Conclusion: 所提出的协作占用预测框架能够有效解决无人机群协作感知中的关键挑战，在保持高精度的同时大幅降低通信需求，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在带噪声标签的开放集域泛化中难以弥合域间差距，特别是在干净标注数据有限的情况下

Method: 采用两阶段证据损失聚类提升标签可靠性感知，提出残差流匹配机制建模结构化的域和类别条件残差，通过元学习优化模型更新方向

Result: 在OSDG-NL任务上优于现有方法，达到最先进性能
...
TL;DR: 提出混合解释引导学习(H-EGL)框架，结合自监督和人工引导约束来增强注意力对齐，提高医学影像分类的泛化能力。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在医学影像中表现优异，但容易学习虚假相关性导致偏见和泛化能力受限。人工-AI注意力对齐可缓解此问题，但依赖昂贵的监督标注。

Method: H-EGL框架包含自监督组件，利用类别区分性注意力而不依赖限制性先验，促进鲁棒性和灵活性。在胸部X光分类中使用Vision Transformer进行验证。

Result: H-EGL在胸部X光分类中优于两种最先进的解释引导学习方法，展现出更优的分类准确率和泛化能力，生成的注意力图与人类专家知识更一致。

Conclusion: H-EGL框架通过结合自监督和人工引导约束，有效增强了注意力对齐，提高了医学影像分类的性能和泛化能力。
...
TL;DR: IRIS是一个评估多模态大语言模型在"与图像一起思考"范式下能力的基准，包含1,204个跨五个领域的视觉任务，显示当前模型在整合视觉和通用工具方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要遵循"关于图像思考"的范式，将图像视为静态输入，而现实场景中用户提供的图像往往不完美，需要主动的图像操作来发现重要视觉线索。

Method: 引入IRIS基准，包含1,204个具有挑战性的开放式视觉任务（603个单轮，601个多轮），涵盖五个不同领域，每个任务都配有详细的评分标准以进行系统评估。

Result: 评估显示当前MLLMs在需要有效整合视觉和通用工具的任务上表现不佳，最强的模型（GPT-5-think）仅达到18.68%的通过率。OpenAI模型受益于多样化的图像操作，而Gemini-2.5-pro则没有改善。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在医疗AI中应用面临数据共享限制和隐私保护问题，联邦学习虽然能解决数据聚合问题，但需要更有效的个性化微调方法来充分利用各客户端数据。

Method: 使用正交LoRA适配器来解耦通用知识和客户端特定知识，在联邦学习框架下进行个性化微调。

Result: 在真实世界联邦医学影像任务上的初步结果显示，该方法与当前联邦微调方法相比具有竞争力。

Conclusion: 所提出的个性化联邦微调方法能有效解决医疗数据隐私保护下的模型微调问题，为医疗AI应用提供了可行解决方案。
...
TL;DR: FlashVSR是首个基于扩散模型的实时视频超分辨率框架，通过三阶段蒸馏、稀疏注意力和微型解码器实现17 FPS的高效处理，在A100 GPU上支持768x1408分辨率。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频超分辨率方法存在延迟高、计算量大、对超高分辨率泛化能力差等问题，需要实现高效、可扩展且实时的解决方案。

Method: 提出三阶段蒸馏管道实现流式超分辨率，使用局部约束稀疏注意力减少冗余计算，采用微型条件解码器加速重建，并构建了包含12万视频和18万图像的VSR-120K数据集。

Result: FlashVSR在超高分辨率下可靠扩展，达到最先进性能，相比之前的一步扩散VSR模型速度提升高达12倍。

Conclusion: FlashVSR使基于扩散模型的视频超分辨率变得实用，实现了效率、可扩展性和实时性能的平衡，将发布代码、预训练模型和数据集以促进相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型中，强大的视觉理解能力往往无法有效转移到视觉生成上，模型能够正确理解图像但无法从文本提示生成忠实图像。
...
Method: 提出SPORTS框架，将VPS、VO和SR任务紧密集成：VPS使用自适应注意力几何融合机制对齐跨帧特征；VO结合全景分割结果和光流图改进动态物体置信度估计；SR通过点云渲染生成神经场来合成高保真视图。
...
TL;DR: 提出了VQArt-Bench，一个用于文化遗产领域的大规模视觉问答基准，通过多智能体管道生成多样化问题，评估多模态大语言模型在深度语义理解方面的能力。
...
Method: 使用新颖的多智能体管道，让专门化的智能体协作生成经过验证的、语言多样化的细微问题，构建沿相关视觉理解维度结构化的基准。
...
TL;DR: 提出了一种名为E-MoFlow的无监督框架，通过隐式时空和几何正则化联合优化自运动和光流估计，在神经形态视觉中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法将光流和6-DoF自运动估计分开处理，对于缺乏鲁棒数据关联的神经形态视觉来说是一个不适定问题。现有方法要么引入偏差和计算开销，要么容易陷入局部最优解。

Method: 将相机自运动建模为连续样条，光流建模为隐式神经表示，通过归纳偏置嵌入时空一致性；结合微分几何约束引入结构和运动先验，避免显式深度估计。

Result: 在一般6-DoF运动场景中表现出色，在无监督方法中达到最先进性能，甚至与有监督方法相媲美。

Conclusion: E-MoFlow框架通过隐式正则化在完全无监督范式下统一了自运动和光流估计，解决了神经形态视觉中的不适定问题。
...
TL;DR: 提出基于深度学习和交叉注意力的头部运动校正方法DL-HMC++，从1秒3D PET原始数据预测刚性头部运动，无需外部硬件跟踪设备，在多种PET扫描仪和放射性示踪剂上表现优异。


<details>
  <summary>Details</summary>
Motivation: 头部运动在脑PET成像中导致图像伪影和示踪剂摄取定量不准确，硬件运动跟踪在临床实践中适用性有限，需要开发数据驱动的运动校正方法。

Method: 利用现有动态PET扫描和外部硬件运动跟踪的金标准运动测量数据，以监督学习方式训练DL-HMC++模型，使用交叉注意力机制从1秒3D PET原始数据预测刚性头部运动。

Result: 在HRRT和mCT两种PET扫描仪及四种放射性示踪剂上的评估显示，DL-HMC++始终优于最先进的数据驱动运动估计方法，生成的运动伪影消除图像与金标准HMT难以区分，标准摄取值分析差异比HRRT为1.2±0.5%，mCT为0.5±0.2%。

Conclusion: DL-HMC++展示了数据驱动PET头部运动校正的潜力，可消除HMT的负担，使运动校正能够应用于研究环境之外的临床人群。
...
TL;DR: AnyUp是一种无需特定编码器训练的特征上采样方法，可应用于任何视觉特征和分辨率，具有特征无关性和泛化能力。
...
Conclusion: AnyUp是一种高效、易于应用且具有泛化能力的特征上采样方法，为各种视觉任务提供了更好的特征表示。
...
TL;DR: 本文提出了高效感知超分辨率(EPSR)的全面研究和基准测试，旨在在严格效率约束下(5M参数、2000 GFLOPs)实现或超越Real-ESRGAN的感知质量结果。


<details>
  <summary>Details</summary>
Motivation: 虽然高效PSNR导向的超分辨率取得了显著进展，但专注于感知质量指标的方法仍然相对低效，存在这一研究空白。

Method: 在严格效率约束下(最大5M参数和2000 GFLOPs)开发解决方案，使用包含500张4K测试图像的新数据集进行评估，每张图像使用多种退化类型处理。
...
<details>
  <summary>Details</summary>
Motivation: 理解物理场景动态需要推理其可能的变化方式，特别是局部交互带来的影响。传统方法通常只能密集采样单一动态实现，缺乏对多模态运动及其不确定性的直接表示。

Method: FPT框架基于Transformer架构，通过稀疏交互条件直接预测局部运动分布，提供可解释的运动表示。

Result: 在密集面部运动生成任务中，通用预训练模型超越专用基线；在强分布外任务中，经过微调的FPT在关节物体运动估计上显著优于域内方法；在基于pokes的运动部件分割任务中取得竞争性表现。

Conclusion: FPT提供了一种灵活且可解释的方法来直接预测多模态场景运动分布，在各种下游任务中展现出优越性能和泛化能力。
...
TL;DR: AIM 2025高效真实世界图像去模糊挑战赛回顾，使用单张图像进行高效去模糊，参赛方案需满足少于500万参数和200 GMACs计算预算，最佳方案PSNR达31.1298 dB。


<details>
  <summary>Details</summary>
Motivation: 推动高效真实世界图像去模糊技术的发展，解决传统方法在计算效率上的限制，促进实际应用部署。

Method: 基于RSBlur数据集构建新测试集，使用双相机系统采集模糊和清晰图像对，要求参赛方案在严格效率约束下进行去模糊处理。

Result: 71个团队注册，4个团队提交有效方案，最佳方案PSNR达到31.1298 dB，展示了高效方法在真实世界图像去模糊中的潜力。
...
Method: 使用冻结的VLM作为统一编码器，提出层注意力池化机制提取多层级信息，并开发VERIFI方法在推理时仅使用VLM生成的文本token进行条件生成。

Result: LAP机制在文本-图像对齐方面优于其他浅层融合架构，VERIFI结合了VLM的推理能力，编辑任务微调不仅改善生成对齐，还展现出强大的泛化能力。

Conclusion: UniFusion的统一编码器设计实现了跨模态知识迁移，在单图像编辑训练后能够零样本泛化到多图像参考，验证了该设计的有效性。
...
TL;DR: 提出ViCO方法，通过视觉一致性学习让多模态大语言模型根据图像语义复杂度动态调整视觉token数量，减少50%推理成本的同时保持感知能力
...
Method: 使用多个不同压缩率的MLP连接器对视觉token进行下采样，通过KL散度最小化训练，推理时使用ViR路由器自动选择合适压缩率
...
<details>
  <summary>Details</summary>
Motivation: CMP虽然为图像拓扑分析提供了强大工具，但由于多滤过结构的复杂性和CMP向量化的困难，其应用受到限制。

Method: 提出新算法向量化立方复形的多参数同调，将CMP分解为可学习的单参数持久性组合，联合学习双滤过函数，并确保可微性。

Result: 在基准医学影像和计算机视觉数据集上的实验表明，CuMPerLay在分类和分割任务中提升了性能，特别是在数据有限的情况下。
...
TL;DR: DriveVLA-W0通过世界建模预测未来图像来解决VLA模型的监督不足问题，使用自回归和扩散两种世界模型架构，在NAVSIM基准测试中显著超越BEV和VLA基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决视觉-语言-动作(VLA)模型在大规模数据上的监督不足问题，即模型容量巨大但只通过稀疏的低维动作进行监督，导致表示能力未充分利用。

Method: 提出DriveVLA-W0训练范式，使用世界建模预测未来图像作为密集的自监督信号；为两种主要VLA架构分别实现自回归世界模型（离散视觉标记）和扩散世界模型（连续视觉特征）；引入轻量级动作专家解决实时部署的推理延迟。

Result: 在NAVSIM v1/v2基准测试和680倍更大的内部数据集上，DriveVLA-W0显著优于BEV和VLA基线；更重要的是，它放大了数据缩放定律，显示随着训练数据集增大，性能增益加速提升。

Conclusion: 世界建模是解决VLA模型监督不足的有效方法，能够学习驾驶环境的底层动态，并通过轻量级动作专家实现实时部署，在数据缩放方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有MLLM在物体检测任务中面临的低召回率、重复预测、坐标不对齐等问题，弥合MLLM与传统回归模型在物体感知性能上的差距。

Method: 1) 使用特殊token表示量化坐标；2) 构建多个数据引擎生成高质量标注数据；3) 采用两阶段训练流程：监督微调+基于GRPO的强化学习后训练。

Result: 在COCO和LVIS基准测试中，Rex-Omni在零样本设置下达到或超过了回归模型（如DINO、Grounding DINO）的性能。

Conclusion: Rex-Omni为构建更通用和语言感知的视觉感知系统铺平了道路，展示了MLLM在物体检测领域的巨大潜力。
...
TL;DR: DeepMMSearch-R1是首个能够按需进行多轮网络搜索的多模态大语言模型，可以动态构建图像和文本搜索查询，通过两阶段训练实现更有效的多模态信息检索。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成方法存在流程僵化、搜索调用过多和查询构建不佳等问题，导致效率低下和结果不理想。需要开发能够动态适应真实世界变化信息的多模态搜索能力。

Method: 采用两阶段训练：冷启动监督微调阶段和在线强化学习优化。创建了DeepMMSearchVQA多模态问答数据集，包含多样化的多跳查询，教导模型何时搜索、搜索什么、使用哪种搜索工具以及如何推理检索到的信息。

Result: 在多个知识密集型基准测试中进行了广泛实验，证明了该方法的优越性。模型能够基于输入图像的相关裁剪启动网络搜索，使图像搜索更有效，并能迭代调整文本搜索查询。

Conclusion: 该方法为推进多模态网络搜索提供了有价值的见解，通过动态查询构建和迭代搜索策略显著提升了多模态信息检索的效果。
...
TL;DR: LLMs在逻辑谜题上表现出色但依赖记忆而非推理，当谜题被轻微修改时性能崩溃。研究引入PHANTOM RECALL基准测试，发现模型存在'幻影回忆'现象，并提出了检测和缓解工具。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在逻辑谜题中是否真正进行推理，还是仅仅依赖记忆模板，以及如何系统评估和改善这种局限性。

Method: 引入PHANTOM RECALL基准测试，包含25个经典逻辑谜题和149个精心设计的扰动版本；评估11个领先LLM；开发自动逻辑等价判断器、细粒度错误分类法和基于提示的缓解框架。

Result: 在未修改谜题上准确率接近完美，但在扰动谜题上表现显著低于人类水平，出现幻影回忆和过度阐述问题。
...
TL;DR: LLMs作为世界模型存在幻觉和长期模拟能力不足的问题，提出的R-WoM通过检索增强显著提升了长时程模拟性能。


<details>
  <summary>Details</summary>
Motivation: LLMs作为世界模型可以增强智能体决策，但其幻觉倾向和静态知识依赖导致长期模拟中的复合错误，限制了实际应用。

Method: 通过三个任务评估LLMs的世界建模能力，并提出检索增强世界模型(R-WoM)，通过检索外部教程知识来增强LLM模拟的准确性。

Result: LLMs能有效捕捉即时状态和重要状态转换，但在完整过程规划中性能快速下降。R-WoM在OSWorld和WebArena上分别实现了25.3%和18.1%的性能提升。

Conclusion: LLMs在长期环境动态建模方面存在局限性，而检索增强方法能显著改善其世界建模能力，特别是在长时程模拟任务中。
...
TL;DR: 研究发现LLM对输入的表层形式变化非常敏感，即使语义保持不变，内部真伪陈述的表征可分离性也会显著下降，这表明LLM学习的是浅层、非鲁棒的知识表示。


<details>
  <summary>Details</summary>
Motivation: 探索LLM性能脆弱性是否源于内部知识表示的不稳定性，特别是测试在语义保持但表层形式变化的输入下，真伪陈述表征分离能力的退化情况。

Method: 通过应用语义保持的扰动（如拼写错误、重述等）使样本分布外，在四个LLM家族、五个评估数据集和三种知识探测方法上测试真伪陈述表征的可分离性。

Result: 当样本呈现形式与预训练数据相似度降低时，内部真伪陈述表征的分离能力会崩溃，LLM区分真伪陈述的能力高度依赖于确切的表层形式。
...
TL;DR: 大型推理模型在机器翻译任务中生成中间"思考标记"并没有带来性能提升，但通过模块化翻译特定提示策略构建的中间标记能带来改进。


<details>
  <summary>Details</summary>
Motivation: 探索大型推理模型在机器翻译任务中生成中间思考过程的影响，特别是在不同资源水平和设置下的多语言对翻译中。

Method: 使用蒸馏思维链方法微调模型，生成详细的分步翻译解释作为中间标记，并与标准输入输出微调进行比较。

Result: 思考标记不能帮助大型推理模型更好地执行机器翻译，基于人类翻译实践的精简思维链微调也不优于标准微调。但通过模块化翻译特定提示策略构建的中间标记能带来改进。

Conclusion: 微调期间中间标记的贡献高度依赖于其中是否包含翻译尝试，使用教师模型改进目标翻译或扩展平行语料库比将思维链解释蒸馏到"思考"翻译模型中更有效。
...
TL;DR: 提出了MIND系统，一个用户参与的事实核查流程，用于检测多语言问答知识库中的事实和文化差异
...
Method: MIND是一个用户参与的事实核查流程，能够识别文化敏感问题中的分歧答案

Result: 在母婴健康领域的双语问答系统上评估MIND，并发布了标注事实和文化不一致的双语问题数据集

Conclusion: MIND能可靠识别不一致性，支持开发更具文化意识和事实一致性的问答系统
...
TL;DR: TopoAlign框架利用代码仓库作为数学大语言模型的训练资源，通过将代码分解为文档字符串、主函数和依赖函数，并重新组装成结构上类似形式数学语句的代码数据，从而提升模型的自动形式化能力。


<details>
  <summary>Details</summary>
Motivation: 当前数学大语言模型在自动形式化方面表现不佳，主要受限于大规模语料库的稀缺，特别是包含非正式和形式数学语句对的数据。虽然现有模型能够从自然语言指令生成代码，但代码与形式数学在结构和语法上的差异限制了有效的迁移学习。

Method: 提出TopoAlign框架，将代码分解为文档字符串、主函数和依赖函数，然后重新组装这些组件，使其在结构上镜像形式数学语句，从而生成结构对齐的代码数据用于训练数学大语言模型，无需额外的人工标注。

Result: 在DeepSeek-Math模型上，TopoAlign显著提升了性能，BEq@10指标提高了17.77%，typecheck@10指标提高了68.82%。对于Herald模型，尽管没有引入新的数学知识，BEq@10和typecheck@10分别提高了0.12%和1.09%。

Conclusion: TopoAlign框架证明，即使对于专门的模型，在结构对齐的代码数据上进行训练也是有益的，能够有效提升数学大语言模型的自动形式化能力。
...
Result: 在400名亚马逊用户的图书描述生成任务中，相比提示条件、标准微调和朴素合成对生成方法，GRAVITY实现了超过4%的偏好增益，用户研究显示其输出在86%的情况下被优先选择。
...
TL;DR: 提出了CRUMQs管道，用于自动生成不可作弊、真实、不可回答和多跳查询的RAG基准测试，显著提升了RAG系统评估的挑战性和真实性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG基准测试难以反映真实任务复杂性，多跳或超出范围的问题容易被作弊解决，限制了发现现有RAG系统局限性的能力。
...
TL;DR: 提出直接多令牌解码（DMTD）方法，通过仅使用后期层生成多个令牌来加速LLM推理，无需重复遍历早期和中间层。


<details>
  <summary>Details</summary>
Motivation: 基于预训练LLM中不同层级承担不同功能的观察：早期层理解输入上下文，中间层处理任务特定处理，后期层将抽象表示转换为输出令牌。假设经过早期和中间层处理后，隐藏状态已包含足够信息支持仅用后期层生成多个令牌。

Method: DMTD方法：仅使用后期层生成多个令牌，无需额外参数、辅助程序或后生成验证。在有限数据集上对Qwen3-4B模型进行微调。

Result: 微调后的DMTD Qwen3-4B模型实现了高达2倍的加速，仅带来轻微性能损失。缩放分析表明使用更大训练数据集性能有望进一步提升。

Conclusion: DMTD是一种有效的LLM推理加速方法，无需额外资源即可实现显著速度提升，且具有良好扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在处理长视野任务时受到上下文长度的根本限制，需要更有效的上下文管理方法。

Method: 开发了端到端强化学习框架FoldGRPO，通过特定过程奖励来鼓励有效的任务分解和上下文管理，使智能体能够程序化地分支处理子任务并折叠中间步骤。

Result: 在复杂长视野任务（深度研究和软件工程）上，折叠智能体匹配或优于ReAct基线，同时使用的活动上下文小10倍，并显著优于基于摘要的上下文管理模型。

Conclusion: Context-Folding框架有效解决了LLM智能体的上下文长度限制问题，为长视野任务提供了更高效的上下文管理解决方案。
...
TL;DR: 论文提出了ConjectureBench数据集和评估框架，专门衡量LLMs的猜想能力，发现当前模型在自动形式化任务中的表现被高估，并设计了Lean-FIRe方法来改进猜想和自动形式化性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自动形式化研究忽视了猜想这一关键前置步骤，许多数学问题需要先猜想结论才能进行形式化。LLMs在自动形式化方面已有困难，而对其猜想能力的评估又很有限且常与自动形式化或证明任务混淆。

Method: 扩展现有数据集创建ConjectureBench，重新设计评估框架和指标来专门衡量LLMs的猜想能力；设计了推理时方法Lean-FIRe来改进猜想和自动形式化。

Result: 评估显示当考虑猜想时，GPT-4.1和DeepSeek-V3.1等基础模型的自动形式化性能被显著高估；使用Lean-FIRe方法，GPT-4.1成功端到端自动形式化了13个PutnamBench问题，DeepSeek-V3.1成功形式化了7个。
...
TL;DR: SAGE是一个用于多轮交互智能体评估的用户模拟框架，通过整合业务上下文知识来生成更真实多样的交互，能发现更多智能体错误。


<details>
  <summary>Details</summary>
Motivation: 现有的多轮交互智能体评估方法依赖人工评估，而模拟用户方法通常建模通用用户，忽略了领域特定原则，无法捕捉真实行为。

Method: 提出SAGE框架，整合自上而下的业务逻辑知识（如理想客户画像）和自下而上的业务基础设施知识（如产品目录、FAQ、知识库），基于真实客户角色生成交互。

Result: 该方法生成的交互更真实多样，能识别多达33%的智能体错误，证明其作为评估工具的有效性。

Conclusion: SAGE是一个有效的评估框架，通过整合业务知识支持错误发现和智能体迭代改进。
...
TL;DR: 提出了一种用于离散数学逻辑等价问题的自动问题生成系统，通过形式化语言和线性时间算法生成难度均匀的问题，以解决学术不端并提供大量练习题。


<details>
  <summary>Details</summary>
Motivation: 在线教学时代抄袭现象日益严重，需要为每个学生生成独特问题来防止作弊，同时提供大量练习题。现有方法生成的问题效率低且难度不一致。

Method: 使用形式化语言定义逻辑等价问题，将其转换为两组生成规则，并开发线性时间算法进行问题生成。

Result: 学生测试显示生成问题的准确性与教科书问题相当；解决步骤分析表明生成问题的难度与教科书问题相似。

Conclusion: 提出的AQG系统能够生成质量与教科书相当的逻辑等价问题，有效解决了现有方法的效率低和难度不一致问题。
...
TL;DR: 比较神经符号系统和LLM系统在农业领域信息抽取的性能，LLM系统表现更好但各有优缺点
...
Method: 在农业领域（猪肉、乳制品、作物）对9个访谈进行信息抽取，比较神经符号系统和LLM系统的性能

Result: LLM系统总体表现更好（F1总分：69.4 vs 52.7；核心信息：63.0 vs 47.2），但各有优缺点：神经符号系统运行更快、控制性更强，LLM系统性能更高、部署维护更易
...
TL;DR: 提出了Curative Prompt Refinement (CPR)框架，通过清理不良提示和生成额外任务描述来减少LLM幻觉，显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常因用户提示结构不良或模糊而产生看似合理但错误的"幻觉"事实，这削弱了信任。

Method: 使用微调的小型语言模型开发了CPR框架，包括清理不良提示和生成额外信息性任务描述，以对齐用户意图与提示。

Result: 应用CPR后，生成质量显著提高，幻觉得到缓解，在无外部知识的情况下，CPR提示的胜率超过90%。

Conclusion: CPR框架能有效减少由不良提示引起的LLM幻觉，提高生成质量，且无需外部知识。
...
TL;DR: 提出了多阶段提示精炼框架MPR，通过系统性地改进格式错误的提示来减少LLM的幻觉问题，在幻觉基准测试中精炼后的提示胜率超过85%


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在幻觉问题上仍面临挑战，特别是格式错误的提示（包含歧义表述、语法错误或不完整信息）对幻觉的影响尚未充分研究

Method: MPR框架通过多个阶段系统改进格式错误的提示，每个阶段使用专门微调的小型语言模型处理特定错误类型，如标点符号、拼写错误和关键词误用，并采用自反思机制进行排名以优先处理最相关的输入

Result: 在幻觉基准测试中，经MPR精炼的提示相比原始提示胜率超过85%，有效减少幻觉并提高LLM输出准确性

Conclusion: MPR提供了一个轻量级且适应性强的解决方案，可增强LLM在不同领域的可靠性，并能与现有的后处理幻觉缓解框架结合使用
...
Method: 比较在多数投票标签上训练与一系列HLV训练方法的效果
...
TL;DR: 本文综述了大语言模型不确定性量化在幻觉检测中的应用，系统分类了现有方法并提供了实证结果，讨论了当前局限性和未来研究方向。
...
Conclusion: 不确定性量化是大语言模型幻觉检测的重要研究方向，当前方法已取得进展但仍存在局限，需要进一步研究来完善这一领域。
...
TL;DR: 提出使用大语言模型进行提示词重写的框架，通过奖励系统和迭代DPO训练优化文本到图像生成的提示词，无需监督微调数据，能提升图像-文本对齐度、视觉质量和美学效果。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在处理简单或描述不足的提示词时表现不佳，导致图像与文本对齐度、美学质量和整体效果不理想。

Method: 基于大语言模型的提示词重写框架，采用精心设计的奖励系统和迭代直接偏好优化训练流程，无需监督微调数据。

Result: 该方法在多种T2I模型和基准测试中一致提升了图像-文本对齐度、视觉质量和美学效果，优于强基线方法，且具有很好的跨模型迁移性。

Conclusion: 提示词重写是改进文本到图像系统的有效、可扩展且实用的模型无关策略，训练好的重写器无需重新训练即可迁移到其他T2I骨干模型。
...
TL;DR: 提出了分层对齐方法，针对Transformer架构中不同功能层进行定向优化，避免传统DPO的统一优化导致的"对齐税"问题


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐技术（如DPO）将模型视为单一实体，忽略了Transformer架构中不同层处理不同任务（从语法到抽象推理）的功能专门化

Method: 分层对齐方法，将模型层划分为局部（语法）、中间（逻辑）和全局（事实性）三个功能块，使用LoRA进行定向微调

Result: 局部层对齐提升语法流畅性，全局层对齐不仅改善事实一致性，还是增强逻辑连贯性的最有效策略，所有分层策略都避免了标准DPO中的"对齐税"
...
TL;DR: APCE是一种上下文感知的解决方案，通过低维语义相似度匹配选择最重要的输入块，在长文本摘要任务中仅使用50%-70%的输入序列就能达到或优于完整密集基线的性能，同时显著减少KV缓存和自注意力内存开销。


<details>
  <summary>Details</summary>
Motivation: 解决长上下文Transformer模型的两个关键挑战：(1) 随着序列长度增加，二次自注意力和线性KV缓存导致内存占用增长；(2) ContextRot现象，即Transformer架构性能随上下文长度增加而下降。

Method: 提出APCE方法，通过低维语义相似度匹配当前查询来选择最重要的输入块，直接操作输入数据，不依赖底层硬件或CUDA环境。

Result: 实证评估显示，APCE仅使用50%-70%的输入序列就能达到或优于完整密集基线的摘要性能，同时显著改善了KV缓存和自注意力的内存效率。
...
TL;DR: 该研究评估了Verily行为健康安全过滤器(VBHSF)在精神健康危机检测中的表现，发现在两个数据集上都表现出高敏感性和特异性，优于NVIDIA NeMo和OpenAI Omni Moderation Latest等现有防护系统。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理精神健康紧急情况时经常提供有害或不适当的建议，需要开发专门的安全过滤器来防止这类风险。

Method: 使用两个临床标注的数据集：Verily精神健康危机数据集(1,800条模拟消息)和NVIDIA Aegis AI内容安全数据集的子集(794条精神健康相关消息)，并与NVIDIA NeMo和OpenAI Omni Moderation Latest进行对比分析。

Result: VBHSF在Verily数据集上表现出高敏感度(0.990)和特异性(0.992)，F1分数为0.939；在NVIDIA数据集上保持高敏感度(0.982)和准确率(0.921)。在所有情况下都显著优于对比系统(p < 0.001)。

Conclusion: VBHSF表现出稳健、可泛化的性能，优先考虑敏感度以最小化遗漏危机，这是医疗应用的关键特性。
...
TL;DR: 提出了PACE方法，通过让LLMs生成并行关联链来评估其创造力，该方法能有效避免数据污染问题，且与人类评估高度相关。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs创造力评估面临数据污染和昂贵人工评估的挑战，受人类创造力评估启发，需要开发更高效可靠的评估方法。

Method: PACE方法要求LLMs生成并行关联链，通过分析这些关联链来评估模型的创造力，该方法简单高效且能最小化数据污染风险。

Result: PACE与Chatbot Arena Creative Writing排名高度相关（Spearman's ρ=0.739），高性能LLMs达到普通人水平但不及专业人士，人类在关联模式多样性上更优。

Conclusion: PACE是评估LLMs创造力的有效方法，LLMs在创造力方面已接近普通人水平，但在专业性和多样性方面仍有提升空间。
...
TL;DR: 本文提出了AdaXEval评估方法，通过构建与训练数据相同双语语料的选择题数据集，研究多语言领域适应中LLMs的知识获取机制。


<details>
  <summary>Details</summary>
Motivation: 多语言领域适应(ML-DA)中，跨语言知识获取机制尚未充分探索，导致在低资源设置下性能不佳。现有研究训练和评估数据集的知识覆盖不匹配，难以直接研究多语言知识获取。

Method: 提出AdaXEval自适应评估方法，从训练用的双语领域语料构建多选题QA数据集；通过持续训练LLMs并跟踪领域事实获取过程，研究从训练数据到知识的转换机制。

Result: 在130亿参数的英日双语LLM上的实验表明，即使使用高质量双语语料，跨语言迁移仍然具有挑战性。

Conclusion: 跨语言知识转移在多语言领域适应中仍然困难，需要更有效的方法来促进跨语言知识获取。
...
<details>
  <summary>Details</summary>
Motivation: 端到端大语音语言模型在对话生成方面表现出色，但在语义理解基准测试中始终落后于传统流水线系统，需要深入理解这种模态差距的成因。
...
TL;DR: 提出了SafeMT基准测试，评估多模态大语言模型在多轮对话中的安全性，发现对话轮数增加会提升攻击成功率，并提出了对话安全调节器来检测恶意意图。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的广泛使用，多轮对话中的安全问题日益突出，现有基准测试未能充分评估这种情况下的风险。

Method: 构建包含10,000个样本的SafeMT基准测试，涵盖17种场景和4种越狱方法，提出安全指数(SI)评估模型安全性，并设计对话安全调节器检测对话中的恶意意图。

Result: 评估17个模型发现，有害对话轮数增加会显著提高攻击成功率，表明现有模型的安全机制不足以识别对话交互中的危险。提出的安全调节器比现有防护模型更有效降低多轮攻击成功率。

Conclusion: 多轮对话对MLLMs构成更大安全风险，需要专门的安全机制来检测对话中的恶意意图，提出的对话安全调节器是有效的解决方案。
...
TL;DR: 论文提出Credal Transformer，通过基于证据理论的Credal注意力机制替代标准注意力，产生置信集而非单一注意力向量，直接量化模型不确定性，从而减少LLM的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLM产生事实错误但自信断言的问题，认为这源于Transformer的Softmax函数造成的"人工确定性"，在每层丢弃了不确定性信息。

Method: 引入Credal Transformer，用基于证据理论的Credal注意力机制(CAM)替代标准注意力，将注意力分数重新概念化为Dirichlet分布的证据质量，产生置信集来直接测量模型不确定性。

Result: Credal Transformer能够识别分布外输入、量化模糊性，并在无法回答的问题上通过弃权显著减少自信错误。

Conclusion: 提出了一种新的架构来缓解幻觉问题，以及将不确定性量化直接集成到模型中的设计范式，为更可靠的AI奠定了基础。
...
TL;DR: 本文对并行推理这一新兴LLM推理范式进行了系统性综述，定义了其概念，分类讨论了非交互式、交互式和效率导向的解码策略，并分析了应用场景、挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，并行推理作为增强推理鲁棒性的新范式出现，旨在克服传统顺序推理方法的脆弱性并提升实际性能。

Method: 首先给出并行推理的正式定义，澄清其与相关概念的区别，然后基于新分类法组织讨论先进技术，包括非交互推理、交互推理和效率导向解码策略。

Result: 系统梳理了并行推理的技术体系和应用场景，如解决复杂问题和提升LLM输出可靠性，并识别了核心挑战。

Conclusion: 为初学者提供了有用的路线图，鼓励更多研究改进并行推理方法，推动该领域发展。
...
TL;DR: 本文研究了将离散空间中的推理技术（如多样本生成和奖励模型重排序）应用到连续空间推理的可行性，发现虽然能生成多样化的推理路径，但由于连续思维表示缺乏关键归纳偏置，导致奖励模型难以有效区分正确和错误推理。


<details>
  <summary>Details</summary>
Motivation: 研究已证明在离散空间中，通过多样本生成和奖励模型重排序能有效提升文本推理性能。本文旨在探索这些技术是否也能成功应用于连续空间推理，特别是使用COCONUT连续空间推理模型作为骨干。

Method: 使用基于dropout的采样方法生成多样化推理路径，在COCONUT连续空间推理模型上进行Pass@N分析，并通过几何属性和轨迹动态等维度探究连续思维空间中的挑战。

Result: 研究表明在连续空间中生成多样化推理路径是可行的，且性能提升潜力类似于离散空间。但离散空间中的数据生成和奖励模型训练方法在连续空间中只能带来边际改进，主要原因是连续思维表示缺乏有效区分正确和错误推理所需的归纳偏置。

Conclusion: 连续推理语言模型的训练框架不仅需要优化准确性，还应明确纳入可在推理时用于区分正确和错误思维的归纳偏置，这是实现连续空间中推理性能显著提升的关键。
...
TL;DR: LLaDR是一个基于大语言模型的药物重定位框架，通过从LLMs提取语义丰富的生物医学实体文本表示来增强知识图谱嵌入，从而改进生物医学概念表示，在药物重定位任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了真实世界实验室中的常识性生物医学概念知识，如某些药物与特定治疗根本不相容的机制先验知识，导致生物医学知识图谱表示不足。

Method: 从大语言模型提取语义丰富的治疗相关生物医学实体文本表示，并用这些表示来微调知识图谱嵌入模型，将治疗相关知识注入到KGE中。

Result: 实验表明LLaDR在不同场景下均达到最先进性能，阿尔茨海默病案例研究进一步证实了其鲁棒性和有效性。

Conclusion: LLaDR通过注入LLM提取的治疗相关知识，显著改善了生物医学概念的表示，增强了对于研究不足或复杂适应症的语义理解。
...
TL;DR: 研究发现大型音频语言模型在时间戳预测中存在系统性偏差，模型预测的事件时间往往比真实时间提前或延后，且偏差随音频长度增加而累积。


<details>
  <summary>Details</summary>
Motivation: 探索大型音频语言模型在定位事件发生时间方面的能力，揭示其在时间戳预测中存在的系统性偏差问题。

Method: 在带时间戳的数据集上进行控制实验，量化时间偏差效应，提出时间偏差指数(TBI)来衡量预测事件时间的系统性错位，并开发可视化框架。
...
TL;DR: 提出基于大语言模型和直接偏好优化的语音翻译分割框架，在分割准确率、翻译质量和延迟方面均优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有分割模型虽然比启发式方法更鲁棒，但仍受限于监督学习目标，缺乏对人类偏好的对齐，这在实时自然口译中至关重要

Method: 使用直接偏好优化训练的大语言模型来预测自然分割点，在ACL 60/60语料库上评估，使用SeamlessM4T v2作为翻译骨干

Result: 在三个语言对上，DPO调优的LLM比SHAS获得更高的分割准确率，在翻译质量和延迟方面都有持续改进
...
TL;DR: 提出了HALF框架，这是一个考虑伤害严重程度的LLM公平性评估框架，将9个应用领域分为3个等级，评估结果显示LLM在不同领域的公平性不一致。
...
Method: 开发了HALF框架，通过五阶段流程将9个应用领域组织为严重、中等、轻度三个等级，并基于伤害严重程度对结果进行加权评估。

Result: 评估8个LLM发现：(1) LLM在不同领域的公平性不一致；(2) 模型大小或性能不能保证公平性；(3) 推理模型在医疗决策支持中表现更好但在教育中表现更差。
...
Result: 发现Knobe效应偏见不仅在学习过程中形成，而且定位在特定层中，只需修补少数关键层就能完全消除这种偏见。
...
TL;DR: 提出DSAS方法解决LLM在多文档问答中的长距离依赖建模和"中间信息丢失"问题，通过两个无需训练参数的模块提升性能。


<details>
  <summary>Details</summary>
Motivation: LLM在多文档问答任务中存在两个主要限制：长距离依赖建模困难，无法关注长文本中的关键信息；以及"中间信息丢失"问题，难以处理长输入中间部分的信息。现有解决方案要么截断全局依赖，要么需要昂贵的微调，缺乏通用简单的解决方案。

Method: 提出双阶段自适应锐化(DSAS)方法，包含两个模块：(1)上下文门控加权(CGW)模块通过层级注意力跟踪和位置感知加权来缓解"中间信息丢失"问题；(2)互斥注意力抑制(RAS)模块通过抑制关键文本与无关文本之间的信息交换来增强对关键段落的关注。DSAS是一个即插即用的解决方案，无需架构修改或额外训练参数。
...
Conclusion: DSAS提供了一个简单有效的解决方案，成功解决了LLM在多文档问答中的关键限制，无需额外训练成本，具有良好的鲁棒性和可扩展性。
...
TL;DR: MedQA-Followup框架评估医学问答中多轮对话的鲁棒性，发现LLMs在复杂医疗咨询场景中存在严重脆弱性，间接干预比直接建议更具破坏性。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架主要针对单轮问答的理想条件，忽略了医疗咨询中常见的冲突输入、误导性上下文和权威影响等复杂性，需要系统评估多轮对话的鲁棒性。

Method: 提出MedQA-Followup框架，区分浅层鲁棒性（抵抗误导初始上下文）和深层鲁棒性（在答案被挑战时保持准确性），引入间接-直接轴分离上下文框架和明确建议。

Result: 在MedQA数据集上评估5个最先进LLM，发现模型在浅层扰动下表现尚可，但在多轮设置中准确率从91.2%降至最低13.5%（Claude Sonnet 4）。间接干预比直接建议造成更大准确率下降。

Conclusion: 多轮鲁棒性是医疗LLM安全可靠部署的关键但未充分探索的维度，间接上下文干预暴露了临床部署的重大脆弱性。
...
TL;DR: 提出了Chinese ModernBERT，一个从头开始训练的中文编码器，通过优化的词汇表、动态掩码策略、两阶段预训练和稳定学习率调度，在中文NLP任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer改进主要针对英语，未能完全迁移到中文，因为中文在分词和形态学上与英语有显著差异。
...
Conclusion: Chinese ModernBERT为中文NLP提供了有效的解决方案，展示了在语义相似度任务上的扩展潜力。
...
TL;DR: 提出了一种基于大语言模型的无监督流水线方法，用于大规模语料库的语法标注，在COHA语料库上实现了98%+的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决语料库快速扩张时人工标注成为方法瓶颈的问题，探索自动化语法标注的可行性。

Method: 四阶段工作流：提示工程、预评估、自动批量处理、后验证，使用GPT-5通过OpenAI API进行标注。

Result: 在60小时内标注了143,933个句子，两个复杂标注任务的准确率均超过98%。

Conclusion: 大语言模型能够以最小人工干预大规模执行数据准备任务，为基于语料库的研究开辟了新可能性，但需注意成本、许可和伦理问题。
...
TL;DR: 提出了一种基于知识检索增强生成(RAG)的反言论生成框架，通过整合联合国数字图书馆等权威知识库，为8个主要目标群体生成可信的反言论内容。


<details>
  <summary>Details</summary>
Motivation: 现有反言论生成方法存在可靠性不足和可扩展性差的问题，需要开发更可信和连贯的生成框架。

Method: 将反言论生成建模为知识驱动的文本生成过程，集成RAG管道，构建包含32,792个文本的知识库，涵盖联合国数字图书馆等权威来源。

Result: 在MultiTarget-CONAN数据集上的评估显示，该框架在标准指标和人工评估中均优于标准LLM基线方法。

Conclusion: 该框架和知识库为研究可信和合理的反言论生成开辟了新途径，适用于仇恨言论及其他领域。
...
TL;DR: 该论文提出了一种细粒度输入归因方法，用于识别对大脑-LLM对齐最重要的特定词语，并研究了大脑对齐(BA)与下一个词预测(NWP)之间的关系。


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型(LLMs)与人类大脑活动之间的对齐可以揭示语言处理背后的计算原理，特别是解决关于大脑对齐与下一个词预测关系这一有争议的研究问题。

Method: 引入细粒度输入归因方法，识别对大脑-LLM对齐最重要的特定词语，并利用该方法分析BA和NWP之间的关系。

Result: 发现BA和NWP依赖于很大程度上不同的词语子集：NWP表现出近因和首因偏差，关注语法；而BA优先考虑语义和语篇层面信息，具有更有针对性的近因效应。

Conclusion: 这项工作推进了我们对LLMs如何与人类语言处理相关的理解，并突出了BA和NWP在特征依赖方面的差异。归因方法可广泛应用于探索模型预测在各种语言处理任务中的认知相关性。
...
<details>
  <summary>Details</summary>
Motivation: 现有MoE模型的卸载策略受限于CPU-GPU互联带宽，预取方法训练开销大且在细粒度专家分割的现代MoE模型中效果不佳。

Method: 提出混合大小专家机制，对不重要token使用半数专家加速，对重要token使用完整专家；设计专用的回退和预取机制在大小专家间切换以提高内存效率。

Result: 在四种典型现代MoE架构和生成任务上，MoBiLE在消费级GPU系统上相比基线实现1.60x到1.72x的加速，精度损失可忽略。

Conclusion: MoBiLE框架有效解决了MoE推理中的带宽瓶颈问题，实现了显著的加速效果且保持模型质量。
...
TL;DR: 研究发现LLM作为审稿人存在系统性偏见：偏向LLM生成的论文风格，对包含批判性陈述的人类论文评分偏低，这给学术公平带来风险。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在学术工作流程中的深度整合，需要探索其在研究和审稿双重角色中可能带来的风险，特别是对学术公平的影响。

Method: 通过模拟实验，设置研究代理生成和修改论文，审稿代理评估提交的论文，并进行人工标注分析。

Result: LLM审稿人系统性地给LLM撰写的论文打高分，而对包含批判性陈述的人类论文持续低分，存在语言特征偏见和批判陈述厌恶两种主要偏见。

Conclusion: LLM作为审稿人存在公平性风险，但LLM指导的修改能提升论文质量，对早期研究者和低质量论文有潜在价值。
...
TL;DR: 本研究对200多种语言进行大规模跨语言分词效率评估，发现当前AI系统中存在结构性不平等：拉丁文字语言分词效率高，而非拉丁文字和形态复杂语言的分词成本高出3-5倍，导致计算成本增加和有效上下文利用减少。


<details>
  <summary>Details</summary>
Motivation: 解决分词差异对语言多样化群体获得公平AI访问权的重要障碍，系统量化大型语言模型中的计算不平等问题。

Method: 使用标准化实验框架，通过tiktoken库对200多种语言样本进行统一分词处理，收集分词统计数据，使用TPS和RTC等评估指标，并以英语为基准进行对比。

Result: 发现显著且系统性的分词差异：拉丁文字语言分词效率高，非拉丁文字和形态复杂语言的分词膨胀显著，RTC比率通常高出3-5倍，导致计算成本增加和有效上下文利用减少。

Conclusion: 当前AI系统存在结构性不平等，低资源和非拉丁语言使用者面临不成比例的计算劣势。未来研究应优先开发语言感知的分词策略和适应性的词汇构建方法，确保更具包容性和计算公平的多语言AI系统。
...
TL;DR: PRoH是一个动态规划和推理框架，通过上下文感知规划、结构化问题分解和实体加权重叠引导的检索算法，解决了现有知识超图RAG方法在静态检索规划、非自适应执行和浅层结构利用方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识超图的RAG方法存在三个主要限制：静态检索规划、非自适应检索执行以及对超图结构和语义的浅层利用，这些限制了它们进行有效多跳问答的能力。

Method: PRoH包含三个核心创新：(1)上下文感知规划模块，绘制局部超图邻域以指导结构化的推理计划生成；(2)结构化问题分解过程，将子问题组织为动态演化的有向无环图，实现自适应多轨迹探索；(3)实体加权重叠引导的推理路径检索算法，优先考虑语义连贯的超边遍历。

Result: 在多个领域的实验中，PRoH实现了最先进的性能，平均在F1分数上超过先前SOTA模型HyperGraphRAG 19.73%，在生成评估分数上超过8.41%，同时在长距离多跳推理任务中保持强大的鲁棒性。

Conclusion: PRoH框架通过动态规划和推理机制，有效克服了现有知识超图RAG方法的局限性，在多跳问答任务中展现出卓越的性能和鲁棒性。
...
TL;DR: CLEAR框架通过细粒度句子级知识分解、隐藏状态探测定位知识冲突以及冲突感知微调，显著提升了RAG系统的准确性和上下文忠实度。
...
Conclusion: CLEAR通过深入理解LLM内部知识整合机制，有效解决了RAG系统中的不忠实问题，为提升LLM的事实性提供了新思路。
...
TL;DR: 该研究通过多语言Wug测试评估LLM在形态学泛化任务中的表现，发现模型能够以类似人类的准确率泛化形态学过程，但准确率模式更受数据可用性和语言社区规模影响，而非结构复杂性。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM的语言能力，特别是它们是否能像人类一样处理新词的形态学泛化任务，以及模型表现是受语言结构复杂性还是训练数据量的影响。

Method: 使用多语言Wug测试，在四种语言（加泰罗尼亚语、英语、希腊语、西班牙语）上测试六个模型，并与人类说话者进行比较。

Result: 模型能够以人类水平的准确率泛化形态学过程，但准确率模式与语言社区规模和数据可用性更相关，而非结构复杂性。西班牙语和英语表现优于资源较少的加泰罗尼亚语和希腊语。

Conclusion: 模型行为主要受语言资源丰富度驱动，而非对语法复杂性的敏感性，这种表现只是表面上类似于人类语言能力。
...
Method: SMEC框架包含：1）序列嵌套表示学习减少训练梯度方差；2）自适应维度选择模块减少维度剪枝时的信息退化；3）可选择性跨批次记忆模块增强高低维嵌入间的无监督学习。

Result: 在图像、文本和多模态数据集上的实验表明，SMEC在显著降维的同时保持性能。在BEIR数据集上，压缩后的256维LLM2Vec嵌入性能比Matryoshka-Adaptor和Search-Adaptor分别提升1.1和2.7个点。

Conclusion: SMEC框架有效解决了高维嵌入带来的计算和存储问题，为LLM嵌入的实际部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型生成能力的提升，身份模仿风险增加，但目前缺乏对个性化机器生成文本检测的研究。

Method: 构建个性化文本检测基准数据集，提出基于特征反转陷阱的M方法，通过识别潜在特征方向来预测检测器性能变化。

Result: 实验显示现有检测器在个性化设置中性能显著下降，提出的M方法能准确预测性能变化方向与幅度，与实际性能差距相关性达85%。

Conclusion: 该工作揭示了现有检测器在个性化文本检测中的局限性，提出的方法能有效预测性能变化，鼓励进一步研究个性化文本检测。
...
TL;DR: 将测试时缩放技术从数学和编程领域转移到LeWiDi-2025任务中，评估三种方法在标注分歧任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放技术此前仅限于有可验证正确答案的领域，本研究旨在将其扩展到评估标注分歧的LeWiDi任务中。

Method: 使用三种测试时缩放方法：模型平均、多数投票和Best-of-N采样，在LeWiDi任务上进行实验。

Result: 两个基准方法在LeWiDi任务上持续提升LLM性能，但Best-of-N方法未能实现有效转移。
...
TL;DR: VLMs在评估非典型实例时，会在语用先验（输入相关）和语义先验（概念通用）之间产生冲突，导致概念理解在输入不一致时退化。


<details>
  <summary>Details</summary>
Motivation: 研究VLMs在处理典型和非典型图像时，如何权衡语用先验和语义先验之间的冲突。

Method: 引入VISaGE评估数据集，包含典型和异常图像，通过精心平衡的实验设计进行分析。

Result: 当语用先验的一致性假设被不一致图像违反时，概念理解会退化，这种效应强于语义先验对个体实例查询的影响。

Conclusion: VLMs在处理非典型实例时，语用先验的影响比语义先验更显著，揭示了模型在概念表示和实例分析之间的权衡机制。
...
TL;DR: 本文提出Faithful Uncertainty Tuning (FUT)方法，通过微调让LLMs能够忠实表达不确定性，而不改变其答案分布。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在表达不确定性方面存在不忠实问题：重复查询会产生不同答案，但生成的响应通常没有使用或使用了不能反映这种变异性的不确定性修饰语。

Method: 构建训练数据，通过将模型样本与不确定性修饰语（如'可能'或'很可能'）对齐来增强样本，这些修饰语与样本一致性相匹配，仅需模型和一组提示即可完成。

Result: 在多个模型和数据集上的开放域问答评估显示，FUT显著减少了忠实性差距，同时保持了问答准确性并引入了最小的语义分布偏移。

Conclusion: FUT是一种简单有效的方法，可以教会LLMs忠实传达不确定性。
...
TL;DR: 提出StyleDecipher框架，通过结合离散风格指标和连续风格表示来检测机器生成文本，在跨领域检测中表现优异，比现有方法提升36.30%，且具有可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法存在泛化能力差、易受改写攻击、缺乏可解释性等问题，特别是在面对风格多样性和人机混合创作时表现不佳。

Method: 使用组合特征提取器量化风格差异，联合建模离散风格指标和基于语义嵌入的连续风格表示，在统一表示空间中捕捉人类与LLM输出的风格差异。

Result: 在五个不同领域（新闻、代码、论文、评论、学术摘要）的实验表明，StyleDecipher在领域内准确率达到最优，跨领域检测性能比基线方法提升36.30%，且对对抗性扰动和混合内容具有鲁棒性。
...
TL;DR: ACADATA是一个高质量的学术翻译平行数据集，包含150万训练数据和6000条评估数据，通过微调LLM在学术翻译质量上显著提升，超越专有和开源模型。
...
Method: 构建ACADATA数据集（包含ACAD-TRAIN训练集和ACAD-BENCH评估集），在96个语言方向上微调大语言模型。

Result: 微调后7B和2B模型在学术翻译质量上分别平均提升6.1和12.4 d-BLEU点，在英语翻译长文本时提升24.9%，最佳模型超越所有专有和开源模型。

Conclusion: ACADATA数据集和微调模型为学术翻译研究提供了宝贵资源，显著提升了学术翻译质量。
...
TL;DR: COSTAR-A是一个改进的提示工程框架，在原有COSTAR（Context, Objective, Style, Tone, Audience, Response）基础上增加了'Answer'组件，旨在提升较小本地优化模型在需要更指令性或约束性输出任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型对提示设计高度敏感，而现有COSTAR框架在较小本地优化模型上的表现不一致，特别是在需要更指令性或约束性输出的任务中。

Method: 通过一系列受控的提示-输出评估，使用参数不超过80亿的微调模型，比较COSTAR和COSTAR-A框架的性能差异。

Result: COSTAR-A能够提升本地化LLM的输出结构和决策性，但其效果因模型和用例而异；Llama 3.1-8B模型在使用COSTAR-A时表现出性能提升。

Conclusion: COSTAR-A作为一个提示框架具有适应性和可扩展性，特别适合在资源受限硬件上进行计算效率高的AI部署。
...
TL;DR: PARO框架通过让LLM生成符合任务特定推理模式的rationales，无需人工标注，在patterned reasoning任务上达到与10倍规模人工标注相当的SFT+RLVR性能


<details>
  <summary>Details</summary>
Motivation: 传统SFT+RLVR范式需要昂贵的人工rationale标注，本文旨在研究如何在patterned reasoning任务中大幅降低标注成本而不影响推理性能

Method: 提出PARO框架，让LLM生成与任务特定推理模式对齐的rationales，无需人工rationale标注，只需有限的人类对推理模式的监督

Result: PARO生成的rationales在SFT+RLVR性能上与10倍规模的人工rationales相当，表明大规模人工标注可被LLM自动标注替代

Conclusion: 对于patterned reasoning任务，推理模式而非rationale数量或质量是性能关键，PARO能有效降低标注成本
...
TL;DR: 论文提出生成空间大小(GSS)概念来解决LLMs在创意任务中输出过于同质化和在事实任务中产生多样化但错误输出的问题，并开发了GSSBench评估套件。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在开放生成任务中存在校准问题：创意任务输出过于同质化，事实任务产生多样化但错误的回答，需要统一框架来解决这两种失败模式。

Method: 提出有效生成空间大小(GSS)概念，构建GSSBench任务套件评估不同指标，使用模型内部状态进行幻觉检测，特别是EigenScore方法。

Result: 发现幻觉检测指标（特别是EigenScore）在仅使用模型内部状态的情况下，持续优于标准多样性和不确定性量化指标，并能提供模型内部任务表示的可解释见解。

Conclusion: GSS概念有三个应用：检测提示模糊性并预测澄清问题、解释推理模型中的过度思考与思考不足、引导模型扩展生成空间以获得高质量多样化输出。
...
TL;DR: 提出了Omni-Detective数据生成管道来生成高质量的多模态详细描述数据，并基于此训练了Audio-Captioner和Omni-Captioner模型，在多个基准测试中取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前全语言模型在捕捉和描述细粒度细节方面的能力尚未充分探索，存在细节与幻觉共增长的问题。

Method: 提出了Omni-Detective代理式数据生成管道，集成工具调用来自主生成高质量多模态数据；基于此数据训练了音频专用和音视频专用的详细描述模型。

Result: Audio-Captioner在MMAU和MMAR基准上超越所有开源模型，性能媲美Gemini 2.5 Pro；Omni-Captioner在VDC基准上创下新纪录，在video-SALMONN 2上实现了细节与幻觉的最佳平衡。

Conclusion: Omni-Detective能有效生成高质量详细描述，Omni-Cloze评估方法能稳定可靠地评估详细描述能力，为全模态细粒度感知提供了系统性解决方案。
...
TL;DR: 该研究扩展了语言模型对类型学常见语法偏好的研究，采用广义范畴语法构建人工语言，评估模型对未见长句的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 扩展语言模型对类型学常见语法偏好的研究，从两个角度改进：采用更贴近自然语言的广义范畴语法，以及更关注模型对未见长句的泛化能力。

Method: 使用广义范畴语法构建人工语言，覆盖无界依赖和轻度上下文敏感结构等自然语言特征，评估语言模型对未见长句的处理能力。

Result: 实验表明类型学上合理的词序更容易让语言模型进行生产性泛化。

Conclusion: 类型学合理的词序有助于语言模型的泛化能力，人工语言设计应更好地捕捉自然语言特征。
...
Method: 提出DGRC方法：分割对话作为提示，使用语言模型生成子部分续写，重新组合对话和续写，比较重组序列的可能性。

Result: 语言模型偏好继续at-issue内容，指令调优模型此效应更强；当存在相关线索时，模型会降低at-issue偏好。

Conclusion: DGRC方法能减轻语言模型分析中的偏见，系统测试话语敏感行为；指令调优虽未增强调节能力，但模式反映了成功对话动态的特征。
...
TL;DR: 本文主张从经验主义角度重新审视语言模型，基于Mańczak的语言学框架，将语言定义为所有口头和书面表达的总和，强调使用频率是语言的主要支配原则。


<details>
  <summary>Details</summary>
Motivation: 针对当前基于de Saussure和Chomsky理论框架对LLMs的批评过于推测性和非建设性，需要更实证的语言学视角来理解和评估语言模型。

Method: 采用Witold Mańczak的经验主义语言学框架，将语言视为使用频率驱动的现象，而非抽象系统或大脑计算系统。

Result: 基于Mańczak框架挑战了先前对LLMs的批评，并为语言模型的设计、评估和解释提供了建设性指导。

Conclusion: 经验主义语言学视角为理解LLMs提供了更富有成效的框架，强调语言使用频率的重要性，有助于更准确地评估语言模型的能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM对所有token使用所有层，导致简单查询浪费计算，复杂查询推理深度不足。现有自适应深度方法依赖昂贵推理搜索、架构修改或大规模重训练，且往往降低准确性。

Method: 使用蒙特卡洛树搜索(MCTS)推导高质量层配置，采用窗口池化稳定路由、焦点损失与类别平衡、瓶颈MLP路由器，确保在类别不平衡和长序列下的鲁棒性。

Result: 在ARC和DART上准确率提升达+3.4%，平均每例节省5层；在跨域任务上仅损失0.85%准确率，同时保持效率，比先前路由方法提升达+7.7%。
...
TL;DR: 为低资源语言创建语音数据集需要大量人工劳动，研究发现转录1小时语音需要30-36小时人工时间


<details>
  <summary>Details</summary>
Motivation: 了解为低资源语言创建高质量语音数据集的实际人工成本，特别是针对低识字率的口语语言

Method: 通过为期一个月的实地研究，让10名母语转录员修正ASR生成的班巴拉语语音转录，分析53小时语音数据

Result: 实验室条件下转录1小时语音平均需要30小时人工，实地条件下需要36小时人工

Conclusion: 研究为具有类似特征的语言创建NLP资源提供了基准和实用见解
...
TL;DR: 提出了Gaussian Splatting for Virtual Environment Rendering and Scene Editing (GS-VERSE)方法，通过将物体网格与高斯泼溅表示直接集成，实现更精确的表面逼近和高度真实的变形交互。


<details>
  <summary>Details</summary>
Motivation: 当前VR中物理操作3D内容的技术存在显著限制，包括依赖工程密集型过程和简化的几何表示，这会损害视觉保真度和物理准确性。

Method: 将物体网格与高斯泼溅表示直接集成，利用现有3D网格资产，系统设计为物理引擎无关，提供部署灵活性。

Result: 在18名参与者的对比用户研究中，该方法在物理感知拉伸操作上统计显著优于现有技术，在扭曲和摇晃等其他物理操作中也更一致。

Conclusion: 该方法在各种交互和场景中始终提供高且可靠的性能，显示出作为现有方法可行替代方案的潜力。
...
TL;DR: 提出Coordinate Condensation方法，通过结合局部坐标更新和基于Schur补的子空间修正来加速物理模拟，相比现有方法消除了阻尼问题，实现接近牛顿法的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如JGS2）使用扰动子空间增强局部求解以考虑全局耦合，但引入了阻尼效应，会降低收敛速度。需要一种既能保持坐标下降效率又能避免阻尼的方法。

Method: 重用子空间但独立求解局部和子空间位移，消除阻尼效应。结合局部坐标更新和Schur补子空间修正。

Result: 在多种材料刚度和网格分辨率下，相比标准坐标下降和JGS2方法，收敛速度显著更快。当子空间能充分捕捉全局耦合时，实现接近牛顿法的收敛性能。

Conclusion: Coordinate Condensation方法在保持坐标下降效率和并行性的同时，实现了接近牛顿法的收敛速度。研究还揭示了基于子空间的坐标方法成功或失败的条件，为未来求解器设计提供见解。
...
TL;DR: 提出LLM4GTA框架，通过保持图与文本表示之间的几何间隙来避免过对齐问题，提升跨模态学习性能


<details>
  <summary>Details</summary>
Motivation: 现有方法过度强调图与文本表示的相似性，导致预训练知识结构破坏和泛化能力下降，因为图编码器和文本编码器分别捕获拓扑模式和语义结构，几何不兼容

Method: LLM4GTA包含自适应间隙保持模块（监控相似性演化防止过对齐）和图空间内模态补偿机制（使用辅助分类器增强判别能力）
...
TL;DR: 提出了多级别草图表示方案和SDGraph深度学习架构，通过稀疏图和稠密图模块分别处理笔画级和点级信息，在草图分类、检索和生成任务中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 手绘草图的稀疏性和抽象性需要不同于图像的学习方法，但现有研究对草图有效信息的定义有限，限制了方法性能。

Method: 提出多级别草图表示方案（草图级、笔画级、点级），并开发SDGraph架构，包含稀疏图（笔画节点）和稠密图（点节点）两个互补模块，通过图卷积和上下采样操作进行编码解码。

Result: 在分类和检索任务中分别比最先进方法提升1.15%和1.70%的准确率，在矢量草图生成质量上提升36.58%。

Conclusion: 多级别草图表示方案能系统识别有效信息，SDGraph架构能有效利用这些信息，在多个草图相关任务中取得优异性能。
...
### [155] [AI Agents for the Dhumbal Card Game: A Comparative Study](https://arxiv.org/abs/2510.11736)
*Sahaj Raj Malla*

Main category: cs.AI

TL;DR: 本研究系统评估了AI代理在Dhumbal文化纸牌游戏中的表现，比较了基于规则、搜索和学习的方法，发现基于规则的激进策略在1024轮模拟中获得了最高胜率(88.3%)。


<details>
  <summary>Details</summary>
Motivation: 评估AI代理在具有文化意义的不完全信息纸牌游戏Dhumbal中的表现，为AI研究提供可复现框架并支持文化游戏的数字保存。

Method: 形式化Dhumbal游戏机制，实现多种AI代理：基于规则的启发式方法（激进、保守、平衡、机会主义）、基于搜索的方法（MCTS、ISMCTS）和强化学习方法（DQN、PPO），通过锦标赛和跨类别冠军赛进行评估。

Result: 在1024轮模拟中，基于规则的激进代理获得最高胜率88.3%（95% CI: [86.3, 90.3]），显著优于ISMCTS(9.0%)和PPO(1.5%)，主要通过有效利用Jhyap声明获胜。

Conclusion: 研究贡献了可复现的AI框架，揭示了在部分信息下启发式方法的有效性，提供了开源代码，推动了AI研究并支持文化游戏的数字保存。
...
### [156] [Beyond Consensus: Mitigating the Agreeableness Bias in LLM Judge Evaluations](https://arxiv.org/abs/2510.11822)
*Suryaansh Jain,Umair Z. Ahmed,Shubham Sahai,Ben Leong*

Main category: cs.AI

TL;DR: 本文提出解决LLM评估器存在的严重正向偏差问题，通过少数否决策略和回归框架显著提高评估准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估器存在强烈的正向偏差，虽然能准确识别有效输出，但难以识别无效输出，导致可靠性评分虚高。

Method: 提出少数否决策略和基于回归的框架，利用少量人工标注数据直接建模验证器偏差。

Result: 在366个高中Python程序代码反馈任务中，回归方法将最大绝对误差降至1.2%，比14个最先进LLM集成性能提升2倍。

Conclusion: 所提出的方法能有效缓解LLM评估器的正向偏差问题，显著提高评估准确性。
...
### [157] [Holistic Agent Leaderboard: The Missing Infrastructure for AI Agent Evaluation](https://arxiv.org/abs/2510.11977)
*Sayash Kapoor,Benedikt Stroebl,Peter Kirgis,Nitya Nadgir,Zachary S Siegel,Boyi Wei,Tianci Xue,Ziru Chen,Felix Chen,Saiteja Utpala,Franck Ndzomga,Dheeraj Oruganty,Sophie Luskin,Kangheng Liu,Botao Yu,Amit Arora,Dongyoon Hahm,Harsh Trivedi,Huan Sun,Juyong Lee,Tengjun Jin,Yifan Mai,Yifei Zhou,Yuxuan Zhu,Rishi Bommasani,Daniel Kang,Dawn Song,Peter Henderson,Yu Su,Percy Liang,Arvind Narayanan*

Main category: cs.AI

TL;DR: 提出了Holistic Agent Leaderboard (HAL)来解决AI代理评估的挑战，包括标准化评估框架、三维分析和LLM辅助日志检查，旨在推动从基准测试表现转向真实世界可靠性的研究。


<details>
  <summary>Details</summary>
Motivation: AI代理评估存在诸多挑战，影响了对代理真实性能的理解，需要建立更可靠的评估体系。

Method: 开发标准化评估框架，在数百个VM上并行评估；进行模型、支架和基准的三维分析；使用LLM辅助日志检查发现未报告行为。

Result: 验证了评估框架，执行了21,730次代理运行，发现意外洞察：更高推理努力在多数运行中反而降低准确性；发现了代理的异常行为模式。

Conclusion: 通过标准化代理评估方法并解决常见陷阱，希望推动研究重点从基准测试表现转向真实世界可靠性。
...
### [158] [CGBench: Benchmarking Language Model Scientific Reasoning for Clinical Genetics Research](https://arxiv.org/abs/2510.11985)
...
Method: 从ClinGen专家标注的临床遗传学文献中构建CGBench基准，测试8种不同语言模型在三个方面的能力：精确提取实验结果、判断证据强度、分类描述实验成果。

Result: 模型在文献解释方面表现出潜力但存在显著差距，推理模型在细粒度任务上表现更好，非推理模型在高层次解释上更优。模型即使正确分类证据，也经常产生幻觉或误解结果。
...
### [159] [Asking Clarifying Questions for Preference Elicitation With Large Language Models](https://arxiv.org/abs/2510.12015)
...
<details>
  <summary>Details</summary>
Motivation: 在用户历史有限的情况下，通过澄清问题获取用户偏好对个性化LLM响应至关重要，但生成跨领域的有效顺序澄清问题仍然具有挑战性。

Method: 采用受扩散模型启发的两阶段过程：前向过程从用户档案生成澄清问题并逐步移除答案作为"噪声"；反向过程训练模型通过提问来"去噪"用户档案。

Result: 该方法显著提高了LLM提出漏斗式问题和有效获取用户偏好的能力。

Conclusion: 提出的两阶段训练方法能够有效提升LLM在顺序澄清问题生成方面的表现，从而更好地获取用户偏好信息。
...
### [160] [CausalTrace: A Neurosymbolic Causal Analysis Agent for Smart Manufacturing](https://arxiv.org/abs/2510.12033)
*Chathurangi Shyalika,Aryaman Sharma,Fadi El Kalach,Utkarshani Jaimini,Cory Henson,Ramy Harik,Amit Sheth*

Main category: cs.AI

TL;DR: CausalTrace是一个集成在工业CoPilot中的神经符号因果分析模块，通过数据驱动的因果分析结合工业本体和知识图谱，提供因果发现、反事实推理和根因分析等功能，实现透明可解释的决策支持。
...
Method: 开发CausalTrace神经符号因果分析模块，集成到SmartPilot工业CoPilot中，利用数据驱动的因果分析结合工业本体和知识图谱，支持因果发现、反事实推理和根因分析，提供实时操作员交互。

Result: 在学术火箭组装测试中，CausalTrace与领域专家达成显著一致（ROUGE-1: 0.91），根因分析性能强劲（MAP@3: 94%, PR@2: 97%, MRR: 0.92, Jaccard: 0.92），C3AN评估得分4.59/5，证明适合实时部署。

Conclusion: CausalTrace通过神经符号因果分析成功解决了工业AI系统的黑盒问题，提供了透明、可解释的决策支持，在制造环境中展现出高精度和可靠性。
...
### [161] [Do Large Language Models Respect Contracts? Evaluating and Enforcing Contract-Adherence in Code Generation](https://arxiv.org/abs/2510.12047)
*Soohan Lim,Joonghyuk Hahn,Hyunwoo Park,Sang-Ki Ko,Yo-Sub Han*

Main category: cs.AI

TL;DR: PACT是一个评估LLM生成代码合同遵从性的框架，通过扩展HumanEval+和MBPP+基准，提供合同违反测试用例，并引入新指标来量化合同遵从性。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准主要评估功能正确性，但忽略了真实软件中合同（前置条件和有效性约束）的重要性，导致无法衡量模型生成代码的鲁棒性和可靠性。

Method: 扩展HumanEval+和MBPP+基准，构建专注于合同违反的测试套件；系统分析不同提示条件下的代码生成；引入合同遵从性量化指标。

Result: 在提示中添加合同违反测试用例相比仅使用合同描述，能显著提升模型对合同的遵从能力；PACT揭示了传统基准忽略的关键错误。

Conclusion: PACT通过提供严谨可解释的指标，能够全面评估LLM生成代码在功能和合同遵从性方面的鲁棒性，填补了现有基准的重要空白。
...
### [162] [Empowering LLM Agents with Geospatial Awareness: Toward Grounded Reasoning for Wildfire Response](https://arxiv.org/abs/2510.12061)
...
Method: 开发GAL层，从原始野火检测数据自动检索并整合基础设施、人口、地形和天气等多源地理数据，生成带单位标注的感知脚本，结合历史类比和每日变化信号进行增量更新。

Result: 在真实野火场景中评估多个LLM模型，显示地理空间接地的智能体性能优于基线方法。
...
TL;DR: ThinkPilot是一个无需训练即可自动优化大型推理模型（LRMs）推理能力的框架，它通过进化过程生成think-prefixes来引导模型实现更优性能。
...
Method: 使用进化过程生成think-prefixes（思考前缀），这些前缀指令基于推理行为分类学驱动演化，引导模型实现更好的性能。

Result: ThinkPilot显著改善了准确率与推理长度的权衡，大幅提升安全性（如将DeepSeek-R1-Distill-Qwen-32B的StrongREJECT分数从27.0%降至0.7），并增强了指令遵循能力，还能与现有基于训练的方法协同工作。

Conclusion: think-prefixes能够可靠地控制LRMs的推理行为，不同任务对特定行为分布有强烈偏好。ThinkPilot通过自动识别和激发这些行为，为将LRMs推理与任务需求对齐提供了可推广的框架。
...
### [164] [AI Agents as Universal Task Solvers](https://arxiv.org/abs/2510.12066)
...
### [165] [HiCoTraj:Zero-Shot Demographic Reasoning via Hierarchical Chain-of-Thought Prompting from Trajectory](https://arxiv.org/abs/2510.12067)
...
Method: 将轨迹转化为语义丰富的自然语言表示，通过分层思维链推理（事实特征提取、行为模式分析、结构化输出的人口统计推断）指导LLMs进行零样本学习。

Result: 在真实世界轨迹数据上的实验表明，HiCoTraj在零样本场景下对多个人口统计属性实现了有竞争力的性能。

Conclusion: HiCoTraj解决了标注人口统计数据稀缺的问题，同时提供了透明的推理链，在零样本人口统计推理方面表现出色。
...
### [166] [EmboMatrix: A Scalable Training-Ground for Embodied Decision-Making](https://arxiv.org/abs/2510.12072)
...
TL;DR: 提出了EmboMatrix训练场概念，通过大规模任务模拟和交互学习，让LLM获得真正的具身决策能力，并培育出EmboBrain模型，在基准测试中超越DeepSeek-R1。


<details>
  <summary>Details</summary>
Motivation: 解决LLM缺乏物理环境暴露的问题，使其能够真正理解具身决策，将高级目标转化为可执行动作。

Method: 构建EmboMatrix训练场，包含多智能体数据引擎、分布式异构硬件系统和多层次奖励架构，通过大规模任务模拟和交互学习培养LLM的具身决策能力。

Result: 培育的EmboBrain-7B模型在两个具身决策基准测试中比671B的DeepSeek-R1基线高出9.5%。

Conclusion: 交互式、环境基础的学习对于构建真正智能的具身智能体具有强大潜力。
...
### [167] [BeSTAD: Behavior-Aware Spatio-Temporal Anomaly Detection for Human Mobility Data](https://arxiv.org/abs/2510.12076)
*Junyi Xie,Jina Kim,Yao-Yi Chiang,Lingyi Zhao,Khurram Shafique*

Main category: cs.AI

TL;DR: BeSTAD是一个无监督框架，通过联合建模空间上下文和时间动态来检测人类移动数据中的个体级异常，能够从大规模人群中捕获个体化行为特征并发现细粒度异常。
...
### [168] [Evaluating the Quality of Randomness and Entropy in Tasks Supported by Large Language Models](https://arxiv.org/abs/2510.12080)
*Rabimba Karanjai,Yang Lu,Ranjith Chodavarapu,Lei Xu,Weidong Shi*

Main category: cs.AI

TL;DR: 本文研究了大型语言模型处理随机性任务的能力，发现虽然LLM能生成具有一定随机性的输出，但表现不一致且与预期行为存在显著偏差。
...
Method: 设计了一系列实验，考虑外部工具访问、任务类型、模型状态和提示策略等因素，涵盖生成随机数、随机字符串、项目洗牌等任务，并使用熵和NIST随机性测试套件评估随机性质量。

Result: LLM生成的输出具有一定随机性，但表现不稳定，与预期行为存在显著偏差。
...
### [169] [One Life to Learn: Inferring Symbolic World Models for Stochastic Environments from Unguided Exploration](https://arxiv.org/abs/2510.12088)
*Zaid Khan,Archiki Prasad,Elias Stengel-Eskin,Jaemin Cho,Mohit Bansal*

Main category: cs.AI

TL;DR: OneLife框架通过概率编程和条件激活的程序化法则学习复杂随机环境中的世界动态，在单次探索的约束下成功学习关键环境动态并优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决在复杂、随机环境中仅通过单次探索学习世界动态的挑战性任务，无需人工指导，专注于真实且具有挑战性的设置。

Method: 使用条件激活的程序化法则在概率编程框架中建模世界动态，每个法则采用前提-效果结构，在相关世界状态中激活，创建动态计算图以避免扩展问题。

Result: 在23个测试场景中的16个场景上优于强基线方法，能够从最小化、无指导的交互中成功学习关键环境动态，并通过模拟推演识别出更优策略。

Conclusion: 为自主构建未知复杂环境的程序化世界模型奠定了基础，证明了在单次探索的约束下学习复杂随机环境动态的可行性。
...
### [170] [ToPolyAgent: AI Agents for Coarse-Grained Topological Polymer Simulations](https://arxiv.org/abs/2510.12091)
...
Result: 通过涉及不同聚合物结构、溶剂条件、恒温器和模拟长度的案例研究展示了ToPolyAgent的多功能性，并证明了其作为研究助手的潜力。
...
### [171] [Precise Attribute Intensity Control in Large Language Models via Targeted Representation Editing](https://arxiv.org/abs/2510.12121)
...
Result: 在LLaMA-3.2-3b和Phi-4-mini上的实验证实了该方法能够以高精度引导文本生成达到用户指定的属性强度，并在偏好数据合成、帕累托前沿近似和优化、对齐行为蒸馏等下游任务中实现了效率提升。

Conclusion: 该方法超越了简单的方向对齐，实现了对属性强度的细粒度连续控制，为LLM的精确可控性提供了有效解决方案。
...
### [172] [MatSciBench: Benchmarking the Reasoning Ability of Large Language Models in Materials Science](https://arxiv.org/abs/2510.12171)
...
TL;DR: MatSciBench是一个包含1340个大学水平材料科学问题的综合基准测试，涵盖6个主要领域和31个子领域，具有三级难度分类和多模态推理功能。评估显示即使最佳模型准确率也不到80%，且不同推理策略在不同场景下表现不一。
...
Method: 创建MatSciBench基准，包含1340个结构化分类的材料科学问题，采用三级难度分类，支持多模态推理，并评估了基本思维链、工具增强和自我校正等不同推理策略。

Result: 评估显示即使是表现最佳的Gemini-2.5-Pro模型在材料科学问题上的准确率也不到80%，不同推理策略在不同场景下表现各异，没有单一方法在所有情况下都表现优异。
...
### [173] [Evolution of meta's llama models and parameter-efficient fine-tuning of large language models: a survey](https://arxiv.org/abs/2510.12178)
...
<details>
  <summary>Details</summary>
Motivation: 为机器学习研究者和实践者提供关于LLaMA模型和高效微调策略的一站式资源，帮助理解这些大型预训练模型的演进和实际应用。
...
Conclusion: 探讨了LLaMA模型和PEFT在现实世界中的成功应用案例（如法律和医疗领域），并讨论了持续挑战和未来研究方向，包括扩展到更大上下文和提升鲁棒性。
...
### [174] [ResearStudio: A Human-Intervenable Framework for Building Controllable Deep-Research Agents](https://arxiv.org/abs/2510.12194)
*Linyi Yang,Yixuan Weng*

Main category: cs.AI

TL;DR: ResearStudio是一个开源框架，将实时人工控制置于核心，允许用户在AI研究代理执行过程中随时干预、修复错误和添加专家知识。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究代理运行在“发射后不管”模式，用户无法在执行过程中修复错误或添加专家知识。

Method: 采用协作工作坊设计，分层规划器-执行器将每个步骤写入实时“计划即文档”，快速通信层将每个动作、文件更改和工具调用流式传输到Web界面。

Result: 在完全自主模式下，ResearStudio在GAIA基准测试中达到最先进结果，超越了OpenAI的DeepResearch和Manus等系统。

Conclusion: 强大的自动化性能和细粒度人工控制可以共存，展示了安全可控研究代理的发展方向。
...
### [175] [On the Design and Evaluation of Human-centered Explainable AI Systems: A Systematic Review and Taxonomy](https://arxiv.org/abs/2510.12201)
*Aline Mangold,Juliane Zietz,Susanne Weinhold,Sebastian Pannasch*

Main category: cs.AI

TL;DR: 本文对65项评估可解释AI(XAI)系统的用户研究进行了全面综述，提出了以人为中心的XAI系统设计目标和评估框架，并根据用户AI专业水平（AI新手和数据专家）区分了不同的设计重点。


<details>
  <summary>Details</summary>
Motivation: 随着AI在日常生活中的普及，对既高性能又可理解的智能系统需求日益增长。当前XAI系统的评估过于技术化，缺乏对用户需求的关注，需要建立以人为中心的评估和设计框架。

Method: 通过对65项XAI用户研究的综合分析，提出了XAI系统特性和以人为中心的评估指标分类，并针对不同AI专业水平的用户扩展了设计目标。

Result: 发现XAI系统由核心系统和解释组件构成；评估指标可分为情感、认知、可用性、可解释性和解释质量等维度；AI新手关注负责任使用、接受度和可用性，数据专家更关注性能导向的人机协作和任务表现。

Conclusion: 提出了扩展的XAI评估和设计框架，强调需要根据用户特征定制XAI系统，为XAI开发者提供了以人为中心的指导原则。
...
### [176] [GOAT: A Training Framework for Goal-Oriented Agent with Tools](https://arxiv.org/abs/2510.12218)
*Hyunji Min,Sangwon Jung,Junyoung Sung,Dosung Lee,Leekyeung Han,Paul Hongsuck Seo*

Main category: cs.AI

TL;DR: GOAT是一个无需人工标注的训练框架，能够自动构建目标导向API执行任务的合成数据集，通过微调使开源LLM代理在复杂工具使用和推理方面达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理在处理需要分解高级目标为多个相互依赖API调用的目标导向查询时能力有限，开源模型尤其难以有效执行复杂工具使用，而现有方法主要依赖零样本评估且缺乏训练数据。

Method: 提出GOAT训练框架，从给定的API文档自动构建目标导向API执行任务的合成数据集，在无需人工标注的情况下对LLM代理进行微调，使其能够推理相互依赖的调用并生成连贯响应。
...
### [177] [MedKGEval: A Knowledge Graph-Based Multi-Turn Evaluation Framework for Open-Ended Patient Interactions with Clinical LLMs](https://arxiv.org/abs/2510.12224)
*Yuechun Yu,Han Ying,Haoan Jin,Wenjian Jiang,Dong Xian,Binghao Wang,Zhou Yang,Mengyue Wu*

Main category: cs.AI

TL;DR: 提出了MedKGEval框架，这是一个基于结构化医学知识的多轮临床LLM评估系统，通过知识图谱驱动的患者模拟和实时评估来改进医学对话评估。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型医学评估方法主要依赖事后回顾完整对话记录，忽略了医学对话的动态性和上下文敏感性，无法捕捉真实临床环境中复杂的医患互动。

Method: 1. 知识图谱驱动的患者模拟机制，从精心构建的知识图谱中检索相关医学事实；2. 实时轮次评估框架，由法官代理在对话进行中评估每个模型响应的临床适当性、事实正确性和安全性；3. 构建包含8个最先进LLM的全面多轮基准测试。

Result: MedKGEval能够识别传统评估流程经常忽略的细微行为缺陷和安全风险，展示了其在临床LLM评估中的有效性。

Conclusion: 该框架虽然最初设计用于中英文医学应用，但可以通过切换输入知识图谱轻松扩展到其他语言，确保双语支持和领域特定适用性。
...
### [178] [PromptFlow: Training Prompts Like Neural Networks](https://arxiv.org/abs/2510.12246)
*Jingyi Wang,Hongyuan Zhu,Ye Niu,Yunhui Deng*

Main category: cs.AI

TL;DR: 提出了PromptFlow框架，通过模块化设计和元学习来自动优化提示工程，减少人工干预，提高LLM在特定任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在特定领域部署时，手动提示工程耗时且依赖专家知识的问题，以及现有自动提示方法缺乏动态策略选择和细粒度更新的局限性。

Method: 设计PromptFlow模块化框架，包含元提示、操作符、优化器和评估器，采用基于梯度的元学习和强化学习来回收经验，自动探索最优提示优化路径。

Result: 在多个数据集上的实验证明了PromptFlow的有效性，能够以最少任务特定数据实现高性能的提示优化。

Conclusion: PromptFlow为自动提示工程提供了灵活高效的解决方案，通过经验回收和细粒度优化显著提升LLM在特定任务上的适应能力。
...
### [179] [$\mathbf{T^3}$: Reducing Belief Deviation in Reinforcement Learning for Active Reasoning](https://arxiv.org/abs/2510.12264)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在主动推理中容易出现信念偏差，导致无法正确建模信念、丢失问题状态跟踪，陷入无信息或重复行动，影响强化学习训练效果

Method: 开发T³方法，跟踪模型信念偏差，检测过度的信念偏差并在训练时截断轨迹，移除无信息的尾部，保留信息前缀的信用分配

Result: 在5个挑战性任务中，T³持续提升训练稳定性、token效率和最终性能，性能提升达30%，同时减少约25%的rollout tokens
...
### [180] [Tensor Logic: The Language of AI](https://arxiv.org/abs/2510.12269)
*Pedro Domingos*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI缺乏兼具自动微分、高效GPU实现、自动推理和知识获取的编程语言，现有方案如PyTorch/TensorFlow和LISP/Prolog各有缺陷，需要统一解决方案。

Method: 设计张量逻辑语言，核心构造是张量方程，将逻辑规则与爱因斯坦求和视为相同操作，并基于此实现各种AI形式。
...
### [181] [RAG-Anything: All-in-One RAG Framework](https://arxiv.org/abs/2510.12323)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的知识库本质上是多模态的，包含丰富的文本、视觉元素、结构化表格和数学表达式组合。然而现有的RAG框架仅限于文本内容，在处理多模态文档时存在根本性差距。

Method: 将多模态内容重新概念化为相互连接的知识实体，引入双图构建来捕获跨模态关系和文本语义，开发结合结构知识导航和语义匹配的跨模态混合检索方法。

Result: 在具有挑战性的多模态基准测试中表现出优越性能，相比最先进方法有显著改进，特别是在传统方法失败的长文档上表现尤为突出。
...
### [182] [O-Forge: An LLM + Computer Algebra Framework for Asymptotic Analysis](https://arxiv.org/abs/2510.12350)
...
Result: 该框架在提出适当域分解方面表现出色，能够有效处理复杂的渐近不等式证明问题。

Conclusion: LLM+CAS框架成功展示了AI在专业数学研究中的应用潜力，为数学家提供了新的研究工具，超越了传统的竞赛数学应用范畴。
...
### [183] [A Survey of Vibe Coding with Large Language Models](https://arxiv.org/abs/2510.12399)
...
TL;DR: 这篇论文对基于大语言模型的Vibe Coding进行了首次系统性综述，通过分析1000多篇研究论文，建立了理论框架和实践分类，揭示了成功Vibe Coding的关键因素。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，从代码生成辅助转向自主编码代理，催生了'Vibe Coding'新开发方法，但其有效性尚未充分探索，实证研究显示存在生产力损失和人机协作挑战。

Method: 通过系统分析1000多篇研究论文，使用约束马尔可夫决策过程形式化Vibe Coding，并综合现有实践提出了五种开发模型分类。

Result: 建立了完整的Vibe Coding生态系统分析框架，包括编码LLM、基于LLM的编码代理、开发环境和反馈机制，提出了五种开发模型分类。

Conclusion: 成功的Vibe Coding不仅依赖于代理能力，更需要系统化的上下文工程、完善的开发环境和人机协作开发模型。
...
### [184] [PricingLogic: Evaluating LLMs Reasoning on Complex Tourism Pricing Tasks](https://arxiv.org/abs/2510.12409)
*Yunuo Liu,Dawei Zhu,Zena Al-Khalili,Dai Cheng,Yanjun Chen,Dietrich Klakow,Wei Zhang,Xiaoyu Shen*

Main category: cs.AI

TL;DR: PricingLogic是首个评估LLMs在旅游定价任务中可靠性的基准，包含300个基于真实定价政策的问题，测试结果显示LLMs在复杂规则解释和算术推理方面存在系统性失败。
...
### [185] [MTOS: A LLM-Driven Multi-topic Opinion Simulation Framework for Exploring Echo Chamber Dynamics](https://arxiv.org/abs/2510.12423)
*Dingyi Zuo,Hongjie Zhang,Jie Ou,Chaosheng Feng,Shuwan Liu*

Main category: cs.AI

TL;DR: 提出了MTOS框架，将多主题上下文与LLMs结合，通过记忆机制、交互策略和信念衰减来模拟跨主题观点演化，解决了现有方法在捕捉多主题认知转移方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实网络中信息常涉及多个相关主题，现有基于LLM的研究多关注单一主题，难以捕捉多主题跨域认知转移；传统数值模型将复杂语言态度简化为离散值，缺乏可解释性和多主题整合能力。

Method: MTOS框架整合LLMs与短长期记忆，采用多种用户选择交互机制和动态主题选择策略，使用信念衰减机制实现跨主题观点更新。
...
### [186] [Biased-Attention Guided Risk Prediction for Safe Decision-Making at Unsignalized Intersections](https://arxiv.org/abs/2510.12428)
...
Conclusion: 该方法在复杂场景中证明了智能决策框架的有效性。
...
### [187] [Evaluating and Mitigating LLM-as-a-judge Bias in Communication Systems](https://arxiv.org/abs/2510.12462)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在通信系统中被用于自主评估内容质量，这些AI"法官"的公正性无法保证，其评估标准中的偏见可能扭曲结果并损害用户信任。

Method: 在点式评分设置下系统研究两种LLM评估模型（GPT-Judge和JudgeLM），涵盖11种偏见类型，包括隐式和显式偏见形式。

Result: 最先进的LLM评估者对偏见输入具有鲁棒性，通常给它们分配比相应干净样本更低的分数；提供详细评分标准能进一步增强鲁棒性；在偏见响应上微调LLM会显著降低其性能；判断分数与任务难度相关。

Conclusion: LLM评估者存在判断偏见风险，需要采取缓解策略来确保在实际通信场景中实现公平可靠的AI评估。
...
### [188] [Using Medical Algorithms for Task-Oriented Dialogue in LLM-Based Medical Interviews](https://arxiv.org/abs/2510.12490)
*Rui Reis,Pedro Rangel Henriques,João Ferreira-Coimbra,Eva Oliveira,Nuno F. Rodrigues*

Main category: cs.AI

TL;DR: 开发了基于有向无环图的任务导向医疗对话框架，包含问题转换、冷启动、自适应分支、终止逻辑和报告生成五个核心模块，在初步评估中显示出低工作负荷、高可用性和用户满意度。


<details>
  <summary>Details</summary>
Motivation: 将医疗算法和指南转化为结构化临床问题，通过系统化对话框架支持高效的医患交互，减少认知负担并集成到临床工作流程中。

Method: 采用DAG结构组织医疗问题，包含问题转换流水线、基于层次聚类的冷启动机制、扩展-剪枝自适应分支、终止逻辑和结构化报告自动生成。

Result: 患者应用：NASA-TLX=15.6（低负荷），SUS=86（高可用性），QUIS=8.1/9（高满意度）；医生应用：NASA-TLX=26（中等负荷），SUS=88.5（优秀可用性），QUIS=8.3/9（高满意度）。

Conclusion: 该框架有效集成到临床工作流程中，降低了认知需求并支持高效报告生成，但存在系统延迟和小样本评估的局限性。
...
### [189] [Artificial Intelligence Virtual Cells: From Measurements to Decisions across Modality, Scale, Dynamics, and Evaluation](https://arxiv.org/abs/2510.12498)
*Chengpeng Hu,Calvin Yu-Chian Chen*

Main category: cs.AI

TL;DR: 本文提出了细胞状态潜在（CSL）视角，通过操作符语法组织学习过程，强调决策对齐的评估框架，以解决当前AI虚拟细胞模型在跨实验室、跨平台迁移性、数据泄漏和跨尺度耦合方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前AI虚拟细胞模型虽然在单数据集验证中表现良好，但在跨实验室和平台迁移、数据泄漏、剂量时间效应处理以及跨尺度耦合方面存在局限，需要更系统化的评估框架。

Method: 提出模型无关的CSL视角，使用操作符语法组织学习：测量、跨尺度耦合的lift/project操作、以及用于剂量和调度的干预操作。

Result: 建立了决策对齐的评估蓝图，涵盖模态、尺度、上下文和干预维度，强调功能空间读出如通路活性、空间邻域和临床相关终点。

Conclusion: 建议采用操作符感知的数据设计、抗泄漏分区以及透明校准和报告，以实现可重复的同类比较，推动AI虚拟细胞模型的可靠发展。
...
### [190] [ProtoSiTex: Learning Semi-Interpretable Prototypes for Multi-label Text Classification](https://arxiv.org/abs/2510.12534)
...
Conclusion: ProtoSiTex是多标签文本分类的稳健半可解释解决方案，能够捕获重叠和冲突语义，并通过自适应原型和多头注意力增强可解释性。
...
### [191] [Inclusive Fitness as a Key Step Towards More Advanced Social Behaviors in Multi-Agent Reinforcement Learning Settings](https://arxiv.org/abs/2510.12555)
*Andries Rosseau,Raphaël Avalos,Ann Nowé*

Main category: cs.AI

TL;DR: 提出了基于包容性适应度的多智能体强化学习框架，通过基因型分配和遗传共享模拟自然选择，在囚徒困境网络游戏中验证了汉密尔顿法则等生物学原理。


<details>
  <summary>Details</summary>
Motivation: 受自然选择竞争合作力量驱动智能进化的启发，旨在建立更接近生物进化过程的多智能体学习框架，超越传统二元团队结构。

Method: 为每个智能体分配基因型，基于包容性适应度设计奖励函数，考虑遗传共享，在囚徒困境网络游戏中研究社会动态。

Result: 实验结果符合汉密尔顿法则等生物学原理，展示了基于遗传相似性的合作谱系，支持非团队化社会动态的出现。

Conclusion: 包容性适应度为更高级战略智能和社交智能的出现提供了基础，可扩展到时空结构、有限资源和种群进化的开放环境。
...
### [192] [HardcoreLogic: Challenging Large Reasoning Models with Long-tail Logic Puzzle Games](https://arxiv.org/abs/2510.12563)
*Jingcong Liang,Shijun Wan,Xuehai Wu,Siyuan Wang,Yitong Li,Qianglong Chen,Duyu Tang,Zhongyu Wei*

Main category: cs.AI

TL;DR: 提出了HardcoreLogic基准测试，包含5000多个逻辑谜题，通过增加复杂性、非常规元素和不可解谜题三个维度来测试大型推理模型的鲁棒性，发现现有模型在非规范变体上表现显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注9x9数独等流行谜题，可能导致模型过度拟合规范格式和记忆解决方案模式，无法真正理解新规则或适应新变体。

Method: 创建HardcoreLogic基准，包含10种游戏的5000多个谜题，通过三个维度系统性地转换规范谜题：增加复杂性(IC)、非常规元素(UE)和不可解谜题(UP)。

Result: 评估显示各种大型推理模型性能显著下降，即使是在现有基准上表现最佳的模型也严重依赖记忆的刻板印象。增加复杂性是主要困难来源，但模型在处理不增加难度的细微规则变化时也遇到困难。
...
### [193] [Memory as Action: Autonomous Context Curation for Long-Horizon Agentic Tasks](https://arxiv.org/abs/2510.12635)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长视野智能体任务中面临内存受限的挑战，现有工作内存方法依赖与核心策略解耦的外部启发式机制，无法实现端到端优化。

Method: 提出Memory-as-Action框架，将内存管理作为统一策略的一部分，通过执行显式编辑操作来主动管理工作内存；为解决轨迹断裂问题，提出了动态上下文策略优化算法。

Result: 端到端联合优化任务推理和内存管理不仅减少了计算消耗，还提高了任务性能，通过自适应上下文策展策略实现了更好的性能表现。

Conclusion: 将内存管理作为可学习的内在能力，通过端到端强化学习联合优化，能够有效解决LLM在长视野任务中的内存限制问题，并提升整体性能。
...
### [194] [ERA: Transforming VLMs into Embodied Agents via Embodied Prior Learning and Online Reinforcement Learning](https://arxiv.org/abs/2510.12693)
...
Method: 两阶段框架：1)具身先验学习，从轨迹增强先验、环境锚定先验和外部知识先验三种数据中提取知识；2)在线强化学习，采用自总结、密集奖励塑造和回合级策略优化来克服训练挑战。
...
### [195] [Multi-Agent Debate for LLM Judges with Adaptive Stability Detection](https://arxiv.org/abs/2510.12697)
*Tianyu Hu,Zhen Tan,Song Wang,Huaizhi Qu,Tianlong Chen*

Main category: cs.AI

TL;DR: 提出了一个多智能体辩论法官框架，通过协作推理和迭代优化来提高LLM自动判断任务的准确性，同时引入稳定性检测机制来保证计算效率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为法官的方法主要依赖简单的聚合方法（如多数投票），即使个体智能体提供正确答案时也可能失败，需要更有效的协作判断机制。

Method: 设计多智能体辩论框架，让智能体协作推理并迭代优化回答；引入基于时间变化Beta-Binomial混合模型的稳定性检测机制，使用Kolmogorov-Smirnov检验进行自适应停止。

Result: 在多个基准测试和模型上的实验表明，该框架相比多数投票方法提高了判断准确性，同时保持了计算效率。

Conclusion: 多智能体辩论框架能够有效提升LLM自动判断任务的性能，通过协作推理和智能停止机制平衡了准确性和效率。
...
### [196] [CAMNet: Leveraging Cooperative Awareness Messages for Vehicle Trajectory Prediction](https://arxiv.org/abs/2510.12703)
*Mattia Grasselli,Angelo Porrello,Carlo Augusto Grazia*

Main category: cs.AI

TL;DR: 该论文研究了利用车辆间通信数据（CAM）进行轨迹预测的方法，提出了基于图神经网络的CAMNet模型，并在真实数据集上验证了CAM数据在轨迹预测中的有效性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶面临安全性挑战，传统传感器存在视野受限和遮挡问题。车辆间通信（特别是CAM消息）可以共享信息，提高对遮挡车辆的感知能力，从而改善轨迹预测。

Method: 设计并训练了基于图神经网络的CAMNet模型，使用运动预测数据集进行训练，并在专门创建的CAM数据集上进行评估。

Result: 方法显示出有希望的结果，证明CAM数据确实可以支持车辆轨迹预测，但同时也存在一些局限性。

Conclusion: CAM数据在车辆轨迹预测中具有应用潜力，但仍需进一步研究解决现有方法的局限性。
...
### [197] [Towards Robust Artificial Intelligence: Self-Supervised Learning Approach for Out-of-Distribution Detection](https://arxiv.org/abs/2510.12713)
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶、交通、医疗等安全关键系统中，AI系统需要在各种条件下保持可靠性能，包括面对分布外样本、对抗攻击和环境变化时。

Method: 结合自监督学习和图论技术，从无标签数据中学习有用表示，更有效地识别和分类OOD样本。

Result: 与现有最先进方法相比，该方法在AUROC指标上达到0.99的优异表现。

Conclusion: 该方法成功提高了AI系统对分布外样本的检测能力，增强了系统鲁棒性，且无需依赖标记数据。
...
### [198] [Clutch Control: An Attention-based Combinatorial Bandit for Efficient Mutation in JavaScript Engine Fuzzing](https://arxiv.org/abs/2510.12732)
...
<details>
  <summary>Details</summary>
Motivation: 现有JavaScript模糊测试方法使用随机选择来确定变异位置，效率不高。作者认为变异目标选择问题适合使用可变臂数的组合老虎机来解决。

Method: 提出CLUTCH深度组合老虎机，使用注意力机制处理可变长度的JavaScript测试用例表示，并通过Concrete Dropout动态调整探索策略。

Result: 相比三种最先进解决方案，CLUTCH将有效测试用例数量平均提高20.3%，每个测试用例的覆盖率平均提高8.9%。在可变和组合设置中，后悔值分别减少至少78.1%和4.1%。

Conclusion: CLUTCH在JavaScript模糊测试中表现出更高的效率，证明了组合老虎机方法在变异目标选择问题上的有效性。
...
### [199] [CTRL-Rec: Controlling Recommender Systems With Natural Language](https://arxiv.org/abs/2510.12742)
*Micah Carroll,Adeline Foote,Kevin Feng,Marcus Williams,Anca Dragan,W. Bradley Knox,Smitha Milli*

Main category: cs.AI

TL;DR: CTRL-Rec是一种允许用户通过自然语言实时控制推荐系统的方法，使用LLM模拟用户对推荐项目的批准判断，并训练嵌入模型来近似这些判断，从而实现对推荐系统的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 当用户对推荐系统的推荐不满意时，他们通常缺乏细粒度的控制手段来改变推荐结果。大语言模型（LLMs）通过允许用户使用自然语言请求来引导推荐，为解决这一问题提供了方案。

Method: 在训练阶段，使用LLM模拟用户基于语言请求对项目的批准判断，并训练嵌入模型来近似这些模拟判断。然后将这些基于用户请求的预测集成到传统推荐系统优化的标准信号加权中。在部署阶段，每个用户请求只需要一次LLM嵌入计算，实现实时推荐控制。

Result: 在MovieLens数据集上的实验表明，该方法能够对各种请求实现一致的细粒度控制。在19名Letterboxd用户的研究中，CTRL-Rec受到用户积极评价，与传统控制相比显著增强了用户的控制感和推荐满意度。

Conclusion: CTRL-Rec方法成功实现了通过自然语言对传统推荐系统的实时控制，在计算效率高的前提下显著提升了用户体验和控制能力。
...
### [200] [Ax-Prover: A Deep Reasoning Agentic Framework for Theorem Proving in Mathematics and Quantum Physics](https://arxiv.org/abs/2510.12787)
...
TL;DR: Ax-Prover是一个基于多智能体系统的自动定理证明工具，使用LLM提供知识和推理，通过MCP协议与Lean工具集成，能在多个科学领域进行定理证明，支持自主和协作两种模式。


<details>
  <summary>Details</summary>
Motivation: 解决科学问题需要创造性推理和严格的形式化验证，传统专业证明系统难以泛化到不同领域。

Method: 将大型语言模型与Lean工具通过模型上下文协议(MCP)结合，构建多智能体系统进行形式化证明生成。

Result: 在公开数学基准测试中与最先进证明器竞争，在新引入的抽象代数和量子理论基准测试中表现优异，并能协助专家数学家完成复杂密码学定理的形式化证明。
