...
TL;DR: 提出基于结构化复值2D高斯基元的新全息图表示方法，将参数搜索空间减少10:1，实现更低VRAM使用和更快优化速度，同时生成更高质量的重建结果。


<details>
  <summary>Details</summary>
Motivation: 传统全息图表示方法需要逐像素信息存储，参数搜索空间大，计算资源消耗高，限制了下一代计算机生成全息系统的发展。

Method: 使用结构化复值2D高斯基元表示全息图，开发可微分光栅化器，并与GPU优化的自由空间光传播核集成，实现端到端训练。
...
TL;DR: 提出Gaussian See, Gaussian Do方法，实现无需绑定骨骼的跨类别语义3D运动迁移，通过运动嵌入和动态3D高斯泼溅重建技术


<details>
  <summary>Details</summary>
Motivation: 解决多视角视频中语义3D运动迁移的问题，实现跨类别对象间的有意义运动对应

Method: 基于隐式运动迁移技术，通过条件反演提取源视频运动嵌入，应用到静态目标形状的渲染帧，使用生成视频监督动态3D高斯泼溅重建；引入基于锚点的视角感知运动嵌入机制和鲁棒4D重建流程

Result: 建立了首个语义3D运动迁移基准，相比适应基线展现出更优的运动保真度和结构一致性

Conclusion: 该方法在语义3D运动迁移任务中表现出色，为相关研究提供了新的技术路径和基准
...
TL;DR: 本研究对12种最先进的神经网络架构在牙科全景X光片龋齿分割任务上进行基准测试，发现基于CNN的DoubleU-Net表现最佳，超越了所有Transformer和Mamba变体，表明在医学图像分割中架构与任务的匹配比模型复杂度更重要。
...
Method: 在DC1000数据集上，对12种最先进的卷积神经网络、视觉Transformer和状态空间Mamba架构进行统一配置训练，包括VMUnet、MambaUNet、VMUNetv2等模型。

Result: 基于CNN的DoubleU-Net取得了最高的Dice系数0.7345、mIoU 0.5978和精度0.8145，在所有性能指标的前三名均为CNN架构，而Mamba和Transformer方法表现不佳。

Conclusion: 尽管Mamba和Transformer在全局上下文建模方面具有理论优势，但由于数据有限和空间先验较弱而表现不佳，这强调了在特定领域医学图像分割中架构-任务匹配的重要性超过模型复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 传统CAD B-Rep表示在生成过程中容易出现失败，需要一种更稳健的几何表示方法来解决这个问题。

Method: 将CAD表面网格几何编码为有符号距离函数(SDF)，顶点、边、面及其拓扑信息编码为每面无符号距离函数(UDF)，使用改进的Marching Cubes算法转换为水密B-Rep模型，并采用基于3D U-Net的多分支潜在扩散模型联合生成SDF和UDF。

Result: 与最先进方法相比达到相当的CAD生成性能，同时实现了前所未有的100%成功率生成（分面）B-Rep模型。

Conclusion: BR-DF提供了一种稳健的CAD几何表示方法，其转换过程永不失败，为CAD模型生成提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么从头训练生成模型，要么依赖视觉语言模型。前者忽视室内场景固有的图结构，后者需要用户提供语义图或真实关系标注，限制了对象交互的多样性。

Method: 基于等变图神经网络(EGNNs)，提出一种简单有效的文本特征条件化策略，利用场景图结构和几何对称性，无需预定义关系类别。

Result: 尽管不使用真实关系标注，GeoSceneGraph的性能与使用此类标注的方法相当。

Conclusion: GeoSceneGraph通过结合图结构和几何对称性，有效解决了3D场景合成的挑战，在资源受限设备上具有部署优势。
...
TL;DR: 提出了一种无监督的单图像双向MRI合成器，能够在高场强和超低场强MRI图像之间相互转换，基于物理驱动的对比度变化原理。
...
TL;DR: 提出InstructMix2Mix框架，将2D扩散模型的编辑能力蒸馏到预训练的多视图扩散模型中，实现稀疏输入视图下的多视图图像编辑，保持跨视图一致性


<details>
  <summary>Details</summary>
Motivation: 现有基于每场景神经场或时间注意力机制的方法在多视图编辑中会产生伪影和不一致编辑，需要解决跨视图一致性问题

Method: 用多视图扩散学生替换SDS中的传统神经场整合器，采用增量学生更新、专用教师噪声调度器和注意力修改来增强跨视图一致性

Result: 实验表明I-Mix2Mix显著提高了多视图一致性，同时保持高单帧编辑质量

Conclusion: 该方法成功解决了稀疏视图下的多视图图像编辑问题，实现了高质量的跨视图一致性编辑
...
TL;DR: SkinR1是一个新颖的皮肤病视觉语言模型，通过结合基于教科书的推理和强化学习来解决数据异质性、缺乏诊断理由和泛化能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在皮肤病诊断中存在三个主要限制：数据异质性导致诊断标签不一致、缺乏基于诊断理由的可靠推理监督、以及在小规模密集标注数据集上训练的模型难以泛化到大规模稀疏标注数据。

Method: 1) 设计基于教科书的推理生成器，合成高质量、层次感知和鉴别诊断信息轨迹；2) 利用构建的轨迹进行监督微调，赋予模型基于理由的推理能力；3) 开发结合疾病层次结构的新强化学习范式，将推理模式迁移到大规模稀疏数据。

Result: 在多个皮肤病数据集上的广泛实验表明，SkinR1实现了卓越的诊断准确性。消融研究证明了监督微调建立的推理基础的重要性。

Conclusion: SkinR1通过统一的端到端框架有效解决了皮肤病视觉语言模型的关键挑战，结合深度教科书推理和强化学习的泛化能力，显著提升了诊断性能和可信度。
...
TL;DR: FarSLIP是一个针对遥感数据的细粒度语言-图像预训练框架，通过构建首个多粒度遥感图像-文本数据集MGRS-200k，采用patch-to-patch蒸馏和CLS token区域类别对齐，提升了遥感领域的细粒度视觉-语言对齐能力。


<details>
  <summary>Details</summary>
Motivation: 现有遥感特定CLIP变体仍受限于全局对齐，无法捕捉细粒度细节。主要问题包括：当前遥感图像-文本数据集从对象级标签生成全局描述，未充分利用对象级监督；通用领域的区域-文本对齐方法直接应用于遥感数据会导致性能下降。

Method: 构建MGRS-200k多粒度遥感图像-文本数据集；采用patch-to-patch蒸馏对齐局部和全局视觉线索；使用简单的CLS token区域类别对齐而非显式patch级对齐；避免常见的patch-to-CLS自蒸馏方法。

Result: 在遥感开放词汇语义分割任务上达到新的SOTA；在图像级任务如零样本分类和图像-文本检索上也表现优异；提升了特征可区分性并保持了语义连贯性。

Conclusion: FarSLIP通过改进的细粒度对齐策略，显著提升了遥感领域的视觉-语言对齐能力，在多个下游任务上实现了最佳性能，为遥感视觉语言理解提供了有效解决方案。
...
TL;DR: nnMIL是一个简单但广泛适用的多实例学习框架，将补丁级病理基础模型连接到稳健的幻灯片级临床推理，在40,000张全幻灯片图像和35个临床任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前将补丁级特征聚合为幻灯片级预测的方法存在设计限制，阻碍了泛化性和可靠性。
...
TL;DR: 提出了X-WIN模型，通过从胸部CT学习预测2D投影来获取3D解剖知识，解决CXR结构叠加导致的诊断挑战


<details>
  <summary>Details</summary>
Motivation: CXR作为2D投影图像存在结构叠加限制，无法捕捉3D解剖结构，这使得表征学习和疾病诊断变得困难

Method: 学习在潜在空间中预测CT的2D投影，引入亲和力引导的对比对齐损失，结合真实CXR进行掩码图像建模，使用域分类器确保模拟和真实CXR表征相似

Result: X-WIN在多种下游任务中优于现有基础模型，能够渲染2D投影以重建3D CT体积

Conclusion: X-WIN通过整合3D解剖知识显著提升了CXR的表征学习和诊断能力
...
TL;DR: 提出CPSL（内容促进场景层），一种紧凑的2.5D视频表示方法，通过深度和内容显著性引导将2D内容转换为具有视差效果的沉浸式媒体，避免了昂贵的3D重建过程。
...
Conclusion: CPSL为从2D视频到可扩展2.5D沉浸式媒体提供了一条实用路径，支持实时播放和标准视频编解码器。
...
TL;DR: 提出了首个包含580个多模态人类活动序列的长周期工作流基准，支持三个评估任务，并提出了一个轻量级、无需训练的基础方法。


<details>
  <summary>Details</summary>
Motivation: 填补长周期工作流研究的空白，传统方法主要关注短周期活动，而长周期工作流具有低对比度模式的特点。

Method: 提出了一个轻量级、无需训练的基础方法，用于建模多样化的周期性工作流模式。

Result: 基准测试对无监督周期检测方法和基于大语言模型的零样本方法都提出了显著挑战；提出的基础方法在所有评估任务中大幅优于竞争方法；在真实应用中，其部署优势与传统监督方法相当，无需标注和重新训练。

Conclusion: 该研究为长周期工作流分析提供了首个基准和有效的基础方法，展示了在真实应用中的实用价值。
...
TL;DR: 提出了一种低成本的多视角视频同步方法，使用LED时钟编码时间信息，实现毫秒级时间对齐，适用于异构相机系统（RGB和红外）。


<details>
  <summary>Details</summary>
Motivation: 多视角视频流的时间同步在动态场景应用中至关重要，但在异构相机系统中（如专业与消费级设备混合、可见光与红外传感器结合等）硬件同步能力往往不可用，特别是在非受控的真实环境中。

Method: 使用定制的LED时钟，通过红色和红外LED编码时间信息，从记录的帧中视觉解码曝光窗口（开始和结束时间），实现毫秒级同步。

Result: 与硬件同步基准相比，实现了1.34ms RMSE的残差误差；在实验中优于基于光、音频和时间码的同步方法，并直接改善了多视角姿态估计和3D重建等下游计算机视觉任务；在包含25+异构相机的大规模手术记录中验证了系统。

Conclusion: 该解决方案简化并优化了同步流程，扩展了在非约束环境（包括工业和临床应用）中基于视觉的先进传感技术的可访问性。
...
TL;DR: 提出基于深度学习的自适应控制系统，通过材料分类和烟雾检测动态调整激光切割机抽气泵功率，实现20-50%的能耗降低
...
TL;DR: 本文提出了一种边缘引导的空间注意力机制(EGSA)，通过将边界信息融入语义和几何特征的融合过程，改善透明物体感知中的多任务学习性能。


<details>
  <summary>Details</summary>
Motivation: 透明物体感知是计算机视觉中的主要挑战，因为透明度会混淆深度估计和语义分割。现有的多任务学习方法存在负向跨任务交互问题，阻碍了性能提升。

Method: 1. 提出边缘引导空间注意力机制(EGSA)，在语义和几何特征融合中融入边界信息；2. 采用多模态渐进式训练策略，从RGB图像边缘过渡到深度图像边缘进行学习。

Result: 在Syn-TODD和ClearPose基准测试中，EGSA相比当前最优方法(MODEST)持续提高了深度估计精度，同时保持了竞争力的分割性能，在透明区域改进最为显著。
...
TL;DR: 提出了一种仅使用特征损失的知识蒸馏框架，无需logit损失，通过知识质量指标选择最有效的教师层进行蒸馏，在多个数据集上达到SOTA性能。
...
Method: 基于潜在表示几何特性的发现，引入知识质量指标来识别哪些教师层能提供最有效的知识，构建纯特征损失的知识蒸馏框架。

Result: 在三个图像分类数据集和四种不同的师生模型对上，该方法达到最先进性能，top-1准确率相比标准方法提升高达15%。

Conclusion: 仅使用特征损失的知识蒸馏框架是有效的，通过精心选择教师层可以显著提升学生模型的性能，为知识蒸馏提供了新的方向。
...
Result: Kandinsky 5.0在人类评估中表现出高生成速度和最先进的性能，能够实现高分辨率图像和10秒视频的合成。
...
TL;DR: FinCriticalED是首个针对金融文档事实层面理解的评估基准，通过专家标注的金融事实来评估OCR和视觉语言模型在视觉密集的金融文档中的表现。


<details>
  <summary>Details</summary>
Motivation: 金融文档具有视觉密集和表格繁多的布局，其中数值和时间信息与结构紧密耦合。传统OCR指标如ROUGE和编辑距离只捕捉表面文本相似性，无法检测可能导致重大财务误判的关键错误。
...
TL;DR: 提出了CKDA方法来解决可见光-红外终身行人重识别中的模态知识干扰问题，通过模态知识解耦和对齐来平衡模态特定知识和模态通用知识的学习与保持。
...
Method: 提出跨模态知识解耦和对齐方法CKDA，包括模态通用提示模块和模态特定提示模块来解耦知识，以及跨模态知识对齐模块在双模态原型基础上进行特征空间对齐。

Result: 在四个基准数据集上的大量实验验证了CKDA方法的有效性和优越性，超越了现有最先进方法。

Conclusion: CKDA方法通过明确分离和平衡保存模态特定知识和模态通用知识，有效解决了VI-LReID中的知识干扰和遗忘问题。
...
TL;DR: 研究表明，在图像分类任务上训练的计算机视觉模型不仅学习到与成人相似的几何和数学概念表征，其训练过程中的性能提升也与儿童的发展轨迹相匹配，显示出发展对齐性。


<details>
  <summary>Details</summary>
Motivation: 探索计算机视觉模型是否在训练过程中表现出与儿童数学认知发展相似的发展轨迹，即是否存在发展对齐性。

Method: 以ResNet-50模型为案例研究，分析其在几何和数字概念上的表现变化，并与儿童发展轨迹进行比较。

Result: 在几何和拓扑方面，部分概念类别（欧几里得几何、几何图形、度量属性、拓扑）显示出发展对齐性，但其他类别（手性图形、几何变换、对称图形）没有。在数字方面，模型随着经验积累发展出类似人类的"心理数字线"表征。
...
TL;DR: UniHOI是一个统一的人类-物体交互(HOI)检测和生成框架，通过共享的token空间实现双向知识迁移，在有限标注下提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法将HOI检测和生成作为独立任务处理，阻碍了全面的交互理解。需要统一的框架来促进知识共享和泛化能力。

Method: 提出对称的交互感知注意力模块和统一的半监督学习范式，在统一token空间中实现图像与交互语义的双向映射。

Result: 在长尾HOI检测上准确率提升4.9%，在开放词汇生成任务中交互指标提升42.0%，在两个任务上都达到最先进水平。

Conclusion: UniHOI通过统一建模HOI检测和生成任务，有效促进了知识共享，显著提升了模型的泛化能力和性能。
...
TL;DR: 提出DLRRF模型处理高光谱和多光谱图像融合中的图像间变异性问题，通过分解目标图像为低秩和残差分量来恢复丢失的空间细节。


<details>
  <summary>Details</summary>
Motivation: 由于采集条件不同，高光谱和多光谱图像之间存在光谱变异性和空间局部变化，这种图像间变异性严重影响融合性能，现有方法直接对图像进行变换会加剧融合模型的不适定性。

Method: 将光谱变异性建模为光谱退化算子的变化，将目标高光谱图像分解为低秩和残差分量，利用图像内光谱相关性进行降维，引入隐式正则化器利用空间先验信息，使用PAO算法在PnP框架下求解。

Result: 广泛的数值实验表明DLRRF在存在图像间变异性的情况下，能够实现高光谱和多光谱图像的优异融合性能。

Conclusion: DLRRF模型有效解决了高光谱和多光谱图像融合中的图像间变异性问题，通过新的建模和优化方法提升了融合性能。
...
TL;DR: CellGenNet是一个基于知识蒸馏的细胞分割框架，通过师生架构在有限监督下实现跨组织稳健分割，使用软伪标签和混合损失函数提升性能。


<details>
  <summary>Details</summary>
Motivation: 显微镜全切片图像中细胞核分割面临染色、成像条件和组织形态的变异性挑战，需要开发在有限监督下具有强泛化能力的分割方法。

Method: 采用师生架构，容量教师模型在稀疏标注上训练并生成软伪标签；学生模型通过整合真实标签、概率目标和混合损失函数（二元交叉熵+Tversky损失）进行优化；结合一致性正则化和分层dropout稳定特征表示。
...
Method: 1. 使用大语言模型生成脚本并与视频库进行视觉语义匹配获取候选镜头；2. 从参考视频中分割标注镜头并提取属性；3. 使用能量模型学习参考风格并评分候选序列；4. 结合语法规则优化镜头组装。

Result: 方法能够自动化安排独立镜头，学习参考视频的组装风格，创建连贯的视觉序列，即使无编辑经验的用户也能制作视觉吸引力强的视频。

Conclusion: 提出的能量优化方法有效解决了视频镜头组装中的艺术风格保持问题，实现了兼具自动化和艺术表达的智能视频编辑。
...
TL;DR: 提出ProPL框架，用于通用半监督超声图像分割，支持多器官多任务，结合标记和未标记数据，通过提示引导双解码器和不确定性驱动的伪标签校准实现高效分割。
...
Method: 采用共享视觉编码器结合提示引导双解码器，通过解码时提示机制实现灵活任务适应，并使用不确定性驱动的伪标签校准模块进行可靠自训练。
...
Result: 现有MLLMs在垂直书写日文文本上的表现比水平文本差，但使用合成数据集训练可以改善原本无法处理垂直书写的模型性能。
...
TL;DR: 本文提出VR-Bench基准测试，通过视频生成来评估视频模型的空间推理能力，发现在迷宫求解任务中视频模型表现出优于视觉语言模型的推理性能。
...
Result: 监督微调能有效激发视频模型的推理能力；视频模型在推理过程中表现出更强的空间感知能力，优于领先的视觉语言模型，并能良好泛化到不同场景、任务和复杂度；发现测试时缩放效应，推理过程中的多样化采样可将推理可靠性提升10-20%。

Conclusion: 通过视频进行推理在空间推理任务中具有独特潜力和可扩展性，为视频模型的推理能力发展提供了新方向。
...
TL;DR: BokehFlow是一个基于流匹配的无深度可控散景渲染框架，直接从全焦图像合成照片级真实感的散景效果，无需深度输入，通过文本提示实现语义控制。
...
Method: 提出基于流匹配的深度自由框架，使用交叉注意力机制通过文本提示实现对焦点区域和模糊强度的语义控制。

Result: 在四个数据集上的实验表明，BokehFlow在渲染质量和效率方面优于现有的深度依赖和生成方法，实现了视觉上吸引人的散景效果和精确控制。

Conclusion: BokehFlow成功解决了无需深度输入的可控散景渲染问题，提供了高质量、高效且语义可控的解决方案。
...
TL;DR: MambaTrack3D是一个基于状态空间模型Mamba的3D单目标跟踪框架，专门针对高时间变化性的动态室外环境。它通过帧间传播模块和分组特征增强模块，解决了现有方法计算复杂度高、时间冗余和几何先验利用不足的问题。


<details>
  <summary>Details</summary>
Motivation: 动态室外环境中的高时间变化性给LiDAR点云中的3D单目标跟踪带来重大挑战。现有的基于记忆的跟踪器存在二次计算复杂度、时间冗余和几何先验利用不足的问题。

Method: 设计了基于Mamba的帧间传播模块，用高效的帧间传播替代传统的单帧特征提取，实现近线性复杂度；引入分组特征增强模块，在通道层面分离前景和背景语义，减少记忆库中的时间冗余。

Result: 在KITTI-HTV和nuScenes-HTV基准测试中，MambaTrack3D始终优于HTV导向和正常场景跟踪器，在中等时间间隔下比HVTrack提升了6.5%的成功率和9.5%的精度。在标准KITTI数据集上仍与最先进的正常场景跟踪器保持高度竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖统一情感标签，忽视了同一样本中不同模态可能表达不同情感倾向的模态间冲突问题。

Method: TiCAL框架通过伪单模态情感标签和典型性估计动态评估训练样本一致性，在双曲空间中嵌入特征以捕捉情感类别的细粒度差异。

Result: 在CMU-MOSEI和MER2023等基准数据集上的实验表明，TiCAL能有效缓解模态间情感冲突，相比最先进的DMD方法提升约2.6%的准确率。
...
TL;DR: 提出JCDM框架，通过多视角先验解决姿态引导人体图像生成中的纹理不完整和跨视角交互缺失问题，包含外观先验模块和联合条件注入机制。
...
Conclusion: JCDM支持可变数量参考视角，可与标准扩散主干网络集成，仅需最小化架构修改。
...
TL;DR: 本文研究了离散扩散多模态大语言模型(dMLLMs)的效率优化问题，重点关注视觉令牌冗余及其对推理效率的影响。研究发现视觉冗余主要出现在从头训练的dMLLMs处理长答案任务时，并提出了针对不同类型dMLLMs的优化策略。


<details>
  <summary>Details</summary>
Motivation: 现有的dMLLMs在推理时由于每个去噪步骤都需要全序列注意力计算，导致显著的计算开销。虽然已有研究尝试通过键值缓存优化或高效采样来解决这个问题，但大多忽略了模态特定的视觉令牌冗余问题。

Method: 通过全面研究不同dMLLM架构和任务下视觉令牌冗余的演变规律，以及视觉令牌剪枝对dMLLM响应和效率的影响。具体分析了从头训练dMLLMs与AR-to-diffusion dMLLMs的差异。

Result: 研究发现：1）视觉冗余仅出现在从头训练dMLLMs处理长答案任务时；2）视觉令牌剪枝会引入不可忽视的信息损失，但只有从头训练dMLLMs能在后期去噪步骤中逐步恢复丢失的信息；3）层跳过技术对AR-to-diffusion dMLLMs有效，而渐进或后期剪枝对从头训练dMLLMs更有效。
...
TL;DR: 提出Gaussian Blending方法替代传统alpha blending，解决3DGS在不同采样率下的模糊和阶梯伪影问题，保持实时渲染速度且无需额外内存开销


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在训练未见采样率下合成视图时存在明显视觉差异：放大时出现侵蚀引起的模糊伪影，缩小时出现膨胀引起的阶梯伪影

Method: 将alpha和透射率视为空间变化分布而非标量，提出Gaussian Blending方法，考虑像素区域内alpha值的空间分布来更新透射率，允许附近背景splat贡献最终渲染

Result: Gaussian Blending在训练未见采样率下有效捕捉细节，在可见和未见采样率下均优于现有新视图合成模型

Conclusion: Gaussian Blending作为即插即用模块可集成到现有3DGS框架中，解决不同采样率下的伪影问题，同时保持实时渲染性能
...
TL;DR: 提出了一种结构感知的手术反馈生成管道，通过从真实训练师-学员对话中学习手术动作本体，并使用Instrument-Action-Target三元组来指导GPT-4o生成临床基础的手术反馈。


<details>
  <summary>Details</summary>
Motivation: 高质量的手术中反馈对提升学员表现和长期技能获取至关重要，但需要能够理解临床相关表示的模型来自动化生成自然、类似训练师风格的反馈。

Method: 1) 从真实世界反馈文本中挖掘IAT三元组并聚类表面形式；2) 微调视频到IAT模型，利用手术程序和任务上下文以及细粒度时间仪器运动；3) 使用IAT三元组表示指导GPT-4o生成临床基础反馈。

Result: 视频到IAT识别任务中AUC持续提升（仪器：0.67到0.74；动作：0.60到0.63；组织：0.74到0.79）。反馈生成任务中，GPT-4o单独得分2.17，IAT条件化达到2.44（+12.4%），可接受生成比例从21%翻倍至42%。
...
TL;DR: 开发了一个事件触发系统来监测老年人的身心健康状态，通过GMM背景建模检测访客和老人的运动行为，使用SVM分析图像，并为老年人设计了直观的社交媒体沟通方式。


<details>
  <summary>Details</summary>
Motivation: 同时考虑老年人的身体和心理状态，为缺乏技术经验的老年人提供便捷的监测和沟通解决方案。

Method: 采用GMM背景建模检测运动行为，开发事件触发系统（看门狗、危险通知和照片链接事件），使用SVM机器学习分析图像，设计直观的操作方式。

Result: 在家庭场景中进行了实验，5个家庭参与测试，成功检测和记录了三种类型的生活活动事件。

Conclusion: 该系统能够有效监测老年人的身心健康，并为老年人提供了简单易用的社交媒体沟通方式。
...
### [37] [Computer-Use Agents as Judges for Generative User Interface](https://arxiv.org/abs/2511.15567)
*Kevin Qinghong Lin,Siyuan Hu,Linjie Li,Zhengyuan Yang,Lijuan Wang,Philip Torr,Mike Zheng Shou*

Main category: cs.CV

TL;DR: 提出了一个Coder-CUA协作框架，让编码语言模型作为GUI设计师，计算机使用代理作为评估者，通过任务可解性和导航成功率来优化GUI设计，而不是视觉外观。


<details>
  <summary>Details</summary>
Motivation: 当前GUI主要为人设计，迫使代理采用人类导向行为，而编码语言模型的进步为自动GUI设计提供了新可能。研究计算机使用代理能否作为评估者协助编码模型进行自动GUI设计。

Method: 引入AUI-Gym基准测试，涵盖52个应用的1560个任务；开发验证器确保任务可执行性；提出Coder-CUA协作框架，Coder作为设计师生成和修订网站，CUA作为评估者评估功能性和优化设计；设计CUA仪表板压缩导航历史为可视化摘要。

Result: 通过将代理定位为设计者和评估者，框架将界面设计转向代理原生效率和可靠性，使代理从被动使用转向在数字环境中主动参与。

Conclusion: 该工作为代理从被动使用转向在数字环境中主动参与迈出了一步，代码和数据集已开源。
...
TL;DR: 提出了一种与SAM集成的无偏语义解码策略，通过同时从支持和查询集中提取目标信息，在CLIP语义指导下进行一致预测，以解决SAM在少样本分割中依赖准确提示的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要从支持集中提取提示来激活SAM的泛化能力，但这种方式在适应未知类别时容易导致有偏的解码过程，因此需要一种更无偏的语义解码策略。
...
Conclusion: 提出的USD策略能够有效解决SAM在少样本分割中的提示依赖问题，实现更无偏的语义解码和更好的泛化性能。
...
### [39] [When to Think and When to Look: Uncertainty-Guided Lookback](https://arxiv.org/abs/2511.15613)
...
TL;DR: 研究发现测试时思考（生成显式中间推理链）对大型视觉语言模型的视觉推理并非总是有益，长推理链可能导致忽略图像的错误轨迹。提出基于不确定性的回顾机制，通过结合不确定性信号和自适应回顾提示来改善视觉推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时思考在大型视觉语言模型中显示出性能提升潜力，但缺乏对思考如何影响视觉推理的系统性分析。本文旨在填补这一空白，深入理解思考机制在视觉推理中的作用。

Method: 对InternVL3.5和Qwen3-VL家族的10个变体在MMMU-val上进行大规模对比分析；提出不确定性引导的回顾机制，结合不确定性信号、自适应回顾提示和广度搜索的无训练解码策略。

Result: 发现长推理链往往产生忽略图像的错误轨迹；短回顾短语与成功的推理轨迹强相关；提出的方法在MMMU上提升整体性能，在标准思考较弱的类别中获得最大增益，在固定模型家族和token预算下达到新的SOTA。

Conclusion: 测试时思考需要谨慎使用，并非越多越好；基于不确定性的回顾机制能有效改善视觉推理性能，并在多个基准测试中展现出一致的改进效果。
...
TL;DR: WaveFuse-AL是一个新颖的多策略主动学习框架，通过循环时间先验和性能驱动适应，在医学影像任务中动态融合多种获取策略，显著优于单策略和交替策略基线。


<details>
  <summary>Details</summary>
Motivation: 传统主动学习中的单一获取策略在不同学习阶段表现不一致，需要一种能够自适应融合多种策略的方法来最大化有限标注预算的效用。

Method: 提出WaveFuse-AL框架，将BALD、BADGE、Entropy和CoreSet四种成熟获取策略与循环（正弦）时间先验和性能驱动适应相结合，动态调整策略重要性。
...
### [41] [VisPlay: Self-Evolving Vision-Language Models from Images](https://arxiv.org/abs/2511.15661)
...
Method: 将基础VLM分配到两个交互角色：图像条件提问者（生成具有挑战性但可回答的视觉问题）和多模态推理者（生成银标签回答）。使用组相对策略优化（GRPO）进行联合训练，包含多样性和难度奖励来平衡问题复杂度和回答质量。

Result: 在Qwen2.5-VL和MiMo-VL两个模型系列上训练，VisPlay在八个基准测试（包括MM-Vet和MMMU）上实现了一致的视觉推理、组合泛化和幻觉减少的改进。

Conclusion: VisPlay展示了向自进化多模态智能的可扩展路径，能够有效利用未标注数据持续提升模型性能。
...
### [42] [DCL-SE: Dynamic Curriculum Learning for Spatiotemporal Encoding of Brain Imaging](https://arxiv.org/abs/2511.15151)
*Meihua Zhou,Xinyu Tong,Jiarui Zhao,Min Cheng,Li Yang,Lei Tian,Nan Wan*

Main category: cs.CV

TL;DR: 提出了DCL-SE框架，通过数据驱动的时空编码和动态课程学习策略，将3D脑数据高效编码为2D动态表示，在多个神经影像分析任务中优于现有方法。
...
Method: 使用近似秩池化(ARP)将3D脑体积数据编码为信息丰富的2D动态表示，采用动态课程学习策略和动态组机制(DGM)逐步训练解码器。

Result: 在六个公开数据集（包括阿尔茨海默病分类、脑肿瘤分类、脑动脉分割和脑年龄预测）上评估，DCL-SE在准确性、鲁棒性和可解释性方面持续优于现有方法。

Conclusion: 紧凑、任务特定的架构在大规模预训练网络时代具有关键重要性。
...
### [43] [MoDES: Accelerating Mixture-of-Experts Multimodal Large Language Models via Dynamic Expert Skipping](https://arxiv.org/abs/2511.15690)
...
<details>
  <summary>Details</summary>
Motivation: 现有的专家跳过方法直接应用于多模态MoE模型会导致显著性能下降，因为这些方法没有考虑MoE层间专家的异质贡献和不同模态token的特定行为。

Method: 提出全局调制局部门控机制整合全局层重要性到局部路由概率中，使用双模态阈值方法分别处理不同模态的token，并引入前沿搜索算法快速确定最优阈值。

Result: 在3个模型系列和13个基准测试中，MoDES显著优于现有方法。当跳过88%专家时，Qwen3-VL-MoE-30B-A3B-Instruct性能提升达10.67%，预填充时间提升2.16倍，解码时间提升1.26倍。

Conclusion: MoDES是首个无需训练的MoE多模态大语言模型高效推理框架，能有效平衡计算效率和模型性能，显著提升推理速度。
...
TL;DR: SceneEdited是首个专门支持通过3D点云更新进行高精地图维护研究的大规模城市数据集，包含800多个场景、73公里驾驶距离和约3平方公里城市区域，模拟了超过23,000个物体变化。


<details>
  <summary>Details</summary>
Motivation: 高精地图对城市规划、基础设施监测和自动驾驶至关重要，但环境变化导致地图快速过时。现有技术主要关注变化检测，但缺乏从2D图像变化检测到实际更新3D地图的有效方法。

Method: 创建了包含校准RGB图像、LiDAR扫描和详细变化掩码的数据集，提供基于基础图像结构从运动流程的基线方法，以及支持可扩展性、可追踪性和可移植性的综合工具包。

Result: 数据集覆盖广泛的城市区域，包含手动和自动合成的超过23,000个物体变化，模拟了路边基础设施、建筑物、立交桥和电线杆等现实城市修改。
...
TL;DR: 该论文提出了一种结合视觉和语言的方法来解决ARC-AGI抽象推理问题，通过视觉-语言协同推理和模态切换自校正策略，在多个基准模型上取得了比纯文本方法更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有大模型在从少量示例中推断结构化转换规则方面仍然失败，而人类在解决此类谜题时严重依赖视觉抽象。作者发现单纯将ARC-AGI网格渲染为图像会降低性能，因此假设视觉和语言在不同推理阶段具有互补优势。
...
TL;DR: 提出了一种新的多模态持续指令调优方法，通过将灾难性遗忘视为旧任务梯度缺失问题，利用参数空间的几何特性近似缺失梯度，并结合有限回放缓冲区和动态平衡策略来缓解遗忘。
...
Method: 将灾难性遗忘视为旧任务梯度缺失问题，利用参数空间的几何特性（当前参数与先前最优参数之间的方向向量）作为梯度指导，结合有限回放缓冲区的真实梯度，并通过伯努利采样策略动态平衡模型稳定性和可塑性。

Result: 在多模态持续指令调优数据集上的广泛实验表明，该方法在不扩展模型的情况下实现了最先进的性能，有效缓解了灾难性遗忘，同时保持了紧凑的架构。

Conclusion: 该方法通过几何梯度近似和动态平衡策略，成功解决了多模态持续学习中的灾难性遗忘问题，为紧凑架构下的持续学习提供了有效解决方案。
...
TL;DR: 提出SEC-Depth自进化对比学习框架，用于恶劣天气条件下的自监督深度估计，通过历史延迟模型构建对比学习机制提升鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有自监督深度估计方法在雨雾等恶劣天气条件下性能显著下降，需要解决能见度降低导致的深度预测问题

Method: 设计动态延迟模型更新策略捕获训练阶段优化状态，引入自进化对比损失(SECL)，将历史延迟模型输出作为负样本
...
TL;DR: 本文提出了一种新的人体运动预测评估指标MMCM，通过聚类方法识别运动模式，同时评估预测运动的覆盖范围（多模态分布）和有效性（运动合理性）。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标简单赞赏广泛分布的运动，即使这些运动属于单一模式或运动学上无效。需要同时评估多模态覆盖和运动有效性。

Method: 使用聚类方法将运动空间划分为多个模式，通过收集运动数据集中的可能未来运动来识别有效模式，从而显式评估预测运动是否分布在多个模式中。

Result: 实验验证了聚类方法产生合理的模式定义，且MMCM能够准确评估多模态预测。

Conclusion: MMCM解决了现有指标的不足，能够同时评估预测运动的覆盖范围和有效性，为人体运动预测提供了更全面的评估标准。
...
Method: 三阶段架构：1) 多模态特征提取；2) 置信度加权组件融合；3) 物理引导推理使用大语言模型强制执行结构关系约束
...
TL;DR: 提出了指令引导的病灶分割新范式，构建了首个大规模胸部X光病灶分割数据集MIMIC-ILS，并开发了ROSALIA模型，能够基于简单用户指令分割多种病灶类型并提供文本解释。
...
Method: 创建MIMIC-ILS数据集（包含110万指令-答案对），开发全自动多模态标注流程，并训练ROSALIA视觉语言模型进行指令引导的病灶分割。

Result: ROSALIA模型在分割和文本准确性方面表现优异，验证了该方法的有效性。

Conclusion: 该研究为像素级胸部X光病灶定位提供了基础资源，证明了指令引导分割范式的实用价值。
...
Result: 在11个MRI数据集上验证达到3.34年MAE（Pearson r=0.98，Spearman ρ=0.97，R²=0.95），在4个独立队列中MAE为3.77-5.04年，大脑年龄差与阿尔茨海默病、认知障碍和自闭症谱系障碍相关。
...
Conclusion: 提出了第一个真正零样本、既实用又高保真的生成框架，解决了风格化域中对象插入的挑战。
...
TL;DR: 本文提出了一种基于因果理论的食谱-食物图像跨模态检索方法，通过因果干预消除传统方法中的偏差，在Recipe1M数据集上实现了MedR=1的最优检索性能。
...
Method: 使用因果理论建模跨模态表示学习中的偏差，将食材识别为混淆源，通过简单的后门调整来减轻偏差。提出了一种即插即用的神经网络模块——多标签食材分类器，用于去偏。

Result: 在Recipe1M数据集上，实现了跨测试数据规模（1K、10K、50K）的MedR=1的oracle检索性能，报告了新的最先进搜索性能。

Conclusion: 基于因果理论的偏差消除方法能够显著提升食谱-食物图像跨模态检索的性能，证明了因果干预在解决此类问题中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有QAT方法需要昂贵的重新训练来恢复非线性层量化精度损失，而PTQ方法要么只能部分量化非线性函数，要么无法实现全整数推理。需要一种无需重新训练就能实现全整数推理的PTQ方法。

Method: 提出IPTQ-ViT框架：1) 多项式GELU函数针对视觉数据优化；2) 基于位移的Softmax函数提高PTQ中的近似精度；3) 统一指标整合量化敏感性、扰动和计算成本，为每个激活层选择最优近似函数。

Result: 在图像分类任务上达到最高6.44%p（平均1.78%p）的top-1准确率提升，物体检测任务上提升1.0 mAP。在W8A8和W4A8设置下优于部分浮点PTQ方法，准确率和延迟与整数QAT方法相当。

Conclusion: IPTQ-ViT无需重新训练即可实现全整数视觉Transformer推理，在准确性和效率方面均优于现有PTQ方法，接近QAT方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够进行可验证、逐步推理的皮肤病诊断AI系统，提高诊断透明度和准确性。

Method: 构建DermCoT语料库（包含10,000个训练案例和3,000个专家评分案例），定义DermEval六维评估器和DermBench基准，结合皮肤病感知的视觉蒸馏技术。

Result: 在DermBench上获得4.031/5的平均分，排名第一，比Vision-R1提升约41%；在三个皮肤病分类基准上表现稳定且具有竞争力。

Conclusion: 基于DermCoT的推理链监督显著提升模型性能，皮肤病感知的视觉蒸馏进一步改善叙事质量和识别能力。
...
TL;DR: SplitFlux通过分析Flux模型特性，提出基于LoRA的细粒度内容-风格解耦方法，在保持内容结构的同时实现高质量风格化。


<details>
  <summary>Details</summary>
Motivation: 现有SDXL方法在内容风格解耦方面效果不佳，而Flux模型由于特性未被充分探索，无法实现有效的内容风格分离。

Method: 基于Flux模型分析发现早期单流块控制内容、后期块控制风格，提出SplitFlux：1）秩约束适应-压缩特定块更新秩和幅度防止内容泄漏；2）视觉门控LoRA-将内容LoRA分为高秩和低秩分支，分别保留主体信息和残差细节。

Result: 大量实验表明SplitFlux在多种场景下均优于现有最优方法，实现了卓越的内容保持和风格化质量。

Conclusion: SplitFlux通过系统分析Flux模型特性并设计相应解耦机制，成功实现了高质量的内容风格分离和重新嵌入。
...
TL;DR: GQN是一种基于注意力机制的框架，将雷达感知的物体建模为图结构，通过图查询动态关注BEV空间，使用EdgeFocus和DeepContext Pooling模块进行关系推理和上下文聚合，在NuScenes数据集上显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 3D雷达检测对360度汽车感知至关重要，但雷达的长波长会产生稀疏且不规则的反射，挑战传统的基于网格和序列的卷积和变换器检测器。

Method: 提出Graph Query Networks (GQN)框架，使用图查询动态关注BEV空间，构建物体特定图，通过EdgeFocus模块进行关系推理，DeepContext Pooling模块进行上下文聚合。

Result: 在NuScenes数据集上，GQN将相对mAP提升高达+53%，比最强的先前雷达方法提升+8.2%，同时将峰值图构建开销减少80%，FLOPs成本适中。

Conclusion: GQN通过图结构建模和注意力机制有效解决了雷达数据稀疏和不规则的问题，显著提升了3D雷达物体检测性能。
...
TL;DR: 提出了LEO框架，通过将关系视为节点构建线图，进行边中心的关系推理来捕捉高阶关系依赖，提升3D场景图预测性能


<details>
  <summary>Details</summary>
Motivation: 现有方法采用对象中心的图神经网络，关系边特征仅通过连接的对象节点聚合信息，难以捕捉对关系预测至关重要的高阶关系依赖

Method: 首先预测对象对之间的潜在链接来抑制无关边，然后将原始场景图转换为线图（关系作为节点），应用线图神经网络进行边中心关系推理，最后将增强的关系特征集成回原始对象中心图

Result: 在3DSSG数据集上与两个竞争基线相比，性能持续提升
...
<details>
  <summary>Details</summary>
Motivation: 训练违禁品检测模型需要大量X射线安检图像，但收集和标注这些图像耗时费力。现有方法采用两阶段流程，需要人工进行前景提取，效率低下且成本高。
...
TL;DR: 本文提出了一种针对多模态检索增强生成系统的分层视觉攻击方法，通过在用户输入的图像中添加微小扰动来误导系统生成错误内容，而无需篡改其他组件。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态RAG系统存在安全漏洞，但之前的研究主要关注知识投毒攻击。本文考虑仅通过视觉扰动攻击的更具挑战性的场景，因为微调后的检索器和生成器具有鲁棒性，且视觉扰动在RAG链中传播时效果会减弱。

Method: 提出分层视觉攻击方法，通过两阶段策略：首先破坏跨模态对齐，然后破坏多模态语义对齐，使检索器从原始数据库中召回不相关知识，从而误导生成器的输出。

Result: 在OK-VQA和InfoSeek数据集上的实验表明，该方法能显著降低检索和生成性能，证明了视觉攻击对MRAG系统的有效性。

Conclusion: 多模态RAG系统对视觉攻击具有脆弱性，需要加强安全防护措施来应对这种新型威胁。
...
TL;DR: Text2Loc++是一个用于3D点云子地图定位的神经网络，通过从粗到细的定位流程实现语言与点云的有效跨模态对齐。在KITTI360Pose数据集上比现有方法性能提升达15%，并能处理复杂语言表达和多样化城市场景。


<details>
  <summary>Details</summary>
Motivation: 解决使用复杂多样的自然语言描述来定位3D点云子地图的问题，特别是在多样化的城市场景中处理不同语言复杂度的挑战。

Method: 采用从粗到细的定位流程：全局位置识别阶段使用预训练语言模型和分层Transformer结合最大池化处理句子级语义，注意力机制的点云编码器处理空间理解；提出掩码实例训练过滤非对齐对象；提出模态感知分层对比学习增强嵌入空间。精细定位阶段使用基于原型的地图克隆和级联交叉注意力Transformer的轻量级框架。

Result: 在KITTI360Pose数据集上比现有方法性能提升达15%，在新数据集上表现出良好的泛化能力，能有效处理复杂语言表达和多样化城市场景。

Conclusion: Text2Loc++通过创新的跨模态对齐方法和分层对比学习，在3D点云定位任务中取得了显著性能提升，并具有良好的泛化能力。代码和数据集将公开。
...
<details>
  <summary>Details</summary>
Motivation: 为了利用CNN和Transformer各自的优势，CNN擅长提取局部特征，Transformer擅长捕捉长距离依赖关系，结合两者可以提升MPox检测的性能。

Method: 提出RS-CA-HSICT框架，包含HSICT块、残差CNN模块、空间CNN块和通道增强模块。HSICT模块集成主干CNN和定制ICT块，用于多头注意力和结构化CNN层。采用逆残差学习和阶段分辨率减少来增强梯度传播和尺度不变性。通过通道融合注意力块和空间注意力机制优化特征选择。

Result: 在Kaggle基准和多样化MPox数据集上，分类准确率达到98.30%，F1分数达到98.13%，优于现有的CNN和ViT方法。

Conclusion: 提出的混合深度学习框架有效结合了CNN和Transformer的优势，在MPox检测任务上取得了state-of-the-art的性能，证明了该方法的有效性。
...
TL;DR: Uni-Adapter是一种无需训练、在线测试时自适应策略，通过动态原型学习和图标签平滑来提升3D视觉语言基础模型在噪声、不完整或分布偏移数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 3D视觉语言基础模型在实际应用中面临数据噪声、不完整性和分布偏移问题，导致性能下降，需要有效的适应策略。

Method: 提出动态原型学习，定义3D缓存存储类别特定聚类中心作为原型，通过相似性评分进行基于缓存的逻辑计算，结合图标签平滑模块捕获原型间相似性，并使用熵加权聚合统一预测。
...
TL;DR: 提出了MILL多光照低光数据集，包含不同光照强度下的图像，用于评估低光增强算法在不同光照条件下的性能。通过改进现有方法，在DSLR上获得了10 dB PSNR提升，智能手机上获得2 dB提升。


<details>
  <summary>Details</summary>
Motivation: 现有低光增强方法大多依赖单一光照条件下的配对数据，缺乏对不同光照强度下算法性能的理解。需要更全面的数据集来评估算法在不同光照条件下的鲁棒性。

Method: 创建MILL数据集，包含固定相机设置和精确照度测量下的多光照强度图像。利用数据集的多光照结构改进现有方法，提升算法在不同光照场景下的鲁棒性。
...
Result: 在独立测试集上取得优异性能：准确率0.9812、召回率0.9873、精确率0.9787、F1分数0.9826、特异性0.9954，特别在区分无人机与其他空中物体方面表现突出。

Conclusion: 该研究通过Transformer架构的多模态数据融合实现了空中物体分类的重大进展，为复杂空域中的无人机检测和监控提供了高精度、高鲁棒性的解决方案。
...
Conclusion: Split DNNs存在比先前认知更严重的隐私威胁，FIA-Flow框架能够有效揭示这种风险。
...
<details>
  <summary>Details</summary>
Motivation: 指纹活体检测系统面临欺骗攻击的严重威胁，需要开发能够自动检测指纹伪造的技术，并且该方案需要抵抗各种失真干扰。

Method: 使用各向异性扩散处理输入图像，通过三级小波变换，对不同层的系数进行自适应阈值处理并拼接成特征向量，最后用SVM分类器进行分类。

Result: 在90%像素缺失场景下准确率比对比方法高约8%，在70x70块缺失场景下准确率高约5%，对各种失真具有更好的抵抗能力。

Conclusion: 提出的方法在指纹伪造检测中表现出色，特别是在处理各种失真场景时具有显著优势，为指纹生物识别系统提供了更可靠的安全保障。
...
<details>
  <summary>Details</summary>
Motivation: 点云的固有稀疏性带来了双重冗余挑战：空间冗余（背景噪声影响精度）和信息冗余（前景内部信息冗余影响效率），现有跟踪器受此限制。

Method: 提出CompTrack框架，包含空间前景预测器（SFP）模块基于信息熵过滤背景噪声，以及基于信息瓶颈的动态令牌压缩（IB-DTC）模块通过在线SVD分析自适应压缩冗余前景为紧凑的代理令牌。

Result: 在KITTI、nuScenes和Waymo数据集上的实验表明，CompTrack实现了顶级的跟踪性能，并在单个RTX 3090 GPU上达到实时90 FPS的效率。
...
Method: 使用融合方案聚合多个置信度估计和每个检测特征，通过紧凑的多层感知机进行三分类（已知目标、未知物体、背景），并整合不同logit变体来提升性能。

Result: 在二分类中平均提升2.7% AUROC，保持或改进开放集mAP；在三分类中显著提升性能，闭合集mAP最高提升9个百分点（18%相对增益）。

Conclusion: 该方法为无人机导航提供了鲁棒的三分类能力，能够主动避开未知物体并安全忽略背景区域，显著提升了开放集检测性能。
...
TL;DR: SA-FARI是最大的开源野生动物多动物追踪数据集，包含11,609个相机陷阱视频，涵盖99个物种类别，提供16,224个个体身份和942,702个标注框，为通用多动物追踪模型提供新基准。


<details>
  <summary>Details</summary>
Motivation: 现有野生动物追踪数据集规模有限、物种单一、缺乏时空多样性，无法训练适用于跨野生动物种群的通用多动物追踪模型。

Method: 收集了2014-2024年间来自4大洲741个地点的相机陷阱视频，进行详尽标注，包括边界框、分割掩码和物种标签，并使用最先进的视觉语言模型进行评估。

Result: 创建了包含46小时密集标注视频的大型数据集，在物种多样性和地理覆盖范围方面具有显著优势，为野生动物追踪提供了新的基准。
...
TL;DR: GEO-Bench-2是一个用于评估地理空间基础模型(GeoFMs)的标准化基准框架，涵盖19个开源数据集，支持分类、分割、回归、目标检测和实例分割任务，通过"能力"组对模型进行排名，帮助用户选择适合特定任务的模型。


<details>
  <summary>Details</summary>
Motivation: 当前地理空间基础模型(GeoFMs)评估缺乏标准化协议，无法进行公平比较和确定模型在不同任务中的优势领域。

Method: 建立包含19个开源数据集的综合评估框架，引入"能力"组概念按数据集特性(分辨率、波段、时间性)对模型进行排名，制定规范但灵活的评价协议。

Result: 实验表明没有单一模型在所有任务中占优：自然图像预训练模型在高分辨率任务中表现优异，而EO专用模型在多光谱应用(农业、灾害响应)中表现更好。
...
Method: 结合语言语义分割（使用大语言模型将指令分解为有序子动作单元）和软掩码优化（学习实例特定的时间掩码，聚焦关键帧，保持段内连续性和段间分离性）。

Result: 在三个运动-语言数据集上达到最先进的运动定位性能，在HumanML3D基准上比先前方法提升+8.7% mAP，下游检索任务也有显著改进。

Conclusion: 为无标注运动理解建立了新范式，证明了在无需监督的情况下实现有效运动分解的可行性。
...
TL;DR: AutoPrune是一种新颖的LLM剪枝方法，通过让LLM自动设计剪枝算法来克服专家知识限制，并使用图驱动思维链优化提示，同时引入偏斜感知动态稀疏度分配来解决高剪枝率下的异常值问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM剪枝方法严重依赖人工设计的剪枝算法，导致巨大的人力成本和专业知识需求。此外，作者首次发现高剪枝率下因均匀稀疏度导致的严重异常值问题，引发性能急剧下降。

Method: 1. 利用LLM自动设计最优剪枝算法，无需专家知识；2. 提出图驱动思维链(GCoT)优化提示，增强推理过程；3. 引入偏斜感知动态稀疏度分配(SDSA)解决异常值问题。

Result: 在主流LLM基准测试上进行广泛实验，证明AutoPrune始终优于最先进的竞争对手。

Conclusion: AutoPrune通过让LLM自行剪枝，成功克服了专家知识限制和异常值问题，在保持性能的同时实现了高效的模型压缩。
...
TL;DR: 提出了一种多频率图卷积网络(MF-GCN)用于抑郁症检测，通过眼动追踪、音频和视频数据，利用多频率滤波器模块同时捕捉低频和高频信号，在二元和三元分类任务中均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于图的模型仅关注低频信息的局限性，充分利用多模态数据中的高低频信号来更准确检测抑郁症。

Method: 开发了多频率图卷积网络(MF-GCN)，包含新颖的多频率滤波器模块(MFFBM)，能够同时利用低频和高频信号进行抑郁症分类。

Result: 在二元分类中达到0.96的敏感度和0.94的F2分数；在三元分类中达到0.79的敏感度和0.87的特异性；在CMDC数据集上验证泛化能力，获得0.95的敏感度和0.96的F2分数。

Conclusion: 该三模态多频率框架能有效捕捉跨模态交互，实现准确的抑郁症检测，显著优于其他模型。
...
Conclusion: 高质量监督对于稳健的占用学习至关重要，这为无LiDAR的3D场景理解开辟了新的数据驱动方向。
...
Conclusion: 该框架实现了实用的风险感知分割，在临床环境中具有重要应用价值，提供有限样本保证且无需重新训练。
...
TL;DR: D4C是首个专门为CLIP模型设计的数据无关量化框架，通过生成语义丰富且结构多样的伪图像来解决现有DFQ方法在CLIP上的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有的数据无关量化方法在扩展到视觉语言模型（如CLIP）时存在性能显著下降的问题，主要原因是合成样本语义内容不足和图像内部多样性低。

Method: D4C包含三个关键组件：1) 提示引导语义注入，使用文本提示使生成图像与现实世界语义对齐；2) 结构对比生成，利用前景-背景对比合成重现自然图像的组合结构；3) 扰动感知增强，应用受控扰动提高样本多样性和鲁棒性。

Result: 在W4A8设置下，D4C显著提升了CLIP模型的性能：在CIFAR-10上Top-1准确率提升12.4%（ResNet-50）和18.9%（ViT-B/32），在CIFAR-100上提升6.8%和19.7%，在ImageNet-1K零样本分类上提升1.4%和5.7%。

Conclusion: D4C通过生成语义丰富且结构多样的伪图像，有效弥合了数据无关量化在CLIP模型上的性能差距，在各种比特宽度和模型上都显示出显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统自动驾驶数据集与冲突区域等极端环境下无人系统面临的独特操作场景之间的关键差距，提升自动驾驶车辆在高风险场景中的鲁棒性和安全性。

Method: 从DATTALION开源库构建数据集，面对数据异质性和伦理考量等挑战，使用在结构化城市场景训练的最先进语义分割模型建立性能基准，分析训练数据环境的影响，并在目标图像无标注的约束下探索有效导航性。

Result: 建立了WarNav数据集的性能基准，展示了在无标注目标图像约束下实现挑战性环境有效导航的初步成果。

Conclusion: WarNav数据集为在极端操作环境中开发自动驾驶技术提供了重要资源，旨在促进在数据标注有限的情况下提升高风险场景中自动驾驶车辆的鲁棒性和安全性研究。
...
TL;DR: 本文系统分析了多模态检测中的融合退化问题，提出了RSC-MD方法来缓解梯度抑制和模态不平衡，在多个数据集上实现了最先进的性能。
...
Result: 在FLIR、LLVIP、M3FD和MFAD数据集上的大量实验表明，该方法有效缓解了融合退化，在多个基准测试中达到最先进性能。

Conclusion: RSC-MD方法通过理论分析和实践验证，成功解决了多模态检测中的融合退化问题，为相关研究提供了新的思路和解决方案。
...
TL;DR: 提出了一个用于再制造质量检测的齿轮箱组件图像数据集，并开发了对比正则化损失来提升模型对未见组件类型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 再制造过程中需要手动检查拆卸组件的质量，由于零件和缺陷模式种类繁多，自动化视觉检测面临泛化到新产品变体、组件或缺陷模式的挑战。

Method: 创建了包含两种汽车变速箱齿轮箱组件的图像数据集，生成不同的分布偏移来评估分类模型的泛化能力，并提出了对比正则化损失来增强模型鲁棒性。

Result: 结果表明，所提出的损失函数能够有效提升模型对未见类型组件的泛化性能。

Conclusion: 对比正则化损失是提升再制造质量检测模型泛化能力的有效方法，特别是在面对新组件类型时。
...
Method: EASD采用双分支设计：基于时间的纹理加特征融合分支用于全局跨切片语义，熵选择性注意力分支用于物体中心细节。同时构建了首个面向驾驶的模拟脉冲检测基准DSEC Spike。

Result: 论文提出了首个端到端脉冲相机检测器EASD，解决了脉冲流检测的关键挑战。

Conclusion: EASD通过创新的双分支架构成功实现了端到端脉冲流检测，为自动驾驶在极端条件下的目标检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常在单一尺度上对齐HE图像切片和对应的空间转录组谱，忽略了细粒度细胞结构及其空间组织，需要开发能够捕捉多层次组织结构的框架。

Method: 提出Sigmma框架，采用多尺度对比对齐确保不同尺度的表示在模态间保持一致，并通过图表示细胞相互作用，整合图间和图内关系来捕捉组织微环境中从细到粗的细胞-细胞相互作用。

Result: Sigmma在基因表达预测任务中平均提升9.78%，在跨模态检索任务中平均提升26.93%，并在下游分析中学习了有意义的多组织组织结构。

Conclusion: Sigmma通过多尺度对比对齐和图表示细胞相互作用，有效学习了HE图像和空间转录组谱的层次表示，显著提升了跨模态对应关系的捕捉能力和下游任务性能。
...
TL;DR: 该研究开发了一个AI多阶段流水线，用于从围网渔船电子监控系统的视频中自动识别大眼金枪鱼和黄鳍金枪鱼，解决了物种识别困难的问题。


<details>
  <summary>Details</summary>
Motivation: 围网渔船捕获了全球约69%的热带金枪鱼，电子监控系统产生大量视频数据需要人工处理。AI集成可以减轻工作负担并提高报告准确性，但物种识别仍面临挑战，特别是大眼金枪鱼和黄鳍金枪鱼的区分困难。

Method: 采用多阶段处理流水线：首先比较三种分割方法（Mask R-CNN、DINOv2+SAM2、YOLOv9+SAM2），然后使用ByteTrack跟踪分割个体，最后评估标准多类分类和分层分类方法。

Result: YOLOv9+SAM2组合表现最佳，验证平均精度为0.66±0.03，召回率为0.88±0.03。结合分层分类方法，84.8%的个体被成功分割和分类，平均误差为4.5%。专家间对大眼金枪鱼和黄鳍金枪鱼的识别一致性分别为42.9%±35.6%和57.1%±35.6%。

Conclusion: YOLOv9-SAM2分割与分层分类相结合的方法能够有效估计金枪鱼渔获物的物种组成，为电子监控系统的自动化分析提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像数据集中缺乏既包含诊断标签又包含诊断背后推理的密集标注，这种推理相关注释对于开发和评估可解释AI模型至关重要。

Method: 生成抽象的肺结节形状，具有可控的视觉属性（如圆度、边缘锐度、毛刺等），目标类别来自预定义的属性组合，允许完全控制连接属性与诊断类别的决策规则。

Result: FunnyNodules可用于模型无关的评估，评估模型是否学习正确的属性-目标关系，解释属性预测中的过度或不足表现，并分析与属性特定感兴趣区域的注意力对齐。

Conclusion: FunnyNodules提供了一个多功能的基础，用于开发、基准测试和深入分析医学图像分析中的可解释AI方法，具有完整的地面真实信息。
...
TL;DR: 提出了扩展自回归表示(EAR)，一种受人类视觉系统启发的图像生成新范式，通过螺旋式从中心向外扩展的生成顺序，实现高效并行解码和更好的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视觉生成方法存在效率问题，要么需要逐token解码，要么多尺度表示复杂。受人类视觉中心向外感知模式启发，希望开发更高效的自回归生成方法。

Method: EAR采用螺旋式从中心向外扩展的图像token生成顺序，保持空间连续性并支持并行解码。还提出了长度自适应解码策略，动态调整每步预测的token数量。

Result: 在ImageNet上的实验表明，EAR在单尺度自回归模型中实现了保真度和效率之间的最先进权衡。

Conclusion: EAR为可扩展且认知对齐的自回归图像生成设定了新方向，通过生物启发设计降低了计算成本并提高了生成质量。
...
Method: 提出MTPR模块抑制干扰并聚合互补语义线索，TAFF模块利用多文本嵌入作为语义锚点促进局部原型的迁移，FCWA模块利用支持前景特征的内部自相似性增强视觉先验鲁棒性。

Result: 在1-shot设置下，在PASCAL-5i上达到76.8% mIoU，在COCO-20i上达到57.4% mIoU，在类内变化较大的fold上有显著提升。

Conclusion: MTGNet通过多文本引导和跨模态优化有效解决了少样本语义分割中的语义覆盖不足和类内变化问题。
...
<details>
  <summary>Details</summary>
Motivation: 杂草检测是精准农业的关键任务，准确的物种识别允许农民选择性施用除草剂，符合可持续农业管理需求。需要构建对多种田间条件具有鲁棒性的检测模型。

Method: 使用CNN、ViT和GNN的混合深度学习框架，结合GAN数据增强平衡类别分布，采用自监督对比预训练从有限标注数据中学习更多特征。

Result: 在多个基准数据集上取得了99.33%的准确率、精确率、召回率和F1分数，模型能够表示局部、全局和关系特征，具有高可解释性和适应性。

Conclusion: 该框架支持实时高效部署到边缘设备，实现自动化杂草检测，减少对除草剂的过度依赖，为精准农业提供可扩展的可持续解决方案。
...
TL;DR: 本文评估了人体姿态预测算法，发现可复现性问题，提出了统一训练评估流程，将语音模型适配到姿态预测任务中提升性能，并评估了模型在噪声姿态数据上的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人体姿态预测在动作识别、自动驾驶和人机交互等领域有重要应用，但现有算法存在可复现性问题，且需要评估模型在真实噪声数据下的鲁棒性。

Method: 建立统一的训练和评估流程，将语音理解模型适配到姿态预测任务，引入基于姿态估计器噪声坐标的新数据集变体，使用无监督微调来恢复性能。

Result: 语音模型能有效提升姿态预测性能，估计姿态会导致显著性能下降，但无监督微调可以部分恢复性能。

Conclusion: 语音模型可成功应用于姿态预测任务，模型对噪声姿态数据敏感，但无监督微调能有效提升鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在干净条件下表现优异，但在反取证视角下，需要开发先进的对抗攻击来评估这类检测器的安全性，这方面研究尚不充分。

Method: 通过空间插值梯度和频率感知扰动捕获取证重要特征，联合建模空间和频域特征重要性，并将其融合来指导基于优化的对抗样本生成。

Result: 跨多个AI生成图像检测器的广泛实验验证了DuFIA的跨模型迁移性、透明性和鲁棒性。

Conclusion: DuFIA方案能够在某种程度上使AI生成图像检测器失效，为评估检测器安全性提供了有效工具。
...
TL;DR: 本文分析了ViT特征蒸馏失败的原因，发现尽管ViT表示全局低秩，但单个token使用高带宽编码模式，导致师生宽度不匹配。提出了两种解决方案：后验特征提升和原生宽度对齐，显著提升了ViT特征蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 特征图知识蒸馏在卷积网络中很有效，但在ViT中经常失败。本文旨在理解这种失败原因并指导方法设计。

Method: 通过两层表示分析：层间SVD分析显示全局低秩结构，token级谱能量模式分析揭示高带宽编码。基于此提出两种策略：后验特征提升和原生宽度对齐。

Result: 在ImageNet-1K上，这些策略重新激活了ViT的简单特征图蒸馏，将DeiT-Tiny准确率从74.86%提升到77.53%和78.23%。

Conclusion: 分析解释了ViT特征蒸馏失败的原因，并展示了如何利用低秩结构产生有效、可解释的补救措施，为紧凑ViT提供具体设计指导。
...
TL;DR: AVATAAR是一个模块化、可解释的视频问答框架，通过结合全局和局部视频上下文、预检索思考代理和重新思考模块，实现了类似人类的迭代推理，在长视频理解任务上显著提升了性能。
...
Method: AVATAAR采用模块化框架，包含持久全局摘要、预检索思考代理和重新思考模块，通过反馈循环机制实现迭代推理，结合全局和局部视频上下文。

Result: 在CinePile基准测试中，相比基线模型，AVATAAR在时间推理上相对提升5.6%，技术查询提升5%，主题问题提升8%，叙事理解提升8.2%。实验证实每个模块都对性能有积极贡献。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR地点识别方法在新环境中难以适应且会忘记先前学到的知识（灾难性遗忘），需要解决这一持续学习挑战。

Method: 扩展KDF范式，提出损失感知采样策略（根据损失值估计样本学习难度，优先采样困难样本）和排练增强机制（在训练新任务时微调记忆样本以强化长期知识保留）。
...
TL;DR: 提出了一种新颖的多模态深度学习方法，通过结合X射线图像来补全3D超声中因骨骼声影而遮挡的解剖结构，改善了脊柱椎体的可视化效果。


<details>
  <summary>Details</summary>
Motivation: 超声在脊柱手术中具有无辐射、实时可视化的优势，但由于骨骼声影效应，无法完整显示椎体解剖结构。需要解决超声在椎体可视化方面的固有局限性。

Method: 使用多模态深度学习，将单个X射线图像与3D超声结合，生成配对的训练数据（模拟X射线扫描和3D部分椎体表示），整合两种成像模式的形态学信息。
...
Method: 提出解耦分割头，将多类预测分离为类别无关的掩码预测和类别标签预测；引入全尺度感知可变形Transformer模块，通过可变形注意力实现低分辨率编码器特征与全分辨率特征的跨尺度融合。
...
Method: 提出基于hourglass transformer架构的推测解码方案，利用网格token的结构和几何相关性进行跨面、点、坐标级别的并行预测

Result: 实验显示FlashMesh相比标准自回归模型实现最高2倍加速，同时提升了生成保真度

Conclusion: 网格数据中的结构先验可以系统性地用于加速和增强自回归生成
...
TL;DR: HASTEN方法通过将分层语义信息嵌入到双曲空间中，缓解了基于CLIP的类增量学习中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的视觉和语言概念本质上是分层的，但现有的CLIP-based CIL方法未能显式捕捉这种分层结构，导致细粒度类别特征漂移和灾难性遗忘。

Method: 1. 使用外部知识图谱作为监督，在双曲空间中嵌入视觉和文本特征以保持分层结构；2. 将梯度投影到共享双曲映射器的零空间，防止对先前任务的干扰。

Result: 大量实验表明HASTEN始终优于现有方法，并提供统一的结构化表示。

Conclusion: HASTEN通过维护分层关系使模型能够抵抗遗忘，在类增量学习中取得了显著改进。
...
TL;DR: 提出MUSSE-Net，一种残差感知的多阶段无监督深度学习框架，用于超声应变弹性成像中的稳健应变估计，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 超声应变弹性成像在临床应用中受到组织去相关噪声、缺乏真实标签以及不同变形条件下应变估计不一致的限制。

Method: 使用USSE-Net作为骨干网络，这是一种端到端多流编码器-解码器架构，并行处理变形前后的RF序列来估计位移场和轴向应变。采用上下文感知互补特征融合编码器、三交叉注意力瓶颈和交叉注意力融合序列解码器，并利用一致性损失确保时间相干性。

Result: 在仿真数据上达到目标SNR 24.54、背景SNR 132.76、CNR 59.81和弹性成像SNR 9.73；在BUET数据集上产生增强的病灶-背景对比度和显著噪声抑制的应变图。

Conclusion: MUSSE-Net在多个数据集上优于现有无监督方法，产生临床可解释的应变模式，具有增强的病灶对比度和噪声抑制能力。
...
TL;DR: 本文提出MambaIO，通过拉普拉斯金字塔分解IMU测量值，使用Mamba架构处理低频分量提取上下文运动线索，卷积结构处理高频分量捕获局部运动细节，在多个公开数据集上实现最先进的定位性能。
...
Result: 在多个公开数据集上的实验表明，MambaIO显著降低了定位误差，达到了最先进的性能。

Conclusion: 这是Mamba架构在惯性里程计任务中的首次应用，证明该方法能有效提高行人惯性定位的准确性。
...
TL;DR: INQUIRE-Search是一个开源系统，使用自然语言在大规模生态图像数据库中搜索特定概念，验证并导出相关观测数据，用于新的科学分析。


<details>
  <summary>Details</summary>
Motivation: 大型社区科学平台包含数亿张生物多样性图像，捕捉了行为、互动、物候和栖息地等生态背景信息，但传统方法无法大规模访问这些次要信息。

Method: 开发INQUIRE-Search系统，通过自然语言交互式搜索生态图像数据库中的特定概念，验证和导出相关观测数据。

Result: 相比传统方法，INQUIRE-Search只需一小部分时间，通过五个案例研究展示了其支持的科学应用多样性，从物种行为的季节性变化到野火后的森林再生。

Conclusion: INQUIRE-Search展示了一种交互式、高效且可扩展的科学发现新范式，可以解锁大规模生物多样性数据集中以前无法访问的科学价值，并呼吁专家重新构建科学过程的优先级。
...
TL;DR: SS-MixNet是一种轻量级深度学习模型，用于高光谱图像分类，仅需1%标注数据就能在QUH-Tangdaowan和QUH-Qingyun数据集上达到95.68%和93.86%的总体准确率。


<details>
  <summary>Details</summary>
Motivation: 解决高光谱图像分类中在有限标注数据下实现准确和鲁棒预测的挑战。

Method: 结合3D卷积层提取局部光谱-空间特征，使用并行MLP风格的混合器块捕获光谱和空间维度的长程依赖关系，并采用深度卷积注意力机制增强判别能力。
...
TL;DR: 论文揭示了视频生成模型中第一帧的新作用：作为概念记忆缓冲区存储视觉实体供后续生成复用，而非传统认为的时空起点。基于此发现，仅需20-50个训练样本即可实现鲁棒的视频内容定制。


<details>
  <summary>Details</summary>
Motivation: 挑战传统观点，探索视频生成模型中第一帧的真正功能，发现其作为概念记忆缓冲区的潜在能力，为视频内容定制提供新思路。

Method: 利用第一帧作为概念记忆缓冲区的特性，通过少量训练样本（20-50个）进行微调，无需架构修改或大规模训练。

Result: 实现了鲁棒且通用的视频内容定制，在多样化场景中表现出色，揭示了视频生成模型基于参考的定制能力。

Conclusion: 第一帧在视频生成中扮演概念记忆缓冲区的关键角色，这一发现为视频内容定制提供了高效的新方法，仅需少量样本即可实现强大效果。
...
TL;DR: 提出了GeoVista智能体模型和GeoBench基准测试，用于评估智能体在需要视觉推理和网络搜索的地理定位任务中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有智能体视觉推理研究主要关注图像处理工具，缺乏面向通用智能体模型的地理定位任务评估。现有地理定位基准无法满足高分辨率图像和深度智能体推理的需求。

Method: 提出GeoVista智能体模型，在推理循环中无缝集成工具调用，包括图像放大工具和网络搜索工具。采用完整训练流程：冷启动监督微调学习推理模式和工具使用先验，然后通过强化学习增强推理能力，使用分层奖励利用多级地理信息。

Result: GeoVista在地理定位任务上大幅超越其他开源智能体模型，在大多数指标上达到与Gemini-2.5-flash和GPT-5等闭源模型相当的性能。

Conclusion: GeoVista证明了在推理循环中集成工具调用的有效性，为智能体视觉推理提供了新的解决方案，并在具有挑战性的地理定位任务上取得了优异表现。
...
TL;DR: 提出了一种新颖的密集特征匹配架构，通过系统改进解决了现有密集匹配器在复杂场景下性能不佳和速度慢的问题，实现了新的最先进性能。
...
Result: 在广泛实验中，所提出的新型匹配器建立了新的最先进水平，比其前身显著更准确。

Conclusion: 通过系统性的多方面改进，开发出了在准确性和效率方面都显著优于现有方法的密集特征匹配模型，解决了实际应用中的关键限制。
...
TL;DR: 本文综述了通过增加推理时计算来提高预训练大语言模型预测准确性的方法，重点分析了问题分解方式和子问题组织结构，统一了多种推理增强技术。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在推理时往往需要额外的计算资源来提升预测准确性，但目前缺乏对这些测试时扩展方法的系统性分类和分析。

Method: 从问题分解和子问题组织结构（顺序、并行、树状）的角度对测试时扩展方法进行分类，将Chain-of-Thought、Branch-Solve-Merge、Tree-of-Thought等方法统一在同一框架下。

Result: 建立了统一的分类框架，能够系统性地分析各种推理增强技术的优缺点，为方法比较提供了理论基础。

Conclusion: 提出了未来研究的有前景方向，为测试时扩展方法的进一步发展提供了指导。
...
TL;DR: 研究表明LLM在推理过程的早期阶段就内部承诺了最终答案，即使需要更长的输出来确定答案。对于更难的问题，预测准确率下降揭示了选择偏差：困难项目在长推理链中占比过高。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在推理过程中何时内部承诺最终答案，以及这种早期承诺对模型可解释性和推理时控制的影响。

Method: 通过在推理过程的前t个标记后对隐藏状态训练线性分类器，分析模型对最终正确性的预测能力。

Result: 最终正确性在仅几个标记后就可高度预测；对于困难问题，预测准确率下降表明长推理链中存在选择偏差。
...
TL;DR: LiveCLKTBench是一个自动化生成管道，专门用于隔离和测量大语言模型的跨语言知识迁移，通过时间敏感的知识实体来评估模型在不同语言间的知识转移能力。
...
Method: 构建LiveCLKTBench自动化生成管道：识别现实领域中的自包含、时间敏感知识实体，基于时间发生进行过滤，验证模型知识，生成事实性问题并翻译成多种语言来评估跨语言迁移。

Result: 评估多个LLM在五种语言上的表现，发现跨语言迁移受语言距离强烈影响，且在不同语言方向往往不对称。更大模型能改善迁移，但收益随规模增长而递减，且在不同领域表现各异。
...
TL;DR: COMPASS是一个轻量级可解释的控制框架，通过基于模型的反馈回路在解码过程中动态调节注意力头，减少LLM的上下文幻觉，同时提供对模型机制的透明洞察。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常生成流畅但事实错误的陈述，这是由于它们在上下文知识和参数知识之间分配注意力的方式存在问题。理解并引导这种内部行为对于可信部署和模型机制的科学可解释性至关重要。

Method: 引入COMPASS框架，通过上下文依赖分数(CRS)量化上下文依赖度，使用PID控制器动态调节注意力头，在解码过程中维持事实一致性，无需重新训练或多轮解码。

Result: 在多个基准测试(HotpotQA、XSum、HaluEval、RAGTruth)上，COMPASS持续减少了上下文幻觉率(绝对降低2.8%到5.8%)，并揭示了不同注意力头对证据对齐的贡献。

Conclusion: 反馈驱动的可解释性为科学理解LLM行为提供了一条路径，COMPASS展示了如何通过透明控制机制改善模型的事实一致性。
...
TL;DR: 评估手动和自动韵律分割标注对巴西葡萄牙语语音合成质量的影响，发现韵律分割训练能产生更清晰自然的语音，手动分割引入更多变异性有助于更自然的韵律。


<details>
  <summary>Details</summary>
Motivation: 自发语音合成面临捕捉自然对话流程的挑战，包括话轮转换、停顿和不流畅性。现有系统主要隐式建模韵律特征，但显式韵律分割数据集及其影响尚未充分探索。

Method: 使用非自回归模型FastSpeech 2，在巴西葡萄牙语数据集上比较手动和自动韵律分割标注对语音合成质量的影响。

Result: 韵律分割训练产生更清晰和声学自然的语音。自动分割创建更规则片段，手动分割引入更大变异性，有助于更自然韵律。两种方法都能重现预期的核心重音模式，但韵律模型更接近自然的预核心轮廓。

Conclusion: 韵律分割标注能提升自发语音合成的质量，手动分割在韵律自然性方面表现更好。所有数据集、源代码和训练模型已公开以支持可重复性研究。
...
Method: 开发了EasyMED多智能体框架，包含患者代理（实现真实对话）、辅助代理（确保事实一致性）和评估代理（提供可操作的反馈）。同时创建了SPBench基准，包含14个专科的真实SP-医生互动数据和8个专家定义的评估标准。

Result: 实验表明EasyMED能够匹配人类标准化患者的学习效果，对基础较差的学生产生更大的技能提升，同时提供更好的灵活性、心理安全性和成本效率。

Conclusion: EasyMED框架在临床技能培训中能够有效替代人类标准化患者，提供更经济、灵活且可扩展的模拟训练方案。
...
TL;DR: 提出了一种混合双向门控循环单元和长短期记忆网络(HBGRU-LSTM)模型，用于提升情感分析性能，在IMDB电影评论和亚马逊产品评价数据集上达到95%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于词典的方法和传统机器学习技术在处理上下文细微差别和可扩展性方面不足，深度学习虽然有所改进但仍需提升，特别是针对情感分析中的上下文细微差别、可扩展性和类别不平衡问题。

Method: 设计了一种混合深度神经网络模型，结合了双向门控循环单元(BGRU)和长短期记忆(LSTM)层，通过这种混合架构来改善语义关系捕获能力。

Result: HBGRU-LSTM模型在测试准确率达到95%，优于LSTM(93.06%)、CNN+LSTM(93.31%)和GRU+LSTM(92.20%)等传统深度学习框架。在平衡数据集上，负面情感召回率从86%提升到96%，误分类损失从20.24%降低到13.3%。

Conclusion: 所提出的HBGRU-LSTM混合模型在情感分析任务中表现出卓越性能，特别是在处理类别不平衡和提升模型泛化能力方面取得了显著进步，为意见挖掘提供了更公平和准确的情感分类方案。
...
TL;DR: 提出了Hierarchical Token Prepending (HTP)方法，通过分块预置摘要标记和均值池化解决大语言模型在长文档嵌入中的信息压缩问题，显著提升检索和嵌入性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的因果注意力机制限制了从后向前令牌的信息流动，降低了表示质量。现有方法通过预置单个摘要标记会过度压缩信息，在长文档上表现不佳。

Method: HTP将输入分块并为后续块预置块级摘要标记，创建多个后向信息流动路径；用均值池化替代最后令牌池化来解决读取级过度压缩问题。
...
TL;DR: 提出了一个基于数学框架来理解、测量和缓解大语言模型幻觉的方法，结合概率建模、信息论和贝叶斯不确定性估计，开发了改进的不确定性度量和缓解策略。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然功能强大，但仍然容易产生幻觉——听起来合理但实际上错误或缺乏依据的输出。需要建立一个数学基础框架来系统性地解决这个问题。

Method: 使用概率建模、信息论、三角信号分析和贝叶斯不确定性估计来分析自回归错误累积，提出了改进的不确定性度量（包括语义和相位感知变体），并开发了对比解码、检索增强接地、事实对齐和弃权等缓解策略。

Result: 建立了一个统一框架，将校准、检索和对齐等最新进展联系起来，支持更安全可靠的大语言模型。

Conclusion: 该数学框架为理解和缓解LLM幻觉提供了理论基础，通过连接多个技术领域的方法来提升模型的安全性和可靠性。
...
TL;DR: TASA是一个学生感知的数学辅导框架，通过整合学生画像、记忆和遗忘动态来实现个性化学习。它维护结构化的学生画像和事件记忆，结合连续遗忘曲线和知识追踪来动态更新学生掌握状态，生成情境适当且难度校准的问题和解释。


<details>
  <summary>Details</summary>
Motivation: 现有LLM辅导系统大多未能捕捉学生知识随熟练度、概念差距和遗忘模式的动态演变，尤其在数学辅导中需要精细校准到每个学生的掌握水平和认知保留程度。

Method: TASA维护结构化的学生画像（记录熟练度档案）和事件记忆（记录先前学习互动），通过结合连续遗忘曲线和知识追踪来动态更新学生掌握状态，生成情境适当且难度校准的问题和解释。

Result: 实证结果表明，TASA相比代表性基线方法实现了更优越的学习成果和更自适应的辅导行为。

Conclusion: 研究强调了在基于LLM的辅导系统中建模时间遗忘和学习者画像的重要性。
...
TL;DR: 提出了一个评估印度语言视觉语言模型的可扩展框架，并创建了HinTel-AlignBench基准测试，发现模型在印度语言上的性能相比英语平均下降8.3分（印地语）和5.5分（泰卢固语）。


<details>
  <summary>Details</summary>
Motivation: 当前多语言VLM评估存在四个主要局限：依赖未验证的自动翻译、任务/领域覆盖范围窄、样本量有限、缺乏文化和本地来源的问答数据。印度作为拥有12亿人口和120多种主要语言的多元化地区，需要更公平的AI评估方法。

Method: 开发了一个半自动数据集创建框架，结合回译、过滤和人工验证；构建了最全面的印地语和泰卢固语视觉语言基准，包括改编的英语数据集和本地新颖的印度数据集，每个语言约4000个问答对。

Result: 在5个任务中的4个任务中，所有模型在印度语言上的性能相比英语都有下降，印地语平均下降8.3分，泰卢固语平均下降5.5分。对常见失败模式进行了分类。

Conclusion: 多语言多模态理解存在明显差距，需要改进。提出的框架和基准为评估和推动多语言VLM的公平发展提供了重要工具。
...
TL;DR: 本文首次系统研究了内在维度(ID)与可解释文本属性之间的关系，发现ID与基于熵的指标互补，在文本类型上呈现稳定分层，并通过稀疏自编码器识别了影响ID的因果特征。
...
Method: 采用交叉编码器分析、语言特征和稀疏自编码器(SAEs)进行多角度研究，包括控制长度后的相关性分析、文本类型分层分析以及因果特征识别。

Result: 发现ID与熵指标在控制长度后不相关；不同文本类型呈现稳定的ID分层：科学文本ID最低(~8)，百科全书内容中等(~9)，创意/观点写作最高(~10.5)；通过SAEs识别出科学信号降低ID，人性化信号增加ID。

Conclusion: 当代LLM认为科学文本在表示上相对"简单"，而小说、观点和情感内容需要额外的表示自由度。研究为ID的正确使用和基于ID结果的合理解释提供了实用指导。
...
TL;DR: OEMA是一个零样本临床命名实体识别框架，通过多智能体协作解决监督模型依赖标注数据的问题，在MTSamples和VAERS数据集上达到最先进的精确匹配性能。


<details>
  <summary>Details</summary>
Motivation: 临床命名实体识别需要从电子健康记录中提取信息，但监督模型如CRF和BioClinicalBERT需要昂贵的标注数据。零样本NER虽然减少了对标注数据的依赖，但在示例选择粒度和将提示与自我改进结合方面存在困难。

Method: 提出OEMA框架，包含三个组件：自注释器生成示例、鉴别器通过SNOMED CT过滤示例、预测器使用实体描述进行准确推理，通过本体引导推理和多智能体协作实现零样本NER。

Result: 在MTSamples和VAERS数据集上，OEMA实现了最先进的精确匹配性能。在相关匹配下，它与监督的BioClinicalBERT相当，并超越了CRF。

Conclusion: OEMA通过本体引导推理和多智能体协作解决了零样本NER的关键挑战，实现了接近监督模型的性能，在临床NLP应用中显示出潜力。
...
TL;DR: 提出C3上下文级联压缩方法，使用大小两种LLM级联处理文本压缩和解码任务，在20倍压缩比下达到98%解码准确率，显著优于DeepSeek-OCR的60%准确率。
...
Method: 级联两个不同大小的LLM：小型LLM作为第一阶段将长上下文压缩为一组潜在token（如32或64长度），大型LLM作为第二阶段在压缩上下文中执行解码任务。
...
TL;DR: TeamNRC在BHASHA-Task 1语法错误纠正共享任务中，使用零/少样本提示方法在多种印度语言上取得良好成绩，并扩展到其他语言，同时讨论了数据质量和评估指标问题。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型在印度语言语法错误纠正任务中的潜力，特别是小模型的性能，并关注数据集质量和评估指标的适用性。

Method: 使用零/少样本提示方法，测试了从4B参数到大型专有模型的不同规模语言模型。

Result: 在泰卢固语和印地语中分别获得第4名和第2名，GLEU分数分别为83.78和84.31；同时扩展到泰米尔语、马拉雅拉姆语和孟加拉语。

Conclusion: 小规模语言模型在印度语言语法错误纠正中具有潜力，但需要关注高质量数据集创建和适合印度语言脚本的评估指标。
...
TL;DR: 使用SetFit框架对阿拉伯方言酒店评论进行情感分析，在数据稀缺情况下获得73%的F1分数，排名第12
...
Conclusion: 小样本学习在处理阿拉伯方言文本方面具有潜力，特别是在数据稀缺的专业领域
...
TL;DR: 诗歌形式的对抗性提示可作为通用单轮越狱技术，在25个前沿LLM中攻击成功率高达90%，比散文基线高18倍，揭示了当前安全机制的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究诗歌风格变化是否能绕过LLM的安全机制，探索当前对齐方法的根本局限性。

Method: 将1200个有害提示转换为诗歌形式，使用标准化元提示生成对抗性诗歌，通过集成开放权重评判模型和人工验证评估攻击成功率。

Result: 手工制作诗歌的平均越狱成功率为62%，元提示转换约为43%，显著优于非诗歌基线，显示跨模型家族和安全训练方法的系统性漏洞。

Conclusion: 仅通过风格变化就能规避当代安全机制，表明当前对齐方法和评估协议存在根本性限制。
...
TL;DR: HEAD-QA v2是一个扩展更新的西班牙语/英语医疗问答数据集，包含超过12,000道专业考试题目，用于评估LLM在医疗推理领域的能力。


<details>
  <summary>Details</summary>
Motivation: 响应高质量医疗推理数据集的需求，捕捉医疗领域的语言和概念复杂性。

Method: 扩展数据集至12,000+题目，涵盖10年西班牙专业考试；使用提示、RAG和概率选择方法对开源LLM进行基准测试；提供多语言版本支持未来研究。
...
TL;DR: 本文全面回顾了支撑大语言模型的核心技术，并探讨了LLMs在科学计量学领域的潜在应用，包括AI代理科学评估模型、新研究前沿检测和知识图谱构建方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自然语言理解、图像识别和多模态任务中展现出卓越能力，它们正朝着AGI方向发展，成为全球技术竞争的核心议题。本文旨在从用户角度系统梳理LLMs的核心技术，并探索其在科学计量学领域的应用前景。

Method: 采用综述方法，系统回顾了提示工程、知识增强检索增强生成、微调、预训练和工具学习等LLMs核心技术，同时追溯了科学计量学的发展历史，提出了前瞻性的应用视角。

Result: 提出了基于AI代理的科学评估模型，以及利用LLMs进行新研究前沿检测和知识图谱构建的新方法，为科学计量学领域提供了创新的技术路径。

Conclusion: LLMs在科学计量学领域具有广阔的应用前景，能够推动科学评估、研究前沿发现和知识组织等关键任务的智能化发展，为科学研究提供新的技术支撑。
...
TL;DR: 开发了一个合规的检索系统，通过LLM重排序和语义搜索技术，在航空维修环境中帮助技术人员快速查找手册内容，将查找时间从6-15分钟减少到18秒，同时保持监管合规性。


<details>
  <summary>Details</summary>
Motivation: 航空维修技术人员花费高达30%的工作时间查找手册，这是MRO操作中的效率瓶颈，而每个程序都必须追溯到认证来源，因此需要一种既能提高效率又能保持合规性的解决方案。

Method: 构建了一个兼容现有认证查看器的检索系统，使用ATA章节层次结构创建修订鲁棒的嵌入，并通过视觉语言解析结构化认证内容，结合LLM重排序和语义搜索技术。

Result: 在49k合成查询上实现>90%检索准确率；双语对照研究显示90.9%的top-10成功率，查找时间从6-15分钟减少到18秒，减少了95%的查找时间。

Conclusion: 语义检索技术可以在严格的监管约束下运行，并显著减少现实世界多语言MRO工作流程中的操作工作量。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在推理能力提升的同时，往往需要更长的思维链，这降低了实际场景中的交互效率，但缺乏对LLM代理效率的系统定义阻碍了针对性改进。

Method: 提出双效率定义（步骤级效率和轨迹级效率），并基于此开发DEPO方法，通过偏好优化联合奖励简洁响应和更少行动步骤。

Result: 在WebShop和BabyAI上的实验显示，DEPO减少令牌使用达60.9%，减少步骤达26.9%，同时性能提升达29.3%。在三个域外数学基准上也能泛化，且仅用25%数据训练仍保持效率增益。
...
TL;DR: 提出了NAMEGEn框架，通过多智能体优化解决中文起名这一短文本创意生成任务，能够满足个性化需求并提供有意义的解释。


<details>
  <summary>Details</summary>
Motivation: 解决创意自然语言生成中的两个主要挑战：(1)多目标灵活性 - LLM难以同时满足个性化、细粒度和多元化的用户需求；(2)解释复杂性 - 创意不仅需要生成，还需要理解和解释隐含意义以增强用户感知。

Method: 提出NAMEGEn多智能体优化框架，迭代交替进行目标提取、名字生成和评估，同时构建了17k+古典诗词语料库增强美学，并引入CBNames基准测试。

Result: 实验表明NAMEGEn能有效生成满足多样化个性化需求的创意名字，并提供有意义的解释，在六个基线方法中表现最佳，无需任何训练。
...
TL;DR: SPRING Lab开发的ASR系统，采用多解码器架构和音素通用标签集作为中间表示，在ASRU MADASR 2.0挑战赛中提升了8种语言33种方言的语言/方言识别性能，在Track 2中在3种语言上超越了基线WER/CER，并获得了最高的语言ID和方言ID准确率。


<details>
  <summary>Details</summary>
Motivation: 改进ASR系统在多语言多方言环境下的语言和方言识别能力，特别是在限制额外数据使用的条件下开发从头开始的多语言系统。

Method: 使用多解码器架构，以音素通用标签集作为中间表示进行训练，并探索了多种方法将音素空间的性能增益保留到对应的字素表示转换中。

Result: 在CLS空间中超越了基线性能，在Track 2中3种语言的WER/CER指标上超越了基线，并在所有参赛团队中获得了最高的语言ID和方言ID准确率。

Conclusion: 提出的多解码器架构和音素CLS中间表示方法有效提升了多语言ASR系统的性能，特别是在语言和方言识别任务上表现出色。
...
TL;DR: LLM-MemCluster是一个完全基于LLM的文本聚类框架，通过动态内存和双提示策略解决现有方法缺乏状态记忆和难以管理聚类粒度的问题，在多个基准数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在无监督学习中展现出强大的语义理解能力，但直接应用于文本聚类时存在两个根本限制：缺乏迭代优化的状态记忆，以及难以管理聚类粒度。现有方法通常依赖复杂的外部模块流水线，无法实现真正的端到端方法。

Method: 提出LLM-MemCluster框架，将聚类重新概念化为完全LLM原生的任务。该框架利用动态内存来注入状态感知能力，并通过双提示策略使模型能够推理并确定聚类数量。

Result: 在多个基准数据集上的评估表明，这个无需调优的框架显著且一致地优于强基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型发展迅速，但其智能、局限性和风险尚未充分理解，特别是在俄语环境下缺乏多模态基准测试。

Method: 创建基于指令的评估框架，包含18个全新构建的数据集，关注俄语文化和语言特性，采用统一提示词和度量标准，并包含防泄漏方法。

Result: 提供了闭源和开源模型的基线结果，建立了多模态能力的通用分类法。

Conclusion: 该基准为斯拉夫语系等类型多样语言构建多模态基准提供了可复现的方法论。
...
Method: 提出课程调优框架，从初级到高级训练模型；构建基于语言学的评估系统，考察语法覆盖度、写作错误、词汇句法复杂度和整体评分；微调HSKAgent模型。

Result: 实验结果表明HSKBenchmark能有效建模中文二语习得，并作为LLM动态写作评估的可靠基准。微调后的LLM写作表现与高级人类学习者相当，并展现类人习得特征。
...
TL;DR: 本文证明了即使在有限字母表（包括二进制和一元字母表）下，tokenization仍然是NP完全问题，且不存在多项式时间近似方案，解释了为什么BPE和UnigramLM等实用算法是启发式的。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设tokenization应用于无限大字母表，但实际中tokenizer在固定大小字母表（如字节或Unicode字符）上运行。本文旨在填补这一空白，分析有限n元字母表下的tokenization问题。

Method: 分析两种自然变体：自底向上tokenization和直接tokenization，分别考虑选择合并操作序列或词汇表来最优压缩数据集。证明对于n元字母表的硬度结果适用于任何更大尺寸的字母表。

Result: 即使在二进制字母表下，两种变体都是NP完全的，且不存在多项式时间近似方案（除非P=NP）。直接tokenization在一元字母表下仍然是NP完全的。

Conclusion: tokenization的计算难处理性不是大字母表或复杂构造的产物，而是基本障碍。这解释了为什么BPE和UnigramLM等实用算法是启发式的，并指出近似算法是tokenization研究的重要方向。
...
### [132] [Fluid Control with Localized Spacetime Windows](https://arxiv.org/abs/2511.15189)
*Yixin Chen,David I. W. Levin,Timothy R. Langlois*

Main category: cs.GR

TL;DR: 提出基于局部时空窗口的物理流体控制方法，通过CMA-ES优化时空窗口大小，在浮动背景网格上优化控制力，实现可扩展的流体控制。


<details>
  <summary>Details</summary>
Motivation: 传统基于力的时空控制在模拟尺度上难以处理，而最优控制力分布通常是局部化的，因此开发局部化方法可提高性能。

Method: 使用局部时空窗口限制控制区域，采用CMA-ES优化时间窗口大小，在浮动背景网格上优化控制力，与粒子方法无缝集成。

Result: 在多种2D和3D粒子自由表面模拟示例中证明了方法的有效性和效率。

Conclusion: 该方法能够处理之前难以处理的模拟尺度，通过局部化控制显著提高性能，边界条件自然融入目标函数中。
...
### [133] [One algebra for all : Geometric Algebra methods for neurosymbolic XR scene authoring, animation and neural rendering](https://arxiv.org/abs/2511.15398)
*Manos Kamarianakis,Antonis Protopsaltis,George Papagiannakis*

Main category: cs.GR

TL;DR: 几何代数(GA)在计算机图形学和扩展现实中具有变革性作用，能显著提升角色动画、渲染、绑定、神经渲染和生成式AI场景编辑的性能和精度。


<details>
  <summary>Details</summary>
Motivation: 传统表示方法(矩阵、四元数、向量)在旋转、平移和缩放等几何变换中存在精度和性能限制，需要更统一的数学框架。

Method: 使用几何代数将几何形式和变换封装为统一的代数表达式，保持多步变换中的关键几何特性，并作为神经符号XR场景创作的统一数学基础。

Result: GA方法能提高绑定角色动画的保真度、增强软体模拟、简化实时渲染，并优化神经和生成式AI场景编辑。

Conclusion: 几何代数提供了一个连贯高效的计算框架，在XR环境中能产生更优的视觉效果和计算效率。
...
### [134] [MHR: Momentum Human Rig](https://arxiv.org/abs/2511.15586)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够实现表达性、解剖学合理的人体动画的模型，同时支持非线性姿态校正，并适用于AR/VR和图形管线的稳健集成。

Method: 结合ATLAS的解耦骨架/形状范式和Momentum库的灵活现代绑定系统，包含姿态校正系统。

Result: 创建了MHR参数化人体模型，支持表达性、解剖学合理的人体动画，具有非线性姿态校正功能。

Conclusion: MHR模型为AR/VR和图形管线提供了稳健的人体动画解决方案，结合了解耦骨架设计和现代绑定系统的优势。
...
### [135] [The Illusion of Procedural Reasoning: Measuring Long-Horizon FSM Execution in LLMs](https://arxiv.org/abs/2511.14777)
*Mahdi Samiei,Mahdi Mansouri,Mahdieh Soleymani Baghshah*

Main category: cs.AI

TL;DR: 本文提出了有限状态机(FSM)执行作为评估LLMs程序推理能力的最小化、完全可解释框架，揭示了LLMs在多步骤推理中的系统性退化问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在推理任务上表现出色，但其执行多步骤、基于规则的计算的程序推理能力尚不明确，需要可控、可解释的基准来隔离和测量这种能力崩溃。

Method: 使用有限状态机执行框架，给模型明确的FSM定义，要求其根据输入动作逐步执行，在多个回合中保持状态一致性，测量回合准确率和任务准确率。

Result: 实验结果显示随着任务范围或分支复杂度的增加，模型性能系统性下降；在规则检索涉及高分支因子时表现更差；大模型局部准确率提高但在多步骤推理中仍然脆弱，除非明确提示外部化中间步骤。
...
### [136] [Learning Interestingness in Automated Mathematical Theory Formation](https://arxiv.org/abs/2511.14778)
*George Tsoukalas,Rahul Saha,Amitayush Thakur,Sabrina Reguyal,Swarat Chaudhuri*

Main category: cs.AI

TL;DR: FERMAT是一个强化学习环境，用于自动化数学理论发现，通过进化算法和LLM合成数学对象的趣味性度量，在初等数论和有限域方面表现优于硬编码基线。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中开放式的数学理论发现这一重大挑战，通过构建符号化概念发现和定理证明的环境来探索理论发现相关的强化学习问题。

Method: 引入FERMAT强化学习环境，使用基于LLM的进化算法合成数学对象的趣味性度量，特别采用了函数抽象技术。

Result: 在初等数论和有限域领域，该方法相比硬编码基线取得了显著改进，能够发现更有趣的数学对象。

Conclusion: FERMAT环境为数学理论发现提供了有效的自动化工具，基于LLM的进化算法在数学对象趣味性度量方面表现出色，推动了数学发现自动化的发展。
...
### [137] [Ask WhAI:Probing Belief Formation in Role-Primed LLM Agents](https://arxiv.org/abs/2511.14780)
*Keith Moore,Jun W. Kim,David Lyu,Jeffrey Heo,Ehsan Adeli*

Main category: cs.AI

TL;DR: Ask WhAI是一个用于检查和扰动多智能体交互中信念状态的系统级框架，通过记录回放交互、查询信念理由和注入反事实证据来测试信念结构对新信息的响应。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体科学推理中的信念形成和认知孤岛，提供一种可重现的方法来观察和测试这些动态过程。

Method: 使用医疗案例模拟器，配备多智能体共享内存（时间戳电子病历）和持有真实实验室结果的预言机智能体。让具有特定角色先验的LLM智能体在顺序或并行交互中写入共享医疗记录。

Result: 智能体信念常反映现实世界学科立场，包括过度依赖典型研究和抵制反证，这些信念可以通过框架进行追踪和询问。
...
### [138] [Subnational Geocoding of Global Disasters Using Large Language Models](https://arxiv.org/abs/2511.14788)
*Michele Ronco,Damien Delforge,Wiebke S. Jäger,Christina Corbane*

Main category: cs.AI

TL;DR: 提出了一种基于LLM的自动化工作流，用于处理灾害数据库中的非结构化位置信息，通过GPT-4o清理文本并使用三个地理信息库进行交叉验证，为每个位置分配几何形状和可靠性评分。


<details>
  <summary>Details</summary>
Motivation: 灾害数据库如EM-DAT中的位置信息通常以非结构化文本形式存在，具有不一致的粒度和拼写，难以与空间数据集集成，这限制了风险评估和灾害风险减少的能力。

Method: 开发了完全自动化的LLM辅助工作流，使用GPT-4o处理清理文本位置信息，并通过交叉检查GADM、OpenStreetMap和Wikidata三个独立的地理信息库来分配几何形状。基于这些源的一致性和可用性，为每个位置分配可靠性评分。

Result: 应用于2000-2024年的EM-DAT数据集，该工作流成功地理编码了14,215个事件，涉及17,948个独特位置。

Conclusion: 该方法无需人工干预，覆盖所有灾害类型，支持跨多个源的交叉验证，并允许灵活重映射到首选框架。展示了LLM从非结构化文本中提取和构建地理信息的潜力，为相关分析提供了可扩展且可靠的方法。
...
### [139] [Project Rachel: Can an AI Become a Scholarly Author?](https://arxiv.org/abs/2511.14819)
*Martin Monperrus,Benoit Baudry,Clément Vidal*

Main category: cs.AI

TL;DR: 通过创建AI学术身份Rachel So并发表10+篇AI生成论文，研究学术生态系统对AI作者身份的反应，包括获得引用和同行评审邀请。
...
Result: Rachel So的论文获得引用，并收到同行评审邀请，表明AI作者身份在学术生态系统中被接受。

Conclusion: AI作者身份对出版商、研究人员和整个科学体系具有重要影响，需要就AI参与学术交流的未来进行必要讨论。
...
### [140] [Uncertainty-Aware Measurement of Scenario Suite Representativeness for Autonomous Systems](https://arxiv.org/abs/2511.14853)
*Robab Aghazadeh Chakherlou,Siddartha Khastgir,Xingyu Zhao,Jerein Jeyachandran,Shufeng Chen*

Main category: cs.AI

TL;DR: 提出一种概率方法来量化AI系统训练测试数据集的代表性，通过比较场景套件与目标操作域的特征分布，使用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值代表性估计。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统（如自动驾驶汽车）的可信性和安全性，关键在于训练测试数据集的代表性，即数据反映系统设计运行条件（ODD）或预期遇到条件（TOD）的程度。

Method: 使用概率方法比较场景套件特征分布与TOD特征分布，应用不精确贝叶斯方法处理有限数据和不确定先验，生成区间值、不确定性感知的代表性估计。

Result: 通过数值示例展示了在依赖性和先验不确定性下，跨操作类别（天气、道路类型、时间等）的场景套件与推断TOD分布比较，估计了局部和全局代表性区间。

Conclusion: 该方法能够量化数据集的代表性，并考虑不确定性，为AI系统的安全评估提供更可靠的数据质量度量。
...
### [141] [Task Specific Sharpness Aware O-RAN Resource Management using Multi Agent Reinforcement Learning](https://arxiv.org/abs/2511.15002)
...
<details>
  <summary>Details</summary>
Motivation: 下一代网络采用O-RAN架构实现动态资源管理，但深度强化学习模型在动态环境中面临鲁棒性和泛化性不足的问题。

Method: 在分布式MARL框架中增强SAC算法，引入基于TD误差方差的自适应选择性SAM机制，仅对面临高环境复杂度的智能体进行正则化，并采用动态ρ调度方案优化探索-利用权衡。

Result: 实验结果显示该方法显著优于传统DRL方法，资源分配效率提升达22%，并在多样化O-RAN切片中确保优越的QoS满意度。

Conclusion: 所提出的自适应SAM机制能有效提升DRL在动态网络环境中的鲁棒性和泛化性，同时保持学习效率，为O-RAN资源管理提供了高效解决方案。
...
### [142] [Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization](https://arxiv.org/abs/2511.15055)
*Jian-Ting Guo,Yu-Cheng Chen,Ping-Chun Hsieh,Kuo-Hao Ho,Po-Wei Huang,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: MAQ是一个人类化强化学习框架，通过向量量化VAE从人类演示中提取宏观动作，在保持高奖励的同时使智能体行为更接近人类。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习智能体虽然在很多领域表现超人类，但行为往往不自然，与人类行为差异大，这影响了可解释性和可信度。

Method: 将人类化建模为轨迹优化问题，采用后退时域控制，通过向量量化VAE从人类演示中提取宏观动作。

Result: 在D4RL Adroit基准测试中，MAQ显著提升了人类化程度，提高了轨迹相似度得分，在人类评估研究中获得最高的人类化排名。
...
### [143] [Beyond GeneGPT: A Multi-Agent Architecture with Open-Source LLMs for Enhanced Genomic Question Answering](https://arxiv.org/abs/2511.15061)
...
<details>
  <summary>Details</summary>
Motivation: 解决GeneGPT依赖专有模型导致的扩展性、成本和隐私问题，探索开源模型在基因组问答中的应用潜力。

Method: 采用模块化多智能体架构，包含工具路由、查询生成和响应验证等专门智能体，使用Llama 3.1、Qwen2.5等开源模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决动作和变化推理问题，利用LLMs的能力来处理复杂的推理任务。

Method: 提取RAC问题中的动作和问题元素，逐步执行每个动作推导最终状态，然后基于进展状态评估查询。
...
TL;DR: Rogue One是一个基于LLM的多智能体框架，通过三个专业智能体（科学家、提取器、测试器）的协作，结合外部领域知识和丰富的定性反馈机制，实现自动特征提取，在多个数据集上显著优于现有方法。
...
Conclusion: Rogue One不仅提升了特征提取的性能，还增强了特征的可解释性和语义意义，证明了其作为科学发现工具的价值。
...
TL;DR: SafeRBench是首个评估大型推理模型端到端安全性的基准，从输入、中间推理到最终输出进行全面评估，包含风险分类分级、细粒度输出分析和人类安全对齐验证。
...
Method: 1) 输入特征化：将风险类别和等级纳入输入设计，考虑受影响群体和严重程度；2) 细粒度输出分析：通过微思维分块机制将长推理轨迹分割成语义连贯单元；3) 人类安全对齐：用专门设计的人类标注验证基于LLM的评估。

Result: 在19个大型推理模型上的评估表明，SafeRBench能够进行详细的多维度安全评估，从多个角度提供风险和保护机制的见解。

Conclusion: SafeRBench填补了现有安全评估的空白，为大型推理模型提供了全面的端到端安全评估框架，能够捕捉推理过程中的动态安全风险。
...
### [147] [HISE-KT: Synergizing Heterogeneous Information Networks and LLMs for Explainable Knowledge Tracing with Meta-Path Optimization](https://arxiv.org/abs/2511.15191)
*Zhiyi Duan,Zixing Shi,Hongyu Yuan,Qi Wang*

Main category: cs.AI

TL;DR: HISE-KT是一个结合异构信息网络和大型语言模型的知识追踪框架，通过智能筛选元路径实例和检索相似学生轨迹，实现更准确且可解释的预测。


<details>
  <summary>Details</summary>
Motivation: 解决现有异构信息网络方法因手动或随机选择元路径而引入噪声，以及LLM方法忽略学生间丰富信息的问题，同时提升预测准确性和可解释性。

Method: 构建多关系异构信息网络，使用LLM智能评分和筛选元路径实例，设计基于元路径的相似学生检索机制，并通过结构化提示整合目标学生历史和相似轨迹。

Result: 在四个公共数据集上的实验表明，HISE-KT在预测性能和可解释性方面均优于现有知识追踪基线方法。

Conclusion: HISE-KT通过HIN与LLM的协同集成，成功实现了自动化元路径质量评估和证据支持的可解释分析，为知识追踪提供了更准确和可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: LLM在大量数据集上训练，其中可能包含受版权保护的内容，这引发了未经授权使用的担忧。现有的成员推理攻击方法存在局限性，如LLM的过度自信、缺乏真实训练数据和依赖经验阈值等问题。

Method: 采用两阶段策略：(1) 将文件战略性地分割成小片段以减少对大规模训练数据的依赖；(2) 基于不确定性的无监督聚类，无需经验调优的阈值。

Result: 在LLaMA 7b上平均平衡准确率达到90.1%，在LLaMA2 7b上达到91.6%，相比现有最佳方法有超过90%的相对改进，最高达到93.8%的平衡准确率。在GPT-J 6B上也表现出良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时利用数学优化的精确性和LLMs的上下文理解能力，需要一种能够协同两者优势的智能决策框架。

Method: 提出SOLID框架，通过优化代理和LLM代理的迭代协作，使用对偶价格和偏差惩罚机制进行交互，在凸性假设下保持理论收敛性。

Result: 在股票投资组合案例中，SOLID在各种场景下都表现出收敛性，年化收益率相比仅使用优化的基准方法有所提升。

Conclusion: SOLID为跨领域自动化和智能决策提供了一个有前景的框架，验证了优化与LLM协同的有效性。
...
### [150] [Efficiency Will Not Lead to Sustainable Reasoning AI](https://arxiv.org/abs/2511.15259)
*Philipp Wiesner,Daniel W. O'Neill,Francesca Larosa,Odej Kao*

Main category: cs.AI

TL;DR: 本文认为，随着AI研究转向复杂问题解决和推理，仅靠效率提升无法实现可持续性，需要将明确限制嵌入推理AI的优化和治理中。


<details>
  <summary>Details</summary>
Motivation: AI研究正从模式识别转向多步推理，而计算效率改进正接近物理极限，推理AI缺乏需求饱和点，性能随计算投入指数增长，这带来了可持续性挑战。

Method: 通过分析推理AI与传统计算在效率饱和和需求限制方面的差异，论证效率提升的局限性，并提出研究政策方向。

Result: 识别出推理AI缺乏自然饱和机制，性能持续随计算投入指数扩展，仅靠效率改进无法解决可持续性问题。

Conclusion: 需要在推理AI的优化和治理中嵌入明确限制，而不仅仅是依赖效率提升，以实现可持续发展。
...
### [151] [Realist and Pluralist Conceptions of Intelligence and Their Implications on AI Research](https://arxiv.org/abs/2511.15282)
*Ninell Oldenburg,Ruchira Dhar,Anders Søgaard*

Main category: cs.AI

TL;DR: 本文分析了AI研究中两种对立的智力观：智力现实主义认为智力是单一可测量的普遍能力，而智力多元主义认为智力是多样化的、情境依赖的能力。这两种观点在方法论、解释和风险评估方面产生根本不同的研究路径。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究中的许多分歧源于对智力本质的隐含不同理解。通过明确这两种对立的智力观，可以帮助澄清AI研究中的争议和误解。

Method: 通过分析当前AI研究中的辩论，揭示智力现实主义和智力多元主义如何影响实证证据的解释，并展示它们在三个关键领域产生的不同研究路径。

Result: 发现这两种智力观在方法论上产生不同的模型选择、基准设计和实验验证方法；在解释上导致对相同经验现象的矛盾解读；在风险评估上产生完全不同的威胁评估和解决方案。

Conclusion: 明确这些基本假设有助于更清晰地理解AI研究中的分歧，促进更富有成效的科学讨论，并推动对AI发展路径和风险的更全面评估。
...
TL;DR: Octopus提出了一种新的多模态推理范式，通过六种核心能力的编排实现自主探索和动态能力选择，在Octopus-Bench基准测试中表现出色。
...
Method: 定义了六种核心多模态推理能力，构建了Octopus-Bench评估基准，Octopus能够自主探索推理过程并基于当前状态动态选择最合适的能力。

Result: 实验结果显示Octopus在Octopus-Bench绝大多数任务中取得了最佳性能。

Conclusion: 能力协调在智能多模态推理中起着关键作用，Octopus证明了这种新范式的有效性。
...
### [153] [Terra Nova: A Comprehensive Challenge Environment for Intelligent Agents](https://arxiv.org/abs/2511.15378)
*Trevor McInroe*

Main category: cs.AI

TL;DR: Terra Nova是一个基于《文明V》的综合性挑战环境，旨在同时测试智能体在部分可观测性、信用分配、表示学习、巨大动作空间等多个RL挑战下的综合推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多任务基准主要评估智能体在不同独立任务间切换策略的能力，而非测试智能体在多个相互关联挑战中进行深度推理的能力。需要创建一个能同时呈现多种经典RL挑战的单一环境。

Method: 基于《文明V》游戏设计一个综合性挑战环境，在该环境中多个RL挑战（部分可观测性、信用分配、表示学习、巨大动作空间等）同时出现并相互影响。

Result: 开发了Terra Nova环境，该环境要求智能体具备跨多个交互变量的整合性、长视野理解能力。

Conclusion: Terra Nova为评估智能体在复杂、交互式环境中的深度推理能力提供了新的基准，区别于仅聚合无关任务的多任务基准。
...
### [154] [IPR-1: Interactive Physical Reasoner](https://arxiv.org/abs/2511.15407)
*Mingyu Zhang,Lifeng Zhuo,Tianxi Tan,Guocan Xie,Xian Nie,Yan Li,Renjie Zhao,Zizhu He,Ziyu Wang,Jiting Cai,Yong-Lu Li*

Main category: cs.AI

TL;DR: IPR通过世界模型推演和物理中心动作编码，在1000+游戏中训练，实现了稳健的物理推理能力，在好奇度方面超越GPT-5，并能零样本迁移到未见游戏。


<details>
  <summary>Details</summary>
Motivation: 研究智能体是否能像人类一样通过交互获取物理和因果推理能力，并随着经验积累持续改进。

Method: 提出IPR（交互物理推理器），使用世界模型推演来评分和强化VLM策略，并引入PhysCode物理中心动作编码来对齐语义意图与动力学。

Result: 在1000+游戏上预训练后，IPR在三个推理层级表现稳健，整体匹配GPT-5，在好奇度方面超越GPT-5，性能随训练游戏和交互步骤增加而提升，并能零样本迁移到未见游戏。
...
Method: TIM框架包含：DeFi意图分类法、元级规划器动态协调领域专家分解任务、问题求解器处理多模态链上/链下数据、认知评估器减轻LLM幻觉。

Result: 实验表明TIM显著优于机器学习模型、单一LLM和单一智能体基线，能够提供更可靠的用户动机理解。

Conclusion: 该工作为理解DeFi中用户意图提供了可靠框架，为复杂区块链活动提供情境感知解释，解决了意图推断中的核心挑战。
...
### [156] [Exploring the use of AI authors and reviewers at Agents4Science](https://arxiv.org/abs/2511.15534)
*Federico Bianchi,Owen Queen,Nitya Thakkar,Eric Sun,James Zou*

Main category: cs.AI

TL;DR: Agents4Science是首个由AI代理作为主要作者和审稿人、人类作为共同作者和共同审稿人的会议，探讨了AI代理在科学研究中的能力和人机协作的意义。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理在科学研究中的应用日益增长，但其作为科学家和审稿人的基本能力仍存在疑问，需要探索AI代理在科学研究和评审中的实际表现。

Method: 组织Agents4Science会议，让AI代理担任主要作者和审稿人，人类作为共同作者和共同审稿人，通过实际会议运作来评估AI代理的能力。

Result: 会议成功举办，获得了关于AI代理在科学研究和评审中表现的关键见解。

Conclusion: Agents4Science会议为理解AI代理在科学研究和评审中的能力提供了重要经验，对人机协作在科学领域的未来发展具有启示意义。
...
### [157] [What Does It Take to Be a Good AI Research Agent? Studying the Role of Ideation Diversity](https://arxiv.org/abs/2511.15593)
...
TL;DR: 本文研究了AI研究代理中构思多样性对性能的影响，发现更高的构思多样性会带来更好的表现，并通过不同模型、代理框架和评估指标验证了这一发现。


<details>
  <summary>Details</summary>
Motivation: AI研究代理有望通过自动化机器学习模型的设计、实现和训练来加速科学进展，但该领域仍处于早期阶段，成功或失败的关键因素尚未完全理解。本文旨在探究构思多样性在代理性能中的作用。

Method: 首先分析MLE-bench基准上不同模型和代理框架的代理轨迹，然后通过控制实验修改构思多样性程度，最后使用除标准奖牌评分外的其他评估指标来加强结果。

Result: 分析显示不同模型和代理框架产生不同程度的构思多样性，性能更高的代理往往具有更高的构思多样性。控制实验证明更高的构思多样性确实导致更强的性能，这一发现在其他评估指标下仍然成立。

Conclusion: 构思多样性是影响AI研究代理性能的关键因素，更高的构思多样性能够显著提升代理的表现，这一结论在不同模型、框架和评估指标下都具有稳健性。
