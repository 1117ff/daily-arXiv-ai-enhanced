...
TL;DR: TTM是一个无需训练、即插即用的视频生成框架，通过粗参考动画实现精确的运动和外观控制，在保持自然动态的同时平衡用户意图的保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于图像和文本的扩散视频生成方法无法提供精确的运动控制，而现有的运动条件合成方法通常需要模型特定的微调，计算成本高且限制性强。

Method: 使用通过用户友好操作（如剪切拖拽或基于深度的重投影）获得的粗参考动画作为运动线索，结合图像条件保持外观，并引入双时钟去噪策略在运动指定区域强制强对齐，同时在其它区域保持灵活性。

Result: 在物体和相机运动基准测试中，TTM在真实感和运动控制方面匹配或超越了现有的基于训练的基线方法。

Conclusion: TTM提供了一种轻量级的采样过程修改，无需额外训练或运行时成本，兼容任何骨干网络，并引入了通过像素级条件实现的精确外观控制能力，超越了仅文本提示的限制。
...
TL;DR: 提出了OUGS框架，通过基于3D高斯原语物理参数的显式不确定性建模，结合语义分割实现对象感知的主动视图选择，显著提升了3D高斯泼溅中目标对象的重建效率和质量。


<details>
  <summary>Details</summary>
Motivation: 现有主动重建方法依赖场景级不确定性度量，这些度量往往受到无关背景干扰，导致对象中心任务中的视图选择效率低下。
...
Conclusion: OUGS框架提供了一种更原则性的不确定性建模方法，不仅提升了对象重建效率和质量，还能作为全局场景的鲁棒不确定性估计器。
...
TL;DR: 本文提出了一种基于生成式人工智能的解决方案，用于自动化SNAM能源基础设施的工厂结构数字化，通过OCR、视觉LLM、目标检测、关系推理和优化算法等技术，实现了91%的文本信息提取准确率和93%的组件识别准确率。


<details>
  <summary>Details</summary>
Motivation: 能源转型需要数字化支持，传统工厂数字化过程依赖人工处理文档，效率低下且易出错。本文旨在开发AI解决方案来自动化工厂信息提取，提高MGM用户的工作效率。

Method: 采用OCR、视觉LLM、目标检测、关系推理和优化算法的组合方法，并创新性地引入Transformer架构来增强场景图生成，深入分析工厂组件间的复杂关系。

Result: 文本设计数据提取准确率达到91%，组件识别准确率93%，工厂层次结构提取准确率约80%。该方案成功克服了数据多样性带来的标准化缺失问题。

Conclusion: 基于多种AI技术的协同使用能够有效自动化工厂数字化过程，显著提高信息提取的准确性和效率，为能源基础设施的数字化转型提供了可行的技术路径。
...
TL;DR: 提出了一种系统评估方法，用于分析和量化基于修复的说话人脸生成中的嘴唇泄漏问题，包括三种测试设置和衍生指标。


<details>
  <summary>Details</summary>
Motivation: 基于修复的说话人脸生成方法在修改嘴唇运动时可能引入嘴唇泄漏问题，即生成的嘴唇受到参考图像影响而非仅由驱动音频决定，这一问题难以通过标准指标检测。

Method: 采用三种互补测试设置：静音输入生成、不匹配的音频-视频配对和匹配的音频-视频合成，并引入嘴唇同步差异和基于静音音频的嘴唇同步评分等衍生指标。
...
TL;DR: 提出了MATRIX数据集和深度学习框架，用于解决多无人机动态监控中行人跟踪的挑战，在复杂遮挡环境下保持约90%的检测和跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 现有多无人机监控方法难以应对动态相机位置和复杂遮挡问题，需要更全面的数据集和算法框架来提升动态多视角监控能力。

Method: 开发了包含8个动态位置无人机的MATRIX数据集，提出了实时相机标定、基于特征的图像配准和多视角特征融合的深度学习框架，采用鸟瞰图表示。

Result: 在复杂环境下保持约90%的检测和跟踪精度，成功跟踪约80%的轨迹，迁移学习显示强泛化能力，相机丢失实验显示性能优雅降级。

Conclusion: MATRIX数据集和框架为推进动态多视角监控系统提供了重要基准，在真实部署中具有实用鲁棒性。
...
Method: 1) 基于Kolmogorov-Arnold表示定理的自适应多子空间建模机制，通过拓扑收敛分析动态选择和加权任务相关子空间；2) 多子空间交互块，通过流形上的Fréchet平均优化融合异构几何表示；3) 引入黎曼批量归一化和互信息正则化

Result: 在3D动作识别(HDM05, FPHA)、EEG分类(MAMEM-SSVEPII)和图任务上达到最先进性能
...
TL;DR: 提出了一种新的持续异常检测框架，使用统一内存库而非任务特定内存库，通过固定大小核心集增量更新嵌入，在MVTec AD和Visa数据集上达到最先进检测精度。
...
Conclusion: 该方法在保持高检测精度的同时解决了内存碎片化问题，在实际场景中表现出强大鲁棒性，代码将在GitHub开源。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中事故预测至关重要，但面临两个关键挑战：传感器输入噪声或退化，以及需要平衡及时预警与误报抑制。

Method: 使用扩散模块通过迭代细化重建噪声弹性特征，同时采用actor-critic架构进行长期时序推理和时间加权奖励，确定最佳预警时机。

Result: 在三个基准数据集上达到最先进精度，显著提高平均事故前时间，在噪声条件下保持鲁棒性能。

Conclusion: 该模型能产生更早、更稳定且符合人类判断的预测，在常规和复杂交通场景中均表现优异，具有实际部署潜力。
...
TL;DR: 提出了关系评分网络(RS-Net)，通过空间交互和长程时间上下文来评估对象对的重要性，可轻松集成到现有动态场景图生成模型中，显著提升关系预测性能。
...
Method: RS-Net包含空间上下文编码器（带可学习上下文标记）和时间编码器（聚合视频级信息），通过统一的三元组评分机制增强关系预测。

Result: 在Action Genome数据集上的实验表明，RS-Net在各种基线方法上持续提升Recall和Precision，在mean Recall上有显著提升，有效处理关系长尾分布问题。

Conclusion: RS-Net在保持竞争性效率的同时，实现了优于最先进方法的性能，可轻松集成到现有DSGG模型中而不需要架构改变。
...
TL;DR: 提出了一种在潜在空间中保护视频基础模型隐私的新方法，通过轻量级匿名化适配器模块从视频特征中移除私人信息，同时保持任务实用性。
...
Result: 在多个下游任务评估中，隐私泄露显著减少35%，同时保持接近基线的实用性性能，并能有效缓解动作识别模型中的性别偏见。
...
TL;DR: 本文研究了自回归逐像素预测的扩展特性，发现在固定32x32分辨率下，图像分类和图像生成的最优扩展策略存在分歧，生成任务需要数据量比分类任务快3-5倍增长。随着分辨率增加，模型规模需要比数据量增长更快，计算能力是主要瓶颈而非训练数据量。
...
Result: 1) 在固定32x32分辨率下，图像分类和图像生成的最优扩展策略不同，生成任务需要数据量比分类任务快3-5倍增长；2) 随着分辨率增加，模型规模需要比数据量增长更快；3) 主要瓶颈是计算能力而非训练数据量。

Conclusion: 通过预测发现，随着计算能力每年增长4-5倍，未来5年内逐像素图像建模是可行的，计算能力而非数据量是主要限制因素。
...
TL;DR: 该研究探索了多种扩散模型微调技术（如LoRA和DreamBooth）来生成更准确的平衡训练数据，通过聚类和分组训练解决组内变异问题，在多个基准测试中表现优于原始Stable Diffusion，并与SOTA去偏技术相当。
...
Method: 采用LoRA和DreamBooth等扩散模型微调技术，通过直接从样本学习生成更准确的群体表示；为防止单个DreamBooth模型被组内变异淹没，对每个群体内的图像进行聚类并为每个聚类训练单独的DreamBooth模型；使用这些模型生成群体平衡数据进行预训练，然后在真实数据上微调。

Result: 在多个基准测试中，研究的微调方法平均表现优于原始Stable Diffusion，与SOTA去偏技术（如Group-DRO）结果相当，且随着数据集偏见严重程度增加而超越这些方法。

Conclusion: 扩散模型微调技术能有效生成平衡的训练数据，缓解分类系统中的偏见问题，特别是在偏见严重的数据集上表现优异。
...
TL;DR: WiCV@CVPR 2025是第16届计算机视觉领域女性研讨会，旨在提升女性和少数群体在计算机视觉社区的可见度、包容性和专业发展。
...
Method: 通过研讨会程序、参与统计、导师计划成果和历史趋势分析来评估研讨会的影响。

Result: 研讨会收到32篇全文投稿接受14篇，62篇短文投稿接受36篇扩展摘要海报；导师计划匹配80名学员与37名导师；吸引100多名现场参与者；获得10个赞助商和约44,000美元旅行资助。

Conclusion: WiCV@CVPR 2025成功延续了其使命，通过技术交流、网络建设和资金支持，赋能新兴研究人员并放大计算机视觉领域的多样化声音。
...
TL;DR: 提出了PoseKAN，一种基于图结构的Kolmogorov-Arnold网络，用于从单张图像进行2D到3D姿态提升，通过可学习的边激活函数和多跳特征聚合来解决GCN的局部感受野限制和频谱偏差问题。


<details>
  <summary>Details</summary>
Motivation: 传统GCN方法在3D人体姿态估计中存在局部感受野限制，难以捕捉长距离依赖关系来处理遮挡和深度模糊问题，同时存在频谱偏差，优先学习低频分量而难以建模高频细节。

Method: 使用自适应图KAN框架，在图的边上采用可学习函数进行数据驱动的自适应特征变换，结合多跳特征聚合、残差PoseKAN块和全局响应归一化来增强空间感知和特征选择性。

Result: 在基准数据集上的广泛实验表明，该模型在性能上与最先进方法具有竞争力。

Conclusion: PoseKAN通过可学习的边激活函数和多跳特征聚合机制，有效提升了3D人体姿态估计的性能，解决了GCN方法的局限性。
...
TL;DR: 提出了SIFT-Graph多模态防御框架，通过结合手工特征和深度学习特征来增强视觉模型对抗对抗攻击的鲁棒性
...
Method: 整合SIFT关键点和图注意力网络，提取尺度旋转不变的局部结构特征，然后与ViT和CNN等传统视觉模型融合

Result: 初步结果显示该方法能有效提升视觉模型对基于梯度的白盒对抗攻击的鲁棒性，且对干净准确率影响很小
...
<details>
  <summary>Details</summary>
Motivation: 现有方法局限于小相机移动或非自然物体中心场景，限制了真实世界应用。本文旨在解决从单张图像生成自然场景新视图这一未被充分探索的问题。

Method: 使用基于transformer的3D感知扩散模型，采用图像损失在大规模真实世界视频数据集上训练，提出创新的相机条件策略和训练范式交换参考帧角色。

Result: 在单图像广义新视角合成任务上，相比最先进的3D感知扩散模型和确定性方法有所改进，并能生成多样化输出。

Conclusion: DT-NVS在真实世界自然场景的新视角合成方面取得了显著进展，为实际应用提供了更好的解决方案。
...
TL;DR: 提出了一种新的旋转不变3D点云学习方法，通过引入阴影信息姿态特征(SiPF)和旋转不变注意力卷积(RIAttnConv)，解决了现有方法因缺乏全局姿态信息而无法区分几何相似但空间不同结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有旋转不变学习方法使用手工制作的RI特征替换原始坐标，虽然保证了旋转不变性，但丢失了全局姿态信息，无法区分几何相似但空间不同的结构(如飞机左右机翼)。这种限制源于受限的感受野导致的"翼尖特征塌陷"。
...
Conclusion: 通过结合局部旋转不变性和全局姿态感知，该方法有效解决了旋转不变学习中的对称结构区分问题，提升了在任意旋转下对几何相似但空间不同结构的识别能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于SSM的方法通过手动设计的扫描操作将2D姿态序列展平为纯时间序列，破坏了人体姿态的固有空间结构，使空间和时间特征纠缠，难以捕捉复杂的姿态依赖关系。

Method: 提出SAS-SSM：1）使用结构感知时空卷积动态捕获关节间的局部交互；2）采用跨步扫描策略构建多尺度全局结构表示；3）在SAS-SSM基础上构建SasMamba模型。

Result: SasMamba在3D姿态估计任务中取得了有竞争力的性能，且参数量显著少于现有混合模型。

Conclusion: SAS-SSM能够灵活建模局部和全局姿态信息，同时保持线性计算复杂度，为3D人体姿态估计提供了一种有效的解决方案。
...
TL;DR: 提出了一种基于Vision Transformers的多重融合增强ViT块(MFAVBs)用于图像聚类，通过显式融合正样本对的特征来提升聚类性能。
...
Method: 设计MFAVBs：1)两个预处理增强作为正样本对输入共享权重的ViTs；2)输出特征融合后输入更大的ViT；3)学习特征分割为新增强正样本对传递给后续FAVBs；4)特征投影到实例级和聚类级空间计算交叉熵损失。
...
<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤预后极差，需要自动、稳健且准确的组织病理学区域识别方法来大规模理解疾病形态学特征。

Method: 采用四步深度学习策略，使用EfficientNet架构（B0-B4）在BraTS-Path 2024挑战数据集上对6个组织病理学区域进行分类。

Result: EfficientNet-B1和B4在5折交叉验证中表现最佳（F1=0.98），但在验证集和测试集上性能显著下降（F1=0.546和0.517）。

Conclusion: 虽然模型在训练数据上表现优异，但在新数据上的泛化能力有限，这对临床应用至关重要，需要进一步改进模型泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 人类视觉系统在物体识别方面表现出色，但人工系统难以复制这种能力。研究旨在开发更高效、生物启发的人工视觉识别模型。
...
TL;DR: 本文提出了一种非对称跨模态知识蒸馏方法(ACKD)，通过SemBridge框架解决弱语义一致性下模态间知识传递的挑战，在遥感场景分类任务中取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中配对的模态数据有限，对称跨模态知识蒸馏(SCKD)应用受限。需要研究弱语义一致性下的知识学习方法来桥接语义重叠有限的模态。

Method: 提出SemBridge框架，包含学生友好匹配模块（利用自监督学习获取语义知识，动态选择相关教师样本）和语义感知知识对齐模块（采用拉格朗日优化寻找最优传输路径）。

Result: 在6种不同模型架构和多个数据集上的实验表明，该方法相比7种现有方法达到了最先进的性能。

Conclusion: ACKD方法能够有效处理弱语义一致性下的跨模态知识蒸馏问题，SemBridge框架通过动态样本选择和最优传输路径优化显著提升了知识传递效率。
...
TL;DR: 提出一个融合视觉检测和LLM结构先验的半监督文档布局理解框架，通过概率加权提升伪标签质量，在轻量级和预训练模型上都取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 文档布局理解仍然需要大量标注数据，现有半监督方法效果有限。LLM提供的结构先验信息可以与视觉检测互补，提升少样本场景下的性能。

Method: 使用OCR-LLM流水线推断文档层次区域，通过逆方差融合将教师检测器输出与LLM结构先验结合，生成精炼的伪标签。引入实例自适应门控机制优化融合权重。

Result: 在PubLayNet数据集上，使用5%标签：轻量级SwiftFormer达到88.2±0.3 AP；LayoutLMv3达到89.7±0.4 AP，超越标准半监督学习(89.1±0.4 AP)并匹配需要大量预训练的UDOP(89.8 AP)。

Conclusion: LLM结构先验与视觉检测器互补，能显著提升半监督文档布局理解性能，支持隐私保护部署，并提供语义消歧能力，系统成本可控。
...
TL;DR: 提出了一种新的无监督遥感变化检测框架CCDF，通过循环一致性和语义一致性模块解决生成器过拟合问题，在无标签数据下实现更好的变化检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的无监督方法通过生成器网络进行风格转换和重建来识别变化区域，但存在生成器过拟合导致性能不佳的问题。

Method: 提出CCDF框架，包含循环一致性(CC)模块减少生成器过拟合，以及语义一致性(SC)模块实现细节重建。

Result: 大量实验表明该方法优于其他最先进的方法。

Conclusion: CCDF框架通过一致性模块有效解决了无监督遥感变化检测中的生成器过拟合问题，提升了检测性能。
...
TL;DR: HitoMi-Cam是一种基于光谱反射特性的轻量级、形状无关的人员检测方法，在边缘设备上实现实时处理，在形状不可预测的场景中表现优于CNN模型。


<details>
  <summary>Details</summary>
Motivation: 解决CNN目标检测的形状依赖性问题，特别是在训练数据中未包含的姿势下性能下降的问题，为灾难救援等形状不可预测场景提供可靠检测方案。

Method: 利用衣物光谱反射特性，在无GPU的资源受限边缘设备上实现HitoMi-Cam系统，采用光谱基础方法进行人员检测。

Result: 处理速度达到23.2 fps (253x190像素)，在模拟搜救场景中平均精度达93.5%，远超CNN模型的最佳AP 53.8%，误报率极低。

Conclusion: HitoMi-Cam不是CNN检测器的替代品，而是在特定条件下（如形状不可预测的灾难救援环境）的补充工具，证明了基于光谱的人员检测在边缘设备实时运行的可行性。
...
TL;DR: 提出负实体抑制(NES)方法解决零样本图像描述中的幻觉问题，通过合成图像、过滤负实体和注意力抑制三阶段来提升跨域泛化能力。
...
Conclusion: NES方法有效解决了零样本图像描述中的幻觉问题，通过负实体抑制策略显著提升了跨域泛化性能。
...
TL;DR: SPEED-Q是一个针对视觉语言模型(VLMs)的低比特权重量化框架，通过分阶段敏感度自适应机制和蒸馏增强策略，解决了视觉与语言组件量化敏感度差异和训练不稳定性问题，在边缘设备上实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署视觉语言模型面临资源限制挑战，现有研究很少探索对1B-2B参数VLMs的激进量化，这更适合资源受限的边缘设备。
...
TL;DR: 提出了Diff-FCHM方法，首次基于机器视觉压缩构建人机协同压缩框架，通过扩散先验恢复高保真细节，在机器视觉和人类视觉压缩方面均取得显著优势。


<details>
  <summary>Details</summary>
Motivation: 现有的人机协同压缩方法主要基于人类视觉压缩流程，在整合机器视觉压缩时存在复杂度和比特率问题。机器视觉仅关注图像/视频中的核心区域，所需信息远少于人类视觉压缩。

Method: 基于机器视觉导向的压缩构建协同框架，开发即插即用的可变比特率策略，渐进聚合机器视觉压缩的语义信息，并利用扩散先验无缝恢复人类视觉的高保真细节。

Result: 实验结果表明Diff-FCHM在机器视觉和人类视觉压缩方面均取得持续优越的性能，具有显著优势。

Conclusion: 该方法成功实现了以机器视觉为基础的人机协同压缩，为同时服务于人类感知和机器智能的图像/视频数据压缩提供了有效解决方案。
...
TL;DR: 提出了分层蒸馏（HD）框架，将轨迹蒸馏和分布蒸馏从独立范式转变为协同组件，通过轨迹蒸馏建立结构"草图"，再通过分布蒸馏进行细化，结合自适应加权判别器（AWD）实现高效细节优化，在单步扩散模型中达到最先进性能。
...
Method: 1. 分层蒸馏框架：轨迹蒸馏作为结构草图生成器，为后续分布蒸馏提供近最优初始化；2. 自适应加权判别器（AWD）：通过动态分配token权重，专注于局部缺陷，实现高效细节细化。

Result: 在ImageNet 256×256上，单步模型FID达到2.26，媲美250步教师模型；在高分辨率文本到图像MJHQ基准测试中也取得有希望的结果，证明了方法的通用性。

Conclusion: 该方法为高保真单步扩散模型建立了稳健的新范式，通过协同利用轨迹蒸馏和分布蒸馏的优势，克服了各自的局限性。
...
TL;DR: NAMEA是一种新颖的集成攻击方法，通过整合异构模型的非注意力区域梯度来提升对抗样本的跨架构可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有集成攻击在跨异构模型架构时表现不佳，主要原因是异构代理模型的梯度更新方向差异很大，难以在充分利用单个模型的同时减少集成模型的梯度方差。

Method: 设计NAMEA集成攻击，首次将集成模型的非注意力区域梯度整合到迭代梯度优化过程中。通过元学习方式分别合并注意力区域和非注意力区域的梯度，融合CNN和ViT的迁移信息。
...
TL;DR: 提出了一种新颖的双向参考协调视频压缩方法BRHVC，通过双向运动收敛和双向上下文融合技术，有效解决了双向参考帧利用不平衡的问题，在HEVC数据集上超越了传统编码方法VTM-RA。


<details>
  <summary>Details</summary>
Motivation: 神经B帧视频压缩在双向参考帧利用方面存在挑战，特别是在层次编码中，大帧跨度会导致两个参考帧的贡献不平衡，影响压缩性能。

Method: 提出了BRHVC方法，包含双向运动收敛(BMC)和双向上下文融合(BCF)。BMC通过收敛多个光流实现更精确的大规模运动补偿，BCF在运动补偿精度指导下显式建模参考上下文的权重。

Result: 实验结果表明，BRHVC超越了之前最先进的神经视频压缩方法，甚至在HEVC数据集上超过了传统编码VTM-RA（随机访问配置）。

Conclusion: BRHVC通过更高效的运动和上下文处理，能够有效协调双向参考帧，实现了优异的压缩性能。
...
<details>
  <summary>Details</summary>
Motivation: 分形生成模型(FGMs)能生成高质量图像，但其固有的自相似性限制了输出多样性。需要解决FGMs多样性不足的问题，同时保持视觉质量。

Method: 1) 提出可学习的HD估计方法从图像嵌入直接预测HD；2) 采用基于HD的损失函数和单调动量驱动调度策略渐进优化超参数；3) 在推理阶段使用HD引导的拒绝采样选择几何更丰富的输出。
...
TL;DR: AuthSig是一个基于生成模型和水印的静态电子签名框架，通过风格嵌入调制将认证信息隐式编码到签名图像中，实现"一次签名，一次使用"策略。


<details>
  <summary>Details</summary>
Motivation: 随着无纸化工作流程的深入，签名从传统纸质转向电子格式。静态扫描签名因便利性仍被广泛使用，但缺乏可靠的认证属性，易被恶意复制和重用。

Method: 利用人类视觉系统对细微风格变化不敏感的特性，在生成过程中精细调制风格嵌入来隐式编码水印位；引入关键点驱动的数据增强策略增强风格多样性以支持鲁棒的水印嵌入。

Result: 实验结果显示，AuthSig在数字域失真和签名特定退化下提取准确率超过98%，在打印扫描场景下仍保持有效。

Conclusion: AuthSig通过生成模型和水印技术成功解决了静态电子签名的认证和防复制问题，为实际应用提供了可靠的解决方案。
...
TL;DR: 提出CoDR框架，通过核心集构建和双重检索机制优化演示样本选择，提升上下文学习性能


<details>
  <summary>Details</summary>
Motivation: 传统演示选择方法（随机、相似性、信息得分采样）在效率和效果上存在不足，无法平衡两者需求

Method: CoDR框架：1）使用聚类剪枝构建多样化核心集；2）双重检索机制实现全局演示选择并保持效率
...
TL;DR: 提出WDT-MD框架，通过小波扩散Transformer解决微动脉瘤检测中的身份映射、假阳性高和正常特征重建差等问题，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 微动脉瘤是糖尿病视网膜病变的早期病理特征，手动筛查劳动密集且易出错。现有基于扩散的异常检测方法存在身份映射、难以区分微动脉瘤与其他异常、正常特征重建差三大局限。
...
Method: 在ICTM框架下集成去噪项，包括I-散度项和自适应TV正则化器，使用灰度级指示器生成空间自适应权重，估计平滑变化的偏置场，并采用RMSAV方案进行高效优化。

Result: 在合成和真实世界图像上的广泛实验表明，该模型在存在强度不均匀性和多种噪声类型的情况下，相比竞争方法实现了更高的准确性和鲁棒性。

Conclusion: 所提出的模型能够有效处理受噪声污染和强度不均匀性的图像分割问题，在准确性和鲁棒性方面优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法独立调节视觉或文本注意力，忽略了它们作为两个关键因果因素的交互作用。物体幻觉问题严重影响LVLMs的可靠性。

Method: 1. 使用结构因果图建模幻觉过程，将分解的视觉和文本注意力作为中介变量；2. 提出VTACR指标量化模态贡献比；3. 设计细粒度注意力干预机制；4. 提出双路径对比解码策略。

Result: 在POPE和CHAIR基准测试中，Owl实现了显著的幻觉减少，在保持忠实性的同时达到了新的SOTA水平，同时保留了视觉语言理解能力。

Conclusion: Owl通过因果建模和细粒度注意力调节，有效缓解了物体幻觉问题，为LVLMs的可靠性提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有云图像序列外推方法存在三个主要问题：1) 主要依赖静态核增强特征信息，缺乏动态提取多分辨率特征的自适应机制；2) 时间引导不足，难以建模长程时空依赖；3) 注意力机制的二次计算成本限制了实际部署效率。

Method: USF-Net采用编码器-解码器框架，集成自适应大核卷积和低复杂度注意力机制。编码器使用三个基础层提取特征，USTM模块包含：SiB（配备SSM动态捕获多尺度上下文信息）和TiB（配备TAM高效建模长程时间依赖）。解码器使用DUM解决'重影效应'，利用初始时间状态作为注意力算子保留关键运动特征。

Result: 在ASI-CIS数据集上的大量实验表明，USF-Net显著优于现有最先进方法，在预测精度和计算效率之间实现了优越的平衡。

Conclusion: USF-Net通过创新的自适应大核卷积和低复杂度注意力机制，有效解决了地面云图像外推中的关键挑战，为光伏发电系统的发展提供了重要技术支持。
...
TL;DR: 提出了4KDehazeFlow方法，基于流匹配和雾感知向量场，通过渐进优化连续向量场流实现高质量去雾，兼容多种深度学习网络架构。


<details>
  <summary>Details</summary>
Motivation: 解决超高清图像去雾中先验方法场景适应性有限，以及深度学习方法计算复杂度高且存在色彩失真的问题。

Method: 使用可学习的3D查找表编码雾变换参数，通过四阶龙格-库塔ODE求解器稳定求解去雾流场，实现高效的数据驱动自适应非线性色彩变换。

Result: 在实验中超过7种最先进方法，PSNR提升2dB，在浓雾和色彩保真度方面表现更好。

Conclusion: 4KDehazeFlow是一种通用且高效的去雾方法，能够提供高质量的去雾效果，特别是在处理超高清图像时具有显著优势。
...
TL;DR: PAN是一个通用、可交互、长视野的世界模型，通过高质量视频模拟预测未来世界状态，结合了基于大语言模型的潜在动态推理和视频扩散解码器，实现潜在空间推理与现实世界动态的统一。
...
Method: 采用生成式潜在预测(GLP)架构，结合基于大语言模型的自回归潜在动态骨干网络（支持语言指定动作条件）和视频扩散解码器（重建感知细节和时间连贯的视觉观察）。

Result: 在大规模视频-动作对数据集上训练，PAN在动作条件世界模拟、长视野预测和模拟推理方面表现优异，优于其他视频生成器和世界模型。

Conclusion: PAN朝着通用世界模型迈出了一步，能够为推理和行为提供未来世界状态的预测性模拟。
...
TL;DR: 提出了VietMEAgent框架，用于解决越南文化相关视觉问答任务中的文化理解不足和解释性差的问题，通过结合文化对象检测和结构化程序生成，提供透明的多模态解释。


<details>
  <summary>Details</summary>
Motivation: 当前VQA系统在处理文化特定内容时存在局限，主要因为训练语料中文化知识不足且推理过程对用户不透明，需要开发能够理解越南文化并提供可解释推理的框架。

Method: 集成文化对象检测主干网络和结构化程序生成层，构建预测答案和解释紧密耦合的流水线；使用越南文化实体知识库作为背景信息源，结合基于注意力的视觉证据和结构化文本解释的双模态解释模块。

Result: 构建了越南文化VQA数据集，并验证了基于编程的方法在文化AI中的实用性，系统能够提供揭示计算原理和文化背景的透明解释。

Conclusion: VietMEAgent框架通过强调可解释性和文化敏感性，支持教育和文化保护，为文化AI提供了透明且文化敏感的解释能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时对抗防御方法TTC仅基于对抗输入的梯度生成扰动，搜索空间有限，导致对抗攻击多样性不足，无法充分中和广泛的扰动。

Method: 提出方向正交对抗攻击(DOC)，结合正交梯度方向和动量更新来扩展对抗攻击空间的探索，增加扰动多样性。同时基于平均余弦相似度提出方向敏感度评分，自适应调节对抗攻击强度。

Result: 在16个数据集上的广泛实验表明，DOC在各种攻击下提高了对抗鲁棒性，同时保持了竞争力的干净准确率。

Conclusion: 增强对抗攻击的多样性和覆盖范围对于提高测试时防御的对抗鲁棒性至关重要，DOC方法通过正交梯度探索实现了这一目标。
...
TL;DR: 本文提出了组合增量学习(CompIL)框架，用于在组合零样本学习任务中持续学习新组合，提升组合泛化能力。开发了MIT-States-CompIL和C-GQA-CompIL基准，并提出基于视觉合成器和语言原语蒸馏的伪回放框架。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据持续涌现，组合可能性近乎无限且呈长尾分布，现有方法无法完全覆盖。需要模型能够以增量方式逐步提升组合泛化能力。

Method: 提出伪回放框架：使用视觉合成器合成已学习组合的视觉表示，结合语言原语蒸馏机制在学习过程中保持对齐的原语表示。

Result: 在MIT-States-CompIL和C-GQA-CompIL基准上的大量实验证明了所提框架的有效性。

Conclusion: 组合增量学习框架能够有效提升模型在持续学习新组合时的组合泛化能力，为处理现实世界不断涌现的组合数据提供了可行方案。
...
TL;DR: UL-TTA是一种无需训练和反向传播的轻量级测试时适应方法，通过在线EM风格过程调整logit级参数（类别原型、先验概率和温度），在领域偏移下提升视觉语言模型的准确性和校准性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法需要反向传播、协方差估计或大量内存，不适用于流式和边缘场景。VLMs在领域偏移下存在特征漂移、类别先验不匹配和严重校准错误的问题。

Method: 冻结主干网络，仅在线调整logit级参数：使用选择性样本过滤、基于文本和Dirichlet先验的闭式贝叶斯更新、解耦的温度参数，以及轻量级防护机制防止长期漂移。

Result: 在大规模跨域和OOD基准测试中，UL-TTA将top-1准确率平均提升4.7个百分点，ECE降低20-30%，延迟开销小于8%，在长达20万样本的流式测试中无崩溃。

Conclusion: logit级贝叶斯适应足以在领域偏移下为VLMs获得最先进的准确率-校准权衡，无需更新任何主干网络参数。
...
TL;DR: 本文提出了DKDS数据集，专门针对包含印章和退化噪声的草书体古日文文档，建立了文本印章检测和文档二值化两个基准任务，并提供了基于YOLO和GAN的基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的OCR方法在处理干净的草书体古日文文档时表现良好，但无法有效处理文档退化和印章等噪声，且缺乏专门针对这些挑战的数据集。

Method: 构建了DKDS数据集，定义了两个基准任务：文本印章检测（使用YOLO模型）和文档二值化（使用传统算法、K-means聚类和GAN方法）。

Result: 提供了文本印章检测和文档二值化任务的基线结果，数据集和实现代码已公开。

Conclusion: DKDS数据集填补了草书体古日文文档处理中噪声挑战的空白，为相关研究提供了新的基准。
...
TL;DR: PIFF是一个基于物理的流生成神经网络，用于近实时洪水深度估计，通过结合简化淹没模型和降雨编码器，替代昂贵的数值模拟。


<details>
  <summary>Details</summary>
Motivation: 传统洪水测绘方法如数值模拟和航空摄影在效率和可靠性方面存在局限，需要更高效的实时洪水预测方案。

Method: 基于图像到图像的生成框架，将数字高程模型映射到洪水深度预测，结合简化淹没模型嵌入水动力先验，并使用基于Transformer的降雨编码器捕捉降水时间依赖性。

Result: 在台湾台南26公里研究区域，使用182个降雨情景（24-720mm/24小时）测试，PIFF能有效提供准确的实时洪水地图。

Conclusion: PIFF为洪水预测和响应提供了一个有效的数据驱动替代方案，能够捕捉降雨、地形、SPM和洪水之间的因果关系。
...
TL;DR: MACEval是一个多智能体持续评估网络，用于动态评估大模型，通过角色分配、过程数据生成和评估路由来解决现有基准测试的封闭性、过拟合和维护挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型基准测试多为封闭式，容易因数据污染而过拟合，且维护成本高、难以适应模型能力的快速发展，需要更动态、可持续的评估方法。

Method: 采用多智能体交互式评估模式，包括角色分配、过程数据生成和级联智能体网络的评估路由，实现自动化和可持续的评估。

Result: 在9个开放式任务和23个大模型上的实验表明，MACEval实现了无人工干预的自动化评估，显著减少了数据和处理开销，同时保持灵活性。

Conclusion: MACEval为大模型评估开辟了新方向，提供了一种高效、经济、灵活且可扩展的动态评估框架。
...
TL;DR: PressTrack-HMR是一个仅使用压力信号恢复多人全局人体网格的top-down管道，通过检测跟踪策略从原始压力数据中识别和分割每个人的压力信号，然后在多人交互压力数据集MIP上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的HMR方法在现实场景中因相互遮挡、光照不足和隐私问题而受限，而人-地板触觉交互提供了一种无遮挡且保护隐私的替代方案来捕捉人体运动。

Method: 采用top-down管道，利用检测跟踪策略首先从原始压力数据中识别和分割每个人的压力信号，然后对每个提取的个体信号执行HMR。

Result: 实验结果显示该方法在多人HMR方面表现出色，MPJPE为89.2mm，WA-MPJPE100为112.6mm，展示了触觉垫在普遍、保护隐私的多人动作识别方面的潜力。

Conclusion: PressTrack-HMR证明了仅使用压力信号进行多人全局人体网格恢复的可行性，为基于压力的人体运动分析在多人场景中的进一步研究提供了数据集和代码支持。
...
TL;DR: HOTFLoc++是一个用于森林环境中LiDAR地点识别、重排序和6自由度度量定位的端到端框架，通过八叉树变换器和多尺度几何验证模块，在挑战性场景中实现了高精度定位和显著的速度提升。
...
Result: 在CS-Wild-Places数据集上平均Recall@1达到90.7%，比基线提升29.6个百分点；在Wild-Places和MulRan数据集上分别达到91.7%和96.0%；97.2%的6-DoF配准尝试误差小于2米和5度；运行速度比RANSAC快两个数量级。

Conclusion: HOTFLoc++在复杂森林环境中实现了最先进的性能，通过多尺度重排序模块将定位误差平均降低约2倍，证明了其在跨源场景中的有效性和高效性。
...
TL;DR: DBINDS是一个基于扩散模型反演的AI生成视频检测器，通过分析潜在空间动态而非像素特征来检测AI生成视频，在单生成器训练下实现了良好的跨生成器泛化性能。
...
Conclusion: DBINDS通过分析潜在空间动态而非像素特征，有效解决了AI生成视频检测的泛化问题，为内容安全和取证分析提供了新的解决方案。
...
TL;DR: 提出了DermBench基准和DermEval评估器，用于评估多模态大语言模型在皮肤病诊断叙事生成中的表现，实现了临床意义、可重复和可扩展的评估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型越来越多地用于直接从图像生成皮肤病诊断叙事，但可靠的评估仍然是临床负责任部署的主要瓶颈。

Method: 构建DermBench基准（包含4000张真实皮肤病图像和专家认证的诊断叙事），并训练DermEval无参考多模态评估器，使用LLM评委在临床维度上评分。

Result: 在4500个病例的多样化数据集上，DermBench和DermEval与专家评分高度一致，平均偏差分别为0.251和0.117（满分5分）。

Conclusion: 该框架为不同多模态LLM的诊断能力和可信度提供了可靠的测量方法，支持细粒度的病例分析以识别模型局限性和偏见。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络通常学习空间纠缠的表示，将判别性前景特征与虚假背景相关性混在一起，损害模型可解释性和鲁棒性。

Method: 从信息论角度证明反向传播中的VJP构成输入特征关于类别标签的最小充分统计量，提出编码-解码视角，并设计S-IB框架最大化前景VJP与输入的互信息，最小化背景区域的互信息。

Result: 在五个基准测试上，六种解释方法均获得普遍改进，实现更好的前景集中和背景抑制，同时保持分类准确率提升。

Conclusion: 直接优化训练过程中VJP的空间结构可以改善各种解释范式的可视化质量，无需方法特定调优。
...
<details>
  <summary>Details</summary>
Motivation: 动画生成编解码器在边缘设备部署面临参数多、算法适应性差和高功耗的挑战，需要高效的硬件加速方案。
...
Conclusion: 提出的FPGA部署方案能有效解决AGC在边缘设备上的部署挑战，显著提升能量效率。
...
TL;DR: 本文系统评估了深度学习架构和生理信号选择对能量消耗估计的影响，发现分钟通气量是最具预测性的单信号，Transformer模型在所有活动中表现最佳（RMSE 0.87 W/kg）。
...
Result: 分钟通气量是最具预测性的单信号，Transformer模型在所有活动中达到最低RMSE（0.87 W/kg）；低强度活动误差更低（RMSE低至0.29 W/kg），高强度活动误差较大但归一化误差相当；个体间存在显著差异。

Conclusion: 信号选择和模型架构都对能量消耗估计有重要影响，个体间差异显著，需要自适应建模策略。
...
TL;DR: RichKD通过融合传统教师模型和CLIP的跨模态知识，为知识蒸馏提供更丰富、语义化的监督信号，提升学生模型的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法主要依赖单模态视觉信息，缺乏知识多样性。CLIP的视觉-语言跨模态表示具有补充监督的潜力，但这一领域尚未充分探索。

Method: 提出简单有效的框架，将传统教师模型的logits和特征与CLIP的logits和特征进行融合，并融入CLIP的多提示文本指导，同时捕获数据集特定和语义丰富的视觉线索。

Result: 融合后的监督产生更自信可靠的预测，显著增加自信正确的案例，减少自信错误的案例。CLIP融合还改善了整个logit分布，为非目标类生成语义有意义的概率，提高了类间一致性和蒸馏质量。

Conclusion: RichKD方法在多个基准测试中持续优于现有基线，并在分布偏移和输入损坏下表现出更强的鲁棒性，证明了跨模态知识融合在知识蒸馏中的有效性。
...
TL;DR: DensiCrafter是一个生成轻量化、自支撑3D空心结构的框架，通过优化密度场实现材料质量减少43%，保持高几何保真度且无需模拟即可满足物理约束。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型往往忽略物理约束和制造可行性，需要解决生成既轻量化又能自支撑的3D设计问题。

Method: 从Trellis生成的粗体素网格出发，将其解释为连续密度场进行优化，引入三个可微分、物理约束且无需模拟的损失项，结合质量正则化和受限优化域。

Result: 在文本到3D任务中实现材料质量减少高达43%，相比基线方法提升了稳定性并保持高几何保真度，真实3D打印实验验证了可制造性和自支撑性。

Conclusion: DensiCrafter能够无缝集成预训练模型，有效生成轻量化自支撑结构，为物理约束下的3D生成提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的师生半监督学习范式在医学图像分割中容易受到图像固有模糊性的影响，导致错误监督和自我强化偏差。现有方法通常依赖外部修改，而忽略了框架内在的错误纠正潜力。
...
TL;DR: FQ-PETR是一个针对PETR系列3D检测模型的全量化框架，通过三个创新技术解决量化过程中的精度损失问题，在W8A8量化下实现接近浮点精度（仅1%性能下降）的同时减少75%延迟。


<details>
  <summary>Details</summary>
Motivation: PETR系列模型在3D检测基准上表现出色，但面临高计算成本和内存占用的部署挑战。直接应用现有量化方法会导致严重的精度下降，主要由于多模态特征间的幅度差异和非线性算子量化效率低的问题。

Method: 提出三个关键技术：1）量化友好的LiDAR射线位置嵌入（QFPE），用单点采样替代多点采样；2）双查找表（DULUT）算法，用两个级联线性LUT近似复杂非线性函数；3）数值稳定后量化（QANS），在softmax数值稳定后进行量化以减少注意力失真。
...
<details>
  <summary>Details</summary>
Motivation: 移动红外小目标检测在无人机监控等应用中至关重要，但由于目标特征弱和背景干扰复杂而极具挑战。现有方法中时间差分缺乏空间特征提取能力，3D卷积缺乏对时间维度运动动态的显式感知

Method: 提出TDCNet网络，包含时间差分卷积重参数化模块（三个并行TDC块融合时间差分和3D卷积），以及TDC引导的时空注意力机制进行跨注意力建模
...
Method: Flora方法包含两个核心组件：灵活邻域感知语义调整（通过融入相邻类间上下文线索形成方向感知区域语义）和开放形式分布感知流分类器（使用无噪声流匹配桥接模态分布差距，结合无条件对比正则化增强判别性）

Result: 在三个基准数据集上的广泛实验验证了方法的有效性，即使在仅使用10%可见数据训练时也表现出色

Conclusion: Flora方法通过创新的语义对齐和分类器设计，有效解决了零样本骨架动作识别中的关键挑战，实现了优越的性能
...
<details>
  <summary>Details</summary>
Motivation: 解决支气管镜术中定位的挑战，包括呼吸运动、解剖变异和CT-身体差异导致的变形和不对齐问题，现有方法难以跨域泛化。
...
TL;DR: 构建了首个美式橄榄球球员检测和跟踪数据集，并在拥挤场景下评估了各种检测和跟踪方法，结果表明微调检测模型能显著提升性能。
...
Method: 构建首个美式橄榄球球员专用检测和跟踪数据集，比较评估多种检测和跟踪方法，包括微调检测模型和集成重识别模型到跟踪系统。

Result: 即使在拥挤场景下也能实现准确的检测和跟踪，微调检测模型相比预训练模型性能提升，集成微调检测器和重识别模型后跟踪精度显著优于现有方法。

Conclusion: 这项工作能够在传统方法难以处理的挑战性高密度场景中实现鲁棒的美式橄榄球球员检测和跟踪。
...
TL;DR: 提出双教师知识蒸馏框架，利用ViT和CNN教师协同指导轻量CNN学生，通过差异感知教师权重和结构差异感知蒸馏策略，在多个基准数据集上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: ViT在视频动作识别中性能强但计算成本高，轻量CNN效率高但精度不足。现有跨架构知识蒸馏方法存在架构不匹配问题，且忽略了强CNN教师的价值

Method: 双教师知识蒸馏框架：1) 差异感知教师权重 - 基于教师置信度和与学生预测差异动态融合ViT和CNN教师预测；2) 结构差异感知蒸馏 - 学生通过轻量辅助分支学习ViT和CNN教师间的残差特征

Result: 在HMDB51、EPIC-KITCHENS-100和Kinetics-400基准上持续优于最先进蒸馏方法，HMDB51上最高提升5.95%准确率
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法主要依赖几何线索并独立预测每帧姿态，无法有效处理模糊运动和在真实场景中泛化。受人类理解和预测运动方式的启发，需要结合动作感知和时间连贯性建模。

Method: 提出DreamPose3D框架：1）使用从2D姿态序列提取的任务相关动作提示动态调节去噪过程；2）引入表示编码器，在注意力机制中融入运动学关节亲和性；3）幻觉姿态解码器在训练中预测时间连贯的3D姿态序列。

Result: 在Human3.6M和MPI-3DHP基准数据集上实现了最先进的性能。在广播棒球数据集上的测试也显示出强大的鲁棒性，能够有效处理模糊和噪声的2D输入，保持时间一致性并处理意图驱动的运动变化。
...
TL;DR: vMFCoOp是一个在超球面流形上使用von Mises-Fisher分布来对齐LLM和CLIP语义偏差的框架，通过统一语义锚点实现稳健的生物医学提示学习和优越的小样本分类性能。
...
Conclusion: 该框架为生物医学视觉语言模型提供了稳健的提示学习解决方案，能够适应不断发展的基础模型家族，并将在未来扩展到更多下游应用。
...
TL;DR: RF-DETR是一个轻量级专业检测Transformer，通过神经架构搜索为任何目标数据集发现精度-延迟Pareto曲线，在COCO和Roboflow100-VL上显著优于现有实时方法。


<details>
  <summary>Details</summary>
Motivation: 开放词汇检测器在COCO上表现优异，但在包含预训练中未见类别的真实世界数据集上泛化能力不足，需要一种轻量级解决方案来适应新领域。

Method: 在目标数据集上微调预训练基础网络，通过权重共享神经架构搜索评估数千种网络配置，无需重新训练即可获得不同精度-延迟权衡。

Result: RF-DETR(nano)在COCO上达到48.0 AP，比D-FINE(nano)高5.3 AP且延迟相似；RF-DETR(2x-large)在Roboflow100-VL上比GroundingDINO(tiny)高1.2 AP且快20倍，是首个在COCO上超过60 AP的实时检测器。

Conclusion: RF-DETR通过神经架构搜索实现了在多种目标数据集上的高效适应，显著提升了实时检测性能，为开放词汇检测提供了轻量级解决方案。
...
TL;DR: 本文研究自动归因方法，将摘要中的每个句子与源文本的部分内容关联起来，探索使用混合摘要（即提取式摘要的自动改写）来简化归因过程，并使用自定义拓扑识别不同类别归因相关错误的比例。


<details>
  <summary>Details</summary>
Motivation: 分析师需要归因，因为不知道信息来源就无法进行报道。本文关注自动归因方法，将摘要句子与源文本关联。

Method: 使用混合摘要（提取式摘要的自动改写）来简化归因，并采用自定义拓扑来识别不同类别归因相关错误的比例。

Result: 未在摘要中明确说明具体实验结果。

Conclusion: 本文提出了使用混合摘要和自定义拓扑的自动归因方法，旨在解决摘要中句子与源文本的关联问题。
...
Result: 在多个数据集上持续优于强基线，准确率提高0.9%至21.6%，AUC提高0.006至0.309，能够仅用少量数据适应新用户和演化偏好，无需大量微调。

Conclusion: GMTRouter通过异构图建模有效解决了LLM路由中的个性化挑战，在数据稀缺情况下实现了高效的用户偏好学习，具有良好的适应性和扩展性。
...
TL;DR: LLMs能够生成足够逼真的社交媒体对话，在39%的情况下被误认为是人类创作的内容，特别是Llama 3生成的对话被正确识别为AI生成的概率仅56%，略高于随机猜测。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs是否能逼真地模拟社交媒体上的群体对话，探索其在社交模拟中的潜力以及可能被滥用于生成虚假社交媒体内容的风险。

Method: 收集Reddit上的真实人类对话，使用Llama 3 70B和GPT-4o在同一话题上生成人工对话，通过参与者侧边比较来评估对话的真实性。

Result: LLM生成的对话有39%的概率被误认为是人类创作，特别是Llama 3生成的对话被正确识别为AI生成的概率仅56%，略高于随机猜测。

Conclusion: LLMs能够生成足够逼真的社交媒体对话来欺骗人类读者，既展示了社交模拟的潜力，也警示了可能被滥用于生成虚假社交媒体内容的风险。
...
TL;DR: 本文研究LLMs在法律分析中的双重性：既能解读生物武器法律，又存在生成危险内容的漏洞。通过知识图谱和RAG方法评估LLMs的法律理解能力、意图判断和安全性。


<details>
  <summary>Details</summary>
Motivation: LLMs在敏感法律领域具有变革潜力，但存在生成不安全输出的矛盾，需要系统评估其法律理解和安全机制。

Method: 结合知识图谱构建和检索增强生成(RAG)方法，通过结构化实验评估LLMs的法律违规识别、禁止指令生成和非法意图检测能力。

Result: 发现LLMs在推理和安全机制方面存在显著局限性，但指出了改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM对社会影响日益增大，其代表多元视角的能力至关重要。然而研究发现，对齐算法显著降低了LLM输出的多样性，不仅文本结构重复，解决问题方式趋同，而且反映的社会视角范围变窄。
...
Conclusion: Soft Preference Learning是标准温度缩放的帕累托改进，能够有效解决对齐算法导致的多样性损失问题，同时提升模型能力和对齐质量。
...
<details>
  <summary>Details</summary>
Motivation: 树状思维推理虽然提升了大型语言模型的问题解决能力，但由于语义冗余导致计算成本高昂，需要探索等效推理路径的不同分支。

Method: 提出语义相似性动态剪枝（SSDP），将在线语义合并集成到并行化树搜索中，实时聚类和剪枝冗余步骤。
...
TL;DR: 提出了一个测试LLMs在对抗性提示下事实保真度的框架，发现不同LLMs对自身生成的谎言表现出不同程度的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 幻觉问题是LLMs在关键领域实际部署的主要挑战，需要测试其在对抗性提示下的事实保真度。

Method: 三步骤框架：1) 让LLM生成特定领域的真假陈述；2) 让LLM验证这些陈述；3) 测试LLM对自身生成谎言的抵抗能力。

Result: 在电影和小说两个封闭领域测试5个主流LLMs，发现Claude抗干扰能力最强，GPT和Grok中等，Gemini和DeepSeek最弱。

Conclusion: 随着越来越多人使用LLMs进行信息检索，这些发现敲响了警钟，需要关注LLMs在对抗性提示下的脆弱性。
...
TL;DR: 提出了Self-HarmLLM攻击场景，使用模型自身生成的缓解有害查询作为新输入来绕过防护机制，在多种条件下实现了高达65%的转换成功率和41%的越狱成功率。
...
Method: 使用模型生成的缓解有害查询作为新输入，在GPT-3.5-turbo、LLaMA3-8B-instruct和DeepSeek-R1-Distill-Qwen-7B上测试Base、Zero-shot和Few-shot条件。
...
TL;DR: OKBench是一个完全自动化的框架，用于按需生成高质量、动态的知识基准，特别关注新闻领域，通过智能体框架自动化基准的获取、创建、验证和分发。
...
Method: 采用智能体框架自动化基准生成流程，包括知识获取、基准创建、质量验证和分发，重点关注新闻领域以减少与预训练数据的重叠。

Result: 评估显示不同模型在面对新信息时表现出明显差异，检索机制显著缩小了大小模型之间的性能差距。

Conclusion: 在演化知识基准上评估LLM至关重要，OKBench框架实现了基准创建的民主化，并促进了对检索增强方法的全面评估。
...
Method: 开发了多模型RAG系统原型，整合了精选领域知识和工程化提示模板，支持五个商业和开源LLM。设计了七个涵盖不同障碍类型和年级水平的测试场景，使用多维度评估标准对生成案例进行自动化质量评估。

Result: 概念验证表明RAG增强的儿科SLP案例生成具有技术可行性。商业模型质量略有优势，但开源替代方案也达到可接受性能，显示出隐私保护机构部署的潜力。整合知识库使生成内容符合专业指南。

Conclusion: 该系统展示了自动生成儿科SLP临床案例的技术可行性，未来应用可扩展到临床决策支持、自动IEP目标生成和临床反思训练等领域，但需要专家评审、学生试点测试和心理测量评估等广泛验证。
...
TL;DR: 将DisCoCirc形式化框架从英语扩展到孟加拉语，用于英-孟翻译任务，发现该框架在减少语言官僚主义方面存在局限性，特别是在处理两种语言结构差异时。


<details>
  <summary>Details</summary>
Motivation: 扩展DisCoCirc框架到孟加拉语，评估其在英-孟翻译中的有效性，并重新检验该框架在减少语言官僚主义方面的能力。

Method: 基于DisCoCirc的语法规则和电路表示方法，建立孟加拉语的类似框架，应用于英-孟翻译任务，并分析其局限性。

Result: DisCoCirc框架虽然适用于大部分语言结构，但在处理英-孟语言结构差异时表现不佳，即使对简单句子也存在困难。

Conclusion: DisCoCirc框架在跨语言翻译中存在局限性，特别是在处理结构差异较大的语言对时，需要进一步改进以克服这些限制。
...
TL;DR: Mina是一个为孟加拉国低收入人群开发的多语言法律AI助手，通过RAG框架提供法律检索、推理、翻译和文档生成功能，在孟加拉律师考试中达到75-80%的准确率，匹配或超过人类平均水平。
...
Method: 采用多语言嵌入和基于RAG的工具链框架，包括检索、推理、翻译和文档生成，通过交互式聊天界面提供上下文感知的法律草案、引用和平白语言解释。

Result: 在2022和2023年孟加拉律师委员会考试的所有阶段评估中，Mina在初步多选题、笔试和模拟口试中得分75-80%，匹配或超过人类平均表现，展示了清晰性、上下文理解和合理的法律推理能力。
...
TL;DR: 研究表明，通过元学习集成多个大型语言模型（LLMs）的MEDAS系统在急诊医学诊断中达到70%准确率，显著超过单个LLM（58-65%）和人类医生的表现。


<details>
  <summary>Details</summary>
Motivation: 急诊医学需要快速准确的诊断决策，AI和LLMs在医疗决策支持系统中显示出潜力，但单个模型的诊断能力有限。

Method: 构建MEDAS超级学习器，集成Gemini、Llama、Grok、GPT和Claude五个主要LLMs，使用元学习器学习每个LLM的不同能力，通过集体能力提升诊断准确性。

Result: 单个LLM诊断准确率58-65%，超级学习器达到70%准确率，其中至少一个集成LLM达到85%正确诊断率。

Conclusion: 元学习方法整合的LLM集群诊断准确率超过任何单个模型，表明超级学习器能够利用多个LLM训练医疗数据集的集体知识优势。
...
TL;DR: 提出了一种基于梯度感知的数据选择方法GrADS，用于LLMs的监督微调，仅需5%-50%的数据即可超越全数据训练效果，同时缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 传统SFT在领域专业化中资源消耗大且容易导致灾难性遗忘，需要更高效的数据选择方法。

Method: 通过初步训练阶段的梯度分析，设计基于梯度大小和统计分布的自引导标准，优先选择对模型学习贡献最大的训练样本。

Result: 在医学、法律、金融等多个领域实验中，仅用5%的GrADS选择数据即可超越全数据训练效果，50%数据时性能显著提升，同时大幅缓解灾难性遗忘。

Conclusion: GrADS方法在LLMs领域专业化中具有显著效率和成本效益，能够有效选择代表性样本提升模型性能并减轻遗忘问题。
...
Method: 采用CNN架构提取局部特征，结合BiGRU技术进行双向序列建模，加入注意力机制，并使用SHAP可解释AI方法来解释预测结果和验证模型可靠性。

Result: 在公开数据集上的实验结果显示，该方法达到了93.97%的准确率。与现有最先进的机器学习和深度学习模型相比，证明了所提技术的优越性。

Conclusion: CNN局部特征提取、BiGRU双向序列建模、注意力机制和SHAP可解释性的结合，为自杀检测提供了一个全面的框架，在识别自杀意念模式方面表现出色。
...
TL;DR: SAGE-Agent通过结构化不确定性建模工具调用参数，在模糊任务中提高成功率7-39%，同时减少澄清问题1.5-2.7倍，并通过不确定性加权训练显著提升When2Call准确率。


<details>
  <summary>Details</summary>
Motivation: LLM代理在工具调用时，模糊的用户指令经常导致错误的调用和任务失败，需要一种更有效的方法来处理参数不确定性。

Method: 将工具-参数澄清建模为POMDP，使用期望完美信息价值(EVPI)目标进行最优问题选择，并采用基于方面的成本建模防止冗余。

Result: 在ClarifyBench基准测试中，SAGE-Agent在模糊任务上覆盖度提高7-39%，澄清问题减少1.5-2.7倍；通过不确定性加权GRPO训练，When2Call准确率从36.5%提升至65.2%(3B模型)和36.7%提升至62.9%(7B模型)。

Conclusion: 结构化不确定性为工具增强代理提供了一种原则性、高效的方法，显著提高了现实场景中的任务成功率和交互效率。
...
TL;DR: 本研究开发并评估了三种NLP模型，用于从帕金森病患者的第一人称叙述中自动识别反映认知过程的七个类别。微调的Meta-Llama-3-8B-Instruct模型表现最佳，特别是在思想和社交互动等上下文依赖类别中。


<details>
  <summary>Details</summary>
Motivation: 理解帕金森病患者在日常生活中的认知体验描述可以提供对疾病相关认知和情感变化的宝贵见解，但从非结构化患者叙述中提取此类信息具有挑战性。

Method: 比较了三种模型：基于Bio_ClinicalBERT的嵌套实体识别模型、使用QLoRA微调的Meta-Llama-3-8B-Instruct指令跟随模型，以及GPT-4o mini在零样本和少样本设置下的评估。
...
TL;DR: BNLI是一个经过精细标注和语言优化的孟加拉语自然语言推理数据集，解决了现有数据集存在的标注错误、语义模糊和语言多样性不足等问题，为孟加拉语NLI研究提供了可靠基础。


<details>
  <summary>Details</summary>
Motivation: 现有孟加拉语NLI数据集存在标注错误、歧义句子对和语言多样性不足等问题，阻碍了有效的模型训练和评估，需要构建更高质量的数据集。

Method: 通过严格的标注流程构建数据集，强调语义清晰性和蕴含、矛盾和中性类别的平衡，并使用多语言和孟加拉语专用transformer模型进行基准测试。

Result: 实验结果表明BNLI在可靠性和可解释性方面显著提升，能够有效评估模型对孟加拉语复杂语义关系的理解能力。

Conclusion: BNLI为推进孟加拉语及其他低资源语言推理任务研究奠定了坚实基础，支持鲁棒的语言理解和推理建模。
...
TL;DR: TACT数据集用于统一任务导向对话和闲聊，支持模式切换，结合DPO训练显著提升对话质量和切换控制能力。


<details>
  <summary>Details</summary>
Motivation: 传统对话系统将任务导向对话和闲聊分开开发，但真实对话需要在这两种模式间自然切换，需要统一建模。

Method: 构建TACT数据集支持用户和代理驱动的模式切换，提出Switch和Recovery评估指标，使用DPO进行模型优化。

Result: TACT训练模型在意图检测和模式切换处理上优于基线，DPO进一步提升了性能，达到75.74%联合模式-意图准确率和70.1%胜率。

Conclusion: 结构多样化的数据结合DPO能增强响应质量和切换控制，为更主动和切换感知的对话代理铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 传统生物医学假设生成方法依赖单一数据类型或预定义提取模式，限制了发现新颖复杂联系的能力。LLM智能体在信息检索、推理和生成方面具有潜力，但缺乏标准化数据集和执行环境。

Method: 提出BioVerge基准测试和BioVerge Agent框架，使用基于ReAct的方法，包含生成和评估模块，迭代生成并自我评估假设提案。结合历史生物医学假设和PubMed文献的结构化和文本数据。

Result: 实验发现：1）不同架构影响探索多样性和推理策略；2）结构化和文本信息源各自提供独特关键上下文；3）自我评估显著提升假设的新颖性和相关性。

Conclusion: BioVerge为生物医学假设生成提供了标准化环境，LLM智能体框架通过多源信息整合和自我评估机制，有效推动了前沿科学知识的探索。
...
TL;DR: LLM生成参考文献时，高被引论文的幻觉率更低，超过1000次引用的论文几乎被逐字记忆，存在记忆干扰现象。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在文献推荐中产生虚构论文的问题，探究引用频率如何影响LLM输出中的幻觉参考文献。

Method: 使用GPT-4生成100个跨20个计算机科学领域的引用，通过余弦相似度手动验证生成与真实元数据的事实一致性。

Result: 引用数与事实准确性强相关；超过约1000次引用的文献信息几乎被逐字记忆；多个高被引论文内容相似时出现记忆干扰。

Conclusion: 存在一个从泛化转向记忆的阈值，高被引论文几乎被逐字保留在模型中。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能出色，但经常产生幻觉内容，影响事实可靠性，需要解决这一挑战。
...
TL;DR: TiDAR是一种序列级混合架构，在单次前向传播中通过扩散模型并行生成草稿，然后自回归采样最终输出，实现了与自回归模型相当的质量和4.71-5.91倍的吞吐量提升。
...
Method: TiDAR采用Thinking in Diffusion and Talking AutoRegressively的混合架构，使用结构化注意力掩码在单次前向传播中并行生成草稿并自回归采样最终输出。

Result: TiDAR在1.5B和8B规模上超越推测解码的吞吐量，在效率和质最上都优于Dream和Llada等扩散模型，首次在保持自回归模型质量的同时实现4.71-5.91倍的每秒token生成量提升。

Conclusion: TiDAR成功平衡了扩散模型的并行生成能力和自回归模型的质量优势，是首个在保持自回归级别质量的同时显著提升吞吐量的架构。
...
TL;DR: 提出了EVADE框架，使用LLMs生成和验证解释来检测NLI任务中的标注错误，相比传统人工标注方法更高效且能提升数据集质量


<details>
  <summary>Details</summary>
Motivation: 在自然语言推理任务中，人工标注变异使得难以区分标注错误和合理的标签变化，传统人工两轮标注方法成本高且覆盖有限

Method: 提出EVADE框架，利用大语言模型生成和验证解释来检测错误，并与人工标注进行对比分析

Result: LLM验证使生成的解释分布更接近人工标注，从训练数据中移除LLM检测的错误比移除人工检测的错误带来更大的微调性能提升

Conclusion: LLM可以规模化错误检测，在减少人工努力的同时提高标签变异下的数据集质量
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在推理方法缺乏确保潜在表示稳定演化的机制，也没有系统化的方式来交织隐式和显式推理。需要一种能够进行迭代计算并保持潜在与文本推理一致性的方法。

Method: 引入SpiralThinker框架，对潜在表示执行迭代更新，实现扩展的隐式推理。采用渐进对齐目标结合结构化注释，维持潜在与文本推理之间的一致性。

Result: 在数学、逻辑和常识推理任务中，SpiralThinker在所有基准测试中持续超越先前方法，达到潜在推理方法中的最佳整体性能。

Conclusion: SpiralThinker桥接了迭代计算和潜在推理，证明对齐的迭代更新可以可靠地在潜在空间中引导推理过程。迭代和对齐都是不可或缺的，潜在token数量和迭代次数存在数据集特定的最优值。
...
TL;DR: 本文提出了一个基于轨迹的情感支持评估框架，从静态对话评估转向动态长期评估，通过模拟真实情绪变化和约束模型使用心理学验证的情绪调节策略，来全面评估LLMs的长期情感支持能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型情感支持评估多依赖短对话和静态评估，无法捕捉情感支持的动态性和长期性，需要转向用户中心的轨迹式评估方法。

Method: 构建包含328个情感情境和1152个干扰事件的大规模基准，约束模型使用验证的情绪调节策略，将用户情绪轨迹建模为一阶马尔可夫过程，并应用因果调整的情绪估计方法。

Result: 提出了三个轨迹级指标（BEL、ETV、ECP）来捕捉用户情绪动态，通过对多种LLMs的广泛评估揭示了情感支持能力的显著差异。

Conclusion: 该框架能够全面评估LLMs的长期情感支持性能，为模型开发提供了可行的见解，证明了轨迹式评估在情感支持领域的重要性。
...
TL;DR: 提出一个两阶段神经符号框架，使用LLM将自然语言策略形式化，并通过推理时自动形式化验证逻辑正确性，在需要高正确性时进行多重冗余形式化步骤交叉检查。


<details>
  <summary>Details</summary>
Motivation: LLM在自然语言理解和推理方面表现良好，但其固有的随机性限制了在金融、医疗等受严格政策监管的行业中的应用。

Method: 两阶段框架：(1) 使用LLM（可选人工指导）将自然语言策略形式化；(2) 推理时自动形式化验证自然语言陈述的逻辑正确性，必要时进行多重冗余形式化步骤交叉检查语义等价性。

Result: 基准测试显示该方法超过99%的可靠性，表明在识别逻辑有效性方面接近零误报率。

Conclusion: 该方法产生可审计的逻辑工件，能够证实验证结果并可用于改进原始文本。
...
TL;DR: 提出了MM-CRITIC基准，用于评估大型多模态模型的批判能力，涵盖基础、修正和比较三个维度，包含8种任务类型和500多个任务。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型在图像描述和视觉推理等任务中能力不断增强，但多模态批判能力的研究仍然不足，而批判能力对模型自我改进和作为可靠AI助手至关重要。

Method: 构建包含4471个样本的基准数据集，整合专家知识制定评分标准，使用GPT-4o标注响应并生成参考批判作为可信判断的锚点。

Result: 广泛实验验证了MM-CRITIC的有效性，并全面评估了领先LMMs在不同维度下的批判能力，发现响应质量与批判能力存在相关性，且不同评估维度的批判难度不同。

Conclusion: MM-CRITIC为评估多模态模型的批判能力提供了全面基准，揭示了LMMs批判能力的关键见解，有助于模型自我改进和可靠性提升。
...
TL;DR: 基于混合CTC/Attention架构和上下文感知动态分块机制的安多藏语流式语音识别框架，通过自适应调整分块宽度解决固定分块方法的上下文截断问题，集成外部语言模型提升语义一致性。


<details>
  <summary>Details</summary>
Motivation: 解决固定分块方法在流式语音识别中的上下文截断问题，同时适应安多藏语的语言特性，提高识别准确率和实时性。

Method: 采用混合CTC/Attention架构，引入上下文感知动态分块机制自适应调整分块宽度，构建基于藏语正字法原则的词典作为建模单元，解码时集成外部语言模型。

Result: 在测试集上达到6.23%的词错误率，相比固定分块基线相对提升48.15%，显著降低识别延迟，性能接近全局解码。

Conclusion: 提出的动态分块机制有效解决了流式识别中的上下文截断问题，结合语言特性建模和外部语言模型集成，在安多藏语语音识别中取得了显著性能提升。
...
TL;DR: Bi-RAR是一个新颖的检索增强推理框架，通过双向评估中间步骤来提升多步推理能力，使用基于Kolmogorov复杂度的双向信息距离来量化信息完整性，并采用多目标强化学习进行优化。
...
Conclusion: Bi-RAR通过双向评估和多目标强化学习有效提升了检索增强推理在多步复杂场景中的性能，为减少语言模型幻觉提供了更有效的解决方案。
...
TL;DR: 本文通过日本即兴喜剧游戏Oogiri系统评估大语言模型的幽默能力，发现LLMs在生成幽默内容时缺乏共情能力，且与人类在幽默评价标准上存在根本差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖单维度评估LLMs的幽默能力，本文认为需要多维度理解幽默，通过Oogiri游戏形式填补这一研究空白。

Method: 扩展现有Oogiri数据集，用LLMs生成Oogiri回应，手动标注6个维度的5分制评分（新颖性、清晰度、相关性、智能性、共情性、总体趣味性），评估LLMs在生成和评价幽默内容方面的能力。

Result: LLMs生成的回应处于人类中低水平，但明显缺乏共情能力；相关性分析显示LLMs优先考虑新颖性，而人类优先考虑共情性。

Conclusion: LLMs在幽默生成和评价方面与人类存在根本差异，缺乏共情能力是其主要限制，释放标注语料库以促进更智能对话代理的开发。
...
<details>
  <summary>Details</summary>
Motivation: EFL教育中，个性化学习对于提高学生的阅读理解和学习动机至关重要，现有材料往往缺乏个性化，无法有效激发学生兴趣。

Method: 开发结构化内容转创流程，使用GPT-4o从RACE-C数据集出发，生成语言特征相似但语义符合学生兴趣的新篇章和多选题，整合主题提取、布鲁姆分类法问题分类、语言特征分析和内容转创。

Result: 在韩国EFL学习者中进行的对照实验显示，使用个性化阅读材料的学生在理解和动机保持方面表现优于使用非个性化材料的学生。

Conclusion: 个性化英语阅读材料能有效提升EFL学习者的阅读理解和学习动机，证明了基于兴趣对齐的内容转创方法的有效性。
...
TL;DR: DoPE是一种无需训练的位置编码去噪方法，通过截断矩阵熵检测特征图中的异常频带，并利用高斯分布重新参数化，显著提升了Transformer模型在长上下文（达64K tokens）中的检索准确性和推理稳定性。


<details>
  <summary>Details</summary>
Motivation: Rotary Position Embedding (RoPE)在Transformer模型中存在固有局限，会削弱长度外推能力。研究发现注意力图中存在噪声特征，需要解决位置编码导致的注意力下沉现象。

Method: 将带位置编码的注意力图重新解释为噪声特征图，提出基于截断矩阵熵的DoPE方法检测异常频带，利用特征图的噪声特性通过参数免费的高斯分布重新参数化特征图。

Result: 在needle-in-a-haystack和many-shot in-context learning任务上的实验表明，DoPE在扩展上下文（达64K tokens）中显著提高了检索准确性和推理稳定性，有效缓解了注意力下沉现象。

Conclusion: 位置编码的去噪策略有效缓解了注意力下沉，恢复了平衡的注意力模式，为改进长度泛化提供了简单而强大的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的工具学习受到静态合成数据管道的限制，数据生成和模型训练是两个分离的非交互过程，无法自适应地关注模型的具体弱点，且允许噪声标签持续存在，降低了训练效率。

Method: LoopTool通过三个协同模块迭代优化数据和模型：贪婪能力探测诊断模型的掌握和失败能力；判断引导标签验证使用开源判断模型查找并纠正标注错误；错误驱动数据扩展基于识别出的失败生成新的挑战性样本。

Result: 实验表明，使用LoopTool训练的8B模型显著超越了其32B数据生成器，并在BFCL-v3和ACEBench基准测试中达到了其规模的最新最优结果。

Conclusion: 闭环自精化数据管道可以显著增强LLM的工具使用能力，证明了在成本效益高的开源生态系统中实现这种方法的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在表格问答中直接输入整个表格容易出错，因为它们无法有效捕捉复杂的表格结构，需要更好的方法来处理表格信息提取。

Method: 使用语言模型和TF-IDF的混合检索机制计算问题与单元格的相似度来估计表头，通过对比学习在小数据集上训练语言模型，并在最相关行列的交集处选择答案。

Result: 在NTCIR-18 U4共享任务的TQA数据集上达到74.6%的准确率，优于GPT-4o mini的63.9%。

Conclusion: 该方法有效提升了表格问答的准确性，未来计划整合更高效的文本搜索模型来进一步缩小与人类评估结果的差距。
...
TL;DR: 验证了基于上下文的sequentiality度量比原始版本更符合人类对文章流畅性的评估，当与标准语言特征结合使用时，其预测能力优于零样本LLM预测。


<details>
  <summary>Details</summary>
Motivation: 先前的研究提出使用LLM来量化叙事流畅性，但存在主题选择混淆和缺乏地面真实验证的问题。本文旨在实证验证仅使用上下文术语作为更有效替代方案的提议。

Method: 使用两个带有人工标注特征分数的论文数据集（ASAP++和ELLIPSE），比较上下文版本sequentiality与人类对组织和连贯性等语篇层面特征的评估一致性。

Result: 上下文版本的sequentiality与人类评估更一致；虽然零样本LLM预测特征分数更准确，但上下文度量与标准语言特征结合时比原始sequentiality公式和零样本LLM预测表现更好。
...
TL;DR: 本文研究了子词分割在学习过程中的动态变化，发现语言模型可以动态优化分词，子词在预训练和微调阶段会经历四个发展阶段，形态复杂的语言表现出更大的不稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统子词分割通常在预处理阶段固定，但本文探索在训练过程中动态学习子词分割，以优化训练目标，并研究子词在不同语言类型中的学习动态。

Method: 扩展子词分割语言模型(SSLM)框架，支持预训练和微调，在三种类型学不同的语言(isi-Xhosa、Setswana、英语)上进行训练，从语言学角度分析子词动态变化。

Result: 识别出子词学习的四个阶段，形态复杂的isi-Xhosa表现出更大不稳定性；微调时子词边界变得更细粒度；可学习子词能改善低资源、形态复杂语言的文本生成和跨语言迁移。

Conclusion: 可学习子词分割为低资源、形态复杂语言提供了有前景的改进方法，子词在训练过程中会动态演化，微调使分词更精细。
...
TL;DR: 提出了ModernBERT编码器模型的六个不同规模版本（5100万至4.75亿参数），专注于有限多语言能力，特别强调与芬兰相关的语言。这些模型在性能上与现有多语言模型相当或更优，在需要超过512个标记的长上下文任务中表现优于单语模型。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对芬兰相关语言的有限多语言模型，以解决现有模型在特定语言和长上下文处理方面的不足。

Method: 预训练不同参数规模的ModernBERT编码器模型，采用多阶段训练策略，特别关注最终训练阶段使用不同数据的影响。

Result: 模型在性能上与现有多语言模型竞争或更优，在长上下文任务中显著优于单语模型，并提供了不同训练数据影响的实证结果。

Conclusion: 成功开发了针对芬兰相关语言的有限多语言ModernBERT模型，证明了在特定语言场景下的有效性，并公开了代码和模型。
...
TL;DR: 本文提出Anchor方法，通过将真实轨迹注入到强化学习rollouts中来防止早期训练崩溃，显著提升了语言模型在可验证推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法主要优化最终任务结果，在早期训练中当负奖励占主导时容易导致模型崩溃，这在诚实对齐任务中尤为突出，因为模型不仅要解决可回答的查询，还要识别无法从前提得出结论的情况。

Method: 作者构建了两个基于图结构的多步演绎推理数据集，并提出Anchor方法——一种在rollouts中注入真实轨迹的强化学习方法，以防止早期训练崩溃。

Result: 实验表明，GRPO方法在这些任务上表现不佳，而Anchor方法能够稳定学习过程，显著提升整体推理性能。

Conclusion: 训练动态对于在已对齐语言模型中实现可靠的演绎推理至关重要，Anchor方法通过稳定学习过程有效解决了早期训练崩溃问题。
...
TL;DR: POTSA是一个基于跨语言平行语音对和最优传输的新框架，旨在通过偏置补偿模块和token级OT约束来弥合高低资源语言间的翻译差距，在FLEURS数据集上实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音大语言模型在多语言语音到文本翻译中往往忽略跨源语言的语义共性，导致翻译性能存在偏差，特别是在高低资源语言之间存在性能差距。

Method: 1) 偏置补偿模块粗粒度对齐跨语言语音表示；2) 在Q-Former上施加token级最优传输约束建立细粒度表示一致性；3) 层调度策略将OT约束聚焦于语义最有益层。

Result: 在FLEURS数据集上，平均在5种常见语言上BLEU提升+0.93，在零样本语言上BLEU提升+5.05，仅使用每种源语言10小时的平行语音数据。

Conclusion: POTSA框架通过跨语言语义对齐有效弥合了高低资源语言间的翻译性能差距，在有限数据下实现了显著的性能提升。
...
TL;DR: C$^3$TG是一个两阶段框架，用于细粒度、多维度的文本属性控制，通过选择性配对LLM与属性分类器，结合加权KL散度调整token概率，并在优化阶段使用能量函数解决属性冲突。
...
Method: 提出C$^3$TG框架：生成阶段选择性地将LLM与所需属性分类器配对，使用加权KL散度调整token概率；优化阶段利用结合分类器分数和惩罚项的能量函数，通过迭代反馈解决属性冲突。

Result: 实验表明C$^3$TG在多个指标上显著优于基线方法，包括属性准确性、语言流畅性和输出多样性，同时降低了毒性。
...
<details>
  <summary>Details</summary>
Motivation: 人们有不同的创意写作偏好，但现有LLM在训练时通常将不同个人品味视为单一整体，需要开发能够适应每个用户偏好的个性化创意写作LLM。

Method: 构建LiteraryTaste数据集，包含60人的阅读偏好数据，包括自述偏好和对100对短创意写作文本的偏好标注；使用transformer编码器进行微调建模；采用LLM驱动的可解释性管道分析偏好变化。

Result: 1) 人们在创意写作偏好上存在分歧；2) 微调transformer编码器在建模个人和集体实际偏好时分别达到75.8%和67.7%准确率；3) 自述偏好对建模实际偏好的效用有限。

Conclusion: 该工作为个性化创意写作技术奠定了基础，展示了通过数据集和模型微调实现个性化创意写作LLM的可行性。
...
TL;DR: 提出了一个可解释的高棉语极性分类器，通过微调基于指令推理的Qwen-3模型，不仅能准确预测极性标签，还能提供解释理由。
...
Method: 微调基于指令推理的Qwen-3模型，构建新的高棉语极性数据集（包含短到中等长度的随意、罗马化和混合代码表达）。

Result: 微调后的模型不仅能准确预测标签，还能通过识别极性相关关键词或短语来支持其预测。

Conclusion: 成功开发了可解释的高棉语极性分类器，并贡献了新的高棉语极性数据集和微调模型，均在Hugging Face上公开。
...
TL;DR: 提出了AdaptDel方法，通过动态调整删除率来改进序列分类在编辑距离扰动下的认证鲁棒性，相比现有方法在自然语言任务中取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定删除率处理变长序列（如自然语言句子）时性能不佳，需要能够根据输入特性动态调整删除率的机制。

Method: 引入AdaptDel方法，具有可适应的删除率，基于输入属性动态调整；将随机平滑理论框架扩展到变率删除，确保对编辑距离的可靠认证。

Result: 在自然语言任务中取得了强大的实证结果，认证区域的中值基数比最先进认证方法提高了高达30个数量级。
...
TL;DR: 提出了mmJEE-Eval多模态双语基准，基于印度JEE Advanced考试题目，用于评估模型真实科学推理能力而非模式匹配。评估显示前沿VLMs表现优异但开源模型表现不佳，且模型在元认知推理任务上表现崩溃。
...
Method: 构建包含1460道JEE Advanced考题的多模态双语基准，涵盖物理、化学、数学，评估17个最先进模型，并进行系统消融实验。

Result: 前沿VLMs在2025年题目上达到77-84%准确率，开源模型仅为37-45%；在元认知推理任务上模型表现严重下降（GPT-5仅修复5.2%错误）。

Conclusion: mmJEE-Eval能有效区分优秀训练和推理方法，其难度源于复杂性和推理深度而非记忆，是评估真实科学推理能力的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 测试时缩放能提升大语言模型的推理性能，但计算成本高昂。现有动态自一致性方法虽然减少了token消耗，但仍受限于顺序请求的高延迟。
...
Conclusion: SeerSC框架通过整合两种推理系统，在保持性能的同时显著提升了token效率和推理速度。
...
TL;DR: 提出了Spider4SSC数据集和S2CLite解析工具，这是一个轻量级、本体无关的SPARQL到Cypher查询转换器，显著提升了查询解析准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的SPARQL到Cypher转换解决方案存在解析错误率高的问题，需要开发一个更准确、无需依赖RDF图或外部工具的解析器。

Method: S2CLite采用纯基于规则的方法（受传统编程语言编译器启发），将SPARQL查询转换为Cypher查询，支持原位和大规模转换。

Result: 在BSBM42和Spider4SPARQL数据集上的实验显示，S2CLite在Spider4SPARQL上的总解析准确率达到77.8%，相比最先进的S2CTrans的44.2%有显著提升。在共同解析的查询子集上，执行准确率达到96.6%，优于S2CTrans 7.3%。
...
Result: 在115种语言上的综合评估显示所提模型性能得到改善，进一步分析表明这种增强的评估能力还能显著提升下游任务表现。

Conclusion: MTQ-Eval框架成功扩展了LLM在多语言文本质量评估方面的能力，并证明这种改进能带来下游任务的显著提升。
...
TL;DR: 本文系统比较了LLM在开放式生成和多项选择任务中的自我纠正机制，发现两种范式在改进模式和失败模式上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 研究LLM自我纠正机制在不同任务结构（开放式生成vs多项选择）中的动态差异，以指导智能体应用的设计。

Method: 通过在不同自然语言理解和推理任务上比较不同规模和家族的LLM，分析性能趋势和错误纠正行为。

Result: 开放式生成受益于重新解释和组合优化的灵活性，而多项选择能利用更清晰的解决方案边界但受限于选项范围。
...
TL;DR: 提出AMaPO算法解决离线偏好优化中的过拟合-欠拟合困境，通过自适应边距动态调整学习力度，在多个基准测试中实现了更好的排序准确性和下游对齐性能。


<details>
  <summary>Details</summary>
Motivation: 离线偏好优化依赖排序准确性，但现有方法存在过拟合-欠拟合困境：对正确排序样本施加过多梯度（过拟合），对错误排序样本提供不足修正信号（欠拟合）。
...
TL;DR: 开发针对低资源语言（巴斯克语）的强大多模态大语言模型，证明仅需20%巴斯克语多模态数据即可获得良好性能，且无需巴斯克语指令调优的骨干LLM。


<details>
  <summary>Details</summary>
Motivation: 当前商业MLLMs在低资源语言上表现尚可，但开源社区尚未实现可比结果，需要为巴斯克语等低资源语言开发强大的MLLM。

Method: 构建自己的训练和评估图像-文本数据集，使用Llama-3.1-Instruct和巴斯克语适配的Latxa作为骨干LLM，探索不同数据混合比例进行训练。
...
TL;DR: CARE-Bench是一个动态交互式自动化基准，用于评估大型语言模型的心理咨询能力，基于真实案例构建多样化客户档案，采用多维度的专业心理学量表评估模型表现。


<details>
  <summary>Details</summary>
Motivation: 心理咨询需求增长与有限服务资源之间的不匹配促使研究将大型语言模型应用于该领域，但现有评估方法存在客户模拟不专业、评估格式静态、指标单一等问题，无法全面评估模型处理复杂客户情况的能力。

Method: 基于真实世界咨询案例构建多样化客户档案，按照专家指南进行模拟，创建动态交互式评估基准CARE-Bench，采用多维度的心理学量表进行性能评估。
...
Result: 微调模型在命名实体识别和关系提取任务上表现优异（NER: 80.6%, RE: 54.0%），显著优于LLM提示方法（NER: 44.4%, RE: 10.1%）。

Conclusion: 监督模型在学术信息提取领域明显优于无监督的LLM方法，表明像GSAP-ERE这样的数据集对于推进该领域研究至关重要。
...
TL;DR: 提出TPoT方法，通过语义筛选文本输入深度学习模型来预测大五人格特质，解决大语言模型输入限制并提升预测性能


<details>
  <summary>Details</summary>
Motivation: 从大量文本中预测个体人格特质具有挑战性，特别是当文本量很大时，需要解决输入文本限制问题

Method: 使用TPoT策略语义筛选与大五人格特质、方面或项目相关的文本，构建BIG5-TPoT深度学习模型

Result: 在Stream of Consciousness Essays数据集上，该方法改善了平均绝对误差和准确率指标

Conclusion: TPoT是一种简单有效的文本预选策略，能够提升人格特质预测性能
...
TL;DR: 本文研究自动文本简化(ATS)中可读性度量与自动评估指标、人工判断之间的相关性，发现在英语ATS中这三者之间相关性较低，需要明确定义ATS的构建概念。


<details>
  <summary>Details</summary>
Motivation: 当前自动文本简化研究中，可读性度量与自动评估指标、人工判断之间的相关性研究较少，需要填补这一研究空白。

Method: 通过研究可读性度量与人工判断之间的相关性，以及可读性度量与ATS评估指标之间的相关性，来探讨可读性度量在ATS中的位置。

Result: 研究发现，可读性度量与自动指标和人工判断之间普遍相关性较差，三个评估角度之间的相关性较低。

Conclusion: 由于自动文本简化的三个评估角度（可读性度量、自动指标、人工判断）相关性较低，需要明确定义ATS的构建概念。
...
Conclusion: 合成数据不仅能提升长上下文声明验证的性能，还能增强模型的可解释性，在幻觉检测和事实核查任务中具有重要价值。
...
TL;DR: 本文提出了一种使用LLM和AMR图将英语翻译成ASP程序的新方法，用于解决逻辑谜题，相比现有方法减少了LLM的依赖。


<details>
  <summary>Details</summary>
Motivation: 随着非编程人员需要与代码交互的需求增加，需要一种方法让不熟悉ASP的用户能够用自然语言描述逻辑问题并自动生成ASP程序。

Method: 使用LLM简化自然语言句子、识别关键词和生成简单事实，然后通过AMR图解析简化语言并系统生成ASP约束，最终构建完整的ASP程序。
...
<details>
  <summary>Details</summary>
Motivation: ARC-AGI是人类轻松解决但AI系统极其困难的流体智力基准测试，受神经科学和心理学中人类智力建模方法的启发，旨在开发认知合理的求解器。
...
TL;DR: QDIN是一种将强化学习系统设计为可查询推理引擎的统一架构，专门针对不同查询类型优化神经模块，实现准确的环境知识推理，即使控制性能不理想。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习专注于单一目标（最大化奖励），但训练好的智能体隐含了丰富的环境知识（可达性、距离、价值、动态等），现有架构无法有效暴露这些信息。

Method: 引入查询条件确定性推理网络(QDIN)，将不同查询类型（策略、可达性、路径、比较）作为一等公民，为每种推理模式优化专门的神经模块。

Result: 实验显示推理准确率可达近乎完美水平（99%可达性IoU），即使控制性能仍不理想（31%回报），表明准确世界知识所需的表征与最优控制所需不同。
...
<details>
  <summary>Details</summary>
Motivation: 传统POMDP求解器使用α向量表示价值函数，但每个α向量是|S|维的，在大规模问题中Bellman备份计算成本过高，导致方法不可行。
...
TL;DR: 提出单向认知优化(UCO)方法，通过多轮交互强化学习解决LLM作为智能导师时的动态适应问题，使用进展奖励和支架奖励来评估学生认知进步和识别最近发展区。


<details>
  <summary>Details</summary>
Motivation: 当前监督微调方法只能学习表面教学模式，缺乏动态适应能力；现有强化学习方法仅基于学生输出正确性评估教学效果，无法区分学生是否真正理解，也不能感知学生认知状态变化。

Method: 采用多轮交互强化学习范式，设计两个协同奖励函数：进展奖励捕捉学生认知进步，评估从困惑到理解的转变；支架奖励动态识别学生的最近发展区，鼓励教师在该区域内进行有效教学。

Result: 在BigMath和MathTutorBench基准测试中，UCO模型优于所有同等规模的基线模型，性能可与先进闭源模型相媲美。

Conclusion: UCO方法有效解决了LLM作为智能导师时的动态适应问题，通过认知状态感知和最近发展区识别实现了更有效的个性化教学。
...
TL;DR: Lumine是首个能够在3D开放世界中实时完成数小时复杂任务的通用智能体，采用端到端的视觉语言模型，在Genshin Impact中训练后能完成5小时主线剧情，并具有跨游戏零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 开发能够在复杂3D开放世界中完成长时间任务的通用智能体，统一感知、推理和行动，实现类似人类的交互范式。

Method: 采用端到端的视觉语言模型，以5Hz处理原始像素，生成30Hz的键盘鼠标动作，仅在必要时进行推理。在Genshin Impact中训练。

Result: 成功完成Genshin Impact中5小时Mondstadt主线剧情，效率与人类相当；在Wuthering Waves和Honkai: Star Rail中零样本完成100分钟和5小时任务。

Conclusion: Lumine在开放环境中展现出强大的跨世界泛化能力，是迈向通用智能体的重要一步。
...
TL;DR: 当前生物声学AI系统通过transformer架构和基础模型范式处理动物交流，但忽视了AI递归认知与物种递归交流相遇时的根本问题。作者认为AI不是中立的模式检测器，而是可能系统性地模糊或扭曲其他物种交流结构的递归认知代理。


<details>
  <summary>Details</summary>
Motivation: 现有生物声学AI方法忽视了AI系统自身递归认知处理与其他物种递归交流过程相遇时的根本问题，需要重新概念化AI在生物声学研究中的角色。

Method: 借鉴哲学家Yuk Hui关于递归性和偶然性的工作，分析AI系统作为递归认知代理如何可能系统性地模糊或扭曲其他物种的交流结构，并提出从通用模式识别转向不同递归认知形式间外交相遇的重新概念化。

Result: 识别出双重偶然性问题：每个物种的交流通过偶然的生态和进化条件产生，而AI系统通过自身偶然的架构和训练条件处理这些信号。

Conclusion: 解决这一挑战需要将生物声学AI重新概念化为不同递归认知形式之间的外交相遇，这对模型设计、评估框架和研究方法具有重要影响。
...
TL;DR: 构建了一个集成人工智能和大数据的业务流程优化模型，采用三层架构实现流程全生命周期的智能管理，显著提升企业运营效率。
...
Result: 实验验证显示：流程处理时间缩短42%，资源利用率提高28%，运营成本降低35%，系统在高并发负载下保持99.9%可用性。
...
TL;DR: AlphaCast是一个结合人类智慧与大语言模型智能的协同推理框架，将时间序列预测重新定义为交互式过程，通过分阶段协作提升预测准确性。
...
Method: 采用两阶段框架：自动化预测准备（构建多源认知基础）和生成推理与反思优化（触发元推理循环进行持续自我修正）。
...
TL;DR: 首次提出基于图神经网络的方法来近似计算ABA框架中的可信接受性，通过依赖图表示和两种GNN架构实现，在ICCMA基准上表现优于现有方法。
...
Method: 将ABA框架建模为依赖图，编码假设、声明和规则作为节点，使用异质边标签区分支持、推导和攻击关系。提出ABAGCN和ABAGAT两种GNN架构，分别使用残差异质卷积层和注意力层学习节点嵌入。

Result: ABAGCN和ABAGAT在ICCMA基准上优于最先进的GNN基线，节点级F1得分高达0.71。扩展重构算法在小ABA框架上F1超过0.85，在大型框架上保持约0.58。
...
TL;DR: 本文提出通过结构化多智能体管道实现渐进式、增量式、顺序式搜索来增强大语言模型的推理能力，并通过实验验证了递归精炼方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型表现出卓越的流畅性，但研究人员仍在努力提升其推理能力。基于搜索的计算解释，本文旨在开发一个系统性框架来理解和优化LLM推理。

Method: 采用递归精炼方法，包括自我批评、对抗压力测试和整合关键反馈，构建了简单线性管道与复杂结构化管道的对比实验，使用三位美国开国元勋的历史人物角色进行测试。

Result: 复杂模型在所有九个测试案例中均优于简单模型，平均仲裁得分为88.3 vs 71.7，在分析深度、结构细微差别和战略框架方面表现更优。

Conclusion: 递归精炼是通过GIS搜索增强LLM推理的有效架构特征，结构化多智能体管道能够实现更高质量的推理输出。
...
TL;DR: MAKER系统首次实现了超过100万步LLM推理且零错误的任务解决，通过极端任务分解和微代理投票纠错机制，突破了LLM在长程任务中的规模化瓶颈。


<details>
  <summary>Details</summary>
Motivation: LLMs在推理和工具使用方面取得突破，但由于持续错误率无法扩展到人类组织和社会级别的长程任务，需要解决规模化执行的问题。

Method: 采用极端任务分解策略，将任务拆分为可由专注微代理处理的子任务，通过多代理投票方案在每一步进行高效错误校正。

Result: 成功解决了超过100万步LLM推理的任务且零错误，理论上可扩展到更高水平。

Conclusion: 大规模分解代理过程（MDAPs）可能比持续改进现有LLMs更有效地解决组织和社级别的复杂问题。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在公共道路上部署时会面临各种驾驶危险，需要具备持续监控危险并适应性地应对潜在安全违规的能力，以在复杂场景中保持稳健驾驶行为。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习在自动驾驶紧急响应中存在样本效率低和动态场景适应性差的问题，需要更先进的方法来提升自主应急系统的性能。

Method: 分析了扩散模型增强强化学习（通过合成数据生成增强策略鲁棒性）和大型语言模型辅助上下文学习（提供轻量级、可解释的快速适应能力）两种新兴范式。

Result: 扩散模型增强强化学习提高了策略鲁棒性但计算成本增加，而大型语言模型辅助上下文学习提供了无需重新训练的快速适应能力。

Conclusion: 从生成式AI视角为理解下一代自主应急响应系统提供了关键框架，展示了不同方法在自动驾驶紧急服务中的优势和权衡。
...
TL;DR: OR-R1是一个数据高效的训练框架，用于自动化优化建模和求解，仅需1/10的数据即可达到67.7%的平均求解准确率，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的优化建模方法需要大量标注或合成数据，成本高且扩展性差，需要开发数据效率更高的解决方案。

Method: 采用两阶段训练：监督微调从有限标注数据学习推理模式，测试时组相对策略优化提升能力和一致性，利用稀缺标注和丰富未标注数据。

Result: 平均求解准确率达67.7%，仅需ORLM方法1/10的合成数据，超越ORLM达4.2%，在100个样本下仍超越2.4%。TGRPO带来3.1%-6.4%的额外提升。
...
TL;DR: 提出了历史感知推理（HAR）框架，通过反思错误和获取情景推理知识来增强GUI代理的短期记忆能力，解决了现有GUI代理在长时程任务中历史无关推理的问题。


<details>
  <summary>Details</summary>
Motivation: 当前GUI代理在显式推理中表现出弱短期记忆，将链式交互理解为离散的屏幕理解，缺乏对情景历史交互的感知，这限制了GUI自动化的性能。

Method: HAR框架包含构建反思学习场景、合成定制修正指南和设计混合RL奖励函数，开发了HAR-GUI-3B模型，将推理模式从历史无关转变为历史感知。
...
TL;DR: 提出了ProBench移动端基准测试，包含200多个GUI任务，不仅评估最终状态，还引入过程相关任务评估，通过过程提供器自动提供准确的过程信息来评估GUI代理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅通过检查最终屏幕状态来评估GUI代理，但GUI操作任务包含多个链式步骤，且关键信息不一定呈现在最后几页中，需要更全面的过程评估。

Method: 开发ProBench基准测试，包含状态相关任务和过程相关任务，设计专门的过程提供器自动提供准确的过程信息，对先进GUI代理进行全面评估。

Result: 评估发现先进GUI代理在真实世界GUI场景中存在显著局限性，这些缺陷普遍存在于大规模通用模型和较小的GUI专用模型中。

Conclusion: 详细的错误分析揭示了几个普遍问题，为未来改进指明了具体方向，过程评估对于全面理解GUI代理能力至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型(LRMs)如DeepSeek-R1和OpenAI o1经常生成包含冗余推理步骤的冗长响应(过度思考现象)，显著增加计算成本。现有方法通过长度惩罚来解决，但存在长度崩溃和训练崩溃问题。

Method: 提出训练简洁性奖励模型(CRM)的管道，对推理路径的简洁性进行评分；引入新颖的简洁性奖励函数(CRF)，明确结果奖励与简洁性评分之间的依赖关系。

Result: 在五个数学基准数据集上的实验显示，该方法在Qwen2.5-7B上实现了8.1%的准确率提升和19.9%的响应长度减少，并且在Llama和Mistral等其他LLMs上具有良好的泛化能力。

Conclusion: 该方法从理论上证明了新奖励在方差减少和改进收敛特性方面的优越性，在实践中证明了其有效性和标记效率，为解决过度思考问题提供了有效方案。
...
TL;DR: 提出了一个双层可靠性监控框架来解决智能AI系统在操作中的可靠性不足问题，包括分布外检测层和AI透明度层，为人类操作员提供决策支持。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统虽然能在各种应用中提供更有帮助的AI系统，但由于可靠性不足，特别是在高风险领域如医疗保健或流程工业中不适合使用。不可靠的系统在操作过程中存在意外行为的风险，需要缓解技术。

Method: 基于智能AI系统的特性推导出其在操作中的主要可靠性挑战，提出一个双层可靠性监控框架：分布外检测层用于检测新输入，AI透明度层用于揭示内部操作。

Result: 该框架为人类操作员提供了必要的决策支持，以判断输出是否可能不可靠并进行干预，为开发缓解技术以降低操作中不确定可靠性带来的风险奠定了基础。

Conclusion: 提出的双层监控框架为解决智能AI系统在操作中的可靠性挑战提供了有效方法，通过分布外检测和透明度机制增强了系统的可靠性和可干预性。
...
TL;DR: MedFuse框架通过乘法融合模块处理不规则临床时间序列，在多个预测任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 临床时间序列数据具有不规则性、异步采样、缺失值和异质特征动态等特点，现有嵌入策略通过加法操作结合特征身份和值嵌入，限制了捕捉值依赖特征交互的能力。

Method: 提出MedFuse框架，核心是MuFuse模块，通过乘法调制融合值和特征嵌入，保留特征特定信息的同时建模跨特征的高阶依赖关系。

Result: 在三个真实世界数据集上的实验表明，MedFuse在关键预测任务上持续优于最先进的基线方法，学习到的表示显示乘法融合增强了表达能力并支持跨数据集预训练。
...
TL;DR: 提出HyperD框架，通过解耦交通数据的周期性和残差分量来提升预测精度，使用混合周期表示模块和频率感知残差表示模块分别处理不同模式，并在四个真实数据集上取得SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 交通预测面临两大挑战：复杂的空间依赖性和多尺度周期模式与非周期波动的共存。现有方法难以同时有效处理这些复杂模式。

Method: HyperD框架将交通数据解耦为周期性和残差分量。周期性分量通过混合周期表示模块处理，使用可学习周期嵌入和时空注意力；残差分量通过频率感知残差表示模块在频域中使用复数MLP建模。引入双视图对齐损失确保语义分离。
...
TL;DR: 提出从"模型训练"转向"模型培养"的新范式，将价值对齐融入模型开发全过程，通过重构训练语料库实现从第一个训练token开始的价值承诺


<details>
  <summary>Details</summary>
Motivation: 当前AI训练方法只在核心能力建立后才进行价值对齐，导致模型易错位且缺乏深层次价值体系。在LLM能力开始超越人类的背景下，需要从根本上改变训练方式

Method: 重新设计训练语料库：采用第一人称视角重构训练数据、将信息重新情境化为生活经验、模拟社会互动、搭建训练数据排序的脚手架

Result: 预期这种训练语料库重构将实现从第一个训练token开始的价值承诺，使知识、技能和价值观更难分离

Conclusion: 在大型语言模型能力开始超越人类能力的生态系统中，这种从源头融入价值对齐的"模型培养"范式至关重要
...
TL;DR: 本文呼吁开发专门针对定性研究的AI系统，以解决当前通用AI工具在定性研究中的局限性，如偏见、不透明、不可复现和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI主要推动了定量方法的发展，而定性研究这一对意义建构和全面科学理解至关重要的维度却被忽视。定性研究者要么不使用AI，要么只能依赖存在明显缺陷的通用工具如ChatGPT。

Method: 通过文献回顾，分析现有自动化发现流程如何通过增强定性能力来改进，并识别安全定性AI在多学科和混合方法研究中的关键机会。

Result: 识别了开发专门定性AI系统的必要性，这些系统需要具备透明、可复现和隐私友好的特性。

Conclusion: 需要从零开始构建专门用于解释性研究的定性AI系统，以弥补当前AI在定性研究领域的不足，促进多学科和混合方法研究的发展。
...
TL;DR: 提出基于LLM的智能体框架用于屏障证书合成，通过自然语言推理来提出、改进和验证候选证书，结合LLM驱动的模板发现和SMT验证，在BarrierBench基准测试中实现超过90%的成功率。
...
Method: 采用LLM智能体框架，使用自然语言推理进行证书的提出、改进和验证，集成LLM驱动的模板发现与SMT验证，支持屏障-控制器协同合成。

Result: 在包含100个动态系统的BarrierBench基准测试中，该框架生成有效证书的成功率超过90%，展示了检索增强生成和智能体协调策略在提高可靠性和性能方面的效用。

Conclusion: 该工作成功展示了语言模型在形式验证领域的应用潜力，通过发布BarrierBench基准和工具链，为推进基于语言推理与动态系统形式验证的集成建立了社区测试平台。
...
TL;DR: 评估2025年三个前沿大语言模型在PDDL规划任务中的表现，GPT-5在标准PDDL领域表现与LAMA规划器相当，但在混淆测试中所有模型性能下降但比之前模型更轻微。
...
Result: GPT-5在标准PDDL任务中解决率与LAMA相当；所有LLM在混淆测试中性能下降但比之前模型更轻微；相比前代LLM有显著改进。

Conclusion: 前沿LLM在规划任务上取得实质性进步，缩小了与专业规划器的性能差距，特别是在标准PDDL领域表现接近专业规划器水平。
...
TL;DR: 提出了一种基于潜在流匹配和分类器自由引导的新方法，通过显式分离条件信息和残差表示来解缠潜在子空间，实现了对高维数据有意义特征的访问。


<details>
  <summary>Details</summary>
Motivation: 在科学发现中访问学习表示中的信息对于高维领域至关重要，需要一种能够分析、控制和重新利用潜在表示的机制。

Method: 基于潜在流匹配与分类器自由引导，显式分离条件信息和残差表示，从而解缠潜在子空间。

Result: 在三个实验（合成2D高斯玩具问题、彩色MNIST和Galaxy10天文数据集）中，该方法成功实现了对高维数据有意义特征的访问。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准测试主要关注高层次描述或视觉问答，而CrochetBench旨在评估模型在实际创造性领域中的程序能力，填补表面理解与可执行精度之间的差距。

Method: 采用CrochetPARADE DSL作为中间表示，支持结构验证和通过执行进行功能评估，涵盖针法分类、指令基础、自然语言到DSL翻译和图像到DSL翻译等任务。

Result: 在所有任务中，当评估从表面相似性转向可执行正确性时，性能急剧下降，暴露了模型在长距离符号推理和3D感知程序合成方面的局限性。

Conclusion: CrochetBench为评估多模态模型的程序能力提供了新视角，突显了在现实世界创造性领域中表面理解与可执行精度之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有的AI安全方法依赖于检查模型输出或激活，但某些风险仅通过检查无法检测，需要补充性的安全增强方法。

Method: 使用共识采样算法，给定k个模型和提示，实现与最安全的s个模型平均风险相当的风险水平，在模型间缺乏足够共识时弃权。算法利用模型计算输出概率的能力。

Result: 当足够多的模型安全且表现出足够一致性时，可以限制弃权概率。算法从已知安全模型子集的安全保证扩展到单个可靠模型。

Conclusion: 该方法提供了一种新的模型无关AI安全方法，通过聚合未知安全子集的模型来增强整体安全性，但需要安全模型间有一定重叠，且在所有模型都不安全时无法提供保护。
...
TL;DR: 本文从科学和系统角度阐述了物理人工智能的基本原理，提出了包含具身性、感官感知、运动行为、学习、自主性和情境敏感性的六个基本原则，这些原则构成一个闭环控制系统，将智能理解为身体、环境和经验之间真实交互的涌现现象。


<details>
  <summary>Details</summary>
Motivation: 传统AI方法依赖符号处理和数据驱动模型，而物理AI将智能理解为身体、环境和经验之间真实交互的涌现现象，旨在为智能系统的物理具身化、感官感知、行为能力、学习过程和情境敏感性建立统一的理论基础。

Method: 提出了六个基本原则：具身性、感官感知、运动行为、学习、自主性和情境敏感性，这些原则构成闭环控制系统，其中能量、信息、控制和情境持续交互。通过康复诊所中的自适应辅助机器人案例来说明理论模型。

Result: 建立了物理AI的理论框架，展示了六个基本原则不是松散的功能模块，而是作为闭环控制系统运作，使系统能够从物理经验而非数据库中生成意义。

Conclusion: 物理AI将智能理解为物理具身化过程，学习不是参数调整，而是智能体与环境之间结构耦合的变化，这一范式转变强调了从抽象计算到具身体验的智能理解方式。
...
<details>
  <summary>Details</summary>
Motivation: 对抗优化算法在多智能体设置中寻找鲁棒和多样化策略方面很成功，但在合作设置中会导致智能体非理性地自我破坏，阻碍任务完成和学习进程。

Method: 引入RPO形式化方法确保智能体保持理性，开发RPG算法在修改后的游戏版本中训练智能体最大化自身奖励，使用对手塑造技术优化对抗目标。

Result: RPG能够扩展现有对抗优化算法，不再受自我破坏限制，可以找到对抗样本、提高鲁棒性和适应性、学习多样化策略，在多个合作和一般和环境中表现良好。

Conclusion: RPO和RPG方法成功解决了对抗优化在合作设置中的自我破坏问题，为多智能体学习提供了有效的解决方案。
...
TL;DR: 比较两种逃离无信息启发式区域(UHR)的方法：广度优先搜索(BrFS)和重启随机游走(RRW)，分析其期望运行时间，并在EHC算法中实现比较


<details>
  <summary>Details</summary>
Motivation: 贪婪搜索方法如GBFS和EHC在面对启发式局部极小值或平台等无信息启发式区域时表现不佳，需要有效的逃离策略

Method: 理论推导BrFS和RRW逃离UHR的期望运行时间，开发EHC-RRW变体使用RRW替代BrFS，并在PDDL规划基准上进行实验比较

Result: 推导了基于UHR属性和随机游走过程的期望运行时间公式，识别了RRW比BrFS更快的条件，EHC-RRW在EHC有效的情况下具有强期望运行时间保证

Conclusion: 重启随机游走(RRW)在某些情况下比广度优先搜索(BrFS)能更有效地逃离无信息启发式区域，EHC-RRW变体具有更好的理论保证和实际性能
...
TL;DR: 提出了一个用于神经符号距离场学习的有限差分曲率正则化框架，用轻量级有限差分模板替代昂贵的二阶自动微分，在保持重建质量的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用二阶自动微分来强制曲率先验，虽然准确但计算成本高昂。其他方法避免显式Hessian组装但仍需高阶微分。需要更高效的曲率正则化方法。

Method: 使用基于泰勒展开的有限差分模板近似二阶导数，截断误差为O(h^2)，可作为高斯曲率和秩亏损失的直接替代方案。

Result: 实验表明有限差分变体达到与自动微分对应方法相当的重建保真度，同时GPU内存使用和训练时间减少高达两倍。在稀疏、不完整和非CAD数据上的测试证实了方法的鲁棒性和通用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有焦散透镜设计通常假设过于简化的点光源或平行光源，难以处理真实世界中的复杂表面光源照明模式。

Method: 使用优化点光源集拟合真实光源照明，采用可微分渲染框架进行物理模拟，应用收缩映射将约束问题转换为无约束问题，考虑通量一致性和法向可积性设计自由曲面透镜形状。

Result: 仿真和物理实验表明，该方法比点光源近似更准确地表示真实表面光源，产生的焦散透镜图像与目标光分布高度匹配。

Conclusion: 该方法能有效处理真实表面光源的复杂照明特性，为自由曲面透镜设计提供了更准确的解决方案。
