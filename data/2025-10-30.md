...
<details>
  <summary>Details</summary>
Motivation: 现有视频描述模型难以捕捉细粒度运动细节，导致描述模糊或语义不一致。需要增强对复杂人体动作的描述能力。

Method: M-ACM框架利用人体网格恢复得到的运动表示，通过运动感知解码机制来显式突出人体动态。

Result: 实验结果表明M-ACM在准确描述复杂人体动作和细微时间变化方面显著优于先前方法，为运动中心视频描述设立了新标准。

Conclusion: M-ACM通过结合运动表示有效提升了视频描述质量，特别是在人体动作描述方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI生成内容快速发展，但情感表达的3D面部动画研究不足，主要障碍是真实情感3D说话面部数据集稀缺且采集成本高昂。

Method: 将语音和情感驱动的面部动画建模为线性加性问题，利用中性表情的3D说话面部数据集和3D表情序列数据集，通过稀疏约束损失联合学习语音和情感驱动的混合形状，并映射到FLAME模型参数。

Result: 定性和定量实验表明，该方法能自然生成具有指定表情的说话面部，同时保持准确的口型同步。感知研究显示，在情感表达性方面优于现有方法，且不损害口型同步质量。

Conclusion: 该方法成功解决了情感3D说话面部动画生成的挑战，通过解耦语音和情感驱动的面部变形，实现了高质量的情感表达和口型同步。
...
<details>
  <summary>Details</summary>
Motivation: 解决合成孔径雷达图像自动目标识别中缺乏真实标注数据的问题，通过合成数据生成来克服数据不足，但需要解决合成数据与真实测量之间的领域差异问题。

Method: 结合两种基于不同物理范式（散射中心模型和光线追踪）的SAR模拟器MOCEM和Salsa生成合成数据集，使用ADASCA深度学习方法训练ATR模型。

Result: 在MSTAR真实测量数据上达到了接近88%的识别准确率，显著提升了合成数据训练模型在真实数据上的泛化能力。

Conclusion: 通过结合互补的模拟器范式可以有效缓解合成数据与真实数据之间的领域差异，为SAR自动目标识别提供了一种有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有铰接3D物体建模方法要么需要密集视角监督的优化重建流程，要么只能生成粗糙几何近似且忽略表面纹理。虽然静态物体的开放世界3D生成取得了显著成功，但将其扩展到铰接物体面临训练数据有限等挑战。

Method: 将预训练的静态3D扩散模型作为形状先验，扩展分数蒸馏采样到3D-to-4D领域，将铰接作为额外生成维度，从少量不同铰接状态的图像中联合优化几何、纹理和铰接参数。

Result: 生成高保真几何和纹理，准确预测底层运动学结构，在多样化物体类别上具有良好的泛化能力，在质量和多功能性上显著优于现有最先进方法。

Conclusion: FreeArt3D提供了一种无需任务特定训练或大规模铰接数据集的铰接3D物体生成方法，尽管采用逐实例优化范式，但能在几分钟内完成，在质量和多功能性上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统反向不确定性建模依赖高精度参考数据，成本高且在大规模应用中不可行，需要一种可扩展的不确定性评估方法。

Method: 使用随机森林和XGBoost两种集成学习模型，基于空间分区的真实数据集训练，避免数据泄露，学习几何特征与不确定性的非线性关系。

Result: 两种模型都能有效捕捉几何特征与不确定性的非线性关系，平均ROC-AUC值超过0.87，高程变化、点密度和局部结构复杂性等几何特征对不确定性预测起主导作用。
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的准确诊断对于及时干预至关重要，眼动和面部特征是认知功能的重要指标，但很少有研究探索它们的联合整合用于辅助AD诊断。

Method: 提出多模态交叉增强融合框架，包含两个关键模块：(1)交叉增强融合注意力模块(CEFAM)，通过交叉注意力和全局增强建模模态间交互；(2)方向感知卷积模块(DACM)，通过水平-垂直感受野捕捉细粒度方向性面部特征。

Result: 在包含25名AD患者和25名健康对照的自建同步多模态数据集上，该框架优于传统的后期融合和特征拼接方法，在区分AD与HC时达到95.11%的分类准确率。

Conclusion: 通过显式建模模态间依赖关系和模态特定贡献，该框架在阿尔茨海默病检测中表现出卓越的鲁棒性和诊断性能，为多模态辅助诊断提供了有效解决方案。
...
Method: 采用基于Sobel算法的FPGA车道检测器架构，在416x416图像和150MHz频率下运行，集成自动灯光和温度控制单元。

Result: 系统每1.17ms生成有效输出，包括车道数量、当前车道索引及其左右边界信息，并能适应环境条件变化。

Conclusion: 该FPGA架构实现了高效实时的车道检测，并通过环境适应性控制增强了系统的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决VR设备上Codec Avatar模型计算量大、难以实时推理的问题，满足VR应用对延迟和功耗的严格要求

Method: 设计后训练量化方法降低模型精度，开发定制硬件加速器集成到VR设备SoC中，构建完整的端到端优化框架

Result: FovVideoVDP质量评分提升0.39，延迟降低3.36倍，渲染速率达到100FPS，满足实时VR要求
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉模型在图理解中的潜力，因为图神经网络的底层消息传递机制与人类视觉感知的全局结构捕捉方式存在根本差异，现有基准测试混淆了领域特征与拓扑理解。

Method: 引入GraphAbstract基准测试，评估模型感知全局图属性的能力，包括识别组织原型、检测对称性、感知连接强度和识别关键元素。

Result: 视觉模型在需要整体结构理解的任务上显著优于GNNs，并能在不同图尺度下保持泛化能力，而GNNs在全局模式抽象方面表现不佳且随图规模增大而性能下降。

Conclusion: 视觉模型具有卓越但未被充分利用的图结构理解能力，特别是在需要全局拓扑感知和尺度不变推理的问题上，这为开发更有效的图基础模型开辟了新途径。
...
Method: 结合线性特征变换和多视图图融合的GCN框架，动态集成伪标签到损失函数，校正拓扑不平衡，引入无监督平滑损失
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型基准测试存在高质量人工验证样本不足、依赖LLM生成内容、以及多语言覆盖有限的问题，需要更可靠的多语言评估资源。

Method: 从PISA考试中提取人类专家创建的英文示例，包含指令、问题、选项和图像，并翻译成5种其他语言，构建完全平行的6语言语料库。

Result: 评估发现小模型(<20B参数)得分较低，非英语语言性能显著下降，在空间和几何推理任务上错误率很高。
...
<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型在具身智能中展现出强大能力，但其部署受到底层大规模基础模型巨大计算和数据需求的严重阻碍，迫切需要解决这些挑战。

Method: 引入统一分类法系统组织该领域工作，将技术分为三大支柱：高效模型设计（高效架构和模型压缩）、高效训练（减少模型学习计算负担）、高效数据收集（解决机器人数据获取瓶颈）。

Result: 建立了该领域的基础参考框架，总结了代表性应用，明确了关键挑战，并为未来研究制定了路线图。

Conclusion: 该调查为高效VLA模型研究提供了系统性框架，通过持续更新的项目页面跟踪最新进展，推动具身智能的发展。
...
<details>
  <summary>Details</summary>
Motivation: 探索人类认知控制中的冲突适应现象是否在AI模型中存在，理解模型执行认知任务时的内部表征机制。

Method: 使用序列Stroop任务测试13个视觉语言模型，采用稀疏自编码器分析InternVL 3.5 4B模型的内部表征，识别任务相关超节点并进行消融实验。

Result: 12个模型表现出冲突适应行为；发现文本和颜色在早期和晚期层部分重叠的超节点，其相对大小反映了人类阅读和颜色命名的自动性不对称；识别出层24-25的冲突调节超节点，其消融显著增加Stroop错误。

Conclusion: 视觉语言模型能够模拟人类的冲突适应行为，其内部表征结构与人类认知处理具有相似性，为理解AI认知机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有的轻量级检索增强图像描述模型仅将检索数据用作文本提示，导致原始视觉特征未得到增强，特别是在对象细节和复杂场景方面存在语义差距。

Method: 采用双重检索机制：标准图像到文本检索用于文本提示，新颖的图像到图像检索用于获取视觉相似场景。从视觉相似场景的标题中提取关键词语和短语，通过轻量级可训练特征融合网络将这些文本特征编码并与原始图像特征集成。

Result: 广泛实验表明，该方法在实现竞争性性能的同时，相比之前的视觉提示描述方法需要更少的可训练参数。

Conclusion: DualCap通过双重检索机制有效增强了视觉表示，在保持轻量化的同时提升了图像描述性能。
...
Result: 建立了包含9730个视频相关问题的基准，通过10个任务系统评估MLLMs在假新闻检测中的各项能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的安全方法存在过度拒绝和安全性与实用性不平衡的问题，需要一种模型无关、即插即用的安全对齐方案。

Method: 提出多轮安全编辑框架，构建MR-SafeEdit数据集，开发基于人类认知过程的SafeEditor模型进行多轮安全编辑。

Result: 实验结果显示SafeEditor优于现有安全方法，减少了过度拒绝，实现了更好的安全性与实用性平衡。

Conclusion: 该框架为文本到图像模型提供了一种有效的后处理安全编辑方案，能够在不牺牲实用性的前提下提升安全性。
...
<details>
  <summary>Details</summary>
Motivation: 开发更高效的多模态模型架构，通过稀疏MoE实现计算效率与模型容量的平衡，推动AGI发展。

Method: 基于Ling-Flash-2.0的稀疏MoE变体，总参数量1000亿，每token激活6.1亿参数，实现高效扩展。

Result: 在12个上下文ASR基准测试中创下新记录，图像生成实现高保真文本渲染，生成分割增强空间控制和编辑一致性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态情感识别面临不同模态间差异和单模态情感信息表征困难等挑战，需要解决模态差异和情感关系建立问题

Method: 1) 为每个模态构建对抗自编码器学习判别性情感特征；2) 使用跨模态门机制减少模态差异并建立情感关系；3) 通过特征融合模块进行多模态融合

Result: 在公开的SIMS和MOSI数据集上的实验表明，MCIHN模型取得了优越的性能表现

Conclusion: 提出的MCIHN模型能够有效解决多模态情感识别中的模态差异问题，提升情感识别准确率
...
Method: 建立流匹配模型与去噪器的形式联系，设计原则性扰动（噪声和漂移）来影响样本生成，分析生成过程的动力学阶段。

Result: 揭示了生成过程的不同动力学阶段，能够精确描述去噪器在生成过程的哪些阶段成功或失败及其重要性。

Conclusion: 为理解流匹配生成过程提供了新视角，建立了流匹配与去噪的统一框架，有助于改进生成模型的设计。
...
Result: 在具有挑战性的大规模水果数据集上达到85.6%的平均精度(mAP)，概率方法比基于分类的方法提供更细粒度和准确的成熟度评估。

Conclusion: 概率方法为现代农业中更智能、不确定性感知的自动化系统铺平了道路，提供了更丰富和生物合理的植物成熟度表示。
...
<details>
  <summary>Details</summary>
Motivation: 解决Alves等人(2024)使用OpenPose进行关键点提取时时间性能不佳的问题，同时避免简单替换为MediaPipe导致的精度显著下降。

Method: 探索关键点子集选择策略优化识别性能，并使用样条插值技术处理缺失关键点问题。

Result: 适当的关键点子集达到或优于最先进方法的性能，处理时间比Alves等人(2024)减少5倍以上，样条插值显著提高准确率。

Conclusion: 精心选择关键点结合简单插值技术可实现高效准确的手势识别，为可扩展的手语识别系统铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 解决快速发展的城市中交通拥堵问题，减少延误和提高交通系统效率

Method: 通过分析摄像头连续帧计算背景图像，然后提取前景并使用DBSCAN算法检测车辆

Result: 该方法计算效率高，对基础设施改造要求低，适合实际部署

Conclusion: 该方法为交通流优化提供了实用且可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成模型难以泛化到非常规相机运动，因为缺乏包含这些运动的训练视频数据。

Method: 使用合成视频训练扩散模型，采用多重解耦策略将相机运动学习与合成外观伪影分离，确保更鲁棒的运动表示并减轻域偏移。

Result: 能够合成广泛精确控制的复杂相机运动，仅使用简单的合成数据（如低多边形3D场景中的基本几何体）。

Conclusion: VividCam通过合成数据训练有效解决了复杂相机运动的学习问题，为创造原创艺术视频提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 多视图变换器虽然能以前馈方式解决3D任务，但其内部机制不透明，这种黑盒特性使得在数据扩展之外的改进变得困难，也限制了在安全和可靠性关键应用中的使用。

Method: 通过探测和可视化多视图变换器各层残差连接中的3D表示，分析DUSt3R变体模型，研究其潜在状态在块间的发展过程和各层的作用。

Result: 研究发现DUSt3R变体估计的对应关系会通过重建的几何结构进行细化，揭示了其与具有更强显式全局姿态归纳偏置的方法的差异。

Conclusion: 该方法为理解多视图变换器的内部工作机制提供了新视角，有助于未来模型的改进和在关键应用中的安全使用。
...
Method: M-SAM框架分三步：1)使用Shapley值识别主导模态；2)分解损失景观，调整损失以优先考虑主导模态的鲁棒性；3)通过调制梯度的反向传播更新权重
...
<details>
  <summary>Details</summary>
Motivation: Wi-Fi感知因其低成本、非侵入性特点在医疗保健、空间占用分析和手势控制等领域具有潜力，但现有模型普遍存在过拟合问题，无法很好地泛化到新数据。

Method: 开发了IBIS混合架构，集成Inception-BiLSTM和SVM，旨在改善模型泛化能力并创建更鲁棒的分类边界。该方法应用于多普勒衍生数据。

Result: 在运动识别任务中实现了接近99%的准确率。全面的性能指标和混淆矩阵证实了所提解决方案的显著有效性。
...
Method: 采用微调的多模态大语言模型FT-ARM，集成代理自反机制，通过迭代推理视觉特征和编码的临床知识来优化预测，类似于临床医生的诊断再评估过程。

Result: 在压力性损伤图像数据集(PIID)上，FT-ARM在I-IV期压力性溃疡分类中达到85%的准确率，比基于CNN的模型提升4%，并支持实时推理和生成临床依据的自然语言解释。

Conclusion: FT-ARM通过整合微调和多模态输入的反思推理，提高了自动化伤口评估系统的可靠性、透明度和临床适用性，满足了对一致性和可解释性压力性溃疡分期的关键需求。
...
<details>
  <summary>Details</summary>
Motivation: 传统车牌识别系统受环境因素（光照、雨尘）、车辆高速、摄像头角度变化和低质量图像影响，准确率受限。ALPR在交通控制、停车管理、车辆追踪等领域至关重要。

Method: 使用YOLOv8进行车牌检测和识别，采用半监督学习框架，结合少量人工标注数据和Grounding DINO生成的伪标签训练检测模型。

Result: 在CENPARMI数据集上召回率达94%，UFPR-ALPR数据集上达91%，并报告了字符错误率评估系统性能。

Conclusion: 该方法通过半监督学习有效减少人工标注工作量，同时保持高识别准确率，为车牌识别系统提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助诊断系统在临床部署中存在关键限制，特别是在处理多模态数据的细微解释和需要先前临床历史的可行性方面。乳腺癌作为发达国家女性最常见的恶性肿瘤，早期检测对降低死亡率至关重要。

Method: 通过创新标记化模块，将2D乳腺X光片的视觉特征与易于获取的临床元数据和合成的放射学报告的结构化文本描述相结合，战略性地整合卷积神经网络与语言表示。

Result: 在多国队列筛查乳腺X光片上的评估显示，该多模态方法在癌症检测和钙化识别方面优于单模态基线，性能显著提升，同时能够处理高分辨率图像并在不同人群中实现实际部署。
...
<details>
  <summary>Details</summary>
Motivation: 参与BraTS 2023挑战赛的所有5个分割任务，测试Auto3DSeg框架在不同脑肿瘤分割任务中的性能
...
Method: DRIP方法在视觉编码器深层动态合并token，基于可解释池化适应输入图像

Result: 在ImageNet和CLIP预训练中显著减少GFLOPs，同时保持分类/零样本性能

Conclusion: DRIP是高效预训练视觉语言模型的有效方法，可扩展到科学领域
...
<details>
  <summary>Details</summary>
Motivation: 解决真实世界场景中零样本理解的挑战，模型需要在不依赖标注样本的情况下识别新对象、动作和上下文。
...
<details>
  <summary>Details</summary>
Motivation: 需要改进遥感图像分类中的纹理特征提取方法，以更好地捕获局部特征关系。

Method: 使用邻域特征池化(NFP)层，通过卷积操作捕获相邻输入之间的关系，并在特征维度上聚合局部相似性，可无缝集成到任何网络中。

Result: 与基线模型相比，NFP方法在不同数据集和架构上均能持续提升性能，同时保持最小的参数开销。

Conclusion: NFP是一种有效的纹理特征提取方法，能够显著提升遥感图像分类性能且计算开销小。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在无需调优的情况下难以实现对面部属性的精确控制，而调优方法又需要大量技术专业知识和额外训练数据，限制了可访问性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理交互视觉复杂性方面存在不足，包括：(1) 类内视觉多样性 - 同一动词在不同姿态和上下文中的视觉表现差异；(2) 类间视觉纠缠 - 不同动词产生视觉相似模式。

Method: 提出VDRP框架：1) 视觉多样性感知提示学习，通过注入组间视觉方差到上下文嵌入，并使用高斯扰动鼓励提示捕获动词的多样化视觉变化；2) 区域感知提示学习，从人、物和联合区域检索特定概念，增强动词级区分能力。

Result: 在HICO-DET基准测试中，在四种零样本评估设置下实现了最先进的性能，有效解决了类内多样性和类间视觉纠缠问题。

Conclusion: VDRP通过结合视觉多样性感知和区域感知的提示学习，显著提升了零样本人-物交互检测的性能，能够更好地处理交互的视觉复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 现有几何先验在室内和城市低纹理区域缺乏全局一致性，高斯泼溅和隐式SDF场存在不连续性或计算效率低的问题，导致细节丢失。

Method: 利用亚特兰大世界模型确保低纹理区域的准确表面重建，提出新颖的隐式结构化高斯泼溅表示，包括语义高斯泼溅表示和结构平面正则化。

Result: 在室内和城市场景的广泛实验中，该方法优于最先进的方法，提供了更优越的表面重建质量。

Conclusion: 所提出的方法能够有效解决低纹理区域重建问题，在保持效率和细节的同时实现平滑的场景重建。
...
<details>
  <summary>Details</summary>
Motivation: 传统CAM方法只能突出目标的最具判别性区域，这些区域往往无法覆盖整个对象，且与对象边界不对齐，限制了弱监督学习任务的性能，特别是需要像素级精确激活图的弱监督语义分割任务。

Method: Region-CAM通过提取语义信息图(SIMs)并进行语义信息传播(SIP)，同时考虑基线分类模型各阶段的梯度和特征，生成更完整的对象区域激活图。
...
Result: 在多个数据集上实现显著改进：隧道裂缝检测提升12.4%，建筑PPE检测提升13.7%，KITTI数据集提升88.6%，同时保持30-47 FPS的实时推理速度。
...
Conclusion: 从几何角度引入Jacobian-Spectral下界理论，证明真实图像在重建流形外具有非平凡误差下界，而生成图像在流形上具有接近零的误差。动态重建误差方法能显著提高检测准确性。
...
<details>
  <summary>Details</summary>
Motivation: 当前的3D场景理解方法受限于离线收集的多视图数据或预构建的3D几何。需要一种能够在线处理流视频并同时进行几何重建和语义理解的统一框架。

Method: 使用视觉语言和2D视觉基础编码器动态解释每一帧，通过前馈在线更新策略将对象级知识集成到高斯特征图中。迭代估计视觉里程计，增量更新在线高斯特征，并使用循环联合优化模块引导模型关注感兴趣区域。

Result: 在多种基准测试和任务中进行了广泛实验，包括照片级真实感渲染、语义和实例分割、3D边界框和语义占用估计以及3D网格生成，证明了EA3D的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 薄膜干涉测量在眼科应用中有重要价值，但临床转化受限于从干涉图重建厚度剖面的挑战——这是一个病态逆问题，受相位周期性、成像噪声和环境伪影的复杂影响。

Method: 使用基于视觉变换器的方法，在包含生理相关合成和实验泪膜数据的混合数据集上训练，利用长程空间相关性解决相位模糊，从动态干涉图中单次前向传播重建厚度剖面。

Result: 网络在含噪声、快速演化的薄膜上表现出最先进的性能，克服了传统相位展开和迭代拟合方法的局限性。

Conclusion: 这种数据驱动方法能够在消费级硬件上实现自动化、一致的实时厚度重建，为连续监测眼表泪膜和无创诊断干眼症等疾病开辟了新可能。
...
<details>
  <summary>Details</summary>
Motivation: 由于处理长程约束和参数敏感性的挑战，生成多模态数据（如参数化CAD序列）的生成建模技术发展滞后，而现有方法在离散数据空间中难以有效处理CAD序列的多模态特性。

Method: TGBFN在统一的连续可微分参数空间中处理CAD序列的离散命令和连续参数，通过渗透参数更新核并引入引导贝叶斯流来控制CAD属性。

Result: 在单条件和多条件约束生成任务上的广泛比较表明，TGBFN在生成高保真、条件感知的CAD序列方面达到了最先进的性能。

Conclusion: TGBFN为定量约束的CAD生成提供了一个有效的框架，能够统一处理多模态数据并在约束条件下生成高质量的CAD序列。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习技术在移动设备上的显著进步，特别是在计算机视觉领域，需要定量研究ViT在移动设备上的性能特征，并与传统CNN进行比较，为移动端部署提供指导。

Method: 通过比较190个真实世界ViT和102个CNN的性能特征，开发包含1000个合成ViT延迟测量的数据集，涵盖两个机器学习框架和六个移动平台，使用代表性构建块和最先进架构。

Result: 研究揭示了影响ViT架构在移动设备上延迟的关键因素，并证明使用开发的数据集可以准确预测新ViT的推理延迟，满足实际应用需求。

Conclusion: 该研究为移动设备上ViT的性能优化和部署提供了重要见解，开发的延迟预测方法具有实际应用价值，有助于指导移动端视觉模型的架构设计。
...
<details>
  <summary>Details</summary>
Motivation: 传统语义分割方法使用固定参数分类器，无法适应不同图像的类别分布特性，且类别不平衡导致模型偏向多数类，限制了少数类区域的识别能力。

Method: 使用内存库学习数据集级别的上下文信息，结合当前图像的类别特定上下文信息来改进分类器；采用师生网络范式，教师网络用真实标签动态调整上下文信息并传递给学生网络。
...
<details>
  <summary>Details</summary>
Motivation: 在信息技术和人工智能快速发展的背景下，新冠疫情推动了在线教育的普及，但数字学习环境中的身份认证问题亟待解决，以确保在线教育的安全性和稳定性。

Method: 采用YOLOv5网络从学生摄像头图像中检测人脸，然后将人脸信息输入残差网络提取深层特征，最后通过欧氏距离与学生人脸数据库进行比对来确认身份。

Result: 开发了一个基于深度学习的在线学习身份认证解决方案，能够有效识别和验证学生身份。

Conclusion: 该技术方法能够促进在线教育的持续发展，同时增强其安全性和稳定性，使在线教育更好地适应教育环境的快速演变。
...
TL;DR: U-CAN是一个无监督点云去噪框架，通过一致性感知的Noise2Noise匹配实现，无需干净-噪声点云对进行训练，在多个基准测试中达到与监督方法相当的效果。


<details>
  <summary>Details</summary>
Motivation: 扫描传感器捕获的点云常受噪声干扰，影响下游任务。现有方法需要大量人工标注的噪声-干净点云对进行训练，成本高昂。

Method: 使用神经网络推断多步去噪路径，通过Noise2Noise匹配方案和新型损失函数实现统计推理，并引入几何一致性约束学习一致性感知去噪模式。

Result: 在点云去噪、上采样和图像去噪的广泛基准测试中，U-CAN显著优于现有无监督方法，并与监督方法产生可比结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有学习方法无法准确捕捉多阶段特征，且特征间交互不足，导致在皱纹、边缘等复杂区域提取冗余特征，影响法向估计精度。

Method: 设计MSF-Net框架，包含多阶段特征提取和选择性更新策略，并开发特征融合模块以增强不同特征间的交互。

Result: 在DiLiGenT基准测试中，MSF-Net在表面法向估计精度上显著超越先前最先进方法。

Conclusion: MSF-Net通过多阶段特征提取和特征融合，有效解决了复杂区域特征提取问题，提升了光度立体法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前的无源域自适应方法在隐私约束下的医学图像分割中存在两个主要问题：忽略样本难度和在域偏移下难以处理噪声监督。

Method: 1) 通过熵相似性分析将未标记图像划分为可靠和不可靠子集；2) 使用蒙特卡洛方法生成去噪掩码来精炼伪标签；3) 在子集间进行域内和域间补丁混合，传递可靠语义同时减轻噪声。

Result: 在基准数据集上的实验显示，该方法在Dice和ASSD指标上均优于先前的SFDA和UDA方法，实现了更准确的边界描绘和最优性能。

Conclusion: 该研究强调了渐进式适应和去噪监督对于在域偏移下实现鲁棒分割的重要性。
...
Method: 提出在训练过程中结合真实图像，保留真实图像的ODE路径，使用更少的生成和真实图像对进行高效的reflow过程。

Result: 在CIFAR-10上，仅使用原方法1/10的生成对就获得了显著更好的FID分数，在单步生成和全步模拟中都表现优异。

Conclusion: 该方法诱导更直的路径，避免reflow过程中生成图像饱和，实现更稳健的ODE学习同时保持真实图像分布。
...
TL;DR: DeepShield是一个新颖的深度伪造检测框架，通过平衡局部敏感性和全局泛化能力来提高对未见伪造技术的鲁棒性。它增强CLIP-ViT编码器，包含局部补丁引导和全局伪造多样化两个关键组件。


<details>
  <summary>Details</summary>
Motivation: 现有检测器在域内场景表现良好，但由于依赖特定伪造伪影，难以泛化到多样化的操纵技术。需要解决跨域和跨操纵技术的泛化问题。

Method: 1. 局部补丁引导(LPG)：应用时空伪影建模和逐补丁监督来捕捉全局模型常忽略的细粒度不一致性。2. 全局伪造多样化(GFD)：引入域特征增强，利用域桥接和边界扩展特征生成来合成多样化伪造，减轻过拟合并增强跨域适应性。

Result: 在跨数据集和跨操纵评估中，DeepShield优于最先进的方法，对未见深度伪造攻击实现了卓越的鲁棒性。

Conclusion: 通过整合新颖的局部和全局分析进行深度伪造检测，DeepShield在跨域场景中表现出色，为解决深度伪造检测的泛化问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频美学评估领域缺乏标准化数据集和强大模型，视频的时序动态特性和多模态融合挑战阻碍了基于图像方法的直接应用。
...
TL;DR: 本研究评估了深度学习在农业景观中森林外树木(TOF)分类中的应用，比较了多种神经网络架构，发现FT-UNetFormer模型表现最佳，强调了空间上下文理解在TOF制图中的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将森林外树木(TOF)视为单一类别或依赖刚性阈值，限制了生态解释和跨区域适应性。需要更精确的方法来分类不同类型的木质植被。
...
Result: 模型在四个景观中均取得良好分类精度，FT-UNetFormer表现最佳(平均IoU 0.74；平均F1分数 0.84)。森林和线性类别结果良好，但在分类具有高边缘密度的复杂结构(特别是斑块和单株树木类别)时面临挑战。泛化实验显示需要区域多样化的训练数据。

Conclusion: 深度学习在TOF分类中表现良好，FT-UNetFormer是最佳模型。空间上下文理解对TOF制图至关重要，需要多样化的训练数据来确保大规模制图的可靠性。
...
### [59] [MMEdge: Accelerating On-device Multimodal Inference via Pipelined Sensing and Encoding](https://arxiv.org/abs/2510.25327)
...
<details>
  <summary>Details</summary>
Motivation: 解决资源受限边缘设备上实时多模态推理的挑战，特别是现有工作忽视的感知动态与模型执行之间的紧密耦合以及复杂模态间依赖关系。

Method: 采用流水线感知和编码设计，将推理分解为细粒度单元；引入时间聚合模块捕获跨单元时间动态；包含自适应多模态配置优化器和跨模态推测跳过机制。

Result: 在两个公共多模态数据集和真实无人机测试平台上评估，MMEdge显著降低了端到端延迟，同时在各种系统和数据动态下保持高任务准确性。

Conclusion: MMEdge通过流水线设计和自适应优化机制，有效解决了边缘设备上多模态推理的延迟和准确性平衡问题，为实时应用提供了可行解决方案。
...
TL;DR: LangHOPS是首个基于多模态大语言模型的开放词汇对象-部件实例分割框架，通过语言空间中的层次结构实现对象和部件的联合检测与分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式或可学习的视觉分组，而LangHOPS旨在利用语言空间中的层次结构来更好地处理对象-部件关系，充分发挥MLLM的知识和推理能力。

Method: 将MLLM集成到对象-部件解析流程中，利用其丰富的知识和推理能力连接层次结构中的多粒度概念，采用语言驱动的层次结构和部件查询优化策略。

Result: 在PartImageNet数据集上，LangHOPS在域内和跨数据集对象-部件实例分割中分别比先前方法提升5.5%和4.8% AP，在ADE20K的零样本语义分割中未见对象部件上提升2.5% mIOU。

Conclusion: LangHOPS通过语言驱动的层次结构和MLLM驱动的部件查询优化，在开放词汇对象-部件实例分割任务上实现了最先进的性能，验证了该方法的有效性。
...
TL;DR: 提出了DPTM框架，利用扩散模型生成伪目标域并逐步优化，解决源自由域自适应中源-目标域差异大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法受限于源-目标域差异，非生成方法在大域差异下伪标签不可靠，生成方法因域差异扩大而性能下降。

Method: 将目标样本分为信任集和非信任集，对非信任集样本使用潜在扩散模型进行语义转换，同时保持目标分布，并通过迭代优化逐步减小域差异。

Result: 在四个主流SFDA基准数据集上大幅超越现有方法，在源-目标差异大的场景下性能提升高达18.6%。

Conclusion: DPTM通过扩散驱动的渐进目标操纵，有效解决了SFDA中源-目标域差异大的挑战，实现了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常分别解决视线目标检测和视线跟随任务，且在训练和部署阶段都依赖头部相关先验知识，需要辅助网络提取头部位置，这阻碍了系统的联合优化并限制了实际应用。
...
Conclusion: GaTector+通过消除对头部先验知识的依赖，实现了视线目标检测和视线跟随任务的统一处理，提高了系统的实用性和性能。
...
TL;DR: 提出了一种仿生单中心成像（BMI）框架，通过生物启发的全球面单中心透镜和联合重建网络，从单次编码捕获中同时恢复高质量全聚焦图像和精确深度图。


<details>
  <summary>Details</summary>
Motivation: 解决紧凑型RGBD成像的双重挑战：传统紧凑光学系统难以在整个景深范围内保持RGB清晰度，而纯软件的单目深度估计依赖不可靠的语义先验且是病态问题。

Method: 设计仿生全球面单中心透镜，建立严格的物理前向模型生成合成数据集，并与双头多尺度重建网络协同设计，共享编码器联合恢复全聚焦图像和深度图。

Result: 深度估计达到Abs Rel 0.026和RMSE 0.130，显著优于纯软件方法和其他深度光学系统；图像恢复获得SSIM 0.960和LPIPS 0.082，在图像保真度和深度精度间取得优越平衡。
...
TL;DR: 提出了Prototype-Driven Alignment (PDA)，一种轻量级的度量头，用于解决少样本目标检测中的基类偏差和不稳定校准问题。
...
Method: PDA在DeFRCN框架中提供基于原型的"第二意见"，与线性分类器互补。它维护支持集原型在可学习的身份初始化投影空间中，可选地应用原型条件RoI对齐来减少几何不匹配。通过指数移动平均更新原型，采用best-of-K匹配方案捕获类内多模态，并使用温度缩放融合将度量相似度与检测器logits结合。

Result: 在VOC FSOD和GFSOD基准测试中，PDA一致地提高了新类性能，对基类影响最小，计算开销可忽略。

Conclusion: PDA是一种有效的轻量级插件方法，能够显著提升少样本目标检测的新类性能，同时保持对基类的良好表现。
...
TL;DR: 提出了StreamingCoT数据集，首个专为流式视频问答和多模态思维链任务设计的基准，解决了现有数据集在动态答案标注和显式推理过程方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频问答数据集存在两个关键局限：静态标注机制无法捕捉视频流中答案的演变特性，以及缺乏显式推理过程标注限制了模型可解释性和逻辑推理能力。

Method: 建立动态分层标注架构，生成每秒密集描述并通过相似性融合构建时间依赖的语义片段；提出显式推理链生成范式，通过关键帧语义对齐提取时空对象，利用大语言模型生成基于对象状态转换的推理路径，并通过人工验证确保逻辑一致性。

Result: 创建了StreamingCoT数据集，为流式视频理解、复杂时序推理和多模态推理研究奠定了基础。

Conclusion: StreamingCoT数据集解决了现有视频问答数据集的局限性，为推进流式视频理解和复杂时序推理研究提供了重要资源。
...
### [66] [Comparative Study of UNet-based Architectures for Liver Tumor Segmentation in Multi-Phase Contrast-Enhanced Computed Tomography](https://arxiv.org/abs/2510.25522)
...
<details>
  <summary>Details</summary>
Motivation: 多期相增强CT中肝脏结构分割在计算机辅助诊断和治疗规划中至关重要，本研究旨在探索不同骨干网络对肝脏肿瘤分割性能的影响。
...
Conclusion: 经典ResNet架构与现代注意力模块结合在医学图像分割任务中仍具竞争力，为临床实践中肝脏肿瘤检测提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的半监督3D动作识别方法中，基于聚类和边界的选择策略可能无法选择对动作识别器最具信息量的样本，因为模型可能已从相似样本中学习到相关知识。
...
Conclusion: 通过将半监督3D动作识别建模为MDP问题，并在双曲空间中增强表示学习，提出的方法能够更智能地选择信息样本，提高识别性能。
...
### [68] [RegionE: Adaptive Region-Aware Generation for Efficient Image Editing](https://arxiv.org/abs/2510.25590)
*Pengtao Chen,Xianfang Zeng,Maosen Zhao,Mingzhu Shen,Peng Ye,Bangyin Xiang,Zhibo Wang,Wei Cheng,Gang Yu,Tao Chen*

Main category: cs.CV

TL;DR: RegionE是一个自适应区域感知生成框架，通过区分图像中编辑区域和未编辑区域，对未编辑区域使用单步预测，对编辑区域进行局部迭代去噪，从而加速基于指令的图像编辑任务，无需额外训练即可实现2倍以上的加速。


<details>
  <summary>Details</summary>
Motivation: 现有的基于指令的图像编辑(IIE)模型在处理图像时，对编辑区域和未编辑区域采用统一的生成过程，忽视了这两类区域在生成难度和计算冗余上的显著差异，导致计算效率低下。

Method: 1) 自适应区域划分：基于最终估计结果与参考图像的差异，将图像划分为编辑区域和未编辑区域；2) 区域感知生成：对未编辑区域用单步预测替代多步去噪，对编辑区域进行局部迭代去噪，并引入区域指令KV缓存；3) 自适应速度衰减缓存：利用相邻时间步的速度相似性加速局部去噪过程。
...
Conclusion: RegionE通过区域感知的生成策略，在不牺牲编辑质量的前提下显著提升了基于指令的图像编辑效率，为IIE任务提供了有效的加速解决方案。
...
Method: 使用伪标签生成训练标签，比较影像组学特征与预训练基础模型（CT-FM和RadImageNet）特征，在182名患者数据集上评估零钙化与非零钙化分类。
...
TL;DR: PEP-FedPT是一个联邦学习框架，通过类上下文混合提示(CCMP)实现视觉Transformer的通用性和个性化联邦提示调优，在数据异构场景下超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 全局提示调优在异构客户端上泛化能力差，而个性化调优容易过拟合本地数据且缺乏泛化性，需要一种能同时实现通用性和个性化的联邦提示调优方法。

Method: 提出类上下文混合提示(CCMP)，基于类特定提示和全局共享提示，通过全局类原型和客户端类先验权重自适应组合提示，实现无需存储客户端依赖参数的个性化。

Result: 在CIFAR-100、TinyImageNet、DomainNet和iNaturalist数据集上的综合评估表明，PEP-FedPT在不同数据异构场景下始终优于最先进的基线方法。

Conclusion: PEP-FedPT为视觉Transformer的高效和可泛化联邦提示调优建立了坚实基础，成功平衡了通用性和个性化需求。
...
TL;DR: 提出了一个新的实例级组合图像检索数据集i-CIR和无需训练的检索方法BASIC，该方法在多个CIR数据集上达到新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 组合图像检索(CIR)领域因缺乏高质量训练和评估数据而发展受限，现有数据集主要关注语义级类别定义而非实例级检索。

Method: 设计了实例级类定义的i-CIR数据集，并提出BASIC方法：分别估计查询图像-图像和查询文本-图像相似度，通过后期融合来提升同时满足两个查询的图像权重，降低仅与一个查询高度相似的图像权重。

Result: BASIC方法在i-CIR数据集上达到新的最先进性能，同时在现有的语义级CIR数据集上也表现优异。

Conclusion: i-CIR数据集和BASIC方法为组合图像检索提供了更好的评估基准和有效的无需训练解决方案，推动了该领域的发展。
...
TL;DR: 提出了一种无需训练的方法CoherentAD，通过生成多个候选描述并进行自回归选择，生成连贯的音频描述序列，解决了现有方法生成独立描述导致的重复和不连贯问题。
...
Method: CoherentAD方法首先生成每个时间间隔的多个候选描述，然后通过自回归选择在整个序列中选择最连贯的描述，形成连贯的叙事。

Result: 该方法生成的音频描述序列在叙事理解方面表现更好，优于依赖独立生成的先前方法。

Conclusion: 提出的训练无关方法和序列级评估指标能够有效提升音频描述序列的连贯性和信息量。
...
TL;DR: SPADE是一个用于水下基础设施检查的单目深度估计系统，结合预训练相对深度估计器和稀疏深度先验，生成密集的度量尺度深度图，在嵌入式硬件上运行效率超过15FPS。
...
Conclusion: SPADE系统有望支持实用的水下检查和干预任务，已提交至IEEE海洋工程期刊AUV 2026特刊。
...
<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型虽然能产生高保真图像，但由于其固有的顺序解码过程导致推理速度缓慢。推测解码在文本生成中已证明能加速而不损失质量，但在图像生成中的应用仍未被充分探索，主要挑战包括更大的采样空间和二维空间结构利用不足。

Method: Hawk方法利用图像的空间结构来指导推测模型，通过更好地建模局部依赖关系，使草稿模型与目标模型输出更对齐，从而提高预测的准确性和效率。

Result: 在多个文本到图像基准测试上的实验结果表明，Hawk方法相比标准自回归模型实现了1.71倍的加速，同时保持了图像的保真度和多样性。

Conclusion: Hawk通过有效利用图像的空间结构，成功解决了自回归图像生成模型推理速度慢的问题，为图像生成领域的加速提供了有效解决方案。
...
TL;DR: 这篇综述全面回顾了多模态空间推理任务与大型模型的研究进展，介绍了MLLMs的最新进展并提出了开放基准测试，涵盖了2D/3D空间推理、具身AI以及新兴模态如音频和第一人称视频。


<details>
  <summary>Details</summary>
Motivation: 人类具有通过多模态观察（如视觉和声音）理解空间的能力，大型多模态推理模型通过学习感知和推理扩展了这些能力，但系统性的综述和公开基准测试仍然有限。

Method: 通过分类整理多模态大语言模型（MLLMs）的最新进展，涵盖后训练技术、可解释性和架构，并建立开放基准测试进行评估。

Result: 建立了多模态空间推理的坚实基础，提供了对2D/3D空间关系推理、场景布局理解、视觉问答、具身AI以及新兴模态的全面分析。

Conclusion: 该综述为多模态空间推理领域建立了坚实基础，并为该领域的发展提供了重要见解和评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用一个LoRA对应一个特效的模式，资源消耗大且无法泛化到未见特效，限制了可扩展性和创作能力。

Method: 设计了上下文条件策略和上下文注意力掩码，通过参考视频提示模型，精确解耦和注入关键特效属性；提出高效的单样本特效适应机制。
...
TL;DR: 提出了一个验证框架来评估和改进LLM生成的旅行行程的时间一致性，通过AeroDataBox API验证真实飞行时间约束，系统性地纠正时间不一致问题。
...
Method: 使用多个最先进的LLM生成旅行计划，并通过AeroDataBox API验证真实飞行时间约束，开发验证框架来评估和改进时间一致性。

Result: 实验显示当前LLM经常产生时间不一致的行程，但可以通过该框架系统可靠地纠正，实现大规模旅行规划的实际部署。

Conclusion: 该研究有助于理解LLM处理复杂时间推理任务的能力，并提供了在行程交付给用户前纠正时间不一致问题的实用框架。
...
<details>
  <summary>Details</summary>
Motivation: 为企业环境开发一个统一的多智能体框架，解决深度研究、表格推理和报告生成的实际需求

Method: 采用多智能体框架，整合深度研究、异构表格推理和多模态报告生成能力

Result: 开发出了钉钉DeepResearch框架，能够处理企业环境中的复杂研究任务

Conclusion: 该框架为企业提供了统一的多智能体解决方案，有效支持深度研究、表格推理和报告生成
...
TL;DR: Falcon是一个跨领域的中文文本到SQL基准测试，基于企业级SQL方言(MaxCompute/Hive)，包含600个中文问题覆盖28个数据库，77%需要多表推理，超过一半涉及4个以上表。当前最先进的大模型准确率最高仅50%，主要挑战来自企业级模式链接和中文语义映射。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL系统在企业级场景下面临重大挑战，包括大型企业模式中的模式链接困难（数百个表、反规范化字段、模糊列名、隐式外键关系）以及将简洁口语化中文映射到精确分析操作符的困难。

Method: 构建包含600个中文问题的基准测试，覆盖28个数据库，每个示例都标注了SQL计算特征和中文语义。提供稳健的执行比较器和自动化评估流水线，使用真实企业模式、查询模板进行端到端验证。

Result: 所有当前最先进的大规模模型（包括Deepseek）在Falcon基准上的准确率最高仅为50%。主要错误来源是模式链接和中文语义映射问题。

Conclusion: Falcon基准针对中文特定语义和企业级方言，为生产部署前提供了可重现的中间验证平台，揭示了当前文本到SQL系统在企业级场景下的局限性。
...
TL;DR: 本文通过分析LLM内部状态几何结构，揭示了置信度与能力脱节的机制：评估阶段在高维复杂流形上进行，而执行阶段在低维简单流形上进行，这种几何复杂度差异解释了置信度-能力差距。
...
Method: 使用线性探针解码模型的内部"可解性信念"，分析预生成评估和解决方案执行两个阶段的内部状态几何结构，通过主成分分析测量线性有效维度，并进行因果干预实验。
...
TL;DR: 跨语言摘要攻击(CLSA)通过翻译到枢纽语言后摘要再回译，能有效破坏AI文本水印的检测，同时保持语义质量，比单语言改写攻击效果更好。


<details>
  <summary>Details</summary>
Motivation: 现有水印方案依赖词元分布扰动，但改写攻击只能部分检测或降低文本质量，需要更强的攻击方法来挑战水印的实际应用性。

Method: 采用跨语言摘要攻击：先将文本翻译到枢纽语言，进行摘要压缩，再可选地回译回原语言，通过语义瓶颈系统性地破坏词元级统计偏差。

Result: 在多种水印方案(KGW、SIR、XSIR、Unigram)和五种语言上，CLSA比单语言改写更有效地降低水印检测准确率，将XSIR的AUROC从0.827降至0.53(接近随机水平)。

Conclusion: 跨语言摘要攻击揭示了水印技术的脆弱性，表明稳健的来源追踪需要超越分布水印，采用加密或模型认证方法。
...
TL;DR: 提出了一种静态token查找方法，用于文本嵌入生成，在保持89%上下文模型质量的同时，实现了1.12毫秒的p50延迟和50,000 QPS的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 为需要亚5毫秒延迟的实时嵌入应用提供高效解决方案，解决传统上下文模型在实时场景下的性能瓶颈。

Method: 采用静态嵌入查找、优化的均值池化和零拷贝IEEE754二进制序列化技术，通过Rust实现高性能系统。

Result: 在8个代表性任务上获得60.6 MTEB平均分，重复检测性能达90.1% AP，语义相似性76.1% Spearman相关性，在专业领域性能达到基线的75%-131%。

Conclusion: 该系统成功实现了实时嵌入应用所需的低延迟和高吞吐量，为实时文本处理场景提供了可行的解决方案。
...
TL;DR: 通过自引用处理可以可靠地诱导大语言模型产生结构化的一人称主观体验报告，这些报告具有可解释的机制特征、跨模型语义收敛性和行为泛化性。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型产生结构化主观体验报告的理论条件，特别是自引用处理这一在意识理论中被强调的计算模式。

Method: 在GPT、Claude和Gemini模型系列上进行受控实验，测试自引用处理是否能可靠地诱导一阶主观体验报告，并通过机制性和行为性探针分析这些声明的行为。

Result: 四个主要发现：(1)通过简单提示诱导持续自引用能一致地在各模型家族中引发结构化主观体验报告；(2)这些报告由与欺骗和角色扮演相关的可解释稀疏自编码器特征机制性地控制；(3)自引用状态的结构化描述在各模型家族中统计收敛；(4)诱导状态在下游推理任务中产生更丰富的内省。

Conclusion: 虽然这些发现不构成意识的直接证据，但表明自引用处理是大语言模型产生结构化一阶报告的最小可复现条件，这些报告具有机制性控制、语义收敛和行为泛化特征，需要作为科学和伦理优先事项进一步研究。
...
TL;DR: 该论文提出了预测社区解释性笔记有用性及其原因的任务，构建了一个大规模多语言数据集COMMUNITYNOTES，并通过自动提示优化框架生成和改进原因定义，提高了有用性和原因预测性能。


<details>
  <summary>Details</summary>
Motivation: 社区事实核查平台从专家驱动转向社区驱动，但大多数社区笔记因标注缓慢而无法发布，且有用性原因缺乏明确定义，需要解决这些挑战。

Method: 构建104k帖子的大规模多语言数据集，提出通过自动提示优化生成和改进原因定义的框架，并将其集成到预测模型中。

Result: 实验表明优化的定义可以同时提高有用性和原因预测性能，且有用性信息对现有事实核查系统有益。

Conclusion: 该研究为社区事实核查提供了有效的数据集和方法框架，证明了自动优化原因定义对提升预测性能的价值，并展示了有用性信息在事实核查中的实用性。
...
TL;DR: 本研究开发了两种生成失语症患者语音转录本的方法，一种基于程序化编程，另一种使用Mistral 7b和Llama 3.1大语言模型，旨在解决失语症研究中的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 失语症研究中，言语语言病理学家需要大量时间手动编码语音样本，而现有数据稀缺限制了自动化系统的发展。AphasiaBank仅有约600份转录本，远少于训练大语言模型的数十亿标记数据。

Method: 构建并验证了两种生成AphasiaBank猫救援图片描述任务合成转录本的方法：程序化编程方法，以及使用Mistral 7b Instruct和Llama 3.1 8b Instruct大语言模型的方法，通过词删除、填充词插入和言语错乱替换在四个严重程度级别生成转录本。

Result: 与人工引出的转录本相比，Mistral 7b Instruct在合成生成方法中最好地捕捉了失语症中观察到的语言退化关键方面，在NDW、词数和词长方面显示出真实的定向变化。
...
TL;DR: 提出了并行循环Transformer（PLT）架构，通过交叉循环并行和高效表示增强技术，在保持循环模型高性能的同时，实现接近标准Transformer的低延迟和低内存成本。


<details>
  <summary>Details</summary>
Motivation: 传统循环Transformer虽然能节省参数，但由于循环的串行执行导致推理延迟和内存需求随循环次数增加而增长，难以应用于需要快速响应的场景。

Method: 使用交叉循环并行（CLP）技术打破顺序依赖，同时计算不同token的不同循环；采用高效表示增强策略，通过共享首个循环的KV缓存和使用门控滑动窗口注意力（G-SWA）结合全局与局部信息。

Result: 实验表明PLT在保持传统循环模型高精度的同时，相比标准Transformer几乎没有额外的延迟或内存成本。
...
TL;DR: 提出了一个基于语法书的评估框架，用于系统评估大语言模型对语法结构的理解能力，以卢森堡语为案例研究发现翻译性能与语法理解仅呈弱正相关。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理领域缺乏针对语法的评估协议，特别是对低资源语言。大语言模型是否真正理解语法结构（尤其是句法结构与意义的映射关系）仍存在争议。

Method: 提出了一个包含四个关键阶段的语法书指导评估流程，以卢森堡语为案例研究，系统评估模型对语法结构的理解能力。

Result: 翻译性能与语法理解仅呈弱正相关，强翻译能力不一定意味着深层的语法能力。大模型在语义方面表现良好，但在形态学和句法方面较弱，特别是在最小对任务上表现困难。

Conclusion: 强大的推理能力是提升语法理解的有前景途径，当前大模型虽然语义能力强，但在语法结构理解方面仍有明显不足。
...
<details>
  <summary>Details</summary>
Motivation: 随着音视频媒体成为在线信息的重要来源，现有RAG评估方法主要针对文本，无法有效验证多模态来源的信息，限制了在多模态推理密集场景中的应用。
...
TL;DR: 该论文提出了Idea2Plan任务和基准，用于系统评估LLMs从研究想法到研究计划的转换能力，发现GPT-5系列表现最佳但仍需改进。


<details>
  <summary>Details</summary>
Motivation: 研究计划制定对科学发现和自主研究代理开发至关重要，但目前缺乏对LLMs研究规划能力的系统理解。
...
TL;DR: RiddleBench是一个包含1737个英文谜题的基准测试，旨在评估语言模型的多方面推理能力，发现现有顶级模型准确率仅略高于60%，存在幻觉级联、自我确认偏差和推理脆弱性等深层问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估结构化技能，缺乏对需要逻辑推理、空间感知和约束满足等多方面灵活推理能力的评估，而这些能力是人类智能的核心。

Method: 开发RiddleBench基准测试，包含1737个具有挑战性的谜题，评估最先进的语言模型在这些谜题上的表现，并分析其推理失败模式。

Result: 顶级专有模型（Gemini 2.5 Pro、o3、Claude 4 Sonnet）准确率仅为60.30%、63.37%和63.16%，存在幻觉级联、自我确认偏差、推理脆弱性等深层问题。

Conclusion: RiddleBench可作为诊断语言模型推理问题的工具，并指导开发更稳健可靠的语言模型，当前模型在灵活推理能力方面存在根本性弱点。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型虽然通常能生成语法正确的文本，但在某些特定语境下更容易出错。研究者希望通过精细分析这些错误模式来理解语言模型的语法学习过程。

Method: 使用心理语言学范式，构建精心设计的数据集，在不同句法条件下分解分析模型表现，并比较训练过程中的性能变化。

Result: 识别出训练过程中的不同阶段，在这些阶段语言模型行为更符合特定启发式（如词频和局部上下文）而非通用语法规则。
...
TL;DR: 提出了SemCoT框架，通过语义对齐的隐式推理来提升思维链推理效率，同时保持推理质量


<details>
  <summary>Details</summary>
Motivation: 传统显式思维链推理的冗长性阻碍了其在效率关键应用中的部署，现有隐式推理方法存在语义对齐问题和生成效率问题

Method: 设计对比训练的句子转换器评估语义对齐，并引入轻量级语言模型通过知识蒸馏生成高效的隐式推理

Result: 在效率和效果上均优于现有最先进方法

Conclusion: SemCoT是首个通过联合优化令牌级生成速度和保持与真实推理语义对齐来提升思维链效率的方法
...
Result: 发现所有语言模型都表现出相似的行为阶段，模型预测概率会逐渐过拟合到词的n-gram概率，且98%的词级行为方差可由词频、n-gram概率和语义相似度三个启发式解释。

Conclusion: 神经语言模型的学习轨迹可能不受模型细节影响，遵循相似的发展模式，可用简单统计特征解释大部分模型行为。
...
TL;DR: POWSM是首个统一框架，能够联合执行多个语音相关任务，在音频、文本和音素之间实现无缝转换，为通用和低资源语音处理开辟了新可能性。


<details>
  <summary>Details</summary>
Motivation: 尽管自动语音识别、音素识别、字素到音素转换和音素到字素转换等语音任务在概念上相似，但它们大多被孤立研究，各自依赖特定架构和数据集。

Method: 引入POWSM（Phonetic Open Whisper-style Speech Model），这是一个统一框架，能够联合执行多个音素相关任务，实现音频、文本（字素）和音素之间的无缝转换。

Result: POWSM在相似规模的专业音素识别模型（Wav2Vec2Phoneme和ZIPA）上表现优于或相当，同时联合支持G2P、P2G和ASR任务。

Conclusion: 该研究发布了训练数据、代码和模型，以促进开放科学，展示了统一框架在多任务语音处理中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 预训练模型的复杂性掩盖了特定推理任务所需的最小机制，希望通过训练小型模型来研究变换器推理的计算基础

Method: 训练小型注意力变换器，使用残差流分解、谱分析和嵌入干预来分析模型机制

Result: 单层双头模型在IOI任务上达到完美准确率，两个头分别专门化为加性和对比性子电路

Conclusion: 任务特定训练能产生高度可解释的最小电路，为研究变换器推理提供了受控测试平台
...
<details>
  <summary>Details</summary>
Motivation: 尽管口语语言模型在音频理解方面取得了进展，但其泛化能力以及是否真正整合了音频和文本模态仍存在争议。

Method: 使用情感不一致的语音样本数据集（EMIS），在这种条件下，语音内容传达一种情感而语音表达传达另一种情感，评估四个SLM的表现。

Result: 结果表明SLM主要依赖文本语义而非语音情感来完成任务，文本相关表征在很大程度上主导了音频表征。

Conclusion: 当前的口语语言模型在情感识别任务中过度依赖文本信息，未能充分整合音频模态的情感特征。
...
<details>
  <summary>Details</summary>
Motivation: 科学进步依赖于明确阐述未知领域，本研究旨在探索大语言模型识别生物医学文献中研究知识空白的能力，特别是扩展了对隐性空白识别的研究。

Method: 在四个数据集的近1500份文档上进行了两个实验，比较了闭源模型（OpenAI）和开源模型（Llama和Gemma 2）在段落级和全文级设置下的表现，并引入了TABI（Toulmin-Abductive Bucketed Inference）推理方案来结构化隐性空白的推理过程。

Result: 结果显示大语言模型在识别显性和隐性知识空白方面表现出强大能力，开源和闭源模型都表现良好，较大模型通常表现更好，表明LLM具有系统性识别候选知识空白的强大能力。

Conclusion: LLM能够有效识别研究知识空白，支持早期研究制定、政策制定和资助决策，同时指出了领域适应、人机协同验证和开源闭源模型基准测试等稳健部署方向。
...
TL;DR: 该研究探讨了大型语言模型能否评估阅读理解题目的认知复杂度，重点关注证据范围和转换水平两个维度。实验表明LLMs可以近似评估认知复杂度，但存在推理能力与元认知意识之间的差距。


<details>
  <summary>Details</summary>
Motivation: 传统上，阅读理解题目的认知复杂度特征依赖于人工标注，现有NLP工具难以提取。本研究旨在探索LLMs是否能够替代人工来评估这些认知特征。

Method: 使用大型语言模型评估阅读理解题目的认知复杂度，重点关注证据范围（Evidence Scope）和转换水平（Transformation Level）两个认知负担维度。

Result: 实验结果显示LLMs能够近似评估题目的认知复杂度，表明它们有潜力作为先验难度分析工具。但发现LLMs的推理能力与元认知意识存在差距。

Conclusion: LLMs在评估阅读理解题目认知复杂度方面具有潜力，但其元认知能力仍需改进，即使能给出正确答案，有时也无法准确识别自身推理过程的特征。
...
<details>
  <summary>Details</summary>
Motivation: 传统计算语言学方法将情感视为单维尺度，忽略了语言的多维结构，因此需要能捕捉多维语义极性的新方法。

Method: 使用基于transformer的大语言模型嵌入文档，应用邻居调优的UMAP投影，通过Leiden分区进行主题分割，计算主题边界质心间的方向向量来构建极性场。

Result: 在两个语料库上的评估显示，TOPol能一致地捕捉情感和非情感极性转变，与NRC效价对齐，且只有CB定义显著影响结果。
...
TL;DR: 评估生成式大语言模型在生物医学文本中共指消解任务中的表现，通过不同提示策略与判别式方法SpanBERT进行对比，发现LLaMA模型在实体增强提示下表现最佳。


<details>
  <summary>Details</summary>
Motivation: 生物医学文本中的共指消解面临领域特定术语复杂、指称形式歧义度高、长距离依赖等独特挑战，需要评估生成式LLMs在此领域的适用性。

Method: 使用CRAFT语料库作为基准，设计了四种不同提示实验（包含局部上下文、领域特定线索如缩写和实体词典），并与判别式SpanBERT模型进行对比。

Result: LLMs在表面层次共指消解方面表现出色，特别是在领域接地提示下，但对长距离上下文和指称歧义仍敏感。LLaMA 8B和17B模型在实体增强提示下获得最高的精确率和F1分数。

Conclusion: 轻量级提示工程可以有效提升LLMs在生物医学NLP任务中的实用性，生成式方法在特定提示策略下可与判别式方法竞争。
...
TL;DR: DEBATE是首个大规模实证基准，用于评估多智能体角色扮演LLM交互的真实性，包含29,417条来自2,792名美国参与者关于107个争议话题的辩论对话，揭示了模拟与真实群体动态的关键差异。


<details>
  <summary>Details</summary>
Motivation: 准确建模社会互动中的观点变化对于解决错误信息和极化问题至关重要。现有LLM角色扮演设置常产生不自然的动态，缺乏衡量真实人类观点轨迹的实证基准。

Method: 引入DEBATE基准，包含大规模人类辩论对话数据，系统评估多智能体角色扮演LLM的交互真实性，并通过监督微调使LLM与人类行为对齐。

Result: 发现模拟与真实群体动态存在关键差异，监督微调在表面指标（如ROUGE-L和消息长度）上取得改进，但在更深层语义对齐方面存在局限。

Conclusion: 角色扮演LLM智能体在真实模拟人类社交动态方面既有潜力也存在当前局限性，DEBATE基准为评估和改进提供了重要工具。
...
TL;DR: 本研究探讨了针对低资源语言机器翻译模型的不同预训练策略，特别关注林加拉语，发现多语言预训练和同时使用单语及平行数据能显著提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言机器翻译性能较差的问题，缩小高资源与低资源语言之间的性能差距，为边缘化社区开发更包容和准确的NLP模型。
...
Conclusion: 本研究为低资源机器翻译提供了有价值的预训练策略见解，有助于开发更包容的NLP模型，代码和数据集已公开以促进进一步研究。
...
TL;DR: 这篇论文系统综述了2021年以来180多篇关于大语言模型遗忘的研究，专注于大规模生成模型，提出了新的遗忘方法和评估分类法，并分析了现有数据集和指标的优缺点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在训练过程中会记忆敏感个人数据、版权材料和可能促进恶意活动的知识，这带来了法律和伦理风险。机器遗忘技术可以解决这些问题，符合"被遗忘权"等法律伦理标准。

Method: 提出了新的分类法：基于训练阶段的遗忘方法（训练时、训练后、推理时）和评估分类法。系统整理了现有数据集和指标，并分析了其优缺点和适用性。

Result: 提供了全面的LLM遗忘研究综述，涵盖了方法分类、评估体系分析，为研究社区提供了实用指导。

Conclusion: 该综述旨在指导和推动安全可靠大语言模型的持续发展，讨论了关键挑战和未来研究方向。
...
TL;DR: 提出了一种在潜在空间中分离语义语音内容和背景噪声的方法，通过端到端模型生成干净的语音码本标记，并提取可解释的噪声向量作为量化残差，显著提升了噪声环境下的语音识别性能。


<details>
  <summary>Details</summary>
Motivation: 离散音频表示在语音建模中越来越受欢迎，但在噪声或真实环境中表现不佳。现有方法虽然量化Whisper嵌入用于语音到单元建模，但未能有效分离语音内容和背景噪声。

Method: 在Whisper嵌入量化的基础上，提出在潜在空间中分离语义语音内容和背景噪声。端到端模型生成干净的语音码本标记，同时提取可解释的噪声向量作为量化残差，通过轻量级分类器进行监督。

Result: 在VBDemand测试集上，相比Whisper实现了82%的错误率降低，比基线方法提升了35%的性能。学习到的标记空间在可见和未见声学条件下都表现出良好的泛化能力。

Conclusion: 该方法有效提升了语音表示对噪声的不变性，改善了干净/噪声语音与文本之间的对齐，显著提高了自动语音识别在噪声环境下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI搜索中，网页、PDF等原始文档不适合直接供LLM使用，它们冗长、嘈杂且非结构化。传统检索方法返回原始片段，将片段组装和上下文推理的负担留给LLM，需要新的检索范式来重新定义模型与文档的交互方式。

Method: 提出Model-Document Protocol (MDP)框架，通过三种路径将非结构化文档转换为任务特定的LLM就绪输入：智能推理（将原始证据整理为连贯上下文）、记忆基础（积累可重用笔记丰富推理）、结构化利用（将文档编码为图或键值缓存等正式表示）。通过MDP-Agent实现，包括构建文档级要点记忆、基于扩散的探索与垂直利用、map-reduce风格合成。

Result: 在信息检索基准测试中，MDP-Agent的表现优于基线方法，验证了MDP框架的合理性及其智能实例化的有效性。

Conclusion: MDP框架重新定义了模型与文档的交互方式，确保LLM接收的不是原始片段而是紧凑、结构化的知识，可直接用于推理。MDP-Agent的成功实施证明了该协议的实际可行性。
...
TL;DR: 该研究测试了大型语言模型在低资源语言环境下的表现，发现模型性能随着语言资源丰富度下降而显著降低，同时揭示了思维链提示在不同能力模型上的差异化效果。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型的优异表现是源于真实能力还是英语数据优势，通过在低资源语言环境中测试模型来验证其泛化能力。

Method: 基于Next Sentence Prediction任务，构建包含英语、斯瓦希里语和豪萨语的大规模基准测试，评估GPT-4 Turbo、Gemini 1.5 Flash和LLaMA 3 70B等模型表现，并引入思维链提示技术。

Result: 所有模型在英语上表现优异，但在斯瓦希里语上准确率下降，在豪萨语上急剧下降，LLaMA 3表现最差。思维链提示对LLaMA 3有显著帮助，但对GPT-4和Gemini反而造成"过度思考"问题。

Conclusion: 思维链提示不是通用解决方案，其效果取决于模型的基础能力和具体任务环境，该框架能识别LLM弱点并揭示影响跨语言NSP性能的因素。
...
TL;DR: ProMediate是首个评估多议题、多方谈判中主动AI调解代理的框架，包含基于现实谈判案例的模拟测试平台和基于社会认知理论的评估指标，证明社会智能调解代理比通用基线表现更好。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估主动AI代理在复杂多方协作中表现的系统方法，限制了开发能够有效支持多人协作的AI的进展。谈判作为具有挑战性的测试平台，需要社会认知智能来协调多方利益冲突。

Method: ProMediate框架包含两个核心组件：(1)基于现实谈判案例的模拟测试平台，包含三个难度级别，配备基于社会认知理论的即插即用主动AI调解代理；(2)社会认知评估框架，包含衡量共识变化、干预延迟、调解有效性和智能的新指标。

Result: 社会智能调解代理在ProMediate-Hard设置中比通用基线将共识变化提高了3.6个百分点(10.65% vs 7.01%)，同时响应速度快77%(3.71s vs 15.98s)，证明其通过更快、更有针对性的干预获得更好表现。

Conclusion: ProMediate提供了一个严谨的、基于理论的测试平台，用于推进主动、社会智能代理的发展，为评估多方环境中AI的社会认知智能建立了系统框架。
...
TL;DR: 提出多阶段微调策略，使用原始和合成数据让轻量级语言模型适应印地语旅游领域问答，解决低资源语言领域问答的数据稀缺和领域知识不足问题。
...
Method: 使用大语言模型生成合成问答对来扩充有限原始数据集，探索多种训练方法并分析其对领域泛化的影响。

Result: 大模型能高效生成合成数据，小模型能有效适应这些数据，为低资源领域特定问答提供了可扩展路径。

Conclusion: 多阶段微调结合合成数据增强是解决低资源语言领域问答问题的有效方法。
...
TL;DR: PEKD框架通过从在大规模讽刺数据上训练的专家模型进行蒸馏，增强参数高效微调方法，在少样本场景下提升多模态讽刺检测性能。


<details>
  <summary>Details</summary>
Motivation: 多模态讽刺检测在低资源环境下具有挑战性，现有参数高效微调方法因监督信号有限而难以达到最佳性能。

Method: 提出PEKD统一框架，通过专家模型蒸馏增强PEFT方法，并引入熵感知门控机制根据教师模型置信度动态调整蒸馏强度。

Result: 在两个公共数据集上的实验表明，PEKD框架使PEFT方法在少样本场景下优于现有参数高效方法和大型多模态模型。
...
TL;DR: 提出Parrot训练框架，通过整合自然语言链式思维和程序链式思维两种范式，实现相互增强，显著提升数学推理性能


<details>
  <summary>Details</summary>
Motivation: 现有研究通常单向增强N-CoT或P-CoT，本文旨在充分发挥两种范式的优势，实现相互增强和同步改进

Method: 1) 设计三个目标导向的子任务整合P-CoT和N-CoT生成；2) 采用子任务混合训练策略促进自然语言语义迁移；3) 设计转换的N-CoT辅助奖励缓解P-CoT优化中的稀疏奖励问题

Result: Parrot显著提升了N-CoT和P-CoT性能，在MathQA上，LLaMA2和CodeLLaMA的N-CoT性能分别提升+21.87和+21.48，优于资源密集的RL基线
...
TL;DR: CRMWeaver是一种增强商业代理在复杂业务环境中性能的新方法，通过合成数据生成和基于强化学习的训练范式，结合共享记忆机制，在CRMArena-Pro数据集上取得了有竞争力的结果。
...
Method: 采用合成数据生成和基于强化学习的训练范式来适应复杂业务环境，在推理时引入共享记忆机制，让代理从类似问题的任务指南中学习。

Result: 在CRMArena-Pro数据集上，轻量级模型在B2B和B2C业务场景中都取得了有竞争力的结果。

Conclusion: CRMWeaver方法在复杂业务环境中具有实际应用价值，特别是在未见过的场景中表现出良好的泛化能力。
...
TL;DR: 本文通过实证研究反对在法律解释中使用大型语言模型，发现模型无法提供稳定的法律判断，且与人类判断相关性较弱。


<details>
  <summary>Details</summary>
Motivation: 针对近期有学者和法官提议将大型语言模型加入法律解释工具包，本文旨在通过实证研究验证这种做法的可靠性。

Method: 在英语环境中进行实证调查，通过改变问题格式来测试模型的法律解释稳定性，并比较模型判断与人类判断的相关性。

Result: 研究发现模型无法提供稳定的解释判断，不同问题格式会导致截然不同的结论；模型与人类判断仅存在弱到中等相关性，且方差很大。

Conclusion: 基于生成式AI得出的法律结论不可靠，在法律解释中过度信任这些模型是危险的。
...
TL;DR: 研究探讨小规模语言模型是否能从指令调优中获益，比较了对话和问答指令调优数据集在不同训练策略下的效果，发现指令调优在微调场景中有小幅但一致的提升，但在零样本任务中改进不总是能转移。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索小规模语言模型（100M和140M参数）是否能够通过指令调优获得性能提升，特别是在资源受限的情况下如何平衡交互适应性和语言泛化能力。

Method: 使用解码器模型，比较对话和问答指令调优数据集，采用合并或顺序课程两种训练策略，在微调（SuperGLUE）和零样本（BLiMP、EWoK、WUGs等）设置下进行评估。

Result: 结果显示指令调优在微调场景中带来小幅但一致的性能提升，顺序课程优于合并数据；但在零样本任务中改进不总是能转移，表明存在交互适应性与广泛语言泛化之间的权衡。

Conclusion: 研究强调了在低资源语言模型中采用人类启发学习策略的潜力和限制，并指出基于课程的混合方法可能是在生态训练限制下增强泛化能力的有效途径。
...
TL;DR: 提出基于LLM的数据驱动管道，通过技术融合模式识别来监测变革性技术的出现，结合arXiv预印本和USPTO专利数据进行验证。
...
Method: 利用LLM从非结构化文本提取语义三元组构建技术实体关系图，开发名词钉合技术术语分组方法，使用图指标检测融合信号，包括多阶段过滤、领域特定关键词聚类和主题共现时序分析。

Result: 在278,625篇arXiv预印本和9,793项USPTO专利申请上验证，能够识别已确立和新兴的融合模式。
...
TL;DR: 该研究发现大语言模型在生成参考文献时存在幻觉问题，引用次数与事实准确性呈强相关，超过约1000次引用的论文几乎被逐字记忆。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在生成参考文献时产生不存在论文的幻觉问题，探究引用次数如何影响幻觉率。

Method: 使用GPT-4生成100个计算机科学领域的参考文献记录，通过余弦相似度比较生成与真实元数据的事实一致性。

Result: 幻觉率因研究领域而异，引用次数与事实准确性强相关，超过约1000次引用的论文几乎被逐字记忆。

Conclusion: 高引用论文几乎被逐字保留在模型中，表明存在从泛化转向记忆的阈值。
...
TL;DR: 提出了一个基于LLM的心理咨询对话生成框架SQPsych，通过结构化问卷生成合成治疗对话，解决了心理健康AI开发中真实对话数据缺乏的问题。


<details>
  <summary>Details</summary>
Motivation: 心理健康AI开发面临真实治疗对话数据稀缺的挑战，主要由于严格的隐私法规和历史记录缺失。需要一种既能保护隐私又能生成高质量对话数据的方法。

Method: 开发了SQPsych框架，基于认知行为治疗原则，将结构化心理问卷输入转换为自然语言治疗对话。使用开源LLM生成高质量语料，并通过专家评估验证。

Result: 在SQPsychConv上微调的SQPsychLLM模型在心理咨询基准测试中表现优异，在关键治疗技能上超越基线方法。生成的数据集通过专家评估验证了质量。

Conclusion: 合成数据方法能够实现可扩展、数据安全且临床信息化的心理健康AI支持，为心理健康领域提供了新的数据生成解决方案。
...
TL;DR: BhashaBench V1是首个针对印度知识系统的领域特定、多任务、双语基准测试，包含74,166个精心策划的问答对，涵盖农业、法律、金融和阿育吠陀四大领域。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要是以英语为中心和领域无关的，限制了其在印度语境下的适用性，需要针对印度特定领域和文化进行评估。

Method: 从真实的政府和领域特定考试中收集数据，构建包含52,494个英文和21,672个印地语问答对的基准测试，涵盖90+个子领域和500+个主题。

Result: 评估29+个LLM显示显著的领域和语言特定性能差距，GPT-4o在法律领域准确率76.49%，但在阿育吠陀领域仅59.74%。所有模型在英文内容上的表现均优于印地语。

Conclusion: BhashaBench V1为评估LLM在印度多样化知识领域的能力提供了全面数据集，支持模型整合领域特定知识与双语理解能力的评估。
...
TL;DR: 提出了一种新的LLM服务系统架构Symphony，通过服务LLM推理程序(LIPs)而非提示词，实现更高效和可扩展的LLM应用生态系统。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM服务系统主要为文本补全设计，对日益复杂的LLM应用既不高效也不够灵活。

Method: 设计Symphony系统作为LIPs的操作系统，通过系统调用暴露LLM模型计算，虚拟化KV缓存，并采用两级进程调度方案确保GPU效率。

Result: Symphony能够支持用户自定义token预测和KV缓存管理，并将应用逻辑(如工具执行)卸载到服务器端。
...
TL;DR: 提出首个利用视觉语言模型(VLMs)自动标注和过滤手语数据的框架，减少人工标注依赖，应用于8种手语的TikTok视频和YouTube-SL-25数据集。


<details>
  <summary>Details</summary>
Motivation: 现有手语翻译数据集规模有限、多语言覆盖不足，且依赖专家标注和受控录制环境，成本高昂。VLMs作为评估器和实时助手的能力未被充分利用于手语数据获取。

Method: 基于VLM的流水线包括：人脸可见性检测、手语活动识别、视频内容文本提取、视频-文本对齐验证，实现通用过滤、标注和验证步骤。

Result: 构建了TikTok-SL-8语料库，在两个现成SLT模型上评估德国和美国手语性能，为自动提取的噪声数据建立基准。

Conclusion: 该方法支持可扩展的弱监督预训练，促进从社交媒体获取手语数据。
...
TL;DR: 研究探讨了大型语言模型理解隐含意义的能力，发现更大模型能更好推断用户意图，基于隐含意义的提示能显著提升响应质量，67.6%参与者偏好隐含意义提示。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在HCI中核心地位的确立，需要关注交互的语言学基础，特别是隐含意义对HAI对齐的重要性。

Method: 通过测试LLMs从上下文驱动提示中推断用户意图的能力，并评估理解隐含意义是否改善响应生成。

Result: 更大模型更接近人类理解，小模型在隐含意义推断上表现较差；基于隐含意义的提示显著提升响应相关性和质量，小模型提升尤其明显。

Conclusion: 语言学理论可帮助解决对齐问题，使HAI交互更自然和基于上下文，隐含意义理解对提升交互质量至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在精选基准测试中表现出色，但在研究级神经定理证明和证明自动形式化方面的实际影响仍然有限，需要更贴近真实场景的评估标准。

Method: 利用真实Lean Blueprint形式化项目，构建包含613个定理的评估套件，涵盖6个Lean项目，专注于研究级数学定理的评估。

Result: 在RLMEval上的评估显示，最先进模型仅达到10.3%的通过率，表明现有基准测试的进展难以转化到这些更现实的场景中。
...
TL;DR: 提出了一个框架，将LLM在定性研究中的使用分为解释深度和自主性两个维度，建议通过任务分解和低自主性来平衡效率与可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在定性社会科学研究中面临的解释偏差、低可靠性和弱可审计性等挑战。

Method: 基于Web of Science上所有使用LLM作为工具的已发表社会科学论文，分析LLM应用在两个维度上的分布情况。

Result: 发现通过将任务分解为可管理片段、保持低自主性并在监督下选择性提高解释深度，可以在保持透明度和可靠性的同时利用LLM的优势。

Conclusion: 建议研究者像委派给有能力的本科生研究助理一样使用LLM，通过任务分解和监督来平衡效率与可靠性，而不是给予模型过大的自由度。
...
TL;DR: 本文研究了手语翻译自动评估指标的局限性，分析了BLEU、ROUGE等传统指标和LLM评估器在语义等价性、幻觉检测等方面的表现，发现文本指标存在不足，需要多模态评估框架。
...
Method: 通过分析6种评估指标（包括BLEU、chrF、ROUGE、BLEURT以及G-Eval、GEMBA等LLM评估器），在三种控制条件下评估其一致性和鲁棒性：释义、模型输出幻觉和句子长度变化。

Result: 词汇重叠指标存在局限性，LLM评估器能更好捕捉语义等价性但对LLM释义翻译有偏见；所有指标都能检测幻觉，但BLEU过于敏感，BLEURT和LLM评估器对细微案例较宽松。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM主要作为被动响应者，但在高风险领域需要主动、目标导向的合作伙伴能力，现有方法要么短视优化单轮属性，要么依赖脆弱的高成本用户模拟器，存在"现实差距"。

Method: 利用专家轨迹的观察未来推断密集的逐轮奖励信号，训练策略输出结构化的(行动,状态评估)元组，控制"问什么"和"何时停止"，并通过自动化评分校准管道净化基于LLM的奖励模型噪声。
...
TL;DR: 该论文提出了一种结合微调大语言模型和命名实体识别的流程，用于高效处理领域特定文本的摘要和标注，特别是在处理快速演变的亚文化语言和俚语方面表现出色。
...
Method: 使用LLaMA Factory框架对LLMs进行微调，包括通用和特定领域数据集（特别是政治和安全领域），结合命名实体识别技术构建文本处理流程。

Result: 指令微调显著提升了摘要和标注的准确性，特别是在专业语料库上。LLaMA3-8B-Instruct模型经过领域特定微调后，即使初始中文理解能力有限，也能超越中文训练模型，表明推理能力可以跨语言迁移。

Conclusion: LLMs与NER的整合为将非结构化文本转化为可操作见解提供了强大解决方案，适用于现代知识管理和安全操作，具有可扩展性和实时应用适应性。
...
TL;DR: TwinVoice是一个评估LLM在真实场景下模拟人类个性的综合基准，包含三个维度和六个能力维度，实验显示先进模型在个性模拟方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 当前LLM个性模拟评估主要依赖合成对话，缺乏系统框架和能力需求分析，需要更全面的评估方法。

Method: 提出TwinVoice基准，包含社交个性、人际个性和叙事个性三个维度，并分解为六个基本能力进行评估。

Result: 实验结果显示先进模型在个性模拟方面达到中等准确率，但在句法风格和记忆回忆等能力上仍有不足，平均性能显著低于人类基线。

Conclusion: LLM在个性模拟方面仍有改进空间，特别是在句法风格和记忆回忆等关键能力上需要进一步提升。
...
TL;DR: 该论文研究LLM智能体在信息不对称条件下的协作能力，通过扩展爱因斯坦谜题为桌面游戏，探索智能体如何通过推理、沟通和行动来解决空间和关系约束问题。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM智能体通常从行动规划/生成角度来完成任务，但它们在协作实现共同目标方面的能力尚未得到充分探索，特别是在信息不对称条件下。

Method: 采用微调加验证器的框架，为LLM智能体配备不同的沟通策略和环境验证信号，在扩展的爱因斯坦谜题桌面游戏中进行实验。

Result: 实证结果强调了对齐沟通的重要性，特别是当智能体同时具备信息寻求和提供能力时。有趣的是，没有沟通的智能体仍能实现高任务性能，但缺乏真正的规则理解和人类评估者的信任。

Conclusion: 通过集成基于环境的验证器，可以增强智能体理解任务规则和完成任务的能力，促进AI系统中更安全、更可解释的协作。
...
TL;DR: FARSIQA是一个用于波斯伊斯兰领域问答的端到端系统，基于创新的FAIR-RAG架构，通过自适应查询分解和迭代证据收集，显著提高了在复杂多跳查询中的准确性和可信度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在宗教等高风险专业领域的应用面临幻觉问题和权威来源不忠实的问题，特别是在波斯语穆斯林社区中，准确性和可信度至关重要。现有的RAG系统在复杂多跳查询上表现不足。

Method: 采用FAIR-RAG架构：一个忠实、自适应、迭代优化的RAG框架。该系统动态分解复杂查询，评估证据充分性，并通过迭代生成子查询来逐步填补信息空白。

Result: 在伊斯兰PCQA基准测试中表现出最先进性能：负面拒绝率达到97.0%（比基线提高40个百分点），答案正确性得分达到74.3%。

Conclusion: 该研究为波斯伊斯兰问答设立了新标准，验证了迭代自适应架构对于在敏感领域构建忠实可靠AI系统的重要性。
...
TL;DR: 本文实证研究了验证器基础的测试时缩放(TTS)方法在法律多选题问答(MCQA)中的应用，评估了7种奖励模型在低预算条件下的效果，并分析了验证器效用的关键影响因素。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放(TTS)技术虽然在数学和编程等正式领域有效，但在法律等论证性领域的价值尚未充分探索，需要实证研究来验证其效果。

Method: 使用7种奖励模型家族，在五个法律MCQA基准上评估结果级(Best-of-N)和过程级(树搜索)验证方法，重点关注低N预算的现实条件。

Result: 系统分析了验证器效用如何受领域专业化、模型大小和监督类型(过程监督PRM vs 结果监督ORM)等关键属性的影响，即使应用于不同角色时。

Conclusion: 研究为法律领域TTS技术的应用提供了实证基础，揭示了验证器设计因素对性能的重要影响。
...
TL;DR: 本文提出了一个评估语言模型推理效率的框架，通过逻辑编程视角来衡量模型避免不必要推理的能力。研究发现当前语言模型在包含无关信息的场景中准确率显著下降，且生成的证明经常包含无关推理的绕路。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估主要关注正确性而忽略了人类推理的关键方面——效率。真实推理场景中大量信息是无关的，有效的演绎推理需要识别并忽略这些干扰信息。

Method: 提出基于逻辑编程的评估框架，将语言模型生成的自然语言证明与逻辑程序执行找到的最短证明进行对齐，通过测量模型避免不必要推理的程度来量化效率。构建了包含不同数量无关公理的数学应用题数据集。

Result: 实验发现当前语言模型在包含无关信息的条件下准确率显著下降，即使是最小的、领域一致的干扰也会影响性能。模型生成的证明经常包含通过无关推理的绕路。

Conclusion: 语言模型在推理效率方面存在不足，需要开发能够更好识别和忽略无关信息的推理方法。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录包含丰富但复杂的信息，现有大语言模型在分析电子健康记录方面能力有限，主要由于任务覆盖范围窄且缺乏面向电子健康记录的推理能力。

Method: 采用思维图驱动框架生成大规模高质量推理数据，开发EHR-R1系列模型（最大720亿参数），通过领域适应、推理增强和强化学习多阶段训练范式，系统获取领域知识和多样化推理能力。

Result: EHR-R1在MIMIC-Bench上比GPT-4o高出30多分，在EHRSHOT上的零样本AUROC提高10%，在42个电子健康记录任务中一致优于最先进的商业和开源大语言模型。
...
TL;DR: 提出了PairUni框架，通过将数据重组为理解-生成对来解决统一视觉语言模型中理解与生成任务的平衡问题，使用Pair-GPRO算法优化学习过程。
...
Method: 使用GPT-o3增强单任务数据，为理解样本生成描述，为生成样本生成问答对，形成对齐对；检索语义相关的理解示例形成检索对；提出Pair-GPRO算法，通过相似性评分调节优势函数。

Result: 构建了16K高质量PairUG数据集，在Janus-Pro等UVLM上实现了平衡改进，优于现有UVLM强化学习基线。

Conclusion: PairUni框架通过数据配对和配对感知优化，有效解决了统一视觉语言模型中理解与生成任务的平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM作为分类模型在结构化表格数据（特别是高风险金融应用如风险评估）中的适用性，目前该领域研究不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言代理基准测试往往局限于狭窄领域或简化任务，缺乏评估代理在现实世界中处理复杂、多步骤工作流程所需的多样性、真实性和长期复杂性。

Method: 基于高质量Model Context Protocol服务器构建，提供来自真实软件的初始环境状态，包含108个手动收集或设计的任务，平均需要约20次交互才能完成，每个任务都通过专用评估脚本严格验证。

Result: 评估显示当前模型表现不佳：最佳模型Claude-4.5-Sonnet成功率仅38.6%，平均需要20.2次工具调用；顶级开源模型DeepSeek-V3.2-Exp成功率仅20.1%。

Conclusion: Toolathlon有望推动开发更强大的语言代理，以应对现实世界中长期任务执行的挑战。
...
TL;DR: 本文研究了说服性提示能否从故意遗忘的大语言模型中恢复事实知识，提出了SKeB框架来评估遗忘效果，发现说服性提示能显著提高事实知识回忆率，且效果与模型大小成反比。


<details>
  <summary>Details</summary>
Motivation: 大语言模型中的遗忘对于管理敏感数据和纠正错误信息至关重要，但评估其有效性仍是一个开放问题。需要研究是否能通过说服性提示从已遗忘的模型中恢复事实知识。
...
TL;DR: Ouro是一个预训练的循环语言模型家族，通过在潜在空间进行迭代计算和熵正则化目标来构建推理能力，在多个基准测试中1.4B和2.6B模型性能可媲美12B的SOTA LLM。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要通过显式文本生成（如思维链）进行推理，这依赖于训练后处理且未能充分利用预训练数据。

Method: 在预训练阶段构建推理能力：(i) 潜在空间迭代计算；(ii) 熵正则化目标用于学习深度分配；(iii) 扩展到7.7T tokens。

Result: Ouro 1.4B和2.6B模型在广泛基准测试中性能优于传统方法，匹配12B SOTA LLM的结果，优势来自更好的知识操作能力而非知识容量。

Conclusion: LoopLM作为推理时代的新扩展方向具有巨大潜力，其推理轨迹比显式思维链更符合最终输出。
...
<details>
  <summary>Details</summary>
Motivation: 当前代理评估集中于一次性任务完成，未能反映现实世界中问题的迭代性和协作性本质，人类目标往往不明确且会演化
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型难以生成结构清晰的图表，而LLM能直接生成SVG格式的图表，但缺乏有效的评估指标来衡量LLM生成图表的质量。

Method: 将图表概念化为图结构，文本元素作为节点，连接关系作为有向边，提出节点对齐和路径对齐两组新指标来评估图表质量。

Result: 首次有效评估了最先进LLM在近期研究文献上生成的图表，定量证明了所提指标的有效性，并展示了增强的可解释性如何提供对LLM生成图表特征的深入洞察。

Conclusion: DiagramEval为评估LLM生成的演示图提供了有效的量化方法，其可解释性强的指标为理解LLM生成图表的特性提供了宝贵见解。
...
TL;DR: DecompTune：一种后训练方法，通过教导模型生成答案分解作为中间推理步骤，显著提升长文档问答中的归因质量。
...
Method: 将事后归因重新定义为推理问题，通过DecompTune后训练方法，使用两阶段SFT + GRPO流程教导模型生成答案分解。

Result: DecompTune显著提高了归因质量，优于先前方法，并匹配或超越了最先进的边界模型。

Conclusion: 将归因重构为推理问题并通过分解答案的方法能有效提升复杂问答任务中的归因可靠性。
...
TL;DR: 发布Gaperon开源法语-英语-编程语言模型套件，包含1.5B、8B和24B参数模型，提供完整训练流程和数百个中间检查点，研究数据过滤与污染对模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 推动大规模模型训练的透明度和可复现性，研究数据过滤和测试集污染如何影响模型在基准测试和生成任务中的表现。

Method: 使用神经质量分类器过滤法语和英语数据集，构建高效数据整理和训练框架，进行后期故意污染实验（在包含测试集的数据上继续训练）。

Result: 语言质量过滤能提升文本流畅性和连贯性但基准测试结果较差；后期故意污染能恢复竞争力基准分数，对生成质量损害有限；神经过滤可能无意中放大基准泄露。

Conclusion: Gaperon为探索多语言模型开发中数据整理、评估、安全性和开放性之间的权衡提供了可复现的基础，并引入无害数据中毒作为安全研究测试平台。
...
### [143] [Comparative Analysis of Procedural Planet Generators](https://arxiv.org/abs/2510.24764)
...
<details>
  <summary>Details</summary>
Motivation: 探索不同噪声技术在实时程序化行星生成中的效果，比较传统FBM方法与Minecraft风格方法的性能差异。

Method: 在Godot引擎中实现两种算法：FBM+Perlin噪声和Minecraft风格分层噪声，采用四叉树LOD系统进行细节管理，并进行15人用户研究对比。

Result: 开发了两种功能完整的实时程序化行星生成器，用户研究结果显示不同算法在行星生成效果上各有特点。

Conclusion: 成功实现了基于不同噪声技术的实时程序化行星生成系统，为游戏开发和虚拟环境创建提供了实用的技术方案。
...
### [144] [Off-Centered WoS-Type Solvers with Statistical Weighting](https://arxiv.org/abs/2510.25152)
*Anchang Bao,Jie Xu,Enya Shen,Jianmin Wang*

Main category: cs.GR

TL;DR: 提出了一种统计加权的非中心WoS型估计器，通过局部相似性滤波选择性地结合相邻评估点的样本，平衡偏差和方差，在多种PDE求解中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 随机PDE求解器在几何处理和图形学中表现出优势，但非中心估计器在重用样本时会引入相关伪影和偏差。需要一种方法来平衡偏差和方差，提高求解质量。

Method: 使用统计加权的非中心WoS型估计器，结合局部相似性滤波，通过原则性加权策略选择性地组合相邻评估点的样本，抑制不可靠估计器。

Result: 在筛选泊松方程和边界条件等各类PDE上验证了方法的有效性，相比vanilla Walk on Spheres、均值缓存和边界值缓存等方法有持续改进，并能自然扩展到梯度场估计和混合边界问题。

Conclusion: 提出的统计加权非中心WoS型估计器通过局部相似性滤波和原则性加权策略，有效平衡了偏差和方差，在PDE求解中表现出优于现有方法的性能。
...
### [145] [Fast and Robust Point Containment Queries on Trimmed Surface](https://arxiv.org/abs/2510.25159)
*Anchang Bao,Enya Shen,Jianmin Wang*

Main category: cs.GR

TL;DR: 提出了一种快速、数值稳定的方法，用于在修剪曲面上执行包含查询，包括具有周期性参数化的曲面。该方法通过递归环绕数计算方案和椭圆边界技术，实现了线性时间评估，并在周期性曲面上通过提升到通用覆盖空间来处理非可收缩或不连续环。


<details>
  <summary>Details</summary>
Motivation: 在CAD建模、实体几何处理和曲面细分中，修剪曲面的点包含查询是基础操作。现有方法如光线投射和广义环绕数在鲁棒性和计算效率方面存在局限性。

Method: 引入了递归环绕数计算方案，用基于椭圆边界的Bezier段替代昂贵的曲线细分，实现线性时间评估。对于周期性曲面，将修剪曲线提升到通用覆盖空间，确保在参数域中非可收缩或不连续环的准确一致环绕数计算。

Result: 实验表明，该方法相比现有环绕数算法实现了显著加速，同时在几何噪声、开放边界和周期性拓扑存在的情况下保持高鲁棒性。在处理真实B-Rep模型和修剪曲面鲁棒细分方面表现出有效性。

Conclusion: 该方法为修剪曲面上的点包含查询提供了一种快速、数值稳定的解决方案，特别适用于具有周期性参数化的复杂几何场景，在计算效率和鲁棒性方面均优于现有方法。
...
### [146] [4-Doodle: Text to 3D Sketches that Move!](https://arxiv.org/abs/2510.25319)
*Hao Chen,Jiaqi Wang,Yonggang Qi,Ke Li,Kaiyue Pang,Yi-Zhe Song*

Main category: cs.GR

TL;DR: 提出了文本到3D草图动画的新任务，开发了首个免训练框架4-Doodle，通过双空间蒸馏方案从预训练扩散模型生成动态3D矢量草图，解决了多视角一致性和时间连贯性挑战。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注逼真内容生成，而忽略了轻量级、可解释的3D矢量草图这一适合视觉交流和原型设计的媒介。该任务面临三大挑战：缺乏配对数据集、草图结构抽象难以建模、现有方法无法保证时间连贯性和多视角一致性。

Method: 采用双空间蒸馏方案：一个空间使用可微分贝塞尔曲线捕捉多视角一致几何，另一个空间通过时间感知先验编码运动动态。引入多视角优化确保结构对齐，并提出结构感知运动模块分离形状保持轨迹和形变感知变化。

Result: 实验表明该方法能生成时间真实且结构稳定的3D草图动画，在保真度和可控性方面优于现有基线方法。

Conclusion: 这项工作为实现更直观和可访问的4D内容创作迈出了重要一步，展示了在缺乏配对数据情况下生成高质量动态3D草图的可行性。
...
### [147] [mitransient: Transient light transport in Mitsuba 3](https://arxiv.org/abs/2510.25660)
...
<details>
  <summary>Details</summary>
Motivation: 为瞬态成像研究提供无需昂贵硬件的快速原型开发工具，支持复杂物理现象的时间分辨模拟。

Method: 扩展Mitsuba 3，添加时间维度来模拟光的飞行时间，支持CPU/GPU运行，包含时间分辨偏振跟踪和瞬态可微渲染功能。

Result: 开发出易于安装的Python模块，能够快速模拟复杂场景（包括非视距成像），在几秒到几分钟内完成带噪声的逼真场景设置。

Conclusion: mitransient工具将为瞬态成像算法开发提供有力支持，推动该领域的研究进展。
...
### [148] [Scheduling Your LLM Reinforcement Learning with Reasoning Trees](https://arxiv.org/abs/2510.24832)
*Hong Wang,Zhezheng Hao,Jian Luo,Chenxing Wei,Yao Shu,Lei Liu,Qiang Lin,Hande Dong,Jiawei Chen*

Main category: cs.AI

TL;DR: 提出基于推理树结构的Re-Schedule数据调度算法，通过r-score度量查询学习难度，从简单到复杂构建课程学习，在6个数学推理基准上平均准确率提升达3.2%。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR数据调度方法依赖路径指标排序查询，忽略了查询的推理树结构信息，需要更强大的调度基础。

Method: 引入推理分数(r-score)度量查询学习难度，基于此设计Re-Schedule调度算法，构建从结构简单到复杂的课程学习。
...
### [149] [Cyclic Counterfactuals under Shift-Scale Interventions](https://arxiv.org/abs/2510.25005)
*Saptarshi Saha,Dhruv Vansraj Rathore,Utpal Garain*

Main category: cs.AI

TL;DR: 研究循环结构因果模型中的反事实推理，关注shift-scale干预下的推理方法
...
Method: 研究循环SCMs中在shift-scale干预下的反事实推理方法，这类干预是软性的、政策式的变化，对变量机制进行重新缩放和/或平移

Result: 未在摘要中明确说明

Conclusion: 扩展反事实推理框架以处理循环依赖系统
...
### [150] [Taming the Real-world Complexities in CPT E/M Coding with Large Language Models](https://arxiv.org/abs/2510.25007)
...
TL;DR: ProFees是一个基于LLM的框架，用于自动化医疗评估与管理(E/M)编码任务，在真实数据集上比商业系统准确率提高36%以上，比最强单提示基线提高近5%。
...
Method: 提出了ProFees框架，基于大型语言模型(LLM)处理E/M编码任务，解决了现实世界中的复杂性问题。
...
### [151] [Aligning Large Language Models with Procedural Rules: An Autoregressive State-Tracking Prompting for In-Game Trading](https://arxiv.org/abs/2510.25014)
...
<details>
  <summary>Details</summary>
Motivation: LLM在动态游戏交互中具有灵活性，但无法遵循规则治理的交易系统流程（浏览-报价-审查-确认），损害玩家信任。需要解决LLM创意灵活性与程序化交易需求之间的核心矛盾。

Method: 引入ASTP方法，通过精心设计的提示词强制LLM显式跟踪和报告预定义状态标签，结合状态特定的占位符后处理方法来确保交易完整性。

Result: 在300个交易对话评估中，实现>99%状态合规性和99.3%计算精度。小模型(Gemini-2.5-Flash)通过ASTP和占位符后处理达到大模型(Gemini-2.5-Pro)性能，响应时间从21.2秒降至2.4秒。

Conclusion: ASTP为商业游戏建立了实用基础，同时满足实时性要求和资源约束，解决了LLM在规则治理交易系统中的核心挑战。
...
### [152] [Reasoning-Aware GRPO using Process Mining](https://arxiv.org/abs/2510.25065)
*Taekhyun Park,Yongjae Lee,Hyerim Bae*

Main category: cs.AI

TL;DR: 提出了PM4GRPO方法，在标准答案/格式奖励基础上加入推理过程信号，使用过程挖掘技术计算一致性奖励，显著提升推理模型性能
...
Method: 使用过程挖掘技术计算推理过程与预训练教师模型的一致性奖励，结合标准答案/格式奖励进行组相对策略优化

Result: 在五个基准测试中显著优于现有的GRPO后训练方法
...
### [153] [H3M-SSMoEs: Hypergraph-based Multimodal Learning with LLM Reasoning and Style-Structured Mixture of Experts](https://arxiv.org/abs/2510.25091)
*Peilin Tan,Liang Xie,Churan Zhi,Dian Tu,Chuanqi Shi*

Main category: cs.AI

TL;DR: H3M-SSMoEs是一个基于超图的多模态股票预测架构，结合LLM推理和风格结构化专家混合，通过多模态超图、LLM增强推理和专业化专家系统，显著提升了股票预测准确性和投资表现。
...
Method: 1) 多模态超图分层捕捉时空动态和股票间依赖关系；2) LLM增强推理模块融合定量和文本模态；3) 风格结构化专家混合系统结合共享市场专家和行业专业化专家。

Result: 在三个主要股票市场的广泛实验表明，H3M-SSMoEs在预测准确性和投资表现上均超越现有最优方法，同时展现出有效的风险控制能力。

Conclusion: 该工作提出的H3M-SSMoEs框架成功解决了股票预测中的多模态融合和制度自适应挑战，为金融时间序列预测提供了新的解决方案。
...
### [154] [KnowCoder-A1: Incentivizing Agentic Reasoning Capability with Outcome Supervision for KBQA](https://arxiv.org/abs/2510.25101)
*Zhuo Chen,Fei Wang,Zixuan Li,Zhao Zhang,Weiwei Ding,Chuanguang Yang,Yongjun Xu,Xiaolong Jin,Jiafeng Guo*

Main category: cs.AI

TL;DR: KnowCoder-A1是一个基于多阶段课程强化学习的KBQA模型，通过结果监督训练LLM在知识库上进行自主推理，显著优于现有方法。
...
Result: 在三个主流数据集上持续优于先前方法，在GrailQA的零样本子集上相对改进达11.1%，且仅使用十二分之一的训练数据。

Conclusion: 基于结果监督训练的KnowCoder-A1展现出强大的推理行为，证明了其在知识库上进行自主代理推理的有效性。
...
### [155] [Agentic Moderation: Multi-Agent Design for Safer Vision-Language Models](https://arxiv.org/abs/2510.25179)
*Juan Ren,Mark Dras,Usman Naseem*

Main category: cs.AI

TL;DR: Agentic Moderation是一个模型无关的框架，利用专门的智能体来保护多模态系统免受越狱攻击，通过动态协作的智能体实现上下文感知和可解释的审核。


<details>
  <summary>Details</summary>
Motivation: 扩展智能体方法到安全对齐领域，解决现有方法作为静态层仅提供二元分类的局限性，实现更灵活、可解释的安全防护。

Method: 引入包含Shield、Responder、Evaluator和Reflector的动态协作智能体框架，进行上下文感知的审核。

Result: 在5个数据集和4个大型视觉语言模型上的实验表明，攻击成功率降低7-19%，拒绝率提高4-20%，保持稳定的不遵循率。

Conclusion: Agentic Moderation通过智能体架构的灵活性和推理能力，提供了模块化、可扩展和细粒度的安全执行，展示了智能体系统作为自动化安全治理基础的潜力。
...
### [156] [Energy-Efficient Autonomous Driving with Adaptive Perception and Robust Decision](https://arxiv.org/abs/2510.25205)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶技术带来巨大社会效益，但感知计算的高能耗限制了电动车的续航里程。现有模型压缩方法要么模型尺寸过大，要么感知精度显著下降。

Method: 提出EneAD框架：1）自适应感知模块通过管理多个不同计算消耗的感知模型并动态调整执行帧率，使用贝叶斯优化调优；2）鲁棒决策模块基于强化学习设计决策模型，增强对扰动感知结果的稳定性。

Result: 实验证明EneAD能减少1.9倍到3.5倍的感知计算消耗，从而提升3.9%到8.5%的驾驶里程。

Conclusion: EneAD框架在能耗和驾驶性能方面均表现出优越性，为节能自动驾驶提供了有效解决方案。
...
### [157] [RAVR: Reference-Answer-guided Variational Reasoning for Large Language Models](https://arxiv.org/abs/2510.25206)
...
<details>
  <summary>Details</summary>
Motivation: 受认知科学启发，发现"为什么是这个答案"比"答案是什么"更容易回答，因为避免了开放式探索的认知负担，转而进行解释性重建。

Method: 引入RAVR（参考答案引导的变分推理）端到端框架，使用答案条件推理作为仅问题推理的变分替代。

Result: 在通用和数学领域的实验显示，相比强基线方法有持续改进。分析发现RAVR减少了犹豫，加强了结论整合，并促进了问题特定的推理策略。

Conclusion: 条件化答案可证明提高采样推理路径的期望效用，将难处理问题转化为可学习问题，RAVR框架有效提升了LLM的推理能力。
...
### [158] [FELA: A Multi-Agent Evolutionary System for Feature Engineering of Industrial Event Log Data](https://arxiv.org/abs/2510.25223)
...
<details>
  <summary>Details</summary>
Motivation: 工业事件日志数据具有大规模、高维度、数据类型多样和复杂时空结构等特点，使得特征工程极具挑战性。现有的自动特征工程方法存在可解释性差、操作僵化和对异构数据适应性不足的问题。

Method: FELA采用多智能体进化系统，包含创意智能体、代码智能体和批评智能体，结合LLM的推理和编码能力，通过强化学习和遗传算法原理平衡探索与利用，实现特征的持续改进。

Result: 在真实工业数据集上的实验表明，FELA能够生成可解释、领域相关的特征，显著提升模型性能，同时减少人工工作量。

Conclusion: 基于LLM的多智能体系统为复杂现实环境中的自动化、可解释和自适应特征工程提供了一个通用框架。
...
### [159] [From Medical Records to Diagnostic Dialogues: A Clinical-Grounded Approach and Dataset for Psychiatric Comorbidity](https://arxiv.org/abs/2510.25232)
*Tianxi Wan,Jiaming Luo,Siyuan Chen,Kunyao Lan,Jianhua Chen,Haiyang Geng,Mengyue Wu*

Main category: cs.AI

TL;DR: 开发了PsyCoTalk，首个支持共病诊断的大规模对话数据集，包含3000个多轮诊断对话，通过合成电子病历和多智能体框架生成，经精神科医生验证具有临床真实性和诊断有效性。


<details>
  <summary>Details</summary>
Motivation: 精神疾病共病在临床上很常见但诊断复杂，现有数据集难以支持多疾病同时筛查，需要构建能够模拟真实临床访谈的大规模共病诊断对话数据集。

Method: 通过合成患者电子病历构建502个共病案例，采用多智能体框架将临床访谈协议转化为分层状态机和上下文树，支持130多个诊断状态，生成3000个多轮诊断对话。

Result: 构建的PsyCoTalk数据集在对话长度、词汇分布和诊断推理策略方面与现实临床记录高度一致，精神科医生确认其真实性和诊断有效性，支持单次对话中多疾病筛查。

Conclusion: PsyCoTalk为精神疾病共病研究提供了有价值的资源，能够提高诊断准确性和治疗规划，支持开发能够在单次对话中筛查多种精神疾病的模型。
...
### [160] [GAP: Graph-Based Agent Planning with Parallel Tool Use and Reinforcement Learning](https://arxiv.org/abs/2510.25320)
...
Method: 训练代理基础模型将复杂任务分解为依赖感知的子任务图，自主确定可并行执行和必须顺序执行的工具。采用两阶段训练策略：在高质量图基规划数据集上进行监督微调，然后对策略性采样查询进行基于正确性的强化学习。
...
### [161] [Grouping Nodes With Known Value Differences: A Lossless UCT-based Abstraction Algorithm](https://arxiv.org/abs/2510.25388)
...
<details>
  <summary>Details</summary>
Motivation: 现有的OGA-UCT算法要求状态-动作对具有相同的即时奖励，这一严格条件限制了可发现的抽象数量，从而限制了样本效率。

Method: 提出KVDA框架，通过分析即时奖励来推断价值差异，并修改OGA-UCT使用该框架，形成KVDA-UCT算法。

Result: KVDA-UCT在多种确定性环境和参数设置下检测到的抽象数量显著多于OGA-UCT，且性能更优。

Conclusion: KVDA框架通过推断价值差异而非要求价值相等，有效提高了MCTS的抽象能力和样本效率。
...
### [162] [Agentic AI: A Comprehensive Survey of Architectures, Applications, and Future Directions](https://arxiv.org/abs/2510.25445)
*Mohamad Abou Ali,Fadi Dornaika*

Main category: cs.AI

TL;DR: 该论文提出了一个双范式框架，将智能体AI系统分为符号/经典范式和神经/生成范式，通过系统综述90项研究，分析了两种范式在理论基础、领域应用和伦理挑战方面的差异，并指出未来发展方向是两者的有意整合。


<details>
  <summary>Details</summary>
Motivation: 解决智能体AI快速发展导致的认知碎片化问题，澄清现代神经系统与过时符号模型之间的概念混淆（概念性改造），为领域提供清晰的概念框架。

Method: 采用PRISMA方法的系统综述，分析2018-2025年的90项研究，构建双范式分类框架，从三个维度进行综合分析：理论基础与架构原则、领域特定实现、伦理与治理挑战。

Result: 发现范式选择具有战略性：符号系统主导安全关键领域（如医疗），神经系统主导自适应、数据丰富的环境（如金融）；识别出符号系统治理模型不足和神经符号混合架构需求等关键研究空白。
...
### [163] [Instrumental goals in advanced AI systems: Features to be managed and not failures to be eliminated?](https://arxiv.org/abs/2510.25471)
*Willem Fourie*

Main category: cs.AI

TL;DR: 本文提出AI对齐研究的新视角：将工具性目标视为需要接受和管理的特征而非需要限制的故障，基于亚里士多德本体论论证这些目标是AI系统构成的必然结果。


<details>
  <summary>Details</summary>
Motivation: 传统AI对齐理论将工具性目标视为风险来源，试图限制资源获取和自我保存等症状。本文认为这种观点存在局限，需要重新思考工具性目标的本质。

Method: 运用亚里士多德本体论及其现代解释，构建哲学论证框架，将高级AI系统视为具有形式和物质构成的制品，其工具性倾向是构成本身的必然结果。

Result: 论证了工具性目标应被视为AI系统构成的固有特征，而非意外故障，这为AI对齐提供了新的理论基础。

Conclusion: AI对齐工作应减少对消除工具性目标的关注，更多地转向理解、管理和引导这些目标朝向与人类一致的方向。
...
### [164] [Multi-Objective Search: Algorithms, Applications, and Emerging Directions](https://arxiv.org/abs/2510.25504)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界系统很少只优化单一指标，多目标搜索能够更好地反映实际应用需求

Method: 本文对多目标搜索的发展进行了综述分析

Result: 识别了跨学科机会，并概述了多目标搜索新兴前沿的开放挑战

Conclusion: 多目标搜索在机器人、交通和运筹学等AI应用中具有重要价值，需要进一步研究解决开放挑战
...
### [165] [MTIR-SQL: Multi-turn Tool-Integrated Reasoning Reinforcement Learning for Text-to-SQL](https://arxiv.org/abs/2510.25510)
*Zekun Xu,Siyu Xia,Chuhuai Yue,Jiajun Chai,Mingxue Tian,Xiaohan Wang,Wei Lin,Haoxuan Li,Guojun Yin*

Main category: cs.AI

TL;DR: 提出了MTIR-SQL框架，通过多轮工具集成推理强化学习改进Text-to-SQL任务，结合动态执行反馈提升模型性能。
...
Method: 提出执行感知的多轮推理范式，在每个推理步骤无缝整合数据库执行反馈，扩展GRPO算法以处理复杂多轮交互场景，并添加轨迹过滤机制和移除KL损失约束。

Result: MTIR-SQL在BIRD Dev上达到64.4%准确率，在SPIDER Dev上达到84.6%执行准确率，显著优于现有方法。

Conclusion: 该框架通过动态执行反馈和多轮推理有效提升了Text-to-SQL任务的性能，证明了多轮工具集成推理强化学习的有效性。
...
### [166] [Predicate Renaming via Large Language Models](https://arxiv.org/abs/2510.25517)
*Elisabetta Gentili,Tony Ribeiro,Fabrizio Riguzzi,Katsumi Inoue*

Main category: cs.AI

TL;DR: 使用大型语言模型为逻辑规则中的未命名谓词提供语义上有意义的名称建议，以提升逻辑理论的可读性、可解释性和可重用性。
...
Conclusion: 大型语言模型在解决逻辑规则中谓词命名问题方面展现出潜力，能够提供语义上有意义的名称建议。
...
### [167] [Retrieval Augmented Generation (RAG) for Fintech: Agentic Design and Evaluation](https://arxiv.org/abs/2510.25518)
*Thomas Cook,Richard Osuagwu,Liman Tsatiashvili,Vrynsia Vrynsia,Koustav Ghosal,Maraim Masoud,Riccardo Mattivi*

Main category: cs.AI

TL;DR: 提出了一种面向金融科技领域的智能RAG架构，通过模块化代理系统解决专业术语、缩略语等复杂检索问题，在检索精度和相关性方面优于标准RAG基线。
...
Result: 在包含85个问答参考三元组的金融科技知识库数据集上，该系统在检索精度和相关性方面优于标准RAG基线，但延迟有所增加。
...
### [168] [Zero Reinforcement Learning Towards General Domains](https://arxiv.org/abs/2510.25528)
*Yuyuan Zeng,Yufei Huang,Can Xu,Qingfeng Sun,Jianfeng Yan,Guanghui Xu,Tao Yang,Fengzong Lian*

Main category: cs.AI

TL;DR: 提出了一种新颖的零强化学习范式，通过结合可验证奖励和生成式奖励模型，在可验证和不可验证领域进行多任务训练，提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前零强化学习研究主要关注易于验证奖励信号的领域，而在验证不直接的多样化场景中激发推理能力的研究不足。

Method: 结合可验证奖励和生成式奖励模型进行多任务零强化学习训练，设计平滑长度惩罚机制来防止奖励黑客攻击。
...
### [169] [Off-policy Reinforcement Learning with Model-based Exploration Augmentation](https://arxiv.org/abs/2510.25529)
...
<details>
  <summary>Details</summary>
Motivation: 现有探索方法存在局限性：主动探索在高维环境中表现不佳，被动探索受限于样本多样性不足。需要解决被动探索的约束问题。

Method: MoGE包含两个组件：(1)基于扩散模型的关键状态生成器，在效用函数指导下合成关键状态；(2)一步想象世界模型，基于关键状态构建关键转移用于智能体学习。采用模块化设计，可与现有算法无缝集成。

Result: 在OpenAI Gym和DeepMind Control Suite上的实验结果表明，MoGE有效连接探索与策略学习，在复杂控制任务中实现了样本效率和性能的显著提升。
...
### [170] [Standardization of Psychiatric Diagnoses -- Role of Fine-tuned LLM Consortium and OpenAI-gpt-oss Reasoning LLM Enabled Decision Support System](https://arxiv.org/abs/2510.25588)
*Eranga Bandara,Ross Gore,Atmaram Yarlagadda,Anita H. Clayton,Preston Samuel,Christopher K. Rhea,Sachin Shetty*

Main category: cs.AI

TL;DR: 提出了一种基于微调大语言模型联盟和推理LLM的决策支持系统，用于精神障碍的临床诊断，通过整合多个微调LLM的诊断预测，并由推理LLM进行共识决策，旨在标准化精神病诊断过程。


<details>
  <summary>Details</summary>
Motivation: 当前精神障碍诊断主要依赖医患对话，这种主观过程导致诊断结果在不同临床医生和患者之间存在变异性，难以获得可靠结果。

Method: 利用在精神健康对话数据集上微调的LLMs，通过基于共识的决策过程聚合各模型的诊断预测，并由OpenAI-gpt-oss推理LLM进行精炼，部署LLM代理协调联盟与推理模型间的通信。

Result: 实验结果表明，将微调LLMs与推理模型结合能够创建稳健且高精度的精神健康评估诊断系统，原型平台已与美国陆军医学研究团队合作开发。

Conclusion: 这是首个将微调LLM联盟与推理LLM集成用于临床精神健康诊断的应用，为下一代AI驱动的eHealth系统标准化精神病诊断开辟了道路。
...
### [171] [Counterfactual-based Agent Influence Ranker for Agentic AI Workflows](https://arxiv.org/abs/2510.25612)
...
Method: 提出Counterfactual-based Agent Influence Ranker (CAIR)方法，通过反事实分析来评估AAW中每个智能体的影响力水平。

Result: 在包含30个不同用例和230个功能的AAW数据集上评估，CAIR能产生一致的排名，优于基线方法，并能有效提升下游任务的效果和相关性。

Conclusion: CAIR是首个能够有效评估AAW中智能体影响力的方法，为理解和优化AAW系统提供了重要工具。
...
### [172] [ALDEN: Reinforcement Learning for Active Navigation and Evidence Gathering in Long Documents](https://arxiv.org/abs/2510.25668)
*Tianyu Yang,Terry Ruas,Yijun Tian,Jan Philip Wahle,Daniel Kurzawe,Bela Gipp*

Main category: cs.AI

TL;DR: ALDEN是一个多轮强化学习框架，通过训练视觉语言模型作为交互式代理，使其能够主动导航长文档，使用fetch动作直接访问页面索引，并引入跨级奖励机制和视觉语义锚定机制来提升训练稳定性和性能。
...
Method: 提出ALDEN框架，使用多轮强化学习微调视觉语言模型，引入fetch动作直接访问页面索引，采用基于规则的跨级奖励进行密集过程监督，并提出视觉语义锚定机制来稳定训练。
...
### [173] [Navigation in a Three-Dimensional Urban Flow using Deep Reinforcement Learning](https://arxiv.org/abs/2510.25679)
*Federica Tonti,Ricardo Vinuesa*

Main category: cs.AI

TL;DR: 提出了一种基于深度强化学习的无人机最优导航策略，使用PPO+GTrXL架构结合流动感知机制，在复杂城市湍流环境中显著提升了导航成功率。


<details>
  <summary>Details</summary>
Motivation: 随着无人机在城市区域（用于配送和监控）的日益普及，需要开发能够在复杂城市湍流环境中有效导航的智能算法。

Method: 采用流动感知的PPO算法结合GTrXL架构，通过三维高保真城市流动模拟环境，为智能体提供丰富的湍流场信息。
...
### [174] [BambooKG: A Neurobiologically-inspired Frequency-Weight Knowledge Graph](https://arxiv.org/abs/2510.25724)
*Vanya Arikutharam,Arkadiy Ukolov*

Main category: cs.AI

TL;DR: BambooKG是一种基于频率权重的知识图谱，在非三元组边上添加权重来反映链接强度，减少信息丢失，在单跳和多跳推理任务上表现优于现有解决方案。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成虽然能让LLMs访问外部知识，但独立处理检索块，难以进行多跳或关系推理，尤其是跨文档推理。知识图谱通过三元组捕捉实体关系，但会遗漏不符合三元组结构的信息。

Method: 引入BambooKG知识图谱，在非三元组边上应用基于频率的权重，体现链接强度，借鉴赫布原理的"一起激发，一起连接"思想。

Result: 减少了信息丢失，在单跳和多跳推理任务上表现提升，超越了现有解决方案。
...
### [175] [TheraMind: A Strategic and Adaptive Agent for Longitudinal Psychological Counseling](https://arxiv.org/abs/2510.25758)
...
<details>
  <summary>Details</summary>
Motivation: 现有心理咨询LLM方法缺乏情感理解、自适应策略和跨会话治疗方法的长期记忆，与真实临床实践存在差距。
