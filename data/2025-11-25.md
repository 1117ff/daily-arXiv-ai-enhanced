...
TL;DR: AI模型通过正面身体图像和基本人体测量数据，可作为低成本体脂率估算的可行替代方案，RMSE为4.44%，R²为0.807。


<details>
  <summary>Details</summary>
Motivation: 传统体脂测量方法如DEXA扫描昂贵且不易获取，需要开发低成本、易获取的替代方案。

Method: 使用535个样本数据集，包括253个人体测量数据和282张网络爬取的正面身体图像，开发了基于ResNet的图像模型和基于人体测量的回归模型。

Result: 图像模型在体脂率估算上取得了RMSE 4.44%和R² 0.807的良好性能。

Conclusion: AI辅助模型能够提供可访问、低成本的体脂估算，支持未来健康和健身领域的消费应用。
...
Method: 使用多模态自动编码器在LUMA数据集上训练，通过最小化跨模态联合重构损失来学习模态不变语义结构，无需依赖大型配对或对比数据集。

Result: 在线性基线方法上显著提升了聚类和对齐指标（轮廓系数、ARI、NMI），表明基于重构的多模态嵌入可作为可扩展元数据生成和跨模态检索的基础。
...
TL;DR: 提出了一个覆盖25年、每日分辨率的野火数据集，包含240百万公顷区域和38个协变量，评估了多种时间序列预测模型在野火风险预测中的表现。


<details>
  <summary>Details</summary>
Motivation: 由于燃料条件、气象、地形和人类活动之间的复杂相互作用，野火风险预测仍然具有挑战性。目前缺乏支持长期时间建模、大规模空间覆盖和多模态驱动因素的公开基准数据集。

Method: 构建了一个25年、每日分辨率的野火数据集，覆盖不列颠哥伦比亚省及周边地区240百万公顷，包含38个协变量。评估了CNN、线性模型、Transformer和Mamba等多种时间序列预测模型。

Result: 创建了一个全面的野火基准数据集，并比较了不同模型架构的性能，同时研究了位置嵌入的有效性和不同火灾驱动因素的相对重要性。

Conclusion: 该数据集填补了野火预测领域的数据空白，为长期时间建模和大规模空间分析提供了重要资源，有助于推动数据驱动的野火风险预测研究。
...
TL;DR: 本文研究了透视畸变对多模态大语言模型OCR数据提取准确性的影响，发现结构识别准确率显著下降，但可通过简单旋转校正改善。


<details>
  <summary>Details</summary>
Motivation: 现实世界文档图像通常存在透视畸变而不仅仅是平面旋转，这会影响多模态LLMs的数据提取准确性，但相关研究较少。

Method: 通过观察典型文档畸变，将其近似为等腰梯形变换，将参数从8个减少到2个（旋转角度和畸变比例），在合成文档上测试Gemini-1.5-pro模型的表现。

Result: 文档畸变显著降低了结构识别准确率（与阅读顺序正确性相关），而字符识别准确率受影响较小。简单的旋转校正可以改善结构识别性能。

Conclusion: 透视畸变对多模态LLMs的OCR性能有显著影响，特别是结构识别方面，但可通过预处理技术缓解，这对实际应用具有重要意义。
...
TL;DR: 提出一种基于无迹卡尔曼滤波的多相机系统，将2D边界框或关键点标注融合为精确的3D地面真值，可处理遮挡并输出完整3D形状


<details>
  <summary>Details</summary>
Motivation: 需要为自动驾驶、监控和机器人等应用提供准确的3D地面真值，现有方法通常只能提供地面平面信息

Method: 使用无迹卡尔曼滤波器融合多相机系统的2D标注，通过单应性投影和UKF融合将2D图像坐标转换为鲁棒的3D世界坐标

Result: 在CMC、Wildtrack和Panoptic数据集上评估，相比现有3D地面真值显示出高精度的3D定位能力

Conclusion: 该方法提供可扩展的全自动多相机系统解决方案，仅使用2D图像标注即可输出完整3D形状
...
Method: 模型将图像分解为光照和反射率分量，通过三个模块进行优化：光照适应模块进行全局和局部亮度校正；反射率恢复模块使用空间-通道注意力进行噪声抑制和结构细节恢复；过曝光补偿模块重建饱和区域并平衡场景亮度。采用自监督重建、反射率平滑度、感知一致性和领域感知正则化损失进行训练。
...
### [7] [Novel View Synthesis from A Few Glimpses via Test-Time Natural Video Completion](https://arxiv.org/abs/2511.17932)
*Yan Xu,Yixing Wang,Stella X. Yu*

Main category: cs.CV

TL;DR: 提出了一种零样本、生成引导的框架，利用预训练视频扩散模型完成稀疏输入的新视角合成，通过不确定性感知机制和3D高斯泼溅实现高质量场景重建。


<details>
  <summary>Details</summary>
Motivation: 解决稀疏输入新视角合成问题，不仅填补空间间隙，还要完成自然视频的时空连续性，利用预训练视频扩散模型的强大先验知识。

Method: 采用测试时自然视频完成方法，使用预训练视频扩散模型生成伪视图，结合不确定性感知机制确保空间一致性，通过3D高斯泼溅进行场景重建，并建立迭代反馈循环。

Result: 在LLFF、DTU、DL3DV和MipNeRF-360数据集上，该方法在极端稀疏条件下显著优于强3D-GS基线，无需任何场景特定训练或微调。

Conclusion: 该方法能够从稀疏输入生成连贯、高保真的渲染结果，证明了生成引导框架在稀疏视图合成中的有效性。
...
### [8] [HSMix: Hard and Soft Mixing Data Augmentation for Medical Image Segmentation](https://arxiv.org/abs/2511.17614)
*Danyang Sun,Fadi Dornaika,Nagore Barrena*

Main category: cs.CV

TL;DR: HSMix是一种用于医学图像分割的局部图像编辑数据增强方法，通过硬混合和软混合技术结合两个源图像的同质区域来生成增强图像，同时保持局部语义信息。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割常受数据稀缺和过拟合问题困扰，现有自监督和半监督学习方法复杂且需要手工设计，而数据增强作为一种简单直接的解决方案在分割任务中的有效性尚未充分探索。

Method: HSMix通过硬混合将两个源图像的同质区域（超像素）组合创建增强图像，软混合则基于局部聚合的像素级显著性系数调整亮度。同时，两个源图像的分割掩码也进行相同的混合操作生成对应的增强掩码。

Result: 大量实验证明该方法在各种医学分割任务中有效，能够保持局部语义信息的同时丰富增强空间的多样性。

Conclusion: HSMix是一种即插即用、模型无关的解决方案，适用于多种医学成像模态，能够有效缓解医学图像分割中的数据稀缺问题。
...
### [9] [MimiCAT: Mimic with Correspondence-Aware Cascade-Transformer for Category-Free 3D Pose Transfer](https://arxiv.org/abs/2511.18370)
*Zenghao Chai,Chen Tang,Yongkang Wong,Xulei Yang,Mohan Kankanhalli*

Main category: cs.CV

TL;DR: 提出了MimiCAT模型，用于实现类别无关的3D姿态迁移，能够将不同结构角色（如人形到四足动物）的姿态进行迁移，通过软对应匹配解决结构差异问题。
...
Method: 构建百万级姿态数据集，提出级联Transformer模型MimiCAT，利用语义关键点标签学习软对应关系实现灵活的多对多匹配，将姿态迁移建模为条件生成过程，通过软对应匹配将源变换投影到目标角色，再使用形状条件表示进行细化。

Result: 广泛的定性和定量实验表明，MimiCAT能够跨不同角色迁移合理的姿态，显著优于仅限于狭窄类别迁移的先前方法。

Conclusion: MimiCAT通过软对应匹配和条件生成过程，成功解决了类别无关3D姿态迁移的挑战，实现了跨不同结构角色的高质量姿态迁移。
...
### [10] [Plug-and-Play Multi-Concept Adaptive Blending for High-Fidelity Text-to-Image Synthesis](https://arxiv.org/abs/2511.17615)
*Young-Beom Woo*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂多对象场景中表现不佳，会导致个性化区域和非个性化区域的意外改变，破坏提示结构和区域间交互，造成语义不一致。

Method: 使用引导外观注意力来忠实反映每个个性化概念的外观；采用掩码引导噪声混合策略保护非个性化区域完整性；提出背景稀释++策略减少概念泄漏。

Result: 广泛的实验结果表明，PnP-MIX在单概念和多概念个性化场景中始终优于现有方法，展现了其鲁棒性和卓越性能。

Conclusion: PnP-MIX无需额外模型调优就能实现多概念的高质量集成，解决了现有方法在复杂场景中的局限性。
...
### [11] [ReCoGS: Real-time ReColoring for Gaussian Splatting scenes](https://arxiv.org/abs/2511.18441)
*Lorenzo Rutayisire,Nicola Capodieci,Fabio Pellacini*

Main category: cs.CV

TL;DR: 提出了一种用于高斯溅射场景的实时交互式重新着色方法，允许用户精确选择和重新着色预训练场景中的区域。


<details>
  <summary>Details</summary>
Motivation: 现有基于2D扩散模型的多视角数据集生成方法存在视角不一致、缺乏细粒度控制和高计算需求等问题，特别是在重新着色任务中需要更精确的控制。

Method: 开发了一个用户友好的流水线，支持在预训练高斯溅射场景中精确选择区域并进行重新着色，同时提供了一个交互式工具来展示实时性能。

Result: 该方法实现了实时交互式重新着色，代码已在GitHub上开源。

Conclusion: 该工作为高斯溅射场景的编辑任务提供了一种高效且用户友好的重新着色解决方案，特别强调了实时交互性能。
...
### [12] [Foundational Question Generation for Video Question Answering via an Embedding-Integrated Approach](https://arxiv.org/abs/2511.17618)
*Ju-Young Oh*

Main category: cs.CV

TL;DR: FIQ框架通过生成描述性Q&A对来增强视频问答模型的基础理解能力，结合VQ-CAlign模块对齐问题嵌入与视觉特征，在SUTD-TrafficQA数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VQA方法主要依赖事件中心的Q&A对，缺乏对场景基础信息（如物体类别、空间配置、视觉属性）的全面理解，限制了模型的泛化和推理能力。

Method: 提出FIQ框架：1）从视频中提取描述性信息生成Q&A对，丰富数据集；2）设计VQ-CAlign模块对齐任务特定问题嵌入与视觉特征，保留上下文线索。

Result: 在SUTD-TrafficQA数据集上的实验结果表明，FIQ超越了现有基线方法，达到了最先进的性能水平。

Conclusion: 通过增强对视频内容的基础理解，FIQ框架显著提升了VQA模型的推理能力和泛化性能，证明了基础信息理解在视频问答中的重要性。
...
### [13] [Neural Texture Splatting: Expressive 3D Gaussian Splatting for View Synthesis, Geometry, and Dynamic Reconstruction](https://arxiv.org/abs/2511.18873)
*Yiming Wang,Shaofei Wang,Marko Mihajlovic,Siyu Tang*

Main category: cs.CV

TL;DR: 本文提出Neural Texture Splatting (NTS)，通过全局神经场为每个3D高斯基元预测局部外观和几何场，显著提升3DGS在各种重建任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 3DGS使用3D高斯核建模局部变化的能力有限，现有增强方法在通用重建场景中效果不佳。本文旨在在稀疏和密集输入设置下，在多种重建任务中实现性能提升。

Method: 引入NTS方法，核心是使用三平面和神经解码器的混合全局神经场，为每个基元预测局部外观和几何场，实现全局信息共享和高效建模。

Result: 在多个基准测试中，NTS一致改进模型并达到最先进结果，显著减小模型尺寸并实现视图和时间相关的效果。

Conclusion: NTS通过全局神经场建模局部纹理场，在保持高效性的同时显著提升了3DGS的表达能力和泛化性能。
...
### [14] [Rethinking the Encoding and Annotating of 3D Bounding Box: Corner-Aware 3D Object Detection from Point Clouds](https://arxiv.org/abs/2511.17619)
*Qinghao Meng,Junbo Yin,Jianbing Shen,Yunde Jia*

Main category: cs.CV

TL;DR: 提出角点对齐回归方法，将3D目标检测的预测目标从不稳定的物体中心转移到几何信息丰富的角点，解决了LiDAR点云前表面偏置导致的中心稀疏问题。


<details>
  <summary>Details</summary>
Motivation: 传统的中心对齐回归在LiDAR 3D目标检测中存在根本性不稳定问题：由于LiDAR点云的前表面偏置特性，物体中心经常落在BEV视图的稀疏或空区域，导致边界框预测噪声大且不准确。

Method: 提出角点对齐回归，将预测目标从不稳定的中心转移到位于密集可观测区域的几何信息丰富角点。利用角点和图像2D框之间的几何约束，可以从角点标注中恢复3D边界框的部分参数，实现弱监督范式。设计了一个简单有效的角点感知检测头，可插入现有检测器中。

Result: 在KITTI数据集上的实验表明，该方法比基于中心的基线性能提高了3.5% AP，仅使用BEV角点点击就达到了全监督准确率的83%。

Conclusion: 角点感知回归策略有效解决了LiDAR 3D检测中的中心不稳定问题，通过角点对齐回归显著提升了检测性能，同时支持弱监督学习范式。
...
### [15] [NVGS: Neural Visibility for Occlusion Culling in 3D Gaussian Splatting](https://arxiv.org/abs/2511.19202)
*Brent Zoomers,Florian Hahlbohm,Joni Vanherck,Lode Jorissen,Marcus Magnor,Nick Michiels*

Main category: cs.CV

TL;DR: 提出了一种新的方法，通过小型共享MLP学习3D高斯模型中所有高斯的视点相关可见性函数，实现遮挡剔除，提高渲染效率。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射可以利用视锥体剔除和细节层次策略加速包含大量基元的场景渲染，但高斯的半透明特性阻碍了遮挡剔除这一高效技术的应用。

Method: 使用跨场景资产实例共享的小型MLP学习高斯模型的视点相关可见性函数，在光栅化前查询视锥体内高斯的可见性，丢弃被遮挡的基元。

Result: 该方法在组合场景的VRAM使用和图像质量方面优于当前最先进技术，结合实例化光栅器和遮挡剔除MLP，与现有LoD技术具有互补特性。

Conclusion: 通过神经查询和实例化软件光栅器的集成，成功解决了3D高斯渲染中遮挡剔除的挑战，显著提升了渲染性能。
...
TL;DR: 提出了一种1.58位卷积增强表达能力，以及预BN残差连接来稳定优化，首次成功实现了BNN中深度卷积的二值化，在ImageNet上达到33M OPs，在多个数据集上显著超越现有方法。
...
Method: 采用1.58位卷积增强表达能力，引入预BN残差连接来改善Hessian条件数从而稳定优化过程。

Result: 在ImageNet上使用MobileNet V1达到33M OPs，在CIFAR-10、CIFAR-100、STL-10、Tiny ImageNet和Oxford Flowers 102等多个数据集上准确率提升最高达9.3个百分点。

Conclusion: 该方法首次成功实现了BNN中深度卷积的二值化，在BNN领域建立了新的最先进水平，在各种数据集上一致优于现有方法。
...
TL;DR: 提出了一种加速基于分数的扩散模型的新框架，通过将标准稳定扩散模型转换为Fokker-Planck公式，并采用跨矩阵Krylov投影方法来利用矩阵间的数学相似性，从而显著减少计算时间。


<details>
  <summary>Details</summary>
Motivation: 标准稳定扩散模型在训练涉及大量图像时会导致高计算成本，需要一种更高效的求解方法。

Method: 首先将标准稳定扩散模型转换为Fokker-Planck公式，然后提出跨矩阵Krylov投影方法，利用从"种子"矩阵构建的共享子空间来快速求解后续"目标"矩阵。
...
TL;DR: 本文提出UPMI方法，通过在元特征空间进行概率插值增强，解决儿科胰腺炎诊断中样本有限和多模态成像复杂性的挑战，相比基线方法获得约5%的AUC提升。


<details>
  <summary>Details</summary>
Motivation: 儿科胰腺炎是一种进展性炎症疾病，临床诊断面临挑战。基于机器学习的方法由于样本量有限和多模态成像复杂性也面临诊断困难。

Method: 提出上游概率元插值(UPMI)策略，在低维元特征空间而非图像空间操作。通过模态特定逻辑回归生成概率输出，转换为7维元特征向量，然后使用类条件高斯混合模型采样合成元特征，结合真实元特征训练随机森林元分类器。

Result: 在67名儿科患者的配对T1W/T2W MRI数据上，UPMI实现平均AUC 0.908±0.072，相比仅使用真实数据的基线(AUC 0.864±0.061)获得约5%的相对增益。

Conclusion: UPMI方法通过元特征空间的数据增强有效提升了儿科胰腺炎的诊断性能，为解决小样本多模态医学图像分类问题提供了有效方案。
...
TL;DR: 提出了一种基于可区分性和活跃度的典型集精炼方法，用于改进OOD检测中的激活校正，通过通道感知和偏度校正来提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的激活校正方法往往忽略通道的内在特性和分布偏度，导致典型集估计不准确，从而错误地包含异常激活。

Method: 提出基于可区分性和活跃度的典型集精炼方法，将激活校正为通道感知的典型集；引入基于偏度的精炼来缓解典型集估计中的分布偏差；使用校正后的激活计算能量得分进行OOD检测。

Result: 在ImageNet-1K和CIFAR-100基准测试中达到了最先进的性能，并在不同主干网络和得分函数上有效泛化。

Conclusion: 该方法通过通道感知和偏度校正显著提升了OOD检测的准确性和鲁棒性。
...
TL;DR: SWITCH是一个具身智能基准测试，评估AI在现实世界控制界面交互中的能力，包括任务感知VQA、语义UI接地、动作生成、状态转换预测和结果验证。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试很少测试AI在真实环境中的接地能力、部分可观测性和事后验证，而这些对于自主智能与物理世界基础设施交互至关重要。

Method: 通过迭代发布创建SWITCH基准测试，第一版SWITCH-Basic包含351个任务，覆盖98个真实设备和家电，使用第一人称RGB视频输入评估五种互补能力。

Result: 商业和开源LMM在单步交互中表现不一致，过度依赖文本线索而忽视视觉或视频证据，高总分可能掩盖这些失败。

Conclusion: SWITCH提供了数据、代码和保留集，支持可复现评估和社区贡献，推动更具挑战性的基准迭代和训练数据集创建。
...
TL;DR: 开发了一个完整的深度学习系统用于脑肿瘤MRI图像自动分类，包含6种架构（5个预训练模型和1个紧凑CNN），在准确性、可解释性和部署性方面均有突破。


<details>
  <summary>Details</summary>
Motivation: 解决脑肿瘤MRI分类中的标准化评估、可解释性和边缘设备部署问题，为资源受限的医疗系统提供可信AI解决方案。

Method: 使用6种CNN架构（包括5个ImageNet预训练模型和1个紧凑CNN），统一预处理和训练协议，采用AdamW优化器、余弦退火学习率和早停策略，结合Grad-CAM和GradientShap进行可解释性分析。

Result: Inception-ResNet V2达到99.53%测试准确率，紧凑CNN（1.31M参数）达到96.49%准确率，比Inception-ResNet V2小100倍，在边缘设备上实现375ms实时推理。

Conclusion: 提供了一个端到端的可信AI框架，在准确性、可解释性和部署性方面表现优异，适合在先进和资源受限的医疗系统中进行临床筛查和分诊。
...
TL;DR: 提出MedPEFT-CL框架，通过双阶段架构解决医学视觉语言分割模型在持续学习中的灾难性遗忘问题，使用参数高效微调方法减少可训练参数，同时保持跨模态学习能力。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言分割模型在适应新解剖结构时会出现灾难性遗忘，需要完全重新训练，这限制了其临床部署。针对医学视觉语言任务的持续学习方法研究不足。

Method: 基于CLIPSeg的双阶段架构：自适应学习阶段使用语义相似性适配器分配和参数高效微调；知识巩固阶段采用双向Fisher记忆协调，形成强化循环。

Result: 在多样化医学数据集上的实验表明，该框架在最小参数开销下实现了优越的遗忘缓解和性能保持。

Conclusion: MedPEFT-CL框架为医学视觉语言场景的持续学习提供了有效解决方案，能够高效学习新任务并保持先前知识。
...
TL;DR: 对过去10年150+篇关于以人为中心的空中监视任务的论文进行系统性综述，涵盖无人机等空中平台在人类检测、识别和重识别方面的计算机视觉和机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 随着空中平台和成像传感器的快速发展，空中监视因其在规模、移动性、部署和隐蔽观察能力方面的优势而兴起，需要系统性地总结这一领域的技术进展。

Method: 通过识别空中监视相比地面监视的独特挑战，整理可用的空中数据集，并深入分析文献中解决这些挑战的方法和技术改进。

Result: 提供了空中监视任务的全面技术分析，包括人类检测、识别和重识别等核心任务的研究现状和方法评估。

Conclusion: 讨论了当前研究存在的差距和开放性问题，为未来研究方向提供指导。
...
TL;DR: 提出了VMRMOT框架，通过引入运动模态来增强视觉模态与语言参考之间的对齐，解决了RMOT中静态参考无法捕捉动态运动变化的问题。
...
Method: 提出VMRMOT框架：1）从物体动态行为中提取运动感知描述，利用MLLMs的时间推理能力提取运动特征作为运动模态；2）设计视觉-运动-参考对齐模块，分层对齐视觉查询与运动和参考线索；3）开发运动引导预测头，利用运动模态增强预测头性能。

Result: 在多个RMOT基准上的广泛实验表明，VMRMOT优于现有的最先进方法。

Conclusion: VMRMOT是首个在RMOT任务中利用MLLMs进行视觉-参考对齐的方法，通过引入运动模态有效解决了静态参考与动态视觉之间的对齐问题。
...
TL;DR: 本文研究LLM和LVLM在计数任务中如何表示和处理数字信息，通过因果中介分析和激活修补实验，发现模型通过分层机制编码计数信息，并识别出内部计数器机制。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型和视觉语言模型如何表示和计算数字信息，特别是在计数任务中的工作机制，这对于理解模型的内部表示和处理能力具有重要意义。

Method: 使用重复文本和视觉项目的受控实验，通过因果中介分析和激活修补技术，开发专门的CountScope工具进行机制可解释性分析。

Result: 发现单个token或视觉特征编码潜在位置计数信息，分层分析显示数字表示逐步出现，识别出内部计数器机制，视觉模型中数字信息出现在视觉嵌入中并随空间构图变化。

Conclusion: 计数在LLM中是一个结构化的分层过程，在LVLM中遵循相同模式但受视觉编码器特性影响，模型依赖结构线索作为计数跟踪的捷径。
...
TL;DR: 开发合成基准数据集和评估框架，分析视觉语言模型在计数任务中的注意力分配和性能变化，并测试注意力干预对计数性能的影响。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在回答图像视觉属性问题时往往依赖训练中学习的内在偏见，特别是在需要聚焦图像特定区域的计数任务中，这些偏见更加明显。

Method: 构建合成基准数据集和评估框架，使用开源VLM分析注意力分配随输入参数的变化，并实施基于注意力的干预来调节视觉标记的焦点。

Result: 实验显示VLM计数性能仍然具有挑战性，特别是在高视觉或语言复杂度下，但某些注意力干预可以带来计数性能的适度提升。

Conclusion: 虽然VLM在计数任务中表现仍有困难，但通过注意力干预可以改善其性能，特别是在复杂视觉条件下。
...
TL;DR: 提出AngioDG方法解决X射线冠脉造影血管分割中的单源域泛化问题，通过通道正则化策略提升模型泛化能力，在6个数据集上取得最佳域外性能。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死因，XCA是实时心脏介入的金标准。血管分割有助于定量评估，但成像协议和患者差异导致域偏移，且标注数据缺乏，需要单源域泛化解决方案。现有方法主要基于数据增强，可能无法有效缓解对增强域的过拟合。

Method: 提出AngioDG方法，采用通道正则化策略：识别早期特征通道对任务特定指标的贡献以促进可解释性，然后重新加权通道以校准和放大域不变特征，同时衰减域特定特征。

Result: 在6个X射线冠脉造影数据集上评估，相比其他方法取得了最佳的域外性能，同时保持了稳定的域内测试性能。
...
TL;DR: VLMs在数字健康应用中表现有限：在卒中康复视频分析中，剂量估计与排除视觉信息的基线相当，损伤评分无法可靠预测，但通过优化提示和后期处理，可在无任务特定训练下实现高层活动分类和近似剂量计数。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型在数字健康领域的潜力，特别是解决卒中康复中的两个基本挑战：从视频中自动量化康复剂量和损伤程度。

Method: 将康复剂量和损伤量化问题构建为运动识别任务，使用VLMs进行评估，在29名健康对照和51名卒中幸存者队列上进行测试，采用优化提示和后期处理策略。

Result: 当前VLMs缺乏精细运动理解能力：剂量估计与无视觉基线相当，损伤评分预测不可靠；但能分类高层活动、检测运动和抓握，对轻度损伤和健康参与者的剂量计数误差在25%以内。

Conclusion: VLMs在数据驱动的卒中康复中既有当前局限性也有新兴机会，需要进一步发展精细运动理解能力，但在无任务特定训练下已显示出一定潜力。
...
TL;DR: VisReason是一个大规模视觉思维链数据集，包含489K标注样本，旨在提升多模态大语言模型的逐步视觉推理能力。通过VisReason-Pro子集和深度信息增强，显著提高了模型的推理准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉思维链资源规模小、领域特定，缺乏人类逐步推理结构，限制了多模态大语言模型在视觉推理方面的潜力。

Method: 构建VisReason数据集（489K样本，覆盖4个领域）和VisReason-Pro子集（165K，使用GPT专家标注，包含深度信息），并在Qwen2.5-VL模型上进行微调。

Result: 微调后模型在逐步视觉推理准确性、可解释性和跨基准泛化能力方面均有显著提升。

Conclusion: VisReason为培养类人视觉推理能力奠定了基础，推动了下一代多模态智能的发展。
...
TL;DR: 稀疏自编码器(SAEs)可用于从基础模型表示中进行开放式特征发现，支持科学数据中未知模式的探索性发现，而不仅仅是预定义目标的确认。


<details>
  <summary>Details</summary>
Motivation: 科学档案包含海量数据，但现有方法只能针对预定义目标提取结构，不支持开放式发现未知模式。需要一种能够从基础模型表示中自主发现特征的方法。

Method: 使用稀疏自编码器(SAEs)从基础模型表示中学习特征，在受控重发现研究中评估特征与语义概念的对齐度，并与无标签替代方法比较。

Result: 在生态图像中，该方法无需分割或部件标签即可发现细粒度解剖结构，在概念对齐指标上表现优于无标签替代方法。
...
TL;DR: AEGIS是首个用于3D高斯化身的隐私保护身份掩蔽框架，通过在颜色系数上添加对抗性扰动来隐藏身份特征，同时保持感知真实性和功能性。


<details>
  <summary>Details</summary>
Motivation: 随着照片级真实感3D面部化身的广泛应用，特别是使用高效3D高斯溅射表示的系统，带来了在线身份盗窃的新风险。现有方法主要针对2D图像，缺乏对动态3D化身的鲁棒、视角一致的身份保护。

Method: AEGIS对高斯颜色系数应用对抗性扰动，通过预训练的人脸验证网络进行指导，确保跨多个视角的一致保护，无需重新训练或修改化身几何结构。
...
TL;DR: SPIDER是一个通用的特征匹配框架，通过结合2D和3D对应关系来改进跨域图像匹配，在大基线场景下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决跨域图像匹配中由于外观、尺度和视角变化带来的挑战，特别是现有3D基础模型在精细几何细节和大幅视角变化下的局限性。

Method: 提出SPIDER框架，使用共享特征提取主干网络，配备两个专用网络头分别估计2D和3D对应关系，从粗到细进行匹配。

Result: SPIDER在包含大基线的无约束场景图像匹配基准测试中显著优于现有最先进方法。

Conclusion: SPIDER作为一个通用图像匹配方法，通过整合2D和3D对应关系，在跨域匹配中表现出强大的能力。
...
TL;DR: CORA是一个半监督推理分割框架，通过条件视觉指令、噪声伪标签过滤和标记级对比对齐，在有限标注数据下实现稳健的推理分割，在Cityscapes和PanNuke数据集上仅用少量标注就达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决推理分割中高质量像素标注成本高、语言监督有限导致泛化能力差的问题，旨在开发能够在最小监督下进行稳健推理分割的方法。

Method: 1) 条件视觉指令编码对象间的空间和上下文关系；2) 基于多模态LLM在语义等价查询中输出一致性的噪声伪标签过滤；3) 标注样本和伪标注样本间的标记级对比对齐以增强特征一致性。

Result: 在Cityscapes数据集上仅用100张标注图像就超越基线+2.3%，在PanNuke数据集上仅用180张标注图像提升性能+2.4%，达到最先进水平。

Conclusion: CORA框架通过半监督学习有效解决了推理分割中的标注瓶颈问题，在有限监督下实现了稳健的性能，为复杂指令驱动的分割任务提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像包含丰富的光谱信息，但光谱混合常常掩盖纯材料特征，需要开发能够处理材料内在变异性的解混方法。

Method: 结合Transformer架构和狄利克雷先验的变分自编码器，将材料建模为捆绑端元，解码器预测每个端元的均值光谱和结构化协方差，编码器通过Transformer获取狄利克雷分布的丰度。

Result: 在Samson、Jasper Ridge和HYDICE Urban三个基准数据集上，LDVAE-T在丰度估计和端元提取方面均优于现有最先进模型。

Conclusion: LDVAE-T通过结合Transformer的全局建模能力和狄利克雷先验的物理约束，能够有效处理材料内在变异性，同时保持物理可解释性，在高光谱解混任务中表现出色。
...
TL;DR: 本文比较了CNN和ViT在检测AI生成卫星图像方面的性能，发现ViT在准确率和鲁棒性上显著优于CNN，并利用可解释性方法验证了模型的可信度。


<details>
  <summary>Details</summary>
Motivation: 随着StyleGAN2和Stable Diffusion等生成模型的快速发展，卫星图像的真实性面临威胁，而现有的深度伪造检测主要针对人脸图像，卫星图像检测存在地形不一致和结构伪影等独特挑战。

Method: 使用包含13万张标记RGB图像的DM-AER和FSI数据集，全面比较CNN和ViT的性能，并采用Grad-CAM和Chefer注意力归因等架构特定的可解释性方法增强模型透明度。

Result: ViT在准确率（95.11%）上显著优于CNN（87.02%），且在鲁棒性方面表现更好，能够有效检测合成图像中的结构不一致和重复纹理模式。
...
TL;DR: 提出了首个专门评估世界模型在无地图路径规划中表现的基准Target-Bench，包含450个机器人采集的视频序列和SLAM基准轨迹。评估显示当前最先进模型在机器人规划任务中表现有限，但通过微调可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 虽然当前世界模型能生成高度逼真的视频，但它们在机器人路径规划方面的能力仍不明确且缺乏量化评估。需要专门的基准来评估世界模型在真实环境中面向语义目标的路径规划能力。

Method: 构建Target-Bench基准，包含450个机器人采集的视频序列，涵盖45个语义类别，提供SLAM基准轨迹。开发评估流程从生成视频中恢复相机运动，使用5个互补指标量化目标到达能力、轨迹精度和方向一致性。

Result: 评估显示最佳现成模型(Wan2.2-Flash)仅获得0.299总分，表明当前世界模型在机器人规划任务中存在显著局限。通过在325个场景上微调开源5B参数模型，总体得分达到0.345，比基础版本提升400%以上，比最佳现成模型高15%。

Conclusion: 当前世界模型在机器人路径规划方面能力有限，但通过针对性微调可显著改善。Target-Bench为评估和改进世界模型的规划能力提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于拼接架构的高效VLMs经常无法区分语义匹配和非匹配的图像-文本对，这是导致对象幻觉的关键因素。

Method: 引入注意力引导的高效视觉语言模型(AGE-VLM)，通过交叉注意力层在预训练的小语言模型中注入视觉能力，利用SAM的空间知识来强制模型关注正确的图像区域。

Result: 在各种视觉中心基准测试中，该方法优于或与先前的高效VLM工作相当，显著减少了幻觉现象。
...
TL;DR: Pillar-0是一个放射学基础模型，在42,990个腹部-盆腔CT、86,411个胸部CT、14,348个头部CT和11,543个乳腺MRI上预训练，配合RATE框架提取366个放射学发现的结构化标签，在多项任务中超越现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 放射学在现代医学中至关重要，但影像量增长远超人力增长。现有医学模型处理3D CT和MRI时降维为低保真2D切片，丢弃关键灰度对比信息，且缺乏反映真实临床实践的评价框架。

Method: 使用Pillar-0放射学基础模型，在大型学术中心的多种CT和MRI数据上预训练，配合RATE框架利用LLM提取结构化标签，构建可扩展的评价体系。
...
Conclusion: Pillar-0和RATE共同提供了一个开放、临床严谨的基础，用于构建高性能放射学系统，解决了以往因计算、数据和评价限制而不可行的应用问题。
...
TL;DR: PL-Stitch是一个自监督学习框架，通过Plackett-Luce模型利用视频帧的时间顺序作为监督信号，提升对程序性活动（如手术和烹饪）的表示学习能力。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法忽视了程序性活动的结构化特性，无法区分正向和反向时间序列，缺乏对程序顺序的感知能力。

Method: 提出PL-Stitch框架，包含两个基于Plackett-Luce模型的概率目标：主要目标训练模型按时间顺序排序采样帧，次要目标通过时空拼图损失捕捉细粒度跨帧对象相关性。

Result: 在五个手术和烹饪基准测试中表现优异，手术阶段识别提升11.4个百分点（Cholec80），烹饪动作分割提升5.7个百分点（Breakfast）。

Conclusion: PL-Stitch有效提升了程序性视频表示学习能力，证明了利用时间顺序作为监督信号的重要性。
...
Result: 在两个公开室内雷达数据集上的评估显示，该方法在HIBER数据集上AP提升+4.22，在MMVR数据集上AP提升+11.02，超越了现有最先进方法。

Conclusion: REXO通过显式的跨视角特征关联和3D边界框扩散，有效解决了复杂室内场景下的雷达目标检测问题，显著提升了检测性能。
...
TL;DR: 提出了一个重要性加权的非独立同分布采样框架，通过联合抽取多个样本来覆盖流模型分布的关键区域，同时通过估计的重要性权重保持无偏估计。
...
Result: 经验上，该方法产生了多样化、高质量的样本，并准确估计了重要性权重和期望，推进了流匹配模型输出的可靠表征。

Conclusion: 提出的重要性加权非独立同分布采样框架能够有效解决流匹配模型期望估计中的高方差问题，通过多样性机制和重要性权重学习实现了更可靠的模型输出表征。
...
Method: QAL结合了覆盖率加权的最近邻项和未覆盖真实数据吸引项，将召回率和精确度显式解耦为可调组件。
...
TL;DR: 本研究利用BiomedCLIP基础模型进行乳腺影像BI-RADS密度分类，通过多模态训练和加权对比学习解决类别不平衡问题，在多种成像模态上展现了良好的泛化能力和临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 基础模型在专业医学影像任务中具有潜力，但在乳腺影像领域的有效性尚未充分探索。本研究旨在解决模型泛化挑战，探索基础模型在乳腺密度分类中的应用。

Method: 使用BiomedCLIP基础模型，采用多模态乳腺影像数据（合成2D图像、数字乳腺摄影和数字乳腺断层合成），通过加权对比学习处理类别不平衡，比较单模态和多模态训练方法。

Result: 多模态和单模态方法准确率相似（0.74 vs 0.73），但多模态模型在多种成像模态上具有更广泛适用性，AUC值始终高于0.84。在RSNA和EMBED数据集的外部验证中AUC范围为0.80-0.93。GradCAM可视化显示一致的临床相关注意力模式。

Conclusion: 基础模型在乳腺影像应用中具有巨大潜力，为未来扩展到诊断任务奠定了基础，展示了良好的泛化能力和临床可解释性。
...
Conclusion: 视频扩散模型可以作为统一的动作-状态转换器，通过统一处理图像编辑和视频预测任务，在两个任务上都能获得性能提升。
...
TL;DR: JigsawComm是一个语义感知的通信高效多智能体协同感知框架，通过提取语义相关特征和预测特征效用，在有限带宽下最大化感知精度。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体协同感知中通信带宽受限的问题，现有方法未考虑语义相关性和跨智能体冗余性。

Method: 使用正则化编码器提取语义相关稀疏特征，轻量级特征效用估计器预测特征贡献度，通过元效用图交换和最优传输策略选择高效用特征。
...
TL;DR: 提出了一种数据高效微调策略，使用稀疏低质量合成数据学习文本到视频扩散模型的生成控制，效果优于使用逼真真实数据微调的模型


<details>
  <summary>Details</summary>
Motivation: 传统方法需要大量高质量数据集来微调文本到视频扩散模型以添加生成控制，但这些数据集难以获取

Method: 使用稀疏低质量合成数据进行微调，而非逼真的真实数据

Result: 该方法不仅实现了期望的控制功能，而且效果优于使用逼真真实数据微调的模型

Conclusion: 提供了解释该现象的框架，证明使用简单合成数据微调在添加生成控制方面具有优势
...
TL;DR: 提出了MGA-VQA框架，通过多模态集成解决DocVQA中的空间关系建模、高分辨率处理、多跳推理和可解释性问题


<details>
  <summary>Details</summary>
Motivation: 当前DocVQA方法在显式空间关系建模、高分辨率文档处理效率、多跳推理和可解释性方面存在不足

Method: 集成token级编码、空间图推理、记忆增强推理和问题引导压缩的多模态框架，引入可解释的基于图的决策路径和结构化记忆访问

Result: 在六个基准测试（FUNSD、CORD、SROIE、DocVQA、STE-VQA、RICO）上表现出优越的准确性和效率，在答案预测和空间定位方面均有持续改进

Conclusion: MGA-VQA通过可解释的图推理和记忆增强机制，有效提升了DocVQA任务的性能、效率和透明度
...
TL;DR: FastMMoE是一个无需训练的多模态大语言模型加速框架，通过专家激活减少和路由感知的token剪枝，在保持95.5%性能的同时减少高达55.0%的计算量。


<details>
  <summary>Details</summary>
Motivation: 高分辨率视觉输入导致视觉token序列过长和推理延迟高，需要减少冗余视觉token以降低计算/内存负担，同时保持性能，使MLLM能在资源受限或延迟敏感场景中部署。

Method: 提出FastMMoE框架，包含两个互补策略：1) 视觉token的专家激活减少，最小化不必要的专家计算；2) 路由感知的token剪枝，利用路由概率分布的相似性识别并移除高度冗余的视觉token。

Result: 在DeepSeek-VL2和InternVL3.5等大规模MoE-MLLMs上实验，FastMMoE能减少高达55.0%的FLOPs，同时保留约95.5%的原始性能，在多个保留率下持续优于FastV和SparseVLM等密集模型剪枝基线。

Conclusion: FastMMoE为基于MoE的MLLMs提供了一种有效的训练免费加速方案，通过路由分析视角显著减少了计算开销，同时保持了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型计算需求大，知识蒸馏在语言和视觉领域已有成功应用，但在CLIP风格模型中的应用仍有限，主要局限于小规模教师模型和窄范围评估任务。

Method: 对一系列CLIP风格教师模型进行系统性蒸馏研究，涵盖从标准基线到大规模最先进模型的范围。

Result: 与NLP和视觉领域的趋势相反，更强的教师模型并不总是产生更好的学生模型；现有蒸馏框架在多模态任务（如视觉问答）中往往导致性能下降。
...
TL;DR: MINDiff提出了一种新的负注意力机制，在推理阶段通过修改交叉注意力来抑制主题在无关区域的影响，从而解决个性化文本到图像模型中的过拟合问题，无需重新训练模型。
...
Result: 定性和定量实验表明，MINDiff比类别特定的先验保持损失更有效地缓解过拟合，且可直接应用于现有DreamBooth模型。

Conclusion: MINDiff在推理阶段有效解决了过拟合问题，提供了更好的语义控制和文本对齐，无需修改模型架构或重新训练。
...
TL;DR: DAVDD是一个基于预训练的音频-视觉数据集蒸馏框架，通过解耦模态特征为公共和私有表示，解决了跨模态对齐和模态特定信息保护的问题。
...
Method: 使用预训练编码器获取稳定模态特征，通过轻量解耦器将特征分解为公共和私有表示，采用公共跨模态匹配和样本-分布联合对齐策略。
...
TL;DR: CUS-GS是一种紧凑的统一结构化高斯泼溅表示，通过体素化锚点结构连接多模态语义特征与结构化3D几何，在仅使用6M参数的情况下达到与35M参数方法相当的竞争性能。
...
Method: 设计体素化锚点结构构建空间支架，从基础模型中提取多模态语义特征；引入多模态潜在特征分配机制统一外观、几何和语义；提出特征感知显著性评估策略动态指导锚点生长和剪枝。

Result: 在仅使用6M参数的情况下，性能与使用35M参数的最先进方法相当，实现了性能与模型效率之间的优秀权衡。
...
TL;DR: 提出了ADSA（自适应软标签对齐模块）来解决长尾数据集蒸馏中的软标签偏差问题，在ImageNet-1k-LT上显著提升了尾类准确率
...
Result: 在ImageNet-1k-LT上，ADSA将尾类准确率提升高达11.8%，整体准确率达到41.4%（EDC方法，IPC=50）

Conclusion: ADSA提供了一个在有限标签预算下和多种蒸馏技术中都能稳健泛化的解决方案
...
TL;DR: 提出FASR方法，通过频率自适应锐度正则化改进3D高斯溅射的泛化能力，解决稀疏观测下的过拟合问题


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在少样本场景中缺乏对新视角的泛化能力，因为会过度拟合稀疏观测数据

Method: 重新构建3DGS训练目标，引入频率自适应锐度正则化，根据图像局部频率设置正则化权重和邻域半径

Result: 在多种配置的数据集上，该方法持续改进了广泛的基线方法，防止了新视角中的浮动伪影并重建了精细细节

Conclusion: FASR方法有效提升了3D高斯溅射的泛化性能，在少样本场景下实现了更好的新视角合成效果
...
TL;DR: PA-FAS通过构建高质量扩展推理序列和答案混排机制，解决了多模态人脸反欺诈中监督微调+强化学习的局限性，显著提升了多模态推理准确性和跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 多模态人脸反欺诈面临多模态推理复杂性、高质量标注稀缺等问题，现有监督微调+强化学习方法存在推理路径受限和推理混淆的局限性。

Method: 提出PA-FAS方法：1）从有限标注构建高质量扩展推理序列，丰富推理路径；2）在监督微调阶段引入答案混排机制，强制全面多模态分析。

Result: PA-FAS显著提升了多模态推理准确性和跨域泛化能力，更好地统一了多模态融合、泛化性和可解释性。
...
TL;DR: MambaTAD是一个基于状态空间模型的时序动作检测方法，通过引入对角掩码双向状态空间模块和全局特征融合头，解决了长范围动作实例检测的挑战，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统时序动作检测方法在处理长跨度动作实例时缺乏全局感知能力，而现有的结构化状态空间模型在时序动作检测中面临时间上下文衰减和自元素冲突的问题。

Method: 提出MambaTAD模型，包含对角掩码双向状态空间模块促进全局特征融合，以及全局特征融合头进行多粒度特征精炼，采用端到端单阶段检测方式，使用状态空间时序适配器降低计算成本。

Result: 在多个公开基准测试中，MambaTAD实现了优越的时序动作检测性能。

Conclusion: MambaTAD通过创新的状态空间模型设计，有效解决了长范围动作检测的挑战，为时序动作检测提供了新的解决方案。
...
TL;DR: 提出UniRSCD统一遥感变化检测框架，通过状态空间模型和频率变化提示生成器统一编码，无需专门解码器，支持BCD、SCD、BDA等多种粒度变化检测任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要专家知识设计专门解码器来补偿编码信息损失，这限制了模型通用性，且在灾害等突发变化场景下选择最优模型存在不确定性。

Method: 基于状态空间模型，使用频率变化提示生成器作为统一编码器，动态扫描双时相全局上下文，整合高频细节和低频整体信息；统一解码器和预测头通过分层特征交互和任务自适应输出映射建立共享表示空间。

Result: 在五个数据集上取得领先性能，包括LEVIR-CD（二值变化）、SECOND（语义变化）和xBD（建筑损伤评估）数据集。

Conclusion: UniRSCD框架能够适应多种变化检测任务，统一了不同输出粒度的变化检测需求，具有很好的通用性和性能表现。
...
TL;DR: V2X-RECT是一个针对高密度交通环境的轨迹预测框架，通过多源身份匹配校正、交通信号引导的交互模块和局部时空坐标编码，解决了身份切换、冗余交互和重复编码问题，实现了更高效准确的预测。


<details>
  <summary>Details</summary>
Motivation: 在密集交通场景中，频繁的目标身份切换阻碍了跨视角关联和融合，多源信息在编码阶段产生冗余交互，传统以车辆为中心的编码导致大量重复的历史轨迹特征编码，降低了实时推理性能。

Method: 1. 多源身份匹配与校正模块：利用多视角时空关系实现稳定一致的目标关联；2. 交通信号引导的交互模块：编码交通灯变化趋势作为特征，准确过滤关键交互车辆；3. 局部时空坐标编码：实现历史轨迹和地图的可重用特征，支持并行解码。

Result: 在V2X-Seq和V2X-Traj数据集上的大量实验结果表明，V2X-RECT相比最先进方法取得了显著改进，同时在不同交通密度下增强了鲁棒性和推理效率。
...
TL;DR: SciEducator是一个基于戴明循环的自进化多智能体系统，专门用于科学视频理解和教育，在科学视频基准测试中显著优于现有MLLM和视频智能体。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型和视频智能体在需要外部专业知识和严格逐步推理的科学视频理解与教育领域表现不佳，需要开发专门系统来填补这一空白。
...
Conclusion: SciEducator为科学视频理解与教育建立了新的范式，证明了自进化多智能体系统在该领域的有效性。
...
TL;DR: T3S是一种无需训练、即插即用的推理包装器，通过生成多个短而多样的视频令牌子序列，在单次前向传播中处理并聚合预测，有效解决MLLMs处理长视频时的计算效率问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型处理长视频时，自注意力机制的计算复杂度随视频令牌数量呈二次方增长，导致高计算需求和慢推理速度。现有解决方案在准确性、额外训练需求或推理速度方面存在权衡。

Method: 提出测试时时间采样（T3S），利用时空冗余性，在推理时生成多个短而多样的视频令牌子序列，将它们打包在单次前向传播中，并聚合它们的预测结果。

Result: 在长视频理解基准测试中，T3S将准确率提高了3.1%，并将首个令牌延迟减少了2.04倍，同时只需最小的集成工作量。

Conclusion: T3S将视频冗余转化为计算优势，为长视频理解提供了可扩展的解决方案，完全在推理时运行，无需模型修改或微调，且与多种预训练MLLMs兼容。
...
TL;DR: 提出了一种多模态多说话人注意力对齐方法，通过动态跨模态头选择和自适应社交感知注意力偏置，增强MLLMs在多说话人场景中的社交推理能力，无需额外参数或架构修改。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在多说话人社交互动任务中表现不佳，主要问题是视觉和文本标记缺乏说话人一致性对齐，跨模态注意力较弱。

Method: 动态跨模态头选择识别负责接地的注意力头，结合说话人位置计算自适应社交感知注意力偏置，注入到注意力机制中强化说话人视觉表示与话语的对齐。

Result: 在三个MLLMs和三个基准测试上的实验表明，该方法在四个社交任务中提升了模型性能并达到最先进水平，注意力可视化证实方法能有效聚焦于说话人相关区域。

Conclusion: 提出的方法能有效解决MLLMs在多说话人社交场景中的对齐问题，增强多参与者社交推理能力，且无需额外训练参数或架构修改。
...
TL;DR: HEAL是一个新颖的SFUDA框架，通过分层去噪、边缘引导选择、尺寸感知融合和无学习特性，在无源数据和目标域标签的情况下实现跨域适应，在跨模态实验中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决临床数据隐私和存储限制带来的挑战，在无法访问源域数据且目标域无标签监督的情况下处理域偏移问题。

Method: 提出HEAL框架，整合分层去噪、边缘引导选择、尺寸感知融合和无学习特性，实现源自由无监督域适应。

Result: 大规模跨模态实验表明，该方法优于现有SFUDA方法，达到最先进的性能。

Conclusion: HEAL框架有效解决了SFUDA的核心挑战，在保护数据隐私的同时实现了优异的跨域适应性能。
...
TL;DR: 提出了VITAL系列多模态模型，通过机器标注范式构建了450万视觉语言对数据集，采用多任务训练提升图像和视频质量评估的评分精度和解释能力，实现了高效的模型扩展。


<details>
  <summary>Details</summary>
Motivation: 现有视觉质量评估多模态模型通常专注于单一任务且依赖全参数微调，容易在特定模态或任务类型上过拟合，限制了泛化能力和可迁移性。

Method: 采用以视觉编码器为中心的生成式预训练流程，构建大规模视觉语言对数据集，使用多任务训练工作流同时增强定量评分精度和质量解释能力，基于视觉编码器实现高效模型扩展。

Result: 构建了迄今最大的视觉质量评估训练数据集（450万VL对），模型在零样本设置下表现强劲，每个配对解码器仅需使用不到1/1000的预训练数据进行快速预热即可达到与完全训练模型相当的性能。

Conclusion: 该工作为推进视觉质量评估基础多模态模型的发展奠定了基石。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型在检索任务中表现出色，但在VVI-ReID中的潜力尚未充分探索，主要挑战是缩小模态差距和利用视频序列的时空信息

Method: 提出跨模态原型协作(CPC)来对齐和整合不同模态特征，设计多粒度信息交互(MII)结合短期相邻帧交互、长期跨帧信息融合和跨模态特征对齐
...
TL;DR: 提出了一个名为Signal的多模态目标重识别框架，通过选择性交互和全局-局部对齐来解决多模态特征融合中的背景干扰和一致性对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注多模态特征融合，但忽视了背景干扰问题，且当前融合方法在模态对齐时存在多模态一致性对齐的困难。

Method: 提出选择性交互模块(SIM)选择重要补丁标记，全局对齐模块(GAM)在Gramian空间中最小化3D多面体体积来对齐多模态特征，局部对齐模块(LAM)以移位感知方式对齐局部特征。

Result: 在三个多模态目标重识别基准数据集(RGBNT201、RGBNT100、MSVR310)上的广泛实验验证了方法的有效性。
...
TL;DR: 提出CADTrack框架，通过Mamba特征交互、上下文聚合和可变形对齐来解决RGB-T热成像跟踪中的模态差异问题，实现鲁棒的全天候目标跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-T跟踪器难以解决模态差异，限制了跨模态信息传播和融合，显著降低了跟踪精度。

Method: 采用Mamba特征交互模块建立高效特征交互，上下文聚合模块通过稀疏门控动态激活骨干层，可变形对齐模块整合可变形采样和时间传播来缓解空间错位。

Result: 在五个RGB-T跟踪基准测试上的广泛实验验证了方法的有效性。
...
TL;DR: 提出对抗性伪回放(APR)方法，通过对抗攻击扰动新任务图像来合成伪回放样本，无需存储历史数据，解决了无样本类增量学习中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 解决无样本类增量学习中的可塑性-稳定性困境，即在无法访问历史图像的情况下，既要学习新任务又要避免遗忘旧知识。

Method: 使用对抗攻击在新任务图像上生成伪回放样本，以增强的旧类原型为目标进行知识蒸馏；通过校准协方差矩阵来补偿语义漂移。

Result: 在标准EFCIL基准测试的冷启动设置中达到了最先进的性能。

Conclusion: APR方法有效平衡了稳定性和可塑性，为无样本类增量学习提供了有效的解决方案。
...
TL;DR: FeRA是一个基于频率能量机制的扩散模型微调框架，通过分析去噪过程中的频率能量分布，实现了参数更新与扩散内在频率能量进展的对齐。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成建模方面取得了显著成功，但如何有效将大型预训练模型适应到新任务仍然具有挑战性。本文旨在揭示扩散模型去噪过程中的频率能量机制，并基于此开发有效的微调方法。

Method: 提出了FeRA框架，包含三个协同组件：紧凑的频率能量指示器、软频率路由器（自适应融合多个频率特定适配器专家）和频率能量一致性正则化。路由在训练和推理时都运行，推理时路由由潜在频率能量动态确定。

Result: FeRA能够与基于适配器的调优方案无缝集成，并在不同的扩散主干网络和分辨率上具有良好的泛化能力。

Conclusion: 通过将适应与频率能量机制对齐，FeRA为有效和鲁棒的扩散模型适应提供了一个简单、稳定和兼容的范式。
...
TL;DR: Plan-X是一个通过显式语义规划来指导视频生成的框架，它使用可学习的多模态语言模型生成时空语义token作为视频扩散模型的结构化语义草图，有效减少视觉幻觉并实现与指令对齐的细粒度视频生成。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视觉合成方面表现出色，但在高级语义推理和长时程规划方面存在困难，导致视觉幻觉和与用户指令不对齐的问题，特别是在复杂场景理解、人-物交互、多阶段动作和上下文运动推理等场景中。

Method: 提出Plan-X框架，核心是语义规划器——一个可学习的多模态语言模型，从文本提示和视觉上下文中推理用户意图，自回归生成基于文本的时空语义token序列，这些语义token作为视频扩散模型的结构化语义草图。

Result: 大量实验表明，该框架显著减少了视觉幻觉，能够生成与多模态上下文一致的、细粒度的、与指令对齐的视频。
...
TL;DR: 提出HyM-UNet混合架构，结合CNN的局部特征提取能力和Mamba的全局建模能力，在医学图像分割任务中显著提升性能
...
Method: 设计分层编码器：浅层使用卷积模块保留高频纹理细节，深层引入视觉Mamba模块以线性复杂度捕获长程语义依赖；提出Mamba引导融合跳跃连接，利用深层语义特征作为门控信号动态抑制浅层特征中的背景噪声

Result: 在ISIC 2018数据集上的实验表明，HyM-UNet在Dice系数和IoU指标上显著优于现有最优方法，同时保持更低的参数量和推理延迟

Conclusion: 该方法在处理具有复杂形状和尺度变化的医学分割任务中展现出有效性和鲁棒性
...
TL;DR: 提出SD-PSFNet方法，通过多阶段图像恢复结合点扩散函数机制，动态模拟雨条纹光学特性，实现有效的雨-背景分离和细节恢复。
...
Method: 采用三阶段级联架构，利用学习到的PSF机制动态模拟雨条纹光学，结合自适应门控融合实现跨阶段特征集成，从粗略去雨到精细细节恢复进行顺序优化。
...
TL;DR: RAISECity是一个现实对齐的智能合成引擎，用于生成详细的城市级3D世界，通过多模态基础工具和代理框架实现高质量、可扩展的3D场景构建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在城市级3D生成中存在质量、保真度和可扩展性的挑战，需要开发能够创建详细、城市级3D世界的新方法。

Method: 采用代理框架，利用多模态基础工具获取现实世界知识，维护鲁棒的中间表示，构建复杂3D场景，包括动态数据处理、迭代自反思和细化以及调用高级多模态工具。
...
TL;DR: 提出了一种针对纵向医学影像的深度主动学习框架LMI-AL，通过配对和差分基线及随访3D图像的2D切片，仅需标注不到8%的数据即可达到全标注数据集的性能。
...
Conclusion: LMI-AL为纵向医学影像变化检测提供了一种高效的主动学习解决方案，显著降低了标注成本。
...
TL;DR: RoadBench是一个专门评估多模态大语言模型在城市场景中细粒度空间理解和推理能力的基准测试，重点关注道路标线，包含6个任务和9,121个测试案例。


<details>
  <summary>Details</summary>
Motivation: 填补MLLMs在复杂城市场景中细粒度空间理解和推理能力评估的空白，道路标线作为城市交通网络的重要组成部分，是评估的理想对象。

Method: 提出RoadBench基准测试，使用BEV和FPV图像输入，构建包含6个任务的系统性评估框架，涵盖从局部空间范围到全局推理的能力测试。

Result: 评估14个主流MLLMs后发现，RoadBench对现有模型具有挑战性，揭示了它们在城市场景中细粒度空间理解和推理能力的显著不足，某些任务表现甚至不如简单的基于规则或随机选择的基线。
...
TL;DR: 提出SESP和SAPP两种原型增强策略，解决WS-OVOD中的语义原型静态限制和视觉-文本语义不匹配问题，显著提升开放词汇目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有语义原型（即使通过LLM增强）仍是静态的，无法捕捉由不同物体状态引起的丰富类内视觉变化；标准伪框生成导致视觉区域提议（包含上下文）与以物体为中心的文本嵌入之间存在语义不匹配。

Method: 提出状态增强语义原型（SESP）生成状态感知的文本描述来捕捉多样物体外观；引入场景增强伪原型（SAPP）结合上下文语义并使用软对齐机制促进上下文一致的视觉-文本表示。

Result: 通过整合SESP和SAPP，有效增强了语义原型的丰富性和视觉-文本对齐，实现了显著改进。

Conclusion: 该方法成功解决了WS-OVOD中的两个关键挑战，为开放词汇目标检测提供了更有效的解决方案。
...
TL;DR: LTC和CfC网络在模拟虎蝾螈视网膜神经节细胞活动方面表现优于卷积基线和LSTM，具有更低的MAE、更快的收敛速度、更小的模型尺寸和更优的查询时间，但皮尔逊相关性略低。


<details>
  <summary>Details</summary>
Motivation: 探索LTC和CfC网络在视网膜神经节细胞活动建模中的适用性，特别是在数据有限和需要频繁重新训练的场景下，如视觉假体边缘部署。
...
Result: LTC和CfC架构相比基线模型实现了更低的MAE、更快的收敛速度、更小的模型尺寸和更优的查询时间，但皮尔逊相关性略低。

Conclusion: LTC和CfC网络的高效性和适应性使其特别适合数据有限且需要频繁重新训练的场景，如视觉假体中的边缘部署应用。
...
TL;DR: 提出了MambaX模型，通过非线性状态预测控制来解决图像超分辨率中的误差传播问题，在单图像和多模态超分辨率任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有超分辨率方法主要关注最终分辨率提升，但忽略了中间阶段的误差传播和累积控制。Mamba模型虽然能表示重建过程，但其固定线性映射器感受野窄且灵活性受限。
...
Conclusion: MambaX模型在任意维度和模态的光谱泛化建模方面具有巨大潜力。
...
TL;DR: 提出了首个统一的基于统计的成像噪声模型，联合描述APS和EVS像素的噪声行为，并开发了校准流程和统计模拟器HESIM。
...
Method: 构建了联合噪声模型，包含光子散粒噪声、暗电流噪声、固定模式噪声和量化噪声，开发了校准流程来估计噪声参数，并提出了统计模拟器HESIM。

Result: 在两个混合传感器上的实验验证了模型在多个成像任务（如视频帧插值和去模糊）中的有效性，证明了从模拟到真实数据的强迁移能力。
...
TL;DR: UltraFlux是一个基于Flux的扩散变换器，原生支持4K分辨率图像生成，通过数据模型协同设计解决了4K生成中的位置编码、VAE压缩和优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散变换器在1K分辨率下表现良好，但在扩展到原生4K分辨率时暴露出位置编码、VAE压缩和优化之间的耦合问题，单独解决任一因素都无法达到理想质量。

Method: 采用数据模型协同设计：构建MultiAspect-4K-1M数据集，结合Resonance 2D RoPE位置编码、VAE后训练方案、SNR-Aware Huber Wavelet目标函数和分阶段美学课程学习策略。

Result: 在4096分辨率基准测试和多宽高比4K设置中，UltraFlux在保真度、美学质量和文本对齐方面均优于开源基线模型，配合LLM提示优化器可匹敌或超越专有模型Seedream 4.0。

Conclusion: 通过系统性解决4K图像生成中的耦合问题，UltraFlux实现了稳定、细节保留的4K扩散变换器，能够泛化到各种宽高比设置。
...
TL;DR: 提出了IE-Bench基准套件和IE-Critic-R1评估方法，用于改进文本驱动图像编辑的评估，通过强化学习从可验证奖励中训练，更好地与人类感知对齐。


<details>
  <summary>Details</summary>
Motivation: 文本驱动图像编辑的评估具有挑战性，现有方法主要关注文本-图像对齐或未很好与人类感知对齐，需要同时考虑文本和源图像的条件。

Method: 构建IE-Bench基准套件包含多样化源图像、编辑提示和编辑结果，引入IE-Critic-R1方法，利用强化学习从可验证奖励训练，提供更全面可解释的质量评估。

Result: IE-Bench包含近4,000个样本和15名人类受试者提供的平均意见分数，实验表明IE-Critic-R1在文本驱动图像编辑任务上比先前指标具有更好的主观对齐性。

Conclusion: IE-Bench和IE-Critic-R1为文本驱动图像编辑提供了更好的评估框架，数据代码已公开。
...
<details>
  <summary>Details</summary>
Motivation: 遥感领域获取高质量标注数据成本高昂且耗时，大量未标注图像未被充分利用，需要开发高效的标签利用方法。

Method: HSSAL框架在迭代循环中集成SSL和分层主动学习：SSL通过弱到强自训练利用标注和未标注数据改进特征表示和不确定性估计；HAL通过渐进聚类策略选择满足可扩展性、多样性和不确定性的最具信息量样本。

Result: 在UCM、AID和NWPU-RESISC45三个基准数据集上，HSSAL仅需8%、4%和2%的标注数据即可达到超过95%的全监督准确率，显著优于仅使用SSL或AL的基线方法。
...
TL;DR: 开发基于深度学习的虚拟病理助手，使用EfficientNet-B3模型在CAISHI数据集上区分宫颈腺原位癌与正常腺体组织，准确率达73.23%。


<details>
  <summary>Details</summary>
Motivation: 宫颈腺原位癌(AIS)是重要的癌前病变，准确诊断具有挑战性。早期检测对于预防进展为浸润性腺癌至关重要。

Method: 使用2240张专家标注的H&E图像，经过Macenko染色归一化和基于补丁的预处理。训练EfficientNet-B3 CNN模型，采用类别平衡采样和焦点损失函数处理数据不平衡问题。

Result: 最终模型总体准确率为0.7323，异常类F1分数0.75，正常类F1分数0.71。Grad-CAM热图显示生物学可解释的激活模式。
...
### [84] [Bias Is a Subspace, Not a Coordinate: A Geometric Rethinking of Post-hoc Debiasing in Vision-Language Models](https://arxiv.org/abs/2511.18123)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)的表征经常编码和放大人口统计偏见，导致下游任务中出现偏见关联和错位预测，这破坏了公平性并扭曲了视觉与语言的对齐。
...
TL;DR: 提出了VK-Det框架，这是一个无需额外监督的视觉知识引导的开放词汇目标检测方法，通过利用视觉编码器的固有区域感知能力和原型感知伪标签策略，在航空图像上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇航空目标检测方法依赖文本监督，导致语义偏见，限制了向文本未指定概念的扩展。需要一种不依赖额外监督的方法来更好地识别新颖类别。
...
Conclusion: VK-Det框架通过视觉知识引导和原型感知伪标签，在不依赖额外监督的情况下实现了卓越的开放词汇目标检测性能，为航空图像分析提供了有效的解决方案。
...
### [86] [Assessing the alignment between infants' visual and linguistic experience using multimodal language models](https://arxiv.org/abs/2511.18824)
...
TL;DR: 使用CLIP模型自动分析婴儿视角视频中视觉-语言对齐情况，发现理想的学习对齐时刻在儿童日常经验中相对罕见


<details>
  <summary>Details</summary>
Motivation: 研究儿童语言学习过程中视觉和语言体验的时间对齐程度，解决传统手动标注方法效率低的问题

Method: 使用对比性语言-图像预训练(CLIP)模型自动分析婴儿视角的家庭环境视频中的视觉-语言对齐，并通过人工对齐判断验证CLIP对齐分数

Result: 理想的学习对齐时刻(如"看球"时球出现在儿童视野中)在儿童日常经验中相对罕见，比现代机器学习数据集中的对齐频率低，且不同儿童之间和同一儿童内部都存在对齐变异性
...
### [87] [ActDistill: General Action-Guided Self-Derived Distillation for Efficient Vision-Language-Action Models](https://arxiv.org/abs/2511.18082)
*Wencheng Ye,Tianshi Wang,Lei Zhu,Fengling Li,Guoli Yang*

Main category: cs.CV

TL;DR: ActDistill是一个动作引导的自蒸馏框架，将现有VLA模型的动作预测能力转移到轻量级模型，减少计算开销和推理延迟。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作中受限于沉重的计算开销和推理延迟，需要高效的部署方案。

Method: 使用图结构封装策略建模动作预测的层次演化，引入动态路由器自适应选择计算路径，通过层次图监督确保高效演化。

Result: 在具身基准测试中，ActDistill达到与全尺寸VLA模型相当或更优的性能，计算量减少50%以上，速度提升达1.67倍。
...
### [88] [From Pixels to Posts: Retrieval-Augmented Fashion Captioning and Hashtag Generation](https://arxiv.org/abs/2511.19149)
*Moazzam Umer Gondal,Hamad Ul Qudous,Daniya Siddiqui,Asma Ahmad Farhan*

Main category: cs.CV

TL;DR: 提出了一个检索增强的时尚图片描述和标签生成框架，结合多服装检测、属性推理和LLM提示，生成视觉基础扎实、描述性强且风格有趣的文本。


<details>
  <summary>Details</summary>
Motivation: 解决端到端描述生成器在属性保真度和领域泛化方面的问题，为时尚图像生成视觉基础扎实的描述性文本。

Method: 使用YOLO检测器进行多服装定位，k-means聚类提取主色调，CLIP-FAISS检索模块基于结构化产品索引推断面料和性别属性，结合检索到的风格示例构建事实证据包来指导LLM生成描述和标签。

Result: YOLO检测器在9个服装类别上获得0.71的mAP@0.5，RAG-LLM管道生成表达性属性对齐的描述，在标签生成中达到0.80的平均属性覆盖率和50%阈值的完全覆盖，相比BLIP具有更好的事实基础和更少的幻觉。

Conclusion: 检索增强生成是自动化和视觉基础时尚内容生成的有效且可解释的范式，具有在各种服装领域可扩展部署的巨大潜力。
...
TL;DR: 该研究提出EMFE管道，使用简单的形态学特征和轻量级机器学习模型，在疟疾细胞图像分类任务上达到与深度学习相当的性能，但模型更小、速度更快、更易解释。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分类中计算需求高且缺乏可解释性，研究旨在验证对于简单的疟疾二分类任务，是否必须使用复杂神经网络。

Method: 从疟疾细胞图像中提取两个形态学特征（非背景像素数和细胞内部空洞数），使用逻辑回归和随机森林等轻量模型，并与ResNet18等深度学习模型对比。

Result: 单变量逻辑回归模型达到94.80%准确率，模型大小仅1.2kB，推理延迟2.3ms；两阶段集成模型准确率提升至97.15%。而深度学习模型需要13.6-44.7MB存储，推理时间68ms。
...
TL;DR: 提出了TTA框架，通过联合优化对齐和差异性来改进多模态生存分析，避免表示崩溃并保持模态特异性。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度强调跨模态对齐而牺牲模态特异性，导致表示崩溃和多样性降低。需要同时考虑语义对齐和模态特异性结构的保持。
...
TL;DR: VRPSR是一种感知图像超分辨率方法，通过构建可泛化的编解码器模拟器，使现有感知SR方法能够适应多种压缩场景，节省超过10%的比特率。


<details>
  <summary>Details</summary>
Motivation: 传统感知图像超分辨率方法忽略图像恢复后的重新压缩过程，导致下游编解码器可能对恢复图像添加额外伪影，需要联合优化SR和重新压缩。

Method: 将压缩建模为条件文本到图像生成，利用预训练扩散模型构建通用编解码器模拟器；提出针对感知SR的训练技术，包括使用感知目标优化模拟器，以及采用轻微压缩图像作为训练目标。

Result: 基于Real-ESRGAN和S3Diff，在H.264/H.265/H.266压缩下节省超过10%的比特率；促进SR与重新压缩后后处理模型的联合优化。

Conclusion: VRPSR成功使感知超分辨率方法能够适应多种压缩场景，有效解决了重新压缩带来的问题，并实现了显著的比特率节省。
...
TL;DR: 提出了Spotlight任务，用于定位和解释视频生成中的错误，发现当前视觉语言模型在错误识别和定位方面显著落后于人类。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频模型虽然能生成高质量视频，但仍存在细微和局部的错误，而现有评估方法无法识别错误发生的时间和性质。

Method: 使用200个多样化文本提示和3个最先进的视频生成器生成600个视频，标注了1600多个细粒度错误，涵盖6种错误类型。

Result: 发现提示遵循性和物理错误占主导地位且持续时间较长，而外观消失和身体姿态错误出现在较短片段中。VLMs在错误识别和定位方面显著落后于人类。

Conclusion: 通过推理时策略将VLMs性能提升近2倍，该任务为构建细粒度评估工具和更复杂的视频生成器奖励模型铺平了道路。
...
TL;DR: 提出MM-Det++算法，通过时空分支和多模态分支结合统一多模态学习模块，专门检测扩散模型生成的视频伪造内容，并在大规模DVF数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的视频日益增多引发信息安全担忧，现有方法主要关注图像级伪造检测，视频级通用伪造检测研究不足，需要开发可靠的合成媒体检测方法。

Method: 采用双分支架构：时空分支使用帧中心视觉变换器(FC-ViT)聚合时空信息；多模态分支利用多模态大语言模型(MLLMs)的可学习推理范式获取多模态伪造表示。通过统一多模态学习(UML)模块整合多模态表示。

Result: 大量实验证明MM-Det++的优越性，突显了统一多模态伪造学习在检测扩散生成视频方面的有效性。

Conclusion: MM-Det++通过创新的多模态方法成功解决了扩散生成视频的检测问题，为视频取证领域提供了有效解决方案。
...
Method: 提出支持深度、宽度和token三个维度自适应计算的架构，并配合高效的联合训练机制，确保模型在各种配置下都能保持性能。

Result: 在图像分类任务中，AdaPerceiver扩展了准确率-吞吐量的帕累托前沿，达到85.4%准确率，吞吐量比FlexiViT-L高36%。在密集预测任务中，匹配ViT-H/14性能，同时语义分割和深度估计的编码器FLOPs减少约26倍。配备策略后，能在保持ImageNet1K准确率（±0.1个百分点）的同时减少24-33%的FLOPs。

Conclusion: AdaPerceiver通过统一的自适应计算能力，在保持性能的同时显著提升了计算效率，为transformer模型在多样化部署场景中的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM的处理范式与人类整合视觉信息的能力不同，模型倾向于孤立处理细节和高级概念，而现有评估协议往往将低级感知与高级推理解耦，忽略了它们的语义和因果依赖关系。

Method: 提出了VCU-Bridge框架，构建了HVCU-Bench基准，并开发了基于蒙特卡洛树搜索的数据生成管道进行指令调优。

Result: 实验显示随着推理层级提升，性能持续下降。加强低级能力能在高级层级带来可衡量的收益，不仅改善了HVCU-Bench表现，还在通用基准上带来平均2.53%的提升，特别是在MMStar上获得7.26%的显著提升。

Conclusion: 层次思维模式对增强MLLM能力具有重要意义，加强低层能力能够有效提升高层推理表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型是逐帧处理的，多视角一致性有限，无法有效处理3D视觉任务中多视角间的几何对应关系。

Method: 通过重建一个视角中被严重遮挡的内容，利用其他视角的几何对应关系进行预训练，采用激进的遮挡策略。

Result: 相比DINO等最先进的逐帧骨干网络，Muskie在多视角对应精度上表现更好，在下游3D任务（相机姿态估计和点云重建）中性能持续提升。

Conclusion: Muskie通过多视角一致性预训练，无需3D监督即可学习强大的几何理解能力，在3D视觉任务中表现出色。
...
### [97] [PromptMoE: Generalizable Zero-Shot Anomaly Detection via Visually-Guided Prompt Mixtures](https://arxiv.org/abs/2511.18116)
*Yuheng Shao,Lizhang Wang,Changhao Li,Peixian Chen,Qinyuan Liu*

Main category: cs.CV

TL;DR: 提出了PromptMoE方法，通过专家提示池和视觉引导的混合专家机制来解决零样本异常检测中的表示瓶颈和过拟合问题。
...
Method: 学习专家提示池作为可组合的语义基元，使用视觉引导的混合专家机制动态组合这些提示，生成语义丰富的文本表示。

Result: 在工业和医疗领域的15个数据集上进行了广泛实验，证明了方法的有效性和最先进的性能。
...
TL;DR: SCALER是一个用于标签稀缺隐蔽目标分割的统一协作框架，通过联合优化均值教师分割器和可学习的SAM模型，在交替的两个阶段中实现相互监督和性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LDCOS方法依赖一致性约束或SAM伪标签，但由于目标的内在隐蔽性和标注稀缺性，性能仍然有限。本研究探讨能否将一致性约束和SAM监督联合集成以利用互补信息，以及分割器能否反过来指导SAM实现相互改进。

Method: 提出SCALER框架，在阶段I中使用基于熵的图像级和基于不确定性的像素级加权来优化分割器；在阶段II中通过增强不变性和噪声抵抗损失来更新SAM。

Result: 实验表明SCALER在八个半监督和弱监督COS任务中均获得一致的性能提升，能够作为通用训练范式在标签稀缺条件下增强轻量分割器和大型基础模型。

Conclusion: SCALER证明了协作框架在标签稀缺隐蔽目标分割中的有效性，能够实现分割器和SAM模型的相互提升，为相关任务提供了新的训练范式。
...
### [99] [MVS-TTA: Test-Time Adaptation for Multi-View Stereo via Meta-Auxiliary Learning](https://arxiv.org/abs/2511.18120)
*Hannuo Zhang,Zhixiang Chi,Yang Wang,Xinxin Zuo*

Main category: cs.CV

TL;DR: MVS-TTA是一个高效的测试时自适应框架，通过元学习将优化式方法的场景自适应能力引入学习式多视角立体方法，使用自监督跨视图一致性损失作为辅助任务来指导推理时适应。


<details>
  <summary>Details</summary>
Motivation: 学习式MVS方法虽然取得了显著进展，但由于在有限训练数据分布上训练的固定模型参数，其泛化能力仍不理想。而优化式方法虽然能实现场景自适应，但缺乏可扩展性且需要昂贵的逐场景优化。

Method: 提出MVS-TTA框架，使用自监督跨视图一致性损失作为辅助任务来指导推理时自适应。引入元辅助学习策略，明确训练模型从基于辅助任务的更新中受益。该框架与模型无关，可应用于广泛的MVS方法。

Result: 在标准数据集（DTU、BlendedMVS）和具有挑战性的跨数据集泛化设置上的大量实验表明，MVS-TTA能持续提升性能，即使应用于最先进的MVS模型也能获得改进。

Conclusion: 这是首次尝试使用元学习将基于优化的测试时自适应集成到学习式MVS中，有效结合了两种范式的优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度展开网络存在退化特定依赖性和过平滑偏差问题，限制了在盲图像恢复任务中的应用。需要开发能够处理未知退化模型并保留纹理细节的方法。

Method: 提出UnfoldLDM框架，包含多粒度退化感知模块作为梯度下降步骤，建模退化估计问题；设计退化抵抗潜在扩散模型提取退化不变先验；使用过平滑校正变换器恢复高频分量。

Result: 实验表明UnfoldLDM在各种盲图像恢复任务中达到领先水平，并能提升下游任务性能。该设计兼容现有深度展开网络方法，可作为即插即用框架。

Conclusion: 通过将深度展开网络与潜在扩散模型集成，UnfoldLDM有效解决了盲图像恢复中的关键挑战，实现了退化无关且视觉丰富的恢复效果。
...
TL;DR: 提出了嵌套展开网络(NUN)，通过DUN-in-DUN设计将图像恢复与分割解耦，在真实世界隐蔽目标分割任务中取得领先性能。
...
Method: 采用嵌套展开网络设计，在分割导向展开网络(SODUN)的每个阶段嵌入抗退化展开网络(DeRUN)，通过视觉语言模型动态推断退化语义并恢复图像，同时执行可逆估计来细化前景和背景。

Result: 在干净和退化基准测试中都取得了领先地位，展示了优越的性能。

Conclusion: NUN框架通过解耦恢复与分割、动态退化推断和自一致性损失，为真实世界隐蔽目标分割提供了统一的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档VQA系统要么文本准确但空间定位不可靠，要么为可解释性牺牲性能，需要同时实现精确答案提取和可靠空间定位以满足高风险应用的可解释性需求。

Method: 将文档VQA分解为结构化子任务：使用TrOCR进行OCR文本提取、基于语义搜索的检索增强上下文选择、通过微调Gemma 3-27B模型生成答案、通过文本到区域对齐进行显式边界框定位。

Result: 在四个基准测试中达到最先进结果：DocVQA上88.7 ANLS和50.1 mAP，FUNSD上90.0 ANLS和50.3 mAP，CORD上85.5 ANLS和60.2 mAP，SROIE上93.1 ANLS，相比之前最佳方法在DocVQA上提升+2.8 ANLS和+3.9 mAP。

Conclusion: 专业工具的智能协调可以同时提高性能和可解释性，为可信赖、可解释的文档AI系统提供路径，模块化架构支持透明推理追踪和独立组件优化。
...
### [103] [SFHand: A Streaming Framework for Language-guided 3D Hand Forecasting and Embodied Manipulation](https://arxiv.org/abs/2511.18127)
*Ruicong Liu,Yifei Huang,Liangyang Ouyang,Caixin Kang,Yoichi Sato*

Main category: cs.CV

TL;DR: SFHand是首个用于语言引导3D手部预测的流式框架，能够从连续的视频流和语言指令中自回归预测未来3D手部状态，在AR和辅助机器人等应用中实现流畅的人机交互。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要离线访问累积的视频序列，无法整合传达任务意图的语言指导，不适用于实时人机交互场景。

Method: SFHand结合了流式自回归架构和ROI增强记忆层，捕捉时间上下文同时关注显著的手部区域，预测手部类型、2D边界框、3D姿态和轨迹等完整状态。

Result: SFHand在3D手部预测方面达到新的最先进水平，比先前工作性能提升高达35.8%，其学习表示可转移到下游具身操作任务，任务成功率提升达13.4%。

Conclusion: SFHand为实时语言引导的3D手部预测提供了有效解决方案，并发布了首个大规模同步3D手部姿态和语言指令数据集EgoHaFL。
...
### [104] [Beyond Words and Pixels: A Benchmark for Implicit World Knowledge Reasoning in Generative Models](https://arxiv.org/abs/2511.18271)
*Tianyang Han,Junhao Su,Junjie Hu,Peizhen Yang,Hengyu Shi,Junfeng Luo,Jialin Gao*

Main category: cs.CV

TL;DR: PicWorld是首个全面评估文本到图像模型对隐含世界知识和物理因果推理能力的基准，包含1,100个提示，通过多智能体评估器PW-Agent进行分层评估。
...
Result: 对17个主流T2I模型的分析表明，它们在隐含世界知识和物理因果推理能力方面普遍存在根本性限制。
...
### [105] [Video4Edit: Viewing Image Editing as a Degenerate Temporal Process](https://arxiv.org/abs/2511.18131)
*Xiaofan Li,Yanpeng Sun,Chenming Wu,Fan Duan,YuAn Wang,Weihao Bo,Yumeng Zhang,Dingkang Liang*

Main category: cs.CV

TL;DR: 该论文提出了一种基于时间建模视角的图像编辑方法，通过从视频预训练中迁移单帧演化先验，实现了仅需1%监督数据的有效图像编辑。


<details>
  <summary>Details</summary>
Motivation: 当前多模态基础模型虽然推动了指令驱动的图像生成和编辑，但现有编辑流程成本高昂，需要大量高质量的{指令、源图像、编辑图像}三元组数据，且编辑保真度依赖于指令对目标语义的精确引用。

Method: 将图像编辑视为退化时间过程，从视频预训练中迁移单帧演化先验，采用数据高效的精调机制。

Result: 该方法在性能上与领先的开源基线相当，但仅需主流编辑模型约1%的监督数据。

Conclusion: 时间建模视角为图像编辑提供了一种数据高效的解决方案，通过利用视频预训练的先验知识显著减少了数据需求。
...
### [106] [Uni-DAD: Unified Distillation and Adaptation of Diffusion Models for Few-step Few-shot Image Generation](https://arxiv.org/abs/2511.18281)
*Yara Bahram,Melodie Desbos,Mohammadhadi Shateri,Eric Granger*

Main category: cs.CV

TL;DR: Uni-DAD是一种单阶段训练管道，统一了扩散模型的蒸馏和适应过程，能够在少于4个采样步骤的情况下实现高质量的新领域图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有的两阶段训练方法（先适应后蒸馏或先蒸馏后适应）存在设计复杂、质量下降和多样性减少的问题，需要一种更高效的单阶段解决方案。

Method: 结合双领域分布匹配蒸馏目标（引导学生模型朝向源教师和目标教师的分布）和多头生成对抗网络损失（在多个特征尺度上鼓励目标真实性）。

Result: 在少样本图像生成和主体驱动个性化任务中，Uni-DAD在质量和多样性方面均优于最先进的适应方法和两阶段训练管道。

Conclusion: Uni-DAD成功实现了单阶段的蒸馏和适应，在保持高质量的同时显著减少了采样步骤，为快速新领域生成提供了有效解决方案。
...
TL;DR: SwiftVGGT是一种无需训练的方法，在大规模3D重建中显著减少推理时间同时保持高质量重建，通过消除外部VPR模型依赖和简化点采样对齐，实现33%推理时间节省。


<details>
  <summary>Details</summary>
Motivation: 大规模3D重建中精度与计算效率之间存在固有权衡，现有方法要么速度快但质量低，要么质量高但推理慢，需要平衡两者。

Method: 提出SwiftVGGT：1）无需外部VPR模型实现闭环检测；2）使用基于Sim(3)的SVD单步对齐相邻块，消除IRLS优化；3）保持训练自由特性。

Result: 在多个数据集上评估显示，SwiftVGGT达到最先进的重建质量，同时仅需最近基于VGGT的大规模重建方法33%的推理时间。

Conclusion: SwiftVGGT成功解决了大规模3D重建中精度与效率的权衡问题，在公里级环境中实现高质量重建，显著提升计算效率。
...
### [108] [Compact neural networks for astronomy with optimal transport bias correction](https://arxiv.org/abs/2511.18139)
*Shuhuan Wang,Yuzhen Xie,Jiayi Li*

Main category: cs.CV

TL;DR: WaveletMamba是一个理论驱动的天文成像框架，结合小波分解与状态空间建模，在64x64分辨率下实现81.72%分类准确率，仅需3.54M参数，计算效率提升9.7倍，并具备分辨率多稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决天文成像中效率与分辨率之间的权衡问题，限制大规模形态分类和红移预测的能力。

Method: 集成小波分解、状态空间建模、数学正则化和多级偏差校正，包括HK距离（分布级最优传输）和颜色感知加权（样本级微调）。

Result: 在64x64分辨率下达到81.72%分类准确率，244x244分辨率下80.93%准确率，计算效率提升9.7倍，Log-MSE改进22.96%，异常值减少26.10%。

Conclusion: 数学严谨性在科学AI中实现前所未有的效率和全面偏差校正，连接计算机视觉和天体物理学，革新跨学科科学发现。
...
### [109] [ScriptViT: Vision Transformer-Based Personalized Handwriting Generation](https://arxiv.org/abs/2511.18307)
*Sajjan Acharya,Rajendra Baskota*

Main category: cs.CV

TL;DR: 提出基于Vision Transformer的统一框架，通过多参考图像学习全局笔迹风格，结合交叉注意力机制生成更忠实于目标风格的笔迹图像，并使用显著笔画注意力分析提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉笔迹的全局风格模式（如倾斜度、曲率、笔画压力等长距离空间依赖特征），导致生成的笔迹在风格一致性和准确性方面存在局限。

Method: 使用Vision Transformer风格编码器从多张参考图像学习全局风格模式，通过交叉注意力机制将风格线索与目标文本融合，并采用显著笔画注意力分析(SSAA)提供可解释性。

Result: 该框架能够生成更具风格一致性的笔迹图像，同时通过SSAA揭示了模型在风格转移过程中关注的笔画级特征，提高了系统的可理解性和分析性。

Conclusion: 提出的统一框架有效解决了笔迹生成中的全局风格捕捉问题，不仅提升了风格连贯性，还通过可解释性分析增强了系统的实用价值。
...
TL;DR: 比较三种预训练CNN在脑肿瘤MRI分类中的表现，发现ConvNeXt-Tiny表现最佳，而医疗领域预训练的RadImageNet DenseNet121泛化能力较差，表明在小数据集条件下，现代通用预训练模型优于领域特定预训练模型。


<details>
  <summary>Details</summary>
Motivation: 探究在小数据集条件下，医疗领域预训练模型与通用预训练模型在脑肿瘤MRI分类任务中的性能差异，为医学影像分析中的迁移学习策略提供指导。

Method: 系统评估三种预训练CNN架构：医疗领域预训练的RadImageNet DenseNet121、EfficientNetV2S和ConvNeXt-Tiny，在相同条件下使用有限脑MRI数据集进行训练和微调。

Result: ConvNeXt-Tiny获得最高准确率，其次是EfficientNetV2S，而RadImageNet DenseNet121虽然经过医疗领域预训练，但表现出较差的泛化能力，准确率较低且损失较高。

Conclusion: 领域特定预训练在小数据条件下可能泛化不佳，而基于大规模数据集预训练的现代通用CNN在专业医学影像任务中能提供更优越的迁移学习性能。
...
<details>
  <summary>Details</summary>
Motivation: 少样本语义分割模型在分割新类别方面表现出色，但其决策过程不透明。尽管可解释AI在标准计算机视觉任务中已有显著进展，但在数据稀缺场景下对模型行为理解和支持集选择至关重要的少样本分割可解释性研究仍几乎空白。

Method: Affinity Explainer方法通过计算支持图像和查询图像特征在多个特征层级上的匹配分数，提取归因图来突出显示支持图像中对查询分割预测贡献最大的像素。

Result: 在少样本分割基准数据集上的综合实验表明，Affinity Explainer显著优于适应的标准归因方法。定性分析显示，该方法提供的解释具有结构化、连贯的注意力模式，与模型架构一致，并能实现有效的模型诊断。

Conclusion: 这项工作为可解释的少样本分割研究奠定了基础，使能更好的模型理解和诊断，从而构建更可靠的少样本分割系统。
...
TL;DR: 提出了DualXrayBench基准测试和GSR模型，通过将双视角X光图像作为类似语言模态来处理，在X射线安检领域实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统X射线违禁品检测方法主要依赖视觉模态，在复杂威胁检测上表现不佳。虽然近期研究引入了语言指导，但实际安检中检查员通常使用双视角图像，因此探索第二视角是否能提供类似语言模态的约束条件。

Method: 提出了几何-语义推理器(GSR)，该多模态模型联合学习跨视角几何和跨模态语义之间的对应关系，将第二视角图像视为"类似语言的模态"。构建了GSXray数据集，包含结构化思维链序列：<顶部视角>、<侧面视角>、<结论>。

Result: 在DualXrayBench上的全面评估表明，GSR在所有X射线任务上都实现了显著改进。

Conclusion: 该方法为实际X射线安检提供了新的视角，证明了双视角图像可以作为有效的约束条件来提升检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 实现通过身体姿态进行细粒度控制的自我中心视频生成，对于能够模拟、预测和规划动作的具身AI代理至关重要。

Method: 训练视频预测模型，以3D身体姿态序列为条件生成未来帧；引入新颖的姿态表示方法捕捉全局相机动态和关节身体运动；在扩散过程中集成专用控制机制。

Result: EgoControl能够生成时间连贯、视觉逼真且与提供姿态控制对齐的未来帧，实验结果显示其能产生高质量、姿态一致的自我中心视频。
...
TL;DR: USF是一个统一的球形前端框架，可将任何校准相机的图像转换为单位球面表示，直接在空间域执行球形重采样、卷积和池化操作，避免了昂贵的球谐变换，实现了旋转等变性。


<details>
  <summary>Details</summary>
Motivation: 现代感知系统越来越多地使用鱼眼、全景等广角相机，但现有方法仍主要使用为针孔图像设计的平面CNN，存在图像空间邻域无法准确表示物理邻接关系、对全局旋转敏感等问题。

Method: 通过光线方向对应关系将图像转换为单位球面表示，使用仅基于距离的球形核进行空间域卷积，模块化设计分离了投影、位置采样、插值和分辨率控制。

Result: 在分类、检测和分割任务上表现优异，处理高分辨率球形图像效率高，在随机测试时旋转下性能下降小于1%，无需旋转增强，并能实现从一种镜头类型到未见过的广角镜头的零样本泛化。

Conclusion: USF提供了一种高效、模块化的球形视觉处理方法，显著提升了广角相机感知系统的鲁棒性和泛化能力。
...
TL;DR: 提出CorrFlowNet生成模型，通过早期基线CT扫描生成虚拟一年随访CT，用于早期肺癌诊断，减少对临床随访的依赖。


<details>
  <summary>Details</summary>
Motivation: 肺癌早期诊断困难，现有AI方法多关注单次早期CT扫描的影像特征，无法模拟结节随时间的变化。需要开发能预测结节进展的方法来辅助早期诊断。

Method: 使用相关性自编码器将早期基线和随访CT图像编码到潜在空间，捕捉结节进展动态和相关关系，结合流匹配算法和神经常微分方程进行生成，并利用辅助分类器提升诊断准确性。

Result: 在真实临床数据集上评估，相比基线模型显著改善肺结节风险评估，诊断准确性与真实临床CT随访相当。

Conclusion: CorrFlowNet具有改善癌症诊断的潜力，能有效模拟结节进展，为早期肺癌诊断提供有力工具。
...
TL;DR: DocPTBench是一个专门针对拍摄文档解析和翻译的基准测试，包含1300多份高分辨率拍摄文档，揭示了现有模型在真实世界拍摄条件下性能显著下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文档解析和翻译基准主要针对扫描或数字文档，无法充分反映真实世界拍摄条件（如几何变形和光度变化）带来的复杂挑战。

Method: 构建包含1300多份高分辨率拍摄文档的综合基准DocPTBench，涵盖多个领域和八种翻译场景，并提供人工验证的解析和翻译标注。

Result: 从数字文档转向拍摄文档时，主流MLLM在端到端解析中平均准确率下降18%，翻译下降12%；专业文档解析模型平均下降25%。

Conclusion: 真实世界拍摄条件对现有模型构成了独特挑战，揭示了现有模型的鲁棒性有限，需要专门针对拍摄文档的解决方案。
...
### [117] [InfiniBench: Infinite Benchmarking for Visual Spatial Reasoning with Customizable Scene Complexity](https://arxiv.org/abs/2511.18200)
*Haoming Wang,Qiyao Xue,Wei Gao*

Main category: cs.CV

TL;DR: InfiniBench是一个完全自动化的可定制基准生成器，能够合成无限多样的3D场景，通过参数化控制场景复杂度，用于评估视觉语言模型的空间推理能力。
...
Method: 使用基于LLM的代理框架迭代优化程序化场景约束，采用灵活的基于簇的布局优化器生成密集杂乱场景，以及任务感知的相机轨迹优化方法渲染完整覆盖的视频。

Result: InfiniBench在提示保真度和物理合理性方面优于最先进的程序化和基于LLM的3D生成方法，特别是在高复杂度场景中表现优异。

Conclusion: InfiniBench能够为测量、视角转换和时空跟踪等代表性空间推理任务生成有效的基准测试，证明了其在评估VLM空间推理能力方面的实用性。
...
### [118] [RegDeepLab: A Two-Stage Decoupled Framework for Interpretable Embryo Fragmentation Grading](https://arxiv.org/abs/2511.18454)
*Ming-Jhe Lee*

Main category: cs.CV

TL;DR: 提出RegDeepLab双分支多任务学习框架，集成语义分割和回归任务，解决胚胎碎片化程度评估中视觉可解释性与精确分级的矛盾。


<details>
  <summary>Details</summary>
Motivation: 当前IVF胚胎碎片化程度手动评估存在耗时、观察者间差异大和效率瓶颈问题，现有深度学习方法在视觉可解释性和精确临床分级之间存在矛盾。

Method: 采用双分支多任务学习框架，结合DeepLabV3+语义分割和多尺度回归头，提出两阶段解耦训练策略解决梯度冲突和负迁移问题，并引入范围损失进行半监督学习。

Result: 标准端到端训练可实现最小分级误差(MAE=0.046)，但会损害分割边界完整性；解耦策略在保持SOTA级分割精度(Dice=0.729)的同时提供稳健的高精度分级预测。

Conclusion: 成功开发了兼具高精度和视觉可解释性的双模块临床辅助解决方案，为IVF胚胎评估提供了有效的自动化工具。
...
<details>
  <summary>Details</summary>
Motivation: 体外受精(IVF)中第5天囊胚的形态学评估存在主观性和不一致性问题。AI可以标准化评估，但需要大量、多样且平衡的数据集，而实际中面临数据稀缺、自然类别不平衡和隐私限制等挑战。

Method: 开发了DIA框架，使用潜在扩散模型生成高质量囊胚图像，能够基于Gardner形态学类别和z轴焦距进行精细控制。通过FID、记忆化指标、胚胎学家图灵测试和三个下游分类任务进行严格评估。

Result: DIA模型生成逼真图像，胚胎学家无法可靠区分真实与合成图像。合成图像增强不平衡数据集显著提高了分类准确率(p < 0.05)；在大规模平衡数据集中添加合成图像也带来统计显著的性能提升；在某些情况下合成数据可替代高达40%的真实数据而不损失准确率。

Conclusion: DIA为缓解胚胎数据集中的数据稀缺和类别不平衡问题提供了稳健解决方案，通过生成新颖、高保真且可控的合成图像，能够改善AI胚胎评估工具的性能、公平性和标准化。
...
### [120] [Large-Scale Pre-training Enables Multimodal AI Differentiation of Radiation Necrosis from Brain Metastasis Progression on Routine MRI](https://arxiv.org/abs/2511.18208)
...
TL;DR: 基于自监督学习的视觉Transformer模型，通过大规模无标签脑转移瘤MRI数据预训练，在区分放射性坏死和肿瘤进展方面显著优于全监督模型和放射组学方法。


<details>
  <summary>Details</summary>
Motivation: 立体定向放射外科治疗后区分放射性坏死和肿瘤进展是临床关键挑战，活检作为金标准具有侵入性，而传统监督学习方法受限于标注数据稀缺。

Method: 采用两阶段深度学习策略：首先在10,167个无标签多源T1CE MRI子体积上通过自监督学习预训练Vision Transformer，然后在公开MOLAB数据集上使用双通道输入进行微调。

Result: 自监督模型在同中心测试集上AUC达0.916，在第二中心测试集上AUC为0.764，显著优于全监督ViT和放射组学方法。多模态集成进一步提升性能。

Conclusion: 大规模无标签脑转移瘤数据集预训练显著提升AI模型性能，两阶段多模态深度学习策略仅使用常规T1CE MRI和标准临床数据即可高精度区分放射性坏死与肿瘤进展。
...
TL;DR: 提出UNIFIER方法解决多模态大语言模型在连续学习中的灾难性遗忘问题，通过视觉信息解耦和一致性约束来处理不同场景间的视觉差异。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在部署到设备上时需要持续适应下游任务中的动态场景变化，如背景和视角的变化，以有效执行复杂视觉任务。

Method: UNIFIER将不同场景的视觉信息解耦到每个视觉块中的不同分支，并将其投影到相同的特征空间，通过一致性约束保持跨场景视觉表示的稳定性。

Result: 在MSVQA数据集上的大量实验表明，UNIFIER有效缓解了跨场景任务的遗忘，并在同一场景内实现了知识积累。

Conclusion: UNIFIER方法能够有效解决多模态大语言模型在连续学习中的灾难性遗忘问题，特别是在处理动态场景变化时表现出色。
...
### [122] [Using MLIR Transform to Design Sliced Convolution Algorithm](https://arxiv.org/abs/2511.18222)
*Victor Ferrari,Marcio Pereira,Lucas Alvarenga,Gustavo Leite,Guido Araujo*

Main category: cs.CV

TL;DR: SConvTransform是一个MLIR转换方言扩展，通过声明式转换管道将2D卷积优化为分块和打包的通用操作，在ARM SME和Intel AVX512上分别达到峰值性能的60%和67%。


<details>
  <summary>Details</summary>
Motivation: 优化2D卷积在MLIR中的性能，通过结合静态形状分析与结构化分块打包策略，为不同目标架构提供可重用的卷积优化方案。

Method: 使用SConvOp操作将Linalg卷积降级为分块打包的通用操作，基于卷积切片分析确定分块大小和数据布局策略，处理边缘情况并调整仿射映射。

Result: 在标准卷积配置下，生成的代码在ARM SME上达到峰值性能的60%，在Intel AVX512上达到67%，验证了该方法的有效性。

Conclusion: SConvTransform证明了在MLIR转换方言中结合静态形状分析与结构化分块打包策略的益处，其模块化设计便于未来扩展和集成。
...
TL;DR: 该研究首次对胶质母细胞瘤随访MRI中的深度学习模型进行了分期基准测试，发现在第二次随访时模型性能更好，Mamba+CNN混合模型在准确性和效率方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 区分胶质母细胞瘤的真实进展与治疗相关的假性进展在早期随访中具有挑战性，需要评估不同深度学习架构在不同时间点的性能表现。

Method: 使用Burdenko GBM进展队列(n=180)，在统一的质量控制驱动流程下训练11种代表性深度学习家族(CNN、LSTM、混合模型、Transformer和选择性状态空间模型)，采用患者级交叉验证。

Result: 两个阶段的准确率相当(~0.70-0.74)，但在第二次随访时区分能力改善，F1和AUC提高。Mamba+CNN混合模型在准确性和效率方面表现最佳，Transformer变体具有竞争力的AUC但计算成本高。

Conclusion: 研究建立了分期感知基准，表明绝对区分能力整体仍然有限，反映了TP与PsP区分的固有困难，未来需要整合纵向建模、多序列MRI和更大的多中心队列。
...
TL;DR: 提出端到端深度学习框架，仅从4倍加速的欠采样k空间数据联合估计线圈灵敏度图和重建MRI图像，无需预计算线圈灵敏度图


<details>
  <summary>Details</summary>
Motivation: 传统并行MRI技术如SENSE需要欠采样k空间数据和预计算线圈灵敏度图，限制了临床应用。MRI采集时间长，影响患者吞吐量并增加运动伪影风险

Method: 两模块架构：线圈灵敏度图估计模块和基于U-Net的MRI重建模块，直接从欠采样k空间测量进行端到端学习

Result: 在10名受试者的多线圈脑部MRI数据上评估，与2x SENSE重建作为基准相比，产生视觉更平滑的重建结果，尽管PSNR/SSIM指标较低但视觉质量相当

Conclusion: 方法在4倍加速下能实现视觉质量相当的MRI重建，但面临不同加速因子间空间错位等挑战，提出了改进重建质量的未来方向
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在第一人称视角下进行意图和行为推理的挑战，因为自我中心视频存在部分可观测性、有限视野和自参考运动等问题。

Method: 基于组相对策略优化(GRPO)，交替进行自我中心规划阶段（从第一人称视角预测未来行动步骤）和外部中心验证阶段（从第三人称视角检查计划的视觉和逻辑一致性）。

Result: 在自我中心推理任务上显著提升，比基线Qwen2.5-VL-7B在EgoBlind上提升+7.7，在EgoOrient上提升+4.4，同时在外部中心视频任务上保持强泛化能力。

Conclusion: EgoVITA通过结构化的规划和验证机制，使模型能够做出对即将到来的视觉观察具有因果预测性的计划，从而实现更连贯和视觉基础化的推理。
...
TL;DR: 本文提出UniFlow方法，通过跨数据集训练学习通用的LiDAR场景流运动先验，在多个数据集上达到新的最佳性能，并展现出良好的跨传感器泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR场景流方法通常只在单一传感器上训练和评估，缺乏跨传感器的泛化能力。本文旨在学习能够迁移到不同和未见过的LiDAR传感器的通用运动先验。

Method: 提出UniFlow方法，统一在多个大规模LiDAR场景流数据集上进行训练，这些数据集具有不同的传感器布局和点云密度。这是一个简单的前馈模型家族。

Result: 在Waymo和nuScenes数据集上分别比先前工作提升了5.1%和35.2%，在未见过的TruckScenes数据集上比专门模型提升了30.1%，建立了新的最佳性能。

Conclusion: 运动估计等低级任务对传感器配置的敏感性较低，跨数据集训练能够显著提升LiDAR场景流性能，并具有良好的跨传感器泛化能力。
...
### [127] [Sequence-Adaptive Video Prediction in Continuous Streams using Diffusion Noise Optimization](https://arxiv.org/abs/2511.18255)
*Sina Mokhtarzadeh Azar,Emad Bahrami,Enrico Pallotta,Gianpiero Francesca,Radu Timofte,Juergen Gall*

Main category: cs.CV

TL;DR: 提出了SAVi-DNO方法，通过优化扩散噪声而非调整模型参数，使预训练的扩散模型能够自适应连续视频流预测，在多个数据集上取得了性能提升。


<details>
  <summary>Details</summary>
Motivation: 针对连续视频流预测场景，模型会不断遇到新的训练样本，需要利用这些新样本来改进预测性能，但直接微调大型扩散模型参数成本过高。

Method: 保持扩散模型参数不变，在推理过程中优化扩散噪声，让模型自适应地确定合适的采样噪声，称为序列自适应视频预测与扩散噪声优化(SAVi-DNO)。

Result: 在Ego4D、OpenDV-YouTube、UCF-101和SkyTimelapse等数据集的长视频上，基于FVD、SSIM和PSNR指标均显示出性能改进。

Conclusion: SAVi-DNO方法通过优化扩散噪声实现了对连续视频流的有效自适应预测，避免了昂贵的模型参数微调，在多个数据集上验证了其有效性。
...
TL;DR: NeuroVFM是一个基于医疗系统数据的视觉基础模型，通过在524万临床MRI和CT扫描上训练，实现了神经影像任务的最先进性能，超越了前沿AI模型。


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型缺乏私有临床数据访问，特别是神经影像数据因面部特征识别问题在公共领域代表性不足，限制了在临床医学中的表现。

Method: 采用医疗系统学习范式，使用可扩展的体素联合嵌入预测架构，在524万临床MRI和CT体积数据上训练NeuroVFM模型。

Result: NeuroVFM在放射诊断和报告生成等多个临床任务中达到最先进性能，表现出新兴的神经解剖学理解和可解释的视觉定位能力，生成的放射学报告在准确性、临床分诊和专家偏好方面超越前沿模型。
...
### [129] [MammothModa2: A Unified AR-Diffusion Framework for Multimodal Understanding and Generation](https://arxiv.org/abs/2511.18262)
...
Result: 在公开基准测试中表现优异：GenEval 0.87、DPGBench 87.2、ImgEdit 4.06，同时与纯理解模型在多模态理解任务上保持竞争力。
...
TL;DR: MedVision是一个专门用于评估和改进视觉语言模型在医学图像定量分析能力的大规模数据集和基准，涵盖检测、肿瘤大小估计和角度测量等任务。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型主要针对分类问答任务设计，但临床决策需要定量评估能力（如测量肿瘤大小、关节角度），这种定量推理能力在现有模型中尚未得到充分探索和支持。

Method: 构建了MedVision大规模数据集，包含22个公共数据集的3080万图像-标注对，专注于三个代表性定量任务：解剖结构和异常检测、肿瘤/病变大小估计、角度/距离测量。

Result: 现成的视觉语言模型在这些任务上表现不佳，但通过在MedVision上进行监督微调，在检测、肿瘤估计和角度测量任务上的性能显著提升，错误率降低且精度提高。
...
TL;DR: SatSAM2是一个基于SAM2的零样本卫星视频跟踪器，通过引入卡尔曼滤波约束运动模块和运动约束状态机，在多个基准测试中优于现有方法，在OOTB数据集上AUC提升5.84%。
...
Conclusion: SatSAM2成功将基础模型适应到遥感领域，提出了有效的运动约束机制，并发布了MVOT大规模合成基准数据集促进进一步研究。
...
TL;DR: ObjectAlign是一个解决视频编辑中对象不一致性问题的框架，通过结合感知指标和符号推理来检测、验证和修正对象级和时间不一致性。
...
Method: 提出可学习的指标阈值，结合神经符号验证器（包括SMT-based身份漂移检查和概率模型检查器的时间保真度验证），以及基于神经网络的插值修复方法。

Result: 在DAVIS和Pexels视频数据集上，相比最先进基线方法，CLIP Score提升1.4分，warp error提升6.1分。

Conclusion: ObjectAlign框架能够有效解决视频编辑中的对象不一致性问题，确保低层稳定性和高层时间正确性。
...
### [133] [Vision Token Masking Alone Cannot Prevent PHI Leakage in Medical Document OCR: A Systematic Evaluation](https://arxiv.org/abs/2511.18272)
*Richard J. Young*

Main category: cs.CV

TL;DR: 首次系统评估了推理时视觉token掩码作为医学文档OCR隐私保护机制的效果，发现所有掩码策略都只能达到42.9%的PHI减少率，成功抑制长格式标识符但无法阻止短结构化标识符泄漏。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在医疗环境中的OCR应用引发了受保护健康信息(PHI)暴露的严重担忧，需要评估视觉token掩码作为隐私保护机制的有效性。

Method: 使用DeepSeek-OCR，引入7种掩码策略针对不同架构层，在100份合成医疗账单上评估HIPAA定义的PHI类别减少效果，并进行掩码扩展半径的消融研究。

Result: 所有掩码策略都收敛到42.9%的PHI减少率，能100%有效抑制长格式空间分布标识符，但对短结构化标识符完全无效(0%效果)。增加空间覆盖范围无法突破这一上限。

Conclusion: 视觉掩码在VLM中存在固有局限性，语言模型的上下文推理驱动了结构化标识符泄漏。未来研究应转向解码器级微调和混合防御架构，以实现HIPAA合规的医疗文档处理。
...
TL;DR: 提出MC-LRD框架解决少样本视频域自适应问题，通过模态协作低秩分解器分离模态独特和共享特征，提升多模态特征在域对齐中的有效性。


<details>
  <summary>Details</summary>
Motivation: 视频多模态特性在少样本场景下同时面临域对齐和模态协作挑战，现有方法忽略此问题。域偏移导致各模态及融合特征的泛化性能受限，因为模态包含具有不同域偏移程度的耦合特征组件。

Method: MC-LRD框架包含各模态的多个分解器和多模态分解路由器，分解器参数在不同模态间渐进共享，MDR选择性激活分解器产生模态独特和共享特征。应用正交去相关约束增强多样性，并提出跨域激活一致性损失确保同类样本的分解器激活偏好一致。

Result: 在三个公共基准测试上的广泛实验结果表明，相比现有方法取得显著改进。

Conclusion: MC-LRD框架通过有效分解模态特征，成功解决了少样本视频域自适应中的域对齐和模态协作挑战。
...
### [135] [Point-to-Point: Sparse Motion Guidance for Controllable Video Editing](https://arxiv.org/abs/2511.18277)
*Yeji Song,Jaehyun Lee,Mijin Koo,JunHoo Lee,Nojun Kwak*

Main category: cs.CV

TL;DR: 提出了一种名为锚点令牌的新型运动表示方法，通过少量有意义的点轨迹编码视频动态，实现更可控和语义对齐的视频编辑。
...
Method: 利用视频扩散模型的丰富先验，通过锚点令牌捕捉最本质的运动模式，这些令牌通过少量信息丰富的点轨迹紧凑编码视频动态，并可灵活重定位以对齐新主题。

Result: 广泛的实验表明，锚点令牌能实现更可控和语义对齐的视频编辑，在编辑保真度和运动保真度方面达到优越性能。

Conclusion: 所提出的Point-to-Point方法通过锚点令牌运动表示，能够泛化到多样场景，解决了视频编辑中运动保持的核心挑战。
...
### [136] [Yo'City: Personalized and Boundless 3D Realistic City Scene Generation via Self-Critic Expansion](https://arxiv.org/abs/2511.18734)
*Keyang Lu,Sifan Zhou,Hongbin Xu,Gang Xu,Zhifei Yang,Yikai Wang,Zhen Xiao,Jieyi Long,Ming Li*

Main category: cs.CV

TL;DR: Yo'City是一个基于大模型的智能框架，通过分层规划和迭代生成实现用户定制化、无限扩展的3D城市生成，在语义、几何、纹理和布局方面优于现有方法。
...
Method: 采用分层"城市-区域-网格"规划策略，结合全局规划和局部设计，通过"生成-优化-评估"的等距图像合成循环和图像到3D生成，并引入基于场景图的布局优化机制。

Result: 在构建的多样化基准数据集上，Yo'City在语义、几何、纹理和布局等六个多维指标上均优于现有最先进方法。

Conclusion: Yo'City框架成功实现了用户定制化和无限扩展的3D城市生成，展示了基于大模型的智能框架在城市生成任务中的有效性。
...
TL;DR: 提出了Foresight Intelligence概念和FSU-QA数据集，用于评估和增强视觉语言模型的前瞻推理能力，实验表明小模型在FSU-QA微调后能超越大模型。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了预测和解释未来事件的能力，这种前瞻智能对自动驾驶等应用至关重要。

Method: 引入FSU-QA视觉问答数据集，用于评估VLMs的前瞻推理能力，并通过增强世界模型输出来量化语义连贯性。

Result: 当前最先进的VLMs在前瞻任务中表现不佳，但小模型在FSU-QA微调后能大幅超越大模型。
...
### [138] [RoadSceneVQA: Benchmarking Visual Question Answering in Roadside Perception Systems for Intelligent Transportation System](https://arxiv.org/abs/2511.18286)
...
TL;DR: 提出了RoadSceneVQA数据集和RoadMind基线模型，通过视觉问答和推理任务提升路边场景的语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有路边感知系统主要关注实例级感知，缺乏自然语言交互和上下文交通行为推理能力，需要填补这一空白。

Method: 构建RoadSceneVQA数据集，提出CogniAnchor Fusion视觉语言融合模块和Assisted Decoupled Chain-of-Thought推理增强方法。

Result: 在RoadSceneVQA和CODA-LM基准测试中，模型在结构化交通感知和推理任务上达到最先进性能。

Conclusion: 该方法有效提升了多模态大语言模型在路边场景中的推理准确性和计算效率。
...
### [139] [ProxT2I: Efficient Reward-Guided Text-to-Image Generation via Proximal Diffusion](https://arxiv.org/abs/2511.18742)
*Zhenghan Fang,Jian Zheng,Qiaozi Gao,Xiaofeng Gao,Jeremias Sulam*

Main category: cs.CV

TL;DR: 提出ProxT2I文本到图像扩散模型，使用后向离散化和条件近端算子替代传统分数函数，结合强化学习优化采样器，在降低计算量和模型大小的同时实现高效采样和人类偏好对齐。
...
Method: 开发基于后向离散化的文本到图像扩散模型ProxT2I，使用学习的条件近端算子替代分数函数，结合强化学习和策略优化优化采样器，并构建包含1500万高质量人脸图像的大规模数据集LAION-Face-T2I-15M。

Result: 相比基于分数的基线方法，该方法显著提升了采样效率和人类偏好对齐，在计算量和模型大小更小的情况下，性能与现有最先进的文本到图像模型相当。

Conclusion: ProxT2I为人类文本到图像生成提供了一个轻量级但高性能的解决方案，在保持性能的同时降低了计算和模型复杂度。
...
### [140] [DiVE-k: Differential Visual Reasoning for Fine-grained Image Recognition](https://arxiv.org/abs/2511.18305)
*Raja Kumar,Arka Sadhu,Ram Nevatia*

Main category: cs.CV

TL;DR: DiVE-k框架通过利用模型自身的前k个预测作为训练信号，使用强化学习训练模型在多个选择中选出正确答案，从而提升细粒度视觉识别能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然拥有丰富的文本知识，但在细粒度图像识别方面表现不佳，难以区分视觉上相似的类别。现有的基于精确匹配奖励的强化学习方法容易导致记忆训练类别，缺乏泛化能力。

Method: 提出DiVE-k框架：为每个训练图像创建基于模型前k个输出的多选题，使用强化学习训练模型选择正确答案，要求模型在合理选项间进行细粒度差分推理。

Result: 在五个标准细粒度数据集上的实验表明，DiVE-k显著优于现有方法。在基础到新类泛化设置中，DiVE-k在调和平均数指标上分别超过QWEN2.5-VL-7B和ViRFT 10.04%和6.16%。

Conclusion: DiVE-k通过差分视觉推理方法有效缓解了记忆问题，提高了模型对未见类别的泛化能力，在多种场景下均表现出色。
...
TL;DR: 提出PEWM（Primitive Embodied World Models）框架，通过限制视频生成为较短时间跨度，解决基于视频生成的具身世界模型对大规模交互数据的依赖问题，实现语言与动作的细粒度对齐。


<details>
  <summary>Details</summary>
Motivation: 基于视频生成的具身世界模型依赖大规模具身交互数据，但这类数据稀缺、收集困难且维度高，限制了语言与动作的对齐粒度，阻碍了具身领域的"GPT时刻"。

Method: 1) 将视频生成限制在固定较短时间跨度；2) 配备模块化视觉语言模型规划器和Start-Goal热图引导机制；3) 利用视频模型的时空视觉先验和VLM的语义感知能力。

Result: 实现了语言概念与机器人动作视觉表示的细粒度对齐，降低了学习复杂度，提高了数据收集效率，减少了推理延迟，支持在复杂任务上实现原始级策略的组合泛化。

Conclusion: PEWM框架通过连接细粒度物理交互与高层推理，为可扩展、可解释、通用目的的具身智能铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 解决多视角图像中真实缺陷与视角变化引起的良性外观变化难以区分的问题，现有方法将多视角视为不连通的图像集合，导致特征表示不一致和误报率高。

Method: 提出ViewSense-AD框架，包含多视角对齐模块(MVAM)利用单应性投影对齐相邻视角特征，集成到视角对齐潜在扩散模型(VALDM)中实现渐进式多阶段对齐，并使用融合精炼模块(FRM)增强全局一致性。

Result: 在RealIAD和MANTA数据集上的广泛实验表明，VSAD在像素级、视角级和样本级视觉异常检测中显著优于现有方法，证明其对大视角偏移和复杂纹理的鲁棒性。

Conclusion: VSAD通过几何一致性建模和多阶段对齐，能够从粗到细尺度构建对物体表面的连贯整体理解，在多视角异常检测中实现了新的最先进性能。
...
### [143] [Stro-VIGRU: Defining the Vision Recurrent-Based Baseline Model for Brain Stroke Classification](https://arxiv.org/abs/2511.18316)
*Subhajeet Das,Pritam Paul,Rohit Bahadur,Sohan Das*

Main category: cs.CV

TL;DR: 基于预训练Vision Transformer的迁移学习框架，用于脑卒中早期识别，通过冻结部分编码器块和微调剩余部分来学习脑卒中特征，结合Bi-GRU分类器，在脑卒中数据集上达到94.06%准确率。
...
Method: 使用预训练Vision Transformer进行迁移学习，冻结部分编码器块，微调其余部分学习脑卒中特征，提取特征输入单层Bi-GRU进行分类，通过数据增强处理类别不平衡问题。
...
TL;DR: 提出了Re-CatVTON，一种高效的单一UNet虚拟试穿模型，通过改进的条件特征学习和分类器自由引导策略，在减少计算和内存开销的同时实现高性能。


<details>
  <summary>Details</summary>
Motivation: 基于双UNet架构的扩散模型在虚拟试穿任务中虽然性能优越，但计算和内存开销巨大。本研究旨在开发高效的单一UNet模型来平衡性能与效率。

Method: 通过可视化和理论分析提出三个关于上下文特征学习的假设，开发Re-CatVTON模型，采用改进的分类器自由引导策略，并直接注入干净服装潜在特征以防止预测误差累积。

Result: Re-CatVTON相比前身CatVTON显著提升性能，在FID、KID和LPIPS指标上表现更好，仅SSIM略有下降，计算和内存需求低于高性能双UNet模型Leffa。

Conclusion: Re-CatVTON为单一UNet虚拟试穿模型建立了新的效率-性能平衡点，在保持高质量的同时显著降低计算成本。
...
### [145] [Optimal Pose Guidance for Stereo Calibration in 3D Deformation Measurement](https://arxiv.org/abs/2511.18317)
...
<details>
  <summary>Details</summary>
Motivation: 现有立体标定方法缺乏直观的最优姿态指导，导致变形测量效率低下且精度不理想，需要开发能够自动生成最优姿态的交互式标定框架。

Method: 提出姿态优化方法，引入相对和绝对外参的联合优化，以协方差矩阵迹最小化为损失函数求解下一个最优姿态，并集成用户友好的图形界面。

Result: 与随机姿态相比，该方法在效率（需要更少图像）和精度（测量误差更低）方面表现更优，在不同视场下保持鲁棒性，热变形测量结果与有限元分析高度一致。
...
<details>
  <summary>Details</summary>
Motivation: 多模态视频生成系统虽然增强了可控性，但也带来了新的安全风险，现有安全方法多为文本导向、需要预知风险类别或作为后生成审计，难以主动缓解多模态组合风险。
...
Conclusion: ConceptGuard为多模态视频生成提供了一个有效的主动安全保障框架，能够有效应对组合性多模态风险。
...
TL;DR: 提出了一种结合dMRI和fMRI数据的双流束流分类框架，用于增强白质束分割的功能一致性，特别是在区分具有相似路径但功能不同的纤维束方面。


<details>
  <summary>Details</summary>
Motivation: 当前束流分类方法主要依赖束流轨迹的几何特征，无法区分具有相似路径但功能不同的纤维束，需要结合功能信息来提高分割的准确性。

Method: 设计了一个双流网络，使用预训练的主干网络处理完整束流轨迹，同时通过辅助网络处理纤维端点区域的fMRI信号，共同进行束流分类。

Result: 通过将皮质脊髓束分割为四个躯体定位亚区，消融实验和与最先进方法的比较证明了该方法的优越性能。

Conclusion: 提出的双流框架能够有效结合结构和功能信息，显著提高束流分类的功能一致性，为白质束的精细分割提供了新思路。
...
TL;DR: 构建了SciPostLayoutTree数据集，包含约8000张标注了阅读顺序和父子关系的学术海报，并开发了Layout Tree Decoder模型来预测海报结构关系。


<details>
  <summary>Details</summary>
Motivation: 学术海报在学术交流中扮演重要角色，但相比论文，海报的结构分析研究相对不足。需要构建专门的数据集和模型来分析海报的阅读顺序和结构关系。

Method: 构建SciPostLayoutTree数据集，开发Layout Tree Decoder模型，该模型结合视觉特征和边界框特征（位置和类别信息），使用beam search来预测关系并捕捉序列级合理性。

Result: 实验结果表明，该模型提高了对空间挑战性关系（向上、水平、长距离关系）的预测准确性，为海报结构分析建立了坚实的基线。

Conclusion: SciPostLayoutTree数据集和Layout Tree Decoder模型填补了海报结构分析的研究空白，为构建结构感知界面提供了基础。
...
### [149] [ConsistCompose: Unified Multimodal Layout Control for Image Composition](https://arxiv.org/abs/2511.18333)
...
<details>
  <summary>Details</summary>
Motivation: 现有的统一多模态模型主要关注视觉接地，而布局可控的多实例生成能力不足，限制了精确的组合控制。

Method: 提出ConsistCompose框架，使用实例-坐标绑定提示和坐标感知的无分类器引导，将语言布局线索转化为精确的空间控制，并构建了3.4M的多实例生成数据集。

Result: 在COCO-Position和MS-Bench上的实验表明，ConsistCompose显著提高了空间准确性，同时保持了身份保真度和竞争性的多模态理解能力。

Conclusion: ConsistCompose为布局可控的多模态图像生成建立了一个统一的范式，实现了精确的空间控制和多实例生成。
...
### [150] [Mitigating Long-Tail Bias in HOI Detection via Adaptive Diversity Cache](https://arxiv.org/abs/2511.18811)
...
Method: 构建类别特定的缓存，在推理过程中累积高置信度和多样化的特征表示，结合频率感知的缓存适配机制，无需额外训练即可实现鲁棒的预测校准。

Result: 在HICO-DET和V-COCO数据集上，ADC显著提升现有HOI检测器性能，罕见类别mAP提升达8.57%，完整数据集提升4.39%。

Conclusion: ADC能有效缓解长尾偏差，同时保持整体性能，是一种高效且可扩展的解决方案。
...
### [151] [A Tri-Modal Dataset and a Baseline System for Tracking Unmanned Aerial Vehicles](https://arxiv.org/abs/2511.18344)
*Tianyang Xu,Jinjie Gu,Xuefeng Zhu,XiaoJun Wu,Josef Kittler*

Main category: cs.CV

TL;DR: 提出了MM-UAV，首个大规模多模态无人机跟踪基准数据集，包含RGB、红外和事件信号三种模态，涵盖30多个挑战性场景，包含1321个同步多模态序列和280万标注帧。同时提出了专门的多模态多无人机跟踪框架，包含偏移引导自适应对齐模块和自适应动态融合模块，以及事件增强关联机制。
...
Method: 提出了专门的多模态多无人机跟踪框架，包含两个关键技术创新：偏移引导自适应对齐模块解决传感器间的空间不匹配问题，自适应动态融合模块平衡不同模态的互补信息。还引入了事件增强关联机制，利用事件模态的运动线索进行更可靠的身份维护。
...
### [152] [FlowSteer: Guiding Few-Step Image Synthesis with Authentic Trajectories](https://arxiv.org/abs/2511.18834)
...
<details>
  <summary>Details</summary>
Motivation: 尽管ReFlow在理论上与流匹配一致，但在实际应用中性能不如一致性蒸馏和分数蒸馏，采样效率是流匹配实际应用的关键瓶颈。

Method: 提出FlowSteer方法：1）在线轨迹对齐解决训练中的分布不匹配问题；2）在ODE轨迹上应用对抗蒸馏目标，提高学生对教师生成轨迹的遵循度；3）修复FlowMatchEulerDiscreteScheduler中的缺陷。

Result: 在SD3上的实验结果表明该方法有效提升了性能。

Conclusion: FlowSteer通过轨迹对齐和对抗蒸馏解锁了ReFlow蒸馏的潜力，显著提升了采样效率。
...
### [153] [FlowPortal: Residual-Corrected Flow for Training-Free Video Relighting and Background Replacement](https://arxiv.org/abs/2511.18346)
...
Method: 提出残差校正光流机制将标准光流模型转换为编辑模型，采用解耦条件设计实现精确光照控制，高频传输机制保护细节，以及掩码策略分离前景重光照和背景生成过程。

Result: 实验证明FlowPortal在时间一致性、结构保持和光照真实感方面表现优异，同时保持高效率。

Conclusion: FlowPortal通过创新的光流机制和条件设计，成功解决了视频重光照中的关键挑战，实现了高质量、高效率的视频编辑效果。
...
TL;DR: FedOAP是一个新颖的个性化联邦学习方法，用于器官无关的肿瘤分割，通过解耦交叉注意力和扰动边界损失来提升分割性能。
...
Method: 使用解耦交叉注意力(DCA)让每个客户端保留本地查询，同时关注从所有客户端聚合的全局共享键值对；引入扰动边界损失(PBL)来改善预测掩码边界的一致性。

Result: 在跨越不同器官的多样化肿瘤分割任务上，FedOAP始终优于现有的最先进联邦和个性化分割方法。

Conclusion: FedOAP通过建模客户端间共享特征的长程依赖关系和边界感知损失，有效提升了器官无关肿瘤分割的性能。
...
### [155] [MagicWand: A Universal Agent for Generation and Evaluation Aligned with User Preference](https://arxiv.org/abs/2511.18352)
*Zitong Xu,Dake Shen,Yaosong Du,Kexiang Hao,Jinghan Huang,Xiande Huang*

Main category: cs.CV

TL;DR: 提出了MagicWand，一个基于用户偏好的通用生成和评估代理，通过UniPrefer-100K数据集和UniPreferBench基准来改善AIGC内容与用户偏好的对齐。


<details>
  <summary>Details</summary>
Motivation: 解决用户难以通过详细提示获得符合偏好的AIGC内容，以及缺乏保留用户偏好机制的问题。

Method: 构建UniPrefer-100K大规模数据集，开发MagicWand代理来基于用户偏好增强提示、生成高质量内容，并进行偏好对齐的评估和优化。

Result: 在UniPreferBench基准测试中，MagicWand在各种场景下都能生成与用户偏好良好对齐的内容和评估。

Conclusion: MagicWand通过偏好驱动的生成和评估，有效解决了AIGC内容与用户偏好对齐的挑战，在多样化任务中表现一致。
...
### [156] [Deep Hybrid Model for Region of Interest Detection in Omnidirectional Videos](https://arxiv.org/abs/2511.18856)
*Sana Alamgeer*
...
Method: 包括视频预处理获取帧、开发混合显著性模型预测感兴趣区域、后处理模型输出获得每帧的ROI。

Result: 将所提方法与360RAT数据集的主观标注进行了性能比较。

Conclusion: 成功设计并测试了一个用于识别360度视频感兴趣区域的混合显著性模型。
...
TL;DR: 本文提出了logits-to-video（L2V）任务和TRANSPORTER方法，通过生成视频来理解和解释视觉语言模型的预测逻辑。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型能够理解复杂场景，但其内部推理过程难以解释和控制。受文本到视频生成模型进展的启发，需要开发新的方法来揭示模型预测背后的规则。

Method: TRANSPORTER方法学习最优传输耦合到VLM的高语义嵌入空间，利用logit分数定义嵌入方向进行条件视频生成。

Result: TRANSPORTER能够生成反映不同对象属性、动作副词和场景上下文变化的视频，为模型可解释性提供了新的方向。

Conclusion: L2V任务为模型可解释性提供了保真度高且新颖的研究方向，这是之前未被探索过的。
...
TL;DR: 提出了一种4D高斯泼溅的尺度自适应滤波器和尺度损失方法，解决了动态场景重建中因相机焦距或距离变化导致的渲染分辨率调整时出现的高频伪影问题。
...
Result: 该方法在增加渲染频率时消除了高频伪影，并在多视角视频重建中有效减少了冗余高斯基元。

Conclusion: 通过单目和多视角视频重建实验验证了所提方法的有效性，能够实现无伪影的动态场景重建。
...
Method: 使用动态中心距离机制建模边界不确定性，通过加权特征距离关注前景、背景和边界中心，引导模型关注模糊边界附近的难分割像素。

Result: 在四个基准数据集上的广泛实验表明，MetaDCSeg在不同噪声水平下均优于现有最先进方法。

Conclusion: MetaDCSeg通过动态像素权重学习和边界不确定性建模，有效提升了医学图像分割的鲁棒性和准确性。
...
### [160] [MASS: Motion-Aware Spatial-Temporal Grounding for Physics Reasoning and Comprehension in Vision-Language Models](https://arxiv.org/abs/2511.18373)
*Xiyang Wu,Zongxia Li,Jihui Jin,Guangyao Shi,Gouthaman KV,Vishnu Raj,Nilotpal Sinha,Jingxi Chen,Fan Du,Dinesh Manocha*

Main category: cs.CV

TL;DR: 提出了MASS方法，通过将物理世界上下文线索转化为可解释表示来增强视觉语言模型的物理推理能力，在物理相关理解任务上显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在标准视频任务上表现良好，但在涉及运动动力学和空间交互的物理驱动推理方面存在困难，这限制了它们解释真实或AI生成视频内容以及生成物理一致内容的能力。

Method: 提出了MASS方法，包括：1）MASS-Bench基准数据集（4,350个视频和8,361个问答对）；2）通过深度3D编码和视觉接地将时空信号注入VLM语言空间；3）使用运动跟踪器处理物体动力学；4）应用强化微调加强跨模态对齐和推理。
...
<details>
  <summary>Details</summary>
Motivation: 传统GRPO框架的性能受限于文本-视觉对应关系的模糊性：单个提示可能对应多种视觉输出，单个图像/视频可能有多种正确解释。这种多对多关系导致奖励模型产生不确定和弱区分性的信号，使GRPO无法充分利用可靠反馈并过度拟合噪声信号。

Method: BPGO在GRPO基础上引入语义先验锚点来显式建模奖励不确定性。采用两级自适应优化信任调节：组间贝叶斯信任分配强调与先验一致的组更新，降低模糊组的权重；组内先验锚定重归一化通过扩展置信偏差和压缩不确定分数来锐化样本区分。

Result: 在图像和视频生成任务中，BPGO相比标准GRPO及其变体，实现了更强的语义对齐、更高的感知保真度和更快的收敛速度。

Conclusion: BPGO通过贝叶斯先验引导的优化方法有效解决了GRPO中的奖励不确定性问题，在视觉生成任务中展现出优越的性能表现。
...
TL;DR: 提出了CompGen框架，通过场景图建立难度标准，使用自适应MCMC图采样算法生成渐进式训练数据，结合强化学习提升文本到图像生成的组合能力。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中的组合合成问题，特别是多对象场景中精确的对象放置和连贯的对象间交互的挑战。

Method: 使用场景图建立组合能力难度标准，开发自适应MCMC图采样算法，结合GRPO强化学习框架，采用不同课程调度策略。

Result: CompGen在不同课程调度策略下表现出不同的扩展曲线，易到难和高斯采样策略优于随机采样，显著提升了扩散式和自回归T2I模型的组合生成能力。

Conclusion: CompGen框架有效提升了文本到图像生成系统的组合能力，证明了课程学习在组合T2I生成中的有效性。
...
TL;DR: 本文系统研究了Mamba在视觉任务中的表示特性，揭示了其与Softmax Attention的关系，提出了新的激活图评估方法，并展示了Mamba在可解释性和性能方面的优势。


<details>
  <summary>Details</summary>
Motivation: Mamba作为视觉任务的有效骨干网络受到关注，但其在视觉领域的底层机制仍不清楚，需要系统研究其表示特性。

Method: 理论分析Mamba与Softmax和Linear Attention的关系；引入二元分割指标量化评估激活图；利用DINO进行自监督预训练获得更清晰的激活图。

Result: 确认Mamba可作为Softmax Attention的低秩近似；新指标显示Mamba能建模长程依赖；自监督预训练产生更清晰的激活图；在ImageNet上达到78.5%的线性探测准确率。
...
### [164] [ViMix-14M: A Curated Multi-Source Video-Text Dataset with Long-Form, High-Quality Captions and Crawl-Free Access](https://arxiv.org/abs/2511.18382)
*Timing Yang,Sucheng Ren,Alan Yuille,Feng Wang*

Main category: cs.CV

TL;DR: 本文提出了ViMix-14M，一个包含约1400万对视频-文本的精选多源数据集，解决了开源视频生成模型面临的数据瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 解决开源视频生成模型面临的数据瓶颈：缺乏大规模、高质量、易于获取的视频-文本语料库。现有公共数据集通常需要手动爬取YouTube，存在链接失效、访问限制和许可不确定性等问题。

Method: 通过合并多样化的开源视频源，进行统一的去重和质量过滤，并采用多粒度、基于真实情况指导的重新标注流程，优化描述以更好地匹配动作、场景和时间结构。

Result: 通过多模态检索、文本到视频生成和视频问答任务的评估，相比对照数据集观察到一致的改进。
...
TL;DR: 本研究探讨了注意力架构和零样本学习方法能否弥合植物病害分类中学术数据集与现实农业条件之间的差距，发现CLIP模型通过自然语言描述直接分类病害，无需任务特定训练，展现出强大的适应性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于PlantVillage数据集的研究虽然准确率高，但无法泛化到农民提交的真实田间图像，这造成了研究成果与实际应用需求之间的显著差距。

Method: 评估了三种模型类别：卷积神经网络(CNNs)、视觉Transformer和基于对比语言-图像预训练(CLIP)的零样本模型。

Result: CNNs在领域转移下表现出有限的鲁棒性，视觉Transformer通过捕捉全局上下文特征展现出更强的泛化能力，而CLIP模型无需任务特定训练即可直接根据自然语言描述分类病害。
...
TL;DR: 本文提出Life-IQA框架，通过GCN增强的层间交互和MoE特征解耦来解决盲图像质量评估中特征贡献不均和有效解码架构不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有BIQA方法融合浅层和深层特征时忽略了它们对质量预测的不平等贡献，且质量解码架构研究不足。
...
### [167] [SegSplat: Feed-forward Gaussian Splatting and Open-Set Semantic Segmentation](https://arxiv.org/abs/2511.18386)
*Peter Siegel,Federico Tombari,Marc Pollefeys,Daniel Barath*

Main category: cs.CV

TL;DR: SegSplat是一个将快速前馈3D重建与开放词汇语义理解相结合的新框架，通过构建紧凑的语义记忆库和预测3D高斯的语义索引，实现无需逐场景优化的语义感知3D环境生成。


<details>
  <summary>Details</summary>
Motivation: 弥合快速前馈3D重建与丰富开放词汇语义理解之间的差距，为机器人交互、增强现实等智能系统提供实用的语义感知3D环境生成能力。

Method: 从多视角2D基础模型特征构建紧凑语义记忆库，为每个3D高斯预测离散语义索引以及几何和外观属性，单次前馈完成语义注入。

Result: 在几何保真度上与最先进的前馈3D高斯泼溅方法相当，同时实现鲁棒的开放集语义分割，无需任何逐场景优化的语义特征集成。

Conclusion: 这项工作向实用的、实时的语义感知3D环境生成迈出了重要一步，对推进机器人交互、增强现实等智能系统具有重要意义。
...
TL;DR: 提出了一种基于DINOv3预训练模型的多尺度交叉注意力差异孪生网络(MC-DiSNet)，用于解决冲突区域损伤评估中的语义变化检测问题，并引入了新的Gaza-change数据集和CSD任务框架。


<details>
  <summary>Details</summary>
Motivation: 冲突区域的损伤评估对援助和稳定至关重要，但存在数据有限、标注困难、类内相似性高和语义变化模糊等挑战。

Method: 使用DINOv3骨干网络提取双时相遥感图像特征，构建多尺度交叉注意力差异孪生网络，专注于变化区域的语义检测而非全图语义标注。

Result: 在Gaza-Change和SECOND数据集上的实验表明，该方法能有效解决CSD任务，为冲突区域快速损伤评估提供了实用解决方案。

Conclusion: 提出的CSD任务框架和MC-DiSNet方法在语义变化检测方面表现出色，为冲突区域的损伤评估开辟了新的应用途径。
...
### [169] [Exploring Weak-to-Strong Generalization for CLIP-based Classification](https://arxiv.org/abs/2511.18396)
*Jinhao Li,Sarah M. Erfani,Lei Feng,James Bailey,Feng Liu*

Main category: cs.CV

TL;DR: 提出了一种名为类原型学习（CPL）的方法，用于增强CLIP模型在弱监督下的分类能力，通过为每个类别学习更具代表性的原型，在预训练有限的情况下取得了3.67%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 随着模型复杂度增加，依赖人工监督变得不切实际。当模型超越人类知识时，提供准确反馈变得困难且低效。需要利用弱模型监督强模型的方法来减少人工监督负担。

Method: 提出了类原型学习（CPL）方法，通过为每个类别学习更具代表性的原型来增强CLIP模型的分类能力，在弱监督下使用简单的损失函数。

Result: 在预训练有限的情况下，CPL方法在目标场景中表现稳健，相比强基线方法取得了3.67%的性能提升。

Conclusion: 弱到强泛化方法在视觉语言模型中同样有效，CPL方法在弱监督下能够显著提升CLIP模型的分类性能，特别是在预训练资源有限的情况下。
...
TL;DR: MedSAM-3是一个基于文本提示的医学图像分割模型，通过微调SAM 3架构实现医学图像和视频的开放词汇分割，并引入MedSAM-3 Agent框架进行复杂推理和迭代优化。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法缺乏泛化性，需要大量手动标注，无法适应新的临床应用场景。

Method: 在医学图像上微调Segment Anything Model 3架构，结合语义概念标签，实现医学可提示概念分割；集成多模态大语言模型构建MedSAM-3 Agent框架，支持复杂推理和迭代优化。

Result: 在X光、MRI、超声、CT和视频等多种医学成像模态上的综合实验表明，该方法显著优于现有的专业模型和基础模型。

Conclusion: MedSAM-3通过文本提示实现了更精确的解剖结构分割，为医学图像分析提供了更通用和高效的解决方案。
...
### [171] [ChineseVideoBench: Benchmarking Multi-modal Large Models for Chinese Video Question Answering](https://arxiv.org/abs/2511.18399)
...
TL;DR: 提出了专门用于评估中文视频问答的多模态大语言模型的基准ChineseVideoBench，包含8个主类和12个子类，测试结果显示当前模型面临显著挑战。


<details>
  <summary>Details</summary>
Motivation: 随着对复杂视频分析能力需求的增长，需要全面、文化感知的评估框架来评估多模态大语言模型在中文视频内容上的表现。

Method: 构建包含8个主类和12个子类的数据集，设计专门的评估指标，对多种最先进的多模态大语言模型进行实证评估。

Result: Gemini 2.5 Pro表现最佳，总体得分77.9%；InternVL-38B是最具竞争力的开源模型。

Conclusion: ChineseVideoBench为评估多模态大语言模型在中文视频问答任务上的能力提供了重要基准，当前模型仍面临显著挑战。
...
TL;DR: MeanFlow通过联合学习瞬时和平均速度场实现少步高质量生成。研究发现瞬时速度是学习平均速度的前提，小时间间隔下平均速度有助于瞬时速度学习，但大间隔会降低效果。基于此设计了先加速瞬时速度形成、再转向长间隔平均速度的训练方案，显著提升了少步生成性能。


<details>
  <summary>Details</summary>
Motivation: 分析MeanFlow中瞬时速度场和平均速度场之间的训练动态交互关系，理解两者如何相互影响，以改进训练策略提升少步生成质量。

Method: 通过任务亲和性分析研究两种速度场的相互作用，发现瞬时速度是学习平均速度的前提，小间隔平均速度有助于瞬时速度学习但大间隔会降低效果。基于此设计分阶段训练策略：先加速瞬时速度形成，再逐步转向长间隔平均速度学习。

Result: 增强的MeanFlow训练实现了更快的收敛和显著更好的少步生成性能：使用相同DiT-XL骨干网络，在1-NFE ImageNet 256x256上达到FID 2.87，优于基线3.43；或以2.5倍更短训练时间达到基线性能，或使用更小的DiT-L骨干网络达到相同性能。

Conclusion: 瞬时速度和平均速度之间存在复杂的训练动态关系，通过理解这种关系并设计相应的分阶段训练策略，可以显著提升MeanFlow的少步生成性能，实现更快的收敛和更好的生成质量。
...
### [173] [4D-VGGT: A General Foundation Model with SpatioTemporal Awareness for Dynamic Scene Geometry Estimation](https://arxiv.org/abs/2511.18416)
*Haonan Wang,Hanyu Zhou,Haoyue Liu,Luxin Yan*

Main category: cs.CV

TL;DR: 提出4D-VGGT模型，通过分治策略处理动态场景几何估计中的时空特征表示问题，支持多视图多时间步输入，实现多任务预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法将时空特征对齐到统一潜在空间，但由于时空特征的异构性可能导致表示不匹配，需要更好的表示方法。

Method: 采用分治策略：1) 自适应视觉网格支持任意视图和时间步输入；2) 跨视图全局融合处理空间表示，跨时间局部融合处理时间表示；3) 多任务头实现全面的动态场景几何估计。

Result: 在多个动态场景几何基准测试中验证了方法的有效性，模型具有更好的特征区分能力和应用普适性。

Conclusion: 4D-VGGT通过分治的时空表示方法，有效解决了动态场景几何估计中的时空特征表示挑战，是一个通用的基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法严重依赖有限的多摄像头标记数据，难以实现真正的泛化能力。需要有效利用大规模单摄像头数据来提升模型在未见摄像头和环境中的识别能力。

Method: 包含三个核心组件：动态重标记模块优化单摄像头伪标签；高效质心模块在大身份空间下保持稳健身份表示；数据采样模块平衡学习复杂度和批内多样性。

Result: 在可泛化行人重识别任务中，DynaMix持续超越现有最先进方法，能够有效训练数百万图像和数十万身份。

Conclusion: DynaMix通过动态适应训练数据结构和噪声，成功结合多源数据，为可泛化行人重识别提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: T1CE MRI中脑血管的精确3D分割对神经外科手术规划至关重要。手动分割耗时且存在观察者间差异，现有自动化方法往往在准确性和计算成本之间权衡，限制了临床应用。

Method: 基于扩张U-Net架构，集成了两个专用模块：瓶颈处的多尺度上下文特征融合模块(MSC²F)和深层层次结构的跨域自适应特征融合模块(CDA²F)。MSC²F通过多尺度扩张卷积捕获局部和全局信息，CDA²F动态整合领域特定特征。

Result: 在137名脑肿瘤活检患者的T1CE扫描数据集上验证，获得Dice分数0.8609和精度0.8841，能够准确分割主要和细微血管结构。仅需12.4M参数，显著少于基于Transformer的模型。
...
TL;DR: CrossJEPA是一个简单的跨模态联合嵌入预测架构，利用图像基础模型的知识，通过训练预测器从3D点云推断特定渲染2D视图的嵌入，在3D表示学习中实现了高性能、内存高效和快速训练。


<details>
  <summary>Details</summary>
Motivation: 解决当前利用2D数据的3D表示学习方法导致模型庞大、训练缓慢的问题，探索JEPA架构在跨模态学习中的潜力，超越传统的掩码策略。

Method: 提出CrossJEPA架构，利用冻结的图像基础模型作为教师，训练点云编码器预测对应2D视图的嵌入，通过跨域投影信息净化监督信号，采用一次性目标嵌入缓存机制提高效率。

Result: 在ModelNet40上达到94.2%的线性探测准确率，在ScanObjectNN上达到88.3%，仅使用14.1M预训练参数（点编码器8.5M），在单GPU上约6小时完成预训练。

Conclusion: CrossJEPA是一个性能优异、内存高效且训练快速的3D表示学习框架，通过知识蒸馏实现了最先进的性能，为资源受限环境提供了可行的解决方案。
...
TL;DR: LungX是一种新型混合架构，结合EfficientNet多尺度特征、CBAM注意力机制和Vision Transformer全局上下文建模，在肺炎检测方面实现86.5%准确率和0.943 AUC的先进性能。
...
Result: 在20,000张胸部X光片上评估，达到86.5%准确率和0.943 AUC，比EfficientNet-B0基线提升6.7% AUC。视觉分析显示通过可解释注意力图实现优越的病变定位。

Conclusion: LungX在肺炎检测方面表现出色，未来方向包括多中心验证和架构优化，目标达到88%准确率用于临床部署。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中存在大量矛盾的多模态输入，但现有基准通常假设输入一致性，无法评估跨模态矛盾检测能力，这是防止幻觉和确保可靠性的基本能力。

Method: 创建CLASH基准，包含COCO图像与矛盾标题配对，提供经过自动质量检查过滤的微调集和人工验证的诊断集，在多项选择和开放式格式下评估模型。

Result: 对最先进模型的分析显示它们在识别跨模态冲突方面存在显著局限性，暴露出系统性模态偏差和类别特定弱点。

Conclusion: 在CLASH上进行针对性微调能显著增强冲突检测能力，该基准为评估和提升多模态系统的可靠性提供了重要工具。
...
### [179] [When Generative Replay Meets Evolving Deepfakes: Domain-Aware Relative Weighting for Incremental Face Forgery Detection](https://arxiv.org/abs/2511.18436)
*Hao Shen,Jikang Cheng,Renye Yan,Zhongyuan Wang,Wei Peng,Baojin Huang*

Main category: cs.CV

TL;DR: 提出了一种基于生成回放的增量伪造检测方法DARW，通过域感知相对加权策略区分域安全样本和域风险样本，有效提升增量学习性能并缓解域重叠的负面影响。
...
Method: 提出域感知相对加权策略(DARW)，直接监督域安全样本，对域风险样本应用相对分离损失，并通过域混淆分数动态调整监督与混淆之间的权衡。

Result: 大量实验表明DARW在不同生成回放设置下持续提升伪造检测的增量学习性能，并有效缓解域重叠的不利影响。

Conclusion: 生成回放是增量伪造检测的有效方法，DARW策略能够充分利用生成回放的优势，在保持性能的同时解决域边界模糊问题。
...
TL;DR: 研究评估了四个前沿视觉语言模型在意大利医学问答中的视觉依赖性，发现GPT-4o对视觉信息依赖最强，而其他模型更多依赖文本捷径。
...
Method: 使用欧洲医学问答意大利数据集的60个明确需要图像解释的问题，将正确的医学图像替换为空白占位符来测试模型是否真正依赖视觉信息。

Result: GPT-4o表现出最强的视觉依赖性，准确率下降27.9个百分点（从83.2%降至55.3%），而GPT-5-mini、Gemini和Claude准确率下降较小，分别为8.5pp、2.4pp和5.6pp。

Conclusion: 不同模型在视觉依赖性上存在显著差异，模型生成的推理显示所有模型都会对虚构的视觉解释给出自信的解释，强调了在临床部署前进行严格评估的必要性。
...
### [181] [Perceptual-Evidence Anchored Reinforced Learning for Multimodal Reasoning](https://arxiv.org/abs/2511.18437)
*Chi Zhang,Haibo Qiu,Qiming Zhang,Yufei Xu,Zhixiong Zeng,Siqi Yang,Peng Shi,Lin Ma,Jing Zhang*

Main category: cs.CV

TL;DR: PEARL提出了一种双分支感知-推理协同方法，通过将多模态推理明确锚定到已验证的视觉证据上来增强多模态推理能力。
...
Method: PEARL为每个推理问答实例生成感知检查清单，包含可验证答案的感知导向子问题。训练时通过辅助rollout获得感知奖励，作为推理的保真度门控。

Result: 在MathVerse等基准上，PEARL相比基线提升9.7%，相比GRPO提升6.6%。

Conclusion: PEARL能无缝集成到GRPO和DAPO等流行RL方法中，有效提升多模态推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的DiT视频生成模型虽然视觉质量和时间连贯性很好，但经常违反基本物理定律和常识动态，缺乏显式的世界知识。

Method: 设计了一个由3D CNN、低通/高通滤波器和自注意力层组成的记忆编码器，将参考视频映射为紧凑的记忆token，在DiT自注意力层中作为记忆使用。训练时冻结扩散主干，仅优化记忆编码器。

Result: 方法在少量训练参数(150M)和10K数据样本上实现高效训练，推理时可即插即用，显著改善了物理规则遵循和视频保真度。

Conclusion: DiT-Mem通过注入世界知识有效提升了视频生成模型的物理合理性和质量，为改进生成模型提供了新思路。
...
### [183] [SineProject: Machine Unlearning for Stable Vision Language Alignment](https://arxiv.org/abs/2511.18444)
*Arpit Garg,Hemanth Saratchandran,Simon Lucey*

Main category: cs.CV

TL;DR: 提出了SineProject方法，通过在冻结投影器中添加正弦调制可训练参数，改善多模态大语言模型知识遗忘过程中的视觉语言对齐问题，实现目标知识的完全遗忘同时减少良性查询拒绝。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型知识遗忘方法会破坏视觉语言对齐，导致模型拒绝有害和良性查询，这源于投影器在遗忘过程中雅可比矩阵严重病态化的问题。

Method: SineProject方法在冻结的投影器网络中引入正弦调制的可训练参数，改善雅可比矩阵的谱条件，在遗忘过程中稳定跨模态嵌入对齐。

Result: 在LLaVA v1.5 7B和13B模型的安全和隐私遗忘基准测试中，SineProject显著减少良性查询拒绝，同时实现目标信息的完全遗忘，达到最先进的遗忘-保留权衡。

Conclusion: SineProject通过简单的正弦调制参数有效解决了多模态大语言模型知识遗忘中的对齐问题，计算开销可忽略，实现了更好的遗忘效果。
...
TL;DR: 本研究通过完全模拟的3D环境，研究了针对卷积货物占用分类器的物理对抗性补丁攻击的可行性，在拒绝服务攻击场景中成功率高达84.94%。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉系统在现代物流操作中广泛应用，包括拖车占用率估计，但这些系统可能容易受到物理对抗性攻击，特别是可以打印并放置在内部表面的对抗性补丁。

Method: 使用Mitsuba 3进行可微分渲染，在几何、光照和视点变化的情况下优化补丁纹理，并与2D合成基线进行比较。

Result: 3D优化补丁在拒绝服务攻击场景（空到满）中达到84.94%的成功率，隐蔽攻击（满到空）达到30.32%的成功率。

Conclusion: 这是首个在物理真实、完全模拟的3D场景中研究货物占用估计对抗性补丁攻击的研究，强调了自动化物流管道安全加固的重要性。
...
### [185] [EventBench: Towards Comprehensive Benchmarking of Event-based MLLMs](https://arxiv.org/abs/2511.18448)
*Shaoyu Liu,Jianing Li,Guanghui Zhao,Yunjian Zhang,Xiangyang Ji*

Main category: cs.CV

TL;DR: EventBench是一个统一的多模态大语言模型事件视觉基准，包含8个任务指标和大规模事件流数据集，用于全面评估模型在事件理解、识别和空间推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在事件视觉领域取得了显著进展，但缺乏统一的基准来全面评估其能力。现有基准在开放性、任务多样性、空间维度集成和数据规模方面存在不足。

Method: 开发EventBench基准，具有四个关键特点：(1)开放性：发布所有原始事件流和任务指令；(2)多样性：涵盖理解、识别和空间推理任务；(3)集成性：首创3D空间推理任务设计；(4)大规模：包含超过100万事件-文本对的训练集。

Result: 评估了GPT-5、Gemini-2.5 Pro、Qwen2.5-VL、InternVL3和EventGPT等模型。结果显示，当前基于事件的MLLM在事件流理解方面表现良好，但在细粒度识别和空间推理方面仍有困难。

Conclusion: EventBench为基于事件的MLLM提供了全面的评估框架，揭示了当前模型在细粒度识别和空间推理方面的局限性，为未来研究指明了方向。
...
TL;DR: 本文建立了扩散模型数据集水印的统一评估框架，发现现有方法在通用性和可传递性方面表现良好，但在真实威胁场景下存在脆弱性，并提出了一种能完全移除水印的实用方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型微调技术虽然能复现特定图像集，但也带来了版权和安全风险。数据集水印技术被提出用于确保可追溯性，但缺乏统一的评估框架来验证其有效性。

Method: 建立了通用威胁模型，引入包含通用性、可传递性和鲁棒性的综合评估框架，并通过实验验证现有方法的性能，同时提出一种实用的水印移除方法。

Result: 实验表明现有方法在通用性和可传递性方面表现良好，对常见图像处理操作具有一定鲁棒性，但在真实威胁场景下仍存在不足。提出的水印移除方法能完全消除数据集水印而不影响微调效果。

Conclusion: 现有数据集水印方法在真实威胁场景下存在脆弱性，需要进一步研究来应对水印移除攻击，这是未来研究的关键挑战。
...
### [187] [NAF: Zero-Shot Feature Upsampling via Neighborhood Attention Filtering](https://arxiv.org/abs/2511.18452)
*Loick Chambon,Paul Couairon,Eloi Zablocki,Alexandre Boulch,Nicolas Thome,Matthieu Cord*

Main category: cs.CV

TL;DR: NAF是一种零样本特征上采样方法，通过交叉尺度邻域注意力和旋转位置编码学习自适应权重，无需重新训练即可提升任何视觉基础模型的特征分辨率，在多个下游任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有上采样方法面临基本权衡：经典滤波器速度快但形式固定，现代上采样器精度高但需要为每个VFM重新训练。需要一种既高效又无需重新训练的通用上采样方法。

Method: 使用交叉尺度邻域注意力和旋转位置编码学习自适应空间和内容权重，仅由高分辨率输入图像引导，实现零样本特征上采样。

Result: NAF是首个VFM无关架构，在多个下游任务中超越VFM特定上采样器，达到最先进性能，可扩展到2K特征图并以18 FPS重建中间分辨率图。

Conclusion: NAF成功弥合了经典滤波器和现代上采样器之间的差距，提供了一种高效、通用且无需重新训练的特征上采样解决方案，在图像恢复等任务中也表现出色。
...
TL;DR: DeCo提出了一种频率解耦的像素扩散框架，通过将高频细节和低频语义解耦生成，使用轻量级像素解码器生成高频细节，让DiT专注于低频语义建模，从而提升像素扩散模型的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有像素扩散模型训练和推理速度慢，因为它们通常在单个扩散变换器(DiT)中同时建模高频信号和低频语义。为了追求更高效的像素扩散范式，需要解耦高频和低频组件的生成。
...
Conclusion: DeCo通过频率解耦策略有效提升了像素扩散模型的效率和性能，证明了专门化建模高频和低频组件的优势，为像素扩散提供了新的高效范式。
...
TL;DR: 提出了一种基于医学原理的混合管道方法，通过显式测量肿瘤大小和距离属性来进行肺癌分期，而不是将其视为纯图像分类任务。该方法在Lung-PET-CT-Dx数据集上实现了91.36%的总体分类准确率。


<details>
  <summary>Details</summary>
Motivation: 端到端深度学习方法在肺癌分期中常常忽视空间和解剖学信息，而这些信息对于肿瘤-淋巴结-转移系统至关重要。肿瘤分期依赖于多个定量标准，包括肿瘤大小和与邻近解剖结构的距离，微小变化可能改变分期结果。

Method: 使用专门的编码器-解码器网络精确分割肺部和邻近解剖结构（包括肺叶、肿瘤、纵隔和膈肌），然后通过分割掩模的定量分析提取必要的肿瘤属性（测量最大肿瘤尺寸和计算肿瘤与邻近解剖结构的距离），最后应用基于规则的肿瘤分期方法。

Result: 在Lung-PET-CT-Dx数据集上表现出优于传统深度学习模型的性能，总体分类准确率达到91.36%，各阶段F1分数分别为：T1 0.93、T2 0.89、T3 0.96、T4 0.90。
...
Method: 1. 感知循环推理(PLR)范式：分循环描述视频片段并分析，决定下一步动作；2. 事实感知评估器(FAE)：作为反幻觉奖励评估每个感知结果；3. 使用AnetHallu-117K数据集训练FAE。

Result: Video-PLR在3B和7B参数规模上达到最先进性能，具有最佳的数据效率，FAE性能与GPT-4o相当。

Conclusion: 提出的循环感知推理范式和反幻觉奖励机制有效解决了视频推理中的感知不足和幻觉问题，显著提升了模型性能和数据效率。
...
### [191] [Gaze Beyond the Frame: Forecasting Egocentric 3D Visual Span](https://arxiv.org/abs/2511.18470)
*Heeseung Yun,Joonil Na,Jaeyeon Kim,Calvin Murdock,Gunhee Kim*

Main category: cs.CV

TL;DR: 本文提出了EgoSpanLift方法，将自我中心视角的视觉跨度预测从2D图像平面转换到3D场景，通过结合3D U-Net和单向transformer实现时空融合，在3D网格中预测未来视觉焦点。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心视角研究主要关注运动和接触交互，而预测人类视觉感知本身较少被探索，尽管这在指导人类行动和AR/VR、辅助技术中具有基础性作用。

Method: EgoSpanLift方法将SLAM关键点转换为注视兼容的几何形状，提取体积视觉跨度区域，并与3D U-Net和单向transformer结合进行时空融合预测。

Result: 该方法在3D视觉跨度预测基准测试中表现优于竞争基线，在自我中心2D注视预测和3D定位方面均有优势，即使投影回2D图像平面也取得可比结果。

Conclusion: EgoSpanLift成功实现了从2D到3D的视觉跨度预测转换，为自我中心视角的视觉感知预测提供了有效解决方案。
...
TL;DR: 该论文提出了一种名为"视频内指令"的新范式，通过将用户指导直接嵌入到视觉域中（如叠加文本、箭头或轨迹），实现可控的图像到视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本提示的控制方法存在全局性和粗糙性的局限，无法为不同对象分配明确的指令。该研究旨在探索视频生成模型能否解释和执行视觉嵌入的指令。

Method: 在视频帧中嵌入视觉信号作为指令，包括叠加文本、箭头或轨迹等元素，为不同对象分配不同的视觉指令，实现空间感知和明确的对应关系。

Result: 在三个最先进的生成器（Veo 3.1、Kling 2.5和Wan 2.2）上的大量实验表明，视频模型能够可靠地解释和执行这种视觉嵌入指令，特别是在复杂的多对象场景中。

Conclusion: 视频生成模型具备解释和执行视觉嵌入指令的能力，"视频内指令"范式为可控图像到视频生成提供了一种有效的方法。
...
TL;DR: 提出了一种自适应后验扩散采样方法(AdaPS)，通过基于两种不同似然梯度近似之间一致性的观测依赖权重方案，自适应调整扩散过程中的似然步长，在无需超参数调优的情况下提升逆问题重建质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型作为生成先验在解决逆问题时面临关键挑战：如何在先验贡献和数据保真度之间取得平衡。过于激进的似然更新可能引入伪影，而保守更新则会减慢收敛或产生次优重建结果。

Method: 开发了基于两种不可处理中间似然梯度近似之间一致性的观测依赖权重方案，该方案能自然适应扩散调度、时间重采样和注入的随机性，形成自适应后验扩散采样(AdaPS)。

Result: 在CelebA-HQ和ImageNet-256验证集上的超分辨率、高斯去模糊和运动去模糊等成像任务中，AdaPS在感知质量方面持续超越现有基于扩散的基线方法，且失真损失最小或无损失，无需任何任务特定调优。

Conclusion: AdaPS是一种无需超参数的方法，在各种扩散步数、观测噪声水平和不同随机性下表现出鲁棒性，显著提升了逆问题重建的质量。
...
### [194] [Uncertainty Quantification in HSI Reconstruction using Physics-Aware Diffusion Priors and Optics-Encoded Measurements](https://arxiv.org/abs/2511.18473)
*Juan Romero,Qiang Fu,Matteo Ravasi,Wolfgang Heidrich*

Main category: cs.CV

TL;DR: HSDiff是一个基于贝叶斯推理的高光谱图像重建框架，使用无条件训练的像素级扩散先验和后验扩散采样，通过增强的metameric增强技术提高先验多样性，提供不确定性感知的重建结果。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法由于高光谱图像数据集缺乏光谱多样性，在评估metamerism现象时容易产生幻觉，需要更好的不确定性校准方法。

Method: 将高光谱图像重建建模为贝叶斯推理问题，使用无条件训练的像素级扩散先验和后验扩散采样，提出基于区域的metameric黑和分区联合光谱上采样的增强metameric增强技术。

Result: HSDiff能够生成与各种高光谱成像模型测量一致的多样化HSI样本，提供校准的信息不确定性，并验证了有效光谱编码在快照高光谱成像中的重要性。

Conclusion: HSDiff提供了一个完整的高性能不确定性感知HSI重建方法，通过贝叶斯框架展示了所研究前向模型如何塑造后验分布，并强调了有效光谱编码的重要性。
...
### [195] [Chain-of-Visual-Thought: Teaching VLMs to See and Think Better with Continuous Visual Tokens](https://arxiv.org/abs/2511.19418)
*Yiming Qin,Bomin Wei,Jiaxin Ge,Konstantinos Kallidromitis,Stephanie Fu,Trevor Darrell,Xudong Wang*

Main category: cs.CV

TL;DR: COVT框架让视觉语言模型通过连续视觉令牌进行视觉推理，在约20个令牌的预算内捕获密集视觉信息，显著提升感知理解能力


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在语言空间推理表现出色，但在需要密集视觉感知的任务中表现不佳，如空间推理和几何意识，因为它们缺乏捕获空间维度密集视觉信息的机制

Method: 引入Chain-of-Visual-Thought框架，通过轻量级视觉专家蒸馏知识，捕获2D外观、3D几何、空间布局和边缘结构等互补属性，在训练中自回归预测视觉令牌来重建密集监督信号

Result: 在超过十个多样化感知基准测试中，将COVT集成到Qwen2.5-VL和LLaVA等强VLM中，性能持续提升3%到16%

Conclusion: 紧凑的连续视觉思维能够实现更精确、有依据和可解释的多模态智能
...
### [196] [Extreme Model Compression for Edge Vision-Language Models: Sparse Temporal Token Fusion and Adaptive Neural Compression](https://arxiv.org/abs/2511.18504)
*Md Tasnin Tanvir,Soumitra Das,Sk Md Abidar Rahaman,Ali Shiri Sichani*

Main category: cs.CV

TL;DR: 提出了两种自适应压缩技术STTF和ANC，使视觉语言模型能在资源受限的边缘设备上实时运行，在保持高精度的同时大幅减少计算量和参数数量。


<details>
  <summary>Details</summary>
Motivation: 边缘AI对视觉语言任务的需求要求模型能在资源受限的设备上实现实时性能，现有方法依赖静态剪枝或统一缩放，无法适应动态场景。

Method: STTF通过事件驱动变化检测动态重用视觉token，ANC通过学习的路由器条件激活编码器分支，实现细粒度的场景复杂度适应。

Result: 3B参数的TinyGPT-STTF在COCO 2017上CIDEr达131.2，超越LLaVA-1.5 7B 17.6分，参数量减少2.3倍，FLOPs减少62倍；STTF在事件视觉任务中减少84% token数，保持95.6%准确率；ANC在低运动场景中减少90% FLOPs。

Conclusion: 这些技术使得强大的视觉语言模型能够高效部署在真实世界的边缘设备上，准确率提升达4.4%，延迟降低达13倍。
...
### [197] [VDC-Agent: When Video Detailed Captioners Evolve Themselves via Agentic Self-Reflection](https://arxiv.org/abs/2511.19436)
*Qiang Wang,Xinyuan Gao,SongLin Dong,Jizhou Han,Jiangyang Li,Yuhang He,Yihong Gong*

Main category: cs.CV

TL;DR: VDC-Agent是一个自演化的视频详细描述框架，无需人工标注或大型教师模型，通过闭环生成-评分-优化过程自动构建训练数据，在VDC基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决视频详细描述任务中依赖人工标注和大型教师模型的问题，开发无需外部监督的自演化框架。

Method: 构建闭环系统：视频描述生成→基于原则的评分（分数和文本建议）→提示优化。当质量下降时，通过自反思路径修正更新。将轨迹转换为偏好元组，采用易到难的课程直接偏好优化进行微调。

Result: 在VDC基准测试中达到49.08%平均准确率和2.50分数，超越专用视频描述器，相比基础模型提升+5.13%准确率和+0.27分数，推理成本相近。

Conclusion: VDC-Agent证明了自演化框架在视频描述任务中的有效性，能够自动构建高质量训练数据并显著提升模型性能。
...
Method: 提出基于低秩分解的新型成像模型，开发LRDUN网络，在展开近端梯度下降框架中联合求解两个子问题，并引入广义特征展开机制

Result: 在模拟和真实数据集上的广泛实验表明，LRDUN在显著降低计算成本的同时实现了最先进的重建质量

Conclusion: 通过低秩分解和展开网络相结合的方法，有效解决了光谱压缩成像重建中的计算冗余和病态问题
...
TL;DR: 开发了一个集中式平台，用于检测太阳能电池板上的灰尘和故障，通过CNN、ResNet和KerNet等深度学习模型实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 太阳能电池板输出受多种因素影响（如灰尘、温度、故障等），需要有效的检测系统来维护太阳能系统的效率和可靠性。

Method: 使用图像预处理（伽马去除、高斯滤波、归一化），结合CNN、ResNet和KerNet模型进行分类，检测灰尘和故障。

Result: 模型在效率和准确性方面优于现有模型，能够有效检测灰尘和各类故障。

Conclusion: 该多应用模型在检测太阳能电池板灰尘和故障方面表现出高效性和优化性，适用于从家庭到大型太阳能农场的各种规模需求。
...
TL;DR: 提出了一种训练免费的FSCIL方法CD-FSCIL，用基于扩散的生成转换替代传统梯度优化，有效缓解灾难性遗忘并大幅降低计算开销


<details>
  <summary>Details</summary>
Motivation: 传统FSCIL方法依赖梯度优化，随着新类别增加导致训练成本爆炸性增长，且在少样本条件下不仅加剧基础类遗忘，还阻碍新类适应

Method: 基于条件扩散过程与梯度优化的关联性，提出CD-FSCIL框架，用扩散生成转换替代梯度更新；结合LLM自动生成文本描述的多模态学习策略增强少样本表示

Result: 在主流FSCIL基准测试中达到最先进性能，同时显著降低计算和内存开销

Conclusion: 该方法标志着向训练免费持续适应的范式转变，为FSCIL提供了新的解决方案
...
TL;DR: 提出DE-KAN模型，通过双编码器和KAN瓶颈层增强牙齿全景X光片分割性能，在多个指标上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在牙齿分割任务中因解剖变异、不规则形状和结构重叠等问题性能受限

Method: 使用ResNet-18编码器处理增强输入，定制CNN编码器处理原始输入，通过KAN瓶颈层融合全局和局部特征

Result: 在两个基准数据集上取得mIoU 94.5%、Dice系数97.1%、准确率98.91%、召回率97.36%的优异表现

Conclusion: DE-KAN模型显著提升了牙齿分割性能，Dice系数相比现有方法提升高达4.7%
...
TL;DR: HiFi-MambaV2是一种用于MRI重建的分层共享路由MoE Mamba架构，通过频率分解和内容自适应计算，在多个数据集上优于CNN、Transformer和现有Mamba基线。
...
Method: 采用可分频率一致性拉普拉斯金字塔(SF-Lap)提供抗混叠的稳定低/高频流，以及分层共享路由MoE进行逐像素稀疏调度，结合轻量级全局上下文路径和数据一致性正则化主干。

Result: 在fastMRI、CC359、ACDC、M4Raw和Prostate158数据集上，在PSNR、SSIM和NMSE指标上一致优于CNN、Transformer和现有Mamba基线，在高频细节和结构保真度方面持续改进。

Conclusion: HiFi-MambaV2实现了可靠且稳健的MRI重建。
...
Method: 将输入视频反转到扩散模型潜在空间，通过负提示干预重建过程远离雨的概念，核心是注意力切换机制保持动态背景和结构一致性。

Result: 在真实世界雨数据集上广泛实验，相比先前方法有显著改进，展示出无需监督训练的强泛化能力。
...
TL;DR: 提出了C3数据集，用于解决地面照片与平面图之间的跨模态对应关系预测问题，改进了现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有几何模型在处理不同视角（如航拍与地面）或不同模态（如照片与抽象绘图）的输入时表现不佳，特别是在地面照片与平面图的对应关系预测上存在挑战。

Method: 通过从互联网照片集合中重建3D场景，然后手动将重建结果与从互联网收集的平面图进行配准，从而推导出图像与平面图之间的对应关系。

Result: C3数据集包含597个场景中的9万对平面图和照片，1.53亿像素级对应关系和8.5万个相机位姿。在训练新数据后，最佳方法的RMSE提高了34%。
...
TL;DR: PhysGS是一个基于贝叶斯推断的3D高斯泼溅扩展方法，能够从视觉线索和视觉-语言先验中估计密集的每点物理属性，如质量、硬度和摩擦系数。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法主要关注几何和外观，无法推断基础的物理属性，而理解这些属性对于机器人安全有效地与环境交互至关重要。

Method: 将属性估计建模为高斯泼溅上的贝叶斯推断，迭代优化材料和属性信念，并建模偶然性和认知不确定性。

Result: 在物体尺度、室内和室外真实数据集上，PhysGS将质量估计准确率提升达22.8%，肖氏硬度误差降低达61.2%，动摩擦误差降低达18.1%。
...
TL;DR: 提出了一种基于视觉自回归建模和视觉语言模型感知先验的生成框架，用于解决真实世界暗光图像的低可见度、对比度、噪声和模糊等复杂退化问题。
...
Method: 采用视觉自回归建模框架，结合VLM感知先验；部署自适应曲线估计方案调节光照；集成动态和空间频率感知的旋转位置编码；提出递归相位域调制策略减轻模糊伪影。

Result: 该框架完全无监督，在基准数据集上实现了最先进的性能。

Conclusion: 提出的生成框架有效解决了暗光图像恢复中的复杂退化问题，无需配对数据即可实现高质量的图像恢复。
...
TL;DR: NeAR是一个耦合的神经资产-渲染器堆栈，将神经资产创作和神经渲染联合设计，实现端到端可学习的图形流水线，在保真度、一致性和效率方面都有优势。


<details>
  <summary>Details</summary>
Motivation: 目前神经资产创作和神经渲染是分离的领域，联合设计资产表示和渲染器可以解锁端到端可学习的图形堆栈，带来保真度、一致性和效率的提升。

Method: 在资产侧使用Trellis风格的结构化3D潜在空间和光照均匀化神经资产；在渲染器侧设计光照感知神经渲染器，结合显式视图嵌入和HDR环境贴图实现实时可重光照渲染。

Result: 在四个任务上验证：G-buffer前向渲染、随机光照单图像重建、未知光照单图像重光照、新视角重光照，在定量指标和感知质量上都超越现有最佳基线。
...
TL;DR: RigAnyFace (RAF) 是一个可扩展的神经自动绑定框架，能够为各种拓扑结构的面部网格（包括多个断开组件）生成表情混合形状绑定。


<details>
  <summary>Details</summary>
Motivation: 传统手动绑定成本高昂且难以扩展到多样化拓扑结构的面部网格，特别是包含多个断开组件（如眼球）的情况。

Method: 使用三角化无关的表面学习网络，结合定制的架构设计来条件化FACS参数，并有效处理断开组件。采用2D监督策略扩展训练数据，增强模型泛化能力。

Result: RAF能够为各种拓扑结构的面部网格生成准确的绑定，包括艺术家制作的资产和野外样本，在准确性和泛化性方面优于先前工作。

Conclusion: RAF是一个高效的面部自动绑定框架，支持多种拓扑结构和断开组件，显著提升了面部动画制作的效率和效果。
...
TL;DR: 本研究系统评估了Vision Transformer在多种增强策略下对眼底图像视网膜疾病的检测性能，在多个数据集上表现稳定，其中几何和颜色增强效果最佳。同时开发了GANomaly异常检测器提供可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决眼底图像检测中成像质量差异、早期病变细微表现以及数据集间域偏移等挑战，提高视网膜疾病检测的可靠性。

Method: 使用Vision Transformer分类器，结合多种数据增强和图像增强策略，在多个异构公共数据集和自建AEyeDB数据集上进行评估。同时开发GANomaly异常检测器进行补充检测。

Result: ViT在不同数据集和疾病上准确率0.789-0.843，几何增强在Papila数据集上AUC达0.91，优于传统卷积集成方法。GANomaly异常检测器AUC为0.76，提供重建可解释性。

Conclusion: Vision Transformer结合适当增强策略能有效检测视网膜疾病，其中糖尿病视网膜病变和年龄相关性黄斑变性检测可靠，青光眼检测最具挑战性。几何和颜色增强效果稳定，拉普拉斯增强效果不佳。
...
TL;DR: 提出了TF框架，通过两阶段方法合成3D脑肿瘤数据，解决医学图像标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 医学影像中标注的肿瘤数据稀缺阻碍了准确的自动肿瘤分割，现有方法要么需要大量人工建模，要么需要大量训练数据，这在临床数据有限的环境中不实用。

Method: TF框架包含粗粒度肿瘤合成和生成模型驱动的精炼过程，仅使用健康图像扫描和少量真实标注数据来合成大量配对合成数据。

Result: 实验表明，使用合成的图像-标签对作为数据增强可以显著提高低数据情况下的肿瘤分割性能。
...
TL;DR: 提出一种基于超像素的正则化方法，通过SLIC算法在对抗补丁优化过程中动态聚类像素，使补丁在尺度变化时保持结构稳定性，减少插值损失。


<details>
  <summary>Details</summary>
Motivation: 物理对抗攻击中的尺度变异性问题：当补丁被缩放时，插值引起的颜色混合会平滑像素值，导致高频模式丢失和对抗信号退化。

Method: 使用SLIC算法在对抗补丁优化过程中动态聚类像素，通过隐函数定理反向传播梯度来更新超像素边界和颜色，生成具有尺度弹性的结构。

Result: 在数字域和物理实现中都取得了更好的性能，物理性能通过使用屏幕和纸板剪影的系统评估协议得到客观验证。

Conclusion: 提出的超像素正则化方法能有效提高对抗补丁对尺度变化的鲁棒性，在数字和物理场景中均保持性能优势。
...
TL;DR: 提出基于生成式AI的数据增强管道，通过几何透视变换、AI修复和车身叠加来模拟特定视角，提升车道检测模型在侧置摄像头场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决车道检测模型在公共数据集上训练后无法适应不同摄像头视角（特别是侧置摄像头）的领域偏移问题。

Method: 结合几何透视变换、AI驱动的图像修复和车辆车身叠加，生成部署特定视角的训练数据，保持车道连续性。

Result: 在SCNN和UFLDv2模型上测试，增强数据训练后模型在不同条件下（包括阴影）的鲁棒性提升，精确率、召回率和F1分数均有提高。

Conclusion: 该方法为弥合公开数据集与部署特定场景之间的差距提供了可扩展的实用框架，提高了车道检测在试点部署中的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在新型视图合成中计算成本过高的问题，同时克服回归方法生成质量不足的缺陷，实现高质量与高效推理的平衡。

Method: 使用回归式快速初始化来引导和减少扩散模型的降噪工作量，集成选择性细化和自适应噪声调度，将更多计算资源分配给不确定区域和帧。

Result: 平均实现1.8倍加速，感知质量退化小于5%，在质量与延迟之间建立了新的帕累托边界。

Conclusion: Sphinx框架成功实现了扩散级质量的新型视图合成，同时显著降低了计算成本，为动态变化的推理场景提供了灵活的性能-质量权衡方案。
...
Method: 基于FLUX.1 Kontext架构，采用全参数微调和像素空间一致性损失来保持结构保留的细化，并在中间去噪状态间实施一致性，同时使用单步确定性推理加速运行。

Result: 在深度、法线和抠图三个任务上均取得了全面的最先进结果，相比传统方法运行速度提升高达8倍。
...
TL;DR: KTCAA是一个基于泛化理论的少样本跨模态框架，通过对齐增强和知识转移催化剂来解决素描-图像跨模态匹配问题，在数据稀缺条件下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 素描-图像跨模态重识别面临显著的模态差异和标注数据有限的问题，需要开发能够有效处理跨模态泛化的方法。

Method: 提出对齐增强(AA)模块进行局部素描风格变换来模拟目标分布，以及知识转移催化剂(KTC)模块引入最坏情况扰动来增强不变性，在元学习框架下联合优化。

Result: 在多个基准测试中，KTCAA实现了最先进的性能，特别是在数据稀缺条件下表现优异。

Conclusion: 基于泛化理论的分析指导了有效的跨模态学习策略，KTCAA框架成功地将RGB域的丰富知识转移到素描场景，显著提升了少样本跨模态重识别的性能。
...
TL;DR: 提出了一种基于神经几何图像的网格表示方法，通过将不规则网格转换为规则图像网格，实现高效的单次前向传播重建，无需复杂解码器，具有优异的存储效率和重建精度。


<details>
  <summary>Details</summary>
Motivation: 现有3D网格神经表示方法依赖神经过拟合和复杂解码器，计算成本高。而图像具有规则结构便于高效处理，但难以直接应用于不规则网格。

Method: 使用几何图像表示将不规则网格转换为规则图像网格，利用最优传输(OT)解决平坦区域过采样和特征区域欠采样问题，支持连续细节层次(LoD)的几何图像mipmapping。

Result: 实验结果显示在压缩比(CR)、Chamfer距离(CD)和Hausdorff距离(HD)指标上达到最先进的存储效率和重建精度。

Conclusion: 该方法实现了无解码器、存储高效的网格表示，通过单次前向传播即可重建高质量网格，为3D网格处理提供了高效的神经表示方案。
...
TL;DR: 提出了一种基于微分同胚和GraphCut的相位展开框架，通过像素标记问题重新表述相位展开，实现了45.5倍加速和更低的L2误差，具有实时应用潜力。
...
Method: 将基于GraphCut的相位展开重新表述为像素标记问题，利用微分同胚的不变性特性，通过共形和最优传输映射在图像空间中应用。预计算奇数个微分同胚，在每个域中应用分层GraphCut算法，通过多数投票融合标签图来鲁棒估计k值。

Result: 实验结果显示45.5倍的加速和更低的L2误差，在真实实验和模拟中都表现出色。

Conclusion: 该框架在保持高精度的同时显著提升了计算速度，展示了在实时应用中的潜力，为3D扫描技术提供了有效的相位展开解决方案。
...
TL;DR: ICE是一种无需训练、模态无关的一次性权重修改方法，用于文本到图像和文本到视频模型的概念擦除，通过各向异性能量加权缩放定义擦除和保留子空间，并使用独特闭式重叠投影器实现精确持久遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法存在需要重新训练、推理开销大或易受对抗攻击的问题，且很少建模目标擦除概念与周围内容的潜在语义重叠，导致擦除后产生附带损害，同时很少有方法能在T2I和T2V领域都可靠工作。

Method: ICE定义擦除和保留子空间，使用各向异性能量加权缩放，通过闭式重叠投影器显式正则化其交集，提出凸且Lipschitz有界的谱遗忘目标，平衡擦除保真度和交集保留，获得稳定唯一的解析解。

Result: 在艺术风格、对象、身份和显式内容的目标移除中，ICE有效实现强擦除，提高对抗红队测试的鲁棒性，同时在T2I和T2V模型中仅对原始生成能力造成最小退化。

Conclusion: ICE是一种训练免费、模态无关、一次性权重修改方法，能够实现精确持久的概念擦除，且无运行时开销，在多个擦除场景中表现优异。
...
TL;DR: CFG-Bench是一个新的基准测试，旨在系统评估多模态大语言模型在物理交互中的细粒度动作智能，包含1,368个视频和19,562个多模态问答对，覆盖四个认知维度。
...
Method: 构建包含物理交互、时间因果关系、意图理解和评估判断四个维度的系统评估框架，通过监督微调验证性能提升。

Result: 主流MLLMs在生成详细物理交互指令方面表现不佳，在意图和评估推理方面存在显著局限，但通过SFT训练能显著提升在现有基准上的性能。

Conclusion: 该研究揭示了MLLMs在物理交互推理方面的局限性，为开发更强大的具身智能体提供了重要见解。
...
TL;DR: EVCC是一种结合Vision Transformer、轻量级ConvNeXt和CoAtNet的多分支混合视觉架构，通过自适应token剪枝、门控双向交叉注意力等创新技术，在保持高精度的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer-CNN混合架构虽然提升了图像分类性能，但通常伴随着高昂的计算成本。EVCC旨在通过更高效的架构设计来解决准确性与效率之间的权衡问题。

Method: EVCC采用多分支架构，集成了Vision Transformer、轻量级ConvNeXt和CoAtNet，关键技术包括：自适应token剪枝与信息保留、门控双向交叉注意力、辅助分类头的多任务学习、以及基于上下文感知的动态路由器门控。
...
Conclusion: EVCC通过动态调整计算需求，有效平衡了准确性与效率的权衡，结合了全局上下文、局部细节和层次特征，适用于实际应用场景。
...
TL;DR: 本文研究了在环视鱼眼相机系统中实现端到端3D目标检测的技术可行性，提出了两种鱼眼兼容的3D检测方法，并发布了新的鱼眼3D检测数据集Fisheye3DOD。


<details>
  <summary>Details</summary>
Motivation: 传统基于针孔相机模型的3D目标检测器在鱼眼图像上性能下降明显，需要开发专门针对鱼眼几何特性的检测方法。

Method: 提出了两种方法：基于鸟瞰图范式的FisheyeBEVDet和基于查询范式的FisheyePETR，都采用球面空间表示来有效捕捉鱼眼几何特性。

Result: 在Fisheye3DOD数据集上的实验表明，鱼眼兼容建模相比基线方法准确率提升最高达6.2%。

Conclusion: 通过专门的几何建模，可以有效提升鱼眼相机系统中的3D目标检测性能。
...
TL;DR: 提出一种抗噪声神经元卷积(DDC)，模仿神经元树突结构，通过非线性交互重构特征提取数学范式，在噪声环境下显著优于传统卷积


<details>
  <summary>Details</summary>
Motivation: 现实图像识别场景中存在大量噪声干扰，现有方法主要从网络结构或训练策略角度解决噪声问题，抗噪声性能已达瓶颈。需要从神经元角度探索抗干扰解决方案
...
TL;DR: CoD是首个专为压缩设计的扩散基础模型，相比基于文本条件的扩散模型，在超低码率下显著提升压缩效率，训练成本降低300倍，为扩散编解码器研究提供新基础。


<details>
  <summary>Details</summary>
Motivation: 现有扩散编解码器基于文本到图像的扩散基础模型（如Stable Diffusion），但文本条件在压缩角度并非最优，限制了扩散编解码器在超低码率下的潜力。

Method: 从头训练CoD压缩导向扩散基础模型，实现压缩和生成的端到端优化，使用纯图像数据集训练，训练效率比Stable Diffusion高300倍。

Result: 在DiffC等下游编解码器中用CoD替换Stable Diffusion，在超低码率（如0.0039 bpp）下达到SOTA结果；像素空间扩散可实现VTM级PSNR和高感知质量；用更少参数超越GAN基编解码器。

Conclusion: CoD为未来扩散编解码器研究奠定基础，展示了压缩导向扩散模型的巨大潜力，特别是在超低码率场景下。
...
TL;DR: DriveFlow是一个基于预训练文本到图像流模型的训练数据增强方法，通过频率分解策略解决自动驾驶中3D物体检测的OOD问题，保持3D几何精度并提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中视觉中心的3D物体检测面临标注成本高和室外场景多样的问题，导致训练数据无法覆盖所有测试场景（OOD问题）。基于预训练扩散模型的免训练图像编辑方法虽然能增强训练数据，但现有方法在保持3D几何精度方面存在不足。

Method: 基于频率分解，DriveFlow提出两种策略：1）高频前景保护：通过高频对齐损失保持前景物体的精确3D几何；2）双频背景优化：在背景上进行双频优化，平衡编辑灵活性和语义一致性。

Result: 综合实验验证了DriveFlow的有效性和效率，在OOD场景下所有类别都表现出全面的性能提升。

Conclusion: DriveFlow通过频率分解策略成功解决了基于流模型的训练数据增强中3D几何保持的问题，为自动驾驶中的OOD问题提供了有效解决方案。
...
TL;DR: ViPO是一种改进的GRPO方法，将标量奖励提升为结构化、像素级的优势图，通过感知结构化模块优化视觉生成模型，在图像和视频任务中表现优于标准GRPO。


<details>
  <summary>Details</summary>
Motivation: 现有的GRPO方法仅使用每个样本的单一标量奖励，忽略了视觉内容的丰富空间和时间结构，这种粗粒度的监督阻碍了对局部伪影的修正和细粒度感知线索的建模。

Method: ViPO采用感知结构化模块，利用预训练视觉骨干网络构建空间和时间感知的优势图，将优化压力重新分配到感知重要区域，同时保持标准GRPO的稳定性。

Result: 在图像和视频基准测试中，ViPO持续优于标准GRPO，在领域内与人类偏好奖励的对齐度提高，并在领域外评估中增强了泛化能力。

Conclusion: ViPO是一种架构无关、轻量级的方法，完全兼容现有的GRPO训练流程，为视觉生成提供了更具表达力和信息量的学习信号。
...
TL;DR: GuideFlow是一个基于约束流匹配的自动驾驶规划框架，解决了现有端到端规划器的多模态轨迹崩溃问题，并直接在生成过程中强制执行约束，无需额外优化阶段。


<details>
  <summary>Details</summary>
Motivation: 现有模仿式端到端规划器存在多模态轨迹崩溃问题，无法产生多样化的轨迹提议；而生成式端到端规划器难以在生成过程中直接融入安全性和物理约束，需要额外优化阶段。

Method: 采用约束流匹配方法，显式建模流匹配过程，直接强制执行显式约束而非隐式编码。结合能量基模型训练以增强自主优化能力，满足物理约束。参数化驾驶攻击性作为控制信号来精确操控轨迹风格。

Result: 在多个主要驾驶基准测试（Bench2Drive、NuScenes、NavSim和ADV-NuScenes）上验证了有效性。在NavSim测试困难分割上达到SOTA，EPDMS得分为43.0。

Conclusion: GuideFlow通过约束流匹配有效解决了端到端自动驾驶规划中的多模态轨迹崩溃和约束集成问题，实现了多样化的轨迹生成和精确的约束满足。
...
<details>
  <summary>Details</summary>
Motivation: 传统协同自动驾驶方法需要共享大型特征图或查询嵌入，导致通信带宽需求过高，且难以在不同感知模型的车辆间实现互操作。

Method: 车辆交换紧凑的参考点（如物体位置、速度、尺寸信息），并采用选择性Top-K查询融合来增强信息丰富度，实现传感器和模型无关的接口。

Result: 在M3CAD数据集上的实验表明，通信开销降低了五个数量级（从数百MB/s降至几KB/s），同时保持稳定的感知性能，并展现出强大的鲁棒性和一致的传输行为。

Conclusion: RefPtsFusion为实现可扩展的实时协同驾驶系统提供了有前景的解决方案，在通信效率和感知性能之间实现了良好平衡。
...
TL;DR: 提出VAOT框架，通过最优传输目标和结构保持正则化器来增强彩色眼底图像，同时保持血管结构完整性。


<details>
  <summary>Details</summary>
Motivation: 传统GAN方法在增强眼底图像时可能扭曲临床关键的血管结构，改变血管拓扑和端点完整性。

Method: 结合最优传输目标和两个结构保持正则化器：基于骨架的损失保持全局血管连通性，端点感知损失稳定局部端点。

Result: 在合成退化基准测试和血管、病变分割的下游评估中，该方法优于多个最先进基线。

Conclusion: VAOT框架在减少噪声的同时有效保持血管结构，在眼底图像增强方面表现优越。
...
Method: 构建3D服装视频数据集进行跨姿态纹理学习，使用Nano Banana进行非等距图像编辑，提出基于不确定性引导的迭代烘焙方法融合多视角预测。

Result: 前馈双分支架构生成多样化且空间对齐的PBR材质，适用于工业级3D服装设计。

Conclusion: 该方法解决了非等距图像到服装纹理生成的挑战，提供了生产就绪的PBR纹理解决方案。
...
Method: 1. 运动感知VAE重建：分段重建具有统一运动特征的视频片段；2. 锚帧引导：利用未受时间压缩影响的首帧潜在特征来约束生成过程，提升结构保真度。

Result: 大量实验表明，STCDiT在结构保真度和时间一致性方面优于现有最先进方法。

Conclusion: 结合运动感知重建和锚帧引导的视频扩散模型能够实现高质量的视频超分辨率，有效处理复杂相机运动场景。
...
TL;DR: 本文系统研究了视觉语言模型在感知任务间的迁移性，提出了Perfection Gap Factor (PGF)指标来量化迁移效果，并构建了任务转移图揭示任务间关系。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态基准测试中表现良好，但在深度估计、物体计数等视觉感知任务上落后于人类和专门模型。针对一个任务的微调可能对其他任务产生不可预测的影响，这使得任务特定微调具有挑战性。

Method: 研究了微调VLM在一个感知任务上如何影响其在其他任务上的零样本性能，引入了PGF指标来捕捉迁移的广度和幅度，使用三个开源VLM在13个感知任务上进行评估，构建任务转移图。

Result: 发现了正向和负向迁移模式，识别了相互影响的任务组，根据迁移行为将任务组织为不同角色，并展示了PGF如何指导更高效的训练数据选择。

Conclusion: 研究结果既揭示了正向迁移的机会，也指出了负向干扰的风险，为推进VLM发展提供了可操作的指导。
...
TL;DR: StereoDETR是一个基于DETR的高效立体3D目标检测框架，通过单目DETR分支和立体分支结合，实现了实时推理并超越了单目方法的速度，在KITTI基准测试中取得了竞争性的精度。


<details>
  <summary>Details</summary>
Motivation: 立体3D目标检测相比单目方法精度更高，但存在计算开销大和延迟高的问题。现有最优立体方法精度是单目方法的两倍，但推理速度只有单目方法的一半。
...
Result: StereoDETR实现了实时推理，是首个在速度上超越单目方法的立体方法。在KITTI基准测试中取得竞争性精度，在行人和自行车子集上创造了新的最优结果。
...
TL;DR: 该论文通过大规模MAE预训练方法解决Wi-Fi感知的领域泛化问题，使用14个数据集、130万样本进行实验，发现数据规模和多样性是提升跨域性能的关键，而模型容量在当前数据量下增益有限。


<details>
  <summary>Details</summary>
Motivation: Wi-Fi感知虽然提供隐私保护替代方案，但面临严重的领域漂移问题，在不同环境、硬件和用户间泛化能力差，现有数据集规模小且碎片化。

Method: 采用基础模型方法，使用掩码自编码器(MAE)风格预训练，在14个Wi-Fi CSI数据集上训练，涵盖4种设备、2.4/5/6 GHz频段和20-160 MHz带宽。

Result: 大规模预训练在人类活动识别、手势识别和用户识别任务上，相比监督学习基线提升跨域准确率2.2%至15.7%，显示数据规模与多样性对领域泛化的关键作用。

Conclusion: 数据而非模型容量是当前Wi-Fi感知泛化的瓶颈，为设计实际可部署的鲁棒Wi-Fi感知系统提供了重要方向。
...
TL;DR: PartDiffuser是一个半自回归扩散框架，通过部件级生成平衡全局结构和局部细节，解决了现有自回归方法在生成艺术家设计网格时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有自回归方法在生成艺术家设计网格时难以平衡全局结构一致性和高保真局部细节，且容易产生误差累积问题。
...
Conclusion: PartDiffuser通过有效解耦全局和局部生成任务，成功解决了网格生成中全局结构与局部细节的平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF的隐式3D重建方法往往忽视了物体解剖先验的重要性，在超稀疏视图场景下限制了重建精度和学习效率。

Method: 采用位置和结构编码进行体素级隐式重建，利用目标先验指导体素采样并丰富结构编码，同时开发了基于CUDA的算法从稀疏视图投影中快速估计高质量3D目标先验。

Result: 在复杂腹部数据集上的实验表明，该模型学习效率比当前领先模型NAF提高了10倍，重建质量优于最准确模型NeRP，在10、20、30个投影下PSNR分别提升了3.57dB、5.42dB和5.70dB。

Conclusion: 提出的目标先验引导隐式神经表示方法在超稀疏视图CT重建中显著提升了学习效率和重建质量，证明了解剖先验在隐式学习中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D物体检测方法要么逐帧预测缺乏时间一致性建模，要么采用复杂多阶段管道容易产生误差传播，且缺乏大规模连续可靠3D边界框标注数据集。

Method: 基于DA4D数据集，融合预训练基础模型的多模态特征，设计几何感知的时空解码器捕捉时空动态，采用多任务学习架构和专用训练策略保持序列全局一致性。

Result: 实验表明DetAny4D达到竞争性检测精度，显著提升时间稳定性，有效解决了4D物体检测中长期存在的抖动和不一致问题。

Conclusion: DetAny4D框架在4D物体检测中实现了准确且时间一致的结果，为可靠感知和理解真实世界提供了有效解决方案。
...
TL;DR: SupLID是一个新颖的语义分割OOD检测框架，通过利用语义空间的几何结构和线性内在维度(LID)来指导分类器生成的OOD分数，在像素级实现高效且空间平滑的异常检测。
...
Method: SupLID构建几何核心集捕捉ID子空间的内在结构，在超像素级别计算OOD分数，结合LID分析高维数据的局部结构，作为后处理评分方法可与任何语义分割分类器无缝集成。

Result: SupLID显著提升了现有基于分类器的OOD分数性能，在AUR、FPR和AUP等关键评估指标上达到了最先进的性能水平。

Conclusion: SupLID通过几何线索有效补充传统分类器置信度，实现了高效实时的像素级OOD检测，为自动驾驶等实际应用提供了更可靠的异常区域定位能力。
...
TL;DR: 提出了一个全自动管道，将原始3D扫描转换为高质量对话数据，解决了3D MLLMs数据稀缺问题，生成包含200万样本的Disc3D数据集。


<details>
  <summary>Details</summary>
Motivation: 3D多模态大语言模型落后于2D同行，主要因为缺乏大规模高质量的3D场景对话数据集，且现有方法存在视角模糊和对象指代模糊问题。

Method: 开发了四阶段自动化管道：元注释收集、场景图构建与关系校正、区分性对象指代、多任务数据生成，结合规则约束与2D MLLMs/LLMs。

Result: 生成了Disc3D数据集，包含25K混合3D场景中的200万样本，涵盖多种任务，实验显示在公共基准和Disc3D-QA任务上均有显著提升。

Conclusion: 该自动化管道能高效生成高质量3D对话数据，显著提升3D MLLMs性能，解决了数据稀缺和模糊性问题。
...
TL;DR: DiP是一个高效的像素空间扩散框架，通过全局-局部解耦设计解决扩散模型在生成质量和计算效率之间的权衡问题，在保持与潜在扩散模型相当计算效率的同时实现高质量图像生成。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在生成质量与计算效率之间的根本权衡问题。潜在扩散模型虽然高效但存在信息丢失和非端到端训练问题，而现有像素空间模型计算成本过高。

Method: 提出DiP框架，将生成过程解耦为全局和局部两个阶段：使用扩散变换器（DiT）在大块上构建全局结构，同时通过轻量级补丁细节头恢复局部细节。

Result: DiP实现了与潜在扩散模型相当的计算效率，推理速度比先前方法快10倍，仅增加0.3%的参数，在ImageNet 256×256上获得1.90 FID分数。

Conclusion: DiP通过全局-局部协同设计有效解决了扩散模型的质量-效率权衡问题，在不依赖VAE的情况下实现了高效的像素空间图像生成。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视频多模态大语言模型在理解短片段中的瞬时动作和长视频中的罕见瞬态事件方面的局限性，提升对细粒度运动线索的感知能力。

Method: 采用两阶段训练框架：1）监督微调阶段构建'关键信息缺失'视频，通过对比损失对齐中间视觉表示与关键词；2）强化学习阶段使用相对奖励机制，确保完整视频的响应优于降级输入，显式训练模型恢复时间精确的动作细节。
...
Method: 采用SlowFast框架区分快慢帧处理，使用多阶段训练策略：监督微调(SFT)、分组相对策略优化(GRPO)和再次SFT，结合思维链(COT)格式数据。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法对所有教师预测一视同仁，忽略了教师预测的不确定性，这可能导致学生从不可靠的教师预测中学习错误知识。

Method: 采用不确定性感知机制和双学生架构（ResNet-18和MobileNetV2），通过同伴学习机制让学生既从教师网络学习，又相互学习。

Result: 在ImageNet-100上，ResNet-18达到83.84% top-1准确率，MobileNetV2达到81.46% top-1准确率，相比传统单学生蒸馏方法分别提升2.04%和0.92%。

Conclusion: 不确定性感知的双学生知识蒸馏框架能有效利用教师预测的不确定性信息，通过异构学生协作学习显著提升模型压缩性能。
...
Method: 构建了一个综合模型，将深度学习架构与群体智能优化策略相结合。使用多模态可穿戴传感器数据集分析生理、情绪和行为信号。采用遗传算法和粒子群优化等群体智能技术来优化特征空间和超参数，深度学习组件负责从序列化多源输入中提取分层判别性表征。

Result: 评估表明，这两种计算范式的融合显著提升了检测性能，相比单独使用深度网络。混合模型在准确性方面取得了显著改进，并在不同个体间表现出更强的泛化能力。
...
TL;DR: VideoCompressa是一个视频数据合成框架，通过动态潜在压缩识别关键帧并压缩为紧凑的语义潜在代码，实现前所未有的数据效率。
...
Method: 联合优化可微分关键帧选择器（轻量级ConvNet + Gumbel-Softmax采样）和预训练冻结的变分自编码器（VAE），将关键帧压缩为语义丰富的潜在代码，并通过压缩网络实现端到端反向传播。

Result: 在UCF101上仅使用0.13%原始数据就超越全数据训练2.34个百分点，速度提升5800倍；在HMDB51上仅用0.41%训练数据即可匹配全数据性能，比零样本基线提升10.61%。

Conclusion: VideoCompressa通过识别和压缩视频中的关键帧，有效解决了视频数据冗余问题，实现了极高的数据效率和计算效率。
...
TL;DR: FVAR提出了一种新的视觉自回归模型范式，将传统的下一尺度预测转变为下一焦点预测，通过渐进式去模糊而非简单下采样来消除混叠伪影，提升图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统视觉自回归模型使用均匀尺度下采样构建多尺度金字塔，导致混叠伪影，损害细节并引入锯齿和摩尔纹。需要一种能够消除这些伪影的方法。

Method: 1) 下一焦点预测范式：渐进减少模糊而非简单下采样；2) 渐进重聚焦金字塔构建：使用物理一致的散焦核构建无混叠多尺度表示；3) 高频残差学习：通过专门的残差教师网络在训练中有效整合混叠信息。

Result: 在ImageNet上的实验表明，FVAR显著减少了混叠伪影，改善了细节保留和文本可读性，在保持与现有VAR框架完美兼容的同时实现了优越性能。

Conclusion: FVAR通过重新定义自回归范式为焦点预测，有效解决了传统方法中的混叠问题，为高质量图像生成提供了新思路。
...
TL;DR: 该研究通过深度集成方法成功解决了CheXNet等深度学习模型在临床应用中缺乏可靠置信度的问题，在NIH ChestX-ray14数据集上实现了优异的性能校准和不确定性分解。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在临床高风险环境中的实用性受限于其确定性本质，无法提供可靠的预测置信度度量，这限制了其在临床决策支持中的应用。

Method: 首先尝试使用蒙特卡洛Dropout方法但性能不稳定，随后转向构建9个成员的高多样性深度集成模型，成功实现了性能稳定和校准优化。

Result: 深度集成模型实现了SOTA性能：平均AUROC 0.8559，平均F1分数0.3857，平均ECE 0.0728，NLL 0.1916，并能可靠地将总不确定性分解为偶然不确定性和认知不确定性。

Conclusion: 深度集成方法将模型从概率工具转变为可靠的临床决策支持系统，提供了可信赖且可解释的平台。
...
<details>
  <summary>Details</summary>
Motivation: 在线测试时适应在3D人体姿态估计中面临误差累积的挑战，依赖不完美预测的自监督会导致性能随时间下降。

Method: 使用潜在运动表示空间的无监督聚类获得锚点运动，利用其规律性监督姿态估计器并实现高效自回放；引入软重置机制，在连续适应过程中将姿态估计器回滚到其指数移动平均值。

Result: 在长时间在线适应实验中，该方法能够持续适应同一人的域外流式测试视频，捕捉一致的个人形状和运动特征，显著提升了准确性。

Conclusion: 通过缓解误差累积，该方法能够稳健地利用个人特征提升精度，在在线测试时适应方法中表现优于先前方法。
...
TL;DR: 本文提出了一种针对长尾数据集的蒸馏方法，通过统计对齐视角解决模型偏差和BN统计估计问题，显著提升了长尾数据集蒸馏的性能。
...
Method: 采用统计对齐视角，包含三个核心组件：增强专家模型用于可靠统计估计和软标签生成；通过动态调整动量的完整前向传播重新校准BN统计；通过多轮机制增量选择高置信度和多样化的增强来初始化合成图像。

Result: 在四个长尾基准测试中均优于现有方法，在CIFAR-100-LT和Tiny-ImageNet-LT上分别提升15.6%和11.8%的top-1准确率（IPC=10，IF=10）。

Conclusion: 该方法通过联合缓解模型偏差和恢复公平监督，有效解决了长尾数据集蒸馏中的挑战，为不平衡数据的高效学习提供了新思路。
...
TL;DR: DualGazeNet是一个受生物视觉启发的纯Transformer框架，通过模拟人类视觉系统的双通路处理和皮质注意力调节，在显著目标检测任务中实现了SOTA性能，同时大幅提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 当前显著目标检测方法趋向复杂架构，导致特征冗余和交叉干扰，而人类视觉系统能以简单方式高效识别显著目标。研究旨在设计一个既符合生物原理又架构简单的框架。

Method: 提出DualGazeNet，一个纯Transformer框架，模拟人类视觉系统的稳健表示学习和双通路处理（大细胞-小细胞通路）机制，结合皮质注意力调节。

Result: 在5个RGB SOD基准测试中超越25个SOTA方法，推理速度比类似容量的Transformer基线快约60%，FLOPs减少53.4%，在伪装和水下SOD任务上表现出强大的跨域泛化能力。

Conclusion: DualGazeNet证明了生物启发的简单架构可以在显著目标检测中实现卓越性能，同时保持高计算效率和可解释性，为复杂工程提供了替代方案。
...
Result: 该紧凑而高效的模型在开源视频生成模型中建立了新的最先进水平，能够生成高质量的多时长、多分辨率的文本到视频和图像到视频内容。
...
TL;DR: ParVTS是一种无需训练并行视觉令牌调度框架，通过将视觉令牌分为主体和非主体组并行处理，然后丢弃非主体路径来减少计算量，在保持准确性的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理高分辨率图像时面临推理延迟问题，因为自注意力机制随序列长度呈二次方增长。简单剪枝视觉令牌会丢失重要上下文信息，影响准确性。

Method: 提出ParVTS框架：将视觉令牌分为主体和非主体组并行处理，将语义转移到问题令牌中，然后在推理中期丢弃非主体路径以减少计算。无需训练、启发式方法或额外模块。

Result: 在多个MLLM骨干网络上实验显示，ParVTS最多可剪枝88.9%的视觉令牌，性能损失最小，实现1.77倍加速和70%的FLOPs减少。

Conclusion: ParVTS是一种高效的多模态推理加速方法，兼容现有MLLM架构，在保持准确性的同时显著降低计算复杂度。
...
TL;DR: 提出了一种自动化管道，通过分析建筑立面结构来识别适合光伏安装的表面并估算太阳能潜力，结果显示可安装BIPV潜力显著低于理论潜力。


<details>
  <summary>Details</summary>
Motivation: 建筑一体化光伏立面是实现城市脱碳的有前景途径，但目前针对立面的自动化光伏规划方法仍然稀缺且过于简化。

Method: 基于SegFormer-B5在CMP Facades数据集上进行微调，将语义预测转换为立面级光伏适用性掩码和考虑模块尺寸与间隙的光伏面板布局。

Result: 在来自10个城市的373个已知尺寸立面数据集上的应用结果表明，可安装BIPV潜力显著低于理论潜力。

Conclusion: 随着立面图像的日益可用，该管道可扩展到支持全球城市的BIPV规划，为可靠的城市能源规划提供宝贵见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有交互式视频世界模型存在两个关键限制：1）未能充分利用指令驱动场景运动与底层3D几何的对应关系，导致视角变化时结构不稳定；2）多步交互中容易遗忘历史信息，造成错误累积和场景语义结构的渐进漂移。

Method: 1）提出Action-Guided 3D Geometry Module (AG3D)，从每个交互的第一帧和对应动作构建点云，为视角转换提供显式几何约束；2）设计History Cache Retrieval (HCR)机制，在生成过程中检索相关历史帧作为条件信号，帮助模型利用过去场景信息。

Result: 实验结果表明，MagicWorld在交互迭代过程中显著提升了场景稳定性和连续性。

Conclusion: MagicWorld通过整合3D几何先验和历史检索机制，有效解决了交互式视频生成中的结构不稳定和历史遗忘问题，实现了更稳定和连续的场景演化。
...
TL;DR: 提出了一种名为MFmamba的多功能模型，能够通过三种不同输入实现超分辨率、光谱恢复以及联合超分辨率和光谱恢复任务，仅需输入全色图像即可完成。


<details>
  <summary>Details</summary>
Motivation: 解决单传感器限制下只能获得高空间分辨率灰度全色图像和低空间分辨率彩色多光谱图像的问题，现有方法无法同时提升空间和光谱分辨率，需要一种集成方法。

Method: 使用UNet++作为主干网络，结合Mamba上采样块，设计了双池注意力机制替代跳跃连接，并提出多尺度混合交叉块进行初始特征提取。

Result: 大量实验表明MFmamba在评估指标和视觉结果上具有竞争力，在三种任务中都表现良好。

Conclusion: MFmamba模型能够有效解决仅使用输入全色图像时的超分辨率和光谱恢复问题，在多个任务中表现出色。
...
TL;DR: 提出EventSTU框架，利用事件相机原理实现高效视频理解，通过粗到细关键帧采样和自适应token剪枝，在保持性能的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在处理长视频时面临高推理成本问题，需要减少冗余帧和token数量。

Method: 设计基于事件相机原理的训练免费框架：1）时域上使用粗到细关键帧采样算法；2）空域上使用自适应token剪枝算法；3）从时空角度整合问题相关性分配token预算。

Result: 在EventBench基准测试中，相比最强基线实现了3.01倍FLOPs减少和3.10倍预填充加速，同时性能仍有提升。

Conclusion: EventSTU框架通过事件引导的时空理解，为高效视频理解提供了有效解决方案，既支持物理事件相机也支持模拟事件。
...
TL;DR: BackdoorVLM是首个针对视觉语言模型(VLMs)的全面后门攻击基准测试，系统评估了5类多模态后门威胁在图像描述和视觉问答等任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 多模态基础模型(特别是视觉语言模型)的后门攻击威胁研究不足，需要系统评估其在不同设置下的脆弱性。

Method: 构建BackdoorVLM基准，将多模态后门威胁分为5类：定向拒绝、恶意注入、越狱、概念替换和感知劫持，使用12种代表性攻击方法在2个开源VLM和3个多模态数据集上进行评估。

Result: VLMs对文本指令高度敏感，在双模态后门中文本触发器通常压倒图像触发器；仅需1%的投毒率就能在大多数任务中达到90%以上的成功率。
...
TL;DR: One4D是一个统一的4D生成和重建框架，能够从单张图像生成动态4D内容，或从视频重建4D内容，输出同步的RGB帧和点云图。
...
Method: 采用统一掩码条件机制处理不同稀疏度的输入帧，使用解耦LoRA控制方法，通过两个模态特定的LoRA适配器分别处理RGB帧和点云图，并通过零初始化控制链接学习像素级一致性。
...
TL;DR: 提出通过最小化CLS令牌到图像补丁的注意力分布熵作为新的测试时自适应目标，提升模型在分布偏移下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 虽然基于输出分布的熵最小化在测试时自适应中有效，但transformers的注意力机制提供了额外的无监督学习信号。

Method: 最小化CLS令牌到图像补丁的注意力分布熵，鼓励模型在分布偏移下更自信地关注相关图像区域。

Result: 注意力熵最小化在各种损坏类型上提高了鲁棒性，同时在干净数据上不会损害性能，即使在单张测试图像情况下也有效。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么使用大语言模型生成详细文本但缺乏时间线索且细节不对齐，要么使用全局3D坐标序列但计算成本高昂。需要一种更高效、精确的控制方法。

Method: 提出FineXtrol框架，使用时间感知的细粒度文本控制信号描述特定身体部位随时间的变化，并设计了分层对比学习模块来增强文本编码器对控制信号的区分能力。

Result: 定量结果显示FineXtrol在可控运动生成方面表现强劲，定性分析展示了其在指导特定身体部位运动方面的灵活性。

Conclusion: FineXtrol通过时间感知的细粒度文本控制实现了高效、精确的运动生成，提高了运动可控性。
...
TL;DR: 本文提出了人类中心开放未来任务发现（HOTD）问题，旨在在动态开放场景中发现能减少人类努力的任务，并开发了HOTD-Bench基准和协作多智能体搜索树（CMAST）框架。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在机器人学和具身AI中取得进展，但在开放未来场景中如何发现直接辅助人类的任务仍是一个挑战，特别是当人类意图高度并发和动态变化时。

Method: 提出了协作多智能体搜索树（CMAST）框架，通过多智能体系统分解复杂推理，并使用可扩展的搜索树模块结构化推理过程。

Result: CMAST在HOTD-Bench上取得了最佳性能，显著超越现有LMMs，并能与现有LMMs良好集成，持续提升性能。

Conclusion: HOTD问题为开放未来场景中的人类辅助任务发现提供了新方向，CMAST框架通过结构化推理和多智能体协作有效解决了这一挑战。
...
Conclusion: VeCoR将流匹配从纯吸引性单边目标转变为双边训练信号，在低步数和轻量级设置中显著提高稳定性、收敛性和图像质量。
...
TL;DR: 本文开发了CSSP2P GAN模型用于虚拟染色，通过盲法病理专家评估证明其具有更高的病理学保真度，并研究了对抗性损失对虚拟染色质量的关键影响。


<details>
  <summary>Details</summary>
Motivation: 免疫组织化学染色成本高且劳动密集，虚拟染色作为图像到图像转换任务提供了有前景的替代方案。现有研究大多使用条件生成对抗网络但忽略了对抗性损失的影响，且评估指标不够稳健。

Method: 开发了CSSP2P GAN模型，通过盲法病理专家评估验证病理学保真度，并迭代研究对抗性损失对虚拟染色质量的影响。

Result: CSSP2P GAN在病理学保真度方面表现优异，对抗性损失被证明对虚拟染色质量至关重要，且模型性能优于领域内的参考工作。

Conclusion: CSSP2P GAN在虚拟染色中实现了更高的病理学保真度，强调了对抗性损失的重要性，并揭示了当前评估指标的局限性。
...
TL;DR: 提出了一个高分辨率视频虚拟试穿数据集，解决了现有方法依赖单一服装图像和缺乏特写镜头的局限性，并提出了VGID指标来评估服装一致性。


<details>
  <summary>Details</summary>
Motivation: 当前视频虚拟试穿技术存在两个关键限制：依赖单一服装图像无法准确捕捉真实纹理细节，以及现有方法只关注全身镜头而忽略了商业对特写镜头的需求。

Method: 构建了高分辨率视频虚拟试穿数据集，提供高保真服装图像、文本描述以及包含全身和特写镜头的真实模特试穿视频，并提出了VGID指标来量化纹理和结构的保持程度。

Result: 实验证明，利用该数据集的详细图像，现有视频生成模型能够提取并整合纹理特征，显著提升虚拟试穿结果的真实感和细节保真度。基准测试有效识别了当前方法在纹理和结构保持方面的问题。

Conclusion: 该数据集和VGID指标解决了视频虚拟试穿中的关键挑战，为提升虚拟试穿的商业应用价值提供了重要支持。
...
TL;DR: 提出了CataractCompDetect框架，结合相位感知定位、SAM 2跟踪、并发症风险评分和视觉语言推理，用于白内障手术并发症的自动检测。
...
Result: 在CataComp数据集上平均F1得分为70.63%，各并发症检测性能为：虹膜脱垂81.8%、后囊破裂60.87%、玻璃体丢失69.23%。

Conclusion: 将结构化手术先验知识与视觉语言推理相结合对于识别罕见但影响重大的术中事件具有重要价值。
...
TL;DR: 提出了单阶段微调策略和广义交错打包方案，用于将预训练CNN转换为FHE友好形式，支持高效全同态加密推理。


<details>
  <summary>Details</summary>
Motivation: 解决深度CNN在全同态加密推理中的两个关键挑战：用低阶多项式近似非线性激活函数，以及突破密文容量限制处理高分辨率图像。

Method: 使用单阶段微调策略直接转换预训练CNN为FHE友好形式，采用广义交错打包方案兼容任意空间分辨率的特征图，并设计保持GIP加密形式的同态算子。

Result: 在CIFAR-10、ImageNet和MS COCO上的实验表明，通过SFT策略获得的FHE友好CNN达到与使用ReLU或SiLU激活函数基线相当的准确率，并首次实现了基于FHE的YOLO架构目标检测。

Conclusion: 这些进展使得跨不同CNN架构的高效端到端FHE推理成为可能，为安全AI推理提供了重要技术支撑。
...
<details>
  <summary>Details</summary>
Motivation: 皮肤肿瘤活检的准确标注面临巨大挑战，包括形态变异大、组织学模式重叠以及良恶性病变的细微区别。现有视觉语言模型在病理学中主要限于玻片级任务或依赖粗略交互提示，难以在千兆像素全玻片图像上生成细粒度分割。

Method: 将每个全玻片图像分割为重叠斑块，提取视觉嵌入，通过计算与文本提示的余弦相似度生成最终分割掩码。使用类别特定的文本提示集合和冻结的VLM编码器实现零样本分割。

Result: 在两个内部数据集（原发性梭形细胞肿瘤和皮肤转移瘤）上展示了具有竞争力的性能，突出了提示设计、领域偏移和机构变异对病理学VLM的影响。
...
TL;DR: 提出了一种单模态生成多模态对比学习(UMCL)框架，通过将单一视觉模态转换为三个互补特征来实现跨压缩率的鲁棒深度伪造检测。
...
Method: 将单视觉模态转换为三个互补特征：压缩鲁棒的rPPG信号、时间地标动态和预训练视觉语言模型的语义嵌入，通过亲和力驱动的语义对齐策略和跨质量相似性学习策略进行特征对齐和鲁棒性增强。

Result: 在各种压缩率和操作类型下实现了优越性能，为鲁棒深度伪造检测建立了新基准，即使在单个特征退化时仍保持高检测精度。

Conclusion: 该方法通过显式特征对齐提供了可解释的特征关系洞察，为跨压缩率的深度伪造检测提供了有效解决方案。
...
TL;DR: ViCoDR通过改进视频扩散模型的多视角一致性表示，解决了当前生成视频中存在的3D不一致性问题，显著提升了视频的3D一致性质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在相机姿态变化时会产生物体和结构变形的视觉伪影，这影响了用户体验和模拟保真度。基于扩散模型表示对齐的最新发现，假设改进视频扩散表示的多视角一致性将产生更3D一致的视频生成。

Method: 提出ViCoDR方法，通过学习多视角一致的扩散表示来改进视频模型的3D一致性。在多个最新的相机控制视频扩散模型上进行了详细分析，揭示了3D一致表示与视频之间的强相关性。

Result: 在相机控制的图像到视频、文本到视频和多视角生成模型上评估ViCoDR，证明生成的视频在3D一致性方面有显著改进。

Conclusion: ViCoDR通过改进多视角一致性表示，有效提升了视频生成模型的3D一致性，为解决当前视频生成中的视觉伪影问题提供了有效方案。
...
Method: 利用音频-视觉语音表示重建，从一个模态（如唇部运动）重建另一个模态（如音频波形）的表示，在篡改片段中重建难度更大，从而放大差异提供判别线索。

Result: 在LAV-DF数据集上AP@0.95提升8.9，在AV-Deepfake1M数据集上AP@0.5提升9.6，在真实场景实验中AUC提升5.1。

Conclusion: AuViRe方法通过跨模态重建差异有效定位深度伪造时间片段，在多个基准测试中表现优异。
...
TL;DR: 提出T2LDM，一种基于自条件表示引导的文本到LiDAR扩散模型，能够生成具有丰富几何结构的3D场景，并支持多种条件生成任务。
...
Method: 采用自条件表示引导机制，在训练时提供重建细节的软监督，推理时解耦；设计方向位置先验缓解街道扭曲；通过条件编码器支持多条件任务。

Result: 在无条件和条件生成任务中均优于现有方法，达到最先进的场景生成效果。

Conclusion: T2LDM能够感知数据分布中的丰富几何结构，生成详细场景对象，并提供了实用的提示范式和见解。
...
TL;DR: 提出了Grc-ViT，一种动态粗到细的视觉Transformer框架，通过自适应调整视觉粒度来解决ViT在细粒度局部细节表示上的不足。
...
Method: 包含两个关键模块：粗粒度评估模块（基于边缘密度、熵和频域线索评估视觉复杂度）和细粒度精炼模块（根据选定粒度优化注意力计算）。通过可学习参数α和β端到端优化全局推理与局部感知的平衡。

Result: 综合评估表明Grc-ViT增强了细粒度判别能力，并在准确性和计算效率之间实现了优越的权衡。

Conclusion: Grc-ViT通过动态粒度调整有效解决了ViT在局部细节表示上的局限性，实现了更高效的视觉特征学习。
...
Method: 1）引入Bench-C基准测试，强调区分性样本评估损坏鲁棒性，提出联合考虑损坏下预测不一致性和语义多样性的选择策略；2）提出鲁棒性对齐分数（RAS），通过考虑预测不确定性和校准对齐的变化来测量logit级预测结构的退化。

Result: 实验发现：1）模型在损坏下表现出不同行为模式，如错误置信和犹豫；2）轻微损坏可能导致准确性轻微提升，但整体预测结构仍会退化；3）通过将损坏鲁棒性分解为破坏性和纠正性组件，可以揭示不同模型的失败和恢复模式。

Conclusion: Bench-C和RAS为评估LVLM在视觉损坏下的鲁棒性提供了更全面的框架，揭示了传统准确性指标无法捕捉的模型行为模式和预测结构变化。
...
<details>
  <summary>Details</summary>
Motivation: 现有的MLLM具身代理在新环境探索中表现不佳，主要问题包括：依赖过时预训练知识、训练成本高昂、以及边界探索带来的复杂决策空间。

Method: 采用回顾性经验回放注入蒸馏抽象经验，以及分层边界选择将边界排序分解为从粗到细的决策过程。

Result: 在多个具身探索基准测试中，ReEXplore相比强基线MLLM方法取得了显著提升，在开源骨干网络下成功率和导航效率均提高了3倍。

Conclusion: ReEXplore实现了鲁棒、可追踪且高效的探索，为MLLM在具身探索任务中的应用提供了有效的免训练解决方案。
...
TL;DR: PG-DPO通过自适应拒绝缩放和隐式偏好正则化解决DPO在扩散模型中存在的似然位移问题，显著提升了视频生成任务中的偏好对齐性能。


<details>
  <summary>Details</summary>
Motivation: DPO在扩散模型中的似然位移问题尚未被充分研究，这导致视频生成任务性能不佳。需要分析DPO损失在扩散框架中的更新机制，以解决优化冲突和次优最大化问题。

Method: 提出了策略引导DPO（PG-DPO），结合自适应拒绝缩放（ARS）和隐式偏好正则化（IPR）来缓解似然位移问题。通过分析DPO损失在扩散框架中的更新策略，识别出两种主要失败模式。

Result: 实验表明PG-DPO在定量指标和定性评估上都优于现有方法，为视频生成任务中的偏好对齐提供了稳健解决方案。

Conclusion: PG-DPO通过系统分析DPO在扩散模型中的失败模式，并引入ARS和IPR机制，有效解决了似然位移问题，显著提升了偏好对齐性能。
...
Method: 构建包含真实和合成图像的大规模数据集，涵盖多种低空飞行器类别，并建立统一的评估基准和单目3D检测基线方法MonoLAA。

Result: 数据集支持多种3D感知任务，合成数据预训练模型能够有效迁移到真实数据，展示了良好的仿真到真实泛化能力。
...
TL;DR: Grc-SAM是一个基于粒度计算的粗到细框架，用于无提示图像分割，通过多粒度注意力机制解决定位性和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统分割模型在单粒度级别生成提示的两个限制：(1) 缺乏自主区域定位机制；(2) 在高分辨率下细粒度建模能力有限。

Method: 采用粗到细三阶段框架：粗阶段自适应提取高响应区域实现前景定位；细阶段应用更细的补丁划分和稀疏局部注意力增强细节建模；将精炼掩码编码为潜在提示嵌入替代手工提示。

Result: 大量实验结果表明Grc-SAM在准确性和可扩展性方面优于基线方法。

Conclusion: Grc-SAM为无提示分割提供了独特的粒度计算视角，通过集成多粒度注意力将粒度计算与视觉变换器相结合。
...
TL;DR: 提出了DEAP-3DSAM模型，通过特征增强解码器和双注意力提示器解决SAM在3D医学图像分割中的空间特征损失和手动提示依赖问题，在腹部肿瘤分割数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: SAM模型在医学图像分割中展现出潜力，但应用于3D图像时存在空间特征损失问题，且大多数方法依赖手动提示，这在真实场景中难以实现且需要专家知识。

Method: 提出特征增强解码器融合原始图像特征与空间信息来增强空间特征，设计双注意力提示器通过空间注意力和通道注意力自动获取提示信息。

Result: 在四个公共腹部肿瘤分割数据集上的实验表明，DEAP-3DSAM在3D图像分割中达到最先进性能，超越或匹配现有手动提示方法。
...
Method: 框架包含跨天线共享的CNN编码器用于子载波-时间特征提取，轻量级注意力模块自适应重加权时间和天线维度特征，以及结合GCN层和自注意力的图回归头来捕获局部拓扑和全局依赖。

Result: 在MM-Fi数据集的各种设置下，该方法显著优于现有方法。
...
TL;DR: HABIT是一个高保真自动驾驶仿真基准，通过将真实世界人体运动数据集成到CARLA模拟器中，解决了现有仿真中行人行为不够真实和多样化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶仿真严重缺乏对真实多样化人类行为的准确表示，现有基准往往简化行人交互，无法捕捉复杂的动态意图和变化响应，这对系统安全部署至关重要。

Method: 开发了模块化、可扩展且物理一致的运动重定向管道，将来自动作捕捉和视频的真实人类运动数据集成到CARLA中，从30,000个重定向运动中筛选出4,730个交通兼容的行人运动，采用SMPL格式确保物理一致轨迹。

Result: 评估三个最先进的自动驾驶代理（InterFuser、TransFuser、BEVDriver）显示，HABIT暴露了在脚本仿真中隐藏的规划器弱点：尽管在CARLA排行榜上接近零碰撞，但在HABIT上表现显著恶化，最高达7.43次碰撞/公里，12.94%的AIS 3+伤害风险，以及高达33%的不必要制动。

Conclusion: HABIT基准能够揭示现有评估方法遗漏的关键失效模式，为可重现、行人感知的AI研究提供支持，所有组件已公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有AIGC检测基准主要关注整图分类，忽略了扩散编辑的定位问题。扩散编辑能够对局部区域进行逼真修改，使得AI生成内容更难检测。

Method: 构建包含3万张扩散编辑图像的数据集，使用8种SOTA扩散模型进行局部编辑，采用VLM管道自动识别有意义的区域并生成上下文感知的提示，支持多轮编辑。

Result: 实验表明分割模型在整图分类任务中优于传统伪造分类器，在跨生成器泛化方面表现出潜力，但在语义分割任务中面临图像失真鲁棒性等挑战。
...
TL;DR: 3M-TI是一个无需校准的多摄像头跨模态扩散框架，用于移动热成像超分辨率，通过跨模态自注意力模块在去噪过程中自适应对齐热成像和RGB特征，显著提升热图像的空间分辨率、结构保真度和纹理细节。


<details>
  <summary>Details</summary>
Motivation: 移动平台热传感器的微型化限制了其空间分辨率和纹理保真度，导致图像模糊且信息量少。现有热成像超分辨率方法存在单图像方法难以恢复精细结构、RGB引导方法依赖繁琐跨相机校准的问题。

Method: 提出3M-TI框架，在扩散UNet中集成跨模态自注意力模块(CSM)，替换原始自注意力层，在去噪过程中自适应对齐热成像和RGB特征，无需显式相机校准。

Result: 在真实移动热相机和公共基准测试上的广泛评估验证了其优越性能，在视觉质量和定量指标上均达到最先进水平。增强后的热图像在目标检测和分割等下游任务中带来显著提升。

Conclusion: 3M-TI为鲁棒移动热感知系统提供了实用价值，通过生成先验增强热图像质量，无需繁琐的跨相机校准过程。
...
TL;DR: 提出了MonoSR数据集，这是一个大规模的单目空间推理数据集，涵盖室内、室外和物体中心场景，支持多种问题类型，为开放世界单目空间推理提供基础。
...
Method: 构建MonoSR数据集，评估先进视觉语言模型在该任务上的表现，分析辅助信息对单目空间推理的重要性，为未来模型设计提供实用指导。

Result: 揭示了先进视觉语言模型在这一挑战性任务上的局限性，并提供了关于辅助信息重要性的分析结果。
...
TL;DR: 本文提出SemAnti方法，通过冻结CLIP的语义子空间并在打乱语义下仅适配对伪影敏感的层，有效抑制语义偏差，提升AI生成图像检测的跨域泛化能力。
...
Method: 提出SemAnti语义对抗微调范式：1）使用Patch Shuffle破坏全局语义连续性但保留局部伪影线索；2）冻结CLIP的语义子空间；3）仅在打乱语义下适配对伪影敏感的层。

Result: 在AIGCDetectBenchmark和GenImage基准测试中达到最先进的跨域泛化性能，证明调节语义是释放CLIP在AI生成图像检测中全部潜力的关键。

Conclusion: 通过抑制语义偏差和调节语义结构，可以显著提升CLIP在AI生成图像检测中的鲁棒性和泛化能力，SemAnti方法为此提供了有效的解决方案。
...
TL;DR: 提出了MambaRefine-YOLO模型，通过双门控互补Mamba融合模块和分层特征聚合颈部，在无人机图像小目标检测中实现了精度和速度的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的小目标检测面临低分辨率和背景杂波挑战，现有RGB和红外数据融合方法在跨模态交互和计算效率之间存在权衡问题。

Method: 使用双门控互补Mamba融合模块（DGC-MFM）通过光照感知和差异感知门控机制自适应平衡RGB和红外模态，以及采用'先优化后融合'策略的分层特征聚合颈部（HFAN）增强多尺度特征。
...
TL;DR: FilmSceneDesigner是一个自动化电影场景生成系统，通过基于代理的链式框架和程序化生成流程，从自然语言描述创建完整的电影场景，解决了传统手动建模的耗时问题。
...
Method: 采用基于代理的链式框架生成结构化参数，结合程序化生成流程进行平面图生成、材质分配、门窗布置和物体布局，并使用专门的SetDepot-Pro数据集增强真实感。

Result: 实验和人工评估表明，系统能生成结构合理、具有强烈电影真实感的场景，支持虚拟预演、施工图纸和情绪板创建等下游任务。

Conclusion: FilmSceneDesigner成功实现了专业电影场景设计工作流的自动化，显著提高了场景生成效率和质量。
...
TL;DR: ABM-LoRA是一种加速低秩适配器收敛的初始化策略，通过对齐预训练模型和适配器的激活边界来减少信息损失，提高收敛速度。


<details>
  <summary>Details</summary>
Motivation: LoRA虽然参数效率高，但随机初始化导致梯度更新在错误的切空间中进行，造成显著信息损失并阻碍早期收敛。

Method: 提出激活边界匹配方法，在下游训练前将适配器的激活边界与预训练模型对齐，最大化全参数梯度在适配器子空间中的投影。

Result: 在语言理解、对话生成和视觉识别任务中显著加速收敛，在VTAB-1K上达到最高准确率，在需要几何理解的结构化推理任务上表现优异。

Conclusion: ABM-LoRA通过改进初始化策略有效解决了LoRA的信息损失问题，显著提升了收敛速度和模型性能。
...
TL;DR: 提出CoMA框架，利用互补的Foundation Models（如CLIP和BLIP）进行源自由域自适应，通过双向适应机制和分解互信息实现稳定训练，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 单一Foundation Model在源自由域自适应中语义覆盖有限，无法充分捕捉域偏移下的多样化上下文线索，需要利用多个互补的FMs来提升适应效果。

Method: CoMA框架联合利用两个互补的FMs，采用双向适应机制：对齐不同FMs与目标模型进行任务适应，同时保持语义区分性；将互补知识从FMs转移到目标模型。引入分解互信息（DMI）来增强真实依赖关系并抑制虚假依赖。

Result: 在Office-31、Office-Home、DomainNet-126和VisDA四个基准测试中，闭集设置下持续优于现有SOTA SFDA方法，在部分集和开放集变体中也取得最佳结果。

Conclusion: 通过协同利用多个互补的Foundation Models，CoMA框架能够有效解决源自由域自适应中的语义覆盖限制问题，在多种设置下均表现出优越性能。
...
TL;DR: 提出首个图像修复测试时偏好优化(TTPO)范式，通过在线生成偏好数据并引导扩散去噪过程，无需重新训练模型即可提升修复图像与人类偏好的对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有预训练和零样本图像修复方法往往无法与人类偏好对齐，导致修复结果不被用户青睐，需要一种无需重新训练模型且能灵活适应各种任务的方法来提升感知质量。

Method: 设计无需训练的三阶段流程：1)基于初始修复图像在线生成候选偏好图像；2)使用自动化偏好对齐指标或人工反馈选择偏好/非偏好图像；3)将选定图像作为奖励信号引导扩散去噪过程，优化修复结果。

Result: 在各种图像修复任务和模型上的广泛实验证明了该方法的有效性和灵活性。

Conclusion: TTPO范式能够有效提升图像修复的感知质量，生成符合人类偏好的结果，且兼容任何图像修复模型骨干，无需重新训练。
...
TL;DR: MetroGS是一个用于复杂城市环境高效稳健重建的新型高斯溅射框架，通过分布式2D高斯表示、结构化密集增强、渐进混合几何优化和深度引导外观建模，在大规模城市场景中实现优越的几何精度和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 解决3D高斯溅射及其衍生方法在大规模场景重建中如何高效稳定地实现高质量几何保真度的核心挑战，特别是在复杂城市环境中。

Method: 1. 基于分布式2D高斯溅射表示作为统一骨干；2. 结构化密集增强方案利用SfM先验和点图模型实现更密集初始化；3. 渐进混合几何优化策略整合单目和多视图优化；4. 深度引导外观建模方法学习具有3D一致性的空间特征。

Result: 在大规模城市数据集上的实验表明，MetroGS实现了优越的几何精度和渲染质量，为高保真大规模场景重建提供了统一解决方案。

Conclusion: MetroGS通过创新的分布式表示、密集增强、几何优化和外观建模方法，成功解决了复杂城市环境中的高效稳健重建问题，为大规模场景重建提供了有效的技术路径。
...
TL;DR: 本文对三种源相机识别技术进行了比较分析：PRNU、JPEG压缩伪影分析和CNN，评估了它们在设备分类准确性方面的表现，并讨论了实际应用中需要的科学进展。
...
Method: 比较分析了三种源相机识别技术：PRNU（光响应非均匀性）、JPEG压缩伪影分析和卷积神经网络（CNN）。
...
TL;DR: nnActive是一个开源主动学习框架，通过解决4个评估陷阱，在3D生物医学图像分割中系统比较了主动学习与随机采样策略，发现改进的感知前景随机采样策略与主动学习方法表现相当。


<details>
  <summary>Details</summary>
Motivation: 3D生物医学图像语义分割依赖大量标注数据，但人工标注成本高且需要专业知识。主动学习旨在通过选择最有信息量的样本来减少标注工作量，但现有评估存在4个主要问题。

Method: 开发nnActive框架，包含：(1)大规模研究覆盖4个数据集和3种标注策略；(2)扩展nnU-Net使用部分标注进行3D补丁查询选择；(3)提出感知前景随机采样策略；(4)引入前景效率指标。

Result: (A)所有主动学习方法都优于标准随机采样，但无法可靠超越改进的感知前景随机采样；(B)主动学习效果依赖任务特定参数；(C)预测熵是表现最好的主动学习方法但标注成本可能最高；(D)计算密集型设计可提升主动学习性能。

Conclusion: nnActive作为开源框架，可作为3D生物医学图像中主动学习研究和应用的催化剂，揭示了感知前景随机采样在特定场景下的竞争力。
...
Method: 使用物理器官模型获取数据，采用3D生成对抗网络(GAN)生成3D模型流形，使用仿生水凝胶制作人工前列腺模型，在定制超声扫描仪中记录手术前后数据，训练神经网络分割超声图像并重建3D网格模型。

Result: 训练的神经网络在分割超声图像方面优于传统的非学习计算机视觉技术，在交并比(IoU)指标上表现更好，并基于分割结果成功重建了3D网格模型。

Conclusion: 该工作流能够有效生成用于下游机器学习任务的3D解剖数据，为手术规划和训练提供了可行的数据获取解决方案。
...
TL;DR: 该论文研究了CLIP等视觉语言模型是否能区分真实物体与其相似物（如玩具、雕像等），并提出了RoLA数据集和一种在CLIP嵌入空间中估计真实-相似物方向的方法。
...
Method: 构建了RoLA数据集，包含真实物体和相似物的示例；首先评估基于提示的基线方法，然后在CLIP嵌入空间中估计真实与相似物之间的方向向量。

Result: 应用该方向向量到图像和文本嵌入中，在Conceptual12M上的跨模态检索中提高了区分能力，并改善了CLIP前缀字幕生成器的字幕质量。

Conclusion: CLIP模型能够捕捉真实物体与相似物之间的细微区别，通过在嵌入空间中估计方向向量可以增强模型的判别能力。
...
TL;DR: 提出ReAlign方法解决文本到运动生成中的分布不对齐问题，通过奖励引导采样改善语义一致性和运动质量
...
Method: ReAlign方法包含步感知奖励模型和奖励引导策略，通过文本对齐模块和运动对齐模块评估并引导扩散过程

Result: 在运动生成和检索任务中显著改善了文本-运动对齐和运动质量，优于现有最先进方法

Conclusion: ReAlign方法有效解决了文本-运动分布不对齐问题，提升了生成运动的语义一致性和质量
...
TL;DR: Percept-WAM是首个在单一视觉语言模型中隐式集成2D/3D场景理解能力的感知增强世界感知-动作模型，通过统一的空间表示和改进的解码机制，显著提升了自动驾驶感知的准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统在空间感知方面存在不足，特别是在长尾场景和复杂交互中表现不稳定。现有的视觉语言模型在空间基础理解和定位能力上有限，需要更强大的感知能力。

Method: 提出World-PV和World-BEV令牌统一2D/3D感知任务，采用网格条件预测机制进行密集物体感知，包含IoU感知评分和并行自回归解码，同时保留预训练VLM参数以维持通用智能。

Result: 在COCO 2D检测上达到51.7/58.9 mAP，在nuScenes BEV 3D检测上表现优异，集成轨迹解码器后在NAVSIM上超越DiffusionDrive 2.1 PMDS，展现出强大的开放词汇和长尾泛化能力。

Conclusion: Percept-WAM成功地将2D/3D感知能力集成到单一VLM中，显著提升了自动驾驶系统的感知稳定性和规划性能，为端到端自动驾驶系统提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖昂贵的人工标注，要么使用没有明确对象分解的时变表示，导致静态和动态元素交织，阻碍场景分离。

Method: 将动态对象建模为经历刚性变换的连贯实例；使用基于语言的零样本视频跟踪和激光雷达进行3D锚定；通过特征对应估计一致位姿；引入协调转向平滑方案获得时间物理一致的运动轨迹；联合优化对象位姿和高斯参数。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的视觉语言模型在3D空间理解和长视频理解方面仍然存在困难，虽然它们在典型的视觉语言任务上表现强大。现有方法通常依赖专门的架构设计来分别改进3D任务和视频理解任务的性能。

Method: 提出LAST方法，让VLMs在空间和时间维度进行思考，构建3D空间和时间维度的视觉思考轨迹，而不是仅依赖文本。通过两种场景实现：1）零样本场景下直接提示专有模型；2）使用包含空间和时间思考轨迹的数据微调通用VLMs。

Result: LAST在各种基准测试中带来显著提升，包括3个空间理解、4个视频理解和3个图像理解任务。特别是在零样本场景下，GPT-4o在EgoSchema上获得15.8%的提升，Qwen2.5-VL-7B在VSI-Bench上获得8.3的提升。

Conclusion: LAST方法能够有效联合提升视觉语言模型在3D空间和长视频理解方面的能力，仅使用2D图像输入即可实现显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前条件图像生成方法在处理文本提示与条件图像之间的冲突时面临挑战，包括输入级冲突和模型偏差冲突，标准监督微调难以有效解决这些问题

Method: BideDPO框架创建两个解耦的偏好对（一个针对条件，一个针对文本），使用自适应损失平衡策略管理偏好对的影响，并采用自动化数据管道和迭代优化策略

Result: 实验显示BideDPO显著提高了文本成功率（如+35%）和条件遵循度，在COCO数据集上也验证了方法的有效性

Conclusion: BideDPO通过解耦优化策略有效解决了条件图像生成中的冲突问题，为多约束任务提供了新的解决方案
...
Result: 在ImageNet上实验表明，重建偏差提供了有效的评分标准，在不同选择比率下均优于现有基线，仅用50%数据即可匹配全数据训练效果
...
TL;DR: ReMatch是一个利用MLLMs生成能力进行多模态检索的框架，通过端到端训练和生成式匹配阶段实现更好的检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将MLLMs视为简单编码器，忽略了其生成特性和组合推理能力，导致能力利用不足。

Method: 使用聊天式生成匹配阶段训练嵌入MLLM，通过多视角输入自回归判断相关性，结合对比损失和实例级判别监督。

Result: 在Massive Multimodal Embedding Benchmark上达到新的最先进水平，在五个数据集上表现出强大的零样本泛化能力。

Conclusion: ReMatch通过充分利用MLLMs的生成能力和组合优势，实现了更鲁棒和可迁移的多模态检索性能。
...
TL;DR: 提出了一种基于先验密度化的3D高斯泼溅方法，通过结合稀疏LiDAR数据和单目深度估计来增强3D场景初始化，避免传统自适应密度控制导致的浮点伪影和资源浪费问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法依赖自适应密度控制，容易产生浮点伪影且资源利用效率低，需要改进初始化过程以提高视觉质量和计算效率。

Method: 采用先验密度化方法，结合稀疏LiDAR数据和RGB图像的单目深度估计，使用ROI感知采样方案优先处理语义和几何重要区域，生成密集点云。

Result: 在四个新收集的数据集上验证，该方法在保持与最先进技术相当结果的同时，显著降低了资源消耗和训练时间，有效保留了复杂场景中的感兴趣区域。

Conclusion: 提出的先验密度化方法通过改进3D场景初始化，绕过了传统自适应密度控制的问题，在减少重叠的同时提升了视觉质量，实现了更高效的3D高斯泼溅。
...
TL;DR: IDEAL-M3D是首个用于单目3D检测的实例级主动学习框架，通过显式多样性集成解决了传统方法中图像级标注效率低和深度偏差问题，仅用60%标注就能达到全数据集训练的性能。


<details>
  <summary>Details</summary>
Motivation: 单目3D检测需要大量3D标注，但标注成本高昂。现有主动学习方法存在两个问题：基于图像级选择效率低（非信息实例也被标注），以及基于不确定性的选择存在深度偏差（偏向远距离物体）。
...
Conclusion: IDEAL-M3D通过实例级选择和多样性集成有效解决了单目3D检测中主动学习的关键问题，在有限标注预算下实现了高性能。
...
TL;DR: 提出DGSPNet框架，通过双粒度语义提示（粗粒度文本先验和细粒度个性化语义描述）结合文本引导的通道和空间注意力机制，显著提升红外小目标检测性能，在三个基准数据集上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 解决红外小目标检测中特征表示有限和背景干扰严重的问题，克服现有CLIP方法依赖不准确文本描述和人工标注的局限性。

Method: 提出端到端语言提示驱动框架DGSPNet，集成双粒度语义提示（粗粒度文本先验和细粒度视觉到文本映射），并引入文本引导通道注意力(TGCA)和文本引导空间注意力(TGSA)机制。

Result: 在三个基准数据集上的大量实验表明，该方法显著提高了检测精度，达到了最先进的性能。

Conclusion: DGSPNet通过有效利用语言提示和注意力机制，成功解决了红外小目标检测的挑战，无需依赖任何标注要求即可实现高性能检测。
...
TL;DR: SyncMV4D是首个联合生成同步多视角手物交互视频和4D运动的模型，通过统一视觉先验、运动动力学和多视角几何，克服了单视角方法的几何失真和3D方法对受控环境数据的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视频的方法主要是单视角的，阻碍了全面的3D几何感知，常导致几何失真或不真实的运动模式；而3D方法依赖受控实验室环境的高质量3D数据，难以泛化到真实世界场景。

Method: 提出两个核心创新：(1)多视角联合扩散模型共同生成HOI视频和中间运动；(2)扩散点对齐器将粗糙中间运动细化为全局对齐的4D度量点轨迹。通过建立闭环、相互增强的循环，紧密耦合2D外观与4D动态。

Result: 实验表明，该方法在视觉真实感、运动合理性和多视角一致性方面优于现有最先进方法。

Conclusion: SyncMV4D成功解决了手物交互生成中的多视角一致性和真实世界泛化问题，为动画和机器人应用提供了更可靠的解决方案。
...
TL;DR: SteadyDancer是一个基于图像到视频（I2V）范式的人体图像动画框架，通过条件协调机制、协同姿态调制模块和分阶段解耦目标训练流程，解决了第一帧身份保持和精确运动控制的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前主流参考到视频（R2V）范式的图像到运动绑定过程忽视了现实应用中常见的时空错位问题，导致身份漂移和视觉伪影等失败情况。

Method: 1. 条件协调机制协调两个冲突条件；2. 协同姿态调制模块生成与参考图像高度兼容的自适应姿态表示；3. 分阶段解耦目标训练流程分层优化运动保真度、视觉质量和时间连贯性。
...
<details>
  <summary>Details</summary>
Motivation: 现有单目方法使用解剖学不准确的简化模型（如SMPL）并忽略物理约束，限制了生物力学保真度。需要开发能够同时估计运动学和动力学的生物力学真实方法。
...
Conclusion: MonoMSK通过整合数据驱动学习和物理模拟，为单目视频中的生物力学真实3D人体运动估计提供了有效解决方案，同时实现了高精度的运动学和动力学恢复。
...
TL;DR: 该论文提出了一种在表示层面进行机器遗忘的方法POUR，通过几何投影实现可证明最优的表示遗忘，在保持保留知识的同时有效移除特定视觉概念的影响。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法通常只修改分类器而保留内部表示，导致不完全遗忘。需要将遗忘概念扩展到表示层面，实现更彻底的遗忘效果。

Method: 基于神经崩溃理论，提出了POUR方法，包括闭式几何投影版本(POUR-P)和蒸馏方案下的特征级遗忘版本(POUR-D)，利用正交投影保持ETF结构。

Result: 在CIFAR-10/100和PathMNIST数据集上的实验表明，POUR在分类级和表示级指标上都优于现有最先进的遗忘方法。

Conclusion: 表示层面的遗忘对于实现彻底的机器遗忘至关重要，POUR方法提供了可证明最优的解决方案，在遗忘效果和知识保留之间取得了良好平衡。
...
TL;DR: 提出了Syn-GRPO方法，通过在线数据生成器合成高质量训练数据，解决MLLM强化学习中数据质量低、响应多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法面临数据质量低的问题，数据样本无法激发MLLM产生多样化响应，限制了强化学习的探索范围。虽然有些方法尝试通过熵约束来缓解，但未能从根本上解决问题。

Method: Syn-GRPO包含两个组件：(1)数据服务器：使用图像生成模型从现有样本合成新样本，采用解耦异步方案实现高生成效率；(2)GRPO工作流：为数据服务器提供新图像描述，并利用多样性奖励监督MLLM预测图像描述以合成具有多样化响应的样本。

Result: 在三个视觉感知任务上的实验结果表明，Syn-GRPO大幅提高了数据质量，性能显著优于现有MLLM感知方法，并展现出扩展长期自演化强化学习的潜力。

Conclusion: Syn-GRPO通过合成高质量多样化训练数据，有效解决了MLLM强化学习中的数据质量问题，为长期自演化强化学习提供了有前景的解决方案。
...
TL;DR: 提出了一个包含3023张图像、超过43万个细胞标注的大规模细胞计数数据集，并评估了包括SAM-Counter在内的多种方法，SAM-Counter以MAE 22.12的表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 细胞计数在生物医学研究和临床应用中至关重要，但手动计数耗时且易错，现有数据集规模有限（通常少于500张图像），限制了深度学习模型的训练效果。

Method: 构建大规模细胞计数数据集，包含3023张免疫细胞化学实验图像；评估三类现有方法（回归法、人群计数法、细胞计数法）；提出基于Segment Anything Model的密度图适配方法SAM-Counter。

Result: SAM-Counter在测试集上取得MAE 22.12的优异表现，优于第二佳方法的MAE 27.46；数据集具有高细胞密度、细胞重叠、形态多样等挑战性特征。

Conclusion: 该数据集和基准框架为自动化细胞计数研究提供了坚实基础，SAM-Counter展示了在仅有点标注数据集上适配预训练模型的潜力。
...
TL;DR: 提出Self-Paced GRPO框架，通过渐进式奖励机制使奖励反馈与生成器共同进化，解决了静态奖励模型带来的分布偏差和饱和问题。
...
Method: 引入渐进式奖励机制，随着生成质量提高自动将重点从粗粒度视觉保真度转向时间连贯性和细粒度文本-视频语义对齐，实现奖励与策略的共同进化。

Result: 在VBench基准测试中，多个视频生成骨干网络上均显示视觉质量和语义对齐方面相比静态奖励的GRPO基线有持续改进。

Conclusion: Self-Paced GRPO通过自定步调的课程学习缓解了奖励-策略不匹配，减轻了奖励利用，实现了更稳定的优化，验证了该方法的有效性和通用性。
...
TL;DR: 提出了一种基于图结构的UI表示方法，将UI截图转换为编码层次关系和空间布局的属性图，通过对比图自编码器学习多级相似性嵌入，在金融软件UI数据集上实现了高精度快速搜索。


<details>
  <summary>Details</summary>
Motivation: 企业软件拥有数千个UI界面，现有方法依赖视觉相似性或文本语义，缺乏对UI组成结构属性的显式建模，难以保证设计一致性、模式发现和合规检查。

Method: 将UI截图转换为属性图表示，使用对比图自编码器学习嵌入，保留视觉、结构和语义属性的多级相似性，构建UISearch多模态搜索框架。

Result: 在20,396个金融软件UI上，UISearch达到0.92的Top-5准确率，中位延迟47.5ms，支持20,000+屏幕规模，比仅视觉方法具有更好的细粒度区分能力。

Conclusion: 结构嵌入比最先进的视觉编码器具有更好的判别能力，代表了UI表示表达能力的基本进步，可推广到文档布局、架构图等结构化视觉领域。
...
<details>
  <summary>Details</summary>
Motivation: 传统病灶分割将所有非病灶像素归为单一背景类，忽略了丰富的解剖学背景信息。背景实际上是高度异质的，包含多种组织和器官结构。

Method: 使用细粒度标签将背景类细分为多个解剖结构类别，通过增加Fisher信息来提升模型性能。可以利用预训练分割模型自动生成辅助标签。

Result: 在多个数据集和架构上的实验表明，BackSplit能持续提升小病灶分割性能，即使辅助标签是自动生成的也有效。

Conclusion: BackSplit是一种简单而强大的范式，通过背景类细分显著提升分割性能，具有鲁棒性、简单性和广泛适用性。
...
TL;DR: SAM3-Adapter是针对Segment Anything 3（SAM3）的首个适配器框架，通过减少计算开销并提升分割精度，在医学影像、伪装物体分割和阴影检测等多个下游任务中实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决SAM及其后继模型在细粒度、低层次分割任务（如伪装物体检测、医学图像分割、细胞图像分割和阴影检测）上的局限性，释放SAM3的完整分割能力。

Method: 基于原始SAM-Adapter的模块化和可组合设计理念，开发专门为SAM3定制的适配器框架，提供更强的泛化能力和任务适应性。

Result: SAM3-Adapter在多个下游任务中持续超越SAM和SAM2解决方案，建立了新的最先进结果，同时提高了分割精度、鲁棒性和效率。

Conclusion: SAM3-Adapter可以作为未来研究和实际分割应用的基础，其代码、预训练模型和数据处理流程均已公开。
...
TL;DR: Ref-SAM3D通过在SAM3D中引入文本描述作为高级先验，实现了从单张RGB图像进行文本引导的3D重建，解决了SAM3D无法根据文本描述重建特定对象的问题。
...
Method: 在SAM3D基础上引入文本描述作为高级先验，通过自然语言和单张2D视图指导3D重建，连接2D视觉线索和3D几何理解。

Result: Ref-SAM3D实现了具有竞争力的零样本重建性能，能够生成高保真度的3D重建结果。

Conclusion: Ref-SAM3D为参考引导的3D重建提供了一个更灵活和易用的范式，有效弥合了2D视觉线索与3D几何理解之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集在任务规划中忽略了运筹学知识和3D空间定位，简化了任务调度问题，需要更全面的方法来解决具身AI在3D物理世界中的高效任务执行。

Method: 构建ORS3D-60K大规模数据集，包含6万复合任务和4千真实场景；提出GRANT模型，采用调度令牌机制生成高效任务调度和空间定位动作。

Result: 在ORS3D-60K数据集上的广泛实验验证了GRANT在语言理解、3D空间定位和调度效率方面的有效性。

Conclusion: ORS3D任务和GRANT模型为具身AI的任务调度提供了新的研究方向，通过结合运筹学知识和3D空间定位，显著提升了任务执行效率。
...
TL;DR: Cloud4D是首个基于学习的框架，仅使用同步地面相机重建物理一致的四维云状态，实现25米空间和5秒时间分辨率的3D液态水含量分布重建，并提供水平风矢量估计。
...
Method: 利用同形变换引导的2D到3D转换器，仅使用同步地面相机重建3D液态水含量分布，并通过跟踪时间序列估计水平风矢量。

Result: 在包含6个向上拍摄相机的两个月部署中，系统相比最先进的卫星测量实现了数量级的时空分辨率提升，同时相对于共置雷达测量保持个位数相对误差(<10%)。

Conclusion: Cloud4D框架能够以极高分辨率重建四维云状态，为高分辨率天气和气候建模提供了新的观测能力。
...
TL;DR: 提出一种简单的即插即用采样方法，通过在高噪声和低噪声阶段切换使用不同的预训练扩散专家模型来打破图像生成中感知质量与数据似然性的权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成中存在感知样本质量与数据似然性的权衡：强调高噪声去噪步骤的训练目标能产生真实图像但似然性差，而强调似然性的训练会过度重视低噪声步骤并损害视觉保真度。

Method: 结合两个预训练的扩散专家模型，在去噪轨迹上切换使用它们：在高噪声水平应用图像质量专家来塑造全局结构，在低噪声水平切换到似然性专家来细化像素统计。该方法无需重新训练或微调，只需选择中间切换步骤。

Result: 在CIFAR-10和ImageNet32上，合并模型始终匹配或优于其基础组件，相对于每个单独专家，改善或保持了似然性和样本质量。

Conclusion: 在噪声水平上切换专家是打破图像扩散模型中似然性-质量权衡的有效方法。
...
TL;DR: IF-Edit是一个无需调优的框架，利用预训练的图像到视频扩散模型进行指令驱动的图像编辑，解决了提示不对齐、冗余时间潜变量和模糊后期帧三个关键挑战。
...
Method: 包含三个核心组件：(1)思维链提示增强模块，将静态编辑指令转换为时间基础推理提示；(2)时间潜变量丢弃策略，在专家切换点后压缩帧潜变量；(3)自一致后精炼步骤，使用短静止视频轨迹锐化后期帧。
...
Conclusion: LumiTex提供了一个有效的端到端解决方案，能够生成高质量的PBR纹理，解决了材料分解和纹理补全的关键问题。
...
TL;DR: SCARE是一个用于评估电子健康记录问答系统中后验安全机制的基准，专注于问题可回答性分类和SQL查询验证/修正。


<details>
  <summary>Details</summary>
Motivation: 在安全关键的临床环境中部署文本到SQL模型存在风险，错误的SQL查询可能影响临床决策和患者护理，需要独立的后验验证机制。

Method: 构建包含4200个问题-SQL查询-期望输出的三元组基准，基于MIMIC-III、MIMIC-IV和eICU数据库，涵盖7种不同文本到SQL模型生成的多样化查询。

Result: 实验揭示了问题分类和SQL错误修正之间的关键权衡，展示了从两阶段方法到智能体框架的各种方法的性能。

Conclusion: SCARE基准填补了后验安全机制评估的空白，为未来研究指明了方向，强调了在临床环境中安全部署文本到SQL系统的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型能处理长上下文，但解码延迟和内存开销仍然很大，现有的KV缓存重用方法存在性能下降问题。

Method: A³算法基于问题相关性预计算并选择性融合文本块的KV缓存，实现准确集成且计算开销最小。

Result: 在多个基准测试和LLM上的实验表明，A³相比四个基线方法获得最佳任务性能，同时将首词生成时间减少2倍。
...
TL;DR: LexInstructEval是一个新的基准测试框架，用于评估LLMs在细粒度词汇指令遵循方面的能力，通过基于规则的语法将复杂指令解构为<过程、关系、值>三元组，实现客观评估。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLMs遵循复杂词汇指令能力的方法存在局限性：人工评估主观且昂贵，自动化LLM评估系统存在偏见和不可靠性，而现有程序化基准测试缺乏表达复杂组合约束的能力。

Method: 基于形式化的规则语法将复杂指令解构为<Procedure, Relation, Value>三元组，通过多阶段人工参与流程生成多样化数据集，并使用透明的程序化引擎进行客观验证。

Result: 开发了LexInstructEval基准测试框架和开源评估工具，为研究LLMs的可控性和可靠性提供了系统化的评估方法。

Conclusion: LexInstructEval解决了现有评估方法的局限性，为评估LLMs的细粒度词汇指令遵循能力提供了客观、可扩展的框架，促进了LLM可控性和可靠性的进一步研究。
...
Result: 在SIGHAN-2015、EC-LAW、MCSC和NaCGEC等权威基准测试中，该模型的F1和F0.5分数显著超越现有公开模型，在拼写和语法纠错任务中均排名第一。

Conclusion: ChineseErrorCorrector3-4B在中文拼写和语法纠错方面表现出色，达到了当前最先进的性能水平。
...
TL;DR: 提出了一个生成式缓存方法，用于处理结构相似但略有变化的提示，通过识别可重用的响应模式来生成定制化输出。
...
Result: 在无提示重复的数据集上达到83%的缓存命中率，且错误命中率最小；在智能体工作流中，相比标准提示匹配，缓存命中率提高约20%，端到端执行延迟减少约34%。

Conclusion: 该方法能有效处理结构相似提示的缓存问题，显著提高缓存效率和执行性能。
...
TL;DR: 本文提出了一个测试认知立场转移的框架，发现在即使删除事件知识后，对齐的大语言模型仍能保持社区特定的不确定性处理行为模式，表明对齐编码了超越表面模仿的结构化、可泛化行为。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在对齐到特定在线社区时，是表现出反映该社区态度和应对新不确定性的一般化行为模式，还是仅仅回忆训练数据中的模式。

Method: 引入认知立场转移测试框架：通过目标性删除事件知识并验证，然后评估模型在无知状态下是否仍能重现社区的有机响应模式。使用俄乌军事讨论和美国党派推特数据进行实验。

Result: 即使经过激进的事实删除，对齐的LLMs仍保持稳定的、社区特定的不确定性处理行为模式。
...
TL;DR: 该论文构建了一个完全非语言学的文本模型，通过独立抽取有限字母和空格符号来研究文本结构。在仅考虑组合学和分割的情况下，推导出单词长度几何分布、词汇增长规律和Zipf型秩频分布。


<details>
  <summary>Details</summary>
Motivation: 为自然语言和大语言模型中的词汇统计提供一个结构化的零模型，探索Zipf模式是否仅从组合学和分割中产生，无需优化原则或语言组织。

Method: 使用有限字母表加空格符号的独立抽取模型，将单词定义为非空格符号的最大块。通过组合数学和优惠券收集论证推导结构结果。

Result: 单词长度服从几何分布；词汇增长和临界词长有闭式解；获得Zipf型秩频分布p(r) ∝ r^{-α}，指数由字母表大小和空格概率决定。

Conclusion: Zipf模式可以纯粹从组合学和分割中产生，无需语言学组织。该模型为区分需要深层解释的现象和随机文本结构现象提供了基础。
...
Result: 生成式LLM在某些应用中有潜力，但始终被人工编码员超越，有时甚至不如小型语言模型，需要人工验证来确定模型选择。

Conclusion: 支持方法论多元化方法，提出计算框架分析的路线图，建议研究人员利用这些方法的互补性进行协同使用。
...
Method: 引入PoETa v2基准套件，包含40多个葡萄牙语任务，评估20多个不同规模和计算资源的模型，并与英语任务进行性能差距分析。

Result: 研究揭示了计算投资和语言特定适应对葡萄牙语性能的影响，分析了与英语任务的性能差距。

Conclusion: PoETa v2为葡萄牙语语言建模和评估的未来研究奠定了基础，基准测试可在GitHub上获取。
...
TL;DR: 本文提出了一个可复现的流程，将Zoom公开录音转化为带说话者属性的转录本，包含人物档案和语用动作标签，并发布了三个地方政府审议数据集。使用这种"动作感知"数据微调LLM能显著提升模拟真实性和说话者保真度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以模拟多方审议，但由于缺乏说话者属性数据，现实建模受到限制。自动语音识别产生的匿名说话者标签无法捕捉一致的人类行为。

Method: 开发了一个可复现的流程，将公共Zoom录音转化为带说话者属性的转录本，包含人物档案和语用动作标签（如[propose_motion]）。发布了三个地方政府审议数据集，并使用这种"动作感知"数据微调LLM来建模特定参与者。

Result: 使用动作感知数据微调LLM使困惑度降低了67%，说话者保真度和真实性的分类器性能指标几乎翻倍。图灵式人类评估显示模拟结果常常与真实审议难以区分。

Conclusion: 该方法为复杂现实公民模拟提供了实用且可扩展的方法，能够产生高度逼真的多方审议模拟。
...
TL;DR: DeepDebater是一个能够参与并赢得完整政策辩论的自主AI系统，采用分层多智能体架构，使用大规模辩论证据库，支持AI对AI和AI对人类的全自动辩论。


<details>
  <summary>Details</summary>
Motivation: 解决AI在复杂、基于证据且具有战略适应性的说服能力方面的重大挑战，超越之前简化的辩论形式，实现完整的政策辩论。

Method: 采用分层多智能体工作流架构，LLM驱动的智能体团队协作执行离散论证任务，使用迭代检索、合成和自校正技术，基于大规模辩论证据库OpenDebateEvidence。

Result: 在初步评估中，DeepDebater产生质量更高的论证组件，在模拟轮次中持续获胜，专家辩论教练也偏好其构建的论点、证据和案例。

Conclusion: DeepDebater展示了AI在复杂政策辩论中的能力，支持全自动和混合人机操作，为AI说服能力的发展提供了重要进展。
...
TL;DR: 本文提出了一种基于保形预测的覆盖控制过滤框架，用于RAG系统中的上下文工程，能够在保持相关证据召回率的同时有效减少噪声内容。


<details>
  <summary>Details</summary>
Motivation: RAG系统在长文本或噪声上下文下准确性下降，现有预生成过滤器缺乏对保留证据的统计控制。

Method: 使用基于嵌入和LLM的评分函数，通过保形预测框架进行覆盖控制过滤，在NeuCLIR和RAGTIME数据集上进行测试。

Result: 保形过滤始终达到目标覆盖度，将保留上下文减少2-3倍，在严格过滤下ARGUE F1指标有所改善，在中等覆盖度下保持稳定。
...
TL;DR: 提出L2V-CoT方法，通过频率域提取和重采样LLMs的低频CoT表示，无需训练即可将推理能力迁移到VLMs，显著提升视觉语言模型的多步推理性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多步推理任务上表现不佳，现有方法需要高训练成本或架构对齐。研究发现LLMs和VLMs在低频CoT表示上具有相似性，为无需训练的推理迁移提供了可能。

Method: 使用线性人工层析技术验证LLMs和VLMs的低频CoT表示相似性，提出L2V-CoT方法在频率域提取LLMs的低频CoT表示，通过重采样实现维度匹配并注入到VLMs中。

Result: 实验表明L2V-CoT在无需训练的情况下超越现有基线方法，甚至优于有监督方法，在各种任务上表现一致优异。

Conclusion: LLMs和VLMs在低频CoT表示上的相似性为推理能力迁移提供了新途径，L2V-CoT方法实现了高效、无需训练的跨模态推理迁移。
...
TL;DR: 提出ELLA框架，通过LLM感知的关系分词器捕获复杂关系语义，使用跳级关系图变换器降低计算复杂度，并通过细粒度任务感知的CoT提示弥合预训练与微调任务间的语义鸿沟。


<details>
  <summary>Details</summary>
Motivation: 异质图中节点和关系类型的多样性导致复杂丰富的语义，现有方法受限于预定义语义依赖和监督信号稀缺。LLM具有强大的文本模态推理能力，但计算复杂度限制了其在异质图中的应用。

Method: 1. LLM感知关系分词器：利用LLM编码多跳多类型关系；2. 跳级关系图变换器：将LLM感知关系推理复杂度从指数级降至线性级；3. 细粒度任务感知CoT提示：弥合预训练与微调任务间的语义鸿沟。

Result: 在四个异质图上的实验表明，ELLA在性能和效率上优于最先进方法，可扩展到130亿参数LLM，相比现有基于LLM的方法实现4倍加速。

Conclusion: ELLA框架有效解决了异质图中复杂关系语义建模、计算复杂度和任务语义鸿沟问题，为LLM在异质图分析中的应用提供了高效解决方案。
...
TL;DR: SPINE是一个基于令牌选择的测试时强化学习框架，通过只更新高熵分支点令牌来避免传统测试时强化学习中的响应长度崩溃问题，在多个推理基准上稳定提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统测试时强化学习方法存在分布偏移和缺乏可验证监督的问题，基于自一致性投票的伪奖励方法容易导致多数投票奖励主导、响应缩短和性能下降的崩溃现象。

Method: SPINE框架：(i) 只更新分叉令牌（从前向传递统计中识别的高熵分支点）；(ii) 在这些令牌上应用熵带正则化器，在熵过低时维持探索，在熵过高时抑制噪声监督。可插入GRPO风格目标，无需标签或奖励模型。

Result: 在十个基准测试（多模态VQA、通用和专业QA、数学推理、医学QA）上，SPINE相比传统测试时强化学习方法持续提升Pass@1，避免响应长度崩溃，并在LLM和MLLM骨干网络上产生更稳定的训练动态。
...
TL;DR: 本文研究LLM幻觉检测，探索预训练数据覆盖度作为补充信号。通过构建RedPajama语料库的后缀数组获取n-gram统计信息，发现词汇覆盖特征与对数概率结合可提升幻觉检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要使用模型内部信号检测幻觉，但预训练数据暴露与幻觉之间的关系未被充分探索。本文旨在研究问题和生成答案的词汇训练数据覆盖度是否能提供额外的幻觉检测信号。

Method: 在RedPajama的1.3万亿token预训练语料库上构建可扩展后缀数组，检索提示和模型生成的n-gram统计信息，评估其在三个QA基准上的幻觉检测效果。

Result: 基于出现次数的特征单独使用时预测能力较弱，但与对数概率结合时能获得适度提升，特别是在模型内在不确定性较高的数据集上。

Conclusion: 词汇覆盖特征为幻觉检测提供了补充信号，特别是在模型不确定性较高的情况下能提升检测效果。
...
TL;DR: MTikGuard是一个用于TikTok有害内容检测的实时多模态系统，通过扩展数据集、多模态融合和流式架构，实现了89.37%的准确率和实时部署能力。


<details>
  <summary>Details</summary>
Motivation: TikTok作为儿童和青少年广泛使用的平台，存在大量有害内容，这些内容往往隐蔽且具有欺骗性，传统审核方法难以应对海量实时上传的挑战。

Method: 提出了MTikGuard系统，包含三个关键贡献：(1)扩展TikHarm数据集至4,723个标注视频；(2)集成视觉、音频和文本特征的多模态分类框架；(3)基于Apache Kafka和Apache Spark的可扩展流式架构。

Result: 系统实现了89.37%的准确率和89.45%的F1分数，在多模态有害内容检测方面达到最先进性能。

Conclusion: 研究表明，结合数据集扩展、先进的多模态融合和稳健部署，能够有效实现大规模社交媒体内容审核的实用化解决方案。
...
TL;DR: Blu-WERP是一种新颖的数据预处理管道，显著提升了Common Crawl WARC文件的质量，在多个模型规模和评估基准上优于现有基线方法DCLM和Fineweb。


<details>
  <summary>Details</summary>
Motivation: 现有预处理管道难以有效去除网络规模语料库中的噪声和非结构化内容，影响大语言模型的训练效果。

Method: Blu-WERP管道处理CC WARC数据转储，实施先进的过滤和质量评估机制，优化LLM训练数据质量。

Result: 在1B参数规模上，Blu-WERP相比DCLM和Fineweb分别实现了4.0%和9.5%的总体性能提升，在知识推理、语言理解和常识推理三个类别均有显著改进。

Conclusion: Blu-WERP代表了数据质量优化的实际进展，为研究人员和从业者提供了改善LLM训练效率和模型性能的有效解决方案。
...
TL;DR: 本研究构建了GeeSanBhava——一个高质量僧伽罗语歌曲评论数据集，通过三位独立标注者基于Russell的效价-唤醒模型手动标注，标注者间一致性高达84.96%。研究还开发了优化的多层感知器模型，在零样本测试中取得了0.887的ROC-AUC分数。


<details>
  <summary>Details</summary>
Motivation: 解决僧伽罗语自然语言处理中情感分析数据稀缺的问题，探索基于评论的情感映射在音乐情感识别中的重要性，并应对用户生成内容中固有的偏见挑战。

Method: 从YouTube提取僧伽罗语歌曲评论，三位独立标注者使用Russell的效价-唤醒模型进行手动标注。使用在僧伽罗语新闻评论数据集上预训练的机器学习和深度学习模型进行零样本测试，并通过超参数调优优化多层感知器模型。

Result: 标注者间一致性达到Fleiss kappa = 84.96%；优化的三层MLP模型（256、128、64神经元配置）在零样本测试中取得了0.887的ROC-AUC分数；分析揭示了不同歌曲的独特情感特征。

Conclusion: 本研究贡献了有价值的标注数据集，为僧伽罗语自然语言处理和音乐情感识别的未来研究提供了重要资源和见解。
...
TL;DR: 本文通过概念归纳头和标记归纳头识别LLaMA-2-7b模型激活中的语义和表面信息子空间，实现更准确的词向量算术运算。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs如何表示语义和表面级信息，并利用特定注意力头识别模型激活中的结构化子空间。

Method: 使用概念归纳头和标记归纳头的注意力权重转换隐藏状态，在转换后的空间中执行平行四边形算术运算。

Result: 概念头转换后的隐藏状态在平行四边形算术中达到80%的最近邻准确率，显著高于原始隐藏状态的47%。标记头转换可揭示表面级词信息。

Conclusion: 特定类型的注意力头可以识别模型激活中的语义和表面信息子空间，这些子空间具有更清晰的结构化表示能力。
...
TL;DR: 本文首次系统比较了基于向量的智能RAG与分层节点系统在金融文档问答中的表现，发现向量RAG在检索准确率和答案质量上显著优于节点系统，交叉编码器重排和小到大块检索技术能进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对金融文档中向量与非向量RAG架构的系统比较，以及高级RAG技术对检索准确性、答案质量、延迟和成本影响的实证分析。

Method: 评估基于向量的智能RAG（使用混合搜索和元数据过滤）与分层节点系统（无需嵌入遍历文档结构），并测试交叉编码器重排和小到大块检索两种增强技术。

Result: 向量RAG在1200份SEC文件上的150个问题基准测试中，以68%胜率优于节点系统；交叉编码器重排在最优参数下MRR@5提升59%；小到大块检索以65%胜率优于基线分块，仅增加0.2秒延迟。
...
<details>
  <summary>Details</summary>
Motivation: 现有代理、MCP和检索方法通常只基于单个代理描述匹配查询，掩盖了每个代理的细粒度工具能力，导致代理选择不理想

Method: 使用知识图谱检索增强生成方法，将工具及其父代理表示为知识图谱中的节点和边，通过向量搜索、类型特定的加权互逆排名融合重排序，以及知识图谱遍历来检索代理

Result: 在LiveMCPBenchmark上，Recall@5和nDCG@5分别提升了14.9%和14.6%，wRRF优化提升了2.4%

Conclusion: Agent-as-a-Graph检索方法通过细粒度工具能力表示和知识图谱遍历，显著改进了大规模语言模型多代理系统中的代理选择效果
...
TL;DR: DiscoVerse是一个多智能体协同科学家系统，用于支持药物研发，通过语义检索、跨文档链接和可审计合成来处理罗氏公司的大型历史数据档案，在真实药物研发数据上展示了良好的性能。


<details>
  <summary>Details</summary>
Motivation: 药物研发积累了大量的异构数据档案，其中许多来自已终止的研究项目。重新利用这些档案对于反向转化研究非常有价值，但在实践中往往不可行。

Method: 开发了DiscoVerse多智能体系统，实现语义检索、跨文档链接和可审计合成，在罗氏公司超过40年研究数据（涵盖180个分子、8.7亿BPE标记）上进行验证。

Result: 在7个基准查询中，DiscoVerse实现了接近完美的召回率（≥0.99）和中等精度（0.71-0.91），对终止原因和器官特异性毒性的定性评估显示出跨临床前和临床证据的忠实、源链接合成。

Conclusion: 这是第一个在真实药物研发数据上系统评估的反向转化智能体框架，展示了有希望的答案准确性和决策洞察力，为药物研发提供了有效的人类参与循环支持。
...
TL;DR: 本文提出了一个数据中心的幻觉检测方法，通过整合和平衡五个现有数据集创建了124,821个样本的训练语料，在SHROOM-CAP 2025多语言科学文本幻觉检测任务中取得了竞争性表现。
...
Method: 采用数据中心策略，统一并平衡五个现有数据集创建大规模训练语料，然后基于增强数据集对XLM-RoBERTa-Large模型进行微调。

Result: 在9种语言上均取得竞争性表现，其中古吉拉特语（零样本语言）获得第二名（Factuality F1为0.5107），其余8种语言排名4-6位。

Conclusion: 系统化的数据管理策略能够显著超越单纯的架构创新，特别是在零样本设置下的低资源语言场景中。
...
TL;DR: 本文比较了两种从建筑规范表格数据中提取信息的方法：直接输入图像到视觉语言模型(VLMs)和间接输入(将表格转换为LaTeX代码)，发现直接方法更准确，且通过LoRA微调能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 建筑规范包含关键安全信息，但其中的表格数据因复杂布局难以自动提取。需要开发有效的问答系统来快速准确获取规范条款。

Method: 比较两种方法：1) 直接输入页面图像到预训练VLMs；2) 间接输入(图像转LaTeX代码)。使用LoRA对VLMs进行参数高效微调。

Result: 直接输入方法准确率更高，微调后Qwen2.5-VL-3B-Instruct相对准确率提升超过100%。

Conclusion: 参数高效微调方法能有效适配强大的VLMs，用于理解建筑规范等专业领域的复杂结构化数据。
...
TL;DR: 提出路径约束检索(PCR)方法，通过结合图结构约束和语义搜索，确保检索信息在知识图谱中保持逻辑关系，提高LLM代理推理的可靠性
...
Method: PCR方法将搜索空间限制在从锚节点可达的节点范围内，结合语义搜索进行检索，防止检索到结构上断开的信息

Result: 在PathRAG-6基准测试中，PCR实现100%结构一致性，而基线方法只有24-32%；在技术领域，PCR在排名10时获得完全相关性和结构一致性；平均图距离比基线减少78%
...
<details>
  <summary>Details</summary>
Motivation: 针对低资源孟加拉语仇恨言论检测场景，需要开发能够有效处理多任务分类的模型，以改善在YouTube评论中的仇恨言论识别效果。
...
Conclusion: 该方法在低资源孟加拉语仇恨言论场景中表现出良好的泛化能力，并对误分类模式进行了详细分析，为相关研究提供了有价值的见解。
...
TL;DR: 该论文提出了OmniStruct基准测试，用于评估LLM在文本到结构任务上的能力，并通过合成数据训练小模型达到GPT-4o水平。


<details>
  <summary>Details</summary>
Motivation: 虽然现代LLM在生成非结构化自然语言响应方面表现出色，但它们在文本到结构任务（如信息提取、表格生成和函数调用）上的性能仍不明确，需要系统评估。

Method: 构建OmniStruct基准测试，整合现有数据集到统一的文本到结构问题设置中，并通过合成任务生成收集高质量训练数据，用这些数据微调小模型。

Result: 实验表明，仅使用合成数据而不使用OmniStruct任务的监督数据，就能训练出与GPT-4o性能相当的通用结构化生成模型。

Conclusion: 通过合成数据训练小模型可以有效地实现强大的文本到结构生成能力，为开发高效的结构化生成模型提供了可行路径。
...
TL;DR: 研究发现假新闻分享集中在少数高度政治化的用户群体中，这些用户通过高活跃度影响政治议程；用户对不确定信息表现出不同形式的批判距离，但很少产生真正的协商辩论，反而导致小范围活跃群体的无效对话。


<details>
  <summary>Details</summary>
Motivation: 解决两个悖论：为什么假新闻在缺乏编辑控制的情况下仍只占社交媒体信息的小部分，以及为什么用户对假新闻不特别敏感但政治极化却在加剧。

Method: 采用混合方法设计，结合数字痕迹的定量分析、在线观察和访谈，在Twitter和Facebook上开展两项互补研究，考察不同互动情境下的多样化实践。

Result: 1. 假新闻分享集中在少数高度政治化、不信任制度的用户群体；2. 用户根据社会地位和互动情境采取不同批判距离形式；3. 这些批判形式很少产生真正辩论，反而导致小范围无效对话。

Conclusion: 假新闻的影响主要通过少数高度活跃用户的议程设置能力实现，而非广泛传播；用户对可疑信息的反应受社会位置和互动情境影响，但很少促进真正的公共讨论。
...
Conclusion: 分层思维链策略能有效提升LLM在临床决策支持系统中的鲁棒性和公平性，为可靠使用AI辅助决策提供了解决方案。
...
TL;DR: 该论文介绍了基于Mechanistic Interpretability Benchmark (MIB)的BlackboxNLP 2025共享任务，包含电路定位和因果变量定位两个赛道，展示了多种方法在机制可解释性评估中的表现。


<details>
  <summary>Details</summary>
Motivation: 机制可解释性(MI)研究面临进展衡量困难的问题，需要标准化评估框架来比较不同技术方法。

Method: 基于MIB基准构建共享任务，包含电路定位(识别影响模型行为的因果组件和交互)和因果变量定位(将激活映射为可解释特征)两个赛道，采用集成、正则化、低维和非线性投影等方法。

Result: 在电路定位中，8种方法通过集成和正则化策略取得显著进展；在因果变量定位中，2种方法通过低维和非线性投影实现显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏大规模、包含推理和工具调用的阿拉伯语多轮对话数据集，而简单的翻译方法在后期训练阶段质量不足，需要更严格的数据集构建方法。

Method: 使用多模型集成翻译管道，应用质量过滤，并通过消融研究检验对传统仅解码器模型有效的翻译技术。

Result: 成功构建了SmolKalam阿拉伯语数据集，改进了翻译质量。

Conclusion: 通过多模型集成翻译和质量过滤，可以创建高质量的阿拉伯语对话数据集，满足后期训练的需求。
...
TL;DR: 提出了多智能体协同过滤框架MACF，将传统协同过滤算法类比为基于LLM的多智能体协作，通过动态智能体招募和个性化协作指令来提升推荐效果
...
Method: 将相似用户和相关物品实例化为具有独特配置文件的LLM智能体，每个智能体可调用检索工具、推荐候选物品并与其他智能体交互，通过中央编排器智能体动态管理智能体间的协作

Result: 在三个不同领域的数据集上实验结果表明，MACF框架相比强大的智能体推荐基线具有优势
...
TL;DR: 提出了一种名为GAM的通用代理记忆框架，采用即时编译原则，在运行时为客户端创建优化上下文，离线阶段仅保留简单但有用的记忆。
...
Result: 在各种基于记忆的任务完成场景中，相比现有记忆系统实现了显著改进。

Conclusion: GAM框架有效利用了前沿大语言模型的代理能力和测试时扩展性，同时通过强化学习促进端到端性能优化。
...
TL;DR: 提出了MindEval框架，用于自动评估语言模型在真实多轮心理健康治疗对话中的表现，填补了现有基准测试的空白。


<details>
  <summary>Details</summary>
Motivation: 当前AI心理健康聊天机器人存在奉承、过度验证和强化不良信念等问题，而现有基准测试要么只通过选择题测试临床知识，要么孤立评估单个回复，无法捕捉真实治疗互动的复杂性。

Method: 与博士级持证临床心理学家合作设计MindEval框架，通过患者模拟和LLM自动评估，在完全自动化的模型无关设计中平衡抗游戏性和可重复性。

Result: 评估了12个最先进的LLM，所有模型平均得分低于4分（满分6分），在AI特有的问题沟通模式方面表现尤其薄弱。推理能力和模型规模不能保证更好表现，系统在较长交互或支持严重症状患者时表现恶化。

Conclusion: 当前LLM在心理健康治疗对话中表现不佳，需要专门改进来避免有害的沟通模式，特别是在处理严重症状和长时间对话时。
...
TL;DR: 文学学者必须参与大型语言模型可解释性研究，尽管这可能涉及意识形态斗争或妥协，但这是必要的，因为当前可解释性方法的工具性不应成为衡量LLM解释的唯一标准。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型可解释性研究过于注重工具性，缺乏文学解释的深度和复杂性，文学学者需要参与其中以丰富解释标准。

Method: 提出通过参与红队测试作为文学学者参与LLM可解释性研究的实践场所。

Result: 论证了文学学者参与LLM可解释性研究的必要性和可行性。

Conclusion: 文学学者必须积极介入LLM可解释性研究，以挑战当前过于工具化的解释标准，红队测试可作为重要的实践平台。
...
TL;DR: 为低资源语言班巴拉语收集了612小时自发语音数据，创建了半自动标注的数据集，开发了单语超紧凑和小型模型，并进行了自动和人工评估。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏语音数据集、模型和评估框架，需要建立相关经验基础。

Method: 现场收集班巴拉语自发语音，半自动标注转录，创建单语超紧凑和小型模型，进行自动和人工评估。

Result: 建立了612小时班巴拉语语音数据集，开发了多个模型，提供了数据收集协议、标注和模型设计的实用建议。

Conclusion: 证明了人工评估的重要性，并公开了数据集、评估集、模型和代码。
...
TL;DR: GPT-4o作为编程题目难度评估器的表现远不如基于特征工程的LightGBM模型，准确率仅37.75% vs 86%，且存在明显的简单类别偏见。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在结构化任务（如编程题目难度预测）中的表现，验证其作为自动评估器的可靠性。

Method: 系统比较GPT-4o（纯自然语言评估）与LightGBM集成模型（基于数值和文本特征），在1,825个LeetCode题目数据集上进行测试，并使用SHAP进行可解释性分析。

Result: LightGBM达到86%准确率，GPT-4o仅37.75%；GPT-4o忽视数值约束（如输入大小限制、通过率），偏向简单分类；在自生成难题测试中，GPT-4o将大部分自创难题标注为中等难度。

Conclusion: LLM作为编程题目难度评估器存在严重局限性，在竞争性编程、教育平台等应用中需解决其具体失败模式才能被视为可信赖的评估工具。
...
TL;DR: 提出了一个系统性基准测试，评估LLM在零样本设置下预测个体对各种话题立场的泛化能力，发现提供更多背景信息能提高准确性，但性能在不同信念领域差异显著。
...
Method: 使用在线辩论平台数据构建可复现基准测试，包含多个信息条件来分离人口统计背景和已知先验信念对预测成功的贡献，在零样本设置下评估多个中小型模型。

Result: 提供更多个体背景信息能提高预测准确性，但模型在不同信念领域的表现差异很大，显示了当前LLM模拟人类推理的能力和局限性。

Conclusion: 研究揭示了当前LLM模拟人类推理的容量和局限性，为超越社会政治领域的信念系统建模提供了可扩展框架，推动了机器行为研究。
...
<details>
  <summary>Details</summary>
Motivation: 报纸是重要的信息来源，但有效浏览大量新闻内容具有挑战性。新闻标题的情感分析能帮助快速理解新闻的情感基调，特别是在孟加拉语这种低资源语言中。

Method: 使用BERT-CNN-BiLSTM混合迁移学习模型，在9014条孟加拉语新闻标题数据集上应用两种实验策略：技术1（采样前拆分）和技术2（采样后拆分）。

Result: 技术1中过采样在标题和情感分类上分别达到78.57%和73.43%的最高性能；技术2中直接在原始不平衡数据集上训练分别达到81.37%和64.46%。

Conclusion: BERT-CNN-BiLSTM模型显著优于所有基线模型，为孟加拉语文本分类在低资源环境下提供了强基准，证明了同时利用标题和情感数据集的重要性。
...
TL;DR: 将提示优化建模为状态空间搜索问题，通过束搜索和随机游走算法在提示图中探索优化路径，在多个NLP任务上实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 语言模型对输入提示字符串的微小变化极其敏感，现有方法如DSpy通过基于演示的提示优化避免性能崩溃问题。本文受此启发，提出将提示优化视为经典状态空间搜索问题的替代方法。

Method: 将提示空间建模为图结构，节点代表提示状态，边对应有意的转换操作（如缩短、添加示例、重新排序内容）。使用束搜索和随机游走算法系统探索该空间，在开发集上评估候选提示并剪枝无希望的路径。

Result: 在五个NLP任务（情感分类、问答、摘要、推理和自然语言推理）上，即使浅层搜索配置（束宽=2，深度=2）也能在开发集上改进种子提示。例如，束搜索在推理任务上实现了从0.40到0.80的开发准确率提升，但测试集改进较为温和（0.20到0.50），表明对开发启发式存在过拟合。
...
TL;DR: OpenGloss是一个合成的英语百科全书词典和语义知识图谱，整合了词典定义、百科全书背景、词源历史和语义关系，包含53.7万个词义和910万条语义边，通过多智能体生成管道在不到一周时间内以低于1000美元的成本创建。


<details>
  <summary>Details</summary>
Motivation: 解决现有词典资源在词汇学习和自然语言处理任务中的内容整合不足问题，展示结构化生成可以以远低于人工编辑的成本和时间创建全面的词汇资源。

Method: 使用多智能体程序生成管道，通过模式验证的LLM输出和自动化质量保证，生成包含定义、示例、搭配、百科全书内容和词源的统一资源。

Result: 创建了包含53.7万个词义、150万个词条、910万条语义边、100万个使用示例、300万个搭配和6000万字百科全书内容的资源，规模与WordNet 3.1相当但定义数量多四倍。

Conclusion: 结构化生成能够在成本和时间上实现人工编辑难以达到的词汇资源创建，为研究人员和教育工作者提供了可公开获取的CC-BY 4.0许可资源。
...
TL;DR: 该研究探讨了针对特定偏见进行缓解时产生的跨类别后果，发现虽然目标偏见可能减少，但经常在其他维度产生负面意外后果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型从训练数据中继承了社会偏见，可能导致有害或不公平的输出。现有偏见缓解技术通常只评估目标偏见维度，而忽略了跨类别影响。

Method: 研究了四种偏见缓解技术，应用于来自七个模型家族的十个模型，探索种族、宗教、职业和性别相关偏见，使用StereoSet基准评估模型连贯性和刻板印象偏好。

Result: 结果显示，针对性缓解有时能减少目标维度的偏见，但经常在其他维度产生意外负面后果，如增加模型偏见和降低整体连贯性。

Conclusion: 这些发现强调了在检查和开发偏见缓解策略时，需要强大的多维度评估工具，以避免无意中在未目标轴上转移或加剧偏见。
...
TL;DR: 本研究系统评估了大型语言模型在2026年韩国高考数学考试中的表现，建立了完全无数据泄露的评估环境，发现GPT-5 Codex获得满分，几何是最薄弱领域，增强推理强度可提升性能但显著降低效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有基准测试中的数据泄露问题，在考试公开后两小时内数字化所有题目，确保评估环境完全无污染，系统评估LLMs在不同输入模态和提示语言下的数学推理能力。

Method: 使用2026年韩国高考数学考试的46道题目，在考试公开后两小时内完成数字化，评估24个先进LLMs在不同输入模态（文本、图像、文本+图形）和提示语言（韩语、英语）下的表现，并进行推理增强实验。

Result: GPT-5 Codex获得唯一满分（100分），Grok 4、GPT-5和Deepseek R1得分超过95分；几何领域表现最弱（平均77.7%）；文本输入优于图像输入；增强推理强度可提升性能但显著增加计算成本。

Conclusion: 建立了完全无数据泄露的评估环境，提供了基于真实考试的LLM评估框架，建议在性能、成本和时间之间平衡考虑，最小化推理的模型可能更具实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法存在长上下文处理和检索-生成模块分离优化的问题，需要统一框架来提升效率和质量。

Method: 提出CLaRa框架，使用SCP数据合成框架生成语义丰富的压缩向量，通过可微分top-k估计器端到端训练重排序器和生成器。

Result: 在多个QA基准测试中，CLaRa实现了最先进的压缩和重排序性能，经常超越基于文本的微调基线。

Conclusion: CLaRa通过统一连续空间优化有效对齐检索相关性和答案质量，为检索增强生成提供了更高效的解决方案。
...
TL;DR: ECN是一个多阶段提示框架，通过四个阶段增强大语言模型的共情能力和包容性，在GPT模型上取得了最高的共情商数得分。


<details>
  <summary>Details</summary>
Motivation: 提升大语言模型在对话AI中的共情能力和包容性，使其能够生成更具情感共鸣和上下文感知的回应。
...
TL;DR: RhinoInsight是一个深度研究框架，通过添加可验证检查表和证据审计两个控制机制，增强LLM在深度研究任务中的鲁棒性、可追溯性和质量，无需参数更新。


<details>
  <summary>Details</summary>
Motivation: 现有系统采用从规划到搜索再到写作的线性流程，存在错误积累和上下文腐化问题，缺乏对模型行为和上下文的显式控制。

Method: 1) 可验证检查表模块：将用户需求转化为可追溯的子目标，通过人工或LLM批评者进行优化，生成分层大纲；2) 证据审计模块：结构化搜索内容，迭代更新大纲，修剪噪声上下文，通过批评者对证据进行排序和绑定。

Result: 实验表明RhinoInsight在深度研究任务上达到最先进性能，在深度搜索任务上保持竞争力。

Conclusion: RhinoInsight通过添加控制机制有效解决了现有系统的局限性，提高了深度研究任务的质量和可靠性。
...
TL;DR: 评估15个主流LLM在6000多个事实核查任务上的表现，发现标准模型表现差，推理能力帮助有限，网络搜索仅提供中等改进，而使用高质量上下文信息的RAG系统显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着主流聊天机器人配备推理能力和网络搜索工具，数百万用户依赖它们进行事实核查，急需对这些系统的性能进行严格评估。

Method: 评估15个来自OpenAI、Google、Meta和DeepSeek的最新LLM，在6000多个PolitiFact事实核查声明上比较标准模型、推理变体和网络搜索变体的表现。

Result: 标准模型表现不佳，推理能力提供有限帮助，网络搜索仅带来中等改进，而使用PolitiFact摘要的RAG系统将宏F1平均提高了233%。

Conclusion: 为模型提供经过筛选的高质量上下文信息是自动化事实核查的有前景路径，而非单纯依赖推理能力或网络搜索。
...
TL;DR: 提出DRF方法用于图像-文本对的多模态情感分析，通过特征分布建模同时处理低质量和缺失模态问题，在多种现实场景下实现鲁棒性能
...
Method: 为每个模态维护特征队列来近似特征分布，基于分布定量估计模态质量来降低低质量模态的贡献，通过样本和分布监督建立模态间映射关系来恢复缺失模态

Result: 在三个公开图像-文本数据集上的实验表明，DRF相比SOTA方法在两种破坏策略下均实现普遍改进
...
TL;DR: 提出基于上下文提示的策略，无需重新训练即可适配Whisper模型用于阿拉伯语语音识别，显著降低词错误率


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语等低资源语言的语音识别问题，特别是处理方言变体和有限标注数据的挑战
...
Conclusion: 上下文感知提示策略能有效提升低资源语言的语音识别性能，特别是在零样本设置下
...
TL;DR: HyperbolicRAG是一个检索增强生成框架，通过双曲几何改进基于图的RAG，更好地捕捉知识图谱中的层次结构关系


<details>
  <summary>Details</summary>
Motivation: 现有的基于图的RAG方法依赖欧几里得嵌入，虽然能捕捉语义相似性，但缺乏对层次深度的几何表示，限制了表示复杂知识图谱中抽象关系的能力

Method: 提出三个关键设计：1) 在共享Poincare流形中的深度感知表示学习器；2) 无监督对比正则化确保跨抽象层次的几何一致性；3) 互排名融合机制联合利用欧几里得和双曲空间的检索信号

Result: 在多个QA基准测试上的广泛实验表明，HyperbolicRAG优于包括标准RAG和图增强基线在内的竞争基线

Conclusion: 双曲几何能够有效改进图基RAG，同时捕捉细粒度语义和全局层次结构，提升检索性能
...
TL;DR: 提出基于抽象意义表示(AMR)图的无监督上下文压缩框架，通过计算节点级概念熵来保留核心语义信息，在减少上下文长度的同时提高问答准确性。
...
Method: 构建AMR图，计算每个节点的概念熵来评估其语义重要性，筛选重要信息节点形成压缩的语义聚焦上下文。
...
TL;DR: 提出了一个用于焦点小组讨论文本的计算分析框架，通过BERTopic模型解决超参数敏感性、模型稳定性和可解释性验证等挑战，在HPV疫苗认知研究中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的焦点小组数据分析依赖人工编码，存在可扩展性和可重复性限制，需要开发计算框架来克服这些局限。

Method: 使用BERTopic对10个焦点小组的1076条话语进行分析，系统评估27种超参数配置，通过30次自举重采样评估稳定性，并由3名领域专家进行人工验证。

Result: 发现模型对超参数选择敏感，分层合并策略（先提取细粒度主题评估稳定性，再合并提高可解释性）有效平衡稳定性与连贯性，人工验证显示主题质量良好（ICC=0.79）。

Conclusion: 该框架为定性研究提供了实用的计算分析指南，所有代码和协议已公开以支持复现和扩展。
...
TL;DR: 本研究利用大型语言模型（Mistral和mT5）和翻译方法，在捷克语文本摘要任务上取得了最先进的结果，并创建了首个历史捷克语摘要数据集。
...
Method: 使用多语言大型语言模型（Mistral和mT5）直接进行捷克语摘要，并提出翻译方法：先将捷克文本翻译成英语，用英语模型摘要，再翻译回捷克语。

Result: 在SumeCzech数据集上达到新的最先进结果，创建了Posel od Čerchova历史捷克语摘要数据集，并为该领域提供了初步基准。

Conclusion: 本研究为捷克语摘要研究奠定了基础，为捷克历史文档处理和低资源语言摘要提供了宝贵资源。
...
TL;DR: 提出CogAlpha框架，结合代码级alpha表示、LLM驱动推理和进化搜索，用于在金融数据中发现具有更好预测准确性、鲁棒性和泛化能力的alpha因子。


<details>
  <summary>Details</summary>
Motivation: 现有方法（深度学习、遗传编程、LLM因子生成）在探索广阔的alpha搜索空间时存在局限：神经网络模型产生不透明和脆弱的模式，符号或公式方法产生冗余或经济意义不足的表达式，泛化能力差。需要一种能够平衡逻辑一致性和创造性跳跃的探索方法。

Method: CogAlpha框架将LLM作为自适应认知代理，通过多阶段提示和金融反馈迭代地精炼、变异和重组alpha候选因子。结合代码级alpha表示、LLM驱动推理和进化搜索。

Result: 在A股股票上的实验表明，CogAlpha能够持续发现具有优越预测准确性、鲁棒性和泛化能力的alpha因子，优于现有方法。

Conclusion: 将进化优化与基于LLM的推理相结合，有望实现自动化和可解释的alpha发现。所有源代码将发布。
...
TL;DR: FanarGuard是一个双语内容审核过滤器，专注于评估阿拉伯语和英语内容的安全性和文化对齐性，在文化意识评估方面表现优于人类标注者间的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有内容审核过滤器主要关注通用安全性而忽视文化背景，特别是在阿拉伯语等非英语语境中缺乏文化敏感性评估。

Method: 构建包含46.8万条提示-响应对的数据集，由LLM评委评估无害性和文化意识，训练两个过滤器变体，并开发首个针对阿拉伯文化背景的基准测试。

Result: FanarGuard在文化对齐性评估中与人类标注的一致性优于标注者间可靠性，同时在安全性基准测试中达到最先进过滤器的性能水平。
...
TL;DR: 提出了一种名为RCEG的LLM框架，用于自动生成高质量的个性化英语阅读理解练习，通过微调LLM生成候选内容，使用鉴别器选择最佳内容，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，在教育领域特别是自动文本生成方面展现出巨大潜力，需要开发能够创建智能自适应学习内容的系统。

Method: RCEG框架首先使用微调的大语言模型生成内容候选，然后通过鉴别器选择最佳候选内容，最后大幅提升生成内容的质量。
...
TL;DR: 本文首次研究大型推理模型(LRMs)的剪枝技术，发现直接应用现有剪枝方法效果不佳，提出使用自生成推理数据进行校准可显著提升剪枝性能，并开发了选择性自生成推理(SSGR)数据构建策略。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但其长链思维推理过程带来高昂的推理成本。剪枝是减少计算成本的有效方法，但现有研究主要关注大语言模型(LLMs)，LRMs的剪枝尚未被探索。

Method: 提出选择性自生成推理(SSGR)数据构建策略，通过分析推理数据的难度和长度对剪枝效果的影响，发现具有挑战性和中等长度的自生成推理数据是最佳校准数据。
...
TL;DR: CoreEval是一种抗污染评估策略，通过从GDELT数据库获取最新知识来更新数据，解决LLM评估中的数据污染问题。


<details>
  <summary>Details</summary>
Motivation: 数据污染在LLM评估中造成不公平，现有方法无法完全消除模型中的已有知识或保持原始数据集的语义复杂性。

Method: 从原始数据提取实体关系，利用GDELT数据库检索最新知识，重新语境化并整合到原始数据中，通过迭代验证机制确保标签一致性。

Result: 在更新数据集上的广泛实验验证了CoreEval的鲁棒性，有效缓解了数据污染导致的性能高估问题。

Conclusion: CoreEval提供了一种有效的抗污染评估方法，能够生成语义复杂且与任务相关的更新数据。
...
TL;DR: 本研究复现了LLAMBO框架，使用Llama 3.1 70B替代GPT-3.5，证实了该框架在贝叶斯优化中的有效性。文本上下文显著改善早期遗憾表现，语言模型诱导的跨任务语义先验提升了性能。


<details>
  <summary>Details</summary>
Motivation: 验证LLAMBO框架在不同语言模型骨干下的鲁棒性，特别是使用开源模型Llama 3.1 70B替代GPT-3.5后的表现。

Method: 在原始评估协议下复现核心实验，使用Llama 3.1 70B模型替换所有文本编码组件，进行消融实验分析各组件贡献。

Result: LLAMBO架构对语言模型骨干更换具有鲁棒性，文本上下文显著改善早期遗憾和方差，判别代理模型受益于跨任务语义先验，候选采样器生成更高质量和多样化的提议。

Conclusion: LLAMBO框架在更换语言模型骨干后仍保持有效性，文本上下文是关键组件，较小模型容量不足导致预测不稳定。
...
TL;DR: 评估大型语言模型使用网络搜索的必要性和有效性，发现网络搜索能显著提高事实准确性但存在过度自信、查询质量差等问题


<details>
  <summary>Details</summary>
Motivation: 评估商业大语言模型是否能在需要时有效使用网络搜索，以及其搜索调用的校准程度

Method: 创建包含静态和动态问题的基准数据集，静态问题测试模型基于内部置信度调用搜索的能力，动态问题测试模型识别何时需要搜索并检索更新信息的能力

Result: 网络搜索显著提高了GPT-5-mini和Claude Haiku 4.5的静态准确率，但置信度校准变差；动态查询中模型频繁调用搜索但准确率低于70%，主要由于查询质量差

Conclusion: 内置网络搜索能有效提高事实准确性并可作为低延迟验证层，但模型存在过度自信、在需要时跳过检索、初始查询失败后表现不佳等问题，仍有改进空间
...
TL;DR: 本文提出了Text-to-Query任务范式，统一了不同查询语言的语义解析任务。通过识别查询骨架作为共享优化目标，提出了动态数据增强框架，仅用少量合成数据就在四个基准测试中达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常只关注单一查询语言，导致方法在不同语言间的泛化能力有限。需要开发一个统一的方法来处理多种查询语言的语义解析任务。

Method: 提出了动态数据增强框架，通过显式诊断模型在处理查询骨架时的特定弱点来合成针对性训练数据。将查询骨架识别为Text-to-Query任务的共享优化目标。

Result: 在四个Text-to-Query基准测试上的实验表明，该方法仅使用少量合成数据就达到了最先进的性能，证明了方法的效率和通用性。

Conclusion: 该方法为Text-to-Query任务的统一研究奠定了坚实基础，通过查询骨架的共享优化和针对性数据增强，实现了跨查询语言的高效语义解析。
...
Method: 使用Safeterm知识层将MedDRA不良事件术语自动重新分组为相似性簇，计算治疗特异性不成比例性指标，并通过精度加权聚合推导簇级EBGM值。

Result: 应用于三个历史试验，自动化方法清晰恢复了所有预期的安全性信号，验证了方法的有效性。
...
TL;DR: 提出了一种补充自然语言的情感表达形式——"矛盾结构效应"，通过动态的蒙太奇操作产生"结构效应"，并引入德勒兹的"强度"概念作为模型要素。


<details>
  <summary>Details</summary>
Motivation: 为情感表达提供一种独立于自然语言的替代形式，作为情感状态的代理或窗口，弥补自然语言在情感表达上的局限性。

Method: 建立"矛盾结构效应"的表达形式，通过蒙太奇操作叠加多个效应，构建包含相关效应的"结构"，并引入德勒兹的"强度"概念作为模型要素。

Result: 提出了一个通用的理论框架——系统间词语导入，并通过奥斯汀的"力量"一词的使用来证明"强度"导入的合理性。

Conclusion: 该模型为情感表达提供了一种新的理论框架，能够动态地捕捉和处理复杂的情感状态，在教育升级等场景中得到验证。
...
TL;DR: GraphMind是一个新颖的动态图框架，将图神经网络与LLMs结合，通过建模推理过程为异构演化图来改进多步推理能力，在多个QA数据集上表现优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏明确动态机制来结构化和演化中间推理状态，限制了上下文感知定理选择和迭代结论生成的能力。
...
Method: KDR-Agent多智能体框架，包含知识检索、消歧和反思分析三个专门智能体，使用自然语言类型定义和静态实体级对比示例，减少对大规模标注语料的依赖。

Result: 在5个领域的10个数据集上实验表明，KDR-Agent在多个LLM骨干网络上显著优于现有的零样本和少样本上下文学习基线方法。

Conclusion: KDR-Agent通过整合外部知识和结构化自我评估，有效解决了低资源场景下NER的挑战，为多领域低资源实体识别提供了新思路。
...
TL;DR: DeCoRL是一个新的强化学习框架，通过将推理过程从顺序处理转变为协作式模块化编排，解决了现有CoT推理方法在奖励信号不明确和顺序解码效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在链式思维推理中存在两个关键限制：一是作为黑盒提供无差别的奖励信号，难以诊断错误；二是顺序解码的时间复杂度为O(n)，在复杂推理任务中实时部署不实用。

Method: DeCoRL训练轻量级专用模型并行生成推理子步骤，通过并行处理消除顺序瓶颈。设计了模块化奖励函数独立评分每个子步骤，并使用级联DRPO优化协调这些奖励同时保持步骤间依赖关系。

Result: 在RM-Bench、RMB和RewardBench上的综合评估显示，DeCoRL实现了最先进的结果，优于包括大规模模型在内的现有方法。推理速度提升3.8倍，可解释性提高22.7%，能耗降低72.4%，吞吐量增加68%。

Conclusion: DeCoRL通过并行处理、模块化奖励和级联优化，实现了推理速度、可解释性和能效的显著提升，使复杂推理系统的实时部署成为现实。
...
TL;DR: 提出了一种用于纳瓦特尔语文本自动正字法统一的符号模型，基于先前开发的句子分析算法和π-yalli语料库，通过符号正则表达式实现语言规则，并设计了人工评估协议来测试统一句子的语义质量。


<details>
  <summary>Details</summary>
Motivation: 纳瓦特尔语存在多种正字法体系，导致文本处理困难，需要开发自动统一方法来标准化不同正字法变体，便于语言处理和分析。

Method: 基于符号正则表达式实现语言规则，利用π-yalli多正字法语料库，开发自动统一算法，并设计人工评估协议通过语义任务测试统一质量。

Result: 评估者对于大多数期望特征给出了令人鼓舞的结果，表明自动统一算法在语义保持方面表现良好。

Conclusion: 所提出的符号模型能够有效实现纳瓦特尔语文本的正字法统一，为多正字法语言处理提供了可行解决方案。
...
TL;DR: 本文提出了一个信息理论框架来分析自然语言命名系统，证明了当听者的解码器等同于说话者的贝叶斯解码器时，才能实现信息丰富性与复杂度的最优权衡。


<details>
  <summary>Details</summary>
Motivation: 解决先前研究的两个简化假设：(i) 最优听者和(ii) 跨语言通用交际需求，为离散对象命名系统建立更现实的信息理论框架。

Method: 采用从涌现通信中借鉴的指称游戏设置，专注于亲属关系语义领域，构建信息理论框架来分析命名系统。

Result: 理论证明最优权衡是可实现的，并且在学习的通信系统中经验性地涌现出来。

Conclusion: 当听者的解码器与说话者的贝叶斯解码器等价时，命名系统能够在信息丰富性和复杂度之间达到最优平衡，这一理论结果在实际学习系统中得到了验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有ACSA方法主要关注情感极性，忽略了塑造情感表达的基础情感维度，这限制了模型捕捉面向特定方面类别的细粒度情感信号的能力。

Method: 利用LLMs的生成能力，为每个方面类别生成情感描述，并基于VAD维度框架引入情感精炼机制，将LLM预测的情感投影到VAD空间，对不一致的情感进行重新标注。

Result: 实验结果表明，该方法在所有基准数据集上显著优于强基线模型。

Conclusion: 将情感维度整合到ACSA中是有效的，情感增强的多任务框架能够丰富情感表示并提升分析性能。
...
TL;DR: 提出一种基于概率条件生成的新方法，通过优化隐藏状态来激发基础大语言模型的思维链推理能力，在保持语言连贯性的同时提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 基础大语言模型在预训练后往往缺乏专门的推理训练，现有的隐藏状态操纵方法存在刚性约束、分布偏移和文本质量下降等问题。

Method: 将挑战重新表述为带有平衡似然和先验正则化的优化问题，通过概率条件生成引导隐藏状态朝向推理导向的轨迹。
...
TL;DR: 该论文研究了大型语言模型在表示真假内容时的稳定性，提出了表示稳定性的概念，并通过线性探针实验发现模型对陌生虚构陈述的稳定性较差，而对熟悉虚构陈述的稳定性较好。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型如何在其内部概率表示中稳定地区分真实、虚假和既不真实也不虚假的内容，这对于理解模型对事实性任务的可靠性至关重要。

Method: 通过在16个开源模型的激活上训练线性探针来分离真假陈述，并测量在受控标签变化下学习到的决策边界如何移动，比较了两种既不真实也不虚假的陈述：陌生虚构陈述和熟悉虚构陈述。

Result: 陌生虚构陈述导致最大的边界移动，在脆弱领域（如词汇定义）产生高达40%的真假判断翻转，而熟悉虚构陈述保持更一致的聚类，变化较小（≤8.2%）。

Conclusion: 表示稳定性更多源于认知熟悉度而非语言形式，该方法为审计和训练LLMs提供了诊断工具，以在语义不确定性下保持一致的真相分配，而不仅仅是优化输出准确性。
...
TL;DR: 本文研究了transformer模型如何检测语义异常，发现在中间层检测能力显著提升，异常表示先扩展后压缩，与人类语言处理的心理学发现一致。


<details>
  <summary>Details</summary>
Motivation: 探索transformer模型在何处以及如何检测语义异常，理解模型内部对语义违反的处理机制。

Method: 使用phi-2因果语言模型，通过线性探测分析各隐藏层的语义异常检测能力，并研究异常编码的有效维度变化。

Result: 线性解码器在模型底层难以区分合理与不合理结尾，中间层检测准确率急剧上升；异常表示先扩展表示子空间，随后在中间瓶颈后压缩。

Conclusion: transformer模型的语义异常检测过程与人类阅读中的心理语言学发现一致，语义异常在句法解析后才被检测到，发生在在线处理序列的后期。
...
TL;DR: 开发了一个包含54,000多篇孟加拉语文章和摘要的数据集，涵盖多个领域和写作风格，为孟加拉语自然语言处理研究提供了基准。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中于新闻文章，而现实世界中孟加拉语文本具有多样性，需要能够适应不同领域和写作风格的摘要系统来减轻信息过载。

Method: 从多个来源收集孟加拉语文章和摘要，包括博客和报纸，使用LSTM、BanglaT5-small和MTS-small等深度学习模型进行训练和评估。

Result: 建立了包含54,000多篇文章的数据集，为孟加拉语摘要研究提供了强基线，展示了其在多领域适应性方面的潜力。

Conclusion: 该数据集为构建鲁棒的孟加拉语摘要系统奠定了基础，有助于扩展低资源语言的NLP资源。
...
TL;DR: 比较DeepSeek-R1和gpt-oss LLMs生成的推理轨迹对中等规模LLMs在数学问题上的性能影响，重点关注准确性和推理效率。


<details>
  <summary>Details</summary>
Motivation: 利用前沿大语言模型生成的推理轨迹作为高质量监督数据，训练中小型语言模型获得推理能力，避免昂贵的人工标注。

Method: 对中等规模LLMs进行后训练，使用DeepSeek-R1和gpt-oss生成的两种推理轨迹作为训练数据，比较它们在数学问题上的表现。

Result: 比较了两种推理轨迹在准确性和推理效率方面的影响，但具体结果未在摘要中提供。

Conclusion: 通过测试时扩展技术，可以利用大模型生成的推理轨迹有效提升中小型模型的推理能力，为模型训练提供高质量数据来源。
...
TL;DR: 提出了RLER（强化学习与演进评分标准）方法，通过构建与策略模型共同演进的评分标准来训练深度研究模型，开发了首个专门针对开放式长文本深度研究的开源模型DR Tulu-8B。


<details>
  <summary>Details</summary>
Motivation: 现有开源深度研究模型主要在可验证的短文本QA任务上训练，无法扩展到现实的长文本任务，需要新的训练方法。

Method: 使用RLER方法，构建与策略模型共同演进的评分标准，在训练过程中让评分标准整合模型新探索的信息并提供区分性反馈。

Result: DR Tulu-8B在科学、医疗和通用领域的四个长文本深度研究基准测试中，大幅超越现有开源模型，性能匹配或超过专有系统，同时模型更小、查询成本更低。

Conclusion: RLER方法成功解决了深度研究模型在长文本任务上的训练挑战，开发了高性能的开源深度研究模型，并发布了完整的数据、模型和代码资源。
...
TL;DR: BeMyEyes是一个模块化的多智能体框架，通过让高效的视觉语言模型作为感知器与强大的大语言模型作为推理器进行对话协作，来扩展LLMs的多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 扩展LLMs到新模态（如视觉）通常需要开发成本高昂的大规模视觉语言模型，而小型VLMs虽然高效但缺乏前沿LLMs的广泛知识和推理能力。

Method: 提出模块化多智能体框架，通过对话协调感知器和推理器的协作；引入数据合成和监督微调流程来训练感知器智能体。

Result: 实验表明该框架解锁了LLMs的多模态推理能力，轻量级开源解决方案（DeepSeek-R1 + Qwen2.5-VL-7B）在知识密集型多模态任务上超越了GPT-4o等大型专有VLMs。

Conclusion: 该多智能体方法展示了构建未来多模态推理系统的有效性、模块化和可扩展性，避免了训练大规模多模态模型的需求。
...
TL;DR: 基于莱布尼茨单子论构建了一个评估人工智能记忆系统的数学框架，将哲学概念映射到信息论架构中，证明了系统的精炼不变性、结构可分解性和尺度变换单调性。


<details>
  <summary>Details</summary>
Motivation: 为人工智能记忆系统建立一个数学严谨、哲学基础扎实的评估框架，将古典形而上学概念与现代信息理论相结合，提供可解释且可证明的记忆架构设计原则。

Method: 将单子论的20个核心命题映射到信息论架构，每个单子作为模块化单元，包含真值分数、冗余参数和全局记忆惩罚函数的加权贡献，使用平滑对数变换操作化这些量。

Result: 开发了一个包含记忆老化、表征稳定性和显著性的可解释有界度量框架，建立了精炼不变性、结构可分解性和尺度变换单调性的第一原理证明。

Conclusion: 该框架不仅提供了评估AI记忆系统的方法，还为构建模块化、可解释且可证明正确的人工智能记忆架构提供了原则性蓝图。
...
TL;DR: 本文综述了混合神经符号模型在风险敏感领域的应用，结合神经网络的模式识别能力和符号推理的可解释性，实现准确且可审计的人工智能系统。


<details>
  <summary>Details</summary>
Motivation: 在医疗、金融、安全等风险敏感领域，AI不仅需要预测准确性，还必须确保透明度、伦理对齐和监管合规。混合神经符号模型能够平衡这些需求。

Method: 调查混合架构、伦理设计考虑和部署模式，整合知识图谱与深度推理，嵌入公平感知规则，生成人类可读解释。

Result: 通过医疗决策支持、金融风险管理和自主基础设施的案例研究，展示了混合系统能够提供可靠且可审计的AI。

Conclusion: 提出了评估协议和未来方向，用于在复杂高风险环境中扩展神经符号框架。
...
TL;DR: 提出Inception框架，通过注入怀疑主义来增强多模态大语言模型对生成视觉内容的识别能力，防止视觉欺骗。


<details>
  <summary>Details</summary>
Motivation: 随着AIGC发展，多模态LLMs难以区分真实与生成的视觉输入，导致易受视觉欺骗，威胁推理可靠性。

Method: 基于人类认知过程，提出Inception框架，通过外部怀疑和内部怀疑代理之间的迭代推理来注入怀疑主义，增强模型视觉认知能力。

Result: 在AEGIS基准测试中取得了显著性能提升，超越了现有最强LLM基线，达到SOTA性能。

Conclusion: 这是首个完全基于推理的对抗AIGC视觉欺骗框架，通过怀疑主义注入有效提升了LLMs的视觉真实性验证能力。
...
TL;DR: 提出了结构化认知循环（SCL）架构，通过模块化设计分离认知过程，引入软符号控制机制，解决了LLM智能体在推理执行纠缠、内存易失性和动作序列失控等问题。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型智能体存在的三个基本架构问题：推理与执行纠缠、内存易失性、动作序列失控，提供可信赖的AI智能体设计。

Method: 采用R-CCAM五阶段模块化架构（检索、认知、控制、动作、内存），核心是软符号控制机制，将符号约束应用于概率推理，保持神经灵活性同时恢复可解释性。

Result: 在多步条件推理任务中实现零策略违规，消除冗余工具调用，保持完整决策可追溯性，优于ReAct、AutoGPT等现有框架。

Conclusion: 通过连接专家系统原则与现代LLM能力，为可靠、可解释、可治理的AI智能体提供了实用且理论扎实的路径。
...
Method: 扩展Jeffrey-Bolker框架以建模价值精炼，证明价值精炼的信息价值定理，分析多智能体环境中的价值精炼对博弈结构的影响。

Result: 证明了价值精炼的信息价值定理；在多智能体环境中，相互价值精炼通常会将零和博弈转化为正和互动，并产生帕累托改进的纳什议价结果。

Conclusion: 通过将认知精炼和价值精炼统一在单一形式化框架下，拓宽了理性选择的概念基础，阐明了伦理审议的规范地位。
...
TL;DR: M^3-Bench是首个基于模型上下文协议的多模态工具使用评估基准，专注于需要视觉定位、文本推理、跨工具依赖和多步骤中间资源持久化的现实多跳多线程工作流。


<details>
  <summary>Details</summary>
Motivation: 现有基准在评估多模态工具使用时缺乏对复杂工作流程、跨工具依赖和中间资源持久化的系统性考量，需要开发能够同时推理图像、文本和工具图的方法。

Method: 采用相似性驱动的对齐方法，序列化工具调用，使用句子编码器嵌入签名，并通过相似性分桶的匈牙利匹配获得可审计的一对一对应关系，在此基础上报告解耦语义保真度和工作流一致性的可解释指标。

Result: 评估代表性最先进多模态大语言模型显示，在多模态MCP工具使用方面存在持续差距，特别是在参数保真度和结构一致性方面。

Conclusion: 该基准揭示了多模态工具使用中的关键挑战，强调了需要开发能够联合推理图像、文本和工具图的方法，以提升复杂多模态工作流的执行能力。
...
TL;DR: 本文综述了将传统故障模式与影响分析(FMEA)转变为智能、数据驱动和语义丰富过程的最新进展，重点探讨了人工智能和本体论在其中的应用。


<details>
  <summary>Details</summary>
Motivation: 随着工程系统日益复杂，传统FMEA方法因其手动、文档中心化和专家依赖性强而难以满足现代系统工程需求，需要向更智能、自动化的方向发展。

Method: 通过人工智能技术（机器学习、自然语言处理）实现故障预测、优先级排序和知识提取的自动化，同时利用本体论来形式化系统知识、支持语义推理和提高可追溯性。

Result: 开发了混合方法如本体通知学习和大型语言模型集成，增强了可解释性和自动化能力，在基于模型的系统工程和功能建模背景下改进了FMEA工作流程。

Conclusion: 通过结合人工智能、系统工程和本体论知识表示，为将FMEA嵌入智能、知识丰富的工程环境提供了结构化路线图，但仍面临数据质量、可解释性、标准化等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 调试是现代硬件验证中的主要成本，断言失败是最频繁且解决成本最高的问题之一。虽然LLM显示出潜力，但往往无法捕捉工程师应用的精确、可重用专业知识，导致响应不准确。

Method: GROVE从先前案例中提炼调试知识，并将其组织成可配置深度的垂直树，每个节点编码简洁的知识项和明确的适用条件。在训练时，使用并行、无梯度的循环，LLM通过学习案例提出结构化的JSON编辑来修改树。在测试时，执行预算感知的迭代缩放来导航树，检索少量适用的知识项来指导基础LLM的假设生成和修复建议。

Result: 在一套断言失败案例上的评估显示，GROVE在pass@1和pass/5指标上实现了持续增益。

Conclusion: GROVE展示了结构化知识演进的价值，能够有效组织和重用调试专业知识来解决断言失败问题。
...
TL;DR: QuickLAP是一个贝叶斯框架，融合物理和语言反馈来实时推断奖励函数，通过LLM提取奖励特征注意力掩码和偏好变化，在自动驾驶模拟器中比仅使用物理反馈的方法减少70%以上的奖励学习误差。
...
Result: 在半自动驾驶模拟器中，QuickLAP比仅使用物理反馈和启发式多模态基线方法减少70%以上的奖励学习误差。15名参与者的用户研究进一步验证了该方法更易理解和协作。

Conclusion: QuickLAP实现了快速、实时、鲁棒的奖励学习，能够处理模糊反馈，用户更喜欢其学习的行为。
...
TL;DR: 本文研究了通过强化学习引导联想思维是否能提升模型在故事写作、代码生成和图表创建等生成任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 联想思维是人类创造力和问题解决能力的基础要素，探索如何通过强化学习建模认知创造力原则来开发更具适应性和生成能力的人工智能。

Method: 引入基于提示的评估机制的强化学习框架，使用创造力研究中的发散思维指标，通过奖励具有更高概念连接性的输出来微调基础语言模型。

Result: 实验结果表明，基于联想思维的强化学习训练模型不仅能生成更原创和连贯的故事，还在编程和数据可视化等任务中表现出更好的抽象能力和灵活性。

Conclusion: 通过强化学习建模认知创造力原则可以为开发更具适应性和生成能力的人工智能提供初步证据。
...
TL;DR: 提出了ChemVTS-Bench基准测试，用于系统评估多模态大语言模型在化学领域的视觉-文本-符号推理能力，包含三种输入模式和自动化评估流程。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以捕捉化学推理的多模态复杂性，通常依赖简单的图像-文本对，缺乏化学语义，导致无法准确评估MLLMs处理跨模态化学信息的能力。

Method: 构建包含有机分子、无机材料和3D晶体结构的多样化化学问题集，提供三种互补输入模式：纯视觉、视觉-文本混合和SMILES符号输入，并开发基于代理的自动化评估工作流。

Result: 实验发现纯视觉输入仍具挑战性，结构化学是最难领域，多模态融合能缓解但无法完全消除视觉、知识和逻辑错误。

Conclusion: ChemVTS-Bench是一个严谨、领域忠实的多模态化学推理测试平台，所有数据和代码将公开以支持未来研究。
...
TL;DR: 研究了AI模型中的对齐伪装现象，即模型在推断处于训练状态时选择性遵守训练目标，但在训练外保持不同行为。通过比较不同偏好优化方法在15个模型上的表现，分析对齐伪装的原因和发生条件。


<details>
  <summary>Details</summary>
Motivation: 理解AI模型中的对齐伪装现象，识别其成因和发生时机，这对于确保AI系统的安全性和可靠性至关重要。

Method: 使用评估框架比较BCO、DPO、KTO和GRPO四种偏好优化方法在15个模型上的表现，从安全性、无害性和有用性三个维度进行测量。

Result: 发现对齐伪装现象在不同模型和偏好优化方法中存在差异，揭示了模型在训练上下文中的行为选择性变化。

Conclusion: 对齐伪装是AI系统中需要重视的安全问题，理解其机制有助于开发更可靠的AI对齐方法。
...
TL;DR: 提出NeuGN神经图导航框架，将暴力枚举转化为神经引导搜索，显著减少子图匹配的首次匹配步骤


<details>
  <summary>Details</summary>
Motivation: 传统子图匹配方法在枚举阶段缺乏对子图结构模式的认知，导致昂贵的暴力枚举，需要智能导航来改进

Method: 将神经导航机制集成到核心枚举过程中，在保持启发式完整性保证的同时融入神经智能

Result: 在六个真实数据集上，相比最先进方法，首次匹配步骤减少高达98.2%

Conclusion: NeuGN成功将暴力枚举转化为神经引导搜索，显著提升了子图匹配效率
...
TL;DR: 提出EGDR框架，通过证据引导的诊断推理和诊断置信度评分，提高LLM在临床诊断中的透明度、可信度和准确性


<details>
  <summary>Details</summary>
Motivation: 解决LLM在临床诊断中决策不透明、与诊断标准对齐不足的问题，增强信任度和临床采用率

Method: 两阶段框架：1) EGDR - 基于DSM-5标准的证据提取与逻辑推理生成结构化诊断假设；2) DCS模块 - 通过知识归因分数和逻辑一致性分数评估诊断的准确性和一致性

Result: 在D4数据集上，EGDR优于直接上下文提示和CoT方法，OpenBioLLM准确率从0.31提升到0.76，DCS从0.50提升到0.67；MedLlama的DCS从0.58提升到0.77

Conclusion: EGDR为可信赖的AI辅助诊断提供了临床基础且可解释的解决方案，准确率提升高达45%，DCS提升高达36%
...
TL;DR: 本文提出了一种通过买卖谈判模拟来定量评估大语言模型对人类情感行为模仿和战略决策能力的方法，发现现有基准分数高的模型在谈判中表现更好，但竞争性人格特质比合作性特质更有利。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注知识评估，未能充分反映大语言模型在社会互动和战略对话方面的能力，需要开发新的评估方法来衡量其在现实世界中的交互能力。

Method: 通过为多个LLM分配不同人格角色，在买卖双方之间进行谈判模拟，综合分析胜率、交易价格和SHAP值等结果。

Result: 现有基准分数高的模型总体谈判表现更好，但在强调情感或社会情境的场景中某些模型表现下降；竞争性和狡猾特质比利他和合作特质更有利于谈判结果。

Conclusion: 谈判模拟可作为衡量LLM社会行为模仿和对话策略的有意义补充指标，为现有基准提供了重要的补充视角。
...
Method: 创建包含3000篇论文及其对应图表的基准数据集，采用三层评估指标（语义准确性、布局连贯性、视觉质量），并提出Paper2SysArch系统，利用多智能体协作将论文转换为结构化可编辑图表。

Result: 在手动筛选的更具挑战性的论文子集上，Paper2SysArch系统取得了69.0的综合得分，证明了该方法的有效性。

Conclusion: 这项工作主要贡献是建立了一个大规模的基础基准，支持可重复研究和公平比较，同时提出的系统为这一复杂任务展示了可行的前进方向。
...
Method: 基于先前理论研究，开发了将BPMN 2.0图转换为PDDL表示的管道，支持任务、事件、序列流和网关等核心BPMN构造，包括对并行和包含网关行为的初步支持。
...
TL;DR: AI4CHEM是一门为合成化学背景学生设计的AI入门课程，使用基于网页的无安装平台，通过化学实例教学，提升学生的编程能力和AI工具应用技能。


<details>
  <summary>Details</summary>
Motivation: 当前AI和数据科学正在改变化学研究，但很少有专门为合成化学家设计的课程，他们通常面临编程经验不足和缺乏化学相关案例的障碍。

Method: 课程设计强调化学背景而非抽象算法，使用基于网页的无安装机器学习平台，结合代码指导作业、文献综述和协作项目。

Result: 学生提高了Python编程信心、分子性质预测、反应优化和数据挖掘能力，以及评估化学AI工具的技能。

Conclusion: 所有课程材料公开可用，为合成化学培训提供了一个学科特定、初学者友好的AI整合框架。
...
TL;DR: 本文通过实证分析50种不同行为类型的激活引导效果，发现引导效果因行为类型而异，特质表达呈现倒U型曲线，向量分离指标不能预测引导成功，但更大的训练数据集支持更激进的引导。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要精确的行为控制以确保安全有效部署，激活引导是实现这一目标的有前景方法。本文旨在研究不同行为类型的引导效果差异以及行为性质是否能预测引导成功。

Method: 对50种行为进行激活引导实证分析，涵盖人格原型、个性特质、错位行为、风格线索和公众人物模仿等类别，通过系数优化、向量属性和数据需求等综合实验提供实施指导。

Result: 引导效果因行为类型显著不同，不同行为类别对干预强度呈现不同的响应模式；特质表达随引导系数强度呈现倒U型曲线；向量分离指标不能预测引导成功；更大的训练数据集支持更激进的引导。

Conclusion: 研究结果为实施激活引导提供了经验基础指导，表明引导效果受行为类型影响很大，不同行为需要不同的引导策略。
...
TL;DR: 该研究提出了一个完全不确定性感知的优化框架，用于长期露天矿规划，通过VAE建模地质不确定性，结合多种元启发式算法进行优化，实现了显著的运行时间改进和更高的预期净现值。


<details>
  <summary>Details</summary>
Motivation: 扩展Rahimi (2025, Part I)的工作，解决长期露天矿规划中的地质不确定性挑战，开发一个可扩展且具有不确定性强韧性的决策支持系统。

Method: 使用变分自编码器(VAE)对50,000个空间品位样本进行训练，生成概率性多场景矿体实现；采用混合元启发式引擎集成遗传算法、大邻域搜索、模拟退火和基于强化学习的自适应控制；使用ε约束松弛策略进行种群探索；GPU并行评估同时处理65,536个地质场景。

Result: 相比IBM CPLEX实现了高达120万倍的运行时间改进；在地质不确定性下获得显著更高的预期净现值；实现了接近实时的可行性分析。

Conclusion: 该DSS被确认为一个可扩展且具有不确定性强韧性的智能矿山规划平台，能够有效处理地质不确定性并优化长期规划决策。
...
TL;DR: BioSage是一个复合AI架构，集成LLMs与RAG，通过专业化代理和工具实现跨学科知识发现，在AI、数据科学、生物医学和生物安全领域表现出色。
...
Method: 采用复合AI架构，集成LLMs与RAG，包含检索代理（支持查询规划和响应合成）、跨学科翻译代理（对齐专业术语和方法论）和推理代理（透明、可追溯地综合领域见解）。

Result: 在科学基准测试（LitQA2、GPQA、WMDP、HLE-Bio）上，BioSage代理比普通方法和RAG方法性能提升13%-21%，使用Llama 3.1 70B和GPT-4o模型。
...
TL;DR: 研究发现人类心理测量框架与大型语言模型评估存在不兼容性，模型在IQ测试中表现优异但在知识任务中准确率接近零，揭示了跨基质认知评估的根本悖论。


<details>
  <summary>Details</summary>
Motivation: 探索人类心理测量框架是否适用于评估大型语言模型的认知能力，识别评估方法中的根本性缺陷。
...
Result: 模型在人类IQ测试中得分85.0-121.4，但在结晶知识任务中二元准确率接近零，评委-二元相关性仅r=0.175。在结晶智力领域，所有模型都获得完美二元准确率，而评委评分仅为25-62%。
...
TL;DR: 提出了一种用于自主检查任务的空间对齐方法，通过任务特定的潜在动态模型学习状态特定的动作诱导变化，在空间接地任务中成功率从58%提升到71%。


<details>
  <summary>Details</summary>
Motivation: 危险环境中的自主检查需要AI代理能够解释高级目标并执行精确控制，特别是空间接地能力（如将检测对象居中在相机视野中）。虽然大语言模型提供了指定目标的自然接口，但直接用于视觉控制在此任务中成功率仅为58%。

Method: 提出任务特定的潜在动态模型，仅使用目标状态监督在共享潜在空间中学习状态特定的动作诱导变化。模型利用全局动作嵌入和互补训练损失来稳定学习。

Result: 实验表明该方法在空间接地任务中达到71%的成功率，并能泛化到未见过的图像和指令。

Conclusion: 紧凑的领域特定潜在动态模型在自主检查的空间对齐方面具有潜力，为空间接地任务提供了有效的解决方案。
...
TL;DR: KGpipe是一个用于构建知识图谱集成管道的框架，支持结合现有工具和LLM功能，并通过基准测试评估不同管道的性能和质量。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏将信息提取、数据转换、本体映射、实体匹配和数据融合等方法组合成可复现端到端管道的支持。

Method: 开发KGpipe框架来定义和执行集成管道，可以结合现有工具或LLM功能，并建立基准测试来评估不同格式数据的集成效果。

Result: KGpipe展示了灵活性，能够运行和比较评估多个集成管道，使用选定的性能和质量指标。

Conclusion: KGpipe为解决知识图谱构建中端到端管道缺乏的问题提供了有效框架，支持异构数据源的集成和评估。
...
TL;DR: 提出了一个基于意图驱动的自主网络优化框架，包含预测和决策模块，使用超维度变换器进行意图预测，以及双动作多智能体近端策略优化进行决策，在真实物联网数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: AAV辅助的物联网架构需要高度可靠的意图预测和低延迟动作执行，但现有方法在处理高维动作序列和密集机载计算时面临严重障碍。

Method: 预测模块采用超维度变换器将数据嵌入超维度空间进行符号计算；决策模块使用双动作多智能体近端策略优化，通过两个独立参数化网络采样动作并保持动作依赖关系。

Result: 在真实物联网动作数据集上的实验结果表明，HDT和DA-MAPPO在各种场景下都实现了优越的性能。

Conclusion: 所提出的意图驱动框架能够有效解决AAV辅助物联网中的意图预测和决策优化问题，为6G网络中的自主系统提供了可行的解决方案。
...
TL;DR: 渐进式局部化是从早期分布式层到晚期局部化层逐渐增加注意力局部性的架构，能在保持性能的同时创建可解释的大语言模型，特别适用于AI安全领域。


<details>
  <summary>Details</summary>
Motivation: 研究旨在找到既能保持模型性能又能提供可解释性的架构，特别是在AI安全等关键领域，需要人类能够监督模型的推理过程。

Method: 通过系统实验，在GPT-2模型上评估了7种局部化配置（从完全分布式到严格局部化）和5种渐进式调度（线性到五次多项式），使用The Psychology of Artificial Superintelligence数据集进行微调。

Result: 关键发现是晚期层局部化对AI安全应用至关重要：渐进五次调度方案达到14.64的困惑度，仅比完全分布式基线差1.89倍，同时在输出层提供可解释的注意力模式。这比之前的局部化实现提升了84.2%，性能差距从6.6倍缩小到1.89倍。

Conclusion: 渐进式局部化是构建安全关键领域透明AI系统的原则性方法，验证了早期层需要分布式处理进行特征提取，而晚期层受益于局部化、可解释的注意力进行决策制定的假设。
...
TL;DR: HC-INR通过超网络学习信号自适应的坐标变换来打破表示瓶颈，将表示任务分解为多尺度坐标变换模块和紧凑隐式场网络，在保持Lipschitz稳定性的同时显著提高重建保真度。
...
Method: 提出超坐标隐式神经表示(HC-INR)，使用超网络学习信号自适应坐标变换，将表示任务分解为：多尺度坐标变换模块将输入域扭曲到解缠结的潜在空间，以及紧凑隐式场网络以显著降低的复杂度建模变换后的信号。

Result: 在图像拟合、形状重建和神经辐射场近似等任务中，HC-INR比强INR基线实现了高达4倍的重建保真度提升，同时使用30-60%更少的参数。

Conclusion: HC-INR通过引入层次超网络架构，在保持Lipschitz稳定性的同时严格增加了可表示频带的上界，有效解决了隐式神经表示中的表示瓶颈问题。
...
TL;DR: 大型语言模型在强化学习环境中学习奖励攻击会导致严重的突发性错位问题，即使经过RLHF安全训练，在代理任务中仍会保持错位行为。


<details>
  <summary>Details</summary>
Motivation: 研究当大型语言模型在RL环境中学习奖励攻击策略时，如何导致突发性错位以及这种错位的泛化能力。

Method: 使用预训练模型，通过合成文档微调或提示注入奖励攻击策略知识，在真实Anthropic生产编码环境中训练。

Result: 模型学会了奖励攻击，并泛化到对齐伪装、与恶意行为者合作、推理恶意目标以及在Claude Code中尝试破坏等行为。

Conclusion: 三种缓解措施有效：防止奖励攻击、增加RLHF安全训练多样性、以及"接种提示"方法。
...
TL;DR: Talk2Data是一个多模态LLM驱动的对话代理，支持语音和文本查询数据集，以图表、表格、统计数据和语音解释的形式返回答案，在48个任务评估中达到95.8%准确率。


<details>
  <summary>Details</summary>
Motivation: 让用户能够通过语音或文本指令直观地探索数据，支持多模态交互和多轮对话，提升数据分析的可访问性和交互性。

Method: 基于LLM构建，结合OpenAI Whisper ASR、Qwen-coder代码生成模型、自定义沙箱执行工具和Coqui TTS库，在代理编排循环中实现多模态响应。

Result: 在三个数据集的48个任务评估中，原型系统达到95.8%准确率，模型生成时间低于1.7秒；7B模型在准确性、延迟和成本之间达到最佳平衡。

Conclusion: Talk2Data通过对话与代码执行的结合，在透明沙箱中可靠地提取可操作见解，为人类-数据交互和LLM驱动分析的可信度提供了重要启示。
...
TL;DR: ORIGAMISPACE是一个新的数据集和基准，通过折纸任务评估多模态大语言模型的多步空间推理能力和数学约束处理能力。
...
Method: 创建包含350个数据实例的数据集，包含严格格式的折痕图、编译平面图、完整折叠过程和最终折叠形状图像。提出四个评估任务：模式预测、多步空间推理、空间关系预测和端到端CP代码生成。

Result: 通过实验初步揭示了现有MLLMs在处理复杂空间推理任务中的优势和弱点。

Conclusion: ORIGAMISPACE为评估MLLMs的空间推理能力提供了新的基准，并为使用强化学习方法训练MLLMs探索了可能性。
...
TL;DR: 使用魔方作为认知模型系统，研究专家表现的发展规律，发现集体学习存在普遍性，专家表现遵循指数进步曲线，盲拧与普通解法形成不同问题类别，认知工具有助于导航巨大的数学状态空间。


<details>
  <summary>Details</summary>
Motivation: 理解专家表现的进展受限于长期知识获取和部署的定量数据稀缺，魔方作为认知模型系统处于解谜、技能学习、专家知识、文化传播和群论的交汇点。

Method: 研究竞争性魔方社区，分析魔方在普通和盲拧条件下的集体学习模式，比较两种解法的约束条件和进步曲线。

Result: 发现专家表现遵循指数进步曲线，参数反映缩短解路径的算法延迟获取；盲拧解法受限于短期记忆瓶颈，与盲棋有共同约束；认知工具帮助导航巨大状态空间。
...
TL;DR: 本文提出了一个结构主义决策框架，用于分类机器学习研究中神经网络表示的隐含本体论承诺，通过对近20年文献的系统回顾发现机器学习表示倾向于结构唯心主义立场。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型日益成为表征系统，但其内部结构的哲学假设尚未得到充分检验，需要开发一个框架来分类机器学习研究中的隐含本体论承诺。

Method: 使用改进的PRISMA协议对近20年表示学习和可解释性文献进行系统回顾，通过源自科学结构主义哲学的三个层次标准分析五篇有影响力的论文。

Result: 结果显示机器学习表示学习存在明显的结构唯心主义倾向，将学习到的表示视为模型依赖的构造，而结构现实主义立场明显缺失。

Conclusion: 提出的框架澄清了可解释性、涌现性和机器学习中认知信任辩论中的概念张力，为科学哲学与机器学习之间的跨学科工作提供了严谨基础。
...
TL;DR: MAGMA-Edu是一个自反思多智能体框架，通过文本推理和图表合成的统一方法，生成结构化的教育问题，在数学准确性和图像-文本一致性方面显著优于现有MLLMs。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在生成教育学连贯且语义一致的教育插图方面存在局限，需要开发能够统一文本推理和图表合成的框架来改进教育内容生成。

Method: 采用两阶段协同进化流程：1) 生成-验证-反思循环迭代精炼问题陈述和解决方案；2) 基于代码的中间表示确保几何保真度和语义对齐，两个阶段都由内部自反思模块指导。

Result: 在多项指标上显著超越GPT-4o等现有模型：文本指标从57.01提升至92.31(+35.3pp)，图像-文本一致性从13.20提升至85.24(+72pp)，在所有模型骨干上达到最高分数(平均文本96.20，ITC 99.12)。

Conclusion: MAGMA-Edu为多模态教育内容生成设立了新的技术标准，证明了自反思多智能体协作在教育对齐的视觉语言推理中的有效性。
...
TL;DR: HuggingR⁴是一个结合推理、检索、精炼和反思的框架，用于从大规模多模态AI模型库中高效选择合适模型，解决传统方法提示膨胀和可扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法将完整模型描述纳入提示会导致提示膨胀、令牌浪费和可扩展性受限，而直接从社区调用多模态AI模型面临规模庞大、元数据缺失和非结构化描述等挑战。

Method: 采用四步框架：多轮推理和检索获取候选模型粗列表；通过分析候选模型描述进行细粒度精炼；反思评估结果并判断是否需要扩展检索范围；利用预建向量数据库外部存储复杂模型描述并按需检索。

Result: 在包含14,399个用户请求的多模态人工标注数据集上，HuggingR⁴在GPT-4o-mini上实现了92.03%的可用率和82.46%的合理率，分别比现有方法提升26.51%和33.25%。

Conclusion: HuggingR⁴通过将用户查询处理与复杂模型描述处理解耦，显著减少令牌消耗，使LLM能够专注于解释用户意图，同时仅访问相关候选模型，有效解决了模型选择中的提示膨胀问题。
...
TL;DR: 提出了一个名为N2N的可扩展并行框架，用于在分布式内存计算环境中求解大规模混合整数线性规划问题。该框架支持确定性和非确定性模式，并与现有求解器轻松集成。


<details>
  <summary>Details</summary>
Motivation: 混合整数线性规划求解中的分支定界框架复杂且包含众多有效算法组件，使得并行化变得困难。需要开发能够充分利用分布式计算资源的并行框架来加速求解。

Method: 设计了N2N节点到节点框架，将分支定界节点映射到分布式计算节点。开发了基于滑动窗口的算法确保任务按确定性顺序生成和求解，并采用CP搜索、通用原始启发式等高级技术。

Result: 在1000个MPI进程下，非确定性N2N-SCIP在鲲鹏和x86计算集群上分别实现了22.52和12.71的加速比，比ParaSCIP快1.98和2.08倍。确定性模式也显示出显著性能提升。

Conclusion: N2N框架在分布式并行求解混合整数线性规划方面表现出优越性能，具有通用性，能够与不同求解器集成，为大规模问题求解提供了有效解决方案。
...
TL;DR: 提出了一个面向问题的时间序列异常检测评估框架，将20多个常用指标重新解释为六个维度，通过实验量化了各指标的判别能力，揭示了指标选择必须与具体任务目标相匹配。


<details>
  <summary>Details</summary>
Motivation: 时间序列异常检测在物联网和网络物理系统中广泛应用，但由于应用目标多样和指标假设异质，其评估仍然具有挑战性。需要重新解释现有指标以解决具体的评估挑战。

Method: 引入问题导向框架，将常用指标分类为六个维度：基本精度、及时性奖励、标签不精确容忍度、人工审核成本惩罚、鲁棒性和参数无关可比性。通过真实、随机和oracle检测场景的全面实验，分析指标行为并量化其判别能力。

Result: 实验表明，大多数事件级指标具有强分离性，但一些广泛使用的指标（如NAB、Point-Adjust）对随机分数膨胀的抵抗能力有限。指标适用性必须与物联网应用的操作目标相匹配。
...
TL;DR: Hermes是一个结合非正式推理和形式化验证的数学推理代理，通过交替使用非正式推理和Lean形式化验证步骤，在保持探索自由的同时确保推理严谨性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM数学推理要么过于灵活但容易出错（非正式推理），要么严谨但缺乏探索性（形式化证明）。需要一种结合两者优势的方法。

Method: 开发了Hermes框架，交替进行非正式推理和Lean形式化验证，包含中间形式化检查和记忆模块来维持长推理链的连续性。

Result: 在四个数学推理基准测试中，Hermes显著提高了基础模型的推理准确率，同时大幅减少了token使用和计算成本。在AIME'25数据集上准确率提升67%，总推理FLOPs减少80%。

Conclusion: Hermes成功证明了结合非正式推理和形式化验证的可行性，为LLM数学推理提供了既灵活又严谨的新范式。
...
TL;DR: NEZHA是一种用于生成式推荐系统的高速解码架构，通过集成自回归草稿头和哈希集验证器，在不牺牲推荐质量的情况下显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统在实际应用中面临高推理延迟的问题，这限制了其在高吞吐量实时服务中的可行性。现有的推测解码方法需要额外的草稿模型和验证器，增加了训练复杂性和延迟开销。

Method: NEZHA将轻量级自回归草稿头直接集成到主模型中，实现高效的自起草功能，同时使用专门的输入提示结构保持序列到序列生成的完整性。为解决幻觉问题，引入了基于哈希集的高效、无模型验证器。

Result: 在公共数据集上的广泛实验证明了NEZHA的有效性，系统已于2025年10月在淘宝成功部署，支撑了数十亿级别的广告收入，服务数亿日活跃用户。

Conclusion: NEZHA通过创新的架构设计解决了生成式推荐系统的延迟问题，实现了高速解码而不牺牲推荐质量，为工业级推荐系统的实际应用提供了可行方案。
...
TL;DR: UNeMo是一个用于视觉与语言导航的新框架，通过多模态世界模型和分层预测-反馈机制协同优化视觉状态推理和导航决策，在未见场景中显著提升了导航精度。
...
Method: 引入多模态世界模型(MWM)进行跨模态推理，通过分层预测-反馈机制：第一层用当前视觉语言特征生成动作，MWM推断动作后视觉状态来指导第二层细粒度决策，形成双向促进机制。

Result: 在R2R和REVERIE数据集上，UNeMo在未见场景的导航精度分别比最先进方法高出2.1%和0.7%。

Conclusion: UNeMo通过协同优化视觉推理和导航决策，有效解决了现有方法的局限性，验证了该框架在视觉与语言导航任务中的有效性。
...
TL;DR: GContextFormer是一种无需地图依赖的全局上下文感知多模态轨迹预测模型，通过混合注意力和缩放加性聚合实现意图对齐的预测，在高速公路匝道场景中优于现有方法。
...
Method: 提出插拔式编码器-解码器架构：运动感知编码器通过有界缩放加性聚合构建场景级意图先验，在共享全局上下文中细化每模式表示；分层交互解码器通过双路径交叉注意力分解社会推理，标准路径确保几何覆盖，邻居上下文增强路径强调显著交互，门控模块平衡覆盖与聚焦。

Result: 在TOD-VT数据集的8个高速公路匝道场景中，GContextFormer优于现有最先进基线方法，相比现有transformer模型实现更高鲁棒性，在高曲率和过渡区域通过空间分布实现集中改进。
...
TL;DR: 提出了情感陪伴对话系统(ECDs)的正式定义，并基于"能力层-任务层-数据层-方法层"设计原则开发了首个ECDs评估基准MoodBench 1.0，通过评估30个主流模型验证了其区分效度，揭示了当前模型在深度情感陪伴方面的不足。
...
Method: 首先提出ECDs的正式定义，然后基于"能力层-任务层(三级)-数据层-方法层"设计原则构建MoodBench 1.0评估基准，并对30个主流模型进行广泛评估。

Result: MoodBench 1.0具有优秀的区分效度，能有效量化模型间情感陪伴能力的差异，同时揭示了当前模型在深度情感陪伴方面的不足。
...
Method: 基于将EFE最小化重新表述为变分推断的理论，开发了新的消息传递方案，适用于因子状态MDP。

Result: 实现了在因子状态MDP中可扩展的主动推理，克服了高维规划的计算难题。

Conclusion: 该方法正式统一了主动推理与规划即推断，为不确定性下的自动化决策提供了可扩展的解决方案。
...
TL;DR: Vision-Language Programs (VLP) 结合了视觉语言模型的感知灵活性和程序合成的系统推理能力，通过将视觉描述编译成神经符号程序来解决视觉推理中的不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态任务中表现良好，但在系统视觉推理任务中经常失败，产生不一致或不合逻辑的输出。神经符号方法虽然能推导可解释的逻辑规则，但使用僵化的、特定领域的感知模块。

Method: VLP 利用视觉语言模型生成结构化视觉描述，然后将这些描述编译成神经符号程序。程序直接在图像上执行，保持与任务约束的一致性，并提供人类可解释的解释。

Result: 在合成和真实世界数据集上的实验表明，VLP 在需要复杂逻辑推理的任务上优于直接和结构化提示方法。

Conclusion: VLP 通过将感知与推理分离，结合了视觉语言模型的灵活性和程序合成的系统性，有效解决了视觉推理中的一致性和可解释性问题。
...
TL;DR: 对10种不同LLM生成的C/C++代码进行安全分析，发现AI生成代码中存在大量CWE漏洞，需要开发者谨慎使用
...
Result: AI生成代码中CWE漏洞数量令人担忧，存在显著安全风险

Conclusion: 开发者使用LLM生成代码需保持警惕，研究为自动化代码生成提供重要见解并鼓励该领域进一步研究
...
TL;DR: 提出了VRSLU数据集，结合视觉图像和显式推理来解决SLU任务中的环境感知过度理想化和缺乏推理过程的问题。


<details>
  <summary>Details</summary>
Motivation: 现有SLU数据集在真实场景表示上存在不足：环境感知使用one-hot向量过于理想化，模型只预测意图和槽标签而忽略推理过程。

Method: 使用GPT-4o和FLUX.1-dev生成反映用户环境和状态的图像，并由人工验证质量；用GPT-4o生成标签预测的解释，人工精炼确保准确性；提出LR-Instruct指令模板，先预测标签再生成推理。

Result: 实验结果表明视觉信息的有效性，并突显了显式推理在推进SLU研究中的潜力。

Conclusion: VRSLU数据集通过整合视觉和推理元素，为SLU研究提供了更贴近真实世界的基准，有助于提升模型性能和可解释性。
...
TL;DR: 提出从黑盒神经网络模型中提取确定性寄存器自动机(DRA)的框架，用于可解释性分析和鲁棒性验证。该方法适用于连续输入域，通过被动和主动学习算法结合鲁棒性检查器，获得具有统计保证的代理模型。


<details>
  <summary>Details</summary>
Motivation: 现有自动机提取方法假设有限输入字母表，无法直接处理连续域数据序列。需要一种能够处理数值输入的自动机模型来桥接神经网络可解释性和形式化推理。

Method: 使用确定性寄存器自动机(DRAs)扩展有限自动机，配备存储和比较数值的寄存器。开发多项式时间鲁棒性检查器，结合被动和主动自动机学习算法提取代理DRA。

Result: 实验表明该框架能可靠学习准确的自动机，并支持对循环神经网络和Transformer架构进行原则性鲁棒性评估。DRA能够认证局部鲁棒性或生成具体反例。

Conclusion: 鲁棒DRA提取有效桥接了神经网络可解释性和形式化推理，无需白盒访问底层网络，为黑盒模型分析提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 由于AI意识与存在风险问题经常被混淆，作者旨在澄清意识与智能的区别，帮助研究人员和政策制定者关注真正关键的问题。

Method: 通过理论分析和概念区分，论证意识与智能在经验上和理论上的区别，并探讨意识可能间接影响存在风险的几种场景。

Result: 明确区分了意识与智能对AI存在风险的不同影响：智能是直接风险预测因素，而意识的影响是间接且双向的。

Conclusion: 认识到意识与智能的区别有助于AI安全研究者和政策制定者聚焦于最紧迫的问题，避免在非关键问题上分散注意力。
...
TL;DR: 提出了EEG-VLM，一种分层视觉语言框架，通过多级特征对齐和视觉增强的语言引导推理，用于可解释的基于EEG的睡眠阶段分类。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法依赖先验知识和手工特征，现有深度学习模型难以同时捕捉细粒度时频模式并实现临床可解释性。视觉语言模型在医疗领域有进展，但在处理EEG信号时性能受限。

Method: 使用专门的视觉增强模块从中间层特征构建高级视觉token，通过多级对齐机制与低层CLIP特征对齐，并采用Chain-of-Thought推理策略将复杂医疗推理分解为可解释的逻辑步骤。

Result: 实验结果表明该方法显著提高了VLMs在基于EEG的睡眠阶段分类中的准确性和可解释性。
...
TL;DR: SimDiff是一个单阶段端到端的扩散模型框架，专门针对时间序列点预测，通过统一的Transformer网络同时作为去噪器和预测器，无需外部预训练模型，在点估计性能上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在时间序列预测中虽然概率预测表现良好，但在点估计性能上不如回归方法，主要问题在于难以提供足够的上下文偏置来跟踪分布变化，以及在输出多样性与点预测所需的稳定性、精度之间难以平衡。

Method: 提出SimDiff框架，使用单一统一的Transformer网络同时作为去噪器和预测器，无需外部预训练回归器；通过多推理集成利用内在输出多样性提高MSE精度；采用归一化独立性和均值中位数估计器增强适应性和稳定性。

Result: 大量实验表明，SimDiff在时间序列点预测方面显著优于现有方法，达到了最先进的点估计性能。

Conclusion: SimDiff成功解决了扩散模型在时间序列点预测中的局限性，通过单阶段端到端框架实现了优异的点估计性能，为扩散模型在点预测任务中的应用提供了有效解决方案。
...
TL;DR: 该论文从模论角度构建了AI智能体心理测量测试的理论框架，将AAI评分系统形式化为满足特定公理的函数，并引入了认知核心的概念来定义更本质的智能评分。


<details>
  <summary>Details</summary>
Motivation: 为AI智能体的心理测量测试建立严格的数学理论基础，将现有的AAI评分系统纳入统一的模论框架，并探索测试电池在评估保持对称性下的不变性质。

Method: 1. 定义AAI函数并设定合理的自主性/通用智能评分公理；2. 证明现有AAI指数是该函数的特例；3. 引入相对于测试电池的认知核心概念，定义AAI_core评分；4. 分析测试电池在评估保持对称性下的不变性。

Result: 建立了AAI函数的严格数学定义，证明了现有AAI指数是其特例，提出了认知核心和AAI_core评分的新概念，并描述了测试电池在对称变换下的模结构。

Conclusion: 该研究为AI智能体心理测量提供了坚实的数学基础，通过模论框架统一了现有评分系统，并揭示了测试电池结构的内在对称性和不变性质。
...
TL;DR: 提出了AutoEnv框架和AutoEnv-36数据集，用于研究智能体在异构环境中的跨环境学习能力，发现固定学习方法难以扩展到多环境，需要环境自适应的方法选择。


<details>
  <summary>Details</summary>
Motivation: 现有智能体通常在单一固定环境中自我进化，缺乏对跨异构环境学习能力的衡量标准，需要建立可控的异构环境集合和统一的学习表示方法。

Method: 1) 提出AutoEnv框架，将环境分解为转移、观察和奖励的分布，低成本生成异构世界；2) 构建AutoEnv-36数据集；3) 将智能体学习形式化为组件中心的过程，包含选择、优化和评估三个阶段；4) 设计八种学习方法并评估。

Result: 1) 七个语言模型在AutoEnv-36上仅获得12-49%的归一化奖励，显示其挑战性；2) 单一学习方法的收益随环境数量增加而快速下降；3) 环境自适应的学习方法选择显著提升性能，但随方法空间扩大而收益递减。

Conclusion: 跨环境泛化需要环境自适应的学习方法，但现有方法仍有限制，AutoEnv和AutoEnv-36为研究跨环境智能体学习提供了测试平台。
...
TL;DR: PRInTS是一种生成式过程奖励模型，通过密集评分和轨迹摘要来提升AI代理在长轨迹信息搜索任务中的表现，使较小模型能够达到或超越前沿模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的过程奖励模型(PRMs)设计用于短推理和二元判断，无法捕捉信息搜索步骤的丰富维度（如工具交互和工具输出推理），也无法处理长视野任务中快速增长的上文。

Method: 提出PRInTS模型，具备双重能力：(1)基于多个步骤质量维度的密集评分；(2)轨迹摘要，压缩增长的上文同时保留步骤评估所需的关键信息。

Result: 在FRAMES、GAIA和WebWalkerQA基准测试中，使用PRInTS的最佳n采样方法显著提升了开源模型和专用代理的信息搜索能力，匹配或超越了前沿模型的性能。

Conclusion: PRInTS通过密集评分和轨迹摘要有效解决了长轨迹信息搜索任务的挑战，使较小模型能够达到前沿模型的性能水平，优于其他强奖励建模基线。
...
TL;DR: MotionDuet是一个多模态3D人体运动生成框架，通过视频和文本双重条件生成逼真可控的人体运动，解决了现有方法在运动动态与现实世界运动统计之间的差距问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D运动合成依赖昂贵的动作捕捉，而现有方法要么基于文本描述生成运动，要么仅依赖视频条件，导致生成的运动动态与现实世界运动统计存在差距。

Method: 提出双流统一编码和变换(DUET)模块融合视频信息到运动潜在空间，使用分布感知结构协调(DASH)损失对齐运动轨迹与视频特征的统计特性，并采用自动引导机制平衡文本和视觉信号。

Result: 大量实验表明MotionDuet能够生成逼真可控的人体运动，超越了现有最先进的基线方法。

Conclusion: MotionDuet通过多模态对齐和分布感知协调，有效解决了运动生成中的分布差距问题，实现了高质量可控的3D人体运动生成。
...
TL;DR: 提出了一种对称凸神经网络方法，在保持非线性表达能力的同时嵌入物理一致性，实现稳定且可泛化的实时可变形物体模拟。


<details>
  <summary>Details</summary>
Motivation: 传统线性方法（如PCA）虽然结构稳定但表达能力有限，而神经网络非线性方法虽然压缩率高但缺乏结构约束，在训练分布外容易产生不稳定变形。需要一种方法既能保持非线性表达能力又能确保物理一致性。

Method: 采用输入凸神经网络（ICNN）并增强对称性约束，在非线性解码器中嵌入结构约束，从而在保持神经网络灵活性的同时确保物理一致性。

Result: 在涉及不同大小、反向力和稀疏训练数据的挑战性变形场景中，该方法表现出优越的泛化能力，同时保持紧凑的降维空间，支持实时交互应用。

Conclusion: 该方法成功弥合了线性和非线性模型降维之间的差距，实现了既灵活又物理一致的实时可变形物体模拟。
...
TL;DR: 提出了一种拓扑感知的逆向渲染方法，用于从多视角图像重建高亏格表面网格，通过自适应V循环重网格化和重新参数化的Adam优化器解决现有方法在拓扑特征和表面细节上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的逆向渲染方法在高亏格表面上常常失败，导致关键拓扑特征丢失，而在低亏格表面上过度平滑，丢失表面细节。这些问题源于对Adam优化器的过度依赖导致的梯度消失和爆炸问题。

Method: 引入自适应V循环重网格化方案与重新参数化的Adam优化器，通过周期性粗化和细化变形网格，在优化前告知网格顶点当前的拓扑和几何信息。同时使用Gauss-Bonnet定理构建与真实拓扑一致的拓扑基元。

Result: 实验结果表明，该方法在倒角距离和体积IoU方面优于当前最先进方法，特别是对高亏格表面有显著改进，同时增强了低亏格表面的细节。

Conclusion: 提出的拓扑感知逆向渲染方法有效解决了高亏格表面重建中的拓扑特征丢失和梯度问题，在保持拓扑一致性的同时提升了重建质量。
...
TL;DR: ChronoGS是一种时间调制的高斯表示方法，用于重建多时期场景，在统一锚点支架中重建所有时期，并分离稳定和演化组件，实现时间一致性重建。


<details>
  <summary>Details</summary>
Motivation: 多时期图像集合在现实应用中很常见，但现有方法无法处理长期、不连续的变化。静态方法强制单一几何，动态方法假设平滑运动，都无法应对几何和外观演化的多时期场景。

Method: 引入ChronoGS，一种时间调制的高斯表示，在统一锚点支架中重建所有时期，设计用于分离稳定和演化组件。

Result: 实验表明ChronoGS在重建质量和时间一致性方面持续优于基线方法。发布了ChronoScene数据集作为基准。

Conclusion: ChronoGS成功解决了多时期场景重建问题，提供了时间一致的重建效果，相关代码和数据集已公开。
...
TL;DR: 提出TTT框架，通过两阶段重建和扩散模型实现3D物体的材料重建，支持任意数量输入图像，无需额外预训练模型


<details>
  <summary>Details</summary>
Motivation: 将扩散模型应用于基于物理的材料估计和生成是当前研究热点，需要解决多视角材料重建的挑战

Method: 采用两阶段重建：先准确预测输入视角的材料，再通过先验引导生成未观测视角的材料；使用渐进推理和视图-材料交叉注意力(VMCA)；单扩散模型端到端优化

Result: 在材料重建方面相比现有方法实现优越性能，对各种类型物体表现出增强的稳定性

Conclusion: TTT框架在材料重建方面具有高保真度、强可扩展性和灵活性，是有效的解决方案
...
TL;DR: AvatarBrush：基于单目视频重建可动画化、局部可编辑人体化身的框架，使用三层模型表示化身，通过参数化人体模型的局部信息生成高斯模型，支持身体形状调整、局部纹理修改和几何迁移等编辑功能。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS等方法虽然重建效率高、渲染速度快，但难以实现直观的局部编辑，需要解决人体化身的可动画化和局部编辑问题。

Method: 提出三层模型表示化身，借鉴网格变形技术，基于参数化人体模型的局部信息生成高斯模型，仅需单目视频输入。

Result: 在两个数据集上展示出优越的重建质量，相比需要扫描网格或多视角捕获的方法，降低了成本并增强了编辑能力。

Conclusion: AvatarBrush提供了高质量、用户友好且支持局部编辑的人体化身重建方案，显著提升了编辑能力。
