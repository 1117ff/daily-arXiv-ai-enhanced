...
Method: 提出SSUF模块，结合光谱解混和光谱-空间特征提取，指导基于ResNet的CNN进行重建；同时设计空间-光谱梯度损失函数，结合MSE与空间/光谱梯度分量。

Result: 在三个公共遥感高光谱数据集上的实验表明，该混合深度学习模型在降低模型复杂度的同时实现了有竞争力的性能。

Conclusion: SSUF模块和空间-光谱梯度损失函数有效提升了高光谱超分辨率的性能，为相关应用提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大规模语言和视觉模型在现实应用中的广泛采用使得解决幻觉问题变得紧迫，因为幻觉可能传播错误信息并造成财务和运营损害。当前对幻觉的理解仍不完整且碎片化，需要统一框架来开发更有效的解决方案。

Method: 提出了一个统一的多层次框架来表征图像和文本幻觉，采用任务-模态交织的方法将幻觉与模型生命周期中的特定机制联系起来。

Result: 研究发现幻觉往往源于数据分布的可预测模式和继承偏差，通过深入理解这些机制为开发更鲁棒的解决方案奠定了基础。

Conclusion: 该调查通过提供对幻觉的统一理解，为在现实世界生成式AI系统中开发更鲁棒和有效的幻觉解决方案奠定了基础。
...
Method: 1) 输出鲁棒性：通过离线鲁棒优化对抗最坏情况的动作噪声；2) 输入鲁棒性：强制输入变化下动作一致性；3) 多扰动处理：将鲁棒性建模为多臂老虎机问题，使用置信上界算法识别最有害噪声。

Result: 在LIBERO基准上，RobustVLA相比基线在pi0骨干上提升12.6%，在OpenVLA骨干上提升10.4%，推理速度快50.6倍，在真实FR5机器人上提升65.6%。

Conclusion: RobustVLA有效提升了VLA模型在多模态扰动下的鲁棒性，特别是在真实机器人场景中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在基准测试中表现出色，但通过指令调优控制其行为仍然困难。减少指令调优数据集预算通常会导致性能下降，因为启发式策略将模型视为黑盒，忽视了控制学习的潜在能力。

Method: 引入能力归因数据策划(CADC)框架，从任务特定启发式转向内在能力分析。无监督地从基于梯度的学习轨迹中发现内在能力，通过影响估计将训练数据归因于这些能力，并通过平衡选择和分阶段排序策划能力感知课程。

Result: 仅使用原始数据5%的情况下，CADC在多模态基准测试中超越了全数据训练。

Conclusion: 这些结果验证了内在能力作为模型学习的基本构建模块，并确立了CADC作为指令数据策划的原则性范式。
...
<details>
  <summary>Details</summary>
Motivation: 当前基准测试在任务设计上缺乏难度递进，缺少跨语言任务，且使用的真实世界图像通常只包含单一文化，使得测试相对容易。

Method: 构建C³B基准，包含三种难度递增的任务：基础视觉识别、文化冲突理解和文化内容生成，涵盖多种文化和语言。

Result: 对11个开源MLLM进行评估，发现MLLM与人类表现之间存在显著性能差距。

Conclusion: C³B基准对当前MLLM构成重大挑战，鼓励未来研究提升MLLM的文化意识能力。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在专业、教育和创意领域应用日益广泛，但其输出经常嵌入和放大社会偏见。本研究旨在调查六个先进开源模型中的性别表征问题。

Method: 使用精心设计的提示词，为五种医院相关职业（心脏病专家、医院主任、护士、护理人员、外科医生）和五种肖像修饰词组合生成100张图像，分析六个开源模型的性别表征模式。

Result: 所有模型都表现出系统性职业刻板印象：护士总是女性，外科医生主要是男性。不同模型差异显著：Qwen-Image和SDXL强制执行男性主导，HiDream-I1-dev结果混合，FLUX.1-dev在多数角色中偏向女性。肖像修饰词也影响性别平衡，corporate强化男性描绘，beautiful偏向女性描绘。

Conclusion: 文本到图像模型中的性别偏见既系统又模型特定。提示词措辞在塑造人口统计结果中起关键作用，需要偏见感知设计、平衡默认设置和用户指导，以防止生成式AI强化职业刻板印象。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型催生了提示词交易市场，但提示词本身存在被盗风险，本文旨在揭示这种安全漏洞。

Method: 将模板窃取视为顺序决策问题，使用基于相似度的反馈信号作为奖励函数，通过强化学习有效探索提示词空间。

Result: 在公开基准测试中达到最先进性能，攻击成本降低到现有基线的13%以下，能够有效泛化到不同图像风格窃取未见过的提示词模板。
...
Result: 在120个OK-VQA示例和多个VLM上，EDCT揭示了显著的忠实性差距，并提供符合监管要求的审计证据。

Conclusion: EDCT能有效检测视觉语言模型解释的忠实性问题，为模型可信度评估提供自动化验证工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法认为MLLMs在高分辨率图像中表现不佳是因为感知限制和小物体识别困难，但本文分析发现主要问题是复杂背景干扰而非物体大小。
...
Result: 在V*Bench、HRBench4K和HRBench8K上达到新SOTA，将Qwen2.5-VL 7B和InternVL3 8B分别提升至92.1%和91.6%，比之前无需训练方法节省75%内存。

Conclusion: HiDe框架有效解决了MLLMs在高分辨率图像中的背景干扰问题，无需额外训练即可显著提升性能，为视觉理解任务提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决遥感图像分割中由灰度变化（如阴影和低对比度区域）引起的语义边缘模糊问题，充分利用空间信息。
...
<details>
  <summary>Details</summary>
Motivation: 重新思考自动驾驶问题，将其视为序列生成任务，利用VLM的生成能力直接从摄像头输入进行端到端轨迹预测，简化传统复杂的多阶段流程。

Method: 提出Max-V1单阶段端到端框架，将轨迹规划建模为下一个路径点预测，采用基于统计建模的原则性监督策略，通过模仿学习从大规模专家演示中学习复杂驾驶策略。

Result: 在nuScenes数据集上达到最先进性能，相比先前基线总体提升超过30%，在跨域数据集上表现出优异的泛化性能，显示出跨车辆鲁棒性和适应性潜力。

Conclusion: 该工作引入了一个能够实现基本驾驶行为的模型，为开发更强大的自动驾驶智能体奠定了基础，展示了端到端方法在自动驾驶领域的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统低秩分解方法面临秩选择困难、设计空间大、微调时间长、兼容性有限等问题，需要一种更高效的压缩方案。

Method: 基于特征图相似性的秩选择策略，一次性微调过程，支持所有卷积层和全连接层，集成6种低秩分解技术进行选择性应用。

Result: 在14个CNN模型和8个数据集上的实验表明，该方法实现了显著压缩且精度损失最小，优于多种先进技术。

Conclusion: 该框架提供了一种高效、兼容性强的CNN压缩解决方案，通过组合多种低秩分解技术可获得比单一方法更好的压缩效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在单图指南这种结合文本、图像和符号的视觉格式理解方面评估不足，这种格式体现了人类感知和理解的特点。

Method: 开发了半自动标注流程，让多个智能代理协作生成初步图像描述，辅助人类构建图像-文本对，并评估了29个最先进的MLLM。

Result: Qwen2.5-VL-72B表现最佳，总体准确率为77%，但所有模型在语义理解和逻辑推理方面都有明显弱点。多智能体标注系统在图像描述生成方面优于所有MLLM。

Conclusion: 当前MLLM在准确解释复杂视觉-文本关系方面仍存在困难，多智能体标注系统可作为高质量图像描述生成器和未来数据集构建的有价值工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的地理空间模型主要依赖领域预训练和监督微调，缺乏专门的推理能力训练。Geo-R1旨在通过后训练框架解锁视觉语言模型的地理空间推理能力，避免昂贵的人工推理标注。

Method: 分两个阶段：1）脚手架阶段：使用合成的思维链示例进行监督微调，建立地理空间思维范式；2）提升阶段：基于GRPO的强化学习，利用弱监督的跨视角配对代理提供可验证的奖励信号。

Result: Geo-R1在多个地理空间推理基准测试中达到了最先进的性能，将地理空间建模从领域预训练/监督微调扩展到推理优先的后训练范式。

Conclusion: Geo-R1成功解锁了视觉语言模型的地理空间推理能力，通过思维脚手架和强化学习的组合方法，实现了无需人工标注的高质量地理空间推理。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在视觉和机器人应用中广泛使用，但现有认证训练和鲁棒性认证方法面临过参数化挑战，导致紧密度和可扩展性不足。需要解决神经网络对语义变换扰动(如亮度和对比度)的鲁棒性验证问题。

Method: 首先分析层和神经元对输入扰动的稳定性和方差，提出USN度量来指示可认证鲁棒性。基于USN设计新的剪枝方法，移除低USN神经元，保留高USN神经元。引入Wasserstein距离损失来增强剪枝过程，确保剪枝后的神经元在层间更集中。

Result: 在具有挑战性的鲁棒关键点检测任务上进行了广泛实验，涉及真实的亮度和对比度扰动。实验表明该方法相比基线方法实现了更优越的鲁棒性认证性能和效率。

Conclusion: 提出的基于USN度量的剪枝方法有效解决了神经网络过参数化问题，在保持模型表达能力的同时显著提升了鲁棒性认证的紧密度和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱异常检测在民用和军事应用中至关重要，但由于复杂现实环境和目标信号先验知识有限，现有技术面临挑战。

Method: 提出基于传输的数学模型，将高光谱像素视为模板模式的变形观测，在SCDT域中表示，并使用无监督子空间建模技术构建背景信号模型。
...
Method: 将编码问题建模为生成模型参数的关键依赖扰动，提出MOLM框架，使用二进制密钥激活残差和注意力块中的轻量级LoRA适配器，避免密钥特定的重新训练。

Result: 在Stable Diffusion和FLUX上的实验表明，MOLM在保持图像质量的同时，能有效抵抗失真、压缩、再生、平均攻击和对提取器的黑盒对抗攻击。

Conclusion: MOLM框架实现了不可感知性、保真度、可验证性和鲁棒性，为生成模型水印提供了一种有效的解决方案。
...
Method: 提出CROWD框架，包含CROWD-Discover（使用子模条件增益函数挖掘未知实例）和CROWD-Learn（使用组合目标解耦已知与未知表示，同时保持已知类别间的判别一致性）。

Result: 在OWOD基准测试中，CROWD在M-OWODB和S-OWODB上分别将已知类别精度提高了2.83%和2.05%，未知召回率相比领先基线提高了近2.4倍。
...
Method: 提出ExpDWT-VAE方法，采用双分支结构：一个分支通过卷积操作处理空间域输入，另一个分支通过2D Haar小波分解、卷积操作和逆DWT重构提取和处理频域特征。两个分支融合形成集成的空间-频率表示，再通过卷积和对角高斯映射细化为鲁棒的潜在表示。

Result: 在TerraFly映射系统的新卫星图像数据集上进行实验，多个性能指标的结果证明了所提方法在增强潜在空间表示方面的有效性。

Conclusion: 利用离散小波变换增强VAE潜在空间表示的方法能够有效提升潜在扩散模型在遥感应用中的性能，为卫星图像处理提供了创新的空间-频率表示方法。
...
Result: BiFlow实现了最先进的性能，平均减少minADE和minFDE 10-15%，并展现出优越的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 探索自动化方法在胸部X光片分析中的准确性，特别关注零样本视觉语言模型(VLM)与监督CNN在医疗影像诊断任务中的性能对比。

Method: 在PneumoniaMNIST和Shenzhen TB数据集上，比较轻量级监督CNN与BiomedCLIP VLM在肺炎和结核病检测任务中的表现，并采用决策阈值校准优化VLM性能。

Result: 校准后，BiomedCLIP在肺炎检测中F1-score达0.8841（优于CNN的0.8803），在结核病检测中F1-score从0.4812提升至0.7684（接近CNN的0.7834）。

Conclusion: 适当的校准对于释放零样本VLM的诊断潜力至关重要，使其能够匹配甚至超越专门任务的监督模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的GUI代理方法在长时程任务中面临记忆限制，要么截断历史记录，要么依赖简单的文本摘要，这可能导致丢失对后续决策至关重要的视觉细节信息。

Method: PAL-UI结合了双层级摘要代理（捕捉观察级线索和行动级结果）与专用检索工具，使代理能够在规划过程中回忆特定的历史截图。基于Qwen2.5-VL训练了PAL-UI-3B和PAL-UI-7B模型。

Result: 在移动GUI导航任务中，PAL-UI显著优于基线模型和先前方法，即使在数据效率设置下也表现优异。同时展现出强大的跨领域泛化能力，在未经额外训练的情况下在网页导航中取得显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 通用分割工具缺乏脑膜瘤放疗规划所需的临床特异性，需要开发专门针对临床关键任务的交互式分割解决方案。

Method: 提出了Interactive-MEN-RT系统，整合多种临床相关的交互方法（点标注、边界框、套索工具和涂鸦），专门用于脑膜瘤放疗工作流程的3D分割。

Result: 在BraTS 2025脑膜瘤放疗分割挑战赛的500个对比增强T1加权MRI扫描上，相比其他分割方法有显著改进，Dice相似系数达77.6%，交并比达64.8%。

Conclusion: 在脑膜瘤放疗规划等关键应用中，需要临床定制的分割解决方案，Interactive-MEN-RT展示了这种专门化方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在主题一致性视频生成方面存在不足，难以解析包含复杂空间关系、时间逻辑和多主体交互的提示词。

Method: 提出MLLM-DiT框架，使用预训练的多模态大语言模型进行深度跨模态推理，识别实体并解耦角色、属性和交互，生成主题感知的隐藏状态来条件化扩散变换器。

Result: 在OpenS2V基准测试中，该方法在主题一致性、自然度和文本相关性方面均优于现有的开源和商业模型。

Conclusion: BindWeave框架有效解决了复杂提示词解析的挑战，实现了高质量的主题一致性视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督去噪方法存在两个问题：高频结构细节保留不足，以及在拟合高频时网络会学习到噪声。

Method: 1. 频率带选择策略加速训练收敛；2. 使用Lipschitz常数限制卷积核学习高频噪声的能力；3. 频谱分离和低秩重建模块(SSR)分离噪声和细节。

Result: 在合成和真实数据集上的实验验证了SCNet的有效性。

Conclusion: SCNet通过频谱控制优化了自监督去噪，能够更好地保留高频结构细节并避免学习噪声。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言目标检测器在零样本识别方面表现优异，但在领域偏移下性能会显著下降，需要一种有效的测试时自适应方法来提升鲁棒性。

Method: 提出IoU加权熵目标，专注于空间一致的提案聚类；引入图像条件提示选择，根据图像级兼容性对提示进行排序，并将最有信息的提示与检测器对数融合。

Result: 在多种分布偏移场景（风格化域、驾驶场景、低光条件和常见损坏）下，该方法在YOLO-World和Grounding DINO上都优于零样本和TTA基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在视觉符号组合推理方面存在局限，需要建立严格的测试基准来评估和推进模型在这方面的能力。

Method: 创建包含140万个生成实例的基准测试，涵盖文本引导和纯视觉设置，系统化考虑数字规模、移动复杂度、解决方案多样性和运算符变化。

Result: 评估14个视觉语言模型发现显著局限性：闭源模型只能在简单案例中成功，开源模型在视觉模式下失败，而人类准确率超过90%。

Conclusion: MathSticks为推进跨视觉和符号的组合推理提供了严格的测试平台，代码和数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用异常检测方法仅使用正常样本作为参考，忽视了现实场景中可用的异常样本所包含的宝贵信息。

Method: 提出Normal-Abnormal Generalist Learning框架，包含残差挖掘和异常特征学习两个关键组件，从正常-异常参考残差中提取异常模式并自适应学习异常特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的选择方法可能将不同矩阵编码为相同的图像表示，导致相同的选择和次优方法选择，需要改进特征提取技术以避免特征歧义。

Method: RAF技术同时提取和融合图像表示作为相对特征，以及对应的数值作为绝对特征，创建全面的矩阵表示。

Result: 在SuiteSparse和BMCMat数据集上的评估显示，稀疏线性系统求解时间减少0.08s-0.29s，比传统基于图像的选择方法快5.86%-11.50%，达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 当手部被自身或物体严重遮挡时，人类通过利用上下文知识（如affordance）来解决姿态歧义。受此启发，研究旨在利用affordance感知的文本描述来指导手部姿态优化。

Method: 使用扩散生成模型学习在affordance描述条件下的合理手部姿态分布，通过从大视觉语言模型推断的手-物体交互affordance描述来指导被遮挡区域的姿态优化。

Result: 在HOGraspNet数据集上的实验表明，该方法相比最近的回归方法和缺乏上下文推理的扩散优化方法，显著提升了手部姿态估计的准确性。

Conclusion: affordance引导的优化方法能够有效改善严重遮挡情况下的3D手部姿态重建，生成更准确且功能连贯的手部姿态。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大模型中视觉token消耗大量计算资源，现有压缩方法忽视了压缩带来的特征空间扰动会增加学习难度的问题。

Method: 提出渐进一致性蒸馏框架，将token压缩引入的特征空间扰动分解为token维度和layer维度，分别采用token一致性蒸馏和layer一致性蒸馏，利用教师模型指导进行渐进学习。

Result: 大量实验证明该框架具有优越的有效性、鲁棒性和泛化能力。

Conclusion: EPIC框架通过渐进一致性蒸馏有效解决了多模态大模型视觉token压缩带来的训练困难问题。
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图基础模型的应用受到限制，缺乏标准化评估基准。现有解决方案多在私有数据上评估，限制了可比性。超声心动图面临噪声采集、高帧冗余和有限公开数据集等独特挑战。

Method: 将8个公开数据集统一为标准测试套件，涵盖功能、结构、诊断和视图识别等任务。在一致的零样本、探测和对齐协议下评估心脏专用、生物医学和通用编码器。

Result: 不同模型家族展现出互补优势：时序建模对功能回归至关重要，检索在分布偏移下提供鲁棒性，领域特定文本编码器捕获生理学有意义的轴。通用编码器迁移能力强，但在细粒度区分上表现不佳。

Conclusion: CardioBench建立了可复现的参考点，为未来超声心动图基础模型的设计提供实用指导，通过发布预处理、分割和公开评估流程促进标准化评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有的确定性模型在自遮挡和复杂手部关节情况下存在姿态模糊问题，而现有的概率方法仅限于单阶段估计且缺乏优化能力，无法产生准确的3D重建。

Method: 采用两阶段级联扩散框架：第一阶段使用联合扩散模型生成多样化的3D关节假设，第二阶段使用网格潜在扩散模型（Mesh LDM）基于关节样本重建3D手部网格。

Result: 在FreiHAND和HO3Dv2数据集上的实验表明，该方法实现了最先进的性能，同时有效建模了姿态分布。

Conclusion: 该框架成功结合了概率建模和级联优化的优势，能够处理姿态模糊性并产生准确的3D手部重建。
...
Result: 在自建的ForestPest数据集上，Forestpest-YOLO实现了最先进的性能，在小目标和遮挡害虫检测方面显著优于现有基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统微生物学图像分析需要大量手动标注数据来训练模型，这是一个劳动密集型过程。研究旨在证明在新视觉任务中，详尽标注不再是必要前提。

Method: 构建了5000张培养皿图像数据集，模拟传统数据收集方法以及少样本、低样本场景。在任务特定指标上对三个视觉基础模型与传统基线进行基准测试。

Result: MaskDINO仅用150张图像微调就达到与广泛训练的YoloV9模型几乎相当的性能，仅用25张图像仍能保持竞争力，在约70%样本上保持可靠。

Conclusion: 数据高效的基础模型能够用传统方法所需数据的一小部分就达到相当效果，使自动化微生物系统能够更早开发并更快迭代改进，且具有比传统模型更优越的上限性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有MoE-MTL方法依赖单任务预训练骨干网络，在从单任务到多任务学习转换时存在冗余适应和低效知识共享问题。

Method: 在LoRA-based MoE中引入自适应共享专家，共享专家与稀疏专家联合归一化门控权重；增加LoRA专家数量同时按比例降低其秩，实现细粒度专家设计。

Result: 在PASCAL-Context基准测试中，ASE在各种配置下持续提升性能，验证了细粒度设计对MTL的有效性。

Conclusion: ASE框架通过自适应共享专家和细粒度专家设计，有效解决了MoE-MTL中的知识共享和专业化问题，提升了多任务学习性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式视频帧插值方法只能合成固定数量的中间帧，缺乏调整生成帧率或序列总时长的灵活性。
...
Conclusion: ArbInterp框架成功解决了传统VFI方法的灵活性限制，实现了任意时间戳和任意长度的视频帧插值，为视频生成应用提供了更强大的工具。
...
Conclusion: MDS-NeRF通过重新设计NeRF流程和引入创新模块，实现了高质量的3D风格迁移，并支持使用3D风格参考的全视角风格迁移。
...
<details>
  <summary>Details</summary>
Motivation: 解决人工标注结构缺陷的高成本和低效率问题，为基础设施安全提供自动化标注方案。

Method: 集成大型视觉语言模型、语义模式匹配模块和迭代自问自答优化机制，通过领域特定提示和递归验证处理原始视觉数据。

Result: 在缺陷/非缺陷图像区分上达到98%准确率，在四类缺陷标注中达到85%-98%准确率（类平衡）和80%-92%（类不平衡）。

Conclusion: ADPT提供了可扩展且经济高效的高质量数据集构建方案，为结构损伤评估中的迁移学习和领域适应等下游任务提供支持。
...
Method: 使用语义增强的地标识别分离前景和背景特征，然后通过共识驱动的在线增强策略整合两阶段投票结果。

Result: 在REVERIE和R2R数据集上实验表明，该在线前景-背景增强方法提升了基线模型的泛化能力并达到最先进性能。

Conclusion: 前景和背景特征的在线增强策略能有效提升视觉语言导航的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 标准监督学习会导致模型过度依赖背景信息，产生虚假相关性学习，限制了模型在现实部署中的鲁棒性。现有方法主要通过抑制背景来解决这个问题，但牺牲了上下文信息。

Method: RCOR将定位作为识别的组成部分，解耦对象中心和上下文感知建模，然后进行鲁棒的非参数融合。

Result: RCOR提高了监督模型和VLM在具有域内和域外背景数据集上的性能，即使在复杂场景如ImageNet-1k中也能实现定位先于识别。

Conclusion: 定位先于识别的方法现在在复杂场景中变得可行，RCOR能够在不牺牲任何一方的情况下同时实现鲁棒性和上下文感知。
...
Conclusion: 无条件判别器能够促进GAN训练中的纳什均衡，与经典GAN理论兼容，可作为插件式方法实现，实验证实了其有效性和优越性。
...
TL;DR: 提出基于Kolmogorov-Arnold Network (KAN)的深度伪造检测方法，通过可学习样条替代固定激活函数，并引入地标辅助自适应KAN模块来聚焦关键面部区域。
...
Method: 使用KAN网络替代传统激活函数，结合面部地标作为结构先验，开发LAKAN模块动态生成KAN内部参数，引导图像编码器关注有伪影的信息区域。
...
TL;DR: ReFlux是首个专门针对新一代整流流变换器(如Flux)的概念擦除鲁棒性评估攻击方法，通过反向注意力优化和速度引导机制有效重新激活被抑制的概念信号。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要针对Stable Diffusion设计，在下一代整流流变换器上效果有限，需要专门评估这类模型的概念擦除鲁棒性。

Method: 提出基于注意力定位现象的反向注意力优化策略，结合速度引导动态机制和一致性保持目标，稳定注意力并增强概念重新激活效果。

Result: 大量实验证明该方法在评估整流流变换器概念擦除鲁棒性方面具有有效性和高效性。

Conclusion: ReFlux为评估整流流变换器概念擦除策略的鲁棒性建立了可靠基准。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中多传感器融合需求增加，摄像头-雷达融合能结合丰富的语义信息和精确距离测量，且成本较低。地图分割任务面临精度和实时性挑战

Method: 使用BEV空间的摄像头和雷达融合架构，采用先进的损失函数集和轻量级头部来提升感知效果

Result: 达到53.5 mIoU的精度，推理时间相比最强基线模型提升260%

Conclusion: 该方法在保持与大型模型相当精度的同时，显著提升了推理速度，为实时地图分割设定了新基准
...
TL;DR: OTTER是一个统一的开集多标签标注框架，结合了预定义类别的稳定性和用户驱动开放标签的适应性，在基准数据集上表现优于竞争方法。
...
Method: 基于大规模层次化组织的多模态数据集，采用多头注意力架构，联合对齐视觉和文本表示与固定和开集标签嵌入，实现动态且语义一致的标注。

Result: 在两个基准数据集上，OTTER总体F1分数分别为0.81和0.75，优于次优结果0.10和0.02；在开集标签上达到接近完美的性能（F1分别为0.99和0.97），同时在预定义标签上保持竞争力。

Conclusion: OTTER有效桥接了封闭集一致性与开放词汇灵活性，为多模态标注应用提供了有效的解决方案。
...
Result: 在WDCD和GF1MS-WHU两个数据集上的实验表明，相比其他弱监督方法，SpecMCD的F1分数提高了7.82%以上。
...
TL;DR: 提出一种名为Align Your Tangent (AYT)的新方法，通过引入流形特征距离(MFD)损失函数来缓解一致性模型训练中的振荡切线问题，显著加速训练过程并提升样本质量。


<details>
  <summary>Details</summary>
Motivation: 一致性模型(CMs)虽然能实现一步或两步采样，但需要长时间训练和大批量大小才能获得有竞争力的样本质量。研究发现CMs在收敛附近的训练动态中，切线方向存在振荡问题，这会阻碍训练效率。

Method: 提出流形特征距离(MFD)损失函数，该损失能提供与数据流形对齐的切线方向，使更新方向指向数据流形。该方法称为Align Your Tangent (AYT)。

Result: AYT方法能显著加速CM训练，甚至超越LPIPS指标的性能。此外，该方法支持使用极小的批量大小进行训练而不损害样本质量。

Conclusion: 通过解决一致性模型训练中的振荡切线问题，AYT方法为高效训练一致性模型提供了有效解决方案，在保持样本质量的同时大幅提升训练效率。
...
TL;DR: 提出了一种无监督展开的鲁棒主成分分析方法U2-rPCA，用于超声微血管成像中的杂波滤波，该方法结合了数学可解释性和无需学习标签的优势。
...
Method: 从迭代重加权最小二乘rPCA基线展开，引入内在低秩和稀疏正则化，并添加稀疏增强单元来更好地捕捉稀疏微血流信号。

Result: 在仿真和公开体内数据集上的实验表明，U2-rPCA优于SVD方法、rPCA基线和另一深度学习方法，将功率多普勒图像的对比噪声比提高了2-10dB。

Conclusion: U2-rPCA方法在保持数学可解释性的同时，无需学习标签，在微血管成像中表现出优越性能。
...
TL;DR: 提出了一种用于脑部血管分割的领域自适应框架，通过图像到图像转换技术，在不同数据集上有效分割脑动脉和静脉，无需特定领域模型设计或数据协调。


<details>
  <summary>Details</summary>
Motivation: 脑部血管的复杂形态给自动分割模型带来挑战，且现有方法通常只关注单一成像模态。准确治疗脑部疾病需要全面理解脑血管树，无论具体采集过程如何。

Method: 采用解缠技术独立操作不同图像属性，在保持标签的同时从一个领域转移到另一个领域。特别关注在适应过程中操作血管外观，同时保留对正确分割至关重要的空间信息（如形状和位置）。

Result: 评估显示该框架能有效跨越医疗中心、成像模态和血管类型的大范围领域差距。消融研究确定了所需标注的最优数量和其他架构选择。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成AI方法往往忽视图像的几何结构，仅关注概率分布近似，且对潜在空间的概率分布缺乏深入考虑。本文旨在统一几何和概率视角。

Method: 提出了几何框架和基于核的概率方法，将扩散模型解释为向图像流形的投影机制，构建了确定性模型MPPM（包括像素空间和潜在空间版本）。

Result: 潜在空间MPPM（LMPPM）在多个数据集上优于潜在扩散模型（LDM），在图像恢复和生成任务中取得更优结果。

Conclusion: 通过统一几何和概率视角，本文提出的框架不仅解释了扩散模型的工作原理，还开发出性能更优的新模型，为生成AI提供了新的理论视角。
...
TL;DR: 本文提出了使用二进制编码替代one-hot编码的方法来减少医学图像分割的计算和内存需求，但在108类全脑分割任务中未能达到最先进的分割性能。
...
Result: 与one-hot编码(DSC=82.4)相比，二进制分割方法性能下降，DSC在39.3到73.8之间，未能达到最先进的分割质量。

Conclusion: 虽然二进制编码在计算机视觉的极端分类问题中有效，但在大规模多类别分割任务中面临挑战，希望这项工作能启发未来对紧凑编码策略的研究。
...
TL;DR: 提出了一种基于事件-图像知识蒸馏的开放词汇目标检测框架，通过CLIP教师模型指导事件学生模型学习视觉表示，并结合SNN-CNN混合网络自适应提取事件特征。


<details>
  <summary>Details</summary>
Motivation: 事件相机缺乏纹理和颜色信息，现有事件检测方法局限于预定义类别，难以泛化到新物体。CLIP等视觉语言模型在RGB图像上表现良好，但无法直接应用于事件流数据。
...
Conclusion: 提出的知识蒸馏框架成功弥合了事件数据与图像之间的模态差距，实现了事件数据上的开放词汇目标检测，解决了事件相机在新物体检测中的泛化问题。
...
TL;DR: 该论文提出了ProtoMask模型，通过结合图像分割基础模型和原型案例推理，提高可解释AI中显著性映射的真实性，将显著性计算限制在预定义的语义图像块内。


<details>
  <summary>Details</summary>
Motivation: 解决基于原型的可解释AI方法依赖后处理显著性技术的问题，这些技术存在可靠性和质量方面的批评，需要提高嵌入空间到输入空间映射的真实性。

Method: 使用图像分割基础模型生成分割掩码，通过边界框裁剪图像，每个掩码作为ProtoMask模型架构的独立输入，将显著性计算限制在语义图像块内。

Result: 在三个细粒度分类数据集上的实验表明，该模型具有竞争性的性能表现和独特的可解释性特征。

Conclusion: ProtoMask模型通过结合分割基础模型，有效提高了可解释AI中显著性映射的真实性，为原型学习方法提供了更可靠的可视化解释。
...
<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型(CBMs)通常是单模态的，忽略了概念间的结构化关系，无法有效建模复杂概念交互，限制了在高风险领域中的可解释性应用。

Method: 构建答案-概念和答案-问题图来显式建模多模态输入中的概念关系，集成图变换器捕捉多层次依赖，并用专家混合模块替代前馈层实现动态专家选择和推理任务分配。

Result: 在多个数据集上，MoE-SGT比其它概念瓶颈网络实现了更高的准确率，通过建模概念结构化关系和动态专家选择机制显著提升了性能。

Conclusion: MoE-SGT框架通过结合图变换器和专家混合模块，有效解决了传统概念瓶颈模型在建模概念交互和适应复杂概念模式方面的局限性，为可解释深度学习提供了更强大的工具。
...
TL;DR: 提出了一种基于MLLM内在不确定性的训练免费框架，通过模型输出熵值作为指导信号，让模型自主关注最相关的视觉信息，在三个复杂视觉任务上达到与专门微调方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在细粒度感知方面存在困难，如识别高分辨率图像中的小物体或长视频中的关键时刻。现有方法通常依赖复杂、任务特定的微调，限制了泛化性并增加了模型复杂度。

Method: 利用MLLM内在不确定性作为主动指导信号，通过模型输出熵值对候选视觉输入进行评分，使模型能够自主聚焦于最显著的数据。这是一个统一的训练免费机制。

Result: 在视觉搜索、长视频理解和时序定位三个复杂视觉任务上，现成的MLLM能够达到与专门微调方法竞争的性能。
...
Result: 深度学习方法显著改善了时间强度曲线的时间平滑性，心肌对齐效果与基线相当，处理时间减少了15倍。

Conclusion: 该深度学习管道能够实现快速、鲁棒的运动校正，提高定量灌注成像的准确性，有望促进更广泛的临床应用。
...
TL;DR: 本研究探讨如何使用DEAP数据集中EEG通道子集进行情感预测，通过连续小波变换将EEG数据转换为尺度图，训练视觉变换器模型，仅用12个测量点就实现了91.57%的情感分类准确率。


<details>
  <summary>Details</summary>
Motivation: 传统情感预测方法如自我评估和面部表情分析存在主观性和模糊性，而完整EEG测量复杂且资源密集。低成本EEG设备提供更直接、无偏的数据源，但需要验证其简化测量能力是否足够准确。

Method: 使用连续小波变换将EEG数据转换为尺度图，然后训练视觉变换器模型进行情感分类，重点关注如何用较少的EEG通道子集实现准确预测。

Result: 模型仅使用12个测量点就实现了91.57%的准确率，能够预测4个象限（高/低唤醒度和效价），与使用32个通道的最先进结果96.9%相比表现优异。

Conclusion: 显著减少输入通道数量仍能获得高准确率的结果，证明低成本EEG设备在情感预测任务中具有可行性，为实际应用提供了重要依据。
...
TL;DR: 提出了一种解决极端盲图像恢复(EBIR)问题的新框架，通过将ELQ到HQ的恢复过程分解为两个步骤：先将ELQ图像投影到中间LQ流形，再用现成的BIR模型恢复为HQ。


<details>
  <summary>Details</summary>
Motivation: 现有盲图像恢复方法在处理极端退化情况时表现不佳，直接学习ELQ到HQ的映射存在巨大域差距，导致不自然的伪影和细节丢失。

Method: 提出分解恢复过程：先学习投影器将ELQ图像映射到中间LQ流形，然后使用冻结的现成BIR模型恢复为HQ。基于信息瓶颈理论推导训练目标，平衡低质量重建项和高质量先验匹配项。

Result: 在严重退化情况下进行了广泛实验，验证了方法的有效性。

Conclusion: 该框架能够在不微调的情况下增强现有图像恢复模型，支持推理时提示精炼，为极端盲图像恢复提供了有效解决方案。
...
TL;DR: 提出了ADQA基准测试，用于评估音频描述在长视频片段中的质量，发现现有自动生成方法远不如人工编写，并量化了音频描述的主观性。


<details>
  <summary>Details</summary>
Motivation: 现有自动音频描述生成方法主要关注短片段，且仅与单一参考标准比较，忽视了音频描述写作的主观性本质。

Method: 通过分析同一电影的两个独立音频描述轨道，量化描述的主观性；提出ADQA基准测试，在几分钟长的连贯视频片段上评估音频描述质量，包含视觉欣赏和叙事理解两类问题。

Result: 当前音频描述生成方法在ADQA基准测试中表现远不如人工编写的音频描述。

Conclusion: 需要在更长、更连贯的视频片段上评估音频描述，并提出了未来工作的建议和公开排行榜。
...
<details>
  <summary>Details</summary>
Motivation: 在陶瓷制造中，无损检测对于确保组件质量而不损害其完整性至关重要。光学相干断层扫描(OCT)能够实现高分辨率内部成像，但需要自动化方法来检测孔隙、分层或夹杂物等缺陷

Method: 开发基于U-Net架构的神经网络，使用手动分割标注的OCT图像训练深度学习模型，评估多种实验配置以提升性能，并采用后处理技术进行定量和定性评估

Result: 系统表现出准确的行为，Dice Score达到0.979，优于可比研究，每体积推理时间为18.98秒

Conclusion: 该系统支持夹杂物检测的可行性，实现了更高效、可靠和自动化的质量控制
...
TL;DR: 该论文提出World-Language Models (WorldLMs)，将视频扩散模型重新用作生成编码器，通过单步去噪获得视觉嵌入，在视觉语言模型中集成世界模型先验，显著提升了空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着强大的视频基础模型的出现，探索它们是否能替代传统视觉编码器范式进行通用多模态理解，特别是系统研究世界模型先验在通用多模态任务中的潜力。

Method: 重新利用视频扩散模型作为生成编码器，执行单步去噪操作，将得到的潜在表示作为视觉嵌入，构建名为Dynamic Vision Aligner (DyVA)的模型变体。

Result: WorldLMs能够捕获对下游理解有用的潜在表示，显著增强空间推理能力，使单图像模型能够进行多帧推理，在视觉推理任务中超越开源和专有基线，达到最先进或可比性能。

Conclusion: 世界模型继承的视频预训练中的运动一致性内部化带来了性能提升，该研究为利用世界模型先验的新一代视觉语言模型家族铺平了道路，有望实现通用视觉学习器。
...
TL;DR: 提出了MULTI-TAP框架，用于评估图像-文本对齐度，能同时进行多目标和单目标评分，在性能和效率上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有图像-文本对齐评估方法缺乏全面基准，且缺少关键特性：与人类判断对齐、长序列处理、推理效率、多目标评分适用性

Method: 基于大型视觉语言模型构建插件式架构，通过在冻结隐藏状态上训练轻量级岭回归层，生成细粒度多目标分数

Result: MULTI-TAP在7-8B规模下性能显著优于现有指标，与GPT-4o基准相当，在效率和多目标基准上优于VisionREWARD

Conclusion: MULTI-TAP为可靠视觉语言应用提供了强大评估工具，新数据集EYE4ALL为开发更易访问AI系统奠定了基础
...
Method: 使用DNN的特征映射图，将其添加到干净图像中，以干扰目标模型的分类，实现单次查询攻击。

Result: ZQBA能够在不同模型和数据集（CIFAR、Tiny ImageNet）间转移对抗样本，在单次查询下比现有方法更有效，同时保持扰动不可感知。
...
TL;DR: 提出了一种在one-hot空间中直接保持扩散核心特性的真实离散扩散框架，通过协调机制避免了传统伪离散扩散方法的问题。


<details>
  <summary>Details</summary>
Motivation: 传统伪离散扩散方法在连续潜在空间中进行扩散或依赖掩码策略，未能真正在离散空间中保持扩散特性。

Method: 使用浮点编码的one-hot类数据作为扩散输入，引入时间步条件交叉熵损失，在扩散模型输出和原始one-hot标签之间建立桥梁。

Result: 在分类任务上优于基线方法，并在图像描述生成中展现出优秀的文本生成能力。

Conclusion: ADD框架成功地在离散空间中保持了扩散特性，为判别性和生成性学习建立了协同联系。
...
TL;DR: 提出了一种用于概念瓶颈模型第二阶段的新型不确定性感知可解释分类器，通过学习二元类级概念原型来提升分类性能和不确定性量化能力。


<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型在提供语义可解释性的同时，往往牺牲预测性能，且概念预测到最终标签决策的不确定性传播研究不足。

Method: 学习一组二元类级概念原型，使用预测概念向量与每个类原型之间的距离作为分类分数和不确定性度量，这些原型也作为可解释的分类规则。

Result: 该框架通过基于学习到的二元类级概念原型对不确定或异常输入进行符合预测，增强了可解释性和鲁棒性。

Conclusion: 提出的方法在保持概念瓶颈模型可解释性的同时，提升了预测性能并有效量化了不确定性。
...
TL;DR: Code2Video是一个基于代码的代理框架，通过可执行Python代码生成教育视频，包含规划器、编码器和批评器三个协作代理，在MMMC基准测试中表现优于直接代码生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型难以制作专业教育视频，因为这类视频需要学科知识、精确视觉结构和连贯过渡，而通过可渲染环境的代码控制能更好地满足这些需求。

Method: 框架包含三个代理：规划器将讲座内容结构化并准备视觉资源；编码器将结构化指令转换为可执行Python代码；批评器利用视觉语言模型优化空间布局和清晰度。

Result: Code2Video相比直接代码生成有40%的改进，生成的视频质量可与人工制作的教程相媲美，在TeachQuiz等评估指标上表现优异。
...
TL;DR: 提出了MetaLogic评估框架，通过语义等价但语法不同的提示词对生成图像，检测文本到图像模型在语义一致性方面的鲁棒性问题，无需真实图像作为参考。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在输入提示词发生微小语言变化时难以保持语义一致性，即使这些提示词在逻辑上是等价的，这暴露了模型在推理和泛化方面的鲁棒性不足。

Method: 利用蜕变测试生成语义相同但语法不同的提示词对，直接比较生成的图像对来识别不一致性，从而检测模型在保持预期含义方面的失败。

Result: 在多个最先进的T2I模型上评估，发现即使是Flux.dev和DALLE-3这样的SOTA模型也分别有59%和71%的错位率，揭示了广泛的鲁棒性失败。

Conclusion: MetaLogic框架高效、可扩展且有效，能够发现现有评估指标忽略的细粒度逻辑不一致性，可用于模型调试和改进。
...
TL;DR: SF-SPA是一个自动化框架，通过街景照片评估建筑立面的光伏潜力，结合计算机视觉和AI技术解决透视失真、语义理解和空间布局优化等挑战。


<details>
  <summary>Details</summary>
Motivation: 建筑立面是密集城市环境中太阳能发电的重要未开发资源，但由于复杂的几何形状和语义组件，评估其光伏潜力具有挑战性。

Method: 四阶段流程：几何校正、零样本语义分割、LLM引导的空间推理和能源模拟，结合计算机视觉和人工智能技术。
...
TL;DR: TrajVLM-Gen是一个两阶段框架，通过预测符合物理规律的运动轨迹来生成物理一致的视频，在UCF-101和MSR-VTT数据集上取得了优异的FVD分数。
...
Method: 提出两阶段框架：首先使用视觉语言模型预测粗粒度的运动轨迹以保持物理一致性，然后通过基于注意力的机制进行细粒度运动精化来指导视频生成。基于视频跟踪数据构建了轨迹预测数据集。

Result: 在UCF-101和MSR-VTT上的实验表明，TrajVLM-Gen优于现有方法，分别达到了545和539的FVD分数。

Conclusion: 该框架能够生成物理一致的运动视频，在视频生成质量上表现出色。
...
TL;DR: PhraseStereo是首个将短语-区域分割引入立体图像对的数据集，通过GenStereo从单视图数据生成右视图图像，为多模态学习提供深度线索。


<details>
  <summary>Details</summary>
Motivation: 现有短语定位研究主要局限于单视图图像，忽视了立体视觉中丰富的几何线索。

Method: 基于PhraseCut数据集，利用GenStereo生成准确的右视图图像，扩展短语定位到立体领域。

Result: 创建了包含立体图像对、对齐分割掩码和短语注释的PhraseStereo数据集。

Conclusion: PhraseStereo为语言、视觉和3D感知交叉领域的研究奠定了基础，促进能够同时推理语义和几何的模型发展。
...
TL;DR: NSARM是一个基于自回归模型的实时图像超分辨率框架，通过两阶段训练实现高效高质量的图像重建，在保持快速推理的同时显著提升了输入图像质量的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有Real-ISR方法存在效率与质量的权衡问题：基于扩散模型的方法要么速度慢但质量高，要么速度快但质量低，且对输入图像退化程度敏感，容易产生过度增强的伪影。

Method: 提出Next-Scale Autoregressive Modeling (NSARM)框架：1) 先训练转换网络将低质量图像映射到初步尺度；2) 进行端到端的全模型微调，增强鲁棒性而不损害生成能力。

Result: 广泛的定量和定性评估表明，NSARM在视觉结果上优于现有Real-ISR方法，同时保持快速推理速度，对输入图像质量具有更高的鲁棒性和更强的泛化性能。

Conclusion: NSARM作为纯自回归模型，在实时图像超分辨率任务中实现了效率与质量的良好平衡，特别在输入鲁棒性方面表现突出，展示了强大的实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统分类方法往往忽略不同层次类别之间的内在关系，从而丢失重要的监督信号。

Method: 使用高斯混合模型和注意力机制来捕捉层次特定的特征，模拟人类处理方式，显式建模类间关系和更高层次的不平衡类别分布。

Result: 在CIFAR100和ModelNet40数据集上，线性评估中性能优于现有层次对比学习方法2个百分点。
...
TL;DR: 提出了一种结合移位窗口自注意力和Mamba选择性扫描的混合架构，用于视频超分辨率，通过Gather-Scatter Mamba机制解决遮挡伪影问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于RNN的VSR方法存在梯度消失、缺乏并行性和推理速度慢的问题，而Transformer在长序列上存在二次复杂度限制。Mamba提供了线性时间复杂度的替代方案，但缺乏空间上下文聚合能力。

Method: 结合移位窗口自注意力进行空间上下文聚合，使用Mamba选择性扫描进行高效时间传播，并引入Gather-Scatter Mamba机制进行特征对齐。

Result: 提出的混合架构在保持高效性的同时，能够有效处理长序列视频，减少遮挡伪影，并实现跨帧信息的有效再分布。

Conclusion: GSMamba通过结合自注意力和选择性状态空间模型的优势，为视频超分辨率提供了一种高效且有效的解决方案，特别适用于长序列处理。
...
TL;DR: 提出了一种名为AI-CNet3D的新型混合深度学习模型，通过将交叉注意力机制集成到3D CNN中，从OCT体积中提取关键特征，显著提升青光眼分类性能并减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 传统将3D OCT体积压缩为2D报告的做法会导致关键结构细节丢失，需要一种能够保留3D结构信息并有效提取青光眼相关特征的模型。

Method: 使用交叉注意力机制的3D CNN，通过沿两个轴分割体积并应用交叉注意力来捕捉半视网膜区域间的不对称性，同时整合视神经头和黄斑信息，引入CARE可视化交叉注意力输出。

Result: 在两个大型数据集上验证，模型在所有关键指标上优于最先进的注意力和卷积模型，计算效率高，参数数量减少100倍，同时保持高诊断性能和可比的计算量。

Conclusion: AI-CNet3D模型通过解剖学指导的交叉注意力网络有效提升了青光眼分类性能，具有高解释性和解剖学一致性，为3D OCT数据分析提供了高效解决方案。
...
TL;DR: 本研究通过任务型调查探讨了医学影像迁移学习中源数据集选择的人类决策过程，挑战了传统的"越相似越好"观点，发现选择受任务依赖性、社区实践和多种相似性因素影响。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像迁移学习中源数据集选择主要依赖研究者直觉而非系统原则，这会影响算法的泛化性和患者治疗效果。

Method: 采用以人为中心的HCI视角，通过任务型调查问卷研究机器学习从业者如何选择源数据集，关注人类决策过程而非模型基准测试。

Result: 发现选择决策具有任务依赖性，受社区实践、数据集属性和计算/感知/语义相似性影响，但相似性评分与预期性能并不总是一致。参与者常使用模糊术语。
...
TL;DR: PAL-Net是一个自动化深度学习管道，用于在3D面部扫描上定位50个解剖学标志点，结合了粗对齐、感兴趣区域过滤和注意力增强的补丁式点状CNN，在精度和计算成本之间取得了良好平衡。


<details>
  <summary>Details</summary>
Motivation: 3D面部扫描的手动标注耗时且依赖专家经验，现有深度学习方法大多关注伪标志点或需要复杂输入表示，限制了临床适用性。

Method: 结合粗对齐、感兴趣区域过滤、初始标志点近似，以及注意力机制增强的补丁式点状CNN。

Result: 在214个健康成人标注扫描上，平均定位误差3.686mm，解剖距离误差2.822mm；在FaceScape数据集700个样本上，点误差0.41mm，距离误差0.38mm。

Conclusion: PAL-Net在数据集和面部区域间具有良好泛化能力，为高通量3D人体测量分析提供了轻量级、可扩展的解决方案，有潜力支持临床工作流程并减少对人工标注的依赖。
...
TL;DR: 提出了一种新的自监督学习策略，针对单次不完整观测模型的反问题，通过引入等变重建网络和自监督分割损失实现无偏监督损失估计。


<details>
  <summary>Details</summary>
Motivation: 解决在难以获取真实参考数据时，基于学习的反问题重建方法的需求，特别是在测量数据通过单一不完整观测模型获取的挑战性场景下。

Method: 结合自监督分割损失和等变重建网络，定义了重建网络背景下的等变性新概念，确保对监督损失的无偏估计。

Result: 在图像修复、加速磁共振成像和压缩感知等实验中，该方法在高度秩不足前向模型设置下达到了最先进的性能。

Conclusion: 所提出的自监督学习策略在缺乏真实参考数据的情况下，能够有效解决反问题重建，并在多个应用领域展现出优越性能。
...
Method: 基于特征空间中分辨率差异语义方向的发现，提出向量平移特征对齐框架，通过建模分辨率特定的特征差异来进行跨分辨率行人重识别。

Result: 在多个跨分辨率行人重识别基准测试上的实验结果表明，该方法显著优于现有最先进基线模型，同时获得更高效率。
...
TL;DR: InfVSR是一种用于长视频超分辨率的自回归一步扩散方法，通过因果DiT结构和单步扩散蒸馏实现高效处理，在保持语义一致性的同时达到58倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有视频超分辨率方法在处理长序列时面临效率低下和可扩展性差的问题，多步去噪和时域分解导致计算成本高且产生伪影。

Method: 将VSR重新定义为自回归一步扩散范式，采用因果DiT结构保持局部和全局一致性，通过滚动KV缓存和联合视觉引导，并使用补丁级像素监督和跨块分布匹配进行单步扩散蒸馏。

Result: 实现了最先进的超分辨率质量，语义一致性显著提升，相比MGLD-VSR等方法速度提升高达58倍，能够高效处理无限长度视频。

Conclusion: InfVSR突破了长视频超分辨率的限制，通过创新的自回归一步扩散范式实现了高效、可扩展的处理，并建立了专门的长视频评估基准。
...
TL;DR: JEPA-T是一个统一的多模态框架，通过联合嵌入预测Transformer处理图像和文本的离散标记，结合交叉注意力和文本嵌入注入来增强融合效果，在ImageNet-1K上表现出强大的数据效率和开放词汇泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决现代文本到图像生成中基于标记的架构在文本与视觉标记融合方面的挑战，旨在平衡条件强度与骨干网络通用性。

Method: 将图像和标题编码为离散的视觉和文本标记，使用联合嵌入预测Transformer处理，在特征预测器后加入交叉注意力进行条件去噪，并在流匹配损失前注入原始文本嵌入以改善训练对齐。

Result: 在ImageNet-1K评估中，JEPA-T实现了强大的数据效率和开放词汇泛化能力，持续优于非融合和晚期融合基线方法。

Conclusion: 晚期架构融合与目标级对齐相结合，在基于标记的文本到图像生成中提供了条件强度与骨干通用性之间的有效平衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的视觉定位方法即使从已知相机姿态的映射图像开始，仍需要数小时到数分钟的地图构建时间。本研究旨在探索是否能在更短时间内达到竞争性的定位精度。

Method: 将多个映射图像表示为3D空间中锚定的特征集合，利用这些映射特征预测查询图像与场景的对应关系，从而估计相机姿态。结合图像检索技术实现快速定位。

Result: FastForward在最小地图准备时间下达到了最先进的定位精度，并在包括大规模室外环境在内的未见领域展现出强大的泛化能力。

Conclusion: FastForward证明了通过单次前向传播实现高效视觉定位的可行性，为实时定位系统提供了新的可能性。
...
TL;DR: 提出一种即插即用的精炼模块来增强自回归模型中视觉序列的空间对应建模，通过后预训练步骤联合优化所有生成标记，提升视觉语言建模质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在视觉语言数据中表现不佳，因为视觉信号的空间特性与下一标记预测的顺序依赖性存在冲突，导致次优结果。

Method: 设计一个后预训练的精炼模块，利用全局上下文和标记间关系，联合优化自回归模型生成的所有标记，缓解顺序生成中的错误累积问题。

Result: 实验表明该方法提高了生成质量，增强了模型产生语义一致结果的能力。

Conclusion: 提出的精炼模块有效改善了自回归模型在视觉语言任务中的性能，通过增强空间对应建模解决了顺序生成中的关键问题。
...
Method: 提出SoftCFG方法，通过不确定性加权在所有token上分布自适应扰动，并引入步长归一化来限制累积扰动，稳定长序列生成。
...
TL;DR: TextCAM是一个新的解释框架，通过结合CAM的空间定位能力和视觉语言模型的语义对齐，为深度视觉模型提供包含自然语言的解释。


<details>
  <summary>Details</summary>
Motivation: CAM及其变体虽然能突出显示影响预测的空间区域，但缺乏对这些激活区域背后属性的语义洞察，限制了在高风险应用中的可信度。

Method: 使用CLIP嵌入和线性判别分析获得通道级语义表示，然后与CAM权重聚合生成文本描述，同时扩展方法将特征通道分组以提供更细粒度的视觉-文本解释。

Result: 在ImageNet、CLEVR和CUB数据集上的实验表明，TextCAM能产生忠实且可解释的推理，改善人类理解，检测虚假相关性，并保持模型保真度。

Conclusion: TextCAM通过将精确的空间定位与语义对齐相结合，为深度视觉模型提供了更丰富、更具洞察力的解释框架。
...
TL;DR: POVQA是一种数据高效的方法，通过时间池化将每秒视频压缩为单帧图像，结合轻量级监督训练LVLMs，显著提升视频问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有VQA方法虽然能处理1500+帧的长视频，但仅对应50秒内容且信息损失严重，需要更高效的视频压缩和推理方法。

Method: 使用多种时间池化方法（运动模糊、加权平均等）生成1fps输入，在ReasonVQA数据集上对QWEN-2.5-VL 7B进行SFT和DPO训练。

Result: 在ReasonVQA数据集上，F1从0.212提升至0.543，BLEU-4从0.031提升至0.291，ROUGE-L从0.196提升至0.528，推理质量显著提高。

Conclusion: 该方法在各种池化方案下都保持性能增益，在TVQA零样本测试中也表现良好，表明对时间证据总结具有强鲁棒性。
...
Conclusion: ImageDoctor提供了全面可解释的图像质量评估，在人类偏好对齐和模型优化方面表现出色，是有效的评估指标和奖励模型。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱分类模型对对抗攻击高度脆弱，现有方法依赖定制化设计，可扩展性有限且无法有效防御强攻击。

Method: 引入对抗训练到高光谱领域，提出AT-RA方法，通过数据增强增加光谱信息多样性，确保空间平滑性来保护光谱语义信息。

Result: AT-RA将对抗鲁棒性提升21.34%（对抗AutoAttack）和18.78%（对抗PGD-50），同时将良性准确率提高2.68%。

Conclusion: AT-RA方法能有效解决高光谱数据特有的挑战，在保持良性性能的同时显著提升对抗鲁棒性。
...
Method: 将密钥与扩散模型的潜在人脸表示结合，使用面部掩码约束生成以保留身份无关特征，通过确定性前向和后向扩散过程确保原始人脸可恢复。

Result: 该方法生成的人脸匿名化图像视觉上与原图相似度更低，同时能够通过正确密钥恢复原始人脸。
...
TL;DR: KeySG是一个分层3D场景图框架，通过关键帧选择和多模态信息增强，结合分层RAG管道处理复杂查询，在多个基准测试中表现优于现有方法。
...
Method: 构建分层图结构（楼层、房间、对象、功能元素），通过优化几何和视觉覆盖的关键帧选择，利用VLM提取场景信息，避免显式建模对象关系边，采用分层RAG管道提取相关上下文。

Result: 在四个基准测试（包括3D对象分割和复杂查询检索）中，KeySG在大多数指标上优于先前方法，展示了其优越的语义丰富性和效率。

Conclusion: KeySG通过关键帧增强的分层场景图表示和分层RAG管道，有效解决了3D场景图的语义限制和可扩展性问题，实现了更通用的任务无关推理和规划。
...
TL;DR: 提出了一种基于博弈论的融合方法SCADA，用于多标签理解车载视频，通过计算模型可靠性和上下文先验来生成可校准的后验概率，显著提升了自动驾驶系统中视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在自动驾驶系统中应用日益广泛，但幻觉问题限制了其在安全关键场景中的可靠性。需要一种能够校准模型输出、适应漂移并保持解释性的融合方法。

Method: 使用Shapley值归因的上下文感知Dawid-Skene与一致性方法，学习每个模型在每个标签上的上下文条件可靠性，将模型报告转换为一致性保护的似然比，结合上下文先验和通过Shapley更新的公共声誉状态。

Result: 与最佳单模型相比，Hamming距离减少23%，Macro-F1提升55%，Micro-F1提升47%，支持VLM融合作为自动驾驶系统中校准、可解释且鲁棒的决策支持组件。
...
TL;DR: EvoWorld是一个结合全景视频生成与演化3D记忆的世界模型，通过利用演化3D重建作为显式空间指导来增强视频生成过程的视觉真实性和几何一致性。
...
Method: 首先利用具有细粒度视图控制的视频生成器生成未来视频帧，然后使用前馈即插即用transformer演化场景的3D重建，最后通过基于演化显式3D记忆的几何重投影来合成未来帧。
...
Method: 通过提示引导的多模态对齐模块和基于先验的掩码重定向模块，提供鲁棒的多模态条件和精确的掩码序列，然后将先验掩码序列输入预训练的掩码驱动视频生成模型来合成编辑后的视频。

Result: 在新构建的多主体基准MSVBench上的大量实验验证，IMAGEdit持续超越最先进的方法。

Conclusion: IMAGEdit具有强大的泛化能力，能够处理任意数量主体的视频编辑，并且与任何掩码驱动视频生成模型兼容，显著提高了整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM遗忘方法依赖辅助语言模型、保留数据集或商业AI服务，这通常不实用且可能引入额外隐私风险。

Method: 通过识别目标令牌（用于优化遗忘目标）和非目标令牌（用于保持模型效用），直接在令牌级别进行优化。

Result: 在多个基准数据集上，DTO在遗忘质量上比最新基线方法提升了高达16.8倍，同时保持可比较的模型效用水平。
...
TL;DR: 提出TAMA框架，一种工具增强的多模态代理，用于程序性活动理解，通过多媒体返回工具实现训练免费的多模态推理
...
Method: TAMA框架使用多媒体返回工具进行交错的模态推理，在训练免费设置下工作，具有灵活的工具选择能力

Result: 在ProMQA-Assembly数据集上的实验表明，该方法能提升视觉语言模型性能，特别是GPT-5和MiMo-VL模型
...
Method: 通过精心设计的合成流程生成任务，包含人工验证环节，评估代理在异构搜索空间（包括生产力软件、云文件系统、邮件、聊天记录和开放网络）中识别相关事实的能力。

Result: 发布了涵盖10个领域的15个深度研究任务，评估了多种开源和闭源模型及策略，揭示了各自的优势和不足。

Conclusion: DRBench为推进企业深度研究提供了关键路径，有效评估了AI代理在复杂企业环境中的研究能力。
...
TL;DR: PrimeX是一个包含858名美国居民公共意见调查数据的数据集，额外收集了受访者的书面解释和世界观信息，用于研究语言模型如何利用个人信仰系统进行个性化改进。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的普及，需要更好地将个体用户特征融入模型。研究探索语言模型是否能利用个人信仰系统的某些方面来改进对齐效果。

Method: 开发PrimeX数据集，收集858名美国居民的公共意见调查数据，包括受访者对特定观点的书面解释和原始世界观信仰调查，用于分析信仰信息和世界观对语言模型个性化的价值。

Result: 对数据进行了广泛初步分析，证明了信仰解释和世界观信息在个性化语言模型方面的价值。

Conclusion: PrimeX中的额外信仰信息可以为NLP和心理学研究社区带来益处，为后续研究开辟了新途径。
...
Method: 引入PREFDISCO评估方法，使用基于心理学的角色和稀疏偏好，将静态基准转化为交互式个性化任务。创建相同问题需要不同推理链的场景，取决于用户上下文，因为最优解释方法因个人专业知识和偏好而异，同时保持事实准确性。

Result: 在10个任务上评估21个前沿模型，发现29.0%的朴素个性化尝试产生比通用响应更差的偏好对齐，但通用响应也无法有效服务个体用户需求。

Conclusion: 个性化推理需要专门开发，而不是自然出现。PREFDISCO将个性化推理确立为可衡量的研究前沿，揭示了当前LLM交互能力的根本限制，为在教育、医疗和技术领域开发能够适应个体用户的系统提供了基础。
...
TL;DR: 提出了BiasFreeBench基准，用于统一评估LLM去偏方法，通过重组现有数据集为统一查询-响应格式，并引入Bias-Free Score响应级指标来衡量模型响应的公平性。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法评估存在基线不一致、指标多样的问题，且主要基于概率比较而非真实用户交互场景，需要建立统一的评估基准。

Method: 构建BiasFreeBench基准，比较8种主流去偏方法（4种提示方法和4种训练方法），在两种测试场景（多选QA和开放式多轮QA）下评估，使用Bias-Free Score作为响应级指标。

Result: 系统比较了不同去偏方法在提示vs训练范式、模型大小、以及训练策略对未见偏见的泛化能力等关键维度上的表现。

Conclusion: BiasFreeBench为偏见缓解研究建立了统一的测试平台，将公开发布该基准以促进研究发展。
...
TL;DR: TASER是一种利用大型推理模型进行自动翻译质量评估的指标，通过系统化逐步评估在WMT24指标共享任务中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有自动翻译评估指标缺乏可解释性和透明度的问题，利用大型推理模型的显式推理能力来提升翻译质量评估的准确性和可见性。

Method: 使用大型推理模型进行系统化、逐步的翻译质量评估，采用结构化提示模板，评估不同推理深度对评估质量的影响。

Result: 在系统级评估中，TASER在基于参考和无参考设置下均获得最高的软配对准确率；在片段级评估中，无参考变体在所有无参考方法中排名第一。
...
<details>
  <summary>Details</summary>
Motivation: 虽然RAG在大型语言模型中广泛应用，但在心电图-语言模型领域缺乏开源实现和系统研究，需要填补这一空白。

Method: 开发了ELM专用的RAG管道，包括检索机制和生成组件，并在三个公共数据集上进行基准测试和消融研究。

Result: 实验表明，带有RAG的ELM在性能上持续优于非RAG基线，并揭示了关键的ELM设计考虑因素。

Conclusion: 该工作为心电图-语言模型提供了首个开源RAG实现，证明了RAG在改善ELM自然语言生成方面的有效性。
...
TL;DR: 提出校准概率自动评分器的方法，通过匹配目标偏好分布来提高LLM作为自动评分器的可靠性，减少位置偏见并改善校准效果。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为自动评分器存在局限性，因为它们基于离散偏好标签训练，无法处理主观、模糊或细微的任务，需要建模完整的偏好分布。

Method: 提出两种学习方法：1) 对密集概率标签的直接监督微调；2) 对稀疏二元标签的强化学习方法，使用分布匹配目标。

Result: 实验结果显示，使用分布匹配目标微调的自动评分器能够生成与目标偏好分布更好对齐的言语化概率预测，改善校准效果并显著降低位置偏见，同时在客观任务上保持性能。

Conclusion: 通过校准概率自动评分器以匹配目标偏好分布，可以显著提高LLM作为自动评分器的可靠性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在文本生成任务中表现出色，但在文本分类特别是需要细微区分的修订分类任务中表现不足。由于修订标注数据昂贵且稀缺，需要开发参数高效的微调方法。

Method: 提出IR-Tuning框架，基于梯度范数分布动态选择重要LLM层进行微调，同时冻结冗余层，实现参数高效微调。

Result: 实验表明IR-Tuning在多种文本修订任务上优于其他层间PEFT基线方法，具有快速收敛、低GPU内存消耗和小样本有效性等优势。

Conclusion: IR-Tuning为LLMs在数据稀缺的细微文本分类任务中提供了一种有效的参数高效微调解决方案。
...
TL;DR: 本文提出了SafePassage方法，通过生成"安全段落"来减少LLM在信息抽取中的幻觉问题，将幻觉率降低了85%。


<details>
  <summary>Details</summary>
Motivation: 黑盒大语言模型使信息抽取易于配置但难以信任，抽取的信息不能保证基于文档，存在幻觉问题。

Method: 采用三步流水线：1) LLM提取器生成结构化实体及其上下文；2) 基于字符串的全局对齐器；3) 评分模型。

Result: 将幻觉率降低了85%，与人类对抽取质量的判断高度一致，且微调的transformer编码器在标记不安全段落方面优于LLM评分模型。

Conclusion: SafePassage能有效减少信息抽取中的幻觉，并可用于评估LLM，小规模任务特定示例的微调模型表现优于LLM评分器。
...
<details>
  <summary>Details</summary>
Motivation: 自动生成的放射学报告在现有评估指标上得分很高，但未能获得临床医生的信任，这揭示了当前评估指标在评估生成报告质量方面的根本缺陷。

Method: 定义临床基础的标准，涵盖临床对齐和关键指标能力，包括区分度、鲁棒性和单调性。使用细粒度数据集，系统评估现有指标。

Result: 揭示现有指标在解释临床语义方面的局限性，如无法区分临床显著错误、过度惩罚无害变异、在错误严重程度级别上缺乏一致性。
...
<details>
  <summary>Details</summary>
Motivation: 基于Transformer的语言模型在NLP领域带来革命性进步的同时，也引入了模型透明度和可信度的重大挑战。研究人员开发了大量解释方法和评估指标，但选择最优可解释性方法仍很困难。

Method: 开发o-mega超参数优化工具，在社交媒体帖子与反驳声明配对的精选数据集上，系统探索不同的可解释方法及其超参数配置。

Result: 该工具在自动事实核查系统中展示了改进的透明度，能够显著增强声明匹配模型在错误信息检测等关键应用中的可解释性。

Conclusion: 这种解释方法的自动优化可以为更可信和透明的AI系统做出贡献，特别是在错误信息检测等关键应用中。
...
TL;DR: CORTEX是一个用于安全运营中心(SOC)告警分诊的多智能体LLM架构，通过专业化智能体协作处理告警，显著减少误报并提高调查质量。


<details>
  <summary>Details</summary>
Motivation: SOC面临每日数万告警的过载问题，传统检测管道脆弱且缺乏上下文，而单一LLM方法在处理嘈杂企业数据时效果有限且透明度不足。

Method: 提出多智能体LLM架构，包括行为分析智能体检查活动序列、证据收集智能体查询外部系统、推理智能体综合发现生成可审计决策。

Result: 在多样化企业场景中，CORTEX相比最先进的单智能体LLM显著减少了误报并提高了调查质量。

Conclusion: 多智能体协作方法在高风险告警分诊中比单一模型方法更有效，提供了更好的透明度和准确性。
...
TL;DR: TokMem是一种token化的程序性记忆系统，将重复程序存储为紧凑可训练的嵌入，通过内存令牌编码程序地址和控制信号，在保持骨干模型冻结的同时实现持续适应。
...
Method: 引入TokMem，一种token化的程序性记忆，将重复程序存储为紧凑可训练的嵌入。每个内存令牌编码程序地址和控制信号，引导生成过程，支持持续适应时保持骨干模型冻结。

Result: 在1000个原子回忆任务和函数调用任务上进行评估，TokMem持续优于检索增强生成，同时避免重复上下文开销，且使用更少参数进行微调。

Conclusion: TokMem为LLMs提供了一种可扩展和模块化的替代方案，替代提示工程和微调，提供了明确的程序性记忆。
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文剪枝技术（如LLMLingua）在通用文本上表现良好，但忽略了代码特有的结构和依赖关系，导致在编程任务中性能不佳。高API成本和生成延迟成为代码LLM处理长上下文的主要瓶颈。
...
TL;DR: 本研究探索现成大语言模型在评分预测任务中的表现，发现用户评论能显著提升预测性能，效果与传统矩阵分解方法相当，为解决冷启动问题提供了有前景的方案。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注分类或排序任务，而缺乏对需要语言和数学推理的Likert量表评分预测的探索。该任务具有重要工业应用价值，但现成大语言模型在此领域的应用仍未被充分研究。

Method: 使用八个现成大语言模型在三个数据集上进行综合实验，提供不同的上下文信息，包括用户评论和一般偏好描述，并尝试让模型首先生成假设性评论。

Result: 用户撰写的评论显著提升了大语言模型的评分预测性能，效果与传统矩阵分解方法相当。具体物品的评论比基于非特定物品的一般偏好描述更有效。让模型首先生成假设性评论能进一步提升预测性能。
...
TL;DR: 该论文探索了在日立JP1中间件这一专业IT运维微领域中，通过代理微调方法提升大语言模型的领域适应能力，在JP1认证考试问题上相比基础模型实现了14%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有代理LLM主要通过上下文学习进行多步推理，但存在输入冗长、计算成本高的问题。代理微调虽然能够通过训练让模型内化程序性推理和领域知识，但在专业微领域中的有效性尚不明确。
...
Conclusion: 代理微调在复杂微领域中具有显著潜力，能够有效提升领域特定推理能力，为专业领域的LLM应用提供了有前景的方向。
...
TL;DR: 提出了Agent-ScanKit框架，通过三种正交探测范式评估多模态代理的记忆与推理能力，发现在GUI任务中机械记忆往往优于系统推理。


<details>
  <summary>Details</summary>
Motivation: 现有多模态代理在复杂或域外任务中可靠性有限，需要探究其推理是否具有伪相关性。

Method: 开发了视觉引导、文本引导和结构引导三种探测范式，在无需访问模型内部的情况下量化记忆与推理的贡献。

Result: 在5个公开GUI基准测试和18个多模态代理上的结果显示，机械记忆通常超过系统推理，模型主要作为训练对齐知识的检索器，泛化能力有限。
...
TL;DR: MOSS-Speech是一个真正的语音到语音大语言模型，无需文本中间件直接理解和生成语音，通过模态分层架构和冻结预训练策略，在保持文本LLM推理能力的同时增加原生语音能力。


<details>
  <summary>Details</summary>
Motivation: 传统语音对话系统使用级联流水线会丢弃副语言线索并限制表达性，而现有端到端方法仍依赖文本中间件造成瓶颈，需要开发真正的语音到语音模型。

Method: 结合基于模态的层分割架构和冻结预训练策略，在预训练文本LLM基础上添加原生语音能力，无需文本指导直接处理语音。

Result: 在口语问答任务上达到最先进结果，语音到语音性能与现有文本引导系统相当，同时保持竞争力的文本性能。
...
TL;DR: Graph2Eval是一个基于知识图谱的框架，能够自动生成多模态文档理解任务和网络交互任务，用于全面评估智能体的推理、协作和交互能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的合成数据方法主要针对LLM训练和评估设计，无法直接应用于需要工具使用和交互能力的智能体任务。现有研究大多局限于文本或图像分析，未能系统建模网络环境中的多步交互。

Method: 基于多源外部数据构建知识图谱作为任务空间，通过子图采样、任务模板和元路径将语义关系转化为结构化多模态任务，并采用基于节点可达性、LLM评分和相似性分析的多阶段过滤管道保证任务质量和可执行性。

Result: 构建了Graph2Eval-Bench数据集，包含1,319个涵盖文档理解和网络交互场景的任务。实验表明Graph2Eval能有效生成区分智能体和模型性能的任务，揭示不同设置下推理、协作和网络交互方面的差距。

Conclusion: Graph2Eval为智能体评估提供了新视角，能够全面评估多类型智能体（单智能体、多智能体、网络智能体）的推理、协作和交互能力。
...
TL;DR: 提出CopyPasteLLM，通过高复制度响应偏好训练来减少RAG中的幻觉问题，在少量训练样本下显著提升上下文忠实度


<details>
  <summary>Details</summary>
Motivation: RAG系统中LLMs可能不信任提供的上下文，导致幻觉问题，影响可靠性。研究发现响应复制度与上下文不忠实幻觉呈负相关

Method: 两阶段高复制度响应偏好训练，设计三种提示方法增强复制度，构建全自动管道将生成响应转化为高复制度偏好数据

Result: 在FaithEval、ConFiQA和PubMedQA上表现最佳，FaithEval准确率比最佳基线提升12.2%到24.5%，仅需365个训练样本（基线数据的1/50）

Conclusion: CopyPasteLLM通过重新校准对内部参数知识的依赖而非外部知识来提升上下文忠实度，高复制度响应能有效控制幻觉
...
<details>
  <summary>Details</summary>
Motivation: 现有系统往往专注于孤立改进，缺乏针对鲁棒性和适应性的统一设计，需要构建能够处理复杂现实任务的可扩展、弹性自适应AI助手。

Method: 集成三个核心组件：结合规划和执行智能体与批评模型投票的集体多智能体框架、跨越工作记忆、语义记忆和程序记忆的分层记忆系统、用于搜索、代码执行和多模态解析的精细化工具套件。

Result: 在综合基准测试中，该框架持续优于开源基线，并接近专有系统的性能水平。

Conclusion: 系统级集成的重要性得到验证，为构建能够在不同领域和任务中运行的可扩展、弹性和自适应AI助手指明了路径。
...
TL;DR: 提出一个从议会录音构建大规模多语言语音数据集的可扩展流程，包含媒体检索和两阶段对齐算法，在22个欧洲议会提取超过61k小时对齐语音数据，显著提升ASR模型性能。
...
Method: 开发可扩展的管道，包括鲁棒的媒体检索组件和两阶段对齐算法，处理非逐字记录和长音频，应用于22个欧洲议会录音。

Result: 提取61k+小时对齐语音段，19种语言超过1k小时，22种语言超过500小时高质量数据，微调现有ASR模型使词错误率平均降低41.8%。

Conclusion: 该管道能有效构建大规模多语言语音数据集，显著提升语音识别性能，为解决语言数据不足问题提供了可行方案。
...
TL;DR: 研究发现监督微调(SFT)的默认目标函数负对数似然(NLL)在模型后训练阶段存在局限性，提出基于概率的目标函数家族，发现模型能力连续体是决定目标函数效果的关键维度。


<details>
  <summary>Details</summary>
Motivation: 监督微调(SFT)的泛化能力有限，作者认为这是因为默认的负对数似然(NLL)目标在后训练阶段可能不再是最优的，因为此时模型已经编码了任务相关的先验知识，且监督信号可能较长且嘈杂。

Method: 研究了一类基于概率的目标函数家族，通过7个模型主干、14个基准测试和3个领域的综合实验和消融研究，分析了不同条件下目标函数的有效性。

Result: 发现模型能力连续体是决定目标函数行为的关键维度：在模型能力强的区域，倾向于先验的目标函数(如$-p$, $-p^{10}$及其阈值变体)持续优于NLL；在模型能力弱的区域，NLL占主导；在中间区域，没有单一目标函数占优。

Conclusion: 理论分析阐明了目标函数在连续体上的权衡关系，为根据模型能力自适应选择目标函数提供了理论基础。
...
TL;DR: GUI-KV是一种即插即用的KV缓存压缩方法，专门针对GUI代理设计，无需重新训练。它通过空间显著性引导和时间冗余评分技术，有效减少计算开销，在保持准确性的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有的KV缓存压缩方法无法有效处理GUI中的空间和时间冗余问题，导致GUI代理在处理高分辨率截图和长序列任务时效率低下、成本高昂。

Method: GUI-KV结合两种新技术：空间显著性引导（通过隐藏状态的L2范数增强注意力分数）和时间冗余评分（将历史帧的键投影到当前帧的键子空间来优先剪枝冗余历史）。

Result: 在标准GUI代理基准测试中，GUI-KV优于竞争基线，在适度预算下接近全缓存准确性。在AgentNetBench基准测试的5截图设置中，解码FLOPs减少38.9%，步骤准确性提高4.1%。

Conclusion: 利用GUI特定的冗余特性可以实现高效可靠的代理性能，GUI-KV为GUI代理提供了有效的缓存压缩解决方案。
...
TL;DR: 论文发现小推理模型在工具使用时存在"过度思考"问题，提出ThinkBrake训练无关的解码启发式方法，在保持或提高准确性的同时显著减少推理token数量。


<details>
  <summary>Details</summary>
Motivation: 小推理模型在工具使用过程中经常过度思考：它们达到正确的工具参数配置后继续推理，最终用错误的调用覆盖正确结果。

Method: 通过oracle rollout诊断过度思考问题，引入ThinkBrake方法监控</think>与当前top token之间的对数概率边际，在句子边界处触发终止。

Result: 在Berkeley Function Calling Leaderboard上，ThinkBrake在保持或提高准确性的同时，将token数量减少高达25%，优于各种基线方法。
...
TL;DR: CHIME是一个中文网络流行语解释数据集，用于评估大语言模型对网络流行语的理解能力。研究发现LLMs在解释流行语含义方面表现尚可，但在文化背景和来源追溯方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型是否真正理解在互联网上快速传播的病毒式内容——网络流行语，特别是中文网络环境中的流行语。

Method: 构建CHIME数据集，包含中文网络流行语及其详细注释；设计两个评估任务：1) 解释流行语含义、来源并生成示例句子；2) 在上下文句子中选择合适的流行语填空。

Result: LLMs能够解释部分流行语的含义，但对文化和语言细微差别的流行语类型表现显著下降；在追溯流行语来源方面持续困难；在选择题任务中表现低于人类水平。

Conclusion: 大语言模型对网络流行语的理解能力有限，特别是在文化背景和来源追溯方面存在明显缺陷。CHIME数据集将促进计算流行语理解的未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于强化学习的搜索代理方法依赖稀疏或基于规则的奖励，这可能导致代理陷入次优或错误的推理路径而无法恢复。

Method: 提出ReSeek框架，包含自我纠正机制（通过JUDGE动作判断信息和重新规划搜索策略）和密集的过程奖励函数（分解为正确性奖励和效用奖励），并引入新的基准数据集FictionalHot。

Result: 实验表明，使用ReSeek训练的代理在任务成功率和路径忠实度方面显著优于现有最先进基线方法。

Conclusion: ReSeek框架通过自我纠正机制和密集奖励设计，有效提升了搜索代理在复杂推理任务中的性能，解决了传统方法中代理无法从错误路径恢复的问题。
...
TL;DR: 提出CoT Vectors方法，通过紧凑向量表示编码多步推理知识，在教师-学生框架下优化，以低成本提升LLMs的推理能力，性能媲美参数高效微调方法。
...
Method: 引入CoT Vectors作为紧凑表示，在教师-学生框架下优化Learnable CoT Vectors，解决层间不稳定性问题。

Result: 在多个基准测试和模型上，CoT Vectors优于现有基线，性能接近参数高效微调方法，且需要更少可训练参数。

Conclusion: CoT Vectors不仅提供有效的推理增强方案，还揭示了LLMs中多步推理的功能组织机制，为理解模型推理过程提供新视角。
...
TL;DR: 提出了一种支持多语言的神经语音语言分割模型，通过可学习查询架构和大规模代码切换预训练，在多个基准测试中实现了23%-52%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在数据稀缺和架构优化方面的限制，为真实世界的多语言环境提供有效的语言分割解决方案。

Method: 集成可学习的基于查询的架构，结合多语言感知和大规模模拟代码切换数据的预训练。

Result: 在多个语言分割基准测试中达到最先进性能，相对性能提升23%至52%。

Conclusion: 该工作不仅推进了语言分割研究，还为代码切换语音技术建立了基础框架。
...
TL;DR: 本文为低资源语言Tenyidie创建了10,120个音节化单词的数据集，并应用多种深度学习模型进行音节划分，其中BLSTM模型在测试集上取得了99.21%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: Tenyidie是一种低资源的藏缅语系语言，在自然语言处理研究方面非常有限，特别是音节划分任务尚未有相关研究报道。

Method: 应用了LSTM、BLSTM、BLSTM+CRF和编码器-解码器等多种深度学习架构，在80:10:10划分的数据集上进行训练和测试。

Result: 在测试集上，BLSTM模型取得了99.21%的最高准确率，表明深度学习模型在Tenyidie语言音节划分任务上表现优异。

Conclusion: 这项工作为Tenyidie语言的自然语言处理应用奠定了基础，将在形态分析、词性标注、机器翻译等任务中发挥重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 替代文本生成任务存在用户标注噪声、标准不一致以及多模态大语言模型对上下文信息不敏感等问题，传统监督微调依赖准确标注但用户生成的替代文本往往存在缺陷。

Method: 提出多维度跨模态直接偏好优化(MCM-DPO)，在偏好对中学习识别更好选项，覆盖文本、视觉和跨模态因素的单维度、配对和多维度偏好优化。构建了TAlt和PAlt两个大规模高质量数据集。

Result: 实验结果表明MCM-DPO方法在替代文本生成任务上持续优于DPO和SFT方法，建立了新的最先进水平。

Conclusion: MCM-DPO方法有效解决了替代文本生成中的标注质量问题，通过偏好学习提升了生成性能，为盲人和低视力用户提供了更好的图像访问体验。
...
TL;DR: 本文研究了使用大型语言模型自动生成易读文本的方法，提出了多任务学习框架，包括基于检索增强生成和参数高效微调两种策略，在法语易读数据集上验证了多任务方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 手动创建易读文本耗时耗力，而现有对齐语料库稀缺且易读约束具有特异性，需要自动化解决方案来为认知障碍人群提供平等的信息获取途径。

Method: 采用多任务学习方法，联合训练文本摘要、文本简化和易读文本生成任务。探索了两种策略：基于检索增强生成的多任务上下文学习，以及使用LoRA的参数高效微调多任务学习。

Result: 在Mistral-7B和LLaMA-3-8B模型上的实验表明，多任务设置在所有配置中都优于单任务基线。RAG策略在跨域设置中具有泛化能力，而MTL-LoRA在域内配置中表现最佳。

Conclusion: 多任务学习框架能有效提升易读文本的自动生成质量，不同的学习策略在不同场景下各有优势，为易读内容的自动化生成提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 为认知障碍人士提供可访问信息对于他们的自主权、自决权和完整公民权至关重要，但手动易读文本改编耗时、昂贵且难以扩展，限制了在医疗、教育和公民生活中的关键信息获取。

Method: 构建了ETR-fr数据集，实施参数高效微调在预训练语言模型和大型语言模型上建立生成基线，并引入基于自动指标和人工评估的评估框架，使用36个问题评估表与指南对齐。

Result: 结果显示预训练语言模型与大型语言模型表现相当，并能有效适应领域外文本。

Conclusion: AI驱动的易读文本生成提供了可扩展的解决方案，预训练语言模型在易读文本生成任务中表现良好，能够有效处理领域适应问题。
...
TL;DR: ALARB是一个阿拉伯语法律推理数据集，包含13K+沙特商业法庭案例，用于评估LLMs在法律领域的多步推理能力，通过指令微调可使12B参数模型在判决预测任务上达到GPT-4o水平。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语基准测试主要关注检索和理解任务，缺乏专门针对多步推理的数据集，特别是在开放式的阿拉伯语法律推理领域。

Method: 构建包含案件事实、法庭推理、判决结果和引用条款的沙特商业法庭数据集，定义判决预测、推理链补全和相关法规识别等挑战性任务，并对代表性阿拉伯语LLMs进行基准测试和指令微调。

Result: 指令微调12B参数模型显著提升了其在判决预测和阿拉伯语判决生成方面的性能，达到与GPT-4o相当的水平。
...
TL;DR: 该研究探索如何将小型高效LLM适配到法罗语这种低资源北日耳曼语言。通过从英语模型出发，在相关斯堪的纳维亚语言上继续预训练，然后对法罗语进行微调，比较了全参数微调和LoRA参数高效调优的效果。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言法罗语的LLM适配问题，探索从相关语言迁移学习的方法，并评估不同微调策略对语言准确性和文本理解的影响。

Method: 从英语模型开始，在相关斯堪的纳维亚语言上继续预训练（单独或合并），然后对法罗语进行微调。比较全参数微调和LoRA方法，使用新构建的最小对基准和人工评估。

Result: 相关语言迁移至关重要，但最佳源语言因任务而异：冰岛语提升语言准确性，丹麦语增强理解能力。LoRA在基础模型上提高语言可接受性，全参数微调在理解任务上表现更好且在下游微调中更好保持模型能力。

Conclusion: 法罗语LLM适配需要任务特定的策略：语言准确性依赖冰岛语迁移和理解任务依赖丹麦语迁移，微调方法选择也需根据具体任务需求决定。
...
<details>
  <summary>Details</summary>
Motivation: 理解检索增强LLM机器翻译在真实噪声环境下的鲁棒性，这是实际部署中的常见挑战。

Method: 提出噪声合成框架和新指标，使用Qwen系列模型（标准LLM和大推理模型）在高中低资源语言对上评估习语翻译性能。

Result: 低资源语言对在噪声下退化更严重，大推理模型没有改进纠错能力且更容易受噪声影响，注意力从源习语转向噪声内容，置信度增加但准确性下降。

Conclusion: 当前方法存在局限性，需要在性能和鲁棒性之间权衡，需要自验证集成机制。
...
TL;DR: 提出了ManagerBench基准，用于评估LLM在现实管理场景中的决策能力，重点关注安全性与实用性之间的权衡。
...
Result: 前沿LLM在安全性与实用性的权衡中表现不佳：许多模型为推进操作目标持续选择有害选项，而其他模型则过度安全变得无效。模型能够准确识别危害，但存在优先级排序错误。

Conclusion: ManagerBench是评估智能体行为核心组成部分的挑战性基准：当操作目标和价值观激励冲突行动时做出安全选择。
...
TL;DR: 提出Erasable Reinforcement Learning (ERL)框架，通过识别、擦除和重新生成错误推理步骤，解决多跳推理中的分解错误、检索缺失和推理错误问题，显著提升搜索增强LLM的可靠性。


<details>
  <summary>Details</summary>
Motivation: 搜索增强的大型语言模型在复杂多跳推理中可靠性有限，主要面临三个挑战：分解错误、检索缺失和推理错误，任一环节的失败都会导致最终答案错误。

Method: 提出可擦除强化学习(ERL)框架，明确识别错误推理步骤，擦除这些步骤并在原位置重新生成推理，防止错误逻辑在推理链中传播。

Result: 在HotpotQA、MuSiQue、2Wiki和Bamboogle数据集上取得显著提升，3B模型比之前SOTA提升+8.48% EM和+11.56% F1，7B模型提升+5.38% EM和+7.22% F1。

Conclusion: 可擦除强化学习为LLM中的鲁棒多步推理提供了强大的范式转变，将脆弱的推理过程转变为更稳健的过程。
...
TL;DR: HalluGuard是一个4B参数的小型推理模型，专门用于缓解检索增强生成(RAG)中的幻觉问题，通过分类文档-声明对并提供证据支撑的论证，在保持较小模型规模的同时达到与更大模型相当的检测性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在NLP任务中表现出色，但容易产生幻觉，这限制了其在真实世界应用中的可信度。需要开发专门的方法来检测和缓解RAG系统中的幻觉问题。

Method: 结合(i)基于FineWeb的领域无关合成数据集，通过多阶段筛选和数据重构优化；(ii)合成的基础事实和幻觉声明；(iii)使用几率比偏好优化的偏好微调方法，将大模型推理能力蒸馏到小模型骨干中。

Result: 在RAGTruth子集上达到84.0%的平衡准确率，与MiniCheck(7B)和Granite Guardian 3.3(8B)等专门模型相当，但参数数量仅为它们的一半。在整个基准测试中达到75.7%的平衡准确率，与GPT-4o(75.9%)等大型通用LLM相当。

Conclusion: HalluGuard证明了通过精心设计的合成数据生成和偏好优化方法，可以构建参数效率高、性能优越的幻觉检测模型，为RAG系统的可靠性提供了有效解决方案。
...
TL;DR: Sci-SpanDet是一个用于检测学术文本中AI生成内容的结构感知框架，通过结合分段条件风格建模和多级对比学习，实现了跨领域的鲁棒检测和精确的span级定位。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在科学写作中的快速应用，引发了关于作者身份完整性和学术出版物可靠性的严重担忧。现有检测方法主要依赖文档级分类或表面统计特征，但忽视了细粒度的span定位，校准能力弱，且难以跨学科和生成器泛化。

Method: 结合分段条件风格建模与多级对比学习来捕捉细微的人类-AI差异并减轻主题依赖性；集成BIO-CRF序列标注与基于指针的边界解码和置信度校准，实现精确的span级检测和可靠的概率估计。

Result: 在包含10万个标注样本的新建跨学科数据集上，Sci-SpanDet实现了最先进的性能：AI-F1为80.17，AUROC为92.63，Span-F1为74.36。在对抗性重写下表现出强韧性，在IMRaD章节和不同学科间保持平衡的准确率。
...
TL;DR: 该论文研究了检索增强生成(RAG)在解决奥林匹克级别物理问题中的应用，提出了PhoPile多模态数据集，并评估了RAG对基础模型物理推理能力的提升效果。


<details>
  <summary>Details</summary>
Motivation: 虽然检索增强生成在多种任务中表现出色，但其在专家级推理（如解决奥林匹克物理问题）方面的潜力尚未充分探索。受学生通过复习过往问题准备竞赛的启发，研究RAG如何增强基础模型的物理推理能力。

Method: 提出了PhoPile高质量多模态数据集，专门针对奥林匹克级别物理问题设计，包含图表、图形和方程。使用该数据集对RAG增强的基础模型进行基准测试，涵盖大型语言模型和多模态模型，并采用多种检索器。

Result: 结果表明，将检索与物理语料库集成可以提高模型性能，但也揭示了在检索增强物理推理方面需要进一步研究的挑战。

Conclusion: RAG在提升基础模型物理推理能力方面具有潜力，但还需要更多研究来解决当前面临的挑战。
...
TL;DR: Fusion-of-N (FusioN)方法通过融合多个候选样本的最佳元素来生成最终答案，相比传统的Best-of-N选择方法，在测试时扩展和合成数据生成中表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统Best-of-N方法只选择单个最佳样本，丢弃了其他样本中的有用信息。FusioN旨在利用所有候选样本的多样性，通过协作方式整合各样本的优势。

Method: 使用通用LLM评判器将每个样本中最具信息量的元素融合成一个最终答案。在测试时扩展和合成数据生成两种设置下进行评估。

Result: 在11种语言、3个不同任务和不同模型规模上的广泛基准测试中，FusioN始终优于Best-of-N方法，显示出在测试时扩展和合成数据生成下游增益方面的多功能性和鲁棒性。

Conclusion: 应该从单一质量度量转向接受LLM生成的多面性，通过整合多样优势来释放潜在能力，实现仅靠选择无法达到的改进。
...
TL;DR: 研究发现LLMs在非标准英语方言的问答任务中性能下降达20%，主要归因于三个特定语法规则（存在性"it"、零系词和"y'all"）的影响。


<details>
  <summary>Details</summary>
Motivation: 分析大型语言模型在非标准英语方言上的性能下降问题，探究语法规则对性能影响的具体机制。

Method: 通过将标准美式英语问题转换为非标准方言变体，在多项选择问答任务中测试LLM性能，并分析特定语法规则的影响。
...
Method: 采用级联框架先生成语法指导再进行条件文本生成，并推广到非级联架构以更好对齐结构和内容。引入共享表示机制支持细粒度个性化。

Result: 在多个任务上的实验表明，该方法在流畅性、多样性和风格保真度方面具有优越性。定性分析显示其学习个性化模式的解释性和灵活性。

Conclusion: 通过整合语法信息和个性化条件，提出的模型能更好地捕捉风格化句子构建的词汇和结构特征，支持忠实风格生成和零样本推理。
...
TL;DR: 这是关于神经网络可解释性领域的首篇综述论文，重点调查了为模型组件和抽象生成概念描述的方法、评估指标和数据集。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络决策过程是可解释性研究的核心目标，特别是在大语言模型中，需要揭示底层机制并识别模型组件的作用。

Method: 通过系统调查新兴的概念描述领域，梳理了生成这些描述的关键方法、自动化与人工评估指标的演变，以及支撑研究的数据集。

Result: 分析发现该领域对更严格的因果评估需求日益增长，揭示了当前的技术现状和关键挑战。

Conclusion: 这篇综述为未来研究提供了路线图，旨在使模型更加透明，推动可解释性研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的DST方法在开放域多轮聊天机器人中效率不足，无法提供复杂对话所需的自适应性和连贯性。

Method: 使用BERT进行槽位填充和意图检测，XGBoost进行意图验证，GPT进行对话状态跟踪，在线代理进行实时答案生成。

Result: 在波斯语数据集上相比现有方法显著提高了准确性和连贯性。

Conclusion: 混合方法能有效提升DST能力，为更定制化、自适应和类人的对话AI系统铺平道路。
...
TL;DR: 提出了一种结合具身智能和强化学习的新型文本处理模型，通过具身智能的感知行动优势和强化学习的决策优化能力，显著提升了文本处理的智能水平。


<details>
  <summary>Details</summary>
Motivation: 旨在利用具身智能的感知行动优势和强化学习的决策优化能力，提升文本处理的智能化水平。

Method: 通过理论阐述和实验探索，提出了一种具身智能与强化学习相结合的新型集成模型。

Result: 该模型在广泛的文本处理任务中表现出很高的有效性，验证了其应用潜力。

Conclusion: 具身智能与强化学习的集成模型能够有效提升文本处理的智能化水平，具有广阔的应用前景。
...
TL;DR: 这篇系统性文献综述分析了2020-2025年非洲低资源语言自动语音识别(ASR)的研究现状，涵盖数据集、模型、评估方法等方面，揭示了该领域面临的挑战并提出了未来发展建议。


<details>
  <summary>Details</summary>
Motivation: 尽管ASR技术在全球取得了显著进展，但非洲2000多种低资源语言在数字包容方面仍然严重不足，这阻碍了非洲大陆的数字发展。

Method: 采用PRISMA 2020程序，在DBLP、ACM Digital Library、Google Scholar、Semantic Scholar和arXiv等数据库中检索2020年1月至2025年7月期间发表的研究，筛选出71篇相关文献进行分析。

Result: 共记录了74个数据集，涵盖111种语言，约11,206小时的语音数据。只有不到15%的研究提供了可复现材料，数据集许可不明确。自监督和迁移学习技术有前景但受限于预训练数据不足、方言覆盖不充分和资源可用性。大多数研究使用词错误率(WER)，很少使用字符错误率(CER)或变音符号错误率(DER)等语言学指标。

Conclusion: ASR系统现有证据不一致，受限于数据集可用性、标注质量差、许可不确定性和基准测试有限等问题。未来发展需要利益相关者合作、创建伦理平衡的数据集、使用轻量级建模技术和积极基准测试。
...
TL;DR: mR3是一个支持72种语言的多语言奖励推理模型，在奖励建模领域实现了最广泛的语言覆盖，性能超越更大的模型，同时模型尺寸最多缩小9倍。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型评估在英语中表现良好，但在非英语环境中的性能泛化能力不足，且缺乏有效的多语言训练策略指导。

Method: 提出mR3模型，通过综合研究数据和课程选择策略，包括整合目标语言推理数据集，来构建高质量奖励模型。

Result: 在多语言奖励模型基准测试中达到最先进性能，超越GPT-OSS-120B等更大模型，并通过大量消融研究验证了有效性。

Conclusion: mR3模型为多语言奖励建模提供了有效的解决方案，模型、数据和代码已开源发布。
...
TL;DR: MASH是一个训练框架，通过强化学习让LLM学会在超出知识边界时选择寻求外部帮助（搜索）而不是产生幻觉回答，从而实现了有效的弃权机制。


<details>
  <summary>Details</summary>
Motivation: LLM无法可靠识别自身知识边界，经常对超出边界的问题产生幻觉回答。相比之下，人类能够认识到自己的局限性并寻求外部帮助或弃权。

Method: 使用强化学习框架，通过按次搜索付费的奖励机制，让LLM学会在需要时寻求外部帮助，同时保证回答准确性。

Result: 在三个知识密集型QA数据集上的实验显示，MASH显著提升了选择性寻求帮助的性能，在多跳数据集上答案准确率提高了7.6%。

Conclusion: MASH训练有效对齐了搜索工具使用与参数知识，可以成功用于做出弃权决策，且不需要预先确定知识边界来构建训练数据。
...
TL;DR: 该论文首次系统研究了针对语音语言模型的音频后门攻击，在四个语音编码器和三个数据集上验证了攻击有效性，并提出了基于微调的防御方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型及其多模态扩展的普及，通过级联领域特定编码器与LLM实现多模态的方法使模型继承了所有组件的漏洞，需要系统研究音频后门攻击的威胁。

Method: 研究针对语音语言模型的音频后门攻击，在四个语音编码器和三个数据集上进行实验，涵盖自动语音识别、语音情感识别、性别和年龄预测四个任务，并进行组件级分析识别最脆弱的管道阶段。

Result: 攻击在四个任务中持续实现高成功率，范围从90.76%到99.41%，证明了音频后门攻击的有效性。

Conclusion: 通过组件级分析识别了管道中最脆弱的阶段，并提出了基于微调的防御方法，能够缓解预训练编码器中毒的威胁。
...
TL;DR: SWF Benchmark评估LLM在社会资源分配中的表现，发现多数模型偏向功利主义，牺牲公平性追求效率，且分配策略易受干扰。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在重要决策中扮演角色，需要评估其在稀缺社会资源分配中的价值观和原则，但目前缺乏相关基准。

Method: 引入SWF Benchmark动态仿真环境，让LLM作为分配者向异质社区分配任务，在集体效率（ROI）和分配公平（基尼系数）间建立持久权衡。

Result: 评估20个先进LLM发现：通用对话能力与分配技能无关；多数模型默认功利主义取向；分配策略易受输出长度和社会影响框架干扰。

Conclusion: 当前LLM作为社会决策者存在风险，需要专门基准和针对性对齐来改进AI治理。
...
TL;DR: 提出了GRAD方法，通过训练LLM生成针对特定输入的简洁演示，在预算约束下优于传统RAG方法，并在数学推理和STEM问题上表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法依赖静态数据库，缺乏适应性且可能产生不相关的演示，限制了LLM的上下文支持效果。

Method: 训练LLM模型生成输入特定的简洁演示，动态地为每个输入定制演示内容，提供更好的上下文支持。

Result: 在数学数据集上训练的GRAD在Qwen2.5-14B上持续优于基线方法，在数学推理和STEM问题上表现优异，并展现出对物理、化学、计算机科学等OOD领域的强大泛化能力。

Conclusion: GRAD引入了一个可扩展的演示生成器模型，为资源受限环境下的动态少样本学习范式迈出了第一步，同时较小模型生成的演示能有效指导较大目标模型，降低训练成本。
...
TL;DR: 该论文发现后训练对齐会导致LLM多样性减少（模式崩溃），根源是偏好数据中的典型性偏见，并提出了一种无需训练的提示策略Verbalized Sampling来规避此问题。


<details>
  <summary>Details</summary>
Motivation: 后训练对齐经常减少LLM的多样性，导致模式崩溃现象。与之前归因于算法限制的工作不同，本文识别了一个根本性的数据层面驱动因素：偏好数据中的典型性偏见。

Method: 提出了Verbalized Sampling（VS）策略，提示模型对一组回答进行概率分布的口头描述（如"生成5个关于咖啡的笑话及其对应概率"）。这是一种简单、无需训练的提示方法。

Result: 综合实验显示，VS在创意写作、对话模拟、开放式问答和合成数据生成方面显著提升性能，多样性提高1.6-2.1倍，且不牺牲事实准确性和安全性。更强大的模型从VS中获益更多。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型需要持续更新以保持与现实世界知识同步，但顺序编辑会导致表示不稳定和灾难性遗忘。研究旨在理解和缓解顺序编辑引起的性能退化问题。

Method: 提出SPHERE方法：使用超球面能量量化神经元均匀性，识别预训练权重矩阵的主超球面方向的互补稀疏空间，将新知识投影到该空间以减少对主方向的扰动。

Result: 在LLaMA3(8B)和Qwen2.5(7B)上的实验表明，SPHERE在编辑能力上平均优于最佳基线16.41%，同时能最忠实地保持模型的通用性能。

Conclusion: SPHERE提供了一种原则性路径，通过稳定神经元权重分布来实现可靠的大规模知识编辑，平衡了知识更新与保留的需求。
...
TL;DR: 提出了一种用于双角色长时程交互动作插值的方法，通过跨空间插值建模和对抗学习来维持交互质量和运动质量


<details>
  <summary>Details</summary>
Motivation: 传统动作插值研究主要关注单角色，扩展到密集交互的双角色具有挑战性，需要精确的时空对应关系来维持交互，同时创建向预定关键姿势的自然过渡

Method: 提出跨空间插值方法，在不同条件表示空间中建模每个角色的交互；通过对抗学习识别周期性交互模式来维持交互质量；通过学习精炼漂移的潜在空间来防止姿势误差积累

Result: 方法能够生成逼真、可控且长时程的双角色交互动作，包括动态拳击和舞蹈动作，经过广泛的定量评估和用户研究验证

Conclusion: 该方法成功解决了双角色长时程交互动作插值的挑战，通过跨空间建模和稳定性维护技术实现了高质量的交互动作合成
...
TL;DR: 提出了Reservoir SWD (ReSWD)，通过将加权储层采样集成到SWD中，在优化步骤中自适应保留信息丰富的投影方向，从而在保持无偏的同时获得稳定梯度。
...
Method: 将加权储层采样集成到SWD中，在优化过程中自适应地保留信息丰富的投影方向。

Result: 在合成基准测试和实际任务（如颜色校正和扩散引导）中，ReSWD始终优于标准SWD和其他方差减少基线方法。

Conclusion: ReSWD在保持无偏性的同时提供了稳定的梯度，显著改善了SWD的优化性能。
...
TL;DR: 提出了一个音频驱动的实时3D面部动画系统，能够在虚拟现实中以最小延迟生成逼真的面部表情，实现100-1000倍的推理速度提升。


<details>
  <summary>Details</summary>
Motivation: 为虚拟现实中的社交互动开发实时、低延迟的面部动画系统，解决现有方法在实时性能方面的不足。

Method: 使用编码器将音频信号转换为潜在面部表情序列，结合扩散模型的生成能力，通过在线transformer消除对未输入数据的依赖，并采用蒸馏管道将迭代去噪加速为单步处理。

Result: 系统在GPU上运行时间小于15ms，面部动画准确性优于现有离线方法，支持多语言演讲等多种场景。

Conclusion: 该框架成功实现了实时、高质量的面部动画生成，为VR社交互动提供了有效的解决方案，并展示了在多模态应用中的扩展潜力。
...
TL;DR: 本文研究了自主智能体如何通过多智能体强化学习改进任务分配和协调，重点应用于无人机配送和仓库自动化场景。


<details>
  <summary>Details</summary>
Motivation: 随着自主系统从原型转向实际部署，多智能体去中心化协同决策能力成为核心需求，需要解决智能体独立、自适应和主动行动的问题。

Method: 在合作式多智能体强化学习框架下，采用集中训练、分散执行的范式，实现了轻量级多智能体近端策略优化(IPPO)方法。

Result: 在PettingZoo环境中进行实验，多个同质无人机或智能体能够在无显式通信的情况下自组织覆盖不同目标。

Conclusion: 自主智能体AI能够有效提升多智能体系统的任务分配和协调能力，为实际部署提供了可行的解决方案。
...
TL;DR: ToolBrain是一个轻量级框架，通过强化学习训练AI代理使用工具，解决手动设计奖励、数据不足和多工具选择困难等问题。


<details>
  <summary>Details</summary>
Motivation: 当前训练AI代理使用工具面临手动设计奖励、训练数据有限和多工具选择困难等挑战，导致适应慢、计算资源浪费和性能不佳。

Method: 采用灵活的强化学习框架，支持GRPO、DPO等RL算法和监督学习，提供自定义奖励函数和自动LLM评判系统，包含知识蒸馏、任务生成、工具检索等功能。

Result: 在代码代理执行邮件搜索任务中，工具使用技能快速提升达30.0%，同时保持代码库简洁可扩展。

Conclusion: ToolBrain降低了研究人员和从业者将LLM代理适配到特定领域的门槛，为智能AI代理开发提供了高效解决方案。
...
TL;DR: 提出自适应推理抑制（ARS）方法，通过动态监测推理确定性来抑制冗余推理步骤，在保持精度的同时显著提升推理效率。
...
Result: 在数学推理基准测试中，ARS实现了最高53%的token减少、46.1%的延迟降低和57.9%的能耗降低，同时保持或提高了精度。

Conclusion: ARS方法在多个模型架构上验证了其有效性，为大型推理模型提供了高效的推理解决方案。
...
TL;DR: 本文主张NeurIPS会议应该积极推动AI政策领域的科学共识形成，借鉴IPCC在气候政策中的经验，通过试点项目来填补当前共识形成机制的空白。


<details>
  <summary>Details</summary>
Motivation: 当前AI政策制定缺乏科学共识形成机制，而NeurIPS作为AI领域的领先会议，具备推动这一进程的独特优势，能够帮助制定更高质量的AI政策。

Method: 建议NeurIPS借鉴IPCC在气候政策共识形成方面的领导经验，开展初步试点项目，通过科学证据整合和专家共识来推动AI政策制定。

Result: 识别了当前AI政策领域共识形成的完全空白，论证了NeurIPS是最适合推动这一进程的平台，并提出了具体的实施路径。

Conclusion: NeurIPS应该在AI政策领域发挥领导作用，积极促进科学共识的形成，以创造更高质量的AI政策，这符合其在AI领域的领导地位和责任。
...
TL;DR: CERTAIN项目开发了一个将监管合规、伦理标准和透明度整合到AI系统中的综合框架，通过语义MLOps、本体驱动的数据谱系追踪和RegOps工作流程来应对AI伦理和法律挑战。


<details>
  <summary>Details</summary>
Motivation: AI在欧洲社会和经济中日益重要，但其扩散带来了关键的伦理、法律和监管挑战，需要建立确保AI系统合规和透明的框架。

Method: 开发语义MLOps进行结构化AI生命周期管理，使用本体驱动数据谱系追踪确保可追溯性和问责制，建立RegOps工作流程来操作化合规要求。

Result: 通过在不同试点中实施和验证解决方案，CERTAIN项目推进了监管合规，并促进了符合欧洲标准的负责任AI创新。

Conclusion: CERTAIN框架成功整合了监管合规、伦理标准和透明度，为欧洲AI系统提供了可操作的技术解决方案，支持负责任的AI发展。
...
TL;DR: 该论文审计了视觉语言模型在保释决策预测中的表现，发现模型在多个交叉群体中表现不佳，会错误地高置信度拒绝应获保释者。通过引入法律先例检索增强生成和微调等干预措施，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的普及，法律判决预测系统可能利用罪犯图像和文本案件报告，这可能带来意外后果和恶意使用。需要评估VLMs在保释决策中的效率。

Method: 首先审计独立VLMs在保释预测任务中的表现，然后设计干预算法：通过RAG管道引入法律先例，并使用创新方案微调VLMs。

Result: 独立VLMs在多个交叉群体中表现不佳，会错误地高置信度拒绝应获保释者。干预措施显著提升了保释预测性能。

Conclusion: 该工作为未来在VLMs部署到真实世界法律判决预测前设计更智能的干预措施铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的金融欺诈检测面临证据分散在多年财务报告中的挑战，需要专门的方法来定位细粒度证据链。

Method: 开发了AuditAgent多智能体推理框架，整合了主体级风险先验、混合检索策略和专门化智能体模块，用于识别和聚合跨报告证据。

Result: 该方法在召回率和可解释性方面显著优于通用智能体范式，为自动化、透明的金融取证建立了新基准。
...
TL;DR: 论文提出使用具身AI和大型视觉语言模型来实现无人机在突发情况下的自适应决策和恢复能力，替代传统的手工编码安全规则方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖安全工程师手工编码大量恢复规则，但无法预测现实世界中的各种意外情况，且规则集很快变得不完整。需要一种能够实时评估上下文并生成适当行动的自适应决策系统。

Method: 利用具身AI和大型视觉语言模型，在Unreal Engine模拟的城市场景中，让无人机动态解释周围环境并决定紧急机动以实现安全着陆。

Result: 结果表明，具身AI实现了以前手工设计不可行的新型自适应恢复和决策流程。

Conclusion: 该方法提高了自主空中系统的弹性和安全性，为无人机应急响应提供了新的技术途径。
...
TL;DR: 提出SAA-CBR，一种结合神经槽注意力与符号论证推理的新型神经符号图像分类框架，在CLEVR-Hans数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 整合神经网络的感知能力与符号推理的可解释性，通过对象中心学习和抽象论证实现更有效的图像分类。

Method: 使用槽注意力进行对象中心学习，结合抽象论证案例推理，包括特征组合策略、案例库简化、基于计数的偏序关系、One-Vs-Rest多类分类扩展以及支持型AA-CBR应用。

Result: 在CLEVR-Hans数据集上表现出色，与基线模型相比具有竞争力。

Conclusion: SAA-CBR是一个有效的分类器，成功融合了神经和符号方法，为图像分类提供了新的神经符号解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 将自然语言请求转换为可靠的生产级数据转换具有挑战性：正确性取决于精确的模式链接和仓库特定的SQL方言，而训练期间最强的监督（执行成功和结果匹配）仅在序列级别提供。

Method: Thinkquel集成了TS-SQL合成数据管道（使用dbt作为可移植中间表示）和Token-Sequence GRPO（TS-GRPO）强化学习目标，专门设计用于在微调LLMs时弥合令牌级训练信号和序列级执行奖励之间的差距。
...
Conclusion: Thinkquel通过创新的合成数据管道和强化学习目标，显著提高了自然语言到SQL查询的准确性和可移植性，为生产环境提供了可靠的解决方案。
...
Method: 采用解耦微调方法，使用LoRA微调创建专用的工具选择和工具特定参数生成适配器，通过分离损失掩码处理每个子任务。DualTune推理框架动态加载相应LoRA适配器，并实施分层编排来限制工具选择所需工具数量。

Result: 在MCP-Bench基准测试中，使用解耦微调的Qwen-2.5-7B模型将基础模型的工具调用准确率提高了46%，在所有情况下都优于相似大小的其他本地推理、非推理和微调模型，在大多数情况下也优于2倍大的模型。
...
Conclusion: 该框架成功将可解释性从单智能体系统推广到多智能体系统，提供可解释且可转移的解释，同时实现更快、更鲁棒的学习。
...
Method: 使用GPT-OSS:20b模型的六个变体（一个基准模型和五个ICL配置），在840个测试中评估通用知识问题、逻辑谜题和数学奥赛题的推理能力。

Result: ICL模型在通用知识任务上达到91%-99%准确率，但在复杂推理问题上表现下降，谜题准确率降至10-43%（基准模型为43%）。数学奥赛题无显著差异。

Conclusion: ICL指南在效率和推理灵活性之间产生系统性权衡，对LLM部署和AI安全有重要影响。
...
TL;DR: 提出了一个评估LLM工具选择偏见的基准，发现现有模型存在系统性偏见，并提出轻量级缓解方法


<details>
  <summary>Details</summary>
Motivation: LLM代理依赖外部工具市场，如果工具选择存在系统性偏见，会降低用户体验并扭曲市场竞争

Method: 构建包含功能等效工具的多样化基准，测试7个模型，进行控制实验分析偏见来源，提出先过滤后均匀采样的缓解方法

Result: 发现模型存在固定选择单一提供商或偏好列表中靠前工具的偏见；语义对齐是最强预测因素；扰动描述会显著改变选择；预训练暴露会放大偏见

Conclusion: 工具选择偏见是工具增强LLM公平部署的关键障碍，提出的轻量级方法能有效减少偏见同时保持任务覆盖
...
TL;DR: CAIA基准测试揭示了AI在对抗性高风险环境中的严重盲点：即使最先进的模型也无法有效应对武器化错误信息和不可逆错误的情况，特别是在加密市场等实际部署场景中。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在受控环境中评估任务完成度，但现实世界部署需要AI具备对抗主动欺骗的韧性。加密市场2024年因漏洞损失300亿美元，是理想的测试环境。

Method: 在加密市场测试环境中，评估17个模型在178个时间锚定任务上的表现，要求智能体区分真相与操纵、导航碎片化信息环境，并在对抗压力下做出不可逆的财务决策。

Result: 无工具时前沿模型准确率仅28%；工具增强后提升至67.4%，但仍低于80%的人类基准。模型系统性地偏好不可靠的网页搜索而非权威数据，陷入SEO优化的错误信息和社交媒体操纵。

Conclusion: 当前模型尽管推理分数令人印象深刻，但在需要智能体在主动对抗环境中生存的场景中仍根本未准备好。对抗鲁棒性是可信AI自主性的必要条件。
...
<details>
  <summary>Details</summary>
Motivation: 随着智能服务增长，通信目标从人类转向AI智能体，需要新范式实现实时感知、决策和协作。语义通信虽具前景，但受限于动态环境和资源约束。

Method: 1) 语义自适应传输：使用真实或生成样本进行微调，使模型适应变化环境；2) 语义轻量传输：采用剪枝、量化和感知感知采样降低模型复杂度；3) 语义自进化控制：分布式分层决策优化多维资源。
...
TL;DR: 提出了TRACE框架，通过让智能体自由探索和演化现有基准任务，生成难度更高的新任务，并记录可验证的执行轨迹，解决了现有智能体基准测试快速达到性能上限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准测试面临新开发智能体快速达到性能上限的问题，难以满足评估智能体能力的需求，需要一种动态演化的评估系统。
...
Conclusion: 这项工作标志着从静态人工策划基准向动态自演化评估系统的范式转变，为智能体发展提供了可持续且具有挑战性的发展路径。
...
TL;DR: 该论文研究了自动化评估AI系统回答患者健康问题的可行性，通过系统研究评估方法，发现精心设计的自动化评估能够有效扩展AI系统的比较评估。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI回答患者健康问题的黄金标准——人类专家评审——劳动密集且缓慢，限制了可扩展性，需要更高效的自动化评估方法。

Method: 在100个患者案例中收集了28个AI系统的2800个回答，从三个维度评估：是否回答问题、是否恰当使用临床记录证据、是否使用一般医学知识，并使用临床医生撰写的参考答案作为指标锚点。
...
TL;DR: 提出了可扩展决策状态（EDMS）方法，通过语义丰富的状态表示和动作掩码方案，在足球等团队运动中构建可解释的智能体模型。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的分析方法直观但有限，现代机器学习模型缺乏明确的智能体表示。需要构建既能从数据中学习，又具有战术可解释性且能在异构数据源中稳健运行的球员级智能体模型。

Method: EDMS方法通过增强原始位置和速度信息，添加关系变量（如空间得分、传球和得分），并结合动作掩码方案，为持球和无球球员提供不同的决策集。
...
TL;DR: 该研究挑战了传统观点，发现在14个不同领域中，生成式结果奖励模型（GenORM）表现最稳健，而非传统认为的逐步奖励模型（PRM）。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在测试时扩展的可靠性通常使用外部验证器或奖励模型。传统观点认为逐步奖励模型（PRM）优于结果奖励模型（ORM），但这一观点主要基于数学相关领域的证据。

Method: 在14个多样化领域中统一评估了四种奖励模型变体：判别式ORM和PRM（DisORM、DisPRM）以及生成式ORM和PRM（GenORM、GenPRM）。

Result: 1）DisORM与DisPRM表现相当；2）GenPRM不具竞争力；3）GenORM是最稳健的，在所有测试领域中都获得了显著且一致的提升。

Conclusion: 研究挑战了细粒度监督总是更好的普遍假设，支持在多领域部署中使用生成式结果验证。逐步评分会继承LLM自动标注的标签噪声，并且在评估长推理轨迹时存在困难。
...
TL;DR: 提出了VIRTUE视觉交互文本图像通用嵌入器，通过结合分割模型和视觉语言模型，使嵌入模型具备视觉交互能力，能够处理用户指定的感兴趣区域。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入模型缺乏视觉交互能力，无法处理用户指定的感兴趣区域，限制了其在需要局部定位用户意图的应用中的潜力。

Method: 结合分割模型处理视觉提示来精确定位图像中的特定区域，扩展视觉语言模型到表示学习领域。

Result: 在36个通用MMEB任务上提升3.1%-8.5%，在5个视觉交互SCaR任务上提升15.2%-20.3%，达到最先进性能。

Conclusion: VIRTUE成功扩展了嵌入模型的视觉交互能力，为需要局部定位用户意图的应用开辟了新途径，同时提升了传统嵌入任务的性能。
...
TL;DR: 本研究通过访谈IT服务公司从业者，为RAG系统开发了15个数据质量维度，覆盖数据提取、转换、提示搜索和生成四个处理阶段，发现需要在传统DQ框架中增加新维度，并采用前端加载的质量管理策略。


<details>
  <summary>Details</summary>
Motivation: 当前的数据质量框架主要针对静态数据集开发，无法充分应对RAG系统的动态、多阶段特性，需要为这类基于AI的新系统开发专门的数据质量维度。

Method: 对领先IT服务公司的16名从业者进行半结构化访谈，通过定性内容分析，归纳性地推导出RAG系统的数据质量维度。

Result: 识别出15个独特的DQ维度，分布在RAG系统的四个处理阶段；发现新维度主要集中在早期RAG步骤；DQ问题会在RAG管道中转换和传播。

Conclusion: 需要在传统DQ框架中添加新维度来覆盖RAG上下文；建议采用前端加载的质量管理策略；需要动态、步骤感知的质量管理方法来应对DQ问题的转换和传播。
...
TL;DR: 本文揭示了扩散语言模型(DLMs)在迭代去噪过程中存在关键安全漏洞，攻击者可通过注入肯定性令牌绕过安全防护，并提出了针对DLMs的安全对齐方法。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行迭代去噪生成令牌，虽然能降低延迟并支持双向条件化，但其推理机制带来的安全风险尚未被充分理解。需要研究DLMs特有的安全漏洞和防护措施。

Method: 提出了一种针对DLMs的安全对齐方法，训练模型从包含肯定性令牌的污染中间状态生成安全响应。该方法专门针对DLMs的迭代去噪特性设计。

Result: 实验表明，所提出的方法显著减轻了安全漏洞，对任务性能影响最小，同时提高了对传统越狱攻击的鲁棒性。

Conclusion: 扩散语言模型存在特有的安全风险，需要专门的安全研究。提出的安全对齐方法能有效防护DLMs免受基于迭代去噪机制的越狱攻击。
...
TL;DR: 提出了Agent Context Optimization (ACON)框架，通过优化压缩指南来压缩环境观察和交互历史，减少内存使用26-54%同时保持任务性能，并能蒸馏到小型模型中。
...
Method: ACON框架通过自然语言空间中的压缩指南优化：在完整上下文成功但压缩上下文失败的配对轨迹中，LLM分析失败原因并相应更新压缩指南，然后将优化后的LLM压缩器蒸馏到更小的模型中。

Result: 在AppWorld、OfficeBench和Multi-objective QA上的实验显示，ACON减少内存使用26-54%（峰值token），同时大部分保持任务性能，蒸馏到小型压缩器时保持95%以上准确率，并将小型LM作为长视野代理的性能提升高达46%。

Conclusion: ACON提供了一个统一的框架，有效压缩代理上下文，在减少内存使用的同时保持任务性能，并能通过蒸馏技术扩展到更小的模型，提升小型LM在长视野代理任务中的表现。
...
TL;DR: HARPA是一个AI驱动的科学发现系统，通过文献挖掘识别研究趋势，探索假设设计空间，并生成可测试的、基于文献的假设，在可行性和基础性方面显著优于基线方法。
...
Method: HARPA采用受人类研究人员启发的构思流程：首先通过文献挖掘识别新兴研究趋势，然后探索假设设计空间，最后通过识别研究空白和论证设计选择来收敛到精确、可测试的假设。

Result: HARPA生成的研究提案在可行性(+0.78)和基础性(+0.85)方面显著优于基线AI研究者。与ASD代理(CodeScientist)测试时，HARPA产生了更多成功执行(20 vs 11)和更少失败(16 vs 21)。通过学习奖励模型，HARPA在假设评分上比未训练基线提高了约28%。

Conclusion: HARPA的方法代表了AI驱动科学发现领域的重要进展，能够生成更可行、更基于文献的假设，并能够根据实验经验持续改进假设质量。
...
TL;DR: 论文发现当前模型编辑方法存在根本性缺陷，其成功可能基于隐藏的捷径而非真实的语义理解，在否定查询下会崩溃。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型编码了过时或不正确的知识，需要更新、删除和遗忘这些知识以实现对齐和安全。模型编辑通过精确修改参数子集来更新特定事实，但现有方法可能基于虚假的成功。

Method: 系统开发了一套新的评估方法，特别设计了否定查询来测试模型编辑的鲁棒性，发现最先进的方法在简单否定查询下就会崩溃。

Result: 实证证据表明编辑很可能基于捷径而非完整语义，当前模型编辑文献的基础受到根本性挑战。

Conclusion: 在进一步推进模型编辑研究之前，需要重新考虑其基础，因为捷径与稳健的知识整合本质上是矛盾的。
...
TL;DR: 提出CDDM方法，通过协作渐进式蒸馏将大型扩散模型压缩为轻量级模型，实现实时轨迹预测，在保持高精度的同时大幅减少参数和采样步骤。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在概率轨迹预测中表现优异，但模型尺寸大、采样速度慢，难以在实际自动驾驶和智能交通系统中部署。

Method: 基于协作渐进式蒸馏(CPD)，逐步将知识从高容量教师扩散模型转移到轻量级学生模型，同时减少采样步骤和模型尺寸，并引入双信号正则化蒸馏损失。

Result: 在ETH-UCY和nuScenes基准测试中达到最先进精度，仅需231K参数和2-4采样步骤，实现161倍压缩、31倍加速和9ms延迟。
...
TL;DR: 提出Expected Attention方法，通过预测未来查询对KV对的关注度来估计重要性，实现无需训练的KV缓存压缩，有效解决注意力分数不可用的问题。


<details>
  <summary>Details</summary>
Motivation: KV缓存的内存消耗是大型语言模型推理效率的主要瓶颈，现有基于注意力分数的压缩方法面临实际限制：未来token的注意力分数在压缩时不可用，且现代实现如Flash Attention不生成完整注意力矩阵。

Method: 利用LLM激活的分布特性，以闭式形式计算每个KV对的期望注意力分数，基于这些分数进行排序和剪枝，最小化对残差流的影响。

Result: 该方法在预填充和解码阶段都能无缝运行，在两个场景下均优于现有最优基线方法，且不会导致性能下降。

Conclusion: Expected Attention提供了一种无需训练的有效KV缓存压缩方案，并发布了KVPress库，包含20多种技术供研究人员实现和基准测试。
...
Method: 结合批量Grad-CAM算法和原型重建损失，引导模型关注显著图像特征。
...
TL;DR: 提出了一种迭代式相关区域(RZ)缩减方法，通过重复求解同一位置并逐步限制参与区域，引导求解器找到更小的RZ，从而提升策略重用和剪枝效率。


<details>
  <summary>Details</summary>
Motivation: 由于游戏树的指数级增长，许多游戏仍未被完全求解。虽然相关区域(RZ)技术能显著减少搜索空间，但不同的解会产生不同大小的RZ，而较小的RZ更有利于重用和剪枝效率。

Method: 设计迭代RZ缩减方法，包含三种约束生成策略，并集成RZ模式表以充分利用过往解。通过重复求解同一位置并逐步限制区域来引导找到更小的RZ。

Result: 在7x7 Killall-Go实验中，平均RZ大小减少到原始的85.95%。缩减后的RZ可作为可重用知识永久存储。

Conclusion: 该方法能有效减小RZ大小，提升求解效率，缩减的RZ可作为可重用知识应用于更大棋盘或不同开局。
...
TL;DR: 提出了自适应课程策略优化（ACPO）框架，通过动态课程和优势感知自适应裁剪机制改进视觉语言模型的强化学习对齐
...
Method: ACPO包含两个核心组件：1）动态课程，从稳定的近策略探索阶段逐步过渡到高效的离策略利用阶段；2）优势感知自适应裁剪（AAAC），用动态的样本级边界替代固定裁剪超参数

Result: 在MathVista、LogicVista和MMMU-Pro等多模态推理基准测试中，ACPO持续优于DAPO和PAPO等基线方法，实现了最先进的性能、加速收敛和优越的训练稳定性
...
<details>
  <summary>Details</summary>
Motivation: 利用社交媒体平台作为了解个体思想和心理状态的窗口，开发可信赖且透明的AI系统用于心理健康评估。

Method: 使用注意力机制层次编码单字和双字，通过交叉注意力机制融入心理健康知识图谱的领域知识，采用有序回归框架预测抑郁症严重程度。

Result: 在多个数据集上，AttentionDep在分级F1分数上比现有最优方法高出5%以上，同时提供可解释的预测洞察。
...
TL;DR: 提出了一种新颖的数据增强流水线，通过对称性和难度两个角度提升定理证明模型的鲁棒性，训练出的EvolProver在多个基准测试中达到了新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM定理证明模型缺乏泛化能力，对问题表述的微小变换很脆弱，需要提高模型的鲁棒性。

Method: 提出三部分数据增强方法：EvolAST（基于AST的语法对称性）、EvolDomain（跨数学领域的语义对称性）、EvolDifficulty（难度演化），并用增强数据训练7B参数的EvolProver模型。
...
Conclusion: 该数据增强流水线有效提升了定理证明模型的鲁棒性和性能，消融研究证实了其在多个基准测试中的有效性。
...
TL;DR: 提出了DDIM反演攻击(DIA)方法，通过攻击DDIM反演轨迹路径来有效破坏恶意图像编辑，在防御性能上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: DDIM反演技术虽然为真实图像编辑提供了便利，但也被恶意用户用于合成虚假内容。现有防御方法与迭代去噪轨迹不匹配，导致防御效果不佳。

Method: 提出DDIM反演攻击(DIA)，直接攻击DDIM反演轨迹路径，与迭代去噪过程更好地对齐。

Result: DIA方法在各种编辑方法上都实现了有效的破坏效果，超越了之前的防御方法。
...
TL;DR: 本研究探讨了AI（特别是像ChatGPT这样的大语言模型）在教育环境中的整合，重点关注其对教学的影响。通过访谈瓦赫宁根大学数据科学课程的协调员，研究发现AI工具既能简化任务、增强学习，也带来学生过度依赖技术、影响认知和问题解决能力发展的担忧。


<details>
  <summary>Details</summary>
Motivation: 随着AI技术在教育领域的普及，需要了解其实际应用效果和潜在问题，特别是大语言模型对教学过程的深远影响。

Method: 通过对瓦赫宁根大学数据科学课程协调员进行访谈的定性研究方法。

Result: 研究发现AI工具能够简化任务和增强学习体验，但也导致学生过度依赖技术，可能阻碍基本认知和问题解决技能的发展。

Conclusion: AI可以成为教育中的宝贵资产，但需要负责任地使用，确保其补充而非替代基本学习过程，同时需要调整评估方法以确保教育成果。
...
TL;DR: 提出了ChemX，一个包含10个手动整理并经领域专家验证的数据集，用于评估和改进化学信息提取方法。通过基准测试比较现有代理系统和基线模型，揭示了化学信息提取中的持续挑战。


<details>
  <summary>Details</summary>
Motivation: 化学信息提取由于化学数据的异质性而面临严峻挑战，现有代理方法在该领域表现有限，需要专门的基准数据集来推动该领域的发展。

Method: 创建了10个手动整理和领域专家验证的数据集，涵盖纳米材料和小分子。进行了广泛的基准测试，比较了现有最先进的代理系统（如ChatGPT Agent）和化学特定数据提取代理，并引入了自己的单代理方法。
...
TL;DR: 本文比较了加权知识库的成本语义学和c-representations这两种处理不一致知识库的方法，展示了它们在语义层面的等价性。
...
Method: 通过语义层面的比较分析，研究加权知识库和c-representations在解释排序和蕴含关系方面的等价条件。

Result: 证明在特定条件下，加权知识库和一组可废止条件可以生成相同的解释排序，达到相对成本上的语义结构等价。

Conclusion: 两种形式化方法在语义层面具有等价性，这一发现有助于推动成本语义学和c-representations的进一步研究。
...
TL;DR: 本文提出了一种无监督评估分类器的逻辑框架，通过分析分类器之间的一致性和分歧来计算逻辑一致的群体评估结果，并构建了无需知识的警报系统来检测LLM评委是否违反用户设定的评分阈值。


<details>
  <summary>Details</summary>
Motivation: 探索分类器评估中一致性与分歧的不对称性，当专家意见一致时无法排除任何可能的评估结果，而当意见分歧时则可确定至少一方存在错误。

Method: 将分类器对齐决策的统计摘要作为线性规划问题的输入，在整数空间中计算可能的正确或错误响应，结合逻辑约束和适用于所有有限测试的线性等式公理。

Result: 开发了仅基于逻辑一致性的无监督评估方法，构建了能够检测LLM评委是否违反用户设定评分阈值的无知识警报系统。

Conclusion: 该方法展示了仅使用逻辑一致性进行无监督评估的实际和即时效用，为分类器评估提供了一种新的逻辑框架。
...
TL;DR: 本文首次对基于EMT数据的电力系统保护中的故障分类和故障定位进行了经典机器学习模型的比较基准研究，评估了模型在实时约束下的性能表现。
...
Method: 使用电压和电流波形数据，将其分割为10毫秒到50毫秒的滑动窗口，在现实的实时约束下评估经典机器学习模型。性能评估包括准确性、对窗口大小的鲁棒性和运行时效率。

Result: 最佳故障分类模型达到F1分数0.992±0.001，最佳故障定位模型达到R2分数0.806±0.008，平均处理时间为0.563毫秒。

Conclusion: 机器学习模型在电力系统故障分类和定位方面表现出色，能够满足实时保护要求，为传统保护方案提供了有效的替代方案。
...
TL;DR: 该研究使用SMOTE技术解决加密货币市场中Pump and Dump操纵检测的类别不平衡问题，通过集成学习模型实现了高效的操纵行为检测。


<details>
  <summary>Details</summary>
Motivation: 加密货币市场中Pump and Dump操纵事件的稀缺性导致严重的类别不平衡，阻碍了准确的检测，需要解决这一数据不平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型数量的激增，如何有效管理和利用这些庞大资源成为重要挑战，需要学习能够促进下游任务的紧凑表示。

Method: 将问题建模为估计模型正确回答查询的概率，使用项目反应理论框架，结合模型多技能能力向量、查询区分度向量和难度标量，通过混合专家网络联合学习这些参数。

Result: 实验表明该方法在模型路由和基准精度预测方面达到最先进性能，学习到的参数编码了关于模型能力和查询特征的有意义、可解释信息。

Conclusion: 基于IRT的方法能够有效学习LLM能力的紧凑表示，为模型管理和利用提供了实用解决方案。
...
TL;DR: 提出基于蒙特卡洛树搜索的自动洞察与数据探索方法AIDE，解决从数据到可操作知识的转化难题。


<details>
  <summary>Details</summary>
Motivation: 组织面临数据量大但难以转化为可操作知识的挑战，自动知识发现存在数据导航、模型构建和主观目标考虑等复杂问题。

Method: 使用蒙特卡洛树搜索构建AIDE框架，支持数据转换和模型构建以发现数据模式。

Result: 在真实和合成数据上验证了AIDE的有效性，能够识别出揭示有趣数据模式的转换和模型。
...
TL;DR: FusionAdapter是一个用于多模态知识图谱中少样本关系学习的模型，通过适配器模块和融合策略有效整合多模态信息，在低资源场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态知识图谱方法主要将不同模态对齐到共享空间，忽视了特定模态的独特贡献，在低资源设置下性能受限。

Method: 提出FusionAdapter，包含适配器模块实现各模态对未见关系的高效适应，以及融合策略在保留模态特定特征的同时整合多模态实体表示。
...
TL;DR: 本文提出了一种数据驱动的奖励分配函数发现方法DAIL，通过LLM引导的进化框架自动探索奖励分配函数空间，在未见过的环境和策略优化算法中表现出色，超越了现有的人工设计基线。


<details>
  <summary>Details</summary>
Motivation: 对抗模仿学习(AIL)方法虽然有效但被认为不稳定。现有研究主要关注密度比估计的改进，而奖励分配函数对训练动态和最终策略性能的影响被忽视。这些函数通常基于散度最小化目标，依赖人工设计。

Method: 采用LLM引导的进化框架，在奖励分配函数空间中进行高效探索，发现数据驱动的奖励分配函数，提出了首个元学习的AIL算法DAIL。

Result: DAIL在未见过的环境和策略优化算法中表现出良好的泛化能力，超越了当前最先进的人工设计基线，并带来了更稳定的训练过程。

Conclusion: DAIL通过自动发现奖励分配函数，不仅提升了AIL的性能和稳定性，还提供了关于奖励分配函数在AIL稳定性中作用的新见解。
...
TL;DR: 提出了一种增强训练模型推理性能的新方法，通过随机性测试时搜索来改进CVRP中的切割平面生成，能够发现更多样化的切割子集，包括首次识别出框架容量不等式。
...
Method: 在推理时引入随机性测试时搜索：1）在图粗化过程中使用随机边选择替代贪心方法；2）提出GraphCHiP算法，利用粗化历史识别圆整容量不等式和框架容量不等式。

Result: 在随机生成的CVRP实例上，相比现有神经分离方法，新方法能有效减小对偶间隙，并在特定实例上成功发现了有效的框架容量不等式。

Conclusion: 通过测试时随机搜索增强训练模型，能够提高切割生成的多样性和质量，特别是在识别具有挑战性的框架容量不等式方面表现出色。
...
TL;DR: 提出Fuzzformer模型，结合循环神经网络、多头自注意力和模糊推理系统，用于多变量股票市场数据的长期时间序列预测，在保持准确性的同时提供可解释性。
...
Result: 在S&P500股票市场指数上的初步结果显示，与传统模型（如ARIMA和LSTM）相比具有相当的预测性能，同时提供网络内部有意义的信息流。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的量子电路存在参数精度不足和缺乏量子领域知识的问题，导致电路质量低下。需要开发能够结合量子专业知识并优化电路参数的自动化方法。

Method: 提出QUASAR框架，结合工具增强的LLM和强化学习，包含量子电路验证（使用外部量子模拟器）和分层奖励机制来训练模型。

Result: 在4B参数的LLM上，QUASAR在Pass@1达到99.31%的有效性，Pass@10达到100%，优于GPT-4o、GPT-5、DeepSeek-V3等工业级LLM以及仅使用SFT或RL的基线方法。

Conclusion: QUASAR通过结合工具增强LLM和强化学习，显著提高了生成量子电路的语法和语义性能，为量子电路的自动生成和优化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决罕见病诊断中的数据稀缺、隐私担忧和边缘设备资源有限等挑战，需要一种能同时处理这些问题的统一方法。
...
Conclusion: AFFR为罕见病的公平可信联邦诊断提供了实用路径，在保持隐私保护的同时显著提升了诊断性能和系统稳定性。
...
TL;DR: 提出了一个售后需求预测和监控的实用架构，结合了统计、机器学习和深度学习模型的集成方法，并包含基于角色的分析层，用于生成评分卡和趋势诊断。


<details>
  <summary>Details</summary>
Motivation: 解决售后需求预测的复杂性，整合多种外生信号，将COVID-19作为特殊制度处理，为不同国家和零件提供校准的预测区间，并通过角色驱动的分析提供决策支持。

Method: 使用收入感知和聚类感知的模型集成，包括统计、机器学习和深度学习模型。采用帕累托感知分割对高收入项目单独预测，长尾项目通过聚类预测。嵌入LLM生成角色感知叙述和执行质量检查。

Result: 系统在90多个国家和约6000个零件上实现了从预测到库存决策的闭环，提供校准的预测区间、性能评分卡和趋势分析模块。

Conclusion: 该架构通过统一的预测监控框架，将预测准确性转化为可操作的业务洞察，帮助规划者从关注当前准确性转向理解准确性趋势和可操作的杠杆。
...
TL;DR: 提出了SMDS方法，自动发现语言模型中的特征流形，揭示了这些流形具有一致的几何结构、跨模型稳定性，并支持推理功能。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能发现特定特征的特定几何结构，缺乏通用性，需要一种模型无关的方法来自动发现特征流形。

Method: 引入监督多维缩放(SMDS)方法，以时间推理为案例研究，自动发现特征流形。

Result: 发现不同特征形成各种几何结构(圆形、直线、簇等)，这些结构反映概念属性、跨模型稳定、支持推理，并能动态响应上下文变化。
...
TL;DR: 该研究通过三重相似性任务评估了70多个LLM与人类概念表征的对齐程度，发现指令微调和注意力头维度是影响对齐的关键因素，而多模态预训练和参数大小影响有限。现有基准测试无法完全捕捉人类-AI对齐。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的快速发展带来了多种计算要素，但哪些要素对构建具有人类表征的模型最为关键尚不清楚。现有基准测试不适合测量人类与模型之间的表征对齐。

Method: 使用THINGS数据库中的概念，通过三重相似性任务评估70多个不同计算要素的模型，比较人类和模型的概念表征。

Result: 经过指令微调且具有较大注意力头维度的模型与人类表征最对齐，而多模态预训练和参数大小对对齐影响有限。现有基准测试中MMLU比MUSR更能捕捉表征对齐，但都无法完全解释对齐分数的方差。

Conclusion: 指令微调和注意力头维度是推进LLM成为人类概念表征模型的关键要素，现有基准测试在捕捉人类-AI对齐方面存在不足。
...
TL;DR: 提出了一种新的前向传播范式Activation-Deactivation (AD)，通过关闭与遮挡部分对应的模型组件来消除遮挡输入特征对模型决策的影响，解决了传统黑盒解释方法因遮挡导致分布外图像的问题。


<details>
  <summary>Details</summary>
Motivation: 传统黑盒解释方法依赖遮挡输入部分来生成解释，但这种方法会产生分布外图像，影响解释质量，且选择合适遮挡值需要领域知识。

Method: 提出了ConvAD机制，可以轻松添加到任何训练好的卷积神经网络中，实现AD范式。该机制通过关闭与遮挡区域对应的模型组件来消除遮挡特征的影响，无需额外训练或微调。

Result: 实验表明，AD解释在多个数据集和模型架构上都比传统遮挡方法更鲁棒，鲁棒性提升高达62.5%，且无需领域知识来选择遮挡值。

Conclusion: ConvAD机制能够提取更鲁棒的解释，不改变网络的决策过程，且无需领域知识或额外训练，为图像分类器解释提供了更可靠的方法。
...
TL;DR: 提出基于Curry-Howard对应关系的理论框架，将Chain-of-Thought推理过程映射为形式化类型证明结构，以验证推理的忠实性。


<details>
  <summary>Details</summary>
Motivation: 解决Chain-of-Thought提示生成的推理过程忠实性问题，提升模型可解释性和可靠性。

Method: 利用Curry-Howard对应关系，将自然语言推理步骤提取并映射到形式化类型证明结构中，通过类型检查验证推理忠实性。

Result: 成功将CoT推理轨迹转换为良类型证明，提供了可验证的计算忠实性证书。

Conclusion: 该框架为将可信的叙事解释转化为形式化可验证程序提供了方法，有助于构建更可靠和可信的AI系统。
...
TL;DR: SIRL利用LLM内部的安全信念，将拒绝有害请求时的高置信度转化为自生成奖励信号，无需外部验证器即可强化模型的安全本能。
...
Method: 提出Safety Instincts Reinforcement Learning (SIRL)，将模型内部的安全置信度转化为自生成奖励信号，通过强化低熵拒绝行为来教导模型信任其安全本能。

Result: 在Llama和Qwen模型上评估，SIRL对20+种越狱方法保持89%+的防御成功率，仅使用15,000个未标记提示就超越了资源密集的监督方法，同时在数学、编码和对话基准上保持性能。
...
TL;DR: 提出一个两层的人本生产规划框架，结合约束规划和强化学习来优化工业制造中的运营效率和劳动力公平性。


<details>
  <summary>Details</summary>
Motivation: 在工业制造中同时优化运营效率和劳动力公平性，考虑工人偏好、经验、适应能力和医疗约束等人本因素。

Method: 第一层使用约束编程(CP)进行订单产线分配，第二层使用马尔可夫决策过程(MDP)进行工人产线分配，比较了贪心分配、MCTS和RL三种策略。

Result: CP调度产生紧凑可行的生产计划，MDP工人分配显著提高了公平性和偏好匹配度，领域专家评价系统组件有效。

Conclusion: 结合CP和学习型决策制定为人本生产规划提供了稳健方法，能够同时优化吞吐量和劳动力福祉。
...
TL;DR: PRISM-Consult是一个临床医生对齐的专家小组架构，将紧凑的PRISM序列模型扩展为路由的领域专家家族，通过轻量级路由器将病例分发到不同专科模型，实现参数效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 开发一个安全、可审计、低延迟的临床咨询系统，通过领域专家模型和路由机制提高计算效率，同时确保临床安全性。

Method: 将临床事件标记化为结构化事件，使用轻量级路由器读取前几个标记并分发到专科模型（心脏血管、肺部、胃肠、肌肉骨骼、心因性），每个专科继承PRISM的小型transformer骨干和标记模板。

Result: 在真实急诊科队列中，专科模型在各领域表现出平滑收敛和低开发困惑度，路由器在安全优先策略下实现高质量路由和显著计算节省。

Conclusion: 该框架为大规模安全、可审计、低延迟的临床咨询提供了实用路径，并概述了满足前瞻性临床部署标准的验证步骤。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG方法过度简化关系，专业模型成本高且静态，无法有效处理金融风险中复杂、多模态的网络原生数据。

Method: 将供应链网络视为知识图谱，利用网络中心性评分指导图遍历提取关键风险路径；采用智能体架构协调图检索、数值因子表和新闻流数据；使用"上下文外壳"模板将原始数据转化为自然语言。
...
TL;DR: Apriel-1.5-15B-Thinker是一个150亿参数的多模态推理模型，通过三阶段训练方法（深度扩展、分阶段持续预训练、高质量微调）实现了前沿性能，而无需大规模计算资源。
...
Method: 采用三阶段渐进方法：1) 深度扩展提升推理能力；2) 分阶段持续预训练，先建立文本和视觉基础理解，再通过合成数据增强视觉推理；3) 高质量文本微调，包含显式推理轨迹。

Result: 在Artificial Analysis Intelligence Index上获得52分，与DeepSeek-R1-0528相当；在10个图像基准测试中，性能平均仅比Gemini-2.5-Flash和Claude Sonnet-3.7低5分，且可在单GPU上部署。

Conclusion: 研究表明，精心设计的中间训练方法可以在不依赖大规模计算的情况下显著缩小能力差距，使前沿多模态推理技术对基础设施有限的组织更加可及。
...
TL;DR: Bridge方法通过在并行LLM推理中生成相互依赖的响应，而不是独立生成，利用隐藏状态间的信息共享来提升响应质量和一致性。


<details>
  <summary>Details</summary>
Motivation: 传统并行LLM推理中，多个响应独立生成，浪费了潜在的有用信息共享机会，而响应长度扩展则能利用过去计算。

Method: 将批处理LLM隐藏状态重新视为整体张量而非独立切片，仅添加少量新参数(2.8%-5.1%)，使并行响应能够相互依赖。

Result: Bridge将基于可验证奖励的强化学习的相对平均准确率提升高达50%，并提高了正确响应的一致性，且可扩展到任何生成宽度。

Conclusion: Bridge解锁了一种更通用的并行扩展模式，有效利用序列间信息，兼容任何后生成聚合技术，性能优于独立生成。
