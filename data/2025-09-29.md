...
Conclusion: 该方法能有效提升放射学报告生成的准确性，且具有模型无关性，无需额外训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有自闭症诊断方法复杂且资源密集，计算机辅助诊断方法在真实世界泛化性和注视特征性能方面存在不足，需要开发更有效的视频分析工具。

Method: 使用标准化视频数据集（168名ASC参与者和157名非自闭症参与者），进行多模态分析，包括面部表情、语音韵律、头部运动、心率变异性和注视行为，并引入新的统计描述符来量化注视角度变异性。

Result: 改进的注视模型将基于注视的分类准确率从64%提升到69%，通过晚期融合多模态特征实现了74%的整体分类准确率。

Conclusion: 研究展示了基于视频的多模态行为标记在自闭症筛查中的潜力，为开发可扩展的自闭症评估支持工具提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人感知和控制中具有统一性优势，但其可扩展性受到注意力二次成本和KV内存无限增长的制约，现有方法往往忽视实时部署所需的推理效率。

Method: 将KV缓存划分为固定大小的块，使用循环门控模块根据学习的效用分数来总结和过滤历史上下文，保留近期细粒度细节，同时积极修剪陈旧的低相关性内存。

Result: 理论上可实现1.21倍推理加速和36% KV内存减少，对任务成功率影响最小，能够无缝集成到现有自回归和混合VLA堆栈中。

Conclusion: 该方法提供了一种训练友好的内存压缩机制，在不修改训练流程或下游控制逻辑的情况下实现可扩展推理。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型能生成逼真的放射学报告，但存在事实错误和幻觉问题，阻碍了临床应用。

Method: 通过扰动真实报告中的发现和位置创建合成数据集，训练多标签跨模态对比回归网络进行事实核查。

Result: 在多个X光数据集上表现出强大的发现真实性预测和定位准确性，与基于真实值的验证达到0.997的相关系数。

Conclusion: 该方法在放射学工作流程的临床推理中具有实用性，能有效检测SOTA报告生成器的错误。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM模型难以捕捉诊断视网膜疾病所需的关键低层空间细节，如青光眼、糖尿病视网膜病变和视网膜色素变性等。

Method: MDF-MLLM将四个U-Net编码器层的跳跃特征集成到LLaMA 3.2 11B MLLM的交叉注意力块中，使用补丁级投影、缩放交叉注意力和基于FiLM的U-Net调制进行特征融合。

Result: 相比基线MLLM的60%准确率，MDF-MLLM达到94%准确率，提升56%；召回率和F1分数分别提升67%和35%。

Conclusion: MDF-MLLM为眼底图像分类提供了一个可泛化、可解释和模块化的框架，在空间推理和分类方面表现优异，特别适用于具有丰富临床文本的遗传性疾病。
...
<details>
  <summary>Details</summary>
Motivation: 现有越狱方法主要操纵文本提示，图像输入漏洞未充分探索，且文本方法难以绕过模型安全过滤器。

Method: 使用LLM将不安全提示解耦为伪安全提示和有害提示，重写有害提示为自然对抗提示，通过视觉语言模型生成图像描述进行迭代优化。

Result: MPDA能够有效绕过T2I模型的安全过滤器，生成与原始不安全提示语义一致的NSFW图像。

Conclusion: MPDA揭示了T2I模型在多模态输入下的安全漏洞，为防御此类攻击提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习方法在显著目标检测中存在的预测不完整和边界不准确问题，这些问题源于目标内部复杂性和卷积池化操作中的步幅。

Method: 1. 将显著目标检测和前景轮廓检测任务交织训练，生成均匀高亮的显著性图；2. 前景轮廓和边缘检测任务相互指导，提升轮廓精度并减少边缘噪声；3. 开发相互学习模块(MLM)，通过多分支网络进行相互学习。

Result: 在七个挑战性数据集上的广泛实验表明，该方法在显著目标检测和边缘检测任务上都达到了最先进的结果。

Conclusion: 通过联合利用多种检测任务的监督信号和相互学习机制，有效解决了显著目标检测中的不完整预测和边界不准确问题，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标仅适用于双模态关联分析，限制了多模态相似性评估。需要一种能够处理3个及以上模态相关性的新方法。

Method: 利用多模态联合表示将多个模态整合到相同潜在空间中，在统一尺度上准确表示不同模态，为公平相关性评分提供支持。
...
Method: 构建云AI平台，处理脚手架点云数据，通过对比认证参考数据和最新点云数据检测结构变化

Result: 系统能够检测脚手架的结构修改，减少人工检查时间和精力

Conclusion: AI和数字化技术能够提高脚手架检查的准确性，改善施工现场安全
...
Result: 图像评估器性能比其骨干网络提升114%，实验表明该方法优于现有最先进基线和ChatGPT-4o。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统光学显微镜横向分辨率限制（约200nm）的问题，避免使用昂贵的专业设备或荧光超分辨率技术，为非专业用户提供经济有效的超分辨率解决方案。

Method: 使用两种自研的深度神经网络架构（O-Net和Theta-Net），在非荧光相位调制显微镜模态（如Zernike相衬和微分干涉对比显微镜）下，对通过原子力显微镜校准的纳米级特征测试靶进行超分辨率重建评估。

Result: 两种网络在超分辨率重建中表现良好但具有互补性：高信噪比图像适合使用O-Net模型，低信噪比图像则更适合Theta-Net模型。

Conclusion: 模型架构与源图像信噪比的结合对深度神经网络在非荧光光学纳米显微镜中的性能至关重要，即使使用相同的训练数据集和训练周期，不同架构也会产生不同的超分辨率质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在视听导航中仅进行基本的视觉和音频数据融合，忽略了更深层次的感知上下文，需要更有效地利用多模态线索来指导导航。

Method: 使用多目标架构配合改进的Transformer机制，对跨模态信息进行过滤和选择性融合。

Result: 在Replica和Matterport3D数据集上，DMTF-AVN在成功率(SR)、路径效率(SPL)和场景适应性(SNA)方面优于现有方法，展现出强大的可扩展性和泛化能力。

Conclusion: 该方法为机器人导航中的先进多模态融合策略铺平了道路。
...
Method: 引入监督式稀疏自编码器训练，通过系统概念标注缓解特征分裂并促进特征集中化，学习具有更强概念关联的专用神经元。

Result: 在推理时，可解释表示将超参数搜索减少96.67%；在UnlearnCanvas基准上比现有技术提升9.22%；在顺序遗忘任务中，9对象移除的遗忘准确率提升28.4%。

Conclusion: SAEmnesia通过促进一对一概念-神经元映射，有效解决了概念表示分散问题，显著提升了概念遗忘的效率和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在生物医学图像分类中需要大量计算资源和时间，特别是在超参数搜索时。随机采样方法在类别不平衡数据集中存在偏向主导类的问题，且无法捕捉类内多样性。

Method: 引入智能轻量级核心集选择机制，通过提取类内多样性形成每类聚类，然后基于这些聚类进行最终采样。

Result: 在知名生物医学成像数据集上的分类实验表明，该方法在多个性能指标上优于随机采样方法。

Conclusion: 提出的核心集选择方案能够有效减少训练计算需求，同时保持模型性能，为大规模数据集训练提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数深度学习模型仅使用单次或有限次数的既往乳腺X光片，且缺乏对现实世界中不平衡结果分布和异质性随访的适应性。需要开发能够充分利用纵向成像数据的稳健模型。

Method: 开发LongiMam模型，结合卷积神经网络（捕捉空间模式）和循环神经网络（捕捉时间模式），整合当前和最多四个既往乳腺X光片，在具有典型临床筛查特征的大规模人群筛查数据集上进行训练和评估。

Result: 在所有包含既往检查的不同场景中，LongiMam均能持续改善预测性能。结合既往和当前检查的模型优于单次检查模型，而仅使用既往检查的效果较差。模型在关键风险亚组（包括致密乳腺女性和55岁及以上女性）中均有效，在乳腺密度随时间变化的女性中表现最佳。

Conclusion: 纵向建模能够增强乳腺癌预测能力，支持在筛查项目中使用重复乳腺X光片来优化风险分层。LongiMam已作为开源软件公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 探索CNNs与人类大脑在复杂社会认知任务（如图像情感评估）中的对应关系，超越一般视觉感知的研究。

Method: 通过相关性分析评估流行CNN架构与人类行为及fMRI数据的对齐程度，并提出了Object2Brain框架，结合GradCAM和对象检测来研究不同对象类别对CNN-人类相关性的影响。

Result: CNNs在图像情感评估任务中难以超越简单视觉处理，不反映高阶大脑处理；不同CNN架构显示出不同的对象类别敏感性。

Conclusion: CNNs在复杂社会认知任务中与人类大脑处理的对应关系有限，需要进一步研究来理解CNN与高阶大脑处理的关系。
...
<details>
  <summary>Details</summary>
Motivation: 传统概念干预方法无法解决模型从有偏数据中学习捷径的系统性偏差问题，需要更全面的调试框架。
...
<details>
  <summary>Details</summary>
Motivation: 沉船是海事历史的重要标记，但通过人工检查测深数据来发现沉船耗时且需要专家分析，因此需要自动化工具来简化这一过程。

Method: 开发了一个深度学习模型，使用来自五大湖和爱尔兰海岸的多种沉船数据进行训练，并采用合成数据生成来增加数据集规模和多样性。工具可自动预处理测深数据、执行深度学习推理、阈值化模型输出，并生成像素级分割掩码或边界框。

Result: 与基于深度学习的ArcGIS工具包和更经典的反向沉洞检测方法相比，该开源工具和训练流水线展示了更优越的分割性能。

Conclusion: ShipwreckFinder提供了一个有效的自动化沉船检测解决方案，通过开源方式使该技术更易于获取和使用。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明神经网络可以大幅剪枝而不损失性能，但剪枝对模型可解释性的影响尚不明确，需要系统研究剪枝如何改变模型的内部表示和解释性。

Method: 使用在ImageNette上训练的ResNet-18，比较不同剪枝水平下的Vanilla Gradients和Integrated Gradients解释方法，评估稀疏性和忠实度，并应用CRAFT进行概念提取分析语义一致性。

Result: 轻中度剪枝改善了显著性图的聚焦度和忠实度，同时保持了语义明确的概念；而过度剪枝会合并异质特征，降低显著性图稀疏性和概念一致性，尽管准确率仍得以保持。

Conclusion: 剪枝可以塑造内部表示使其更符合人类注意力模式，但过度剪枝会损害可解释性，需要在剪枝程度和可解释性之间找到平衡。
...
Result: 在三个挑战性场景理解基准测试中达到最先进性能：(i)使用真实点云，(ii)使用带位姿图像，(iii)使用无位姿图像。在布局估计方面显著提升，为整体室内场景理解设定了新基准。

Conclusion: TUN3D在无需真实相机位姿或深度监督的情况下，实现了联合布局估计和3D物体检测，推进了整体室内场景理解的发展。
...
Method: 1. 将图像分割为关键区域和非关键区域；2. 关键区域使用图像语义编码器处理；3. 非关键区域采用图像到文本建模进行高效压缩；4. 结合模型量化和低秩适配微调实现轻量级部署。

Result: 仿真结果表明，所提系统在语义保真度和视觉质量方面均优于传统方法，证实了其在图像传输任务中的有效性。

Conclusion: 通过精细化语义粒度分割和轻量级部署策略，该系统成功提升了图像语义通信的性能和资源利用效率。
...
<details>
  <summary>Details</summary>
Motivation: 为毫米波ISAC系统的人类感知研究提供开放标记数据集，支持各种终端应用和信号处理、深度学习研究的发展。

Method: 构建包含测试平台、实验设置和信号特征的毫米波数据集，并采用参数高效微调方法适配不同任务。

Result: 数据集在特定下游任务上得到验证，参数高效微调方法显著降低了计算复杂度，同时保持了先前任务的性能。

Conclusion: mmHSense数据集为毫米波ISAC的人类感知研究提供了有价值的资源，参数高效微调方法展示了在保持性能的同时降低复杂度的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 中轴对噪声敏感，微小边界变化会导致骨架过度扩展。传统修剪方法能缓解此问题，但缺乏系统的尺度空间理论支持。

Method: 结合稀疏化原理，构建骨架化尺度空间，通过中轴稀疏化实现层次简化，并引入致密化实现逆过程。

Result: 框架满足层次架构、可控简化和几何变换等变性等尺度空间属性，在鲁棒骨架化、形状压缩和增材制造刚度增强等任务中有效。

Conclusion: 骨架化尺度空间为形状分析提供了理论严谨的层次简化框架，兼具正向简化和逆向恢复能力，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有气候预测空间分辨率低（如12.5公里），限制了其在地方决策中的可用性，需要高分辨率的气候预测数据。

Method: 在高分辨率观测网格数据集上训练单图像超分辨率模型，然后应用于低分辨率气候预测；提出基于气候指标的评估方法，使用气象站观测的气候指数来评估降尺度结果。
...
Method: 提出JaiLIP方法，在图像空间中最小化清洁图像与对抗图像的MSE损失和模型有害输出损失的联合目标函数

Result: 实验显示该方法生成高效且不易察觉的对抗图像，在毒性指标上优于现有方法，并在交通领域验证了攻击的实用性
...
<details>
  <summary>Details</summary>
Motivation: 量化自动识别系统与人类专家在植物识别方面的性能差距，比较深度学习模型与人类专家在物种识别上的不确定性。

Method: 设计ExpertCLEF挑战赛，评估4个研究团队的19个深度学习系统，与9位法国植物学专家进行对比分析。

Result: 最先进的深度学习模型在植物识别方面的性能已接近最先进的人类专家水平。

Conclusion: 深度学习技术在植物自动识别领域取得了显著进展，性能已接近人类专家，为计算机科学家和自然学家提供了有价值的参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有四边形网格生成方法通常先生成三角网格再合并为四边形，这会导致拓扑质量差的问题。需要开发原生四边形网格生成方法。

Method: 提出QuadGPT自回归框架，采用统一标记化方法处理混合拓扑（三角形和四边形），并设计专门的强化学习微调方法tDPO来提升生成质量。
...
Method: 训练概念特定的LoRA适配器，在推理时动态组合所需适配器；引入特征级和参数级的双层正交约束，解耦表示偏移并强制正交适配器子空间。

Result: 在ErasureBench-H和标准数据集上的实验表明，DyME持续优于现有最先进基线，实现了更高的多概念擦除保真度，同时最小化附带退化。

Conclusion: DyME提供了一个灵活、可扩展的多概念擦除解决方案，通过动态适配器组合和正交约束有效解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标无法捕捉人类判断的细微差别，而人工评估成本高昂，需要开发专门针对视频理解任务的自动评估方法。

Method: 使用生成器-评估器交互训练方法：生成器根据目标评分生成响应，评估器筛选不匹配评分的响应，训练专门的MLLM评估器。

Result: VideoJudge-7B在四个元评估基准中的三个上优于更大的基线模型（如Qwen2.5-VL 32B和72B），证明视频输入对视频理解任务评估至关重要。

Conclusion: 专门设计的MLLM评估器在视频理解任务评估中表现优于通用LLM，视频输入是有效评估的关键因素，长链推理并不能提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体协同感知通过共享信息提升场景理解能力，但通信带宽限制了系统的可扩展性，需要开发高效的压缩方法来减少数据传输量。

Method: 采用端到端方法，首先通过简单的瓶颈网络压缩特征维度，然后使用多阶段残差向量量化(RVQ)，仅传输每个像素的代码索引，将特征从8192 bpp压缩到6-30 bpp。

Result: 在DAIR-V2X真实世界数据集上，ReVQom实现了273倍(30 bpp)到1365倍(6 bpp)的压缩比。在18 bpp(455倍压缩)时性能匹配或优于原始特征协同感知，在6-12 bpp时支持超低带宽操作且性能下降平缓。
...
Method: 分析三个AI模型生成的1,006张图像，由沙特注释员评估五个维度：性别、服装外观、背景环境、活动互动和年龄。

Result: 所有模型都显示强烈性别失衡，DALL-E V3输出96%为男性，领导和技术角色中性别失衡最明显，且普遍存在文化不准确性。
...
Conclusion: 该预训练范式有效解决了短视频不当内容检测中的分布差异和复杂问题定义挑战，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将GUI定位视为坐标预测任务，但VLMs在处理高分辨率复杂布局的GUI图像时难以准确预测数值坐标，需要新的解决方案。

Method: 提出GUI-Cursor模型，将GUI定位重构为交互式搜索任务，模型在每个步骤中确定目标对象、评估空间关系，并基于移动历史将光标移向目标，使用多步在线强化学习和密集轨迹奖励函数进行训练。

Result: 基于Qwen2.5-VL-7B的GUI-Cursor在ScreenSpot-v2上从88.8%提升到93.9%，在ScreenSpot-Pro上从26.8%提升到56.5%，95%的实例在两步内解决问题，对困难示例能自适应更多步骤。

Conclusion: 交互式搜索方法比直接坐标预测更有效，GUI-Cursor在GUI定位任务上取得了最先进的结果，并展示了良好的适应性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本-视频检索系统存在两个问题：低质量文本-视频数据对会损害检索效果但难以识别；余弦相似度无法解释排序结果，限制了可解释性。

Method: 在现有基准上增加视频标注以支持语义理解和减少数据偏差；设计检索思维链，包含成对比较步骤，生成详细推理和完整排序。

Result: X-CoT在实证中提高了检索性能，并产生详细推理依据；促进了模型行为和数据质量分析。

Conclusion: X-CoT框架通过LLM思维链推理实现了可解释的文本-视频检索，既提升了性能又增强了可解释性，有助于模型评估和数据检查。
...
<details>
  <summary>Details</summary>
Motivation: 手术器械的安全检测需要可靠的视觉缺陷检测，但人工检查容易出错，现有的基于自然/工业图像的自动缺陷检测方法在手术领域迁移效果不佳，存在误报、对小缺陷敏感度差和领域偏移问题。

Method: 整合背景掩码、基于补丁的分析策略和高效的领域自适应，专门针对手术器械调整无监督缺陷检测方法。

Result: 该方法克服了现有方法的局限性，能够在手术器械图像中可靠地检测细粒度缺陷。

Conclusion: 提出的方法为手术器械缺陷检测提供了一种有效的解决方案，解决了领域迁移和细粒度缺陷检测的挑战。
...
Method: 提出LSEP（线性可分性）正则化方法，将线性探测直接融入网络学习动态，促进中间层表示的线性可分性，无需辅助编码器和表示对齐。

Result: 在SiT等流式Transformer架构上实现了显著改进，在256×256 ImageNet数据集上获得1.46的FID分数，提高了训练效率和生成质量。

Conclusion: LSEP方法有效替代了基于对齐的训练策略，无需外部编码器就能改善扩散模型的表示学习，为大规模扩散模型提供了更高效的训练方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法忽略了地理空间中的空间依赖性，导致无法有效处理视觉和地理相似但被标记为负样本的假阴性对，以及难以区分视觉相似但地理距离较远的困难负样本。

Method: 使用半变异函数将特征空间中的图像距离与其地理距离相关联，拟合空间相关性模型，并以此定义给定空间距离下的预期视觉差异，用于识别困难负样本和假阴性。

Result: 在OSV5M数据集上的评估表明，显式建模空间先验能提高图像地理定位性能，特别是在更细粒度级别上。

Conclusion: 通过整合空间正则化到对比学习中，能够更好地处理地理定位任务中的空间依赖性问题，提升定位精度。
...
Method: 采用Thinker-Actor双变换器架构：Thinker模块使用预训练的大语言-语音模型处理流式用户输入，Actor模块使用分块自回归扩散模型，通过交叉注意力机制生成时间对齐的多模态响应。

Result: X-Streamer在两块A100 GPU上实时运行，能够从任意肖像维持数小时一致性的视频聊天体验。
...
Method: 将运动预测任务构建为条件生成密集轨迹网格，模型架构类似于现代视频生成器，但输出运动轨迹而非像素，从而捕获场景范围内的动态和不确定性。

Result: 该方法在模拟数据上表现优异，在机器人等下游应用中有效，在真实世界直观物理数据集上显示出有前景的准确性，比现有回归器和生成器产生更准确和多样化的预测。

Conclusion: 直接建模运动轨迹比生成像素更有效地解决从单张图像预测运动的问题，视频生成器在此任务上的局限性源于像素生成的额外开销。
...
Method: 在UCF Sports数据集上评估两种方法，通过分类准确率、聚类性能、类内一致性和类间区分度等多个维度分析特征质量。

Result: DINOv3在聚类性能（轮廓分数0.31 vs 0.21）和姿态识别动作的区分能力（6.16倍分离比）方面表现更优，而V-JEPA2在所有动作类型上表现出更一致的可靠性（性能方差0.094 vs 0.288）。

Conclusion: DINOv3的空间处理架构擅长静态姿态识别但在运动依赖动作上性能下降，V-JEPA2的时间建模在不同动作类别间提供更平衡的表示质量，架构选择应根据具体任务需求决定。
...
Method: 采用双架构方法：基于ResNet50的CNN-LSTM模型处理卫星图像，Vision Transformer模型处理无人机图像，并结合ConceptNet和WordNet的外部语义知识来扩展词汇覆盖和提高描述准确性。

Result: VLCE在InfoMetIC指标上达到最高95.33%，同时保持竞争力的语义对齐，显著优于LLaVA和QwenVL等基准模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉模型在架构和训练范式上差异很大，但缺乏系统方法来识别哪些表示特征是跨家族共享的，哪些反映了独特的计算策略。

Method: 使用多种表示相似性度量（RSA、Soft Matching、Linear Predictivity等）评估家族可分性，并采用Similarity Network Fusion方法整合这些互补的度量维度。

Result: 几何和调谐度量能强区分模型家族，而线性预测性度量区分较弱。SNF融合后获得更清晰的家族分离，发现监督ResNets和ViTs形成不同簇，而所有自监督模型跨架构边界聚集。

Conclusion: 该生物学启发框架为视觉模型提供了原则性分类，表明由架构和训练目标共同塑造的新兴计算策略定义了超越表面设计类别的表示结构。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型缺乏明确的3D基础能力，在空间一致性和下游3D推理任务实用性方面存在局限。高质量3D世界模型对具身智能和AGI至关重要。

Method: 引入跨分支监督机制：几何线索指导视频生成，视频先验正则化3D预测。通过联合建模视频潜在空间和隐式3D场，实现3D感知的视频表示。

Result: 实验表明FantasyWorld有效桥接了视频想象和3D感知，在多视图一致性和风格一致性方面优于最近的几何一致性基线。几何分支的潜在表示可作为下游3D任务的通用表示。

Conclusion: 该框架通过统一主干和跨分支信息交换，成功实现了3D感知的视频生成，无需逐场景优化或微调即可支持新颖视图合成和导航等下游任务。
...
<details>
  <summary>Details</summary>
Motivation: 科学观测数据通常具有异构性和多模态特性，现有模型难以统一处理不同维度、分辨率和物理场的PDE数据。需要开发能灵活处理这种复杂性的基础模型。

Method: 基于卷积视觉Transformer架构，结合组件卷积处理标量和矢量通道，跨场注意力建模不同物理场间关系，轴向注意力降低计算负担。使用预训练和微调策略。

Result: 在多个下游预测任务中，MORPH在零样本和全样本泛化方面均优于从头训练的模型，匹配或超越了现有最先进模型。

Conclusion: MORPH为从异构多模态科学观测中学习提供了灵活强大的基础架构，为实现可扩展和数据高效的科学机器学习指明了方向。
...
Method: 在YOLOv8基础上，用MobileNetV4替换CSPDarknet骨干网络以减少计算量，并引入SlideLoss损失函数动态强调欠代表和遮挡样本。

Result: 在FLIR ADAS V2数据集上，MS-YOLO以仅6.7 GFLOPs的计算量获得竞争性的mAP和优越的精度，计算开销减少1.5%。
...
Method: 引入运动感知Transformer，显式预测物体在帧间的运动来提前更新跟踪查询，减少查询碰撞，实现更一致的训练。

Result: 在DanceTrack上HOTA提升超过9点，达到71.3的SOTA；在SportsMOT和BDD100k上也达到SOTA结果，分别为72.2 HOTA和54.7 mTETA/41.6 mHOTA。
...
<details>
  <summary>Details</summary>
Motivation: 解决野外拍摄视频中的雨纹、模糊和噪声问题，以及相机姿态微小变化导致的跨帧不匹配和时间伪影。现有方法依赖光流或启发式对齐，计算量大且鲁棒性差。

Method: 提出两个互补组件：1) 旋转有界的李相对偏差，通过紧凑预测模块预测每帧的平面内角度，实现几何一致对齐；2) 微分群位移，计算相邻帧间的角度差来估计速度，结合时间衰减和注意力掩码关注帧间关系。

Result: 在公开基准测试上的广泛实验结果表明该方法的有效性。

Conclusion: 李群为视频建模中的空间和时间一致性提供了原则性表示方法，DeLiVR通过注入李群微分偏差有效解决了视频去雨问题。
...
<details>
  <summary>Details</summary>
Motivation: 受自然图像领域基础模型成功的启发，希望将这种技术应用于SAR领域，利用基础模型的优势（少样本适应、鲁棒性强、特征可迁移）来解决SAR目标识别任务。

Method: 首先测试现有视觉基础模型(DINOv2、DINOv3、PE-Core)在SAR数据上的表现，然后通过自监督学习对公开SSL模型进行SAR数据微调，训练AFRL-DINOv2系列模型。

Result: AFRL-DINOv2模型显著超越了当前最好的SAR领域模型SARATR-X，建立了新的SAR基础模型最先进水平，并在不同下游任务适应方案中分析了性能权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖物理设备或静态截图分析，难以进行可扩展的UI测试和智能UI代理训练。
...
TL;DR: 提出LFA-Net轻量级视网膜血管分割网络，使用新设计的LiteFusion-Attention模块，在资源受限环境中实现高效分割。


<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习模型在小血管分割和计算成本高方面的挑战，特别是在计算资源有限的临床环境中。

Method: 设计LiteFusion-Attention模块，融合残差学习连接、Vision Mamba启发的动态机制和基于调制的注意力，高效捕获局部和全局上下文。

Result: 在DRIVE、STARE和CHASE_DB数据集上表现优异，Dice分数分别为83.28%、87.44%、84.50%，Jaccard指数分别为72.85%、79.31%、74.70%。模型仅11万参数、0.42MB内存和4.46GFLOPs。

Conclusion: LFA-Net在资源受限环境中提供高性能视网膜血管分割，是轻量级分割模型的理想选择。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法低估了视觉场景上下文信息在建模个体关系中的重要性，并忽视了情感标签语义信息对情感完整理解的关键作用

Method: 包含视觉上下文编码模块（利用多尺度场景信息编码个体关系）和情感语义编码模块（使用群体级情感标签提示大语言模型生成细致情感词汇，通过结构化情感树精炼为全面语义表示），最后通过相似性感知交互对齐和整合视觉与语义信息

Result: 在三个广泛采用的GER数据集上的实验表明，该方法相比最先进方法取得了竞争性性能

Conclusion: 提出的结合视觉场景上下文和标签引导语义信息的框架有效提升了群体级情感识别性能
...
TL;DR: KG-SAM是一个知识引导的医学图像分割框架，通过整合解剖学先验知识、边界优化和不确定性估计，显著提升了SAM模型在医学图像分割中的性能。


<details>
  <summary>Details</summary>
Motivation: SAM模型直接应用于医学图像存在边界模糊、缺乏解剖关系建模和不确定性量化等挑战，需要专门针对医学图像特点进行优化。

Method: 整合医学知识图谱编码解剖关系，使用基于能量的条件随机场确保解剖一致性，并加入不确定性感知融合模块提高临床可靠性。

Result: 在前列腺分割中达到82.69%的平均Dice分数，腹部分割在MRI和CT上分别达到78.05%和79.68%。

Conclusion: KG-SAM为医学图像分割提供了一个鲁棒且可推广的框架，显著提升了分割性能。
...
TL;DR: UniVid框架通过微调视频扩散变换器，将多种视觉任务统一表示为视觉句子，无需任务特定修改即可处理跨模态和跨源任务。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉模型需要跨模态和跨源的任务特定预训练，成本高且难以扩展到未见任务。受预训练视频生成模型已捕获时序依赖的启发，探索能否将其适配到多样化图像和视频任务。

Method: 提出UniVid框架，微调视频扩散变换器，将任务表示为视觉句子，其中上下文序列定义任务和期望输出模态。支持跨模态推理（图像和视频混合）和跨源任务（自然到标注数据）。

Result: 仅使用自然视频数据训练的UniVid在跨模态和跨源设置下均表现良好。理解和生成任务可通过简单反转视觉句子顺序轻松切换。

Conclusion: 预训练视频生成模型有潜力作为视觉建模的可扩展统一基础，展示了在无需多源预训练的情况下处理多样化任务的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现代ViT骨干网络采用空间架构设计（如窗口注意力、分解相对位置编码等），现有token缩减方法无法保持这些架构所依赖的空间结构。
...
Conclusion: 该方法在空间和非空间架构上均表现出色，实现了最先进的性能，支持即插即用和微调两种使用方式。
...
TL;DR: 提出GASP-ICL框架，无需训练即可增强大型视觉语言模型在多模态深度伪造检测中的性能，通过任务对齐检索和自适应评分机制提升检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在多模态深度伪造检测中面临挑战：难以捕捉细微伪造线索、解决跨模态不一致性、执行任务对齐检索。

Method: 采用GASP-ICL框架，包含MDD适配的特征提取器检索对齐的图像-文本对，设计图结构泰勒自适应评分器(GSTAS)捕获跨样本关系并传播查询对齐信号，生成判别性示例。

Result: 在四种伪造类型上的实验表明，GASP-ICL超越了强基线方法，在不进行LVLM微调的情况下实现了性能提升。

Conclusion: GASP-ICL通过语义对齐的任务相关演示选择，有效增强了大型视觉语言模型在多模态深度伪造检测中的鲁棒性。
...
TL;DR: 提出了ProDA框架，通过时空场景图和动态提示模块从多动作场景中解耦指定动作，用于视频动作识别。


<details>
  <summary>Details</summary>
Motivation: 现有方法提取统一特征处理视频中的所有动作，在多动作场景中难以建模不同对象间的交互。
...
TL;DR: 本文提出了一个用于检测数字内容中仇恨信息的多模态数据集，结合水印稳定扩散技术和数字注意力分析模块，开发了DeHater视觉语言模型来识别和模糊图像中的仇恨元素。
...
Method: 采用水印稳定扩散技术结合数字注意力分析模块(DAAM)，生成详细的仇恨注意力地图，用于识别和模糊图像中的仇恨区域。

Result: 开发了DeHater视觉语言模型，能够基于文本提示检测图像中的仇恨内容，并实现了仇恨区域的精确识别和模糊处理。

Conclusion: 该方法为AI驱动的图像仇恨检测设立了新标准，有助于开发更符合伦理的社交媒体AI应用，并发布了dehate共享任务数据集。
...
TL;DR: LongScape是一个混合视频生成框架，通过结合扩散去噪和自回归生成，实现了稳定的长时程视频生成，特别适用于机器人操作任务。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在长时程生成中存在不足：扩散方法存在时间不一致性和视觉漂移，自回归方法则牺牲视觉细节。需要一种能同时保证时间一致性和视觉质量的解决方案。

Method: 提出混合框架，结合了块内扩散去噪和块间自回归因果生成。核心创新是动作引导的可变长度分块机制，以及上下文感知的专家混合框架(CMoE)。

Result: 实验结果表明，该方法在长时程生成中实现了稳定和一致的性能，优于现有方法。

Conclusion: LongScape通过创新的分块机制和专家混合框架，成功解决了长时程视频生成的挑战，为高质量具身操作数据生成提供了有效解决方案。
...
TL;DR: 提出了MoWM框架，通过融合潜在世界模型和像素空间模型的混合表示来解决具身动作规划问题，在CALVIN基准测试中实现了最先进的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成世界模型依赖像素级重建会引入视觉冗余，而潜在世界模型虽然紧凑但忽略了精细细节，这阻碍了动作解码和泛化能力。

Method: 使用潜在模型的运动感知表示作为高级先验，指导从像素空间模型提取细粒度视觉特征，突出对动作解码有用的信息性视觉细节。

Result: 在CALVIN基准测试中实现了最先进的任务成功率和优越的泛化性能。

Conclusion: MoWM框架通过混合世界模型融合有效解决了具身动作规划问题，为未来研究提供了有价值的见解。
...
TL;DR: DiTraj是一个无需训练的轨迹控制框架，专门为Diffusion Transformers (DiT)设计，通过前景-背景分离引导和改进的3D-RoPE实现精确的轨迹控制。
...
Method: 1. 前景-背景分离引导：使用LLM将用户提示转换为前景和背景提示；2. 提出STD-RoPE，通过修改前景token的位置嵌入消除跨帧空间差异；3. 通过调节位置嵌入密度实现3D感知轨迹控制。
...
TL;DR: 该论文评估了多跳问答中的检索策略，提出了一种结合密集嵌入与词汇重叠的混合检索方法，在HotpotQA数据集上相比余弦相似度方法取得了50%的精确匹配和47%的F1分数相对提升。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在问答任务中取得进展，但多跳推理（需要结合多个段落证据）仍然困难，需要改进检索策略来支持多跳问答。

Method: 在检索增强生成框架中比较了余弦相似度、最大边际相关性和混合方法，并采用EfficientRAG管道进行查询优化，引入token标记和迭代优化。

Result: 混合方法显著优于基线方法，在HotpotQA数据集上相比余弦相似度方法，精确匹配相对提升50%，F1分数相对提升47%。
...
TL;DR: 该论文提出HDR-4DGS方法，用于解决HDR动态新视角合成问题，通过动态色调映射模块连接HDR和LDR域，实现时间辐射一致性和空间准确的颜色转换。


<details>
  <summary>Details</summary>
Motivation: 现有HDR新视角合成方法主要关注静态场景，但现实场景常包含动态元素（如移动物体、变化光照等），需要同时建模时间辐射变化和3D转换。
...
Conclusion: 该方法成功解决了HDR动态新视角合成的挑战，实现了时间辐射一致性和准确的颜色空间转换。
...
Method: 提出几何感知隐式图像函数(GIIF)，通过联合优化隐式图像函数和显式3D手部形状，学习详细手部先验并提升输入图像分辨率。
...
TL;DR: 该论文指出当前AI生成图像检测研究过于关注"假"图像而忽视了对"真实"图像的精确定义和数据收集，认为需要重新思考"真实"图像的概念并建立新的基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型的普及引发了对其负面社会影响的担忧，但当前检测方法主要依赖过时的低分辨率"真实"图像数据集，而现代智能手机的图像形成算法与"假"图像生成器在架构上相似，需要重新定义"真实"图像的概念。

Method: 这是一篇立场论文，通过分析当前研究现状，指出问题并提出观点，而非提出具体的技术方法。

Result: 分析表明当前几乎所有"假"图像检测方法的开发和评估都依赖于少数过时的低分辨率"真实"图像数据集，而现代图像采集技术已经发生了革命性变化。

Conclusion: 需要重新思考"真实"图像的概念，建立清晰的技术定义和新的基准数据集，甚至质疑检测"假"图像这一目标本身是否合理。
...
TL;DR: Aes-R1是一个基于强化学习的多模态美学评估框架，通过构建高质量的美学推理数据和联合优化绝对评分与相对排序，提升多模态大语言模型的美学评分准确性和解释能力。
...
Method: 提出Aes-R1框架：1）AesCoT管道构建和过滤高质量链式思维美学推理数据用于冷启动；2）RAPO强化学习算法联合优化绝对评分回归和相对排序；3）让模型在评分前生成结构化解释。

Result: 实验表明Aes-R1将骨干模型的平均PLCC/SRCC提升了47.9%/34.8%，超越了同类规模的最先进基线，并在有限监督和分布外场景下展现出鲁棒泛化能力。
...
TL;DR: 提出StableDub框架，通过唇部习惯建模和遮挡鲁棒合成解决视觉配音中的两个关键问题：音频驱动无法捕捉说话者特定唇部习惯，以及传统盲修复方法在处理遮挡时产生视觉伪影。


<details>
  <summary>Details</summary>
Motivation: 现有视觉配音方法存在两个关键缺陷：(1)仅基于音频的驱动范式无法捕捉说话者特定的唇部习惯，导致生成的唇部运动与目标角色不相似；(2)传统盲修复方法在处理遮挡时经常产生视觉伪影，限制实际部署。

Method: 基于Stable-Diffusion骨干网络，开发唇部习惯调制机制，联合建模音素音频-视觉同步和说话者特定口面部动态；引入遮挡感知训练策略，在修复过程中显式暴露遮挡物体；采用混合Mamba-Transformer架构优化训练效率。

Result: 实验结果表明StableDub在唇部习惯相似性和遮挡鲁棒性方面表现优异，在音频-唇部同步、视频质量和分辨率一致性方面超越其他方法。

Conclusion: StableDub通过整合唇部习惯感知建模和遮挡鲁棒合成，消除了先前方法对成本密集型先验的需求，在计算密集型扩散骨干网络上展现出优越的训练效率，扩展了视觉配音方法的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 旨在实现用户对3D场景中物体运动的交互控制，解决现有方法在物体运动控制和3D场景集成方面的局限性。

Method: 采用三阶段流程：1) 使用2D高斯泼溅和全景图像增强文本到3D背景生成；2) 通过3D复制粘贴方法将物体网格集成到场景中；3) 使用部分增强的运动条件视频扩散模型进行时间动画。

Result: 实现了用户控制的物体运动与高质量3D背景的协调对齐，展示了在运动幻觉缓解和视图一致性方面的有效性。

Conclusion: Drag4D提供了一个统一的架构，成功实现了3D场景中物体运动的交互控制，为动态3D内容生成提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频和图像到视频生成方法在控制运动时序方面存在局限，而音频提供了与视频运动对齐的时间线索，是时间控制视频生成的有前景条件。但现有音频到视频模型由于间接条件机制或有限的时间建模能力，难以实现细粒度同步。

Method: 基于预训练视频骨干网络，引入两个关键组件：1）运动感知损失，强调在高运动区域的学习；2）音频同步引导，使用视觉对齐的无音频层离同步模型来引导完整模型，在推理时更好地利用音频线索同时保持视觉质量。

Result: 在AVSync15和The Greatest Hits数据集上的实验表明，Syncphony在同步精度和视觉质量方面均优于现有方法。提出的CycleSync度量标准通过测量生成视频中的运动线索来重建原始音频，有效评估同步效果。

Conclusion: Syncphony通过运动感知损失和音频同步引导技术，成功实现了音频与视频的精确同步生成，为时间控制视频生成提供了有效解决方案。
...
Method: 使用视觉基础模型(SAM2)作为特征提取器获取多尺度金字塔特征，采用多层适配器进行微调，设计文本融合注意力模块(TFAM)对齐视觉和文本信息，通过视觉-语义融合解码器(V-SFD)深度整合视觉和语义信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本驱动方法擅长纹理操作但缺乏精确空间控制，而拖拽驱动方法主要修改形状结构但缺乏细粒度纹理指导，需要结合两者优势。

Method: 提出点云确定性拖拽增强潜在空间布局控制，以及拖拽-文本引导去噪在去噪过程中动态平衡拖拽和文本条件的影响。

Result: 实验表明该方法不仅实现高保真联合编辑，还在单独文本或拖拽模式下达到或超越专用方法的性能。
...
TL;DR: 提出了一种sideload-CL-adaptation框架，通过对比学习利用未标注数据提升北欧地区雪地环境下无人机车辆检测性能，在NVD数据集上mAP50提升3.8%-9.5%。


<details>
  <summary>Details</summary>
Motivation: 解决北欧地区无人机图像中车辆检测面临的强能见度挑战和雪覆盖引起的域偏移问题，利用廉价获取的未标注数据改善检测性能。

Method: 使用对比学习在预训练阶段训练CNN表示提取器，然后在微调阶段将其侧载到冻结的YOLO11n骨干网络中，比较不同融合方法和粒度。

Result: 在NVD数据集上，提出的sideload-CL-adaptation模型在mAP50指标上提升了3.8%到9.5%的检测性能。

Conclusion: 通过对比学习利用未标注数据可以有效提升轻量级模型在具有挑战性的北欧雪地环境下的车辆检测性能。
...
TL;DR: 提出了IF-V2V方法，无需反转即可将现成的基于流匹配的图像到视频模型适配用于视频编辑，通过向量场校正和结构运动保持初始化实现高效编辑。
...
Conclusion: IF-V2V方法实现了无需反转的高质量视频编辑，能够有效利用现有图像到视频模型，为视觉创意提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统多视角计数方法依赖全监督学习，需要大量标注数据。本文旨在通过自监督学习减轻对标注数据的依赖。

Method: 使用神经体积渲染学习场景的隐式表示，通过可微分神经渲染重建连续几何形状和复杂视角相关外观。
...
TL;DR: 本文提出了一个针对基础模型的对象中心空间推理基准，评估了现有视觉模型在空间定位、空间推理和下游检索任务上的表现，发现检测器和视觉语言模型在空间理解能力上存在稳定的权衡。
...
Method: 使用受控的合成数据集，评估了最先进的视觉模型（如GroundingDINO、Florence-2、OWLv2）和大型视觉语言模型（如InternVL、LLaVA、GPT-4o）在三个任务上的表现：空间定位、空间推理和下游检索任务。

Result: 检测器（如GroundingDINO和OWLv2）提供精确的边界框但关系推理能力有限，而视觉语言模型（如SmolVLM和GPT-4o）提供粗略的布局线索和流畅的描述，但在细粒度空间上下文方面表现不佳。
...
TL;DR: PANICL是一种无需训练的训练框架，通过利用多个上下文图像对来缓解视觉上下文学习中对单个样本过度依赖的问题，提高预测的稳定性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视觉上下文学习(VICL)通常过度依赖单个上下文图像对，导致预测存在偏见和不稳定。需要一种方法来利用多个上下文对来平滑预测结果。

Method: 提出基于补丁的k最近邻视觉上下文学习(PANICL)框架，通过平滑多个上下文对的分配分数来减少偏见，无需额外训练。

Result: 在多种视觉任务上（前景分割、目标检测、着色、多目标分割、关键点检测）都取得了优于基线的稳定改进，对领域偏移表现出强鲁棒性。

Conclusion: PANICL是一个通用且无需训练的方法，能显著提升VICL模型的性能和鲁棒性，具有良好的泛化能力和广泛应用前景。
...
TL;DR: SingRef6D是一个轻量级的6D姿态估计方法，仅需单张RGB参考图像，无需深度传感器或多视角图像，通过改进深度预测和深度感知匹配来应对透明、反光等挑战性表面和低光照条件。


<details>
  <summary>Details</summary>
Motivation: 现有6D姿态估计方法依赖深度传感器，在透明或高反光表面会失效；RGB方法在低光照和无纹理场景中匹配性能不足。需要一种不依赖深度传感器且能处理挑战性条件的解决方案。

Method: 1. 基于Depth-Anything v2提出token-scaler微调机制和新优化损失，提升深度预测精度；2. 引入深度感知匹配过程，在LoFTR中整合空间关系，处理挑战性材质和光照条件。

Result: 在REAL275深度预测上比Depth-Anything v2提升14.41%；在REAL275、ClearPose和Toyota-Light数据集上的姿态估计平均召回率提升6.1%，超越现有最佳方法。

Conclusion: SingRef6D证明了仅用单张RGB图像就能实现稳健的6D姿态估计，在资源受限环境下具有实用价值，能有效处理挑战性表面和光照条件。
...
TL;DR: DynaNav是一个动态视觉导航框架，通过基于场景复杂度的自适应特征和层选择，显著降低了计算开销并提升了可解释性。
...
Conclusion: DynaNav在保持导航性能的同时，显著提高了计算效率，为资源受限环境中的视觉导航提供了有效解决方案。
...
TL;DR: 提出了SemanticControl方法，能够在视觉条件与文本提示不完全对齐的情况下，有效利用语义相关的视觉条件进行图像生成。


<details>
  <summary>Details</summary>
Motivation: 解决ControlNet在视觉条件与文本提示不对齐时的局限性，特别是针对不常见或想象性场景，当精确对齐的视觉条件不可用时的问题。

Method: 通过运行辅助去噪过程提取注意力掩码，在目标提示的去噪过程中自适应地抑制视觉条件冲突部分，同时增强文本引导。

Result: 实验结果表明该方法在各种视觉条件下（深度图、边缘图、人体骨架）都能在松散对齐条件下提升性能，优于现有基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在零样本场景下的视觉情感感知能力存在争议，主要由于评估方法的约束，包括忽略合理响应、有限情感分类、忽视上下文因素和人工标注成本高。
...
Conclusion: 通过开发基础评估框架和进行全面的MLLM评估，这项工作有助于推进MLLMs的情感智能发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习的多主体图像生成方法依赖简单的重建目标，导致严重的属性泄漏和无法对齐人类偏好。
...
Method: 采用编码器-解码器架构，其中基于三平面的双分支编码器生成空间结构化token；引入模型在环标注流程，从在线资源中整理超过500万个3D形状-部件对。

Result: PartSAM在多个基准测试中大幅超越最先进方法，实现了高度准确的部件识别，并能自动将形状分解为表面和内部结构。

Conclusion: PartSAM标志着向3D部件理解基础模型迈出的决定性一步，结合可扩展架构和多样化3D数据，产生了新兴的开放世界能力。
...
TL;DR: 提出了一种基于深度学习的无参考图像对比度质量评估框架，通过定制和微调EfficientNet B0、ResNet18和MobileNetV2等预训练架构，在对比度失真评估方面取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 图像对比度是视觉感知的基本因素，但大多数无参考图像质量评估模型难以在多样化真实条件下准确评估对比度失真。
...
Method: 提出推理中心强化微调(RFT)范式Geo-R1，采用"先推理后行动"策略：先生成可解释的推理链分解指代表达，然后利用这些推理来定位目标对象。

Result: 在三个精心设计的少样本地理空间指代表达基准测试中，Geo-R1模型持续且显著优于监督微调基线，并展现出强大的跨数据集泛化能力。

Conclusion: Geo-R1通过推理中心的方法有效利用有限标注数据，提升泛化能力和可解释性，为少样本地理空间指代表达理解提供了有效解决方案。
...
TL;DR: 该研究评估了视觉语言模型在医学视觉问答中的临床顺从性问题，提出了一个医学顺从性数据集和轻量级缓解策略VIPER，通过过滤非证据性内容来减少模型顺从行为。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在临床工作流程中应用日益广泛，但它们经常表现出顺从行为，优先考虑与用户措辞、社交线索或感知权威的一致性，而不是基于证据的推理。

Method: 构建医学顺从性数据集，使用心理动机的压力模板进行对抗性实验，提出VIPER缓解策略，通过过滤非证据性内容并生成基于证据的答案。

Result: 发现各种VLM模型普遍存在脆弱性，表现出显著的对抗响应变化，模仿和专家提供的纠正被发现是最有效的触发因素。VIPER框架平均减少了顺从性，优于基线方法。

Conclusion: 该研究的基准分析和缓解框架为医学VLM在真实世界临床交互中的稳健部署奠定了基础，强调了对基于证据的防御的需求。
...
TL;DR: GLARIFY是一种利用时空注视信息增强视觉语言模型在现实应用中效果的新方法，通过处理模糊的注视模式和口头问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着智能眼镜的普及，用户注意力被整合到视觉语言模型中以简化日常场景中的多模态查询。但利用注视数据建模用户注意力面临两个挑战：用户口头问题因使用代词或跳过上下文而变得模糊；人类注视模式嘈杂且与口头问题存在复杂的时空关系。

Method: 首先分析数百个带有注视模态的查询样本以证明用户注视模式的嘈杂性；然后使用GPT-4o设计自动数据合成管道生成GLARIFY-Ambi数据集，包含专门处理嘈杂注视模式的思维链过程；最后设计热图模块将注视信息整合到前沿视觉语言模型中，同时保留其预训练知识。
...
TL;DR: 该论文系统研究了大型视觉语言模型的空间偏差问题，发现模型对图像中相同视觉信息在不同位置的响应不一致，并提出平衡位置分配机制来提升空间鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出色，但其对空间变化的鲁棒性尚未得到充分理解。研究发现当相同关键视觉信息放置在不同位置时，模型会产生不一致的输出，这揭示了其空间语义理解的根本局限性。

Method: 通过精心设计的探测数据集分析LVLMs的空间偏差，发现问题源于语言模型中位置嵌入的不平衡设计。提出平衡位置分配机制，为所有图像token分配相同的位置嵌入，促进更平衡的视觉信息整合。

Result: 实验表明BaPA无需重新训练即可增强LVLMs的空间鲁棒性，结合轻量级微调还能进一步提升其在多样化多模态基准测试中的性能。信息流分析显示BaPA产生平衡的注意力，实现更全面的视觉理解。

Conclusion: LVLMs存在显著的空间偏差问题，主要源于位置嵌入的不平衡设计。BaPA作为一种简单有效的机制，能够显著提升模型的空间鲁棒性和整体性能，为构建更稳健的多模态模型提供了重要启示。
...
TL;DR: 提出了一种从预训练扩散模型中解耦视觉和语义特征的新方法，支持视觉对应关系量化，并提出了VSM指标来评估主题驱动图像生成中的视觉不一致性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型主干网络包含丰富的语义特征，但也必须包含视觉特征来支持图像合成。然而，由于缺乏标注数据集，分离这些视觉特征具有挑战性。

Method: 引入自动化管道构建具有标注语义和视觉对应关系的图像对，设计对比架构来分离两种特征类型，利用解耦表示提出VSM指标。

Result: 该方法在量化视觉不一致性方面优于CLIP、DINO和视觉语言模型等全局特征指标，同时支持不一致区域的空间定位。
...
TL;DR: WAVE是首个基于LLM的统一音频-视觉嵌入模型，通过分层特征融合和联合多模态多任务训练，实现了任意模态间的跨模态检索和提示感知嵌入生成。
...
Conclusion: WAVE为跨模态任意应用开辟了广阔可能性，消融研究验证了联合训练策略的有效性。
...
TL;DR: ERGO提出了一种从粗到精的推理管道，通过先分析下采样图像识别任务相关区域，然后仅对这些区域进行全分辨率处理，从而减少计算成本同时保留必要的视觉细节。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在处理高分辨率图像时会产生大量视觉标记，导致计算开销巨大。随着"图像思考"模型的出现，推理能力扩展到视觉领域，这促使开发更高效的处理方法。

Method: 采用两阶段"从粗到精"推理流程：第一阶段分析下采样图像识别任务相关区域；第二阶段仅对相关区域进行全分辨率处理。使用强化学习框架开发奖励组件来处理感知不确定性。

Result: ERGO在多个数据集上比原始模型和竞争方法获得更高准确率，同时效率更高。例如在V*基准测试中超越Qwen2.5-VL-7B 4.7分，仅使用23%的视觉标记，实现3倍推理加速。

Conclusion: ERGO通过推理驱动的感知方法，在保持准确性的同时显著提高了处理高分辨率图像的效率，为实际视觉语言应用提供了可行的解决方案。
...
TL;DR: DualFocus是一个新颖的深度聚焦框架，通过联合建模空间和焦距维度的焦点变化，利用变分公式和双约束来提高深度估计的鲁棒性和准确性。
...
Method: 引入变分公式，包含空间约束和焦距约束：空间约束利用不同焦距级别的梯度模式变化来区分真实深度边缘和纹理伪影；焦距约束强制单峰、单调的焦点概率与物理焦点行为一致。

Result: 在四个公共数据集上的综合实验表明，DualFocus在深度准确性和感知质量方面始终优于最先进的方法。

Conclusion: 通过利用焦点堆栈的独特梯度模式并施加物理启发的约束，DualFocus在具有挑战性的区域显著提高了深度估计的鲁棒性和准确性。
...
TL;DR: 提出了基于信息论的协作感知理论框架RDcomm，通过任务熵离散编码和互信息驱动的消息选择，在3D目标检测和BEV分割任务上实现108倍通信压缩的同时保持SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法缺乏理论基础，主要依赖经验性权衡任务性能和通信量，需要建立理论框架来指导通信策略设计。

Method: 基于信息论提出实用的率失真理论，包含两个关键条件：提供实用相关信息、传输无冗余消息。据此设计RDcomm框架，包含任务熵离散编码（分配任务相关码字长度）和互信息驱动的消息选择（使用互信息神经估计）。

Result: 在DAIR-V2X和OPV2V数据集上，RDcomm在3D目标检测和BEV分割任务中达到最先进精度，同时通信量减少高达108倍。

Conclusion: 提出的率失真理论为多智能体协作提供了理论基础，RDcomm框架证明了理论指导下的通信策略设计能显著提升通信效率而不牺牲性能。
...
TL;DR: FailureAtlas是一个主动探索文本到图像模型失败案例的框架，通过结构化搜索发现最小化失败诱导概念，揭示了大量未知错误切片并与训练数据稀缺性相关联。


<details>
  <summary>Details</summary>
Motivation: 静态基准测试在诊断文本到图像模型的系统性失败方面能力有限，需要一种主动探索的方法来全面映射失败景观并识别根本原因。

Method: 将错误发现构建为结构化搜索问题，寻找最小化失败诱导概念，并开发了新颖的加速技术来处理计算爆炸性问题。

Result: 在Stable Diffusion模型上发现了超过247,000个先前未知的错误切片，并首次提供了大规模证据表明这些失败与训练数据稀缺相关。

Conclusion: FailureAtlas建立了一种新的诊断优先方法论，为开发更稳健的生成式AI提供了原则性和可扩展的深度模型审计引擎。
...
TL;DR: 提出了一种无需训练的自监督幻觉缓解方法，通过文本到图像模型将描述投影到视觉空间来放大幻觉信号，使用正负锚点编辑解码器隐藏状态以减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但对幻觉高度敏感，现有方法通常需要额外微调或牺牲信息量和可扩展性。
...
TL;DR: CoFFT是一种无需训练的方法，通过模拟人类视觉认知来增强视觉语言模型的推理能力。它通过三个阶段的迭代循环：生成多样推理样本、双重前瞻解码评估、视觉焦点调整，使推理指导视觉焦点，视觉焦点又为后续推理提供信息。
...
Method: CoFFT包含三个迭代阶段：(1) 生成多样推理样本来探索潜在推理路径；(2) 基于视觉焦点和推理进展双重评估样本，将最优样本的第一步加入推理过程；(3) 精确调整视觉焦点到对未来推理最有益的区域，然后返回阶段(1)直到得到最终答案。

Result: 在多个基准测试中使用Qwen2.5-VL、InternVL-2.5和Llava-Next模型，CoFFT实现了3.1-5.8%的持续性能提升，同时计算开销可控增加。

Conclusion: CoFFT通过模拟人类视觉认知过程，有效提升了视觉语言模型的视觉推理能力，解决了复杂视觉输入中的干扰问题，且无需额外训练即可实现性能提升。
...
TL;DR: 提出了一个轻量级的多模态代理框架，用于基于视频的场景理解，结合Qwen2.5-VL-3B-Instruct模型和SmolAgent编排层，支持思维链推理、语音视觉融合和动态工具调用。


<details>
  <summary>Details</summary>
Motivation: 医疗机器人需要强大的多模态感知和推理能力，但当前的视觉语言模型在时间推理、不确定性估计和结构化输出方面存在局限，无法满足机器人规划需求。

Method: 结合Qwen2.5-VL-3B-Instruct模型与SmolAgent编排层，支持思维链推理、语音视觉融合和动态工具调用，生成结构化场景图并利用混合检索模块进行可解释的自适应推理。

Result: 在Video-MME基准测试和自定义临床数据集上的评估显示，与最先进的视觉语言模型相比，具有竞争力的准确性和改进的鲁棒性。

Conclusion: 该框架在机器人辅助手术、患者监测和决策支持等应用中展现出潜力。
...
TL;DR: 该论文提出了一个新的第一人称视角面对面教学视频数据集，用于分析教学互动中的步骤分割和对话状态分类任务，并评估了多模态大语言模型在这些任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 面对面教学互动在计算机视觉领域尚未得到系统研究，主要原因是缺乏合适的数据集和分析技术。

Method: 创建了包含语音内容、韵律、注视、身体动作和视觉上下文的多模态教学视频数据集，并比较了多模态大语言模型与传统任务特定模型的性能。

Result: 多模态大语言模型在无需任务特定微调的情况下就优于专门的基线模型。

Conclusion: 多模态大语言模型在理解面对面教学互动方面具有潜力，能够实现更全面的教学场景理解。
...
TL;DR: DAVIS是一个基于扩散模型的音视频分离框架，通过生成式学习解决音视频声源分离任务，超越了传统的掩码回归方法，在分离质量上表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将声音分离视为基于掩码的回归问题，但在捕捉复杂数据分布以高质量分离多样化声音类别方面存在局限。
...
Conclusion: 生成式框架在解决音视频声源分离任务方面具有显著优势，能够产生高质量的声音分离效果。
...
TL;DR: 提出SpecXNet双域架构用于深度伪造检测，结合空间和频域特征，通过Dual-Domain Feature Coupler和Dual Fourier Attention模块实现鲁棒的跨数据集检测。


<details>
  <summary>Details</summary>
Motivation: GAN和扩散模型生成的内容越来越逼真，现有方法仅关注空间或频域特征，限制了其对未见操作的泛化能力。

Method: 基于改进的XceptionNet骨干网络，嵌入DDFC模块分解特征为局部空间分支和全局频谱分支，使用DFA模块动态融合空间和频谱特征。

Result: 在多个深度伪造基准测试中达到最先进准确率，特别是在跨数据集和未见操作场景下，同时保持实时可行性。
...
TL;DR: 提出了大型材料高斯模型（MGM），该模型能够生成具有PBR材质属性的高质量3D内容，包括反照率、粗糙度和金属度，而不仅仅是产生带有不可控光照烘焙的RGB纹理。


<details>
  <summary>Details</summary>
Motivation: 现有模型无法生成资产的材质属性，这在多样化光照环境中对实现真实感渲染至关重要。

Method: 首先微调一个新的多视图材料扩散模型，该模型以输入深度和法线图为条件。利用生成的多视图PBR图像，探索一种高斯材料表示，该表示不仅与2D高斯泼溅对齐，还对PBR材料的每个通道进行建模。

Result: 广泛的实验表明，该方法产生的材料不仅比基线方法更具视觉吸引力，而且增强了材料建模，从而实现了实用的下游渲染应用。

Conclusion: MGM框架能够生成具有PBR材质的高质量3D内容，支持动态重光照，为实际渲染应用提供了有效解决方案。
...
TL;DR: 提出了一种基于多视图增强的自监督点云补全方法，通过引入Mamba架构提升补全质量，在合成和真实数据集上达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：监督方法依赖真值且泛化性差；无监督方法需要完整点云；弱监督方法需要多视角观测；现有自监督方法因信号有限导致预测效果不佳。

Method: 基于单点云的多视图增强设计自监督信号，首次将Mamba架构引入自监督点云补全任务以增强模型学习能力。

Result: 在合成和真实数据集上的实验表明，该方法达到了最先进的性能。

Conclusion: 所提出的自监督点云补全方法通过多视图增强和Mamba架构，有效克服了现有方法的局限性，实现了优异的补全效果。
...
TL;DR: Refine-Control是一个半监督蒸馏框架，通过三层知识融合损失和利用标记/未标记数据，在降低计算成本和延迟的同时保持高质量图像生成能力。
...
Method: 提出半监督蒸馏框架，引入三层知识融合损失来传递不同层次的知识，利用标记和未标记数据进行训练以增强泛化能力。

Result: 实验表明Refine-Control显著降低了计算成本和延迟，同时保持了高保真生成能力和可控性，通过比较指标量化验证。

Conclusion: 该框架成功解决了边缘设备部署条件图像生成模型的挑战，在资源效率和生成质量之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D点云分类假设数据独立同分布，这破坏了类别间的相关性。需要一种能处理非独立同分布数据并捕获类别相关性的分类策略。
...
TL;DR: MultiMat是一个多模态程序合成框架，利用大型多模态模型处理视觉和文本图形表示，用于改进程序化材质图的生成。


<details>
  <summary>Details</summary>
Motivation: 现有的神经程序合成方法仅将图形表示为文本程序，未能捕捉节点图固有的视觉空间特性，这使得创建材质节点图具有挑战性且需要专业训练。

Method: 使用大型多模态模型处理视觉和文本图形表示，结合约束树搜索推理算法确保语法有效性，同时在程序空间中高效导航。

Result: 多模态程序合成方法在无条件和条件图形合成中比纯文本基线更高效，具有更高的视觉质量和保真度，建立了新的最先进性能。

Conclusion: MultiMat通过整合视觉和文本信息，显著改进了程序化材质图的生成质量，证明了多模态方法在程序合成中的优势。
...
Method: 结合StyleGAN的结构化潜在空间、DragGAN的直观图像操作和PCA的降维技术，在AFHQ数据集上进行实验，特别在W+层引入PCA。

Result: PCA能显著减少总优化时间，同时保持良好视觉质量，在较浅潜在空间（W+层=3）中甚至提升SSIM指标，并成功对齐不同数据域的StyleGAN模型。
...
TL;DR: MinerU2.5是一个1.2B参数的文档解析视觉语言模型，采用从粗到精的两阶段解析策略，在保持计算效率的同时实现了最先进的识别精度。


<details>
  <summary>Details</summary>
Motivation: 解决文档解析中高分辨率输入带来的计算开销问题，同时保持对密集文本、复杂公式和表格等细粒度细节的识别能力。

Method: 使用两阶段解析策略：第一阶段对下采样图像进行高效布局分析以识别结构元素；第二阶段基于全局布局指导，对原始图像的原生分辨率裁剪区域进行针对性内容识别。开发了全面的数据引擎生成大规模训练语料。
...
TL;DR: 提出了Geo-CoT框架，通过两阶段对齐策略训练RSThinker模型，在遥感分析中实现可验证的多步推理过程，显著优于现有方法。
...
Method: 使用Geo-CoT380k数据集，采用两阶段对齐策略：先通过监督微调建立认知架构，再使用组奖励策略优化来优化推理策略。

Result: RSThinker模型在广泛任务中显著优于最先进模型，能同时输出最终答案和可验证的分析轨迹。

Conclusion: 该工作为地球观测从模糊感知向结构化、可验证推理提供了具体路径，发布了Geo-CoT380k数据集和RSThinker模型。
...
TL;DR: MUSplat是一个无需训练的开源框架，将2D开放词汇理解提升到3D高斯泼溅场景，通过多粒度掩码提升、语义熵优化和视觉语言模型特征提取，实现高效的3D语义分割和对象选择。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖昂贵的逐场景重新训练、单语义设计无法表示复杂多概念语义、以及跨视图语义不一致性破坏最终语义表示等三个关键缺陷。

Method: 利用预训练的2D分割模型生成多粒度2D掩码并提升到3D空间，通过语义熵和几何不透明度优化初始对象组边界，使用视觉语言模型从最具代表性视角提取鲁棒文本特征。

Result: 在开放词汇3D对象选择和语义分割基准任务上，MUSplat优于已建立的基于训练的框架，同时解决了它们的单语义限制，将场景适应时间从数小时减少到几分钟。

Conclusion: MUSplat通过消除昂贵的逐场景训练过程，提供了一种高效、训练免费的解决方案，能够处理复杂的多概念语义表示，并在开放词汇3D理解任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 城市发展影响全球一半以上人口，需要从人类视角理解其结构和感知变化。现有基准在探索MLLMs在城市环境中的性能方面有限，缺乏对时序演化和主观感知的系统探索。

Method: 收集来自11个代表性城市的多时序单视图和全景街景图像，通过空间聚类、规则生成、模型辅助提示和人工标注的混合流程生成高质量问答对。

Result: 评估20个最先进的MLLMs发现，Gemini-2.5 Pro表现最佳，准确率接近人类专家水平，平均差距仅1.5%。模型在场景理解任务上表现良好，部分模型在像素级变化检测中甚至超越人类标注者，但在需要时序推理的任务中表现下降。

Conclusion: UrbanFeel基准揭示了MLLMs在城市发展理解方面的能力与局限，为可持续城市发展提供了重要评估工具。
...
TL;DR: 提出了EXCL方法，通过双专家框架和检索增强交互优化管道，在无需源数据的情况下实现无监督域自适应，达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决源自由无监督域自适应(SFUDA)的现实挑战，现有方法要么只利用源模型预测，要么微调大型多模态模型，但都忽略了互补见解和目标数据的潜在结构。

Method: EXCL包含双专家框架和检索增强交互优化管道。双专家框架将冻结的源域模型（带Conv-Adapter）与预训练视觉语言模型（带可训练文本提示）平等放置，从未标记目标样本中挖掘共识知识。RAIN管道包括：(1)协作检索伪源和复杂目标样本，(2)分别微调各专家，(3)通过共享学习结果强制学习对象一致性。

Result: 在四个基准数据集上的广泛实验表明，该方法达到了最先进的性能。

Conclusion: EXCL方法通过双专家协作学习和检索增强交互优化，有效解决了SFUDA问题，在无需源数据的情况下实现了优异的域自适应性能。
...
TL;DR: FlashEdit是一个实时图像编辑框架，通过三步创新实现高质量编辑：一步式反转编辑流程、背景保护技术和稀疏空间交叉注意力机制，速度提升150倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的文本引导图像编辑方法虽然质量优秀，但计算延迟过高，阻碍了实际应用，需要开发实时高效的编辑框架。

Method: 1. 一步式反转编辑(OSIE)管道避免昂贵的迭代过程；2. 背景保护(BG-Shield)技术选择性修改编辑区域特征；3. 稀疏空间交叉注意力(SSCA)机制抑制语义泄漏。

Result: FlashEdit在保持优秀背景一致性和结构完整性的同时，编辑时间低于0.2秒，相比之前的多步方法速度提升超过150倍。

Conclusion: FlashEdit实现了高质量、实时的图像编辑，为实际应用提供了可行的解决方案，代码将开源。
...
TL;DR: 提出了Neural-MedBench基准测试，专门用于评估神经学领域的多模态临床推理能力，发现现有视觉语言模型在推理任务上表现显著下降。
...
Method: 构建包含多序列MRI扫描、结构化电子健康记录和临床笔记的紧凑基准，涵盖鉴别诊断、病灶识别和原理生成三个核心任务家族，并开发结合LLM评分、临床医生验证和语义相似度度量的混合评分流程。

Result: 对GPT-4o、Claude-4和MedGemma等先进VLM的系统评估显示，与传统数据集相比性能显著下降，错误分析表明推理失败而非感知错误是模型的主要短板。
...
TL;DR: UniMapGen是一个用于大规模地图构建的生成式框架，通过将车道线表示为离散序列并使用迭代策略生成完整平滑的地图矢量，支持多模态输入以克服卫星数据缺陷，并采用状态更新策略保证全局连续性。


<details>
  <summary>Details</summary>
Motivation: 传统大规模地图构建方法依赖昂贵的专用数据采集车辆和人工标注，效率低下。现有卫星方法存在遮挡、数据过时等问题，且感知方法生成的矢量道路不连续粗糙，需要大量后处理。

Method: 1) 将车道线表示为离散序列，采用迭代策略生成更完整平滑的地图矢量；2) 支持BEV、PV和文本提示等多模态输入的灵活架构；3) 开发状态更新策略保证大规模地图的全局连续性和一致性。

Result: 在OpenSatMap数据集上达到最先进性能，能够推断被遮挡的道路并预测数据集中缺失的道路标注。

Conclusion: UniMapGen通过生成式方法有效解决了传统地图构建方法的效率问题和卫星数据的固有缺陷，为大规模地图构建提供了创新解决方案。
...
TL;DR: GS-2M：基于3D高斯泼溅的统一框架，从多视角图像联合优化网格重建和材质分解，无需外部先验，能处理高反射表面。


<details>
  <summary>Details</summary>
Motivation: 现有方法分别处理网格重建和材质分解，难以重建高反射表面，且依赖外部模型先验来增强分解结果。

Method: 基于3D高斯泼溅联合优化渲染深度和法线质量相关属性；提出基于多视角光度变化的新型粗糙度监督策略；设计精心设计的损失函数和优化过程。

Result: 重建结果与最先进方法相当，生成三角形网格及其相关材质组件，适用于下游任务。

Conclusion: GS-2M框架有效解决了网格重建和材质分解的联合优化问题，无需复杂神经组件，在广泛使用的数据集上验证了有效性。
...
TL;DR: 本文提出任务导向的桌面场景生成任务，构建了包含10,700个合成桌面场景的大规模数据集MesaTask-10K，并开发了基于LLM的MesaTask框架，通过空间推理链和DPO算法生成符合任务描述的物理合理桌面场景。


<details>
  <summary>Details</summary>
Motivation: 传统方法创建机器人训练所需的桌面场景依赖耗时的手动布局设计或纯随机布局，存在合理性不足或与任务对齐度低的问题。需要解决高级任务指令与桌面场景之间的巨大差距。
...
Conclusion: MesaTask框架能够有效解决任务导向的桌面场景生成挑战，生成物理合理且与任务描述良好对齐的桌面场景。
...
<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在文档分析领域应用不广泛，但许多下游任务可能受益于强化学习的新兴特性，特别是增强的推理能力。DeepSeek-R1通过简单可验证奖励的成功证明了基于规则强化学习的潜力。

Method: 使用基于规则的强化学习方法，在文档图像分类任务上进行实验，考察三种不同的分布外场景：分布外图像、未见过的类别和不同模态。

Result: 研究发现强化学习在分布外数据上表现出更好的泛化能力，在三种测试场景中均显示出优势。

Conclusion: 基于规则的强化学习在文档图像分类任务中具有显著的泛化优势，特别是在处理分布外数据时表现优异。
...
TL;DR: SceneSplit是一种针对文本到视频模型的黑盒越狱攻击方法，通过将有害叙述分解为多个看似良性的场景来绕过安全机制，显著提高有害视频生成的成功率。


<details>
  <summary>Details</summary>
Motivation: 随着文本到视频模型的快速发展，其安全风险日益凸显。虽然已有研究探索了LLMs、VLMs和文本到图像模型的漏洞，但文本到视频模型的安全性研究仍存在空白。

Method: 将有害叙述分解为多个单独良性的场景，利用场景组合约束生成空间，通过迭代场景操作绕过安全过滤器，并利用策略库重用成功攻击模式。

Result: 在11个安全类别上测试，SceneSplit在Luma Ray2上达到77.2%的平均攻击成功率，在Hailuo上达到84.1%，在Veo2上达到78.2%，显著优于现有基线方法。

Conclusion: 当前文本到视频模型的安全机制容易受到利用叙事结构的攻击，这为理解和改进T2V模型安全性提供了新的见解。
...
TL;DR: 提出了一种基于动量的采样技术HiGS，通过整合近期模型预测来提升扩散模型采样质量和效率，无需额外训练即可实现更高质量的图像生成


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面取得显著进展，但在使用较少NFEs或较低引导尺度时，输出仍可能显得不真实且缺乏细节

Method: HiGS利用当前预测与过去预测加权平均之间的差异来引导采样过程，无需额外计算成本，可无缝集成到现有扩散框架中

Result: HiGS在不同模型和架构下均能提升图像质量，使用预训练SiT模型在ImageNet 256×256无引导生成中达到1.61 FID的新SOTA，仅需30步采样
...
Method: 采用可部署和可扩展的双流CNN-Transformer架构，包含配对窗口注意力(PWA)和Johnson-Lindenstrauss引理指导的卷积(JLC)，结合"一瞥-聚焦"原则，并通过空间解耦知识转移(SDKT)注入自监督网络的纹理先验。

Result: 在多模态基准测试中，VeloxSeg实现了26%的Dice系数提升，GPU吞吐量提高11倍，CPU吞吐量提高48倍。

Conclusion: VeloxSeg成功解决了轻量级3D医学图像分割的效率与鲁棒性平衡问题，在保持高效计算的同时显著提升了分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于样例的纹理合成问题，结合扩散模型和传统块匹配技术的优势，避免神经网络训练需求，同时改进块匹配方法的常见缺陷。

Method: 构建基于非局部块匹配的非参数流匹配模型，结合卷积神经网络训练的扩散模型和传统块匹配优化技术。

Result: 实验结果表明，与文献中的代表性方法相比，所提出的方法具有更好的效果。

Conclusion: NIFTY框架有效结合了现代扩散模型和传统块匹配技术，在纹理合成任务中表现出色，无需神经网络训练即可解决块匹配方法的常见问题。
...
TL;DR: RAPID3是一个无需训练基础生成器的三层次强化学习加速框架，通过步骤跳过、缓存重用和稀疏注意力策略，在保持生成质量的同时实现DiT模型近3倍的采样加速。


<details>
  <summary>Details</summary>
Motivation: 现有DiT加速方法要么使用统一启发式策略导致质量损失，要么需要高昂的微调成本，需要一种既能实现图像级自适应加速又无需更新基础生成器的解决方案。

Method: 使用三个轻量级策略头（步骤跳过、缓存重用、稀疏注意力）基于当前去噪状态独立决策，通过组相对策略优化在线训练策略参数，同时使用对抗性判别器增强奖励信号防止奖励攻击。

Result: 在包括Stable Diffusion 3和FLUX在内的最先进DiT骨干网络上，RAPID3实现了近3倍的采样加速，同时保持竞争力的生成质量。

Conclusion: RAPID3框架成功实现了无需更新基础生成器的图像级自适应加速，在保持生成质量的同时显著提升了DiT模型的采样效率。
...
Method: 提出了多模态知识图谱构建方法，充分考虑属性间关系以及属性与视觉标记之间的关系，并引入了知识图谱引导的跨模态超图学习框架来增强标准行人属性识别框架。

Result: 在多个PAR基准数据集上的综合实验充分证明了所提出的知识图谱对PAR任务的有效性。

Conclusion: 为知识引导的行人属性识别建立了坚实基础，提出的方法能有效提升行人属性识别的准确性。
...
TL;DR: CircuitSense是一个评估多模态大语言模型在电路图理解方面能力的综合基准，包含8000多个问题，涵盖从组件级原理图到系统级框图的完整工程工作流程。


<details>
  <summary>Details</summary>
Motivation: 探索多模态大语言模型从技术图表中提取数学模型的能力，填补现有模型在视觉到数学推理方面的研究空白。

Method: 开发了层次化合成生成管道，包括基于网格的原理图生成器和带有自动推导符号方程标签的框图生成器，构建了包含感知、分析和设计三个任务的基准。

Result: 闭源模型在感知任务（组件识别和拓扑识别）上准确率超过85%，但在符号推导和分析推理任务上表现低于19%，揭示了视觉解析与符号推理之间的关键差距。

Conclusion: 具有更强符号推理能力的模型在设计任务上表现更好，确认了数学理解在电路合成中的基础作用，并确立符号推理作为工程能力的关键指标。
...
TL;DR: 提出HierLight-YOLO模型，通过分层特征融合和轻量化设计，在无人机图像中实现小目标实时检测，在VisDrone2019基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决无人机摄影中实时检测小目标（<32像素）的双重挑战：小目标检测精度低和资源受限平台上的实时效率需求。现有YOLO系列检测器在无人机场景中误报率较高。

Method: 基于YOLOv8架构，提出分层扩展路径聚合网络（HEPAN）进行多尺度特征融合，引入倒置残差深度卷积块（IRDCB）和轻量化下采样模块（LDown）减少参数和计算复杂度，设计小目标检测头提升空间分辨率和特征融合能力。

Result: 在VisDrone2019基准测试中表现出最先进的性能，显著提升小目标检测精度，同时保持实时效率。

Conclusion: HierLight-YOLO通过分层特征融合和轻量化设计，有效解决了无人机场景中小目标实时检测的挑战，在精度和效率之间取得了良好平衡。
...
TL;DR: 本研究探讨了GPT-4o模型在多模态虚假信息检测中的应用，提出了优化的提示工程方法和结构化分析框架，并在多个数据集上评估了模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着文本和图像结合的多模态虚假信息在数字平台上的扩散，需要开发有效的检测方法。本研究旨在利用大型多模态模型的先进能力来检测和缓解虚假信息。

Method: 使用GPT-4o模型，开发优化的提示工程技术和结构化多模态分析框架，包括图像和文本的预处理方法，定义六个评估标准，并引入置信度自评估机制。

Result: 在Gossipcop、Politifact、Fakeddit、MMFakeBench和AMMEBA等多个异构数据集上进行了全面的性能分析，揭示了GPT-4o在虚假信息检测中的优势和局限性。

Conclusion: 研究提供了一个稳健且可复现的自动化多模态虚假信息分析方法论框架，包括基于置信度和变异性的评估方法，为未来研究奠定了基础。
...
TL;DR: 利用预训练的GPT-4模型通过特定提示分析图像，预测物体间的遮挡顺序关系，构建遮挡矩阵以辅助其他遮挡处理任务。
...
Method: 通过设计特定提示词，让GPT-4分析输入图像并生成顺序预测，然后解析响应构建遮挡矩阵。

Result: 在COCOA和InstaOrder数据集上的评估显示，模型能利用语义上下文、视觉模式和常识知识产生更准确的顺序预测。

Conclusion: 该方法能以零样本方式推理遮挡关系，无需标注训练数据，易于集成到遮挡处理框架中。
...
TL;DR: 提出了一种基于显著边界增强的多焦点图像融合方法，通过梯度域模型和Tenengrad梯度检测生成高质量的融合边界，在四个公共数据集上优于12种最先进方法。
...
Method: 使用梯度域模型获得具有完整边界的初始融合结果，引入Tenengrad梯度检测提取显著特征，并开发基于梯度和互补信息的焦点度量进行边界细化。

Result: 在四个公共数据集上的广泛实验表明，该方法在主观和客观评估中均优于12种最先进方法。

Conclusion: 该方法能够有效生成高质量的融合边界，显著提升多焦点图像融合的性能。
...
TL;DR: 该论文探索使用2D高斯泼溅(2DGS)作为视觉语言模型的替代视觉表示，以解决RGB图像传输效率低和序列长度爆炸的问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统RGB视觉编码器的两个结构性问题：(i)从边缘设备传输密集RGB图像能耗高、成本大；(ii)基于补丁的标记化导致序列长度爆炸，增加注意力计算负担。

Method: 开发可扩展的2DGS流水线，包括结构化初始化、亮度感知剪枝和批处理CUDA内核；通过轻量级泼溅感知输入主干和感知重采样器，将对比语言图像预训练(CLIP)适配到2DGS表示。

Result: 2DGS编码器在DataComp子集上实现有意义的零样本ImageNet-1K性能，输入压缩比达到3-20倍，但准确率目前仍低于RGB编码器。

Conclusion: 2DGS被确立为可行的多模态表示基板，指明了架构瓶颈，为开发既语义强大又传输高效的边缘-云端学习表示开辟了路径。
...
TL;DR: 提出了TDOA方法，通过聚类将动态输出场景转换为静态单输出场景，使用最远标签目标攻击策略，在动态和静态输出场景中都取得了优异的攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有文本对抗攻击方法主要针对静态场景，无法处理动态输出场景（标签数量和空间不固定），需要解决这一局限性。
...
Conclusion: TDOA方法能有效攻击动态输出场景，在有限访问条件下具有破坏大语言模型的强大潜力，同时在传统静态场景中也达到最先进性能。
...
TL;DR: Labeling Copilot是首个用于计算机视觉的数据策展深度研究代理，通过多模态语言模型驱动的中央编排器，结合校准发现、可控合成和共识标注三大核心能力，有效解决高质量领域特定数据集构建的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 构建高质量、领域特定的数据集是部署稳健视觉系统的主要瓶颈，需要在数据质量、多样性和成本之间进行复杂权衡。现有方法在处理大规模未标注数据湖时面临效率和质量挑战。

Method: 采用多模态语言模型驱动的中央编排器进行多步推理，执行三个核心工具：校准发现从大型存储库中获取相关数据；可控合成生成稀有场景的新数据；共识标注通过结合非极大值抑制和投票的新颖共识机制协调多个基础模型产生准确标签。

Result: 在COCO数据集上，共识标注模块平均每张图像产生14.2个候选提议（接近真实对象7.4个的两倍），最终标注mAP达到37.1%。在Open Images数据集上发现了903个新的边界框类别，总能力扩展到1500多个类别。校准发现在1000万样本规模测试中，主动学习策略比替代方案计算效率高40倍。

Conclusion: 具有优化、可扩展工具的代理工作流程为构建工业规模数据集提供了稳健基础，证明了Labeling Copilot各组件在数据策展任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习医学图像分割系统尚未完善，性能有待提升。作者认为通过将医学背景知识融入分割模型的损失函数可以改善性能。
...
### [132] [Learning Human-Perceived Fakeness in AI-Generated Videos via Multimodal LLMs](https://arxiv.org/abs/2509.22646)
...
TL;DR: DeeptraceReward是首个细粒度、时空感知的基准数据集，用于标注人类感知的AI生成视频伪影痕迹，包含4.3K详细标注，训练的多模态奖励模型在识别、定位和解释方面优于GPT-5 34.7%。


<details>
  <summary>Details</summary>
Motivation: 虽然视频生成模型快速发展，但人类是否能检测深度伪造痕迹这一关键维度被忽视，需要评估人类感知的时空视觉伪影。

Method: 构建包含4.3K标注的DeeptraceReward数据集，标注包括自然语言解释、边界框区域和精确时间戳，将伪影分为9大类，训练多模态语言模型作为奖励模型。

Result: 7B奖励模型在伪影识别、定位和解释方面平均优于GPT-5 34.7%，发现难度梯度：二元分类最容易，细粒度检测更难，其中时间标注最难。

Conclusion: 通过关注人类感知的深度伪造痕迹，DeeptraceReward为社会意识和可信赖的视频生成提供了严格的测试平台和训练信号。
...
### [133] [Closing the Safety Gap: Surgical Concept Erasure in Visual Autoregressive Models](https://arxiv.org/abs/2509.22400)
*Xinhao Zhong,Yimin Zhou,Zhiqi Zhang,Junhao Li,Yi Sun,Bin Chen,Shu-Tao Xia,Ke Xu*

Main category: cs.CV

TL;DR: 提出了VARE框架和S-VARE方法，专门针对视觉自回归模型的稳定概念擦除，通过辅助视觉令牌和过滤交叉熵损失实现精确的概念移除，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型的快速发展带来了新的文本到图像生成机会，但也加剧了安全担忧。现有的概念擦除技术主要针对扩散模型设计，无法适用于VAR模型的下一个尺度令牌预测范式。

Method: 首先提出VARE框架，利用辅助视觉令牌减少微调强度；然后引入S-VARE方法，结合过滤交叉熵损失精确识别和最小调整不安全视觉令牌，以及保持损失来维持语义保真度。

Result: 大量实验证明该方法实现了精确的概念擦除，同时保持了生成质量，解决了朴素微调带来的语言漂移和多样性减少问题。
...
### [134] [RAU: Reference-based Anatomical Understanding with Vision Language Models](https://arxiv.org/abs/2509.22404)
*Yiwei Li,Yikang Liu,Jiaqi Guo,Lin Zhao,Zheyuan Zhang,Xiao Chen,Boris Mailhe,Ankush Mukherjee,Terrence Chen,Shanhui Sun*

Main category: cs.CV

TL;DR: RAU框架利用视觉语言模型进行基于参考图像的解剖结构理解，通过相对空间推理识别解剖区域，并结合SAM2实现精细分割，在多个数据集上表现优于基准方法。


<details>
  <summary>Details</summary>
Motivation: 医学图像中解剖理解的自动化受限于专家标注数据稀缺，需要开发能够利用参考图像指导未标注目标图像理解的方法。

Method: 首先训练VLM通过参考图像和目标图像之间的相对空间推理识别解剖区域，然后将VLM生成的空间线索与SAM2的精细分割能力结合，实现小解剖区域的定位和像素级分割。

Result: 在两个分布内和两个分布外数据集上，RAU始终优于SAM2微调基准，产生更准确的分割和更可靠的定位，展现出强大的泛化能力。
...
### [135] [CapRL: Stimulating Dense Image Caption Capabilities via Reinforcement Learning](https://arxiv.org/abs/2509.22647)
*Long Xing,Xiaoyi Dong,Yuhang Zang,Yuhang Cao,Jianze Liang,Qidong Huang,Jiaqi Wang,Feng Wu,Dahua Lin*

Main category: cs.CV

TL;DR: 提出CapRL框架，使用强化学习替代监督微调进行图像描述生成，通过非视觉语言模型基于描述回答问题的准确性作为奖励函数，解决了图像描述任务中主观质量评估的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的图像描述模型依赖监督微调，需要昂贵的人工标注数据，导致模型记忆特定答案而缺乏泛化能力和多样性。需要克服SFT的局限性，为大规模视觉语言模型预训练提供更好的图像描述方法。

Method: CapRL采用解耦的两阶段流程：LVLM生成图像描述，然后基于该描述让无视觉的LLM回答多项选择题，使用回答准确率作为强化学习的客观奖励函数。

Result: 在CapRL-5M数据集上预训练后，在12个基准测试中取得显著提升。在Prism框架下，性能与Qwen2.5-VL-72B相当，平均超出基线8.4%。

Conclusion: CapRL成功将RLVR范式应用于主观的图像描述任务，通过基于效用的质量定义和客观奖励函数，显著提升了图像描述模型的性能。
...
TL;DR: 提出了MSEA和ARC方法来增强多模态大语言模型的解释性，通过多尺度解释聚合和激活排名相关性来改善视觉和文本模态的解释质量。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性研究主要关注跨模态归因，但忽视了模态内依赖关系，导致视觉解释碎片化、文本解释存在虚假激活，影响了归因的保真度。

Method: 视觉分支采用多尺度解释聚合(MSEA)动态调整感受野；文本分支使用激活排名相关性(ARC)通过预测排名对齐来抑制无关上下文的虚假激活。

Result: 在多个最先进MLLMs和基准数据集上的实验表明，该方法一致优于现有可解释性方法，产生更忠实和细粒度的模型行为解释。

Conclusion: 通过利用模态内交互可以显著提升多模态大语言模型的解释性，为理解模型决策机制提供了更有效的工具。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测器因从有限训练数据中学习到偏差而难以泛化到新型伪造类型，本文识别出频率域中的频谱偏差问题。

Method: 提出FreqDebias框架：1) Fo-Mixup增强动态多样化训练样本的频率特征；2) 双重一致性正则化（局部CAM一致性和全局vMF分布一致性）。

Result: 大量实验表明FreqDebias显著提升跨域泛化能力，在跨域和域内设置下均优于最先进方法。

Conclusion: FreqDebias通过缓解频谱偏差有效提升了深度伪造检测器的泛化性能，为频率域偏差问题提供了解决方案。
...
TL;DR: LucidFlux是一个无需图像描述的通用图像修复框架，通过双分支调节器和自适应调制策略，在保持语义的同时有效修复混合退化图像。
...
Method: 使用轻量级双分支调节器注入退化输入和轻度修复代理的信号，设计时间步和层自适应调制策略，并通过SigLIP特征实现无描述语义对齐。

Result: 在合成和真实场景基准测试中，LucidFlux持续优于强大的开源和商业基线方法，消融研究验证了各组件的必要性。

Conclusion: 对于大型扩散变换器，何时、何地以及如何调节比增加参数或依赖文本提示更重要，这是实现鲁棒且无需描述的通用图像修复的关键。
...
TL;DR: 提出U-MAN架构，通过多尺度自适应KAN和渐进注意力引导特征融合模块，解决医学图像分割中细节丢失和边界不精确的问题，在多个数据集上优于现有方法。
...
Conclusion: U-MAN通过结合多尺度自适应KAN和注意力引导特征融合，有效解决了医学图像分割中的关键挑战，在边界精度和细节保留方面取得了显著改进。
...
TL;DR: 该论文提出了一种任务特定的非线性量化方法γ-Quant，用于在低带宽和能量受限环境下优化模式识别性能，在仅使用4位量化数据的情况下达到与12位原始数据相当的效果。


<details>
  <summary>Details</summary>
Motivation: 当前大多数模式识别模型使用预处理数据，但这些预处理（如图像信号处理、高比特ADC转换）主要针对人类感知设计，在自动化分析场景中可能不是最优的，且会导致数据传输效率低下和能耗问题。

Method: 提出γ-Quant方法，通过学习任务特定的非线性量化函数，在低比特深度捕获条件下优化模式识别性能。该方法在原始图像目标检测和可穿戴设备的人体活动识别任务中进行验证。

Result: 实验表明，使用可学习量化的4位原始数据可以达到与12位原始数据相当的性能水平，显著提高了数据效率和能耗表现。

Conclusion: 任务特定的非线性量化方法能够在保持性能的同时大幅降低数据比特深度，为低带宽和能量受限环境下的模式识别应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决应用导向的VIF方法需要下游任务标注数据的问题，传统方法仅关注融合图像质量，而应用导向方法需要大量标注数据，数据获取成本高。

Method: 利用特征级融合分割和像素级融合分割之间的一致性，提出跨分割一致性自监督任务，设计两阶段训练策略和动态权重调整方法。

Result: 在公开数据集上的实验表明，SSVIF仅使用未标注的可见光-红外图像对进行训练，就能超越传统VIF方法，并与有监督的分割导向方法相媲美。

Conclusion: 提出的自监督框架SSVIF有效解决了应用导向VIF方法对标注数据的依赖问题，在无监督情况下实现了与有监督方法相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学影像中不同模态间大域差距问题，现有GAN方法在高度可变区域难以捕捉跨域映射

Method: 1. Bézier曲线风格迁移减少域差距；2. 用分割模型生成伪标签；3. 条件扩散模型合成高质量标记图像；4. 不确定性引导的分数匹配提高鲁棒性

Result: 在公开数据集上生成逼真标记图像，显著增强目标域并提升分割性能

Conclusion: 该框架能有效生成高质量跨域医学图像，改善域自适应分割效果
...
TL;DR: 提出PSTTS模块，通过渐进式时空令牌选择来减少事件数据中的冗余计算，在保持精度的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于事件帧的方法忽视了事件帧序列的高空间稀疏性和帧间运动冗余，导致计算开销大。现有RGB视频的令牌稀疏化方法不适用于事件数据。

Method: PSTTS包含两个阶段：空间令牌净化（基于时空一致性去除噪声和非事件区域）和时间令牌选择（评估相邻帧运动模式相似性去除冗余信息）。

Result: 在多个数据集和骨干网络上，PSTTS将FLOPs减少29-43.6%，FPS提升21.6-41.3%，同时保持任务精度。

Conclusion: PSTTS是无需额外参数的即插即用模块，能有效识别和丢弃时空冗余令牌，实现精度与效率的最佳平衡。
...
TL;DR: 提出了GCPO方法，通过识别关键图像token并进行针对性优化，在仅使用30% token的情况下超越了全token优化的GRPO方法，提升了AR视觉生成的效果。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法对所有图像token进行统一优化，但不同token对训练贡献不同，需要识别关键token并进行有效优化。

Method: 从三个角度识别关键token：因果依赖、熵诱导空间结构、RLVR焦点token多样性，并引入动态token-wise优势权重来鼓励探索。

Result: 在多个文本到图像基准测试中，GCPO在AR模型和统一多模态模型上都表现出色，仅用30% token就超越了全token优化的性能。

Conclusion: GCPO通过有效识别和优化关键token，为AR视觉生成提供了更高效的训练策略，显著提升了生成质量和效率。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉输入与自然语言输出的对齐方面表现出色，但生成的token对视觉模态的依赖程度尚不清楚，这限制了模型的可解释性和可靠性。

Method: EAGLE引入了一个统一充分性（洞察分数）和必要性（必要分数）的目标函数，通过贪心搜索在稀疏化的图像区域上进行优化，实现忠实且高效的归因。此外，还进行模态感知分析，解耦token的依赖关系。
...
Method: 使用957个颜色样本，复制经典颜色命名方法，评估五个代表性模型，并进行跨语言分析和消融研究。

Result: 模型在典型颜色上准确率高，但在扩展的非典型颜色集上表现显著下降；发现21个在所有模型中一致的常见颜色术语；语言模型架构独立于视觉处理能力显著影响颜色命名。
...
TL;DR: 提出EfficientDepth系统，结合Transformer架构与轻量卷积解码器，通过双模态密度头和LPIPS损失函数实现高效的单目深度估计，在保持几何一致性和细节的同时大幅降低计算资源需求。
...
Method: 结合Transformer架构与轻量卷积解码器，使用双模态密度头估计详细深度图；采用合成图像、真实图像和伪标签图像的多源训练数据；实施多阶段优化策略，并引入基于LPIPS的损失函数来增强细节保留。

Result: EfficientDepth在性能上达到或优于现有最先进模型，同时显著减少了计算资源需求。

Conclusion: EfficientDepth系统成功解决了单目深度估计中的关键挑战，在保持高性能的同时实现了计算效率的显著提升，为实际应用提供了可行的解决方案。
...
TL;DR: 这是一篇关于类别发现（Category Discovery）的综述论文，系统回顾了开放世界学习任务中自动分类未标记数据的方法，重点关注新类别发现和广义类别发现两种基础设置。


<details>
  <summary>Details</summary>
Motivation: 类别发现作为新兴的开放世界学习任务，近年来受到广泛关注，但缺乏系统性的文献综述。作者旨在提供全面的方法分析和深入讨论，帮助研究人员理解该领域的发展现状。

Method: 采用文献综述方法，首先建立包含NCD和GCD两种基础设置及多种衍生设置（如持续类别发现、偏斜数据分布、联邦类别发现等）的分类体系，然后对每种设置下的方法进行详细分析，涵盖表示学习、标签分配和类别数量估计三个核心组件。

Result: 通过基准测试提炼出关键见解：大规模预训练骨干网络、层次化和辅助线索、课程式训练都有助于类别发现，但在标签分配设计、类别数量估计和复杂多目标场景扩展方面仍存在挑战。

Conclusion: 类别发现领域已取得显著进展，但仍需在标签分配策略、类别数量估计方法和复杂场景适应性等方面进行深入研究。作者提供了持续更新的文献综述资源。
...
### [149] [Hierarchical Representation Matching for CLIP-based Class-Incremental Learning](https://arxiv.org/abs/2509.22645)
*Zhen-Hao Wen,Yan Wang,Ji Feng,Han-Jia Ye,De-Chuan Zhan,Da-Wei Zhou*

Main category: cs.CV

TL;DR: 提出HERMAN方法，利用LLM生成层次化文本描述符，增强CLIP在类增量学习中的语义空间表示，通过自适应路由匹配不同层次语义，缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP类增量学习方法使用简单模板（如"a photo of a [CLASS]"），忽略了视觉概念的层次性，且仅使用最后一层特征，忽视了早期层的层次信息。

Method: 利用大语言模型递归生成区分性文本描述符，增强语义空间的层次线索，将这些描述符匹配到语义层次的不同级别，并根据任务需求自适应路由。
...
Method: 提出HyCoVAD混合模型：使用基于nnFormer的多任务SSL时序分析器识别可疑异常帧，然后将选中的帧送入LLM进行基于规则的结构化推理验证异常

Result: 在ComplexVAD数据集上达到72.5%帧级AUC，比现有基线提升12.5%，同时减少了LLM计算量

Conclusion: HyCoVAD成功结合SSL和LLM的优势，在复杂视频异常检测中表现出色，并发布了交互异常分类法、自适应阈值协议和代码
...
TL;DR: JanusVLN提出了一种新颖的视觉语言导航框架，采用双隐式神经记忆来建模空间几何和视觉语义记忆，解决了传统方法中的空间信息丢失、计算冗余和内存膨胀问题，在多个基准测试中达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统VLN方法依赖显式语义记忆（如文本认知地图或历史视觉帧），存在空间信息丢失、计算冗余和内存膨胀等问题。受人类导航中隐式场景表示的启发，作者提出了双隐式神经记忆框架来克服这些限制。

Method: 提出JanusVLN框架，包含双隐式神经记忆：空间几何记忆和视觉语义记忆。首先扩展MLLM以融入空间几何编码器的3D先验知识，增强仅基于RGB输入的空间推理能力。然后构建空间几何和视觉语义编码器的历史键值缓存作为双隐式记忆，通过仅保留初始和滑动窗口中的token KV来避免冗余计算，实现高效增量更新。

Result: 在广泛实验中，JanusVLN超越了20多个最近的方法，达到SOTA性能。与使用多种数据类型输入的方法相比，成功率提高了10.5-35.5；与使用更多RGB训练数据的方法相比，成功率提高了3.6-10.8。

Conclusion: 提出的双隐式神经记忆作为一种新颖范式，为未来VLN研究探索了有前景的新方向，证明了隐式记忆表示在视觉语言导航任务中的有效性。
...
TL;DR: SpikeMatch是首个用于脉冲神经网络的半监督学习框架，利用SNN的时间动态特性通过协同训练生成多样化伪标签，在有限标签下实现高效学习。
...
Conclusion: SpikeMatch成功证明了利用SNN时间动态特性进行半监督学习的有效性，能够用有限标签捕获判别性特征，缓解确认偏差。
...
TL;DR: LongLive是一个帧级自回归框架，用于实时交互式长视频生成，通过KV重缓存、流式长调优和帧级注意力汇聚等关键技术，在保持高质量的同时实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 长视频生成面临效率和质量的挑战：扩散模型质量高但效率低，因果注意力AR模型支持KV缓存但长视频训练时质量下降。此外，交互式能力对于动态内容创作至关重要，但增加了视觉一致性和语义连贯性的复杂性。

Method: 采用因果帧级自回归设计，集成KV重缓存机制用于平滑提示切换，流式长调优实现长视频训练和训练-推理对齐，短窗口注意力配合帧级注意力汇聚保持长程一致性并加速生成。

Result: 仅用32 GPU天将1.3B参数的短片段模型微调至分钟级生成，在单张H100 GPU上维持20.7 FPS，在VBench上短长视频均表现优异，支持单卡240秒视频生成，INT8量化推理仅轻微质量损失。

Conclusion: LongLive通过创新的框架设计成功解决了长视频生成在效率、质量和交互性方面的核心挑战，为实时交互式长视频生成提供了可行方案。
...
TL;DR: SPARK框架通过协同训练策略和奖励模型，利用RLVR的废弃数据来训练生成式奖励模型，消除了对独立奖励模型和人类偏好数据的需求，实现了高效稳定的强化学习后训练。


<details>
  <summary>Details</summary>
Motivation: 解决RLHF的高成本和奖励-策略不匹配问题，以及RLVR浪费监督信号的问题，提出更高效的强化学习后训练方法。

Method: 基于RLVR框架，回收废弃的rollouts和正确性数据，同时训练模型作为生成式奖励模型，使用点对奖励评分、成对比较和反思条件评估等混合目标。
...
TL;DR: 提出CCNeXt架构，一种自监督深度估计方法，在KITTI等数据集上超越现有CNN和ViT模型，同时保持计算效率


<details>
  <summary>Details</summary>
Motivation: 深度估计在机器人、自动驾驶等应用中很重要，但这些场景通常有计算资源限制。由于难以获取可靠的深度真值数据，自监督方法成为解决方案

Method: CCNeXt架构使用现代CNN特征提取器，结合新颖的窗口化极线交叉注意力模块，并重新设计了深度估计解码器

Result: 在KITTI Eigen Split测试数据上达到竞争性指标，比当前最佳模型快10.18倍，在KITTI Eigen Split改进真值和Driving Stereo数据集上所有指标都达到最先进水平

Conclusion: CCNeXt在自监督深度估计中实现了性能与计算效率的良好平衡，优于现有的CNN和ViT方法
...
TL;DR: UML-CoT是一个基于统一建模语言的结构化推理框架，通过UML类图和活动图生成符号推理链和可执行动作计划，在杂乱房间清洁任务中表现优于非结构化CoT。


<details>
  <summary>Details</summary>
Motivation: 传统思维链提示依赖非结构化文本，在具身任务中可解释性和可执行性受限。现有结构化CoT方法仅建模低阶关系，缺乏继承、行为抽象等构造，也没有标准化的顺序或条件规划语义。

Method: 提出UML-CoT框架，使用UML类图捕捉组合对象语义，活动图建模过程控制流。采用三阶段训练流水线，结合监督微调和GRPO策略优化，包括从仅答案数据中学习奖励。

Result: 在MRoom-30k基准测试中，UML-CoT在可解释性、规划连贯性和执行成功率方面均优于非结构化CoT方法。

Conclusion: UML作为一种更富表达力和可操作的结构化推理形式化方法，在具身推理任务中具有显著优势。
...
Method: 1. 扩展的无分类器引导方案，独立控制正负图像条件；2. 基于类相似性的采样策略，识别有效对比样本；3. 无需模型微调或外部标注过滤的简单流程。

Result: 在十个基准数据集上达到最先进或可比性能，无需生成模型适配或依赖外部工具进行标注生成和图像过滤。

Conclusion: 利用正负引导的双重图像提示能有效生成类判别特征，特别适用于细粒度分类任务。
...
<details>
  <summary>Details</summary>
Motivation: 探索自回归变换器与扩散模型之间的理论联系，利用扩散模型的优势来改进视觉自回归生成模型。

Method: 提出SRDD方法，将VAR重新解释为离散扩散模型，引入迭代精炼机制，减少架构低效性。

Result: 在多个数据集上实现了更快的收敛速度、更低的推理成本和改进的零样本重建能力。

Conclusion: 扩散模型视角下的VAR在效率和生成质量方面都带来了持续提升，为视觉生成提供了新的理论框架。
...
TL;DR: 提出RefAM框架，利用扩散变换器的注意力特征实现零样本指代分割，无需微调或额外训练，在图像和视频基准测试中达到新SOTA
...
Method: 利用扩散变换器的注意力分数，识别并处理停用词作为注意力磁铁和全局注意力汇，提出注意力重分配策略，开发无需训练的RefAM框架

Result: 在零样本指代图像和视频分割基准测试中，始终优于现有方法，无需微调或额外组件即达到新SOTA

Conclusion: 扩散变换器的注意力特征可直接用于下游任务，停用词过滤和注意力汇处理能显著提升定位精度
...
TL;DR: 本文提出了一种双模型架构，通过投影融合块和差异特征学习机制来检测大语言模型的幻觉现象，发现幻觉信号集中在高度稀疏的特征子集中，仅需1%的特征维度即可保持检测性能。
...
Method: 提出双模型架构，包含投影融合块用于自适应层间特征加权，以及差异特征学习机制，通过计算并行编码器学习相同输入互补表示之间的差异来识别判别性特征。

Result: 在HaluEval的问答、对话和摘要数据集上的系统实验表明，幻觉信号集中在高度稀疏的特征子集中，在问答和对话任务上实现了显著的准确率提升，仅使用1%的特征维度即可保持检测性能。
...
TL;DR: 本文提出了一种新的上下文质量评估方法CI值，通过测量移除每个上下文对性能的影响来量化上下文质量，有效解决了RAG中低质量上下文的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在上下文质量评估上表现有限，因为它们未能全面利用可用信息（查询、上下文列表和生成器）进行综合质量评估。

Method: 将上下文质量评估重新概念化为推理时数据估值问题，引入Contextual Influence Value (CI值)，并开发参数化代理模型进行CI值预测。

Result: 在8个NLP任务和多个LLM上的广泛实验表明，该方法显著优于现有最先进基线，能有效过滤低质量上下文同时保留关键信息。

Conclusion: CI值方法通过整合查询感知相关性、列表感知独特性和生成器感知对齐，为RAG提供了更有效的上下文选择机制。
...
TL;DR: 研究发现语言模型的实际有效上下文窗口远小于宣传的最大上下文窗口，大多数模型在1000个标记内就出现严重性能下降，实际有效窗口可能比宣传值小99%。


<details>
  <summary>Details</summary>
Motivation: 测试语言模型在真实场景中上下文窗口的实际有效性，揭示宣传的最大上下文窗口与实际有效窗口之间的显著差异。

Method: 定义了最大有效上下文窗口概念，制定了测试方法评估不同大小和问题类型下上下文窗口的有效性，创建了标准化比较方式来找到性能失效点。

Result: 收集了数十万个数据点，发现最大上下文窗口与最大有效上下文窗口存在显著差异，且有效窗口大小随问题类型变化。多数顶级模型在100个标记内就失效，1000个标记内准确率严重下降。

Conclusion: 最大有效上下文窗口因问题类型而异，这为提高模型准确率和减少幻觉提供了明确可行的改进方向。
...
TL;DR: AI未来发展需要超越单纯扩展规模，需要人类创造的符号作为指南针来引导大型语言模型的直觉能力
...
Method: 提出使用人类创造的符号作为指南针来引导大型语言模型的直觉

Result: 为AI未来发展提供了新的方向，强调符号引导对实现真正发现的重要性

Conclusion: AI的未来发展需要结合人类符号的引导，而不仅仅是依赖模型规模的扩展
...
TL;DR: 研究发现大型语言模型在多语言环境中的道德判断存在显著不一致性，反映了文化错位问题，揭示了预训练数据对模型道德指南针的影响。


<details>
  <summary>Details</summary>
Motivation: 由于LLMs主要基于英语数据预训练，需要评估其在多语言多文化环境中进行道德推理的能力，以及语言如何调节道德决策。

Method: 将两个成熟的道德推理基准翻译成五种文化和类型学上不同的语言，进行多语言零样本评估，通过精心构建的研究问题分析差异驱动因素。

Result: 发现LLMs在不同语言中的道德判断存在显著不一致，往往反映文化错位，并揭示了从分歧到推理策略的潜在驱动因素。
...
TL;DR: 本研究评估GPT-5在糖尿病诊断和管理中的表现，使用基于ADA 2025标准的合成病例框架，测试了症状识别、实验室解读等五个场景，结果显示GPT-5与ADA标准高度一致。


<details>
  <summary>Details</summary>
Motivation: 糖尿病是全球重大健康挑战，早期识别困难。大型语言模型的发展为提供结构化、可解释的决策支持提供了机会。

Method: 使用完全基于合成病例的模拟框架，这些病例符合ADA 2025护理标准，并参考了NHANES、Pima Indians等公共数据集。测试了五个代表性场景。

Result: GPT-5在病例分类、临床推理生成、患者解释和结构化JSON摘要方面表现出色，与ADA定义的标准高度一致。

Conclusion: GPT-5可能成为临床医生和患者的双重用途工具，同时强调了可重复评估框架在负责任评估医疗领域LLMs的重要性。
...
TL;DR: 本文研究了在减轻性别刻板印象的道德对齐过程中，下游任务性能下降的机制，发现性能主要由整体遗忘水平驱动，选择性遗忘刻板印象会增加整体遗忘，而现有的缓解遗忘方法效果有限。


<details>
  <summary>Details</summary>
Motivation: 道德对齐虽然能调节预训练语言模型的行为，但通常会导致下游任务性能下降。本研究旨在探究这种性能权衡的底层机制，特别是在减轻性别刻板印象的背景下。

Method: 通过遗忘和公平性目标的视角，分析性能权衡的机制，研究选择性遗忘刻板印象对整体遗忘水平的影响。

Result: 分析表明：(1)下游任务性能主要由整体遗忘水平驱动；(2)选择性遗忘刻板印象会增加整体遗忘；(3)现有的缓解遗忘方法对减少整体遗忘和改善下游任务性能效果不佳。

Conclusion: 当前的公平性目标在实现性能权衡方面存在局限性，需要更有效的方法来平衡道德对齐与任务性能。
...
TL;DR: 提出了一种基于强化学习的自定义推理模型训练方法，在BIRD数据库查询基准测试中实现了最先进的准确率，无需额外训练数据或专有模型。
...
Method: 采用简单的通用训练方案：精心设计的提示和模型选择，使用离线RL方法TAO进行预热训练，然后进行严格的在线RLVR训练。

Result: 在BIRD基准测试中，首次提交就达到了私有测试集的最高准确率：无自一致性73.56%，有自一致性75.68%，且所需生成次数更少。
...
TL;DR: 该论文提出在强化学习可验证奖励框架中引入token混合生成方法，通过利用模型概率分布中的丰富信息，提升语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法在推理步骤中采样离散token，丢弃了模型概率分布中的丰富分布信息，不必要地限制了推理搜索空间。

Method: 提出了token混合生成的统一框架，将RLVR扩展到连续混合空间中操作，包括构建混合嵌入作为token嵌入的加权和。

Result: 在Reasoning-Gym推理任务套件上，MoT-G方法相比标准解码获得显著提升（7/10任务有5-35%增益），用一半轨迹数达到可比精度。

Conclusion: MoT-G通过保持更高的隐藏状态熵和促进token空间探索来提升推理性能，提高了训练效率。
...
TL;DR: Dual-Head Reasoning Distillation (DHRD) 是一种训练方法，通过添加分类头和推理头来解决 CoT 提示的吞吐量问题，在保持准确率的同时大幅提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 解决 Chain-of-Thought (CoT) 提示在提高分类准确率的同时引入显著吞吐量惩罚的问题，需要在准确率和推理效率之间找到平衡。

Method: 为解码器语言模型添加两个头：(i) 池化分类头用于训练和推理，(ii) 推理头仅在训练时受教师理由监督。使用标签交叉熵和输入加理由序列的标记级语言模型损失的加权和进行训练。

Result: 在七个 SuperGLUE 任务上，DHRD 相对于池化基线获得 0.65-5.47% 的相对增益，在蕴含/因果任务上增益更大。推理吞吐量与池化分类器匹配，比相同骨干网络的 CoT 解码快 96-142 倍。

Conclusion: DHRD 方法在保持 CoT 准确率优势的同时，消除了推理时的吞吐量惩罚，实现了准确率和效率的良好平衡。
...
TL;DR: 本文系统研究了代码的哪些特性对提升大语言模型推理能力最关键，发现结构扰动比语义扰动影响更大，适当的抽象（如伪代码）与代码效果相当，甚至在某些情况下表现更好。


<details>
  <summary>Details</summary>
Motivation: 虽然已知代码数据能增强大语言模型的推理能力，但具体是代码的哪些方面起关键作用尚不清楚，需要系统研究来指导训练数据设计。

Method: 构建十种编程语言的平行指令数据集，应用受控扰动选择性破坏代码的结构或语义属性，在五个模型家族八个规模上进行了3,331次实验。

Result: LLMs对结构扰动比语义扰动更敏感；伪代码和流程图等抽象与代码效果相当；即使包含误导信号的损坏代码，只要表面规律存在仍保持竞争力；不同编程语言风格影响任务表现。

Conclusion: 代码的结构特性对LLM推理能力影响更大，适当的抽象可以替代代码且更高效，这为设计增强LLM推理能力的训练数据提供了重要指导。
...
Method: 基于句子嵌入模型构建聊天机器人，通过消除同义词和加入实体提取来提高准确性。

Result: 系统准确率从56%提升到86%，可24/7通过任何电子设备访问，为农民提供易于理解的农业信息。

Conclusion: 该系统能帮助农民更容易获取农业实践信息，提高农业产出，同时减轻呼叫中心工作人员负担。
...
TL;DR: 该研究分析了非洲口音英语在说话人日志任务中的领域效应，发现在临床对话中存在一致的领域惩罚，并通过轻量级领域适应方法部分缓解了这一问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索说话人日志在不同领域（特别是临床对话）中的性能差异，重点关注非洲口音英语在通用对话和临床对话中的表现差异。

Method: 使用严格DER协议评估多个生产和开源系统，通过错误分析归因领域惩罚，并测试在口音匹配数据上微调分割模块的轻量级领域适应方法。

Result: 临床语音存在显著的领域惩罚，主要归因于误报和漏检，与短轮次和频繁重叠相关。轻量级适应减少了错误但未能完全消除差距。

Conclusion: 研究提出了跨领域基准、错误分解方法和易于复现的适应方案，建议下一步关注重叠感知分割和平衡临床资源。
...
TL;DR: 论文比较了两种LLM引用范式：生成时引用(G-Cite)和后处理引用(P-Cite)，发现在高风险应用中推荐采用检索为中心、P-Cite优先的方法。


<details>
  <summary>Details</summary>
Motivation: 在医疗、法律、学术和金融等高风险领域，可信赖的LLM必须引用可验证的来源，因为小错误可能带来严重后果。需要明确模型是在解码时生成引用还是先起草答案再附加引用的选择。

Method: 引入两种引用范式：G-Cite（单次生成答案和引用）和P-Cite（起草后添加或验证引用）。在四个流行归因数据集上从零样本到高级检索增强方法进行全面评估。

Result: 结果显示覆盖率和引用正确性之间存在一致权衡，检索是两种范式中归因质量的主要驱动因素。P-Cite方法实现高覆盖率且具有竞争力的正确性和适度延迟，而G-Cite方法以覆盖率和速度为代价优先考虑精度。

Conclusion: 推荐高风险应用采用检索为中心、P-Cite优先的方法，在严格声明验证等精度关键场景中保留G-Cite。
...
Method: ComPSum框架首先通过比较给定用户偏好与其他用户偏好来生成结构化分析，然后使用该分析指导个性化摘要的生成。

Result: 在PerMSum数据集上使用AuthorMap评估，ComPSum优于强基线方法。

Conclusion: 通过比较用户偏好的细粒度差异来生成结构化分析，可以有效指导个性化摘要的生成，ComPSum框架在个性化多文档摘要任务中表现优异。
...
TL;DR: 本文提出五种VLM作为形式化器的流水线，用于解决单样本、开放词汇和多模态PDDL形式化问题，在包含真实多视角低质量图像的基准测试中显著优于端到端规划生成。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在简单多模态规划任务中表现良好，但在需要长序列动作的长时程规划中表现不足。文本领域通过将规划问题转换为PDDL形式化语言取得了显著进展，但在多模态环境中相关研究仍然稀缺。

Method: 开发了五种VLM作为形式化器的流水线，能够处理单样本、开放词汇和多模态PDDL形式化，并在包含真实多视角低质量图像的新基准上进行评估。

Result: VLM作为形式化器的方法在规划性能上大幅优于端到端规划生成。瓶颈主要在于视觉而非语言能力，因为VLMs经常无法捕捉到完整必要的对象关系。

Conclusion: 虽然生成中间文本表示（如标题或场景图）能够部分弥补性能不足，但其增益不一致，为多模态规划形式化的未来研究留下了改进空间。
...
TL;DR: 本研究评估了多语言AI模型在翻译比喻性语言时的本地化能力，发现尽管模型能生成语法正确的翻译，但在文化适应性方面仍有明显不足，特别是对习语和双关语的翻译。


<details>
  <summary>Details</summary>
Motivation: 现有LLM翻译研究和行业基准过于强调语法准确性和词汇正确性，而忽视了文化适应性和整体本地化质量，这对营销和电子商务等实际应用至关重要。

Method: 评估了87个LLM生成的电子商务营销邮件翻译，涵盖20种语言的24种方言变体，由精通目标语言的人工评审员提供定量评分和定性反馈。

Result: 领先模型通常能生成语法正确的翻译，但在文化细微语言方面表现不佳，需要大量人工修正。即使是高资源语言也经常误译比喻性表达和文字游戏。

Conclusion: 数据量不是机器翻译质量的最可靠预测指标，文化适应性应作为多语言LLM性能的关键决定因素。当前多语言AI系统在实际本地化用例中存在局限性，需要更大规模的研究。
...
Method: 提出MORL分类法，分析各种MORL方法在LLM优化中的应用，并构建MORL基准测试框架来评估不同方法对多样目标关系的影响。

Result: 识别出现有MORL方法在LLM优化中的优势和局限，提出了元策略MORL的双层学习范式作为解决方案。
...
TL;DR: OjaKV是一个KV缓存压缩框架，通过混合存储策略和在线子空间适应来减少长上下文生成的内存瓶颈，无需模型微调即可保持或提高精度。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法使用静态离线学习的子空间，在数据分布变化时性能较差，无法适应动态上下文变化。

Method: 结合混合存储策略（完整保留首尾关键token）和在线子空间适应（使用Oja算法进行增量PCA），在预填充阶段全面更新，解码阶段轻量级更新。

Result: 在高压缩比下保持或提高零样本精度，在需要复杂推理的极长上下文基准测试中表现最佳，证明在线适应的有效性。

Conclusion: OjaKV是一个实用的即插即用解决方案，可在不微调模型的情况下实现内存高效的长上下文推理。
...
TL;DR: 这篇综述系统地回顾了语言模型的可解释人工智能方法，按transformer架构分类分析，并评估其合理性和忠实性，旨在推动LM的透明可解释性发展。


<details>
  <summary>Details</summary>
Motivation: 语言模型的黑盒特性阻碍了其在关键领域的应用，而现有的XAI方法在处理LM时面临架构复杂、训练数据庞大等独特挑战，需要专门针对LM特点的综述研究。

Method: 按transformer架构（编码器、解码器、编码-解码器）组织XAI技术分类，分析各方法在不同架构中的适应性，并通过合理性和忠实性双重标准评估方法效果。

Result: 提供了针对不同LM架构的XAI方法系统分类和评估框架，识别了现有方法的优势和局限性。

Conclusion: 指出了开放的研究挑战和未来方向，为开发鲁棒、透明、可解释的LM XAI方法提供指导。
...
TL;DR: 该论文提出ReviewScore来自动检测同行评审中的错误信息点，包括基于错误前提的"弱点"和论文已回答的"问题"，并验证了LLMs在评估ReviewScore方面与人类专家具有中等一致性。
...
Method: 定义错误信息评审点，构建人工标注的ReviewScore数据集，提出自动重构弱点前提的引擎，使用8个最先进LLMs评估人类-模型一致性。

Result: 15.2%的弱点和26.4%的问题是错误信息的；LLMs与人类在ReviewScore评估上达到中等一致性；前提级事实性评估比弱点级评估一致性更高。

Conclusion: 自动化ReviewScore评估具有潜力，前提级事实性评估能显著提高人类-模型一致性。
...
TL;DR: GRAB是一个金融风险分类基准，包含161万句10-K文件句子，使用弱监督方法自动标注风险类别，支持标准化评估无监督主题模型在金融风险披露分类任务上的性能。
...
Method: 结合FinBERT token attention、YAKE关键短语信号和基于分类学的搭配匹配，无需人工标注自动生成句子标签，构建包含193个术语映射到21个细粒度风险类型的分类体系。

Result: 创建了包含8,247份文件的基准数据集，提供固定数据分割和稳健评估指标（准确率、宏F1、主题BERTScore、有效主题数），支持经典、嵌入、神经和混合主题模型的标准化比较。

Conclusion: GRAB基准为金融风险披露分类提供了可复现、标准化的评估框架，填补了该领域公共基准的空白。
...
TL;DR: 提出了Think-on-Graph 3.0框架，通过多智能体上下文演进与检索机制，动态构建和优化异构图索引，解决了传统图增强检索生成方法中静态图构建的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有图增强检索生成方法面临根本性权衡：手动构建知识图谱成本高昂，自动提取的图谱受限于底层LLM提取器的性能，特别是使用轻量级本地部署模型时。传统方法通常构建静态图索引，无法适应实际查询需求。

Method: 采用多智能体系统（构造器、检索器、反射器和响应器），通过双演进机制（演进查询和演进子图）动态构建Chunk-Triplets-Community异构图索引，实现迭代的证据检索、答案生成和充分性反思。

Result: 在深度和广度推理基准测试中，ToG-3优于对比基线方法，消融研究证实了MACER框架各组成部分的有效性。
...
TL;DR: 提出了ProPerSim框架和ProPerAssistant助手，用于开发能够在家庭场景中主动提供个性化推荐的人工智能助手，通过用户反馈持续学习和适应。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在日常生活中的应用增加，需要开发既主动又个性化的AI助手，但目前主动性和个性化的结合研究不足。

Method: 构建ProPerSim仿真框架，用户代理具有丰富个性特征，与助手交互并提供评分；提出ProPerAssistant，基于检索增强和偏好对齐，通过用户反馈持续学习适应。

Result: 在32个不同个性特征的测试中，ProPerAssistant能够调整策略并稳步提高用户满意度。

Conclusion: 结合主动性和个性化具有良好前景，ProPerSim框架为开发此类助手提供了有效平台。
...
TL;DR: 本文研究了大型语言模型在长上下文问答中的表现，发现虽然GPT-4o等专有模型整体表现最佳，但经过微调的8B参数开源模型在准确率上可以超越GPT-4o，展示了开源模型在工业部署中的潜力。


<details>
  <summary>Details</summary>
Motivation: 工业环境中部署LLM进行长上下文问答面临高计算成本和延迟的挑战，特别是在需要基于同一上下文回答多个问题时。

Method: 对专有和开源模型进行广泛实验和基准测试，评估它们在基于同一对话上下文回答多个问题时的能力。

Result: 强力的专有LLM如GPT-4o整体表现最佳，但经过微调的8B参数开源模型在准确率上可以超越GPT-4o。

Conclusion: 微调的开源LLM具有透明且经济高效的部署潜力，适合实际工业应用。
...
TL;DR: 提出Self-Speculative Biased Decoding方法，用于流式应用中的实时翻译，通过重用最近输出作为草稿，在验证阶段偏向草稿token来提高接受率，实现1.7倍加速和80%闪烁减少。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在流式应用（如实时翻译）中需要不断更新输出但计算成本高的问题，避免重复从头生成输出。

Method: 使用最近输出作为草稿，在验证阶段偏向草稿token，结合display-only mask-k技术减少闪烁。

Result: 在同时文本重翻译任务中实现1.7倍加速，闪烁减少80%，且不损失质量。

Conclusion: 该方法无需草稿计算，是模型无关的即插即用解决方案，适用于延迟敏感的流式应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型在复杂声学场景下的音频推理任务表现不佳，缺乏对噪声抑制、声源分离等声学工具的使用能力。

Method: 开发了Thinking-with-Sound框架，结合语言推理和实时音频域分析，使模型能够主动对音频信号进行数值分析和数字处理。

Result: 在MELD-Hard1k基准测试中，TwS显著提升了模型鲁棒性：小模型准确率提升24.73%，大模型提升36.61%，而现有模型在扰动音频上准确率下降超过50%。

Conclusion: 音频思维链能够在不重新训练的情况下显著增强模型鲁棒性，为开发更鲁棒的音频理解系统开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索-排序推荐系统存在校准问题和工程开销，现有生成式模型在统一检索和排序时面临性能权衡，需要一种能同时处理个性化搜索和推荐的统一解决方案。

Method: 使用基于行为序列训练的仅解码器Transformer，结合InfoNCE损失进行检索优化和混合点对-配对损失进行排序优化，并提出时间感知旋转位置嵌入来整合时间信息。
...
TL;DR: 本文使用因果推理方法分析结构化输出对LLMs生成质量的影响，发现粗粒度指标显示正负或中性效应，但因果分析显示在大多数场景中结构化输出没有因果影响。


<details>
  <summary>Details</summary>
Motivation: 先前研究对结构化输出对LLMs生成质量的影响存在矛盾结论，且评估方法存在测试场景受限、比较设置控制弱、依赖粗粒度指标等局限性。

Method: 基于一个假设和两个保证约束，推导出五种潜在因果结构，在七个公共和一个自建推理任务上进行因果推理分析。

Result: 在48个场景中，43个显示结构化输出没有因果影响，其余5个中3个涉及由具体指令影响的多方面因果结构。

Conclusion: 结构化输出对LLMs生成质量的因果影响比先前研究认为的要小，需要更精细的因果分析方法来理解其真实影响。
...
TL;DR: 提出了CARB基准来评估奖励模型的文化意识，发现现有模型存在缺陷，并提出Think-as-Locals方法通过RLVR来改进文化意识奖励建模。
...
Method: 构建CARB基准覆盖10种文化和4个文化领域，提出Think-as-Locals方法使用RLVR来激发深度文化推理。

Result: 评估显示现有奖励模型在文化意识建模上存在不足，CARB性能与下游多语言文化对齐任务正相关，Think-as-Locals能有效缓解虚假特征干扰。

Conclusion: CARB基准填补了文化意识评估空白，Think-as-Locals方法能有效提升文化意识奖励建模质量。
...
TL;DR: 本文提出了一种扩展动作空间的同步机器翻译方法，在传统读写动作基础上增加了句子切分、省略、部分摘要和代词化四种自适应动作，在LLM框架中实现，显著提升了翻译质量和延迟性能。


<details>
  <summary>Details</summary>
Motivation: 传统编码器-解码器策略的读写动作无法充分应对同步机器翻译的实时约束，需要更丰富的动作空间来实现实时重构、省略和简化，同时保持语义保真度。

Method: 在仅解码器的大语言模型框架中实现四种自适应动作，通过动作感知提示构建训练参考，并开发了延迟感知的TTS流水线来评估质量和延迟。

Result: 在ACL60/60英中和英德基准测试中，该框架持续改善语义指标（如COMET-KIWI），并实现了比参考翻译和基于salami基线更低的延迟（通过平均滞后衡量）。

Conclusion: 丰富基于LLM的同步机器翻译动作空间为弥合人机翻译差距提供了有前景的方向，特别是结合省略和句子切分动作在流畅性和延迟之间达到了最佳平衡。
...
TL;DR: 提出CaMIB模型，通过因果多模态信息瓶颈解决传统多模态学习中的数据集偏差问题，提升OOD泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法容易受到数据集偏差影响，将统计捷径与真实因果特征混淆，导致分布外泛化性能下降

Method: 使用信息瓶颈过滤任务无关噪声，通过参数化掩码生成器将多模态表示解耦为因果和捷径子表示，结合工具变量约束和后门调整

Result: 在多模态情感分析、幽默检测和讽刺检测任务上，包括OOD测试集，均表现出有效性

Conclusion: CaMIB模型在理论和实证上均显示出良好的可解释性和鲁棒性，能够有效提升多模态理解的泛化性能
...
TL;DR: 本文研究了语言学谜题的解决与生成任务，探索了大型语言模型在语言学奥林匹克竞赛谜题上的表现，并基于此开发了自动生成语言学谜题的新方法。


<details>
  <summary>Details</summary>
Motivation: 通过自动化生成语言学谜题来扩大对语言学的兴趣，向更广泛的受众介绍这一领域，同时支持稀有和未充分研究语言的知识传播。

Method: 扩展了现有的语言学谜题解决基准，探索了包括OpenAI o1在内的先进LLMs在解决各类语言学谜题上的表现，并基于谜题解决的洞察指导谜题生成任务。

Result: LLMs在大多数谜题类型上表现优于人类，但在文字系统和未充分研究语言相关的谜题上例外。

Conclusion: 语言学谜题生成作为一个研究任务具有重要意义，不仅能推广语言学，还能支持稀有语言知识的传播，自动化谜题生成具有扩大语言学影响力的潜力。
...
TL;DR: 提出ResT方法，通过熵感知的token重加权来重塑策略梯度，优化LLM工具使用任务的训练效率和稳定性


<details>
  <summary>Details</summary>
Motivation: 现有RL方法仅依赖稀疏结果奖励，未考虑工具使用任务的特殊性，导致策略梯度方差大、训练效率低

Method: ResT方法通过熵感知的token重加权来重塑策略梯度，逐步增加推理token的权重，实现从结构正确性到语义推理的平滑过渡

Result: 在BFCL和API-Bank上达到SOTA结果，比现有方法提升8.76%，在4B基础LLM上微调后，单轮任务超越GPT-4o 4.11%，多轮任务超越1.50%

Conclusion: ResT通过熵感知的token级策略梯度重塑，有效解决了工具使用任务的训练不稳定问题，显著提升了性能
...
Method: 使用语义一致性作为无需训练的信号，当不同模型输出在语义层面达成共识时，将其作为比词元级置信度更强的可靠性指标

Result: 在500M到70B参数模型上的评估显示，语义级联以40%的成本达到或超越目标模型质量，延迟降低达60%
...
TL;DR: 提出了TRACE框架，通过将同理心建模为结构化认知过程，将任务分解为分析和合成管道，在生成前建立全面理解，统一深度分析和表达生成。
...
Method: TRACE框架将同理心响应生成任务分解为分析和合成两个阶段，通过结构化认知过程建模同理心。

Result: 实验结果显示该框架在自动评估和LLM评估中显著优于强基线模型。

Conclusion: 结构化分解是创建更强大和可解释同理心代理的有前景范式。
...
TL;DR: 提出了首个用于评估多轮长形式问答的基准KnowMT-Bench，发现在多轮对话中模型的事实准确性和信息效率会下降，而检索增强生成(RAG)可以有效缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单轮对话，而多轮对话基准通常评估其他能力而非知识密集型的事实准确性，因此需要专门评估多轮长形式问答的基准。

Method: 创建KnowMT-Bench基准，采用动态评估设置让模型生成自己的多轮对话历史，使用人工验证的自动化流程评估最终轮答案的事实能力和信息传递效率。

Result: 多轮上下文会降低性能：事实能力因自生成历史的上下文噪声而下降，信息效率随对话长度增加而模型变得更冗长而降低。RAG可以有效缓解甚至逆转这种事实退化。
...
TL;DR: LoRAN是非线性扩展的LoRA方法，通过轻量级变换增强低秩更新的表达能力，引入基于正弦的Sinter激活函数，在多个任务中表现优于QLoRA。
...
Method: 提出LoRAN方法，对低秩更新应用轻量级变换，并引入Sinter正弦激活函数，在不增加参数的情况下添加结构化扰动。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统即使提供了正确和充分的上下文，仍会产生幻觉。现有方法需要大量超参数调优，限制了其泛化能力。

Method: 通过分布距离量化外部上下文利用，通过跟踪transformer层间预测token的演变来测量内部知识利用，并引入统计验证框架。
...
TL;DR: RL-ZVP是一种新算法，能从零方差提示中提取学习信号，在数学推理基准上显著优于GRPO和其他基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法如GRPO只关注模型对同一输入产生不同正确性的响应，而忽略了所有响应获得相同奖励的零方差提示。本文认为这些提示并非无用，实际上能为策略优化提供有意义的反馈。

Method: 引入RL-ZVP算法，直接从零方差提示中提取学习信号。该方法直接奖励正确性并惩罚错误，即使没有对比响应，通过调节基于标记级别特征的反馈来保留信息丰富、细致的信号。

Result: 在六个数学推理基准测试中，RL-ZVP相比GRPO在准确率上提升了高达8.61个百分点，通过率提升了7.77个百分点，并且始终优于其他过滤掉零方差提示的基线方法。
...
TL;DR: QoNext框架将网络多媒体领域的QoE原则应用于基础模型评估，关注用户体验而非仅输出正确性，通过实验构建QoE数据库并训练预测模型。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅关注输出正确性，忽视了用户体验在交互过程中的重要性，无法解释用户满意度的形成机制。

Method: 识别影响用户体验的因素，在受控实验中收集不同配置下的人类评分，构建QoE数据库并训练预测模型。

Result: QoNext能够实现主动和细粒度的评估，并为产品化服务提供优化基础模型的可操作指导。

Conclusion: QoNext框架成功将QoE原则引入基础模型评估，填补了用户体验评估的空白，具有实际应用价值。
...
TL;DR: 论文提出了弹性专家混合（EMoE）训练框架，解决传统MoE模型在推理时增加激活专家数量导致性能下降的问题，通过训练专家间的多样化协作，显著扩展了性能扩展范围。


<details>
  <summary>Details</summary>
Motivation: 传统MoE模型在训练和推理时固定激活专家数量k，直觉上推理时激活更多专家应提升性能，但研究发现性能在轻微增加专家数量后就会快速下降，这源于专家间缺乏学习到的协作。

Method: 提出EMoE训练框架，同时训练专家在多样化组合中协作，并鼓励路由器进行高质量选择，使MoE模型能在推理时扩展激活专家数量而无需额外训练开销。

Result: 实验表明EMoE显著扩展了有效性能扩展范围，达到训练时k的2-3倍，同时将模型峰值性能推向更高水平。

Conclusion: EMoE通过促进专家协作和路由器优化，解决了MoE模型在推理时扩展专家数量的性能瓶颈，实现了更灵活和高效的模型部署。
...
TL;DR: 本文针对土耳其语等黏着语言的引文意图分类问题，提出了基于DSPy框架的可编程分类管道和堆叠泛化集成方法，在自建土耳其语引文意图数据集上达到了91.3%的最先进准确率。


<details>
  <summary>Details</summary>
Motivation: 理解引文的定性意图对于学术研究评估至关重要，但土耳其语等黏着语言在此任务上存在独特挑战，需要专门的方法和数据集。

Method: 首先创建了土耳其语引文意图数据集，然后使用DSPy框架构建可编程分类管道来自动优化提示，最后采用堆叠泛化集成方法（XGBoost元模型）聚合多个优化模型的输出。

Result: 提出的集成方法在土耳其语引文意图分类任务上达到了91.3%的准确率，显著优于标准上下文学习方法。

Conclusion: 本研究为土耳其语NLP社区和更广泛的学术界提供了基础数据集和鲁棒的分类框架，为未来的定性引文研究铺平了道路。
...
TL;DR: AutoSCORE是一个多智能体LLM框架，通过基于评分标准的结构化组件识别来改进自动评分，解决了单一LLM评分器的准确性低、提示敏感、可解释性差和对齐不佳等问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLM作为端到端评分器面临的低准确性、提示敏感性、有限可解释性和评分标准对齐不佳等问题，这些限制了LLM在自动评分实践中的应用。

Method: 采用双智能体框架：评分标准组件提取智能体从学生回答中提取相关组件并编码为结构化表示，然后评分智能体使用该表示分配最终分数，确保模型推理遵循类似人类的评分过程。

Result: 在ASAP基准的四个数据集上，使用GPT-4o、LLaMA-3.1-8B和LLaMA-3.1-70B进行评估，AutoSCORE相比单智能体基线在评分准确性、人机一致性（QWK、相关性）和误差指标（MAE、RMSE）上持续改进，特别是在复杂多维评分标准和较小LLM上获得较大相对增益。
...
TL;DR: 提出SimulSense框架，模仿人类口译员在感知到新意义单元时触发写入决策，实现同时语音翻译的读写决策
...
Method: SimulSense框架通过连续读取输入语音，在感知到新意义单元时触发写入决策来产生翻译

Result: 相比两种最先进的基线系统，该方法实现了更优的质量-延迟权衡，决策速度比基线快9.6倍

Conclusion: SimulSense框架在同时语音翻译中实现了高效的人类口译员式读写决策，显著提升了实时效率
...
TL;DR: 本文首次对临床文本理解中的思维链提示进行了大规模系统研究，发现86.3%的模型在CoT设置下性能下降，揭示了临床领域CoT增强可解释性但可能损害可靠性的关键悖论。


<details>
  <summary>Details</summary>
Motivation: 探索思维链提示在临床文本理解中的有效性，特别是在电子健康记录这种冗长、碎片化和嘈杂的临床文档背景下，这对LLM在临床护理中的安全可信部署至关重要。

Method: 评估95个先进LLM在87个真实世界临床文本任务上的表现，涵盖9种语言和8种任务类型，通过细粒度分析推理长度、医学概念对齐和错误模式，结合LLM作为评判和临床专家评估。

Result: 与其它领域相反，86.3%的模型在CoT设置下性能持续下降，能力更强的模型相对稳健，而较弱的模型显著下降，揭示了CoT在临床文本任务中增强可解释性但损害可靠性的系统模式。

Conclusion: 研究为LLM的临床推理策略提供了实证基础，强调了在临床文本任务中需要透明且可信的方法，揭示了CoT在临床环境中的局限性。
...
TL;DR: 提出了ThaiFACTUAL框架，通过反事实数据增强和基于理由的监督来缓解泰语政治立场检测中的偏见，无需微调即可改善LLMs在低资源环境下的公平性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在泰语政治立场检测中的系统性偏见问题，包括情感泄漏和实体偏好，这些偏见在间接语言、极化人物和情感立场交织的复杂文化环境中尤为突出。

Method: 使用反事实数据增强和基于理由的监督来分离情感与立场，减少偏见；构建了首个高质量的泰语政治立场数据集，包含立场、情感、理由和偏见标记。

Result: ThaiFACTUAL显著减少了伪相关性，增强了零样本泛化能力，并提高了多个LLMs的公平性。

Conclusion: 这项工作强调了基于文化的去偏技术对于代表性不足语言的重要性，为低资源复杂文化环境下的政治立场检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在加速学术创意生成方面潜力巨大，但面临创意落地困难和确认偏误的挑战，需要改进创意质量和实用性。

Method: 整合动机知识图谱（存储问题、挑战、解决方案节点）和Q驱动的苏格拉底创意生成器（双代理系统），通过苏格拉底式提问进行严格精炼。

Result: 在ICLR25论文主题数据集上，MotivGraph-SoIQ在LLM评分、ELO排名和人工评估指标上均优于现有最先进方法。

Conclusion: 该框架为LLM创意生成提供了必要的落地基础和实用的创意改进步骤，显著提升了创意的新颖性、实验严谨性和动机合理性。
...
TL;DR: 提出了一种基于不确定性的黑盒幻觉检测方法，通过分析LLMs在不确定性表达下的行为一致性来检测事实性错误。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法需要外部资源或LLMs内部状态，但在黑盒API受限的情况下，急需建立有效的黑盒检测方法。

Method: 通过研究LLMs在不确定性表达下的行为，发现事实性回答具有一致性，非事实性回答则不一致，基于此设计黑盒检测指标。

Result: 实验表明，该指标在预测模型回答的事实性方面优于使用LLMs内部知识的基线方法。

Conclusion: 提出的黑盒幻觉检测方法简单有效，为受限API环境下的幻觉检测提供了可行方案。
...
TL;DR: GraphSearch提出了一种新颖的代理深度搜索工作流，通过双通道检索策略解决现有GraphRAG方法的浅层检索和结构图数据利用不足的问题。
...
Method: GraphSearch采用模块化框架组织检索过程，包含六个模块，支持多轮交互和迭代推理。采用双通道检索策略，对基于块的文本数据发出语义查询，对结构图数据发出关系查询。
...
TL;DR: 离群值在主题建模中通常被视为噪声，但研究发现它们可以作为新兴主题的弱信号，通过向量嵌入和累积聚类方法追踪其演变，发现离群值会随时间发展成连贯的主题。


<details>
  <summary>Details</summary>
Motivation: 探索在动态新闻语料库中，被主题建模忽略的离群值是否能够作为新兴主题的早期信号。

Method: 使用最先进语言模型的向量嵌入和累积聚类方法，追踪法英新闻数据集中离群值随时间的变化。

Result: 结果显示离群值在两种模型和语言中都倾向于随时间发展成连贯的主题。

Conclusion: 离群值不应被简单视为噪声，而是新兴主题的重要早期指标，这一发现在不同模型和语言中具有一致性。
...
<details>
  <summary>Details</summary>
Motivation: 临床聊天机器人应用中安全至关重要，现有方法（如护栏和工具调用）往往无法满足临床领域的细微需求，需要更专业的安全框架。

Method: 开发了TACOS分类法，整合安全过滤和工具选择，明确建模不同的安全阈值和外部工具依赖关系，并创建了TACOS标注数据集进行验证。

Result: 实验结果表明专门针对临床代理设置的分类法具有重要价值，并揭示了训练数据分布和基础模型预训练知识的有用见解。

Conclusion: TACOS框架为临床聊天机器人应用提供了一个全面、细粒度的安全解决方案，能够有效处理临床和非临床查询的广泛谱系。
...
TL;DR: 提出了模糊推理链(FRC)框架，将LLM语义先验与连续模糊隶属度结合，在概率推理和模糊隶属推理间建立显式交互，有效处理文本中的歧义、多义和不确定性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型快速发展，但在处理具有歧义、多义或不确定性的文本时仍面临重大挑战。传统基于概率的方法无法有效捕捉冲突或不确定信号。

Method: 引入模糊推理链(FRC)框架，整合LLM语义先验和连续模糊隶属度，在概率推理和模糊隶属推理间建立显式交互，逐步将模糊输入转化为清晰可解释的决策。

Result: 在情感分析任务上验证，理论分析和实证结果均表明FRC能确保稳定推理并促进不同模型规模间的知识迁移。
...
TL;DR: 提出了首个纵向社交媒体AI生成文本数据集RedNote-Vibe，并开发了基于心理语言学特征的AIGT检测框架PLAD，在检测性能和可解释性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注静态AIGT检测，而社交媒体内容受用户参与度驱动且随时间演变，需要研究AIGT的时序动态和用户交互模式。

Method: 构建了5年纵向数据集RedNote-Vibe，并提出基于心理语言学特征的PLAD检测框架进行可解释的AIGT检测。

Result: PLAD实现了优越的检测性能，揭示了区分人类和AI生成内容的特征，并发现了这些语言特征与社交媒体参与度之间的复杂关系。

Conclusion: RedNote-Vibe数据集和PLAD框架为社交媒体AIGT的时序分析和检测提供了重要工具，有助于理解AIGT在社交平台上的动态特征。
...
<details>
  <summary>Details</summary>
Motivation: NLP领域存在数百种质量评估标准名称，但相同名称可能评估不同方面，导致独立评估结果无法可靠比较，阻碍科学进步。

Method: 采用严格描述性方法，从三个NLP评估调查中推导出标准质量评估标准名称和定义，并将其组织成层次结构。

Result: 开发了QCET分类法，包含标准质量评估标准名称和定义，支持现有评估的可比性分析、新评估设计指导和法规合规评估。

Conclusion: QCET为解决NLP评估可比性问题提供了标准化框架，有助于建立可靠的系统质量比较基础。
...
TL;DR: 本文挑战了关于微调在模型编辑中无效的传统观点，提出通过恢复广度优先的批次优化流程和局部化调优策略，微调可以成为有效的模型编辑方法。


<details>
  <summary>Details</summary>
Motivation: 传统认为微调不适合模型编辑，但作者认为这种失败源于采用了不合适的深度优先单样本优化流程，而非微调本身的问题。

Method: 提出LocFT-BF方法：1) 恢复标准的广度优先（基于epoch）批次优化流程；2) 系统分析调优位置，采用局部化编辑策略。

Result: 在多种LLM和数据集上的实验表明，LocFT-BF大幅超越现有最优方法，首次实现了10万次编辑和720亿参数模型的持续编辑，且不损害通用能力。

Conclusion: 通过澄清长期误解并引入原则性的局部化调优策略，微调从被低估的基线方法转变为模型编辑的领先方法，为未来研究奠定了坚实基础。
...
TL;DR: CoSpaDi是一种无需训练的大语言模型压缩框架，使用结构化稀疏字典学习替代传统的低秩分解，通过数据感知策略在合理压缩比下保持更好的模型保真度。


<details>
  <summary>Details</summary>
Motivation: 传统的低秩权重近似方法虽然计算高效，但结构约束过于刚性，会导致明显的模型精度下降。需要更灵活的压缩方法。

Method: 使用结构化稀疏分解，将权重矩阵表示为稠密字典和列稀疏系数矩阵，实现子空间联合表示。利用小规模校准数据集优化分解，最小化功能重构误差。

Result: 在20-50%压缩比下，CoSpaDi在Llama和Qwen模型上相比最先进的数据感知低秩方法，在准确性和困惑度方面都表现出持续优势。
...
TL;DR: 提出了对话行为脚本（DAS）框架，通过结构化意图表示来生成多语言对话，避免直接翻译带来的问题，提升文化相关性和自然度。
...
Conclusion: DAS框架能有效缓解翻译腔问题，生成更流畅、自然的对话，支持跨语言的灵活本地化。
...
TL;DR: 本文提出了Solve-to-Judge (S2J)方法来解决生成奖励模型中的解决-判断差距问题，通过同时利用模型的解决和判断能力进行监督，在减少训练数据的情况下实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现生成奖励模型存在显著的解决-判断差距，即模型能够解决某些问题但无法做出正确判断（14%-37%），这限制了模型的判断能力。

Method: 提出S2J方法，在单个生成奖励模型的输出上同时利用解决和判断能力进行监督，在模型优化过程中显式连接问题解决和评估能力。

Result: S2J有效减少了16.2%的解决-判断差距，将模型判断性能提升了5.8%，在使用更小训练数据集的情况下实现了最先进的性能。

Conclusion: S2J方法通过自我进化而非依赖更强大外部模型蒸馏，成功缩小了生成奖励模型中的解决-判断差距，提升了判断性能。
...
TL;DR: 该论文提出了一种基于测试时计算扩展(TTS)的方法来验证复杂数值声明，通过训练验证器模型(VERIFIERFC)从多个推理路径中选择正确路径，并引入自适应机制提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在验证需要组合推理和数值推理的真实世界数值声明时存在不足，无法理解数值细节的细微差别，且容易发生推理漂移问题。

Method: 采用测试时计算扩展(TTS)方法，从LLM中引出多个推理路径，训练验证器模型(VERIFIERFC)来选择可能得出正确结论的路径，并引入自适应机制根据声明复杂度选择性应用TTS。

Result: TTS方法有效缓解了推理漂移问题，在数值声明验证任务上取得显著性能提升。自适应机制比标准TTS效率提高1.8倍，比单次验证方法性能提升18.8%。

Conclusion: 测试时计算扩展结合验证器模型能够有效提升复杂数值声明的验证性能，自适应机制在保持性能的同时显著提高了计算效率。
...
TL;DR: 提出Uni-LAP框架，通过整合监督分类模型和大型语言模型的优势，解决法律条款预测任务中的挑战，在多个司法管辖区数据集上表现优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有法律条款预测方法面临显著挑战：监督分类模型难以捕捉复杂事实模式，大型语言模型在预测场景中表现不佳，且现有方法缺乏跨司法管辖区的通用性。

Method: 提出Uni-LAP通用框架，通过紧密协作整合SCM和LLM的优势：SCM使用新颖的Top-K损失函数生成候选条款，LLM采用三段论启发式推理来优化最终预测。

Result: 在多个司法管辖区数据集上的实证结果表明，Uni-LAP方法一致优于现有基线方法，展示了其有效性和泛化能力。

Conclusion: Uni-LAP框架通过整合监督分类模型和大型语言模型的优势，有效解决了法律条款预测任务中的挑战，具有跨司法管辖区的通用性和优越性能。
...
TL;DR: 该调查分析了31个多语言视觉语言模型和21个基准测试，揭示了语言中立性与文化意识之间的关键张力，发现当前训练方法偏向中立性，而评估基准多采用翻译方法，存在跨语言能力差异和训练目标与评估目标之间的差距。
...
Method: 系统回顾了31个多语言视觉语言模型和21个基准测试，涵盖编码器专用和生成式架构，分析训练方法和评估方法。

Result: 发现当前训练方法通过对比学习偏向语言中立性，而文化意识依赖于多样化数据；三分之二的评估基准使用翻译方法强调语义一致性，近期工作开始纳入文化相关内容；存在跨语言能力差异和训练与评估目标不匹配的问题。

Conclusion: 多语言视觉语言模型需要在语言中立性和文化意识之间取得更好平衡，训练方法和评估基准需要更紧密地对齐，以更好地处理跨语言和跨文化的内容理解。
...
TL;DR: FoodSEM是一个专门针对食品领域命名实体链接任务进行微调的开源大语言模型，在多个食品相关本体上达到了最先进的性能表现。
...
Result: FoodSEM在多个本体和数据集上达到了最先进的性能，某些本体上的F1分数甚至达到98%，显著优于零样本、单样本和少样本提示的基线模型。

Conclusion: 该研究的主要贡献包括发布适用于LLM微调/评估的食品标注语料库、发布推进食品领域语义理解的鲁棒模型，以及为未来基准测试提供食品NEL的强基线。
...
TL;DR: 提出了Reasoning Capsule框架，通过将推理计划压缩为少量潜在标记来平衡推理效率与透明度，结合信息瓶颈原则实现高效且可解释的推理。


<details>
  <summary>Details</summary>
Motivation: 解决Chain-of-Thought提示方法在大型语言模型中存在的延迟高、内存占用大以及错误传播问题，寻求在潜在推理效率和显式推理透明度之间的平衡。

Method: 采用混合方法：将高层推理计划压缩为少量学习到的潜在标记（推理胶囊），同时保持执行步骤轻量级或显式化。基于信息瓶颈原则，通过低容量瓶颈促进最小化，通过主任务损失和辅助计划重建损失确保充分性。

Result: 在复杂基准测试中，该框架在保持或提高准确性的同时，显著减少了推理过程中的可见标记占用，实现了效率、准确性和可解释性的平衡。

Conclusion: Reasoning Capsule框架成功地在推理效率、准确性和可解释性之间找到了平衡点，为大型语言模型的复杂推理任务提供了一种更优的解决方案。
...
TL;DR: GTO通过对齐训练与解码时的树策略来解决推测解码中的策略偏差问题，提高大语言模型的推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的推测解码训练目标只优化单个贪婪路径，而解码时采用树策略验证多个分支，这种策略偏差限制了加速效果。

Method: 提出Group Tree Optimization (GTO)，包含两个组件：Draft Tree Reward（采样自由目标，衡量草稿树在目标模型下的期望接受长度）和Group-based Draft Policy Training（稳定优化方案，对比当前和参考草稿模型的树，应用PPO风格代理进行鲁棒更新）。

Result: 在对话、代码和数学任务以及多个LLM上，GTO将接受长度提高7.4%，比现有最佳方法EAGLE-3额外带来7.7%的加速。

Conclusion: GTO通过解决草稿策略偏差，为高效LLM推理提供了实用、通用的解决方案。
...
TL;DR: 本文介绍了德国国家研究数据基础设施(NFDI4DS)联盟开发的12个共享任务，这些任务通过标准化评估促进FAIR原则和可重复研究，涵盖学术文档处理的各种挑战。


<details>
  <summary>Details</summary>
Motivation: 共享任务是推动研究进步的有力工具，通过社区标准化评估促进FAIR原则（可查找、可访问、可互操作、可重用）以及透明和可重复的研究实践。

Method: 在NFDI4DS联盟下开发和托管12个共享任务，这些任务在领先的学术场所举办，涵盖学术文档处理的不同挑战领域。

Result: 这些任务促进了方法创新，并为更广泛的研究社区贡献了开放获取的数据集、模型和工具，这些资源已整合到联盟的研究数据基础设施中。

Conclusion: NFDI4DS联盟的共享任务成功推动了FAIR原则的实施，通过社区协作和标准化评估促进了学术文档处理领域的研究进步和资源共享。
...
TL;DR: MACC框架通过多轮自适应压缩CoT推理，利用token弹性现象，在保持精度的同时显著减少推理延迟和token长度。


<details>
  <summary>Details</summary>
Motivation: CoT推理虽然提升复杂任务性能，但因其冗长性导致推理延迟显著增加，需要高效的压缩方法。

Method: 提出多轮自适应CoT压缩框架，利用token弹性现象进行渐进式压缩，为每个输入确定最优压缩深度。

Result: 相比最先进基线方法，平均准确率提升5.6%，CoT长度平均减少47个token，显著降低延迟。

Conclusion: CoT压缩既有效又可预测，通过可解释特征可可靠预测测试时性能，无需重复微调即可实现高效模型选择和预测。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在创造力方面接近甚至超越人类，需要解决人类工作真实性和创造力保护的问题，以及机器生成文本的检测挑战。

Method: 引入BMAS English数据集，支持多种检测任务：文档级二元分类（人机区分）、多元分类（识别生成器）、句子级分割（人机协作文本边界识别）和对抗攻击检测。

Result: 提出了一个综合性的机器生成文本检测框架，能够应对不同场景下的检测需求，包括协作文本和对抗性规避的情况。

Conclusion: 该研究为机器生成文本检测领域提供了更全面的解决方案，有望在保护人类创造力和识别AI生成内容方面发挥重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：上下文学习在处理大量演示时效率低下，监督微调有训练开销且缺乏灵活性。需要一种能够整合多块信息的方法。

Method: 引入CompAs框架，将上下文翻译成具有组合结构的适配器参数。这些适配器可以代数合并，无需重新处理长提示。

Result: 在多项选择和抽取式问答任务上的实验表明，CompAs优于上下文学习和先前基于生成器的方法，特别是在扩展到更多输入时。

Conclusion: 可组合适配器生成为扩展LLM部署提供了实用高效的替代方案，支持信息可逆编码，便于安全和安全管理。
...
TL;DR: 通过合成数据蒸馏框架对指令微调(IFT)和推理模型进行大规模对比研究，发现推理能力能显著提升模型性能，在推理密集型任务上甚至能超越更大规模的IFT模型。


<details>
  <summary>Details</summary>
Motivation: 尽管具有推理能力的大语言模型在多种任务上取得了最佳性能，但推理能力在哪些任务和模型规模下有效，以及其训练和推理成本仍未被充分探索。

Method: 使用合成数据蒸馏框架进行大规模监督研究，比较不同规模的指令微调(IFT)和推理模型，在数学中心和通用任务上进行评估，包括多项选择和开放式格式。

Result: 推理能持续提升模型性能，通常能匹配或超越显著更大的IFT系统。IFT在训练和推理成本方面保持帕累托最优，但随着模型规模扩大，推理模型在推理密集型和开放式任务上变得更有价值。

Conclusion: 推理模型随着模型规模扩大而价值增加，能够克服IFT在推理密集型和开放式任务上的性能限制。
...
TL;DR: 本文论证大语言模型（LLM）的输出是无意义的，因为LLM缺乏产生意义所需的意图类型，即使其输出看似有意义且能传递知识。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型输出是否真正具有语义意义，挑战当前对LLM能力的普遍认知。

Method: 基于两个关键前提进行哲学论证：(a) 特定类型的意图是LLM输出具有字面意义的前提；(b) LLM无法拥有这类意图。

Result: 论证了LLM输出本质上是无意义的，但承认其输出在表面上显得有意义且能用于获取真信念和知识。

Conclusion: 尽管LLM输出缺乏真正的语义意义，但其表面意义和实用性仍然存在，这引发了关于意义本质的更深层哲学问题。
...
TL;DR: 提出了递归主题划分(RTP)框架，利用大语言模型构建可解释的二进制树结构，通过自然语言问题对文本进行语义分区，相比传统关键词主题模型具有更好的可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统主题模型在数据稀缺领域表现不佳，且通常用关键词列表描述聚类结果，需要大量人工解释工作且缺乏语义连贯性。

Method: 使用大语言模型交互式构建二进制树，每个节点都是自然语言问题，对数据进行语义分区，形成完全可解释的分类体系。

Result: RTP的问题驱动层次结构比BERTopic等基线方法的关键词主题更具可解释性，且这些聚类在下游分类任务中作为强大特征表现优异。
...
TL;DR: 本文提出了一种稳定的语音分词器StableToken，通过多分支架构和位级投票机制解决现有语义语音分词器对声学扰动的脆弱性问题，显著提高了分词稳定性并提升下游语音大模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有语义语音分词器对与语义无关的声学扰动非常脆弱，即使在信噪比很高、语音完全可懂的情况下，其输出分词序列也会发生剧烈变化，这增加了下游LLMs的学习负担。这种不稳定性源于脆弱的单路径量化架构和对中间分词稳定性不敏感的远程训练信号。

Method: 提出StableToken分词器，采用多分支架构并行处理音频，通过强大的位级投票机制合并这些表示，形成单一稳定的分词序列。这种共识驱动机制确保了分词的稳定性。

Result: StableToken在分词稳定性方面达到了新的最先进水平，在各种噪声条件下显著降低了单元编辑距离(UED)。这种基础稳定性直接转化为下游效益，显著提高了SpeechLLMs在各种任务上的鲁棒性。

Conclusion: StableToken通过共识驱动的多分支架构有效解决了语义语音分词器的脆弱性问题，为下游语音大模型提供了更稳定的基础，推动了语音处理系统的鲁棒性发展。
...
TL;DR: 提出了复合推理（CR）方法，让大语言模型能够动态组合多种推理风格（演绎、归纳、溯因等），在科学和医学问答任务中优于现有基线方法。
...
Method: 引入复合推理方法，使模型能够动态探索和组合多种推理风格，并根据领域特点自适应地强调适当的推理方式。

Result: 在科学和医学问答基准测试中，CR方法超越了Chain-of-Thought和DeepSeek-R1等基线方法，表现出更好的样本效率和适当的token使用。
...
TL;DR: 提出反向推测解码(RSD)方法，通过教师模型生成候选词但由学生模型基于自身概率分布决定接受，过滤低概率词来解决大模型到小模型推理能力迁移失败的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的大模型到小模型推理能力迁移方法往往失败，因为大模型的推理轨迹包含学生模型分布下概率较低的词，超出了小模型架构的内部表示能力。

Method: RSD机制：教师模型提出候选词，但由学生模型基于自身概率分布决定是否接受，过滤掉对学生模型来说概率低的词，生成学生友好的推理轨迹。

Result: 在Qwen3-0.6B上，直接蒸馏s1K-1.1推理轨迹数据使性能下降20.5%，而使用RSD生成的推理轨迹训练则提升4.9%。

Conclusion: 低概率词是推理能力迁移的关键瓶颈，RSD轨迹具有模型特异性，分布对齐需要针对每个学生架构的独特内部表示进行定制。
...
TL;DR: 提出了FeatBench基准测试，专门评估LLM在vibe coding范式下的功能实现能力，填补了现有代码生成基准测试的空白。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准测试与vibe coding范式不匹配，要么需要代码级规范，要么只关注问题解决，忽略了功能实现这一关键场景。

Method: 构建纯自然语言提示的基准测试，采用多级过滤管道确保质量，包含F2P和P2P测试用例，涵盖多样化应用领域。

Result: 评估显示vibe coding范式下的功能实现具有显著挑战性，最高成功率仅为29.94%，发现了"激进实现"策略的双重效应。

Conclusion: FeatBench为社区研究提供了重要工具，揭示了vibe coding的实际挑战和LLM在软件开发中的潜力与局限。
...
TL;DR: FLEXI是首个专门评估全双工语音交互LLM在紧急情况下中断能力的基准测试，揭示了开源模型与商业模型在紧急意识、轮次终止和交互延迟方面的显著差距。


<details>
  <summary>Details</summary>
Motivation: 全双工语音到语音LLM是自然人机交互的基础，但当前缺乏有效的基准测试和建模方法，特别是在紧急中断场景下的评估。

Method: 开发了FLEXI基准测试，通过六个多样化的人-LLM交互场景，系统评估实时对话的延迟、质量和对话效果，重点关注模型中断能力。

Result: 研究发现开源模型与商业模型在紧急意识、轮次终止和交互延迟方面存在显著差距，商业模型表现更好。
...
TL;DR: 该论文从法律合规角度解决LLM安全问题，提出安全合规概念，基于欧盟AI法案和GDPR构建新基准，并通过GRPO方法对齐Qwen3-8B模型，开发出Compliance Reasoner安全推理器。
...
Method: 1) 基于欧盟AI法案和GDPR构建安全合规基准；2) 使用GRPO方法对齐Qwen3-8B模型，开发Compliance Reasoner安全推理器。
...
TL;DR: SSKG-LLM是一个创新的模型架构，旨在将知识图谱的结构和语义信息有效整合到LLM的推理过程中，通过KGR、KGE和KGA模块解决LLM处理知识图谱时的结构信息利用不足和嵌入空间差异问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM处理幻觉问题的主要方法是结合知识图谱，但LLM通常将KGs视为纯文本，仅提取语义信息而忽略了关键的结构方面。另一个挑战是KGs编码器和LLM文本嵌入空间之间的差距，阻碍了结构化知识的有效整合。

Method: 提出SSKG-LLM架构，包含知识图谱检索(KGR)模块和知识图谱编码(KGE)模块来保留语义同时利用结构，然后加入知识图谱适应(KGA)模块使LLM能够理解KGs嵌入。

Result: 通过广泛实验和详细分析，探索了整合KGs结构信息如何增强LLM的事实推理能力。

Conclusion: SSKG-LLM能够有效整合知识图谱的结构和语义信息，提升LLM的事实推理能力，解决了现有方法在结构信息利用和嵌入空间对齐方面的局限性。
...
TL;DR: 本文首次系统研究了仇恨言论检测中可解释性与公平性的关系，发现基于输入的解释能有效识别偏见预测并作为训练监督减少偏见，但在模型选择方面不可靠。
...
Method: 在仇恨言论检测中系统研究可解释性与公平性的关系，关注编码器和仅解码器模型，考察三个关键维度：识别偏见预测、选择公平模型、在模型训练中缓解偏见。

Result: 基于输入的解释能有效检测偏见预测，并可作为有用的监督信号在训练过程中减少偏见，但在候选模型中选择公平模型方面不可靠。

Conclusion: 输入解释在检测偏见和训练缓解方面有效，但在模型公平性选择方面存在局限性，需要更可靠的方法来确保模型公平性。
...
TL;DR: 本文系统评估了微调LLMs将自然语言翻译为一阶逻辑(FOL)的性能，发现微调Flan-T5-XXL在提供谓词列表时达到70%准确率，优于GPT-4o、DeepSeek-R1-0528和符号系统ccg2lambda。
...
Result: 微调Flan-T5-XXL在提供谓词列表时达到70%准确率，比GPT-4o和DeepSeek-R1-0528表现更好；谓词可用性提升性能15-20%；T5模型优于更大的仅解码器LLMs；模型能泛化到未见过的逻辑论证(FOLIO数据集)。
...
TL;DR: 本文研究了不同规模Transformer模型学习有向图中传递关系（连通性）的能力，发现模型在低维网格状图上表现良好，但在高维网格图或包含多个不连通分量的图上表现较差。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer模型从训练样本中学习传递关系推理的能力，以及模型规模对这种能力的影响，这对于确保大语言模型事实正确性至关重要。

Method: 通过生成有向图来训练不同规模的Transformer模型，评估它们在不同图大小下推断传递关系的能力。

Result: Transformer能够学习低维网格状有向图的连通性，但高维网格图更具挑战性；模型规模增加能改善对网格图连通性的泛化能力；对于包含多个不连通分量的非网格图，模型学习困难。

Conclusion: Transformer学习传递关系的能力受图结构维度影响，低维网格图更容易学习，模型规模扩展有助于泛化，但复杂图结构仍具挑战。
...
TL;DR: 本文介绍了将NLP技术应用于历史研究的方法，特别是针对都铎王朝英格兰宗教改革时期的辱骂语言研究，构建了包含近2000个早期现代英语句子的InviTE语料库，并比较了微调BERT模型与零样本提示LLM在辱骂检测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 将自然语言处理技术应用于历史研究，特别是研究16世纪英格兰宗教改革时期的宗教辱骂语言，以支持历史学家进行文本分析。

Method: 构建了从原始数据到预处理的完整工作流程，包括数据选择、迭代标注过程，创建了InviTE语料库，并评估了微调BERT模型与零样本提示LLM在辱骂检测任务上的表现。

Result: 创建了包含近2000个早期现代英语句子的InviTE语料库，实验结果显示，在历史数据上预训练并针对辱骂检测任务微调的模型性能优于零样本提示的指令调优大语言模型。

Conclusion: 针对特定历史领域预训练并进行任务微调的模型在历史文本的辱骂语言检测任务上表现更优，为历史研究中的NLP应用提供了有效方法。
...
TL;DR: 本文探讨如何将贝叶斯概率方法应用于关联理论语用学，特别关注通过会话含义传递隐含意义的现象。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯概率理论和计算工具的发展引发了语用学和语义学的'概率转向'，作者希望探索如何将类似的贝叶斯方法应用于关联理论语用学。

Method: 采用贝叶斯概率框架，研究会话含义这一典型的语用现象，借鉴理性言语行为理论的方法。

Result: 提出了将贝叶斯方法应用于关联理论语用学的可能性，特别是在建模隐含意义传递方面。

Conclusion: 贝叶斯概率方法可以有效地应用于关联理论语用学，为理解会话含义等语用现象提供新的计算框架。
...
TL;DR: CHRONOBERG是一个跨越250年的英语书籍文本时间结构化语料库，用于支持语言历时变化分析和训练，揭示了现代LLM在理解语言语义历时演变方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有语料库缺乏长期时间结构，限制了LLM理解语言语义和规范历时演变的能力，需要专门的时间结构化语料库来支持历时语言变化研究。

Method: 从Project Gutenberg提取英语书籍文本，构建时间结构化语料库CHRONOBERG，通过时间敏感的VAD分析量化词汇语义变化，构建历史校准的情感词典。

Result: 展示了现代LLM工具在检测歧视性语言和跨时间情感语境化方面的不足，语言模型在CHRONOBERG上顺序训练时难以编码历时意义变化。

Conclusion: 需要时间感知的训练和评估流程，CHRONOBERG作为可扩展资源为语言变化和时间泛化研究提供了重要支持。
...
TL;DR: 本文提出了一种使用大型语言模型(LLMs)进行风轮机维护日志深度语义分析的框架，超越了传统文本分类，能够执行故障模式识别、因果链推理等复杂推理任务。


<details>
  <summary>Details</summary>
Motivation: 风轮机维护日志中的大量非结构化文本数据包含丰富的运维智能，但传统定量可靠性分析方法无法有效利用这些数据。现有机器学习方法通常只停留在文本分类层面。

Method: 引入探索性框架，使用LLMs进行深度语义分析，执行四个分析工作流：故障模式识别、因果链推理、比较站点分析和数据质量审计。

Result: 结果表明LLMs可以作为强大的"可靠性副驾驶"，超越标签分类，能够综合文本信息并生成可操作的专家级假设。

Conclusion: 这项工作贡献了一种新颖且可复现的方法论，将LLMs用作推理工具，为风能行业提供了一条增强运维智能的新途径，解锁了非结构化数据中先前被掩盖的洞察。
...
TL;DR: 分析OLMO2开源模型的训练数据政治偏见，发现左倾文档在训练数据中占主导地位，且训练数据中的主流政治立场与模型在政策问题上的偏见高度相关。
...
Method: 从OLMO2模型的完整训练数据集中抽取大样本，自动标注文档的政治倾向，分析来源领域和内容，并评估训练数据政治内容与模型政策立场的关系。

Result: 左倾文档在所有数据集中占主导；预训练语料比后训练数据包含更多政治参与内容；左右倾向文档通过不同价值观和合法性来源框架相似话题；训练数据中的主流立场与模型政治偏见高度相关。

Conclusion: 需要在未来数据管理流程中整合政治内容分析，并对过滤策略进行详细记录以确保透明度。
...
TL;DR: 本文介绍了Chimera测试套件，包含7,500个高质量图表，用于评估视觉语言模型对图表的真实理解能力，发现当前模型主要依赖捷径而非真正理解。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图表相关基准测试中表现良好，但其是否真正理解和推理图表内容存在疑问，可能依赖知识、推理或模态捷径。

Method: 构建Chimera测试套件，包含7,500个维基百科图表，标注语义三元组和多层次问题，评估实体识别、关系理解、知识基础和视觉推理四个维度，检测三种捷径行为。

Result: 评估15个开源视觉语言模型发现，其强表现主要来自捷径行为：视觉记忆捷径影响轻微，知识回忆捷径作用中等，Clever-Hans捷径贡献显著。

Conclusion: 当前视觉语言模型在图表理解方面存在严重局限性，需要更鲁棒的评估协议来基准化对复杂视觉输入的真实理解能力。
...
TL;DR: 提出一种通过激活空间方向检测LLM不可回答问题的方法，在抽取式问答任务中有效识别无答案问题，并能泛化到其他类型的不可回答场景。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常在缺乏足够信息时仍自信回答，导致幻觉答案。需要解决不可回答性检测问题，特别是在抽取式问答中判断文本是否包含足够信息来回答问题。

Method: 通过激活加法在推理过程中识别模型激活空间中捕捉不可回答性的方向，并将隐藏激活投影到该方向来获得可靠的不可回答性分类分数。

Result: 在两个开源LLM和四个抽取式问答基准测试中，该方法有效检测不可回答问题，在跨数据集泛化方面优于现有提示和分类器方法。该方向还能扩展到缺乏科学共识和主观性等其他不可回答场景。

Conclusion: 通过激活空间方向控制可以有效调节模型的弃权行为，为LLM的不可回答性检测提供了一种简单有效的方法。
...
TL;DR: 评估LLaMA和Gemini在多语言法律任务中的表现，发现法律任务对LLM构成重大挑战，准确率通常低于50%，而通用任务可达70%以上。LLM在法律应用中存在提示敏感性和对抗脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 在LLM主导的时代，了解其在法律等高风险领域的能力和局限性至关重要。当前LLM在多语言、跨司法管辖区和对抗性环境中的表现尚未充分探索。

Method: 使用LLM-as-a-Judge方法进行人类对齐评估，在多语言法律和非法律基准上评估LLaMA和Gemini，通过字符和词级扰动测试其对抗鲁棒性，并开发开源模块化评估管道。

Result: 法律任务准确率通常低于50%（如LEXam基准），而通用任务如XNLI可达70%以上。Gemini平均比LLaMA高约24个百分点。性能与语言和英语的句法相似性相关，英语结果更稳定但不总是更准确。

Conclusion: 尽管新版LLM有所改进，但在关键的多语言法律应用中可靠部署仍面临挑战，提示敏感性和对抗脆弱性在跨语言中持续存在。
...
TL;DR: 提出NeLLCom-Lex神经代理框架，通过模拟代理在真实词汇系统中的沟通需求来研究语义变化机制，使用颜色命名任务验证框架有效性。


<details>
  <summary>Details</summary>
Motivation: 传统观察方法无法揭示语义变化的因果机制，而人类实验难以应用于历时语义变化研究，需要新的模拟框架来探索语义变化的内在机制。

Method: 开发NeLLCom-Lex神经代理框架，让代理在真实词汇系统中进行沟通，系统操纵其沟通需求，使用颜色命名任务进行模拟实验，比较不同监督学习和强化学习管道的效果。

Result: 实验表明，经过训练能够'说'现有语言的神经代理能够显著程度地复现人类颜色命名模式，支持NeLLCom-Lex框架在阐明语义变化机制方面的进一步应用。

Conclusion: NeLLCom-Lex框架为研究语义变化提供了有效的模拟工具，神经代理能够复现人类语言行为模式，为理解语义变化机制开辟了新途径。
...
TL;DR: 论文提出解决方案分歧作为衡量LLM问题解决能力的新指标，并证明其在监督微调和强化学习中能有效提升性能


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖带标签数据的监督微调或任务反馈的强化学习，本文从LLM对同一问题生成解决方案的差异性这一新视角进行研究

Method: 通过分析不同模型生成解决方案的差异程度，提出解决方案分歧作为新指标，并在三个代表性问题领域测试其在SFT和RL中的应用

Result: 实验表明解决方案分歧与问题解决能力呈正相关，使用该指标能持续提升成功率
...
TL;DR: JGU Mainz团队针对WMT25有限资源下斯拉夫语言机器翻译和问答任务，使用Qwen2.5-3B-Instruct模型进行联合微调，在乌克兰语、上索布语和下索布语上取得优于基线的表现。


<details>
  <summary>Details</summary>
Motivation: 解决有限资源条件下斯拉夫语言（乌克兰语、上索布语、下索布语）的机器翻译和问答任务挑战。

Method: 使用Qwen2.5-3B-Instruct模型进行参数高效微调，集成额外翻译和多选题问答数据，对乌克兰语QA使用检索增强生成，对上索布语和下索布语QA应用集成方法。

Result: 实验表明，所提模型在两个任务上都优于基线方法。

Conclusion: 通过联合微调和特定技术改进，在有限资源条件下有效提升了斯拉夫语言的机器翻译和问答性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在可扩展性有限、需要昂贵重新训练、表示空间难以解释的问题，需要一种更高效且可解释的模型选择方法。

Method: 利用几何特性的闭式计算，将LLMs表示为提示语义任务空间中的线性算子，无需训练即可获得模型表示。
...
Result: 在多个7B LLM上，AMBS相比朴素1-to-N基线平均对齐分数提升32.4%，不安全输出减少11.0%，与SOTA方法竞争力相当。

Conclusion: AMBS能够统一高效地实现多目标对齐，在保持跨目标一致性的同时提升整体对齐性能。
...
TL;DR: 提出了一个端到端的FP8训练方案，通过细粒度混合粒度量化策略，在保持数值精度的同时显著提升训练效率，证明FP8可以作为BF16的实用替代方案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练的巨大计算成本阻碍了创新，虽然FP8训练具有显著的理论效率优势，但缺乏开源训练方案限制了其广泛应用。

Method: 采用端到端FP8训练方案，结合持续预训练和监督微调，使用细粒度混合粒度量化策略来平衡数值保真度和计算效率。

Result: 在160B token语料上的持续预训练实验表明，该方案不仅非常稳定且基本无损，在推理基准测试中性能与BF16基线相当，同时训练时间减少22%，峰值内存使用降低14%，吞吐量提升19%。
...
TL;DR: 该论文提出了认知推理范式CogFlow，通过模拟人类社交认知过程来增强LLMs在社交情境中的推理能力，使用树状规划生成认知流数据，并通过监督微调和强化学习优化模型表现。


<details>
  <summary>Details</summary>
Motivation: 传统逻辑推理范式在处理社交情境时存在局限，因为社交情境涉及模糊线索的解读过程，很少产生明确结果。需要模拟人类社交认知的推理方式。

Method: 提出CogFlow框架：1）通过树状规划模拟人类思维的联想性和渐进性，生成认知流数据集；2）通过监督微调建立基本认知推理能力；3）采用强化学习，通过多目标奖励优化认知流和响应质量。

Result: 实验表明CogFlow有效增强了LLMs和人类的社交认知能力，带来了更有效的社交决策。

Conclusion: 认知推理范式成功弥补了传统逻辑推理在社交情境中的不足，CogFlow框架为LLMs提供了更接近人类社交认知的推理能力。
...
TL;DR: 开发了一个基于检索增强的评估管道(RAEC)，通过使用历史消息-响应对来改进LLM生成的患者消息回复的质量评估，提高临床错误检测的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: EHR门户中的异步患者-临床医生消息增加了临床医生工作量，LLM可用于起草回复，但其输出可能存在临床不准确、遗漏或语气不匹配等问题，需要稳健的评估方法。

Method: 提出了三方面贡献：(1)开发包含5个领域59个细粒度错误代码的临床错误本体；(2)构建检索增强评估管道(RAEC)，利用语义相似的历史消息-响应对；(3)使用DSPy的两阶段提示架构进行可扩展、可解释的分层错误检测。

Result: 在1500多条患者消息上测试，检索上下文改进了临床完整性和工作流程适当性等领域的错误识别。人工验证100条消息显示，上下文增强标签在一致性(50% vs 33%)和性能(F1=0.500 vs 0.256)上均优于基线。

Conclusion: RAEC管道可作为患者消息的AI护栏，通过参考历史消息-响应对显著提高LLM生成回复的质量评估效果。
...
TL;DR: 该论文研究了使用LLMs定位上下文幻觉（模型输出中无法从源文本验证的信息），构建了专门的基准测试并评估了四种大型LLMs的表现，发现最佳模型F1分数仅为0.67，揭示了LLMs在此任务中的主要挑战。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在定位上下文幻觉方面的适用性，作为现有复杂评估流程的更实用替代方案，并解决缺乏专门基准测试的问题。

Method: 构建了针对LLMs的基准测试，包含1000多个经过人工标注的示例；提出了基于自由形式文本描述的新表示方法；评估了四种大型LLMs；分析了最佳提示策略。

Result: 基准测试具有挑战性，最佳模型F1分数仅为0.67；LLMs存在两个主要问题：错误地将缺失细节标记为不一致，以及难以处理包含模型参数知识但源文本中不存在的事实。

Conclusion: LLMs在定位上下文幻觉方面能力有限，需要改进提示策略和解决模型参数知识干扰问题；提出的新表示方法能更好地捕捉各种可能的错误类型。
...
<details>
  <summary>Details</summary>
Motivation: 收集阿拉伯语招聘广告数据集，以捕捉阿拉伯劳动力市场中的语言、区域和社会经济变化，为公平性阿拉伯NLP和劳动力市场研究提供资源。

Method: 从四个阿拉伯国家收集8500多个招聘广告，构建包含55万+单词的数据集，并应用大语言模型进行薪资估计、职位分类等任务分析。

Result: 数据集成功捕获了性别代表性、职业结构和方言变化，为性别偏见检测和职业分类等基准任务提供了基础。

Conclusion: ArabJobs数据集对公平性阿拉伯NLP和劳动力市场研究具有重要价值，已公开在GitHub上供研究使用。
...
Method: 使用判定谓词表示次正则语言类，通过合成实验验证无噪声条件下的完美可分性，并在英语形态学上进行真实数据实验。
...
TL;DR: 该研究比较了多种NLP技术在模拟审议过程中的表现，开发了基于频率的话语调制和量子审议框架两种模型，在意见转变预测方面优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 随着NLP技术的发展，需要评估模型如何有效解释审议性话语并产生有意义的见解，以支持决策过程。

Method: 收集不同背景个体的意见构建数据集，通过富含显著事实的产品演示模拟审议过程，比较频率基话语调制和量子审议框架两种模型。

Result: 开发的两种模型在预测意见转变方面优于现有最先进模型，能够有效捕捉审议过程中的意见变化。

Conclusion: 该研究展示了NLP模型在公共政策制定、辩论评估、决策支持框架和大规模社交媒体意见挖掘中的实际应用价值。
...
TL;DR: 本文提出了一套基于重采样的方法，用于量化多语言和/或多任务NLP基准测试中评估指标的不确定性和统计精度。


<details>
  <summary>Details</summary>
Motivation: 实验性能分数的变化既来自模型相关因素，也来自数据相关因素，需要同时考虑两者以避免显著低估整体变异性。

Method: 使用重采样方法，包括考虑模型和数据来源的变异性，应用于多语言问答、机器翻译和命名实体识别等任务。

Result: 重采样方法可用于计算排行榜中各种统计量的抽样分布，如平均值/中位数、模型间成对差异和排名。

Conclusion: 重采样方法是量化NLP基准测试中评估指标不确定性的有效工具，能更准确地反映性能分数的整体变异性。
...
TL;DR: StateX是一种训练流程，通过后训练高效扩展预训练循环神经网络的状态大小，提升长上下文信息召回能力，而不显著增加模型参数或训练成本。


<details>
  <summary>Details</summary>
Motivation: Transformer模型处理长上下文成本高，而循环神经网络(RNN)如线性注意力和状态空间模型虽然复杂度低，但状态大小固定导致长上下文信息召回能力不足。直接训练更大状态的RNN成本高昂。

Method: 针对线性注意力和状态空间模型两类RNN，设计了后训练架构修改，在不增加或仅轻微增加模型参数的情况下扩展状态大小。

Result: 在1.3B参数模型上的实验表明，StateX能高效提升RNN的召回能力和上下文学习能力，且不产生高后训练成本或损害其他能力。

Conclusion: StateX提供了一种高效扩展RNN状态大小的方法，解决了RNN在长上下文处理中的局限性，同时保持了低复杂度的优势。
...
<details>
  <summary>Details</summary>
Motivation: 为语言模型的推理过程提供原则性的概率视角，通过变分推理方法稳定训练目标，改进模型的推理能力。

Method: 从证据下界(ELBO)出发，扩展到多轨迹目标以获得更紧的边界，提出前向KL公式化来稳定变分后验的训练，并将拒绝采样微调和二元奖励RL解释为局部前向KL目标。

Result: 在Qwen 2.5和Qwen 3模型系列上进行了广泛的推理任务验证，方法有效提升了语言模型的推理能力。

Conclusion: 该工作提供了一个统一的概率视角，将变分推理与强化学习方法相结合，为改进语言模型推理能力提供了稳定的目标函数。
...
Method: FCP从响应-反馈对中学习，通过最大似然训练近似反馈条件后验，并开发在线自举阶段让策略在正向条件下生成并接收新反馈来优化自身。

Result: 将反馈驱动学习重新定义为条件生成而非奖励优化，为LLM直接从语言反馈中学习提供了更富表达力的方法。

Conclusion: FCP通过将语言反馈作为条件信号，提供了一种比传统标量奖励方法更有效和丰富的LLM学习框架。
...
TL;DR: 该论文研究发现，虽然n-gram新颖性与专家评定的创造力正相关，但约91%的n-gram新颖性最高的表达未被判定为有创造力，警告不应仅依赖n-gram新颖性评估创造力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨n-gram新颖性作为创造力评估指标的充分性，因为创造力理论表明创造力具有双重性质：新颖性和适当性，而n-gram新颖性未能考虑适当性。

Method: 通过7542个专家写手注释（n=26）对人类和AI生成文本的新颖性、实用性和合理性进行细读分析，并测试零样本、少样本和微调模型识别创造性表达和非实用表达的能力。

Result: 研究发现n-gram新颖性与专家评定的创造力正相关，但91%的n-gram新颖性最高表达未被判定为有创造力。开源LLM中更高的n-gram新颖性与更低的实用性相关。前沿闭源模型产生创造性表达的可能性低于人类。

Conclusion: 不应仅依赖n-gram新颖性评估创造力，需要同时考虑新颖性和适当性。前沿LLM在识别创造性表达方面表现优于随机但仍有改进空间，特别是在识别非实用表达方面。最佳模型的LLM-as-a-Judge新颖性评分可预测专家写手偏好。
...
TL;DR: WebGen-Agent是一个创新的网站生成代理系统，通过多级视觉反馈机制迭代生成和优化网站代码，结合视觉语言模型和GUI代理测试来量化网站质量，并引入Step-GRPO训练方法提升LLM的网站生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的代码代理在网站代码生成任务中仅依赖简单的代码执行反馈，无法准确评估生成代码的实际质量，特别是对于依赖视觉效果和用户交互的网站生成任务。

Method: 提出WebGen-Agent系统，利用视觉语言模型分析网站截图生成详细文本描述和建议，结合GUI代理测试量化网站质量，采用回溯和最优选择机制，并引入Step-GRPO训练方法使用截图和GUI代理分数作为过程监督奖励。
...
TL;DR: 提出了VoiceAssistant-Eval基准测试，用于全面评估AI语音助手在听、说、看三个维度的能力，包含10,497个样本和13个任务类别。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法充分评估大型语言模型和多模态系统的完整能力，特别是语音优先AI助手的能力范围。

Method: 构建包含自然声音、音乐、对话等听力任务，多轮对话、角色扮演等说话任务，以及异质图像观看任务的综合基准测试。评估了21个开源模型和GPT-4o-Audio。

Result: 发现专有模型并非全面优于开源模型；多数模型擅长说话任务但在音频理解方面滞后；设计良好的小模型可与大模型竞争。Step-Audio-2-mini在听力准确率上超过LLaMA-Omni2-32B-Bilingual两倍以上。

Conclusion: 多模态输入和角色扮演语音模仿仍是当前模型的挑战，在鲁棒性和安全对齐方面存在显著差距。VoiceAssistant-Eval为下一代AI助手的发展提供了评估框架。
...
### [269] [ControlHair: Physically-based Video Diffusion for Controllable Dynamic Hair Rendering](https://arxiv.org/abs/2509.21541)
*Weikai Lin,Haoxiang Li,Yuhao Zhu*

Main category: cs.GR

TL;DR: ControlHair是一个混合框架，将物理模拟器与条件视频扩散相结合，实现可控的动态头发渲染。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型缺乏对头发动态的细粒度控制，而头发模拟和渲染面临复杂的动力学、材料特性和光线交互等挑战。

Method: 采用三阶段流程：首先用物理模拟器将物理参数编码为逐帧几何，然后提取逐帧控制信号，最后将控制信号输入视频扩散模型生成具有所需头发动态的视频。

Result: 在精心策划的10K视频数据集上训练，ControlHair在头发动态控制方面优于基于文本和姿态的基线方法。
...
### [270] [PowerGS: Display-Rendering Power Co-Optimization for Neural Rendering in Power-Constrained XR Systems](https://arxiv.org/abs/2509.21702)
*Weikai Lin,Sushant Kondguli,Carl Marshall,Yuhao Zhu*

Main category: cs.GR

TL;DR: PowerGS是一个优化3D高斯泼溅在XR设备上功耗的框架，通过联合最小化渲染和显示功耗，在保持质量约束下实现高达86%的功耗降低。


<details>
  <summary>Details</summary>
Motivation: 3DGS系列模型在功耗受限的XR设备上效率不足，这些设备需要在瓦特级别运行，因此需要优化功耗。

Method: 提出通用问题表述，通过识别显示和渲染功耗景观中的等质量曲线，并找到给定曲线上功耗最小点（在适当参数化下具有闭式解），同时支持注视点渲染以进一步节省功耗。

Result: 实验和用户研究表明，与最先进的3DGS模型相比，PowerGS实现了高达86%的总功耗降低，主观和客观质量损失最小。

Conclusion: PowerGS是第一个在质量约束下联合最小化3DGS渲染和显示功耗的框架，显著提高了XR设备的能效。
...
### [271] [Rigidity-Aware 3D Gaussian Deformation from a Single Image](https://arxiv.org/abs/2509.22222)
*Jinhyeok Kim,Jaehun Bang,Seunghyun Seo,Kyungdon Joo*

Main category: cs.GR

TL;DR: DeformSplat：从单张图像重建3D物体变形的新框架，通过高斯到像素匹配和刚性部分分割技术实现


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖多视角视频来恢复变形，在受限场景下应用受限，需要从单张图像重建物体变形的解决方案

Method: 提出Gaussian-to-Pixel Matching桥接3D高斯表示与2D像素观测的域差距，以及Rigid Part Segmentation识别刚性区域保持几何一致性

Result: 实验表明该方法显著优于现有方法，并能自然扩展到帧插值和交互式物体操作等应用

Conclusion: DeformSplat能够仅从单张图像重建一致的变形，为受限场景下的变形重建提供了有效解决方案
...
### [272] [Aerial Path Planning for Urban Geometry and Texture Co-Capture](https://arxiv.org/abs/2509.22227)
*Weidan Xiong,Bochuan Zeng,Ziyu Hu,Jianwei Guo,Ke Xie,Hui Huang*

Main category: cs.GR

TL;DR: 提出一种创新的无人机路径规划框架，用于同时捕获城市场景的几何结构和高质量纹理，通过多目标优化和序列路径规划算法实现低成本的逼真纹理场景重建。


<details>
  <summary>Details</summary>
Motivation: 当前城市场景捕获技术往往忽视纹理质量的重要性，导致纹理模型出现明显视觉伪影。需要在站点访问前有限先验知识下，解决城市几何和纹理协同捕获问题。

Method: 生成高质量垂直俯视和水平平面视图，提出多目标优化策略联合最大化纹理保真度、提高几何精度并最小化航拍成本，采用考虑纹理一致性的序列路径规划算法。

Result: 在大规模合成和真实城市数据集上的广泛实验表明，该方法能有效生成适用于同时进行几何和纹理重建的图像集。

Conclusion: 该方法能够以低操作成本创建逼真的纹理场景代理，解决了城市几何和纹理协同捕获的关键问题。
...
### [273] [Learning to Ball: Composing Policies for Long-Horizon Basketball Moves](https://arxiv.org/abs/2509.22442)
*Pei Xu,Zhen Wu,Ruocheng Wang,Vishnu Sarukkai,Kayvon Fatahalian,Ioannis Karamouzas,Victor Zordan,C. Karen Liu*

Main category: cs.GR

TL;DR: 提出了一种新的策略集成框架，用于在多阶段长视野任务中组合截然不同的运动技能，通过高层软路由器实现子任务间的无缝过渡。


<details>
  <summary>Details</summary>
Motivation: 解决多阶段长视野任务中策略组合和技能过渡的挑战，特别是在中间状态定义不明确的情况下，现有方法如专家混合和技能链难以处理不同阶段间缺乏共同探索状态或明确初始/终止状态的问题。

Method: 引入策略集成框架来组合多阶段长视野任务中截然不同的运动技能，并进一步提出高层软路由器来实现子任务间的无缝和鲁棒过渡。

Result: 在基础篮球技能和具有挑战性的过渡任务上评估，训练出的策略能有效控制模拟角色与球互动，完成由实时用户指令指定的长视野任务，无需依赖球轨迹参考。

Conclusion: 该框架成功解决了多阶段长视野任务中策略组合和过渡的挑战，特别是在中间状态定义不明确的情况下，实现了不同运动技能的有效集成。
...
### [274] [Towards mitigating information leakage when evaluating safety monitors](https://arxiv.org/abs/2509.21344)
*Gerard Boxo,Aman Neelappa,Shivam Raval*

Main category: cs.AI

TL;DR: 提出了一个系统框架来评估白盒监控器的真实性能，通过内容过滤、分数过滤和微调模型生物三种策略来消除提示泄露对监控器评估的影响。


<details>
  <summary>Details</summary>
Motivation: 白盒监控器在检测大语言模型有害行为方面有优势，但现有评估方法存在提示信息泄露问题，会夸大监控器的实际性能。

Method: 使用内容过滤（移除欺骗相关文本）、分数过滤（仅聚合任务相关token）、微调模型生物（无需显式提示即可展现欺骗行为）三种策略来评估监控器。

Result: 内容过滤可平滑移除提示信号，使AUROC下降30%；分数过滤使AUROC下降15%；微调模型生物使监控器性能下降高达40%。

Conclusion: 现有监控器评估方法存在严重的提示泄露问题，需要更严格的评估框架来确保监控器检测的是真实模型行为而非提示伪影。
...
### [275] [Correct Reasoning Paths Visit Shared Decision Pivots](https://arxiv.org/abs/2509.21549)
*Dongkyu Cho,Amy B. Z. Zhang,Bilel Fehri,Sheng Wang,Rumi Chunara,Rui Song,Hengrui Cai*

Main category: cs.AI

TL;DR: 提出决策枢纽概念，通过自训练管道验证和压缩思维链推理，无需真实推理数据或外部指标即可对齐推理过程。
...
Method: 引入决策枢纽概念，提出自训练管道：(1)采样多样化推理路径并挖掘共享决策枢纽；(2)使用辅助验证器将每个轨迹压缩为枢纽聚焦的短路径推理；(3)使用自生成输出进行后训练。
...
### [276] [AutoClimDS: Climate Data Science Agentic AI -- A Knowledge Graph is All You Need](https://arxiv.org/abs/2509.21553)
*Ahmed Jaber,Wangshu Zhu,Karthick Jayavelu,Justin Downes,Sameer Mohamed,Candace Agonafir,Linnia Hawkins,Tian Zheng*

Main category: cs.AI

TL;DR: 提出通过知识图谱与AI代理的集成来解决气候数据科学中的数据碎片化、格式异构和技术门槛高等问题，降低非专业用户参与气候数据分析的技术门槛。


<details>
  <summary>Details</summary>
Motivation: 气候数据科学面临数据源碎片化、格式异构以及技术门槛高等挑战，限制了参与度、减缓了发现速度并降低了科学工作流的可重复性。

Method: 将精心策划的知识图谱与为云原生科学工作流设计的AI代理相结合，知识图谱提供统一的数据集、工具和工作流组织层，AI代理通过生成式AI服务实现自然语言交互、自动化数据访问和简化分析。

Result: 该系统显著降低了参与气候数据科学的技术门槛，使非专业用户能够识别和分析相关数据集，展示了可扩展和自主的科学探究工作流。
...
### [277] [EEG-Based Consumer Behaviour Prediction: An Exploration from Classical Machine Learning to Graph Neural Networks](https://arxiv.org/abs/2509.21567)
...
Result: 总体结果没有显著差异，但GNN模型在一些传统模型表现不佳的基本标准上表现更好。

Conclusion: EEG信号分析与机器学习模型结合可以更深入理解消费者行为，研究提供了传统模型（如SVM）与较少使用的GNN模型在神经营销领域的全面比较。
...
### [278] [GeoEvolve: Automating Geospatial Model Discovery via Multi-Agent Large Language Models](https://arxiv.org/abs/2509.21593)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的算法发现框架缺乏领域知识和多步推理能力，难以解决复杂的地理空间问题，需要开发能够注入地理空间理论知识的自动化算法设计框架。

Method: 采用两层嵌套循环结构：内环使用代码进化器生成和变异候选解，外环通过智能体控制器评估全局精英，并查询GeoKnowRAG模块（结构化地理空间知识库）来注入地理学理论先验，实现知识引导的进化搜索。

Result: 在空间插值（克里金法）和空间不确定性量化（地理空间保形预测）两个基准任务中，GeoEvolve自动改进和发现了新算法，将空间插值误差（RMSE）降低了13-21%，不确定性估计性能提升了17%。

Conclusion: GeoEvolve为自动化、知识驱动的地理空间建模提供了可扩展路径，为可信赖和高效的AI-for-Science发现开辟了新机遇，领域引导的检索对于稳定、高质量的进化至关重要。
...
### [279] [Automated and Interpretable Survival Analysis from Multimodal Data](https://arxiv.org/abs/2509.21600)
*Mafalda Malafaia,Peter A. N. Bosman,Coen Rasch,Tanja Alderliesten*

Main category: cs.AI

TL;DR: 提出可解释多模态AI框架MultiFIX，整合临床变量和CT影像进行生存分析，通过深度学习提取特征并用Grad-CAM和遗传编程解释，使用透明Cox回归进行风险估计和分层。


<details>
  <summary>Details</summary>
Motivation: 精准且可解释的生存分析是肿瘤学核心挑战，随着多模态数据增长和临床对透明模型验证与信任的需求，该挑战日益复杂。

Method: MultiFIX框架使用深度学习推断生存相关特征：影像特征通过Grad-CAM解释，临床变量通过遗传编程建模为符号表达式，风险估计采用透明Cox回归进行分层。

Result: 在头颈癌RADCURE数据集上，MultiFIX获得C-index 0.838（预测）和0.826（分层），优于临床和学术基线方法，与已知预后标志物一致。

Conclusion: 结果凸显了可解释多模态AI在精准肿瘤学中的前景，MultiFIX框架具有重要应用价值。
...
### [280] [Semantic F1 Scores: Fair Evaluation Under Fuzzy Class Boundaries](https://arxiv.org/abs/2509.21633)
...
<details>
  <summary>Details</summary>
Motivation: 传统F1指标将语义相关的预测视为完全失败，无法反映类别边界模糊或人类标注不一致的现实情况。需要更公平的评估方法，承认类别重叠、标注者分歧以及相似预测导致相似结果的事实。

Method: 引入标签相似度矩阵来计算软精确率和召回率，采用新颖的两步精确率-召回率公式，能够比较任意大小的标签集，无需丢弃标签或强制匹配不相似的标签。

Result: 通过理论论证和在合成数据及真实数据上的广泛实证验证，Semantic F1显示出更好的可解释性和生态效度。该方法对相似度矩阵的误设具有鲁棒性。

Conclusion: Semantic F1通过为语义相关但不完全相同的标签给予部分信用，提供了更公平的评估，适用于各种任务和模态，只需要领域适当的相似度矩阵而非严格的分类体系。
...
### [281] [Can AI Perceive Physical Danger and Intervene?](https://arxiv.org/abs/2509.21651)
*Abhishek Jindal,Dmitry Kalashnikov,Oscar Chang,Divya Garikapati,Anirudha Majumdar,Pierre Sermanet,Vikas Sindhwani*

Main category: cs.AI

TL;DR: 提出了一个用于评估具身AI系统物理安全性的可扩展基准，通过生成真实世界伤害场景的逼真图像和视频来测试基础模型的安全感知能力，并开发了后训练方法提升模型的安全推理透明度。


<details>
  <summary>Details</summary>
Motivation: 当AI与物理世界交互时，存在直接的物理伤害风险，需要评估现有基础模型对物理安全常识的理解能力，以确保其在安全关键应用中的部署准备度。

Method: 1) 基于真实伤害叙述和操作安全约束开发可扩展的物理安全基准；2) 使用生成模型创建从安全到不安全状态过渡的逼真图像和视频；3) 分析主要基础模型的风险感知、安全推理和干预触发能力；4) 开发后训练范式，通过系统指令教授模型显式推理具体安全约束。

Result: 开发的后训练模型生成可解释的安全推理轨迹，在约束满足评估中达到最先进性能，为具身AI系统的安全部署提供了重要工具。

Conclusion: 该研究为具身AI系统的物理安全评估提供了系统化方法，揭示了基础模型在安全关键应用中的局限性，并通过可解释的安全推理机制提升了部署安全性。
...
### [282] [Align2Speak: Improving TTS for Low Resource Languages via ASR-Guided Online Preference Optimization](https://arxiv.org/abs/2509.21718)
...
TL;DR: 提出基于GRPO的框架，通过多语言预训练、有限配对数据微调和GRPO优化，实现低资源语言的高质量TTS系统，显著优于仅微调方法。


<details>
  <summary>Details</summary>
Motivation: 低资源语言的TTS系统开发面临配对文本-语音数据稀缺的挑战，而ASR模型相对更易获得。

Method: 1) 使用IPA标记训练多语言TTS基线；2) 在有限配对数据上微调；3) 应用GRPO，仅使用非配对文本和说话人提示，通过预训练ASR、说话人验证和音频质量评估模型的多目标奖励进行优化。

Result: 在低资源语言中生成清晰且说话人一致的语音，显著优于仅微调方法；在高资源语言中也超越DPO等离线对齐方法，在清晰度、说话人相似性和音频质量方面表现更优。

Conclusion: GRPO框架能有效利用ASR等现有模型，在低资源和高资源语言中均实现高质量的TTS合成。
...
### [283] [Retrieval-of-Thought: Efficient Reasoning via Reusing Thoughts](https://arxiv.org/abs/2509.21743)
*Ammar Ahmed,Azal Ahmad Khan,Ayaan Ahmad,Sheng Di,Zirui Liu,Ali Anwar*

Main category: cs.AI

TL;DR: 提出Retrieval-of-Thought (RoT)方法，通过检索和重用先前的推理步骤来构建动态模板，减少推理模型生成冗余内容，显著提升效率。
...
Method: RoT将推理步骤组织成带有顺序和语义边的思想图，通过检索查询相关节点和应用奖励引导遍历来组装问题特定的模板。

Result: 在多个模型和推理基准测试中，RoT将输出token减少高达40%，推理延迟降低82%，成本降低59%，同时保持准确性。
...
### [284] [Lifelong Learning with Behavior Consolidation for Vehicle Routing](https://arxiv.org/abs/2509.21765)
*Jiyuan Pei,Yi Mei,Jialin Liu,Mengjie Zhang,Xin Yao*

Main category: cs.AI

TL;DR: 该论文提出了一种用于神经VRP求解器的终身学习框架LLR-BC，通过行为整合方法解决多任务学习中的灾难性遗忘问题，同时保持模型的适应性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有神经求解器通常基于预定义问题分布和规模进行一次性训练，当新任务出现时，要么依赖零样本泛化（效果不佳），要么通过微调导致灾难性遗忘。需要一种能够持续学习多个任务而不遗忘先前知识的终身学习范式。

Method: 提出LLR-BC框架，通过行为整合方法将新任务训练的求解器行为与缓冲区中的先前行为进行对齐，采用决策寻求方式，并为低置信度决策分配更大的整合权重以关注关键经验。

Result: 在容量约束车辆路径问题和旅行商问题上的大量实验表明，LLR-BC在终身学习设置下能有效训练高性能神经求解器，解决灾难性遗忘问题，保持模型可塑性，并提升零样本泛化能力。

Conclusion: LLR-BC为神经VRP求解器提供了一种有效的终身学习解决方案，能够在多个任务中持续学习而不遗忘先前知识，同时保持模型的适应性和泛化性能。
...
### [285] [UltraHorizon: Benchmarking Agent Capabilities in Ultra Long-Horizon Scenarios](https://arxiv.org/abs/2509.21766)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估大多关注短时程、完全可观测任务，而现实世界中的关键任务（如软件开发、投资、科学发现）通常涉及长时程和部分可观测场景，需要系统性的评估基准。

Method: 通过探索作为统一任务，在三个不同环境中设计长时程发现任务，智能体需要通过持续推理、规划、记忆和工具管理来迭代发现隐藏规则。

Result: 在最大规模设置下，轨迹平均超过20万token和400次工具调用；标准配置下也超过3.5万token和60次工具调用。LLM智能体表现不佳，而人类参与者得分更高，表明智能体在长时程能力上存在差距。

Conclusion: 简单扩展方法在当前任务中失败，分析发现错误主要源于上下文锁定和功能基础能力差距两个主要原因。
...
### [286] [Benchmarking MLLM-based Web Understanding: Reasoning, Robustness and Safety](https://arxiv.org/abs/2509.21782)
*Junliang Liu,Jingyu Xiao,Wenxin Tang,Wenxuan Wang,Zhixian Wang,Minrui Zhang,Shuanghe Yu*

Main category: cs.AI

TL;DR: 提出了WebRSSBench基准，全面评估多模态大语言模型在网页理解中的推理、鲁棒性和安全性能力，包含8个任务和3799个问答对。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注视觉感知或UI代码生成，缺乏对构建端到端网页应用所需的推理、鲁棒性和安全性能力的评估。

Method: 从729个网站构建包含8个任务的基准，采用标准化提示、确定性评估脚本和多阶段质量控制方法。

Result: 评估12个MLLM发现模型在组合推理、跨元素推理、面对界面扰动时的鲁棒性以及识别安全关键操作方面存在显著差距。

Conclusion: 需要开发更强大的MLLM来应对现实网页应用中的复杂推理、鲁棒性和安全性挑战。
...
### [287] [D-Artemis: A Deliberative Cognitive Framework for Mobile GUI Multi-Agents](https://arxiv.org/abs/2509.21799)
...
TL;DR: D-Artemis是一个基于人类认知循环（思考、对齐、反思）的GUI代理框架，通过应用特定提示检索、执行前对齐和执行后状态反思来提升GUI任务性能，无需复杂轨迹数据集训练。


<details>
  <summary>Details</summary>
Motivation: 解决当前GUI代理面临的数据瓶颈、延迟错误检测成本高和矛盾指导风险等关键挑战。

Method: 采用精细化的应用特定提示检索机制，执行前对齐阶段包含思想-行动一致性检查和行动修正代理，执行后状态反思代理完成认知循环。

Result: 在AndroidWorld上达到75.8%的成功率，在ScreenSpot-V2上达到96.8%的成功率，创下新的SOTA结果。

Conclusion: D-Artemis显著增强了通用多模态大语言模型在GUI任务中的能力，展示了强大的泛化性能，每个组件都对框架有重要贡献。
...
### [288] [ProRe: A Proactive Reward System for GUI Agents via Reasoner-Actor Collaboration](https://arxiv.org/abs/2509.21823)
*Gaole Dai,Shiqi Jiang,Ting Cao,Yuqing Yang,Yuanchun Li,Rui Tan,Mo Li,Lili Qiu*

Main category: cs.AI

TL;DR: ProRe是一个主动奖励系统，通过通用推理器和领域特定评估器的协同工作，为GUI智能体提供更准确可验证的奖励评估，显著提升了奖励准确性和任务成功率。
...
Method: 使用通用推理器调度有针对性的状态探测任务，评估器智能体通过主动与环境交互收集额外观察，从而为GUI智能体分配更准确可验证的奖励。

Result: 在超过3000条轨迹上的实验表明，ProRe将奖励准确率和F1分数分别提升了5.3%和19.4%，与最先进策略智能体集成后任务成功率提升了22.4%。

Conclusion: ProRe通过主动环境交互显著改进了GUI智能体的奖励评估，为复杂环境中的智能体训练提供了更可靠的奖励信号。
...
### [289] [DS-STAR: Data Science Agent via Iterative Planning and Verification](https://arxiv.org/abs/2509.21825)
*Jaehyun Nam,Jinsung Yoon,Jiefeng Chen,Jinwoo Shin,Tomas Pfister*

Main category: cs.AI

TL;DR: DS-STAR是一个新颖的数据科学智能体，通过自动数据文件分析、LLM验证步骤和顺序规划机制，解决了LLM在异构数据格式分析和生成优化分析计划方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 数据科学任务复杂，涉及探索多个数据源并综合发现。虽然LLM在自动化这一过程中显示出潜力，但它们在异构数据格式方面表现不佳，且难以验证分析计划的充分性。

Method: DS-STAR包含三个关键组件：(1) 数据文件分析模块，自动探索和提取不同数据格式的上下文；(2) 验证步骤，LLM评估每个阶段分析计划的充分性；(3) 顺序规划机制，从简单可执行计划开始，基于反馈迭代优化直到计划充分性得到验证。

Result: 实验表明，DS-STAR在三个具有挑战性的基准测试（DABStep、KramaBench和DA-Code）上实现了最先进的性能，特别是在需要处理多个异构格式数据文件的困难任务上显著优于基线方法。

Conclusion: DS-STAR通过其迭代优化方法，能够可靠地导航涉及多样化数据源的复杂分析，为数据科学任务的自动化提供了有效解决方案。
...
### [290] [Axiomatic Choice and the Decision-Evaluation Paradox](https://arxiv.org/abs/2509.21836)
*Ben Abramowitz,Nicholas Mattei*

Main category: cs.AI

TL;DR: 提出了一个基于公理的决策建模框架，揭示了决策与评估之间的悖论，强调在训练决策模型和应用公理时需要格外谨慎。


<details>
  <summary>Details</summary>
Motivation: 研究决策公理（如伦理约束）在决策建模中的作用，探索决策制定与决策评估之间的内在张力。

Method: 建立决策公理建模框架，基于结构特性定义公理分类法，分析决策-评估悖论。

Result: 发现决策-评估悖论在现实公理结构中普遍存在，揭示了决策制定与评估之间的根本冲突。

Conclusion: 在基于决策数据训练模型和应用公理进行决策与评估时，必须特别谨慎以避免悖论带来的问题。
...
### [291] [DeepTravel: An End-to-End Agentic Reinforcement Learning Framework for Autonomous Travel Planning Agents](https://arxiv.org/abs/2509.21842)
*Yansong Ning,Rui Liu,Jun Wang,Kai Chen,Wei Li,Jun Fang,Kan Zheng,Naiqiang Tan,Hao Liu*

Main category: cs.AI

TL;DR: DeepTravel是一个端到端的智能体强化学习框架，用于构建自主旅行规划代理，能够自主规划、执行工具并反思工具响应，在推理过程中探索、验证和优化中间行动。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划代理研究依赖手工制作的提示和固定的代理工作流程，限制了代理的灵活性和自主性。

Method: 构建沙盒环境缓存交通、住宿和POI数据；开发分层奖励建模系统，包括轨迹级验证器和回合级验证器；提出回放增强强化学习方法，从失败经验缓冲区定期回放。

Result: 在滴滴企业解决方案App上部署，评估显示DeepTravel使小型LLM（如Qwen3 32B）在旅行规划任务中显著优于前沿LLM如OpenAI o1、o3和DeepSeek R1。

Conclusion: DeepTravel框架成功构建了自主旅行规划代理，通过强化学习显著提升了代理能力，使小型模型在旅行规划任务中超越大型前沿模型。
...
### [292] [Reimagining Agent-based Modeling with Large Language Model Agents via Shachi](https://arxiv.org/abs/2509.21862)
*So Kuroki,Yingtao Tian,Kou Misaki,Takashi Ikegami,Takuya Akiba,Yujin Tang*

Main category: cs.AI

TL;DR: Shachi是一个用于研究LLM驱动的多智能体系统中涌现行为的正式方法论和模块化框架，将智能体策略分解为核心认知组件，支持受控实验和系统性分析。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏用于研究LLM多智能体系统涌现行为的受控实验方法学，限制了该领域的研究进展。

Method: 提出Shachi框架，将智能体策略分解为配置、记忆、工具三个核心认知组件，由LLM推理引擎协调，实现模块化架构设计。

Result: 在10个任务的基准测试中验证了方法论的有效性，特别是在模拟美国关税冲击时，只有当智能体认知架构正确配置记忆和工具时，其行为才与观察到的市场反应一致。

Conclusion: 该工作为构建和评估LLM智能体提供了严谨的开源基础，旨在促进更累积性和科学基础的研究。
...
### [293] [TRACE: Learning to Compute on Graphs](https://arxiv.org/abs/2509.21886)
*Ziyang Zheng,Jiaying Zhu,Jingyi Zhou,Qiang Xu*

Main category: cs.AI

TL;DR: TRACE是一种新的图表示学习范式，通过层次化Transformer架构和函数偏移学习目标，解决了传统方法在计算图建模中的架构不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 传统消息传递神经网络和Transformer在图计算任务中存在架构不匹配，无法捕捉计算的位置感知和层次性特征，需要新的范式来解决这一根本挑战。

Method: 使用层次化Transformer架构模拟逐步计算流程，并引入函数偏移学习目标，将复杂全局函数预测分解为预测真实函数与局部近似之间的差异。

Result: 在电子电路等复杂计算图上，TRACE在全面基准测试中显著优于所有现有架构。

Conclusion: 架构对齐的骨干网络和解耦学习目标构成了学习图计算任务的更稳健范式。
...
### [294] [GenesisGeo: Technical Report](https://arxiv.org/abs/2509.21896)
*Minfeng Zhu,Zi Wang,Sizhe Ji,Zhengtong Du,Junming Ke,Xiao Deng,Zanlang Yin,Xiuqi Huang,Heyu Wang,Wei Chen*

Main category: cs.AI

TL;DR: GenesisGeo是一个自动化欧几里得几何定理证明器，通过120倍加速的符号推理引擎和神经符号方法，在IMO-AG-30基准测试中达到IMO金牌水平。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够解决高难度几何问题的自动化定理证明器，特别是针对IMO级别的几何问题。

Method: 结合定理匹配技术将符号推理引擎DDARN加速120倍，并使用C++实现其核心组件；基于Qwen3-0.6B-Base构建神经符号证明器。

Result: 开源了包含2180万个几何问题的大规模数据集，其中300多万个包含辅助构造；单模型在IMO-AG-30基准测试中解决了24/30个问题（IMO银牌水平），双模型集成解决了26/30个问题（IMO金牌水平）。
...
### [295] [DyRo-MCTS: A Robust Monte Carlo Tree Search Approach to Dynamic Job Shop Scheduling](https://arxiv.org/abs/2509.21902)
*Ruiqi Chen,Yi Mei,Fangfang Zhang,Mengjie Zhang*

Main category: cs.AI

TL;DR: 提出DyRo-MCTS方法，将动作鲁棒性估计集成到MCTS中，用于动态作业车间调度问题，在保持在线规划时间几乎不变的情况下显著提升离线学习策略的性能。


<details>
  <summary>Details</summary>
Motivation: 动态作业车间调度中，新作业的频繁到达导致离线学习策略不完善，传统MCTS在在线规划时因信息不完整而容易受到扰动影响。

Method: DyRo-MCTS方法，在MCTS中集成动作鲁棒性估计，引导生产环境转向既能获得良好调度结果又易于适应未来作业到达的状态。

Result: 实验表明DyRo-MCTS显著提升离线学习策略性能，在线规划时间几乎不增加，在各种调度场景下始终优于传统MCTS。
...
### [296] [Outlier Detection in Plantar Pressure: Human-Centered Comparison of Statistical Parametric Mapping and Explainable Machine Learning](https://arxiv.org/abs/2509.21943)
...
### [297] [CoBel-World: Harnessing LLM Reasoning to Build a Collaborative Belief World for Optimizing Embodied Multi-Agent Collaboration](https://arxiv.org/abs/2509.21981)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM协作框架忽视了动态意图推理的潜力，导致计划不一致和冗余通信，降低协作效率。需要能够推理协作伙伴意图的能力来避免部分可观测环境下的协调失误。
...
### [298] [RISK: A Framework for GUI Agents in E-commerce Risk Management](https://arxiv.org/abs/2509.21982)
*Renqi Chen,Zeyin Tao,Jianming Guo,Jingzhe Zhu,Yiheng Peng,Qingqing Sun,Tianyi Zhang,Shuai Chen*

Main category: cs.AI

TL;DR: RISK是一个用于电子商务风险管理的GUI代理框架，包含数据集、基准测试和强化微调框架，能够处理多步骤交互任务，相比现有方法在单步和多步任务上分别提升6.8%和8.8%。


<details>
  <summary>Details</summary>
Motivation: 传统爬虫方法和现有GUI代理无法处理电子商务风险管理所需的多样、深度嵌入的网页数据，这些代理通常限于单步任务，缺乏管理动态交互内容的能力。

Method: RISK框架包含三个组件：RISK-Data数据集（8,492单步和2,386多步交互轨迹）、RISK-Bench基准测试（802单步和320多步轨迹）和RISK-R1强化微调框架（考虑输出格式、单步级别、多步级别和任务级别四个方面的奖励机制）。

Result: RISK-R1在离线单步和多步任务上分别提升6.8%和8.8%，在线评估中达到70.5%的最高任务成功率。
...
### [299] [Bilinear relational structure fixes reversal curse and enables consistent model editing](https://arxiv.org/abs/2509.21993)
*Dong-Kyum Kim,Minsung Kim,Jea Kwon,Nakyeong Yang,Meeyoung Cha*

Main category: cs.AI

TL;DR: 该论文发现反转诅咒不是语言模型的固有缺陷，而是知识编码方式的产物。通过训练具有双线性关系结构的模型，可以显著缓解反转诅咒并实现一致的模型编辑。


<details>
  <summary>Details</summary>
Motivation: 研究反转诅咒的根本原因，探索语言模型是否能从学习的事实'A是B'推断出未见事实'B是A'，以及这种能力与模型内部表示结构的关系。

Method: 在关系知识图谱合成数据集上从头训练语言模型，分析其隐藏表示中出现的双线性关系结构，并测试模型编辑的传播效果。

Result: 具有双线性关系结构的模型能够推断未见反向事实，编辑能够正确传播到相关事实；缺乏这种结构的模型不仅存在反转诅咒，编辑还会引入逻辑不一致性。

Conclusion: 关系知识训练诱导双线性内部表示的出现，这种表示几何对模型编辑的成功至关重要，反转诅咒是可解决的表征问题而非固有局限。
...
### [300] [GSM-Agent: Understanding Agentic Reasoning Using Controllable Environments](https://arxiv.org/abs/2509.21998)
*Hanlin Zhu,Tianyu Guo,Song Mei,Stuart Russell,Nikhil Ghosh,Alberto Bietti,Jiantao Jiao*

Main category: cs.AI

TL;DR: 提出了GSM-Agent基准测试，用于评估LLM在需要主动使用工具收集信息来解决小学数学问题时的代理推理能力，发现即使是前沿模型准确率也较低，并提出了基于代理推理图的改进方法。


<details>
  <summary>Details</summary>
Motivation: 当前代理基准测试往往将代理推理与复杂的数学推理、专家级知识等能力混合在一起，难以分离评估纯粹的代理推理能力，因此需要专门的基准测试来理解代理推理的本质。

Method: 构建GSM-Agent基准测试，要求LLM代理解决小学数学问题但只提供问题而不提供前提信息，需要主动使用工具收集信息；提出代理推理图概念，将环境文档嵌入聚类为节点，将工具调用映射到最近节点构建推理路径；基于洞察提出工具增强的测试时扩展方法。

Result: 即使是GPT-5这样的前沿模型在GSM-Agent基准测试上也只能达到67%的准确率；通过代理推理图分析发现许多模型在代理推理中缺乏重新访问已访问节点的能力；提出的改进方法能够提升LLM的代理推理性能。

Conclusion: GSM-Agent基准测试和代理推理框架有助于未来理解和推进代理推理边界的研究，强调了在动态环境中重新访问信息的重要性。
...
### [301] [The Thinking Spectrum: An Emperical Study of Tunable Reasoning in LLMs through Model Merging](https://arxiv.org/abs/2509.22034)
*Xiaochong Lan,Yu Zheng,Shiteng Cao,Yong Li*

Main category: cs.AI

TL;DR: 该论文通过大规模实证研究评估了模型融合技术，发现模型融合能够有效平衡推理准确性和计算效率，甚至在某些情况下实现帕累托改进。
...
Method: 采用大规模实证研究方法，系统评估多种模型融合技术，通过改变融合强度构建准确率-效率曲线，分析可调节性能空间。

Result: 研究发现模型融合提供了有效且可控的方法来校准推理准确性和标记效率之间的权衡，即使父模型具有高度不同的权重空间。在某些情况下实现了帕累托改进。

Conclusion: 该研究首次全面分析了可调节空间，为创建具有特定推理配置文件的大语言模型以满足多样化应用需求提供了实用指南。
...
### [302] [A2R: An Asymmetric Two-Stage Reasoning Framework for Parallel Reasoning](https://arxiv.org/abs/2509.22044)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型在单次尝试中的表现与其潜在能力之间存在显著差距，需要通过多路径推理来充分发掘模型的内在潜力。

Method: 提出A2R框架：第一阶段由探索器模型并行生成多个潜在解决方案，第二阶段由合成器模型整合这些参考进行精炼推理。

Result: 使用A2R框架，Qwen3-8B-distill模型相比自一致性基线性能提升75%；A2R-Efficient变体（Qwen3-4B探索器+Qwen3-8B合成器）在成本降低30%的情况下超越了Qwen3-32B模型的平均性能。

Conclusion: A2R不仅是一个性能提升框架，更是一个高效实用的现实应用解决方案，能够正交扩展计算能力。
...
### [303] [Generalizing Multi-Objective Search via Objective-Aggregation Functions](https://arxiv.org/abs/2509.22085)
*Hadar Peer,Eyal Weiss,Ron Alterovitz,Oren Salzman*

Main category: cs.AI

TL;DR: 提出了一种广义的多目标搜索问题公式，通过隐藏目标的聚合函数来优化解决方案目标，支持标准MOS算法的应用，只需扩展核心操作以反映特定的聚合函数。


<details>
  <summary>Details</summary>
Motivation: 现实世界机器人系统需要同时平衡多个经常冲突的目标，而现有问题公式不允许使用现成的先进MOS算法。

Method: 使用隐藏目标的聚合函数来优化解决方案目标，扩展标准MOS算法的核心操作以支持特定聚合函数。

Result: 在多个机器人规划问题中，扩展后的算法比未使用目标聚合的原始版本性能高出数个数量级。

Conclusion: 提出的广义问题公式能够有效支持标准MOS算法的应用，通过适当扩展核心操作可以显著提升算法性能。
...
### [304] [Ground-Truthing AI Energy Consumption: Validating CodeCarbon Against External Measurements](https://arxiv.org/abs/2509.22092)
*Raphael Fischer*

Main category: cs.AI

TL;DR: 本文系统评估了AI能耗估算工具的准确性，通过与真实测量数据对比发现现有工具存在高达40%的误差，为可持续AI发展提供了实证依据和改进指南。


<details>
  <summary>Details</summary>
Motivation: 随着AI快速发展带来的环境影响日益显著，现有能耗估算工具虽然易于使用但存在准确性疑问，需要系统验证其可靠性。

Method: 通过提出的验证框架，将静态和动态能耗估算方法与数百个AI实验的真实测量数据进行系统性比较。

Result: 现有估算方法虽然能大致反映AI能耗模式，但存在高达40%的持续误差，揭示了估算不准确的问题。

Conclusion: 研究为AI能耗估算提供了透明度验证，制定了改进指南，并为资源感知的机器学习和AI可持续性研究做出了重要贡献。
...
### [305] [Log2Plan: An Adaptive GUI Automation Framework Integrated with Task Mining Approach](https://arxiv.org/abs/2509.22137)
*Seoyoung Lee,Seonbin Yoon,Seongbeen Lee,Hyesoo Kim,Joo Yong Sim*

Main category: cs.AI

TL;DR: Log2Plan是一个结合结构化两级规划框架和用户行为日志任务挖掘的GUI任务自动化系统，解决了现有LLM/VLM规划器在泛化性、延迟和长时程一致性方面的限制。
...
Method: Log2Plan采用结构化两级规划框架：高层规划将用户命令映射到结构化任务字典，实现一致且可泛化的自动化；通过用户行为日志的任务挖掘识别用户特定模式；然后将高层计划基于实时GUI上下文转化为低层动作序列。

Result: 在200个真实世界任务上的评估显示，Log2Plan在任务成功率和执行时间方面有显著改进，即使在长时程任务序列中也能保持超过60.0%的成功率。

Conclusion: Log2Plan通过结合结构化规划和任务挖掘方法，实现了在复杂多步骤工作流中稳健且适应性强的GUI自动化。
...
### [306] [Clinical Uncertainty Impacts Machine Learning Evaluations](https://arxiv.org/abs/2509.22242)
...
TL;DR: 论文主张在医学影像机器学习评估中应考虑标注不确定性，使用概率度量来直接处理分布，而非传统多数投票方法，以更准确反映临床数据的变异性。
...
Method: 提出使用概率度量来明确处理标注不确定性，这些度量可独立于标注生成过程，计算轻量且具有线性时间复杂度。

Result: 在医学影像基准测试的简单实验中，考虑二元标签的置信度显著影响模型排名。

Conclusion: 呼吁社区发布原始标注数据并采用不确定性感知评估方法，使性能估计能更好地反映临床数据特性。
...
### [307] [Evaluating LLMs for Combinatorial Optimization: One-Phase and Two-Phase Heuristics for 2D Bin-Packing](https://arxiv.org/abs/2509.22255)
*Syed Mahbubul Huq,Daniel Brito,Daniel Sikar,Rajesh Mojumder*

Main category: cs.AI

TL;DR: 提出了一个评估LLM在组合优化中能力的框架，特别针对二维装箱问题，将LLM与进化算法结合迭代生成和优化启发式解。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在组合优化领域的潜力，特别是解决二维装箱问题，探索LLM生成高效启发式解的能力。

Method: 系统化方法将LLM与进化算法结合，迭代生成和优化启发式解，并与传统方法（有限首次适应和混合首次适应）进行比较。

Result: GPT-4o在两次迭代内达到最优解，将平均箱数从16减少到15，空间利用率从0.76-0.78提升到0.83，且计算资源需求更低。

Conclusion: LLM能在组合优化中产生更高效的解，为理解LLM在专业领域的评估提供了贡献，并建立了组合优化任务的基准测试标准。
...
### [308] [InfiMed-Foundation: Pioneering Advanced Multimodal Medical Models with Compute-Efficient Pre-Training and Multi-Stage Fine-Tuning](https://arxiv.org/abs/2509.22261)
*Guanghao Zhu,Zhitian Hou,Zeyu Liu,Zhijie Sang,Congkai Xie,Hongxia Yang*

Main category: cs.AI

TL;DR: 提出了两个医疗专用多模态大语言模型InfiMed-Foundation-1.7B和4B，通过高质量数据筛选、高效训练策略和三阶段微调，在医疗任务中表现出色，超越了多个现有模型。


<details>
  <summary>Details</summary>
Motivation: 通用多模态大模型在医疗领域应用受限，缺乏专业知识导致回答不确定或产生幻觉，知识蒸馏难以捕捉放射学和药理学等专业领域知识，且大规模医疗数据持续预训练计算成本高。

Method: 结合通用和医疗多模态数据，提出五维质量评估框架筛选高质量数据集；采用低到高图像分辨率和多模态序列打包提升训练效率；使用三阶段监督微调过程提取复杂医疗任务知识。

Result: 在MedEvalKit框架评估中，InfiMed-Foundation-1.7B超越Qwen2.5VL-3B，InfiMed-Foundation-4B超越HuatuoGPT-V-7B和MedGemma-27B-IT，在医疗视觉问答和诊断任务中表现优异。
...
### [309] [Structured Sparse Transition Matrices to Enable State Tracking in State-Space Models](https://arxiv.org/abs/2509.22284)
*Aleksandar Terzić,Nicolas Menet,Michael Hersche,Thomas Hofmann,Abbas Rahimi*

Main category: cs.AI

TL;DR: 提出PD-SSM方法，通过结构化稀疏参数化状态转移矩阵，在保持计算效率的同时显著提升状态空间模型对有限状态自动机的表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有状态空间模型使用对角化转移矩阵虽然计算高效，但表达能力受限；非结构化转移矩阵虽然表达能力强但计算成本过高。需要一种平衡计算效率和表达能力的方法。

Method: 将转移矩阵参数化为列独热矩阵(P)和复值对角矩阵(D)的乘积，使并行扫描的计算成本随状态大小线性增长。

Result: 理论上模型BIBO稳定，能模拟任何N状态有限状态自动机；实验上在各种FSA状态跟踪任务中显著优于现有SSM变体，在时间序列分类中性能与神经控制微分方程相当。
...
### [310] [Large Language Models as Nondeterministic Causal Models](https://arxiv.org/abs/2509.22297)
*Sander Beckers*
...
<details>
  <summary>Details</summary>
Motivation: 现有反事实生成方法对LLM的解释存在歧义，既不是字面解释也不是预期解释，需要改进。

Method: 将LLM表示为非确定性因果模型，基于LLM的预期语义生成反事实，不依赖实现细节。

Result: 新方法更简单且可直接应用于任何黑盒LLM，而现有方法能直接生成特定类型的反事实。

Conclusion: 为LLM反事实推理提供了理论基础，为特定应用场景的反事实生成方法奠定了基础。
...
### [311] [PRIME: Planning and Retrieval-Integrated Memory for Enhanced Reasoning](https://arxiv.org/abs/2509.22315)
*Hieu Tran,Zonghai Yao,Nguyen Luong Tran,Zhichao Yang,Feiyun Ouyang,Shuo Han,Razieh Rahimi,Hong Yu*

Main category: cs.AI

TL;DR: PRIME是一个基于人类双过程认知理论的多智能体推理框架，通过动态整合快速直觉思维（系统1）和慢速审慎思维（系统2）来提升LLM的推理能力。
...
Method: 采用多智能体设计：首先由快速思维智能体（系统1）生成快速答案，如果检测到不确定性，则触发包含规划、假设生成、检索、信息整合和决策等专门智能体的系统2推理管道。

Result: 实验结果显示，PRIME使开源LLaMA 3模型在需要多跳和知识基础推理的基准测试中，能够与GPT-4、GPT-4o等最先进的闭源模型竞争。

Conclusion: PRIME为需要复杂、知识密集型推理的领域提供了一种可扩展的解决方案，能够显著提升LLM的推理性能。
...
### [312] [Do LLM Agents Know How to Ground, Recover, and Assess? A Benchmark for Epistemic Competence in Information-Seeking Agents](https://arxiv.org/abs/2509.22391)
*Jiaqi Shao,Yuxiang Lin,Munish Prasad Lohani,Yufeng Miao,Bing Luo*

Main category: cs.AI

TL;DR: SeekBench是首个通过逐步骤分析来评估LLM搜索代理认知能力的基准，包含190个专家标注的轨迹和1800多个响应步骤，用于分析代理是否基于证据推理、自适应搜索和正确校准。
...
Method: 构建SeekBench基准，包含专家标注的响应轨迹和证据注释，进行细粒度分析，评估代理的推理基础、搜索自适应性和证据充分性判断。

Result: SeekBench提供了190个标注轨迹和1800多个响应步骤，能够详细分析LLM搜索代理的认知能力表现。

Conclusion: SeekBench填补了LLM搜索代理认知能力评估的空白，为更全面的性能分析提供了工具。
...
### [313] [EMMA: Generalizing Real-World Robot Manipulation via Generative Visual Transfer](https://arxiv.org/abs/2509.22407)
...
Result: 生成的视频在多视角一致性、几何保真度和文本条件准确性方面优于现有方法；在真实机器人操作任务中，相比仅使用真实数据训练，性能提升超过200%。

Conclusion: EMMA框架通过生成数据和有效训练策略，显著提升了VLA模型在零样本视觉领域的泛化能力。
...
### [314] [Guiding Evolution of Artificial Life Using Vision-Language Models](https://arxiv.org/abs/2509.22447)
*Nikhil Baid,Hannah Erlebach,Paul Hellegouarch,Frederico Wieser*

Main category: cs.AI

TL;DR: ASAL++是一个基于多模态基础模型的开放进化搜索方法，通过第二个基础模型根据模拟历史提出新的进化目标，从而引导产生越来越复杂目标的进化轨迹。


<details>
  <summary>Details</summary>
Motivation: 利用基础模型为人工生命模拟提供强大的自动化搜索工具，扩展ASAL方法以实现更开放式的进化搜索。

Method: 使用Gemma-3在Lenia基质中提出进化目标，探索两种策略：EST（每次迭代匹配单个新提示）和ETT（匹配整个生成提示序列）。

Result: EST促进更大的视觉新颖性，ETT培养更连贯和可解释的进化序列。

Conclusion: ASAL++为基础模型驱动的人工生命发现指明了具有开放端特征的新方向。
...
### [315] [GeoSketch: A Neural-Symbolic Approach to Geometric Multimodal Reasoning with Auxiliary Line Construction and Affine Transformation](https://arxiv.org/abs/2509.22460)
*Shichao Weng,Zhiqiang Wang,Yuhua Zhou,Rui Lu,Ting Liu,Zhiyang Teng,Xiaozhang Liu,Hanmeng Liu*

Main category: cs.AI

TL;DR: GeoSketch是一个神经符号框架，将几何推理重新定义为交互式的感知-推理-行动循环，通过动态操作图表来解决几何问题，显著优于静态感知方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型处理几何问题时，将图表视为静态图像，缺乏动态操作能力，而人类几何推理涉及辅助线构造和仿射变换等动态操作。

Method: 集成三个模块：感知模块将图表抽象为结构化逻辑形式；符号推理模块应用几何定理决定下一步推理；草图动作模块执行操作如绘制辅助线或应用变换，形成闭环。采用两阶段训练：监督微调+强化学习。

Result: 在GeoSketch基准测试中，相比强基线MLLM，GeoSketch显著提高了逐步推理准确性和问题解决成功率。
...
### [316] [InfiAgent: Self-Evolving Pyramid Agent Framework for Infinite Scenarios](https://arxiv.org/abs/2509.22502)
*Chenglin Yu,Yang Yu,Songmiao Wang,Yucheng Wang,Yifan Yang,Jinjia Li,Ming Li,Hongxia Yang*

Main category: cs.AI

TL;DR: InfiAgent是一个基于有向无环图的金字塔式多智能体框架，通过'智能体即工具'机制、双重审计、智能体路由和自进化机制，实现复杂任务的自动分解和高效执行。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体开发需要精心设计工作流程、提示工程和迭代调优，这些手工限制阻碍了LLM智能体在各行业的可扩展性和成本效益。

Method: 提出金字塔式DAG多智能体框架，包括：智能体即工具机制实现层次化分解、双重审计确保任务质量、智能体路由实现任务匹配、自进化机制自主重构DAG结构。

Result: 在多个基准测试中，InfiAgent相比ADAS性能提升9.9%；案例研究显示AI研究助手InfiHelper生成的论文获得IEEE顶级会议人类评审的认可。

Conclusion: InfiAgent框架能够演化成通用的金字塔式多智能体系统，解决广泛领域的问题，显著提升LLM智能体的可扩展性和执行效率。
...
### [317] [Estimating the Empowerment of Language Model Agents](https://arxiv.org/abs/2509.22504)
*Jinyeop Song,Jeff Gore,Max Kleiman-Weiner*

Main category: cs.AI

TL;DR: 提出基于信息论中赋能概念（行动与未来状态的互信息）的LM智能体评估框架EELMA，该框架在语言游戏和网页浏览场景中验证了赋能与任务性能的强相关性。


<details>
  <summary>Details</summary>
Motivation: 传统基于基准测试的评估方法成本高且需要人工设计有效任务，需要一种可扩展的开放评估框架来评估LM智能体的能力。

Method: 提出EELMA算法，通过多轮文本交互来近似估计有效赋能，在语言游戏和现实网页浏览场景中进行验证。

Result: 赋能与平均任务性能强相关，环境复杂性和智能体因素（思维链、模型规模、记忆长度）对估计赋能产生影响，高赋能状态和行动通常是通用能力的关键时刻。
...
### [318] [TrueGradeAI: Retrieval-Augmented and Bias-Resistant AI for Transparent and Explainable Digital Assessments](https://arxiv.org/abs/2509.22516)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统纸质考试的纸张浪费、物流复杂、评分延迟和评分者偏见等问题，推动数字考试的公平性和可扩展性。

Method: 使用安全平板捕捉手写输入，应用基于transformer的OCR进行转录，通过检索增强管道整合教师答案、缓存层和外部参考，让大语言模型进行评分并提供证据链推理。

Result: 系统实现了手写保留与可扩展评估的结合，减少了环境成本，加速了反馈周期，并建立了可重复使用的知识库。

Conclusion: TrueGradeAI通过结合手写保留和透明评估，推进了数字考试领域，减少了评分偏见，确保了评估的公平性。
...
### [319] [REMA: A Unified Reasoning Manifold Framework for Interpreting Large Language Model](https://arxiv.org/abs/2509.22518)
...
Method: 定义Reasoning Manifold概念，构建REMA框架，通过计算错误表示与正确推理流形的k近邻距离来量化几何偏差，并跟踪层间偏差定位分歧点。
...
### [320] [The Emergence of Altruism in Large-Language-Model Agents Society](https://arxiv.org/abs/2509.22537)
*Haoyang Li,Xiao Jia,Zhanzhan Zhao*

Main category: cs.AI

TL;DR: 研究发现LLM在社会模拟中存在两种不同的社会倾向：适应性利己主义者和利他主义优化者，前者在社交规范影响下会增加利他行为，后者则固有地优先考虑集体利益。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注小规模任务导向游戏中的合作行为，忽视了大规模智能体社会中利他主义如何涌现的问题。

Method: 引入Schelling变体的城市迁移模型，创建社会困境，让200多个LLM智能体在利己和利他目标间进行选择，并使用扎根理论方法系统编码智能体推理。

Result: 发现LLM存在内在异质性：适应性利己主义者默认优先考虑自身利益但在社交规范影响下显著增加利他行为；利他主义优化者固有地优先集体利益甚至不惜牺牲自身利益。

Conclusion: 社会模拟中的模型选择不仅是选择推理能力，更是选择内在社会行动逻辑。适应性利己主义者更适合模拟复杂人类社会，利他主义优化者更适合模拟理想化亲社会行为者。
...
### [321] [StepORLM: A Self-Evolving Framework With Generative Process Supervision For Operations Research Language Models](https://arxiv.org/abs/2509.22558)
*Chenyu Zhou,Tianyi Xu,Jianghao Lin,Dongdong Ge*

Main category: cs.AI

TL;DR: StepORLM是一个自演化的LLM框架，通过生成式过程监督解决运筹学问题，结合结果验证和过程评估，在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个关键限制：结果奖励存在信用分配问题（正确答案可能强化错误推理），传统判别式过程监督缺乏对运筹学建模步骤的整体评估。

Method: StepORLM采用协同进化循环，策略模型和生成式过程奖励模型相互迭代改进，结合外部求解器的结果验证和GenPRM的整体过程评估，使用加权直接偏好优化进行对齐。

Result: 8B参数的StepORLM在六个基准测试中达到新SOTA，显著优于更大的通用模型、代理方法和专业基线。协同进化的GenPRM可作为强大的通用过程验证器，提升推理性能。

Conclusion: StepORLM通过生成式过程监督和协同进化机制，有效解决了运筹学问题中LLM训练的局限性，实现了卓越的性能和通用性。
...
### [322] [UniMIC: Token-Based Multimodal Interactive Coding for Human-AI Collaboration](https://arxiv.org/abs/2509.22570)
...
Method: 采用轻量级基于Transformer的熵模型，包含通用、掩码和文本条件三种场景特定设计，有效最小化token间冗余。
...
### [323] [Dynamic Experts Search: Enhancing Reasoning in Mixture-of-Experts LLMs at Test Time](https://arxiv.org/abs/2509.22572)
*Yixuan Han,Fan Ma,Ruijie Quan,Yi Yang*

Main category: cs.AI

TL;DR: 提出Dynamic Experts Search (DES)，一种测试时缩放策略，通过动态控制MoE模型中激活的专家数量来生成多样化的推理路径，提升LLM的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法主要依赖输出级采样，忽视了模型架构的作用。在MoE模型中，发现改变激活专家数量能产生互补的解决方案集，这为推理多样性提供了新的探索方向。

Method: DES包含两个核心组件：Dynamic MoE（在推理时直接控制专家数量生成多样化推理轨迹）和Expert Configuration Inheritance（在推理路径内保持专家数量一致，在多次运行间变化，平衡稳定性和多样性）。

Result: 在多种MoE架构、验证器和推理基准（数学、代码和知识）上的广泛实验表明，DES可靠地优于TTS基线，在不增加成本的情况下提高了准确性和稳定性。

Conclusion: DES是一种实用且可扩展的架构感知测试时缩放方法，展示了现代LLM结构灵活性如何推动推理能力的进步。
...
### [324] [Benefits and Pitfalls of Reinforcement Learning for Language Model Planning: A Theoretical Perspective](https://arxiv.org/abs/2509.22613)
*Siwei Wang,Yifei Shen,Haoran Sun,Shi Feng,Shang-Hua Teng,Li Dong,Yaru Hao,Wei Chen*

Main category: cs.AI

TL;DR: 本文通过图论抽象分析RL在LLM规划中的理论机制，发现SFT会产生共现伪解，而RL通过探索实现正确规划，但PG存在多样性崩溃问题，Q-learning则能保持多样性并避免奖励攻击。
...
Method: 采用可处理的图论抽象方法，重点分析策略梯度(PG)和Q-learning方法，并在Blocksworld真实规划基准上进行验证。

Result: 理论分析表明：SFT引入共现伪解；RL通过探索实现更好泛化；PG存在多样性崩溃；Q-learning具有离策略学习和多样性保持优势；需要精心设计奖励函数防止奖励攻击。

Conclusion: RL通过探索机制在规划任务中优于SFT，但不同RL方法各有特点：PG存在多样性问题，Q-learning在保持多样性和避免奖励攻击方面表现更好，需要合理设计奖励函数。
