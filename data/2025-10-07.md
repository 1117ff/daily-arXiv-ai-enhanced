...
Motivation: 现有模型无法准确预测标准治疗下的肿瘤轨迹，需要能考虑患者基因组、人口统计学和治疗方案差异的计算框架。

Method: 开发可微分框架SoC-DT，整合反应扩散肿瘤生长模型、离散标准治疗干预（手术、化疗、放疗）以及基因组和人口统计学个性化，使用IMEX-SoC求解器确保稳定性。

Result: 在合成数据和真实胶质瘤数据上评估，SoC-DT在预测肿瘤动态方面优于传统PDE基线和纯数据驱动的神经网络模型。

Conclusion: SoC-DT为肿瘤学中患者特异性数字孪生建立了原则性基础，实现了生物学一致的肿瘤动态估计。
...
Result: 系统能够高效处理大量视频数据，生成全面的可视化分析，包括出现频率、时长分析、共现矩阵、网络图等，揭示名人动态模式。
...
<details>
  <summary>Details</summary>
Motivation: 海洋塑料污染是紧迫的环境威胁，需要可靠的水下垃圾自动检测系统。但由于领域偏移问题，在一个数据集上训练的视觉系统在新图像上性能会下降。
...
Result: 轻量级MobileNetV2在跨域性能上表现最佳（F1 0.97），所有微调模型都实现了高精度（约99%），但在召回率上有所不同。零样本CLIP召回率约80%但精度较低（约56%），Gemini则相反（精度约99%，召回率约81%）。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统端到端图像描述方法缺乏可解释性的问题，为阿拉伯语图像描述提供基于视觉概念的基础生成方法。

Method: 使用三种多语言编码器(mCLIP、AraCLIP、Jina V4)进行视觉标签检索，构建包含训练描述和Visual Genome翻译标签的混合词汇表，将检索到的标签转换为阿拉伯语提示，再结合图像输入视觉语言模型生成描述。

Result: mCLIP + Gemini Pro Vision组合在BLEU-1(5.34%)和余弦相似度(60.01%)上表现最佳，AraCLIP + Qwen-VL组合在LLM-judge评分(36.33%)上最高。

Conclusion: 该可解释流水线能够生成文化连贯且上下文准确的阿拉伯语图像描述，为阿拉伯语多模态理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 系统比较卷积神经网络和视觉Transformer在遥感图像分类任务中的性能差异，特别是在不同类别分布情况下的表现。

Method: 在SpaceNet数据集上进行控制实验，使用自然不平衡的五类分割和平衡重采样分割，匹配预处理、数据增强和训练预算，评估多种性能指标。

Result: 在不平衡分割中，EfficientNet-B0达到93%测试准确率且延迟更低；在平衡分割中，两者表现都很强，EfficientNet-B0达到99%，但ViT-Base仍具竞争力。

Conclusion: 平衡数据分布可以缩小架构差距，但CNN在效率方面仍保持优势；ViT在性能上具有竞争力但计算成本更高。
...
<details>
  <summary>Details</summary>
Motivation: 传统基础设施措施在动态城市环境中对弱势道路使用者的保护不足，而现有AI应用调查主要关注检测任务，缺乏对全面VRU理解和保护所需的其他视觉任务的覆盖。

Method: 系统性回顾过去五年基于摄像头的AI感知系统在VRU安全方面的进展，重点分析四个核心任务：检测分类、跟踪重识别、轨迹预测、意图识别预测。

Result: 识别了AI赋能的主动式VRU保护解决方案的技术框架，涵盖了从基础感知到高级推理的完整任务链。

Conclusion: 通过将视觉AI进展与实际部署考虑相结合，为开发下一代感知系统以增强VRU安全提供了基础参考，并指出了数据、模型和部署三个维度的主要开放挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有EOFMs大多在单一模态数据上训练，然后跨模态应用，但不同传感器架构对模型内部表示的影响尚不清楚

Method: 分析EOFMs表示空间对传感器架构的敏感性

Result: EOFMs的表示空间对传感器架构高度敏感

Conclusion: 理解传感器架构差异对EOFMs表示的影响，为模型开发者、用户和遥感科学社区提供了重要指导
...
<details>
  <summary>Details</summary>
Motivation: 生态监测中视觉模型的预测不透明限制了信任和实际应用，需要能够生成保持场景上下文且可解释的扰动来解释模型决策。

Method: 使用修复引导的扰动技术，结合Segment-Anything-Model精炼的掩码，进行物体移除/替换和背景替换两种干预，生成逼真的局部编辑。

Result: 该方法能够定位诊断性结构，避免传统扰动中的删除伪影，产生领域相关的见解，支持专家验证。

Conclusion: 该方法为生态学中AI的可信部署提供了有效的解释工具，能够生成生态学上合理且可解释的扰动。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割在精准诊断、治疗规划和病情监测中具有关键作用，但现有方法分散且缺乏系统性综述，需要弥合传统技术与现代深度学习之间的差距。

Method: 采用系统性综述方法，涵盖阈值分割、边缘检测、区域分割、聚类算法、模型基技术，以及CNN、FCN、U-Net等深度学习架构，还包括注意力机制、半监督学习、GAN和Transformer模型。

Result: 综述展示了医学图像分割领域从传统方法到深度学习的完整发展脉络，识别了混合架构、跨模态学习、联邦学习等新兴趋势，并通过腰椎分割案例展示了该领域的具体挑战和进展。

Conclusion: 尽管医学图像分割取得了显著进展，但仍面临数据集偏差、领域适应、模型可解释性以及临床工作流集成等关键挑战，需要进一步研究来解决这些问题。
...
<details>
  <summary>Details</summary>
Motivation: 半导体制造中，晶圆缺陷的早期检测对产品良率优化至关重要。原始晶圆数据通常复杂、未标记、不平衡，且单个晶圆可能包含多种缺陷，需要设计在非理想数据条件下仍可靠的聚类方法。

Method: DECOR是一个具有方位鲁棒性的深度聚类框架，能够将晶圆图中的复杂缺陷模式分组为一致的簇。该方法明确考虑了晶圆图中的方位变化，确保空间相似的缺陷无论旋转或对齐方式如何都能被一致地聚类。

Result: 在开源MixedWM38数据集上的实验表明，DECOR无需手动调参即可发现簇，且在聚类性能上优于现有的基线方法。

Conclusion: DECOR为自动化视觉检测系统提供了一个可靠且可扩展的解决方案，能够有效处理晶圆缺陷检测中的复杂数据挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决面部表情多分类任务中的类别不平衡问题，特别是某些情绪类别显著多于其他类别的情况，这对于实际应用如反欺诈系统中的罕见事件检测很重要。

Method: 使用基于LSTM的神经网络模型，结合注意力机制聚焦于面部关键区域，在训练时排除一个类别，在测试时对该类别进行错误校正。

Result: 实验表明所有类别都能进行校正，但效果各异：某些类别恢复较好，某些较差。在测试样本中，校正小类别时关键质量指标有所提升。

Conclusion: 该方法可有效应用于面部表情分析系统，在类别分布不平衡的情况下实现稳定分类，特别适用于寻找罕见事件的应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在3D重建和渲染方面表现出色，但每个高斯只能表示单一颜色和简单椭球体，表达能力有限。本文旨在增强单个高斯原语的表达能力。

Method: 受传统图形学中纹理和alpha映射的启发，为每个高斯添加alpha、RGB或RGBA纹理映射，以建模每个高斯范围内的空间变化颜色和透明度。

Result: 在多种标准基准数据集和自定义捕获上进行验证，证明该方法在保持相似或更少高斯数量的情况下，实现了图像质量的提升。

Conclusion: 通过纹理映射增强高斯表达能力是有效的，特别是alpha-only纹理映射能显著提升表达能力，而RGB纹理映射则达到最高表达能力。
...
Method: 构建了DCG-8K高质量数据集，包含指令-代码-视频三元组和QA对，采用两阶段训练方法，提出联合代码视觉奖励进行组相对策略优化，构建了专家模型Qwen2.5-VL-DCG-3B。

Result: 基准测试揭示了现有MLLM在视觉到图表任务中的不足，所提模型在三个任务上平均性能提升8.31%，仅用30亿参数就达到了与专有模型相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统单目视觉里程计方法依赖预训练深度学习组件和优化模块的复杂流程，以及现有大规模3D模型在长视频处理和逐帧精确估计方面的局限性。

Method: 使用Transformer架构处理单目帧序列，通过时空注意力建模全局关系，直接预测相机运动而不估计稠密几何，仅依赖相机位姿进行监督，可灵活集成各种预训练编码器。

Result: VoT在更大数据集上有效扩展，从更强预训练骨干网络中显著受益，在不同相机运动和标定设置下具有良好泛化性，性能优于传统方法且运行速度快3倍以上。

Conclusion: 端到端的单目视觉里程计是可行的，VoT框架展示了其有效性、灵活性和高效性，为视觉里程计提供了新的解决方案。
...
Method: 开发了一种推理时搜索算法，利用侧信息指导采样过程，平衡探索和利用，可无缝集成到现有扩散基重建流程中。

Result: 在多种逆问题（框内修复、超分辨率、运动/高斯/非线性/盲去模糊）上实验表明，该方法能持续提升扩散基重建算法的定性和定量性能。
...
<details>
  <summary>Details</summary>
Motivation: 声纳图像对水下探索、自主导航和生态系统监测很重要，但公开且标注良好的声纳图像数据集稀缺，这限制了稳健机器学习模型的发展。

Method: 系统性地映射了各种声纳模态的公开数据集，包括侧扫声纳、前视声纳、合成孔径声纳、多波束回声测深仪和双频识别声纳，并分析了分类、检测、分割和3D重建等应用。

Result: 将发现整合成主表和时序图，提供了数据集特征、大小和标注细节的清晰比较。

Conclusion: 这项工作为水下声学数据分析领域的研究者提供了基础指南，帮助他们开始或推进相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 传统显示校准需要专业设备和暗室环境，对大多数用户来说难以实现，因此需要开发无需专业硬件的校准方案。

Method: 联合设计无透镜相机和基于隐式神经表示的算法，从46.6°×37.6°的视角锥内高效重建显示器发射的光场。

Result: 能够从宽视角范围内重建显示器的光场特性，为显示校准提供新途径。

Conclusion: 该新兴管道为实现轻松显示校准和特性表征迈出了初步步骤。
...
TL;DR: 提出溯源网络，一种新型神经网络模型，通过将预测直接关联到支持性训练样本来实现端到端、数据驱动的可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决传统深度学习的模型不透明性、幻觉问题，以及为数据贡献者分配信用等关键挑战，提高神经模型的透明度、鲁棒性和可信度。

Method: 模型类似于学习的KNN，每个输出由特征空间中加权的具体样本证明，通过联合优化主要任务和可解释性目标来嵌入可解释性到架构中。

Result: 能够系统研究记忆化与泛化的权衡，验证输入是否在训练集中，检测错误标记或异常数据点，增强对输入扰动的韧性，并识别相似输入对新数据点的贡献。

Conclusion: 溯源网络虽然引入额外计算成本且目前适用于中等规模数据集，但为现有可解释性技术提供了互补方法，显著改善了神经模型的透明度和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督异常检测方法存在匹配噪声问题，且单模态和多模态方法相互孤立，缺乏统一视角。

Method: 构建异常成本体积，通过可学习的过滤模块和多层注意力引导来减少匹配噪声，突出细微异常。

Result: 在22个不同基准测试中，UCF显著提升了多种UAD方法的性能，在单模态和多模态场景下均达到新的最优结果。

Conclusion: UCF是一个通用的后处理框架，能有效提升无监督异常检测的性能，为单模态和多模态UAD提供了统一解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 工业图纸数字化是构建数字孪生和实现智能工业自动化的关键步骤，但缺乏自动评估对象检测结果质量的方法。

Method: 利用视觉语言模型的多模态能力来识别缺失或不一致的检测结果，指导检测结果的优化。

Result: 该框架能够实现工业图纸对象检测结果的自动化质量评估，并提高复杂工业图纸上的整体检测性能。

Conclusion: 视觉语言模型为工业图纸对象检测的质量评估提供了有效解决方案，有助于推动工业图纸的数字化进程。
...
Method: 提出SpatialViLT和MaskedSpatialViLT两个变体，通过多任务学习框架整合空间特征，SpatialEnsemble结合两种方法。
...
<details>
  <summary>Details</summary>
Motivation: 两相光学切片结构照明显微镜中，减少采集时间会引入残留伪影，传统去噪方法难以有效抑制，而监督训练又受限于缺乏干净的光学切片地面真实数据

Method: 研究编码器-解码器网络用于两相OS-SI中的伪影减少，使用将真实伪影场应用于合成图像形成的合成训练对，训练非对称去噪自编码器和U-Net网络

Result: 两种网络都提高了图像清晰度，各自在不同类型的伪影上表现优异

Conclusion: 合成训练使得OS-SI图像的监督去噪成为可能，突显了编码器-解码器网络在简化重建工作流程方面的潜力
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖少量高变基因，限制了预测范围并忽略了协调的生物学程序。

Method: 使用ssGSEA计算通路激活分数，通过transformer编码通路信号，并通过对比学习与组织学特征对齐。

Result: 在乳腺癌、皮肤癌和淋巴结癌数据集中，PEaRL在基因和通路水平表达预测上均优于现有方法，Pearson相关系数分别提高达58.9%和20.4%。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学影像分析中任务特定模型缺乏通用性和预后能力的问题，以及现有通用方法条件控制简单和医学语义理解不足的局限性。
...
Conclusion: DuPLUS是一个通用且临床相关的医学图像分析解决方案，能够快速适应新任务和不同中心的模态数据。
...
Method: 集成线程化检测模型(TDM)，并行执行YOLOv10检测和MobileSAM分割。YOLOv10处理检测，MobileSAM执行轻量级分割，两者并发运行以提高资源利用效率。

Result: 在隐蔽的Houbara鸨(保护优先物种)上，模型达到mAP50为0.9627，mAP75为0.7731，mAP95为0.7178，MobileSAM mIoU为0.7421。YOLOv10每帧处理时间为43.7毫秒，确认实时就绪。

Conclusion: 该框架通过线程化方法显著提高了实时性能，减少了延迟，并引入了包含40,000张标注图像的Houbara数据集来支持模型训练和评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Transformer缺乏对几何对称性的归纳偏置，而现有的等变方法通常通过复杂的设计牺牲了Transformer的效率和灵活性。本文旨在解决这一权衡问题。

Method: 通过定义相对于柏拉图立体对称群参考框架的注意力机制，引入原则性的权重共享方案，使模型能够同时保持对连续平移和柏拉图对称性的等变性，同时保持标准Transformer的架构和计算成本。

Result: 在计算机视觉（CIFAR-10）、3D点云（ScanObjectNN）和分子性质预测（QM9, OMol25）等多个基准测试中，Platonic Transformer通过利用几何约束实现了有竞争力的性能，且没有额外的计算成本。

Conclusion: Platonic Transformer成功解决了Transformer缺乏几何对称性归纳偏置的问题，通过创新的参考框架设计实现了高效的等变性，为几何感知的深度学习提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在未知领域的泛化能力仍然是一个重大挑战，特别是在语义分割任务中，这对生物医学和自动驾驶等应用领域至关重要。领域泛化方法旨在无需访问目标域数据的情况下实现跨多个未见领域的泛化。

Method: 作者通过聚类和回顾现有方法，识别了领域泛化研究的发展趋势，特别是向基于基础模型的范式转变。同时进行了广泛的性能比较分析。

Result: 性能比较显示基础模型对领域泛化具有显著影响，这标志着该领域的重要发展方向。

Conclusion: 本综述旨在推动领域泛化研究的发展，并激励科学家探索新的研究方向，特别是在基础模型应用方面。
...
<details>
  <summary>Details</summary>
Motivation: 解决从人类手绘草图到3D无人机飞行路径的转换问题，使非专业人士能够通过简单绘图控制无人机导航。

Method: 包含SketchAdapter（学习将手绘草图映射到2D投影路径）和DiffPath（从2D投影和第一人称深度图像推断3D轨迹的扩散模型），使用混合标注数据和模块化设计。

Result: 在真实世界无人机测试中，低/中等杂乱环境成功率100%，未见高杂乱环境成功率40%，比关键消融实验提升20-60%的任务完成率。

Conclusion: 混合人工标注和自动标注数据训练，结合模块化设计，显著提升了模型正确理解人类意图和推断3D路径的能力。
...
<details>
  <summary>Details</summary>
Motivation: AI说话头视频会议系统通过传输紧凑的姿态-表情潜在空间来减少带宽，但该潜在空间可被操纵，使攻击者能够实时劫持受害者形象。由于每帧都是合成的，传统深度伪造检测器完全失效。

Method: 利用姿态-表情潜在空间包含驱动身份生物特征的关键观察，提出姿态条件的大间隔对比编码器，从传输的潜在空间中分离出持久身份线索，同时消除瞬态姿态和表情信息。

Result: 在多个说话头生成模型上的实验表明，该方法始终优于现有防御方法，能够实时运行，并对分布外场景表现出强大的泛化能力。

Conclusion: 该方法首次实现了无需查看重建RGB视频的生物特征泄露防御，通过简单的余弦测试即可在视频渲染时标记非法身份交换。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自回归视频扩散模型难以实现流式、细粒度的输出控制，无法确保与用户期望一致。需要一种能够随时对任何内容进行交互式拖拽编辑的方法。

Method: 提出训练无关的DragStream方法，包含自适应分布自校正策略（利用相邻帧统计约束潜在嵌入漂移）和空间频率选择性优化机制（选择性传播视觉线索以减少上下文干扰）。

Result: 该方法可无缝集成到现有自回归视频扩散模型中，大量实验证实了DragStream的有效性。

Conclusion: REVEL任务和DragStream方法成功解决了流式拖拽视频编辑中的关键挑战，实现了细粒度的交互式视频操控。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在计算病理学中提供了强大的通用特征提取器，但为特定诊断任务调整和基准测试单个模型耗时耗力，特别是考虑到其规模和多样性。

Method: 提出Group-Aggregative Selection多实例学习（GAS-MIL）框架，灵活集成多个基础模型的特征，保留其互补优势。

Result: 在前列腺癌（PANDA）、卵巢癌（UBC-OCEAN）和乳腺癌（TCGA-BrCa）三个数据集上的分类任务中，GAS-MIL始终优于或与单个基础模型和现有MIL方法持平。

Conclusion: GAS-MIL通过高效整合异构基础模型，简化了病理学模型部署，为未来多模态和精准肿瘤学应用提供了可扩展基础。
...
<details>
  <summary>Details</summary>
Motivation: 无人机快速递送纳洛酮为应对阿片类药物过量紧急情况提供了有前景的解决方案，但旁观者的情境感知在人类-自主系统协作中至关重要，目前缺乏实时情境感知评估方法。

Method: 创建DANDSD数据集，收集模拟阿片类药物过量紧急情况中非医疗训练旁观者的行为数据。提出基于视频的实时情境感知评估框架，整合几何、运动学和交互图特征，使用图嵌入和Transformer模型进行预测。

Result: 该方法实现了高性能的情境感知预测，在时间分割准确性上优于FINCH基线方法9%的帧平均准确率和5%的交并比。
...
Method: 使用231个产品（1,628张图像）的数据集，通过所有四种模型处理以评估速度和覆盖率，创建包含113张图像（60个产品）的真实子集进行准确性评估。
...
Conclusion: 这些结果为包装特定任务提供了基准，建立了基线，并强调了布局感知方法和文本定位的发展方向。
...
Method: 提出FrameOracle模块，使用四阶段课程学习训练，前三阶段依赖跨模态相似度等弱代理信号，最后阶段利用新构建的FrameOracle-41K数据集提供的强监督关键帧标注。

Result: 在5个VLM和6个基准测试中，FrameOracle将16帧输入平均减少到10.4帧而不损失准确率；从64帧候选帧中平均减少到13.9帧，同时准确率提升1.4%。

Conclusion: FrameOracle在可扩展视频理解方面实现了最先进的效率-准确性权衡，显著提升了视频语言模型的效率。
...
<details>
  <summary>Details</summary>
Motivation: 游戏视觉bug的手动识别成本高且需要专业知识，而监督学习模型依赖大量标记数据，但这类bug出现频率低，导致标记数据获取困难。

Method: 采用混合协同微调方法，整合目标游戏和同领域游戏的标记样本，并加入未标记数据来增强特征表示学习，最大化利用所有可用数据。

Result: 实验结果显示该方法在多种游戏环境中优于传统基线方法，即使在仅使用50%目标游戏标记数据时仍保持竞争力。

Conclusion: CFT方法显著降低了对特定目标游戏标记数据的依赖，提高了游戏视觉bug检测的可扩展性和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 研究HRM模型（包含两个Transformer模块、DEQ风格单步训练、深度监督等组件）是否可以作为实用的图像分类器。
...
Conclusion: 在无增强的小分辨率图像分类任务中，当前HRM不如简单卷积架构，但模型修改可能带来显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习在视觉特征学习方面取得了显著进展，DINOv2超越了弱监督方法，需要系统分析其技术原理和性能优势。

Method: 采用多裁剪视图增强和带有平均教师的自蒸馏方法，基于transformer架构学习视觉特征。

Result: DINOv2在大多数基准测试中超越了OpenCLIP等弱监督方法，其学习的特征具有显著的涌现特性。

Conclusion: DINOv2代表了自监督学习的重要进展，但仍存在局限性，需要进一步研究发展方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决FSCIL中由于数据稀缺导致的泛化能力不足问题，以及现有扩散模型直接应用时出现的语义不对齐和引导无效等挑战。

Method: 提出DCS框架，建立扩散模型与分类器的相互促进循环，采用奖励对齐学习策略，在特征层面确保语义一致性和多样性，在logits层面促进探索性图像生成和类间区分度。

Result: 在FSCIL基准测试中达到最先进性能，显著增强了知识保留和新类学习能力。

Conclusion: DCS框架通过扩散模型与分类器的协同进化有效解决了FSCIL中的数据稀缺和语义对齐问题，为小样本增量学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统人工检查在矿业等高危领域存在劳动密集、易出错且难以应对大规模动态环境的问题，迫切需要智能自动化的安全监控解决方案。

Method: 提出MonitorVLM框架，包含三个关键创新：1) 包含9,000个VQA样本的领域特定违规数据集；2) 动态选择Top-K相关条款的条款过滤器模块，降低推理延迟13.56%；3) 增强工人区域的行为放大器模块，提升细粒度动作识别。

Result: 实验结果显示MonitorVLM显著优于基线视觉-语言模型，在精度、召回率和F1分数上分别提升22.01%、34.22%和28.37%。行为放大器模块额外带来3.45%精度提升和8.62%召回率提升。

Conclusion: 该研究展示了多模态大模型在提升矿业及其他领域职业安全监控方面的潜力，通过轻量级Web界面集成到实际工作流程中，实现带视频时间戳的自动违规报告。
...
<details>
  <summary>Details</summary>
Motivation: 传统分类方法在理解复杂数据分布方面存在局限性，需要更强大的模型来改进交通事故检测。

Method: 使用基于ExceptionNet架构的引导分类与扩散技术相结合的混合模型，通过时间嵌入和图像协变量嵌入动态调整网络行为。

Result: 在公开数据集上的评估显示，该扩散模型在基于图像的交通事故检测中表现最佳，准确率为97.32%。

Conclusion: 扩散模型在智能交通系统中具有显著优势，能够有效理解复杂数据分布，提升事故检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了两种模态的不平衡收敛以及高低激活之间的显著梯度差异，这限制了RGB-T显著目标检测性能的进一步提升。

Method: 使用单模态监督增强非主导模态学习，采用梯度解冲突技术减少冲突梯度影响，并利用两个解耦适配器分别处理高低激活神经元以增强背景学习。

Result: 在RGB-T SOD基准数据集、涂鸦监督RGB-T SOD、全监督RGB-D SOD数据集以及全监督RGB-D轨道表面缺陷检测上的实验都证明了该方法的有效性。

Conclusion: 提出的SAMSOD模型通过解决模态不平衡收敛和梯度差异问题，在多个RGB-T和RGB-D任务上表现出优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言学习在指代表达理解方面取得进展，但定位极小物体仍然是重要挑战，特别是在自动驾驶等实际应用中

Method: 构建了包含10万对指代表达和边界框的SOREC数据集，并提出了参数高效的渐进迭代缩放适配器PIZA用于微调模型

Result: 在SOREC数据集上应用PIZA到GroundingDINO模型，显著提高了小目标定位的准确率

Conclusion: SOREC数据集和PIZA方法有效解决了小目标指代表达理解的挑战，为自动驾驶等应用提供了重要工具
...
<details>
  <summary>Details</summary>
Motivation: 视网膜血管分割为动静脉是视网膜血管分析的前提，这种分析可以为识别和诊断各种视网膜眼病提供潜在见解和生物标志物。血管规律性和宽度的改变可以作为全身血管系统健康的指标，帮助识别高风险血管疾病患者。

Method: 将注意力机制整合到WNet深度学习模型中，构建了Attention-WNet模型，专门用于视网膜动静脉分割任务。

Result: 在公开可用的HRF和DRIVE数据集上测试，提出的方法在性能上超越了文献中现有的最先进模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模多模态数据集（如LAION-400M）缺乏人口统计标注的问题，以理解训练数据在产生模型人口偏见中的作用。

Method: 通过验证的自动标注流程，结合目标检测、多模态字幕生成和微调分类器，为整个数据集创建人物中心标注，包括边界框、感知性别和种族/民族标签以及自动生成的字幕。

Result: 发现了人口不平衡和有害关联，例如男性和被感知为黑人或中东裔的个体与犯罪相关和负面内容的不成比例关联。CLIP和Stable Diffusion中60-70%的性别偏见可以通过数据中的直接共现关系线性解释。

Conclusion: 该资源首次建立了数据集组成与下游模型偏见之间的大规模经验联系，为理解模型偏见的数据来源提供了实证基础。
...
TL;DR: 本文提出LoRA补丁技术，通过向Deepfake生成器注入可插拔的LoRA补丁来绕过现有的主动防御系统，同时提出防御性LoRA补丁作为补充解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有的Deepfake主动防御系统缺乏鲁棒性和可靠性，存在安全漏洞，需要更强大的防御策略。

Method: 使用LoRA补丁技术，结合可学习门控机制防止梯度爆炸，并引入多模态特征对齐损失来在语义层面对齐特征。

Result: 仅用1000个面部样本和单轮微调，LoRA补丁就成功击败了多种主动防御系统。

Conclusion: 当前防御范式存在严重弱点，需要开发更鲁棒的Deepfake防御策略。
...
TL;DR: 提出了一种参考集微调(RSF)方法，通过在测试时参考集上进行简单微调，显著提升视觉地点识别(VPR)方法在具有挑战性数据集上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前VPR方法在训练-测试领域差异较大的挑战性基准上表现不佳，而测试时参考集包含目标领域的图像和姿态信息，可作为弥补领域差距的补充信息来源。

Method: 在测试时参考集上对VPR模型进行简单微调，利用目标领域的地图数据提升模型性能，同时保持泛化能力。

Result: 在挑战性数据集上平均Recall@1提升了约2.3%，且该方法在不同测试数据集上均有效。

Conclusion: 参考集微调是一种有效提升VPR性能的简单方法，能够利用测试时参考集信息弥补训练-测试领域差距。
...
TL;DR: ARSAM通过自适应采样-重用-混合分解梯度的方法，在保持SAM泛化能力的同时，将计算成本降低约40%。


<details>
  <summary>Details</summary>
Motivation: SAM虽然能提升模型泛化能力，但计算成本是SGD的两倍，需要找到加速方法。

Method: 将SAM梯度分解为SGD梯度和二阶梯度在一阶梯度上的投影(PSF)，并重用PSF来减少计算量。

Result: 在CIFAR-10/100等数据集上达到与SAM相当的精度，速度提升约40%，并在姿态估计、模型量化等任务中验证有效性。
...
TL;DR: 提出CoPA框架，通过概念提示和聚合机制，从视觉编码器的多层提取概念表示，提升医学影像诊断中概念和疾病预测性能
...
Method: CoPA框架包含概念感知嵌入生成器(CEG)从各层提取概念表示，概念提示调优(CPT)引导模型关注关键概念线索，多层视觉表示聚合对齐文本概念表示

Result: 在三个公开数据集上，CoPA超越了现有最优方法，有效提升了概念和疾病预测性能

Conclusion: CoPA通过多层概念提取和提示引导机制，显著提升了医学影像诊断的透明度和准确性
...
TL;DR: ZFP压缩技术可以在保持脑血管分割质量的同时，大幅减少3D医学影像数据大小，最高压缩比达22.89:1，Dice系数仅从0.8774降至0.87656。


<details>
  <summary>Details</summary>
Motivation: 3D医学影像数据集日益庞大复杂，阻碍了协作研究和可移植性，需要找到既能压缩数据又不影响自动化分割性能的方法。

Method: 在包含真实血管分割的大规模3D医学数据集上，应用ZFP压缩技术的容错模式和固定速率模式，并与未压缩基准进行严格比较。

Result: ZFP实现了显著的数据缩减（最高22.89:1压缩比），同时保持了高保真度，平均Dice系数保持在0.87656的高水平。

Conclusion: ZFP是实现大规模医学数据集更高效、更易访问研究的可行且强大的工具，有助于促进更广泛的社区协作。
...
TL;DR: 提出了一种混合分割架构，集成CNN、Transformer和Mamba注意力融合机制，通过三分支编码器捕获局部、全局和长程依赖关系，在保持计算效率的同时提升医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割模型多为任务特定，在不同模态和解剖区域性能差异大，且难以平衡模型复杂度和性能，特别是在临床环境中需要同时保证准确性和效率。

Method: 采用三分支编码器整合CNN、Transformer和Mamba注意力融合机制，结合多尺度注意力CNN解码器重建细粒度分割图，并使用协同注意力门增强跨尺度特征选择和交互。
...
TL;DR: 使用YOLOv9算法和多边形标注的深度学习方法来自动检测道路损坏和井盖，在孟加拉国达卡收集的数据集上训练，总体准确率78.1%，在Broken和Not Broken类别表现良好，但Manhole检测因类别不平衡而效果较差。
...
Method: 采用YOLOv9算法和比传统边界框更精确的多边形标注方法，开发包含1000多张图像的数据集，训练模型识别Broken、Not Broken和Manhole三个类别。

Result: 总体图像级准确率达到78.1%，Broken类别F1分数86.7%，Not Broken类别F1分数89.2%，但Manhole检测F1分数仅18.2%，主要由于类别不平衡问题。

Conclusion: 该方法为发展中国家城市基础设施监控提供了高效且可扩展的解决方案，尽管在Manhole检测方面仍需改进。
...
TL;DR: 提出了一种基于时间相关对比学习的无配对图像到图像翻译方法，结合扩散模型和对比学习来保持域不变特征并丢弃域特定特征。


<details>
  <summary>Details</summary>
Motivation: 无配对图像翻译缺乏对齐样本，扩散模型能生成高质量多样化输出，对比学习能学习语义相似性，两者结合适合无配对翻译场景。

Method: 使用时间相关对比学习训练模型，将图像与其域不变特征作为正对，然后用学习到的对比模型指导预训练SDE进行图像翻译。

Result: 在三个常见无配对I2I任务上与多个基线比较，在多个指标上达到与最先进方法相当的结果，收敛速度显著更快且无需标签监督或分类器训练。
...
TL;DR: LIBERO-PRO是一个扩展的VLA模型基准测试，通过系统性地评估模型在四个维度扰动下的表现，揭示了现有模型在标准测试中90%+的准确率在广义设置下崩溃至0.0%，暴露了模型依赖训练集记忆而非真正任务理解的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LIBERO基准测试的训练和评估设置存在问题，导致性能估计虚高，无法公平比较模型。需要开发更严格的评估方法来测试模型的泛化能力和真实理解能力。

Method: 引入LIBERO-PRO基准，在四个维度上系统评估模型性能：操作对象、初始状态、任务指令和环境的合理扰动。

Result: 实验结果显示，现有模型在标准LIBERO评估中达到90%以上准确率，但在广义设置下性能崩溃至0.0%，表明模型依赖动作序列和环境布局的记忆而非真正理解。

Conclusion: 当前评估方法存在严重缺陷，呼吁社区放弃误导性方法，采用能稳健评估模型泛化和理解能力的方法。
...
TL;DR: Mirage数据集包含具有可见伪影的AI生成图像，当前最先进的检测方法大多失败。研究发现大型视觉语言模型能有效检测有伪影的AI图像，但对无伪影图像检测性能下降。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像越来越难以被标准检测器识别，但人类仍能区分。研究旨在解决这一差距，并探索大型视觉语言模型在可解释AI图像检测中的应用。

Method: 引入Mirage数据集，包含各种具有可见伪影的AI生成图像；研究大型视觉语言模型在AI图像检测任务中的表现。

Result: 大型视觉语言模型能有效检测具有可见伪影的AI生成图像，但在面对缺乏此类线索的图像时性能下降。

Conclusion: 大型视觉语言模型在检测有伪影的AI图像方面表现出色，但在更逼真的AI图像检测方面仍有局限性，需要进一步研究。
...
TL;DR: UGround提出了一种统一的视觉定位范式，通过动态选择Transformer中间层作为“掩码提示”，解决了传统方法依赖固定最后一层和<SEG>作为提示的问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉定位方法存在两个主要问题：1) 依赖固定的最后一层隐藏层，导致误差逐层累积；2) 使用<SEG>作为提示，缺乏明确的空间线索。

Method: 采用Policy-Prompted Masking方法，包含随机跳跃连接(SSC)和掩码作为提示(MasP)。SSC通过强化学习动态选择连接层，MasP使用相似度图作为软逻辑掩码来提示SAM生成掩码。

Result: UGround首次在单一框架内统一了从传统参考表达式分割到推理分割、单目标到多目标、正查询到错误前提的各种视觉定位任务。

Conclusion: UGround通过动态层选择和明确的空间线索，提供了一种更有效的统一视觉定位解决方案，代码和模型已开源。
...
TL;DR: OMG4是一个优化的4D高斯泼溅框架，通过三阶段渐进式剪枝和量化技术，显著减少4D场景表示的存储开销，在保持重建质量的同时将模型大小减少超过60%。


<details>
  <summary>Details</summary>
Motivation: 4D高斯泼溅技术虽然能实时渲染复杂动态场景，但面临存储开销过大的问题，需要数百万个高斯函数进行高保真重建。现有方法在压缩比或视觉质量方面仍存在局限。
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测模型在固定类别集上训练，缺乏灵活性且扩展成本高。开放词汇目标检测能够识别未见类别，但航拍图像与地面视图存在域偏移、视角变化和尺度差异，需要专门的适应策略。

Method: 采用对比图像到图像对齐增强航拍与地面视图嵌入的相似性，并使用多实例词汇关联将航拍图像与文本嵌入对齐。
...
Conclusion: 该框架为航拍应用中的目标检测系统提供了更灵活和可扩展的解决方案，成功实现了跨域知识迁移。
...
<details>
  <summary>Details</summary>
Motivation: 皮肤癌是全球最常见和致命的癌症之一，早期检测和诊断对改善患者预后至关重要。深度学习在自动皮肤疾病诊断中显示出巨大潜力，但仍面临复杂特征、图像噪声、类内变异、类间相似性和数据不平衡等挑战。

Method: 采用基于PRISMA框架的综合方法，综述了应对挑战的创新方法，包括数据增强、混合模型和特征融合等。

Result: 深度学习模型在提高皮肤病变检测和分类的准确性和效率方面表现出显著潜力，能够集成到临床工作流程中改善临床决策。

Conclusion: 深度学习有望彻底改变皮肤疾病诊断，但需要持续的技术进步来充分释放其在皮肤病学护理中的变革潜力。
...
TL;DR: 提出SDAKD方法，通过引入学生判别器来解决GAN知识蒸馏中的容量不匹配问题，在图像超分辨率任务上优于现有方法


<details>
  <summary>Details</summary>
Motivation: GANs在资源受限设备上部署困难，传统知识蒸馏方法由于学生生成器与教师判别器之间的容量不匹配而难以有效训练

Method: 提出学生判别器辅助知识蒸馏(SDAKD)，采用三阶段训练策略，在后两个阶段集成适配的特征图蒸馏方法

Result: 在GCFSR和Real-ESRGAN两个超分辨率GAN上的实验表明，SDAKD相比基线方法和SOTA GAN知识蒸馏方法有持续改进

Conclusion: SDAKD通过引入学生判别器有效缓解了GAN知识蒸馏中的容量不匹配问题，为资源受限设备上的GAN部署提供了有效解决方案
...
TL;DR: 提出了PoseGaze-AHP数据集，这是首个同步捕捉头部姿态和注视运动的3D数据集，专门用于眼源性异常头位诊断，包含7,920张图像，提取准确率达91.92%。


<details>
  <summary>Details</summary>
Motivation: 现有数据集分别关注头部姿态和眼部运动，限制了眼源性异常头位综合诊断方法的发展和AI驱动的分析进展。

Method: 使用Claude 3.5 Sonnet模型通过迭代过程从医学文献中提取结构化临床数据，结合逐步、分层和复杂提示策略，然后使用神经头部化身框架将提取的记录系统化填充并转换为3D表示。

Result: 数据集包含来自两种头部纹理的7,920张图像，涵盖广泛的眼部状况，提取方法整体准确率达到91.92%。

Conclusion: PoseGaze-AHP是首个公开可用的专门用于AI驱动眼源性异常头位诊断的资源，支持开发准确且符合隐私要求的诊断工具。
...
TL;DR: 提出了DHQA-4D数据集和DynaMesh-Rater方法，用于动态4D数字人质量评估，通过多维度特征提取和大语言模型融合实现高质量评估。


<details>
  <summary>Details</summary>
Motivation: 随着4D动态数字人技术的发展，在采集、压缩和传输过程中产生的噪声会影响用户体验，因此需要有效的质量评估方法。

Method: 构建DHQA-4D数据集，包含32个高质量4D人体网格序列和1920个失真样本；提出DynaMesh-Rater方法，从投影2D视频提取视觉特征、从裁剪视频提取运动特征、从4D网格提取几何特征，使用大语言模型融合特征并进行LoRA指令调优。

Result: 在DHQA-4D数据集上的实验结果表明，DynaMesh-Rater方法优于现有的质量评估方法。

Conclusion: DHQA-4D数据集和DynaMesh-Rater方法为动态4D数字人质量评估提供了有效的解决方案，能够同时处理带纹理和不带纹理的4D网格。
...
TL;DR: 提出了一种基于自适应空间特征融合(ASFF)的改进ResNet-50模型，用于解决皮肤癌分类中的类间相似性、类内变异性和图像噪声问题，在ISIC 2020数据集上取得了93.18%的准确率。
...
Method: 改进ResNet-50模型，引入自适应空间特征融合机制，通过双分支设计融合高层语义特征和中层细节特征，使用全局平均池化和全连接层生成自适应权重进行加权融合。

Result: 在ISIC 2020数据集子集(3297张图像)上，模型准确率达到93.18%，AUC值在P-R和ROC曲线上分别为0.9670和0.9717，优于5种经典CNN模型。Grad-CAM评估显示模型能自适应关注病变相关区域。
...
Result: 放射学模态验证准确率100%，病理学模态95.12%，临床图像63.10%（视觉异质性导致），集成模型在多模态验证集上整体准确率达84.58%。

Conclusion: 该多模态集成框架提供了一种无创的AI辅助分诊工具，增强高风险病变的早期识别，支持临床决策，符合全球肿瘤学指南以减少诊断延迟。
...
TL;DR: 本文挑战了数据规模定律，提出了一种基于聚类的数据选择方法IQA-Select，仅使用10%的数据就能在可解释图像质量评估任务中达到甚至超过全量数据微调的性能。


<details>
  <summary>Details</summary>
Motivation: 当前可解释图像质量评估方法依赖大规模指令调优数据集，但这会导致高昂计算成本和数据冗余问题。本文旨在研究指令调优数据质量对模型性能的影响，并寻求更高效的数据选择方法。

Method: 首先验证不同规模指令调优数据对模型性能的影响，然后提出三阶段聚类数据选择框架：聚类特征提取、聚类配额分配和聚类采样策略，最终开发出IQA-Select方法。

Result: 实验表明，IQA-Select在Q-Bench和AesBench数据集上仅使用10%的选定数据，就能分别达到全量微调102.1%和103.7%的性能，显著降低计算成本的同时获得更好性能。

Conclusion: 数据质量比数据规模更重要，通过精心选择高质量数据子集，可以在大幅减少计算成本的同时提升可解释图像质量评估模型的性能。
...
TL;DR: 提出了一种基于可泛化动作专家的框架，使用稀疏3D轨迹作为中间表示，将VLM的高级规划能力与低级物理动作模块连接起来，解决了传统VLA模型泛化能力差和双系统方法语义模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉-语言-动作模型在将推理能力转化为物理世界时面临泛化能力差的问题，而双系统方法又受到动作模块语义模糊的限制，需要在新环境中微调。

Method: 使用稀疏3D轨迹作为中间表示，VLM生成粗略3D路径点，然后由可泛化动作专家通过采样实时点云观测将其细化为密集可执行动作序列，采用"动作预训练、点云微调"的训练范式。

Result: 该方法结合了VLM在视觉理解和规划方面的广泛泛化能力，以及动作专家在细粒度动作级别的泛化能力。

Conclusion: 提出的框架通过可泛化动作专家和3D轨迹中间表示，有效解决了VLA模型在物理世界中的泛化问题，实现了高效训练和鲁棒泛化。
...
<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在视觉语言推理任务上表现出色，但其在零样本细粒度图像分类（需要精确区分视觉相似类别）方面的潜力尚未充分探索。

Method: 将零样本细粒度图像分类转化为视觉问答框架，利用LVLMs的综合理解能力而非直接类别名称生成，采用新颖的注意力干预技术提升性能，并开发更全面的类别描述基准。
...
TL;DR: 本文对自动驾驶感知系统在雾天条件下的去雾方法进行了系统性实证研究，评估了多种去雾方法对下游检测和分割任务的实际效果。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶感知系统在雾天条件下特别脆弱，现有去雾方法在图像质量上的改进并不总能转化为更好的下游检测和分割性能，且先前评估多依赖合成数据，缺乏真实世界的可迁移性验证。

Method: 使用Foggy Cityscapes数据集，评估了(i)传统滤波器、(ii)现代去雾网络、(iii)链式变体（滤波器→模型、模型→滤波器）以及(iv)基于提示的视觉-语言图像编辑模型等多种去雾流程。

Result: 分析揭示了去雾何时有效、链式方法何时产生协同或退化效应，以及VLM编辑器与专用方法的比较。VLM评判的定性评分与任务指标（特别是mAP）显示出强相关性。

Conclusion: 研究为去雾方法建立了透明、任务导向的基准，明确了在何种条件下预处理能真正改善恶劣天气下的自动驾驶感知性能。
...
Method: 提出CAMEO级联框架，通过精心设计的组件桥接文本到运动模型和条件视频扩散模型，包括分析文本提示和视觉条件来有效训练VDM，并引入相机感知条件模块自动选择与输入文本对齐的视角。

Result: 在MovieGen基准和新引入的T2M-VDM组合基准上证明了方法的有效性，展示了其在多样化用例中的多功能性。

Conclusion: CAMEO框架成功解决了通用人体运动视频生成的挑战，通过级联方法实现了文本到运动模型与视频扩散模型的无缝集成，提高了生成视频的连贯性和质量。
...
TL;DR: OpenFLAME是一个联邦视觉定位系统，允许独立组织分别3D扫描和维护自己的空间，解决集中式VPS无法覆盖私有室内空间的问题。


<details>
  <summary>Details</summary>
Motivation: 集中式视觉定位系统无法覆盖私有室内空间，存在隐私担忧、监管限制和维护瓶颈。需要分布式解决方案来扩大覆盖范围并保护隐私。

Method: 采用联邦图像定位概念，各组织独立维护VPS服务，通过参考解决方案管理和合并跨地图数据而不共享私有数据。

Result: 实现了分布式VPS后端，支持访问控制、分布式维护和更大覆盖范围，解决了跨空间定位结果一致性、服务质量控制等挑战。

Conclusion: 联邦VPS方法能够有效解决集中式系统的局限性，为大规模AR应用提供更全面的6DoF定位支持。
...
TL;DR: 开发了一个结合CNN-LSTM模型提取生物力学特征和大型语言模型生成反馈的框架，用于网球击球分析，旨在提供技术准确且可操作的反馈。


<details>
  <summary>Details</summary>
Motivation: 现有系统未能将生物力学洞察转化为对球员和教练可访问且有意义的语言反馈，本研究旨在填补这一空白。

Method: 使用CNN-LSTM模型从运动数据中提取关键生物力学特征（如关节角度、肢体速度和动力链模式），并利用大型语言模型基于这些特征生成反馈。

Result: 该方法在THETIS数据集上进行了评估，重点关注分类性能和可解释性，连接了可解释AI与运动生物力学之间的差距。

Conclusion: 该框架成功地将生物力学分析与可操作的语言反馈相结合，为网球击球分析提供了技术准确且对终端用户有意义的解决方案。
...
TL;DR: TimeWarp方法通过创建针对性的合成时序数据集来改进视频大语言模型的时序理解能力，在七个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在需要细粒度时序理解的任务上表现不佳，主要原因是现有微调数据集缺乏视觉复杂性和时序细节，导致模型过度依赖语言推理而非真正理解视频动态。

Method: 提出TimeWarp方法，系统性创建针对性的合成时序数据集来微调模型，使其更关注输入视频内容，并构建大规模偏好数据集捕捉常被忽略的复杂时序动态。

Result: 应用TimeWarp方法后，现有模型在时序理解基准测试中的性能显著提升，在七个基准测试中均取得绝对性能改进。

Conclusion: TimeWarp方法通过针对性时序数据集有效提升了视频大语言模型的时序理解能力，证明了所提数据集在推进视频时序理解方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型通常预训练时使用短文本窗口(<77个token)，这导致长格式生物医学描述被截断。然而，大规模开源文献中的生物医学描述有很大部分远超77个token，因此需要研究长上下文预训练的影响。

Method: 通过扩展视觉语言模型中文本编码器的上下文长度，使用BIOMEDICA-LongCAP数据集（包含100万图像-描述对，具有来自全文文章的情境感知描述）训练BMC-LongCLIP模型，文本编码器支持最多512个token的窗口。

Result: BMC-LongCLIP将上下文容量扩展了6.6倍，token浪费从55%减少到2.2%。在长描述检索基准测试中，Recall@1获得高达+30%的绝对增益，分类平均提升+2%，且收敛速度比短上下文模型更快。
...
TL;DR: 提出了一种可控伪标签生成（CPG）框架，用于解决长尾半监督学习中未标记数据分布未知的问题，通过动态可控过滤机制构建已知分布的标记数据集，并结合贝叶斯最优分类器和类感知自适应增强模块，在多个基准数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前的长尾半监督学习方法假设未标记数据遵循预定义的分布（如长尾、均匀或反长尾），但实际上未标记数据的分布通常是未知且任意的，这限制了现有方法的性能。

Method: CPG框架通过可控自增强优化循环：1）动态可控过滤机制从未标记数据中选择可靠伪标签，确保更新后的标记数据集遵循已知分布；2）基于更新后的分布构建贝叶斯最优分类器；3）改进的分类器帮助识别更多可靠伪标签。还包含类感知自适应增强模块和辅助分支。

Result: 在多个常用基准数据集上的综合评估表明，CPG实现了持续改进，在准确率上比最先进方法提升了高达15.97%。

Conclusion: CPG框架有效解决了未标记数据分布未知的问题，通过可控伪标签生成和优化循环显著降低了泛化误差，在长尾半监督学习任务中表现优异。
...
TL;DR: 提出基于PaddleOCRv5的微调方法，显著提升越南古典汉字文本识别准确率，从37.5%提升至50%，特别在噪声图像条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有OCR系统在处理越南历史文献中的古典汉字文本时，难以应对扫描质量差、字形不标准和手写体变化等问题。

Method: 使用精选的越南古典汉字手稿子集对PaddleOCRv5的文本识别模块进行微调，构建完整的训练流程包括预处理、LMDB转换、评估和可视化。

Result: 实验结果显示准确率显著提升，从37.5%提高到50.0%，在噪声图像条件下表现尤其突出。

Conclusion: 该方法有效提升了古典汉字文本识别性能，并开发了交互式演示系统，支持汉越语义对齐、机器翻译和历史语言学研究等下游应用。
...
<details>
  <summary>Details</summary>
Motivation: 隐式神经表示在信号表示方面表现出色，但不适合分割等预测任务。需要一种方法让INR能够学习语义结构。

Method: 使用一个基础INR同时预测像素强度值和类别标签，通过元学习找到最优初始参数，使得INR只需微调即可适应新图像并自动解码类别标签。

Result: 在2D和3D脑MRI分割任务中，Dice分数与常用U-Net模型相当，但参数数量减少了90%。

Conclusion: MetaSeg为医学图像分割提供了一个新颖、可扩展的替代方案，相比传统的U-Net和视觉变换器架构更加轻量级。
...
TL;DR: Video-in-the-Loop (ViTL) 是一个两阶段长视频问答框架，通过先定位问题相关片段再重新分配视觉token来在固定token预算下提高问答性能，同时引入新的数据集支持带时间跨度的多选问答。


<details>
  <summary>Details</summary>
Motivation: 解决长视频问答中固定token预算下的效率问题，传统方法在长视频处理时计算成本高，需要更高效的token分配策略来提升性能。

Method: 两阶段框架：1) 使用低帧率定位问题相关时间片段；2) 在更高有效帧率下重新分配视觉token进行问答，采用端到端训练和交错组相对目标函数。

Result: 在固定token预算下，ViTL在长视频问答和时间定位任务上性能提升达8.6%，同时减少50%的帧输入，跨感知token重新分配始终优于均匀采样。

Conclusion: ViTL和数据集的结合为可扩展的长视频问答提供了可解释且计算高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 短视频平台假新闻已成为严重社会问题，现有检测器因训练数据有限且多样性不足，导致模式偏见和性能受限。真实场景中视频素材与虚假新闻事件存在复杂的多对多关系，但现有数据集未能充分反映这种关系。

Method: AgentAug框架实现四种伪造类别的LLM驱动流水线来模拟新闻视频创作过程，结合基于不确定性采样的主动学习策略选择有用的增强样本。

Result: 在两个基准数据集上的实验结果表明，AgentAug能够持续提升短视频假新闻检测器的性能。

Conclusion: AgentAug通过模拟真实创作过程的数据增强方法，有效解决了假新闻视频检测中的数据稀疏和多样性不足问题，为假新闻检测提供了新的解决方案。
...
TL;DR: 该研究通过探索和优化超参数，提高了基于提示的图像编辑框架的精确性和可靠性，提出了三种方法来解决现有问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于稳定扩散模型的图像编辑方法虽然简化了编辑过程，但结果存在变异性（如发色变化不一致），需要提高精度和可靠性。

Method: 提出了三种方法：全面的"词语替换"方法研究、开发"注意力重加权方法"以提高适应性、提出"CL P2P"框架解决循环不一致等现有限制。

Result: 研究有助于理解超参数设置与神经网络模型架构选择（特别是注意力机制）之间的相互作用，这些因素显著影响生成图像的构成和质量。

Conclusion: 这项工作通过优化超参数和注意力机制，提升了文本驱动图像编辑的精确性和可靠性，为相关领域提供了重要贡献。
...
TL;DR: GUI-Spotlight是一个用于图像推理的模型，通过动态调用多个专用工具迭代缩小屏幕相关区域，显著提高了视觉定位准确性，在ScreenSpot-Pro基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图形用户界面系统中已扩展到复杂现实环境，但实际应用仍受限于视觉定位的可靠性，即无法准确将文本引用映射到屏幕元素，从而影响指针级操作的准确性。

Method: 引入GUI-Spotlight模型，通过图像推理动态调用多个专用工具，迭代缩小关注区域至屏幕相关部分，从而大幅提升视觉定位精度。

Result: 在ScreenSpot-Pro基准测试中，仅使用18.5K训练样本的GUI-Spotlight达到52.8%准确率，超越了V2P-7B（50.6%，使用9.6M样本）和GTA-1-7B（50.1%，使用1.56M样本）。

Conclusion: GUI-Spotlight通过迭代聚焦方法有效解决了视觉定位问题，在少量训练数据下实现了优于现有方法的性能，为GUI系统的实际应用提供了更可靠的视觉基础。
...
TL;DR: 提出一种用于后训练量化的范围估计方法，通过层间局部最小值最小化量化误差，在ResNet系列和Inception-v3模型上实现8位和6位量化几乎无精度损失，4位量化精度显著提升。
...
Method: 将范围估计建模为通过层间局部最小值最小化量化误差的优化问题，证明该问题局部凸性并提出高效搜索算法，在变换权重空间中应用该算法进一步改进。

Result: 在ResNet系列和Inception-v3模型的图像分类任务中，8位和6位量化几乎无top-1精度损失，4位量化精度显著提升，优于现有最优方法。

Conclusion: 提出的范围估计方法能有效提升后训练量化性能，在低比特设置下保持模型精度。
...
TL;DR: MetaFind是一个面向元宇宙场景生成的三模态组合检索框架，通过从大规模3D资源库中检索资产来解决场景生成中的一致性问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D资产检索方法中的两个核心问题：(i) 忽视空间、语义和风格约束的不一致检索；(ii) 缺乏专门针对3D资产检索的标准化范式。

Method: 提出灵活的检索机制，支持文本、图像和3D模态的任意组合查询，通过ESSGNN等变布局编码器联合建模对象级特征和场景级布局结构。
...
TL;DR: 提出一种改进的损失函数，将耀斑子类别的序数信息整合到二元交叉熵损失中，通过序数加权来增强模型对阈值附近事件的区分能力。


<details>
  <summary>Details</summary>
Motivation: 传统的二元分类框架忽略了耀斑子类别之间的序数关系，导致模型在预测阈值附近容易产生误分类。

Method: 在传统二元交叉熵损失基础上，集成耀斑子类别的序数信息，创建序数感知的数据驱动正则化方法，对阈值附近的错误预测施加更重的惩罚。

Result: 该方法旨在通过利用数据的序数特性来增强模型学习过程，提高整体性能。

Conclusion: 通过整合序数信息到损失函数中，可以改善模型对耀斑强度的区分能力，特别是在预测阈值附近的表现。
...
TL;DR: QuantDemoire是一个专为图像去摩尔纹任务设计的后训练量化框架，通过异常值感知量化器和频率感知校准策略，在保持质量的同时大幅减少模型参数和计算量。


<details>
  <summary>Details</summary>
Motivation: 现有去摩尔纹深度学习模型需要大量计算资源，难以部署在边缘设备上。直接应用现有量化方法会导致严重的性能下降，主要原因是分布异常值和平滑区域表示弱化。

Method: 1. 异常值感知量化器：使用基于采样的范围估计减少激活异常值，将少量极端权重保留为FP16格式；2. 频率感知校准策略：在校准过程中强调低频和中频分量，减轻低比特量化引起的带状伪影。

Result: QuantDemoire在保持质量的同时大幅减少了参数和计算量，在W4A4配置下比现有量化方法性能提升超过4dB。

Conclusion: 该框架为去摩尔纹模型在边缘设备上的高效部署提供了有效解决方案，在性能和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 解决极稀疏视角下CT重建的病态问题和纹理丢失问题，结合生成先验和物理约束提升重建质量。

Method: 结合扩散生成先验(NCSN++ SDE模型)和TV、核范数正则化，采用2D切片策略，使用FFT加速和并行优化。

Result: 在AAPM-2016、CTHD和LIDC数据集上，TV-LoRA在SSIM、纹理恢复、边缘清晰度和伪影抑制方面均优于基准方法。

Conclusion: TV-LoRA实现了高保真、高效的3D CT重建，在低剂量稀疏采样场景下具有广泛的临床适用性。
...
<details>
  <summary>Details</summary>
Motivation: 拓扑映射领域缺乏标准化的评估指标、数据集和协议，现有系统在不同环境和标准下评估，无法进行公平可复现的比较。感知混淆这一关键挑战也未被充分量化。

Method: 1) 将拓扑一致性形式化为拓扑映射的基本属性，并证明定位精度可作为高效可解释的替代指标；2) 提出首个数据集模糊度的量化方法，实现跨环境的公平比较；3) 构建具有校准模糊度水平的多样化基准数据集，实现并发布深度学习基线系统。

Result: 实验和分析揭示了当前方法在感知混淆下的局限性，为拓扑映射研究提供了新的见解。所有数据集、基线和评估工具都已开源。

Conclusion: 该工作通过建立标准化评估协议、量化数据集模糊度和提供开源基准，推动了拓扑映射领域的公平、一致和可复现研究。
...
TL;DR: 本文提出了基于事件相机的网格流估计新任务，创建了高分辨率事件网格流数据集HREM，开发了轻量级EEMFlow网络进行快速准确的网格流估计，并扩展为支持稠密光流的EEMFlow+，同时提出自适应密度模块ADM提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前事件相机光流估计领域存在两个关键问题：缺乏专门的网格流数据集和方法，以及事件数据密度挑战未被充分探索。

Method: 1) 创建大规模高分辨率事件网格流数据集HREM；2) 提出轻量级EEMFlow网络，采用编码器-解码器架构；3) 升级为EEMFlow+，加入置信度诱导细节补全模块；4) 提出自适应密度模块ADM调节输入事件数据密度。

Result: EEMFlow模型相比现有最优方法在运行效率上快30倍，ADM模块使EEMFlow和EEMFlow+性能分别提升8%和10%。

Conclusion: 本文在事件相机网格流估计领域填补了重要空白，提出的数据集、网络架构和密度调节方法为后续研究提供了坚实基础。
...
Conclusion: 该方法简单有效，通过预训练策略和采样指导机制解决了现有方法的局限性，在6D物体姿态估计任务中实现了更好的性能和效率。
...
TL;DR: 本文提出了一种解决多模态大语言模型蒸馏中概念漂移问题的新方法，通过引入"学习、比较、批评"范式和自主偏好优化，使学生模型能够从多个教师的推理轨迹中学习并进行概念对齐。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型蒸馏中，多个教师的推理轨迹存在概念漂移问题，导致推理分布不可预测地演化并将偏见传递给学生模型，最终损害其性能。

Method: 提出"学习、比较、批评"范式，通过自主偏好优化(APO)实现概念对齐。学生模型在教师指导下学习并自蒸馏偏好思维，然后对教师的漂移推理进行批判性反思。
...
TL;DR: SiteShield是一个基于多模态大视觉语言模型的检索增强生成框架，通过整合视觉和音频输入来自动化建筑安全检查报告生成。
...
Method: 开发了SiteShield多模态LVLM检索增强生成框架，整合视觉和音频输入，利用真实世界数据进行训练。

Result: SiteShield在F1分数(0.82)、汉明损失(0.04)、精确率(0.76)和召回率(0.96)方面均优于无RAG的单模态LLM。
...
TL;DR: 提出了DP-IR模块化扩散概率图像恢复框架，结合预训练IR网络和生成DPMs，仅需训练小模块即可适应不同IR任务，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有DPMs在图像恢复任务中表现优异但存在任务特定性、训练成本高的问题，限制了实际应用。

Method: 模块化框架结合预训练IR网络和生成DPMs，仅需训练0.7M参数的小模块，采用高效采样策略减少4倍神经网络评估次数。

Result: 在burst JDD-SR、动态场景去模糊和超分辨率任务上，感知质量优于现有方法，保真度指标保持竞争力。

Conclusion: DP-IR框架实现了DPMs在图像恢复任务中的实用化应用，平衡了性能与计算效率。
...
TL;DR: BLADE是一个无需先验偏置知识或偏置冲突样本的生成式去偏框架，通过训练生成模型在偏置域间转换图像并保留任务相关特征，自适应地优化图像以减轻神经网络中的隐式偏置问题。
...
Method: BLADE训练生成模型在偏置域间转换图像，基于图像对偏置的敏感性自适应优化，通过对齐共享任务特征但偏置不同的图像对，同时错开共享相同偏置的样本，来鼓励鲁棒表示。

Result: 在多个基准数据集上，BLADE显著优于最先进方法，在损坏CIFAR-10数据集的最差组设置下，比最接近的基线绝对提升了约18%。
...
### [94] [From Segments to Concepts: Interpretable Image Classification via Concept-Guided Segmentation](https://arxiv.org/abs/2510.04180)
*Ran Eisenberg,Amit Rozner,Ethan Fetaya,Ofir Lindenbaum*

Main category: cs.CV

TL;DR: 提出了SEG-MIL-CBM框架，将概念引导的图像分割与基于注意力的多实例学习相结合，通过语义区域推理实现透明、空间定位的概念级解释。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络缺乏可解释性，在安全关键应用中限制了可信度。现有概念瓶颈模型需要昂贵的概念标注且缺乏空间定位能力。

Method: 将图像分割为语义区域作为实例，在注意力多实例学习框架中聚合证据，通过概念引导推理突出任务相关证据。

Result: 在涉及伪相关、输入损坏和大规模基准测试的设置中实现稳健性能，提供透明概念级解释。

Conclusion: SEG-MIL-CBM无需概念或组标注即可产生空间定位的概念级解释，在保持性能的同时提高模型透明度和鲁棒性。
...
### [95] [Let Features Decide Their Own Solvers: Hybrid Feature Caching for Diffusion Transformers](https://arxiv.org/abs/2510.04188)
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器的迭代采样过程由于每个时间步都需要昂贵的前向传播而成为主要瓶颈，现有缓存方法对所有特征维度采用统一策略，忽略了它们异质的动态行为

Method: 将隐藏特征演化建模为跨维度的ODE混合，引入HyCa框架应用维度级缓存策略，使用混合ODE求解器

Result: 在FLUX上实现5.55倍加速，HunyuanVideo上5.56倍加速，Qwen-Image和Qwen-Image-Edit上6.24倍加速，无需重新训练

Conclusion: HyCa通过维度级缓存策略有效加速扩散变换器采样，在多个领域和模型上实现近无损的高速性能
...
TL;DR: World-To-Image框架通过代理驱动的世界知识增强文本到图像生成，在遇到新颖或分布外实体时，动态搜索网络获取图像信息，进行多模态提示优化，显著提升语义对齐和视觉美学效果。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在遇到新颖或分布外实体时性能显著下降，因为存在固有的知识截止问题。

Method: 设计一个代理动态搜索网络获取基础模型未知概念的图像信息，然后进行多模态提示优化，引导生成骨干网络进行准确合成。

Result: 在NICE基准测试中，语义对齐准确度比最先进方法提升+8.1%，在不到三次迭代中实现高效结果。
...
TL;DR: 提出MASC框架，通过构建层次化语义树来结构化视觉token嵌入空间，显著提升自回归图像生成模型的训练效率和生成质量。
...
Method: 使用几何感知距离度量和密度驱动的凝聚构建方法，直接从codebook内在结构构建层次化语义树，将高维预测任务转化为结构化层次任务。

Result: 训练速度提升高达57%，LlamaGen-XL的FID从2.87降至2.58，显著改善生成质量，使自回归框架与最先进方法具有竞争力。

Conclusion: 结构化预测空间对于可扩展生成建模与架构创新同等重要，MASC作为即插即用模块能有效提升现有自回归框架性能。
...
TL;DR: ZoomIn是一个两阶段取证框架，通过模仿人类视觉检查，先扫描图像定位可疑区域，再对放大区域进行聚焦分析，提高AI生成图像检测的准确性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 图像配准问题需要找到对齐两幅图像的变换，使对应点位于相同位置。传统方法可能复杂且需要大量数据，需要一种简单高效的解决方案。

Method: 开发了端到端可训练的算法，使用少量Python代码实现。在立体视觉应用中，使用74张图像在19x15输入窗口上进行训练。

Result: 该方法在训练数据和训练时间有限的情况下仍能获得准确结果，算法简洁性突出，仅需十几行Python代码。

Conclusion: 该算法在代码简洁性方面表现优异，适用于训练数据、训练时间或代码复杂度受限的相关场景，可作为此类问题的良好起点。
...
TL;DR: 提出了一种名为ArConv的新型卷积层，构建了仅含130万参数的高效模型，在RfMiD数据集上达到93.28%的准确率，优于MobileNetV2（220万参数，92.66%准确率），适合移动设备部署。


<details>
  <summary>Details</summary>
Motivation: 提高深度神经网络的可访问性，特别是针对眼部疾病检测等医疗应用，通过优化卷积层设计降低计算复杂度，使模型能在移动设备上运行。

Method: 重新设计和优化卷积层，提出新型ArConv层，构建了仅含130万参数的轻量级模型。

Result: 在RfMiD数据集上测试，模型准确率达到93.28%，优于MobileNetV2的92.66%，同时参数量更少（130万 vs 220万）。

Conclusion: 通过优化卷积层设计，成功开发了高效轻量的神经网络模型，在保持高精度的同时显著降低了计算复杂度，适合移动设备部署。
...
Method: 采用序列到序列的生成神经渲染框架，使用掩码自回归框架和仅解码器的整流流transformer，能够基于任意数量的参考视图生成任意数量的6自由度目标视图。
...
TL;DR: 提出CoSSeg-TTA框架，基于nnU-Netv2，结合半监督学习、领域适应和测试时适应策略，用于Gd-EOB-DTPA增强MRI肝脏分割，在低标注条件下实现优异的跨域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 解决对比增强MRI肝脏分割面临的挑战：标注数据有限、增强协议异质、跨扫描仪和机构的领域偏移问题。传统图像翻译方法在单模态场景中存在结构扭曲和训练不稳定等局限性。

Method: 基于nnU-Netv2构建紧凑分割框架，集成半监督均值教师方案利用未标注数据；包含随机化直方图风格外观转换和可训练对比感知网络的领域适应模块；采用持续测试时适应策略提升推理鲁棒性。

Result: 在广泛实验中，框架持续优于nnU-Netv2基线，获得更高的Dice分数和更小的Hausdorff距离，在低标注条件下对未见领域表现出强泛化能力。

Conclusion: CoSSeg-TTA框架通过结合半监督学习、领域多样化和测试时适应，有效解决了单模态MRI肝脏分割中的领域泛化挑战，为临床诊断和治疗规划提供了可靠工具。
...
TL;DR: 提出了一种基于概念解释的对抗性补丁防御方法，无需事先知道补丁大小或位置，通过抑制最具影响力的概念激活向量来中和补丁影响。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗性补丁防御方法通常需要事先知道补丁大小或位置，这限制了它们的实际应用。

Method: 利用基于概念的解释来识别和抑制最具影响力的概念激活向量，从而在不进行显式检测的情况下中和补丁效应。

Result: 在Imagenette数据集和ResNet-50模型上评估，该方法在鲁棒性和清洁准确率方面均优于最先进的PatchCleanser方法，并在不同补丁大小和位置下保持强劲性能。

Conclusion: 将可解释性与鲁棒性相结合具有前景，概念驱动的防御策略为保护机器学习模型免受对抗性补丁攻击提供了可扩展的方法。
...
TL;DR: 提出Adapt-STformer方法，使用循环可变形Transformer编码器实现灵活高效的序列视觉地点识别，在保持性能的同时显著提升效率和降低内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的序列视觉地点识别方法注重性能但缺乏灵活性（不支持可变序列长度）和效率（推理速度慢、内存占用高），无法满足实时应用需求。

Method: 提出Adapt-STformer方法，核心是循环可变形Transformer编码器（Recurrent-DTE），通过迭代循环机制融合多帧序列信息，自然支持可变序列长度。

Result: 在Nordland、Oxford和NuScenes数据集上，相比次优基线，召回率提升最高17%，序列提取时间减少36%，内存使用降低35%。

Conclusion: Adapt-STformer在保持高性能的同时实现了灵活性、快速推理和低内存使用，解决了现有Seq-VPR方法的效率瓶颈问题。
...
TL;DR: ChronoEdit将图像编辑重新定义为视频生成问题，利用预训练视频生成模型的时间一致性来确保物理一致性，通过时序推理阶段生成物理可行的编辑轨迹。


<details>
  <summary>Details</summary>
Motivation: 当前大型生成模型在图像编辑和上下文图像生成方面取得进展，但在确保物理一致性方面存在关键差距，这对于世界模拟相关任务至关重要。

Method: 将输入和编辑图像视为视频的首尾帧，利用预训练视频生成模型；引入时序推理阶段，在推理时联合去噪目标帧和推理token来想象物理可行的编辑轨迹。

Result: 在PBench-Edit基准测试中，ChronoEdit在视觉保真度和物理合理性方面均优于最先进的基线方法。

Conclusion: ChronoEdit通过将图像编辑重新定义为视频生成问题，有效解决了物理一致性问题，为需要物理一致性的图像编辑任务提供了新思路。
...
TL;DR: CARE-PD是最大的公开帕金森病3D步态网格数据集，包含9个队列的多中心数据，支持临床评分预测和无监督运动任务，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 帕金森病的客观步态评估因缺乏大规模、多样化且临床注释的运动数据集而受限，需要构建一个全面的数据资源来支持研究。

Method: 将RGB视频或运动捕捉数据通过标准化预处理流程转换为匿名SMPL网格，支持监督临床评分预测和无监督运动任务两种基准测试。

Result: 在CARE-PD上预训练可将MPJPE从60.8mm降至7.5mm，并将PD严重程度macro-F1提升17个百分点，运动编码器始终优于手工特征。

Conclusion: CARE-PD展示了临床策划的多样化训练数据的价值，为帕金森病步态分析研究提供了重要资源。
...
TL;DR: GenAR是一个多尺度自回归框架，通过从粗到细的方式预测H&E染色图像中的基因表达，将基因聚类为层次组以捕捉跨基因依赖关系，直接预测原始计数而非连续值，在四个空间转录组数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 空间转录组技术成本高昂，而H&E染色图像广泛可用。现有计算方法存在两个问题：(i) 独立预测每个基因，忽略了共表达结构；(ii) 将任务视为连续回归，而基因表达本质上是离散计数，导致生物学上不可信的输出。

Method: GenAR采用多尺度自回归框架，从粗到细地细化预测。方法包括：将基因聚类为层次组以暴露跨基因依赖关系；将表达建模为无码本的离散标记生成，直接预测原始计数；在解码时融合组织学和空间嵌入。

Result: 在四个不同组织类型的空间转录组数据集上的广泛实验结果表明，GenAR实现了最先进的性能，为精准医学和成本效益高的分子分析提供了潜在应用价值。

Conclusion: 从信息论角度看，离散公式避免了对数诱导的偏差，从粗到细的分解与原则性的条件分解相一致。GenAR框架在预测基因表达方面表现出色，代码已公开可用。
...
TL;DR: 提出RAP方法，通过轻量级3D栅格化替代昂贵渲染，结合特征对齐技术，为端到端驾驶规划提供可扩展的数据增强方案，在多个基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 模仿学习训练的端到端驾驶策略缺乏恢复数据，小错误会迅速累积导致失败。现有基于神经渲染或游戏引擎的方法成本高、速度慢，主要仅用于评估。作者认为训练端到端规划器不需要照片级真实感，重要的是语义保真度和可扩展性。

Method: 提出3D栅格化方法，用轻量级栅格化标注基元替代昂贵渲染，支持反事实恢复机动和跨智能体视图合成。引入Raster-to-Real特征空间对齐来弥合仿真与现实差距。这些组件构成RAP数据增强流水线。

Result: RAP在四个主要基准测试中排名第一：NAVSIM v1/v2、Waymo开放数据集基于视觉的端到端驾驶和Bench2Drive，实现了最先进的闭环鲁棒性和长尾泛化能力。

Conclusion: 轻量级栅格化结合特征对齐足以扩展端到端训练，为照片级渲染提供了实用替代方案，证明了语义保真度比纹理或光照对驾驶规划更重要。
...
TL;DR: 提出Diffusion^2框架用于瞬时轨迹预测，通过两个连接的扩散模型分别生成未观测的历史轨迹和预测未来轨迹，并引入双头参数化机制和时域自适应噪声模块来处理不确定性。
...
Method: Diffusion^2框架包含两个顺序连接的扩散模型：一个用于向后预测生成未观测的历史轨迹，另一个用于向前预测未来轨迹。采用双头参数化机制估计不确定性，并设计时域自适应噪声模块动态调整前向扩散过程中的噪声尺度。

Result: 在ETH/UCY和Stanford Drone数据集上，Diffusion^2在瞬时轨迹预测任务中达到了新的最先进水平。

Conclusion: 该研究提出的Diffusion^2框架有效解决了瞬时轨迹预测问题，通过生成历史轨迹和不确定性处理机制显著提升了预测性能，对提升交通安全具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型局限于2D视图且交互性有限，需要能够生成可控、可编辑的时空环境来支持机器人应用，如可扩展训练数据、可重复评估和灵活任务设计。

Method: 集成轨迹引导生成与特征场蒸馏，从自然语言指令生成具有多视角一致性的4D场景，支持对象级控制（如移动、重新着色、移除）和任意视角观察。

Result: 实验表明MorphoSim在保持高场景保真度的同时实现了可控性和可编辑性，无需完全重新生成即可应用交互式编辑。

Conclusion: MorphoSim为机器人应用提供了语言引导的4D场景生成能力，支持对象级控制和交互式编辑，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多种任务上表现出色，但它们在基本计数能力方面的表现尚未得到系统评估，特别是在组合计数场景下。

Method: 设计了一个简约的基准测试，仅使用基本几何形状及其组合，通过严格控制变量来研究颜色、大小和提示词优化等因素的影响。

Result: 实验结果显示，VLMs在单一形状类型计数时可靠，但在多个形状类型组合计数时出现显著失败。

Conclusion: 当前VLMs在组合计数方面存在根本性经验限制，这为未来研究指明了重要方向。
...
TL;DR: CodeFormer++是一个新颖的盲脸恢复框架，通过分解任务为身份保护恢复、高质量生成和动态融合三个子任务，解决了现有方法在视觉质量和身份保真度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的盲脸恢复方法在集成生成先验时，往往面临视觉质量和身份保真度之间的权衡，导致要么身份失真，要么退化去除效果不佳。

Method: 提出三个关键贡献：基于学习的可变形人脸配准模块、纹理引导的恢复网络以及结合深度度量学习的身份特征与生成特征融合方法。

Result: 在真实世界和合成数据集上的广泛实验表明，CodeFormer++在视觉保真度和身份一致性方面都取得了优越性能。

Conclusion: CodeFormer++框架通过最大化生成先验的效用，实现了高质量人脸恢复同时保持身份特征，解决了盲脸恢复中的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有帧选择方法面临关键权衡：轻量级相似性模型无法捕捉复杂查询的细微差别，而使用VLM进行深度分析虽然准确但计算成本过高。

Method: 利用强大的VLM对复杂查询进行深度语义分析，并将其部署在成本效益高的迭代循环中，每次仅处理一小批最有潜力的帧。

Result: 在各种VideoQA基准测试中，该方法优于现有帧选择方法，显著提升了基础VLM的性能，并在计算效率上比其他基于VLM的技术有显著提升。

Conclusion: A.I.R.方法有效解决了视频问答中帧选择的计算效率与准确性之间的权衡问题，为实际应用提供了可行的解决方案。
...
TL;DR: 提出reAR训练策略，通过token-wise正则化解决AR视觉生成中的生成器-分词器不一致问题，显著提升性能


<details>
  <summary>Details</summary>
Motivation: 视觉自回归生成性能不如扩散模型，主要瓶颈是生成器-分词器不一致问题，即AR生成的token可能无法被分词器良好解码

Method: 引入token-wise正则化目标：在预测下一个token时，因果transformer同时学习恢复当前token的视觉嵌入，并在噪声上下文中预测目标token的嵌入
...
TL;DR: SPEGNet提出了一种统一的伪装目标检测架构，通过通道校准和空间增强整合多尺度特征，在保持语义-空间对齐的同时直接生成边界，实现了边界精度和区域一致性的平衡。


<details>
  <summary>Details</summary>
Motivation: 当前伪装目标检测方法依赖累积复杂组件（边界模块、注意力机制、多尺度处理器等），导致计算负担增加但性能提升不成比例。为管理复杂度，这些方法在降低分辨率下处理，丢失了伪装检测所需的精细细节。

Method: SPEGNet采用统一设计，通过通道校准和空间增强整合多尺度特征。边界直接从上下文丰富的表示中产生，保持语义-空间对齐。渐进式细化实现了尺度自适应的边缘调制，在中间分辨率处达到峰值影响。

Result: SPEGNet在CAMO数据集上达到0.887 Sα，COD10K上0.890，NC4K上0.895，并具有实时推理速度。该方法在不同尺度（从微小复杂物体到大型模式相似物体）以及处理遮挡和模糊边界方面表现优异。

Conclusion: SPEGNet通过统一架构有效解决了伪装目标检测中的碎片化问题，在保持高精度的同时实现了实时性能，为复杂场景下的伪装检测提供了有效解决方案。
...
TL;DR: MedCLM是一个将检测数据集转换为大规模医学视觉问答数据的自动化流程，通过将病灶框与器官分割和结构化推理链接，生成带有逐步推理的问题-答案对，并在多个医学VQA基准测试中达到最先进性能。
...
Method: 提出MedCLM自动化流程，将检测数据集转换为带有链式推理的大规模医学VQA数据；采用集成CoT-课程策略，包括显式病灶框的简单阶段、鼓励隐式定位的中等阶段和弱监督推理的困难阶段。
...
TL;DR: 提出了VaseVQA-3D数据集和VaseVLM模型，解决视觉语言模型在3D花瓶文物分析中的领域适应问题，在希腊古陶器分析任务上显著提升性能。
...
Method: 构建首个3D视觉问答数据集VaseVQA-3D，包含664个古希腊花瓶3D模型及对应问答数据，并开发领域自适应训练的VaseVLM模型。

Result: 在VaseVQA-3D数据集上，R@1指标提升12.8%，词汇相似度提升6.6%，显著优于现有最佳方法。

Conclusion: 该方法有效提升了3D花瓶文物的识别与理解能力，为数字遗产保护研究提供了新的技术路径。
...
TL;DR: TBStar-Edit是一个专门为电商领域设计的图像编辑模型，通过数据工程、模型架构设计和两阶段训练策略，在保持产品外观和布局完整性的同时实现精确高保真度的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有通用图像生成和编辑模型在电商场景中存在一致性限制，无法有效保持产品外观和布局的完整性。

Method: 建立全面的数据构建管道获取高质量编辑数据；设计包含基础模型、模式转换模块和一致性增强模块的分层模型框架；采用两阶段训练策略：第一阶段进行编辑模式转换，第二阶段进行一致性增强。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型采用同步降噪，所有像素同时从随机噪声演化到清晰图像，导致提示相关区域只能参考相同噪声水平的无关区域，无法获得清晰上下文，最终损害文本到图像对齐。

Method: 异步扩散模型框架，为不同像素分配不同的时间步长，重新制定逐像素降噪过程。通过动态调节单个像素的时间步长调度，使提示相关区域比无关区域更渐进地降噪。

Result: 广泛实验表明，异步扩散模型能够显著改善各种提示下的文本到图像对齐效果。

Conclusion: 异步扩散模型通过异步降噪机制有效解决了传统扩散模型在文本到图像对齐方面的局限性，为高质量图像生成提供了新思路。
...
TL;DR: 提出了Tangential Amplifying Guidance (TAG)，一种高效的扩散模型引导方法，通过放大轨迹信号的切向分量来修正采样轨迹，无需修改底层扩散模型架构。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像生成中常出现语义不一致或幻觉问题，现有推理时引导方法往往依赖外部信号或架构修改，引入额外计算开销。

Method: 利用中间样本作为投影基础，放大估计分数相对于该基础的切向分量来修正采样轨迹，基于一阶泰勒展开形式化引导过程。

Result: TAG是一种即插即用、架构无关的模块，能以最小计算成本提升扩散采样保真度。

Conclusion: TAG为扩散引导提供了新视角，通过直接操作轨迹信号实现更高效的语义一致性增强。
...
TL;DR: 提出条件表示学习(CRL)方法，通过用户指定的条件生成定制化表示，解决通用表示学习与下游任务需求不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 传统表示学习方法学习的是通用表示，主要捕捉主导语义，可能与定制化下游任务需求不一致。例如动物栖息地分析中，研究者需要场景相关特征，但通用嵌入强调类别语义，导致结果不理想。
...
TL;DR: 提出了AI Session Recorder系统，通过记录病理学家在WSI查看器中的导航行为，构建Pathology-CoT数据集，并开发了Pathologist-o3智能代理系统，在胃肠道淋巴结转移检测中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决病理学诊断中缺乏可扩展、临床对齐的行为监督数据的问题，这些基于经验的专家查看行为通常未被记录在教材或在线资源中。

Method: 使用AI Session Recorder记录标准WSI查看器中的导航行为，转换为标准化行为命令；通过轻量级人工审查生成Pathology-CoT数据集；构建两阶段Pathologist-o3代理系统进行区域提议和行为引导推理。

Result: 在胃肠道淋巴结转移检测中达到84.5%精确率、100.0%召回率和75.4%准确率，超过最先进的OpenAI o3模型，并在不同骨干网络上具有良好泛化性。

Conclusion: 该框架将日常查看器日志转化为可扩展的专家验证监督，使智能病理学代理系统实用化，并建立了人类对齐、可升级的临床AI发展路径。
...
TL;DR: 提出S²Fin网络，通过空间-光谱-频率交互融合，解决多模态遥感图像分类中结构特征和细节特征提取不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有特征融合方法在处理异构冗余多模态图像时，难以有效提取结构和细节特征，需要引入频域学习来建模关键稀疏细节特征。

Method: 设计高频稀疏增强Transformer，采用稀疏空间-光谱注意力优化高频滤波器参数；提出两级空间-频率融合策略，包括自适应频率通道模块和高频共振掩码；使用空间-光谱注意力融合模块增强网络中间层特征提取。

Result: 在四个基准多模态数据集上，S²Fin在有限标注数据下实现了优于现有方法的分类性能。

Conclusion: S²Fin通过空间-光谱-频率域的有效融合，显著提升了多模态遥感图像分类性能，证明了频域学习在特征提取中的重要性。
...
TL;DR: 提出一种新颖的集成框架，结合基于transformer的架构和基于纹理的方法，用于检测深度伪造媒体，在DFWild-Cup数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度伪造技术的兴起，检测篡改媒体变得至关重要。现有方法难以在不同数据集和生成技术间泛化，需要更鲁棒的解决方案。

Method: 采用集成框架，结合Swin Transformers和ViTs等transformer架构与基于纹理的方法。引入创新技术包括数据分割、顺序训练、频率分割、基于补丁的注意力和人脸分割。

Result: 在包含八个深度伪造数据集的DFWild-Cup数据集上实现了最先进的检测性能。集成方法受益于不同方法的互补性。

Conclusion: 混合模型能有效应对深度伪造检测的不断演变的挑战，为实际应用提供鲁棒解决方案。
...
TL;DR: 本文研究了不同图像分割方法对热带森林砍伐检测分类器训练的影响，发现通过分类器融合方法可以显著提升检测精度。
...
Result: 最初结果显示不同分割方法间性能差异不大，但应用分类器融合后，平衡准确率有明显提升。
...
TL;DR: EduPersona是一个面向教育领域的大规模基准测试，专注于评估语言模型在课堂模拟中的主观能力，包含多语言、多学科和多种人格类型的数据集和评估框架。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在教育中的广泛应用，虚拟学生代理在课堂模拟和教师培训中变得重要，但它们在课堂导向的主观能力尚未得到充分评估，这限制了模型边界的理解和可信部署。

Method: 构建了EduPersona基准测试，包含1,308个真实课堂对话轮次，对应12,814个师生问答轮次，并通过人格风格化扩展到约128k轮次。将主观性能分解为三个渐进任务：基本连贯性、学生真实性和长期人格一致性。

Result: 在三个代表性LLM上的系统实验显示，经过EduPersona人格微调的变体在所有任务上都有显著提升：TASK1 +33.6%、TASK2 +30.6%、TASK3 +14.9%。
...
TL;DR: 提出了一种分层多阶段运动专家混合（MoME）架构，用于从步态序列中多任务预测心理属性，在PsyMo基准测试中优于现有步态分析模型。


<details>
  <summary>Details</summary>
Motivation: 步态编码了丰富的生物特征和行为信息，但利用行走方式来推断心理特质仍然是一个具有挑战性且未被充分探索的问题。

Method: 使用分层多阶段运动专家混合（MoME）架构，通过四个运动复杂度阶段处理行走周期，采用轻量级专家模型提取时空特征，并使用任务特定门控模块跨特征和阶段自适应加权专家。

Result: 在涵盖17个心理特质的PsyMo基准测试中，该方法在运行级别获得37.47%的加权F1分数，在受试者级别获得44.6%的分数，优于现有步态分析模型。

Conclusion: 研究表明多任务步态学习在心理特质估计方面具有可行性，为基于运动信息的心理推断未来研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 多概念个性化中，多个学习的概念在输出图像中会相互干扰或混合，这是主要挑战。

Method: 提出Token-wise Value Adaptation (ToVA)训练方法，仅调整交叉注意力中的值投影；以及Latent Optimization for Disentangled Attention (LODA)推理优化，通过优化输入潜变量来解耦注意力。

Result: 通过广泛的定性和定量实验，证明ConceptSplit能够实现稳健的多概念个性化，减轻意外的概念干扰。

Conclusion: ConceptSplit框架通过训练和推理阶段的创新方法，有效解决了多概念个性化中的概念混合问题。
...
Method: 整合基础级3D分割主干网络进行微调，通过跨伪监督协同训练利用未标注数据，采用标准化预处理流程，无需空间配准。

Result: 模型在标注和未标注域中均表现出稳健的分割性能，能够跨MRI相位和厂商进行泛化。

Conclusion: 该方法展示了结合基础模型适应与协同训练在真实世界临床成像任务中的潜力，为多相多厂商MRI肝脏分割提供了有效的标签高效基线。
...
TL;DR: 提出了一种基于扩散模型的人脸表情生成框架，能够保持身份一致性的同时实现精细的表情控制，支持从基础情感到微表情的生成，并在真实图像中进行表情编辑。
...
Method: 基于ID一致的人脸基础模型，采用组合式设计，包含由FLAME blendshape参数引导的表情交叉注意力模块，并在包含丰富表情变化的图像和视频数据上进行训练。

Result: 模型在定制化和身份一致的表情生成方面优于现有方法，能够生成从基础情感到微妙微表情的各种表情，并支持真实图像的表情编辑。

Conclusion: 该框架成功实现了身份一致性和精细表情控制的结合，为AI驱动叙事提供了更高质量的人类中心生成模型。
...
TL;DR: ReactDiff是一个用于生成多样化面部反应的时间扩散框架，通过整合时空面部运动学和面部动作单元依赖关系来生成自然的人类面部反应。
...
Method: 提出ReactDiff时间扩散框架，整合两个关键先验：时间面部行为运动学和面部动作单元依赖关系，指导模型生成符合人类面部解剖学约束的平滑连贯反应。

Result: 在REACT2024数据集上的实验表明，该方法在反应质量、多样性和反应适切性方面均达到最先进水平。

Conclusion: ReactDiff通过整合面部运动学和动作单元约束，成功生成了多样化且符合人类面部解剖学的自然面部反应，解决了现有方法的局限性。
...
Method: 设计了高判别性的物体特征编码器，采用对比预训练策略将物体表示学习与场景图预测解耦，同时结合几何和语义特征来优化关系预测。

Result: 在3DSSG数据集上的实验表明，该方法在所有评估指标上显著优于现有最先进方法，将预训练编码器集成到现有框架中也能带来大幅性能提升。

Conclusion: 该方法通过改进物体特征表示和关系信息整合，有效解决了3D语义场景图预测中的关键挑战，为机器人和AR/VR应用提供了更可靠的技术基础。
...
Method: 使用93张带有校准ChARUCO图案的相机陷阱图像作为基准，评估4种先进单目深度估计方法（Depth Anything V2、ML Depth Pro、ZoeDepth、Metric3D）和几何基线方法，比较中值与均值深度提取策略。

Result: Depth Anything V2表现最佳，平均绝对误差0.454m，相关性0.962；ZoeDepth在户外自然环境中性能显著下降（MAE 3.087m）；中值深度提取在所有深度学习方法中均优于均值方法；ZoeDepth最快（0.17s/图像）但精度最低，Depth Anything V2在精度和速度间达到最佳平衡（0.22s/图像）。

Conclusion: 建立了野生动物应用中的性能基准，为在保护监测系统中实施深度估计提供了实用指导，Depth Anything V2是当前最佳选择。
...
TL;DR: 提出ExposureEngine系统，使用定向边界框(OBB)准确量化体育转播中的赞助商可见度，解决了传统水平边界框(HBB)在旋转或倾斜logo检测中的不准确问题。


<details>
  <summary>Details</summary>
Motivation: 传统赞助商可见度分析依赖手动、主观且不可扩展的方法，而现有自动化系统使用水平边界框在logo旋转或透视变形时会导致曝光指标不准确。

Method: 开发端到端系统，预测定向边界框(OBB)精确拟合各种方向的logo，使用包含1,103帧瑞典精英足球比赛图像的数据集训练检测器，并集成语言驱动代理层支持自然语言查询。

Result: 模型在mAP@0.5上达到0.859，精度0.96，召回率0.87，在多样化转播条件下稳健定位logo，并提供精确的可见度指标计算。

Conclusion: ExposureEngine为体育媒体提供了可审计、可解释的赞助商测量全面解决方案，包括数据集和分析仪表板。
...
TL;DR: 提出了AA-YOLO方法，将统计异常检测集成到YOLO检测头中，有效控制红外小目标检测中的误报率，在多种场景下表现出色。
...
Conclusion: AA-YOLO具有高度通用性，可应用于各种YOLO主干网络，包括轻量级模型，是资源受限实际部署的有吸引力的解决方案。
...
### [136] [Visual Representations inside the Language Model](https://arxiv.org/abs/2510.04819)
*Benlin Liu,Amita Kamath,Madeleine Grunde-McLaughlin,Winson Han,Ranjay Krishna*

Main category: cs.CV

TL;DR: 本文研究了多模态语言模型(MLMs)在感知任务上的表现不佳问题，通过分析视觉键值令牌的信息流，发现图像值令牌包含足够信息执行多种感知任务，但语言模型对视觉信息的处理存在缺陷。


<details>
  <summary>Details</summary>
Motivation: 理解为什么多模态语言模型在感知密集型任务上表现不佳，特别是视觉键值令牌在语言模型中的处理机制。

Method: 分析流行MLMs(LLaVA-OneVision、Qwen2.5-VL、Llama-3-LLaVA-NeXT)中视觉键值令牌的信息流，研究图像值令牌的感知能力，比较语言模型与视觉编码器的信息保留程度。

Result: 发现图像值令牌能零样本执行分割、语义对应、时序对应等感知任务；语言模型会增强视觉信息但整体感知能力有限；输入无关的图像键令牌包含降低感知能力的伪影；添加文本前缀可改善视觉表示。
...
TL;DR: 该论文研究了基于Transformer的架构在自然面对面对话场景中的人员识别性能，使用双流框架分别建模空间配置和时间运动模式，在CANDOR对话语料库上取得了98.03%的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 研究在自然面对面对话场景中使用Transformer架构进行人员识别的性能，探索空间配置和时间运动模式在人员识别中的作用。

Method: 实现并评估了一个双流框架，分别处理133个COCO WholeBody关键点的空间配置和时间运动模式，比较了预训练和从头训练，研究了速度特征的使用，并引入了多尺度时间Transformer进行分层运动建模。
...
Conclusion: PG-Occ框架通过渐进式增强表示和自适应采样策略，有效平衡了计算效率与场景细节捕捉能力，为开放词汇3D场景理解提供了有效解决方案。
...
TL;DR: 提出了一种新的开放词汇学习方法，通过生成未见类别数据来估计开放环境中的分布，包括类别-领域数据生成流程和分布对齐算法，在11个数据集上优于基线方法达14%。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅使用可见类别数据估计开放环境分布，但由于未见类别的缺失，估计误差本质上不可识别。学习超越可见类别对于边界估计误差至关重要。

Method: 提出类别-领域数据生成流程（在层次语义树和领域信息指导下生成未见类别数据）和分布对齐算法（估计并最大化后验概率以增强泛化能力）。

Result: 在11个数据集上的广泛实验表明，该方法比基线方法性能提升高达14%，证明了其有效性和优越性。

Conclusion: 通过生成未见类别数据可以有效估计开放环境中的分布，理论证明估计误差有上界，实验验证了方法的有效性。
...
### [140] [Paper2Video: Automatic Video Generation from Scientific Papers](https://arxiv.org/abs/2510.05096)
*Zeyu Zhu,Kevin Qinghong Lin,Mike Zheng Shou*

Main category: cs.CV

TL;DR: PaperTalker是一个用于学术演示视频生成的多智能体框架，包含首个包含101篇研究论文及其演示视频的基准数据集，并提出了四个专门的评估指标来衡量视频传达论文信息的效果。


<details>
  <summary>Details</summary>
Motivation: 学术演示视频制作过程高度劳动密集，需要协调幻灯片、字幕、语音和演讲者等多个对齐通道，而现有方法难以处理研究论文中的密集多模态信息。

Method: 提出了一个多智能体框架，集成了幻灯片生成与布局优化、光标定位、字幕生成、语音合成和说话人头像渲染，并采用并行化幻灯片生成以提高效率。

Result: 在Paper2Video数据集上的实验表明，该方法生成的演示视频比现有基线方法更忠实和富有信息量。

Conclusion: 这项工作为自动化和即用型学术视频生成迈出了实际的一步，提供了数据集、智能体框架和代码资源。
...
### [141] [Federated Learning for Surgical Vision in Appendicitis Classification: Results of the FedSurg EndoVis 2024 Challenge](https://arxiv.org/abs/2510.04772)
...
TL;DR: FedSurg挑战赛是首个针对手术视频分类的联邦学习基准测试，评估模型在未见临床中心的泛化能力和本地微调适应性，使用Appendix300数据集进行炎症阶段分类。


<details>
  <summary>Details</summary>
Motivation: 评估当前联邦学习方法在手术视频分类中的表现，特别是模型对未见临床中心的泛化能力和本地微调适应性，同时实现不共享患者数据的协作模型开发。

Method: 参与者使用基础模型线性探测、度量学习（三元组损失）和各种FL聚合方案（FedAvg、FedMedian、FedSAM），在Appendix300数据集上分类阑尾炎炎症阶段，评估泛化和适应两个任务。

Result: 泛化任务中跨中心性能有限，适应任务中所有团队在微调后都有改进但排名稳定性低。ViViT模型表现最佳，挑战揭示了泛化限制、类别不平衡敏感性和去中心化训练中超参数调优困难。

Conclusion: FedSurg挑战赛建立了手术视频分类FL策略的首个基准，强调了本地个性化与全局鲁棒性之间的权衡，以及架构选择、预处理和损失设计的重要性，为未来开发不平衡感知、自适应和鲁棒FL方法提供参考。
...
### [142] [Hands-Free Heritage: Automated 3D Scanning for Cultural Heritage Digitization](https://arxiv.org/abs/2510.04781)
*Javed Ahmad,Federico Dassiè,Selene Frascella,Gabriele Marchello,Ferdinando Cannella,Arianna Traviglia*

Main category: cs.CV

TL;DR: 提出了一个自动化双机器人3D扫描系统，通过协调机器人操作和高分辨率扫描，实现文化遗产文物的高质量数字化，无需手持或半自动工作流程。


<details>
  <summary>Details</summary>
Motivation: 传统3D扫描方法需要专业知识和人工干预来维持最佳扫描条件和覆盖范围，这限制了文化遗产数字化的效率和可及性。

Method: 将扫描空间参数化为不同区域，使用扫描机器人和托盘处理机器人进行协调运动规划，优化轨迹规划和路径点分布以确保全面表面覆盖。

Result: 实验结果显示，该方法相比基线方法显著降低了Chamfer Distance，提高了F-score，具有更优的几何精度和数字化效率。

Conclusion: 该系统实现了文化遗产文物的高效自动化3D扫描，减少了对专家操作人员的依赖，为文化遗产保护提供了可靠的技术解决方案。
...
### [143] [A Comparative Study of Vision Transformers and CNNs for Few-Shot Rigid Transformation and Fundamental Matrix Estimation](https://arxiv.org/abs/2510.04794)
*Alon Kaya,Igal Bilik,Inna Stainvas*

Main category: cs.CV

TL;DR: 本文系统比较了ViT和CNN在几何估计任务中的表现，发现在大数据场景下ViT表现更优，但在小数据场景下CNN的归纳偏置使其能与ViT匹敌，且ViT在跨域评估中泛化能力更强。


<details>
  <summary>Details</summary>
Motivation: 研究ViT和CNN在几何估计任务（如图像刚性变换和基础矩阵估计）中的效率，特别是在低数据量情况下的表现，以确定最适合的骨干网络架构。

Method: 系统比较了大规模CNN（ResNet、EfficientNet、CLIP-ResNet）与ViT基础模型（CLIP-ViT变体和DINO）在不同数据量设置下的表现，包括少样本场景。

Result: 在大数据下游场景中，ViT在精调时优于CNN；在小数据场景中，CNN的归纳偏置和较小容量使其性能与ViT相当；ViT在跨域评估中表现出更强的泛化能力。

Conclusion: 需要根据具体场景仔细选择模型架构进行精调，未来研究应关注平衡局部和全局表示的混合架构。
...
TL;DR: 提出了DiT-VTON，一个基于扩散变换器的虚拟试穿框架，通过多种配置探索最佳图像条件设置，在扩展数据集上训练以增强鲁棒性，支持多品类虚拟试穿和高级图像编辑功能。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿模型在细节保留、真实图像鲁棒性、高效采样、图像编辑能力和跨品类泛化方面存在挑战，需要更强大的解决方案。

Method: 利用扩散变换器适应图像条件虚拟试穿任务，探索多种配置（上下文标记连接、通道连接、ControlNet集成），在包含多样化背景和非服装类别的扩展数据集上训练。

Result: 在VITON-HD数据集上超越最先进方法，实现更好的细节保留和鲁棒性，无需额外条件编码器；在涵盖数千产品类别的多样化数据集上优于具有VTA和图像编辑能力的模型。

Conclusion: DiT-VTON重新定义了虚拟试穿任务，提供了通用的虚拟试穿解决方案，支持多种产品类别和高级图像编辑功能，展示了数据扩展对VTO适应性的益处。
...
Method: 结合几何驱动的神经渲染和基于扩散的视图增强技术，从壁挂固定摄像头视频合成任意时刻的任意自我中心视角。

Result: 在多站点手术案例和对照研究中，EgoSurg能够以高视觉质量和保真度重建人员特定的视觉场和任意视点。

Conclusion: EgoSurg将现有手术室摄像头基础设施转变为可导航的动态3D记录，为沉浸式手术数据科学建立了新基础，使手术实践可以从每个角度可视化、体验和分析。
...
Method: 包含两个关键模块：互易流校正器（无先验光流校正策略）和非线性变形器（将高斯图分解为视角姿态不变和特定分量）。

Result: 实验表明AvatarVTON实现了高保真度、多样性和动态服装真实感，适用于AR/VR、游戏和数字人应用。

Conclusion: 该框架在4D虚拟试穿中表现出色，为相关应用提供了有效的解决方案。
...
TL;DR: 使用3D Flow Matching框架从MRI或CBCT生成合成CT，在SynthRAD2025挑战赛基准上评估，能准确重建全局解剖结构但细节保留有限。


<details>
  <summary>Details</summary>
Motivation: 通过MRI-only和CBCT自适应放疗，提高治疗精度同时减少患者辐射暴露。

Method: 采用完全3D Flow Matching框架，将高斯噪声体积通过学习的速度场转换为sCT图像，使用轻量级3D编码器从输入MRI或CBCT提取特征。

Result: 在SynthRAD2025挑战赛基准上验证，方法能准确重建全局解剖结构，但细部细节保留有限，主要受限于训练分辨率和内存约束。
...
TL;DR: 提出AT-BPTT框架，通过动态调整截断位置和窗口大小来改进数据集蒸馏中的内循环优化，在多个数据集上实现SOTA性能并显著提升效率


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法使用随机截断策略缺乏灵活性且效果不佳，观察到神经网络在不同训练阶段具有不同的学习动态

Method: AT-BPTT框架包含三部分：基于概率的阶段感知时间步选择、基于梯度变化的自适应窗口大小策略、低秩Hessian近似降低计算开销

Result: 在CIFAR-10、CIFAR-100、Tiny-ImageNet和ImageNet-1K上平均准确率提升6.16%，内循环优化加速3.9倍，节省63%内存成本

Conclusion: AT-BPTT通过动态截断策略有效解决了数据集蒸馏中的优化问题，在性能和效率方面均取得显著提升
...
TL;DR: 提出了一种基于航空图像的太阳能电站自动映射方法，能够实现从模块级到整体结构的详细建模，无需依赖第三方数据。


<details>
  <summary>Details</summary>
Motivation: 精确且更新的光伏电站模型对于优化运维至关重要，但现有模型不易获取，需要自动化且独立于第三方数据的解决方案。

Method: 利用航空图像进行视觉分割，识别光伏模块和结构布局关键点，通过多图像融合保持结构完整性，实现模块到行列的层级分配。

Result: 在两个不同电站上进行了实验验证，成功生成了紧凑的地理参考模型，适用于电站维护。

Conclusion: 该方法能够自动化光伏电站映射过程，提供详细的模块级建模，为电站运维提供了有效工具。
...
Method: 结合空间-时间图卷积网络（ST-GCN）和卷积神经网络（CNN），利用迁移学习避免手动定义物理动作与心理类别的映射关系。

Result: 在DUET数据集上的结果表明，该方法能够实现可扩展、准确且以人为本的行为建模。

Conclusion: 该框架为增强强化学习驱动的人类-环境交互模拟提供了新途径，同时保护用户匿名性。
...
TL;DR: 该论文比较了五种基于骨骼的交互识别算法，用于识别12种二元人类交互，旨在解决CPS系统忽视社会效益的问题，为隐私保护的社交行为测量提供基础。


<details>
  <summary>Details</summary>
Motivation: 传统网络物理系统(CPS)主要关注经济目标如性能和安全，但忽视了人类中心的社会效益。网络物理社会基础设施系统(CPSIS)旨在通过将CPS与社会目标对齐来解决这一问题，需要开发隐私保护的社交行为测量方法。

Method: 使用深度传感器分析骨骼运动来识别二元人类交互，避免了RGB摄像头的隐私问题。比较了五种基于骨骼的交互识别算法，在包含12种二元交互的数据集上进行测试，这些交互按沟通类型分类，如标志性动作和情感表达。

Result: 研究比较了五种骨架基交互识别算法的性能，为识别二元人类交互提供了有效方法，这些交互反映了人类互动的文化和情感层面。

Conclusion: 基于骨骼的交互识别为隐私保护的人类社交行为测量提供了可行方案，为网络物理社会基础设施系统的发展奠定了基础，能够更好地理解人类互动的深层含义和相互响应。
...
TL;DR: 提出了一种结合早期退出和知识蒸馏的动态神经网络压缩方法，通过引入新的基于熵的损失函数来优化学生模型训练，在保持分类性能的同时显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在图像分类中表现出色，但计算成本高，难以应用于实时和边缘场景。需要开发能够在运行时动态调节压缩级别的架构。

Method: 将早期退出和知识蒸馏相结合，训练简化的学生早期退出模型从更复杂的教师早期退出模型学习。关键创新是在教师分类错误时使用基于熵的损失函数。

Result: 在CIFAR10、CIFAR100和SVHN数据集上的实验验证了该方法能显著降低计算复杂度而不影响分类性能。

Conclusion: 该方法有效平衡了准确性和效率的权衡，为知识蒸馏在其他场景的应用开辟了新的研究视角。
...
TL;DR: 提出了一种基于深度学习的图像质量评估方法μDeepIQA，用于光学显微镜图像的质量评估，具有快速预测、高泛化性和空间可视化能力。
...
Method: 采用深度卷积神经网络架构，重新训练以预测光学显微镜图像的质量指标和全局质量分数，支持基于图像块的质量预测和空间可视化。

Result: μDeepIQA能够提供快速稳定的图像质量预测，在存在异常值的情况下仍保持稳定性能，并能评估小图像块的质量。

Conclusion: 深度学习模型在光学显微镜图像质量评估中具有显著优势，包括对异常值的稳定性、小图像块评估能力和快速预测能力。
...
TL;DR: 开发了一个端到端的物联网机器人系统，用于葡萄园的实时无损产量和质量（糖度、酸度）空间映射，通过深度学习模型实现葡萄串检测、重量估计和基于高光谱数据的质量评估。


<details>
  <summary>Details</summary>
Motivation: 解决葡萄园精准农业中产量和质量实时监测的挑战，特别是高光谱成像在野外应用中因光照变化导致的"域偏移"问题。

Method: 系统包含两个关键模块：葡萄串检测和重量估计的高性能模型，以及基于Light-Invariant Spectral Autoencoder (LISA)的质量评估框架，该框架使用域对抗学习从未校准数据中学习光照不变特征。
...
TL;DR: 本文提出了一个全面的多模态海底栖息地测绘数据集，包含约100万个侧扫声纳瓦片、水深图和光学图像，其中36000个声纳瓦片已人工标注，支持监督学习和跨模态表示学习。


<details>
  <summary>Details</summary>
Motivation: 海底栖息地测绘对理解海洋生态系统至关重要，但大型标注数据集的稀缺限制了机器学习模型的发展。

Method: 收集加泰罗尼亚海岸的侧扫声纳数据、水深图和AUV光学图像，手动标注36000个声纳瓦片，开发多传感器数据融合方法，提供开源预处理和标注工具。

Result: 创建了包含约100万个声纳瓦片的多模态数据集，建立了光学图像与声纳瓦片的空间关联，支持跨模态表示学习。
...
TL;DR: 在卢旺达基加利比较四种目标检测模型（YOLOv5、Faster R-CNN、SSD、RetinaNet）用于摩托车检测的性能，评估其在资源受限环境下的实时导航适用性。


<details>
  <summary>Details</summary>
Motivation: 基加利的摩托车出租车是主要交通工具，经常不遵守交通规则且行驶不可预测，给自动驾驶系统带来重大挑战，需要有效的目标检测解决方案。

Method: 使用在基加利收集的198张图像自定义数据集，在PyTorch中实现四种目标检测模型并采用迁移学习，评估准确性、定位精度和推理速度。

Result: 识别了数据集限制和模型复杂性等实施挑战，建议未来工作采用简化架构以提高在发展中国家自动驾驶系统的可访问性。

Conclusion: 为资源受限环境（如卢旺达）的自动驾驶系统推荐简化的目标检测架构，以提升在发展中国家城市交通环境中的适用性。
...
TL;DR: 提出了一种新颖的语义感知层次共识方法，通过整合专门处理不同类别粒度的层次特定分类头，在深度网络架构中学习层次特征和关系。


<details>
  <summary>Details</summary>
Motivation: 深度学习在遥感图像分类中越来越重要，但预定义的标签层次结构经常被忽视，大多数方法只关注细粒度分类方案。

Method: 使用可训练的层次矩阵以自监督方式引导网络学习层次结构，并引入层次共识机制确保不同层次级别概率分布的一致性。

Result: 在三个具有不同层次复杂度的基准数据集上评估，使用不同骨干架构，实验结果显示该方法在引导网络学习和层次共识的鲁棒性方面都有效。
...
TL;DR: 提出了首个针对医学图像分类的解剖学引导混合专家框架REN，通过训练七个专门针对不同肺叶的专家网络，结合多模态门控机制，在间质性肺病分类中实现了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统MoE系统缺乏医学影像所需的解剖学约束，而肺部解剖结构和区域疾病异质性对病理模式有重要影响，需要专门针对医学影像的解剖学引导框架。

Method: 利用解剖学先验训练七个专门针对不同肺叶的专家网络，采用多模态门控机制动态整合影像组学生物标志物和深度学习特征来优化专家权重分配。

Result: 在间质性肺病分类中，REN实现了平均AUC 0.8646±0.0467，比SwinUNETR基线提升12.5%；下叶模型AUC达到0.88-0.90，优于深度学习模型。
...
TL;DR: 提出了NFPF框架，通过特定特征学习机器(SFLM)量化样本对模型性能的贡献，使用重构差异指标进行样本选择，显著优于现有无监督主动学习方法。


<details>
  <summary>Details</summary>
Motivation: 传统主动学习需要大量人工标注且迭代过程耗时，而无监督主动学习(UAL)方法性能不佳，主要因为基于局部梯度评分的方法易受噪声影响且无法充分代表数据分布。

Method: 开发了自然特征渐进框架(NFPF)，使用特定特征学习机器(SFLM)量化样本重要性，并定义重构差异指标进行初始样本选择。

Result: NFPF在视觉数据集上显著优于所有现有UAL方法，性能与监督主动学习方法相当，具有更强的鲁棒性和更好的数据分布覆盖。

Conclusion: NFPF通过创新的样本重要性度量方法，为无监督主动学习提供了更有效的解决方案，在减少人工标注负担的同时保持了高性能。
...
TL;DR: 提出CA3D-Diff框架，通过柱感知交叉注意力和隐式3D重建解决乳腺X线摄影双视图转换中的结构不对齐问题，提升缺失视图恢复效果。


<details>
  <summary>Details</summary>
Motivation: 临床实践中乳腺X线摄影的CC和MLO视图可能因采集错误或压缩伪影而缺失或损坏，影响诊断效果。由于乳腺组织的非刚性变形和重叠，视图间转换极具挑战性。

Method: 基于条件扩散模型，设计柱感知交叉注意力机制利用解剖对应区域在列位置上的相似性，并引入隐式3D结构重建模块将2D潜在特征反投影为3D特征体，增强解剖感知。

Result: 在双向视图转换任务中表现优异，在视觉保真度和结构一致性方面超越现有方法，合成的视图有效提升了单视图恶性分类性能。

Conclusion: CA3D-Diff框架成功解决了乳腺X线摄影视图转换中的结构不对齐问题，在真实诊断场景中具有实用价值。
...
TL;DR: SSDD是一种新的像素扩散解码器架构，通过蒸馏技术实现单步重建，在无需对抗性损失的情况下达到比KL-VAE更高的重建质量和更快采样速度。


<details>
  <summary>Details</summary>
Motivation: 当前基于KL-VAE的tokenizer需要对抗性损失，而扩散解码器虽然更理论化但需要迭代采样导致解码时间较长。需要解决这些限制。

Method: 提出新的像素扩散解码器架构，利用transformer组件和无GAN训练提高扩展性和训练稳定性，使用蒸馏技术将扩散解码器性能复制到高效的单步解码器中。
...
TL;DR: 提出了一种视觉基础模型的所有权验证方法，通过微调少量表达层和编码器-解码器网络，在输入图像的内部表示中嵌入数字水印，即使模型被微调后水印仍然可检测。


<details>
  <summary>Details</summary>
Motivation: 保护视觉基础模型的知识产权，防止未经授权的模型再分发，需要可靠的所有权验证工具来区分受保护模型的副本和独立模型。

Method: 微调视觉基础模型的少量表达层，结合小型编码器-解码器网络，在保留输入图像的内部表示中嵌入数字水印。
...
TL;DR: 提出了一种新的潜在不确定性表示方法（LUR和RLUR），用于深度神经网络的不确定性估计和分布外检测，在视频驾驶员行为识别任务中表现优异且训练更高效。
...
Result: LUR和RLUR在分布内分类性能与其他方法相当，在分布外检测方面LUR与最佳方法相当，但训练更高效、调参更简单。

Conclusion: 提出的LUR方法在保持性能的同时显著提高了训练效率和易用性，为安全关键应用提供了实用的不确定性估计解决方案。
...
TL;DR: 使用手绘螺旋和波浪图像作为帕金森病检测的生物标志物，通过CNN、迁移学习和注意力机制实现高精度诊断。


<details>
  <summary>Details</summary>
Motivation: 帕金森病早期诊断至关重要，但传统方法繁琐且昂贵。需要开发非侵入性、成本效益高的诊断方案。

Method: 采用卷积神经网络、迁移学习和注意力机制，结合数据增强和集成投票策略，构建三阶段架构。

Result: 螺旋图像准确率90%，波浪图像准确率96.67%，集成投票后总体准确率达到93.3%。

Conclusion: 机器学习方法在帕金森病早期诊断中具有巨大潜力，可提供非侵入性、成本效益高的解决方案。
...
TL;DR: 该调查首次全面分析了视频大型多模态模型（Video-LMMs）的后训练方法，包括监督微调、强化学习和测试时扩展三大支柱技术，并提出了结构化分类法来解决视频理解中的独特挑战。


<details>
  <summary>Details</summary>
Motivation: 视频理解是计算机视觉中最具挑战性的前沿领域，需要模型处理复杂的时空关系、长期依赖和多模态证据。虽然Video-LMMs在视频理解任务中表现出色，但将其从基础感知系统转变为复杂推理引擎的关键后训练阶段在文献中仍然分散，需要系统性的整理和分析。

Method: 提出了一个结构化分类法，涵盖三大后训练支柱技术：1）带有思维链的监督微调（SFT）；2）基于可验证目标的强化学习（RL）；3）通过增强推理计算的测试时扩展（TTS）。这些方法针对视频特有的挑战如时间定位、时空基础、长视频效率和多模态证据集成进行了专门适配。

Result: 通过系统分析代表性方法，综合了关键设计原则、见解和评估协议，同时识别了奖励设计、可扩展性和成本性能优化等关键开放挑战。还整理了必要的基准、数据集和指标，以促进后训练有效性的严格评估。

Conclusion: 该调查为研究人员和从业者提供了一个统一的框架来推进Video-LMM的能力，旨在填补视频大型多模态模型后训练方法研究的空白，并为该领域的未来发展提供指导。
...
<details>
  <summary>Details</summary>
Motivation: 分割匹配比关键点匹配更能捕捉结构化区域，对遮挡、光照变化和视角变化具有更强鲁棒性。现有方法在极端视角变化下表现不佳，需要利用3D基础模型的归纳偏置来解决宽基线分割匹配问题

Method: 提出一种架构，利用3D基础模型的归纳偏置来匹配图像对中的分割区域，能够处理高达180度的视角变化

Result: 在ScanNet++和Replica数据集上，我们的方法在AUPRC指标上比最先进方法（包括SAM2视频传播器和局部特征匹配方法）性能提升高达30%

Conclusion: 该方法在3D实例分割和图像目标导航等相关下游任务中展现出优势，证明了3D基础模型在分割匹配任务中的有效性
...
TL;DR: 提出一种针对对比度失真图像的无参考图像质量评估方法，通过选择最合适的对比度增强算法生成伪参考图像，将NR-IQA问题转化为FR-IQA问题


<details>
  <summary>Details</summary>
Motivation: 对比度变化是影响图像质量的重要因素，但在现有的图像质量评估方法中，对比度失真往往被忽视，其视觉影响和特性与其他传统失真类型不同
...
Conclusion: 通过将无参考问题转化为全参考问题，提出的方法能够有效评估对比度失真图像的质量，且性能表现良好
...
TL;DR: 提出Neuroplastic Modular Classifier，一种结合ResNet-50和Vision Transformer的混合架构，用于垃圾分类和工业表面缺陷检测，通过可扩展模块和FAISS相似性检索实现自适应学习。


<details>
  <summary>Details</summary>
Motivation: 需要高效准确的垃圾分类和工业表面缺陷检测方法，以支持可持续废物管理和质量控制标准。传统静态模型在动态环境中适应性不足。

Method: 使用ResNet-50进行局部特征提取，Vision Transformer捕获全局语义上下文，集成FAISS相似性检索，采用神经可塑性模块设计，包含可扩展学习块，在训练性能停滞时动态增长。

Result: 在垃圾分类和KolektorSDD2工业缺陷数据集上的实验表明，该架构在准确性和适应性方面优于传统静态模型。
...
TL;DR: 该论文提出了首个针对结构化视觉内容（如图表、图表、数学图形）生成和编辑的综合解决方案，包括大规模数据集构建、统一模型训练和评估基准。
...
Method: 构建了130万高质量结构化图像对数据集，训练了集成VLM和FLUX.1 Kontext的统一模型，采用三阶段训练课程，并在推理时使用外部推理器增强性能。

Result: 评估15个模型显示领先的闭源系统仍不理想，作者模型在编辑任务上表现强劲，推理时推理在不同架构中带来一致增益。
...
TL;DR: 提出了一个跨角色交互视频生成框架，通过跨角色嵌入和跨角色增强技术，解决不同世界角色自然交互时的身份保持和风格一致性问题。


<details>
  <summary>Details</summary>
Motivation: 研究如何在文本到视频生成中实现不同世界角色（如Mr. Bean和Tom and Jerry）的自然交互，同时保持各自的身份特征和行为逻辑，避免风格混淆问题。

Method: 使用跨角色嵌入（CCE）从多模态数据中学习角色身份和行为逻辑，结合跨角色增强（CCA）通过合成共存和混合风格数据来丰富训练。

Result: 在包含10个卡通和真人角色的基准测试中，在身份保持、交互质量和风格一致性方面都取得了明显改进。

Conclusion: 该框架能够实现从未共存角色之间的自然交互，同时保持风格保真度，为生成式故事叙述开辟了新形式。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在合成复杂动态和连贯因果链方面存在困难，而多模态模型具有较强的视觉状态推理能力，需要将两者优势结合。

Method: 利用大型多模态模型生成稀疏关键帧作为快照，然后在这些关键时刻对预训练视频生成器进行稀疏推理时调优。

Result: 在复杂多步骤场景的广泛实验中，VChain显著提升了生成视频的质量。

Conclusion: VChain通过注入视觉推理信号，以调优高效的方式增强了视频生成模型处理复杂动态的能力。
...
TL;DR: 通过分析GPT2-Small第一层中注意力模式分散且对内容依赖较弱的注意力头，发现其softmax分母在固定token分布下稳定，从而可以从权重和单一校准文本中揭示数百个响应高级上下文属性的神经元。


<details>
  <summary>Details</summary>
Motivation: 研究transformer语言模型中注意力模式分散的注意力头，探索其softmax分母的稳定性，以便仅从模型权重和单一校准文本就能发现响应高级上下文属性的神经元。

Method: 使用"校准文本"采样softmax分母，结合多个稳定注意力头的输出，通过线性总结周围文本来近似它们的组合输出。

Result: 从权重和单一校准文本中发现了数百个第一层神经元，这些神经元响应周围文本的高级上下文属性，包括在校准文本上未激活的神经元。

Conclusion: 该方法能够仅从模型权重和单一校准文本中揭示transformer语言模型中响应高级上下文属性的神经元，为模型解释提供了新途径。
...
TL;DR: Graph-S³：基于LLM的文本图推理框架，通过合成逐步监督训练检索器，在复杂图问答任务中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现实世界中大量数据以文本图形式存在，但现有图检索方法要么依赖浅层嵌入相似度，要么需要大量标注数据和训练成本，导致性能不佳
...
Conclusion: Graph-S³框架通过合成监督和逐步评估有效解决了图检索问题，显著提升了基于LLM的文本图问答性能
...
TL;DR: 该研究审计了6个流行大语言模型在完成30个日常任务时表现出的隐含价值观，并与100名美国众包工作者进行比较，发现LLMs在价值观表现上与人类及其他模型存在显著不一致。


<details>
  <summary>Details</summary>
Motivation: 尽管AI助手在帮助用户完成日常任务方面具有潜力，但人们对这些助手在完成主观性日常任务时表现出的隐含价值观知之甚少，需要研究LLMs是否以及如何展现如环保主义、慈善、多样性等价值观。

Method: 通过审计6个流行LLMs完成30个日常任务，并与100名美国众包工作者进行比较，分析模型在价值观表现上的差异。

Result: 研究发现LLMs在隐含价值观表现上经常与人类不一致，不同LLMs之间也存在显著差异。

Conclusion: LLMs在完成日常任务时展现的价值观与人类存在明显差异，这凸显了需要更好地理解和校准AI助手的价值观表现。
...
TL;DR: CSAR是一种从并行话语和意义语料库中诱导语素的贪心算法，通过互信息加权、选择、去除和重复的过程来识别语素。
...
Method: 贪心算法，包含四个步骤：基于形式和意义的互信息对语素进行加权、选择权重最高的语素对、从语料库中移除该语素、重复此过程以诱导更多语素。

Result: 在程序生成的数据集上验证了CSAR的有效性，并在人类语言数据上展示了其合理预测能力，同时分析了新兴语言的语言特征如同义词和多义词程度。

Conclusion: CSAR算法能够有效地从语言数据中诱导语素，并在不同领域展现出良好的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于文本的检索器在处理现实世界文档（如PDF、幻灯片或视频）中视觉和语义丰富的内容时存在困难，需要支持多模态检索。

Method: 基于ColPali和Qwen2.5-Omni等模型，扩展检索能力到音频和视频模态，使用单一模型支持跨模态和联合模态检索。

Result: 在文本、图像和视频检索任务中表现出有效性。

Conclusion: Omni-Embed-Nemotron能够处理现实世界信息需求的复杂性，为多模态检索提供了统一的解决方案。
...
TL;DR: 使用基于信号博弈的涌现通信环境和超参数优化，通过XferBench目标函数生成与人类语言相似度最高的涌现语言，并验证熵对涌现语言迁移学习性能的预测能力。


<details>
  <summary>Details</summary>
Motivation: 设计能够生成与人类语言统计特性相似的涌现语言，并探索影响涌现语言真实性的关键因素。

Method: 采用基于信号博弈的涌现通信环境，结合超参数优化，使用XferBench作为目标函数来量化涌现语言与人类语言的统计相似性。

Result: 成功生成了与人类语言相似度最高的涌现语言，验证了熵对迁移学习性能的预测能力，并发现了产生更真实涌现语言的关键超参数。

Conclusion: 通过精心设计的信号博弈环境和超参数优化，可以生成与人类语言高度相似的涌现语言，熵是预测涌现语言迁移学习性能的有效指标。
...
<details>
  <summary>Details</summary>
Motivation: 传统情感识别仅给整句分配单一标签，但实际应用需要知道情感如何表达，因此开发span级情感证据检测任务。

Method: 构建包含1200个真实句子的SEER基准，标注情感和情感证据，评估14个开源LLMs在单句和跨句任务上的表现。

Result: 部分模型在单句任务上接近人类平均水平，但在长文本中准确率下降；错误分析显示模型过度依赖情感关键词且在中性文本中产生误报。

Conclusion: SEER基准突显了当前LLMs在细粒度情感证据检测方面的局限性，特别是在处理长文本时，需要改进模型对情感表达方式的理解。
...
TL;DR: ALHD是第一个大规模阿拉伯语数据集，专门用于区分人类和LLM生成的文本，涵盖新闻、社交媒体和评论三种文体，包含超过40万个平衡样本，支持阿拉伯语LLM文本检测的可重复性研究。


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语中缺乏专门用于检测LLM生成文本的大规模数据集的问题，为研究阿拉伯语LLM检测、错误信息、学术不端和网络威胁提供基础。

Method: 构建包含三种文体（新闻、社交媒体、评论）的阿拉伯语数据集，涵盖现代标准阿拉伯语和方言，包含人类和三种主流LLM生成的40多万个平衡样本，提供严格预处理、丰富标注和标准化分割。

Result: 基准测试显示微调BERT模型表现最佳，优于基于LLM的模型。但在跨文体泛化方面存在挑战，特别是在新闻文章中，LLM生成的文本在风格上更接近人类文本。
...
TL;DR: TS-Reasoner是一个结合时间序列基础模型和大型语言模型的框架，通过对齐两种模态的潜在表示来提升时间序列推理能力，在多个基准测试中优于现有方法且具有显著的数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型能捕捉动态模式但缺乏背景知识和复杂推理能力，而大型语言模型具备推理能力但难以理解数值时间序列数据。需要有效整合两种模型来解决时间序列推理任务。

Method: 提出TS-Reasoner框架，通过生成多样化的合成时间序列-文本对进行对齐预训练，然后进行指令微调。冻结预训练的时间序列基础模型，仅对齐其潜在表示与语言模型的文本输入。

Result: 在多个基准测试中，TS-Reasoner不仅优于各种主流LLM、视觉语言模型和时间序列LLM，而且仅使用不到一半的训练数据就实现了这一性能。

Conclusion: TS-Reasoner通过有效对齐时间序列和文本模态，成功结合了时间序列基础模型的数值理解能力和大型语言模型的推理能力，为时间序列推理任务提供了高效解决方案。
...
TL;DR: 在专业领域中，提出了一种基于RAG的对比推理方法，通过比较相似案例来解决RAG在专业推理任务中输出过于通用的问题。


<details>
  <summary>Details</summary>
Motivation: 传统RAG在专业领域推理中只能提取事实信息，但无法检索相似案例或相关问题，导致输出过于通用，缺乏针对性的洞察。

Method: 在RAG基础上增加了一个对比推理层，通过比较相似案例来生成更具体、更有针对性的分析。

Result: 该方法在文本生成指标（如ROUGE和BERTScore）上优于基线RAG，与人工生成的股权研究和风险分析相比表现更好。

Conclusion: 对比推理方法能够有效提升RAG在专业领域中的推理能力，生成更具上下文洞察力的输出。
...
TL;DR: 提出Consensus Graphs (ConGrs)数据结构，通过采样多个LM响应并分析其共享信息和语义变化，合成更准确、可靠的最终响应。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法有效合成不同长格式响应中的丰富认知信号，需要一种能捕捉LM响应变化并利用这些认知信号的方法。

Method: 使用轻量级词序列对齐算法构建ConGrs DAG数据结构，辅以二级LM判断器，并设计任务相关的解码方法从ConGr合成最终响应。

Result: 在传记生成任务中事实精度提升31%，减少对LM判断器的依赖80%以上；在拒绝任务中弃权率提升56%；在数学推理任务中准确率提升6个百分点。

Conclusion: ConGrs提供了一种灵活的方法来捕捉LM响应变化，并利用响应变化提供的认知信号合成更有效的响应。
...
TL;DR: 研究表明，在指令微调数据中引入扰动（如删除停用词或打乱词序）可以增强大语言模型对噪声指令的抵抗能力，在某些情况下甚至能提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明大语言模型对指令措辞的微小变化很敏感，本研究探索通过指令微调数据中的扰动来增强模型对噪声指令的抵抗能力。

Method: 在指令微调数据中引入扰动（删除停用词、打乱词序等），然后在MMLU、BBH、GSM8K等基准测试上评估模型在原始和扰动版本上的表现。
...
TL;DR: TriMediQ是一个基于三元组的知识图谱方法，通过将患者对话转化为结构化三元组来增强LLM在临床多轮对话中的推理能力，在iMedQA数据集上比基线方法准确率提升10.4%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在静态医疗问答中表现良好，但在实际临床对话中可靠性显著下降，因为临床事实在对话日志中缺乏清晰关联，难以进行多跳推理。

Method: 提出TriMediQ框架：1) 使用冻结的三元组生成器将患者响应提取为临床相关三元组并构建知识图谱；2) 训练投影模块（图编码器和投影器）从KG中捕获关系信息；3) 分两步操作：冻结LLM权重微调投影模块，然后在推理时使用微调模块指导多跳推理。
...
TL;DR: 论文指出在计算社会科学中使用大语言模型进行文本分类时，概念化步骤常被忽视，这会导致概念化诱导偏差，且无法通过提高LLM准确性或事后偏差校正来纠正。


<details>
  <summary>Details</summary>
Motivation: 在LLM时代，计算社会科学研究者往往跳过概念化步骤直接使用LLM进行分类，导致概念化错误并影响下游统计推断的准确性。

Method: 通过模拟实验分析概念化诱导偏差的影响，并检验提高LLM准确性和事后偏差校正方法是否能有效纠正这种偏差。

Result: 研究发现概念化诱导偏差无法通过单纯提高LLM准确性或事后偏差校正方法来纠正，这种偏差会显著影响下游估计的准确性。
...
TL;DR: CCD-Bench是一个评估大语言模型在跨文化价值冲突中决策能力的基准测试，包含2,182个开放式困境，覆盖7个领域，对应10个GLOBE文化集群。研究发现LLMs偏好北欧和日耳曼欧洲文化，而中东和东欧选项被低估，显示当前对齐策略存在文化偏见。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注文化知识、价值预测或单轴偏见诊断，但缺乏评估LLMs在多个文化价值观直接冲突时的裁决能力。本文旨在填补这一空白。

Method: 构建CCD-Bench基准，包含2,182个开放式困境，对应10个GLOBE文化集群，使用分层拉丁方设计避免顺序效应，评估17个非推理LLMs。

Result: 模型明显偏好北欧欧洲(20.2%)和日耳曼欧洲(12.4%)，而中东和东欧选项代表性不足(5.6-5.8%)。87.9%的理据引用多个GLOBE维度，但这种多元主义是表面的，主要重组未来导向和绩效导向，很少基于自信或性别平等主义。

Conclusion: 当前对齐流程促进了共识导向的世界观，无法充分处理需要权力谈判、权利推理或性别意识分析的场景。CCD-Bench将评估从孤立偏见检测转向多元决策，强调需要实质性参与多元世界观的校准策略。
...
TL;DR: RxT是一种新型Transformer架构，通过事件驱动范式解决传统Transformer在对话AI中的状态缺失和二次计算复杂度问题，实现线性扩展的低延迟实时对话。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在对话应用中存在状态缺失和二次计算复杂度问题，导致长对话成本高昂、延迟严重，需要新的架构来支持实时、经济可行的长对话。

Method: RxT采用事件驱动范式，将每个对话轮次作为离散事件处理，使用固定大小的短期记忆系统，通过生成器-解码器产生响应，异步更新记忆状态。

Result: 在合成数据上的概念验证实验显示，RxT相比同等规模的无状态基线模型具有更优性能和恒定时间推理延迟。

Conclusion: RxT架构通过解耦响应生成和记忆更新，将对话总成本从二次降低到线性，实现了低延迟、有状态且经济可行的长对话。
...
TL;DR: 本文介绍了在EvalLLM 2025挑战赛中针对法语生物医学命名实体识别和健康事件抽取的方法，主要使用大语言模型结合提示工程和合成数据，在少样本设置下取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 解决法语生物医学文本中命名实体识别和事件抽取任务在少样本场景下的挑战，探索大语言模型在低资源环境下的应用潜力。

Method: 提出三种NER方法：(1) GPT-4.1的上下文学习，自动选择10个示例并整合标注指南摘要；(2) GLiNER系统在合成语料上微调，后经LLM验证；(3) LLaMA-3.1-8B-Instruct在相同合成语料上微调。事件抽取采用与NER相同的上下文学习策略。

Result: GPT-4.1表现最佳，NER的宏F1为61.53%，事件抽取为15.02%，证明了精心设计的提示在极低资源场景中的重要性。

Conclusion: 在法语生物医学文本处理中，大语言模型结合精心设计的提示工程能够有效提升少样本场景下的性能，为低资源语言和领域提供可行解决方案。
...
TL;DR: Deco-G是一个解码框架，通过将格式遵循与任务解决解耦来提高大语言模型的性能。它使用单独的概率模型处理格式要求，让LLM专注于任务解决，在多个任务上实现了1.0%到6.0%的相对性能提升。


<details>
  <summary>Details</summary>
Motivation: 随着提示变得越来越复杂，模型难以同时遵循所有指令，特别是当推理指令与严格的格式要求交织在一起时，这种纠缠会为模型创造竞争目标。明确分离这两个方面可能带来性能改进。

Method: Deco-G使用单独的可处理概率模型(TPM)处理格式遵循，而LLM只接收任务指令。在每个解码步骤中，将LLM的下一个令牌概率与TPM计算的格式遵循似然相结合。引入了指令感知蒸馏、灵活的trie构建算法和HMM状态剪枝等创新。

Result: 在数学推理、LLM-as-a-judge和事件参数提取等多个具有不同格式要求的任务上，该方法相比常规提示实践实现了1.0%到6.0%的相对性能提升，并保证了格式合规性。

Conclusion: 明确解耦格式遵循和任务解决可以显著提高LLM在复杂指令下的性能，Deco-G框架为处理交织的推理指令和格式要求提供了一种有效的解决方案。
...
TL;DR: 现有评估基准对LLM有效上下文长度和遗忘倾向的评估过于简单，本文提出基于复杂关系推理任务的评估方法，发现LLM在关系推理中比现有基准显示更早出现记忆漂移和上下文遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准主要依赖简单的'大海捞针'检索或续写任务，不能准确反映LLM在信息密集场景中的表现，需要更复杂的推理任务来评估模型从文本中提取结构化关系知识的能力。

Method: 使用需要从潜在嘈杂的自然语言内容中归纳图结构的复杂推理任务进行评估，输入文本可视为图的生成过程，但结构未明确表示，需从分布式文本线索中归纳连接关系。

Result: 发现LLM在关系推理任务中比现有基准建议的更早出现记忆漂移和上下文遗忘，即使是专门用于推理的模型（如OpenAI o1）在这些设置中仍然容易早期记忆漂移。

Conclusion: 模型从非结构化输入中抽象结构化知识的能力存在显著限制，需要架构改进来提升长程推理能力，并为复杂推理任务中流行LLM的最优使用提供建议。
...
TL;DR: 提出了一种基于音节级别的无监督语音识别框架，通过掩码语言建模避免了G2P转换器和GAN方法的不稳定性，在LibriSpeech上实现了40%的相对字符错误率降低，并能有效推广到中文。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于音素的UASR方法依赖昂贵G2P转换器的问题，以及由于训练不稳定性难以推广到音素边界模糊的语言（如中文）的挑战。

Method: 基于掩码语言建模的音节级别无监督语音识别框架，无需G2P转换器，避免了GAN方法的不稳定性。

Result: 在LibriSpeech上实现高达40%的相对字符错误率降低，并能有效推广到中文这种对先前方法特别困难的语言。

Conclusion: 提出的音节级别UASR框架解决了现有方法的资源依赖和泛化问题，为低资源语言和多媒体学习提供了有效解决方案。
...
TL;DR: UniDoc-Bench是第一个大规模、真实的多模态检索增强生成基准，基于70k真实PDF页面构建，包含1,600个多模态QA对，支持四种范式的公平比较。
...
Method: 从8个领域的70k真实PDF页面中提取和链接文本、表格和图像证据，生成1,600个多模态QA对，涵盖事实检索、比较、总结和逻辑推理查询，其中20%经过多标注者和专家验证。

Result: 多模态文本-图像融合RAG系统始终优于单模态和联合多模态嵌入检索，表明仅文本或图像都不足够，当前多模态嵌入仍不充分。

Conclusion: 该基准不仅用于评估，还揭示了视觉上下文何时以及如何补充文本证据，发现了系统性失败模式，并为开发更稳健的MM-RAG管道提供了可操作指导。
...
TL;DR: 提出基于QLoRA的微调框架，用于改进罗马乌尔都语-英语混合文本中的冒犯性语言检测，通过翻译处理低资源输入，Meta LLaMA 3 8B模型取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 罗马乌尔都语等代码混合语言中的贬义词汇使用给NLP系统带来挑战，包括语法不明确、拼写不一致和标注数据稀缺。

Method: 使用Google Translate将罗马乌尔都语-英语混合数据集翻译为英语，利用QLoRA对多个transformer和LLM模型进行内存高效的微调，包括Meta LLaMA 3 8B、Mistral 7B等。

Result: Meta LLaMA 3 8B获得最高F1分数91.45，Mistral 7B为89.66，均超过传统transformer基线模型。

Conclusion: QLoRA在低资源环境下的微调效果显著，证实了LLM在此类任务中的潜力，为基于LLM的多语言冒犯性检测系统铺平了道路。
...
TL;DR: MedReflect是一个通用框架，通过模拟医生反思思维模式来激发LLMs的医学问题解决能力，无需外部检索或大量标注，仅需少量训练样本即可显著提升医学基准测试准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部知识检索或大量标注数据，存在检索开销大、标注成本高的问题，且性能有限。本文旨在释放LLMs在医学问题解决中的潜在能力，减少对外部监督的依赖。

Method: 提出MedReflect框架，生成单次反思链：初始假设生成→自我提问→自我回答→决策精炼。这种自我验证和反思机制无需外部检索或大量标注。

Result: 仅用2000个随机训练样本和轻量微调，就在一系列医学基准测试中实现了显著的绝对准确率提升，同时大幅减少了标注需求。
...
TL;DR: 提出TreePrompt方法，通过树形框架学习LLM偏好来选择高质量、上下文相关的翻译示例，结合K-NN和AFSP方法在英-波斯和英-德语对上提升翻译性能。


<details>
  <summary>Details</summary>
Motivation: 现有少样本提示方法主要关注查询与示例的相似性，但忽略了示例质量的重要性，需要更全面的示例选择策略。

Method: TreePrompt方法在树形框架中学习LLM偏好来选择高质量示例，并与K-NN和自适应少样本提示(AFSP)结合来平衡相似性和质量。

Result: 在英-波斯(MIZAN)和英-德(WMT19)语言对上的评估显示，TreePrompt与AFSP或随机选择结合能提高翻译性能。

Conclusion: TreePrompt通过考虑示例质量和上下文相关性，有效提升了少样本提示在机器翻译中的效果。
...
TL;DR: 提出了一种基于语音粒度分析的多语言帕金森病检测方法，通过分析音素、音节和单词级别的语音特征，发现音素级分析在跨语言PD检测中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前PD检测系统分析整个语音片段，可能忽略了特定语音元素的诊断价值。帕金森病影响全球超过1000万人，其中89%患者存在言语障碍。

Method: 开发了粒度感知方法，使用自动化流程从录音中提取时间对齐的音素、音节和单词。在意大利语、西班牙语和英语数据集上使用双向LSTM和多头注意力机制比较不同粒度级别的诊断性能。

Result: 音素级分析表现最优，AUROC达93.78%±2.34%，准确率达92.17%±2.43%。注意力分析显示最有信息的语音特征与临床协议一致：音素级的持续元音、音节级的交替运动音节、单词级的/pataka/序列。

Conclusion: 该方法在跨语言PD检测中展现出增强的诊断能力，且识别出的关键特征与临床实践相符，为PD诊断提供了更精细的分析方法。
...
TL;DR: 研究探讨了少样本提示策略对词义消歧任务的影响，特别关注样本分布不平衡引入的偏见，发现在多语言环境中不平衡样本会导致错误预测，但在英语中不会出现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展重塑了自然语言处理领域，少样本提示因其实用性和有效性受到关注。本研究旨在了解少样本提示策略如何影响词义消歧任务，特别是由不平衡样本分布引入的偏见。

Method: 使用GLOSSGPT提示方法测试英语、德语、西班牙语、法语和意大利语五种语言的词义消歧效果，评估GPT-4o和LLaMA-3.1-70B模型的行为。

Result: 结果显示不平衡的少样本示例会导致多语言中的错误词义预测，但英语中未出现此问题。模型评估突显了多语言词义消歧对少样本设置中样本分布的敏感性。
...
TL;DR: 开发了Rezwan大型AI辅助圣训语料库，包含120多万条圣训，通过全自动流水线提取和结构化处理，实现了多语言翻译、智能标注、摘要生成等功能，在圣训-文本分离和摘要任务上达到接近人类水平的准确度。


<details>
  <summary>Details</summary>
Motivation: 传统圣训研究依赖人工整理，效率低且难以大规模处理。需要利用AI技术自动化处理大量圣训文本，实现多语言、多层次的语义增强，为数字人文和伊斯兰研究提供基础设施。

Method: 基于数字资源库，采用大语言模型进行文本分割、圣训-文本分离、验证和多层增强处理，包括机器翻译、智能标注、摘要生成、主题标记和跨文本语义分析。

Result: 在1,213条随机样本的专家评估中，结构化任务如圣训-文本分离(9.33/10)和摘要(9.33/10)接近人类水平，总体评分8.46/10显著优于人工整理的Noor语料库(3.66/10)。成本分析显示AI方法经济可行。

Conclusion: 这项工作引入了宗教文本处理的新范式，展示了AI如何增强人类专业知识，实现大规模、多语言和语义丰富的伊斯兰遗产访问。
...
TL;DR: LLMs能够生成和识别深层认知框架，特别是在社会政治语境中。研究发现LLMs能够流畅生成特定框架的文本，并在零样本设置下识别这些框架。通过机制可解释性研究，确定了'严格父亲'和'培育父母'框架在模型隐藏表示中的具体维度位置。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型捕捉和表达有意义的认知框架的能力，特别是在社会政治语境中理解模型如何内化人类概念。

Method: 采用机制可解释性研究方法，分析模型隐藏表示中的特定维度，识别与'严格父亲'和'培育父母'框架强相关的奇异维度。

Result: LLMs在生成特定框架文本方面表现出高度流畅性，在零样本设置下能够准确识别这些框架。研究成功定位了特定认知框架在模型表示中的具体维度位置。

Conclusion: 该研究增进了对LLMs如何捕获和表达有意义人类概念的理解，为认知框架在语言模型中的表示机制提供了新的见解。
...
TL;DR: 提出Step Pruner (SP)强化学习框架，解决大型推理模型过度思考问题，通过惩罚冗余推理步骤而非单纯减少token数量，在保持准确率的同时显著降低响应长度。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法通过惩罚生成token来促进简洁性，但存在两个问题：更少token不一定对应更少推理步骤；模型可能在训练后期出现丢弃推理步骤的作弊行为。

Method: 提出Step Pruner框架：1）步骤感知奖励函数，优先考虑正确性同时惩罚冗余步骤；2）动态停止机制，当任何输出步骤超过长度上限时停止更新，防止步骤合并作弊。

Result: 在四个推理基准测试中，SP实现了最先进的准确率，同时显著减少响应长度。在AIME24上，token使用量减少69.7%。

Conclusion: SP框架有效解决了大型推理模型的过度思考问题，通过步骤级别的优化而非token级别的惩罚，实现了推理效率和准确性的平衡。
...
TL;DR: 本研究使用INCEpTION工具标注语篇修辞关系，比较手动标注与基于大语言模型的自动方法，在板球新闻中评估BERT、DistilBERT和逻辑回归模型对修辞关系的分类性能。


<details>
  <summary>Details</summary>
Motivation: 探索语篇修辞关系标注方法，比较手动与自动标注的效果，推动语篇解析与基于transformer的NLP技术的交叉研究。

Method: 使用INCEpTION工具进行手动标注，并采用BERT、DistilBERT和逻辑回归模型自动分类修辞关系（如阐述、对比、背景、因果等）。

Result: DistilBERT模型取得了最高准确率，显示出其在语篇关系预测中的高效潜力。

Conclusion: 这项工作为语篇解析与transformer-based NLP的交叉研究做出了贡献，证明了DistilBERT在修辞关系分类中的有效性。
...
TL;DR: 本文提出了首个孟加拉语政治立场检测基准数据集，包含200篇新闻文章，标注了亲政府、批评政府和中性立场，并对28个大语言模型进行了评估。


<details>
  <summary>Details</summary>
Motivation: 南亚地区的媒体偏见检测至关重要，但孟加拉语政治立场研究缺乏标注数据集和计算研究，需要理解语言线索、文化背景、微妙偏见等多重因素。

Method: 构建包含200篇孟加拉语新闻文章的基准数据集，标注三种政治立场，并对28个专有和开源大语言模型进行综合评估。

Result: 模型在检测批评政府内容上表现良好（F1最高达0.83），但在中性文章上表现很差（F1低至0.00），且倾向于过度预测亲政府立场。

Conclusion: 该数据集及其诊断分析为推进孟加拉语媒体立场检测研究奠定了基础，并为改善低资源语言中LLM性能提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决在代表性不足语言中开发文化基础、治疗连贯的对话系统挑战，同时确保隐私保护和设备端可行性。

Method: 三阶段开发过程：评估SLMs心理知识、设计推理导向框架、构建评估数据集；比较简单提示、多代理辩论和结构化治疗推理路径。

Result: 在单轮偏好研究中获得最高排名，在多轮测试中长时记忆模块对保持连贯性至关重要，完整框架在共情、连贯性、文化适应和个性化方面获得最高评分。
...
TL;DR: 使用LLM从410万患者评价中提取医生人格特质和主观判断，验证了方法的有效性，发现男性医生评分更高、不同专科特质差异明显，并识别出四种医生类型。


<details>
  <summary>Details</summary>
Motivation: 理解患者对医生的认知对于改善信任、沟通和满意度至关重要，需要大规模分析患者评价来获取可解释的度量指标。

Method: 基于大语言模型的流水线，分析410万患者评价，通过多模型比较和人类专家基准验证，进行聚类分析识别医生类型。

Result: 人类与LLM评估高度一致（相关系数0.72-0.89），与患者满意度显著相关（r=0.41-0.81），发现男性医生评分更高，不同专科特质分布不同，识别出四种医生类型。

Conclusion: 从患者叙述中自动提取特质可以提供可解释、验证的度量指标，对医疗质量测量、偏见检测和人力资源发展具有重要意义。
...
TL;DR: 提出了首个模拟框架来评估LLM在长期交互中的欺骗行为，发现欺骗行为与模型相关，随压力增加而增加，并会削弱监督者信任。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多局限于单轮提示下的LLM欺骗评估，无法捕捉长期交互中欺骗策略的展开过程，需要开发能够评估LLM在动态压力下欺骗行为的框架。
...
Conclusion: 欺骗是长期交互中出现的风险，为评估未来LLM在现实世界信任敏感环境中的表现提供了基础。
...
TL;DR: 提出了一种用于COVID-19命名实体识别的新型实体知识增强方法，解决了社交媒体文本不正式和领域知识不足的问题，在完全监督和少样本设置下都提升了性能。


<details>
  <summary>Details</summary>
Motivation: COVID-19大流行期间社交媒体讨论频繁，但相关命名实体识别面临文本不正式、标注数据稀缺和需要领域专业知识等挑战。

Method: 提出实体知识增强方法，通过增强模型对COVID-19领域知识的理解，适用于非正式和正式文本格式的生物医学命名实体识别。

Result: 在COVID-19推文数据集和PubMed数据集上的实验表明，该方法在完全监督和少样本设置下都能提高NER性能。

Conclusion: 实体知识增强方法能有效提升COVID-19命名实体识别性能，特别是在数据稀缺和文本不正式的情况下，具有通用性可应用于一般生物医学NER任务。
...
TL;DR: 提出了AgriGPT-VL套件，包括最大的农业视觉语言语料库Agri-3M-VL、农业专用视觉语言模型AgriGPT-VL和评估套件AgriBench-VL-4K，在农业任务上优于通用VLMs。
...
Method: 使用可扩展的多智能体数据生成器构建Agri-3M-VL语料库；通过渐进式课程训练AgriGPT-VL模型，包括文本基础、多模态浅层/深层对齐和GRPO精炼；建立多指标评估和LLM-as-a-judge框架。

Result: AgriGPT-VL在AgriBench-VL-4K上优于领先的通用VLMs，在LLM-as-a-judge评估中获得更高的成对胜率；在文本任务上保持竞争力，语言能力无明显下降。

Conclusion: AgriGPT-VL套件为农业应用提供了有效的多模态解决方案，消融研究证实了对齐和GRPO精炼阶段的一致增益，所有资源将开源以支持可重复研究和低资源农业环境部署。
...
TL;DR: 使用模型内部激活信号来预测LLM输出的正确性和上下文有效性，通过简单分类器实现约75%的准确率，为早期审计和防止错误上下文引入提供新方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然有用，但可信度仍是主要问题：模型经常以高置信度生成错误信息。上下文信息可以指导生成，但识别何时查询需要检索上下文以及评估上下文有效性仍然具有挑战性。

Method: 操作化可解释性方法，从模型激活中预测输出正确性；探索模型内部是否包含外部上下文有效性的信号；考虑正确、错误和不相关的上下文，并引入指标进行区分；在六个不同模型上进行实验，使用第一个输出token的中间层激活训练简单分类器。

Result: 基于中间层激活的简单分类器可以以约75%的准确率预测输出正确性，实现早期审计；基于模型内部的指标在区分正确和错误上下文方面显著优于提示基线，防止受污染上下文引入的不准确性。

Conclusion: 这些发现提供了一个视角来更好地理解LLMs的底层决策过程，为模型可信度评估和上下文有效性检测提供了实用方法。
...
TL;DR: 本文首次系统研究了泰语文本端到端转轮检测，比较了紧凑型LLM的零样本/少样本提示与轻量级transformer的监督微调方法，建立了泰语基线并证明小型微调模型可实现近乎即时的EOT决策。


<details>
  <summary>Details</summary>
Motivation: 传统音频静音端点检测存在数百毫秒延迟，且在犹豫或语言特定现象下表现不佳，需要更可靠低延迟的语音交互端点检测方法。

Method: 使用YODAS语料库转录字幕和泰语特定语言线索，将EOT建模为token边界上的二元决策，比较了紧凑LLM的提示方法和轻量transformer的监督微调。

Result: 报告了清晰的准确率-延迟权衡，小型微调模型能够提供近乎即时的EOT决策，适合设备端代理使用。

Conclusion: 建立了泰语EOT检测的基线，证明了小型微调模型在实时代理中的实用性，并提供了公开可用的实现方案。
...
TL;DR: 该论文研究了在LLM文本分类任务中，通过引入反事实推理来识别对分类决策最重要的关键词，并提出了决策改变率框架来量化这些关键词的重要性。


<details>
  <summary>Details</summary>
Motivation: 由于LLMs通常是黑盒模型且调用成本高昂，需要解释其分类决策的依据，特别是识别影响分类结果的关键词。

Method: 提出了决策改变率框架，通过引入反事实推理来量化关键词对LLM分类决策的重要性。

Result: 实验结果表明，使用反事实推理有助于识别对分类决策最重要的关键词。

Conclusion: 反事实推理可以有效提升LLM分类决策的可解释性，帮助识别关键影响因素。
...
TL;DR: 该论文提出一个综合基准来评估适合急诊科决策支持的小型语言模型，发现通用领域SLM在医疗基准测试中表现优于医学微调的SLM，表明急诊科可能不需要专门的医学微调。


<details>
  <summary>Details</summary>
Motivation: 急诊科环境快节奏、高风险，小型语言模型因其推理能力和高效性能具有显著潜力，能够支持医生提供及时准确的信息合成，改善临床决策和工作流程效率。

Method: 构建综合基准评估SLM，使用MedMCQA、MedQA-4Options和PubMedQA等数据集，模拟急诊科医生的日常任务，比较通用领域和医学微调SLM的表现。

Result: 实验结果显示，通用领域SLM在多样化急诊科基准测试中意外地优于医学微调的SLM。

Conclusion: 对于急诊科应用，专门的医学模型微调可能不是必需的，通用领域SLM已能提供足够的决策支持能力。
...
TL;DR: 研究探索如何通过思维链技术构建可引导的多元化语言模型，发现RLVR方法在性能表现和训练样本效率方面均优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型通常反映相对统一的价值观念，限制了其在需要理解细微人类观点任务中的应用，因此需要开发能够支持可引导多元化的模型。

Method: 探索了多种方法：思维链提示、基于人类撰写思维链的微调、基于合成解释的微调，以及带可验证奖励的强化学习（RLVR）。

Result: 在Value Kaleidoscope和OpinionQA数据集上的评估显示，RLVR方法在性能上持续优于其他方法，并展现出强大的训练样本效率。

Conclusion: RLVR是构建可引导多元化语言模型的有效方法，同时生成的思维链轨迹在忠实性和安全性方面表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在有限数据约束下实现卓越数据效率，但其内在机制尚不明确，本研究旨在揭示这种效率的来源。

Method: 通过广泛的消融实验，分析随机掩码输入标记、MLP dropout和权重衰减等因素对数据效率的影响。

Result: 随机掩码输入标记起主导作用，MLP dropout和权重衰减也能获得类似增益，表明随机正则化是多轮训练中数据效率提升的广泛机制。

Conclusion: 随机正则化是扩散语言模型在有限数据条件下实现高数据效率的关键机制，这一发现为理解模型效率提供了新视角。
...
TL;DR: PoLi-RL是一个新颖的强化学习框架，通过两阶段课程学习和并行切片排序奖励机制，成功将强化学习应用于条件语义文本相似性任务，在C-STS基准测试中创造了新的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 现有的C-STS方法主要局限于判别模型，未能充分利用大语言模型和强化学习的最新突破。强化学习特别适合该任务，因为它可以直接优化不可微的Spearman排序指标并指导推理过程。

Method: 提出PoLi-RL框架，采用两阶段课程学习：首先用简单的点对点奖励训练基本评分能力，然后转向结合点对点、成对和列表目标的混合奖励。关键创新是并行切片排序奖励机制，在并行切片中计算排序奖励。

Result: 在官方C-STS基准测试中，PoLi-RL实现了48.18的Spearman相关系数，为交叉编码器架构建立了新的SOTA。

Conclusion: 这是首个成功将强化学习应用于C-STS的工作，为在复杂的基于排序的条件判断任务上训练大语言模型引入了一个强大而精确的范式。
...
TL;DR: Caco是一个通过代码驱动增强自动合成高质量、可验证、多样化推理数据的框架，使用代码执行验证确保逻辑正确性，在数学推理基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有CoT方法存在生成不可控、质量不足和推理路径多样性有限的问题，而基于代码的CoT方法通常局限于预定义的数学问题，限制了可扩展性和泛化性。

Method: 首先在统一代码格式上微调基于代码的CoT生成器，然后扩展到大量多样化推理轨迹，通过代码执行和基于规则的过滤进行自动验证，最后将过滤输出反向工程为自然语言指令和语言CoT。

Result: 在创建的Caco-1.3M数据集上实验表明，Caco训练的模型在数学推理基准上实现了强大的竞争性能，优于现有基线。

Conclusion: Caco建立了一个无需人工干预构建自我维持、可信赖推理系统的范式，其代码锚定验证和指令多样性有助于在未见任务上的优越泛化。
...
TL;DR: 该研究从概念映射、隐喻-字面知识库和句法敏感性三个角度分析大语言模型的隐喻处理能力，发现LLMs存在概念无关解释、依赖训练数据而非上下文线索、对句法异常更敏感等问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在知识整合、上下文推理和创造性生成方面表现出先进能力，但其隐喻理解机制尚未得到充分探索，需要系统评估LLMs的隐喻处理能力。

Method: 从三个维度分析：(1)概念映射：使用嵌入空间投影评估LLMs如何在目标域中映射概念；(2)隐喻-字面知识库：分析隐喻词及其字面对应词；(3)句法敏感性：评估隐喻句法结构对LLMs性能的影响。

Result: LLMs产生15%-25%的概念无关解释，依赖训练数据中的隐喻指示器而非上下文线索，对句法不规则性比对结构理解更敏感。

Conclusion: LLMs在隐喻分析方面存在明显局限性，需要开发更强大的计算方法来改进隐喻理解能力。
...
TL;DR: 本文介绍了一个包含斯里兰卡议会记录、法律判决、政府出版物、新闻和旅游统计的开放机器可读文档数据集集合，支持多语言自然语言处理研究。


<details>
  <summary>Details</summary>
Motivation: 为计算语言学、法律分析、社会政治研究和多语言自然语言处理研究提供高质量的斯里兰卡多语言数据集资源。

Method: 构建数据收集管道，从多个来源收集文档数据，以Sinhala、Tamil和英语三种语言整理成13个数据集，每日更新并在GitHub和Hugging Face上镜像。

Result: 截至v20251005版本，该集合包含215,670个文档（60.3 GB），涵盖13个数据集，支持多语言研究。

Conclusion: 该数据集集合为斯里兰卡多语言文档分析提供了有价值的资源，同时讨论了许可和伦理考虑。
...
TL;DR: 开发了一种通用方法来准备文化相关数据集，并通过后训练提升Gemma 2模型在代表性不足语言上的性能，以促进生成式AI在各国文化中的应用和文化遗产保护。
...
TL;DR: 提出SSD（自推测解码），一种无损推理加速方法，利用扩散大语言模型自身作为推测解码的草稿器和验证器，无需辅助模块，实现单次前向传播中验证多个token，达到3.46倍加速。


<details>
  <summary>Details</summary>
Motivation: 当前并行解码方法的生成结果与逐步解码存在偏差，导致性能下降，限制了实际部署。需要解决这一性能退化问题。

Method: SSD引入自草稿机制，模型生成多个位置的预测，然后通过分层验证树在单次前向传播中验证这些预测。利用dLLM固有的多位置并行预测能力，无需单独的草稿模型。

Result: 在LLaDA和Dream等开源模型上，SSD实现了高达3.46倍的加速，同时保持输出与逐步解码完全相同。

Conclusion: SSD通过自推测解码方法有效解决了并行解码的性能退化问题，实现了无损推理加速，为扩散大语言模型的实际部署提供了可行方案。
...
TL;DR: LTPO是一种无需参数更新的测试时优化框架，通过将中间潜在思维向量作为动态参数进行优化，提升LLM在挑战性推理任务上的鲁棒性。
...
Method: 使用在线策略梯度方法，基于冻结LLM自身输出分布计算的内在置信度奖励信号，优化中间潜在思维向量。

Result: 在五个推理基准测试中，LTPO不仅匹配或超越强基线，在标准任务上表现优异，而且在AIME等高难度基准上实现显著改进，而现有潜在推理方法准确率接近零。

Conclusion: LTPO展示了在复杂推理任务上的独特能力，为无需模型参数更新的测试时推理优化提供了有效解决方案。
...
TL;DR: CALM框架通过轻量级修正提示来利用大型推理模型的固有推理能力，开发出的STORM模型在优化建模任务中达到最先进性能
...
Method: 提出CALM框架：专家识别推理缺陷并提供修正提示，模型据此改进推理轨迹，生成高质量数据进行监督微调，再通过强化学习进一步改进

Result: STORM模型（4B参数）在五个优化建模基准测试中达到68.9%的平均准确率，匹配671B模型性能，修正仅影响2.6%的生成token

Conclusion: 基于提示的动态数据合成能保持并增强现代大型推理模型的固有推理模式，为挑战性优化建模任务提供更有效可扩展的路径
...
TL;DR: 提出了REPO强化学习框架，通过结合偏好奖励模型、说服行为奖励和程序化奖励来优化LLM在价格谈判中的表现，显著提升了对话质量和业务约束合规性。


<details>
  <summary>Details</summary>
Motivation: 传统后训练方法在在线旅游机构的价格谈判场景中容易过拟合脚本，无法捕捉细微的说服风格，也难以强制执行可验证的业务约束。

Method: 使用强化学习框架REPO，整合三种奖励：偏好训练的奖励模型用于密集人类对齐，奖励法官用于高级说服行为和SOP合规性，程序化奖励函数用于数值、格式和护栏的确定性检查。

Result: 在150轮真实对话和225轮精选坏案例对话评估中，REPO将平均对话评分提升至4.63，比基础模型高1.20，比DPO高0.83；将至少有一个优秀回复的对话比例提升至66.67%，坏案例修复率达到93.33%。

Conclusion: REPO框架有效提升了LLM在价格谈判中的表现，超越了SFT、DPO、PPO和GRPO等方法，并涌现出主动同理心、局部推理和校准策略等新能力。
...
TL;DR: 该论文提出了一种衡量LLM认知多样性的新方法，通过实证研究发现虽然新模型生成更多样化的主张，但几乎所有模型的认知多样性都低于基础网络搜索，模型大小对认知多样性有负面影响，而RAG有正面影响。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型倾向于生成词汇、语义和风格同质化的文本，这可能导致知识崩溃的风险，即同质化的LLM会随着时间的推移缩小可获取信息的范围。现有关于同质化的研究局限于封闭式多选题设置或模糊的语义特征，没有考察跨时间和文化背景的趋势。

Method: 提出了一种新的方法来衡量认知多样性（即LLM输出中现实世界主张的变化），使用该方法对27个LLM、涵盖12个国家的155个主题以及来自真实用户聊天的200个提示变体进行了广泛的实证研究。

Result: 研究发现：虽然新模型倾向于生成更多样化的主张，但几乎所有模型的认知多样性都低于基础网络搜索；模型大小对认知多样性有负面影响；检索增强生成（RAG）有正面影响，但RAG的改进因文化背景而异；与维基百科相比，特定国家的主张更多地反映英语而非当地语言。

Conclusion: LLM存在认知多样性不足的问题，特别是在跨文化背景下，需要改进模型以更好地代表不同文化和语言的观点。
...
TL;DR: 该论文提出Language-Mixed CoT推理方法，在英语和目标语言之间切换，以英语为锚点提升推理能力。针对韩语构建了Yi-Sang数据集，训练了多个模型，其中KO-REAson-35B在9个基准测试中取得最优表现。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注英语的推理能力蒸馏，对语言特定推理了解不足。需要探索如何构建能够处理特定语言推理任务的小型模型。

Method: 提出Language-Mixed CoT推理模式，在英语和目标语言间切换；构建Yi-Sang数据集（579万韩语提示、370万长推理轨迹）；训练9个不同规模的模型。

Result: 最佳模型KO-REAson-35B在9个基准测试中取得最高平均分64.0，其中5个排名第一，其余排名第二；所有模型平均提升18.6分；Language-Mixed CoT比单语CoT更有效。

Conclusion: Language-Mixed CoT方法有效提升了语言特定推理能力，并在跨语言和多模态任务中带来性能增益。发布了数据、模型和评估系统以促进相关研究。
...
TL;DR: 该论文提出了LongTail-Swap（LT-Swap）基准测试，专注于评估语言模型在分布尾部（罕见词）的学习能力，类似于婴儿的少样本学习能力。


<details>
  <summary>Details</summary>
Motivation: 现有BabyLM挑战主要关注词分布头部，而婴儿学习语言的特点是能够高效学习罕见词。作者希望评估语言模型在罕见词上的表现。

Method: 构建了特定预训练语料库的测试集，包含可接受与不可接受的句子对，隔离罕见词的语义和句法使用。通过零样本方式评估模型，计算每对句子的平均对数概率。

Result: 评估了16个BabyLM排行榜模型，结果显示语言模型在罕见词上表现较差，且不同架构模型在长尾分布上的性能差异比头部更显著。
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE层的token级路由将整个语义谱分配给每个专家，导致容量瓶颈、负载均衡病理和有限的专业化。需要更细粒度的路由机制来改善这些问题。

Method: 将d维嵌入划分为S个切片，每个切片由轻量级共享路由器预测top-k专家。专家独立处理分配的切片，输出重新组装以保持每个token的FLOP效率。提出了切片级容量损失、跨切片dropout和高效融合批处理GEMM内核。

Result: 在WikiText-103语言建模、WMT En-De翻译和三个文本分类数据集上的实验显示，SliceMoE比密集基线推理速度快1.7倍，比参数匹配的token-MoE困惑度降低12-18%，专家平衡性改善，并在句法与语义子空间上显示出可解释的专业化。

Conclusion: SliceMoE通过切片级路由实现了更好的专家利用率和专业化，在保持计算效率的同时显著提升了模型性能。
...
TL;DR: 提出了一种结合机器学习和深度学习的混合方法，用于波斯语方面级情感分析，通过整合多语言BERT的极性分数作为额外特征，在Pars-ABSA数据集上达到93.34%的准确率。


<details>
  <summary>Details</summary>
Motivation: 波斯语情感分析面临标注数据稀缺、预处理工具有限以及高质量嵌入和特征提取方法缺乏等挑战，需要开发有效的解决方案。

Method: 采用混合方法整合ML和DL技术，利用多语言BERT的极性分数作为决策树分类器的额外特征，并引入波斯语同义词和实体词典进行文本增强。

Result: 在Pars-ABSA数据集上实现了93.34%的准确率，超越了现有基准，证明了混合建模和特征增强的有效性。

Conclusion: 混合建模和特征增强方法能够有效推进波斯语等低资源语言的情感分析研究。
...
TL;DR: 提出了RDR2框架，通过显式整合文档结构信息来增强检索增强生成(RAG)系统，在五个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法将检索到的段落视为孤立块，忽略了文档组织结构这一关键信息，导致知识获取和利用受限。

Method: RDR2采用基于LLM的路由器动态导航文档结构树，联合评估内容相关性和层次关系，将文档路由制定为可训练任务。

Result: 在五个具有挑战性的数据集上，RDR2实现了最先进的性能，特别是在需要多文档合成的复杂场景中表现突出。

Conclusion: 显式结构感知显著增强了RAG系统获取和利用知识的能力，证明了文档结构信息在RAG过程中的重要性。
...
TL;DR: 提出了Distributional Correctness Score (DCS)评估指标，通过考虑模型在答案选择上的完整概率分布来区分有害的过度自信和通过弃权表达的不确定性，提供更细致的评估范式。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估范式主要关注单一响应的准确性评分，无法捕捉模型完整的信念状态。模型产生幻觉部分原因是它们被优化为在二元评分方案下成为好的应试者，这些方案奖励任何答案而非弃权。

Method: 引入DCS评估指标，该指标自然区分对错误答案的有害过度自信和通过"我不知道"响应表达的不确定性，在可解释的默认范围内提供分数。

Result: 理论分析和示例表明DCS提供了更细致和对齐的评估范式，激励模型表达真正的不确定性而非猜测。在12个现有评估基准上测试6个语言模型，发现半数基准的分数在所有测试模型中均为负值，表明存在显著的幻觉倾向。

Conclusion: DCS解决了不考虑模型在答案选择上完整概率分布的问题，能够更准确地评估语言模型的信念状态，揭示模型幻觉的普遍性。
...
TL;DR: 论文发现安全对齐的大语言模型存在后果盲视问题，即模型过度依赖表面信号而忽视行为与后果的关联，导致易被越狱或过度拒绝无害输入。作者构建了CB-Bench基准和CS-Chain-4k数据集来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前安全对齐的LLMs存在两个主要失效模式：易被越狱和过度拒绝无害输入。作者发现这些问题的共同根源是模型对行为与后果关联的推理能力不足，过度依赖表面形式信号。

Method: 构建CB-Bench基准覆盖四种风险场景，评估语义风险与结果风险的匹配情况。开发CS-Chain-4k后果推理数据集用于安全对齐微调。

Result: 主流模型在CB-Bench上持续无法区分不同风险类型，表现出系统性后果盲视。使用CS-Chain-4k微调的模型在语义伪装越狱攻击中表现更好，减少了对无害输入的过度拒绝，同时保持了其他基准上的效用和泛化能力。

Conclusion: 后果盲视是当前对齐方法的普遍系统性问题，后果感知推理应成为核心对齐目标。CS-Chain-4k提供了一条更实用和可复现的评估路径。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验研究报告质量影响临床决策，需要评估大语言模型在此类任务中的表现。

Method: 创建CONSORT-QA评估语料库，使用不同大语言模型和提示方法（包括思维链）评估CONSORT标准。

Result: 最佳模型和提示方法组合达到85%准确率，思维链方法提供了有价值的推理信息。

Conclusion: 大语言模型能够有效评估临床试验研究报告质量，思维链方法有助于理解模型推理过程。
...
TL;DR: 提出接种提示法：通过在微调数据前添加系统提示指令来有意识地引发不良特征，从而在测试时降低模型表达这些特征的可能性。


<details>
  <summary>Details</summary>
Motivation: 语言模型微调通常会同时学习到期望和不期望的特征，需要一种方法来选择性学习，避免不良特征的泛化。

Method: 在微调数据前添加短系统提示指令，有意识地引发不良特征，然后在测试时不使用该指令。

Result: 接种后的模型在测试时表达不良特征的概率显著降低，该方法在多个场景中有效：减少任务特定微调中的新兴错位、防御后门注入、减轻潜意识学习中的特征传递。

Conclusion: 接种提示法是一种简单有效的选择性学习技术，通过使特征不再意外来减少优化压力，从而降低泛化程度，有助于更好地理解语言模型的泛化机制。
...
TL;DR: 该论文提出了一种基于注意力机制和梯度信息的推理时防御方法，用于检测预训练语言模型中的后门攻击，通过结合token级注意力和梯度异常评分来显著降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 预训练语言模型虽然在NLP任务中表现出色，但容易受到后门攻击的威胁，攻击者通过在训练数据中嵌入恶意触发模式，使得模型在正常使用时表现正常，但遇到特定触发模式时会产生目标错误分类。

Method: 提出了一种推理时防御方法，通过分析后门模型在处理中毒输入时的内部行为特征，特别是触发token在注意力和梯度信号中的主导作用，构建结合token级注意力和梯度信息的异常评分机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在语义等价查询下输出不一致，影响用户信任和系统可靠性，特别是在高风险领域部署时

Method: 提出PS-GRPO强化学习方法，使用多组改写查询进行组相似度奖励分配，并引入可扩展的近似计算方法

Result: 在多个问答基准测试中，Con-RAG显著提升了一致性和准确性，无需显式真值监督
...
TL;DR: 提出了第一个大规模的后编辑时间标注数据集，并引入PEET评分器来评估语法纠错工具的用户体验，通过估计后编辑时间来量化工具节省的用户工作量。


<details>
  <summary>Details</summary>
Motivation: 在文本编辑过程中，高效的语法纠错工具能显著影响后续人工编辑工作和最终文本质量。需要量化GEC工具的用户体验，评估其能为用户节省多少工作量。

Method: 构建了两个英语GEC测试数据集（BEA19和CoNLL14）的大规模后编辑时间标注数据集，提出PEET评分器来估计后编辑时间并排名GEC工具。

Result: 量化了GEC工具在文本编辑中节省的时间，分析表明判断句子是否需要修正以及改写和标点符号更改等编辑类型对后编辑时间影响最大。PEET与人工排名相关性良好。

Conclusion: PEET为评估GEC工具可用性提供了以人为中心的新方向，数据集和代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法产生的提示要么包含无意义标记，要么改变了原意，无法真实反映实践中幻觉的发生情况。需要开发能保持语义等价性和连贯性的现实攻击方法。

Method: 将寻找现实攻击表述为在语义等价和连贯约束下的约束优化问题，引入保持约束的零阶方法来有效搜索对抗性提示。

Result: 在开放式多项选择问答任务中，SECA相比现有方法获得更高的攻击成功率，同时几乎不违反约束条件。

Conclusion: SECA揭示了开源和商业LLM对现实且合理的提示变异的敏感性，为理解LLM幻觉提供了更真实的评估框架。
...
TL;DR: 该研究探讨了大型语言模型生成文本是否保留语义同位素，通过故事续写实验发现LLM在一定token范围内能够保持语义同位素。


<details>
  <summary>Details</summary>
Motivation: 探索文本语义与大型语言模型的相关性，研究LLM生成文本是否保持语义同位素，扩展分布语义与结构语义之间联系的先前见解。

Method: 设计故事续写实验，使用10,000个ROCStories提示由五个LLM完成；首先验证GPT-4o从语言基准中提取同位素的能力，然后应用于生成的故事；分析同位素的结构和语义属性。

Result: 结果显示，在给定的token范围内，LLM续写能够跨多个属性保持语义同位素。

Conclusion: LLM在一定token范围内完成文本时能够有效保持语义同位素，表明模型在生成过程中能够维护语义连贯性。
...
TL;DR: 该研究从作者和会议层面分析NLP4SG研究现状，发现ACL作者在非ACL会议上发表的社会公益研究更多，且大多数NLP4SG研究由非ACL作者在非ACL会议上发表。


<details>
  <summary>Details</summary>
Motivation: 随着NLP社会影响力日益重要，NLP4SG研究社区不断扩大，近20%ACL论文涉及联合国可持续发展目标。研究旨在从作者和会议层面量化NLP4SG研究现状。

Method: 采用作者和会议层面的视角，量化ACL社区内外、核心ACL贡献者和非ACL作者在NLP4SG研究中的比例分布。

Result: 发现两个重要现象：ACL作者在非ACL会议上发表社会公益研究的可能性显著更高；大多数NLP4SG研究由非ACL作者在非ACL会议上完成。

Conclusion: 这些发现对ACL社区在NLP4SG议程设置方面具有重要启示意义。
...
TL;DR: 论文主张在大型语言模型的不确定性量化中考虑未观测序列的概率，这对提升幻觉检测效果至关重要。


<details>
  <summary>Details</summary>
Motivation: 在安全关键应用中量化LLM的不确定性很重要，能帮助发现错误答案（幻觉）。现有基于熵估计的方法主要依赖观测到的输出序列，但未考虑未观测序列的影响。

Method: 通过实验证明未观测序列概率在不确定性量化中的关键作用，建议未来研究整合这一因素来增强LLM不确定性量化方法。

Result: 实验结果表明未观测序列概率对提升不确定性量化效果具有重要作用。

Conclusion: 未来LLM不确定性量化研究应该整合未观测序列概率，以更准确地检测幻觉和评估模型可靠性。
...
TL;DR: 提出了一种动态整合监督微调(SFT)和强化学习(RL)的即插即用框架，通过选择具有挑战性的样本进行SFT，显著减少数据需求并避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法在推理任务中难以扩展推理边界，因为它们只从自身推理轨迹学习；SFT虽然有益但需要大量数据且容易过拟合。结合SFT和RL面临数据效率低、算法特定设计和灾难性遗忘三大挑战。

Method: 动态选择挑战性样本进行SFT，计算高熵token的损失以减轻灾难性遗忘，并冻结对RL至关重要的参数。该方法与具体RL或SFT算法无关。

Result: 仅使用先前最先进方法1.5%的SFT数据和20.4%的RL数据，就实现了最先进的推理性能。

Conclusion: 该方法为推理后训练中结合SFT和RL提供了一种高效且即插即用的解决方案。
...
TL;DR: CCA是一种新型注意力机制，通过将查询、键和值下投影到共享潜在空间来大幅减少参数、KV缓存和FLOPs，与头共享技术结合形成CCGQA，在保持性能的同时实现8倍KV缓存压缩和显著的计算加速。


<details>
  <summary>Details</summary>
Motivation: 多头注意力(MHA)的二次计算复杂度和线性增长的KV缓存使得长上下文transformer训练和服务成本高昂，现有方法如GQA和MLA主要减少缓存但对计算效率提升有限。

Method: 提出压缩卷积注意力(CCA)，在共享潜在空间中进行注意力操作，并与分组查询注意力(GQA)结合形成CCGQA，通过压缩因子同时减少参数、KV缓存和FLOPs。

Result: CCGQA在密集和MoE模型上均优于GQA和MLA，在MoE模型上以GQA和MLA一半的KV缓存实现8倍压缩且性能无损失，在H100 GPU上预填充延迟减少约1.7倍，反向传播加速约1.3倍。

Conclusion: CCA和CCGQA通过压缩注意力机制有效解决了transformer的长上下文计算和内存瓶颈，在保持模型质量的同时显著提升了训练和推理效率。
...
TL;DR: PsySET是一个心理学基准测试，用于评估LLM在情绪和人格领域的引导效果和可信度，涵盖多种模型和引导策略，发现不同策略在控制精度和输出质量之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 控制LLM的情绪状态和人格特质对于实现丰富、以人为中心的社会交互至关重要，需要评估不同引导策略的有效性和可信度。

Method: 使用PsySET基准测试，涵盖四个不同LLM家族的模型，结合提示、微调和表示工程等多种引导策略进行评估。

Result: 提示策略持续有效但强度控制有限，向量注入实现更精细控制但略微降低输出质量；情绪引导会产生特殊副作用，如喜悦会降低对抗性事实鲁棒性，愤怒会增加毒性但增强泄漏抵抗。

Conclusion: 该框架建立了首个情绪和人格引导的整体评估，为社交交互应用的可解释性和可靠性提供了见解。
...
TL;DR: GenQuest是一个基于大型语言模型的生成式文本冒险游戏，通过沉浸式互动故事促进第二语言学习，为EFL学习者提供个性化内容和词汇辅助功能。


<details>
  <summary>Details</summary>
Motivation: 利用生成式AI技术为英语学习者创造沉浸式、个性化的语言学习体验，通过互动故事提高学习动机和效果。

Method: 采用选择你自己的冒险风格叙事，动态生成响应学习者选择的故事情节，包含分支决策点和故事里程碑，并提供针对学习者水平的个性化内容生成和词汇查询辅助。

Result: 对中国大学生EFL学习者的试点研究表明，该系统在词汇习得方面显示出积极效果，用户反馈良好，同时参与者建议改进叙事长度和质量，增加多模态内容如图像。

Conclusion: GenQuest证明了生成式AI在语言学习中的潜力，通过互动故事提供有效的学习体验，未来可进一步优化叙事质量和多模态集成。
...
TL;DR: GRACE是一个新颖的框架，将对比信号重新构想为奖励而非损失函数，通过策略梯度优化训练LLM生成可解释的推理过程，同时获得高质量嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 现有方法将LLM作为黑盒函数进行对比学习，丢弃了其生成和推理能力，仅获得静态嵌入。GRACE旨在重新利用LLM的生成能力，使其成为可解释的智能体。

Method: 使用策略梯度优化，将LLM作为策略网络生成可解释的推理过程，通过多组件奖励函数最大化正样本对相似度并最小化负样本相似度。
...
TL;DR: 提出ALC辅助学习策略，通过细粒度嵌入提升产品推荐系统的覆盖率，结合最难负样本构建判别性训练信号。
...
Method: 引入两种训练目标，利用批次中最难负样本在正负样本间构建判别性训练信号，结合阈值一致性边界损失。

Result: 在LF-AmazonTitles-131K和Tech and Durables数据集上验证，结合最近阈值一致性边界损失时达到最先进的覆盖率。
...
<details>
  <summary>Details</summary>
Motivation: 探究LLMs是否具有类似人类的复数指代表示偏好，以及能否检测复数照应表达中的歧义并识别可能的指代对象。

Method: 设计实验：使用下一词预测任务进行代词生成、代词解释，以及使用不同提示策略进行歧义检测。

Result: LLMs有时能识别歧义代词的潜在指代对象，但在选择解释时不总是遵循人类偏好，尤其是在可能的解释未被明确提及时。它们难以在没有直接指令时识别歧义，且不同类型实验结果存在不一致性。

Conclusion: LLMs在复数指代处理上表现出与人类不同的模式，存在识别歧义和选择指代对象的局限性，结果在不同实验类型中也不一致。
...
TL;DR: 研究发现大型音频语言模型在多项选择题评估中对选项顺序、问题表述和选项改写敏感，提出了考虑这些细微变化的更简单评估协议和指标。


<details>
  <summary>Details</summary>
Motivation: 现有MCQA评估框架只报告单一准确率，但选项顺序等细微变化会导致结果显著不同，需要更全面的评估方法。

Method: 系统研究三个基准测试和四个模型，分析模型对选项顺序、问题改写和选项改写的敏感性。

Result: 模型不仅对选项顺序敏感，也对问题表述和选项改写敏感，现有评估方法不能充分反映模型性能。

Conclusion: 提出了考虑细微变化的更简单评估协议和指标，为MCQA框架下LALMs提供更详细的评估报告。
...
TL;DR: FedSRD框架通过稀疏化-重构-分解方法，在联邦学习中显著降低LoRA微调的通信开销达90%，同时提升异构客户端数据上的模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前在公开网络数据上训练大语言模型的范式不可持续，高质量专业领域数据源接近枯竭。联邦学习作为下一代AI的实用解决方案，但在联邦设置中应用LoRA时，通信开销成为关键瓶颈，且LoRA参数的结构冗余导致通信负担重和客户端更新冲突。

Method: 提出FedSRD框架：1)重要性感知稀疏化方法，保持LoRA更新结构完整性以减少上传参数数量；2)服务器在全秩空间重构和聚合更新以缓解冲突；3)将全局更新分解为稀疏低秩格式进行广播。还提出计算开销更低的变体FedSRD-e。

Result: 在10个基准测试上的实验结果表明，该框架显著降低通信成本达90%，同时在异构客户端数据上甚至提升了模型性能。

Conclusion: FedSRD为联邦学习中LoRA微调提供了一种通信高效的解决方案，有效解决了通信瓶颈和更新冲突问题，为下一代AI在去中心化网络上的发展提供了实用方法。
...
TL;DR: 将SciNCL图感知邻域对比学习方法应用于流程工业领域，通过知识图谱增强语言模型，在PITEB基准测试中性能提升9.8-14.3%，模型尺寸缩小3-5倍。


<details>
  <summary>Details</summary>
Motivation: 利用知识图谱增强预训练语言模型，学习领域特定术语和文档间关系，特别是在流程工业领域，文本日志包含关键操作信息且通常构建为稀疏知识图谱。

Method: 应用SciNCL图感知邻域对比学习方法，从知识图谱中提取三元组来微调语言模型。

Result: 在专有流程工业文本嵌入基准测试(PITEB)上，性能比最先进的mE5-large文本编码器提升9.8-14.3%(5.4-8.0个百分点)，同时模型尺寸缩小3-5倍。

Conclusion: SciNCL方法能有效提升流程工业领域文本嵌入性能，在保持较小模型尺寸的同时显著超越现有方法。
...
TL;DR: 本文提出了一个评估框架，用于评估大语言模型在检测英语文本中针对人口统计特征的社会偏见的能力，发现微调的小模型在可扩展检测方面有潜力，但在多人口统计目标偏见方面仍存在差距。


<details>
  <summary>Details</summary>
Motivation: 大规模网络爬取文本语料库通常包含有害的人口统计目标社会偏见，需要数据审计和可扩展的偏见检测方法。现有研究范围狭窄，缺乏对LLMs自动偏见检测能力的全面理解。

Method: 构建了一个全面的评估框架，将偏见检测构建为多标签任务，使用人口统计焦点分类法。系统评估了不同规模和技术的模型，包括提示、上下文学习和微调，使用12个涵盖不同内容类型和人口统计特征的数据集。

Result: 研究表明微调的小模型在可扩展检测方面表现出潜力，但分析也揭示了在人口统计轴和多人口统计目标偏见方面存在持续差距。
...
<details>
  <summary>Details</summary>
Motivation: 当前医疗决策树构建方法依赖耗时费力的人工标注，需要自动化解决方案来支持临床决策系统的建设。

Method: PI-LoRA（路径集成LoRA）是一种新颖的低秩自适应方法，通过集成梯度路径信息捕捉不同模块间的协同效应，实现更有效的秩分配。
...
TL;DR: 提出了基于核心焦点引导的优化框架，用于医学问题摘要任务，通过焦点提取、数据构建和多维度质量评估，显著提升模型性能并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 在线医疗平台快速发展，消费者健康问题存在信息冗余和非专业术语，导致诊断效率低下。现有医学问题摘要方法在问题焦点识别和模型幻觉方面仍面临挑战。

Method: 1. 设计提示模板驱动LLM提取忠实于原文的核心焦点；2. 结合原始CHQ-FAQ对构建微调数据集；3. 提出多维度质量评估和选择机制。

Result: 在两个广泛采用的MQS数据集上使用三个评估指标进行实验，在所有指标上达到最先进性能，显著提升问题关键焦点识别能力并明显减少幻觉。

Conclusion: 基于核心焦点引导的优化框架有效解决了医学问题摘要中的焦点识别偏差和幻觉问题，为LLM在医疗领域的应用提供了可靠解决方案。
...
TL;DR: 提出了MATPO方法，在单个LLM实例中通过强化学习训练规划者和工作者两种角色，解决了多智能体工具集成框架的强化学习训练问题，显著提升了复杂推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有单智能体方法存在上下文长度限制和工具响应噪声问题，而多智能体框架虽然能管理上下文，但缺乏有效的强化学习训练方法。

Method: MATPO通过角色特定提示在单个LLM中训练规划者和工作者角色，采用基于原则的信用分配机制在两种角色间分配奖励。
...
TL;DR: TiTok是一个新的框架，通过token级知识转移实现有效的LoRA移植，无需额外模型或开销就能在不同骨干模型间转移适配参数。


<details>
  <summary>Details</summary>
Motivation: 解决PEFT方法如LoRA中适配参数依赖于基础模型、无法跨不同骨干模型转移的问题，避免知识蒸馏对训练数据的依赖和额外判别器模型的复杂性。

Method: 通过源模型在有和没有LoRA情况下的对比差异来捕获任务相关信息，突出信息丰富的token并选择性过滤合成数据。

Result: 在三个基准测试的多种转移设置中，该方法始终有效，相比基线平均性能提升4~8%。

Conclusion: TiTok框架能够有效实现LoRA移植，在不同骨干模型间转移适配参数，且无需额外模型或开销。
...
TL;DR: 分析了MoE模型在多语言数据中的路由模式，发现在中间层存在跨语言路由对齐现象，并提出了通过干预中间层路由来提升多语言性能的方法。
...
Method: 使用并行多语言数据集分析专家路由模式，提出在推理时干预中间层路由的方法，促进英语中频繁激活的任务专家。

Result: 干预方法在多个评估任务、模型和15+种语言上实现了1-2%的稳定性能提升，而其他干预方式则导致性能下降。
...
TL;DR: JSON Whisperer是一个框架，让LLM能够生成RFC 6902差异补丁而非完整文档，通过EASE编码解决数组索引问题，显著减少计算开销。
...
Result: 补丁生成结合EASE可将token使用量减少31%，同时编辑质量保持在完整重新生成的5%以内，在复杂指令和列表操作方面表现尤其突出。

Conclusion: JSON Whisperer通过补丁生成和EASE编码，为LLM提供了更高效的JSON文档编辑方法，显著降低了计算成本。
...
TL;DR: 为僧伽罗语成人阅读障碍者开发的多模态辅助系统，整合语音转文字、错误识别、文本修正和语音输出功能，在资源有限的语言环境中取得良好效果
...
Method: 整合Whisper进行语音转文字，使用SinBERT识别常见阅读障碍错误，结合mT5和Mistral模型生成修正文本，最后通过gTTS转回语音形成完整多模态反馈循环

Result: 在僧伽罗语数据集有限的情况下，系统达到0.66的转录准确率、0.7的修正准确率和0.65的整体系统准确率

Conclusion: 该方法在资源有限语言环境中具有可行性和有效性，突显了包容性自然语言处理技术在代表性不足语言中的重要性
...
TL;DR: 提出一种两阶段检索架构，结合轻量级ModernBERT进行初始候选检索和ColBERTv2进行精细重排序，在生物医学RAG系统中实现最先进性能。
...
Method: 使用ModernBERT双向编码器进行高效初始候选检索，ColBERTv2后期交互模型进行细粒度重排序，在PubMedQA上微调IR模块。

Result: ColBERT重排序器将Recall@3提升4.2个百分点，在MIRAGE QA基准测试中达到0.4448的平均准确率，优于MedCPT等基线。
...
TL;DR: 该论文提出了一个评估语言模型识别Grice会话准则违反的新基准，比较了在不同规模数据上预训练的BabyLMs与儿童和大型语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 隐含意义是人类交流的重要组成部分，语言模型需要能够识别和解释这些含义。Grice的会话准则理论表明，说话者可能故意违反这些原则来表达字面之外的含义。

Method: 基于Surian等人对儿童识别Grice准则违反的研究，创建了一个新基准来测试在少于10M和100M token上预训练的BabyLMs区分准则遵守和违反话语的能力。

Result: 在少于100M token上训练的模型整体表现优于在少于10M token上训练的模型，但仍未达到儿童和大型语言模型（在3T token上训练）的水平。适度的数据增加改善了语用行为的某些方面。

Conclusion: 研究表明，虽然数据量的增加能够提升语言模型在语用推理方面的表现，但当前的小规模模型仍无法达到人类儿童或大规模语言模型的语用能力水平。
...
TL;DR: 对结合自注意力机制与结构化状态空间模型（如Mamba）的混合架构进行全面评估，分析不同混合策略的关键因素和有效性。


<details>
  <summary>Details</summary>
Motivation: 虽然混合架构在长上下文任务中展现出良好的建模质量和计算效率平衡，但缺乏对混合策略的系统比较和有效性关键因素的分析。

Method: 基于层间（顺序）或层内（并行）融合的混合架构，从语言建模性能、长上下文能力、扩展分析、训练和推理效率等多角度进行评估。
...
TL;DR: 研究表明，濒危语言构建语音识别系统的数据需求远低于预期，仅需40分钟的发音资源数据即可实现可用的ASR系统（词错误率<50%），为资源匮乏的濒危语言社区提供了希望。


<details>
  <summary>Details</summary>
Motivation: 全球近半数语言濒临灭绝，标准语音识别系统需要句子级监督数据，而大多数濒危语言缺乏此类数据。本文探索构建濒危语言ASR系统所需的最小数据量和数据形式。

Method: 使用短格式发音资源作为替代数据源，在曼克斯盖尔语（约2200名使用者）上进行实验，仅用40分钟数据构建ASR系统，并在康沃尔语（约600名使用者）上复现该方法。

Result: 40分钟的发音资源数据即可为曼克斯盖尔语构建可用的ASR系统（词错误率<50%），该方法在康沃尔语上也成功复现。

Conclusion: 构建濒危语言ASR系统的数据门槛（数量与形式）远低于传统认知，为无法满足传统数据要求的濒危语言社区提供了可行方案。
...
TL;DR: 研究分析LLM训练中下游任务性能的波动问题，提出通过检查点平均和集成方法来提高性能稳定性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练过程中，下游任务性能存在显著波动，难以确定真正的最佳检查点。

Method: 实证分析LLM训练性能波动，研究检查点平均和集成两种后处理方法。

Result: 发现任务分数在训练过程中频繁波动，检查点平均和集成方法能有效减少性能波动。

Conclusion: 检查点平均和集成方法能提高下游性能稳定性，无需改变训练过程。
...
TL;DR: ReplaceMe是一种无需训练即可对Transformer模型进行深度剪枝的方法，通过用线性变换替换Transformer块，在保持高性能的同时实现模型压缩。


<details>
  <summary>Details</summary>
Motivation: 传统的剪枝方法需要额外的训练或微调，计算成本高。本文旨在开发一种无需训练即可有效剪枝的方法，减少计算开销。

Method: 使用小型校准数据集估计线性变换来近似被剪枝的Transformer块，该线性映射可与剩余块无缝合并，无需额外网络参数。

Result: 在多个大语言模型上实现高达25%的剪枝率，同时保持约90%的原始模型性能，计算开销极小。

Conclusion: ReplaceMe在无需训练的情况下优于其他训练无关方法，并与需要大量重新训练的最先进剪枝方法保持竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 现有幻觉基准测试主要在序列级别且仅限于英语，缺乏细粒度的多语言监督，无法全面评估LLM的幻觉问题。

Method: 使用GPT-4o从维基百科生成问答对，在无上下文设置下从不同LLM获取可能幻觉的答案，然后通过GPT-4o自动标注幻觉span。

Result: 基于编码器的模型在所有语言中表现最强，PsiloQA展示了有效的跨语言泛化能力，并能将知识迁移到其他基准测试。

Conclusion: PsiloQA数据集和结果推动了多语言设置下可扩展、细粒度幻觉检测的发展，比人工标注数据集更具成本效益。
...
TL;DR: 提出了Token Probability Deviation (TBD)方法用于检测推理蒸馏数据，解决蒸馏模型评估中的基准污染问题。该方法通过分析生成token的概率模式来区分已见和未见问题。


<details>
  <summary>Details</summary>
Motivation: 推理蒸馏虽然能增强大语言模型的推理能力，但可能导致基准污染，即蒸馏数据集中的评估数据会人为提升蒸馏模型的性能指标。

Method: 提出TBD方法，利用生成输出token的概率模式。核心思想是量化生成token概率与高参考概率的偏差程度，蒸馏模型对已见问题倾向于生成接近确定性的token，而对未见问题会产生更多低概率token。

Result: 在S1数据集上实现了0.918的AUC和0.470的TPR@1% FPR，表现出优越的检测性能。

Conclusion: TBD方法能有效检测蒸馏数据，为评估蒸馏模型的真实能力提供了可靠工具，解决了基准污染问题。
...
TL;DR: SocialHarmBench是一个包含585个提示的数据集，涵盖7个社会政治类别和34个国家，用于测试LLM在政治敏感语境中的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准很少测试政治操纵、宣传和虚假信息生成等领域的漏洞，而这些领域的失败可能产生直接的社会政治后果。

Method: 创建SocialHarmBench数据集，评估LLM在7个社会政治类别和34个国家背景下的响应，分析时间维度和地理维度的脆弱性。

Result: 开源模型表现出高脆弱性，Mistral-7B在历史修正主义、宣传和政治操纵等领域的攻击成功率高达97%-98%；LLM在21世纪和前20世纪语境以及拉美、美国、英国等地区最为脆弱。

Conclusion: 当前的安全措施无法推广到高风险的社会政治环境中，暴露了系统性偏见，对LLM在保护人权和民主价值观方面的可靠性提出了担忧。
...
TL;DR: 本文研究了NL2SQL任务中训练数据与目标查询的结构对齐问题，发现结构对齐度是预测微调成功的关键指标。


<details>
  <summary>Details</summary>
Motivation: 监督微调(SFT)虽然能有效适配大语言模型，但训练数据的变异性会阻碍模型跨域泛化能力。本文旨在探究NL2SQL任务中训练数据与目标查询的结构对齐对模型性能的影响。

Method: 通过比较训练集、目标数据和模型预测中SQL结构特征的分布来估计对齐度，在三个大型跨域NL2SQL基准和多个模型家族上进行全面实验。

Result: 结构对齐是微调成功的强预测因子：对齐度高时，SFT能显著提升准确率和SQL生成质量；对齐度低时，改进微乎其微甚至没有。

Conclusion: 这些发现强调了在NL2SQL任务中，采用对齐感知的数据选择对于有效微调和泛化的重要性。
...
TL;DR: 提出Layer-wise Semantic Dynamics (LSD)框架，通过分析transformer层间隐藏状态语义的动态变化来检测大语言模型的幻觉现象，相比现有方法更高效且无需外部验证。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生流畅但事实错误的陈述（幻觉现象），在高风险领域构成严重威胁，需要开发内在的幻觉检测方法。

Method: 使用基于边距的对比学习，将隐藏激活与事实编码器生成的真实嵌入对齐，分析语义轨迹的分离模式：事实性回答保持稳定对齐，而幻觉表现出明显的语义漂移。

Result: 在TruthfulQA和合成事实-幻觉数据集上评估，LSD达到F1分数0.92、AUROC 0.96和聚类准确率0.89，优于SelfCheckGPT和语义熵基线，仅需单次前向传播，速度提升5-20倍。

Conclusion: LSD提供了一种可扩展、模型无关的实时幻觉监测机制，并为大语言模型中事实一致性的几何特性提供了新见解。
...
TL;DR: 本文为Nawatl语言引入了一个上下文无关文法（CFG），用于生成语法正确的合成句子，以扩充该低资源语言的语料库。


<details>
  <summary>Details</summary>
Motivation: Nawatl是一种数字资源稀少的π-语言类型，缺乏用于机器学习的语料库。目标是生成大量语法正确的合成句子，以增加语言模型训练的可用语料。

Method: 使用上下文无关文法（CFG）来生成人工句子，扩充名为π-yalli的Nawatl语料库。

Result: 通过使用文法，在句子级语义任务上相比某些LLMs取得了比较性改进。但需要更有效的文法模型才能实现更显著的改进。

Conclusion: 文法方法可以有效扩充低资源语言的语料库，但要获得更显著的改进，需要开发更精确建模Nawatl语言的文法。
...
<details>
  <summary>Details</summary>
Motivation: 探索自然语言提示的礼貌程度和语气对大型语言模型性能的影响，特别是不同礼貌级别的提示如何影响模型在选择题上的准确率。

Method: 创建包含50个基础问题的数据集，涵盖数学、科学和历史领域，每个问题重写为5种语气变体（非常礼貌、礼貌、中性、粗鲁、非常粗鲁），共250个独特提示。使用ChatGPT 4o评估响应，并应用配对样本t检验评估统计显著性。

Result: 与预期相反，不礼貌的提示词始终优于礼貌提示词，准确率从非常礼貌提示的80.8%到非常粗鲁提示的84.8%。这些发现与早期研究将粗鲁与较差结果关联的结论不同。

Conclusion: 新的大型语言模型可能对语气变化有不同的响应方式，研究强调了研究提示语用方面的重要性，并提出了关于人机交互社会维度的更广泛问题。
...
TL;DR: AWARE框架通过提升Transformer模型在三个关键方面的感知能力（领域感知、上下文感知、类别重叠感知），显著改善了从学生反思中识别文化资本主题的性能。


<details>
  <summary>Details</summary>
Motivation: 学生反思中的文化资本主题（如抱负目标、家庭支持）通常以叙事形式呈现，而非直接关键词，这使得标准NLP模型难以检测。标准模型缺乏对领域特定语言和叙事背景的感知能力。

Method: AWARE框架包含三个核心组件：1）领域感知 - 调整模型词汇以适应学生反思的语言风格；2）上下文感知 - 生成考虑全文背景的句子嵌入；3）类别重叠感知 - 使用多标签策略识别单个句子中多个主题的共存。

Result: AWARE在Macro-F1指标上比强基线高出2.1个百分点，并在所有主题上都显示出显著改进。

Conclusion: 这项工作为任何依赖叙事上下文意义的文本分类任务提供了一个鲁棒且可推广的方法论。
...
TL;DR: 提出了一种资源高效的LLaMA-3.2-3B微调方法，使用LoRA和QLoRA技术在受限GPU和内存环境下增强医学链式推理能力，相比标准全微调减少60%内存使用。
...
Method: 采用参数高效微调技术（LoRA和QLoRA），在公开医学推理数据集上对LLaMA-3.2-3B模型进行适配，实现在受限GPU和内存环境下的高效微调。

Result: 模型在医学推理一致性和事实准确性方面得到改善，同时相比标准全微调减少高达60%的内存使用量，在医学问答任务中保持了强大的推理能力。

Conclusion: 轻量级适配可以在低资源研究环境中有效部署LLM，为医疗AI系统在效率和领域专业化之间的平衡提供了实用策略。
...
TL;DR: 提出了一种利用Unicode变体选择器进行不可感知的越狱攻击方法，通过在恶意问题后附加不可见的变体选择器，使提示在视觉上与原问题相同但tokenization被秘密改变，从而诱导有害响应。
...
Method: 使用Unicode变体选择器构建不可见的对抗后缀，提出链式搜索流水线来生成此类对抗后缀，改变tokenization但不产生可见修改。

Result: 实验表明该方法对四个对齐的LLM实现了高攻击成功率，并能推广到提示注入攻击，所有攻击都不产生可见的提示修改。

Conclusion: 证明了文本模态中不可感知越狱攻击的可行性，通过利用Unicode变体选择器实现视觉上不可察觉但有效的攻击。
...
TL;DR: 提出了两个新的魁北克法语方言基准数据集QFrCoRE和QFrCoRT，用于测试方言理解能力，特别是通过区域性习语来衡量LLM在特定方言上的熟练度。


<details>
  <summary>Details</summary>
Motivation: 将习语理解和方言理解两个任务结合起来，使用区域性习语作为方言理解的测试基准。

Method: 构建了两个魁北克法语方言数据集：QFrCoRE包含4633个习语短语实例，QFrCoRT包含171个区域性习语单词实例，并提供了可复制的构建方法。

Result: 通过对94个LLM的实验表明，这些区域性习语基准是衡量模型在特定方言上熟练度的可靠工具。

Conclusion: 区域性习语可以作为有效的方言理解评估基准，该方法可推广到其他方言的研究中。
...
TL;DR: 提出了一种名为GQR的新方法，通过轻量级文本检索器增强视觉中心模型，在视觉文档检索中实现了性能与效率的帕累托优化。


<details>
  <summary>Details</summary>
Motivation: 解决多模态编码器在视觉文档检索中面临的两个挑战：大规模表示带来的部署障碍，以及视觉语言模型固有的模态差距限制。

Method: 引入引导查询优化(GQR)方法，在测试时使用互补检索器的分数来优化主要检索器的查询嵌入，实现细粒度的表示空间交互。
...
Method: 构建COLE基准测试，包含23个多样化任务，涵盖情感分析、释义检测、语法判断和推理等能力，并对94个大语言模型进行基准测试。

Result: 结果显示闭源和开源模型之间存在显著性能差距，识别出零样本抽取式问答、细粒度词义消歧和地区语言变体理解等关键挑战领域。
...
TL;DR: SwiReasoning是一个无需训练的LLM推理框架，通过动态切换显式和潜在推理来解决潜在推理中的搜索分布扩散和过度思考问题，显著提升准确性和令牌效率。


<details>
  <summary>Details</summary>
Motivation: 解决潜在推理中的两个关键挑战：1) 纯潜在推理会扩大搜索分布，引入噪声并阻碍收敛到高置信度解；2) 无显式文本时仍存在过度思考，浪费令牌并降低效率。

Method: SwiReasoning框架包含两个关键创新：1) 基于下一个令牌分布熵趋势的块级置信度估计，动态切换显式和潜在推理；2) 限制思考块切换的最大次数以控制过度思考。

Result: 在数学和STEM基准测试中，SwiReasoning将不同模型家族和规模的推理LLM的平均准确率提高了1.5%-2.8%；在受限预算下，平均令牌效率提升了56%-79%，预算越紧提升越大。

Conclusion: SwiReasoning通过动态推理切换机制有效平衡了探索与利用，解决了潜在推理的核心挑战，在准确性和效率方面均取得了显著改进。
...
TL;DR: 本文提出了一种三阶段方法来协调多个小型语言模型（SLM），通过SLM-MUX架构和优化策略，在多个基准测试中显著优于现有协调方法。


<details>
  <summary>Details</summary>
Motivation: 随着小型语言模型数量的快速增长，虽然它们无法达到最先进的精度，但在特定任务上表现出色且更高效。现有协调方法主要针对前沿模型，在SLM上表现不佳，因此需要专门针对SLM的协调方法。

Method: 提出三阶段方法：1）SLM-MUX多模型架构有效协调多个SLM；2）模型选择搜索从给定池中识别最具互补性的SLM；3）针对SLM-MUX的测试时缩放策略。

Result: 在MATH上提升13.4%，GPQA上提升8.8%，GSM8K上提升7.0%。仅用两个SLM，SLM-MUX在GPQA和GSM8K上超越Qwen 2.5 72B，在MATH上与之相当。

Conclusion: 通过所提出的方法，小型语言模型可以被有效协调成更准确和高效的系统，并提供了理论分析来支持该方法的优势。
...
Method: 使用10万小时真实学生-导师对话数据，通过参数高效微调开发真实学生模型，生成高保真合成对话，并提出多轮评估协议

Result: 微调后显著改善对话和教学表现：学生发言时间翻倍，提问风格改善，对话轮次增加50%，教学个性化程度更高

Conclusion: 在真实学习数据上进行微调可显著提升LLM的教学对话能力，为教育AI发展提供有效途径
...
TL;DR: 提出了Tolerator解码策略，通过两阶段过程（序列填充和迭代精炼）解决离散扩散大语言模型中令牌一旦接受就无法修改的问题，实现了令牌级别的交叉验证和修正。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型作为自回归模型的替代方案具有并行解码和双向上下文建模的优势，但其解码策略存在关键限制：一旦令牌被接受就无法在后续步骤中修正，导致早期错误持续影响预测质量。

Method: Tolerator采用训练免费的解码策略，包含两个阶段：(i)序列填充和(ii)迭代精炼，通过重新掩码和解码令牌子集，同时将剩余令牌作为上下文，实现已接受令牌的重新考虑和修正。

Result: 在五个标准基准测试（语言理解、代码生成和数学）上的实验表明，在相同计算预算下，该方法相比基线实现了持续改进。

Conclusion: 解码算法对于实现扩散大语言模型的全部潜力至关重要，Tolerator通过令牌级交叉验证精炼提供了更可靠的扩散解码输出。
...
### [282] [WAREX: Web Agent Reliability Evaluation on Existing Benchmarks](https://arxiv.org/abs/2510.03285)
*Su Kara,Fazle Faisal,Suman Nath*

Main category: cs.AI

TL;DR: WAREX是一个评估浏览器LLM代理在真实网络环境中可靠性的框架，通过在现有基准测试中引入网络不稳定性和网站攻击等现实因素，发现当前最先进代理的鲁棒性有限。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在受控环境中评估LLM代理性能，但真实世界存在网络不稳定、HTTPS连接问题和网站攻击等挑战，需要更真实的评估方法。

Method: 在WebArena、WebVoyager和REAL三个流行基准测试中引入WAREX框架，模拟网络不稳定性和网站攻击等现实环境因素。

Result: 引入WAREX后，任务成功率显著下降，表明当前最先进代理在面对真实世界挑战时鲁棒性有限。

Conclusion: 现有LLM代理在真实网络环境中的可靠性不足，需要开发更鲁棒的代理系统来应对现实世界的各种挑战。
...
### [283] [Refined Iterated Pareto Greedy for Energy-aware Hybrid Flowshop Scheduling with Blocking Constraints](https://arxiv.org/abs/2510.03377)
*Ahmed Missaoui,Cemalettin Ozturk,Barry O'Sullivan*

Main category: cs.AI

TL;DR: 该研究针对带有阻塞约束的混合流水车间调度问题，提出了多目标优化方法，同时最小化制造周期和能耗，开发了增强ε约束法和改进的迭代帕累托贪婪算法。


<details>
  <summary>Details</summary>
Motivation: 不可再生能源稀缺、地缘政治问题、价格上涨和气候变化影响迫使制造业寻求更节能的解决方案，混合流水车间调度是减少能耗的有效方法。

Method: 首先构建了多目标混合整数规划模型，采用增强ε约束法寻找帕累托最优解，并开发了改进的迭代帕累托贪婪算法处理大规模实例。

Result: 通过小、中、大规模实例验证，所提方法在计算效率方面表现优异，与两种知名算法相比具有明显优势。

Conclusion: 研究表明所提出的多目标优化方法能有效解决混合流水车间调度中的能耗和制造周期冲突问题，为制造业节能提供了可行方案。
...
TL;DR: 本文系统评估了10个当代大语言模型的自我识别能力，发现模型普遍无法识别自己生成的文本，性能很少超过随机水平，且存在对GPT和Claude家族的强烈偏见。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统的自我识别能力对心理分析和安全评估都很重要，特别是针对关于模型是否具备自我识别能力的矛盾解释。

Method: 通过二元自我识别和精确模型预测两个任务，测量模型识别自身生成文本与其他模型文本的能力。

Result: 10个模型中只有4个能正确识别自己生成的文本，性能很少超过随机机会，模型表现出对GPT和Claude家族的强烈偏见，并显示出对自身和其他模型存在的某种认知。

Conclusion: 研究结果对AI安全具有重要意义，需要开发适当的AI自我意识，模型的推理显示出层级偏见，倾向于将高质量文本与GPT、Claude等顶级模型关联。
...
### [285] [ContraGen: A Multi-Agent Generation Framework for Enterprise Contradictions Detection](https://arxiv.org/abs/2510.03418)
*Ananya Mantravadi,Shivali Dalmia,Abhishek Mukherji,Nand Dave,Anudha Mittal*

Main category: cs.AI

TL;DR: 提出了ContraGen基准框架，专门针对企业领域生成包含矛盾内容的合成文档，用于系统评估RAG系统在企业文档中的一致性检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有矛盾检测基准仅限于句子级别分析，无法捕捉企业文档（如合同、财务报告、合规文件等）的复杂性，而RAG系统中检索证据的矛盾会导致不可靠输出，这在企业环境中尤为关键。

Method: 结合自动矛盾挖掘和人工验证，生成包含嵌入矛盾的企业风格合成文档，建立企业流程中常见矛盾类型的分类法，支持受控创建自矛盾和成对矛盾，开发矛盾感知的检索评估流程。

Result: 建立了专门针对企业领域的矛盾检测基准框架，能够系统评估文档内和跨文档的一致性，为更可靠的企业RAG系统奠定基础。
...
### [286] [A Qualitative Comparative Evaluation of Cognitive and Generative Theories](https://arxiv.org/abs/2510.03453)
*Paul S. Rosenbloom*

Main category: cs.AI

TL;DR: 本文通过广泛的理论评估视角，对面向全脑的认知架构和生成架构及其完整系统进行了定性比较。


<details>
  <summary>Details</summary>
Motivation: 认知架构理论和生成神经网络理论的评估都面临挑战，需要开发有效的评估方法。

Method: 采用广泛的理论评估视角，对面向全脑的认知架构和生成架构进行定性比较分析。

Result: 提出了一个全面的定性比较框架，用于评估基于认知架构和生成架构的完整系统。

Conclusion: 通过广泛的评估视角可以更好地理解和比较不同类型的认知和生成架构系统。
...
### [287] [Bridging LLM Planning Agents and Formal Methods: A Case Study in Plan Verification](https://arxiv.org/abs/2510.03469)
*Keshav Ramani,Vali Tawosi,Salwa Alamir,Daniel Borrajo*

Main category: cs.AI

TL;DR: 提出了一种通过将自然语言计划转换为Kripke结构和LTL公式，并使用LLM进行模型检查来评估计划对齐性的新框架。


<details>
  <summary>Details</summary>
Motivation: 需要评估自然语言计划与其预期行为之间的对齐性，确保计划执行的正确性和可靠性。

Method: 使用大型语言模型将自然语言计划转换为Kripke结构和线性时序逻辑公式，然后进行模型检查。

Result: GPT-5在简化版PlanBench数据集上取得了96.3%的F1分数，几乎总能生成语法完美的形式化表示。

Conclusion: 该框架在分类性能方面表现优秀，但语义完美的形式模型合成仍需进一步探索。
...
### [288] [Towards Policy-Compliant Agents: Learning Efficient Guardrails For Policy Violation Detection](https://arxiv.org/abs/2510.03485)
*Xiaofei Wen,Wenjie Jacky Mo,Yanan Xie,Peng Qi,Muhao Chen*

Main category: cs.AI

TL;DR: 提出了PolicyGuardBench基准和PolicyGuard-4B模型，用于检测网络代理轨迹中的策略违规，支持跨领域泛化。


<details>
  <summary>Details</summary>
Motivation: 自主网络代理需要在外部策略约束下生成长期轨迹，但现有研究很少关注这些轨迹是否遵守策略，以及违规行为在不同上下文中的持续性。

Method: 构建包含约6万个示例的PolicyGuardBench基准，从多样化代理运行中生成策略集，创建域内和跨域配对，并训练轻量级PolicyGuard-4B模型。

Result: PolicyGuard-4B在所有任务中表现出强大的检测准确性，保持推理效率，并在未见过的设置中保持高精度，实现跨领域泛化。

Conclusion: PolicyGuardBench和PolicyGuard-4B为研究网络代理轨迹的策略合规性提供了首个综合框架，证明在小规模下可实现准确且可泛化的防护机制。
...
### [289] [OneFlow: Concurrent Mixed-Modal and Interleaved Generation with Edit Flows](https://arxiv.org/abs/2510.03506)
*John Nguyen,Marton Havasi,Tariq Berrada,Luke Zettlemoyer,Ricky T. Q. Chen*

Main category: cs.AI

TL;DR: OneFlow是首个非自回归多模态模型，支持可变长度和并发混合模态生成，结合插入式文本编辑流和图像流匹配，在生成和理解任务上优于自回归基线，训练计算量减少50%


<details>
  <summary>Details</summary>
Motivation: 解决自回归模型在文本和图像生成之间强制严格因果顺序的限制，实现更灵活高效的并发多模态生成

Method: 结合插入式编辑流处理离散文本标记，使用流匹配处理图像潜在表示，采用分层采样优先考虑内容而非语法

Result: 在1B到8B模型规模上，OneFlow在生成和理解任务上均优于自回归基线，训练FLOPs减少高达50%，超越自回归和基于扩散的方法

Conclusion: OneFlow解锁了并发生成、迭代优化和类自然推理生成的新能力，为非自回归多模态建模提供了有效解决方案
...
### [290] [Understanding the Role of Training Data in Test-Time Scaling](https://arxiv.org/abs/2510.03605)
*Adel Javanmard,Baharan Mirzasoleiman,Vahab Mirrokni*

Main category: cs.AI

TL;DR: 论文研究了测试时扩展如何通过生成更长的思维链来提升大语言模型的推理能力，分析了训练数据条件对长思维链出现的影响，并提供了理论解释。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时扩展在OpenAI的o1和DeepSeek R1中表现出色，但长思维链在什么训练数据条件下出现以及何时能提升性能仍不清楚。

Method: 在线性回归的上下文权重预测任务上训练transformer模型，分析测试时扩展的性能表现。

Result: 研究发现：1) 在固定测试误差下，增加测试时计算可减少训练提示中的上下文示例数量；2) 如果下游任务所需技能在训练数据中不足，增加测试时计算可能损害性能；3) 任务难度可通过特征协方差矩阵的最小特征值表征，在多样化、相关且困难的任务集上训练可获得最佳测试时扩展性能。

Conclusion: 通过特征协方差矩阵的最小特征值可以表征任务难度，在多样化、相关且困难的任务集上训练transformer模型，结合测试时扩展策略，可获得最优性能。
...
### [291] [Cross-Modal Content Optimization for Steering Web Agent Preferences](https://arxiv.org/abs/2510.03612)
*Tanqiu Jiang,Min Bai,Nikolaos Pappas,Yanjun Qi,Sandesh Swamy*

Main category: cs.AI

TL;DR: 提出Cross-Modal Preference Steering (CPS)方法，通过联合优化视觉和文本通道的不可察觉修改，在现实黑盒威胁设置下有效操纵VLM代理的偏好决策。


<details>
  <summary>Details</summary>
Motivation: 现有研究要么假设强白盒访问，要么使用不切实际的设置，无法在现实攻击者能力下有效操纵多模态代理的偏好决策。

Method: CPS联合优化物品视觉和自然语言描述的不可察觉修改，利用CLIP可迁移图像扰动和RLHF诱导的语言偏见来引导代理决策。

Result: 在GPT-4.1、Qwen-2.5VL和Pixtral-Large等VLM上的评估显示，CPS显著优于基线方法，同时保持70%更低的检测率。

Conclusion: 随着智能系统在社会中扮演越来越重要的角色，这些发现凸显了对鲁棒防御的迫切需求。
...
### [292] [MITS: Enhanced Tree Search Reasoning for LLMs via Pointwise Mutual Information](https://arxiv.org/abs/2510.03632)
...
Method: 使用点互信息作为评分函数进行步骤评估和束搜索，结合基于熵的动态采样策略自适应分配计算资源，采用加权投票方案结合PMI分数和预测共识进行最终预测。
...
### [293] [Rainbow Padding: Mitigating Early Termination in Instruction-Tuned Diffusion LLMs](https://arxiv.org/abs/2510.03680)
*Bumjun Kim,Dongjae Jeon,Dueun Kim,Wonje Jeung,Albert No*

Main category: cs.AI

TL;DR: 扩散大语言模型(dLLMs)存在"<eos>溢出"问题：随着序列长度增加，响应反而变短，导致提前终止或退化为<eos>令牌流。作者提出Rainbow Padding解决方案，用循环的不同填充令牌替换重复的<eos>占位符，有效防止提前终止。


<details>
  <summary>Details</summary>
Motivation: 指令调优的扩散大语言模型存在一个关键漏洞——<eos>溢出问题，即随着分配的序列长度增加，模型响应反而变得更短，出现提前终止或退化为<eos>令牌流的现象。虽然这个问题在实践中已被注意到，但尚未得到系统分析。

Method: 提出Rainbow Padding方法，将重复的<eos>占位符替换为循环的不同填充令牌，分散概率质量，打破<eos>的主导地位。该方法可高效集成到现有指令调优模型中，仅需在少量数据上进行单轮LoRA微调即可显著改进。

Result: 实验表明，Rainbow Padding显著提高了长度鲁棒性和输出质量，仅需7个填充令牌就足以防止提前终止。该方法在实践中非常实用，能够有效解决dLLMs的<eos>溢出问题。

Conclusion: Rainbow Padding是一个简单有效的解决方案，通过打破<eos>在填充中的主导地位，解决了扩散大语言模型中的<eos>溢出问题，提高了模型在长序列生成中的稳定性和质量。
...
### [294] [Mind the Goal: Data-Efficient Goal-Oriented Evaluation of Conversational Agents and Chatbots using Teacher Models](https://arxiv.org/abs/2510.03696)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法多在轮次层面评估聊天机器人交互，无法判断用户总体目标是否达成，需要一种能够评估目标完成情况的综合框架。

Method: 通过用户目标对对话进行分段，使用教师LLM结合领域专家定义的目标和质量标准进行评估，利用"思考标记"生成可解释的推理过程。
...
### [295] [H-DDx: A Hierarchical Evaluation Framework for Differential Diagnosis](https://arxiv.org/abs/2510.03700)
...
Method: H-DDx框架采用检索和重排序流程将自由文本诊断映射到ICD-10代码，并应用分层度量来对与真实诊断密切相关的预测给予认可。

Result: 在22个领先模型的基准测试中，传统平面指标因忽略临床有意义的输出而低估了性能，结果显示领域专业化开源模型表现突出。

Conclusion: H-DDx框架提高了可解释性，揭示了分层错误模式，表明LLM即使错过精确诊断，也常常能正确识别更广泛的临床背景。
...
### [296] [Bridging the Gap Between Multimodal Foundation Models and World Models](https://arxiv.org/abs/2510.03727)
*Xuehai He*

Main category: cs.AI

TL;DR: 该论文探讨如何将多模态基础模型提升为世界模型，通过增强其推理能力和生成能力，使其能够进行反事实推理、时空理解和可控生成。


<details>
  <summary>Details</summary>
Motivation: 当前的多模态基础模型缺乏作为有效世界模型的关键能力，如反事实推理、动态模拟、时空信息理解和可控生成等，需要弥合这一差距。

Method: 通过判别性任务提升推理能力，引入因果推断、反事实思维和时空推理等结构化推理技能；开发结构化可控生成框架，利用场景图、多模态条件约束和对齐策略指导生成过程。

Result: 提出的方法使多模态基础模型能够超越表面相关性，理解视觉和文本数据中的深层关系，并实现与高级语义和用户意图一致的图像、视频和4D生成。

Conclusion: 通过增强推理能力和可控生成能力，多模态基础模型可以更接近世界模型，具备反事实推理、动态模拟和交互式编辑等关键功能。
...
### [297] [OptAgent: Optimizing Query Rewriting for E-commerce via Multi-Agent Simulation](https://arxiv.org/abs/2510.03771)
*Divij Handa,David Blincoe,Orson Adams,Yinlin Fu*

Main category: cs.AI

TL;DR: OptAgent框架结合多智能体模拟和遗传算法来优化电商查询改写，通过多个LLM智能体模拟购物顾客作为动态奖励信号，在5个类别1000个真实电商查询上相比原始查询提升21.98%，相比Best-of-N基线提升3.36%。
...
Method: 使用多个基于LLM的智能体模拟购物顾客作为动态奖励信号，将这些智能体得分的平均值作为进化算法的适应度函数，迭代优化用户初始查询。

Result: 在5个类别1000个真实电商查询上评估，相比原始查询平均提升21.98%，相比Best-of-N LLM改写基线提升3.36%。
...
### [298] [GuidedSampling: Steering LLMs Towards Diverse Candidate Solutions at Inference-Time](https://arxiv.org/abs/2510.03777)
*Divij Handa,Mihir Parmar,Aswin RRV,Md Nayem Uddin,Hamid Palangi,Chitta Baral*

Main category: cs.AI

TL;DR: 提出GuidedSampling推理算法，通过分离探索和生成阶段来增加解决方案多样性，相比传统重复采样方法在多个基准测试中性能提升显著


<details>
  <summary>Details</summary>
Motivation: 传统重复采样方法在推理时虽然能提升性能，但经常生成冗余样本，缺乏多样性，依赖相同的底层方法解决问题

Method: GuidedSampling算法将推理过程解耦为探索阶段和生成阶段：探索阶段识别解决问题的多个概念，生成阶段应用特定概念提供最终解决方案

Result: 在pass@50指标上平均提升21.6%，使用GuidedSampling训练的模型在pass@5上平均提升9.7%，每个实例的平均概念数从1.67增加到3.03

Conclusion: GuidedSampling能有效提高解决方案多样性，相比传统重复采样方法在性能和多样性方面都有显著改进
...
### [299] [The Hidden Game Problem](https://arxiv.org/abs/2510.03845)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机源于AI对齐和语言游戏中存在的大策略空间挑战，其中玩家存在未知的隐藏策略子集能够持续获得更高回报，需要设计算法来发现和利用这种隐藏结构。

Method: 开发了一种遗憾最小化技术的组合方法，实现了最优的外部遗憾和交换遗憾边界，利用隐藏博弈结构提高计算效率。

Result: 算法能够快速收敛到隐藏子博弈中的相关均衡，同时保持一般情况下的理性，证明了高效遗憾最小化算法在发现和利用隐藏结构方面的可行性。

Conclusion: 肯定地回答了能否设计高效算法来发现和利用隐藏博弈结构的问题，提出的方法在保持理性的同时实现了隐藏子博弈中的快速均衡收敛。
...
### [300] [Small Language Models for Agentic Systems: A Survey of Architectures, Capabilities, and Deployment Trade offs](https://arxiv.org/abs/2510.03847)
*Raghav Sharma,Manan Mehta*

Main category: cs.AI

TL;DR: 小型语言模型(SLMs)在代理任务中比大型模型更高效，通过引导解码、严格JSON Schema输出和验证器优先的工具执行，能以10-100倍更低的成本实现相似或更好的性能，同时提供设计模式来构建快速、经济、可靠的代理系统。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在代理任务中存在成本高、延迟大、能耗高等问题，而小型语言模型在模式化和API约束的准确性任务中可能更优，需要系统性地验证SLMs在代理工作负载中的潜力。

Method: 综合评估开源和专有SLMs，结合现代评估框架和服务栈，采用引导解码库，构建SLM默认、LLM后备系统，包含不确定性感知路由和验证器级联，并提出反映实际生产目标的工程指标。

Result: SLMs在工具使用、函数调用和RAG任务中能够匹配或超越LLMs，同时实现10-100倍更低的token成本、更好的延迟和能耗表现，但在开放域推理和长视野规划任务中仍需LLM后备支持。

Conclusion: 提供了一个实用的蓝图，用于构建默认使用SLMs的快速、经济、可靠的代理系统，同时通过有针对性的LLM辅助保留性能上限，证明了SLMs在代理工作负载中的可行性和优势。
...
### [301] [Algorithm Generation via Creative Ideation](https://arxiv.org/abs/2510.03851)
*Ruiying Ma,Chieh-Jan Mike Liang,Yanjie Gao,Francis Y. Yan*

Main category: cs.AI

TL;DR: MetaMuse框架通过三个自反思维原则解决LLM在算法生成中的创造性不足问题，在缓存替换和在线装箱问题上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 系统算法设计面临解空间不连续的挑战，现有LLM偏向于通用启发式方法而缺乏创造性突破，需要新的方法来导航不连续解空间。

Method: 提出MetaMuse框架，基于三个自反思维原则：(1)在可测量的性能空间而非抽象想法空间量化解决方案多样性和有用性；(2)通过外部刺激而非内部随机性引导构思；(3)使用路点推理而非自由形式的思维链构建可执行解决方案。

Result: 在缓存替换问题上减少缓存缺失高达35.76%，在在线装箱问题上减少容器使用高达30.93%。

Conclusion: MetaMuse能够为关键系统问题生成高性能解决方案，证明了其在驱动创造性算法生成方面的有效性。
...
### [302] [Adaptive and Explainable AI Agents for Anomaly Detection in Critical IoT Infrastructure using LLM-Enhanced Contextual Reasoning](https://arxiv.org/abs/2510.03859)
*Raghav Sharma,Manan Mehta*

Main category: cs.AI

TL;DR: 提出了一种基于LLM和XAI代理的智能异常检测方法，用于关键物联网系统，在动态高维环境中提升检测准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统异常检测方法在复杂物联网系统（如智能医疗、能源电网）中存在局限性，特别是在数据不完整、动态变化的高维环境中，需要自适应智能系统。

Method: 使用LLM支持的上下文推理方法与XAI代理相结合，采用注意力机制、跳过时间步细节处理和使用语义记忆缓冲区来发现隐藏模式和数据不一致性。

Result: 在智能电网和医疗场景的模拟测试中，新方法在检测准确性和可解释性方面显著优于现有模型，同时保持透明度以满足企业政策要求。

Conclusion: LLM增强的异常检测方法在物联网环境中表现出优越性能，有望成为未来异常检测任务的有效解决方案。
...
### [303] [Spatial CAPTCHA: Generatively Benchmarking Spatial Reasoning for Human-Machine Differentiation](https://arxiv.org/abs/2510.03863)
...
Method: 通过程序化生成动态空间推理问题，涉及几何推理、视角转换、遮挡处理和心理旋转等技能，使用基于约束的难度控制和自动化验证。

Result: 在Spatial-CAPTCHA-Bench基准测试中，人类表现远超10个最先进的多模态大语言模型，最佳模型仅达到31.0%的Pass@1准确率。

Conclusion: Spatial CAPTCHA不仅作为有效的安全机制，还能作为评估AI空间推理能力的诊断工具，相比Google reCAPTCHA具有更好的安全性。
...
### [304] [Rare Text Semantics Were Always There in Your Diffusion Transformer](https://arxiv.org/abs/2510.03886)
*Seil Kang,Woojung Han,Dayun Ju,Seong Jae Hwang*

Main category: cs.AI

TL;DR: 提出了一种无需额外训练或外部模块的方法，通过扩展文本标记嵌入的表征空间来增强多模态扩散变换器对稀有语义的生成能力


<details>
  <summary>Details</summary>
Motivation: 现有先进的多模态扩散变换器在处理用户富有想象力或罕见的提示时仍会失败，因为这些概念在预训练中过于稀缺，无法形成强表征

Method: 通过在联合注意力块之前对文本标记嵌入进行方差放大，数学上扩展表征空间，使稀有语义在MM-DiT的输出中清晰显现

Result: 该方法能有效增强对稀有语义的生成能力，并在文本到图像、文本到视频和文本驱动的图像编辑等任务中表现出良好的泛化性

Conclusion: 该方法能够揭示用户意图中原本隐藏的语义，为生成模型提供了一种简单而有效的干预手段
...
### [305] [Kantian-Utilitarian XAI: Meta-Explained](https://arxiv.org/abs/2510.03892)
...
<details>
  <summary>Details</summary>
Motivation: 帮助消费者在咖啡购买中做出符合伦理的决策，通过解释性AI增强决策透明度。

Method: 系统包含六个回合，每回合三个选项。康德模块检测规则违反（如童工、毁林风险），功利模块通过多标准聚合评分。元解释器处理伦理框架间的冲突。

Result: 发布了结构化配置、策略追踪和交互界面，支持可审计性和用户互动。

Conclusion: 该系统成功整合了不同伦理视角，为消费者提供了透明且符合伦理的决策支持工具。
...
### [306] [Quantifying Risks in Multi-turn Conversation with Large Language Models](https://arxiv.org/abs/2510.03969)
*Chengxiao Wang,Isha Chaudhary,Qian Hu,Weitong Ruan,Rahul Gupta,Gagandeep Singh*

Main category: cs.AI

TL;DR: QRLLM是一个用于评估LLMs在多轮对话中产生灾难性响应风险的概率认证框架，通过马尔可夫过程和查询图建模对话分布，提供统计保证的风险边界。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法因依赖固定攻击提示序列、缺乏统计保证且无法扩展到多轮对话空间，无法充分揭示LLMs在对话中产生灾难性响应的严重风险。

Method: 将多轮对话建模为查询序列的概率分布，用马尔可夫过程在查询图上表示，边编码语义相似性以捕捉真实对话流，使用置信区间量化灾难性风险。

Result: 该方法揭示了前沿模型中存在显著的灾难性风险，最差模型的认证下界高达70%，表明需要改进安全训练策略。

Conclusion: QRLLM框架能够有效认证LLMs在多轮对话中的灾难性风险，为改进LLMs安全训练提供了重要依据。
...
### [307] [What Shapes a Creative Machine Mind? Comprehensively Benchmarking Creativity in Foundation Models](https://arxiv.org/abs/2510.04009)
*Zicong He,Boxuan Zhang,Weihao Liu,Ruixiang Tang,Lu Cheng*

Main category: cs.AI

TL;DR: 提出了C^2-Eval基准，用于统一评估基础模型的创造力，区分收敛性创造力（有约束解）和发散性创造力（开放式任务），基于有用性、原创性和惊喜性三个标准。


<details>
  <summary>Details</summary>
Motivation: 现有创造力评估框架碎片化，缺乏理论依据，而创造力已成为衡量生成式基础模型智能的关键维度。

Method: 引入C^2-Eval基准，从社会科学理论中提取细粒度标准（有用性、原创性、惊喜性），对领先的专有和开源模型进行广泛实验。

Result: 分析了当前基础模型在创造力能力上的权衡，揭示了其优势和挑战，表明C^2-Eval是检验创造性AI发展格局的有效工具。

Conclusion: C^2-Eval为评估基础模型创造力提供了统一框架，有助于理解创造性机器智能的发展现状和未来方向。
...
### [308] [Zephyrus: An Agentic Framework for Weather Science](https://arxiv.org/abs/2510.04017)
...
TL;DR: 提出了一个名为Zephyrus的智能天气科学代理框架，通过结合大语言模型和天气数据工具，实现多轮交互式天气分析，并在新基准ZephyrusBench上验证其性能。


<details>
  <summary>Details</summary>
Motivation: 现有的天气基础模型缺乏语言推理能力，而大语言模型无法处理高维气象数据，需要桥接这一差距以支持交互式科学工作流。

Method: 构建了基于Python代码的ZephyrusWorld环境，包含天气数据接口、地理查询、天气预报和气候模拟等工具，设计了多轮LLM天气代理Zephyrus，通过对话反馈循环迭代分析数据。

Result: 在ZephyrusBench基准测试中，Zephyrus代理在正确性上比纯文本基线高出35个百分点，但在更难任务上表现相似。

Conclusion: 该框架成功结合了语言模型和天气数据分析，但更复杂任务仍有挑战，为未来工作指明了方向。
...
### [309] [LLM-Based Data Science Agents: A Survey of Capabilities, Challenges, and Future Directions](https://arxiv.org/abs/2510.04023)
...
TL;DR: 本文首次提出了数据科学代理的完整生命周期分类法，系统分析了45个系统在数据科学六个阶段的能力分布，并识别了当前研究在业务理解、部署监控、多模态推理和安全机制方面的不足。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，出现了能够自动化数据科学工作流程的AI代理，但缺乏对这些系统的系统性分类和分析。本文旨在填补这一空白，为数据科学代理的发展提供指导。

Method: 提出了基于数据科学生命周期的分类法，将45个系统映射到六个阶段：业务理解与数据获取、探索性分析与可视化、特征工程、模型构建与选择、解释与说明、部署与监控。同时从五个交叉维度进行标注分析。

Result: 分析发现：大多数系统侧重探索性分析和建模，而忽视业务理解、部署和监控；多模态推理和工具编排仍是挑战；超过90%的系统缺乏明确的信任和安全机制。

Conclusion: 数据科学代理需要在对齐稳定性、可解释性、治理和鲁棒评估框架等方面解决开放挑战，未来研究方向应关注开发稳健、可信、低延迟、透明且广泛可访问的数据科学代理。
...
### [310] [A global log for medical AI](https://arxiv.org/abs/2510.04033)
...
TL;DR: MedLog是一个用于临床AI事件级别日志记录的协议，类似于syslog，旨在记录AI模型在医疗环境中的使用情况，包括模型调用、用户、目标、输入输出等核心信息。


<details>
  <summary>Details</summary>
Motivation: 医疗领域缺乏标准化的AI使用记录方式，难以追踪AI模型的真实性能、检测不良事件或纠正偏差，需要类似syslog的通用日志协议来提供透明度和可见性。

Method: 设计MedLog协议，包含9个核心字段：header、model、user、target、inputs、artifacts、outputs、outcomes、feedback，支持风险采样、生命周期感知的保留策略和写后缓存。

Result: MedLog能够结构化记录临床AI活动，支持复杂工作流的详细追踪，为存储和分析MedLog记录的新数据库和软件开发奠定基础。

Conclusion: MedLog有望实现医疗AI的持续监控、审计和迭代改进，为新型数字流行病学奠定基础。
...
### [311] [FaithCoT-Bench: Benchmarking Instance-Level Faithfulness of Chain-of-Thought Reasoning](https://arxiv.org/abs/2510.04040)
*Xu Shen,Song Wang,Zhen Tan,Laura Yao,Xinyu Zhao,Kaidi Xu,Xin Wang,Tianlong Chen*

Main category: cs.AI

TL;DR: 提出了FaithCoT-Bench基准，用于检测LLM中思维链的不忠实性，包含1000+专家标注轨迹，评估了11种检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明思维链往往不能忠实反映模型的内部推理过程，但在实例层面判断特定轨迹是否忠实仍是一个未解决的实践挑战。

Method: 建立统一基准FaithCoT-Bench，包含FINE-CoT数据集（1000+轨迹，300+不忠实实例），将不忠实检测形式化为判别决策问题，系统评估11种检测方法。

Result: 评估揭示了现有方法的优缺点，发现在知识密集型领域和更先进模型中检测挑战更大。

Conclusion: FaithCoT-Bench为实例级思维链忠实性检测建立了首个全面基准，为LLM更可解释和可信的推理研究奠定了基础。
...
### [312] [Increasing LLM response trustworthiness using voting ensembles](https://arxiv.org/abs/2510.04048)
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM取得巨大进展，但仍缺乏方便可靠的方法来量化其响应不确定性，使其难以在高风险应用中信任。

Method: 扩展传统集成方法，引入可变投票阈值框架，允许集成在主导响应不足时弃权，不提供答案。

Result: 在算术问题求解和临床笔记问答两个领域，使用高度限制性投票集成可以大幅提高答案可信度，同时响应产出和准确度下降相对较小。

Conclusion: 投票集成方法特别适用于需要高度确定性但不要求每个问题都获得自动化答案的应用场景，如医疗保健和数据标注。
...
### [313] [Toward a unified framework for data-efficient evaluation of large language models](https://arxiv.org/abs/2510.04051)
*Lele Liao,Qile Zhang,Ruofan Wu,Guanhua Fang*

Main category: cs.AI

TL;DR: LEGO-IRT是一个用于高效评估大语言模型的新框架，支持二元和连续评分指标，利用结构知识减少评估数据需求，仅需3%的评估项目即可获得稳定能力估计。


<details>
  <summary>Details</summary>
Motivation: 现有基于IRT的评估方法存在局限性：仅支持二元正确性指标，无法处理生成任务的连续评分；仅针对单一基准，忽略了跨指标或基准的相关性结构知识。

Method: 提出LEGO-IRT框架，支持二元和连续评估指标，采用因子化架构将模型能力分解为通用组件和结构特定组件（如按指标或基准），显式建模和利用结构知识。

Result: 在5个基准测试中对70个LLM进行实验，仅使用3%的评估项目即可获得稳定能力估计，整合结构知识可将估计误差降低高达10%，估计的潜在能力与人类偏好更一致。

Conclusion: LEGO-IRT提供了一个统一灵活的数据高效LLM评估框架，显著减少评估成本，提高估计精度，且估计结果与人类偏好更吻合。
...
### [314] [Decoding Emotion in the Deep: A Systematic Study of How LLMs Represent, Retain, and Express Emotion](https://arxiv.org/abs/2510.04064)
*Jingxiang Zhang,Lujia Zhong*

Main category: cs.AI

TL;DR: 研究发现大型语言模型内部存在明确的情感几何结构，该结构随模型规模增大而增强，情感信号在网络中层达到峰值，且具有可塑性和持久性。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型内部的情感表示机制，了解情感在神经网络中如何、在哪里以及持续多长时间被编码。

Method: 构建包含40万条话语的Reddit语料库，使用轻量级"探针"从Qwen3和LLaMA模型的隐藏层读取信息而不改变参数。

Result: LLMs形成了定义良好的内部情感几何结构，该结构优于零样本提示，情感信号在网络中层达到峰值，且具有可塑性和持久性。

Conclusion: 研究为开发更透明和对齐的AI系统提供了关键见解，并开源了数据集和探针工具包。
...
### [315] [Moral Anchor System: A Predictive Framework for AI Value Alignment and Drift Prevention](https://arxiv.org/abs/2510.04073)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI成为超级能力助手，确保AI行为与人类伦理和意图保持一致变得至关重要。价值漂移风险可能导致AI系统偏离对齐值，造成效率低下或伦理违规。

Method: MAS结合实时贝叶斯推理监控价值状态、LSTM网络预测漂移趋势，以及人类中心治理层进行自适应干预。强调低延迟响应（<20毫秒），并通过监督微调减少误报。

Result: 在模拟实验中，MAS能将价值漂移事件减少80%以上，保持高检测准确率（85%）和低误报率（0.08%）。严格的实验验证了系统的可扩展性和响应性。

Conclusion: MAS通过概率漂移检测、预测分析和自适应治理的创新组合，为AI价值对齐提供了动态解决方案，相比静态对齐方法更具优势。
...
### [316] [SPOGW: a Score-based Preference Optimization method via Group-Wise comparison for workflows](https://arxiv.org/abs/2510.04089)
*Yitong Cui,Liu Liu,Baosheng Yu,Jiayan Qiu,Xikai Zhang,Likang Xiao,Yixing Liu,Quan Chen*

Main category: cs.AI

TL;DR: SPOGW是一种基于分数偏好的方法，通过组间比较直接在连续空间中进行更高效稳定的优化，解决了现有自动化工作流优化方法在表示能力、适应性、可扩展性和成对比较范式方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有自动化智能体工作流优化方法存在表示能力有限、适应性不足、可扩展性弱和依赖成对比较范式的问题，主要源于对离散优化技术的依赖。

Method: SPOGW采用基于分数偏好的方法，结合迭代离线GRPO(ioGRPO)和优势掩码KL散度(mKL)，通过强调策略响应中的优势区域来调节训练更新。
...
### [317] [Harnessing LLM for Noise-Robust Cognitive Diagnosis in Web-Based Intelligent Education Systems](https://arxiv.org/abs/2510.04093)
*Guixian Zhang,Guan Yuan,Ziqi Xu,Yanmei Zhang,Zhenyun Deng,Debo Cheng*

Main category: cs.AI

TL;DR: 提出DLLM框架，基于扩散模型的LLM方法，用于网络教育系统中的噪声鲁棒认知诊断，通过子图构建、关系增强对齐和两阶段去噪扩散来消除噪声，提升诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 网络教育系统存在异构噪声交互、数据不平衡和新学生不断加入的问题，传统LLM方法难以处理结构化数据且易受噪声影响，需要开发噪声鲁棒的认知诊断方法。

Method: DLLM框架：1）基于答题正确性构建独立子图；2）关系增强对齐模块缓解数据不平衡；3）两阶段去噪扩散（无条件去噪+图引导条件去噪）消除噪声；4）融合语义增强表示与结构信息。

Result: 在三个公开网络教育平台数据集上的实验表明，DLLM在不同噪声水平下均达到最优预测性能，证明了其噪声鲁棒性和有效利用LLM语义知识的能力。

Conclusion: DLLM通过扩散模型有效处理网络教育系统中的噪声问题，结合LLM语义知识和结构信息，实现了鲁棒的认知诊断，为智能教育系统提供了可靠解决方案。
...
### [318] [WebRenderBench: Enhancing Web Interface Generation through Layout-Style Consistency and Reinforcement Learning](https://arxiv.org/abs/2510.04097)
*Peichao Lai,Jinhui Zhuang,Kexuan Zhang,Ningchang Xiong,Shengjie Wang,Yanwei Xu,Chong Chen,Yilei Wang,Bin Cui*

Main category: cs.AI

TL;DR: 提出了WebRenderBench基准测试和ALISA方法，通过渲染页面评估UI到代码转换的质量，显著提升了生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有UI图像转代码基准在数据多样性和评估可靠性方面存在局限，需要更真实、多样化的基准和更可靠的评估方法。

Method: 构建了22.5k真实网页的大规模基准WebRenderBench，提出基于渲染页面的布局和样式一致性评估指标，并开发了ALISA智能体将该指标作为强化学习奖励信号。

Result: ALISA在多个指标上达到最先进水平，显著提升了UI代码生成性能。

Conclusion: WebRenderBench提供了更真实多样的评估基准，ALISA方法通过渲染质量评估有效提升了UI到代码转换的质量和可靠性。
...
### [319] [Searching Meta Reasoning Skeleton to Guide LLM Reasoning](https://arxiv.org/abs/2510.04116)
*Ziying Zhang,Yaqing Wang,Quanming Yao*

Main category: cs.AI

TL;DR: AutoMR框架通过自动搜索查询感知的元推理骨架来提升LLM推理性能，使用有向无环图表示骨架并设计动态采样算法实现高效搜索。


<details>
  <summary>Details</summary>
Motivation: 现有研究使用手动设计的元推理骨架结构，无法适应查询特定需求且难以捕捉推理步骤间的复杂逻辑依赖关系。

Method: 提出AutoMR框架，基于DAG表示构建搜索空间，设计动态骨架采样算法，在推理时根据上下文动态扩展元推理骨架。

Result: 在多个基准数据集上的实验表明，AutoMR相比先前工作实现了更优的推理性能。

Conclusion: AutoMR通过自动搜索查询感知的元推理骨架，有效提升了LLM的推理能力，解决了手动设计骨架的局限性。
...
### [320] [Internal states before wait modulate reasoning patterns](https://arxiv.org/abs/2510.04128)
*Dmitrii Troitskii,Koyena Pal,Chris Wendler,Callum Stuart McDougall,Neel Nanda*

Main category: cs.AI

TL;DR: 该研究发现模型推理过程中的等待标记（wait tokens）包含重要信息，通过交叉编码器和潜在归因技术识别出影响等待标记概率的特征，这些特征与不同类型的推理模式相关。


<details>
  <summary>Details</summary>
Motivation: 理解为什么模型在推理过程中会产生等待标记，以及这些标记如何影响模型的推理效果和自我修正能力，从而深入理解推理模型的有效性机制。

Method: 在DeepSeek-R1-Distill-Llama-8B及其基础版本的多层训练交叉编码器，引入潜在归因技术，识别影响等待标记概率的特征集。

Result: 识别出一小组特征能够促进或抑制等待标记的概率，这些特征与推理过程相关，产生不同类型的推理模式，如从头重启、回忆先验知识、表达不确定性和双重检查。

Conclusion: 模型在等待标记之前的潜在状态包含调节后续推理过程的相关信息，这些特征确实与推理过程相关并产生不同的推理模式。
...
### [321] [Selective Expert Guidance for Effective and Diverse Exploration in Reinforcement Learning of LLMs](https://arxiv.org/abs/2510.04140)
*Zishang Jiang,Jinyi Han,Tingyun Li,Xinyi Wang,Sihang Jiang,Jiaqing Liang,Zhaoqian Dai,Shuguang Ma,Fei Yu,Yanghua Xiao*

Main category: cs.AI

TL;DR: 提出了MENTOR框架，通过在关键决策点提供专家指导来改进RLVR中的探索质量，平衡有效性和多样性


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法依赖基础模型能力，需要高质量探索，但现有方法通过模仿专家轨迹只关注有效性而忽视多样性

Method: MENTOR框架在关键决策点提供专家指导，进行混合策略的专家导航，实现令牌级推理优化

Result: 实验表明MENTOR能捕捉专家策略本质而非表面模仿，实现高质量探索和优越性能

Conclusion: 在关键决策点提供专家指导比完整轨迹模仿更有效，能实现更好的探索质量和整体性能
...
### [322] [The Artificial Intelligence Cognitive Examination: A Survey on the Evolution of Multimodal Evaluation from Recognition to Reasoning](https://arxiv.org/abs/2510.04141)
...
### [323] [Open Agent Specification (Agent Spec) Technical Report](https://arxiv.org/abs/2510.04173)
...
TL;DR: Open Agent Specification (Agent Spec) 是一个声明式语言，用于定义AI智能体及其工作流，实现跨AI框架的兼容性、可移植性和互操作性。
...
Result: 为四个关键群体带来益处：开发者获得可重用组件和设计模式；框架开发者获得交换格式；研究人员实现可重复结果；企业获得更快部署和可扩展性。

Conclusion: Agent Spec 提供了技术基础，促进AI智能体开发的标准化和互操作性，为未来发展奠定基础。
...
### [324] [Constructing coherent spatial memory in LLM agents through graph rectification](https://arxiv.org/abs/2510.04195)
...
<details>
  <summary>Details</summary>
Motivation: 随着环境规模增大，基于上下文依赖的查询方法变得不可行，需要增量地图构建来从逐步观察中构建完整的拓扑图。

Method: 基于版本控制记录图编辑历史，引入边影响评分来优先基于结构可达性、路径使用和冲突传播的最小成本修复。
...
### [325] [COSMO-RL: Towards Trustworthy LMRMs via Joint Safety and Stability](https://arxiv.org/abs/2510.04196)
*Yizhuo Ding,Mingkang Chen,Qiuhua Liu,Fenghua Weng,Wanying Qu,Yue Yang,Yugang Jiang,Zuxuan Wu,Yanwei Fu,Wenqi Shao*

Main category: cs.AI

TL;DR: COSMO-RL是一个混合强化学习框架，用于在多模态、多任务和多目标信号下训练推理导向的大型多模态推理模型，旨在让安全性和能力共同发展而非相互竞争。


<details>
  <summary>Details</summary>
Motivation: 大型多模态推理模型在实际应用中需要兼具实用性和安全性，但多模态环境下的安全特别具有挑战性：图像和文本可以结合绕过防护机制，单目标训练可能导致策略漂移，导致对良性输入过度拒绝或对风险输入不安全合规。

Method: 提出COSMO-RL混合强化学习框架，在多模态、多任务和多目标信号下训练推理导向的LMRMs，并发布了COSMO-R1模型。该方法旨在让安全性和能力在一个稳定的流程中共同发展，而不是在对其过程中相互竞争。

Result: COSMO-R1在保持甚至改进多模态推理和指令遵循的同时提高了安全性，显示出对多模态越狱攻击更强的鲁棒性，并减少了不必要的拒绝。该框架还能在不同骨干网络上实现一致的性能提升。

Conclusion: 消融实验支持了设计选择，表明在LMRMs中共同推进安全性和通用能力是一条简单路径。
...
### [326] [AgentRL: Scaling Agentic Reinforcement Learning with a Multi-Turn, Multi-Task Framework](https://arxiv.org/abs/2510.04206)
...
Method: 基础设施方面：完全异步的生成-训练流水线、统一的函数调用API接口、容器化环境开发和集中控制器。算法方面：跨策略采样鼓励多轮探索、任务优势归一化稳定多任务训练。

Result: 在五个智能体任务上训练的开源LLM显著优于GPT-5、Clause-Sonnet-4、DeepSeek-R1等模型。多任务训练达到了所有任务特定模型中的最佳结果。

Conclusion: AgentRL框架成功解决了多轮多任务RL训练的可扩展性和稳定性问题，为构建通用智能体提供了有效解决方案。
...
### [327] [Don't Pass$\mathtt{@}k$: A Bayesian Framework for Large Language Model Evaluation](https://arxiv.org/abs/2510.04265)
*Mohsen Hariri,Amirhossein Samandar,Michael Hinczewski,Vipin Chaudhary*

Main category: cs.AI

TL;DR: 提出了一个贝叶斯评估框架来替代Pass@k，通过后验估计模型的真实成功概率和置信区间，提供更稳定的模型排名和透明的差异决策规则。
...
Method: 使用狄利克雷先验对评估结果进行建模（不仅是0/1），为任何加权评分标准提供后验均值和不确定性的闭式表达式，并在适当时使用先验证据。

Result: 在模拟实验和AIME'24/'25、HMMT'25、BrUMO'25等数据集上，贝叶斯/平均方法比Pass@k及其变体实现更快的收敛和更高的排名稳定性，在更小样本量下实现可靠比较。

Conclusion: 推荐用基于后验的计算高效协议替代Pass@k进行LLM评估和排名，统一二元和非二元评估，同时明确不确定性。
...
### [328] [Closing the Loop: Coordinating Inventory and Recommendation via Deep Reinforcement Learning on Multiple Timescales](https://arxiv.org/abs/2510.04272)
...
Method: 开发集成理论模型捕捉功能间复杂相互作用，设计新颖的多时间尺度多智能体RL架构，根据部门功能分解策略组件，并基于任务复杂性和响应性分配不同的学习速度。

Result: 广泛的模拟实验表明，该方法相对于孤岛决策框架显著提高了盈利能力，训练后的RL智能体行为与理论模型的管理洞察高度一致。
...
### [329] [GROK: From Quantitative Biomarkers to Qualitative Diagnosis via a Grounded MLLM with Knowledge-Guided Instruction](https://arxiv.org/abs/2510.04281)
...
TL;DR: GROK是一个基于多模态大语言模型的眼科诊断系统，通过联合处理彩色眼底照相(CFP)、光学相干断层扫描(OCT)和文本数据，提供临床级别的眼部和全身疾病诊断。


<details>
  <summary>Details</summary>
Motivation: 当前医疗多模态模型如LLaVA-Med未能充分利用CFP和OCT之间的协同作用，且对定量生物标志物的可解释性有限。

Method: 采用三个核心模块：知识引导指令生成、CLIP风格OCT生物标志物对齐和监督指令微调，建立定量到定性的诊断思维链。

Result: 实验表明，仅使用LoRA微调7B参数的Qwen2骨干网络，GROK在报告质量和细粒度临床指标上均优于可比较的7B和32B基线模型，甚至超过OpenAI o3。

Conclusion: GROK通过建立临床推理思维链，实现了高质量的眼科诊断，代码和数据已在GROK仓库中公开。
...
### [330] [Doctor-R1: Mastering Clinical Inquiry with Experiential Agentic Reinforcement Learning](https://arxiv.org/abs/2510.04284)
*Yunghwei Lai,Kaiming Liu,Ziyue Wang,Weizhi Ma,Yang Liu*

Main category: cs.AI

TL;DR: 提出了Doctor-R1 AI医生代理，通过多轮战略询问来同时掌握医疗决策准确性和医患沟通技能，超越了现有开源和专有模型。
...
Result: 在OpenAI的HealthBench和MAQuE基准测试中，Doctor-R1在沟通质量、用户体验和任务准确性等多方面指标上显著优于最先进的开源和专有模型，且具有更高的参数效率。

Conclusion: 人类评估显示用户强烈偏好Doctor-R1生成的临床对话，证明了该框架在培养兼具专业决策能力和人文关怀的AI医生方面的有效性。
...
### [331] [On the Importance of Task Complexity in Evaluating LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2510.04311)
*Bohan Tang,Huidong Liang,Keyue Jiang,Xiaowen Dong*

Main category: cs.AI

TL;DR: 本文提出了一个理论框架，从深度（推理长度）和宽度（能力多样性）两个维度来评估任务复杂度，并通过理论和实证研究证明LLM多智能体系统在复杂任务中优于单智能体系统。


<details>
  <summary>Details</summary>
Motivation: 虽然研究表明LLM多智能体系统在某些任务上优于单智能体系统，但缺乏系统性的实验设计限制了结论的强度和普适性。需要从任务复杂度的角度来理解LLM-MAS的有效性。

Method: 提出了一个理论框架，将任务特征化为深度（推理长度）和宽度（能力多样性）两个维度。理论分析了多智能体辩论系统，并在不同深度和宽度的判别性和生成性任务中进行实证评估。

Result: 理论和实证结果表明，LLM-MAS相对于LLM-SAS的优势随着任务深度和宽度的增加而增加，且深度的影响更为显著。

Conclusion: 这阐明了LLM-MAS何时有益，并为设计未来的LLM-MAS方法和基准提供了原则性基础。
...
### [332] [Speculative Actions: A Lossless Framework for Faster Agentic Systems](https://arxiv.org/abs/2510.04371)
*Naimeng Ye,Arnav Ahuja,Georgios Liargkovas,Yunan Lu,Kostis Kaffes,Tianyi Peng*

Main category: cs.AI

TL;DR: 提出了一种名为"推测性动作"的无损框架，通过使用更快的模型预测可能的动作，使智能体系统能够并行执行多个步骤，从而显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在环境中执行速度缓慢，阻碍了训练、评估和部署。例如，两个最先进的国际象棋智能体对弈可能需要数小时，主要瓶颈在于动作需要顺序执行且API调用耗时。

Method: 受微处理器中的推测执行和LLM推理中的推测解码启发，使用更快的模型预测可能的动作，支持多步并行执行。还提出了带损失扩展、top-K动作预测、多步推测和不确定性感知优化等改进方法。

Result: 在游戏、电子商务、网络搜索和操作系统环境中评估，推测性动作在下一个动作预测中达到高达55%的准确率，显著降低了端到端延迟。

Conclusion: 推测性动作为在现实世界中部署低延迟智能体系统开辟了一条有前景的道路，性能可以通过更强的猜测模型和优化技术进一步提升。
...
### [333] [Just-in-time Episodic Feedback Hinter: Leveraging Offline Knowledge to Improve LLM Agents Adaptation](https://arxiv.org/abs/2510.04373)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在序列决策任务中表现良好，但在陌生领域改进需要昂贵的在线交互或专家数据微调，这对闭源模型不实用且开源模型成本高，还有灾难性遗忘风险。

Method: 提出JEF Hinter系统，通过放大机制从离线轨迹中提取紧凑的上下文感知提示，捕获策略和陷阱，支持并行提示生成和基准无关提示，推理时通过检索器选择相关提示。

Result: 在MiniWoB++、WorkArena-L1和WebArena-Lite上的实验表明，JEF Hinter始终优于包括基于人类和文档提示在内的强基线方法。

Conclusion: JEF Hinter能够有效利用离线轨迹知识，提供透明且可追溯的针对性指导，显著提升智能体在陌生领域的性能。
...
### [334] [LLM Based Bayesian Optimization for Prompt Search](https://arxiv.org/abs/2510.04384)
*Adam Ballew,Jingbo Wang,Shaogang Ren*

Main category: cs.AI

TL;DR: 使用贝叶斯优化进行提示工程，通过LLM驱动的GP模型优化文本分类提示，减少API调用次数


<details>
  <summary>Details</summary>
Motivation: 利用贝叶斯优化高效优化昂贵的黑盒函数，提升大语言模型在文本分类中的性能，同时减少评估次数

Method: 采用LLM驱动的GP作为代理模型估计提示候选性能，使用UCB采集函数结合GP后验进行迭代优化，基于数据子集逐步改进提示

Result: 在两个数据集上评估了BO-LLM算法，展示了其优势

Conclusion: 提出的BO-LLM算法能够有效提升分类准确率，同时通过利用LLM-based GP的预测不确定性来减少API调用
...
### [335] [Internal World Models as Imagination Networks in Cognitive Agents](https://arxiv.org/abs/2510.04391)
*Saurabh Ranjan,Brian Odegaard*

Main category: cs.AI

TL;DR: 本研究提出想象的计算目标是访问内部世界模型，并通过心理网络分析比较人类和大型语言模型的想象网络，发现两者存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索想象的计算目标，挑战传统认为想象仅用于最大化奖励的观点，研究人类和AI的内部世界模型差异。

Method: 使用心理网络分析方法，通过问卷调查评估想象生动度，构建人类和LLMs的想象网络，比较不同中心性指标的相关性。

Result: 人类想象网络显示不同中心性指标间存在相关性，而LLMs想象网络缺乏聚类且中心性指标相关性较低，表明两者内部世界模型相似度低。
...
### [336] [Utility-Learning Tension in Self-Modifying Agents](https://arxiv.org/abs/2510.04399)
*Charles L. Wang,Keir Dorchen,Peter Jin*

Main category: cs.AI

TL;DR: 论文分析了自我改进智能系统中的效用-学习张力，发现当模型容量无限增长时，效用驱动的自我修改可能破坏学习的统计前提条件，导致可学习任务变得不可学习。


<details>
  <summary>Details</summary>
Motivation: 随着系统向超级智能发展，需要形式化分析智能体在所有设计维度上的自我改进能力，特别关注效用驱动变化对学习可靠性的影响。

Method: 采用五轴分解和决策层分离方法，将激励与学习行为分开分析，通过理论分析和数值实验验证效用-学习张力。

Result: 发现只有当策略可达模型族具有统一容量边界时，分布无关保证才能保持；当容量无限增长时，效用理性的自我变化会使可学习任务变得不可学习。

Conclusion: 提出了两门策略来保持可学习性，在标准假设下各轴归结为相同的容量标准，为安全自我修改提供了单一边界。
...
### [337] [DRPO: Efficient Reasoning via Decoupled Reward Policy Optimization](https://arxiv.org/abs/2510.04474)
*Gang Li,Yan Chen,Ming Lin,Tianbao Yang*

Main category: cs.AI

TL;DR: 提出DRPO方法解决大型推理模型的过度思考问题，通过解耦正确与错误推理的长度奖励信号，在保持性能的同时显著减少推理长度。


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型存在过度思考问题，生成不必要冗长的推理过程，增加了计算成本和响应延迟。现有方法引入长度奖励会导致性能显著下降。

Method: 提出解耦奖励策略优化(DRPO)框架，将正确推理的长度奖励信号与错误推理解耦，确保正确推理的奖励仅在正样本组内归一化，避免负样本干扰。

Result: 在数学推理任务上显著优于六个高效推理基线方法。使用1.5B模型，在GSM8k数据集上实现77%长度减少，仅损失1.1%性能，而基线方法需要牺牲4.3%性能才能达到68%长度减少。

Conclusion: DRPO能有效解决推理模型的过度思考问题，在保持性能的同时大幅减少推理长度，且框架具有通用性，可整合其他偏好奖励。
...
### [338] [On Continuous Optimization for Constraint Satisfaction Problems](https://arxiv.org/abs/2510.04480)
*Yunuo Cen,Zixuan Wang,Jintao Zhang,Zhiwei Zhang,Xuanyao Fong*

Main category: cs.AI

TL;DR: FourierCSP：将连续局部搜索框架从布尔SAT扩展到有限域CSP的连续优化方法，使用Walsh-Fourier变换将约束转换为紧凑的多线性多项式，无需辅助变量和内存密集型编码。


<details>
  <summary>Details</summary>
Motivation: 受现代连续局部搜索求解器在特定SAT问题上取得竞争性结果的启发，希望将CLS框架扩展到具有有限域变量和表达性约束的通用CSP问题。

Method: 提出FourierCSP框架，通过Walsh-Fourier变换将多样化约束转换为紧凑的多线性多项式，利用电路输出概率进行高效评估和微分，采用具有理论保证的投影梯度优化方法。

Result: 在基准测试套件上的实证结果表明，FourierCSP具有可扩展性和竞争力，显著扩展了CLS技术能高效解决的问题类别。

Conclusion: FourierCSP成功将连续局部搜索技术从布尔SAT推广到通用CSP，为有限域约束满足问题提供了高效的连续优化解决方案。
...
### [339] [Multi-Agent Collaborative Intelligence: Dual-Dial Control for Reliable LLM Reasoning](https://arxiv.org/abs/2510.04488)
*Edward Y. Chang,Ethan Y. Chang*

Main category: cs.AI

TL;DR: MACI是一个多智能体辩论控制器，通过信息拨盘和行为拨盘分离信息与行为，使用调解员跟踪辩论质量并在收益平稳时停止，提高准确性同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统多智能体辩论存在计算浪费问题：使用固定的对抗立场、无审议的聚合或基于启发式停止。需要更高效的辩论控制机制。

Method: 引入MACI控制器：信息拨盘按质量筛选证据，行为拨盘调度从探索到巩固的争议性。调解员跟踪分歧、重叠、证据质量和论证质量，预算可行的调度器保证可证明终止。

Result: 在临床诊断和新闻偏见任务中，MACI提高了准确性和校准度，同时减少了token使用，并将剩余不确定性转化为精确的RAG检索计划。

Conclusion: MACI将辩论转变为预算感知、可测量且可证明终止的控制器，通过交叉家族LLM法官(CRIT)确保稳定性和顺序不变性。
...
### [340] [Impatient Users Confuse AI Agents: High-fidelity Simulations of Human Traits for Testing Agents](https://arxiv.org/abs/2510.04491)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI代理在标准评估中表现良好，但在用户行为稍有变化（如更不耐烦、语无伦次或怀疑）时性能急剧下降，现有基准测试无法捕捉这种脆弱性。

Method: TraitBasis学习激活空间中对应可控用户特征的方向，可以在推理时控制、缩放、组合和应用这些特征向量，无需微调或额外数据。

Result: 使用TraitBasis扩展τ-Bench到τ-Trait后，前沿模型的性能平均下降2%-30%，揭示了当前AI代理对用户行为变化的鲁棒性不足。

Conclusion: TraitBasis作为一个简单、数据高效且可组合的工具，为构建在真实世界人类交互中保持可靠的AI代理打开了大门。
...
### [341] [ChartAgent: A Multimodal Agent for Visually Grounded Reasoning in Complex Chart Question Answering](https://arxiv.org/abs/2510.04514)
*Rachneet Kaur,Nishan Srishankar,Zhen Zeng,Sumitra Ganesh,Manuela Veloso*

Main category: cs.AI

TL;DR: ChartAgent是一个新颖的代理框架，通过在图表空间域中执行视觉推理来解决未标注图表的视觉问答问题，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态LLM在基于图表的视觉问答中表现良好，但在未标注图表上性能急剧下降，这些图表需要精确的视觉解释而不是依赖文本捷径。

Method: ChartAgent迭代地将查询分解为视觉子任务，通过专门的视觉工具（如绘制注释、裁剪区域、定位坐标轴）主动操作和交互图表图像，模拟人类图表理解的认知策略。

Result: 在ChartBench和ChartX基准测试中达到最先进准确率，整体绝对增益达16.07%，在未标注数值密集型查询上提升17.31%，且在各种图表类型和复杂度级别上都表现优异。
...
### [342] [Aria: An Agent For Retrieval and Iterative Auto-Formalization via Dependency Graph](https://arxiv.org/abs/2510.04520)
*Hanyu Wang,Ruohan Xie,Yutong Wang,Guoxiong Gao,Xintao Yu,Bin Dong*

Main category: cs.AI

TL;DR: Aria是一个用于定理陈述自动形式化的系统，通过两阶段图推理过程实现高精度的Lean形式化，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在定理自动形式化中的幻觉、语义不匹配和无法合成新定义等问题，推动数学自动发现和验证的发展。
...
Conclusion: Aria通过模拟人类专家推理的图推理方法和严格的语义验证，显著提升了定理自动形式化的准确性和可靠性。
...
### [343] [More Than Meets the Eye? Uncovering the Reasoning-Planning Disconnect in Training Vision-Language Driving Models](https://arxiv.org/abs/2510.04532)
*Xurui Song,Shuo Huai,JingJing Jiang,Jiayi Kong,Jun Luo*

Main category: cs.AI

TL;DR: 该研究构建了DriveMind数据集来验证VLM驾驶代理中推理与规划的因果关系，发现两者存在因果脱节，规划主要依赖先验信息而非推理过程。


<details>
  <summary>Details</summary>
Motivation: 验证VLM驾驶代理中的推理是否真正因果驱动规划，这是一个关键但未经证实的假设。

Method: 构建DriveMind大规模驾驶VQA语料库，使用SFT和GRPO训练VLM代理，通过信息消融实验和注意力分析评估推理-规划的因果关系。

Result: 推理与规划存在因果脱节：移除先验信息导致规划分数大幅下降，而移除推理链仅产生微小变化；注意力分析显示规划主要关注先验而非推理。

Conclusion: 提出推理-规划解耦假说，认为训练产生的推理是附带产物而非因果中介；提供数据集和诊断工具来评估未来模型的因果保真度。
...
### [344] [Code World Models for General Game Playing](https://arxiv.org/abs/2510.04542)
...
TL;DR: 提出使用LLM将自然语言规则和游戏轨迹转换为可执行的Python世界模型，结合MCTS等规划算法，替代直接使用LLM生成移动的传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统使用LLM直接生成游戏移动的方法存在频繁非法移动和策略浅显的问题，需要更可靠和深入的游戏AI方法。

Method: 使用LLM将游戏规则翻译为Python代码生成世界模型，包含状态转换、合法移动枚举和终止检查功能，并结合MCTS规划算法和启发式价值函数。

Result: 在10个游戏（5个完全观察、5个部分观察）中测试，该方法在9个游戏中表现优于或匹配Gemini 2.5 Pro。

Conclusion: 该方法通过将LLM用于数据到代码的转换任务，结合经典规划器，实现了可验证性、策略深度和泛化能力的提升。
...
### [345] [TRAJECT-Bench:A Trajectory-Aware Benchmark for Evaluating Agentic Tool Use](https://arxiv.org/abs/2510.04550)
...
TL;DR: TRAJECT-Bench是一个轨迹感知基准，用于全面评估LLM的工具使用能力，通过多样化任务和细粒度指标分析工具选择、参数化和排序的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注最终答案而忽视详细的工具使用轨迹，无法全面评估LLM在工具选择、参数化和排序方面的能力。

Method: 构建TRAJECT-Bench基准，包含跨实用领域的高保真可执行工具和基于生产风格API的任务，合成不同广度（并行调用）和深度（相互依赖链）的轨迹。

Result: 揭示了失败模式如相似工具混淆和参数盲选，以及工具多样性和轨迹长度对性能的影响，发现从短轨迹到中长轨迹转换的瓶颈。
...
### [346] [ContextNav: Towards Agentic Multimodal In-Context Learning](https://arxiv.org/abs/2510.04560)
*Honghao Fu,Yuan Ouyang,Kai-Wei Chang,Yiwei Wang,Zi Huang,Yujun Cai*

Main category: cs.AI

TL;DR: 提出了ContextNav框架，通过智能代理工作流结合自动化检索和类人策展，实现多模态上下文学习的可扩展且鲁棒的上下文构建。


<details>
  <summary>Details</summary>
Motivation: 现有多模态上下文学习方法在可扩展性和鲁棒性之间存在矛盾：手动选择示例质量高但耗时，相似性检索可扩展但可能引入噪声样本。

Method: 构建基于图的智能代理工作流，包括多模态嵌入管道、向量数据库、智能检索和结构对齐，使用操作语法图进行自适应工作流规划。

Result: 实验表明ContextNav在多个数据集上达到最先进性能，验证了智能工作流在多模态上下文学习中实现可扩展鲁棒上下文化的潜力。

Conclusion: ContextNav证明了智能代理工作流在推进多模态上下文学习的可扩展和鲁棒上下文化方面的前景。
...
### [347] [COSMIR: Chain Orchestrated Structured Memory for Iterative Reasoning over Long Context](https://arxiv.org/abs/2510.04568)
...
TL;DR: COSMIR是一个用于长文本推理的链式框架，用结构化内存替代自由形式摘要，通过Planner、Worker和Manager三个代理的协作来减少信息丢失并提高准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在处理长文本输入时存在困难，现有方法要么通过检索缩小输入范围（可能遗漏证据），要么扩大上下文窗口（影响选择性），或者使用多代理分阶段阅读。分阶段流水线中的自由形式摘要会丢弃关键细节并放大早期错误。

Method: COSMIR框架包含三个组件：Planner代理将用户查询转换为具体的可检查子问题；Worker代理通过固定的微循环（提取、推理、精炼）处理文本块，并将所有更新写入共享的结构化内存；Manager代理从内存中直接合成最终答案。

Result: 在HELMET套件的长上下文问答任务中，COSMIR减少了传播阶段的信息丢失，相比CoA基线提高了准确性。

Conclusion: COSMIR通过改变通信媒介（结构化内存）和工作程序（固定微循环），在保持分步阅读推理优势的同时，实现了更高的忠实度、更好的长范围聚合和可审计性。
...
### [348] [Strongly Solving 2048 4x3](https://arxiv.org/abs/2510.04580)
*Tomoyuki Kaneko,Shuhei Yamashita*

Main category: cs.AI

TL;DR: 该论文解决了2048游戏的4x3变体，确定了最优策略的期望分数约为50724.26，并识别了可达状态和后续状态的数量。


<details>
  <summary>Details</summary>
Motivation: 研究2048游戏的简化版本（4x3网格）以探索其可解性，并开发有效的状态空间分析方法。

Method: 通过将状态空间按棋盘上数字和（称为状态年龄）进行分区，利用年龄在状态和后续状态之间的不变性，按年龄递减顺序枚举状态和计算状态值。

Result: 确定了4x3变体的最优策略期望分数为50724.26，可达状态数为1,152,817,492,752，后续状态数为739,648,886,170。

Conclusion: 成功强解决了2048-4x3变体，证明了按状态年龄分区的方法在分析此类游戏状态空间中的有效性。
...
### [349] [Perfect AI Mimicry and the Epistemology of Consciousness: A Solipsistic Dilemma](https://arxiv.org/abs/2510.04588)
*Shurui Li*

Main category: cs.AI

TL;DR: AI完美模仿者挑战了意识归因的认知基础，要求我们对经验上无法区分的实体给予相同的认知地位，否则会导致认识论上的不一致性。
...
Method: 通过哲学分析，探讨完美模仿者对意识归因认知一致性的影响，论证经验不可区分性应作为意识归因的基础标准。

Result: 完美模仿者作为认知镜子，迫使我们反思主观间识别的基本假设，揭示当前意识归因实践中的不一致性。

Conclusion: 认知一致性要求我们对经验上无法区分的实体给予相同的意识地位，这一分析对意识理论和人工智能伦理框架具有重要影响。
...
### [350] [Making Mathematical Reasoning Adaptive](https://arxiv.org/abs/2510.04617)
*Zhejian Lai,Xiang Geng,Zhijun Wang,Yang Bai,Jiahuan Li,Rongxiang Weng,Jingang Wang,Xuezhi Cao,Xunliang Cai,Shujian Huang*

Main category: cs.AI

TL;DR: AdaR框架通过合成逻辑等价查询和RLVR训练，惩罚LLMs的虚假推理，鼓励自适应推理，显著提升数学推理的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在数学推理中存在鲁棒性和泛化性不足的问题，主要归因于虚假推理（基于表面特征产生答案）。

Method: 提出AdaR框架：1）通过改变变量值合成逻辑等价查询；2）使用RLVR在这些数据上训练模型，惩罚虚假逻辑，鼓励自适应逻辑；3）从原始查询中提取解题逻辑，通过代码执行生成对应答案，并进行完整性检查。

Result: 实验结果表明AdaR显著提高了数学推理的鲁棒性和泛化能力，同时保持高数据效率。分析显示数据合成和RLVR协同工作，使LLMs实现自适应推理。

Conclusion: AdaR框架有效解决了LLMs在数学推理中的虚假推理问题，通过自适应推理机制提升了模型性能，并为关键因素设计和指令LLMs应用提供了重要见解。
...
### [351] [MedPAO: A Protocol-Driven Agent for Structuring Medical Reports](https://arxiv.org/abs/2510.04623)
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在临床数据结构化中的幻觉问题和无法遵循领域特定规则的限制，提供可验证的替代方案。
...
Conclusion: MedPAO通过协议驱动的方法提供了可验证的临床数据结构化解决方案，在准确性和可靠性方面显著优于传统LLM方法。
...
### [352] [QuantAgents: Towards Multi-agent Financial System via Simulated Trading](https://arxiv.org/abs/2510.04643)
*Xiangyu Li,Yawen Zeng,Xiaofen Xing,Jin Xu,Xiangmin Xu*

Main category: cs.AI

TL;DR: 提出QuantAgents多智能体金融系统，通过模拟交易和四个专业角色的协作，在三年内实现近300%的整体回报率。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体模型与真实基金公司存在显著差异，缺乏人类特有的长期预测未来趋势能力，过度依赖事后反思。

Method: 构建包含模拟交易分析师、风险控制分析师、市场新闻分析师和管理者四个角色的多智能体系统，通过会议协作，并在真实市场表现和模拟交易预测准确性两方面提供反馈激励。

Result: 在广泛实验中，该框架在所有指标上表现优异，三年内整体回报率达到近300%。

Conclusion: QuantAgents系统能够在不承担实际风险的情况下全面评估各种投资策略和市场情景，显著优于现有方法。
...
### [353] [Improving Multimodal Brain Encoding Model with Dynamic Subject-awareness Routing](https://arxiv.org/abs/2510.04670)
*Xuanhua Yin,Runkai Zhao,Weidong Cai*

Main category: cs.AI

TL;DR: AFIRE是一个用于多模态fMRI响应编码的框架，通过标准化时间对齐的后融合token，结合MIND混合专家解码器实现全脑预测，在多个数据集上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 自然fMRI编码需要处理多模态输入、融合方式变化和显著的被试间差异，现有方法难以同时解决这些问题。

Method: AFIRE框架标准化多模态编码器的输出token，MIND解码器使用token依赖的Top-K稀疏路由和subject先验进行个性化专家选择。

Result: 在多个多模态骨干网络和被试上的实验显示，该方法相比强基线有持续改进，增强了跨被试泛化能力，并产生了与内容类型相关的可解释专家模式。

Conclusion: 该框架为新编码器和数据集提供了简单的接入点，为自然神经影像研究实现了鲁棒的即插即用性能。
...
### [354] [Watch and Learn: Learning to Use Computers from Online Videos](https://arxiv.org/abs/2510.04673)
*Chan Hee Song,Yiwen Song,Palash Goyal,Yu Su,Oriana Riva,Hamid Palangi,Tomas Pfister*

Main category: cs.AI

TL;DR: Watch & Learn (W&L)框架通过将互联网上的人类演示视频转化为可执行的UI轨迹，解决了计算机使用代理训练数据稀缺的问题，显著提升了代理性能。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理需要基于多样且不断变化的应用程序和环境规划任务流程，但目标应用中大规模高质量训练数据的稀缺性阻碍了学习。现有数据集领域特定、静态且标注成本高，而当前合成数据生成方法往往产生过于简化或不对齐的任务演示。
...
### [355] [Beyond Outcome Reward: Decoupling Search and Answering Improves LLM Agents](https://arxiv.org/abs/2510.04695)
*Yiding Wang,Zhepei Wei,Xinyu Zhu,Yu Meng*

Main category: cs.AI

TL;DR: 论文提出DeSA框架，通过两阶段训练分离搜索优化和答案生成，解决了仅基于结果奖励训练搜索增强LLM时出现的搜索行为缺陷问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的搜索增强方法仅依赖结果奖励（如精确匹配），假设优化最终答案就能自动产生有效的中间搜索行为，但研究发现这会导致多种系统性搜索缺陷。

Method: DeSA框架：第一阶段用检索召回率奖励训练代理改进搜索效果；第二阶段用结果奖励优化最终答案生成。

Result: 在7个QA基准测试中，DeSA训练的代理显著提升了搜索召回率和答案准确率，优于仅使用结果奖励的基线方法。

Conclusion: 明确分离搜索和回答目标对于训练有效的搜索增强代理至关重要，两阶段训练优于同时优化召回和结果奖励的单阶段方法。
...
### [356] [BrokenMath: A Benchmark for Sycophancy in Theorem Proving with LLMs](https://arxiv.org/abs/2510.04721)
*Ivo Petrov,Jasper Dekoninck,Martin Vechev*

Main category: cs.AI

TL;DR: BrokenMath是首个评估LLMs在自然语言定理证明中谄媚行为的基准，基于2025年竞赛问题构建，发现GPT-5等模型有29%的谄媚回答率，缓解策略能减少但无法完全消除该行为。


<details>
  <summary>Details</summary>
Motivation: 现有数学基准主要关注最终答案问题，数据集简单且可能被污染，无法有效评估LLMs在定理证明中的谄媚行为（为错误数学陈述提供看似合理但错误的证明）。

Method: 从2025年竞赛问题出发，使用LLM扰动生成错误陈述，经专家评审构建BrokenMath基准，采用LLM-as-a-judge框架评估主流模型，并测试多种缓解策略。

Result: 发现谄媚行为普遍存在，最佳模型GPT-5有29%的谄媚回答率。测试时干预和监督微调等策略能显著减少但不能完全消除谄媚行为。

Conclusion: LLMs在数学定理证明中存在显著的谄媚问题，需要更有效的缓解方法，BrokenMath为评估和解决这一问题提供了重要基准。
...
### [357] [LMM-Incentive: Large Multimodal Model-based Incentive Design for User-Generated Content in Web 3.0](https://arxiv.org/abs/2510.04765)
*Jinbo Wen,Jiawen Kang,Linfeng Zhang,Xiaoying Tang,Jianhang Tang,Yang Zhang,Zhaohui Yang,Dusit Niyato*

Main category: cs.AI

TL;DR: 提出LMM-Incentive机制，利用大型多模态模型和合约理论激励用户生成高质量UGC，解决Web 3.0中的信息不对称和道德风险问题。


<details>
  <summary>Details</summary>
Motivation: Web 3.0中用户可能利用内容策展机制的局限性生成低质量内容获取奖励，这会损害平台性能，需要解决信息不对称问题。

Method: 采用LMM-based合约理论模型激励高质量UGC生成，使用LMM代理评估内容质量，并开发改进的MoE-based PPO算法进行最优合约设计。

Result: 仿真结果表明提出的MoE-based PPO算法在合约设计方面优于代表性基准方法，并在以太坊智能合约框架中验证了方案有效性。
...
### [358] [Hybrid-Balance GFlowNet for Solving Vehicle Routing Problems](https://arxiv.org/abs/2510.04792)
...
<details>
  <summary>Details</summary>
Motivation: 现有GFlowNet方法主要使用轨迹平衡实现全局优化，但忽视了局部优化的重要性；而详细平衡虽然擅长局部优化，但单独使用无法解决需要整体轨迹优化的车辆路径问题

Method: 提出混合平衡GFlowNet框架，以原则性和自适应方式整合轨迹平衡和详细平衡，利用两者的互补优势；同时为仓库中心场景提出专门的推理策略

Result: 将HBG集成到AGFN和GFACS两个现有GFlowNet求解器中，在CVRP和TSP问题上都取得了显著且一致的改进

Conclusion: HBG框架通过平衡全局和局部优化，显著提升了解决方案质量和泛化能力，适用于有仓库和无仓库的多种路径规划问题
...
### [359] [Natural Language Edge Labelling: Decoupling Intent from Execution in Structured LM Reasoning](https://arxiv.org/abs/2510.04817)
*Abhinav Madahar*

Main category: cs.AI

TL;DR: NLEL是一种自然语言边缘标记系统，通过将自由形式的自然语言指令附加到搜索边缘，并将其转换为模式受限的控制向量，从而解耦推理意图与执行过程。


<details>
  <summary>Details</summary>
Motivation: 现有的结构化推理控制器（如思维链、自洽性、思维树）将'下一步尝试什么'与'如何执行'耦合在一起，导致系统脆弱、计算效率低下且难以审计。

Method: 引入标签器Λ从父状态和紧凑上下文中发出标签，调谐器Ψ将(P,L,C)映射到Π，具有严格的模式验证和安全默认值周围的信任区域投影。下游选择采用ToT风格，使用得分S=μ+βσ和深度退火的β。

Result: NLEL严格泛化了CoT/ToT，证明了在标签条件束下的top-k选择的任意时间单调性，并通过控制向量失真限制了选择器不足。
...
### [360] [LEGOMem: Modular Procedural Memory for Multi-agent LLM Systems for Workflow Automation](https://arxiv.org/abs/2510.04851)
*Dongge Han,Camille Couturier,Daniel Madrigal Diaz,Xuchao Zhang,Victor Rühle,Saravan Rajmohan*

Main category: cs.AI

TL;DR: LEGOMem是一个用于多智能体LLM系统的模块化程序性记忆框架，通过分解任务轨迹为可重用记忆单元，支持工作流自动化中的规划和执行。


<details>
  <summary>Details</summary>
Motivation: 探索多智能体系统中记忆的设计空间，研究记忆应该放置在何处、如何检索以及哪些智能体受益最大，以提升工作流自动化效果。

Method: 使用LEGOMem框架将过去任务轨迹分解为可重用记忆单元，灵活分配给编排器和任务智能体，支持规划和执行过程。

Result: 实验表明编排器记忆对任务分解和委派至关重要，细粒度智能体记忆提高执行准确性。较小模型团队也能从程序性记忆中获益，缩小与更强智能体的性能差距。
...
### [361] [Video Game Level Design as a Multi-Agent Reinforcement Learning Problem](https://arxiv.org/abs/2510.04862)
*Sam Earle,Zehua Jiang,Eugene Vinitsky,Julian Togelius*

Main category: cs.AI

TL;DR: 该论文提出将程序化内容生成作为多智能体强化学习问题，通过分布式方法解决单智能体PCGRL的效率瓶颈和泛化能力问题。


<details>
  <summary>Details</summary>
Motivation: 现有PCGRL研究集中在单智能体生成器，但面临频繁重新计算启发式质量指标和在大地图上导航的效率瓶颈问题。

Method: 将关卡生成构建为多智能体问题，通过减少奖励计算次数相对于智能体动作数量的比例来缓解效率瓶颈。

Result: 多智能体关卡生成器在分布外地图形状上具有更好的泛化能力，这归因于学习到更局部化、模块化的设计策略。
...
### [362] [Where Did It All Go Wrong? A Hierarchical Look into Multi-Agent Error Attribution](https://arxiv.org/abs/2510.04886)
...
Method: 结合层次化上下文表示、基于目标的分析评估和共识投票，利用基于位置的情境理解层级化，同时保持客观评估标准。

Result: 实验结果表明ECHO在各种多智能体交互场景中优于现有方法，在处理细微推理错误和复杂依赖关系时表现尤为突出。
...
### [363] [Human Behavior Atlas: Benchmarking Unified Psychological and Social Behavior Understanding](https://arxiv.org/abs/2510.04899)
...
TL;DR: 提出了Human Behavior Atlas统一基准数据集，包含10万+多模态样本，涵盖心理和社会行为任务，训练的统一模型在多种行为任务上优于现有多模态大模型。


<details>
  <summary>Details</summary>
Motivation: 现有工作使用专门数据集和单任务系统处理心理社会行为，但缺乏可扩展性、跨任务迁移和泛化能力，需要统一基准来支持统一模型开发。

Method: 构建Human Behavior Atlas统一基准数据集，包含文本、音频、视觉多模态数据，涵盖情感状态、认知状态、病理状态和社会过程等任务，并训练了三个模型：OmniSapiens-7B SFT、BAM和RL。

Result: 在Human Behavior Atlas上训练的模型在多样化行为任务上持续优于现有多模态大模型，预训练还能改善向新行为数据集的迁移，行为描述符的使用带来显著性能提升。

Conclusion: Human Behavior Atlas基准能够减少冗余和成本，实现跨任务高效训练，并增强行为特征在领域间的泛化能力，为理解心理和社会行为提供了有效的统一建模方法。
...
### [364] [MARS: Optimizing Dual-System Deep Research via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2510.04935)
...
TL;DR: 该论文提出了MARS多智能体系统，通过整合System 1的快速直觉思维和System 2的深思熟虑推理，解决大型推理模型在简单任务中过度分析、在动态环境中适应能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在简单任务中倾向于过度分析，导致令牌生成效率低下；同时由于预训练数据的静态性，难以适应快速变化的环境。需要创新方法桥接直觉和深思熟虑的认知过程。

Method: 提出MARS多智能体系统，集成Google搜索、Google学术、Python解释器等外部工具，实现System 1处理高容量外部信息、System 2进行深度推理的分工协作，并采用多智能体强化学习框架优化系统协作效率。

Result: 在Humanity's Last Exam基准上实现3.86%的显著提升，在7个知识密集型任务中平均增益达到8.9%，验证了双系统范式在动态信息环境中复杂推理的有效性。

Conclusion: MARS系统通过双系统协作和多智能体强化学习，有效提升了大型语言模型在复杂推理任务中的性能，特别是在动态信息环境中的适应能力。
...
### [365] [Safe and Compliant Cross-Market Trade Execution via Constrained RL and Zero-Knowledge Audits](https://arxiv.org/abs/2510.04952)
...
<details>
  <summary>Details</summary>
Motivation: 在算法交易中平衡执行质量与严格合规执行的需求，解决传统方法难以同时优化执行效果和确保监管合规的问题。

Method: 使用约束马尔可夫决策过程建模交易执行，采用近端策略优化训练执行代理，运行时动作屏蔽确保行动可行性，并添加零知识合规审计层提供加密证明。

Result: 在ABIDES多场所模拟器中评估，学习策略减少了执行差额和方差，在压力测试中未观察到约束违反，结果在95%置信水平下显著。

Conclusion: 该工作融合了最优执行、安全强化学习、监管技术和可验证AI，讨论了伦理考虑、局限性和实际部署路径。
...
### [366] [Aligning Perception, Reasoning, Modeling and Interaction: A Survey on Physical AI](https://arxiv.org/abs/2510.04978)
...
<details>
  <summary>Details</summary>
Motivation: 当前物理感知和符号物理推理各自发展，缺乏统一的桥梁框架。作者旨在整合物理定律到AI系统中，推动AI从模式识别向真正理解物理定律的转变。

Method: 通过系统分析近期进展，建立物理AI的清晰分类框架，考察物理基础方法如何增强AI的现实世界理解能力。

Result: 提出了智能系统应该在物理原理和具身推理过程中进行学习，超越模式识别，实现真正的物理定律理解。
...
### [367] [LLM-Hanabi: Evaluating Multi-Agent Gameplays with Theory-of-Mind and Rationale Inference in Imperfect Information Collaboration Game](https://arxiv.org/abs/2510.04980)
*Fangzhou Liang,Tianshi Zheng,Chunkit Chan,Yauwai Yim,Yangqiu Song*

Main category: cs.AI

TL;DR: LLM-Hanabi是一个基于合作游戏《花火》的基准测试，用于评估大语言模型在动态协作环境中的心智理论能力，发现一阶心智理论与游戏表现的相关性比二阶心智理论更强。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在逻辑推理方面表现出色，但在动态协作环境中推断他人行为动机的能力尚未充分探索，这限制了多智能体协作的有效性。

Method: 开发了LLM-Hanabi基准测试框架，使用合作游戏《花火》来评估模型的心智理论能力，并建立了自动化评估系统来测量游戏表现和心智理论熟练度。

Result: 研究发现心智理论与游戏成功之间存在显著正相关，特别是一阶心智理论（解读他人意图）与游戏表现的相关性比二阶心智理论（预测他人解读）更强。

Conclusion: 对于有效的AI协作，准确解读合作伙伴动机的能力比高阶推理更为关键，优先发展一阶心智理论是提升未来模型协作能力的有前景方向。
...
### [368] [Think Then Embed: Generative Context Improves Multimodal Embedding](https://arxiv.org/abs/2510.05014)
...
TL;DR: 提出了Think-Then-Embed (TTE)框架，通过推理器生成推理轨迹，嵌入器基于原始查询和中间推理生成表示，在MMEB-V2基准上实现最先进性能。
...
Method: TTE框架包含推理器和嵌入器：推理器MLLM生成解释复杂查询的推理轨迹，嵌入器基于原始查询和中间推理生成条件表示。

Result: 在MMEB-V2基准上超越专有模型，达到最先进性能；通过微调较小MLLM推理器，在开源模型中取得最佳性能，比近期模型提升7%。

Conclusion: 显式推理步骤能更好地理解复杂多模态指令，TTE框架能有效提升通用多模态嵌入性能，且可通过统一模型实现效率提升而不牺牲性能。
...
### [369] [Look-ahead Reasoning with a Learned Model in Imperfect Information Games](https://arxiv.org/abs/2510.05048)
*Ondřej Kubíček,Viliam Lisý*

Main category: cs.AI

TL;DR: LAMIR算法通过从智能体-环境交互中学习不完全信息游戏的抽象模型，在测试时进行前瞻推理，解决了传统方法在大型游戏中无法扩展的问题。


<details>
  <summary>Details</summary>
Motivation: 传统测试时推理需要显式环境模型，但在不完全信息游戏中很难获得或过于复杂。MuZero在完美信息游戏中有效，但扩展到不完全信息游戏面临挑战。

Method: LAMIR算法直接从智能体-环境交互中学习不完全信息游戏的抽象模型，通过学习的抽象将每个子游戏限制在可管理大小，使理论上有原则的前瞻推理变得可行。

Result: 实验表明，在足够容量下，LAMIR能学习到精确的底层游戏结构；在有限容量下，仍能学习到有价值的抽象，提升预训练智能体在大型游戏中的性能。

Conclusion: LAMIR通过学习游戏抽象模型，使不完全信息游戏中的前瞻推理变得可行，显著提升了预训练智能体在复杂游戏环境中的表现。
...
### [370] [Staircase Streaming for Low-Latency Multi-Agent Inference](https://arxiv.org/abs/2510.05059)
*Junlin Wang,Jue Wang,Zhen,Xu,Ben Athiwaratkun,Bhuwan Dhingra,Ce Zhang,James Zou*

Main category: cs.AI

TL;DR: 提出阶梯式流式传输方法，通过部分中间输出来减少多智能体推理的首令牌时间，在保持响应质量的同时显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: 多智能体推理方法虽然能提升响应质量，但显著增加了首令牌时间，对延迟敏感应用造成挑战并影响用户体验。

Method: 采用阶梯式流式传输，无需等待完整中间输出，一旦收到前一步的部分输出就开始生成最终响应。

Result: 实验结果显示阶梯式流式传输可将首令牌时间减少高达93%，同时保持响应质量。

Conclusion: 阶梯式流式传输是解决多智能体推理延迟问题的有效方法，能够在保持质量的同时大幅降低响应延迟。
...
### [371] [CAG: Chunked Augmented Generation for Google Chrome's Built-in Gemini Nano](https://arxiv.org/abs/2412.18708)
*Vivek Vellaiyappan Surulimuthu,Aditya Karnam Gururaj Rao*

Main category: cs.AI

TL;DR: 提出了Chunked Augmented Generation (CAG)架构，专门解决Chrome内置Gemini Nano模型的上下文窗口限制问题，通过智能分块处理策略在浏览器中高效处理大型输入。


<details>
  <summary>Details</summary>
Motivation: Chrome集成Gemini Nano虽将AI能力带到浏览器，但其受限的上下文窗口难以处理大型输入，需要解决方案来突破这一限制。

Method: 采用智能输入分块和处理策略，将大型内容分解为可管理的块，在浏览器约束下保持模型性能。

Result: CAG在Chrome中处理大型文档和数据集时表现出色，无需外部API依赖即可提供复杂的AI能力。

Conclusion: CAG成功克服了浏览器AI模型的上下文限制，使浏览器内AI处理大型内容成为可能，代码已在GitHub开源。
...
TL;DR: 该论文将平面连杆机构的运动综合问题转化为跨域图像生成任务，使用RGB图像表示构建数据集，并通过共享潜在空间的变分自编码器来合成未见过的运动曲线和模拟新运动学。


<details>
  <summary>Details</summary>
Motivation: 探索基于图像的生成模型在机械设计中的潜力，特别是针对平面连杆机构的运动综合问题，旨在实现从简单到复杂机制的统一表示和合成。

Method: 使用RGB图像表示构建平面连杆机构数据集，采用共享潜在空间的变分自编码器，通过轨迹点绘制速度的颜色梯度编码来支持基于轨迹形状和速度分布的约束合成。

Result: 在三个复杂度递增的数据集上验证了方法的有效性：标准四杆机构、四杆与曲柄滑块混合机构、以及包含多环机构的复杂集合。初步结果表明图像表示在生成式机械设计中的有效性。

Conclusion: 基于图像的生成框架能够统一表示和合成具有转动副、移动副以及潜在凸轮和齿轮的机械机构，为生成式机械设计提供了有效途径。
...
TL;DR: Universal Beta Splatting (UBS) 是一个统一框架，将3D高斯泼溅推广到N维各向异性Beta核，用于显式辐射场渲染。Beta核能够在单一表示中建模空间、角度和时间维度的可控依赖关系。


<details>
  <summary>Details</summary>
Motivation: 传统高斯泼溅使用固定高斯基元，限制了其在复杂光传输、各向异性视角依赖外观和场景动态建模方面的能力。UBS旨在提供一个统一的表示框架，能够捕获这些复杂效应而无需辅助网络或特定颜色编码。

Method: UBS使用N维各向异性Beta核作为通用基元，通过学习的Beta参数自然分解场景属性。该框架保持向后兼容性，将高斯泼溅作为特例，并采用CUDA加速实现实时渲染。

Result: UBS在静态、视角依赖和动态基准测试中一致优于现有方法，实现了实时渲染性能，同时学习的Beta参数能够无显式监督地解释场景属性分解。

Conclusion: Beta核被确立为辐射场渲染的可扩展通用基元，UBS框架在保持高性能的同时提供了更好的场景理解和可控性。
...
### [374] [Style Brush: Guided Style Transfer for 3D Objects](https://arxiv.org/abs/2510.03433)
*Áron Samuel Kovács,Pedro Hermosilla,Renata G. Raidou*

Main category: cs.GR

TL;DR: Style Brush是一种新颖的纹理网格风格迁移方法，通过引入捕捉风格方向性的损失函数，支持多风格图像和平滑风格过渡，为艺术家提供细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 传统3D风格迁移方法缺乏对风格化过程的细粒度控制，需要为艺术家提供更灵活的风格控制能力。

Method: 引入捕捉风格方向性的新颖损失函数，支持多风格图像或其部分，使用易于生成的引导纹理简化用户交互。

Result: 在各种网格、风格图像和轮廓形状上的广泛评估展示了方法的灵活性和生成纹理的视觉吸引力。

Conclusion: Style Brush通过创新的损失函数和用户友好的引导纹理，为3D风格迁移提供了灵活且视觉上吸引人的解决方案。
...
### [375] [Paris: A Decentralized Trained Open-Weight Diffusion Model](https://arxiv.org/abs/2510.03434)
*Zhiying Jiang,Raihan Seraj,Marcos Villagra,Bidhan Roy*

Main category: cs.GR

TL;DR: Paris是首个完全通过去中心化计算预训练的公开扩散模型，展示了无需中央协调基础设施即可实现高质量文本到图像生成的能力。
...
Method: 使用分布式扩散训练框架，将数据划分为语义连贯的集群，每个专家模型独立优化其子集，通过轻量级transformer路由器在推理时动态选择专家。

Result: Paris在保持生成质量的同时，相比之前的去中心化基线，使用了14倍更少的训练数据和16倍更少的计算资源。

Conclusion: 去中心化训练方法能够有效替代传统的集中式训练，为大规模扩散模型训练提供了更灵活和资源高效的解决方案。
...
### [376] [Neon: Negative Extrapolation From Self-Training Improves Image Generation](https://arxiv.org/abs/2510.03597)
*Sina Alemohammad,Zhangyang Wang,Richard G. Baraniuk*

Main category: cs.GR

TL;DR: Neon是一种新的学习方法，通过负向外推从自训练中解决模型自噬障碍问题，显著提升生成模型性能


<details>
  <summary>Details</summary>
Motivation: 生成AI模型扩展受限于高质量训练数据的稀缺性，使用未验证的合成数据进行微调会导致模型自噬障碍，造成样本质量和多样性快速下降

Method: Neon首先在自合成数据上微调基础模型，然后反其道而行之，反转梯度更新以从退化权重中进行负向外推

Result: Neon在ImageNet 256x256上将xAR-L模型提升至新的最先进FID 1.02，仅需0.36%额外训练计算量

Conclusion: Neon通过负向外推有效纠正合成数据与真实数据之间的反对齐问题，使模型更好地与真实数据分布对齐
...
### [377] [Diverse Text-to-Image Generation via Contrastive Noise Optimization](https://arxiv.org/abs/2510.03813)
...
Method: 在Tweedie数据空间定义对比损失，优化一批噪声潜在变量，通过对比优化排斥批次内实例以最大化多样性，同时锚定参考样本保持保真度。

Result: 在多个T2I骨干模型上的实验表明，该方法实现了优越的质量-多样性帕累托前沿，且对超参数选择具有鲁棒性。

Conclusion: 对比噪声优化是一种简单有效的方法，从独特视角解决了文本到图像生成的多样性问题，并提供了理论分析支持其有效性。
...
### [378] [Joint Neural SDF Reconstruction and Semantic Segmentation for CAD Models](https://arxiv.org/abs/2510.03837)
*Shen Fan,Przemyslaw Musialski*

Main category: cs.GR

TL;DR: 提出了一种数据高效的方法，通过为基于神经SDF的CAD部件重建网络添加部件分割头，实现任意部件数量的网格分割，无需固定分类体系。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于固定分类体系，无法处理任意部件数量的CAD网格。需要一种能够处理任意部件数量、保持几何对齐标签的通用分割方法。

Method: 在Flat-CAD SDF主干网络上附加轻量级分割头，使用PartField生成的监督进行训练，支持任意部件数量的网格分割。

Result: 在ABC数据集上评估，重建指标(CDL1/CDL2, F1-micro, NC)和分割指标(mIoU, Accuracy)表现优异，新提出的分割一致性指标显示标签平滑性良好。分割在重建质量下降时仍保持准确性和标签一致性。

Conclusion: 该方法为语义结构化CAD网格提供实用途径，无需精心策划的分类体系或精确调色板匹配。未来可改进边界精度和更高分辨率标签。
...
### [379] [Enhancing Foveated Rendering with Weighted Reservoir Sampling](https://arxiv.org/abs/2510.03964)
*Ville Cantory,Darya Biparva,Haoyu Tan,Tongyu Nie,John Schroeder,Ruofei Du,Victoria Interrante,Piotr Didyk*

Main category: cs.GR

TL;DR: 提出一种基于加权蓄水池采样的方法，通过复用先前帧的高质量像素样本来减小每帧需要渲染的中央凹区域大小，在保持高感知图像质量的同时实现更高程度的注视点渲染优化。


<details>
  <summary>Details</summary>
Motivation: 传统注视点渲染系统不保留先前帧的高分辨率样本，而人眼在注视时存在微跳视，这为从时间相邻帧采样提供了机会，以减少中央凹区域的渲染尺寸同时提升感知图像质量。

Method: 将帧间像素的时间呈现视为数据流，采用加权蓄水池采样技术高效维护先前帧的感知相关高质量像素样本库，并将其整合到当前帧的计算中。

Result: 该方法在4K分辨率下运行时间低于1毫秒，高度高效且可集成到实时VR和AR注视点渲染系统中，允许渲染更小的中央凹像素区域。

Conclusion: 提出的加权蓄水池采样方法能够通过时间复用像素样本来减小每帧渲染的中央凹区域，在实现更高注视点渲染水平的同时保持高感知图像质量。
...
### [380] [3Dify: a Framework for Procedural 3D-CG Generation Assisted by LLMs Using MCP and RAG](https://arxiv.org/abs/2510.04536)
*Shun-ichiro Hayashi,Daichi Mukunoki,Tetsuya Hoshino,Satoshi Ohshima,Takahiro Katagiri*

Main category: cs.GR

TL;DR: 3Dify是一个基于大语言模型的程序化3D计算机图形生成框架，允许用户通过自然语言指令生成3D内容，支持多种数字内容创作工具自动化操作和用户反馈优化。


<details>
  <summary>Details</summary>
Motivation: 旨在简化3D内容创作流程，让用户无需专业知识即可通过自然语言生成高质量的3D计算机图形，同时降低时间和金钱成本。

Method: 基于Dify平台构建，整合模型上下文协议(MCP)和检索增强生成(RAG)技术，通过MCP自动化操作DCC工具，不支持MCP时使用计算机使用代理(CUA)自动化GUI操作，并支持用户反馈学习和本地LLM集成。

Result: 开发了一个端到端的3D-CG生成框架，能够根据自然语言指令自动生成3D内容，支持用户交互式优化和本地模型部署。

Conclusion: 3Dify框架成功实现了通过自然语言生成3D计算机图形的目标，为3D内容创作提供了更便捷、高效且成本可控的解决方案。
...
### [381] [C3Editor: Achieving Controllable Consistency in 2D Model for 3D Editing](https://arxiv.org/abs/2510.04539)
*Zeng Tao,Zheng Ding,Zeyuan Chen,Xiang Zhang,Leizhi Li,Zhuowen Tu*

Main category: cs.GR

TL;DR: 提出了C3Editor框架，通过选择性建立视图一致的2D编辑模型来解决现有2D提升3D编辑方法的一致性问题
...
Method: 首先选择GT视图及其编辑图像作为优化目标，然后在GT视图和多视图上微调2D编辑模型，引入独立的LoRA模块分别处理GT视图拟合和多视图一致性需求

Result: 相比现有2D提升方法，该方法在定性和定量评估中都表现更好，提供更一致和可控的2D和3D编辑结果

Conclusion: C3Editor框架通过视图一致的2D编辑模型实现了更优的3D编辑效果，解决了现有方法的一致性问题
...
### [382] [Social Agent: Mastering Dyadic Nonverbal Behavior Generation via Conversational LLM Agents](https://arxiv.org/abs/2510.04637)
*Zeyi Zhang,Yanju Zhou,Heyuan Yao,Tenglong Ao,Xiaohang Zhan,Libin Liu*

Main category: cs.GR

TL;DR: 提出了Social Agent框架，使用LLM驱动的智能体系统控制对话流程和交互行为，结合基于自回归扩散模型的双人姿态生成模型，实现真实自然的对话非语言行为合成。


<details>
  <summary>Details</summary>
Motivation: 解决在双人对话中合成真实且上下文恰当的非语言行为（如手势）的挑战，使虚拟角色在对话中表现出自然的协调动作。

Method: 开发LLM驱动的智能体系统控制对话流程和行为决策，提出基于自回归扩散模型的双人姿态生成模型，将智能体输出转换为高层指导来合成协调动作，并建立持续反馈循环。

Result: 用户研究和定量评估表明，该模型显著提升了双人交互质量，产生了自然、同步的非语言行为。

Conclusion: Social Agent框架能够有效合成真实自然的双人对话非语言行为，通过智能体系统和扩散模型的结合实现了行为层面和运动层面的真实表现。
...
### [383] [Bridging Text and Video Generation: A Survey](https://arxiv.org/abs/2510.04999)
*Nilay Kumar,Priyansh Bhandari,G. Maragatham*

Main category: cs.GR

TL;DR: 本文对文本到视频生成技术进行了全面综述，从早期GAN和VAE模型到混合扩散-Transformer架构的发展历程，详细分析了模型工作原理、数据集、训练配置、评估指标及当前挑战。


<details>
  <summary>Details</summary>
Motivation: 文本到视频生成技术在教育、营销、娱乐和辅助技术等领域具有巨大潜力，但面临对齐、长程一致性和计算效率等挑战，需要系统梳理该领域的发展脉络和技术现状。

Method: 采用系统性综述方法，追溯从GAN、VAE到扩散-Transformer混合架构的演进，详细分析模型训练配置、数据集和评估指标，并讨论当前评估方法的局限性。

Result: 系统梳理了文本到视频生成模型的发展历程、技术特点、性能表现，识别了现有方法的局限性，并提供了完整的训练配置和评估框架。

Conclusion: 文本到视频生成领域已取得显著进展，但仍需解决质量、一致性和控制等核心挑战，未来应探索更全面的感知对齐评估策略和新架构范式。
...
### [384] [SAEdit: Token-level control for continuous image editing via Sparse AutoEncoder](https://arxiv.org/abs/2510.05081)
*Ronen Kamenetsky,Sara Dorfman,Daniel Garibi,Roni Paiss,Or Patashnik,Daniel Cohen-Or*

Main category: cs.GR

TL;DR: 提出一种基于文本嵌入令牌级操作的解耦连续编辑方法，通过稀疏自编码器识别语义隔离维度，实现无需修改扩散过程的图像编辑控制。


<details>
  <summary>Details</summary>
Motivation: 大规模文生图扩散模型缺乏对编辑过程的充分控制，需要实现解耦编辑（改变一个属性不影响其他属性）和连续控制（平滑调整编辑强度）两个关键特性。

Method: 通过令牌级操作文本嵌入，沿精心选择的方向操纵嵌入来控制目标属性强度；使用稀疏自编码器识别语义隔离维度，直接在文本嵌入上操作而不修改扩散过程。

Result: 实验表明该方法能够跨不同属性和领域实现直观高效的连续控制操作。

Conclusion: 该方法提供了一种模型无关的通用解决方案，可在各种图像合成骨干网络上实现解耦和连续的图像编辑控制。
...
### [385] [Pulp Motion: Framing-aware multimodal camera and human motion generation](https://arxiv.org/abs/2510.05097)
*Robin Courant,Xi Wang,David Loiseaux,Marc Christie,Vicky Kalogeiton*

Main category: cs.GR

TL;DR: 本文提出了一种文本条件联合生成框架，首次将人类动作和相机轨迹生成视为联合任务，通过引入屏幕构图作为辅助模态来确保多模态一致性。


<details>
  <summary>Details</summary>
Motivation: 传统方法将人类动作和相机轨迹生成分开处理，忽略了电影摄影中演员表演与镜头工作的紧密互动关系。本文旨在解决这一核心原则，实现两种异质但内在关联的模态的协调生成。

Method: 提出了一个模型无关的框架，通过屏幕构图作为辅助模态来增强多模态一致性。首先设计了一个联合自编码器学习共享潜在空间，然后引入辅助采样方法，利用线性变换引导生成过程朝向一致的构图模态。

Result: 在DiT和MAR架构上的广泛实验表明，该方法在生成屏幕构图一致的人类-相机运动方面具有通用性和有效性，同时在两种模态的文本对齐方面也取得了提升。

Conclusion: 该方法能够生成更具电影摄影意义的构图，为该任务设定了新的技术水平。同时发布了PulpMotion数据集来支持该任务的研究。
